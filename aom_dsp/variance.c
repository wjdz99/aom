...
void aom_upsampled_pred_c(uint8_t *comp_pred, int width, int height,
  ...
  if (!subpel_x_q3 && !subpel_y_q3) {
    ...
    if(!subpel_y_q3) {
      ...
      aom_convolve8_horiz_c(ref, ref_stride, comp_pred, width,
                            kernel, 16, NULL, -1, width, height);
    } else if(!subpel_x_q3) {
      const int16_t *kernel;
      kernel = av1_get_interp_filter_subpel_kernel(filter, subpel_y_q3 << 1);
      /*Directly call C version to allow this to work for small (2x2) sizes.*/
      aom_convolve8_vert_c(ref, ref_stride, comp_pred, width,
  ...
  if (!subpel_x_q3 && !subpel_y_q3) {
    ...
    if(!subpel_y_q3) {
      ...
      aom_highbd_convolve8_horiz_c(ref8, ref_stride,
                                   CONVERT_TO_BYTEPTR(comp_pred), width,
                                   kernel, 16, NULL, -1, width, height, bd);
    } else if(!subpel_x_q3) {
      const int16_t *kernel;
      kernel = av1_get_interp_filter_subpel_kernel(filter, subpel_y_q3 << 1);
      /*Directly call C version to allow this to work for small (2x2) sizes.*/
      aom_highbd_convolve8_vert_c(ref8, ref_stride,
                                  CONVERT_TO_BYTEPTR(comp_pred), width,
                                  NULL, -1, kernel, 16, width, height, bd);
    } else {
      DECLARE_ALIGNED(16, uint8_t,
                      temp[((MAX_SB_SIZE * 2 + 16) + 16)*MAX_SB_SIZE]);
      const uint16_t *ref;
      ...
      aom_highbd_convolve8_horiz_c(
          ...
          ref_stride, CONVERT_TO_BYTEPTR(temp), MAX_SB_SIZE,
          kernel_x, 16, NULL, -1, width, intermediate_height, bd);
      aom_highbd_convolve8_vert_c(
          CONVERT_TO_BYTEPTR(temp + MAX_SB_SIZE*((filter.taps >> 1) - 1)),
          MAX_SB_SIZE, CONVERT_TO_BYTEPTR(comp_pred), width,
...
void aom_highbd_comp_avg_upsampled_pred_c(uint16_t *comp_pred,
  ...
  aom_highbd_upsampled_pred(comp_pred, width, height,
                            subpel_x_q3, subpel_y_q3, ref8, ref_stride, bd);
  for (i = 0; i < height; ++i) {
    for (j = 0; j < width; ++j) {
