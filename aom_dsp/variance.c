...
void aom_upsampled_pred_c(MACROBLOCKD *xd, const AV1_COMMON *const cm,
                          ...
                          int ref_offset, int ref_stride) {
  const MODE_INFO *const mi = xd->mi[0];
  const int ref_num = 0;
  // TODO(now): Ignore intrabc here (or use it in av1_find_best_sub_pixel_tree
  // too).
  ...
  if (is_scaled) {
    ...
    const WarpedMotionParams *const wm =
        &xd->global_motion[mi->mbmi.ref_frame[ref_num]];
    ...
    if (ref_num && is_masked_compound_type(mi->mbmi.interinter_compound_type)) {
      av1_make_masked_inter_predictor(
          pre, pre_buf->stride, comp_pred, width, subpel_x, subpel_y, sf, width,
          height, &conv_params, mi->mbmi.interp_filters, xs, ys, plane,
          &warp_types, (mi_x >> pd->subsampling_x) + x,
          (mi_y >> pd->subsampling_y) + y, ref_num, xd);
    } else {
      const int build_for_obmc = 0;  // TODO(now): OK?
      av1_make_inter_predictor(
          pre, pre_buf->stride, comp_pred, width, subpel_x, subpel_y, sf, width,
          height, &conv_params, mi->mbmi.interp_filters, &warp_types,
          (mi_x >> pd->subsampling_x) + x, (mi_y >> pd->subsampling_y) + y,
          plane, ref_num, mi, build_for_obmc, xs, ys, xd);
    }

#if !CONFIG_LOWPRECISION_BLEND
    if (conv_params.is_compound) {
      assert(conv_params.dst != NULL);
      const int round_bits = get_compound_post_rounding_bits(&conv_params);
      if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH)
        av1_highbd_convolve_rounding(tmp_dst, MAX_SB_SIZE, comp_pred, width,
                                     width, height, round_bits, xd->bd);
      else
        av1_convolve_rounding(tmp_dst, MAX_SB_SIZE, comp_pred, width, width,
                              height, round_bits);
    }
    ...
    if (!subpel_x_q3 && !subpel_y_q3) {
      ...
      /*Directly call C versions to allow this to work for small (2x2)
       * sizes.*/
...
void aom_comp_avg_upsampled_pred_c(MACROBLOCKD *xd, const AV1_COMMON *const cm,
  ...
  aom_upsampled_pred(xd, cm, mi_row, mi_col, mv, comp_pred, width, height,
                     subpel_x_q3, subpel_y_q3, ref, ref_offset, ref_stride);
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
...
void aom_jnt_comp_avg_upsampled_pred_c(
    ...
    int ref_offset, int ref_stride, const JNT_COMP_PARAMS *jcp_param) {
  int i, j;
  const int fwd_offset = jcp_param->fwd_offset;
  const int bck_offset = jcp_param->bck_offset;
