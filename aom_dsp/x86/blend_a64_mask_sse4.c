...
void aom_lowbd_blend_a64_d16_mask_sse4_1(uint8_t *dst, uint32_t dst_stride,
  ...
  if (subw == 0 && subh == 0) {
    ...
    for (i = 0; i < h; ++i) {
      for (j = 0; j < w; j += 4) {
        const __m128i m_i0 =
            xx_loadl_64(&mask[i * mask_stride + (2 * j)]);
        const __m128i m_ac = _mm_maddubs_epi16(m_i0, one_b);
    ...
    for (i = 0; i < h; ++i) {
      for (j = 0; j < w; j += 4) {
        const __m128i m_i0 =
            xx_loadl_64(&mask[(2 * i) * mask_stride + j]);
        const __m128i m_i1 =
            xx_loadl_64(&mask[(2 * i + 1) * mask_stride + j]);
