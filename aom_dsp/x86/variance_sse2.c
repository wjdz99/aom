  ...
  if (width == 8) {
    ...
    do {
      for(int x=0; x< width/32; x++){
        for (int j = 0; j < 2; j++) {
          const __m128i s0 = _mm_loadu_si128((const __m128i *)(src0 + x*32 + j * 16));
          const __m128i s2 =
              _mm_loadu_si128((const __m128i *)(src0 + x*32 + 8 + j * 16));
          const __m128i s1 = _mm_loadu_si128((const __m128i *)(src1 + x*32 + j * 16));
          const __m128i s3 =
              _mm_loadu_si128((const __m128i *)(src1 + x*32 + 8 + j * 16));
  
          const __m128i m_8 = _mm_loadu_si128((const __m128i *)(mask + x*32 + j * 16));
          const __m128i m01_16 = _mm_unpacklo_epi8(m_8, zero);
          const __m128i m23_16 = _mm_unpackhi_epi8(m_8, zero);
