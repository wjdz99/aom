  ...
  if (!subpel_x_q3 && !subpel_y_q3) {
    if (width >= 16) {
      ...
      for (i = 0; i < height; i++) {
        ...
        __m128i s2 = _mm_cvtsi32_si128(*(const uint32_t *)(ref + 2*ref_stride));
        __m128i s3 = _mm_cvtsi32_si128(*(const uint32_t *)(ref + 3*ref_stride));
    ...
    if(!subpel_y_q3) {
      ...
      aom_convolve8_horiz(ref, ref_stride, comp_pred, width,
                          kernel, 16, NULL, -1, width, height);
    } else if(!subpel_x_q3) {
      const int16_t *kernel;
      kernel = av1_get_interp_filter_subpel_kernel(filter, subpel_y_q3 << 1);
      aom_convolve8_vert(ref, ref_stride, comp_pred, width,
                         NULL, -1, kernel, 16, width, height);
    } else {
      ...
      aom_convolve8_horiz(ref - ref_stride*((filter.taps >> 1) - 1), ref_stride,
                         temp, MAX_SB_SIZE, kernel_x, 16, NULL, -1,
...
void aom_comp_avg_upsampled_pred_sse2(uint8_t *comp_pred, const uint8_t *pred,
  ...
  int n;
  int i;
  aom_upsampled_pred(comp_pred, width, height, subpel_x_q3, subpel_y_q3,
                     ref, ref_stride);
