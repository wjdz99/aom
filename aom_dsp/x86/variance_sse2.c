    ...
    if (is_scaled) {
      ...
      if (ref_num && is_masked_compound_type(mi->interinter_compound_type)) {
        av1_make_masked_inter_predictor(
            pre, pre_buf->stride, comp_pred, width, subpel_x, subpel_y, sf,
            width, height, &conv_params, filters, xs, ys, plane, &warp_types,
        ...
        av1_make_inter_predictor(
            ...
            mi_x >> pd->subsampling_x, mi_y >> pd->subsampling_y, plane,
            ref_num, mi, build_for_obmc, xs, ys, xd, cm->allow_warped_motion);
      }
      return;
    }
  }
