  ...
  for (i = 0; i < non_zero_regs; i++) {
    ...
    for (j = 0; j < 4; j++) {
      if (test & (1 << (4 * j))) {
        ...
        dqcoeff_ptr[k] = qcoeff_ptr[k] * dequant_ptr[k != 0] / (1 << QUANT_TABLE_BITS);
...
void aom_highbd_quantize_b_32x32_sse2(
  ...
  for (i = 0; i < idx; i++) {
    ...
    dqcoeff_ptr[rc] = qcoeff_ptr[rc] * dequant_ptr[rc != 0] / (2 << QUANT_TABLE_BITS);
...
void aom_highbd_quantize_b_64x64_sse2(
  ...
  for (i = 0; i < idx; i++) {
    ...
    qcoeff_ptr[rc] = (int)(abs_qcoeff ^ coeff_sign) - coeff_sign;
    dqcoeff_ptr[rc] = qcoeff_ptr[rc] * dequant_ptr[rc != 0] / (4 << QUANT_TABLE_BITS);
    if (abs_qcoeff) eob = iscan[idx_arr[i]] > eob ? iscan[idx_arr[i]] : eob;
  }
  *eob_ptr = eob + 1;
}
