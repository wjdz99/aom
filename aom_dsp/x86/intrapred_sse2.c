...
#if CONFIG_FLEX_PARTITION
void aom_dc_predictor_64x8(uint8_t *dst, ptrdiff_t stride, const uint8_t *above,
                           const uint8_t *left) {
  __m128i sum_above = dc_sum_64(above);
  const __m128i sum_left = dc_sum_8(left);
  sum_above = _mm_add_epi16(sum_above, sum_left);
...
void aom_dc_predictor_64x4(uint8_t *dst, ptrdiff_t stride, const uint8_t *above,
                           const uint8_t *left) {
  __m128i sum_above = dc_sum_64(above);
  const __m128i sum_left = dc_sum_4(left);
  sum_above = _mm_add_epi16(sum_above, sum_left);
