...
void aom_quantize_b_adaptive_avx2(
  ...
  if (_mm256_movemask_epi8(cmp_mask) == 0) {
    ...
    _mm256_storeu_si256((__m256i *)(qcoeff_ptr + 8), zero);
    _mm256_storeu_si256((__m256i *)(dqcoeff_ptr), zero);
    _mm256_storeu_si256((__m256i *)(dqcoeff_ptr + 8), zero);
  ...
  while (index < n_coeffs) {
    ...
    if (_mm256_movemask_epi8(cmp_mask) == 0) {
      _mm256_storeu_si256((__m256i *)(qcoeff_ptr + index), zero);
      _mm256_storeu_si256((__m256i *)(qcoeff_ptr + index + 8), zero);
      _mm256_storeu_si256((__m256i *)(dqcoeff_ptr + index), zero);
      _mm256_storeu_si256((__m256i *)(dqcoeff_ptr + index + 8), zero);
      index += 16;
      continue;
    }
    calculate_qcoeff_avx2(&qcoeff, &round, &quant, &shift);
