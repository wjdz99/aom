...
#define STACK_V(h, fun)                                                 \
  do {                                                                  \
    ...
    fun(diff + diff_stride * h, diff_stride,                            \
        src + src_stride * h, src_stride,                               \
...
#define STACK_H(w, fun)                                                 \
  do {                                                                  \
    fun(diff, diff_stride, src, src_stride, pred, pred_stride);         \
    fun(diff + w, diff_stride,                                          \
        src + w, src_stride, pred + w, pred_stride);                    \
  } while (0)

#define SUBTRACT_FUN(size)                                              \
  static void                                                           \
  subtract_ ## size(int16_t *diff, ptrdiff_t diff_stride,               \
                    const uint16_t *src, ptrdiff_t src_stride,          \
                    const uint16_t *pred, ptrdiff_t pred_stride)
