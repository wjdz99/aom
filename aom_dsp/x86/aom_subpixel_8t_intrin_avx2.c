...
static INLINE __m256i xx_loadu2_mi128(const void *hi, const void *lo) {
  __m256i a = _mm256_castsi128_si256(_mm_lddqu_si128((const __m128i *)(lo)));
  a = _mm256_inserti128_si256(a, _mm_lddqu_si128((const __m128i *)(hi)), 1);
  return a;
}
...
static void aom_filter_block1d4_h8_avx2(
  ...
  __m128i filtersReg;
  __m256i roundshift6, filt1Reg, filt2Reg;
  ...
  src_ptr -= 3;
  roundshift6 = _mm256_set1_epi16(1 << (15 - 6));
  filtersReg = _mm_lddqu_si128((const __m128i *)filter);
  ...
  for (i = output_height; i > 1; i -= 2) {
    ...
    srcRegFilt32b1_1 = _mm256_mulhrs_epi16(srcRegFilt32b1_1, roundshift6);
  ...
  if (i > 0) {
    ...
    srcReg1 = _mm_lddqu_si128((const __m128i *)(src_ptr));
    ...
    // shift by 6 bit each 16 bit
    srcRegFilt1_1 =
        _mm_mulhrs_epi16(srcRegFilt1_1, _mm256_castsi256_si128(roundshift6));
...
static void aom_filter_block1d8_h8_avx2(
    ...
    ptrdiff_t output_pitch, uint32_t output_height, const int16_t *filter) {
  __m128i filtersReg;
  __m256i roundshift6, filt1Reg, filt2Reg, filt3Reg, filt4Reg;
...
static void aom_filter_block1d16_h8_avx2(
  ...
  for (i = output_height; i > 1; i -= 2) {
    ...
    // shift by 6 bit each 16 bit
    srcRegFilt32b1_1 = _mm256_mulhrs_epi16(srcRegFilt32b1_1, roundshift6);
    srcRegFilt32b2_1 = _mm256_mulhrs_epi16(srcRegFilt32b2_1, roundshift6);
  ...
  if (i > 0) {
    ...
    // (part of it was being read by earlier read)
    srcReg2 = _mm_lddqu_si128((const __m128i *)(src_ptr + 8));
    ...
    srcRegFilt2_1 =
        _mm_mulhrs_epi16(srcRegFilt2_1, _mm256_castsi256_si128(roundshift6));
...
static void aom_filter_block1d8_v8_avx2(
  ...
  for (i = output_height; i > 1; i -= 2) {
    ...
    // shift by 6 bit each 16 bit
    srcReg32b10 = _mm256_mulhrs_epi16(srcReg32b10, roundshift6);
  ...
  if (i > 0) {
    ...
    // shift by 6 bit each 16 bit
    srcRegFilt1 =
        _mm_mulhrs_epi16(srcRegFilt1, _mm256_castsi256_si128(roundshift6));
...
static void aom_filter_block1d16_v8_avx2(
  ...
  srcReg32b7 = _mm256_castsi128_si256(
      _mm_lddqu_si128((const __m128i *)(src_ptr + src_pitch * 6)));
  ...
  for (i = output_height; i > 1; i -= 2) {
    ...
    // consecutive loads in the same 256 bit register
    srcReg32b8 = _mm256_castsi128_si256(
        _mm_lddqu_si128((const __m128i *)(src_ptr + src_pitch * 7)));
    srcReg32b7 = _mm256_inserti128_si256(srcReg32b7,
                                         _mm256_castsi256_si128(srcReg32b8), 1);
    srcReg32b9 = _mm256_castsi128_si256(
        _mm_lddqu_si128((const __m128i *)(src_ptr + src_pitch * 8)));
    srcReg32b8 = _mm256_inserti128_si256(srcReg32b8,
                                         _mm256_castsi256_si128(srcReg32b9), 1);
    ...
    // shift by 6 bit each 16 bit
    srcReg32b10 = _mm256_mulhrs_epi16(srcReg32b10, roundshift6);
    srcReg32b1 = _mm256_mulhrs_epi16(srcReg32b1, roundshift6);
  ...
  if (i > 0) {
    ...
    // load the last 16 bytes
    srcRegFilt8 = _mm_lddqu_si128((const __m128i *)(src_ptr + src_pitch * 7));
    ...
    srcRegFilt1 =
        _mm_mulhrs_epi16(srcRegFilt1, _mm256_castsi256_si128(roundshift6));
    srcRegFilt3 =
        _mm_mulhrs_epi16(srcRegFilt3, _mm256_castsi256_si128(roundshift6));
