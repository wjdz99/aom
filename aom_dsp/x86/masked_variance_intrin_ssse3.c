...
// Last two arguments are const pointers to satisfy the Windows x86
// calling convention, which does not guarantee alignment of stack-allocated
// __m128i arguments.
...
static INLINE void accumulate_block(const __m128i src, const __m128i a,
                                    const __m128i b, const __m128i *m_ptr,
                                    __m128i *sum, __m128i *sum_sq) {
  const __m128i zero = _mm_setzero_si128();
  const __m128i one = _mm_set1_epi16(1);
  const __m128i mask_max = _mm_set1_epi8((1 << AOM_BLEND_A64_ROUND_BITS));
  const __m128i m = *m_ptr;
