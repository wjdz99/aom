...
static AOM_FORCE_INLINE void highbd_lpf_internal_14_dual_sse2(__m128i* p, __m128i* q, int pitch,
                                                    const uint8_t *blt,
                                                    const uint8_t *lt,
                                                    const uint8_t *thr,
                                                    int bd) {
...
void aom_highbd_lpf_horizontal_14_dual_sse2(uint16_t *s, int pitch,
	...
	highbd_lpf_internal_14_dual_sse2(p, q, pitch, _blimit, _limit, _thresh,
	  bd);
	for (i=0; i<6; i++){
...
void aom_highbd_lpf_vertical_4_dual_sse2(
 ...
 // highbd_transpose8x16_sse2(x0,x1,x2,x3,x4,x5,x6,x7, d0,d1,d2,d3,d4,d5,d6,d7);
  highbd_transpose8x8_sse2(x0 + 1, x1 + 1, x2 + 1, x3 + 1, x4 + 1, x5 + 1, x6 + 1, x7 + 1, d0 + 1, d1 + 1, d2 + 1, d3 + 1, d4 + 1, d5 + 1, d6 + 1, d7 + 1);
  ...
  __m128i d2_4, d3_4,  d4_4, d5_4;
   ...
   highbd_lpf_internal_4_sse2(&d2_4, &d3_4, &d4_4, &d5_4, blimit1, limit1, thresh1, bd);
...
void aom_highbd_lpf_vertical_8_dual_sse2(
  ...
  highbd_lpf_internal_8_sse2(d0, d7, d1, d6, d2, d5, d3, d4, q1q0, p1p0, blimit0, limit0, thresh0, bd);
  highbd_lpf_internal_8_sse2(&d0_4, &d7_4, &d1_4, &d6_4, &d2_4, &d5_4, &d3_4, &d4_4, q1q0+1, p1p0+1, blimit1, limit1, thresh1, bd);
  ...
  highbd_transpose8x8_sse2(d0, d1, &p1p0_hi, &p1p0_lo, &q1q0_lo, &q1q0_hi, d6, d7, x0, x1,
	  x2, x3, x4, x5, x6, x7);
...
void aom_highbd_lpf_vertical_14_dual_sse2(uint16_t *s, int pitch,
  ...
  highbd_transpose8x8_sse2(&p6, &p5, &p4, &p3, &p2, &p1, &p0, &q0, 
	  &d0, &p[6], &p[5], &p[4], &p[3], &p[2], &p[1], &p[0]);
  ...
  highbd_transpose8x8_sse2(&p6_2, &p5_2, &p4_2, &p3_2, &p2_2, &p1_2, &p0_2,&q0_2,
	  &q[0], &q[1], &q[2], &q[3], &q[4], &q[5], &q[6], &d7);

  highbd_lpf_internal_14_dual_sse2(p, q, pitch, blimit, limit, thresh, bd);

  highbd_transpose8x8_sse2(&d0, &p[6], &p[5], &p[4], &p[3], &p[2], &p[1], &p[0], &d0_out, &d1_out,
  ...
  highbd_transpose8x8_sse2(&q[0], &q[1], &q[2], &q[3], &q[4], &q[5], &q[6], &d7, &d0_out, &d1_out,
	  &d2_out, &d3_out, &d4_out, &d5_out, &d6_out, &d7_out);
