...
static AOM_FORCE_INLINE void highbd_lpf_internal_6_sse2(
	__m128i *p2, __m128i *p1, __m128i *p0, __m128i *q0, __m128i *q1, __m128i *q2,__m128i *p1p0_out, __m128i *q1q0_out,
	const uint8_t *_blimit,
	const uint8_t *_limit,
	const uint8_t *_thresh, int bd) {
  const __m128i zero = _mm_setzero_si128();
...
void aom_highbd_lpf_horizontal_6_sse2(uint16_t *s, int p,
  ...
  highbd_lpf_internal_6_sse2(
	  &p2, &p1, &p0, &q0, &q1, &q2, &p1p0_out,  &q1q0_out,
...
void aom_highbd_lpf_horizontal_8_sse2(uint16_t *s, int p,
  ...
  highbd_lpf_internal_8_sse2(&p3, &q3, &p2, &q2, &p1, &q1, &p0, &q0,
	  &q1q0, &p1p0, _blimit, _limit, _thresh,bd);
...
void aom_highbd_lpf_horizontal_4_sse2(uint16_t *s, int p,
  ...
  highbd_lpf_internal_4_sse2( &p1, &p0, &q0, &q1,
	  _blimit, _limit, _thresh, bd);
...
void aom_highbd_lpf_vertical_4_sse2(uint16_t *s, int p, const uint8_t *blimit,
  ...
  x0 =
	  _mm_loadu_si128((__m128i *)(s-4 + 0 * p)); 
  x1 =				
	  _mm_loadu_si128((__m128i *)(s-4 + 1 * p)); 
  x2 =							 
	  _mm_loadu_si128((__m128i *)(s-4 + 2 * p)); 
  x3 =							  
	  _mm_loadu_si128((__m128i *)(s-4 + 3 * p)); 
  x4 =							 
	  _mm_loadu_si128((__m128i *)(s-4 + 4 * p)); 
  x5 =							  
	  _mm_loadu_si128((__m128i *)(s-4 + 5 * p)); 
  x6 =							 
	  _mm_loadu_si128((__m128i *)(s-4 + 6 * p)); 
  x7 =							  
	  _mm_loadu_si128((__m128i *)(s-4 + 7 * p)); 
  ...
  highbd_transpose8x8_low_sse2(&d0, &d1, &d2, &d3, &d4, &d5, &d6, &d7, &d0, &d1, &d2, &d3);
...
void aom_highbd_lpf_vertical_8_sse2(uint16_t *s, int p, const uint8_t *blimit,
  ...
  highbd_transpose8x8_sse2(&p3, &p2, &p1, &p0, &q0, &q1, &q2, &q3,
...
void aom_highbd_lpf_vertical_14_sse2(uint16_t *s, int pitch, const uint8_t *blimit,
                                     const uint8_t *limit,
	...
	highbd_transpose8x8_sse2(&p6, &p5, &p4, &p3, &p2, &p1, &p0, &q0, &d0, &p[6], &p[5], &p[4], &p[3], &p[2],
		&p[1], &p[0]);
	...
	highbd_transpose8x8_sse2(&p6_2, &p5_2, &p4_2, &p3_2, &p2_2, &p1_2, &p0_2, &q0_2,
		&q[0], &q[1], &q[2], &q[3], &q[4], &q[5], &q[6], &d7_2);


	highbd_lpf_internal_14_sse2(p, q, pq, blimit, limit, thresh, bd);

	highbd_transpose8x8_low_sse2(&d0, &p[6], &pq[5], &pq[4], &pq[3], &pq[2], &pq[1], &pq[0], &d0,&d1,
		&d2, &d3);
