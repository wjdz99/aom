...
static INLINE void highbd_hev_filter_mask_x_sse2(__m128i *pq, int x, __m128i *p1p0, __m128i *q1q0,                                             
                                                __m128i *abs_p1p0,
                                       __m128i *l,  __m128i *bl, __m128i *t, __m128i *hev,
                                      __m128i *mask) {
  const __m128i zero = _mm_setzero_si128();
  const __m128i one = _mm_set1_epi16(1);
  const __m128i ffff = _mm_set1_epi16(0xFFFF);
...
static INLINE void flat_mask_internal(const __m128i *th, const __m128i *pq,
  ...
  int i;
  __m128i max = _mm_max_epi16(abs_diff16(pq[start], pq[0]), abs_diff16(pq[start+1], pq[0]) );
...
static INLINE void highbd_flat_mask4_sse2(__m128i *pq,  __m128i *flat, __m128i *flat2, int bd) {
  // check the distance 1,2,3 against 0
 __m128i th = _mm_set1_epi16(1);
  th = _mm_slli_epi16(th, bd - 8);
  flat_mask_internal(&th, pq, bd, 1, 4, flat);
  flat_mask_internal(&th, pq, bd, 4, 7, flat2);
}

static INLINE void highbd_flat_mask4_dual_sse2(const __m128i *p,
                                     const __m128i *q, __m128i *flat, __m128i *flat2, int bd) {
  // check the distance 1,2,3 against 0
  __m128i th = _mm_set1_epi16(1);
  th = _mm_slli_epi16(th, bd - 8);
  flat_mask_internal_dual(&th, p, q, bd, 1, 4, flat);
  flat_mask_internal_dual(&th, p, q, bd, 4, 7, flat2);
}
...
static AOM_FORCE_INLINE void highbd_lpf_internal_14_sse2(
 ...
 __m128i p1p0, q1q0, abs_p1p0;
  ...
  highbd_hev_filter_mask_x_sse2(pq, 4, &p1p0, &q1q0,  &abs_p1p0 , &limit,
                               &blimit, &thresh,
                               &hevhev, &mask);
...

  {
    ...
    __m128i work0 = _mm_add_epi16(sum_p6, _mm_add_epi16(pq[1], _mm_add_epi16(pq[2], pq[0])));
	...
	work0 =
        _mm_add_epi16(sum_p6, _mm_add_epi16(pq[2], _mm_add_epi16(pq[3], pq[1])));
    ...
    sum_q = _mm_sub_epi16(sum_q, p[2]);
 	...
 	work0 =
        _mm_add_epi16(sum_p6, _mm_add_epi16(pq[4], _mm_add_epi16(pq[5], pq[3])));
...
static AOM_FORCE_INLINE void highbd_lpf_internal_6_sse2(
  ...
  highbd_hev_filter_mask_x_sse2(pq, 3, &p1p0, &q1q0, &abs_p1p0, &limit,
                               &blimit,
                               &thresh, &hev, &mask);
...
static AOM_FORCE_INLINE void highbd_lpf_internal_4_sse2(
  ...
  highbd_hev_filter_mask_x_sse2(pq, 2, &p1p0, &q1q0, &abs_p1p0, &limit,
                                      &blimit, &thresh, &hev, &mask);
