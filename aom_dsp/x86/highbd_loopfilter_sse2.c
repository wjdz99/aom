...
static INLINE void highbd_lpf_horz_edge_8_internal(uint16_t *s, int pitch,
...
#endif
  {
...
#if CONFIG_DEBLOCK_13TAP
    __m128i sum_p =
        _mm_add_epi16(p[5], _mm_add_epi16(p[4], p[3]));
    __m128i sum_q =
        _mm_add_epi16(q[5], _mm_add_epi16(q[4], q[3]));
    ...
    flat2_p[0] =
         _mm_srli_epi16(_mm_add_epi16(sum_p, _mm_add_epi16(_mm_add_epi16(p[6],p[0]),_mm_add_epi16(p[1],q[0]))), 4);
    flat2_q[0] =
         _mm_srli_epi16(_mm_add_epi16(sum_p, _mm_add_epi16(_mm_add_epi16(q[6],q[0]),_mm_add_epi16(p[0],q[1]))), 4);
    flat_p[0] =
    ...
    flat2_p[1] =
        _mm_srli_epi16(_mm_add_epi16(sum_p, _mm_add_epi16(sum_p6, _mm_add_epi16(p[1],_mm_add_epi16(p[2],p[0])))), 4);
    flat2_q[1] =
        _mm_srli_epi16(_mm_add_epi16(sum_q, _mm_add_epi16(sum_q6, _mm_add_epi16(q[1],_mm_add_epi16(q[0],q[2])))), 4);
    ...
    flat2_p[2] =
        _mm_srli_epi16(_mm_add_epi16(sum_p, _mm_add_epi16(sum_p6, _mm_add_epi16(p[2],_mm_add_epi16(p[3],p[1])))), 4);
    flat2_q[2] =
        _mm_srli_epi16(_mm_add_epi16(sum_q, _mm_add_epi16(sum_q6, _mm_add_epi16(q[2],_mm_add_epi16(q[1],q[3])))), 4);
    ...
    for (i = 3; i < 6; ++i) {
      sum_p6 = _mm_add_epi16(sum_p6, p[6]);
      sum_q6 = _mm_add_epi16(sum_q6, q[6]);
      sum_p = _mm_sub_epi16(sum_p, q[6 - i]);
      sum_q = _mm_sub_epi16(sum_q, p[6 - i]);
      flat2_p[i] =
          _mm_srli_epi16(_mm_add_epi16(sum_p, _mm_add_epi16(sum_p6, _mm_add_epi16(p[i],_mm_add_epi16(p[i+1],p[i-1])))), 4);
      flat2_q[i] =
          _mm_srli_epi16(_mm_add_epi16(sum_q, _mm_add_epi16(sum_q6, _mm_add_epi16(q[i],_mm_add_epi16(q[i-1],q[i+1])))), 4);
    }
#else
    __m128i sum_p =
