...
static INLINE void highbd_blend_a64_d16_mask_w4_avx2(
  ...
  // Some 32-bit platforms don't have _mm256_extract_epi64...
  *(uint64_t *)(dst + 3 * dst_stride) =
    _mm_extract_epi64(_mm256_extracti128_si256(clip, 1), 1);
  *(uint64_t *)(dst + 2 * dst_stride) =
    _mm_extract_epi64(_mm256_extracti128_si256(clip, 1), 0);
  *(uint64_t *)(dst + 1 * dst_stride) =
    _mm_extract_epi64(_mm256_castsi256_si128(clip), 1);
  *(uint64_t *)(dst + 0 * dst_stride) =
    _mm_extract_epi64(_mm256_castsi256_si128(clip), 0);
}
