...
static INLINE void highbd_blend_a64_d16_mask_w4_avx2(
  ...
  *(uint64_t *)(dst + 3 * dst_stride) = _mm256_extract_epi64(clip, 3);
  *(uint64_t *)(dst + 2 * dst_stride) = _mm256_extract_epi64(clip, 2);
  *(uint64_t *)(dst + 1 * dst_stride) = _mm256_extract_epi64(clip, 1);
  *(uint64_t *)(dst + 0 * dst_stride) = _mm256_extract_epi64(clip, 0);
}
