...
void aom_lpf_horizontal_14_quad_avx2(unsigned char *s, int p,
...

  {
    ...
    abs_p0q0 = _mm_adds_epu8(abs_p0q0, abs_p0q0);
    abs_p1q1 = _mm_srli_epi16(_mm_and_si128(abs_p1q1, fe), 1);
    mask = _mm_subs_epu8(_mm_adds_epu8(abs_p0q0, abs_p1q1),
                         _mm_set1_epi8(_blimit0[0]));
    mask = _mm_xor_si128(_mm_cmpeq_epi8(mask, zero), ff);
    // mask |= (abs(p0 - q0) * 2 + abs(p1 - q1) / 2  > blimit) * -1;
    mask = _mm_max_epu8(flat, mask);
    // mask |= (abs(p1 - p0) > limit) * -1;
    ...
    work = _mm_max_epu8(
        ...
        _mm_or_si128(_mm_subs_epu8(q3, q2), _mm_subs_epu8(q2, q3)));
    mask = _mm_max_epu8(work, mask);
    mask = _mm_subs_epu8(mask, _mm_set1_epi8(_limit0[0]));
    mask = _mm_cmpeq_epi8(mask, zero);
  }
  ...
  // loop filter
  {
    ...
    hev = _mm_subs_epu8(flat, _mm_set1_epi8(_thresh0[0]));
    hev = _mm_xor_si128(_mm_cmpeq_epi8(hev, zero), ff);
    ...
    __m256i p1q1256 = _mm256_blend_epi32(p256_1, q256_1, 0xf0);
    ...
    if (0xffff != _mm_movemask_epi8(_mm_cmpeq_epi8(flat, zero))) {
      ...
      flat2 = _mm_max_epu8(_mm256_castsi256_si128(flat2_256),
      ...
      res_q =
          _mm256_srli_epi16(_mm256_add_epi16(pixetFilter_q2q1q0, q256_0), 3);
      __m256i flat_p0q0 = _mm256_permute4x64_epi64(_mm256_packus_epi16(res_p, res_q), 0xd8);
      __m256i p0q0256 = _mm256_andnot_si256(flat256, ps0qs0256);
      ...
      p1q1256 = _mm256_andnot_si256(flat256, ps1qs1256);
      ...
      if (0xffff != _mm_movemask_epi8(_mm_cmpeq_epi8(flat2, zero))) {
        ...
        p4 = _mm256_castsi256_si128(p4q4256);
        q4 = _mm256_extractf128_si256(p4q4256, 1);
        ...
        p5 = _mm256_castsi256_si128(p5q5256);
        q5 = _mm256_extractf128_si256(p5q5256, 1);
