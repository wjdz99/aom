...
void aom_lpf_vertical_14_quad_avx2(unsigned char *s, int p,
  ...
  // Transpose 16x16
  transpose_16x8(s - 8, s - 8 + 8 * p, p, t_dst, 16);
  transpose_16x8(s, s + 8 * p, p, t_dst + 8 * 16, 16);
  ...
  // Transpose back
  transpose_16x8(t_dst, t_dst + 8 * 16, 16, s - 8, p);
  transpose_16x8(t_dst + 8, t_dst + 8 + 8 * 16, 16, s - 8 + 8 * p, p);
}

void aom_lpf_vertical_8_quad_avx2(uint8_t *s, int p, const uint8_t *_blimit0,
                                  const uint8_t *_limit0,
                                  const uint8_t *_thresh0) {
  ...
  // Transpose 16x8
  transpose_16x8(s - 4, s - 4 + p * 8, p, t_dst, 16);

  // Loop filtering
  aom_lpf_horizontal_8_quad_avx2(t_dst + 4 * 16, 16, _blimit0, _limit0,
                                 _thresh0);
  src[0] = t_dst;
  src[1] = t_dst + 8;
  ...
  // Transpose back
  transpose_8xn(src, 16, dst, p, 2);
}

void aom_lpf_vertical_6_quad_avx2(uint8_t *s, int p, const uint8_t *_blimit0,
                                  const uint8_t *_limit0,
                                  const uint8_t *_thresh0) {
  DECLARE_ALIGNED(16, unsigned char, t_dst[16 * 8]);
  unsigned char *src[2];
  ...
  // Loop filtering
  aom_lpf_horizontal_6_quad_avx2(t_dst + 4 * 16, 16, _blimit0, _limit0,
                                 _thresh0);
  src[0] = t_dst;
  src[1] = t_dst + 8;
