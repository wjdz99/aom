...
#define FILTER_HEV_MASK                                                       \
 ...
 AOM_FORCE_INLINE void   filter4_sse2(__m128i *p1p0, __m128i *q1q0, __m128i *hev, __m128i  *mask, __m128i* qs1qs0, __m128i* ps1ps0)
{                                                                      
    const __m128i t3t4 =                                                    
...
#else
      pixelFilter_p = _mm_add_epi16(p5_16,
                                    _mm_add_epi16(p4_16, p3_16));
      pixelFilter_q = _mm_add_epi16(q5_16,
                                    _mm_add_epi16(q4_16, q3_16));
#endif
...
#else
          _mm_add_epi16(pixelFilter_p, _mm_add_epi16(_mm_add_epi16(p6_16, p0_16), _mm_add_epi16(p1_16, q0_16))), 4);
#endif
      res_q = _mm_srli_epi16(
#if !CONFIG_DEBLOCK_13TAP
          _mm_add_epi16(pixelFilter_p, _mm_add_epi16(q7_16, q0_16)), 4);
#else
          _mm_add_epi16(pixelFilter_p, _mm_add_epi16(_mm_add_epi16(q6_16, q0_16), _mm_add_epi16(p0_16, q1_16))), 4);
#endif
...
#else
          _mm_add_epi16(pixelFilter_p, _mm_add_epi16(sum_p6, _mm_add_epi16(p1_16, _mm_add_epi16(p2_16, p0_16)))), 4);
#endif
      res_q = _mm_srli_epi16(
#if !CONFIG_DEBLOCK_13TAP
          _mm_add_epi16(pixelFilter_q, _mm_add_epi16(sum_q7, q1_16)), 4);
#else
          _mm_add_epi16(pixelFilter_q, _mm_add_epi16(sum_q6, _mm_add_epi16(q1_16, _mm_add_epi16(q0_16, q2_16)))), 4);
#endif
...
#else
          _mm_add_epi16(pixelFilter_p, _mm_add_epi16(sum_p6, _mm_add_epi16(p2_16, _mm_add_epi16(p3_16, p1_16)))), 4);
#endif
      res_q = _mm_srli_epi16(
#if !CONFIG_DEBLOCK_13TAP
          _mm_add_epi16(pixelFilter_q, _mm_add_epi16(sum_q7, q2_16)), 4);
#else
          _mm_add_epi16(pixelFilter_q, _mm_add_epi16(sum_q6, _mm_add_epi16(q2_16, _mm_add_epi16(q1_16, q3_16)))), 4);
#endif
...
#else
          _mm_add_epi16(pixelFilter_p, _mm_add_epi16(sum_p6, _mm_add_epi16(p3_16, _mm_add_epi16(p4_16, p2_16)))), 4);
#endif
      res_q = _mm_srli_epi16(
#if !CONFIG_DEBLOCK_13TAP
          _mm_add_epi16(pixelFilter_q, _mm_add_epi16(sum_q7, q3_16)), 4);
#else
          _mm_add_epi16(pixelFilter_q, _mm_add_epi16(sum_q6, _mm_add_epi16(q3_16, _mm_add_epi16(q2_16, q4_16)))), 4);
#endif
...
#else
          _mm_add_epi16(pixelFilter_p, _mm_add_epi16(sum_p6, _mm_add_epi16(p4_16, _mm_add_epi16(p5_16, p3_16)))), 4);
#endif
      res_q = _mm_srli_epi16(
#if !CONFIG_DEBLOCK_13TAP
          _mm_add_epi16(pixelFilter_q, _mm_add_epi16(sum_q7, q4_16)), 4);
#else
          _mm_add_epi16(pixelFilter_q, _mm_add_epi16(sum_q6, _mm_add_epi16(q4_16, _mm_add_epi16(q3_16, q5_16)))), 4);
#endif
...
#else
          _mm_add_epi16(pixelFilter_p, _mm_add_epi16(sum_p6, _mm_add_epi16(p5_16, _mm_add_epi16(p6_16, p4_16)))), 4);
#endif
      res_q = _mm_srli_epi16(
#if !CONFIG_DEBLOCK_13TAP
          _mm_add_epi16(pixelFilter_q, _mm_add_epi16(sum_q7, q5_16)), 4);
#else
          _mm_add_epi16(pixelFilter_q, _mm_add_epi16(sum_q6, _mm_add_epi16(q5_16, _mm_add_epi16(q6_16, q4_16)))), 4);
#endif
...
void aom_lpf_horizontal_6_sse2(unsigned char *s, int p,
	...
	const unsigned char *_thresh) {
			...
			const __m128i ff = _mm_cmpeq_epi8(fe, fe);
			{
				...
				//considering sse doesn't have unsigned elements comparison the idea is to find at least one case when X > limit, it means the corresponding  mask bit is set.
				//to achieve that we find global max value of all inputs of abs(x-y) or (abs(p0 - q0) * 2 + abs(p1 - q1) / 2 If it is > limit the mask is set otherwise - not
				...
				maskmmask = _mm_movemask_epi8(mask); //if 0 we don't need to do 5 tap filter, otherwise don't need to do filter4_sse2
				...
				flatmmask = _mm_movemask_epi8(flat); //if 0 we don't need to do 5 tap filter, otherwise don't need to do filter4_sse2
			...
			unsigned char *src = s;
				...
				workp_a = _mm_add_epi16(_mm_add_epi16(workp_a, four), p2);  // p2 + p0 * 2 + p1 * 2 + 4

				workp_b = _mm_add_epi16(_mm_add_epi16(p2, p2), q0); 
				workp_shft0 = _mm_srli_epi16(_mm_add_epi16(workp_a, workp_b), 3);//p2 * 3 + p1 * 2 + p0 * 2 + q0 + 4

																					//op0
				workp_b = _mm_add_epi16(_mm_add_epi16(q0, q0), q1);  //q0 * 2 + q1
				workp_a = _mm_add_epi16(workp_a, workp_b);   // p2 + p0 * 2 + p1 * 2 + q0 * 2 + q1 + 4
				workp_shft1 = _mm_srli_epi16(workp_a, 3);

				flat_p1p0 = _mm_packus_epi16(workp_shft1, workp_shft0);
	
				//oq0
				workp_a = _mm_sub_epi16(_mm_sub_epi16(workp_a, p2), p1); //p0 * 2 + p1  + q0 * 2 + q1 + 4
				...
				//oq1
				workp_a = _mm_sub_epi16(_mm_sub_epi16(workp_a, p1), p0); //p0   + q0 * 2 + q1 * 2 + q2 + 4
				workp_b = _mm_add_epi16(q2, q2);
				workp_shft1 = _mm_srli_epi16(_mm_add_epi16(workp_a, workp_b), 3); //p0  + q0 * 2 + q1 * 2 + q2 * 3 + 4
					
				flat_q0q1 = _mm_packus_epi16(workp_shft0, workp_shft1);
			}
			else {
				flat_p1p0 = _mm_setzero_si128(); flat_q0q1 = _mm_setzero_si128();
			}
			if ((flatmmask & maskmmask) != 0xffff)
			{
				...
				ps1ps0 = _mm_setzero_si128(); qs1qs0 = _mm_setzero_si128();
			}
...
static INLINE void transpose6x6(unsigned char *src[], int in_p,
	...
	do {
		...
		x0 = _mm_unpacklo_epi8(x0, x1);// 00 10 01 11 02 12 03 13  04 14 05 15 xx xx  xx xx

		x2 =
			_mm_loadl_epi64((__m128i *)(in + 2 * in_p));  // 20 21 22 23 24 25
		x3 =
			_mm_loadl_epi64((__m128i *)(in + 3 * in_p));  // 30 31 32 33 34 35

		x1 = _mm_unpacklo_epi8(x2, x3);// 20 30 21 31 22 32 23 33  24 34 25 35 xx xx  xx xx

		x4 =
			_mm_loadl_epi64((__m128i *)(in + 4 * in_p));  // 40 41 42 43 44 45
		x5 =
			_mm_loadl_epi64((__m128i *)(in + 5 * in_p));  // 50 51 52 53 54 55
		...
		x5 = _mm_unpacklo_epi16(x2, x0); // 40 50 xx xx 41 51 xx xx 42 52 xx xx 43 53 xx xx  
		
		x6 = _mm_unpacklo_epi32(x4, x5); // 00 10 20 30 40 50 xx xx 01 11 21 31 41 51 xx xx
		_mm_store_si128((__m128i*)(temp_dst), x6);
		memcpy(out + 0 * out_p, temp_dst, 6);
		memcpy(out + 1 * out_p, temp_dst + 8, 6);	

		x6 = _mm_unpackhi_epi32(x4, x5); // 02 12 22 32 42 52 xx xx 03 13 23 33 43 53 xx xx
		_mm_store_si128((__m128i*)(temp_dst), x6);
		memcpy(out + 2 * out_p, temp_dst, 6);	
		memcpy(out + 3 * out_p, temp_dst + 8, 6);	
									
		x4 = _mm_unpackhi_epi16(x0, x1); // 04 14 24 34 05 15 25 35 xx xx xx xx xx xx xx xx	
		x5 = _mm_unpackhi_epi16(x2, x3); // 44 54 xx xx 45 55 xx xx xx xx xx xx xx xx xx xx	
		x6 = _mm_unpacklo_epi32(x4, x5); // 04 14 24 34 44 54 xx xx 05 15 25 35 45 55 xx xx
