...
#define FILTER_HEV_MASK4                                                      \
  do {                                                                        \
    ...
    hev = _mm_packs_epi16(hev, hev);                                          \
                                                                              \
    /* const int8_t mask = filter_mask4(*limit, *blimit, */                   \
    /*                                  p1, p0, q0, q1); */                   \
    abs_p0q0 =                                                                \
...
#endif // (0 == CONFIG_PARALLEL_DEBLOCKING)
...
#if (0 ==  CONFIG_PARALLEL_DEBLOCKING)
  FILTER_HEV_MASK;
#else // !(0 ==  CONFIG_PARALLEL_DEBLOCKING)
  FILTER_HEV_MASK4;
#endif // (0 ==  CONFIG_PARALLEL_DEBLOCKING)
  FILTER4;
...
#define punpcklbw(r0, r1) \
    _mm_unpacklo_epi8(r0, r1)

#define punpcklwd(r0, r1) \
    _mm_unpacklo_epi16(r0, r1)

#define punpckhwd(r0, r1) \
    _mm_unpackhi_epi16(r0, r1)

#define movd(p, r) \
    *((uint32_t *) (p)) = _mm_cvtsi128_si32(r)

#define pshufd(r, imm) \
    _mm_shuffle_epi32(r, imm)

enum
{
    ROTATE_DWORD_RIGHT = 0x39
};

static INLINE void transpose16x4(uint8_t *pDst, const ptrdiff_t dstStride,
                                 const uint8_t *pSrc, const ptrdiff_t srcStride) {
