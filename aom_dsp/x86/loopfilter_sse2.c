...
static INLINE __m128i filter8_mask_1(const __m128i *const flat,
                                     const __m128i *const other_filt,
                                     const __m128i *const f8_lo,
                                     const __m128i *const f8_hi) {
  const __m128i f8 =
...
static INLINE __m128i filter16_mask_1(const __m128i *const flat,
                                      const __m128i *const other_filt,
...
static INLINE void transpose(unsigned char *src[], int in_p,
  ...
  do {
    ...
    mm_storehu(out + 1 * out_p,
               x6);  // 01 11 21 31 41 51 61 71
    ...
    mm_storehu(out + 5 * out_p,
               x6);  // 05 15 25 35 45 55 65 75
...
void aom_lpf_horizontal_14_quad_sse2(unsigned char *s, int p,
  ...
  // const __m128i ff = _mm_cmpeq_epi8(zero, zero);
  // if (0xffff == _mm_movemask_epi8(_mm_cmpeq_epi8(flat2, ff))) {
  //  const __m128i eight = _mm_set1_epi16(8);
  ...
  //  f_lo =
  //      _mm_add_epi16(_mm_add_epi16(p3_lo, f_lo), _mm_add_epi16(p2_lo,
  //      p1_lo));
  ...
  //  op1 = filter16_mask_1(&flat2, &op1, &f_lo, &f_hi);
  ...
  //  op0 = filter16_mask_1(&flat2, &op0, &f_lo, &f_hi);
  ...
  //  oq0 = filter16_mask_1(&flat2, &oq0, &f_lo, &f_hi);
  ...
  //  oq1 = filter16_mask_1(&flat2, &oq1, &f_lo, &f_hi);
  ...
  //  oq2 = filter16_mask_1(&flat2, &oq2, &f_lo, &f_hi);
...
void aom_lpf_horizontal_8_quad_sse2(unsigned char *s, int p,
  ...
  // const __m128i ff = _mm_cmpeq_epi8(zero, zero);
  // if (0xffff == _mm_movemask_epi8(_mm_cmpeq_epi8(flat, ff))) {
  //  const __m128i four = _mm_set1_epi16(4);
  //  const __m128i p3_lo = _mm_unpacklo_epi8(p3, zero);
  //  const __m128i p2_lo = _mm_unpacklo_epi8(p2, zero);
  //  const __m128i p1_lo = _mm_unpacklo_epi8(p1, zero);
  ...
  //  f8_lo =
  //      _mm_add_epi16(_mm_add_epi16(p3_lo, four), _mm_add_epi16(p3_lo,
  //      p2_lo));
  //  f8_lo =
  //      _mm_add_epi16(_mm_add_epi16(p3_lo, f8_lo), _mm_add_epi16(p2_lo,
  //      p1_lo));
  //  f8_lo = _mm_add_epi16(_mm_add_epi16(p0_lo, q0_lo), f8_lo);

  //  f8_hi =
  //      _mm_add_epi16(_mm_add_epi16(p3_hi, four), _mm_add_epi16(p3_hi,
  //      p2_hi));
  //  f8_hi =
  //      _mm_add_epi16(_mm_add_epi16(p3_hi, f8_hi), _mm_add_epi16(p2_hi,
  //      p1_hi));
  //  f8_hi = _mm_add_epi16(_mm_add_epi16(p0_hi, q0_hi), f8_hi);

  //  op2 = filter8_mask_1(&flat, &p2, &f8_lo, &f8_hi);
  //  _mm_storeu_si128((__m128i *)(s - 3 * p), op2);

  //  f8_lo = filter_add2_sub2(&f8_lo, &q1_lo, &p1_lo, &p2_lo, &p3_lo);
  //  f8_hi = filter_add2_sub2(&f8_hi, &q1_hi, &p1_hi, &p2_hi, &p3_hi);
  //  op1 = filter8_mask_1(&flat, &op1, &f8_lo, &f8_hi);
  //  _mm_storeu_si128((__m128i *)(s - 2 * p), op1);

  //  f8_lo = filter_add2_sub2(&f8_lo, &q2_lo, &p0_lo, &p1_lo, &p3_lo);
  //  f8_hi = filter_add2_sub2(&f8_hi, &q2_hi, &p0_hi, &p1_hi, &p3_hi);
  //  op0 = filter8_mask_1(&flat, &op0, &f8_lo, &f8_hi);
  //  _mm_storeu_si128((__m128i *)(s - 1 * p), op0);

  //  f8_lo = filter_add2_sub2(&f8_lo, &q3_lo, &q0_lo, &p0_lo, &p3_lo);
  //  f8_hi = filter_add2_sub2(&f8_hi, &q3_hi, &q0_hi, &p0_hi, &p3_hi);
  //  oq0 = filter8_mask_1(&flat, &oq0, &f8_lo, &f8_hi);
  //  _mm_storeu_si128((__m128i *)(s - 0 * p), oq0);

  //  f8_lo = filter_add2_sub2(&f8_lo, &q3_lo, &q1_lo, &q0_lo, &p2_lo);
  //  f8_hi = filter_add2_sub2(&f8_hi, &q3_hi, &q1_hi, &q0_hi, &p2_hi);
  //  oq1 = filter8_mask_1(&flat, &oq1, &f8_lo, &f8_hi);
  //  _mm_storeu_si128((__m128i *)(s + 1 * p), oq1);

  //  f8_lo = filter_add2_sub2(&f8_lo, &q3_lo, &q2_lo, &q1_lo, &p1_lo);
  //  f8_hi = filter_add2_sub2(&f8_hi, &q3_hi, &q2_hi, &q1_hi, &p1_hi);
  //  oq2 = filter8_mask_1(&flat, &q2, &f8_lo, &f8_hi);
  //  _mm_storeu_si128((__m128i *)(s + 2 * p), oq2);
...
void aom_lpf_horizontal_6_quad_sse2(unsigned char *s, int p,
  ...
  //  f8_lo =
  //      _mm_add_epi16(_mm_add_epi16(p2_lo, four), _mm_add_epi16(p2_lo,
  //      p2_lo));
  //  f8_lo =
  //      _mm_add_epi16(_mm_add_epi16(p1_lo, f8_lo), _mm_add_epi16(p1_lo,
  //      p0_lo));
  //  f8_lo = _mm_add_epi16(_mm_add_epi16(p0_lo, q0_lo), f8_lo);

  //  f8_hi =
  //      _mm_add_epi16(_mm_add_epi16(p2_hi, four), _mm_add_epi16(p2_hi,
  //      p2_hi));
  //  f8_hi =
  //      _mm_add_epi16(_mm_add_epi16(p1_hi, f8_hi), _mm_add_epi16(p1_hi,
  //      p0_hi));
  //  f8_hi = _mm_add_epi16(_mm_add_epi16(p0_hi, q0_hi), f8_hi);
