...
static INLINE void variance8_quad_avx2(const uint8_t *src, const int src_stride,
                                       const uint8_t *ref, const int ref_stride,
                                       const int h, unsigned int *const sse,
                                       int *const sum) {
  ...
  for (int i = 0; i < h; i += 2) {
    // Process 8x32 block in first row.
    calc_sum_sse_for_8x32_block_avx2(src, ref, sse_8x16, sum_8x16);

    // Process 8x32 block in second row.
...
void aom_get8x8var_quad_avx2(const uint8_t *src_ptr, int src_stride,
                             const uint8_t *ref_ptr, int ref_stride,
                             unsigned int *sse, int *sum) {
  variance8_quad_avx2(src_ptr, src_stride, ref_ptr, ref_stride, 8, sse, sum);
}
