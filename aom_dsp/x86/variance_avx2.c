...
void aom_highbd_comp_mask_pred_avx2(uint8_t *comp_pred8, const uint8_t *pred8,
  ...
  if (width == 8) {
    ...
    do {
      for(int x=0; x < width/32; x++){
        const __m256i s0 = _mm256_loadu_si256((const __m256i *)(src0 + x*32));
        const __m256i s2 = _mm256_loadu_si256((const __m256i *)(src0 + x*32 + 16));
        const __m256i s1 = _mm256_loadu_si256((const __m256i *)(src1 + x*32));
        const __m256i s3 = _mm256_loadu_si256((const __m256i *)(src1 + x*32 + 16));
  
        const __m256i m01_16 =
            _mm256_cvtepu8_epi16(_mm_loadu_si128((const __m128i *)(mask + x*32)));
        const __m256i m23_16 =
            _mm256_cvtepu8_epi16(_mm_loadu_si128((const __m128i *)(mask + x*32 + 16)));
