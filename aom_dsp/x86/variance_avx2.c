...
uint64_t aom_mse_4xh_16bit_avx2(uint8_t *dst, int dstride, uint16_t *src,
  ...
  for (i = 0; i < h; i += 4) {
    ...
    src1_8x16 = _mm256_castsi128_si256(_mm_unpacklo_epi64(src2_4x16, src3_4x16));
...
uint64_t aom_mse_8xh_16bit_avx2(uint8_t *dst, int dstride, uint16_t *src,int sstride,int h) {
  uint64_t sum = 0;
  int i;
  ...
  for (i = 0; i < h; i += 2) {
    ...
    sub_result = _mm256_abs_epi16(_mm256_sub_epi16(src_16x16, dst_16x16));
    //use src_16x16 and dst_16x16 as intermediate scratch buffer
    src_16x16 = _mm256_unpacklo_epi16(sub_result, zeros);
    dst_16x16 = _mm256_unpackhi_epi16(sub_result, zeros);

    // we don't need src0 and src1,use it as intermediate scratch buffer
    src_16x16 = _mm256_madd_epi16(src_16x16, src_16x16);  // 32 bit store
    dst_16x16 = _mm256_madd_epi16(dst_16x16, dst_16x16);  // 32bit store
  ...
  const __m128i sum_2x64 = _mm_add_epi64(_mm256_castsi256_si128(square_result),
                    _mm256_extracti128_si256(square_result, 1));
  const __m128i sum_1x64 = _mm_add_epi64(sum_2x64, _mm_srli_si128(sum_2x64, 8));
  xx_storel_64(&sum, sum_1x64);
  return sum;
...
uint64_t aom_mse_wxh_16bit_avx2(uint8_t *dst, int dstride, uint16_t *src,
  ...
  assert((w == 8 || w == 4) && (h == 8 || h == 4) && "w=8/4 and h=8/4 must satisfy");
  switch (w) {
    case 4: return aom_mse_4xh_16bit_avx2(dst, dstride, src, sstride,h); break;
    case 8: return aom_mse_8xh_16bit_avx2(dst, dstride, src, sstride,h); break;
    default:
