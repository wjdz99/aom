...
// Visual Studio only supports restrict when /std:c11 (available in 2019+) or
// greater is specified; __restrict can be used in that case.
// TODO(slavarnway): This should be moved to rtcd and used consistently between
// the function declarations and definitions to avoid warnings in Visual
// Studio when defining LIBAOM_RESTRICT to restrict or __restrict.
#if defined(_MSC_VER)
...
void aom_smooth_h_predictor_16x4_ssse3(
    uint8_t *dest, ptrdiff_t stride, const uint8_t *LIBAOM_RESTRICT top_row,
