...
static INLINE void load_weight_v_w8(const uint8_t *weight_array, int height,
  ...
  if (height < 16) {
    const int we_offset = height < 8 ? 4 : 8;
    const __m128i we =
        _mm_loadu_si128((const __m128i *)&weight_array[we_offset]);
    weight_h[0] = _mm_unpacklo_epi8(we, zero);
    weight_h[1] = _mm_sub_epi16(d, weight_h[0]);
  }

  if (height == 16) {
    const __m128i we = _mm_loadu_si128((const __m128i *)&weight_array[16]);
    weight_h[0] = _mm_unpacklo_epi8(we, zero);
    weight_h[1] = _mm_sub_epi16(d, weight_h[0]);
    weight_h[2] = _mm_unpackhi_epi8(we, zero);
    weight_h[3] = _mm_sub_epi16(d, weight_h[2]);
  ...
  if (height == 32) {
    const __m128i we = _mm_loadu_si128((const __m128i *)&weight_array[32]);
    weight_h[0] = _mm_unpacklo_epi8(we, zero);
    weight_h[1] = _mm_sub_epi16(d, weight_h[0]);
    weight_h[2] = _mm_unpackhi_epi8(we, zero);
    weight_h[3] = _mm_sub_epi16(d, weight_h[2]);
    ...
    weight_h[4] = _mm_unpacklo_epi8(we2, zero);
    weight_h[5] = _mm_sub_epi16(d, weight_h[4]);
    weight_h[6] = _mm_unpackhi_epi8(we2, zero);
