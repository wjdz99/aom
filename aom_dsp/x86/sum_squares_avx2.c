...
static uint64_t aom_sse_sum_max_min_2d_i16_nxn_avx2(const int16_t *src,
...
uint64_t aom_sse_2d_i16_avx2(const int16_t *src, int src_stride, int width,
                             int height, int *sum) {
  if (LIKELY(width == 4 && height == 4)) {
    ...
    return aom_sse_sum_max_min_2d_i16_nxn_avx2(src, src_stride, width, height,
                                               sum);
  } else {
