...
static INLINE void highbd_transpose6x6(uint16_t *src[], int in_p, uint16_t *dst[],
	int out_p, int num_6x6_to_transpose) {
	int idx6x6 = 0;
	...
	do {
		...
		*(int*)(out + 4 + 0 * out_p) = _mm_cvtsi128_si32(x2); // 40 50
		...
		_mm_storel_epi64((__m128i *)(out + 1 * out_p), _mm_srli_si128(p5, 8)); //01 11 21 31
		*(int*)(out + 4 + 1 * out_p) = _mm_cvtsi128_si32(_mm_srli_si128(x2,4)); // 41 51

		p1 = _mm_unpackhi_epi32(x0,x1); // 02 12 22 32 03 13 23 33
		_mm_storel_epi64((__m128i *)(out + 2 * out_p), p1); // 02 12 22 32
		*(int*)(out + 4 + 2 * out_p) = _mm_cvtsi128_si32(_mm_srli_si128(x2, 8)); // 42 52

		_mm_storel_epi64((__m128i *)(out + 3 * out_p), _mm_srli_si128(p1, 8)); //03 13 23 33
		*(int*)(out + 4 + 3 * out_p) = _mm_cvtsi128_si32(_mm_srli_si128(x2,12)); // 43 53
		...
		*(int*)(out + 4 + 4 * out_p) = _mm_cvtsi128_si32(x5); // 44 54
		...
		_mm_storel_epi64((__m128i *)(out + 5 * out_p), _mm_srli_si128(p2, 8)); // 05 15 25 35
		*(int*)(out + 4 + 5 * out_p) = _mm_cvtsi128_si32(_mm_srli_si128(x5, 4)); // 45 55

	} while (++idx6x6 < num_6x6_to_transpose);
}

static INLINE void highbd_transpose8x8(uint16_t *src[], int in_p, uint16_t *dst[],
