...
int aom_vector_var_sse4_1(const int16_t *ref, const int16_t *src,
  ...
  for (int i = 0; i < width; i += 16) {
    __m128i src_line = _mm_loadu_si128((const __m128i *)src);
    __m128i ref_line = _mm_loadu_si128((const __m128i *)ref);
    __m128i src_line2 = _mm_loadu_si128((const __m128i *)(src + 8));
    __m128i ref_line2 = _mm_loadu_si128((const __m128i *)(ref + 8));
    __m128i diff = _mm_sub_epi16(ref_line, src_line);
    __m128i diff2 = _mm_sub_epi16(ref_line2, src_line2);
    __m128i diff_sqr = _mm_madd_epi16(diff, diff);
    __m128i diff_sqr2 = _mm_madd_epi16(diff2, diff2);
  ...
  const unsigned int mean_int = _mm_extract_epi32(mean, 0);
  const unsigned int sse_int = _mm_extract_epi32(sse, 0);
  const unsigned int var = sse_int - ((mean_int * mean_int) >> (log_bw + 2));
  return var;
}
