...
static INLINE void transpose4x4_simd(const float *A, float *B, const int lda,
                                     const int ldb) {
...
void aom_transpose_float_sse2(const float *A, float *B, int n) {
  for (int y = 0; y < n; y += 4) {
    for (int x = 0; x < n; x += 4) {
      transpose4x4_simd(A + y * n + x, B + x * n + y, n, n);
    }
  }
}
...
// Generate definitions for 1d transforms using float and mm128
...
// The inverse transform needs to use non vectorized 1d forward transforms also
void aom_fft1d_4_float(const float *, float *, int stride);
void aom_fft1d_8_float(const float *, float *, int stride);
void aom_fft1d_16_float(const float *, float *, int stride);
void aom_fft1d_32_float(const float *, float *, int stride);
