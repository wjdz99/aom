...
static INLINE void prepare_coeffs_12tap(
  ...
  coeffs_y = _mm_loadu_si128((__m128i *)(y_filter + 4));
  // coeffs 8 9 8 9 10 11 10 11
  const __m128i tmp_2 = _mm_unpackhi_epi32(coeffs_y, coeffs_y);
  ...
  coeffs[4] = _mm_unpacklo_epi64(tmp_2, tmp_2);  // coeffs 8 9 8 9 8 9 8 9
  coeffs[5] =
      _mm_unpackhi_epi64(tmp_2, tmp_2);  // coeffs 10 11 10 11 10 11 10 11
}
...
static INLINE __m128i convolve_lo_x_12tap(const __m128i *const s,
  ...
  ss[5] = _mm_unpackhi_epi8(s[3], zero);  // 10 11 12 13 14 15 16 17
...
static INLINE void av1_convolve_x_sr_12tap_sse2(
  ...
  do {
    ...
    do {
      ...
      const __m128i res_lo = convolve_lo_x_12tap(s, coeffs, zero);
      ...
      const __m128i res16 = _mm_packs_epi32(res_lo_round, zero);
