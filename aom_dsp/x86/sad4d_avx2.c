...
static AOM_FORCE_INLINE void aom_sad16xNx4d_avx2(
    int N, const uint8_t *src, int src_stride, const uint8_t *ref[4],
    int ref_stride, int32_t err[4], uint32_t *min_value, int32_t *min_pos) {
  ...
  }
  {
    ...
    DECLARE_ALIGNED(16, uint16_t, mask[8]) = { 0xffff, 0, 0xffff, 0,
                                               0xffff, 0, 0xffff, 0 };
    int src_16 = _mm_test_all_zeros(sum_mlow, *(__m128i *)mask);
    // src16 is zero if we stay within 16 bits
    if (src_16) {
      aom_sadx4d_get_min16(sum_mlow, min_value, min_pos);
    } else {
      DECLARE_ALIGNED(16, uint32_t, res[4]);
      _mm_store_si128((__m128i *)(res), sum_mlow);
      get_min_value_pos(res, 4, min_value, min_pos);
    }
  }
}

static AOM_FORCE_INLINE void aom_sadMxNx4d_avx2(
    int M, int N, const uint8_t *src, int src_stride, const uint8_t *ref[4],
    int ref_stride, int32_t err[4], uint32_t *min_value, int32_t *min_pos) {
  ...
  }
  {
    ...
    DECLARE_ALIGNED(16, uint16_t, mask[8]) = { 0xffff, 0, 0xffff, 0,
                                               0xffff, 0, 0xffff, 0 };
    int src_16 = _mm_test_all_zeros(sum, *(__m128i *)mask);
    // src16 is 1 if we stay within 16 bits
    if (src_16) {
...
#define sad16xNx4d_avx2(n)                                                     \
  void aom_sad16x##n##x4d_avx2(                                                \
      const uint8_t *src, int src_stride, const uint8_t *ref[4],               \
      int ref_stride, int32_t err[4], uint32_t *min_value, int32_t *min_pos) { \
    aom_sad16xNx4d_avx2(n, src, src_stride, ref, ref_stride, err, min_value,   \
                        min_pos);                                              \
  }

#define sadMxNx4d_avx2(m, n)                                                   \
  void aom_sad##m##x##n##x4d_avx2(                                             \
      const uint8_t *src, int src_stride, const uint8_t *ref[4],               \
      int ref_stride, int32_t err[4], uint32_t *min_value, int32_t *min_pos) { \
    aom_sadMxNx4d_avx2(m, n, src, src_stride, ref, ref_stride, err, min_value, \
                       min_pos);                                               \
  }
