...
uint64_t aom_sse_sum_2d_i16_4x4_sse2(const int16_t *src, int stride, int *sum) {
...
uint64_t aom_sse_sum_2d_i16_4xn_sse2(const int16_t *src, int stride, int height,
                                     int *sum) {
  int r = 0;
  uint64_t sse = 0;
  do {
    int curr_sum = 0;
    sse += aom_sse_sum_2d_i16_4x4_sse2(src, stride, &curr_sum);
    *sum += curr_sum;
    src += stride << 2;
    r += 4;
  } while (r < height);
...
#ifdef __GNUC__
// This prevents GCC/Clang from inlining this function into
// aom_sse_sum_2d_i16_nxn_sse2, which in turn saves some stack
// maintenance instructions in the common case of 4x4.
__attribute__((noinline))
#endif
uint64_t
aom_sse_sum_2d_i16_nxn_sse2(const int16_t *src, int stride, int width,
                            int height, int *sum) {
  int r = 0;
  uint64_t result;
...
uint64_t aom_sse_2d_i16_sse2(const int16_t *src, int src_stride, int width,
                             int height, int *sum) {
  if (LIKELY(width == 4 && height == 4)) {
    return aom_sse_sum_2d_i16_4x4_sse2(src, src_stride, sum);
  } else if (LIKELY(width == 4 && (height & 3) == 0)) {
    return aom_sse_sum_2d_i16_4xn_sse2(src, src_stride, height, sum);
  } else if (LIKELY((width & 7) == 0 && (height & 3) == 0)) {
    // Generic case
    return aom_sse_sum_2d_i16_nxn_sse2(src, src_stride, width, height, sum);
  } else {
