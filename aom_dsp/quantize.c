...
void aom_quantize_b_adaptive_helper_c(
  ...
  int prescan_add[2];
  for (i = 0; i < 2; ++i)
    prescan_add[i] = ROUND_POWER_OF_TWO(dequant_ptr[i] * EOB_FACTOR, 7 + QUANT_TABLE_BITS);
  ...
  for (i = 0; i < non_zero_count; i++) {
    ...
    if (abs_coeff * wt >= (zbins[rc != 0] << AOM_QM_BITS)) {
      ...
      const int dequant =
          ...
          AOM_QM_BITS;
      const tran_low_t abs_dqcoeff = (tmp32 * dequant) >> (log_scale + QUANT_TABLE_BITS);
  ...
  if (eob >= 0 && first == eob) {
    ...
    if (qcoeff_ptr[rc] == 1 || qcoeff_ptr[rc] == -1) {
      ...
      const int prescan_add_val =
          ROUND_POWER_OF_TWO(dequant_ptr[rc != 0] * factor, 7 + QUANT_TABLE_BITS);
      if (coeff < (zbins[rc != 0] * (1 << AOM_QM_BITS) + prescan_add_val) &&
          coeff > (nzbins[rc != 0] * (1 << AOM_QM_BITS) - prescan_add_val)) {
        qcoeff_ptr[rc] = 0;
        dqcoeff_ptr[rc] = 0;
  ...
  for (i = 0; i < non_zero_count; i++) {
    ...
    if (abs_coeff * wt >= (zbins[rc != 0] << AOM_QM_BITS)) {
      ...
      const int dequant =
          (dequant_ptr[rc != 0] * iwt + (1 << (AOM_QM_BITS - 1))) >>
          AOM_QM_BITS;
      const tran_low_t abs_dqcoeff = (abs_qcoeff * dequant) >> (log_scale + QUANT_TABLE_BITS);
      dqcoeff_ptr[rc] = (tran_low_t)((abs_dqcoeff ^ coeff_sign) - coeff_sign);
      if (abs_qcoeff) {
        eob = i;
#if SKIP_EOB_FACTOR_ADJUST
