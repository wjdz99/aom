...
#endif  // CONFIG_AOM_QM
        int64_t tmp =
            clamp(abs_coeff + ROUND_POWER_OF_TWO(
                                  (round_ptr[rc != 0] * FACTOR), log_scale),
                  INT16_MIN, INT16_MAX);
#if CONFIG_AOM_QM
        tmp *= wt;
...
void aom_quantize_dc(const tran_low_t *coeff_ptr, int n_coeffs, int skip_block,
  ...
  if (!skip_block) {
    tmp = clamp(abs_coeff + (round_ptr[rc != 0] * FACTOR), INT16_MIN,
                INT16_MAX);
    tmp = (tmp * quant) >> 16;
...
void aom_quantize_dc_32x32(const tran_low_t *coeff_ptr, int skip_block,
  ...
  if (!skip_block) {
    tmp = clamp(
        abs_coeff + ROUND_POWER_OF_TWO((round_ptr[rc != 0] * FACTOR), 1),
        INT16_MIN, INT16_MAX);
    tmp = (tmp * quant) >> 15;
    qcoeff_ptr[rc] = (tmp ^ coeff_sign) - coeff_sign;
...
void aom_highbd_quantize_dc_32x32(const tran_low_t *coeff_ptr, int skip_block,
  ...
  if (!skip_block) {
    ...
    const int abs_coeff = (coeff ^ coeff_sign) - coeff_sign;
    const int64_t tmp =
        abs_coeff + ROUND_POWER_OF_TWO((round_ptr[0] * FACTOR), 1);
...
void aom_highbd_quantize_b_c(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  if (!skip_block) {
    ...
    for (i = 0; i < non_zero_count; i++) {
      ...
      if (abs_coeff >= zbins[rc != 0]) {
        const int64_t tmp1 = abs_coeff + (round_ptr[rc != 0] * FACTOR);
