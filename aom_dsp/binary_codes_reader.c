...
uint16_t aom_read_primitive_refbilevel(aom_reader *r, uint16_t n, uint16_t p,
  ...
  int v;
  if (aom_read_bit(r, ACCT_STR_NAME)) {
    v = aom_read_primitive_quniform(r, p ACCT_STR_ARG(ACCT_STR_NAME)) + lolimit;
  } else {
    v = aom_read_primitive_quniform(r, n - p ACCT_STR_ARG(ACCT_STR_NAME));
    if (v >= lolimit) v += p;
  }
  return v;
}
...
uint16_t aom_read_primitive_subexpfin(aom_reader *r, uint16_t n,
  ...
  while (1) {
    ...
    int a = (1 << b);
    if (n <= mk + 3 * a) {
      v = aom_read_primitive_quniform(r, n - mk ACCT_STR_ARG(ACCT_STR_NAME)) +
          mk;
      break;
    } else {
      if (aom_read_bit(r, ACCT_STR_NAME)) {
...
uint16_t aom_read_primitive_refsubexpfin(aom_reader *r, uint16_t n, uint16_t k,
  ...
  return inv_recenter_finite_nonneg(
      n, ref,
      aom_read_primitive_subexpfin(r, n, k ACCT_STR_ARG(ACCT_STR_NAME)));
}
...
int16_t aom_read_signed_primitive_refsubexpfin(aom_reader *r, uint16_t n,
  ...
  return aom_read_primitive_refsubexpfin(r, scaled_n, k,
