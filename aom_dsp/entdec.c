...
static void od_ec_dec_refill(od_ec_dec *dec) {
  ...
  for (; s >= 0 && bptr < end; s -= 8, bptr++) {
    /*Each time a byte is inserted into the window, bptr advances and cnt is
       incremented by 8, so the total number of consumed bits (the return value
       of od_ec_dec_tell) does not change.*/
