...
void assign_default(aom_film_grain_t *params) {
  ...
  int scaling_points_y[][2] = { { 16, 0 },    { 25, 136 },  { 33, 144 },
                                { 41, 160 },  { 48, 168 },  { 56, 136 },
                                { 67, 128 },  { 82, 144 },  { 97, 152 },
                                { 113, 144 }, { 128, 176 }, { 143, 168 },
                                { 158, 176 }, { 178, 184 } };  // 36

  params->num_y_points = 14;

  int scaling_points_cb[][2] = { { 16, 0 },    { 20, 64 },  { 28, 88 },
                                 { 60, 104 },  { 90, 136 }, { 105, 160 },
                                 { 134, 168 }, { 168, 208 } };

  params->num_cb_points = 8;

  int scaling_points_cr[][2] = { { 16, 0 },    { 28, 96 },   { 56, 80 },
                                 { 66, 96 },   { 80, 104 },  { 108, 96 },
                                 { 122, 112 }, { 137, 112 }, { 169, 176 } };
    ...
    for (int j = left_pad; j < chroma_block_size_x - right_pad; j++) {
      ...
      cb_grain_block[i * chroma_grain_stride + j] =
          clamp(cb_grain_block[i * chroma_grain_stride + j] +
                    (wsum_cb >> params->ar_coeff_shift),
                grain_min, grain_max);
      cr_grain_block[i * chroma_grain_stride + j] =
          clamp(cr_grain_block[i * chroma_grain_stride + j] +
                    (wsum_cr >> params->ar_coeff_shift),
                grain_min, grain_max);
    }
}
