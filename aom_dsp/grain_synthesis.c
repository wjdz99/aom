...
void dealloc_arrays(aom_film_grain_t *params, int ***pred_pos_luma,
  ...
  for (int row = 0; row < num_pos_luma; row++) {
    if ((*pred_pos_luma)[row]) free((*pred_pos_luma)[row]);
  }
  if ((*pred_pos_luma)) free(*pred_pos_luma);

  for (int row = 0; row < num_pos_chroma; row++) {
    if ((*pred_pos_chroma)[row]) free((*pred_pos_chroma)[row]);
  }
  if (*pred_pos_chroma) free((*pred_pos_chroma));

  if (*y_line_buf) free(*y_line_buf);

  if (*cb_line_buf) free(*cb_line_buf);

  if (*cr_line_buf) free(*cr_line_buf);

  if (*y_col_buf) free(*y_col_buf);

  if (*cb_col_buf) free(*cb_col_buf);

  if (*cr_col_buf) free(*cr_col_buf);

  if (*luma_grain_block) free(*luma_grain_block);

  if (*cb_grain_block) free(*cb_grain_block);

  if (*cr_grain_block) free(*cr_grain_block);
}
...
void add_film_grain(aom_film_grain_t *params, uint8_t *luma, uint8_t *cb,
