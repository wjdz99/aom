...
void assign_default(aom_film_grain_t *params) {
  ...
  memcpy(params->scaling_points_y, scaling_points_y,
         sizeof(scaling_points_y[0][0]) * 14 * 2);
  memcpy(params->scaling_points_cb, scaling_points_cb,
         sizeof(scaling_points_cb[0][0]) * 8 * 2);
  memcpy(params->scaling_points_cr, scaling_points_cr,
         sizeof(scaling_points_cr[0][0]) * 9 * 2);
  ...
  memcpy(params->ar_coeffs_y, arc_y, sizeof(arc_y[0]) * num_pos_luma);
  memcpy(params->ar_coeffs_cb, arc_cb, sizeof(arc_cb[0]) * num_pos_chroma);
  memcpy(params->ar_coeffs_cr, arc_cr, sizeof(arc_cr[0]) * num_pos_chroma);

  params->ar_coeff_lag = ar_coeff_lag;

  params->cb_mult = 247;       // 8 bit
  params->cb_luma_mult = 192;  // 8 bit
  params->cb_offset = 18;      // 9 bit

  params->cr_mult = 229;       // 8 bit
  params->cr_luma_mult = 192;  // 8 bit
  params->cr_offset = 54;      // 9 bit
...
void init_arrays(aom_film_grain_t *params, int luma_stride, int chroma_stride,
                 int ***pred_pos_luma_p, int ***pred_pos_chroma_p,
  ...
  pred_pos_luma = (int **)malloc(sizeof(*pred_pos_luma) * num_pos_luma);

  for (int row = 0; row < num_pos_luma; row++) {
    pred_pos_luma[row] = (int *)malloc(sizeof(**pred_pos_luma) * 3);
  }

  pred_pos_chroma = (int **)malloc(sizeof(*pred_pos_chroma) * num_pos_chroma);

  for (int row = 0; row < num_pos_chroma; row++) {
    pred_pos_chroma[row] = (int *)malloc(sizeof(**pred_pos_chroma) * 3);
  }
  ...
  *y_line_buf = (int *)malloc(sizeof(**y_line_buf) * luma_stride * 2);
  *cb_line_buf = (int *)malloc(sizeof(**cb_line_buf) * chroma_stride);
  *cr_line_buf = (int *)malloc(sizeof(**cr_line_buf) * chroma_stride);

  *y_col_buf =
      (int *)malloc(sizeof(**y_col_buf) * (luma_subblock_size + 2) * 2);
  *cb_col_buf =
      (int *)malloc(sizeof(**cb_col_buf) * (chroma_subblock_size + 1));
  *cr_col_buf =
      (int *)malloc(sizeof(**cr_col_buf) * (chroma_subblock_size + 1));

  *luma_grain_block =
      (int *)malloc(sizeof(**luma_grain_block) * luma_grain_samples);
  *cb_grain_block =
      (int *)malloc(sizeof(**cb_grain_block) * chroma_grain_samples);
  *cr_grain_block =
      (int *)malloc(sizeof(**cr_grain_block) * chroma_grain_samples);
}
...
void generate_luma_grain_block(aom_film_grain_t *params, int **pred_pos_luma,
  ...
  // can be removed if support for 12 bits in not needed
...
void copy_rect(uint8_t *src, int src_stride, uint8_t *dst, int dst_stride,
               int width, int height, int high_bit_depth) {
...
void add_film_grain(aom_film_grain_t *params, aom_image_t *src,
                    aom_image_t *dst) {
  uint8_t *luma, *cb, *cr;
  ...
  luma_stride = (dst->bit_depth > 8) ? dst->stride[AOM_PLANE_Y] / 2
                                     : dst->stride[AOM_PLANE_Y];
  chroma_stride = (dst->bit_depth > 8) ? dst->stride[AOM_PLANE_U] / 2
                                       : dst->stride[AOM_PLANE_U];
  width = dst->d_w;
  height = dst->d_h;
  params->bit_depth = dst->bit_depth;

  add_film_grain_run(params, luma, cb, cr, height, width, luma_stride,
                     chroma_stride);
  return;
}
