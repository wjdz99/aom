...
void aom_hadamard_lp_8x8_neon(const int16_t *src_diff, ptrdiff_t src_stride,
  ...
  store_s16q_to_tran_lp(coeff + 0, a0);
  store_s16q_to_tran_lp(coeff + 8, a1);
  store_s16q_to_tran_lp(coeff + 16, a2);
  store_s16q_to_tran_lp(coeff + 24, a3);
  store_s16q_to_tran_lp(coeff + 32, a4);
  store_s16q_to_tran_lp(coeff + 40, a5);
  store_s16q_to_tran_lp(coeff + 48, a6);
  store_s16q_to_tran_lp(coeff + 56, a7);
}
...
void aom_hadamard_lp_16x16_neon(const int16_t *src_diff, ptrdiff_t src_stride,
  ...
  for (int i = 0; i < 64; i += 8) {
    const int16x8_t a0 = load_tran_lp_to_s16q(coeff + 0);
    const int16x8_t a1 = load_tran_lp_to_s16q(coeff + 64);
    const int16x8_t a2 = load_tran_lp_to_s16q(coeff + 128);
    const int16x8_t a3 = load_tran_lp_to_s16q(coeff + 192);
    ...
    store_s16q_to_tran_lp(coeff + 0, c0);
    store_s16q_to_tran_lp(coeff + 64, c1);
    store_s16q_to_tran_lp(coeff + 128, c2);
    store_s16q_to_tran_lp(coeff + 192, c3);
