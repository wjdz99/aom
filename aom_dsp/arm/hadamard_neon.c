...
void aom_hadamard_16x16_neon(const int16_t *src_diff, ptrdiff_t src_stride,
  ...
  for (int i = 0; i < 64; i += 8) {
    const int16x8_t a0 = load_tran_low_to_s16q(temp_coeff + 0);
    const int16x8_t a1 = load_tran_low_to_s16q(temp_coeff + 64);
    const int16x8_t a2 = load_tran_low_to_s16q(temp_coeff + 128);
    const int16x8_t a3 = load_tran_low_to_s16q(temp_coeff + 192);
