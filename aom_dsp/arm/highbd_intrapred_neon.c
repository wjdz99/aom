...
static INLINE void smooth_4xh_neon(uint16_t *dst, ptrdiff_t stride,
                                   const uint16_t *const top_row,
                                   const uint16_t *const left_column,
                                   const int height) {
...
static INLINE void calculate_pred8(uint16_t *dst,
...
static void smooth_8xh_neon(uint16_t *dst, ptrdiff_t stride,
                            const uint16_t *const top_row,
                            const uint16_t *const left_column,
                            const int height) {
  ...
  for (int y = 0; y < height; ++y) {
    ...
    calculate_pred8(dst, weighted_corners_low, weighted_corners_high, top_vals,
                    weights_x, left_column[y], weights_y[y]);
    dst += stride;
  }
}

#define HIGHBD_SMOOTH_NXM(N, M)                                 \
  void aom_highbd_smooth_predictor_##N##x##M##_neon(            \
      uint16_t *dst, ptrdiff_t y_stride, const uint16_t *above, \
      const uint16_t *left, int bd) {                           \
    (void)bd;                                                   \
    smooth_##N##xh_neon(dst, y_stride, above, left, M);         \
  }
  ...
  static void smooth_##N##xh_neon(                                             \
    ...
    for (int y = 0; y < height; ++y) {                                         \
      ...
      for (int i = 0; i<N>> 3; ++i) {                                          \
        ...
        calculate_pred8(dst_x, weighted_corners_low, weighted_corners_high,    \
...
#define HIGHBD_SMOOTH_NXM_WIDE(N, M)                            \
  void aom_highbd_smooth_predictor_##N##x##M##_neon(            \
      uint16_t *dst, ptrdiff_t y_stride, const uint16_t *above, \
      const uint16_t *left, int bd) {                           \
    (void)bd;                                                   \
