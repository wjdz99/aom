      ...
      do {
        ...
        if (base + c >= max_base_x) {
          ...
          if (base + c + 7 >= max_base_x) {
            ...
            a0 = vreinterpretq_u16_u8(vqtbl1q_u8(
                vreinterpretq_u8_u16(vld1q_u16(above + (max_base_x - 7))),
                vld1q_u8(&kLoadMaxShuffles[16 * shuffle_idx])));
          } else {
            a0 = vld1q_u16(above + base + c);
          }
          if (base + c + 8 >= max_base_x) {
            int shuffle_idx = max_base_x - base - c - 1;
            a1 = vreinterpretq_u16_u8(vqtbl1q_u8(
                vreinterpretq_u8_u16(vld1q_u16(above + (max_base_x - 7))),
                vld1q_u8(&kLoadMaxShuffles[16 * shuffle_idx])));
          } else {
...
static INLINE uint16x8x2_t z3_load_left_neon(const uint16_t *left0, int ofs,
  ...
  if (ofs + 7 >= max_ofs) {
    ...
    r0 = vreinterpretq_u16_u8(
        vqtbl1q_u8(vreinterpretq_u8_u16(vld1q_u16(left0 + (max_ofs - 7))),
                   vld1q_u8(&kLoadMaxShuffles[16 * shuffle_idx])));
  } else {
