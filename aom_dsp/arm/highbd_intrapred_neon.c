...
static void av1_highbd_dr_prediction_z1_upsample0_neon(uint16_t *dst,
...
static void av1_highbd_dr_prediction_z1_upsample1_neon(uint16_t *dst,
                                                       ptrdiff_t stride, int bw,
                                                       int bh,
                                                       const uint16_t *above,
                                                       int dx) {
...
void av1_highbd_dr_prediction_z1_neon(uint16_t *dst, ptrdiff_t stride, int bw,
  ...
  if (upsample_above) {
    av1_highbd_dr_prediction_z1_upsample1_neon(dst, stride, bw, bh, above, dx);
  } else {
    av1_highbd_dr_prediction_z1_upsample0_neon(dst, stride, bw, bh, above, dx);
  }
}
...
// Y0, Y1, Y2, Y3
// clang-format off
const uint8_t z2_merge_shuffles_u16x4[5][8] = {
...
// clang-format off
const uint8_t z2_merge_shuffles_u16x8[9][16] = {
...
#if AOM_ARCH_AARCH64
// clang-format off
const uint16_t z2_y_iter_masks_u16x4[5][4] = {
  {      0U,      0U,      0U,      0U },
  { 0xffffU,      0U,      0U,      0U },
...
// clang-format off
const uint16_t z2_y_iter_masks_u16x8[9][8] = {
...
static AOM_FORCE_INLINE uint16x4_t highbd_dr_prediction_z2_tbl_left_x4_from_x8(
  ...
  uint8x8_t left_indices = vreinterpret_u8_s16(indices - vdup_n_s16(base));
...
static AOM_FORCE_INLINE uint16x8_t highbd_dr_prediction_z2_tbl_left_x8_from_x8(
  ...
  // `base`-based indices and then adjust from uint16x4 indices to uint8x8
  // indices so we can use a tbl instruction (which only operates on bytes).
  uint8x16_t left_indices = vreinterpretq_u8_s16(indices - vdupq_n_s16(base));
...
static AOM_FORCE_INLINE uint16x4_t highbd_dr_prediction_z2_apply_shift_x4(
  ...
  const uint32x4_t res =
      vmull_u16(a0, vdup_n_u16(32) - vreinterpret_u16_s16(shift)) +
      vmull_u16(a1, vreinterpret_u16_s16(shift));
...
static AOM_FORCE_INLINE uint16x4_t highbd_dr_prediction_z2_step_x4(
  ...
  const int16x4_t iota = { 0, 1, 2, 3 };

  const int x0 = (c << 6) - (r + 1) * dx;
  const int y0 = (r << 6) - (c + 1) * dy;

  const int16x4_t x0123 = vdup_n_s16(x0) + (iota << 6);
  const int16x4_t y0123 = vdup_n_s16(y0) - vmul_n_s16(iota, dy);
  const int16x4_t shift_x0123 = vshr_n_s16(x0123 & 0x3F, 1);
  const int16x4_t shift_y0123 = vshr_n_s16(y0123 & 0x3F, 1);
  const int16x4_t base_y0123 = y0123 >> 6;

  const int base_shift = ((((r + 1) * dx) - 1) >> 6) - c;

  // Based on the balue of `base_shift` there are three possible cases to
  // compute the result:
  ...
  if (base_shift <= 0) {
    ...
    const uint16x4x2_t l01 =
        highbd_dr_prediction_z2_gather_left_x4(left, base_y0123 + 1, 4);
...
static AOM_FORCE_INLINE uint16x8_t highbd_dr_prediction_z2_step_x8(
    ...
    const uint16_t *left, int dx, int dy, int r, int c) {
  const int16x8_t iota = { 0, 1, 2, 3, 4, 5, 6, 7 };

  const int x0 = (c << 6) - (r + 1) * dx;
  const int y0 = (r << 6) - (c + 1) * dy;

  const int16x8_t x01234567 = vdupq_n_s16(x0) + (iota << 6);
  const int16x8_t y01234567 = vdupq_n_s16(y0) - vmulq_n_s16(iota, dy);
  const int16x8_t shift_x01234567 = vshrq_n_s16(x01234567 & 0x3F, 1);
  const int16x8_t shift_y01234567 = vshrq_n_s16(y01234567 & 0x3F, 1);
  const int16x8_t base_y01234567 = y01234567 >> 6;
  ...
  if (base_shift <= 0) {
    ...
    const uint16x8x2_t l01 = highbd_dr_prediction_z2_gather_left_x8(
        left, base_y01234567 + 1, base_shift);
    ...
    const uint16x8x2_t l01 =
        highbd_dr_prediction_z2_gather_left_x8(left, base_y01234567 + 1, 8);
...
static void highbd_dr_prediction_z2_4x4_neon(uint16_t *dst, ptrdiff_t stride,
  ...
  for (int r = 0; r < 4; ++r) {
    const int base_shift =
        (((min_base_x << frac_bits_x) + (r + 1) * dx + 63) >> 6);
    const int16x4_t iota = { 0, 1, 2, 3 };
    const int16x4_t x0123 = (iota << 6) - vdup_n_s16((r + 1) * dx);
    const int16x4_t base_x0123 = x0123 >> frac_bits_x;
    if (base_shift <= 0) {
      ...
      uint16x4_t a0, a1;
      if (upsample_above) {
        const uint16x4x2_t a01 = vld2_u16(above + base_x0123[0]);
        a0 = a01.val[0];
        a1 = a01.val[1];
      } else {
        a0 = vld1_u16(above + base_x0123[0]);
        a1 = vld1_u16(above + base_x0123[0] + 1);
      }
      vst1_u16(dst,
      ...
      const int16x4_t y0123 = vdup_n_s16(r << 6) - vmul_n_s16(iota + 1, dy);
      const int16x4_t base_y0123 = y0123 >> frac_bits_y;
      ...
      uint16x4_t a0, a1;
      if (upsample_above) {
        const uint16x4x2_t a01 =
            vld2_u16(above + (base_x0123[0] % 2 == 0 ? -2 : -1));
        a0 = a01.val[0];
        a1 = a01.val[1];
      } else {
        a0 = vld1_u16(above - 1);
...
static void highbd_dr_prediction_z2_4x8_neon(uint16_t *dst, ptrdiff_t stride,
  ...
  for (int r = 0; r < 8; ++r) {
    ...
    const int32x4_t x0123 = (iota << 6) - (r + 1) * dx;
    const int32x4_t base_x0123 = x0123 >> frac_bits_x;
    if (base_shift <= 0) {
      const int16x4_t shift_x0123 =
          vshrn_n_s32((x0123 * (1 << upsample_above)) & 0x3F, 1);
      uint16x4_t a0, a1;
      if (upsample_above) {
        const uint16x4x2_t a01 = vld2_u16(above + base_x0123[0]);
        a0 = a01.val[0];
      ...
      // Calculate Y component from `left`.
      const int y_iters = base_shift;
      const int16x4_t iota1234 = { 1, 2, 3, 4 };
      const int16x4_t y0123 = vdup_n_s16(r << 6) - vmul_n_s16(iota1234, dy);
      const int16x4_t base_y0123 = y0123 >> frac_bits_y;
      const int16x4_t shift_y0123 = vshr_n_s16(
          vand_s16(vmul_n_s16(y0123, 1 << upsample_left), vdup_n_s16(0x3F)), 1);
...
static void highbd_dr_prediction_z2_8x4_neon(uint16_t *dst, ptrdiff_t stride,
  ...
  for (int r = 0; r < 4; ++r) {
    const int base_shift =
        (((min_base_x << frac_bits_x) + (r + 1) * dx + 63) >> 6);
    const int32x4_t iota0123 = { 0, 1, 2, 3 };
    const int32x4_t iota4567 = { 4, 5, 6, 7 };
    const int32x4_t x0123 = (iota0123 << 6) - (r + 1) * dx;
    const int32x4_t x4567 = (iota4567 << 6) - (r + 1) * dx;
    const int32x4_t base_x0123 = x0123 >> frac_bits_x;
    if (base_shift <= 0) {
      const int16x4_t shift_x0123 =
          vshrn_n_s32((x0123 * (1 << upsample_above)) & 0x3F, 1);
      const int16x4_t shift_x4567 =
          vshrn_n_s32((x4567 * (1 << upsample_above)) & 0x3F, 1);
      uint16x8_t a0, a1;
      if (upsample_above) {
        const uint16x8x2_t a01 = vld2q_u16(above + base_x0123[0]);
        a0 = a01.val[0];
        a1 = a01.val[1];
      } else {
        a0 = vld1q_u16(above + base_x0123[0]);
        a1 = vld1q_u16(above + base_x0123[0] + 1);
      }
      vst1q_u16(dst, highbd_dr_prediction_z2_apply_shift_x8(
                         a0, a1, vcombine_s16(shift_x0123, shift_x4567)));
    } else if (base_shift < 8) {
      // Calculate Y component from `left`.
      const int y_iters = base_shift;
      const int16x8_t iota12345678 = { 1, 2, 3, 4, 5, 6, 7, 8 };
      const int16x8_t y01234567 =
          vdupq_n_s16(r << 6) - vmulq_n_s16(iota12345678, dy);
      const int16x8_t base_y01234567 = y01234567 >> frac_bits_y;
      ...
      if (upsample_above) {
        const uint16x8x2_t a01 =
            vld2q_u16(above + (base_x0123[0] % 2 == 0 ? -2 : -1));
        a0 = a01.val[0];
        a1 = a01.val[1];
      } else {
        a0 = vld1q_u16(above - 1);
      ...
      const uint16x8_t out_x = highbd_dr_prediction_z2_apply_shift_x8(
          a0, a1, vcombine_s16(shift_x0123, shift_x4567));
...
static highbd_dr_prediction_z2_ptr dr_predictor_z2_arr_neon[5][5] = {
  { &highbd_dr_prediction_z2_4x4_neon, &highbd_dr_prediction_z2_4x8_neon,
    &highbd_dr_prediction_z2_4x16_neon, NULL, NULL },
  { &highbd_dr_prediction_z2_8x4_neon, &highbd_dr_prediction_z2_8x8_neon,
    &highbd_dr_prediction_z2_8x16_neon, &highbd_dr_prediction_z2_8x32_neon,
    NULL },
  { &highbd_dr_prediction_z2_16x4_neon, &highbd_dr_prediction_z2_16x8_neon,
    &highbd_dr_prediction_z2_16x16_neon, &highbd_dr_prediction_z2_16x32_neon,
    &highbd_dr_prediction_z2_16x64_neon },
  { NULL, &highbd_dr_prediction_z2_32x8_neon,
    &highbd_dr_prediction_z2_32x16_neon, &highbd_dr_prediction_z2_32x32_neon,
    &highbd_dr_prediction_z2_32x64_neon },
  { NULL, NULL, &highbd_dr_prediction_z2_64x16_neon,
...
void av1_highbd_dr_prediction_z2_neon(uint16_t *dst, ptrdiff_t stride, int bw,
                                      ...
                                      int upsample_left, int dx, int dy,
                                      int bd) {
  int bw_idx = bw == 64 ? 4 : bw == 32 ? 3 : bw == 16 ? 2 : bw == 8 ? 1 : 0;
  int bh_idx = bh == 64 ? 4 : bh == 32 ? 3 : bh == 16 ? 2 : bh == 8 ? 1 : 0;
  highbd_dr_prediction_z2_ptr f = dr_predictor_z2_arr_neon[bw_idx][bh_idx];
...
static void av1_highbd_dr_prediction_z3_upsample0_neon(uint16_t *dst,
                                                       ptrdiff_t stride, int bw,
                                                       int bh,
                                                       const uint16_t *left,
                                                       int dy) {
  ...
  const uint16x4_t iota1x4 = { 0, 1, 2, 3 };
  const uint16x8_t iota1x8 = { 0, 1, 2, 3, 4, 5, 6, 7 };
  ...
  if (bh == 4) {
    ...
    do {
      ...
      uint16x4_t out[4];
      HIGHBD_DR_PREDICTOR_Z3_STEP_X4(&out[0], iota1x4, base0, l00, l01, shifts0,
                                     shifts1, 0, 6);
      HIGHBD_DR_PREDICTOR_Z3_STEP_X4(&out[1], iota1x4, base1, l10, l11, shifts0,
                                     shifts1, 1, 6);
      HIGHBD_DR_PREDICTOR_Z3_STEP_X4(&out[2], iota1x4, base2, l20, l21, shifts0,
                                     shifts1, 2, 6);
      HIGHBD_DR_PREDICTOR_Z3_STEP_X4(&out[3], iota1x4, base3, l30, l31, shifts0,
                                     shifts1, 3, 6);
...
static void av1_highbd_dr_prediction_z3_upsample1_neon(uint16_t *dst,
                                                       ptrdiff_t stride, int bw,
                                                       int bh,
                                                       const uint16_t *left,
                                                       int dy) {
  ...
  const uint16x4_t iota1x4 = { 0, 1, 2, 3 };
  const uint16x4_t iota2x4 = { 0, 2, 4, 6 };
  const uint16x8_t iota2x8 = { 0, 2, 4, 6, 8, 10, 12, 14 };
...
void av1_highbd_dr_prediction_z3_neon(uint16_t *dst, ptrdiff_t stride, int bw,
  ...
  if (upsample_left) {
    av1_highbd_dr_prediction_z3_upsample1_neon(dst, stride, bw, bh, left, dy);
  } else {
    av1_highbd_dr_prediction_z3_upsample0_neon(dst, stride, bw, bh, left, dy);
  }
}
