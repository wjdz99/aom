...
static INLINE void highbd_obmc_variance_xlarge_neon(
  ...
  do {
    ...
    do {
      ...
      do {
        int16x8_t pre0 = vld1q_u16(pre_ptr + i);
        highbd_obmc_variance_8x1_s16_neon(pre0, wsrc, mask, &sse_u32[0],
                                          &sum_s32);

        int16x8_t pre1 = vld1q_u16(pre_ptr + i + 8);
...
static INLINE void highbd_obmc_variance_neon_8xh(const uint8_t *pre8,
  ...
  uint32x4_t sse_u32 = vdupq_n_s32(0);
  int32x4_t sum_s32 = vdupq_n_s32(0);

  do {
    uint16x8_t pre_s16 = vld1q_u16(pre);

    highbd_obmc_variance_8x1_s16_neon(pre_s16, wsrc, mask, &sse_u32, &sum_s32);
...
static INLINE void highbd_obmc_variance_neon_4xh(const uint8_t *pre8,
  ...
  do {
    uint16x8_t pre_s16 = load_unaligned_u16_4x2(pre, pre_stride);
