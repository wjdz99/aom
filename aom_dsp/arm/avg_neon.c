...
void aom_int_pro_row_neon(int16_t *hbuf, const uint8_t *ref,
  ...
  for (int wd = 0; wd < width; wd += 16) {
    ...
    vec0 = vshrq_n_u16(vec0, norm_factor);
    vec1 = vshrq_n_u16(vec1, norm_factor);

    vst1q_s16(hbuf + wd, vreinterpretq_s16_u16(vec0));
    vst1q_s16(hbuf + wd + 8, vreinterpretq_s16_u16(vec1));
  }
}
