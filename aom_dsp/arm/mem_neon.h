...
static INLINE void load_unaligned_u8_2x2(const uint8_t *buf, int stride,
  ...
  buf += stride;
  a_u16 = vdup_n_u16(0);
  a_u16 = vset_lane_u16(a, a_u16, 0);
  memcpy(&a, buf, 2);
  a_u16 = vset_lane_u16(a, a_u16, 1);
  *tu0 = vreinterpret_u8_u16(a_u16);
}

static INLINE void load_unaligned_u8_4x1(const uint8_t *buf, uint8x8_t *tu0) {
  uint32_t a;
  uint32x2_t a_u32;
...
static INLINE void load_unaligned_u8_4x4(const uint8_t *buf, int stride,
  ...
  load_unaligned_u8_4x2(buf, stride, tu0);
  buf += 2 * stride;
  load_unaligned_u8_4x2(buf, stride, tu1);
}
