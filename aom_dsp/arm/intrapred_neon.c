...
static void dr_prediction_z2_Nx8_neon(int N, uint8_t *dst, ptrdiff_t stride,
  ...
  const uint8x16x3_t left_vals = (uint8x16x3_t){ left_m2, left_14, left_30 };
#endif  // defined(__aarch64__)
...
static void dr_prediction_z2_HxW_neon(int H, int W, uint8_t *dst,
  ...
  const uint8x16x4_t left_vals0 =
      (uint8x16x4_t){ left_m1, left_15, left_31, left_47 };
  const uint8x16x4_t left_vals1 =
      (uint8x16x4_t){ left_0, left_16, left_32, left_48 };
#endif  // defined(__aarch64__)
