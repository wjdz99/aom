...
static INLINE void paeth_4or8_x_h_neon(uint8_t *const dest, ptrdiff_t stride,
                                       const void *const top_row,
                                       const void *const left_column, int width,
                                       int height) {
  uint8_t *dest_u8 = dest;
  const uint8_t *const top_row_u8 = (const uint8_t *)top_row;
  const uint8_t *const left_col_u8 = (const uint8_t *)left_column;
  const uint8x8_t top_left = vdup_n_u8(top_row_u8[-1]);
  const uint16x8_t top_left_x2 = vdupq_n_u16(top_row_u8[-1] + top_row_u8[-1]);
  uint8x8_t top;
  if (width == 4) {
    load_u8_4x1(top_row_u8, &top, 0);
  } else {  // width == 8
  ...
  for (int y = 0; y < height; ++y) {
    ...
    if (width == 4) {
      store_unaligned_u8_4x1(dest_u8, result, 0);
    } else {  // width == 8
...
static INLINE void paeth16_plus_x_h_neon(uint8_t *const dest, ptrdiff_t stride,
                                         const void *const top_row,
