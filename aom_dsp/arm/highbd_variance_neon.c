  ...
  if (width > 8) {
    do {
      ...
      do {
        ...
        cp0 = vrshlq_u32(cp0, shift);
        cp1 = vrshlq_u32(cp1, shift);
        uint16x8_t avg = vcombine_u16(vmovn_u32(cp0), vmovn_u32(cp1));
    ...
    do {
      ...
      cp = vrshlq_u32(cp, shift);
      uint16x4_t avg = vmovn_u32(cp);
