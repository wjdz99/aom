...
uint64_t aom_sum_squares_2d_i16_4x4_neon(const int16_t *src, int stride) {
  ...
  uint32x4_t v_sum_d =
      vaddq_u32(v_sum_0123_d, vreinterpretq_u32_u64(vshrq_n_u64(
                                  vreinterpretq_u64_u32(v_sum_0123_d), 32)));
  v_sum_d = vaddq_u32(v_sum_d, vextq_u32(v_sum_d, v_sum_d, 2));
  return (uint64_t)vgetq_lane_u32(v_sum_d, 0);
}
...
uint64_t aom_sum_squares_2d_i16_4xn_neon(const int16_t *src, int stride,
  ...
  const uint64x2_t v_zext_mask_q = vdupq_n_u64(0xffffffff);
  uint64x2_t v_acc_64 =
      vandq_u64(vreinterpretq_u64_u32(v_acc_q), v_zext_mask_q);
  v_acc_64 = vsraq_n_u64(v_acc_64, vreinterpretq_u64_u32(v_acc_q), 32);
...
uint64_t aom_sum_squares_2d_i16_nxn_neon(const int16_t *src, int stride,
  ...
  do {
    ...
    v_acc_q = vaddq_u64(v_acc_q,
                        vandq_u64(vreinterpretq_u64_s32(v_sum), v_zext_mask_q));
    v_acc_q = vsraq_n_u64(v_acc_q, vreinterpretq_u64_s32(v_sum), 32);
    src += 4 * stride;
    r += 4;
  } while (r < height);
