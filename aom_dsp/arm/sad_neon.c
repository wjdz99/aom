...
unsigned int aom_sad128x128_neon(const uint8_t *src, int src_stride,
  ...
  for (int i = 0; i < 128; ++i) {
    ...
    vec_accum_hi = vabal_u8(vec_accum_hi, vget_high_u8(vec_src_48),
                            vget_high_u8(vec_ref_48));
    tmp_lo = vaddl_u16(vget_low_u16(vec_accum_lo), vget_low_u16(vec_accum_hi));
    tmp_hi =
        vaddl_u16(vget_high_u16(vec_accum_lo), vget_high_u16(vec_accum_hi));
    vec_accum_32lo = vaddq_u32(tmp_lo, vec_accum_32lo);
    vec_accum_32hi = vaddq_u32(tmp_hi, vec_accum_32hi);
  ...
  return vaddvq_u32(vec_accum_32lo) + vaddvq_u32(vec_accum_32hi);
}
