...
/*static INLINE unsigned int horizontal_add_16x8_new(const uint16x8_t vec_16x8) {
  const uint32x4_t a = vpaddlq_u16(vec_16x8);
  const uint64x2_t b = vpaddlq_u32(a);
  return (uint32_t)vadd_u64(vget_low_u64(b),vget_high_u64(b));
}*/
...
static INLINE unsigned int sad4xh_neon(const uint8_t *src_ptr, int src_stride,
  ...
  for (int y = 0; y < h/2; y++) {
  /*  for (int x = 0; x < 4; x++) {
      sum += abs(src_ptr[x] - ref_ptr[x]);
    }*/
    memcpy(&src4,src_ptr,4);
    memcpy(&ref4,ref_ptr,4);
    src_ptr += src_stride;
    ref_ptr += ref_stride;
    uint32x2_t q0 = vset_lane_u32(src4,q0,0);
    uint32x2_t q1 = vset_lane_u32(ref4,q1,0);
...
#define FSADS128_H(h)                                                           \
unsigned int aom_sad_skip_128x##h##_neon(const uint8_t *src_ptr, int src_stride,\
                                       const uint8_t *ref_ptr, int ref_stride) {\
  const uint32_t sum =                                                          \
      sad128xh_neon(src_ptr, 2 * src_stride, ref_ptr, 2 * ref_stride, h/2);     \
  return 2 * sum;                                                               \
}
FSADS128_H(128);
FSADS128_H(64);
