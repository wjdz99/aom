...
static void RGBPixelToYUV(uint8_t r, uint8_t g, uint8_t b, uint8_t *y,
                          uint8_t *u, uint8_t *v) {
  *y = ClampRound8(RGBtoYUVMatrixAdd[0] + RGBtoYUVMatrix[0] * (float)r +
                   RGBtoYUVMatrix[1] * (float)g + RGBtoYUVMatrix[2] * (float)b);
  *u = ClampRound8(RGBtoYUVMatrixAdd[1] + RGBtoYUVMatrix[3] * (float)r +
                   RGBtoYUVMatrix[4] * (float)g + RGBtoYUVMatrix[5] * (float)b);
  *v = ClampRound8(RGBtoYUVMatrixAdd[2] + RGBtoYUVMatrix[6] * (float)r +
...
static void YUVPixelToRGB(uint8_t yv, uint8_t uv, uint8_t vv, uint8_t *r,
  ...
  *r = ClampRound8(YUVtoRGBMatrix[0] * y + YUVtoRGBMatrix[1] * u +
                   YUVtoRGBMatrix[2] * v);
  *g = ClampRound8(YUVtoRGBMatrix[3] * y + YUVtoRGBMatrix[4] * u +
                   YUVtoRGBMatrix[5] * v);
  *b = ClampRound8(YUVtoRGBMatrix[6] * y + YUVtoRGBMatrix[7] * u +
                   YUVtoRGBMatrix[8] * v);
}
...
static void RGBToXYB(const uint8_t *rgb, int stride_rgb, uint8_t *x,
                     int x_stride, uint8_t *y, int y_stride, uint8_t *b,
...
static void XYBToRGB(const uint8_t *x, int x_stride, const uint8_t *y,
                     int y_stride, const uint8_t *b, int b_stride, uint8_t *rgb,
...
void aom_yuv_to_xyb(const YV12_BUFFER_CONFIG *yuv, YV12_BUFFER_CONFIG *xyb) {
  assert(yuv->bit_depth == 8);
  assert(xyb->bit_depth == 8);
