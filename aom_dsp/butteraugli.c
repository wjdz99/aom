...
const float RGBtoYUVMatrix[9] = {
  .183f / 255.f, .614f / 255.f, .062f / 255.f,  -.101f / 255.f, -.339f / 255.f,
  .439f / 255.f, .439f / 255.f, -.399f / 255.f, -.040f / 255.f
};
const float RGBtoYUVMatrixAdd[3] = { 16.f / 255.f, 128.f / 255.f,
                                     128.f / 255.f };

const float YUVtoRGBMatrix[9] = {
  1.164f * 255.f, 0.000f * 255.f,  1.793f * 255.f,
  1.164f * 255.f, -0.213f * 255.f, -0.533f * 255.f,
  1.164f * 255.f, 2.112f * 255.f,  0.000f * 255.f,
};
...
static void RGBPixelToYUV(uint8_t r, uint8_t g, uint8_t b, uint8_t *y,
                          uint8_t *u, uint8_t *v) {
  *y = ClampRound8(
      255.0f * (RGBtoYUVMatrixAdd[0] + RGBtoYUVMatrix[0] * (float)r +
                RGBtoYUVMatrix[1] * (float)g + RGBtoYUVMatrix[2] * (float)b));
  *u = ClampRound8(
      255.0f * (RGBtoYUVMatrixAdd[1] + RGBtoYUVMatrix[3] * (float)r +
                RGBtoYUVMatrix[4] * (float)g + RGBtoYUVMatrix[5] * (float)b));
  *v = ClampRound8(
      255.0f * (RGBtoYUVMatrixAdd[2] + RGBtoYUVMatrix[6] * (float)r +
                RGBtoYUVMatrix[7] * (float)g + RGBtoYUVMatrix[8] * (float)b));
}

static void YUVPixelToRGB(uint8_t yv, uint8_t uv, uint8_t vv, uint8_t *r,
                          uint8_t *g, uint8_t *b) {
  const float maxv = 255.f;
  const float y = (float)yv - RGBtoYUVMatrixAdd[0] * maxv;
  const float u = (float)uv - RGBtoYUVMatrixAdd[1] * maxv;
  const float v = (float)vv - RGBtoYUVMatrixAdd[2] * maxv;
  *r = ClampRound8(
...
static void RGBtoYUV(const uint8_t *rgb, uint8_t *y, int y_stride, uint8_t *u,
...
static void YUVtoRGB(const uint8_t *y, int y_stride, const uint8_t *u,
                     int u_stride, const uint8_t *v, int v_stride, uint8_t *rgb,
...
static void SrgbToJxlXyb(const uint8_t srgb[3], float xyb[3]) {
  float srgb01[3] = { (float)srgb[0] / 255.0f, (float)srgb[1] / 255.0f,
                      (float)srgb[2] / 255.0f };
...
void JxlXybToSrgb(const float xyb[3], uint8_t srgb[3]) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    srgb[i] = ClampRound8(srgb01 * 255.0f);
  }
}

void RGBToXYB(const uint8_t *rgb, int stride_rgb, uint8_t *y, int y_stride,
              uint8_t *b, int b_stride, uint8_t *x, int x_stride, int width,
              int height) {
  for (int j = 0; j < height; ++j) {
    for (int i = 0; i < width; ++i) {
      const uint8_t *srgb = &rgb[i * 3 + (j * stride_rgb)];
      float xyb[3];
...
void XYBToRGB(const uint8_t *y, int y_stride, const uint8_t *b, int b_stride,
              const uint8_t *x, int x_stride, uint8_t *rgb, int stride_argb,
              int width, int height) {
  for (int j = 0; j < height; ++j) {
    for (int i = 0; i < width; ++i) {
      ...
      uint8_t *srgb = &rgb[i * 3 + (j * stride_argb)];
...
void aom_yuv_to_xyb(const YV12_BUFFER_CONFIG *yuv,
  ...
  uint8_t *rgb = (uint8_t *)aom_malloc(buffer_size);

  YUVtoRGB(yuv->y_buffer, yuv->y_stride, yuv->u_buffer, yuv->uv_stride,
           yuv->v_buffer, yuv->uv_stride, rgb, width, height);

  RGBToXYB(rgb, stride_rgb, xyb->y_buffer, xyb->y_stride, xyb->u_buffer,
           xyb->uv_stride, xyb->v_buffer, xyb->uv_stride, width, height);
...
void aom_xyb_to_yuv(const YV12_BUFFER_CONFIG *xyb,
  ...
  XYBToRGB(xyb->y_buffer, xyb->y_stride, xyb->u_buffer, xyb->uv_stride,
           xyb->v_buffer, xyb->uv_stride, rgb, stride_rgb, width, height);
