...
static AOM_INLINE void write_enc_data_to_out_buf(unsigned char* out, uint32_t offs, 
    uint64_t output, uint64_t carry, uint32_t* enc_offs, uint8_t num_bytes_ready) {
#if defined(__GNUC__)
    // GNU builtin
    const uint64_t reg =
