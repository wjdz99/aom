...
static void convolve_add_src_horiz_hip(const uint8_t *src, ptrdiff_t src_stride,
  ...
  for (y = 0; y < h; ++y) {
    ...
    for (x = 0; x < w; ++x) {
      ...
      for (k = 0; k < SUBPEL_TAPS; ++k) sum += src_x[k] * x_filter[k];
      dst[x] =
          (int16_t)clamp(ROUND_POWER_OF_TWO(sum, FILTER_BITS - EXTRAPREC_BITS),
                         -EXTRAPREC_CLAMP_LIMIT, EXTRAPREC_CLAMP_LIMIT - 1);
      x_q4 += x_step_q4;
    }
    src += src_stride;
    dst += dst_stride;
...
static void convolve_add_src_vert_hip(const int16_t *src, ptrdiff_t src_stride,
                                      uint8_t *dst, ptrdiff_t dst_stride,
                                      const InterpKernel *y_filters, int y0_q4,
                                      int y_step_q4, int w, int h) {
  int x, y;
...
static void convolve_add_src_hip(const uint8_t *src, ptrdiff_t src_stride,
                                 ...
                                 const InterpKernel *const y_filters, int y0_q4,
                                 int y_step_q4, int w, int h) {
  int16_t temp[MAX_EXT_SIZE * MAX_SB_SIZE];
  int intermediate_height =
      (((h - 1) * y_step_q4 + y0_q4) >> SUBPEL_BITS) + SUBPEL_TAPS;
...
void aom_convolve8_add_src_vert_hip_c(const int16_t *src, ptrdiff_t src_stride,
                                      uint8_t *dst, ptrdiff_t dst_stride,
                                      const int16_t *filter_x, int x_step_q4,
                                      const int16_t *filter_y, int y_step_q4,
                                      int w, int h) {
...
static void highbd_convolve_add_src_horiz_hip(
    const uint8_t *src8, ptrdiff_t src_stride, int16_t *dst8,
...
static void highbd_convolve_add_src_vert_hip(
    const int16_t *src8, ptrdiff_t src_stride, uint8_t *dst8,
    ptrdiff_t dst_stride, const InterpKernel *y_filters, int y0_q4,
    int y_step_q4, int w, int h, int bd) {
  int x, y;
  int16_t *src = (int16_t *)CONVERT_TO_SHORTPTR(src8);
  uint16_t *dst = CONVERT_TO_SHORTPTR(dst8);
  src -= src_stride * (SUBPEL_TAPS / 2 - 1);
  for (x = 0; x < w; ++x) {
    int y_q4 = y0_q4;
...
void aom_highbd_convolve8_add_src_vert_hip_c(
    const int16_t *src, ptrdiff_t src_stride, uint8_t *dst,
    ptrdiff_t dst_stride, const int16_t *filter_x, int x_step_q4,
    const int16_t *filter_y, int y_step_q4, int w, int h, int bd) {
  const InterpKernel *const filters_y = get_filter_base(filter_y);
  const int y0_q4 = get_filter_offset(filter_y, filters_y);
