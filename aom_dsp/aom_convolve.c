...
void aom_convolve8_c(const uint8_t *src, ptrdiff_t src_stride, uint8_t *dst,
  ...
  aom_convolve8_horiz_c(src - src_stride * (SUBPEL_TAPS / 2 - 1), src_stride,
                        temp, 64, filter[x0_q4], x_step_q4, NULL, y_step_q4, w,
                        intermediate_height);
  aom_convolve8_vert_c(temp + 64 * (SUBPEL_TAPS / 2 - 1), 64, dst, dst_stride,
                       NULL, x_step_q4, filter[y0_q4], y_step_q4, w, h);
}
