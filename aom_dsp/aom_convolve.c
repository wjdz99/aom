...
static void convolve_horiz_scale_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  for (y = 0; y < h; ++y) {
    int x_q4 = x0_q4;
    for (x = 0; x < w; ++x) {
      const uint8_t *const src_x = &src[x_q4 >> SCALE_SUBPEL_BITS];  // q8
      const int16_t *const x_filter =
          x_filters[x_q4 & SCALE_SUBPEL_MASK];  // SUBPEL_MASK
      int k, sum = 0;
      for (k = 0; k < SUBPEL_TAPS; ++k) sum += src_x[k] * x_filter[k];
      dst[x] = clip_pixel(ROUND_POWER_OF_TWO(sum, FILTER_BITS));
      x_q4 += x_step_q4;
...
static void convolve_vert_scale_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  for (x = 0; x < w; ++x) {
    int y_q4 = y0_q4;
    for (y = 0; y < h; ++y) {
      const unsigned char *src_y = &src[(y_q4 >> SCALE_SUBPEL_BITS) * src_stride];
      const int16_t *const y_filter =
          y_filters[y_q4 & SCALE_SUBPEL_MASK];  // SUBPEL_MASK
      int k, sum = 0;
      for (k = 0; k < SUBPEL_TAPS; ++k)
        sum += src_y[k * src_stride] * y_filter[k];
      dst[y * dst_stride] = clip_pixel(ROUND_POWER_OF_TWO(sum, FILTER_BITS));
...
static void convolve_avg_vert_scale_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  for (x = 0; x < w; ++x) {
    ...
    for (y = 0; y < h; ++y) {
      const unsigned char *src_y = &src[(y_q4 >> SCALE_SUBPEL_BITS) * src_stride];
      const int16_t *const y_filter = y_filters[y_q4 & SCALE_SUBPEL_MASK];
      int k, sum = 0;
      for (k = 0; k < SUBPEL_TAPS; ++k)
        sum += src_y[k * src_stride] * y_filter[k];
      dst[y * dst_stride] = ROUND_POWER_OF_TWO(
...
static void convolve_scale_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  uint8_t temp[MAX_EXT_SIZE * MAX_SB_SIZE];
  int intermediate_height =
      (((h - 1) * y_step_q4 + y0_q4) >> SCALE_SUBPEL_BITS) + SUBPEL_TAPS;

  assert(w <= MAX_SB_SIZE);
  assert(h <= MAX_SB_SIZE);

  assert(y_step_q4 <= SCALE_SUBPEL_BITS * 2);
  assert(x_step_q4 <= SCALE_SUBPEL_BITS * 2);
...
void aom_convolve8_horiz_scale_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  convolve_horiz_scale_c(src, src_stride, dst, dst_stride, filters_x, x0_q4,
                         x_step_q4, w, h);
}
...
void aom_convolve8_avg_horiz_scale_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  convolve_avg_horiz_scale_c(src, src_stride, dst, dst_stride, filters_x, x0_q4,
                             x_step_q4, w, h);
}
...
void aom_convolve8_vert_scale_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  convolve_vert_scale_c(src, src_stride, dst, dst_stride, filters_y, y0_q4,
                        y_step_q4, w, h);
}
...
void aom_convolve8_avg_vert_scale_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  convolve_avg_vert_scale_c(src, src_stride, dst, dst_stride, filters_y, y0_q4,
                            y_step_q4, w, h);
}
...
void aom_convolve8_scale_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  convolve_scale_c(src, src_stride, dst, dst_stride, filters_x, x0_q4,
...
void aom_convolve8_avg_scale_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  aom_convolve8_scale_c(src, src_stride, temp, MAX_SB_SIZE, filter_x, x_step_q4,
                        filter_y, y_step_q4, w, h);
  aom_convolve_avg_c(temp, MAX_SB_SIZE, dst, dst_stride, NULL, 0, NULL, 0, w,
                     h);
}
...
void aom_scaled_2d_scale_c(const uint8_t *src, ptrdiff_t src_stride,
                           ...
                           const int16_t *filter_y, int y_step_q4, int w,
                           int h) {
  aom_convolve8_scale_c(src, src_stride, dst, dst_stride, filter_x, x_step_q4,
                        filter_y, y_step_q4, w, h);
}
