...
static uint64_t get_variance(const uint8_t *const pixels,
                             ptrdiff_t pixel_increment, int num_pixels) {
  double total = pixels[0];
  int i;
  for (i = 1; i < num_pixels; ++i) {
    const uint8_t this_pixel = pixels[i * pixel_increment];
    total += this_pixel;
  }
  const double mean = total / num_pixels;
  double variance_sum = 0.0;
  for (i = 0; i < num_pixels; ++i) {
    const uint8_t this_pixel = pixels[i * pixel_increment];
    variance_sum += pow(this_pixel - mean, 2);
  }
  return (uint64_t)lround(variance_sum / num_pixels);
}

#define SHARP_RANGE_THRESHOLD 300
#define NORMAL_RANGE_THRESHOLD 4

static InterpFilter pick_implicit_filter(uint64_t variance) {
  if (variance > SHARP_RANGE_THRESHOLD)
    return MULTITAP_SHARP;
  else if (variance > NORMAL_RANGE_THRESHOLD)
...
const InterpKernel *adapt_filter(const InterpKernel *const given_filter_kernel,
                                 ...
                                 ptrdiff_t pixel_increment, int num_pixels) {
  if (given_filter_kernel != NULL) {
    return given_filter_kernel;
  }
  const uint64_t variance = get_variance(pixels, pixel_increment, num_pixels);
  const InterpFilter selected_filter = pick_implicit_filter(variance);
