...
#if __GNUC__ && !__clang__ && !__CC_ARM &&                            \
...
#if __clang__
#if 0
  typedef uint32_t __attribute__((aligned(1))) uint32_unaligned_t;
  *((uint32_unaligned_t *)p) = a;
#else
...
SIMD_INLINE ssd64_internal v64_ssd_u8_init() { return (ssd64_internal)(uint64_t)0; }
...
SIMD_INLINE uint32_t v64_ssd_u8_sum(ssd64_internal s) { return (uint32_t)(uint64_t)s; }
