...
SIMD_INLINE v256 v256_wideshuffle_8(v256 x, v256 y, v256 pattern) {
...
#if defined(__aarch64__)
  uint8x16x4_t p = {{
      vreinterpretq_u8_s64(y.val[0]), vreinterpretq_u8_s64(y.val[1]),
      vreinterpretq_u8_s64(x.val[0]), vreinterpretq_u8_s64(x.val[1]),
  }};
  return v256_from_v128(
  ...
  v256 r2 = v256_from_64(
      (uint64_t)vreinterpret_s64_u8(
          vtbl4_u8(q, vreinterpret_u8_s64(vget_high_s64(pattern.val[1])))),
      (uint64_t)vreinterpret_s64_u8(
          vtbl4_u8(q, vreinterpret_u8_s64(vget_low_s64(pattern.val[1])))),
      (uint64_t)vreinterpret_s64_u8(
          vtbl4_u8(q, vreinterpret_u8_s64(vget_high_s64(pattern.val[0])))),
      (uint64_t)vreinterpret_s64_u8(
          vtbl4_u8(q, vreinterpret_u8_s64(vget_low_s64(pattern.val[0])))));
...
#define v256_shl_n_byte(a, n)                                                  \
  ((n) < 16 ? v256_from_v128(v128_or(v128_shl_n_byte(a.val[1], n),             \
            ...
            : v256_from_v128((n) > 16 ? v128_shl_n_byte(a.val[0], (n)-16)      \
                                      : a.val[0],                              \
                             v128_zero()))
