...
static INLINE void aom_update_raw_counts(const aom_reader *r,
                                         const int is_binary) {
  if (r->accounting != NULL) {
    r->accounting->syms.num_raw_syms++;
    r->accounting->syms.num_raw_binary_syms += !!is_binary;
  }
}
