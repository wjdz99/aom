...
#define ACCT_STR_PARAM , const char *acct_str
...
#define aom_read(r, prob, acct_str) aom_read_(r, prob ACCT_STR_ARG(acct_str))
#define aom_read_bit(r, acct_str) aom_read_bit_(r ACCT_STR_ARG(acct_str))
#define aom_read_tree(r, tree, probs, acct_str) \
  aom_read_tree_(r, tree, probs ACCT_STR_ARG(acct_str))
#define aom_read_literal(r, bits, acct_str) \
  aom_read_literal_(r, bits ACCT_STR_ARG(acct_str))
#define aom_read_tree_bits(r, tree, probs, acct_str) \
  aom_read_tree_bits_(r, tree, probs ACCT_STR_ARG(acct_str))
#define aom_read_symbol(r, cdf, nsymbs, acct_str) \
  aom_read_symbol_(r, cdf, nsymbs ACCT_STR_ARG(acct_str))
...
#if CONFIG_ACCOUNTING
static INLINE void aom_process_accounting(const aom_reader *r,
                                          const char *acct_str) {
  if (r->accounting != NULL) {
    uint32_t tell_frac;
    tell_frac = aom_reader_tell_frac(r);
    aom_accounting_record(r->accounting, acct_str,
...
#endif
#if CONFIG_ACCOUNTING
  if (acct_str) aom_process_accounting(r, acct_str);
#endif
  return ret;
}
