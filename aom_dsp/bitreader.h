...
#define aom_read_tree_cdf(r, cdf, nsymbs, str) \
  aom_read_tree_cdf_(r, cdf, nsymbs, str)
#else
...
#define aom_read_tree_cdf(r, cdf, nsymbs, str) \
  aom_read_tree_cdf_(r, cdf, nsymbs)
#endif
...
static INLINE int aom_read_literal_(aom_reader *r,
  ...
  for (bit = bits - 1; bit >= 0; bit--)
    literal |= aom_read_bit(r, acct_str) << bit;
...
static INLINE int aom_read_tree_cdf_(aom_reader *r, const uint16_t *cdf,
  ...
  ret = rans_read(r, cdf);
#elif CONFIG_DAALA_EC
  ret = daala_read_tree_cdf(r, cdf, nsymbs);
#else
  (void)r;
  (void)cdf;
  (void)nsymbs;
