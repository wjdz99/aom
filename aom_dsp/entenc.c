...
static void od_ec_enc_normalize(od_ec_enc *enc, enc_od_ec_window low,
                                unsigned rng) {
  int d;
  int c;
  int s;
...
void od_ec_enc_patch_initial_bits(od_ec_enc *enc, unsigned val, int nbits) {
  ...
  if (enc->offs > 0) {
    ...
    enc->low = (enc->low & ~((enc_od_ec_window)mask << (16 + enc->cnt))) |
               (enc_od_ec_window)val << (16 + enc->cnt + shift);
  } else {
