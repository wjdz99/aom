...
static void propagate_carry_bwd(unsigned char *buf, uint32_t offs) {
...
static void od_ec_enc_normalize(od_ec_enc *enc, od_ec_window low,
  ...
  /* We flush every time "low" cannot safely and efficiently accomodate any more
     data. Overall, c must not exceed 63 at the time of byte flush out. To
     ...
     (e < d). But, this would have caused pushing leading 0x00's, which needed
     special handling with more computations
  */
  if (s >= 40) {  // 56 - 16
    unsigned char *out = enc->buf;
    uint32_t storage = enc->storage;
    uint32_t old_storage = storage;
    uint32_t offs = enc->offs;
    if (offs + 8 > storage) {
      storage = 2 * storage + 8;
      out = (unsigned char *)aom_memalign_realloc(
          out, sizeof(*out) * old_storage, sizeof(*out) * storage, 64);
      if (out == NULL) {
        enc->error = -1;
        enc->offs = 0;
        return;
    ...
    // Update "c", prepare output and update "low"
    ...
    *(uint64_t *)&out[offs] = reg;
    // Propagate carry backwards if exists
    if (carry) {
      assert(offs > 0);
      propagate_carry_bwd(out, offs - 1);
    }
    enc->offs = offs + num_bytes_ready;
#elif defined(USE_MSC_INTRINSICS)
    // MSVC
    const uint64_t reg =
        _byteswap_uint64(output) >> ((8 - num_bytes_ready) << 3);
    *(uint64_t *)&out[offs] = reg;
    // Propagate carry backwards if exists
    if (carry) {
      assert(offs > 0);
...
void od_ec_enc_init(od_ec_enc *enc, uint32_t size) {
  od_ec_enc_reset(enc);
  enc->buf = (unsigned char *)aom_memalign(64, sizeof(*enc->buf) * size);
  enc->storage = size;
...
void od_ec_enc_clear(od_ec_enc *enc) { aom_free(enc->buf); }
...
unsigned char *od_ec_enc_done(od_ec_enc *enc, uint32_t *nbytes) {
  ...
  if (offs + b > storage) {
    storage = offs + b;
    out = (unsigned char *)aom_memalign_realloc(out, sizeof(*out) * old_storage,
                                                sizeof(*out) * storage, 64);
    if (out == NULL) {
      enc->error = -1;
      return NULL;
    }
