...
static void grain_table_entry_read(FILE *file,
  ...
  int num_read = fscanf(file, "E %lld %lld %d %hd %d\n", &entry->start_time,
                        &entry->end_time, &pars->apply_grain,
  ...
  if (pars->update_parameters) {
    ...
    if (!fscanf(file, "\n\tcY ")) {
      aom_internal_error(error_info, AOM_CODEC_ERROR,
                         "Unable to read cY coeff header");
      return;
    }
    const int n = 2 * pars->ar_coeff_lag * (pars->ar_coeff_lag + 1);
    for (int i = 0; i < n; ++i) {
      if (2 != fscanf(file, "%d ", &pars->ar_coeffs_y[i])) {
        aom_internal_error(error_info, AOM_CODEC_ERROR,
    ...
    if (!fscanf(file, "\n\tcCb ")) {
      aom_internal_error(error_info, AOM_CODEC_ERROR,
                         "Unable to read Cb coeff header");
      return;
    }
    for (int i = 0; i <= n; ++i) {
      if (2 != fscanf(file, "%d ", &pars->ar_coeffs_cb[i])) {
        aom_internal_error(error_info, AOM_CODEC_ERROR,
    ...
    if (!fscanf(file, "\n\tcCr ")) {
      aom_internal_error(error_info, AOM_CODEC_ERROR,
                         "Unable to read Cr coeff header");
      return;
    }
    for (int i = 0; i <= n; ++i) {
      if (2 != fscanf(file, "%d ", &pars->ar_coeffs_cr[i])) {
        aom_internal_error(error_info, AOM_CODEC_ERROR,
...
void grain_table_entry_write(FILE *file, aom_film_grain_table_entry_t *entry) {
  ...
  fprintf(file, "E %lld %lld %d %d %d\n", entry->start_time, entry->end_time,
          pars->apply_grain, pars->random_seed, pars->update_parameters);
  if (pars->update_parameters) {
