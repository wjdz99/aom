...
// This must only be called while holding img_pyr->mutex
static INLINE void fill_pyramid(const YV12_BUFFER_CONFIG *img, int bit_depth,
                                ImagePyramid *img_pyr) {
  int n_levels = img_pyr->n_levels;
  const int img_width = img->y_crop_width;
  const int img_height = img->y_crop_height;
  const int img_stride = img->y_stride;
  assert((img_width >> n_levels) >= 0);
  assert((img_height >> n_levels) >= 0);

  PyramidLayer *first_layer = &img_pyr->layers[0];
  if (img->flags & YV12_FLAG_HIGHBITDEPTH) {
    // For frames stored in a 16-bit buffer, we need to downconvert to 8 bits
    assert(first_layer->width == img_width);
    assert(first_layer->height == img_height);

    uint16_t *img_buffer = CONVERT_TO_SHORTPTR(img->y_buffer);
    uint8_t *pyr_buffer = first_layer->buffer;
    int pyr_stride = first_layer->stride;
    for (int y = 0; y < img_height; y++) {
      uint16_t *img_row = img_buffer + y * img_stride;
      uint8_t *pyr_row = pyr_buffer + y * pyr_stride;
      for (int x = 0; x < img_width; x++) {
        pyr_row[x] = img_row[x] >> (bit_depth - 8);
      }
    }

    fill_border(pyr_buffer, img_width, img_height, pyr_stride);
  } else {
    // For frames stored in an 8-bit buffer, we need to configure the first
    // pyramid layer to point at the original image buffer
    first_layer->buffer = img->y_buffer;
    first_layer->width = img_width;
    first_layer->height = img_height;
    first_layer->stride = img_stride;
  }

  // Fill in the remaining levels through progressive downsampling
  for (int level = 1; level < n_levels; ++level) {
    PyramidLayer *prev_layer = &img_pyr->layers[level - 1];
    uint8_t *prev_buffer = prev_layer->buffer;
    int prev_stride = prev_layer->stride;

    PyramidLayer *this_layer = &img_pyr->layers[level];
...
void aom_compute_pyramid(const YV12_BUFFER_CONFIG *img, int bit_depth,
                         ImagePyramid *pyr) {
  assert(pyr);
  ...
  if (!pyr->valid) {
    fill_pyramid(img, bit_depth, pyr);
    pyr->valid = true;
  }
