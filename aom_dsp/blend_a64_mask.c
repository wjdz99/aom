...
void aom_highbd_blend_a64_d16_mask_c(
  ...
  const int clamp_low = 0;
  int clamp_high;
  switch (bd) {
    case 8:
    default: clamp_high = 255; break;
    case 10: clamp_high = 1023; break;
    case 12: clamp_high = 4095; break;
  }
  ...
  if (subw == 0 && subh == 0) {
    for (int i = 0; i < h; ++i) {
      for (int j = 0; j < w; ++j) {
        ...
        res -= round_offset;
        dst[j] =
            clamp(ROUND_POWER_OF_TWO(res, round_bits), clamp_low, clamp_high);
      }
      mask += mask_stride;
      src0 += src0_stride;
      src1 += src1_stride;
