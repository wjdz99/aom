...
void aom_blend_a64_d32_mask_c(int32_t *dst, uint32_t dst_stride,
  ...
  if (subw == 0 && subh == 0) {
    for (i = 0; i < h; ++i) {
      for (j = 0; j < w; ++j) {
        const int64_t m = mask[i * mask_stride + j] << 2;
        dst[i * dst_stride + j] = (int32_t)(
    ...
    for (i = 0; i < h; ++i) {
      for (j = 0; j < w; ++j) {
        const int64_t m = (mask[(2 * i) * mask_stride + (2 * j)] +
    ...
    for (i = 0; i < h; ++i) {
      for (j = 0; j < w; ++j) {
        const int64_t m = (mask[i * mask_stride + (2 * j)] +
    ...
    for (i = 0; i < h; ++i) {
      for (j = 0; j < w; ++j) {
        const int64_t m = (mask[(2 * i) * mask_stride + j] +
