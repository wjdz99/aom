...
void aom_blend_a64_d32r4_mask_c(int32_t *dst, uint32_t dst_stride,
  ...
  if (subw == 0 && subh == 0) {
    for (i = 0; i < h; ++i) {
      for (j = 0; j < w; ++j) {
        ...
        const int s0 = ROUND_POWER_OF_TWO(src0[i * src0_stride + j], 4);
        const int s1 = ROUND_POWER_OF_TWO(src1[i * src1_stride + j], 4);
        dst[i * dst_stride + j] = AOM_BLEND_A64(m, s0, s1);
      }
    }
