...
static INLINE void d207_predictor(uint8_t *dst, ptrdiff_t stride, int bw,
                                  int bh, const uint8_t *above,
                                  const uint8_t *left) {
  int r, c;
  (void)above;
  // first column
  for (r = 0; r < bh - 1; ++r) dst[r * stride] = AVG2(left[r], left[r + 1]);
  dst[(bh - 1) * stride] = left[bh - 1];
  dst++;

  // second column
  for (r = 0; r < bh - 2; ++r)
    dst[r * stride] = AVG3(left[r], left[r + 1], left[r + 2]);
  dst[(bh - 2) * stride] = AVG3(left[bh - 2], left[bh - 1], left[bh - 1]);
  dst[(bh - 1) * stride] = left[bh - 1];
  dst++;

  // rest of last row
  for (c = 0; c < bw - 2; ++c) dst[(bh - 1) * stride + c] = left[bh - 1];
...
static INLINE void d63_predictor(uint8_t *dst, ptrdiff_t stride, int bw, int bh,
  ...
  for (r = 2, size = bw - 2; r < bh; r += 2, --size) {
    if (size < 0) {
      fprintf(stderr, "pred = d63: bw = %d, bh = %d, r = %d, size = %d\n", bw,
              bh, r, size);
      assert(0);
    }
    memcpy(dst + (r + 0) * stride, dst + (r >> 1), size);
    memset(dst + (r + 0) * stride + size, above[bw - 1], bw - size);
    memcpy(dst + (r + 1) * stride, dst + stride + (r >> 1), size);
    memset(dst + (r + 1) * stride + size, above[bw - 1], bw - size);
  }
}
...
static INLINE void d45_predictor(uint8_t *dst, ptrdiff_t stride, int bw, int bh,
                                 const uint8_t *above, const uint8_t *left) {
  const uint8_t above_right = above[bw - 1];
  ...
  dst[bw - 1] = above_right;
  dst += stride;
  for (x = 1, size = bw - 2; x < bh; ++x, --size) {
    if (size < 0) {
      fprintf(stderr, "pred = d45: bw = %d, bh = %d, r = %d, size = %d\n", bw,
              bh, x, size);
      assert(0);
    }
...
static INLINE void smooth_predictor(uint8_t *dst, ptrdiff_t stride, int bw,
  ...
  // width.
  const int arr_index_w = get_msb(bw) - 1;
  assert(arr_index_w >= 0);
  assert(arr_index_w < NUM_BLOCK_DIMS);
  const uint8_t *const sm_weights_w = sm_weight_arrays[arr_index_w];
  // height.
  const int arr_index_h = get_msb(bh) - 1;
  assert(arr_index_h >= 0);
  assert(arr_index_h < NUM_BLOCK_DIMS);
  const uint8_t *const sm_weights_h = sm_weight_arrays[arr_index_h];
...
// TODO(now): Update the HBD versions.
...
static INLINE void highbd_d207_predictor(uint16_t *dst, ptrdiff_t stride,
  ...
  // Rest of last row.
  for (c = 0; c < bw - 2; ++c) dst[(bs - 1) * stride + c] = left[bs - 1];
...
static INLINE void highbd_d63_predictor(uint16_t *dst, ptrdiff_t stride, int bw,
...
static INLINE void highbd_d45_predictor(uint16_t *dst, ptrdiff_t stride, int bw,
                                        int bh, const uint16_t *above,
                                        const uint16_t *left, int bd) {
  int r, c;
  (void)left;
