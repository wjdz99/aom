...
// Weights are quadratic from 'block_size' to '1', scaled by
// (2^sm_weight_log2_scale) / block_size.
static const int sm_weight_log2_scale = 14;

#if CONFIG_TX64X64
static const uint16_t sm_weight_arrays[6][64] = {
#else
static const uint16_t sm_weight_arrays[5][32] = {
#endif  // CONFIG_TX64X64
  // bs = 2
  { 16384, 8192 },
  // bs = 4
  ...
  // bs = 8
  { 16384, 12581, 9362, 6729, 4681, 3218, 2341, 2048 },
  // bs = 16
  { 16384, 14404, 12561, 10854, 9284, 7851, 6554, 5393, 4369, 3482, 2731, 2116,
    1638, 1297, 1092, 1024 },
  // bs = 32
  { 16384, 15377, 14402, 13461, 12552, 11677, 10835, 10025, 9249, 8506, 7796,
    7118,  6474,  5863,  5285,  4740,  4228,  3749,  3303,  2890, 2510, 2164,
    1850,  1569,  1321,  1107,  925,   776,   661,   578,   529,  512 },
#if CONFIG_TX64X64
  // bs = 64
  { 16384, 15876, 15376, 14885, 14401, 13926, 13458, 12999, 12548, 12105, 11670,
    11244, 10825, 10415, 10012, 9618,  9232,  8854,  8485,  8123,  7769,  7424,
    7087,  6758,  6437,  6124,  5819,  5522,  5234,  4953,  4681,  4417,  4161,
    3913,  3673,  3442,  3218,  3003,  2796,  2597,  2406,  2223,  2048,  1881,
    1723,  1573,  1430,  1296,  1170,  1052,  943,   841,   748,   662,   585,
    516,   455,   402,   358,   321,   293,   272,   260,   256 },
#endif  // CONFIG_TX64X64
};
...
static INLINE void smooth_predictor(uint8_t *dst, ptrdiff_t stride, int bs,
  ...
  const uint16_t *const sm_weights = sm_weight_arrays[arr_index];
  // scale = 2 * 2^sm_weight_log2_scale
  const int log2_scale = 1 + sm_weight_log2_scale;
  assert(log2_scale + sizeof(*dst) < 32);  // sanity check: no overflow.
  const uint16_t scale = sm_weights[0];
  ...
  for (r = 0; r < bs; ++r) {
    ...
    for (c = 0; c < bs; ++c) {
      ...
      const uint16_t weights[] = { sm_weights[r], scale - sm_weights[r],
