...
static INLINE void highbd_ibp_dc_left_predictor(uint16_t *dst, ptrdiff_t stride,
    ...
    for (r = 0; r < bh; r++) {
        for (int c = 0; c < len; c++) {
            int val = (left[r] * (256 - weights[c]) + dst[c] * weights[c] + ibp_offset) >> ibp_shift;
            dst[c] = val;
        }
        dst += stride;
    }
...
static INLINE void highbd_ibp_dc_top_predictor(uint16_t *dst, ptrdiff_t stride,
    ...
    for (r = 0; r < len; r++) {
        for (int c = 0; c < bw; c++) {
            int val = (above[c] * (256 - weights[r]) + dst[c] * weights[r] + ibp_offset) >> ibp_shift;
            dst[c] = val;
        }
        dst += stride;
    }
...
static INLINE void highbd_ibp_dc_predictor(uint16_t *dst, ptrdiff_t stride, int bw,
                                         int bh, const uint16_t *above,
                                         const uint16_t *left, int bd) {
    int i, r, expected_dc, sum = 0;
    const int count = bw + bh;
...
static INLINE void ibp_dc_predictor(uint8_t *dst, ptrdiff_t stride, int bw, int bh,
                                  const uint8_t *above, const uint8_t *left) {
    int i, r, expected_dc, sum = 0;
    const int count = bw + bh;
    uint8_t *orig_dst = dst;
