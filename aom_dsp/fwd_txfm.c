...
void aom_fdct4x4_c(const int16_t *input, tran_low_t *output, int stride) {
  ...
  for (int pass = 0; pass < 2; ++pass) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      temp[1] = (tran_low_t) fdct_round_shift(step[2] * cospi_24_64 + step[3] * cospi_8_64);
      temp[3] = (tran_low_t) fdct_round_shift(-step[2] * cospi_8_64 + step[3] * cospi_24_64);
...
void aom_fdct4x4_lp_c(const int16_t *input, int16_t *output, int stride) {
  ...
  for (int pass = 0; pass < 2; ++pass) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      temp[1] = (int16_t) fdct_round_shift(step[2] * cospi_24_64 + step[3] * cospi_8_64);
      temp[3] = (int16_t) fdct_round_shift(-step[2] * cospi_8_64 + step[3] * cospi_24_64);
      // Only transpose the first pass.
      if (pass == 0) {
        out[0] = temp[0];
        out[1] = temp[1];
