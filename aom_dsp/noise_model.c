        ...
        for (x = x_start; x < x_end; ++x) {
          ...
          if (alt_data && alt_denoised) {
            ...
            for (int dy_i = 0; dy_i < (1 << sub_log2[1]); dy_i++) {
              int y_up = ((y_o + y) << sub_log2[1]) + dy_i;
              for (int dx_i = 0; dx_i < (1 << sub_log2[0]); dx_i++) {
                int x_up = ((x_o + x) << sub_log2[0]) + dx_i;
    ...
    for (bx = 0; bx < num_blocks_w; ++bx) {
      ...
      int y_end =
          (by + 1 < num_blocks_h && flat_blocks[(by + 1) * num_blocks_w + bx])
              ? (block_size >> sub_log2[1])
              : AOMMIN((h >> sub_log2[1]) -
                           num_blocks_h * (block_size >> sub_log2[1]) - lag,
                       (block_size >> sub_log2[1]) - lag);
      int x_end =
          (bx + 1 < num_blocks_w && flat_blocks[by * num_blocks_w + bx + 1])
              ? (block_size >> sub_log2[0])
              : AOMMIN((w >> sub_log2[0]) -
                           num_blocks_w * (block_size >> sub_log2[0]) - lag,
                       (block_size >> sub_log2[0]) - lag);
      for (int y = y_start; y < y_end; y++) {
        for (int x = x_start; x < x_end; x++) {
          ...
          if (alt_data && alt_denoised) {
            ...
            sum += coeffs[num_coords] * (avg_data / n - avg_denoised / n);
          }
...
aom_noise_status_t aom_noise_model_update(
  ...
  for (channel = 0; channel < 3; ++channel) {
    ...
    int *sub = channel > 0 ? chroma_sub : no_subsampling;
    ...
    if (!equation_system_solve(&noise_model->latest_state[channel].eqns)) {
      if (channel > 0) {
        int last = noise_model->n;
        aom_equation_system_t *eqns = &noise_model->latest_state[channel].eqns;
        memset(eqns->x, 0, sizeof(double) * eqns->n);
        if (fabs(eqns->A[last * eqns->n + last]) > 1e-6) {
