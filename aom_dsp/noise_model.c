...
static int is_noise_model_different(aom_noise_model_t *const noise_model) {
  ...
  for (int c = 0; c < 1; ++c) {
    const double corr = aom_normalized_cross_correlation(
        noise_model->latest_state[c].eqns.x,
        noise_model->combined_state[c].eqns.x,
        noise_model->combined_state[c].eqns.n);
    if (corr < kCoeffThreshold) return 1;

    const double dx = 1.0 /
        noise_model->latest_state[c].strength_solver.num_bins;
        //       (noise_model->latest_state[c].strength_solver.max_intensity -
        //        noise_model->latest_state[c].strength_solver.min_intensity) /
    ...
    for (int j = 0; j < latest_eqns->n; ++j) {
      ...
      weight = sqrt(weight);
      fprintf(stderr, " [%f %f] ", latest_eqns->x[j], combined_eqns->x[j]);
      diff += weight * fabs(latest_eqns->x[j] - combined_eqns->x[j]);
      total_weight += weight;
    }
    fprintf(stderr, "corr = %lf, diff = %lf, weighted = %lf\n",
            corr, diff * dx, diff * dx / total_weight);
...
aom_noise_status_t aom_noise_model_update(
  ...
  for (channel = 0; channel < 3; ++channel) {
    ...
    if (channel == 0 &&
        ...
        0 && is_noise_model_different(noise_model)) {
      y_model_different = 1;
    }
...
void aom_noise_model_swap(aom_noise_model_t *noise_model) {
  for (int c = 0; c < 3; c++) {
    equation_system_copy(
        &noise_model->combined_state[c].eqns,
...
int aom_wiener_denoise_2d(const uint8_t *const data[3], uint8_t *denoised[3],
  ...
  double *plane = NULL, *block = NULL, *window_full = NULL, *window_chroma = NULL;
  ...
  init_success &= (tx_full != NULL) && (tx_chroma != NULL) &&
      (plane != NULL) && (block != NULL) &&
      (window_full != NULL) && (result != NULL) &&
      (window_chroma != NULL);
  ...
  for (c = init_success ? 0 : 3; c < 3; ++c) {
        ...
        for (by = -1; by < num_blocks_h; ++by) {
          for (bx = -1; bx < num_blocks_w; ++bx) {
            ...
            for (y = 0; y < (block_size >> chroma_sub_h); ++y) {
              const int y_result = y + (by + 1) * (block_size >> chroma_sub_h) + offsy;
              for (x = 0; x < (block_size >> chroma_sub_w); ++x) {
                const int x_result = x + (bx + 1) * (block_size >> chroma_sub_w) + offsx;
    ...
    for (y = 0; y < (h >> chroma_sub_h); ++y) {
      for (x = 0; x < (w >> chroma_sub_w); ++x) {
        const int result_idx =
            (y + (block_size >> chroma_sub_h)) * result_stride +
            x + (block_size >> chroma_sub_w);
        uint8_t new_val = (uint8_t)AOMMIN(
            AOMMAX(result[result_idx] * kBlockNormalization + 0.5, 0),
            kBlockNormalization);
        const float err = -(((float)new_val) / kBlockNormalization - result[result_idx]);
        denoised[c][y * stride[c] + x] = new_val;
