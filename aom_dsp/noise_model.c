...
static void equation_system_clear(aom_equation_system_t* eqns) {
  const int n = eqns->n;
  memset(eqns->A, 0, sizeof(double) * n * n);
  memset(eqns->x, 0, sizeof(double) * n);
  memset(eqns->b, 0, sizeof(double) * n);
}

static int equation_system_init(aom_equation_system_t* eqns, int n) {
  eqns->A = (double*)aom_malloc(sizeof(double) * n * n);
  eqns->b = (double*)aom_malloc(sizeof(double) * n);
  eqns->x = (double*)aom_malloc(sizeof(double) * n);
  eqns->n = n;
  if (!eqns->A || !eqns->b || !eqns->x) {
    ...
    aom_free(eqns->b);
    aom_free(eqns->x);
    memset(eqns, 0, sizeof(aom_equation_system_t));
    return 0;
  }
...
static int equation_system_solve(aom_equation_system_t* eqns) {
  const int n = eqns->n;
  double* b = (double*)aom_malloc(sizeof(double) * n);
  double* A = (double*)aom_malloc(sizeof(double) * n * n);
  int ret = 0;
  if (A == NULL || b == NULL) {
  ...
  memcpy(A, eqns->A, sizeof(double) * n * n);
  memcpy(b, eqns->b, sizeof(double) * n);
...
int aom_noise_strength_lut_init(aom_noise_strength_lut_t* lut, int num_points) {
  if (!lut) return 0;
  lut->points = (double(*)[2])aom_malloc(num_points * sizeof(double) * 2);
  if (!lut->points) return 0;
  lut->num_points = num_points;
  memset(lut->points, 0, sizeof(double) * 2 * num_points);
  return 1;
}
...
void aom_noise_strength_lut_free(aom_noise_strength_lut_t* lut) {
  if (!lut) return;
  aom_free(lut->points);
  memset(lut, 0, sizeof(aom_noise_strength_lut_t));
}
...
int aom_noise_strength_solver_solve(aom_noise_strength_solver_t* solver) {
  ...
  memcpy(A, old_A, sizeof(double) * n * n);
...
int aom_noise_strength_solver_init(aom_noise_strength_solver_t* solver,
                                   int num_bins) {
  if (!solver) return 0;
  memset(solver, 0, sizeof(aom_noise_strength_solver_t));
...
int aom_flat_block_finder_init(aom_flat_block_finder_t* block_finder,
  ...
  AtA_inv = (double*)aom_malloc(kLowPolyNumParams * kLowPolyNumParams *
                                sizeof(double));
  A = (double*)aom_malloc(kLowPolyNumParams * n * sizeof(double));
  ...
  for (i = 0; i < kLowPolyNumParams; ++i) {
    memset(eqns.b, 0, sizeof(double) * kLowPolyNumParams);
    eqns.b[i] = 1;
...
void aom_flat_block_finder_free(aom_flat_block_finder_t* block_finder) {
  ...
  memset(block_finder, 0, sizeof(aom_flat_block_finder_t));
}
...
int aom_flat_block_finder_run(const aom_flat_block_finder_t* block_finder,
  ...
  int num_flat = 0;
  int bx = 0, by = 0;
  double* plane = (double*)aom_malloc(n * sizeof(double));
  double* block = (double*)aom_malloc(n * sizeof(double));
...
int aom_noise_model_init(aom_noise_model_t* model,
  ...
  memset(model, 0, sizeof(aom_noise_model_t));
  if (params.lag < 1) {
  ...
  model->n = n;
  model->coords = (int(*)[2])aom_malloc(sizeof(int) * 2 * n);
...
static int add_block_observations(
  ...
  double* buffer = (double*)aom_malloc(sizeof(double) * (num_coords + 1));
