...
} aom_denoise_and_model;

aom_denoise_and_model *aom_denoise_and_model_alloc(int bit_depth,
                                                   int block_size,
                                                   float noise_level) {
  aom_denoise_and_model *ctx =
      (aom_denoise_and_model *)aom_malloc(sizeof(*ctx));
  if (!ctx) {
...
void aom_denoise_and_model_free(aom_denoise_and_model *ctx) {
...
static int denoise_and_model_realloc_if_necessary(aom_denoise_and_model *ctx,
...
int aom_denoise_and_model_run(aom_denoise_and_model *ctx,
                              YV12_BUFFER_CONFIG *sd,
                              aom_film_grain_t *film_grain) {
  const int block_size = ctx->block_size;
  const int use_highbd = (sd->flags & YV12_FLAG_HIGHBITDEPTH) != 0;
