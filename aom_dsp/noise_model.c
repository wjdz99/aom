...
static void pointwise_multiply(const float *const a, float *b, int n) {
  for (int i = 0; i < n; ++i) {
    b[i] *= a[i];
  }
}
...
int aom_wiener_denoise_2d(const uint8_t *const data[3], uint8_t *denoised[3],
  ...
  result = (float *)aom_malloc((num_blocks_h + 2) * block_size * result_stride *
                               sizeof(float));
  plane = (float *)aom_malloc(block_size * block_size * sizeof(float));
  block = (float *)aom_malloc(2 * block_size * block_size * sizeof(float));
  block_d = (double *)aom_malloc(block_size * block_size * sizeof(double));
  plane_d = (double *)aom_malloc(block_size * block_size * sizeof(double));
      ...
      for (offsx = 0; offsx < (block_size >> chroma_sub_w);
        ...
        for (by = -1; by < num_blocks_h; ++by) {
          for (bx = -1; bx < num_blocks_w; ++bx) {
            ...
            int n = (block_size >> chroma_sub_w) * (block_size >> chroma_sub_h);
            for (int j = 0; j < n; ++j) {
              block[j] = block_d[j];
              plane[j] = plane_d[j];
            }
