...
if (aom_config("CONFIG_AV1_ENCODER") eq "yes") {
  ...
  foreach (@block_sizes) {
    ($w, $h) = @$_;
    add_proto qw/void/, "aom_sad${w}x${h}x4d", "const uint8_t *src_ptr, int src_stride, const uint8_t * ref_ptr[], int ref_stride, int32_t *err, uint32_t* min_value,
    int32_t* min_pos";
  }
  ...
  if (aom_config("CONFIG_AV1_HIGHBITDEPTH") eq "yes") {
    foreach (@block_sizes) {
      ($w, $h) = @$_;
      add_proto qw/void/, "aom_highbd_sad${w}x${h}x4d", "const uint8_t *src_ptr, int src_stride, const uint8_t * ref_ptr[], int ref_stride,  int32_t *err, uint32_t* min_value,
    int32_t* min_pos";
      if ($w != 128 && $h != 128) {
