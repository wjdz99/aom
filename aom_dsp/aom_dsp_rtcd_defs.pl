...
if ((aom_config("CONFIG_ALT_INTRA") eq "yes")) {
  @pred_names = ('dc', 'dc_top', 'dc_left', 'dc_128', 'v', 'h', 'paeth',
                 'smooth', 'he', 've', 'd207', 'd207e', 'd63', 'd63e', 'd63f',
                 'd45', 'd45e', 'd117', 'd135', 'd153');
} else {
  @pred_names =
      ('dc', 'dc_top', 'dc_left', 'dc_128', 'v', 'h', 'tm', 'he', 've', 'd207',
       'd207e', 'd63', 'd63e', 'd63f', 'd45', 'd45e', 'd117', 'd135', 'd153');
}
...
foreach (@tx_sizes) {
  ...
  foreach $pred_name (@pred_names) {
    add_proto split('\s+', "void aom_${pred_name}_predictor_${w}x${h}"),
              "uint8_t *dst, ptrdiff_t y_stride, const uint8_t *above, const uint8_t *left";
    if (aom_config("CONFIG_AOM_HIGHBITDEPTH") eq "yes") {
      add_proto split('\s+',
                      "void aom_highbd_${pred_name}_predictor_${w}x${h}"),
