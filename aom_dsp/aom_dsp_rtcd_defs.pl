...
if (aom_config("CONFIG_AV1_ENCODER") eq "yes") {
  ...
  foreach (@block_sizes) {
    ...
    if ($w != 128 && $h != 128 && $w != 4 &&
     ((aom_config("CONFIG_ERP") eq "") || (!($w == 16 && $h == 32)))) {
     # 8x16 and 16x32 functions have an alignment issue with CONFIG_ERP experiment. So disabled for now.
...
if (aom_config("CONFIG_ERP") eq "") {
  ...
  foreach $bd (8, 10, 12) {
    ...
    foreach (@block_sizes) {
      ...
      if ($w != 128 && $h != 128 && $w != 4 &&
          ((aom_config("CONFIG_ERP") eq "") || (!($w == 8 && $h == 16) && !($w == 16 && $h == 32)))) {
        # 8x16 and 16x32 functions have an alignment issue with CONFIG_ERP experiment. So disabled for now.
