...
typedef void (*aom_fft_1d_func_t)(const float *, float *, int stride);
typedef void (*aom_fft_transpose_func_t)(const float *, float *, int n);
typedef void (*aom_fft_unpack_func_t)(const float *, float *, int n);
 ...
 * \param[in]  temp      Working buffer for computing the transform (size n x n)
 * \param[in]  output    Output buffer (size 2 * n * n)
 ...
 * \param[in]  temp       Working buffer for computing the transform (size 2 x n
 * x n) \param[in]  output     Output buffer (size n * n) \param[in]  fft_single
 * Forward transform function (non vectorized) \param[in]  fft_multi  Forward
 * transform function (vectorized) \param[in]  ifft_multi Inverse transform
 * function (vectorized) \param[in]  transpose  Trnspose function (for n x n
 * matrix) \param[in]  vec_size  Vector size (the transform is done vec_size
 * units at a time)
 */
