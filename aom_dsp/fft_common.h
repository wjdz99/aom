 ...
 *
 * Vectorized implementations are parallelized along the columns. In such cases
 * the data block for input and output is typically square (n x n) and the
 * stride will correspond to the spacing between rows.
 *
 * \param[in]  input   Input buffer (size should be n for transform of size n)
 * \param[out] output  Output buffer (size should be n for transform of size n)
