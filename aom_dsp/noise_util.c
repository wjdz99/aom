...
void aom_noise_synth(int lag, int n, const int (*coords)[2],
  ...
  int x = 0, y = 0;
  double* padded = (double*)aom_malloc(padded_w * padded_h * sizeof(double));
  ...
  for (y = 0; y < h; ++y) {
    memcpy(data + y * w, padded + y * padded_w, sizeof(double) * w);
  }
  aom_free(padded);
}
...
int aom_noise_data_validate(const double* data, int w, int h) {
  ...
  mean_x = (double*)aom_malloc(sizeof(double) * w);
  var_x = (double*)aom_malloc(sizeof(double) * w);
  mean_y = (double*)aom_malloc(sizeof(double) * h);
  var_y = (double*)aom_malloc(sizeof(double) * h);

  memset(mean_x, 0, sizeof(double) * w);
  memset(var_x, 0, sizeof(double) * w);
  memset(mean_y, 0, sizeof(double) * h);
  memset(var_y, 0, sizeof(double) * h);
  ...
  for (y = 0; y < h; ++y) {
    ...
    if (abs(var_y[y] - var) >= kVarianceThreshold) {
      fprintf(stderr, "Variance distance too large %f %f\n", var_y[y], var);
      ret_value = 0;
      break;
    }
    if (abs(mean_y[y] - mean) >= kMeanThreshold) {
  ...
  for (x = 0; x < w; ++x) {
    ...
    if (abs(var_x[x] - var) >= kVarianceThreshold) {
      fprintf(stderr, "Variance distance too large %f %f\n", var_x[x], var);
      ret_value = 0;
      break;
    }
    if (abs(mean_x[x] - mean) >= kMeanThreshold) {
      fprintf(stderr, "Mean distance too large %f %f\n", mean_x[x], mean);
      ret_value = 0;
      break;
    }
