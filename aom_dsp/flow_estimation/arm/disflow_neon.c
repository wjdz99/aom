...
static INLINE void sobel_filter_x(const uint8_t *src, int src_stride,
  ...
  for (int i = 0; i < DISFLOW_PATCH_SIZE + 2; i++) {
    ...
    uint8x8_t t0 = vget_low_u8(s);
    uint8x8_t t1 = vget_low_u8(vextq_u8(s, s, 2));

    // Given that the kernel is [1, 0, -1] the convolution is a simple
    // subtraction.
    int16x8_t diff = vreinterpretq_s16_u16(vsubl_u8(t0, t1));
  ...
  int16x8_t t0 = vld1q_s16(tmp);
  int16x8_t t1 = vld1q_s16(tmp + DISFLOW_PATCH_SIZE);
  int16x8_t add01 = vaddq_s16(t0, t1);
  for (int i = 0; i < DISFLOW_PATCH_SIZE; i++) {
    int16x8_t t2 = vld1q_s16(tmp + (i + 2) * DISFLOW_PATCH_SIZE);

    int16x8_t add12 = vaddq_s16(t1, t2);
    int16x8_t sum = vaddq_s16(add01, add12);
...
static INLINE void sobel_filter_y(const uint8_t *src, int src_stride,
  ...
  for (int i = 0; i < DISFLOW_PATCH_SIZE + 2; i++) {
    uint8x16_t s = vld1q_u8(src_start + i * src_stride);
    uint8x8_t t0 = vget_low_u8(s);
    uint8x8_t t1 = vget_low_u8(vextq_u8(s, s, 1));
    uint8x8_t t2 = vget_low_u8(vextq_u8(s, s, 2));

    uint16x8_t add01 = vaddl_u8(t0, t1);
    uint16x8_t add12 = vaddl_u8(t1, t2);
    uint16x8_t sum = vaddq_u16(add01, add12);
