...
static INLINE double getCubicValue_dbl(double *p, double *kernel) {
  return kernel[0] * p[0] + kernel[1] * p[1] + kernel[2] * p[2] +
         kernel[3] * p[3];
}

static INLINE int getCubicValue_int(int *p, int *kernel) {
  return kernel[0] * p[0] + kernel[1] * p[1] + kernel[2] * p[2] +
         kernel[3] * p[3];
}
...
static INLINE int compute_flow_error(unsigned char *ref, unsigned char *frm,
  ...
  for (int i = 0; i < DISFLOW_PATCH_SIZE; ++i) {
    for (int j = 0; j < DISFLOW_PATCH_SIZE; ++j) {
      ...
      int warped =
          clamp(ROUND_POWER_OF_TWO(result, CUBIC_PREC_BITS + 6), 0, 255);
...
void aom_compute_flow_at_point_c(unsigned char *frm, unsigned char *ref, int x,
    ...
    for (int i = FLOW_BORDER; i < cur_flow_height - FLOW_BORDER; i += 1) {
      for (int j = FLOW_BORDER; j < cur_flow_width - FLOW_BORDER; j += 1) {
        ...
        aom_compute_flow_at_point(
            frm_pyr->level_buffer + cur_loc, ref_pyr->level_buffer + cur_loc,
            patch_tl_x, patch_tl_y, cur_width, cur_height, cur_stride,
