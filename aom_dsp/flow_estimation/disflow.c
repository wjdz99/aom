...
#define DISFLOW_SSE_TR ((int)(DISFLOW_MSE_TR * PATCH_SIZE * PATCH_SIZE + 0.5))
...
// TODO(rachelbarker):
// This function takes most of the runtime of the disflow code.
// It could probably be made much faster if integerized, and much faster
// again with vectorization
static int compute_warp_and_error(unsigned char *ref, unsigned char *frm,
  ...
  for (int i = 0; i < PATCH_SIZE; ++i) {
    for (int j = 0; j < PATCH_SIZE; ++j) {
      ...
      int warped = clamp((int)(result + 0.5), 0, 255);
      int err = warped - frm[(x + j) + (y + i) * stride];
      sse += err * err;
...
static INLINE void compute_flow_at_point(unsigned char *frm, unsigned char *ref,
  ...
  for (int itr = 0; itr < DISFLOW_MAX_ITR; itr++) {
    int sse = compute_warp_and_error(ref, frm, width, height, stride, x, y, *u,
                                     *v, dt);
