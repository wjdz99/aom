...
static INLINE void getCubicKernel_int(double x, int *kernel) {
  ...
  kernel[0] = (int)rint(k0 * (1 << DISFLOW_INTERP_BITS));
  kernel[1] = (int)rint(k1 * (1 << DISFLOW_INTERP_BITS));
  kernel[2] = (int)rint(k2 * (1 << DISFLOW_INTERP_BITS));
  kernel[3] = (int)rint(k3 * (1 << DISFLOW_INTERP_BITS));
}
...
static int determine_disflow_correspondence(CornerList *corners,
  ...
  for (int i = 0; i < corners->num_corners; ++i) {
    int x0 = corners->corners[2 * i];
    int y0 = corners->corners[2 * i + 1];
...
static INLINE void compute_flow_error(const uint8_t *ref, const uint8_t *src,
  ...
  for (int i = 0; i < DISFLOW_PATCH_SIZE; ++i) {
    for (int j = 0; j < DISFLOW_PATCH_SIZE; ++j) {
      ...
      int src_px = src[(x + j) + (y + i) * stride] << 3;
