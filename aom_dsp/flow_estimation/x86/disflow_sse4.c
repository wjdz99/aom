...
static INLINE void getCubicKernel_int(double x, int16_t *kernel) {
  ...
  kernel[0] = (int16_t)rint(k0 * (1 << DISFLOW_INTERP_BITS));
  kernel[1] = (int16_t)rint(k1 * (1 << DISFLOW_INTERP_BITS));
  kernel[2] = (int16_t)rint(k2 * (1 << DISFLOW_INTERP_BITS));
  kernel[3] = (int16_t)rint(k3 * (1 << DISFLOW_INTERP_BITS));
}
...
static INLINE void compute_flow_error(const uint8_t *ref, const uint8_t *frm,
  ...
  for (int i = -1; i < DISFLOW_PATCH_SIZE + 2; ++i) {
    ...
    for (int j = 0; j < DISFLOW_PATCH_SIZE; ++j) {
      ...
      // which is too large.
      int c_value = ROUND_POWER_OF_TWO(getCubicValue_int(arr, h_kernel),
                                       DISFLOW_INTERP_BITS - 6);
  ...
  for (int i = 0; i < DISFLOW_PATCH_SIZE; ++i) {
    ...
    for (int j = 0; j < DISFLOW_PATCH_SIZE; ++j) {
      ...
      int c_warped = ROUND_POWER_OF_TWO(result, round_bits);
      int c_src_px = frm[(x + j) + (y + i) * stride] << 3;
      int c_err = c_warped - c_src_px;
      (void)c_err;
...
void aom_compute_flow_at_point_sse4_1(const uint8_t *frm, const uint8_t *ref,
  ...
  for (int itr = 0; itr < DISFLOW_MAX_ITR; itr++) {
    ...
    // at this point
    double step_u = (M_inv[0] * b[0] + M_inv[1] * b[1]) * DISFLOW_STEP_SIZE;
    double step_v = (M_inv[2] * b[0] + M_inv[3] * b[1]) * DISFLOW_STEP_SIZE;
    *u += step_u;
    *v += step_v;
