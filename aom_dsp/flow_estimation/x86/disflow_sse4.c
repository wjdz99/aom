...
static INLINE void sobel_filter_x(const uint8_t *src, int src_stride,
  ...
  for (int y = 0; y < im_h; ++y) {
    ...
    _mm_storeu_si128((__m128i *)im_row, out);

#if CHECK_RESULTS
    // Cross-check
    static const int16_t h_kernel[3] = { 1, 0, -1, 0 };
  ...
  for (int y = 0; y < DISFLOW_PATCH_SIZE; ++y) {
    const int16_t *im_row = im_block + y * im_stride;
    int16_t *dst_row = dst + y * dst_stride;

    __m128i px0 = _mm_loadu_si128((__m128i *)im_row);
    __m128i px1 = _mm_loadu_si128((__m128i *)(im_row + im_stride));
    __m128i px2 = _mm_loadu_si128((__m128i *)(im_row + 2 * im_stride));
...
#if CHECK_RESULTS
    static const int16_t v_kernel[3] = { 1, 2, 1, 0 };
    for (int x = 0; x < DISFLOW_PATCH_SIZE; ++x) {
      int sum = 0;
      for (int k = 0; k < taps; ++k) {
