...
*/
void aom_compute_mean_stddev_sse4_1(const unsigned char *frame, int stride,
  ...
  for (int i = 0; i < MATCH_SZ; ++i) {
    const __m128i v = _mm_loadu_si128((__m128i *)&frame[i * stride]);
  ...
  // Start by doing 4 parallel sums
  const __m128i partial_sum =
      _mm_add_epi32(_mm_cvtepu16_epi32(sum_vec),
                    _mm_cvtepu16_epi32(_mm_srli_si128(sum_vec, 8)));
  const int sum =
      _mm_extract_epi32(partial_sum, 0) + _mm_extract_epi32(partial_sum, 1) +
      _mm_extract_epi32(partial_sum, 2) + _mm_extract_epi32(partial_sum, 3);
...
double aom_compute_correlation_sse4_1(const unsigned char *frame1, int stride1,
  ...
  for (int i = 0; i < MATCH_SZ; ++i) {
    const __m128i v1 = _mm_loadu_si128((__m128i *)&frame1[i * stride1]);
    const __m128i v2 = _mm_loadu_si128((__m128i *)&frame2[i * stride2]);
