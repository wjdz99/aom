...
//
// This implementation takes O(minpts^2) time, due to the need to compare each
// sample against all previous samples, but this is okay since minpts is
// very small. The main alternative is the Fisher-Yates shuffle, which requires
// building an explicit index list to shuffle, and so takes O(npoints + minpts)
// time.
static void get_rand_indices(int npoints, int minpts, int *indices,
                             unsigned int *seed) {
  assert(minpts < npoints);
  indices[0] = (int)lcg_randint(seed, npoints);
  for (int i = 1; i < minpts; i++) {
  resample:
    int index = (int)lcg_randint(seed, npoints);
    for (int j = 0; j < i; j++) {
      if (index == indices[j]) {
