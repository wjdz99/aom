...
*/
void aom_compute_mean_stddev_c(const unsigned char *frame, int stride, int x,
...
static int determine_correspondence(const unsigned char *src,
  ...
  for (int i = 0; i < num_src_corners; i++) {
    ...
    aom_compute_mean_stddev(src, src_stride, src_x, src_y, &point->mean,
  ...
  for (int j = 0; j < num_ref_corners; j++) {
    ...
    point->y = ref_y;
    point->best_match_corr = THRESHOLD_NCC;
    aom_compute_mean_stddev(ref, ref_stride, ref_x, ref_y, &point->mean,
                            &point->one_over_stddev);
    ref_point_count++;
  }
