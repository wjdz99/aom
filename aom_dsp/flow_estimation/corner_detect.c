...
void compute_corner_list(const ImagePyramid *pyr, CornerList *corners) {
  ...
  int *scores = NULL;
  int num_corners;
  xy *const img_corners_xy = aom_fast9_detect_nonmax(
  ...
  if (num_corners <= 0) {
    ...
    memcpy(corners->corners, img_corners_xy,
           sizeof(*img_corners_xy) * num_corners);
    ...
    for (int i = 0; i < num_corners; i++) {
      if (scores[i] > threshold) {
        ...
        corners->corners[2 * copied_corners + 0] = img_corners_xy[i].x;
        corners->corners[2 * copied_corners + 1] = img_corners_xy[i].y;
        copied_corners += 1;
      }
    }
