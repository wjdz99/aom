...
// C reference code that mimics the YUV assembly.
static __inline void YuvJPixel(uint8 y, uint8 u, uint8 v, uint8 *b, uint8 *g,
                               uint8 *r) {
...
void J422ToARGBRow_C(const uint8 *src_y, const uint8 *src_u, const uint8 *src_v,
  ...
  int x;
  for (x = 0; x < width - 1; x += 2) {
    YuvJPixel(src_y[0], src_u[0], src_v[0], rgb_buf + 0, rgb_buf + 1,
              rgb_buf + 2);
    rgb_buf[3] = 255;
    YuvJPixel(src_y[1], src_u[0], src_v[0], rgb_buf + 4, rgb_buf + 5,
              rgb_buf + 6);
    rgb_buf[7] = 255;
    src_y += 2;
    src_u += 1;
...
// Blend 2 rows into 1.
static void HalfRow_C(const uint8 *src_uv, int src_uv_stride, uint8 *dst_uv,
                      int pix) {
  int x;
  for (x = 0; x < pix; ++x) {
