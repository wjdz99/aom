...
bool Track::Write(IMkvWriter *writer) const {
  ...
  if (!strcmp(codec_id_, Tracks::kAv1CodecId) && !codec_private_) return false;
...
VideoTrack::VideoTrack(unsigned int *seed)
    : Track(seed), display_height_(0), display_width_(0), pixel_height_(0),
      ...
      alpha_mode_(0), width_(0), colour_space_(NULL), colour_(NULL),
      projection_(NULL) {}
...
Segment::Segment()
    : chunk_count_(0), chunk_name_(NULL), chunk_writer_cluster_(NULL),
      ...
      estimate_file_duration_(false), payload_pos_(0), size_position_(0),
