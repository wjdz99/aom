...
static int obu_read_temporal_unit(FILE *infile, uint8_t **buffer, size_t *bytes_read,
                          size_t *buffer_size) {
    size_t ret;
    ...
    *bytes_read = 0;
    while (!feof(infile)) {
        ...
        // augment the buffer to just contain the next size field and the first byte of the header
        ...
        *bytes_read += obu_length_header_size;
        ...
        if (feof(infile) || ((data[4] >> 3) & 0xF) == 2) {
            // Stop when a temporal delimiter is found
            fprintf(stderr, "Found temporal delimiter\n");
            *bytes_read -= obu_length_header_size; // prevent the decoder to start decoding a second frame from this buffer
            break;
        }
...
static int read_frame(struct AvxDecInputContext *input, uint8_t **buf,
  ...
  switch (input->aom_input_ctx->file_type) {
    ...
    case FILE_TYPE_IVF:
      ...
      case FILE_TYPE_OBU:
          return obu_read_temporal_unit(input->aom_input_ctx->file, buf, bytes_in_buffer, buffer_size);
#endif
      default: return 1;
  }
}
