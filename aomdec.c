...
#if CONFIG_FILM_GRAIN
    if (got_data && grain_params.apply_grain) // will depend on a flag from the bistream
    {
      if (grain_img && (img->d_w != grain_img->d_w || img->d_h != grain_img->d_h ||
      ...
      if (!grain_img) {
        grain_img =
        aom_img_alloc(NULL, img->fmt, img->d_w, img->d_h, 16);
