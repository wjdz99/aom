...
AV1Decoder::AV1Decoder()
    : showPadding(false), reader(NULL), info(NULL), decoder(NULL), image(NULL), frame(0) {}
...
int AV1Decoder::getWidth() const { return info->frame_width + 2*getWidthPadding(); }

int AV1Decoder::getWidthPadding() const {
  return showPadding ? AOMMAX(info->frame_width + 16, ALIGN_POWER_OF_TWO(info->frame_width, 6)) - info->frame_width : 0;
}

int AV1Decoder::getHeight() const { return info->frame_height + 2*getHeightPadding(); }

int AV1Decoder::getHeightPadding() const {
  return showPadding ? AOMMAX(info->frame_height + 16, ALIGN_POWER_OF_TWO(info->frame_height, 6)) - info->frame_height : 0;
}
...
AnalyzerFrame::AnalyzerFrame(const bool bit_accounting)
  ...
  viewMenu = new wxMenu();
+  viewMenu->Append(wxID_PADDING, _("Toggle padding\tCtrl-p"), _("Show padding"));
