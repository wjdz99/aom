...
AV1Decoder::AV1Decoder() : reader(NULL), info(NULL), decoder(NULL),
  showPadding(false), image(NULL), frame(0) {
}

AV1Decoder::~AV1Decoder() {
}

void AV1Decoder::togglePadding() {
...
int AV1Decoder::getWidthPadding() const {
  return showPadding ? AOMMAX(info->frame_width + 16,
    ALIGN_POWER_OF_TWO(info->frame_width, 6)) - info->frame_width : 0;
}
...
int AV1Decoder::getHeightPadding() const {
  return showPadding ? AOMMAX(info->frame_height + 16,
    ALIGN_POWER_OF_TWO(info->frame_height, 6)) - info->frame_height : 0;
}
...
bool AV1Decoder::setInspectionCallback() {
  ...
  return
   aom_codec_control(&codec, AV1_SET_INSPECTION_CALLBACK, &ii) == AOM_CODEC_OK;
}
...
int AnalyzerPanel::getDisplayWidth() const {
...
bool AnalyzerPanel::open(const wxString &path) {
  ...
  if (bit_accounting) {
    bpp_q3 =
     (double *)malloc(sizeof(*bpp_q3)*decoder.getWidth()*decoder.getHeight());
    if (bpp_q3 == NULL) {
...
int AnalyzerPanel::getZoom() const {
  return zoom;
}
...
AnalyzerFrame::AnalyzerFrame(const bool bit_accounting) : wxFrame(NULL,
 wxID_ANY, _("AV1 Stream Analyzer"), wxDefaultPosition, wxDefaultSize,
...
void AnalyzerFrame::onQuit(wxCommandEvent &WXUNUSED(event)) {
  Close(true);
}
...
void AnalyzerFrame::onAbout(wxCommandEvent& WXUNUSED(event)) {
  ...
  info.SetDescription(_("This program implements a bitstream analyzer for AV1"));
  info.SetCopyright(wxT("(C) 2017 Alliance for Open Media <negge@mozilla.com>"));
  wxAboutBox(info);
}
...
static const wxCmdLineEntryDesc CMD_LINE_DESC [] = {
  ...
  { wxCMD_LINE_SWITCH, _("a"), _("bit-accounting"),
...
bool Analyzer::OnCmdLineParsed(wxCmdLineParser &parser) {
  ...
  if (bit_accounting && !CONFIG_ACCOUNTING) {
    fprintf(stderr, "Bit accounting support not found.  "
