...
static int set_temporal_layer_pattern(int layering_mode, int frame_cnt,
  ...
  int layer_flags = AOM_EFLAG_NO_REF_LAST2 | AOM_EFLAG_NO_REF_LAST3 |
      AOM_EFLAG_NO_REF_ARF | AOM_EFLAG_NO_REF_BWD | AOM_EFLAG_NO_REF_ARF2;
...
int main(int argc, char **argv) {
  ...
  while (frame_avail || got_data) {
    ...
    // buffer index.
    flags = set_temporal_layer_pattern(layering_mode, frame_cnt,
                                       &layer_id, &svc_ref_map_idx);
    aom_codec_control(&codec, AOME_SET_LAYER_ID, &layer_id);
    aom_codec_control(&codec, AOME_SET_SVC_REF_MAP_IDX, &svc_ref_map_idx);
