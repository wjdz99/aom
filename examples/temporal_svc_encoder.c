...
static void set_temporal_layer_pattern(int layering_mode, int *layer_flags, 
...
int main(int argc, char **argv) {
  ...
  if (argc < min_args) {
    die("Usage: %s <infile> <outfile> <codec_type(aom)> <width> <height> "
  ...
  set_temporal_layer_pattern(layering_mode, layer_flags, &flag_periodicity);
...
// Initialize codec.
  ...
  aom_codec_control(&codec, AOME_SET_NUMBER_SPATIAL_LAYERS, ss_number_layers);
  aom_codec_control(&codec, AOME_SET_NUMBER_TEMPORAL_LAYERS, ts_number_layers);
  aom_codec_control(&codec, AOME_SET_SVC_PARAMS, &svc_params);
  ...
  while (frame_avail || got_data) {
    ...
    if (ts_number_layers == 2) {
      if (frame_cnt % 2 != 0) layer_id.temporal_layer_id = 1;
    } else if (ts_number_layers == 3) {
      if (frame_cnt % 4 == 0)
        layer_id.temporal_layer_id = 0;
    ...
    aom_codec_control(&codec, AOME_SET_LAYER_ID, &layer_id);
    ...
    while ((pkt = aom_codec_get_cx_data(&codec, &iter))) {
      ...
      switch (pkt->kind) {
        case AOM_CODEC_CX_FRAME_PKT:
          for (i = layer_id.temporal_layer_id;
               i < ts_number_layers; ++i) {
