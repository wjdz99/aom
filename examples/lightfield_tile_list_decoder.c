...
static void write_tile_yuv1d(const aom_image_t *img, FILE *file,
...
int main(int argc, char **argv) {
  ...
  for (n = 0; n < num_tile_lists; n++) {
    ...
    if (output_format == YUV1D) {
      ...
      write_tile_yuv1d(img, outfile, output_frame_width_in_tiles, tile_count,
                       tile_width, tile_height);
    } else if (output_format == YUV) {
      aom_img_write(img, outfile);
    } else {  // NV12 output format
