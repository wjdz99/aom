...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < num_references; ++i) {
    ...
    if (i == 0) {
      av1_color_format_t color_format;

      if (aom_codec_control(&codec, AV1D_GET_COLOR_FORMAT, &color_format))
        die_codec(&codec, "Failed to get the color format");

      // Allocate memory to store decoded references.
      aom_img_fmt_t ref_fmt =
          aom_get_img_format(&color_format, CONFIG_LOWBITDEPTH);
