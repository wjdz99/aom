...
static void set_layer_pattern(
  ...
  *use_svc_control = 1;
  ...
  for (i = 0; i < INTER_REFS_PER_FRAME; i++) {
    ...
    ref_frame_config->ref_idx_arf[i] = i;
    ref_frame_config->reference_arf[i] = 0;
  }
  for (i = 0; i < REF_FRAMES; i++) {
    ref_frame_config->refresh[i] = 0;
    ref_frame_config->refresh_arf[i] = 0;
  }
      ...
      if (use_rps_example == 0) {
        ...
        if (gop_interval > 1) {
          ...
          if ((frame_cnt  - 1) % gop_interval == 0) {
            ref_frame_config->refresh[0] = 1;
            // For ARF
            ref_frame_config->refresh_arf[6] = 1;
            ref_frame_config->reference_arf[SVC_LAST_FRAME] = 1;
            ref_frame_config->reference_arf[SVC_GOLDEN_FRAME] = 1;
            ref_frame_config->reference_arf[SVC_ALTREF_FRAME] = 1;
