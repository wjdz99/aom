...
static void writeout_stats(const aom_codec_cx_pkt_t *pkt,
  ...
  for (int sl = spatial_layer_id; sl < ss_layers_write; ++sl) {
    for (int tl = temporal_layer_id; tl < ts_number_layers; ++tl) {
      ...
      if (output_obu) {
        fwrite(pkt->data.frame.buf + shift, 1, frame_size, obu_files[j]);
      } else {
        aom_video_writer_write_frame(
            outfile[j],
            reinterpret_cast<const uint8_t *>(pkt->data.frame.buf + shift),
            frame_size, pts);
      }
  ...
  *got_data = 1;
  ...
  if (output_obu) {
    fwrite(pkt->data.frame.buf + shift, 1, frame_size, total_layer_obu_file);
  } else {
    aom_video_writer_write_frame(
        total_layer_file,
        reinterpret_cast<const uint8_t *>(pkt->data.frame.buf + shift),
...
int main(int argc, const char **argv) {
  ...
  // For testing fixed QP.
  /*
  if (cfg.g_lag_in_frames > 0) {
    svc_params.max_quantizers[0] = 24;
    svc_params.min_quantizers[0] = 24;
  }
  */
  ...
  while (frame_avail || got_data) {
    ...
    int slx_max = 1;
    int gop_interval = cfg.g_lag_in_frames;
    if (frame_cnt >= gop_interval - 1) slx_max = ss_number_layers;
    for (int slx = 0; slx < slx_max; slx++) {
      ...
      if (app_input.layering_mode >= 0) {
        ...
        set_layer_pattern(
            ...
            (app_input.layering_mode == 10), app_input.speed, gop_interval);
        ...
        switch (pkt->kind) {
          case AOM_CODEC_CX_FRAME_PKT:
            if (pkt->data.frame.sz > pkt->data.frame.vis_frame_size &&
              ...
              for (int sl = 0; sl < ss_number_layers; sl++) {
                int size = pkt->data.frame.frame_size_sl_inv[sl];
                tot_frame_size += size;
              ...
              int size = pkt->data.frame.vis_frame_size;
              tot_frame_size += size;
              assert(tot_frame_size == (int)pkt->data.frame.sz);
