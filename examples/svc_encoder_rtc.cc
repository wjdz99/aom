...
static void write_literal(struct aom_write_bit_buffer *wb, int64_t data,
                          int bits, int offset = 0) {
  const int64_t to_write = data - offset;
  const int64_t max = ((int64_t)1 << bits);
  if (to_write < 0 || to_write >= max) {
    die("Invalid data, value %ld out of range [%d, %ld]\n", data, offset,
        max + offset);
  }
