...
int main(int argc, const char **argv) {
  ...
  while (frame_avail || got_data) {
    ...
    for (int slx = 0; slx < ss_number_layers; slx++) {
      ...
      // the same value of 1 for this example encoder. Set it once per
      // superframe, so only once on the base layer (slx == 0), and just before
      // the codec_encode() call. If set this will override the duration passed
      // via codec_encode(). This can also be set once for the sequence, if the
      // prediction pattern repeats, but it has to be after SET_SVC_PARAMS
      // control (which sets the number of spatial layers).
