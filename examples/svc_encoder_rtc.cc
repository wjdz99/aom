...
static void add_multilayer_metadata(
  ...
  uint8_t data[300000];
  struct aom_write_bit_buffer buffer = { data, 0 };
  ...
  if (multilayer->layers.empty()) {
    ...
    die("Invalid multilayer metadata, too many layers (%zu > %d)\n",
        multilayer->layers.size(), MAX_NUM_SPATIAL_LAYERS);
  }
  write_literal(&buffer, multilayer->layers.size() - 1, 2);
  ...
  for (int i = 0; i < multilayer->layers.size(); ++i) {
    ...
    write_literal(&buffer, i, 2);  // ml_spatial_id
    ...
    if (i == 0) {
      same_view_type = true;
      init_view_type = layer.layer_view_type;
    } else {
      if (layer.layer_view_type != init_view_type) {
      ...
      if (layer.layer_type == libaom_examples::MULTIALYER_LAYER_TYPE_ALPHA &&
        ...
        if (alpha_info.alpha_use_idc == libaom_examples::ALPHA_STRAIGHT) {
          ...
          if (!alpha_info.label_type_id.empty()) {
            const int num_values = std::abs(alpha_info.alpha_transparent_value -
            ...
            if (!alpha_info.label_type_id.empty() &&
              ...
              die("Invalid multilayer metadata, label_type_id size must be "
                  ...
                  "alpha_transparent_value and alpha_opaque_value (expected %d "
                  "values, found %zu values)\n",
                  num_values, alpha_info.label_type_id.size());
            }
        ...
        if (depth_info.depth_representation_type == 3) {
          ...
          if (depth_info.depth_nonlinear_representation_model.empty() ||
            ...
            die("Invalid multilayer metadata, if depth_nonlinear_precision "
                ...
                "%d elements, found %zu elements\n",
                1 << 6, depth_info.depth_nonlinear_representation_model.size());
          }
          write_literal(
              &buffer,
