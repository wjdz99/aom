...
struct ParsedValue {
  ...
  long IntValueOrDie(long min, long max, int line_idx) {
    switch (type) {
      case Type::kInt:
        if (int_value < min || int_value > max) {
          fprintf(stderr, "Integer value %d out of range [%d, %d] at line %d\n",
                  min, max, line_idx);
          exit(EXIT_FAILURE);
        }
        return int_value;
 ...
 * if 'float_value' is not nullptr, it's set to the float value of the line, 0
 * zero if the line doesn't contain a number
 * else if 'int_value' is not nullptr, it's set to the integer value of the
 * line, or to 0 if the line doesn't contain a number
 */
...
std::vector<T> parse_integer_list(std::fstream &file, int min_indent,
  ...
  while (parse_line(file, min_indent, /*is_list=*/true, &indent,
    ...
    if (!field_name.empty()) {
      ...
      result.push_back(value.IntValueOrDie((long)std::numeric_limits<T>::min(),
...
ColorProperties parse_color_properties(std::fstream &file, int min_indent,
  ...
  while (parse_line(file, min_indent, /*is_list=*/false, &indent,
    ...
    if (field_name == "color_range") {
      color.color_range = value.IntValueOrDie(/*min=*/0, /*max=*/1, *line_idx);
    } else if (field_name == "color_primaries") {
      color.color_primaries =
          value.IntValueOrDie(/*min=*/0, /*max=*/255, *line_idx);
...
AlphaInformation parse_multilayer_layer_alpha(std::fstream &file,
  ...
  if (alpha_info.alpha_bit_depth == 0) {
    fprintf(stderr, "Error: Missing alpha_bit_depth field for alpha info\n");
    exit(EXIT_FAILURE);
  }
  const int alpha_max = (1 << alpha_info.alpha_bit_depth) - 1;
  if (alpha_info.alpha_transparent_value < 0 ||
      alpha_info.alpha_transparent_value > alpha_max) {
    fprintf(stderr, "Error: alpha_transparent_value %d out of range [0, %d]\n",
            alpha_max);
    exit(EXIT_FAILURE);
  }
  if (alpha_info.alpha_opaque_value < 0 ||
      alpha_info.alpha_opaque_value > alpha_max) {
    fprintf(stderr, "Error: alpha_opaque_value %d out of range [0, %d]\n",
            alpha_max);
    exit(EXIT_FAILURE);
  }
  if ((alpha_info.alpha_use_idc == ALPHA_SEGMENTATION) !=
      (!alpha_info.label_type_id.empty())) {
    fprintf(
        stderr,
        "Error: label_type_id must be set if and only if alpha_use_idc is %d\n",
        ALPHA_SEGMENTATION);
    exit(EXIT_FAILURE);
  }
  if (!alpha_info.label_type_id.empty() &&
      alpha_info.label_type_id.size() !=
          (std::abs(alpha_info.alpha_opaque_value -
                    alpha_info.alpha_transparent_value) +
           1)) {
    fprintf(stderr,
            "Error: label_type_id size must be "
...
float depth_representation_element_to_float(
  ...
  // (−1)^s*2^(e−31) * (1+m÷2^n). Otherwise (e is equal to 0), x is set equal
  // to (−1)^s*2^−(30+v)*n.
  if (e.exponent > 0) {
...
DepthInformation parse_multilayer_layer_depth(std::fstream &file,
  ...
  while (parse_line(file, min_indent, /*is_list=*/false, &indent,
    ...
    if (field_name == "z_near") {
      ...
      depth_info.depth_representation_type =
          value.IntValueOrDie(/*min=*/0, /*max=*/15, *line_idx);
      ...
      depth_info.depth_nonlinear_precision =
          value.IntValueOrDie(/*min=*/8, /*max=*/23, *line_idx);
  ...
  if (depth_info.depth_nonlinear_precision == 0) {
    fprintf(stderr,
            "Error: Missing depth_nonlinear_precision field for depth info\n");
    exit(EXIT_FAILURE);
  }
    ...
    if (has_list_prefix) {
      ...
      if (!layers.empty()) {
        if (layer_has_alpha !=
          ...
          fprintf(stderr,
                  "Error: alpha info can only be set if layer_type is %d and "
                  "layer_metadata_scpoe is >= %d\n",
        ...
        if (layer_has_depth !=
            (layers.back().layer_type == MULTILAYER_LAYER_TYPE_DEPTH &&
             layers.back().layer_metadata_scope >= SCOPE_GLOBAL)) {
          fprintf(stderr,
                  "Error: depth info can only be set if layer_type is %d and "
                  "layer_metadata_scpoe is >= %d\n",
...
MultilayerMetadata parse_multilayer_metadata(std::fstream &file) {
  ...
  while (parse_line(file, /*min_indent=*/0, /*is_list=*/false, &indent,
    ...
    if ((field_name == "use_case")) {
      multilayer.use_case = (MultilayerUseCase)value.IntValueOrDie(
...
std::string format_depth_representation_element(
  ...
  if (!element.second) {
    ...
    return std::to_string(
               depth_representation_element_to_float(element.first)) +
...
void validate_multilayer_metadata(const MultilayerMetadata &multilayer) {
  ...
  for (int i = 0; i < multilayer.layers.size(); ++i) {
      ...
      case MULTILAYER_USE_CASE_420_444:
        if (!same_view_type) {
          fprintf(stderr,
                  ...
                  "type for layer %d (zero-based index)\n",
    ...
    if ((layer.layer_dependency_idc >> i) > 0) {
      fprintf(stderr,
              "Error: layer_dependency_idc of layer %d (zero-based index) must "
              "be in [0, %d], "
              "found %d for layer %d (zero-based index)\n",
              i, (1 << i), layer.layer_dependency_idc, i);
      exit(EXIT_FAILURE);
    }
    if ((layer.layer_type == MULTILAYER_LAYER_TYPE_ALPHA ||
      ...
      fprintf(stderr,
              "Error: alpha for depth layers cannot have "
