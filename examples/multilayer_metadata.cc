...
class ParsedValue {
 public:
  ...
  void SetIntegerValue(long v) {
    type = Type::kInteger;
    long_value = v;
  }

  void SetFloatingPointValue(double v) {
    type = Type::kFloatingPoint;
    double_value = v;
  }

  void Clear() { type = Type::kNone; }
  ...
  double ValueAsFloatingPoint(int line_idx) {
    ...
    return (type == Type::kFloatingPoint) ? double_value : (double)long_value;
  }

  long IntegerValueInRange(long min, long max, int line_idx) {
    switch (type) {
      case Type::kInteger:
        if (long_value < min || long_value > max) {
          fprintf(stderr,
                  "Integer value %ld out of range [%ld, %ld] at line %d\n",
                  long_value, min, max, line_idx);
          exit(EXIT_FAILURE);
        }
        return long_value;
