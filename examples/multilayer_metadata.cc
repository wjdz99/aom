...
class ParsedValue {
 public:
  ...
  double ValueAsFloatingPoint(int line_idx) {
    ...
    return (type_ == Type::kFloatingPoint) ? double_value_
                                           : static_cast<double>(int_value_);
  }

  int64_t IntegerValueInRange(int64_t min, int64_t max, int line_idx) {
    switch (type_) {
      case Type::kInteger:
  ...
  while (prev_file_position = file.tellg(), std::getline(file, line)) {
    ...
    if (!value_str.empty()) {
      ...
      if (line.find('.') != std::string::npos) {
        value->SetFloatingPointValue(strtod(&line[colon_pos + 1], &endptr));
        if (*endptr != '\0') {
          fprintf(stderr,
        ...
        value->SetIntegerValue(strtol(&line[colon_pos + 1], &endptr, 10));
        if (*endptr != '\0') {
          fprintf(stderr,
...
std::vector<T> parse_integer_list(std::fstream &file, int min_indent,
  ...
  while (parse_line(file, min_indent, /*is_list=*/true, &indent,
    ...
    if (!field_name.empty()) {
      ...
      result.push_back((T)value.IntegerValueInRange(
          static_cast<int64_t>(std::numeric_limits<T>::min()),
          static_cast<int64_t>(std::numeric_limits<T>::max()), *line_idx));
    }
  }
  return result;
}
  ...
  while (parse_line(file, min_indent, /*is_list=*/false, &indent,
    ...
    if (field_name == "color_range") {
      ...
      color.color_primaries =
          value.IntegerValueInRange(/*min=*/0, /*max=*/255, *line_idx);
...
AlphaInformation parse_multilayer_layer_alpha(std::fstream &file,
  ...
  while (parse_line(file, min_indent, /*is_list=*/false, &indent,
    ...
    if (field_name == "alpha_use_idc") {
      alpha_info.alpha_use_idc =
          (AlphaUse)value.IntegerValueInRange(/*min=*/0, /*max=*/7, *line_idx);
    } else if (field_name == "alpha_bit_depth") {
      alpha_info.alpha_bit_depth =
          value.IntegerValueInRange(/*min=*/8, /*max=*/15, *line_idx);
    } else if (field_name == "alpha_clip_idc") {
      alpha_info.alpha_clip_idc =
          value.IntegerValueInRange(/*min=*/0, /*max=*/3, *line_idx);
      ...
      alpha_info.alpha_transparent_value = value.IntegerValueInRange(
          std::numeric_limits<uint16_t>::min(),
          std::numeric_limits<uint16_t>::max(), *line_idx);
    } else if (field_name == "alpha_opaque_value") {
      // At this point we may not have parsed 'alpha_bit_depth' yet, so the
      // exact range is checked later.
      alpha_info.alpha_opaque_value = value.IntegerValueInRange(
          std::numeric_limits<uint16_t>::min(),
          std::numeric_limits<uint16_t>::max(), *line_idx);
  ...
  const int alpha_range =
      (std::abs(static_cast<int>(alpha_info.alpha_opaque_value) -
...
double depth_representation_element_to_double(
  ...
  if (e.exponent > 0) {
    return (e.sign_flag ? -1 : 1) *
           std::pow(2.0, static_cast<int>(e.exponent) - 31) *
           (1 + static_cast<double>(e.mantissa) /
                    (static_cast<int64_t>(1) << e.mantissa_len));
  } else {
    return (e.sign_flag ? -1 : 1) * e.mantissa *
           std::pow(2.0, -30 + static_cast<int>(e.mantissa_len));
  }
}
...
DepthRepresentationElement double_to_depth_representation_element(double f) {
  ...
  const DepthRepresentationElement element = { sign,
                                               static_cast<uint8_t>(exp + 31),
...
void validate_layer(const LayerMetadata &layer, bool layer_has_alpha,
...
std::vector<LayerMetadata> parse_multilayer_layer_metadata(std::fstream &file,
  ...
  while (parse_line(file, min_indent, /*is_list=*/true, &indent,
    ...
    if ((field_name == "layer_type")) {
      layer->layer_type = (LayerType)value.IntegerValueInRange(
          /*min=*/0, /*max=*/31, *line_idx);
    } else if ((field_name == "luma_plane_only_flag")) {
      layer->luma_plane_only_flag =
          value.IntegerValueInRange(/*min=*/0, /*max=*/1, *line_idx);
    } else if ((field_name == "layer_view_type")) {
      layer->layer_view_type = (MultilayerViewType)value.IntegerValueInRange(
      ...
      layer->layer_dependency_idc =
          value.IntegerValueInRange(/*min=*/0, /*max=*/7, *line_idx);
    } else if ((field_name == "layer_metadata_scope")) {
      layer->layer_metadata_scope =
          (MultilayerMetadataScope)value.IntegerValueInRange(
              /*min=*/0, /*max=*/3, *line_idx);
...
MultilayerMetadata parse_multilayer_metadata(std::fstream &file) {
  ...
  while (parse_line(file, /*min_indent=*/0, /*is_list=*/false, &indent,
    ...
    if ((field_name == "use_case")) {
      multilayer.use_case = (MultilayerUseCase)value.IntegerValueInRange(
