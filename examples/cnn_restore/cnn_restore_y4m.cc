...
static int restore_cnn_img_tflite_lowbd(
    std::unique_ptr<tflite::Interpreter> &interpreter, const uint8_t *dgd,
...
static int restore_cnn_img_tflite_highbd(
    std::unique_ptr<tflite::Interpreter> &interpreter, const uint16_t *dgd,
    int width, int height, int dgd_stride, uint16_t *rst, int rst_stride,
    int bit_depth) {
  // Prepare input.
