...
const map_entry prediction_mode_map[] = {
  ...
  ENUM(D135_PRED),       ENUM(D113_PRED),
  ENUM(D157_PRED),       ENUM(D203_PRED),
  ENUM(D67_PRED),        ENUM(SMOOTH_PRED),
  ENUM(SMOOTH_V_PRED),   ENUM(SMOOTH_H_PRED),
  ENUM(PAETH_PRED),      ENUM(NEARESTMV),
  ENUM(NEARMV),          ENUM(GLOBALMV),
  ENUM(NEWMV),           ENUM(NEAREST_NEARESTMV),
  ENUM(NEAR_NEARMV),     ENUM(NEAREST_NEWMV),
  ENUM(NEW_NEARESTMV),
#if !CONFIG_NEW_INTER_MODES
  ENUM(NEAR_NEWMV),      ENUM(NEW_NEARMV),
#endif
  ENUM(GLOBAL_GLOBALMV), ENUM(NEW_NEWMV),
