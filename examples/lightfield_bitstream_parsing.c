...
void process_tile_list(
    const TILE_LIST_INFO* tiles, int num_tiles, aom_codec_pts_t tl_pts,
    unsigned char** frames, const size_t* frame_sizes,
    aom_codec_ctx_t* codec, unsigned char* tl_buf, AvxVideoWriter* writer) {
...
int main(int argc, char **argv) {
  ...
  while ((nread = getline(&line, &nalloc, tile_list_fptr)) > 0) {
    if (line[0] == 'F' || num_tiles >= MAX_TILES) {
      ...
      if (num_tiles > 0) {
        process_tile_list(tiles, num_tiles, tl_pts, frames, frame_sizes,
                          &codec, tl_buf, writer);
        ++tl_pts;
      }
      num_tiles = 0;
  ...
  printf("Finished processing tile lists.  Num tile lists: %ld.\n",
         tl_pts - pts);
  free(tl_buf);
  for (int f = 0; f < num_frames; ++f) {
    free(frames[f]);
