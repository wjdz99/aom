...
void process_tile_list(const TILE_LIST_INFO *tiles, int num_tiles,
  ...
  for (i = 0; i <= num_tiles_minus_1; i++) {
    ...
    aom_codec_control_set_int(codec, AV1_SET_DECODE_TILE_ROW, tr);
    aom_codec_control_set_int(codec, AV1_SET_DECODE_TILE_COL, tc);

    aom_codec_err_t aom_status =
        aom_codec_decode(codec, frame, frame_size, NULL);
    if (aom_status) die_codec(codec, "Failed to decode tile.");

    aom_codec_control(codec, AV1D_GET_TILE_DATA, &tile_data);
...
int main(int argc, char **argv) {
  ...
  aom_codec_control_set_uint(&codec, AV1_SET_TILE_MODE, 0);
  ...
  aom_codec_control_set_int(&codec, AV1D_EXT_TILE_DEBUG, 1);
  ...
  // only once.
  {
    ...
    aom_codec_control_set_int(&codec, AV1_SET_DECODE_TILE_ROW, 0);
    aom_codec_control_set_int(&codec, AV1_SET_DECODE_TILE_COL, 0);

    aom_codec_err_t aom_status =
        aom_codec_decode(&codec, frame, frame_size, NULL);
    if (aom_status) die_codec(&codec, "Failed to decode tile.");

    aom_codec_control_get_ptr(&codec, AV1D_GET_FRAME_HEADER_INFO,
                              &frame_header_info, sizeof(frame_header_info));
