...
const TILE_LIST_ENTRY tile_list[2][4] = {
...
int main(int argc, char **argv) {
  ...
  // How many anchor images we have.
  ...
  for (i = 0; i < num_references; ++i) {
    ...
    aom_codec_pts_t pts = aom_video_reader_get_frame_pts(reader);
    ...
    *(uint16_t *)tl++ = tile_count_minus_1;
    ...
    for (i = 0; i < num_tiles; i++) {
      ...
      printf("\n get : frame_cnt: %d;   %d, %p\n ", frame_cnt,
             ...
             (uint8_t *)tile_data.coded_tile_data);
      ...
      *tl++ = tr;
      *tl++ = tc;
      *(uint16_t *)tl++ = tile_data.coded_tile_data_size - 1;
      memcpy(tl, (uint8_t *)tile_data.coded_tile_data,
             tile_data.coded_tile_data_size);
      tile_list_obu_size += 5 + tile_data.coded_tile_data_size;
    }
