...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  unsigned int output_all_layers = (data[IVF_FILE_HDR_SZ - 1] & 0x10) != 0;
  unsigned int set_operating_point = data[IVF_FILE_HDR_SZ - 2] & 0x1F;
  ...
  AOM_CODEC_CONTROL_TYPECHECKED(&codec, AV1D_SET_OPERATING_POINT, set_operating_point);
