...
// lightfield images in each dimension.  The uv_blocksize determines the
// number of reference images used for MCP.  All images within a block
// will use the center image in that block as the reference image for
// MCP.
//
...
static const unsigned long deadline = AOM_DL_GOOD_QUALITY;
...
static aom_fixed_buf_t pass0(aom_image_t *raw, FILE *infile,
  ...
  for (bv = 0; bv < v_blocks; ++bv) {
    for (bu = 0; bu < u_blocks; ++bu) {
      ...
      fseek(infile,
            (block_ref_u + block_ref_v * lf_width) * image_size_bytes,
            SEEK_SET);
      ...
      get_frame_stats(&codec, raw, frame_count, 1,
                      AOM_EFLAG_FORCE_GF | AOM_EFLAG_NO_UPD_ENTROPY,
                      deadline, &stats);
      ref_frame.idx = 0;
    ...
    for (bu = 0; bu < u_blocks; ++bu) {
      ...
      for (v = block_v_min; v < block_v_end; ++v) {
        for (u = block_u_min; u < block_u_end; ++u) {
          ...
          if (aom_codec_control(&codec, AOM_SET_REFERENCE, &ref))
            die_codec(&codec, "Failed to set reference frame");

          fseek(infile, (u + v * lf_width) * image_size_bytes,
                SEEK_SET);
          aom_img_read(raw, infile);
          ++frame_count;
          get_frame_stats(
              &codec, raw, frame_count, 1,
  ...
  // Flush encoder.
  while (get_frame_stats(&codec, NULL, frame_count, 1, 0, deadline,
                         &stats)) {
  }
...
int main(int argc, char **argv) {
  ...
  if (lf_width <= 0 || lf_height <= 0)
    die("Invalid lf_width and/or lf_height: %dx%d", lf_width,
        lf_height);
  if (lf_blocksize <= 0)
    die("Invalid lf_blocksize: %d", lf_blocksize);
  ...
  stats = pass0(&raw, infile, encoder, &cfg, lf_width, lf_height,
                lf_blocksize);
