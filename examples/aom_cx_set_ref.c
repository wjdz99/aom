...
int main(int argc, char **argv) {
  ...
  // is AOM_IMG_FMT_I420.
  if (!aom_img_alloc(&raw, raw_fmt, info.frame_width,
                     info.frame_height, 32)) {
  ...
  flags |= (cfg.g_bit_depth > AOM_BITS_8 || !CONFIG_LOWBITDEPTH) ? AOM_CODEC_USE_HIGHBITDEPTH : 0;
  ...
  while (aom_img_read(&raw, infile)) {
    ...
    encode_frame(&ecodec, frame_to_encode, frame_in, writer, test_decode, &dcodec,
