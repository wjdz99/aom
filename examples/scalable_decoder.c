...
int main(int argc, char **argv) {
  ...
  for (i = 0; i <= si.enhancement_layer_count; i++) {
  ...
  while (!obudec_read_temporal_unit(&obu_ctx, &buf, &bytes_in_buffer,
    ...
    while ((img = aom_codec_get_frame(&codec, &iter)) != NULL) {
      ...
      } else if (img->enhancement_id <= (int)si.enhancement_layer_count) {
        printf("Writing enhancemnt layer %d %d\n", img->enhancement_id,
               frame_cnt);
        aom_img_write(img_shifted, outfile[img->enhancement_id]);
        if (img->enhancement_id == (int)si.enhancement_layer_count)
      ...
      if (img->enhancement_id == (int)si.enhancement_layer_count) ++frame_cnt;
    }
  }
  ...
  for (i = 0; i <= si.enhancement_layer_count; i++) fclose(outfile[i]);
