...
int main(int argc, char **argv) {
  ...
  for (i = 0; i <= si.operating_points_minus1_count; i++) {
  ...
  while (!obudec_read_temporal_unit(&obu_ctx, &buf, &bytes_in_buffer,
    ...
    while ((img = aom_codec_get_frame(&codec, &iter)) != NULL) {
      ...
      } else if (img->spatial_id <= (int)si.operating_points_minus1_count) {
        printf("Writing enhancemnt layer %d %d\n", img->spatial_id,
               frame_cnt);
        aom_img_write(img_shifted, outfile[img->spatial_id]);
        if (img->spatial_id == (int)si.operating_points_minus1_count)
      ...
      if (img->spatial_id == (int)si.operating_points_minus1_count) ++frame_cnt;
    }
  }
  ...
  for (i = 0; i <= si.operating_points_minus1_count; i++) fclose(outfile[i]);
