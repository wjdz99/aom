...
// data (in IVF format), treaing it as a lightfield instead of a video and
// will decode a single lightfield tile.  The tile to decode is specified by
// the tile_u, tile_v, tile_s, tile_t aguments.  The tile_u, tile_v specify
// the image and tile_s, tile_t specify the tile in the image.
...
void usage_exit(void) {
  fprintf(stderr, "Usage: %s <infile> <outfile> <tlie_u> <tile_v> <tile_s> "
          "<tile_t> <lf_blocksize> <lf_width> <lf_height>\n", exec_name);
  exit(EXIT_FAILURE);
}
...
int main(int argc, char **argv) {
  ...
  for (int bv = 0; bv < v_blocks; ++bv) {
    for (int bu = 0; bu < u_blocks; ++bu) {
      ...
      while ((img = aom_codec_get_frame(&codec, &iter)) != NULL) {
        ...
        char name[1024];
        snprintf(name, 1024, "/usr/local/google/home/rover/tmp/vase/ref_%d_%d.yuv", bu, bv);
        printf("rover writing ref image to %s, %d, %d\n", name, img->d_w, img->d_h);
  ...
  aom_codec_err_t aom_status = aom_codec_decode(
      &codec, frame, frame_size, NULL, 0);
  if (aom_status)
    die_codec(&codec, "Failed to decode tile.");
  aom_codec_iter_t iter = NULL;
