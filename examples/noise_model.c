...
void usage_exit(void) {
  fprintf(stderr,
          "Usage: %s --input=<input> --input-denoised=<denoised> --output=<outfile> "
...
static const arg_def_t skip_frames_arg =
    ARG_DEF("h", "skip-frames", 1, "Number of frames to skip (default = 1)");
static const arg_def_t fps_arg =
    ARG_DEF("f", "fps", 1, "Frame rate");
static const arg_def_t input_arg =
    ARG_DEF(NULL, "input", 1, "Input filename");
static const arg_def_t output_noise_arg =
    ARG_DEF("n", "output-noise", 1, "Output noise file");
static const arg_def_t input_denoised_arg =
    ARG_DEF("i", "input-denoised", 1, "Input denoised filename (Y4M) only");
...
int main(int argc, char* argv[]) {
  noise_model_args_t args = { 0, 0, 25, 0, 0, 0,
                              AOM_IMG_FMT_I420, 32, 0, 0, 1 };
  ...
  info.time_base.denominator = args.fps;

  if (info.frame_width <= 0 || info.frame_height <= 0 ||
      (info.frame_width % 2) != 0 || (info.frame_height % 2) != 0) {
    die("Invalid frame size: %dx%d", info.frame_width, info.frame_height);
  }
  if (!aom_img_alloc(&raw, args.img_fmt, info.frame_width,
                     info.frame_height, 1)) {
  ...
  if (args.input_denoised) {
    ...
    if (!denoised_file)
      die("Unable to open input_denoised");
  } else {
  ...
  while (aom_img_read(&raw, infile)) {
    ...
    if (frame_count % args.skip_frames == 0) {
      ...
      if (args.run_flat_block_finder) {
        ...
        num_flat_blocks =
            aom_flat_block_finder_run(
                &block_finder, raw.planes[0], info.frame_width,
                info.frame_height, info.frame_width, flat_blocks);
        fprintf(stdout, "Num flat blocks %d\n", num_flat_blocks);
      }

      const uint8_t* planes[3] = {raw.planes[0], raw.planes[1], raw.planes[2]};
      uint8_t* denoised_planes[3] = {denoised.planes[0], denoised.planes[1], denoised.planes[2]};
      ...
      int64_t cur_timestamp = frame_count * 1000000ULL / args.fps;
      if (status == AOM_NOISE_STATUS_DIFFERENT_NOISE_TYPE) {
        fprintf(stdout, "Noise type is different, updating parameters %ld %ld\n",
                prev_timestamp, cur_timestamp);
        aom_film_grain_t grain;
        aom_noise_model_get_grain_parameters(&noise_model, &grain);
        aom_film_grain_table_append(&grain_table, prev_timestamp, cur_timestamp, &grain);
        aom_noise_model_swap(&noise_model);
        prev_timestamp = cur_timestamp;
      }
  ...
  if (args.output_noise) {
    ...
    if (AOM_CODEC_OK != aom_film_grain_table_write(&grain_table, args.output_noise, &error_info)) {
