 ...
 *
 * The --output-noise file can be passed as input to the encoder (in aomenc
 * this is done through the "--film-grain-table" parameter).
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static const arg_def_t fps_arg = ARG_DEF("f", "fps", 1, "Frame rate");
static const arg_def_t input_arg = ARG_DEF(NULL, "input", 1, "Input filename");
...
int main(int argc, char *argv[]) {
  noise_model_args_t args = {
    0, 0, 25, 0, 0, 0, AOM_IMG_FMT_I420, 32, 0, 0, 1
  };
  ...
  if (args.input_denoised) {
    ...
    if (!denoised_file) die("Unable to open input_denoised");
  } else {
  ...
  while (aom_img_read(&raw, infile)) {
    ...
    if (frame_count % args.skip_frames == 0) {
      ...
      if (status == AOM_NOISE_STATUS_DIFFERENT_NOISE_TYPE) {
        fprintf(stdout,
                "Noise type is different, updating parameters %" PRId64
                " %" PRId64 "\n",
  ...
  if (args.output_noise) {
    ...
    if (AOM_CODEC_OK != aom_film_grain_table_write(
                            &grain_table, args.output_noise, &error_info)) {
