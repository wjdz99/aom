...
unsigned char tmpbuffer[2 * 1280 * 720 * 12];

void usage_exit(void) {
  fprintf(stderr,
          "Usage: %s <scalabilit_mode> <width> <height> <infile0> <infile1> "
...
int main(int argc, char **argv) {
  ...
  if (scalability_mode == 0) {
    ...
    die("scalability_mode must be 0 (L2T1), 1 (L1T2), or 2 (L2T2)");
  }
