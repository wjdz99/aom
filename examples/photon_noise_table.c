...
/*!\file
 * \brief Creates a film grain table representing realistic noise.
 *
 ...
 * specifies the CICP index of the tone response curve that will be used when
 * encoding the actual image. Example values include 13 for sRGB (the default
 * for this tool), 16 for PQ HDR, and 18 for HLG HDR.
 *
...
static arg_def_t transfer_function_arg =
    ARG_DEF("t", "transfer-function", 1,
            "CICP code of the transfer function (default = 13, i.e. sRGB)");

void usage_exit(void) {
  fprintf(stderr,
          "Usage: %s [--transfer-function=<cicp>] --width=<width> "
...
static const transfer_function_t *find_transfer_function(int cicp);
...
static void parse_args(int argc, char **argv,
  ...
  for (i = 1; i < argc; i += arg.argv_step) {
    ...
    if (arg_match(&arg, &help_arg, argv + i)) {
      ...
      const int cicp = arg_parse_int(&arg);
      photon_noise_args->transfer_function = find_transfer_function(cicp);
    } else {
...
static const transfer_function_t *find_transfer_function(int cicp) {
  ...
  switch (cicp) {
    ...
    default: fatal("unimplemented transfer function %d", cicp);
  }
}
