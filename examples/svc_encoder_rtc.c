...
// Temporal layer pattern configuration.
static int set_temporal_layer_pattern(int layering_mode, int frame_cnt,
                                      aom_svc_layer_id_t *layer_id,
                                      aom_svc_ref_map_idx_t *svc_ref_map_idx) {
  int i;
  ...
  for (i = 0; i < INTER_REFS_PER_FRAME; i++)
    svc_ref_map_idx->ref_map_idx[i] = (i < 3) ? 0 : i;
  ...
  switch (layering_mode) {
    ...
    case 1:
      ...
      if (frame_cnt % 2 == 0) {
        ...
        // No updates on layer 1, only reference LAST.
    ...
    case 2:
      ...
      if (frame_cnt % 4 == 0) {
        ...
        // First top layer: no updates, only reference LAST.
        layer_flags |= AOM_EFLAG_NO_UPD_LAST | AOM_EFLAG_NO_UPD_GF |
                       AOM_EFLAG_NO_UPD_ARF | AOM_EFLAG_NO_REF_GF;
      } else if ((frame_cnt - 2) % 4 == 0) {
        layer_id->temporal_layer_id = 1;
        // Middle layer: update ARF, only reference LAST. Updating ARF
        // corresponds to buffer slot 6.
        layer_flags |=
        ...
        // updated in previous frame.
        layer_flags |= AOM_EFLAG_NO_UPD_LAST | AOM_EFLAG_NO_UPD_GF |
                       AOM_EFLAG_NO_UPD_ARF | AOM_EFLAG_NO_REF_GF;
        svc_ref_map_idx->ref_map_idx[0] = 6;
        svc_ref_map_idx->ref_map_idx[6] = 0;
      }
      break;
...
int main(int argc, char **argv) {
  ...
  while (frame_avail || got_data) {
    ...
    // buffer index.
    flags = set_temporal_layer_pattern(layering_mode, frame_cnt, &layer_id,
                                       &svc_ref_map_idx);
    aom_codec_control(&codec, AV1E_SET_SVC_LAYER_ID, &layer_id);
    aom_codec_control(&codec, AV1E_SET_SVC_REF_MAP_IDX, &svc_ref_map_idx);
