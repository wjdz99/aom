...
static int mode_to_num_layers[6] = { 1, 2, 3, 3, 3, 2};


// For rate control encoding stats.
struct RateControlMetrics {
  // Number of input frames per layer.
  int layer_input_frames[AOM_MAX_LAYERS];
  // Total (cumulative) number of encoded frames per layer.
  int layer_tot_enc_frames[AOM_MAX_LAYERS];
  // Number of encoded non-key frames per layer.
  int layer_enc_frames[AOM_MAX_LAYERS];
  // Framerate per layer layer (cumulative).
  double layer_framerate[AOM_MAX_LAYERS];
...
static void set_rate_control_metrics(struct RateControlMetrics *rc,
  ...
  for (sl = 0; sl < ss_number_layers; ++sl) {
    unsigned int i = sl * ts_number_layers;
    rc->layer_framerate[i] = framerate / ts_rate_decimator[i];
    rc->layer_pfb[i] =
        1000.0 * rc->layer_target_bitrate[i] / rc->layer_framerate[i];
    for (tl = 0; tl < ts_number_layers; ++tl) {
      i += tl;
      if (i > 0) {
        rc->layer_framerate[i] = framerate / ts_rate_decimator[i];
        rc->layer_pfb[i] =
  ...
  switch (layering_mode) {
    ...
    case 5:
      ...
      if (layer_id->spatial_layer_id == 0) {
        ...
        // and GOLDEN to slot 0. Update LAST (slot 3.
        ref_frame_config->ref_idx[0] = 3;
        ref_frame_config->ref_idx[3] = 0;
        ref_frame_config->refresh[3] = 1;   
      }
    ...
    for (sl = 0; sl < ss_number_layers; sl++) {
      ...
      if (use_svc_control)
        aom_codec_control(&codec, AV1E_SET_SVC_REF_FRAME_CONFIG,
                          &ref_frame_config);
      layer = sl * ts_number_layers + layer_id.temporal_layer_id;
      if (frame_avail) ++rc.layer_input_frames[layer];
      ...
      while ((pkt = aom_codec_get_cx_data(&codec, &iter))) {
        ...
        switch (pkt->kind) {
          case AOM_CODEC_CX_FRAME_PKT:
            for (sl = layer_id.spatial_layer_id; sl < ss_number_layers; ++sl) {
              for (tl = layer_id.temporal_layer_id; tl < ts_number_layers;
                   ++tl) {
                unsigned int j = sl * ts_number_layers + tl;
                aom_video_writer_write_frame(outfile[j], pkt->data.frame.buf,
                                             pkt->data.frame.sz, pts);
                ++rc.layer_tot_enc_frames[j];
