  ...
  *use_svc_control = 1;
  ...
  switch (layering_mode) {
    ...
    case 6:
      ...
      // either LAST or GOLDEN, which are always valid references (i.e., not
      // smaller than 1/2 the current frame (since decoder will check if any
      // of the 7 references is valid scale (valid_ref_frame_size()).
      layer_id->temporal_layer_id = 0;
      if (layer_id->spatial_layer_id == 0) {
