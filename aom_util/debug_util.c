...
static int frame_size = 512 * 512;
static uint8_t frame_pre[5][3][512 * 512];  // prediction only
static uint8_t frame_tx[5][3][512 * 512];   // prediction + txfm
static int frame_stride = 512;
static int frame_height = 512;
...
void mismatch_record_block_pre(const uint8_t *src, int src_stride, int plane,
  ...
  int ref_pixel_c = 162;
  int ref_pixel_r = 16;
  if (frame_idx_w == ref_frame_idx && plane == ref_plane && ref_pixel_c >= pixel_c && ref_pixel_c < pixel_c + blk_w && ref_pixel_r >= pixel_r && ref_pixel_r < pixel_r + blk_h) {
