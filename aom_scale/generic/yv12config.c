...
int aom_realloc_lookahead_buffer(YV12_BUFFER_CONFIG *ybf, int width, int height,
  ...
  if (ybf) {
    ...
    const int aligned_width = (width + 7) & ~7;
    const int aligned_height = (height + 7) & ~7;
    const int aligned_width128 = (aligned_width + 127) & ~127;
    const int aligned_height128 = (aligned_height + 127) & ~127;
    int y_stride = ((aligned_width + 2 * border) + 31) & ~31;
    uint64_t yplane_size =
        (aligned_height + 2 * border) * (uint64_t)y_stride + byte_alignment;
    /*for no scale case*/
    if (AOM_BORDER_IN_PIXELS - border) {
      y_stride = (aligned_width128 + 2 * border);
      yplane_size = (aligned_height128 + 2 * border) * (uint64_t)y_stride +
                    byte_alignment;
    }
    const int uv_width = aligned_width >> ss_x;
    const int uv_height = aligned_height >> ss_y;
