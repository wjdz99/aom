...
/* Shift down with rounding for use when n >= 0. Usually value >= 0, but can
 * be used with a negative value if the direction of rounding is acceptable.
 */
...
/* Shift down with rounding for use when n >= 0 (value 64 bit). Usually
 * value >= 0, but can be used with a negative value if the direction of
 * rounding is acceptable.
 */
#define ROUND_POWER_OF_TWO_64(value, n) \
  (((value) + ((((int64_t)1 << (n)) >> 1))) >> (n))
/* Shift down with rounding for signed integers, for use when n >= 0 (value
 * 64 bit)
 */
