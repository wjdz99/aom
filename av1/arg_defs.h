...
struct CodecArgDefs {
  const arg_def_t help;
  const arg_def_t debugmode;
  const arg_def_t outputfile;
  const arg_def_t use_yv12;
  const arg_def_t use_i420;
  const arg_def_t use_i422;
  const arg_def_t use_i444;
  const arg_def_t codecarg;
  const arg_def_t passes;
  const arg_def_t pass_arg;
  const arg_def_t fpf_name;
  const arg_def_t limit;
  const arg_def_t skip;
  const arg_def_t good_dl;
  const arg_def_t rt_dl;
  const arg_def_t quietarg;
  const arg_def_t verbosearg;
  const arg_def_t psnrarg;
  const arg_def_t use_cfg;
  const arg_def_t recontest;
  const arg_def_t framerate;
  const arg_def_t use_webm;
  const arg_def_t use_ivf;
  const arg_def_t use_obu;
  const arg_def_t q_hist_n;
  const arg_def_t rate_hist_n;
  const arg_def_t disable_warnings;
  const arg_def_t disable_warning_prompt;
  const arg_def_t bitdeptharg;
  const arg_def_t inbitdeptharg;
  const arg_def_t input_chroma_subsampling_x;
  const arg_def_t input_chroma_subsampling_y;
  const arg_def_t usage;
  const arg_def_t threads;
  const arg_def_t profile;
  const arg_def_t width;
  const arg_def_t height;
  const arg_def_t forced_max_frame_width;
  const arg_def_t forced_max_frame_height;
#if CONFIG_WEBM_IO
  const arg_def_t stereo_mode;
#endif
  const arg_def_t timebase;
  const arg_def_t global_error_resilient;
  const arg_def_t lag_in_frames;
  const arg_def_t large_scale_tile;
  const arg_def_t monochrome;
  const arg_def_t full_still_picture_hdr;
  const arg_def_t use_16bit_internal;
  const arg_def_t dropframe_thresh;
  const arg_def_t resize_mode;
  const arg_def_t resize_denominator;
  const arg_def_t resize_kf_denominator;
  const arg_def_t superres_mode;
  const arg_def_t superres_denominator;
  const arg_def_t superres_kf_denominator;
  const arg_def_t superres_qthresh;
  const arg_def_t superres_kf_qthresh;
  const arg_def_t end_usage;
  const arg_def_t target_bitrate;
  const arg_def_t min_quantizer;
  const arg_def_t max_quantizer;
  const arg_def_t undershoot_pct;
  const arg_def_t overshoot_pct;
  const arg_def_t buf_sz;
  const arg_def_t buf_initial_sz;
  const arg_def_t buf_optimal_sz;
  const arg_def_t bias_pct;
  const arg_def_t minsection_pct;
  const arg_def_t maxsection_pct;
  const arg_def_t fwd_kf_enabled;
  const arg_def_t kf_min_dist;
  const arg_def_t kf_max_dist;
  const arg_def_t kf_disabled;
  const arg_def_t sframe_dist;
  const arg_def_t sframe_mode;
  const arg_def_t save_as_annexb;
  const arg_def_t noise_sens;
  const arg_def_t sharpness;
  const arg_def_t static_thresh;
  const arg_def_t auto_altref;
  const arg_def_t arnr_maxframes;
  const arg_def_t arnr_strength;
  const arg_def_t tune_metric;
  const arg_def_t cq_level;
  const arg_def_t max_intra_rate_pct;
#if CONFIG_AV1_ENCODER
  const arg_def_t cpu_used_av1;
  const arg_def_t rowmtarg;
  const arg_def_t tile_cols;
  const arg_def_t tile_rows;
  const arg_def_t enable_tpl_model;
  const arg_def_t enable_keyframe_filtering;
  const arg_def_t tile_width;
  const arg_def_t tile_height;
  const arg_def_t lossless;
  const arg_def_t enable_cdef;
  const arg_def_t enable_restoration;
  const arg_def_t enable_rect_partitions;
  const arg_def_t enable_ab_partitions;
  const arg_def_t enable_1to4_partitions;
  const arg_def_t min_partition_size;
  const arg_def_t max_partition_size;
  const arg_def_t enable_dual_filter;
  const arg_def_t enable_chroma_deltaq;
  const arg_def_t enable_intra_edge_filter;
  const arg_def_t enable_order_hint;
  const arg_def_t enable_tx64;
  const arg_def_t enable_flip_idtx;
  const arg_def_t enable_rect_tx;
  const arg_def_t enable_dist_wtd_comp;
  const arg_def_t enable_masked_comp;
  const arg_def_t enable_onesided_comp;
  const arg_def_t enable_interintra_comp;
  const arg_def_t enable_smooth_interintra;
  const arg_def_t enable_diff_wtd_comp;
  const arg_def_t enable_interinter_wedge;
  const arg_def_t enable_interintra_wedge;
  const arg_def_t enable_global_motion;
  const arg_def_t enable_warped_motion;
  const arg_def_t enable_filter_intra;
  const arg_def_t enable_smooth_intra;
  const arg_def_t enable_paeth_intra;
  const arg_def_t enable_cfl_intra;
  const arg_def_t force_video_mode;
  const arg_def_t enable_obmc;
  const arg_def_t enable_overlay;
  const arg_def_t enable_palette;
  const arg_def_t enable_intrabc;
  const arg_def_t enable_angle_delta;
  const arg_def_t disable_trellis_quant;
  const arg_def_t enable_qm;
  const arg_def_t qm_min;
  const arg_def_t qm_max;
  const arg_def_t reduced_tx_type_set;
  const arg_def_t use_intra_dct_only;
  const arg_def_t use_inter_dct_only;
  const arg_def_t use_intra_default_tx_only;
  const arg_def_t quant_b_adapt;
  const arg_def_t coeff_cost_upd_freq;
  const arg_def_t mode_cost_upd_freq;
  const arg_def_t mv_cost_upd_freq;
  const arg_def_t num_tg;
  const arg_def_t mtu_size;
  const arg_def_t timing_info;
#if CONFIG_TUNE_VMAF
  const arg_def_t vmaf_model_path;
#endif
  const arg_def_t film_grain_test;
  const arg_def_t film_grain_table;
#if CONFIG_DENOISE
  const arg_def_t denoise_noise_level;
  const arg_def_t denoise_block_size;
#endif
  const arg_def_t enable_ref_frame_mvs;
  const arg_def_t frame_parallel_decoding;
  const arg_def_t error_resilient_mode;
  const arg_def_t aq_mode;
  const arg_def_t deltaq_mode;
  const arg_def_t deltalf_mode;
  const arg_def_t frame_periodic_boost;
  const arg_def_t gf_cbr_boost_pct;
  const arg_def_t max_inter_rate_pct;
  const arg_def_t min_gf_interval;
  const arg_def_t max_gf_interval;
  const arg_def_t gf_min_pyr_height;
  const arg_def_t gf_max_pyr_height;
  const arg_def_t max_reference_frames;
  const arg_def_t reduced_reference_set;
  const arg_def_t target_seq_level_idx;
  const arg_def_t set_min_cr;
  const arg_def_t input_color_primaries;
  const arg_def_t input_transfer_characteristics;
  const arg_def_t input_matrix_coefficients;
  const arg_def_t input_chroma_sample_position;
  const arg_def_t tune_content;
  const arg_def_t cdf_update_mode;
  const arg_def_t superblock_size;
  const arg_def_t set_tier_mask;
  const arg_def_t use_fixed_qp_offsets;
  const arg_def_t fixed_qp_offsets;
  const arg_def_t vbr_corpus_complexity_lap;
#endif  // CONFIG_AV1_ENCODER
};

static const struct CodecArgDefs codecArgDefs = {
