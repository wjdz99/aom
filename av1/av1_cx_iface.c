...
static aom_codec_err_t create_context_and_bufferpool(
    ...
    struct aom_codec_pkt_list *pkt_list_head, COMPRESSOR_STAGE stage,
    int num_lap_buffers) {
...
#endif
  *p_cpi = av1_create_compressor(oxcf, *p_buffer_pool, stage, num_lap_buffers);
  if (*p_cpi == NULL)
    ...
    if (res == AOM_CODEC_OK) {
      ...
      *num_lap_buffers = 0;
      ...
      res = create_context_and_bufferpool(&priv->cpi, &priv->buffer_pool,
                                          ...
                                          ENCODE_STAGE, *num_lap_buffers);

      // Create another compressor if look ahead is enabled
      if (res == AOM_CODEC_OK && *num_lap_buffers) {
        res = create_context_and_bufferpool(&priv->cpi_lap,
                                            &priv->buffer_pool_lap, &priv->oxcf,
                                            NULL, LAP_STAGE, *num_lap_buffers);
      }
    }
  }
