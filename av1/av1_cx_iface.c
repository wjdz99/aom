...
static void update_encoder_config(cfg_options_t *cfg,
  ...
  cfg->superblock_size = (extra_cfg->superblock_size
    == AOM_SUPERBLOCK_SIZE_64X64) ? 64 :
  ...
  cfg->enable_ref_frame_mvs = (extra_cfg->allow_ref_frame_mvs ||
...
static void report_stats(AV1_COMP *cpi, size_t frame_size, uint64_t cx_time) {
  ...
  if (!cm->show_existing_frame) {
    ...
    fprintf(stdout,
            "POC:%6d [%s][Q:%3d]: %6I64u Bytes,"
            "%6.1fms, %2.4f dB(Y), %2.4f dB(U), "
            "%2.4f dB(V), "
            "%2.4f dB(Avg)",
            cm->cur_frame->absolute_poc,
            frameType[cm->current_frame.frame_type],
            base_qindex, (unsigned __int64)frame_size,
            cx_time / 1000.0, psnr[1], psnr[2], psnr[3],
            psnr[0]);
