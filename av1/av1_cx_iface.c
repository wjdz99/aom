...
static aom_codec_err_t ctrl_get_enc_sub_gop_config(aom_codec_alg_priv_t *ctx,
                                                   va_list args) {
  SUBGOP_INFO *subgop_info = va_arg(args, SUBGOP_INFO *);
  ...
  // As key frame is independent of sub-gop structure,
  ...
  if (subgop_cfg) {
    ...
    subgop_info->code = subgop_cfg->subgop_in_gop_code;
  }
  return AOM_CODEC_OK;
}
...
static aom_codec_err_t ctrl_get_enc_frame_info(aom_codec_alg_priv_t *ctx,
                                               va_list args) {
  SUBGOP_DATA *sub_gop_data = va_arg(args, SUBGOP_DATA *);
  AV1_COMP *const cpi = ctx->cpi;
  SubGOPStatsEnc *subgop_stats = &cpi->subgop_stats;
  SUBGOP_STEP_DATA *step_data = sub_gop_data->step;
  int curr_step_idx = subgop_stats->stat_count;
