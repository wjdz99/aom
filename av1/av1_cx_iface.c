...
#if !CONFIG_REALTIME_ONLY
  {
      // The following defaults were chosen to enhance the subjective quality
      // and SSIMULACRA2 scores of images. They are also expected to benefit
      // actual all-intra video sequences (to some degree). Callers can always
      // override these settings by using aom_codec_set_option() or
      // aom_codec_control().
      AOM_USAGE_ALL_INTRA,  // usage
      0,                    // cpu_used
      1,                    // enable_auto_alt_ref
      0,                    // enable_auto_bwd_ref
      0,                    // noise_sensitivity
          // We can turn on loop filter sharpness, as frames do not have to
          // serve as references to others
      7,  // sharpness
      0,  // static_thresh
      1,  // row_mt
      0,  // fp_mt
      ...
      0,                                          // gf_cbr_boost_pct
      0,                                          // lossless
          // CDEF has been found to blur images, so it's disabled in all-intra
          // mode
      0,  // enable_cdef
      1,  // enable_restoration
      0,  // force_video_mode
      1,  // enable_obmc
      3,  // disable_trellis_quant
          // These QM settings have been found to be optimal for images, when
          // used with an alternative QM formula (see
          // aom_get_qmlevel_allintra()). These values could also be beneficial
          // for other usage modes, but further testing is required.
      1,  // enable_qm
      DEFAULT_QM_Y,                 // qm_y
      DEFAULT_QM_U,                 // qm_u
      DEFAULT_QM_V,                 // qm_v
      DEFAULT_QM_FIRST_ALLINTRA,    // qm_min
      DEFAULT_QM_LAST_ALLINTRA,     // qm_max
      1,                            // max number of tile groups
      0,                            // mtu_size
      AOM_TIMING_UNSPECIFIED,       // No picture timing signaling in bitstream
      0,                            // frame_parallel_decoding_mode
