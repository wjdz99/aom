...
static aom_codec_err_t ctrl_set_svc_params(aom_codec_alg_priv_t *ctx,
  ...
  for (unsigned int sl = 0; sl < cpi->common.number_spatial_layers; ++sl) {
    for (unsigned int tl = 0; tl < cpi->common.number_temporal_layers; ++tl) {
      const int layer =
...
static aom_codec_err_t ctrl_set_svc_ref_map_idx(aom_codec_alg_priv_t *ctx,
                                                va_list args) {
  printf("SET_SVC_REF_MAP_IDX\n");
  AV1_COMP *const cpi = ctx->cpi;
  aom_svc_ref_map_idx_t *const data = va_arg(args, aom_svc_ref_map_idx_t *);
  cpi->svc.apply_external_ref_map = 1;
  for (unsigned int i = 0; i < INTER_REFS_PER_FRAME; ++i)
    cpi->svc.ref_map_idx[i] = data->ref_map_idx[i];
  return AOM_CODEC_OK;
}
...
static aom_codec_ctrl_fn_map_t encoder_ctrl_maps[] = {
  ...
  { AV1E_SET_SVC_REF_MAP_IDX, ctrl_set_svc_ref_map_idx },
