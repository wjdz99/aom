...
static aom_codec_err_t ctrl_set_vmaf_model_path(aom_codec_alg_priv_t *ctx,
                                                va_list args) {
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  const char *string = CAST(AV1E_SET_VMAF_MODEL_PATH, args);
  if (allocate_and_set_string(string, default_extra_cfg.vmaf_model_path,
...
static aom_codec_err_t ctrl_set_partition_info_path(aom_codec_alg_priv_t *ctx,
                                                    va_list args) {
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  const char *string = CAST(AV1E_SET_PARTITION_INFO_PATH, args);
  if (allocate_and_set_string(string, default_extra_cfg.partition_info_path,
...
static aom_codec_err_t ctrl_set_film_grain_table(aom_codec_alg_priv_t *ctx,
                                                 va_list args) {
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  const char *string = CAST(AV1E_SET_FILM_GRAIN_TABLE, args);
  if (allocate_and_set_string(string,
