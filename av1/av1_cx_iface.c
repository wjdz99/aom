...
aom_codec_err_t av1_create_context_and_bufferpool(AV1_PRIMARY *ppi,
  ...
  if (buffer_pool == NULL) {
    ...
    *p_buffer_pool = buffer_pool;
    buffer_pool->num_ref_frame_bufs = (oxcf->kf_cfg.key_freq_max == 0)
                                          ? FRAME_BUFFERS_ALLINTRA
                                          : FRAME_BUFFERS;
    buffer_pool->frame_bufs = (RefCntBuffer *)aom_calloc(
        buffer_pool->num_ref_frame_bufs, sizeof(*buffer_pool->frame_bufs));
    if (buffer_pool->frame_bufs == NULL) return AOM_CODEC_MEM_ERROR;
