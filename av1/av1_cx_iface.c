...
static aom_codec_err_t encoder_init(aom_codec_ctx_t *ctx) {
  ...
  if (ctx->priv == NULL) {
    ...
    // However, if users set --enable-cdef, --qm-min, or --qm-max from command
    // line, the encoder still respects it.
    if (priv->cfg.g_usage == AOM_USAGE_ALL_INTRA) {
      priv->extra_cfg.enable_cdef = 0;
      priv->extra_cfg.qm_min = DEFAULT_QM_ALLINTRA_FIRST;
      priv->extra_cfg.qm_max = DEFAULT_QM_ALLINTRA_LAST;
    }
