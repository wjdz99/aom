...
static aom_codec_err_t ctrl_set_apply_inloop_postproc_filters(
  ...
  // Apply in-loop post-processing filters for modes other than allintra
  // encoding.
  if (ctx->cfg.g_usage != ALLINTRA) apply_inloop_postproc_filters = 1;
  extra_cfg.apply_inloop_postproc_filters = apply_inloop_postproc_filters;
  return update_extra_cfg(ctx, &extra_cfg);
}
