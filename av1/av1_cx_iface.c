...
#if CONFIG_NEW_TX_PARTITION
static aom_codec_err_t ctrl_set_use_inter_4way_tx_split(
    aom_codec_alg_priv_t *ctx, va_list args) {
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  extra_cfg.use_inter_4way_tx_split =
      CAST(AV1E_SET_USE_INTER_4WAY_TX_SPLIT, args);
  return update_extra_cfg(ctx, &extra_cfg);
}

static aom_codec_err_t ctrl_set_use_intra_4way_tx_split(
    aom_codec_alg_priv_t *ctx, va_list args) {
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  extra_cfg.use_intra_4way_tx_split =
      CAST(AV1E_SET_USE_INTRA_4WAY_TX_SPLIT, args);
  return update_extra_cfg(ctx, &extra_cfg);
}
#endif  // CONFIG_NEW_TX_PARTITION
