...
static aom_codec_err_t ctrl_set_tile_columns(aom_codec_alg_priv_t *ctx,
  ...
  // If the control AUTO_TILES is used then don't override the
  // tile_columns set via the AUTO_TILES control.
...
static aom_codec_err_t ctrl_set_tile_rows(aom_codec_alg_priv_t *ctx,
  ...
  AV1_COMP *const cpi = ppi->cpi;
  // If the control AUTO_TILES is used then don't override the
  // tile_rows set via the AUTO_TILES control.
  if (cpi->auto_tiles_enabled) return AOM_CODEC_OK;
...
static aom_codec_err_t ctrl_set_auto_tiles(aom_codec_alg_priv_t *ctx,
  ...
  // Only change the number of tiles if cpi->auto_tiles_enabled == 1.
  if (cpi->auto_tiles_enabled && ctx->cfg.g_threads >= 2) {
