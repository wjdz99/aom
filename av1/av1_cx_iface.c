...
aom_codec_err_t aom_codec_set_option(aom_codec_ctx_t *codec_ctx,
  ...
  } else if (arg_match_helper(&arg, &enable_intra_edge_filter, argv,
                              &err_msg)) {
  ...
  } else if (arg_match_helper(&arg, &enable_smooth_interintra, argv,
                              &err_msg)) {
  ...
  if (err_msg) {
    ...
    if (err_message) {
      snprintf(err_message, strlen(err_msg) + 1, "%s", err_msg);
    }
    aom_free(err_msg);
    codec_ctx->err_detail = err_message;
    if (codec_ctx->priv) codec_ctx->priv->err_detail = err_message;
    return AOM_CODEC_INVALID_PARAM;
  }
