...
static double get_modeled_qp_offset(int qp, int level, int bit_depth) {
  ...
  /* at higher end of QP the slope grows exponentially captured by qp_threshold
   */
#if CONFIG_EXTQUANT
  int qp_threshold = ((bit_depth == AOM_BITS_8
                           ? MAXQ_8_BITS
                           : bit_depth == AOM_BITS_10 ? MAXQ_10_BITS : MAXQ) *
                      7) /
                     10;
#else
  int qp_threshold = (MAXQ * 7) / 10;
#endif

  if (q_based_qp_offsets && (qp < qp_threshold)) {
