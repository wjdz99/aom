...
static bool validate_bitdepth(const unsigned char *src, int rows, int cols,
  ...
  const uint64_t mask64 = mask16 | ((uint64_t)mask16 << 16) |
                          ((uint64_t)mask16 << 32) | ((uint64_t)mask16 << 48);
...
static aom_codec_err_t validate_img(aom_codec_alg_priv_t *ctx,
  ...
  if (bit_depth > 8) {
    ...
    bitdepth_valid &=
        validate_bitdepth(img->planes[AOM_PLANE_Y], img->h, img->w,
                          img->stride[AOM_PLANE_Y], bit_depth);
    if (!img->monochrome) {
      const int uv_height =
          (img->h + img->y_chroma_shift) >> img->y_chroma_shift;
      const int uv_width =
          (img->w + img->x_chroma_shift) >> img->x_chroma_shift;
      bitdepth_valid &=
          validate_bitdepth(img->planes[AOM_PLANE_U], uv_height, uv_width,
                            img->stride[AOM_PLANE_U], bit_depth);
      bitdepth_valid &=
          validate_bitdepth(img->planes[AOM_PLANE_V], uv_height, uv_width,
