...
static void update_default_encoder_config(
  const cfg_options_t *cfg, struct av1_extracfg *extra_cfg) {
  ...
  extra_cfg->superblock_size = (cfg->SuperBlockSize == 64) ?
    AOM_SUPERBLOCK_SIZE_64X64 : (cfg->SuperBlockSize == 128) ?
    AOM_SUPERBLOCK_SIZE_128X128 : AOM_SUPERBLOCK_SIZE_DYNAMIC;
...
static aom_codec_err_t ctrl_set_min_partition_size(
  aom_codec_alg_priv_t *ctx, va_list args) {
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  extra_cfg.min_partition_size =
    CAST(AV1E_SET_MIN_PARTITION_SIZE, args);
  return update_extra_cfg(ctx, &extra_cfg);
}

static aom_codec_err_t ctrl_set_max_partition_size(
  aom_codec_alg_priv_t *ctx, va_list args) {
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  extra_cfg.max_partition_size =
    CAST(AV1E_SET_MAX_PARTITION_SIZE, args);
  return update_extra_cfg(ctx, &extra_cfg);
}
...
static aom_codec_ctrl_fn_map_t encoder_ctrl_maps[] = {
  ...
  { AV1E_SET_ENABLE_1TO4_PARTITIONS, ctrl_set_enable_tshape_partitions },
