...
static aom_codec_err_t set_encoder_config(
  ...
  // Initialize to input resolution if no scaling or dynamic scaling.
  if (oxcf->resize_mode == RESIZE_NONE || oxcf->resize_mode == RESIZE_DYNAMIC) {
    oxcf->scaled_frame_width = oxcf->width;
    oxcf->scaled_frame_height = oxcf->heigt;
  }
