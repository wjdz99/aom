...
static aom_codec_err_t validate_config(aom_codec_alg_priv_t *ctx,
  ...
  if (cfg->g_pass == AOM_RC_LAST_PASS) {
    ...
    const int cq_level = av1_quantizer_to_qindex(extra_cfg->cq_level);
    ...
    if (use_ml_model_to_decide_alt_ref(cfg->rc_end_usage, cq_level) &&
      ...
      fprintf(stderr,
              "Warning: First pass stats required in the ML model to predict "
              "a flat GOP structure is invalid. Continuing encoding by "
              "disabling the ML model\n");
  }
