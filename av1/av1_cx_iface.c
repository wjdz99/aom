...
static aom_codec_err_t validate_config(aom_codec_alg_priv_t *ctx,
  ...
  RANGE_CHECK(extra_cfg, vbr_corpus_complexity_lap, 0, MAX_VBR_CORPUS_COMPLEXITY);
...
#ifdef TEST_CORPUS_COMPLEXITY
  if (oxcf->rc_mode == AOM_VBR) {
      int vbr_corpus_complexity_lap;
      FILE *dump_fp = fopen("corpus_complexity.bin", "rb");
      if(dump_fp == NULL)
        printf("unable to fopen (rb) corpus_complexity.bin\n");
      fread(&vbr_corpus_complexity_lap, sizeof(vbr_corpus_complexity_lap), 1, dump_fp);
      fclose(dump_fp);
      oxcf->vbr_corpus_complexity_lap = vbr_corpus_complexity_lap;
      printf("1pass LAP vbr_corpus_complexity_lap = %d\n", oxcf->vbr_corpus_complexity_lap);
    }
#endif
  for (int i = 0; i < FIXED_QP_OFFSET_COUNT; ++i) {
...
static aom_codec_err_t ctrl_set_vbr_corpus_complexity_lap(aom_codec_alg_priv_t *ctx,
                                              va_list args) {
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  extra_cfg.vbr_corpus_complexity_lap = CAST(AV1E_SET_VBR_CORPUS_COMPLEXITY_LAP, args);
  return update_extra_cfg(ctx, &extra_cfg);
}
