...
static aom_codec_err_t validate_config(aom_codec_alg_priv_t *ctx,
  ...
  RANGE_CHECK_BOOL(extra_cfg, apply_loopfilter, 1);
...
static aom_codec_err_t ctrl_set_apply_loopfilter(aom_codec_alg_priv_t *ctx,
  ...
  // Apply loop filtering for modes other than allintra encoding.
  if (ctx->cfg.g_usage != ALLINTRA) apply_loopfilter = 1;
...
static aom_codec_err_t ctrl_copy_reference(aom_codec_alg_priv_t *ctx,
                                           va_list args) {
  if (!ctx->ppi->cpi->oxcf.algo_cfg.apply_loopfilter)
