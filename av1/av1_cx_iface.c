...
#else
      res = av1_create_context_and_bufferpool(priv->ppi, &priv->ppi->cpi,
...
static aom_codec_err_t encoder_destroy(aom_codec_alg_priv_t *ctx) {
  ...
  if (ctx->ppi) {
    ...
    ppi->cpi = NULL;
#else
    av1_destroy_context_and_bufferpool(ppi->cpi, &ctx->buffer_pool);
#endif  // CONFIG_FRAME_PARALLEL_ENCODE
    if (ppi->cpi_lap) {
...
static aom_codec_err_t encoder_encode(aom_codec_alg_priv_t *ctx,
  ...
  if (res == AOM_CODEC_OK) {
    ...
    // Reset gf_frame_index at the beginning of sub gop
    if (ppi->gf_group.size > 0 && cpi->gf_frame_index == ppi->gf_group.size) {
