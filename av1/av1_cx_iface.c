...
static aom_codec_err_t ctrl_set_min_active_seg_unit_size(aom_codec_alg_priv_t *ctx,
	va_list args) {
	struct av1_extracfg extra_cfg = ctx->extra_cfg;
	extra_cfg.min_active_seg_unit_size = CAST(AV1E_SET_MIN_ACTIVE_SEG_UNIT_SIZE, args);
	return update_extra_cfg(ctx, &extra_cfg);
}
...
static aom_codec_err_t ctrl_set_min_quality_seg_unit_size(aom_codec_alg_priv_t *ctx,
  va_list args) {
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  extra_cfg.min_quality_seg_unit_size = CAST(AV1E_SET_MIN_QUALITY_SEG_UNIT_SIZE, args);
  return update_extra_cfg(ctx, &extra_cfg);
}
#endif
