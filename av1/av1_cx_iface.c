...
static aom_codec_err_t encoder_encode(aom_codec_alg_priv_t *ctx,
  ...
  if (img != NULL) {
    ...
    if (res == AOM_CODEC_OK) {
      ...
      // allocated with lesser size for the following as no-show frames are not
      // possible in these cases
      // 1) all intra frame encoding and forward key frames are not enabled.
      // 2) alt-ref frames are not enabled.
      int max_no_show_frames_plus_1 = 8;
      if ((cpi->oxcf.kf_cfg.key_freq_max == 0 &&
           !cpi->oxcf.kf_cfg.fwd_kf_enabled) ||
          !cpi->oxcf.gf_cfg.enable_auto_arf)
        max_no_show_frames_plus_1 = 1;
      size_t data_sz = frame_sz * max_no_show_frames_plus_1;
