...
static void update_encoder_config(cfg_options_t *cfg,
	...
	cfg->superblock_size = (extra_cfg->superblock_size == AOM_SUPERBLOCK_SIZE_64X64)
		? 64 : (extra_cfg->superblock_size == AOM_SUPERBLOCK_SIZE_128X128)
		? 128 : 0;
	...
	cfg->enable_ref_frame_mvs = (extra_cfg->allow_ref_frame_mvs  || extra_cfg->enable_ref_frame_mvs );
...
static aom_codec_err_t set_encoder_config(AV1EncoderConfig *oxcf,
	const aom_codec_enc_cfg_t *cfg,
	struct av1_extracfg *extra_cfg, int update_config) {

  if (cfg->encoder_cfg.init_by_cfg_file && update_config == 0) {
...
static void calculate_psnr(AV1_COMP *cpi, double *psnr) {
  int i;
  PSNR_STATS stats;
...
static void report_stats(AV1_COMP *cpi, size_t frame_size, uint64_t cx_time) {
  ...
  if (!cm->show_existing_frame) {
    ...
    unsigned int ref_poc[INTER_REFS_PER_FRAME];
    for (int ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ref_frame++) {
      const int ref_idx = ref_frame - LAST_FRAME;
      const RefCntBuffer *const buf = get_ref_frame_buf(cm, ref_frame);
      ref_poc[ref_idx] = buf ? buf->absolute_poc : -1;
    }

    fprintf(stdout,
            "POC:%6d [%s][Q:%3d]: %6lu Bytes, %6.1fms, %2.4f dB(Y), %2.4f dB(U), "
            "%2.4f dB(V), "
            "%2.4f dB(Avg)",
            cm->cur_frame->absolute_poc, frameType[cm->current_frame.frame_type],
            base_qindex, (unsigned long)frame_size, cx_time / 1000.0, psnr[1], psnr[2], psnr[3],
            psnr[0]);

    fprintf(stdout, "    [");
    for (int ref_idx = 0; ref_idx < 7; ref_idx++) {
      fprintf(stdout, "%2d,", ref_poc[ref_idx]);
    }
    fprintf(stdout, "]\n");
  }
}
