  ...
  if (res == AOM_CODEC_OK) {
    ...
    while (!is_frame_visible) {
      if (cx_data_sz < ctx->max_frame_sz) {
        ...
        fprintf(stderr,
                "WTC WTC WTC: Increased ctx->cx_data_sz from %zu to %zu\n",
    ...
    fprintf(stderr, "WTC WTC WTC: frame_count = %d, cx_data_sz = %zu frames\n",
            frame_count,
            ctx->cx_data_sz == 0 ? 0 : ctx->cx_data_sz / ctx->max_frame_sz);
    if (is_frame_visible) {
