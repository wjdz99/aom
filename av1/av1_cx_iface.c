...
struct av1_extracfg {
  ...
  // Indicates if the application of in-loop post-processing filters should be
  // skipped on reconstructed frame.
  unsigned int skip_inloop_postproc_filtering;
...
static const struct av1_extracfg default_extra_cfg = {
  ...
  0,               // skip_inloop_postproc_filtering
...
static aom_codec_err_t validate_config(aom_codec_alg_priv_t *ctx,
  ...
  RANGE_CHECK_BOOL(extra_cfg, skip_inloop_postproc_filtering);
...
static aom_codec_err_t set_encoder_config(AV1EncoderConfig *oxcf,
  ...
  algo_cfg->skip_inloop_postproc_filtering =
      extra_cfg->skip_inloop_postproc_filtering;
...
static aom_codec_err_t ctrl_set_skip_inloop_postproc_filtering(
    aom_codec_alg_priv_t *ctx, va_list args) {
  // Skipping the application of in-loop post-processing filters is allowed only
  // for ALLINTRA mode.
  if (ctx->cfg.g_usage != AOM_USAGE_ALL_INTRA) return AOM_CODEC_UNSUP_FEATURE;
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  extra_cfg.skip_inloop_postproc_filtering =
      CAST(AV1E_SET_SKIP_INLOOP_POSTPROC_FILTERING, args);
  return update_extra_cfg(ctx, &extra_cfg);
}
...
static aom_codec_err_t ctrl_copy_reference(aom_codec_alg_priv_t *ctx,
                                           va_list args) {
  if (ctx->ppi->cpi->oxcf.algo_cfg.skip_inloop_postproc_filtering)
...
static aom_codec_ctrl_fn_map_t encoder_ctrl_maps[] = {
  ...
  { AV1E_SET_SKIP_INLOOP_POSTPROC_FILTERING,
    ctrl_set_skip_inloop_postproc_filtering },
