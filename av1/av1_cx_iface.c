...
#if CONFIG_REALTIME_ONLY
// Changes settings for realtime only build.
static void set_realtime_extra_cfg_defaults(struct av1_extracfg *extra_cfg) {
  extra_cfg->cpu_used = 10;
  extra_cfg->enable_auto_alt_ref = 0;
  extra_cfg->enable_auto_bwd_ref = 0;
  extra_cfg->noise_sensitivity = 0;
  extra_cfg->sharpness = 0;
  extra_cfg->static_thresh = 0;
  extra_cfg->row_mt = 1;
  extra_cfg->fp_mt = 0;
  extra_cfg->tile_columns = 0;
  extra_cfg->tile_rows = 0;
  extra_cfg->auto_tiles = 1;
  extra_cfg->enable_tpl_model = 0;
  extra_cfg->enable_keyframe_filtering = 0;
  extra_cfg->arnr_max_frames = 7;
  extra_cfg->arnr_strength = 5;
  extra_cfg->min_gf_interval = 0;
  extra_cfg->max_gf_interval = 0;
  extra_cfg->gf_min_pyr_height = 0;
  extra_cfg->gf_max_pyr_height = 0;
  extra_cfg->tuning = AOM_TUNE_PSNR;
  extra_cfg->vmaf_model_path = "/usr/local/share/model/vmaf_v0.6.1.json";
  extra_cfg->partition_info_path = ".";
  extra_cfg->rate_distribution_info = "./rate_map.txt";
  extra_cfg->dist_metric = AOM_DIST_METRIC_PSNR;
  extra_cfg->cq_level = 10;
  extra_cfg->rc_max_intra_bitrate_pct = 300;
  extra_cfg->rc_max_inter_bitrate_pct = 0;
  extra_cfg->gf_cbr_boost_pct = 0;
  extra_cfg->lossless = 0;
  extra_cfg->enable_cdef = 1;
  extra_cfg->enable_restoration = 0;
  extra_cfg->force_video_mode = 0;
  extra_cfg->enable_obmc = 0;
  extra_cfg->disable_trellis_quant = 1;
  extra_cfg->enable_qp = 0;
  extra_cfg->qm_y = DEFAULT_QM_Y;
  extra_cfg->qm_u = DEFAULT_QM_U
  extra_cfg->qm_v = DEFAULT_QM_V;
  extra_cfg->qm_min = DEFAULT_QM_FIRST;
  extra_cfg->qm_max = DEFAULT_QM_LAST;
  extra_cfg->num_tg = 1;
  extra_cfg->mtu_size = 0;
  extra_cfg->timing_info_type = AOM_TIMING_UNSPECIFIED;
  extra_cfg->frame_parallel_decoding_mode = 0;
  extra_cfg->enable_dual_filter = 0;
  extra_cfg->enable_chroma_deltaq = 0;
  extra_cfg->aq_mode = CYCLIC_REFRESH_AQ
  extra_cfg->deltaq_mode = NO_DELTA_Q;
  extra_cfg->deltaq_strength = 100;
  extra_cfg->deltalf_mode = 0;
  extra_cfg->frame_periodic_boost = 0;
  extra_cfg->bit_depth = AOM_BITS_8;
  extra_cfg->content = AOM_CONTENT_DEFAULT;
  extra_cfg->color_primaries = AOM_CICP_CP_UNSPECIFIED;
  extra_cfg->transfer_characteristics = AOM_CICP_TC_UNSPECIFIED;
  extra_cfg->matrix_coefficients = AOM_CICP_MC_UNSPECIFIED;
  extra_cfg->chroma_sample_position =  AOM_CSP_UNKNOWN;
  extra_cfg->color_range = 0;
  extra_cfg->render_width = 0;
  extra_cfg->render_height = 0;
  extra_cfg->superblock_size = AOM_SUPERBLOCK_SIZE_DYNAMIC;
  extra_cfg->single_tile_decoding = 1;
  extra_cfg->error_resilient_mode = 0;
  extra_cfg->sframe_mode = 0;
  extra_cfg->film_grain_test_vector = 0;
  extra_cfg->film_grain_table_filename = NULL;
  extra_cfg->motion_vector_unit_test = 0;
#if CONFIG_FPMT_TEST
  extra_cfg->fpmt_unit_test = 0;
#endif
  extra_cfg->cdf_update_mode = 1;
  extra_cfg->enable_rect_partitions = 0;
  extra_cfg->enable_ab_partitions = 0;
  extra_cfg->enable_1to4_partitions = 0;
  extra_cfg->min_partition_size = 4;
  extra_cfg->max_partition_size = 128;
  extra_cfg->enable_intra_edge_filter = 0;
  extra_cfg->enable_order_hint = 0;
  extra_cfg->enable_tx64 = 0;
  extra_cfg->enable_flip_idtx = 1;
  extra_cfg->enable_rect_tx = 1;
  extra_cfg->enable_dist_wtd_comp = 0;
  extra_cfg->max_reference_frames = 3;
  extra_cfg->enable_reduced_reference_set = 0;
  extra_cfg->enable_ref_frame_mvs = 0;
  extra_cfg->allow_ref_frame_mvs = 0;
  extra_cfg->enable_masked_comp = 0;
  extra_cfg->enable_onesided_comp = 0;
  extra_cfg->enable_interintra_comp = 0;
  extra_cfg->enable_smooth_interintra = 0;
  extra_cfg->enable_diff_wtd_comp = 0;
  extra_cfg->enable_interinter_wedge = 0;
  extra_cfg->enable_interintra_wedge = 0;
  extra_cfg->enable_global_motion = 0;
  extra_cfg->enable_warped_motion = 0;
  extra_cfg->allow_warped_motion = 0;
  extra_cfg->enable_filter_intra = 0;
  extra_cfg->enable_smooth_intra = 0;
  extra_cfg->enable_paeth_intra = 0;
  extra_cfg->enable_cfl_intra = 0;
  extra_cfg->enable_directional_intra = 1;
  extra_cfg->enable_diagonal_intra = 1;
  extra_cfg->enable_superres = 0;
  extra_cfg->enable_overlay = 0;
  extra_cfg->enable_palette = 0;
  extra_cfg->enable_intrabc = 0;
  extra_cfg->enable_angle_delta = 0;
#if CONFIG_DENOISE
  extra_cfg->noise_level = 0;
  extra_cfg->noise_block_size = 32;
  extra_cfg->enable_dnl_denoising = 0;
#endif
  extra_cfg->chroma_subsampling_x = 0;
  extra_cfg->chroma_subsampling_y = 0;
  extra_cfg->reduced_tx_type_set = 0;
  extra_cfg->use_intra_dct_only = 0;
  extra_cfg->use_inter_dct_only = 0;
  extra_cfg->use_default_intra_tx_type = 1;
  extra_cfg->enable_tx_size_search = 1;
  extra_cfg->quant_b_adapt = 0;
  extra_cfg->vbr_corpus_complexity_lap = 0;
  extra_cfg->target_seq_level_idx[MAX_NUM_OPERATING_POINTS] = {
      SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX,
      SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX,
      SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX,
      SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX, SEQ_LEVEL_MAX,
  ...
  extra_cfg->tier_mask = 0;
  extra_cfg->min_cr = 0;
  extra_cfg->coeff_cost_upd_freq = COST_UPD_OFF;
  extra_cfg->mode_cost_upd_freq = COST_UPD_OFF;
  extra_cfg->mv_cost_upd_freq = COST_UPD_OFF;
  extra_cfg->dv_cost_upd_freq = COST_UPD_OFF;
  extra_cfg->ext_tile_debug = 0;
  extra_cfg->sb_multipass_unit_test = 0;
  extra_cfg->passes = -1;
  extra_cfg->fwd_kf_dist = -1;
  extra_cfg->loopfilter_control = LOOPFILTER_ALL;
  extra_cfg->skip_postproc_filtering = 0;
  extra_cfg->two_pass_output = NULL;
  extra_cfg->second_pass_log = NULL;
  extra_cfg->auto_intra_tools_off = 0;
  extra_cfg->strict_level_conformance = 0;
  extra_cfg->kf_max_pyr_height = -1;
  extra_cfg->sb_qp_sweep = 0;
}
#endif
