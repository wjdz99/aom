...
aom_codec_err_t aom_codec_set_option(aom_codec_ctx_t *codec_ctx,
  ...
  if (arg_match_helper(&arg, &codecArgDefs.enable_keyframe_filtering, argv,
  ...
  } else if (arg_match_helper(&arg, &codecArgDefs.auto_altref, argv, err_msg)) {
    extra_cfg.enable_auto_alt_ref = arg_parse_uint_helper(&arg, err_msg);
  } else if (arg_match_helper(&arg, &codecArgDefs.noise_sens, argv, err_msg)) {
    extra_cfg.noise_sensitivity = arg_parse_uint_helper(&arg, err_msg);
  } else if (arg_match_helper(&arg, &codecArgDefs.sharpness, argv, err_msg)) {
  ...
  } else if (arg_match_helper(&arg, &codecArgDefs.rowmtarg, argv, err_msg)) {
    extra_cfg.row_mt = arg_parse_uint_helper(&arg, err_msg);
  } else if (arg_match_helper(&arg, &codecArgDefs.tile_cols, argv, err_msg)) {
    extra_cfg.tile_columns = arg_parse_uint_helper(&arg, err_msg);
  } else if (arg_match_helper(&arg, &codecArgDefs.tile_rows, argv, err_msg)) {
  ...
  } else if (arg_match_helper(&arg, &codecArgDefs.tune_metric, argv, err_msg)) {
...
#endif
  else if (arg_match_helper(&arg, &codecArgDefs.cq_level, argv, err_msg)) {
  ...
  } else if (arg_match_helper(&arg, &codecArgDefs.lossless, argv, err_msg)) {
    extra_cfg.lossless = arg_parse_uint_helper(&arg, err_msg);
  } else if (arg_match_helper(&arg, &codecArgDefs.enable_cdef, argv, err_msg)) {
  ...
  } else if (arg_match_helper(&arg, &codecArgDefs.enable_obmc, argv, err_msg)) {
  ...
  } else if (arg_match_helper(&arg, &codecArgDefs.enable_qm, argv, err_msg)) {
    extra_cfg.enable_qm = arg_parse_uint_helper(&arg, err_msg);
  } else if (arg_match_helper(&arg, &codecArgDefs.qm_max, argv, err_msg)) {
    extra_cfg.qm_max = arg_parse_uint_helper(&arg, err_msg);
  } else if (arg_match_helper(&arg, &codecArgDefs.qm_min, argv, err_msg)) {
    extra_cfg.qm_min = arg_parse_uint_helper(&arg, err_msg);
  } else if (arg_match_helper(&arg, &codecArgDefs.num_tg, argv, err_msg)) {
    extra_cfg.num_tg = arg_parse_uint_helper(&arg, err_msg);
  } else if (arg_match_helper(&arg, &codecArgDefs.mtu_size, argv, err_msg)) {
    extra_cfg.mtu_size = arg_parse_uint_helper(&arg, err_msg);
  } else if (arg_match_helper(&arg, &codecArgDefs.timing_info, argv, err_msg)) {
  ...
  } else if (arg_match_helper(&arg, &codecArgDefs.aq_mode, argv, err_msg)) {
    extra_cfg.aq_mode = arg_parse_uint_helper(&arg, err_msg);
  } else if (arg_match_helper(&arg, &codecArgDefs.deltaq_mode, argv, err_msg)) {
  ...
  } else if (arg_match_helper(&arg, &codecArgDefs.sframe_mode, argv, err_msg)) {
  ...
  } else if (arg_match_helper(&arg, &codecArgDefs.enable_tx64, argv, err_msg)) {
  ...
  } else if (arg_match_helper(&arg, &codecArgDefs.set_min_cr, argv, err_msg)) {
    ...
    match = 0;
    snprintf(err_msg, ARG_ERR_MSG_MAX_LEN, "Cannot find aom option %s", name);
  }
  aom_free(argv[0]);
