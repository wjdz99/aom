...
static aom_codec_err_t set_encoder_config(
  ...
  oxcf->enable_still = extra_cfg->enable_still;
...
static aom_codec_err_t ctrl_set_enable_still(aom_codec_alg_priv_t *ctx,
                                             va_list args) {
  struct av1_extracfg extra_cfg = ctx->extra_cfg;
  extra_cfg.enable_still = CAST(AV1E_SET_ENABLE_STILL, args);
  return update_extra_cfg(ctx, &extra_cfg);
}
...
static aom_codec_ctrl_fn_map_t encoder_ctrl_maps[] = {
  ...
  { AV1E_SET_ENABLE_STILL, ctrl_set_enable_still },
