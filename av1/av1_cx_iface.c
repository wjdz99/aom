...
static aom_codec_err_t set_encoder_config(AV1EncoderConfig *oxcf,
  ...
  if ((int)oxcf->pass > oxcf->passes) {
    warn(
        "The current pass=%d should not be larger than number of passes (%d), "
        "setting passes = %d.",
        (int)oxcf->pass, oxcf->passes, (int)oxcf->pass);
    oxcf->passes = (int)oxcf->pass;
  }
  if (oxcf->pass == AOM_RC_ONE_PASS && oxcf->passes != 1) {
    warn(
        "Number of passes should be 1 for one-pass encoding, setting it to 1.");
    oxcf->passes = 1;
  }
...
static aom_codec_err_t encoder_encode(aom_codec_alg_priv_t *ctx,
  ...
  if (res == AOM_CODEC_OK) {
    ...
    if ((num_workers > 1) && (cpi->mt_info.num_workers == 0)) {
      ...
      av1_init_tile_thread_data(cpi, cpi->oxcf.pass == AOM_RC_FIRST_PASS);
#if CONFIG_MULTITHREAD
      av1_init_mt_sync(cpi, cpi->oxcf.pass == AOM_RC_FIRST_PASS);
      if (cpi_lap != NULL) {
