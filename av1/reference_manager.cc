...
PrimaryRefKey get_primary_ref_key(const GopFrame &cur_frame,
                                  const GopFrame &ref_frame) {
  int abs_layer_depth_delta =
      abs(cur_frame.layer_depth - ref_frame.layer_depth);

  bool is_key_frame_differs = cur_frame.is_key_frame != ref_frame.is_key_frame;
  bool is_golden_frame_differs =
      cur_frame.is_golden_frame != ref_frame.is_golden_frame;
  bool is_arf_frame_differs = cur_frame.is_arf_frame != ref_frame.is_arf_frame;
  bool is_show_frame_differs =
      cur_frame.is_show_frame != ref_frame.is_show_frame;
  bool encode_ref_mode_differs =
      cur_frame.encode_ref_mode != ref_frame.encode_ref_mode;

  int abs_order_idx_delta = abs(cur_frame.order_idx - ref_frame.order_idx);
  PrimaryRefKey key = { abs_layer_depth_delta,   is_key_frame_differs,
                        is_golden_frame_differs, is_arf_frame_differs,
                        is_show_frame_differs,   encode_ref_mode_differs,
                        abs_order_idx_delta };
  return key;
}
...
void RefFrameManager::UpdateRefFrameTable(GopFrame *gop_frame) {
  ...
  gop_frame->primary_ref_frame = GetPrimaryRefFrame(
      *gop_frame);  // Call this after ref_frame_list is assigned
