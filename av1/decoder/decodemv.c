...
static void read_inter_block_mode_info(AV1Decoder *const pbi,
  ...
  if (cm->current_video_frame == 1 && cm->show_frame == 0 &&
      mi_row == 12 && mi_col == 6) {
    fprintf(stdout, "We'd like to take a further look ...\n");
    fflush(stdout);
  }
  if (cm->current_video_frame == 1 && cm->show_frame == 0) {
    int mv_row[2], mv_col[2];
    if (mbmi->sb_type < BLOCK_8X8) {
      mv_row[0] = mi->bmi[0].as_mv[0].as_mv.row;
      mv_col[0] = mi->bmi[0].as_mv[0].as_mv.col;
      if (mbmi->ref_frame[1] > 0) {
        mv_row[1] = mi->bmi[0].as_mv[1].as_mv.row;
        mv_col[1] = mi->bmi[0].as_mv[1].as_mv.col;
      } else {
        mv_row[1] = -1;
        mv_col[1] = -1;
      }
    ...
    fprintf(stdout, "Frame=%d, (mi_row,mi_col)=(%d,%d), bsize=%d, mode=%d, "
            ...
            "aom_reader->range=%d, aom_reader->count=%d\n",
            cm->current_video_frame, mi_row, mi_col, mbmi->sb_type,
            mbmi->mode, mbmi->ref_frame[0], mbmi->ref_frame[1],
            mv_row[0], mv_col[0], mv_row[1], mv_col[1], r->range, r->count);
    fflush(stdout);
  }
#endif  // 0
}
