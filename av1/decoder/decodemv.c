...
static PREDICTION_MODE read_inter_mode(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  if (aom_read(r, mode_prob, AOM_ACCT_STR) == 0) {
  ...
  if (aom_read(r, mode_prob) == 0, AOM_ACCT_STR) {
  ...
  const int mode = aom_read_tree(r, av1_inter_mode_tree,
                                 cm->fc->inter_mode_probs[ctx], AOM_ACCT_STR);
  FRAME_COUNTS *counts = xd->counts;
  ...
  if (mbmi->mode == NEWMV) {
    ...
    for (idx = 0; idx < 2; ++idx) {
      if (xd->ref_mv_count[ref_frame_type] > idx + 1) {
        ...
        if (!aom_read(r, drl_prob), AOM_ACCT_STR) {
          mbmi->ref_mv_idx = idx;
  ...
  if (is_motion_variation_allowed(mbmi)) {
    ...
    motion_mode =
        aom_read_tree(r, av1_motion_mode_tree,
                      cm->fc->motion_mode_prob[mbmi->sb_type], AOM_ACCT_STR);
...
static int read_segment_id(aom_reader *r,
  ...
  return aom_read_tree(r, av1_segment_tree, segp->tree_probs, AOM_ACCT_STR);
}
...
static TX_SIZE read_selected_tx_size(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  int tx_size = aom_read(r, tx_probs[0], AOM_ACCT_STR);
  if (tx_size != TX_4X4 && max_tx_size >= TX_16X16) {
    tx_size += aom_read(r, tx_probs[1], AOM_ACCT_STR);
    if (tx_size != TX_8X8 && max_tx_size >= TX_32X32)
      tx_size += aom_read(r, tx_probs[2], AOM_ACCT_STR);
  }
...
static int read_inter_segment_id(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (seg->temporal_update) {
    ...
    mbmi->seg_id_predicted = aom_read(r, pred_prob, AOM_ACCT_STR);
#if CONFIG_MISC_FIXES
...
static int read_skip(AV1_COMMON *cm, const MACROBLOCKD *xd, int segment_id,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_SKIP)) {
    ...
    const int skip = aom_read(r, cm->fc->skip_probs[ctx], AOM_ACCT_STR);
    FRAME_COUNTS *counts = xd->counts;
...
static INLINE int read_uniform(aom_reader *r, int n) {
  ...
  const int m = (1 << l) - n;
  const int v = aom_read_literal(r, l - 1, AOM_ACCT_STR);

  assert(l != 0);
  return (v < m) ? v : ((v << 1) - m + aom_read_literal(r, 1), AOM_ACCT_STR);
}
...
static void read_intra_frame_mode_info(AV1_COMMON *const cm,
  ...
  if (mbmi->tx_size < TX_32X32 && cm->base_qindex > 0 && !mbmi->skip &&
    ...
    mbmi->tx_type = aom_read_tree(
        r, av1_ext_tx_tree,
        cm->fc->intra_ext_tx_prob[mbmi->tx_size][tx_type_nom], AOM_ACCT_STR);
    if (counts)
...
static int read_mv_component(aom_reader *r, const nmv_component *mvcomp,
                             int usehp) {
  int mag, d, fr, hp;
  const int sign = aom_read(r, mvcomp->sign, AOM_ACCT_STR);
  const int mv_class =
      aom_read_tree(r, av1_mv_class_tree, mvcomp->classes, AOM_ACCT_STR);
  const int class0 = mv_class == MV_CLASS_0;

  // Integer part
  if (class0) {
    d = aom_read_tree(r, av1_mv_class0_tree, mvcomp->class0, AOM_ACCT_STR);
    mag = 0;
  } else {
    int i;
    ...
    d = 0;
    for (i = 0; i < n; ++i)
      d |= aom_read(r, mvcomp->bits[i], AOM_ACCT_STR) << i;
    mag = CLASS0_SIZE << (mv_class + 2);
  }

  // Fractional part
  fr = aom_read_tree(r, av1_mv_fp_tree,
                     class0 ? mvcomp->class0_fp[d] : mvcomp->fp, AOM_ACCT_STR);

  // High precision part (if hp is not used, the default value of the hp is 1)
  hp = usehp
           ? aom_read(r, class0 ? mvcomp->class0_hp : mvcomp->hp, AOM_ACCT_STR)
           : 1;
...
static INLINE void read_mv(aom_reader *r, MV *mv, const MV *ref,
                           ...
                           int allow_hp) {
  const MV_JOINT_TYPE joint_type = (MV_JOINT_TYPE)aom_read_tree(
      r, av1_mv_joint_tree, ctx->joints, AOM_ACCT_STR);
...
static REFERENCE_MODE read_block_reference_mode(AV1_COMMON *cm,
  ...
  if (cm->reference_mode == REFERENCE_MODE_SELECT) {
    ...
    const REFERENCE_MODE mode =
        (REFERENCE_MODE)aom_read(r, cm->fc->comp_inter_prob[ctx], AOM_ACCT_STR);
    FRAME_COUNTS *counts = xd->counts;
...
static void read_ref_frames(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    ...
    if (mode == COMPOUND_REFERENCE) {
      ...
      const int bit_fwd =
          aom_read(r, fc->comp_fwdref_prob[ctx_fwd][0], AOM_ACCT_STR);
      if (counts) ++counts->comp_fwdref[ctx_fwd][0][bit_fwd];
      if (!bit_fwd) {
        const int ctx_fwd1 = av1_get_pred_context_comp_fwdref_p1(cm, xd);
        const int bit_fwd1 =
            aom_read(r, fc->comp_fwdref_prob[ctx_fwd1][1], AOM_ACCT_STR);
        ...
        const int bit_fwd2 =
            aom_read(r, fc->comp_fwdref_prob[ctx_fwd2][2], AOM_ACCT_STR);
      ...
      // Read backward references.
      {
        ...
        const int bit_bwd =
            aom_read(r, fc->comp_bwdref_prob[ctx_bwd][0], AOM_ACCT_STR);
      ...
      const int bit = aom_read(r, fc->comp_ref_prob[ctx], AOM_ACCT_STR);
...
static int read_is_inter_block(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    ...
    const int is_inter =
        aom_read(r, cm->fc->intra_inter_prob[ctx], AOM_ACCT_STR);
    FRAME_COUNTS *counts = xd->counts;
  ...
  if (mbmi->tx_size < TX_32X32 && cm->base_qindex > 0 && !mbmi->skip &&
    ...
    if (inter_block) {
      ...
      mbmi->tx_type = av1_ext_tx_inv[aom_read_tree_cdf(
          r, cm->fc->inter_ext_tx_cdf[mbmi->tx_size], TX_TYPES, AOM_ACCT_STR)];
#else
      mbmi->tx_type =
          aom_read_tree(r, av1_ext_tx_tree,
                        cm->fc->inter_ext_tx_prob[mbmi->tx_size], AOM_ACCT_STR);
#endif
