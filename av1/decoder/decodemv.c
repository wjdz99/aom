...
static void read_inter_frame_mode_info(AV1Decoder *const pbi,
  ...
  if (cm->is_skip_mode_allowed) {
    if (inter_block) {                           // inter
      if (cm->ref_frame_idx_1 != INVALID_IDX) {  // Bi-directional prediction
        ++cm->counts.bidir_inter[0];
        ...
        ++cm->counts.bidir_inter[1];
        if (mbmi->ref_frame[0] == (LAST_FRAME + cm->ref_frame_idx_0)) {
          ++cm->counts.bidir_refs[3];
          ++cm->counts.bidir_modes[3][mbmi->mode];
        }
      }
    } else {                                   // intra
      if (cm->ref_frame_idx_1 != INVALID_IDX)  // Bi-directional prediction
        ++cm->counts.bidir_intra[0];
      else
        ++cm->counts.bidir_intra[1];
    }
  }
