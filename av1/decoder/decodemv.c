...
static int read_unpred_segment_id(aom_reader *r, struct segmentation_probs *segp, int num_seg) {
#if CONFIG_EC_MULTISYMBOL
	return aom_read_symbol(r, segp->tree_cdf, num_seg, ACCT_STR);
#else
	return aom_read_tree(r, av1_segment_tree[num_seg - 2], segp->tree_probs, ACCT_STR);
#endif
}
#else
...
static int dec_get_segment_id(const AV1_COMMON *cm, const uint8_t *segment_ids,
                              SEG_CATEGORIES seg_cat_idx, int mi_offset, int x_mis, int y_mis) {
...
static void set_segment_id(AV1_COMMON *cm, int mi_offset, int x_mis, int y_mis,
  ...
  for (y = 0; y < y_mis; y++)
    for (x = 0; x < x_mis; x++)
      av1_store_segment_id_into_map(segment_id,
...
static int read_segment_id(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  predicted_segment_id = cm->last_frame_seg_map && seg->temporal_update
    ? dec_get_segment_id(cm, cm->last_frame_seg_map,
     seg_cat_idx, mi_offset, x_mis, y_mis)
    : 0;
  ...
  if (!seg->update_map) {
    ...
    const int seg_id_read = !((-xd->mb_to_left_edge >> (MI_SIZE_LOG2 + 3)) & (seg_map_minb_size - 1)) &&
    ...
    const int last_block_seg_minb =
      (mi_row + mi_size_high[xd->mi[0]->mbmi.sb_type] == cm->mi_rows ||
        !((mi_row + mi_size_high[xd->mi[0]->mbmi.sb_type]) & (seg_map_minb_size - 1)))
      &&
      (mi_col + mi_size_wide[xd->mi[0]->mbmi.sb_type] == cm->mi_cols ||
        !((mi_col + mi_size_wide[xd->mi[0]->mbmi.sb_type]) & (seg_map_minb_size - 1)));
    MODE_INFO* top_mi = xd->mi[-((mi_row & (seg_map_minb_size - 1)) + 1) * cm->mi_stride
      - (mi_col & (seg_map_minb_size - 1))];
    MODE_INFO* left_mi = xd->mi[-(mi_row & (seg_map_minb_size - 1)) * cm->mi_stride
      - ((mi_col & (seg_map_minb_size - 1)) + 1)];
    const int pred_segment_id =
      get_segment_id(cm, cm->last_frame_seg_map,
        seg_map_minb_size == 1 ? BLOCK_8X8 : seg_map_minb_size == 2 ? BLOCK_16X16 :
    ...
    if (seg_id_read || !xd->seg_id_is_done[seg_cat_idx]) {
      ...
      if (seg->temporal_update) {
        if (skip_seg_id_read && !seg->skip_seg_id_disabled) {
          ...
          if (seg->skip_seg_id_replacement == SKIP_SEG_ID_TEMPORAL_REPLACE) {
            segment_id = pred_segment_id;
            mbmi->seg_id_predicted[seg_cat_idx] = 1;
          } else {
            mbmi->seg_id_predicted[seg_cat_idx] = 0;
          ...
          const int ctx = (top_mi ? top_mi->mbmi.seg_id_predicted[seg_cat_idx] : 0) +
            (left_mi ? left_mi->mbmi.seg_id_predicted[seg_cat_idx] : 0);
          const aom_prob pred_prob = segp->pred_probs[ctx];
          xd->seg_id_is_done[seg_cat_idx] = 1;
          mbmi->seg_id_predicted[seg_cat_idx] = aom_read(r, pred_prob, ACCT_STR);
          if (counts) ++counts->seg[seg_cat_idx].temp_pred[ctx][mbmi->seg_id_predicted[seg_cat_idx]];
          if (mbmi->seg_id_predicted[seg_cat_idx]) {
            segment_id = pred_segment_id;
          }
          else {
            const int spatial_ctx = top_mi && left_mi &&
              top_mi->mbmi.segment_id[seg_cat_idx] == left_mi->mbmi.segment_id[seg_cat_idx] ? 1 : 0;
            if (spatial_ctx) {
              seg_id_spatial_predicted = aom_read(r, segp->harmonic_spatial_pred_probs[0], ACCT_STR);
              if (counts) ++counts->seg[seg_cat_idx].harmonic_spatial_pred[seg_id_spatial_predicted];
            }
            else {
#if CONFIG_EC_MULTISYMBOL
              seg_id_spatial_predicted = aom_read_symbol(r, segp->heterogeneous_spatial_pred_cdf, HETEROGENEOUS_SPATIAL_PRED_PROBS + 1, ACCT_STR);
#else
              seg_id_spatial_predicted = aom_read_tree(r, av1_seg_id_spatial_prediction_tree, segp->heterogeneous_spatial_pred_probs, ACCT_STR);
#endif
              if (counts) ++counts->seg[seg_cat_idx].heterogeneous_spatial_pred[seg_id_spatial_predicted];
            }
            if (!seg_id_spatial_predicted) {
              ...
              segment_id = seg_id_spatial_predicted == SEG_ID_COPY_LEFT ? left_segment_id : above_segment_id;
            }
          }
        }
      }
        ...
        if (skip_seg_id_read && !seg->skip_seg_id_disabled) {
          //this is not final segment id
          segment_id = xd->left_mi ? left_segment_id : above_segment_id;
          mbmi->seg_id_spatial_predicted[seg_cat_idx] = xd->left_mi ? SEG_ID_COPY_LEFT : SEG_ID_COPY_ABOVE;
        }
        else {
          const int spatial_ctx = top_mi && left_mi &&
            top_mi->mbmi.segment_id[seg_cat_idx] == left_mi->mbmi.segment_id[seg_cat_idx] ? 1 : 0;
          ...
          mbmi->seg_id_spatial_predicted[seg_cat_idx] = seg_id_spatial_predicted;
        }
      }
      if (!skip_seg_id_read || seg->skip_seg_id_disabled) {
        MODE_INFO* l_mi = xd->mi[-(mi_row & (seg_map_minb_size - 1)) * cm->mi_stride
          - ((mi_col & (seg_map_minb_size - 1)))];
        l_mi->mbmi.segment_id[seg_cat_idx] = segment_id;
        l_mi->mbmi.seg_id_predicted[seg_cat_idx] = mbmi->seg_id_predicted[seg_cat_idx];
        l_mi->mbmi.seg_id_spatial_predicted[seg_cat_idx] = mbmi->seg_id_spatial_predicted[seg_cat_idx];
        av1_store_segment_id_into_map(segment_id,
          &cm->current_frame_seg_map[mi_offset - (mi_row & (seg_map_minb_size - 1)) * cm->mi_cols
          - ((mi_col & (seg_map_minb_size - 1)))],
          seg_cat_idx);
    ...
    else if (xd->seg_id_is_done[seg_cat_idx])
    {
      ...
      mbmi->seg_id_predicted[seg_cat_idx] = l_mi->mbmi.seg_id_predicted[seg_cat_idx];
      mbmi->seg_id_spatial_predicted[seg_cat_idx] = l_mi->mbmi.seg_id_spatial_predicted[seg_cat_idx];
      set_segment_id(cm, mi_offset, x_mis, y_mis, segment_id, seg_cat_idx);
    }
    if (last_block_seg_minb) {
      //for the whole blocks with size of seg_map_minb_size are skipped
      MODE_INFO* topleft_mi = xd->mi[-(mi_row & (seg_map_minb_size - 1)) * cm->mi_stride
        - ((mi_col & (seg_map_minb_size - 1)))];
      for (i = 0; i < AOMMIN(AOMMAX(mi_size_high[xd->mi[0]->mbmi.sb_type], seg_map_minb_size),
        cm->mi_rows - (mi_row & ~(seg_map_minb_size - 1))); i++) {
        for (j = 0; j < AOMMIN(AOMMAX(mi_size_wide[xd->mi[0]->mbmi.sb_type], seg_map_minb_size),
          cm->mi_cols - (mi_col & ~(seg_map_minb_size - 1))); j++) {
          MODE_INFO* l_mi = xd->mi[-(mi_row & (seg_map_minb_size - 1)) * cm->mi_stride
            - ((mi_col & (seg_map_minb_size - 1))) + i * cm->mi_stride + j];
          if (!xd->seg_id_is_done[seg_cat_idx]) {
            if (cm->seg[seg_cat_idx].temporal_update &&
              ...
              l_mi->mbmi.segment_id[seg_cat_idx] = left_mi ? left_mi->mbmi.segment_id[seg_cat_idx] :
            ...
            av1_store_segment_id_into_map(l_mi->mbmi.segment_id[seg_cat_idx],
              &cm->current_frame_seg_map[mi_offset - (mi_row & (seg_map_minb_size - 1)) * cm->mi_cols
              - ((mi_col & (seg_map_minb_size - 1))) + i * cm->mi_cols + j],
              seg_cat_idx);
          }
          else if (i != 0 || j != 0) {
            l_mi->mbmi.segment_id[seg_cat_idx] = topleft_mi->mbmi.segment_id[seg_cat_idx];
            l_mi->mbmi.seg_id_predicted[seg_cat_idx] = topleft_mi->mbmi.seg_id_predicted[seg_cat_idx];
            l_mi->mbmi.seg_id_spatial_predicted[seg_cat_idx] = topleft_mi->mbmi.seg_id_spatial_predicted[seg_cat_idx];
            segment_id = l_mi->mbmi.segment_id[seg_cat_idx];
            av1_store_segment_id_into_map(l_mi->mbmi.segment_id[seg_cat_idx],
              &cm->current_frame_seg_map[mi_offset - (mi_row & (seg_map_minb_size - 1)) * cm->mi_cols
              - ((mi_col & (seg_map_minb_size - 1))) + i * cm->mi_cols + j],
...
static void read_intra_frame_mode_info(AV1_COMMON *const cm,
...
#if CONFIG_EXT_SEGMENT
  mbmi->segment_id[ACTIVE_SEG_IDX] = read_segment_id(cm, xd, 0, ACTIVE_SEG_IDX, mi_row, mi_col, r);
  ...
  if (cm->seg[QUALITY_SEG_IDX].enabled) {
    int segment_id_skip =
      !av1_is_segfeature_enabled(&cm->seg[QUALITY_SEG_IDX], QUALITY_SEG_LVL_ALT_LF)
      && mbmi->skip;    
    mbmi->segment_id[QUALITY_SEG_IDX] = read_segment_id(cm, xd, segment_id_skip, QUALITY_SEG_IDX, mi_row, mi_col, r);
  }  
#endif
...
static void read_ref_frames(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (segfeature_active(&cm->seg[ACTIVE_SEG_IDX], segment_id, ACTIVE_SEG_LVL_REF_FRAME)) {
    ref_frame[0] = (MV_REFERENCE_FRAME)get_segdata(&cm->seg[ACTIVE_SEG_IDX], segment_id,
...
#if CONFIG_EXT_SEGMENT
  if (segfeature_active(&cm->seg[ACTIVE_SEG_IDX], segment_id, ACTIVE_SEG_LVL_REF_FRAME)) {
    return get_segdata(&cm->seg[ACTIVE_SEG_IDX], segment_id, ACTIVE_SEG_LVL_REF_FRAME) != INTRA_FRAME;
#else
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
...
#endif    
#if CONFIG_EXT_SEGMENT
    inter_block = read_is_inter_block(cm, xd, mbmi->segment_id[ACTIVE_SEG_IDX], r);
    if (cm->seg[QUALITY_SEG_IDX].update_map) {
      int seg_id_skip = (inter_block ||
        !av1_is_segfeature_enabled(&cm->seg[QUALITY_SEG_IDX], QUALITY_SEG_LVL_ALT_LF)
        )
        && mbmi->skip;
      mbmi->segment_id[QUALITY_SEG_IDX] = read_segment_id(cm, xd, seg_id_skip, QUALITY_SEG_IDX, mi_row, mi_col, r);      
    }
#else
