...
static void read_ref_frames(AV1_COMMON *const cm, MACROBLOCKD *const xd,
...
#endif
  {
    ...
    if (mode == COMPOUND_REFERENCE) {
      ...
      const int bit1 =
          aom_read_symbol(r, av1_get_pred_cdf_single_ref(xd, 1), 2, ACCT_STR);
      if (bit1) {
        const int bit2 =
            aom_read_symbol(r, av1_get_pred_cdf_single_ref(xd, 2), 2, ACCT_STR);
        if (!bit2) {
          const int bit6 = aom_read_symbol(
              r, av1_get_pred_cdf_single_ref(xd, 6), 2, ACCT_STR);
          ref_frame[0] = bit6 ? ALTREF2_FRAME : BWDREF_FRAME;
        } else {
          ref_frame[0] = ALTREF_FRAME;
        }
      } else {
        const int bit3 =
            aom_read_symbol(r, av1_get_pred_cdf_single_ref(xd, 3), 2, ACCT_STR);
        if (bit3) {
          const int bit5 = aom_read_symbol(
              r, av1_get_pred_cdf_single_ref(xd, 5), 2, ACCT_STR);
          ref_frame[0] = bit5 ? GOLDEN_FRAME : LAST3_FRAME;
        } else {
          const int bit4 = aom_read_symbol(
              r, av1_get_pred_cdf_single_ref(xd, 4), 2, ACCT_STR);
          ref_frame[0] = bit4 ? LAST2_FRAME : LAST_FRAME;
        }
      }
