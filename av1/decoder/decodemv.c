...
static int read_mv_component(aom_reader *r, const nmv_component *mvcomp,
  ...
  const int mv_class =
#if CONFIG_DAALA_EC
      av1_mv_class_inv[aom_read_symbol(r, mvcomp->class_cdf, MV_CLASSES,
                                       ACCT_STR)];
#else
  ...
  // Fractional part
#if CONFIG_DAALA_EC
  fr = av1_mv_fp_inv[aom_read_symbol(
      r, class0 ? mvcomp->class0_fp_cdf[d] : mvcomp->fp_cdf, MV_FP_SIZE,
      ACCT_STR)];
#else
