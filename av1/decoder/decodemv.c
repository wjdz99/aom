  ...
  if (seg_id_read || !xd->seg_id_is_done[seg_cat_idx]) {
    ...
    if (seg->temporal_update) {
      if (skip_seg_id_read) {
        ...
        const int ctx = av1_get_pred_context_seg_id(xd, seg_cat_idx);
        ...
        if (mbmi->seg_id_predicted) {
          ...
          const int spatial_ctx = av1_get_spatial_pred_context(xd, seg_cat_idx);
          if (spatial_ctx) {
...
#else
            mbmi->seg_id_spatial_predicted[seg_cat_idx] = aom_read_tree(r, av1_seg_id_spatial_prediction_tree, segp->heterogeneous_spatial_pred_probs, ACCT_STR);
#endif
    ...
    MODE_INFO *topleft_mi = xd->mi[0] - (mi_row - (mi_row & ~(seg_map_minb_size - 1))) * xd->mi_stride
      - (mi_col - (mi_col & ~(seg_map_minb_size - 1)));
    mbmi->seg_id_predicted[seg_cat_idx] = topleft_mi->mbmi.seg_id_predicted[seg_cat_idx];
    mbmi->seg_id_spatial_predicted[seg_cat_idx] = topleft_mi->mbmi.seg_id_spatial_predicted[seg_cat_idx];
    segment_id = topleft_mi->mbmi.segment_id[seg_cat_idx];
  }
