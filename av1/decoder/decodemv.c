...
static void read_intrabc_info(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (mbmi->use_intrabc) {
    ...
    av1_find_best_ref_mvs(MV_SUBPEL_QTR_PRECISION, ref_mvs[INTRA_FRAME], &nearestmv,
                          &nearmv, 0);
...
static void read_inter_block_mode_info(AV1Decoder *const pbi,
  ...
  if (!is_compound && mbmi->mode != GLOBALMV) {
    av1_find_best_ref_mvs(precision, ref_mvs[mbmi->ref_frame[0]], &nearestmv[0],
  ...
  if (is_compound && mbmi->mode != GLOBAL_GLOBALMV) {
    ...
    lower_mv_precision(&nearestmv[0].as_mv, precision,
                       cm->cur_frame_force_integer_mv);
    lower_mv_precision(&nearestmv[1].as_mv, precision,
                       cm->cur_frame_force_integer_mv);
    lower_mv_precision(&nearmv[0].as_mv, precision,
                       cm->cur_frame_force_integer_mv);
    lower_mv_precision(&nearmv[1].as_mv, precision,
                       cm->cur_frame_force_integer_mv);
  ...
  int mv_corrupted_flag =
      !assign_mv(cm, xd, mbmi->mode, mbmi->ref_frame, mbmi->mv, ref_mv,
                 nearestmv, nearmv, mi_row, mi_col, is_compound, precision, r);
