...
static void read_inter_block_mode_info(AV1Decoder *const pbi,
  ...
  if (mbmi->skip_mode) {
    ...
    if (segfeature_active(&cm->seg, mbmi->segment_id, SEG_LVL_SKIP) ||
      ...
      if (mbmi->derived_mv_allowed) {
        mbmi->pick_derived_mv =
            aom_read_symbol(r, ec_ctx->pick_derived_mv_cdf[bsize], 2, ACCT_STR);
      }
#endif  // CONFIG_DERIVED_MV
  ...
  if (mbmi->derived_mv_allowed && mbmi->pick_derived_mv) {
    ...
    mbmi->derived_mv = av1_refine_mv(cm, xd, mbmi, xd->plane[0].dst.buf,
...
#endif  // CONFIG_DERIVED_INTRA_MODE
#if CONFIG_DERIVED_MV
  mi->derived_mv_allowed = 0;
  mi->pick_derived_mv = 0;
#endif  // CONFIG_DERIVED_MV
  mi->ref_mv_idx = 0;
