...
static uint8_t read_mrl_index(FRAME_CONTEXT *ec_ctx, aom_reader *r,
                              uint8_t mrls_flag, PREDICTION_MODE mode) {
  if (mrls_flag == 0 || av1_is_directional_mode(mode) == 0) return 0;
  const uint8_t mrl_index =
      aom_read_symbol(r, ec_ctx->mrl_index_cdf, MRL_LINE_COUNT, ACCT_STR);
  return mrl_index;
}
#endif
...
static void read_intra_frame_mode_info(AV1_COMMON *const cm,
  ...
  if (xd->tree_type != CHROMA_PART)
    ...
    mbmi->mrl_index =
        read_mrl_index(ec_ctx, r, cm->seq_params.enable_mrls, mbmi->mode);
#endif
#if CONFIG_SDP
