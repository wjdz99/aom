...
static PREDICTION_MODE read_intra_mode_cdf(aom_reader *r,
  ...
  return (PREDICTION_MODE)av1_intra_mode_inv[aom_read_symbol(r, cdf,
      INTRA_MODES, ACCT_STR)];
}
#endif
...
#if CONFIG_DAALA_EC
      read_intra_mode_cdf(r, cm->fc->uv_mode_cdf[y_mode]);
#else
...
#else
#if CONFIG_DAALA_EC
  const int mode = av1_inter_mode_inv[aom_read_symbol(r,
      cm->fc->inter_mode_cdf[ctx], INTER_MODES, ACCT_STR)];
#else
  ...
  switch (bsize) {
    case BLOCK_4X4:
      for (i = 0; i < 4; ++i)
        mi->bmi[i].as_mode =
#if CONFIG_DAALA_EC
            read_intra_mode_cdf(r, get_y_mode_cdf(cm, mi, above_mi, left_mi, i));
#else
...
static INLINE void read_mv(aom_reader *r, MV *mv, const MV *ref,
                           ...
                           int allow_hp) {
  const MV_JOINT_TYPE joint_type = (MV_JOINT_TYPE)
#if CONFIG_DAALA_EC
      av1_mv_joint_inv[aom_read_symbol(r, ctx->joint_cdf, MV_JOINTS, ACCT_STR)];
#else
      aom_read_tree(r, av1_mv_joint_tree, ctx->joints, ACCT_STR);
#endif
