...
#if CONFIG_DERIVED_INTRA_MODE
  const int is_dr =
      aom_read_symbol(r, get_kf_is_dr_mode_cdf(ec_ctx, xd->above_mbmi,
                                               xd->left_mbmi), 2, ACCT_STR);
  if (is_dr) {
    if (av1_enable_derived_intra_mode(xd, bsize)) {
      mbmi->use_derived_intra_mode =
          aom_read_symbol(r, get_derived_intra_mode_cdf(
              ec_ctx, xd->above_mbmi, xd->left_mbmi), 2, ACCT_STR);
    }
    ...
    if (read_y_mode) {
      const int index =
          aom_read_symbol(
              r, get_kf_dr_mode_cdf(ec_ctx, xd->above_mbmi,
                                    xd->left_mbmi), DIRECTIONAL_MODES, ACCT_STR);
      mbmi->mode = dr_index_to_mode[index];
    }
  } else {
    const int index =
        aom_read_symbol(
            r, get_kf_none_dr_mode_cdf(ec_ctx, xd->above_mbmi,
                                       xd->left_mbmi), NONE_DIRECTIONAL_MODES,
                                       ACCT_STR);
