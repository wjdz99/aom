...
static void read_intra_block_mode_info(AV1_COMMON *const cm, const int mi_row,
  ...
  const int is_dr = aom_read_symbol(
      r, ec_ctx->bf_is_dr_mode_cdf[ctx], 2, ACCT_STR);
  if (is_dr) {
    if (av1_enable_derived_intra_mode(xd, bsize)) {
      mbmi->use_derived_intra_mode[0] = aom_read_symbol(
          r, get_derived_intra_mode_cdf(ec_ctx, xd->above_mbmi, xd->left_mbmi),
    ...
    if (mbmi->use_derived_intra_mode[0]) {
      ...
      const int index = aom_read_symbol(
          r, ec_ctx->bf_dr_mode_cdf[ctx], DIRECTIONAL_MODES, ACCT_STR);
      mbmi->mode = dr_index_to_mode[index];
    }
  } else {
    const int index = aom_read_symbol(
        r, ec_ctx->bf_none_dr_mode_cdf[ctx], NONE_DIRECTIONAL_MODES, ACCT_STR);
    mbmi->mode = none_dr_index_to_mode[index];
  }
#else
