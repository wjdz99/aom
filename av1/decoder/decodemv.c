...
static void read_inter_block_mode_info(AV1Decoder *const pbi,
  ...
  if (ref_frame < REF_FRAMES) { 
    ref_frame_nrs = convert_named_ref_to_ranked_ref_index(&cm->new_ref_frame_data, ref_frame);
    // TODO(sarahparker) Temporary assert
    assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data,
                                           ref_frame_nrs) ==
     ref_frame);
  }
  av1_find_mv_refs(cm, xd, mbmi, ref_frame, ref_frame_nrs, dcb->ref_mv_count, xd->ref_mv_stack,
