...
static INLINE int assign_mv(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  switch (mode) {
    ...
    case NEAREST_NEARESTMV: {
      ...
      if (!is_compound) {
        printf("decodemv.c: Sth went wrong Frame=%d, (mi_row,mi_col)=(%d,%d), "
               "bsize=%d, mode=%d, is_comp_inter_mode=%d, show_frame=%d\n",
               cm->current_video_frame, mi_row, mi_col, bsize, mode, is_comp_inter_mode,
               cm->show_frame);
        assert(0);
      }
#else  // COMPOUND_SINGLEREF_DEBUG
      assert(is_compound);
...
static void read_inter_block_mode_info(AV1Decoder *const pbi,
...
#if CONFIG_EXT_INTER && CONFIG_COMPOUND_SINGLEREF
  is_comp_inter_mode = is_compound;
  ...
  } else if (!is_compound && bsize >= BLOCK_8X8) {
