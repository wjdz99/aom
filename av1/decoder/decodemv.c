...
static void read_intra_frame_mode_info(AV1_COMMON *const cm,
  ...
  const int is_dr = aom_read_symbol(
      r, get_kf_is_dr_mode_cdf(ec_ctx, above_mi, left_mi), 2,
      ACCT_STR);
  if (is_dr) {
    if (av1_enable_derived_intra_mode(xd, bsize)) {
      mbmi->use_derived_intra_mode[0] = aom_read_symbol(
          r, get_derived_intra_mode_cdf(ec_ctx, above_mi, left_mi),
          2, ACCT_STR);
    }
    ...
    if (read_y_mode) {
      const int index = aom_read_symbol(
          r, get_kf_dr_mode_cdf(ec_ctx, above_mi, left_mi),
          DIRECTIONAL_MODES, ACCT_STR);
      mbmi->mode = dr_index_to_mode[index];
    }
  } else {
    const int index = aom_read_symbol(
        r, get_kf_none_dr_mode_cdf(ec_ctx, above_mi, left_mi),
