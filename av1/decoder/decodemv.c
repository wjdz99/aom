...
static INLINE int assign_mv(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  switch (mode) {
    ...
    case SR_NEAREST_NEARMV: {
      assert(!is_compound && bsize >= BLOCK_8X8);
      mv[0].as_int = nearest_mv[0].as_int;
    ...
    case SR_NEAREST_NEWMV: {
      ...
      nmv_context_counts *const mv_counts =
          counts ? &counts->mv[nmv_ctx] : NULL;
      read_mv(r, &mv[1].as_mv, &ref_mv[0].as_mv, &ec_ctx->nmvc[nmv_ctx],
              mv_counts, allow_hp);
#else
      nmv_context_counts *const mv_counts = counts ? &counts->mv : NULL;
    ...
    case SR_NEW_NEWMV: {
      ...
      for (i = 0; i < 2; ++i) {
        ...
        nmv_context_counts *const mv_counts =
            counts ? &counts->mv[nmv_ctx] : NULL;
        read_mv(r, &mv[i].as_mv, &ref_mv[0].as_mv, &ec_ctx->nmvc[nmv_ctx],
                mv_counts, allow_hp);
#else
...
static void read_inter_block_mode_info(AV1Decoder *const pbi,
  ...
  printf(
      "Frame=%d, (mi_row,mi_col)=(%d,%d), bsize=%d, is_compound=%d, "
      "ref_frame=(%d,%d), is_singleref_comp_mode=%d\n",
      cm->current_video_frame, mi_row, mi_col, bsize, is_compound,
      mbmi->ref_frame[0], is_compound ? mbmi->ref_frame[1] : -1,
      is_singleref_comp_mode);
