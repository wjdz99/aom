...
static int read_cfl_alphas(FRAME_CONTEXT *const ec_ctx, aom_reader *r,
  ...
  assert(sign_u == (joint_sign + 1) / CFL_SIGNS);
  const int sign_v = signs_out[CFL_PRED_V] = CFL_SIGN_V(joint_sign);
  assert(sign_v == (joint_sign + 1) % CFL_SIGNS);

  int ind = 0;
  // Magnitudes are only coded for nonzero values
  if (sign_u != CFL_SIGN_ZERO)
    ind = aom_read_symbol(r, ec_ctx->cfl_alpha_cdf[joint_sign][CFL_PRED_U],
                          UV_ALPHABET_SIZE, "cfl:alpha_u")
          << UV_ALPHABET_SIZE_LOG2;
  if (sign_v != CFL_SIGN_ZERO)
    ind += aom_read_symbol(r, ec_ctx->cfl_alpha_cdf[joint_sign][CFL_PRED_V],
