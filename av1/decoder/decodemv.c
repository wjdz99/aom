...
static void read_cdef(AV1_COMMON *cm, aom_reader *r, MB_MODE_INFO *const mbmi,
  ...
  if (!(mi_col & (cm->mib_size - 1)) && !(mi_row & (cm->mib_size - 1))) {
    ...
    if (cm->sb_size == BLOCK_128X128) {
      cm->cdef_unassigned_strengths[0] = aom_read_literal(r, cm->cdef_bits, ACCT_STR);
      cm->cdef_unassigned_strengths[1] = aom_read_literal(r, cm->cdef_bits, ACCT_STR);
      cm->cdef_unassigned_strengths[2] = aom_read_literal(r, cm->cdef_bits, ACCT_STR);
    }
#endif
    return;
  }
