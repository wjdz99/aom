...
static void update_block_reference_mode(AV1_COMMON *cm, const MACROBLOCKD *xd,
                                        REFERENCE_MODE mode, uint8_t allow_update_cdf) {
  if (cm->reference_mode == REFERENCE_MODE_SELECT) {
...
static void update_comp_reference_type(AV1_COMMON *cm, const MACROBLOCKD *xd,
                                       COMP_REFERENCE_TYPE comp_ref_type, uint8_t allow_update_cdf) {
...
static void set_ref_frames_for_skip_mode(AV1_COMMON *const cm,
                                         ...
                                         MV_REFERENCE_FRAME ref_frame[2], uint8_t allow_update_cdf) {
...
static void read_ref_frames(AV1_COMMON *const cm, MACROBLOCKD *const xd,
...
#endif
  {
    ...
    if (xd->mi[0]->mbmi.skip_mode) {
      set_ref_frames_for_skip_mode(cm, xd, r, ref_frame);
      return;
    }
#endif  // CONFIG_EXT_SKIP
...
static void update_block_intra_inter(AV1_COMMON *const cm,
                                     ...
                                     const int is_inter, uint8_t allow_update_cdf) {
  if (!segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    const int ctx = av1_get_intra_inter_context(xd);
#if CONFIG_NEW_MULTISYMBOL
    FRAME_CONTEXT *ec_ctx = xd->tile_ctx;
...
static void read_inter_frame_mode_info(AV1Decoder *const pbi,
  ...
  if (mbmi->skip_mode) {
    ...
    update_block_intra_inter(cm, xd, mbmi->segment_id, inter_block, r->allow_update_cdf);
  } else {
#endif  // CONFIG_EXT_SKIP
    mbmi->skip = read_skip(cm, xd, mbmi->segment_id, r);
