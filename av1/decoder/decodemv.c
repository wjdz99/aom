  ...
  if (is_compound && (bsize >= BLOCK_8X8 || unify_bsize) &&
    ...
    if (xd->ref_mv_count[ref_frame_type] > 1) {
      ...
      if (mbmi->mode == NEW_NEARMV || mbmi->mode == NEAREST_NEARMV ||
          mbmi->mode == NEAR_NEARMV) {
#if CONFIG_COMPOUND_SINGLEREF
        if (mbmi->mode == NEAREST_NEARMV && !is_compound)
          nearmv[1] = xd->ref_mv_stack[ref_frame_type][1].this_mv;
        else
#endif  // CONFIG_COMPOUND_SINGLEREF
...
#define FRAME_TO_CHECK 87
  if (cm->current_video_frame == FRAME_TO_CHECK &&
    ...
    mv[0].as_int =
        (bsize < BLOCK_8X8) ? mi->bmi[0].as_mv[0].as_int : mbmi->mv[0].as_int;
    mv[1].as_int =
        (!is_comp_ref) ? 0 : ((bsize < BLOCK_8X8) ? mi->bmi[0].as_mv[1].as_int
                                                  : mbmi->mv[1].as_int);
