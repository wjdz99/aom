...
#if CONFIG_DAALA_EC
  const INTERP_FILTER type = (INTERP_FILTER)od_ec_decode_cdf_q15(&r->ec,
      cm->fc->switchable_interp_cdf[ctx], SWITCHABLE_FILTERS, "aom");
#else
...
static void read_inter_frame_mode_info(AV1Decoder *const pbi,
  ...
  if (mbmi->tx_size < TX_32X32 && cm->base_qindex > 0 && !mbmi->skip &&
    ...
    if (inter_block) {
#if CONFIG_DAALA_EC
      mbmi->tx_type = av1_ext_tx_inv[od_ec_decode_cdf_q15(&r->ec,
          cm->fc->inter_ext_tx_cdf[mbmi->tx_size], TX_TYPES, "aom")];
#else
