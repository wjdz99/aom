...
static void read_inter_block_mode_info(AV1Decoder *const pbi,
  ...
  if (is_compound) {
    ...
    av1_find_mv_refs(cm, xd, mi, ref_frame, xd->ref_mv_count, xd->ref_mv_stack,
                     compound_inter_mode_ctx, ref_mvs[ref_frame], mi_row,
                     mi_col, fpm_sync, (void *)pbi, inter_mode_ctx, 0);
    if (xd->ref_mv_count[ref_frame] <= 1) {
      for (int ref = 0; ref < 1 + is_compound; ++ref) {
        MV_REFERENCE_FRAME frame = mbmi->ref_frame[ref];

        av1_find_mv_refs(cm, xd, mi, frame, xd->ref_mv_count, xd->ref_mv_stack,
                         compound_inter_mode_ctx, ref_mvs[frame], mi_row,
                         mi_col, fpm_sync, (void *)pbi, inter_mode_ctx, 0);
      }
    }
  } else {
    ...
    av1_find_mv_refs(cm, xd, mi, frame, xd->ref_mv_count, xd->ref_mv_stack,
                     compound_inter_mode_ctx, ref_mvs[frame], mi_row, mi_col,
