...
static void read_drl_idx(FRAME_CONTEXT *ec_ctx, const AV1_COMMON *cm,
  ...
  if (mbmi->pb_mv_precision < mbmi->max_mv_precision &&
    ...
    for (int idx = 0; idx < range_adj; ++idx) {
      aom_cdf_prob *drl_cdf = av1_get_drl_cdf(mode_ctx, ec_ctx, mbmi->mode,
                                              xd->ref_mv_info.ref_mv_weight_adj, idx);
  ...
  int range = AOMMIN(xd->ref_mv_info.ref_mv_count[ref_frame_type] - 1, MAX_DRL_BITS);
...
static void read_inter_block_mode_info(AV1Decoder *const pbi,
  ...
  if (is_compound && mbmi->mode != GLOBAL_GLOBALMV) {
    ...
    int_mv cur_mv =
        xd->ref_mv_info.ref_mv_stack[mbmi->ref_frame[0]][mbmi->ref_mv_idx].this_mv;
    nearmv[0] = cur_mv;
  }
#else
  } else if (mbmi->ref_mv_idx > 0 && mbmi->mode == NEARMV) {
    nearmv[0] =
        xd->ref_mv_info.ref_mv_stack[mbmi->ref_frame[0]][1 + mbmi->ref_mv_idx].this_mv;
  }
#endif  // CONFIG_NEW_INTER_MODES
