  ...
  if (cm->interp_filter != SWITCHABLE) {
    ...
    for (int dir = 0; dir < 2; ++dir) {
      if (has_subpel_mv_component(xd->mi[0], xd, dir) ||
        ...
        const int raw = aom_read_symbol(r, ec_ctx->switchable_interp_cdf[ctx],
                                        SWITCHABLE_FILTERS, ACCT_STR);
        ref0_filter[dir] = (InterpFilter)av1_switchable_interp_inv[raw];
    ...
    mbmi->interp_filters =
        av1_duplicate_interp_filters(ref0_filter[0], ref0_filter[1]);
#else   // CONFIG_DUAL_FILTER
    const int ctx = av1_get_pred_context_switchable_interp(xd);
    const int raw = aom_read_symbol(r, ec_ctx->switchable_interp_cdf[ctx],
                                    SWITCHABLE_FILTERS, ACCT_STR);
    InterpFilter filter = (InterpFilter)av1_switchable_interp_inv[raw];
