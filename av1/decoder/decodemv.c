...
#if CONFIG_DAALA_EC
      const InterpFilter type = (InterpFilter)av1_switchable_interp_inv[
          aom_read_tree_cdf(r, cm->fc->switchable_interp_cdf[ctx],
          SWITCHABLE_FILTERS)];
#else
...
static void read_inter_frame_mode_info(AV1Decoder *const pbi,
  ...
  if (mbmi->tx_size < TX_32X32 && cm->base_qindex > 0 && !mbmi->skip &&
    ...
    if (inter_block) {
#if CONFIG_DAALA_EC
      mbmi->tx_type = av1_ext_tx_inv[aom_read_tree_cdf(r,
