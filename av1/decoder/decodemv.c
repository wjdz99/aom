...
static int read_angle_delta_hv(aom_reader *r, aom_cdf_prob *cdf) {
	const int sym = aom_read_symbol(r, cdf, 2 * MAX_ANGLE_DELTA + 1 + ADDITIONAL_ANGLE_DELTA, ACCT_STR);
...
static void read_intra_frame_mode_info(AV1_COMMON *const cm,
  ...
  if (use_angle_delta && av1_is_directional_mode(mbmi->mode))
  {
	  ...
	  aom_cdf_prob *anglecdf = signal_intra_filter ? ec_ctx->angle_delta_cdf_hv[mbmi->mode - V_PRED] : ec_ctx->angle_delta_cdf[mbmi->mode - V_PRED];
	  mbmi->angle_delta[PLANE_TYPE_Y] = signal_intra_filter ? read_angle_delta_hv(r, anglecdf) : read_angle_delta(r, anglecdf);
  }
  else
  {
