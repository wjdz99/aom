...
static int uint16_comparer(const void *a, const void *b) {
  const uint16 fa = *(const uint16 *)a;
  const uint16 fb = *(const uint16 *)b;
  return (fa > fb) - (fa < fb);
}
#endif  // CONFIG_HIGHBITDEPTH
static int uint8_comparer(const void *a, const void *b) {
  const uint8_t fa = *(const uint8_t *)a;
  const uint8_t fb = *(const uint8_t *)b;
  return (fa > fb) - (fa < fb);
}
...
#if CONFIG_HIGHBITDEPTH
  qsort(pmi->palette_colors, n, sizeof(pmi->palette_colors[0]),
        uint16_comparer);
#else
  qsort(pmi->palette_colors, n, sizeof(pmi->palette_colors[0]), uint8_comparer);
#endif  // CONFIG_HIGHBITDEPTH
}
...
static void read_palette_colors_uv(MACROBLOCKD *const xd, int bit_depth,
  ...
  for (i = 0; i < n_cache && idx < PALETTE_MAX_SIZE + n; ++i) {
    if (aom_read_bit(r, ACCT_STR)) pmi->palette_colors[idx++] = color_cache[i];
  }
  ...
  qsort(pmi->palette_colors + PALETTE_MAX_SIZE, n,
        sizeof(pmi->palette_colors[0]), uint16_comparer);
#else
  qsort(pmi->palette_colors + PALETTE_MAX_SIZE, n,
        sizeof(pmi->palette_colors[0]), uint8_comparer);
#endif  // CONFIG_HIGHBITDEPTH
