  ...
  if (mbmi->pb_mv_precision < mbmi->max_mv_precision &&
    ...
    for (int idx = 0; idx < range_adj; ++idx) {
      aom_cdf_prob *drl_cdf =
          av1_get_drl_cdf(mode_ctx, ec_ctx, mbmi->mode, xd->ref_mv_info.weight_adj, idx);
...
static void read_intrabc_info(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (mbmi->use_intrabc) {
    ...
    av1_find_mv_refs(cm, xd, mbmi, INTRA_FRAME, &xd->ref_mv_info, ref_mvs, /*global_mvs=*/NULL,
                     inter_mode_ctx);
...
static void read_inter_block_mode_info(AV1Decoder *const pbi,
  ...
  av1_find_mv_refs(cm, xd, mbmi, ref_frame, &xd->ref_mv_info, ref_mvs, /*global_mvs=*/NULL, inter_mode_ctx);
    ...
    if (mbmi->mode == NEWMV) {
      ...
      if (mbmi->pb_mv_precision < mbmi->max_mv_precision) {
        if (xd->ref_mv_info.ref_mv_count_adj > 0)
          ref_mv[0] = xd->ref_mv_info.ref_mv_stack_adj[mbmi->ref_mv_idx_adj].this_mv;
      } else {
#endif  // CONFIG_FLEX_MVRES && ADJUST_DRL_FLEX_MVRES
        if (xd->ref_mv_info.count[ref_frame] > 0)
          ref_mv[0] = xd->ref_mv_info.stack[ref_frame][mbmi->ref_mv_idx].this_mv;
#if CONFIG_FLEX_MVRES && ADJUST_DRL_FLEX_MVRES
      }
#endif  // CONFIG_FLEX_MVRES && ADJUST_DRL_FLEX_MVRES
    }
