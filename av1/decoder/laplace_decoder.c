...
#if OD_ACCOUNTING
# define aom_decode_pvq_split(r, adapt, sum, ctx, str) aom_decode_pvq_split_(r, adapt, sum, ctx, str)
#else
# define aom_decode_pvq_split(r, adapt, sum, ctx, str) aom_decode_pvq_split_(r, adapt, sum, ctx)
#endif
...
static int aom_decode_pvq_split_(aom_reader *r, od_pvq_codeword_ctx *adapt,
 int sum, int ctx OD_ACC_STR) {
  ...
  if (shift) count = aom_read_literal(r, shift, acc_str);
  count += msbs << shift;
  if (count > sum) {
    count = sum;
#if CONFIG_DAALA_EC
 ...
 int max OD_ACC_STR) {
  ...
  do {
    ...
    if (ms > 0 && ms < 15) {
      ...
      sym = aom_read_symbol_unscaled(r, cdf, ms + 1, acc_str);
    }
    else sym = aom_read_symbol(r, cdf, 16, acc_str);
    xs += sym;
    ms -= 15;
  }
  while (sym >= 15 && ms != 0);
  if (shift) pos = (xs << shift) + aom_read_literal(r, shift, acc_str);
  else pos = xs;
