...
static void store_bitmask_other_info(AV1_COMMON *cm, int mi_row, int mi_col,
  ...
  const int id_vert_border = mask_id_table_vert_border[bsize];
  const int vert_shift = block_size_high[bsize] <= 8 ? shift : col_start;
  for (int i = 0; i + index < 4; ++i) {
    lfm->is_vert_border.bits[i + index] |=
        (left_mask_univariant_reordered[id_vert_border].bits[i] << vert_shift);
  }
  const int is_skip = mbmi->skip && is_inter_block(mbmi);
  if (is_skip) {
    const int id_skip_mask = mask_id_table_tx_4x4[bsize];
    for (int i = 0; i + index < 4; ++i) {
      lfm->skip.bits[i + index] |=
          (above_mask_univariant_reordered[id_skip_mask].bits[i] << shift);
    }
  }
...
static void store_bitmask_info(AV1_COMMON *cm, int mi_row, int mi_col,
  ...
  if (type == 0) {
    ...
    */
    ...
    LoopFilterMask *lfm = get_loop_filter_mask(cm, mi_row, mi_col);
    /*
    // vertical direction --- implementation 1
    for (int r = mi_row; r < mi_row + mi_size_high[bsize]; ++r) {
      for (int c = mi_col; c < mi_col + mi_size_wide[bsize];) {
        int index = 0;
        ...
        const uint64_t mask = ((uint64_t)1 << shift);
        // Y plane
        const TX_SIZE tx_size = txsize_horz_map[mbmi->tx_size];
        lfm->tx_size_ver[0][tx_size].bits[index] |= mask;
        // U/V plane
        const TX_SIZE tx_size_uv = txsize_horz_map[av1_get_max_uv_txsize(
            mbmi->sb_type, cm->subsampling_x, cm->subsampling_y)];
    ...
    for (int c = mi_col; c < mi_col + mi_size_wide[bsize]; ++c) {
      for (int r = mi_row; r < mi_row + mi_size_high[bsize];) {
        ...
        const uint64_t mask = ((uint64_t)1 << shift);
        // Y plane
        const TX_SIZE tx_size = txsize_vert_map[mbmi->tx_size];
        lfm->tx_size_hor[0][tx_size].bits[index] |= mask;
        // U/V plane
        const TX_SIZE tx_size_uv = txsize_vert_map[av1_get_max_uv_txsize(
            mbmi->sb_type, cm->subsampling_x, cm->subsampling_y)];
    ...
    */
    ...
    for (int r = mi_row; r < mi_row + mi_size_high[bsize]; ++r) {
      for (int c = mi_col; c < mi_col + mi_size_wide[bsize]; ++c) {
        ...
        // TODO(chengchen): memset
        lfm->lfl_y_ver[row][col] = level_vert_y;
        lfm->lfl_y_hor[row][col] = level_horz_y;
        lfm->lfl_u[row][col] = level_u;
        lfm->lfl_v[row][col] = level_v;
      }
    }
    */
  }
...
static void reset_dec_workers(AV1Decoder *pbi, int num_workers) {
...
static const uint8_t *decode_tiles_mt(AV1Decoder *pbi, const uint8_t *data,
  ...
  reset_dec_workers(pbi, num_workers);
