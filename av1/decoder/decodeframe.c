...
static void read_tx_partition(MACROBLOCKD *xd, MB_MODE_INFO *mbmi,
  ...
  const int max_blocks_wide = max_block_wide(xd, bsize, 0);
  if (blk_row >= max_blocks_high || blk_col >= max_blocks_wide) return;
  TX_SIZE txs = sub_tx_size_map[max_txsize_rect_lookup[bsize]];
  ...
  TX_PARTITION_TYPE partition = aom_read_symbol(
  ...
  //printf("OUT tx size %d max tx %d part %d\n", mbmi->tx_size, max_tx_size, partition);
  ...
  if (mbmi->tx_size != max_tx_size) {
    //printf("\n\nmax %d og size %d\n", max_tx_size, tx_size);
    const TX_SIZE sub_txs = sub_tx_size_map[max_tx_size];
    const int bsw = tx_size_wide_unit[sub_txs];
    const int bsh = tx_size_high_unit[sub_txs];
    for (int row = 0; row < tx_size_high_unit[max_tx_size]; row += bsh)
      for (int col = 0; col < tx_size_wide_unit[max_tx_size]; col += bsw) {
        ...
        txfm_partition_update(xd->above_txfm_context + offsetc,
                          xd->left_txfm_context + offsetr, mbmi->tx_size, mbmi->tx_size);
     //   printf("subtx %d\n", mbmi->inter_tx_size[txb_size_index2]);
      }
  }
  */
//printf("tx size %d max tx %d\n", tx_size, max_tx_size);
//////////////////////////
}
