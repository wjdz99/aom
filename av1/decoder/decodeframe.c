...
static void read_coeffs_tx_intra_block_void(AV1_COMMON *cm,
...
static void read_coeffs_tx_inter_block_void(const AV1_COMMON *const cm,
                                            MACROBLOCKD *const xd,
...
static void decode_reconstruct_tx(AV1_COMMON *cm, ThreadData *const td,
  ...
  if (tx_size == plane_tx_size || plane) {
    td->read_coeffs_tx_inter_block_visit(cm, xd, r, blk_row, blk_col, plane,
                                         tx_size);

    td->inverse_tx_inter_block_visit(cm, xd, r, blk_row, blk_col, plane,
                                     tx_size);
    eob_info *eob_data = pd->eob_data + xd->txb_offset[plane];
...
static void set_decode_func_pointers(ThreadData *td, int parse_decode_flag) {
  td->read_coeffs_tx_intra_block_visit = read_coeffs_tx_intra_block_void;
  td->predict_and_recon_intra_block_visit =
      predict_and_reconstruct_intra_block_void;
  td->read_coeffs_tx_inter_block_visit = read_coeffs_tx_inter_block_void;
  td->inverse_tx_inter_block_visit = inverse_transform_inter_block_void;
