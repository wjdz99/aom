...
static TX_MODE read_tx_mode(aom_reader *r) {
  TX_MODE tx_mode = aom_read_literal(r, 2, AOM_ACCT_STR);
  if (tx_mode == ALLOW_32X32) tx_mode += aom_read_bit(r, AOM_ACCT_STR);
  return tx_mode;
}
#endif
...
static void read_tx_mode_probs(struct tx_probs *tx_probs, aom_reader *r) {
  ...
  for (i = 0; i < TX_SIZE_CONTEXTS; ++i)
    for (j = 0; j < TX_SIZES - 3; ++j)
      av1_diff_update_prob(r, &tx_probs->p8x8[i][j], AOM_ACCT_STR);

  for (i = 0; i < TX_SIZE_CONTEXTS; ++i)
    for (j = 0; j < TX_SIZES - 2; ++j)
      av1_diff_update_prob(r, &tx_probs->p16x16[i][j], AOM_ACCT_STR);

  for (i = 0; i < TX_SIZE_CONTEXTS; ++i)
    for (j = 0; j < TX_SIZES - 1; ++j)
      av1_diff_update_prob(r, &tx_probs->p32x32[i][j], AOM_ACCT_STR);
}
...
static void read_switchable_interp_probs(FRAME_CONTEXT *fc, aom_reader *r) {
  ...
  for (j = 0; j < SWITCHABLE_FILTER_CONTEXTS; ++j) {
    for (i = 0; i < SWITCHABLE_FILTERS - 1; ++i)
      av1_diff_update_prob(r, &fc->switchable_interp_prob[j][i], AOM_ACCT_STR);
#if CONFIG_DAALA_EC
...
static void read_inter_mode_probs(FRAME_CONTEXT *fc, aom_reader *r) {
  ...
  for (i = 0; i < NEWMV_MODE_CONTEXTS; ++i)
    av1_diff_update_prob(r, &fc->newmv_prob[i], AOM_ACCT_STR);
  for (i = 0; i < ZEROMV_MODE_CONTEXTS; ++i)
    av1_diff_update_prob(r, &fc->zeromv_prob[i], AOM_ACCT_STR);
  for (i = 0; i < REFMV_MODE_CONTEXTS; ++i)
    av1_diff_update_prob(r, &fc->refmv_prob[i], AOM_ACCT_STR);
  for (i = 0; i < DRL_MODE_CONTEXTS; ++i)
    av1_diff_update_prob(r, &fc->drl_prob[i], AOM_ACCT_STR);
#else
  int j;
  for (i = 0; i < INTER_MODE_CONTEXTS; ++i)
    for (j = 0; j < INTER_MODES - 1; ++j)
      av1_diff_update_prob(r, &fc->inter_mode_probs[i][j], AOM_ACCT_STR);
#endif
}
...
static REFERENCE_MODE read_frame_reference_mode(const AV1_COMMON *cm,
                                                aom_reader *r) {
  if (is_compound_reference_allowed(cm)) {
    return aom_read_bit(r, AOM_ACCT_STR)
               ? (aom_read_bit(r, AOM_ACCT_STR) ? REFERENCE_MODE_SELECT
                                                : COMPOUND_REFERENCE)
               : SINGLE_REFERENCE;
  } else {
    return SINGLE_REFERENCE;
...
static void read_frame_reference_mode_probs(AV1_COMMON *cm, aom_reader *r) {
  ...
  if (cm->reference_mode == REFERENCE_MODE_SELECT)
    for (i = 0; i < COMP_INTER_CONTEXTS; ++i)
      av1_diff_update_prob(r, &fc->comp_inter_prob[i], AOM_ACCT_STR);

  if (cm->reference_mode != COMPOUND_REFERENCE)
    for (i = 0; i < REF_CONTEXTS; ++i)
      for (j = 0; j < (SINGLE_REFS - 1); ++j)
        av1_diff_update_prob(r, &fc->single_ref_prob[i][j], AOM_ACCT_STR);

  if (cm->reference_mode != SINGLE_REFERENCE)
#if CONFIG_EXT_REFS
    for (i = 0; i < REF_CONTEXTS; ++i) {
      for (j = 0; j < (FWD_REFS - 1); ++j)
        av1_diff_update_prob(r, &fc->comp_fwdref_prob[i][j], AOM_ACCT_STR);
      for (j = 0; j < (BWD_REFS - 1); ++j) (r, &fc->comp_bwdref_prob[i][j]);
    }
#else
    for (i = 0; i < REF_CONTEXTS; ++i)
      av1_diff_update_prob(r, &fc->comp_ref_prob[i], AOM_ACCT_STR);
#endif  // CONFIG_EXT_REFS
}
...
static void update_mv_probs(aom_prob *p, int n, aom_reader *r) {
  ...
  for (i = 0; i < n; ++i)
#if CONFIG_MISC_FIXES
    av1_diff_update_prob(r, &p[i], AOM_ACCT_STR);
#else
    if (aom_read(r, MV_UPDATE_PROB, AOM_ACCT_STR))
      p[i] = (aom_read_literal(r, 7, AOM_ACCT_STR) << 1) | 1;
#endif
}
...
static PARTITION_TYPE read_partition(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  if (has_rows && has_cols)
    ...
    p = (PARTITION_TYPE)aom_read_symbol(r, cm->fc->partition_cdf[ctx],
                                        PARTITION_TYPES, AOM_ACCT_STR);
#else
    p = (PARTITION_TYPE)aom_read_tree(r, av1_partition_tree, probs,
                                      AOM_ACCT_STR);
#endif
  else if (!has_rows && has_cols)
    p = aom_read(r, probs[1], AOM_ACCT_STR) ? PARTITION_SPLIT : PARTITION_HORZ;
  else if (has_rows && !has_cols)
    p = aom_read(r, probs[2], AOM_ACCT_STR) ? PARTITION_SPLIT : PARTITION_VERT;
  else
    p = PARTITION_SPLIT;
...
static void read_coef_probs_common(av1_coeff_probs_model *coef_probs,
  ...
  if (aom_read_bit(r, AOM_ACCT_STR))
    for (i = 0; i < PLANE_TYPES; ++i)
      for (j = 0; j < REF_TYPES; ++j)
        for (k = 0; k < COEF_BANDS; ++k)
          for (l = 0; l < BAND_COEFF_CONTEXTS(k); ++l)
            for (m = 0; m < UNCONSTRAINED_NODES; ++m)
              av1_diff_update_prob(r, &coef_probs[i][j][k][l][m], AOM_ACCT_STR);
}
...
#if CONFIG_ACCOUNTING
        tile_data->bit_reader.accounting->last_tell =
...
static void read_ext_tx_probs(FRAME_CONTEXT *fc, aom_reader *r) {
  int i, j, k;
  if (aom_read(r, GROUP_DIFF_UPDATE_PROB, AOM_ACCT_STR)) {
  ...
  if (aom_read(r, GROUP_DIFF_UPDATE_PROB, AOM_ACCT_STR)) {
    for (i = TX_4X4; i < EXT_TX_SIZES; ++i) {
      for (k = 0; k < TX_TYPES - 1; ++k)
        av1_diff_update_prob(r, &fc->inter_ext_tx_prob[i][k], AOM_ACCT_STR);
#if CONFIG_DAALA_EC
...
static int read_compressed_header(AV1Decoder *pbi, const uint8_t *data,
  ...
  for (k = 0; k < SKIP_CONTEXTS; ++k)
    av1_diff_update_prob(&r, &fc->skip_probs[k], AOM_ACCT_STR);

#if CONFIG_MISC_FIXES
  if (cm->seg.enabled && cm->seg.update_map) {
    if (cm->seg.temporal_update) {
      for (k = 0; k < PREDICTION_PROBS; k++)
        av1_diff_update_prob(&r, &cm->fc->seg.pred_probs[k], AOM_ACCT_STR);
    }
    for (k = 0; k < MAX_SEGMENTS - 1; k++)
      av1_diff_update_prob(&r, &cm->fc->seg.tree_probs[k], AOM_ACCT_STR);
  }

  for (j = 0; j < INTRA_MODES; j++)
    for (i = 0; i < INTRA_MODES - 1; ++i)
      av1_diff_update_prob(&r, &fc->uv_mode_prob[j][i], AOM_ACCT_STR);

  for (j = 0; j < PARTITION_CONTEXTS; ++j) {
    for (i = 0; i < PARTITION_TYPES - 1; ++i)
      av1_diff_update_prob(&r, &fc->partition_prob[j][i], AOM_ACCT_STR);
#if CONFIG_DAALA_EC
  ...
  if (frame_is_intra_only(cm)) {
    ...
    for (k = 0; k < INTRA_MODES; k++)
      for (j = 0; j < INTRA_MODES; j++)
        for (i = 0; i < INTRA_MODES - 1; ++i)
          av1_diff_update_prob(&r, &cm->kf_y_prob[k][j][i], AOM_ACCT_STR);
#endif
  } else {
#if !CONFIG_REF_MV
    ...
    for (j = 0; j < BLOCK_SIZES; ++j)
      if (is_motion_variation_allowed_bsize(j)) {
        for (i = 0; i < MOTION_MODES - 1; ++i)
          av1_diff_update_prob(&r, &fc->motion_mode_prob[j][i], AOM_ACCT_STR);
      }
#endif  // CONFIG_MOTION_VAR
    ...
    for (i = 0; i < INTRA_INTER_CONTEXTS; i++)
      av1_diff_update_prob(&r, &fc->intra_inter_prob[i], AOM_ACCT_STR);
    ...
    for (j = 0; j < BLOCK_SIZE_GROUPS; j++)
      for (i = 0; i < INTRA_MODES - 1; ++i)
        av1_diff_update_prob(&r, &fc->y_mode_prob[j][i], AOM_ACCT_STR);
