...
static void decode_reconstruct_tx(
    ...
    tx_inter_block_visitor_fn_t tx_inter_block_visit) {
  const struct macroblockd_plane *const pd = &xd->plane[plane];
  ...
  if (tx_size == plane_tx_size || plane) {
    tx_inter_block_visit(cm, xd, r, blk_row, blk_col, plane, tx_size);
...
static void decode_token_or_recon_block(
    AV1Decoder *const pbi, MACROBLOCKD *const xd, int mi_row, int mi_col,
    aom_reader *r, BLOCK_SIZE bsize, intra_block_visitor_fn_t intra_block_visit,
  ...
  if (!is_inter_block(mbmi)) {
        ...
        for (int plane = 0; plane < num_planes; ++plane) {
          ...
          for (int blk_row = row >> pd->subsampling_y; blk_row < unit_height;
            ...
            for (int blk_col = col >> pd->subsampling_x; blk_col < unit_width;
                 blk_col += stepc) {
              intra_block_visit(cm, xd, r, plane, blk_row, blk_col, tx_size);
              set_cb_buffer_offsets(xd, tx_size, plane);
            }
          }
        }
    ...
    if (predict_inter_block_visit)
      predict_inter_block_visit(cm, xd, mi_row, mi_col, bsize);
#if CONFIG_MISMATCH_DEBUG
    for (int plane = 0; plane < num_planes; ++plane) {
      const struct macroblockd_plane *pd = &xd->plane[plane];
      int pixel_c, pixel_r;
    ...
    if (cfl_store_inter_block_visit) cfl_store_inter_block_visit(cm, xd);
  }
...
static void parse_block(AV1Decoder *const pbi, MACROBLOCKD *const xd,
                        int mi_row, int mi_col, aom_reader *r,
                        PARTITION_TYPE partition, BLOCK_SIZE bsize) {
  ...
  decode_token_or_recon_block(pbi, xd, mi_row, mi_col, r, bsize,
                              read_coeffs_tx_intra_block, NULL,
                              av1_read_coeffs_txb_facade, NULL);
...
static void decode_block(AV1Decoder *const pbi, MACROBLOCKD *const xd,
                         ...
                         PARTITION_TYPE partition, BLOCK_SIZE bsize) {
  (void)partition;
  decode_token_or_recon_block(
      pbi, xd, mi_row, mi_col, r, bsize, predict_and_reconstruct_intra_block,
      predict_inter_block, inverse_transform_inter_block,
...
static void decode_partition(AV1Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  static const block_visitor_fn_t block_visit[2] = { parse_block,
                                                     decode_block };
...
#define DEC_BLOCK(db_r, db_c, db_subsize)                             \
  block_visit[!is_parse](pbi, xd, DEC_BLOCK_STX_ARG(db_r), (db_c), r, \
...
static void decode_tile_sb_row(AV1Decoder *pbi, ThreadData *const td,
  ...
  for (int mi_col = tile_info.mi_col_start; mi_col < tile_info.mi_col_end;
    ...
    decode_partition(pbi, &td->xd, mi_row, mi_col, td->bit_reader,
                     cm->seq_params.sb_size, is_parse);
  }
}
#endif
