...
static int av1_pvq_decode_helper(MACROBLOCKD *xd, tran_low_t *ref_coeff,
...
#ifdef CONFIG_AOM_HIGHBITDEPTH
  quantizer_downshift =  xd->bd - 8;
#endif
  ...
  if (lossless)
    ...
    if (use_activity_masking)
      pvq_dc_quant = OD_MAXI(
                             1, (quant[0] << (OD_COEFF_SHIFT - 3) >> quantizer_downshift) *
                     dec->state.pvq_qm_q4[pli][od_qm_get_index(bs, 0)] >>
                 4);
    else
      pvq_dc_quant = OD_MAXI(1, quant[0] << (OD_COEFF_SHIFT - 3) >> quantizer_downshift);
  }
  ...
  for (i = 0; i < blk_size * blk_size; i++) {
    ref_int32[i] =
        AOM_SIGNED_SHL(ref_coeff_pvq[i], OD_COEFF_SHIFT - coeff_shift) >> quantizer_downshift;
  }

  od_pvq_decode(dec, ref_int32, out_int32, quant[1] << (OD_COEFF_SHIFT - 3) >> quantizer_downshift,
  ...
  for (i = 0; i < blk_size * blk_size; i++) {
    dqcoeff_pvq[i] = AOM_SIGNED_SHL(out_int32[i] + (out_int32[i] < 0) + rounding_mask, quantizer_downshift) >>
