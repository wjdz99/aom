  ...
  if (!is_inter_block(mbmi)) {
    ...
    for (row_y = 0; row_y < tu_num_h_y; row_y++) {
      for (col_y = 0; col_y < tu_num_w_y; col_y++) {
        // luma
        predict_and_reconstruct_intra_block(cm, xd, r, mbmi, 0, row_y * tx_sz_y,
    ...
    if (!mbmi->skip) {
      ...
      for (row_y = 0; row_y < tu_num_h_y; row_y++) {
        for (col_y = 0; col_y < tu_num_w_y; col_y++) {
          ...
          if (tu_idx_c < tu_num_c) {
            row_c = (tu_idx_c / tu_num_w_c) * tx_sz_c;
            col_c = (tu_idx_c % tu_num_w_c) * tx_sz_c;
            eobtotal += reconstruct_inter_block(cm, xd, r, mbmi, 1, row_c,
                                                col_c, tx_log2_c);
            eobtotal += reconstruct_inter_block(cm, xd, r, mbmi, 2, row_c,
                                                col_c, tx_log2_c);
            tu_idx_c++;
          }
        }
      ...
      while (tu_idx_c < tu_num_c) {
        row_c = (tu_idx_c / tu_num_w_c) * tx_sz_c;
        col_c = (tu_idx_c % tu_num_w_c) * tx_sz_c;
        eobtotal += reconstruct_inter_block(cm, xd, r, mbmi, 1, row_c, col_c,
                                            tx_log2_c);
        eobtotal += reconstruct_inter_block(cm, xd, r, mbmi, 2, row_c, col_c,
                                            tx_log2_c);
        tu_idx_c++;
      }
