...
// If the cm->current_frame.refresh_frame_flags bitmask is set, update
// reference frame id values and mark frames as valid for reference.
...
static void show_existing_frame_reset(AV1Decoder *const pbi,
  ...
  // Note that the displayed frame be valid for referencing in order to have
  // been selected.
  if (cm->seq_params.frame_id_numbers_present_flag) {
    update_ref_frame_id(pbi, cm->ref_frame_id[existing_frame_idx]);
  }
...
static int read_uncompressed_header(AV1Decoder *pbi,
  ...
  if (seq_params->frame_id_numbers_present_flag) {
    update_ref_frame_id(pbi, cm->current_frame_id);
  }
