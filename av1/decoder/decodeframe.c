...
#if CONFIG_PVQ
static void daala_dec_init(AV1_COMMON *const cm, daala_dec_ctx *daala_dec, od_ec_dec *ec) {
  daala_dec->ec = ec;
  od_adapt_ctx_reset(&daala_dec->state.adapt, 0);

#if PVQ_ACTIVITY_MASKING
  // TODO(yushin) : activity masking info needs be signaled by a bitstream
  daala_dec->use_activity_masking = 1;
