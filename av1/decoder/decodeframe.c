...
static void setup_q_segmentation(AV1_COMMON *const cm,
  ...
  if (cm->frame_type == LAST_FRAME  || cm->frame_type == LAST2_FRAME ||
      cm->frame_type == LAST3_FRAME) {
    seg->q_lvls = 0;
    return;
  }
  ...
  for (i = 0; i < MAX_SEGMENTS; i++) {
    if (segfeature_active(seg, i, SEG_LVL_ALT_Q) ||
        segfeature_active(seg, i, SEG_LVL_SKIP)) {
      seg->q_lvls = 0;
      return;
    }
  }
  ...
  else if (!seg->enabled) seg->enabled = 1;
  ...
  for (i = 0; i < seg->q_lvls; i++) {
    ...
    av1_set_segdata(seg, i, SEG_LVL_ALT_Q, val);
  }
}
#endif
