...
static void decode_tile(AV1Decoder *pbi, ThreadData *const td, int tile_row,
  ...
  MACROBLOCKD *const xd = &td->xd;
  av1_zero_above_context(cm, xd, tile_info.mi_col_start,
                         tile_info.mi_col_end, tile_row);
  av1_reset_loop_filter_delta(xd, num_planes);
  av1_reset_loop_restoration(xd, num_planes);
#if CONFIG_NON_LOCAL_REFMV
  av1_zero(xd->nl_ref_mv_info_above);
  xd->nl_ref_mv_info_above_pt = NULL;
  av1_zero(xd->nl_ref_mv_info_left);
  xd->nl_ref_mv_info_left_pt = NULL;
...
#if CONFIG_NON_LOCAL_REFMV
    av1_copy(td->nl_ref_mv_info_above, xd->nl_ref_mv_info_above);
    xd->nl_ref_mv_info_above_pt = td->nl_ref_mv_info_above;
#endif  // CONFIG_NON_LOCAL_REFMV
...
#if CONFIG_NON_LOCAL_REFMV
      td->nl_ref_mv_info_left = xd->nl_ref_mv_info_left;
      xd->nl_ref_mv_info_left_pt = &td->nl_ref_mv_info_left;
#endif  // CONFIG_NON_LOCAL_REFMV

      // Bit-stream parsing and decoding of the superblock
      decode_partition(pbi, td, mi_row, mi_col, td->bit_reader,
                       cm->seq_params.sb_size, xd->sbi,
                       xd->sbi->ptree_root, 0x3);
