...
static INLINE void dec_build_inter_predictors(const AV1_COMMON *cm,
    ...
    for (int y = 0; y < b8_h; y += b4_h) {
      ...
      for (int x = 0; x < b8_w; x += b4_w) {
        ...
        ConvolveParams conv_params = get_conv_params_no_round(
            0, plane, cm->tmp_conv_dst, tmp_dst_stride, is_compound, xd->bd);
...

  {
    ...
    ConvolveParams conv_params = get_conv_params_no_round(
        0, plane, cm->tmp_conv_dst, MAX_SB_SIZE, is_compound, xd->bd);
...
static void dec_build_obmc_inter_predictors_sb(const AV1_COMMON *cm,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    dst_buf1[0] = CONVERT_TO_BYTEPTR(cm->tmp_obmc_bufs[0]);
    dst_buf1[1] =
        CONVERT_TO_BYTEPTR(cm->tmp_obmc_bufs[0] + MAX_SB_SQUARE * len);
    dst_buf1[2] =
        CONVERT_TO_BYTEPTR(cm->tmp_obmc_bufs[0] + MAX_SB_SQUARE * 2 * len);
    dst_buf2[0] = CONVERT_TO_BYTEPTR(cm->tmp_obmc_bufs[1]);
    dst_buf2[1] =
        CONVERT_TO_BYTEPTR(cm->tmp_obmc_bufs[1] + MAX_SB_SQUARE * len);
    dst_buf2[2] =
        CONVERT_TO_BYTEPTR(cm->tmp_obmc_bufs[1] + MAX_SB_SQUARE * 2 * len);
  } else {
    dst_buf1[0] = cm->tmp_obmc_bufs[0];
    dst_buf1[1] = cm->tmp_obmc_bufs[0] + MAX_SB_SQUARE;
    dst_buf1[2] = cm->tmp_obmc_bufs[0] + MAX_SB_SQUARE * 2;
    dst_buf2[0] = cm->tmp_obmc_bufs[1];
    dst_buf2[1] = cm->tmp_obmc_bufs[1] + MAX_SB_SQUARE;
    dst_buf2[2] = cm->tmp_obmc_bufs[1] + MAX_SB_SQUARE * 2;
  }
