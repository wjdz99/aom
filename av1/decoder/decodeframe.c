...
static void build_inter_predictors_for_plane(const AV1_COMMON *cm,
  ...
  if (!is_interintra_pred(xd->mi[0])) {
    dec_build_inter_predictors(cm, xd, plane, xd->mi[0], build_for_obmc, bw, bh,
                               mi_x, mi_y);
    return;
  }

  BUFFER_SET default_ctx = { { xd->plane[plane].dst.buf, NULL, NULL },
                             { xd->plane[plane].dst.stride, 0, 0 } };
  ...
  const DecCalcSubpelFuncArgs args = { xd->mi[0], mi_x, mi_y, build_for_obmc };
  av1_build_inter_predictors(cm, xd, plane, xd->mi[0], build_for_obmc, bw, bh,
                             mi_x, mi_y, dec_calc_subpel_params_and_extend,
                             &args, interpred, interpred_stride, border);
