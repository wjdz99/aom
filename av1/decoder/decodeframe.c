...
#if CONFIG_SDP
  if (!mbmi->skip_txfm[cm->tree_type != CHROMA_PART ? 0 : 1]) {
#else
  if (!mbmi->skip_txfm) {
#endif
#if TXCOEFF_TIMER
...
static AOM_INLINE void decode_reconstruct_tx(
  ...
  const TX_SIZE plane_tx_size =
      plane ? av1_get_max_uv_txsize(mbmi->sb_type[plane > 0 ? 1 : 0],
                                    pd->subsampling_x, pd->subsampling_y)
...
#if CONFIG_SDP
  av1_get_block_dimensions(mbmi->sb_type[plane > 0 ? 1 : 0], plane, xd,
