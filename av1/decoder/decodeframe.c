...
void superres_post_decode(AV1Decoder *pbi) {
  ...
  if (cm->current_video_frame == 1 && !cm->show_frame) {
    ...
    char fname[64];
    sprintf(fname, "Dec_presuperres%d_%d.y", cm->current_video_frame, cm->show_frame);
    FILE *fp = fopen(fname, "wb");
    for (i = 0; i < frame_to_show->y_crop_height; i++)
      fwrite(frame_to_show->y_buffer + frame_to_show->y_stride * i,
             frame_to_show->y_crop_width, 1, fp);
    fclose(fp);
  }
  */
