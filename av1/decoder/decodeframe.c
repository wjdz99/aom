...
static void decode_restoration_mode(AV1_COMMON *cm,
  ...
  for (int p = plane_start; p < num_planes; ++p) {
    ...
    cm->frame_contains_merge[p] = aom_rb_read_bit(rb);
...
static void loop_restoration_read_sb_coeffs(const AV1_COMMON *const cm,
  ...
  if (rsi->frame_restoration_type == RESTORE_SWITCHABLE) {
    ...
    switch (rui->restoration_type) {
      case RESTORE_WIENER:
        read_wiener_filter(xd, wiener_win, &rui->wiener_info, wiener_info,
#if CONFIG_RST_MERGECOEFFS
                           cm->frame_contains_merge[plane],
#endif  // CONFIG_RST_MERGECOEFFS
                           r);
        break;
      case RESTORE_SGRPROJ:
        read_sgrproj_filter(xd, &rui->sgrproj_info, sgrproj_info,
#if CONFIG_RST_MERGECOEFFS
                            cm->frame_contains_merge[plane],
#endif  // CONFIG_RST_MERGECOEFFS
                            r);
