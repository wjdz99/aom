...
static TX_SIZE read_tx_partition(MACROBLOCKD *xd, MB_MODE_INFO *mbmi,
  ...
  */
  if (allow_horz && allow_vert) {
    // Read 4way tree type
    const int split4_ctx = is_inter ? txfm_partition_split4_inter_context(
        xd->above_txfm_context + blk_col, xd->left_txfm_context + blk_row,
        bsize, max_tx_size) : get_tx_size_context(xd);
    aom_cdf_prob *split4_cdf = is_inter ?  
    ...
    if (((split4_partition == TX_PARTITION_VERT) && allow_vert4) ||
      ...
      aom_cdf_prob *split2_rect_cdf = is_inter ?  
          ec_ctx->inter_2way_rect_txfm_partition_cdf :
          ec_ctx->intra_2way_rect_txfm_partition_cdf;
      const int further_split = aom_read_symbol(
          r, split2_rect_cdf, 2, ACCT_STR);
      if (further_split)
    ...
    */
    ...
    const int has_first_split =
        aom_read_symbol(r, split2_cdf, 2, ACCT_STR);
    if (has_first_split) {
  ...
  const int index = is_inter ? av1_get_txb_size_index(bsize, blk_row, blk_col) : 0;
...
static TX_SIZE read_tx_size(MACROBLOCKD *xd, TX_MODE tx_mode,
