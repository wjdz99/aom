...
void av1_read_qStep_config(AV1_COMMON *cm, struct aom_read_bit_buffer *rb,
    ...
    if(seq_params->qStep_mode == 0 || seq_params->qStep_mode == 1) {
        ...
        for (int idx = 1; idx <= seq_params->num_qStep_intervals; idx++) {
            ...
            assert(delta_val >=0);
            seq_params->num_qsteps_in_interval[idx] = delta_val + seq_params->num_qsteps_in_interval[idx -1];
        }
        ...
        for (int idx = 1; idx <= seq_params->num_qStep_levels; idx++) {
            int delta_qsteps_sign = aom_rb_read_bit(rb);
            int delta_val = aom_rb_read_uvlc(rb);
            seq_params->qSteps_level[idx] = (delta_val * (delta_qsteps_sign ? -1 : 1)) + seq_params->qSteps_level[idx -1];
        }
        ...
        for (int i = 0; i <= seq_params->num_table_templates_minus1; i++) {
            ...
            for (int idx = 1; idx <= seq_params->num_entries_in_table_minus1[i]; idx++) {
                int delta_qsteps_sign = aom_rb_read_bit(rb);
                int delta_val = aom_rb_read_uvlc(rb);
                seq_params->qSteps_level_in_table[i][idx] = (delta_val * (delta_qsteps_sign ? -1 : 1)) + seq_params->qSteps_level_in_table[i][idx-1];
            }
        }
        seq_params->num_qStep_intervals = aom_rb_read_literal(rb, 4);
...
uint32_t av1_decode_frame_headers_and_setup(AV1Decoder *pbi,
  ...
  xd->global_motion = cm->global_motion;
    ...
    if((seq_params->qStep_mode == 0) || (seq_params->qStep_mode == 1)) {
        ...
        set_qStep_table_mode_2(seq_params->qStep_mode,
