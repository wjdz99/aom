...
static void read_tx_partition(MACROBLOCKD *xd, MB_MODE_INFO *mbmi,
  ...
  const TX_PARTITION_TYPE partition = 0;
  if (allow_horz && allow_vert) {
    const int split4_ctx = 
      txfm_partition_split4_inter_context(xd->above_txfm_context + blk_col,
                                         xd->left_txfm_context + blk_row,
                                         mbmi->sb_type, max_tx_size);
    const TX_PARTITION_TYPE split4_partition = get_split4_partition(partition);
      aom_read_symbol(r, ec_ctx->inter_4way_txfm_partition_cdf[is_rect][split4_ctx],
                      4, ACCT_STR);
    if ((split4_partition == TX_PARTITION_VERT) || 
        (split4_partition  == TX_PARTITION_HORZ)) {
      const int further_split = aom_rb_read_bit(rb);
      if (further_split) {
        partiton = (split4_partition == TX_PARTITION_VERT) ? 
                    TX_PARTITION_VERT4 : TX_PARTITION_VERT_HORZ;
      } else {
        partition = split4_partition; 
      }
    }
  } else if (allow_horz || allow_vert) {
    const int has_first_split = aom_rb_read_bit(rb);
    if (has_first_split) {
      const int has_second_split = aom_rb_read_bit(rb);
      if (has_second_split) {
