...
static INLINE int update_extend_mc_border_params(
  ...
  // Extension is still needed in optical flow refinement so MV offset can
  // be obtained
  (void)scaled_mv;
...
static void dec_calc_subpel_params(
  ...
  if (is_scaled) {
    ...
    if (use_optflow_refinement) {
      orig_pos_y +=
          src_mv->row / (1 << (MV_REFINE_PREC_BITS - SUBPEL_BITS + ssy));
      orig_pos_x +=
          src_mv->col / (1 << (MV_REFINE_PREC_BITS - SUBPEL_BITS + ssx));
    } else {
