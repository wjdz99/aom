...
static INLINE int update_extend_mc_border_params(
  ...
  if ((!is_intrabc) && (!do_warp) &&
      (is_scaled || scaled_mv.col || scaled_mv.row ||
       (CONFIG_NEW_TX64X64 && USE_SUPERRES_FILTER_TX64) ||
...
#if CONFIG_NEW_TX64X64 && USE_SUPERRES_FILTER_TX64
    if (!*x_pad) {
...
#endif  // CONFIG_NEW_TX64X64 && USE_SUPERRES_FILTER_TX64
...
static INLINE void dec_calc_subpel_params(
    ...
    int mi_y, MV32 *scaled_mv, int *subpel_x_mv, int *subpel_y_mv) {
  struct macroblockd_plane *const pd = &xd->plane[plane];
  const int is_scaled = av1_is_scaled(sf);
  if (is_scaled) {
  ...
  if (!is_inter_block(mbmi)) {
    ...
    for (row = 0; row < max_blocks_high; row += mu_blocks_high) {
      for (col = 0; col < max_blocks_wide; col += mu_blocks_wide) {
        for (int plane = 0; plane < num_planes; ++plane) {
          ...
          av1_get_unit_width_height_coeff(xd, plane, mi_row, mi_col,
                                          plane_bsize, row_plane, col_plane,
    ...
    if (!mbmi->skip) {
      ...
      assert(max_unit_bsize ==
             get_plane_block_size(mi_row, mi_col, BLOCK_64X64,
                                  xd->plane[0].subsampling_x,
                                  xd->plane[0].subsampling_y));
      int mu_blocks_wide =
          block_size_wide[max_unit_bsize] >> tx_size_wide_log2[0];
...
static void decode_partition(AV1Decoder *const pbi, ThreadData *const td,
  ...
  if (get_plane_block_size(mi_row, mi_col, subsize, pd_u->subsampling_x,
