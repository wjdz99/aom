...
static int get_next_job_info(AV1Decoder *const pbi,
  ...
  int num_mis_available, num_threads_working;
  ...
  if (pbi->num_tiles_completed == num_tiles || pbi->row_mt_exit == 1) {
    return 1;
  }
  ...
  if (job_available) {
    for (tile_row_idx = tile_rows_start; tile_row_idx < tile_rows_end;
      ...
      for (tile_col_idx = tile_cols_start; tile_col_idx < tile_cols_end;
        ...
        if (num_threads_working == min_threads_working &&
            num_mis_available > max_mis_available) {
          max_mis_available = num_mis_available;
    ...
    if (dec_row_mt_sync->mi_rows == dec_row_mt_sync->mi_rows_decode_started)
      pbi->num_tiles_completed++;
...
static int row_mt_worker_hook(void *arg1, void *arg2) {
  ...
  if (call_setjmp(thread_data, pbi)) {
    ...
    while (end_of_frame == 0) {
      ...
      end_of_frame = (pbi->num_tiles_completed == num_tiles);
