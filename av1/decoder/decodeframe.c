...
static void setup_segmentation_dequant(AV1_COMMON *const cm) {
  ...
  for (int i = 0; i < max_segments; ++i) {
    ...
    cm->u_dequant[i][0] = av1_dc_quant(qindex, cm->u_dc_delta_q, cm->bit_depth);
    cm->v_dequant[i][0] = av1_dc_quant(qindex, cm->v_dc_delta_q, cm->bit_depth);
    cm->u_dequant[i][1] = av1_ac_quant(qindex, cm->u_ac_delta_q, cm->bit_depth);
    cm->v_dequant[i][1] = av1_ac_quant(qindex, cm->v_ac_delta_q, cm->bit_depth);
#if CONFIG_AOM_QM
