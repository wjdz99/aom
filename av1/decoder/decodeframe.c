...
static int read_global_motion_params(WarpedMotionParams *params,
                                     ...
                                     struct aom_read_bit_buffer *rb,
                                     int allow_hp, int frame) {
  uint16_t k, n;
#if CONFIG_GM_MODEL_CODING
  if (frame != LAST_FRAME) {
    k = GM_DIFF_SUBEXPFIN_K;
    n = GM_DIFF_ALPHA_MAX + 1;
  } else {
    k = SUBEXPFIN_K;
    n = GM_ALPHA_MAX + 1;
  }
  ...
  *params = default_warp_params;
  ...
  if (type >= ROTZOOM) {
    ...
    params->wmmat[3] =
        aom_rb_read_signed_primitive_refsubexpfin(
            rb, n, k, (ref_params->wmmat[3] >> GM_ALPHA_PREC_DIFF)) *
        GM_ALPHA_DECODE_FACTOR;
  }
  ...
  if (type >= AFFINE) {
    params->wmmat[4] =
        aom_rb_read_signed_primitive_refsubexpfin(
            rb, n, k, (ref_params->wmmat[4] >> GM_ALPHA_PREC_DIFF)) *
        GM_ALPHA_DECODE_FACTOR;
  ...
  for (int frame = LAST_FRAME; frame <= ALTREF_FRAME; ++frame) {
    ...
    int good_params =
        read_global_motion_params(&cm->global_motion[frame], ref_params, rb,
                                  cm->features.allow_high_precision_mv, frame);
    if (!good_params) {
