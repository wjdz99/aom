...
static void read_global_motion(AV1_COMMON *cm, struct aom_read_bit_buffer *rb) {
  ...
  for (frame = LAST_FRAME; frame <= ALTREF_FRAME; ++frame) {
    if (cm->current_video_frame == 1 && cm->show_frame == 0 && frame == 7)
      printf("debug");
    ...
    */
    ...
    printf("Dec Ref %d [%d/%d]: %d %d %d %d\n\n\n",
           frame, cm->current_video_frame, cm->show_frame,
           cm->prev_frame->global_motion[frame].wmmat[0],
           cm->prev_frame->global_motion[frame].wmmat[1],
           cm->prev_frame->global_motion[frame].wmmat[2],
           cm->prev_frame->global_motion[frame].wmmat[3]);
  }
