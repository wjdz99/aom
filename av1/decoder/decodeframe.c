...
static void dec_predict_b_extend(
  ...
  if (!b_sub8x8)
    av1_build_inter_predictors_cb_extend(&pbi->common, xd,
#if CONFIG_EXT_INTER
                                         mi_row_ori, mi_col_ori,
#endif  // CONFIG_EXT_INTER
                                         mi_row_pred, mi_col_pred, bsize_pred);
  else
    av1_build_inter_predictors_cb_sub8x8_extend(&pbi->common, xd,
#if CONFIG_EXT_INTER
                                                mi_row_ori, mi_col_ori,
#endif  // CONFIG_EXT_INTER
...
static void decode_token_and_recon_block(AV1Decoder *const pbi,
...
#if CONFIG_COEF_INTERLEAVE
  {
    ...
    if (!is_inter_block(mbmi)) {
      ...
      // Prediction
      av1_build_inter_predictors_cb(cm, xd, mi_row, mi_col, NULL,
...
#if CONFIG_CB4X4
    av1_build_inter_predictors_cb(cm, xd, mi_row, mi_col, NULL, bsize);
#else
    av1_build_inter_predictors_cb(cm, xd, mi_row, mi_col, NULL,
                                  AOMMAX(bsize, BLOCK_8X8));
#endif
