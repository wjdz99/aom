...
static const uint8_t *decode_tiles(AV1Decoder *pbi, const uint8_t *data,
  ...
  if (cm->clpf_strength && !cm->skip_loop_filter) {
    YV12_BUFFER_CONFIG dst;  // Temporary buffer for the result

    dst = pbi->cur_buf->buf;
    CHECK_MEM_ERROR(cm, dst.y_buffer, aom_malloc(dst.y_height * dst.y_stride));
    CHECK_MEM_ERROR(cm, dst.u_buffer,
                    aom_malloc(dst.uv_height * dst.uv_stride));
    ...
    // Free the old buffers and the update pointers
    aom_free(pbi->cur_buf->buf.y_buffer);
    aom_free(pbi->cur_buf->buf.u_buffer);
    aom_free(pbi->cur_buf->buf.v_buffer);
    pbi->cur_buf->buf.y_buffer = dst.y_buffer;
    pbi->cur_buf->buf.u_buffer = dst.u_buffer;
    pbi->cur_buf->buf.v_buffer = dst.v_buffer;
  }
