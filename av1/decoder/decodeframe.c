...
static void get_ls_tile_buffer(
    ...
    int tile_size_bytes, const int col, const int row, int tile_copy_mode) {
  size_t size;
...
// Returns the end of the raw tile buffer in the bit stream.
static const uint8_t *get_ls_tile_buffers(
  ...
  if (!have_tiles) {
    ...
    assert(last_row == tile_rows - 1);
    (void)last_row;
    assert(last_col == tile_cols - 1);
    (void)last_col;
    raw_data_end = data;
  }
  return raw_data_end;
...
static const uint8_t *decode_tiles(AV1Decoder *pbi, const uint8_t *data,
  ...
  if (cm->large_scale_tile) {
    ...
    assert(tile_buffers[tile_rows - 1][tile_cols - 1].raw_data_enx ==
           raw_data_end);
    return raw_data_end;
  }
