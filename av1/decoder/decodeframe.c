...
static void set_inter_tx_size(MB_MODE_INFO *mbmi, int stride_log2,
                              int tx_w_log2, int tx_h_log2, int txs,
                              int tx_size, int blk_row, int blk_col) {
  for (int idy = 0; idy < tx_size_high_unit[tx_size];
       idy += tx_size_high_unit[txs]) {
    for (int idx = 0; idx < tx_size_wide_unit[tx_size];
         idx += tx_size_wide_unit[txs]) {
...
static void read_tx_size_vartx(MACROBLOCKD *xd, MB_MODE_INFO *mbmi,
  ...
  if (is_split) {
    ...
    if (sub_txs == TX_4X4) {
      set_inter_tx_size(mbmi, stride_log2, tx_w_log2, tx_h_log2, txs, sub_txs,
                        blk_row, blk_col);
