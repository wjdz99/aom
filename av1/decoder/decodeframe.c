...
static void decode_tile_sb_row(AV1Decoder *pbi, ThreadData *const td,
  ...
  for (int mi_col = tile_info.mi_col_start; mi_col < tile_info.mi_col_end;
    ...
    for (int frame = LAST_FRAME; frame < REF_FRAMES; frame++) {
      backup_global_motion[frame] = td->xd.global_motion[frame];
    }
#endif  // CONFIG_SB_WARP
...
#if CONFIG_SB_WARP
    for (int frame = LAST_FRAME; frame < REF_FRAMES; frame++) {
      td->xd.global_motion[frame] = backup_global_motion[frame];
    }
#endif  // CONFIG_SB_WARP
...
static int read_global_motion_params(WarpedMotionParams *params,
  ...
  if (type != IDENTITY) {
    if (aom_rb_read_bit(rb)) {
      type = ROTZOOM;
#if CONFIG_SB_WARP
      if (type == ROTZOOM && valid_frame_for_delta_gm == 1) {
...
static void read_global_motion(AV1_COMMON *cm, struct aom_read_bit_buffer *rb) {
#endif  // CONFIG_SB_WARP
#if CONFIG_SB_WARP
  int valid_frame_for_delta_gm = aom_rb_read_bit(rb);
#endif  // CONFIG_SB_WARP
  for (int frame = LAST_FRAME; frame <= ALTREF_FRAME; ++frame) {
    ...
    int good_params = read_global_motion_params(
        &cm->global_motion[frame], ref_params, rb, cm->fr_mv_precision,
        valid_frame_for_delta_gm);
    *is_delta_present_for_at_least_one_frame |=
...
#else
  int is_delta_present_for_at_least_one_frame = 0;
  pbi->is_delta_present_for_at_least_one_frame = 0;
  if (!frame_is_intra_only(cm))
    read_global_motion(cm, rb, &is_delta_present_for_at_least_one_frame);
