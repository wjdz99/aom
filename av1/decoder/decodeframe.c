...
// showing a frame directly
  ...
  *cm->fc = cm->frame_contexts[cm->frame_context_idx];
  ...
  if (cm->clpf_strength && !cm->skip_loop_filter) {
    ...
    dst = pbi->cur_buf->buf;
    dst.buffer_alloc = 0;
    if (aom_yv12_realloc_frame_buffer(&dst, dst.y_width, dst.y_height,
                                      dst.border))
      aom_internal_error(&cm->error, AOM_CODEC_MEM_ERROR,
                         "Failed to allocate dst at %s:%d", __FILE__, __LINE__);
...
#if 0  // TODO(stemidts): Why does this give a memleak?
    // Free the old buffer and use the result buffer instead.
    aom_yv12_de_alloc_frame_buffer(&pbi->cur_buf->buf);
    pbi->cur_buf->buf = dst;
#else
    // Copy result
    ...
    memcpy(pbi->cur_buf->buf.v_buffer, dst.v_buffer,
           dst.uv_height * dst.uv_stride);
    aom_yv12_de_alloc_frame_buffer(&dst);
#endif
  }
