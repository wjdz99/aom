...
static void decode_token_and_recon_block(AV1Decoder *const pbi,
                                         ...
                                         BLOCK_SIZE bsize, int do_decode) {
  AV1_COMMON *const cm = &pbi->common;
  const int num_planes = av1_num_planes(cm);
  const int bw = mi_size_wide[bsize];
  const int bh = mi_size_high[bsize];
  ...
  if (!is_inter_block(mbmi)) {
    intra_block_visitor_fn_t intra_block_visit =
        do_decode ? predict_and_reconstruct_intra_block
                  : predict_and_reconstruct_intra_block_void;
    int row, col;
    ...
    if (do_decode) predict_inter_block(cm, xd, mi_row, mi_col, bsize);
    // Reconstruction
    if (!mbmi->skip) {
      inverse_tx_inter_block_visitor_fn_t inverse_tx_inter_block_visit =
          do_decode ? inverse_transform_inter_block
                    : inverse_transform_inter_block_void;
      int eobtotal = 0;
    ...
    if (do_decode) cfl_store_inter_block(cm, xd);
  }
...
static void parse_decode_block(AV1Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  decode_token_and_recon_block(pbi, xd, mi_row, mi_col, r, bsize, do_decode);
...
static void decode_partition(AV1Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  int do_decode = (parse_decode_flag & 2) >> 1;
...
#define DEC_BLOCK(db_r, db_c, db_subsize)                                     \
  block_visit[parse_decode_flag](pbi, xd, DEC_BLOCK_STX_ARG(db_r), (db_c), r, \
                                 DEC_BLOCK_EPT_ARG(db_subsize), do_decode)
...
static void decode_tile_sb_row(AV1Decoder *pbi, ThreadData *const td,
  ...
  for (int mi_col = tile_info.mi_col_start; mi_col < tile_info.mi_col_end;
    ...
    decode_partition(pbi, &td->xd, mi_row, mi_col, td->bit_reader,
                     cm->seq_params.sb_size, parse_decode_flag);
  }
}
#endif
