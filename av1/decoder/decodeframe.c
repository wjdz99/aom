...
static REFERENCE_MODE read_frame_reference_mode(
    const AV1_COMMON *cm, struct vpx_read_bit_buffer *rb) {
...
static void read_frame_reference_mode_probs(AV1_COMMON *cm, vpx_reader *r) {
...
static PARTITION_TYPE read_partition(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  if (has_rows && has_cols)
    p = (PARTITION_TYPE)vpx_read_tree(r, av1_partition_tree, probs);
...
#if CONFIG_CLPF
static void setup_clpf(AV1_COMMON *cm, struct vpx_read_bit_buffer *rb) {
  cm->clpf = vpx_rb_read_literal(rb, 1);
}
#endif

#if CONFIG_DERING
static void setup_dering(AV1_COMMON *cm, struct vpx_read_bit_buffer *rb) {
...
static void setup_render_size(AV1_COMMON *cm, struct vpx_read_bit_buffer *rb) {
...
static void setup_frame_size(AV1_COMMON *cm, struct vpx_read_bit_buffer *rb) {
  int width, height;
...
static void setup_tile_info(AV1_COMMON *cm, struct vpx_read_bit_buffer *rb) {
...
static struct vpx_read_bit_buffer *init_read_bit_buffer(
    AV1Decoder *pbi, struct vpx_read_bit_buffer *rb, const uint8_t *data,
...
int av1_read_sync_code(struct vpx_read_bit_buffer *const rb) {
  return vpx_rb_read_literal(rb, 8) == AV1_SYNC_CODE_0 &&
         vpx_rb_read_literal(rb, 8) == AV1_SYNC_CODE_1 &&
         vpx_rb_read_literal(rb, 8) == AV1_SYNC_CODE_2;
}

void av1_read_frame_size(struct vpx_read_bit_buffer *rb, int *width,
                          int *height) {
  *width = vpx_rb_read_literal(rb, 16) + 1;
  *height = vpx_rb_read_literal(rb, 16) + 1;
}

BITSTREAM_PROFILE av1_read_profile(struct vpx_read_bit_buffer *rb) {
