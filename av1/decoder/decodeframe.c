...
static OBU_TYPE read_obu_header(struct aom_read_bit_buffer *rb,
  ...
  *header_size = 1;
  ...
  forbidden_bit = aom_rb_read_bit(rb);
  assert(!forbidden_bit);
...
static uint32_t read_metadata(AV1Decoder *pbi, const uint8_t *data,
                              uint32_t sz) {
...
// The obu size is only needed for tile group OBUs
    ...
    switch (obu_type) {
      ...
      case OBU_TILE_GROUP:
        ...
        break;
      case OBU_METADATA:
        obu_payload_size = read_metadata(pbi, data, obu_size);
        break;
      default: break;
    }
