...
static AOM_INLINE void read_tile_info_max_tile(
  ...
  cm->tiles.uniform_spacing = aom_rb_read_bit(rb);

  // Read tile columns
  if (cm->tiles.uniform_spacing) {
    cm->tiles.log2_cols = cm->tiles.min_log2_cols;
    while (cm->tiles.log2_cols < cm->tiles.max_log2_cols) {
      if (!aom_rb_read_bit(rb)) {
        break;
      }
      cm->tiles.log2_cols++;
    ...
    for (i = 0, start_sb = 0; width_sb > 0 && i < MAX_TILE_COLS; i++) {
      const int size_sb =
          1 + rb_read_uniform(rb, AOMMIN(width_sb, cm->tiles.max_width_sb));
      cm->tiles.col_start_sb[i] = start_sb;
      start_sb += size_sb;
      width_sb -= size_sb;
    }
    cm->tiles.cols = i;
    cm->tiles.col_start_sb[i] = start_sb + width_sb;
  }
  av1_calculate_tile_cols(cm);

  // Read tile rows
  if (cm->tiles.uniform_spacing) {
    cm->tiles.log2_rows = cm->tiles.min_log2_rows;
    while (cm->tiles.log2_rows < cm->tiles.max_log2_rows) {
      if (!aom_rb_read_bit(rb)) {
        break;
      }
      cm->tiles.log2_rows++;
    ...
    for (i = 0, start_sb = 0; height_sb > 0 && i < MAX_TILE_ROWS; i++) {
      const int size_sb =
          1 + rb_read_uniform(rb, AOMMIN(height_sb, cm->tiles.max_height_sb));
      cm->tiles.row_start_sb[i] = start_sb;
      start_sb += size_sb;
      height_sb -= size_sb;
    }
    cm->tiles.rows = i;
    cm->tiles.row_start_sb[i] = start_sb + height_sb;
  }
...
static const uint8_t *decode_tiles(AV1Decoder *pbi, const uint8_t *data,
  ...
  if (tile_rows_end <= tile_rows_start || tile_cols_end <= tile_cols_start ||
      ...
      tile_rows_start * cm->tiles.cols + tile_cols_start > end_tile ||
      // Last tile is smaller than start_tile.
      (tile_rows_end - 1) * cm->tiles.cols + tile_cols_end - 1 < start_tile)
    return data;
...
#if EXT_TILE_DEBUG
  if (cm->tiles.large_scale && !pbi->ext_tile_debug)
    raw_data_end = get_ls_single_tile_buffer(pbi, data, tile_buffers);
  else if (cm->tiles.large_scale && pbi->ext_tile_debug)
  ...
  for (tile_row = tile_rows_start; tile_row < tile_rows_end; ++tile_row) {
    ...
    for (tile_col = tile_cols_start; tile_col < tile_cols_end; ++tile_col) {
      ...
      TileDataDec *const tile_data =
          pbi->tile_data + row * cm->tiles.cols + col;
...
static const uint8_t *decode_tiles_mt(AV1Decoder *pbi, const uint8_t *data,
...
#if EXT_TILE_DEBUG
  if (cm->tiles.large_scale) assert(pbi->ext_tile_debug == 1);
  if (cm->tiles.large_scale)
    raw_data_end = get_ls_tile_buffers(pbi, data, data_end, tile_buffers);
  else
#endif  // EXT_TILE_DEBUG
  ...
  for (int row = 0; row < tile_rows; row++) {
    for (int col = 0; col < tile_cols; col++) {
      TileDataDec *tile_data = pbi->tile_data + row * cm->tiles.cols + col;
...
void av1_decode_tg_tiles_and_wrapup(AV1Decoder *pbi, const uint8_t *data,
  ...
  if (end_tile != cm->tiles.rows * cm->tiles.cols - 1) {
    return;
  }

  if (!cm->features.allow_intrabc && !cm->tiles.single_tile_decoding) {
