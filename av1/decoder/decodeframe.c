...
#if CONFIG_CLPF
  if (bsize == BLOCK_64X64 && cm->clpf_strength_y && cm->clpf_size) {
    const int tl = mi_row * MI_SIZE / MIN_FB_SIZE * cm->clpf_stride +
                   mi_col * MI_SIZE / MIN_FB_SIZE;

    if (!((mi_row * MI_SIZE) & 127) && !((mi_col * MI_SIZE) & 127) &&
        cm->clpf_size == 3) {  // 128x128
      cm->clpf_blocks[tl] = aom_read_literal(r, 1, ACCT_STR);
    } else if (cm->clpf_size == 2 &&
               !clpf_all_skip(cm, mi_col, mi_row, 64 / MI_SIZE)) {  // 64x64
      cm->clpf_blocks[tl] = aom_read_literal(r, 1, ACCT_STR);
    } else if (cm->clpf_size == 1) {  // 32x32
      const int tr = tl + 1;
