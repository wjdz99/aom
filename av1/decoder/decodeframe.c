...
#if CONFIG_CCSO
static AOM_INLINE void setup_ccso(AV1_COMMON *cm, struct aom_read_bit_buffer *rb) {
  if (cm->features.allow_intrabc) return;
    ...
    if (cm->ccso_info.ccso_enable[plane]) {
      ...
      for (int d0 = 0; d0 < 3; d0++) {
        for (int d1 = 0; d1 < 3; d1++) {
          ...
          cm->ccso_info.filter_offset[plane][lut_idx_ext] = ccso_offset[offset_idx];
        }
      }
    }
  }
...
#if CONFIG_CCSO
    const int use_ccso = !pbi->skip_loop_filter && !cm->features.coded_lossless && (cm->ccso_info.ccso_enable[0] || cm->ccso_info.ccso_enable[1]);
    uint16_t *ext_rec_y;
    if (use_ccso) {
      av1_setup_dst_planes(xd->plane, cm->seq_params.sb_size, &cm->cur_frame->buf, 0, 0, 0, num_planes);
      const int ccso_stride_ext = xd->plane[0].dst.width + (CCSO_PADDING_SIZE << 1);
      ext_rec_y = aom_malloc(sizeof(*ext_rec_y) * (xd->plane[0].dst.height + (CCSO_PADDING_SIZE << 1)) * (xd->plane[0].dst.width + (CCSO_PADDING_SIZE << 1)));
        ...
        for (int r = 0; r < pic_height; ++r) {
          for (int c = 0; c < pic_width; ++c) {
            if (cm->seq_params.use_highbitdepth) {
              ext_rec_y[(r + CCSO_PADDING_SIZE) * ccso_stride_ext + c + CCSO_PADDING_SIZE] =
...
#if CONFIG_CCSO
    const int optimized_loop_restoration = !do_cdef && !do_superres && !use_ccso;
#else
