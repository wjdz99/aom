  ...
  */
  if (allow_horz && allow_vert) {
    ...
    const TX_PARTITION_TYPE split4_partition = aom_read_symbol(
        r, ec_ctx->inter_4way_txfm_partition_cdf[is_rect][split4_ctx], 4,
        ACCT_STR);
    ...
    if (((split4_partition == TX_PARTITION_VERT) && allow_vert2) ||
      ...
      const int further_split = aom_read_symbol(
          r, ec_ctx->inter_2way_rect_txfm_partition_cdf, 2, ACCT_STR);
      if (further_split)
    ...
    */
    ...
    const int has_first_split =
        aom_read_symbol(r, ec_ctx->inter_2way_txfm_partition_cdf, 2, ACCT_STR);
    if (has_first_split) {
      // If further splitting is allowed, read a bit determine the fineal
      // partition type
      const int has_second_split =
          (allow_horz2 || allow_vert2)
              ? aom_read_symbol(r, ec_ctx->inter_2way_rect_txfm_partition_cdf,
                                2, ACCT_STR)
              : 0;
      if (has_second_split)
...
static AOM_INLINE void parse_decode_block(AV1Decoder *const pbi,
  ...
  if (cm->features.tx_mode == TX_MODE_SELECT && block_signals_txsize(bsize) &&
    ...
    for (int idy = 0; idy < height; idy += bh)
      for (int idx = 0; idx < width; idx += bw)
        ...
        read_tx_partition(xd, mbmi, max_tx_size, idy, idx, r,
                          cm->seq_params.use_inter_4way_tx_split);
#else
    ...
    mbmi->tx_size =
        read_tx_size(xd, cm->features.tx_mode, inter_block_tx, !mbmi->skip_txfm,
#if CONFIG_NEW_TX_PARTITION
                     cm->seq_params.use_intra_4way_tx_split,
#endif  // CONFIG_NEW_TX_PARTITION
                     r);
    if (inter_block_tx)
...
static AOM_INLINE void decode_tile_sb_row(AV1Decoder *pbi, ThreadData *const td,
  ...
  for (int mi_col = tile_info.mi_col_start; mi_col < tile_info.mi_col_end;
    ...
    decode_partition(pbi, td, mi_row, mi_col, td->bit_reader,
                     cm->seq_params.sb_size, td->dcb.xd.sbi,
                     td->dcb.xd.sbi->ptree_root, 0x2);
...
#if CONFIG_NEW_TX_PARTITION
  seq_params->use_inter_4way_tx_split = aom_rb_read_bit(rb);
  seq_params->use_intra_4way_tx_split = aom_rb_read_bit(rb);
#endif  // CONFIG_NEW_TX_PARTITION
