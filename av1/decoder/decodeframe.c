...
#if CONFIG_BUFFER_MODEL
    if (cm->timing_info_present && cm->timing_info.equal_picture_interval == 0) {
...
#if CONFIG_BUFFER_MODEL
  if (cm->decoder_model_info_present){
    cm->buffer_removal_delay_present = aom_rb_read_bit(rb);
    if (cm->buffer_removal_delay_present) {
      for (int op_num = 0; op_num < cm->operating_points_decoder_model_cnt; op_num++)
      {
        if ( ((cm->op_params[op_num].decoder_model_operating_point_idc >> cm->temporal_layer_id) & 0x1 &&
            (cm->op_params[op_num].decoder_model_operating_point_idc >> (cm->enhancement_layer_id + 8)) &
            0x1) || cm->op_params[op_num].decoder_model_operating_point_idc == 0 )  {
          cm->op_frame_timing[op_num].buffer_removal_delay_flag = 1;
          cm->op_frame_timing[op_num].buffer_removal_delay = aom_rb_read_literal(rb, cm->buffer_model.buffer_removal_delay_length);
          printf("\ndts: %d\n",
                   (int)cm->op_frame_timing[op_num].buffer_removal_delay);
        }
      }
    }
  }
