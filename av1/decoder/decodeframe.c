...
#if CONFIG_OBU
      pbi->refresh_frame_flags = (cm->frame_type == S_FRAME) ? ~(1 << REF_FRAMES) : aom_rb_read_literal(rb, REF_FRAMES);
#else
      pbi->refresh_frame_flags = aom_rb_read_literal(rb, REF_FRAMES);
#endif
...
#if CONFIG_OBU
		cm->ref_frame_sign_bias[LAST_FRAME + i] = (cm->frame_type == S_FRAME) ? 0 : aom_rb_read_bit(rb);
#else
...
void av1_decode_frame_tiles_and_wrapup(AV1Decoder *pbi, const uint8_t *data,
                                       const uint8_t *data_end,
                                       const uint8_t **p_data_end) {
  AV1_COMMON *const cm = &pbi->common;
  MACROBLOCKD *const xd = &pbi->mb;
  ...
  first_partition_size = pbi->first_partition_size;
  new_fb = xd->cur_buf;
...
#if CONFIG_OBU
    *p_data_end =
        decode_tiles(pbi, data, data_end, 0, cm->tile_rows * cm->tile_cols - 1);
#else
    *p_data_end =
        decode_tiles(pbi, data + pbi->uncomp_hdr_size + first_partition_size,
                     data_end, 0, cm->tile_rows * cm->tile_cols - 1);
#endif
  }
...
static OBU_TYPE read_obu_header(struct aom_read_bit_buffer *rb,
  ...
  *header_size = 1;
  ...
  if (obu_extension_flag) {
    *header_size++;
    int tempora_id = aom_rb_read_literal(rb, 3);
    int spatial_id = aom_rb_read_literal(rb, 2);
    int quality_id = aom_rb_read_literal(rb, 2);
...
static uint32_t read_temporal_delimiter_obu(struct aom_read_bit_buffer *rb) {
  return 0;
}
...
static uint32_t read_sequence_header_obu(AV1Decoder *pbi,
  ...
  cm->profile = av1_read_profile(rb);
  level = aom_rb_read_literal(rb, 4);
...
void av1_decode_frame_from_obus(struct AV1Decoder *pbi, const uint8_t *data,
  ...
  int temporal_delimiter_received = 0;
  int sequence_header_received = 0;
  ...
  while (!frame_decoding_finished && !cm->error.error_code) {
    ...
    uint32_t obu_size, obu_header_size, obu_payload_size;
    OBU_TYPE obu_type;
    ...
    switch (obu_type) {
      case OBU_TD:
        temporal_delimiter_received = 1;
        obu_payload_size = read_temporal_delimiter_obu(&rb);
        break;
      case OBU_SEQUENCE_HEADER:
        sequence_header_received = 1;
