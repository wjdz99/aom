...
static void decode_block(AV1Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  const TX_SIZE tx_log2_c = dec_get_uv_tx_size(mbmi, pd_c->n4_wl, pd_c->n4_hl);
  ...
  if (!is_inter_block(mbmi)) {
    ...
    for (row_y = 0; row_y < tu_num_h_y; row_y++) {
      for (col_y = 0; col_y < tu_num_w_y; col_y++) {
        // luma
        predict_and_reconstruct_intra_block(xd, r, mbmi, 0, row_y * tx_sz_y,
    ...
    if (!mbmi->skip) {
      ...
      for (row_y = 0; row_y < tu_num_h_y; row_y++) {
        for (col_y = 0; col_y < tu_num_w_y; col_y++) {
          // luma
          eobtotal += reconstruct_inter_block(xd, r, mbmi, 0, row_y * tx_sz_y,
                                              col_y * tx_sz_y, tx_log2_y);
          // chroma
          if (tu_idx_c < tu_num_c) {
            row_c = (tu_idx_c / tu_num_w_c) * tx_sz_c;
