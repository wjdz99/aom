...
static void setup_quantization(AV1_COMMON *const cm,
  ...
  if (num_planes > 1) {
    int diff_uv_delta = 0;
    if (cm->seq_params.separate_uv_delta_q) diff_uv_delta = aom_rb_read_bit(rb);
  ...
  cm->dequant_bit_depth = cm->seq_params.bit_depth;
...
static void read_bitdepth(struct aom_read_bit_buffer *rb,
                          ...
                          struct aom_internal_error_info *cm_error) {
  ...
  if (seq_params->profile == PROFILE_2 && high_bitdepth) {
    ...
    aom_internal_error(cm_error, AOM_CODEC_UNSUP_BITSTREAM,
...
void av1_read_color_config(struct aom_read_bit_buffer *rb,
                           ...
                           struct aom_internal_error_info *cm_error) {
  read_bitdepth(rb, seq_params, cm_error);
  ...
  if (seq_params->color_primaries == AOM_CICP_CP_BT_709 &&
    ...
    if (!(seq_params->profile == PROFILE_1 ||
      ...
      aom_internal_error(
          cm_error, AOM_CODEC_UNSUP_BITSTREAM,
