...
static INLINE void extend_mc_borders(MACROBLOCKD *xd,
                                     ...
                                     YV12_BUFFER_CONFIG *buf, MV32 scaled_mv,
                                     PadBlock block, int plane, int subpel_x_mv,
                                     int subpel_y_mv, int do_warp) {
  const int is_scaled = av1_is_scaled(sf);
  int frame_width, frame_height;
  uint8_t *ref_frame;
  // Get reference frame pointer, width and height.
  if (plane == 0) {
    frame_width = buf->y_crop_width;
    frame_height = buf->y_crop_height;
    ref_frame = buf->y_buffer;
  } else {
    frame_width = buf->uv_crop_width;
    frame_height = buf->uv_crop_height;
    ref_frame = plane == 1 ? buf->u_buffer : buf->v_buffer;
  }
  ...
  if ((!do_warp) && (is_scaled || scaled_mv.col || scaled_mv.row ||
    ...
    if (block.x0 < 0 || block.x1 > frame_width - 1 || block.y0 < 0 ||
      ...
      uint8_t *buf_ptr = ref_frame + block.y0 * pre_buf->stride + block.x0;
...
static INLINE void dec_build_inter_predictors(const AV1_COMMON *cm,
  ...
  if (sub8x8_inter) {
    ...
    for (int y = 0; y < b8_h; y += b4_h) {
      ...
      for (int x = 0; x < b8_w; x += b4_w) {
        ...
        extend_mc_borders(xd, sf, pre_buf, ref_buf->buf, scaled_mv, block,
                          plane, subpel_x_mv, subpel_y_mv, 0);
...

  {
    ...
    for (ref = 0; ref < 1 + is_compound; ++ref) {
      ...
      extend_mc_borders(xd, sf, pre_buf, xd->block_refs[ref]->buf, scaled_mv,
                        block, plane, subpel_x_mv, subpel_y_mv, do_warp);
    }
