...
// Called before each decode_tile() call to copy the delta-related fields of
// MACROBLOCKD from src (pbi->mb) to dest (td->xd).
static void copy_delta_before_decode_tile(MACROBLOCKD *dest,
                                          const MACROBLOCKD *src) {
  dest->prev_qindex = src->prev_qindex;
  memcpy(dest->prev_delta_lf, src->prev_delta_lf, sizeof(dest->prev_delta_lf));
  dest->prev_delta_lf_from_base = src->prev_delta_lf_from_base;
}
...
static const uint8_t *decode_tiles(AV1Decoder *pbi, const uint8_t *data,
  ...
  for (tile_row = tile_rows_start; tile_row < tile_rows_end; ++tile_row) {
    ...
    for (tile_col = tile_cols_start; tile_col < tile_cols_end; ++tile_col) {
      ...
      copy_delta_before_decode_tile(&td->xd, &pbi->mb);
