...
uint8_t av1_read_coeffs_txb(const AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  *eob = rec_eob_pos(eob_pt, eob_extra);
  ...
  while (i < *eob) {
    ...
    int golomb_flag = 0;
    int n_nz;
    // Parse up to 4 non-zero coeffs
    for (n_nz = 0; n_nz < 4 && i < *eob; i++) {
      const int c = *eob - 1 - i;
      ...
      if (level > NUM_BASE_LEVELS) {
        ...
        for (int idx = 0; idx < COEFF_BASE_RANGE; idx += BR_CDF_SIZE - 1) {
          const int k = aom_read_symbol(
              r,
              ec_ctx
                  ->coeff_br_cdf[AOMMIN(txs_ctx, TX_32X32)][plane_type][br_ctx],
    ...
    if (n_nz > 0) {
      ...
      int sign = 0;
      if (n_ac_sign > 0) {
        sign = aom_read_multibit(r, n_ac_sign, ACCT_STR);
      }
      if (nz_pos[n_nz - 1] == 0) {
      ...
      for (int j = 0; j < n_nz; j++) {
        const int pos = nz_pos[j];
        int level = nz_level[j];
        if (golomb_flag & (1 << j)) {
...
#if CONFIG_NEW_QUANT
#if CONFIG_AOM_QM
        dq_coeff = av1_dequant_abscoeff_nuq(level, dequant[!!pos], dq_profile,
                                            !!pos, nq_shift);
#else
        dqv_val = &dq_val[pos != 0][0];
        dq_coeff =
            av1_dequant_abscoeff_nuq(level, dequant[!!pos], dqv_val, nq_shift);
#endif  // CONFIG_AOM_QM
#else
