  ...
  for (c = 0; c < seg_eob; ++c) {
    ...
    if (c < seg_eob - 1) {
      ...
      is_nz = av1_read_record_bin(
          counts, r, fc->nz_map_cdf[txs_ctx][plane_type][coeff_ctx],
          &fc->bin_count, 2, ACCT_STR);
#else
    ...
    if (is_nz) {
      ...
      int is_eob = av1_read_record_bin(
          counts, r, fc->eob_flag_cdf[txs_ctx][plane_type][eob_ctx],
          &fc->bin_count, 2, ACCT_STR);
#else
...
static INLINE int read_nz_map_vert(aom_reader *r, tran_low_t *tcoeffs,
  ...
  for (int col = 0; col < width; ++col) {
    ...
    int empty_line = av1_read_record_bin(
        counts, r, fc->empty_line_cdf[txs_ctx][plane_type][tx_class][el_ctx],
        &fc->bin_count, 2, ACCT_STR);
#else
        ...
        if (row + 1 != height) {
          ...
          if (is_nz) {
            ...
            if (row + 1 != height) {
              ...
              int is_eob = av1_read_record_bin(
                  counts, r,
                  fc->hv_eob_cdf[txs_ctx][plane_type][tx_class][eob_ctx],
                  &fc->bin_count, 2, ACCT_STR);
#else
              int is_eob = aom_read(
...
uint8_t av1_read_coeffs_txb(const AV1_COMMON *const cm, MACROBLOCKD *xd,
  ...
  int all_zero = av1_read_record_bin(
      counts, r, ec_ctx->txb_skip_cdf[txs_ctx][txb_ctx->txb_skip_ctx],
      &ec_ctx->bin_count, 2, ACCT_STR);
#else
  int all_zero =
  ...
  if (tx_class == TX_CLASS_2D) {
    *eob =
    ...
    const int eob_mode = av1_read_record_bin(
        counts, r, ec_ctx->eob_mode_cdf[txs_ctx][plane_type][tx_class],
        &ec_ctx->bin_count, 2, ACCT_STR);
#else
    const int eob_mode =
  ...
  *max_scan_line = *eob;
  ...
  for (i = 0; i < NUM_BASE_LEVELS; ++i) {
    ...
    for (c = *eob - 1; c >= 0; --c) {
      ...
      if (av1_read_record_bin(
              counts, r, ec_ctx->coeff_base_cdf[txs_ctx][plane_type][i][ctx],
              &ec_ctx->bin_count, 2, ACCT_STR))
#else
        ...
        *v = i + 1;
        ...
        if (c == 0) {
          ...
          sign = av1_read_record_bin(
              counts, r, ec_ctx->dc_sign_cdf[plane_type][dc_sign_ctx],
              &ec_ctx->bin_count, 2, ACCT_STR);
#else
          sign =
  ...
  for (c = update_eob; c >= 0; --c) {
    ...
    if (c == 0) {
      int dc_sign_ctx = txb_ctx->dc_sign_ctx;
#if LV_MAP_PROB
      sign = av1_read_record_bin(counts, r,
                                 ec_ctx->dc_sign_cdf[plane_type][dc_sign_ctx],
                                 &ec_ctx->bin_count, 2, ACCT_STR);
#else
