...
static INLINE int read_nz_map(aom_reader *r, tran_low_t *tcoeffs, int plane,
  ...
  for (c = 0; c < seg_eob; ++c) {
    ...
    if (c < seg_eob - 1) {
#if LV_MAP_PROB
      is_nz = aom_read_bin(r, fc->nz_map_cdf[txs_ctx][plane_type][coeff_ctx], 2,
                           ACCT_STR);
#else
...
#if LV_MAP_PROB
    const int eob_mode = aom_read_bin(
  ...
  *max_scan_line = *eob;
...
#if LV_MAP_PROB
      if (aom_read_bin(r, ec_ctx->coeff_base_cdf[txs_ctx][plane_type][i][ctx],
                       2, ACCT_STR))
#else
...
#if LV_MAP_PROB
          sign = aom_read_bin(r, ec_ctx->dc_sign_cdf[plane_type][dc_sign_ctx],
                              2, ACCT_STR);
#else
          sign =
  ...
  for (c = update_eob; c >= 0; --c) {
    ...
    if (c == 0) {
      int dc_sign_ctx = txb_ctx->dc_sign_ctx;
#if LV_MAP_PROB
      sign = aom_read_bin(r, ec_ctx->dc_sign_cdf[plane_type][dc_sign_ctx], 2,
                          ACCT_STR);
#else
...
#if LV_MAP_PROB
      if (aom_read_bin(r, ec_ctx->coeff_br_cdf[txs_ctx][plane_type][idx][ctx],
                       2, ACCT_STR))
#else   // LV_MAP_PROB
...
#if LV_MAP_PROB
          if (aom_read_bin(r, ec_ctx->coeff_lps_cdf[txs_ctx][plane_type][ctx],
                           2, ACCT_STR))
#else
...
#if LV_MAP_PROB
      if (aom_read_bin(r, ec_ctx->coeff_lps_cdf[txs_ctx][plane_type][ctx], 2,
                       ACCT_STR))
#else
