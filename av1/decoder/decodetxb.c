...
void av1_read_vq_txb(MACROBLOCKD *xd, aom_reader *const r, const int blk_row,
  ...
  mbmi->use_vq[get_plane_type(plane)][blk_idx] = 1;

  int gain_sym1 =
      aom_read_symbol(r, ec_ctx->vq_gain_sym1_cdf, VQ_GAIN_SYMBOLS_1, ACCT_STR);
  int gain_sym2 = aom_read_symbol(r, ec_ctx->vq_gain_sym2_cdf[gain_sym1],
                                  VQ_GAIN_SYMBOLS_2, ACCT_STR);
  int qgain_idx = gain_sym1 * VQ_GAIN_SYMBOLS_2 + gain_sym2;
  mbmi->qgain_idx[plane][blk_idx] = qgain_idx;

  if (qgain_idx == 0) {
    mbmi->gain_sign[plane][blk_idx] = 1;
    mbmi->shape_idx[plane][blk_idx] = 0;
  } else {
    mbmi->gain_sign[plane][blk_idx] = aom_read_bit(r, ACCT_STR);
    ...
    mbmi->shape_idx[plane][blk_idx] =
        shape_sym1 * VQ_SHAPE_SYMBOLS_2 + shape_sym2;
  }
}
#endif
...
void av1_read_coeffs_txb_facade(const AV1_COMMON *const cm,
  ...
  if (tx_set_type == EXT_TX_SET_VQ &&
      aom_read_symbol(r, xd->tile_ctx->use_vq_cdf[plane_type][mbmi->mode], 2,
                      ACCT_STR)) {
    ...
    mbmi->use_vq[plane_type][blk_idx] = 0;
#endif
