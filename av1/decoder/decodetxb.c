...
uint8_t av1_read_coeffs_txb(const AV1_COMMON *const cm, DecoderCodingBlock *dcb,
  ...
  const int txb_skip_ctx =
      txb_ctx->txb_skip_ctx + (plane == 2 ? xd->eob_u_flag : 0);
      ...
      *max_scan_line = AOMMAX(*max_scan_line, pos);
      if (c == 0) {
        ...
        if (plane == AOM_PLANE_Y || plane == AOM_PLANE_U)
          ...
          sign = aom_read_symbol(
              r, ec_ctx->dc_v_sign_cdf[xd->tmp_sign[0]][dc_sign_ctx], 2,
              ACCT_STR);
        ...
        if (plane == AOM_PLANE_Y || plane == AOM_PLANE_U)
          ...
          sign = aom_read_symbol(r, ec_ctx->ac_v_sign_cdf[xd->tmp_sign[pos]], 2,
                                 ACCT_STR);
