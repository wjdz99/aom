...
void av1_decode_palette_tokens(MACROBLOCKD *const xd, int plane,
  ...
  const int block_width = AOMMIN(cols - col * tx_block_width, block_size_wide[bsize]);
  const int block_height = AOMMIN(rows - row * tx_block_height, block_size_high[bsize]);
  ...
  for (i = ((row == 0 && col == 0) ? 1 : 0); i < block_width + block_height - 1;
    ...
    for (j = AOMMIN(i, block_width - 1); j >= AOMMAX(0, i - block_height + 1);
         --j) {
      int color_ctx, color_idx;
      color_ctx = av1_get_palette_color_index_context(
          color_map, plane_block_width, row * tx_block_height + (i - j),
          col * tx_block_width + j, n, color_order, NULL);
      color_idx = aom_read_tree(r, av1_palette_color_index_tree[n - 2],
