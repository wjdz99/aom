...
static void token_to_value(aom_reader *const r, int token, TX_SIZE tx_size,
                           int bit_depth, int *const val) {
  switch (token) {
    case ZERO_TOKEN:
    case ONE_TOKEN:
    case TWO_TOKEN:
...
#else
      const int skip_bits =
          (int)sizeof(av1_cat6_prob) - av1_get_cat6_extrabits_size(tx_size, 8);
      (void)bit_depth;
#endif
...
static int decode_coefs(MACROBLOCKD *xd, PLANE_TYPE type, tran_low_t *dqcoeff,
  ...
  while (more_data) {
    ...
    *max_scan_line = AOMMAX(*max_scan_line, scan[c]);
    ...
    token_to_value(r, token, tx_size,
#if CONFIG_HIGHBITDEPTH
                   xd->bd,
#else
                   8,
