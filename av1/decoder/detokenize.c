  ...
  for (int i = 0; i < rows; ++i) {
    for (int j = (i == 0 ? 1 : 0); j < cols; ++j) {
      const int mask_ctx = av1_get_palette_color_index_context(
          mrc_mask, plane_block_width, i, j, n, color_order, NULL);
      const int color_idx = aom_read_symbol(
          r, palette_cdf[n - PALETTE_MIN_SIZE][mask_ctx], n, ACCT_STR);
      assert(mask_idx >= 0 && mask_idx < n);
      mrc_mask[i * plane_block_width + j] = color_order[mask_idx];
    }
    memset(color_map + i * plane_block_width + cols,
           mrc_mask[i * plane_block_width + cols - 1],
...
int av1_decode_block_tokens(AV1_COMMON *cm, MACROBLOCKD *const xd, int plane,
  ...
  if (tx_type == MRC_DCT) {
    ...
    av1_decode_mrc_tokens(xd, aom_reader *r, tx_size);
  }
#endif  // CONFIG_MRC_TX
