...
int ifd_inspect(insp_frame_data *fd, void *decoder, int skip_not_transform) {
  ...
  for (j = 0; j < mi_params->mi_rows; j++) {
    for (i = 0; i < mi_params->mi_cols; i++) {
      ...
      mbmi->ref_frame_nrs[0] = convert_named_ref_to_ranked_ref_index(&cm->new_ref_frame_data, mi->ref_frame[0]);
  mbmi->ref_frame_nrs[1] = convert_named_ref_to_ranked_ref_index(&cm->new_ref_frame_data, mi->ref_frame[1]);
