...
static void alloc_read_metadata(AV1Decoder *const pbi, uint32_t metadata_type,
                                const uint8_t *data, size_t sz,
                                uint32_t insert_flag) {
  AV1_COMMON *const cm = &pbi->common;
  aom_metadata_t *metadata =
      aom_img_metadata_alloc((OBU_METADATA_TYPE)metadata_type, data, sz,
...
static size_t read_metadata_hdr_mdcv(AV1Decoder *const pbi, const uint8_t *data,
  ...
  // 24 is the established mdcv payload size described in the mdcv standard
  if (bytes_read != 24) {
    aom_internal_error(&cm->error, AOM_CODEC_CORRUPT_FRAME,
