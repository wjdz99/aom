...
aom_codec_err_t aom_read_obu_header_and_size(const uint8_t *data,
  ...
  if (!obu_header->has_size_field) {
    ...
    *payload_size = obu_size - obu_header->size;
    ...
    status = read_obu_size(data + obu_header->size + length_field_size_obu,
                           bytes_available - obu_header->size -
                           length_field_size_obu, payload_size,
                           &length_field_size_payload);
    if (status != AOM_CODEC_OK) return status;
  }

  *bytes_read = length_field_size_obu + length_field_size_payload +
                obu_header->size;
  return AOM_CODEC_OK;
}
