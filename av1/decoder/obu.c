...
static uint32_t read_sequence_header_obu(AV1Decoder *pbi,
  ...
  if (seq_params->reduced_still_picture_hdr) {
    ...
    seq_params->tier[0] = 0;
    // TODO(wtc): This is not in the spec.
    ...
    for (int i = 0; i < operating_points_cnt_minus_1 + 1; i++) {
      seq_params->operating_point_idc[i] =
          aom_rb_read_literal(rb, OP_POINT_IDC_BITS);
      if (!read_bitstream_level(&seq_params->level[i], rb)) {
        cm->error.error_code = AOM_CODEC_UNSUP_BITSTREAM;
        return 0;
      }
      // seq_level_idx 7 is equivalent to level 3.3.
  ...
  // read_sequence_header() reports errors by calling rb->error_handler() or
  // aom_internal_error().
