...
static void read_metadata_itut_t35(struct aom_read_bit_buffer *rb) {
  int itu_t_t35_country_code = aom_rb_read_literal(rb, 8);
...
static void scalability_structure(struct aom_read_bit_buffer *rb) {
  int spatial_layers_cnt_minus_1 = aom_rb_read_literal(rb, 2);
...
// Checks the data for correct syntax but ignores the parsed data.
//
...
static size_t read_metadata(AV1Decoder *pbi, const uint8_t *data, size_t sz) {
  ...
  struct aom_read_bit_buffer rb;
  av1_init_read_bit_buffer(pbi, &rb, data + type_length,
                           data + sz - type_length);
