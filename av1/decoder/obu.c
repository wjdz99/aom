...
static uint32_t read_sequence_header_obu(AV1Decoder *pbi,
  ...
  for (int op_num = 0; op_num < cm->operating_points_decoder_model_cnt; ++op_num){
    cm->op_params[op_num].decoder_model_operating_point_idc = aom_rb_read_literal(rb, 12);
    cm->op_params[op_num].display_model_param_present_flag = aom_rb_read_bit(rb);
    if (cm->op_params[op_num].display_model_param_present_flag)
      cm->op_params[op_num].initial_display_delay = aom_rb_read_literal(rb, 4) + 1;
    cm->op_params[op_num].decoder_model_param_present_flag = aom_rb_read_bit(rb);
