...
static size_t read_metadata_itut_t35(AV1Decoder *const pbi, const uint8_t *data,
  ...
  pbi->metadata->metadata_array[pbi->metadata->sz - 1] = aom_img_metadata_alloc(
      OBU_METADATA_TYPE_ITUT_T35, data, (size_t)bytes_read);
  if (!pbi->metadata->metadata_array[pbi->metadata->sz - 1])
    aom_internal_error(&cm->error, AOM_CODEC_CORRUPT_FRAME,
