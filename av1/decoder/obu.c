...
static uint32_t read_sequence_header_obu(AV1Decoder *pbi,
  ...
  cm->operating_points_cnt_minus_1 = 0;
  ...
  if (seq_params->reduced_still_picture_hdr) {
    ...
    for (int i = 0; i < cm->operating_points_cnt_minus_1 + 1; i++) {
      ...
      if (cm->display_model_info_present_flag) {
  ...
  if (operating_point < 0 || operating_point > cm->operating_points_cnt_minus_1)
    operating_point = 0;
