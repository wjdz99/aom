...
static int32_t read_tile_group_header(AV1Decoder *pbi,
  ...
  const int num_tiles = pbi->common.tiles.rows * pbi->common.tiles.cols;

  if (!pbi->common.tiles.large_scale && num_tiles > 1) {
  ...
  if (pbi->common.tiles.large_scale || num_tiles == 1 ||
    ...
    *end_tile = num_tiles - 1;
    ...
    int tile_bits = cm->tiles.log2_rows + cm->tiles.log2_cols;
