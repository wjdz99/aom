...
static void alloc_tile_list_buffer(AV1Decoder *pbi) {
  ...
  // image format 4:2:0, the output frame of U plane and V plane is ¼ of the
  // output frame.
  AV1_COMMON *const cm = &pbi->common;
  const int tile_width_in_pixels = cm->tile_width * MI_SIZE;
  const int tile_height_in_pixels = cm->tile_height * MI_SIZE;
  const int tiles_per_frame = (pbi->output_frame_width_in_tiles_minus_1 + 1) *
                              (pbi->output_frame_height_in_tiles_minus_1 + 1);
  // The number of frames needed to store the tile list. The last output frame
  // can be only partially filled.
  const int num_frames =
      (pbi->tile_count_minus_1 + tiles_per_frame) / tiles_per_frame;
  ...
  // is 8, allow_lowbitdepth is 0. In this case, we could allocate less memory,
  // namely, 8 bits/pixel.
  // Re-allocate the buffer if it is not large enough.
  if (aom_realloc_multi_frame_buffer(
          &pbi->tile_list_outbuf, output_frame_width, output_frame_height,
          cm->seq_params.subsampling_x, cm->seq_params.subsampling_y,
          (cm->seq_params.bit_depth > AOM_BITS_8), 0, cm->byte_alignment, NULL,
          NULL, NULL, num_frames))
    aom_internal_error(&cm->error, AOM_CODEC_MEM_ERROR,
                       "Failed to allocate the tile list output buffer");

  memset(pbi->tile_list_outbuf.buffer_alloc, 0,
         pbi->tile_list_outbuf.buffer_alloc_sz);
}
...
static void yv12_tile_copy(const YV12_BUFFER_CONFIG *src, int hstart1,
  ...
  uint8_t *dst8 =
      dst->buffers[plane] + dst_offset + vstart2 * dst_stride + hstart2;
  ...
  for (plane = 0; plane < num_planes; ++plane) {
    ...
    if (cm->seq_params.use_highbitdepth &&
      ...
      yv12_tile_copy(cur_frame, hstart1, hend1, vstart1, vend1,
                     &pbi->tile_list_outbuf, hstart2, vstart2, frame_offset,
                     plane);
    } else {
      switch (plane) {
        case 0:
...
static uint32_t read_and_decode_one_tile_list(AV1Decoder *pbi,
  ...
  const size_t frame_size =
      (pbi->tile_list_outbuf.flags & YV12_FLAG_HIGHBITDEPTH)
          ? (pbi->tile_list_outbuf.frame_size >> 1)
          : pbi->tile_list_outbuf.frame_size;
  ...
  for (i = 0; i <= pbi->tile_count_minus_1; i++) {
    ...
    copy_decoded_tile_to_tile_list_buffer(pbi, frame_offset, tile_idx);
    tile_idx++;
  }
