...
static uint32_t read_and_decode_one_tile_list(AV1Decoder *pbi,
  ...
  for (i = 0; i <= pbi->tile_count_minus_1; i++) {
    ...
    uint8_t *bufs[MAX_MB_PLANE];
    int strides[3];
    int plane;

    bufs[AOM_PLANE_Y] = cur_frame->buffers[0];
    bufs[AOM_PLANE_U] = cur_frame->buffers[1];
    bufs[AOM_PLANE_V] = cur_frame->buffers[2];
    strides[AOM_PLANE_Y] = cur_frame->strides[0];
    strides[AOM_PLANE_U] = cur_frame->strides[1];
    strides[AOM_PLANE_V] = cur_frame->strides[1];
    if (is_hbd) {
      bufs[AOM_PLANE_Y] = (uint8_t *)CONVERT_TO_SHORTPTR(cur_frame->buffers[0]);
      bufs[AOM_PLANE_U] = (uint8_t *)CONVERT_TO_SHORTPTR(cur_frame->buffers[1]);
      bufs[AOM_PLANE_V] = (uint8_t *)CONVERT_TO_SHORTPTR(cur_frame->buffers[2]);
      strides[AOM_PLANE_Y] = 2 * cur_frame->strides[0];
      strides[AOM_PLANE_U] = 2 * cur_frame->strides[1];
      strides[AOM_PLANE_V] = 2 * cur_frame->strides[1];
    }
