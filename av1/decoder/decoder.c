...
static void release_frame_buffers(AV1Decoder *pbi) {
  ...
  if (pbi->hold_ref_buf) {
    ...
    for (mask = pbi->refresh_frame_flags; mask; mask >>= 1) {
      RefCntBuffer *old_buf = cm->ref_frame_map[ref_index];
      // Current thread releases the holding of reference frame.
      decrease_ref_count(old_buf, pool);

      // Release the reference frame holding in the reference map for the
      // decoding of the next frame.
      if (mask & 1) {
        RefCntBuffer *new_buf = cm->next_ref_frame_map[ref_index];
        decrease_ref_count(new_buf, pool);
      }
      ++ref_index;
    }
  ...
  if (frame_decoded) {
    ...
    if (!pbi->camera_frame_header_ready) {
      ...
      for (mask = pbi->refresh_frame_flags; mask; mask >>= 1) {
        ...
        // Release the reference frame holding in the reference map for the
        // decoding of the next frame.
        if (mask & 1) decrease_ref_count(old_buf, pool);
...
int av1_receive_compressed_data(AV1Decoder *pbi, size_t size,
  ...
  if (size == 0) {
    ...
    if (ref_buf) ref_buf->buf.corrupted = 1;
  }
