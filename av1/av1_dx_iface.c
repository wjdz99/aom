...
static aom_image_t *add_grain_if_needed(aom_codec_alg_priv_t *ctx,
  ...
  const unsigned int buf_align = 16;
  const unsigned int stride_align = 16;
  const unsigned int s = (w_even + stride_align - 1) & ~(stride_align - 1);
  const unsigned int stride_in_bytes =
      (img->fmt & AOM_IMG_FMT_HIGHBITDEPTH) ? s * 2 : s;
  const uint64_t alloc_size =
      (uint64_t)h_even * stride_in_bytes * bps / 8 + buf_align - 1;
  if (alloc_size != (size_t)alloc_size) return NULL;

  aom_codec_frame_buffer_t *fb =
      &ctx->grain_image_frame_buffers[ctx->num_grain_image_frame_buffers];
  BufferPool *const pool = ctx->buffer_pool;
  if (pool->get_fb_cb(pool->cb_priv, (size_t)alloc_size, fb) < 0) return NULL;
  if (fb->data == NULL || fb->size < alloc_size) return NULL;
  unsigned char *img_data = align_addr(fb->data, buf_align);

  grain_img->img_data = NULL;
  grain_img->img_data_owner = 0;
  grain_img->self_allocd = 0;
  if (!aom_img_wrap(grain_img, img->fmt, w_even, h_even, 16, img_data)) {
