...
// Copies the metadata from AV1Decoder and clears it from it, leaving it ready
// for next decode call.
static void copy_decoder_metadata_to_img(AV1Decoder *pbi, aom_image_t *img) {
  if (pbi->metadata && img) {
    ...
    do {
      ...
      if (winterface->sync(worker)) {
        ...
        if (av1_get_raw_frame(frame_worker_data->pbi, *index, &sd,
          ...
          copy_decoder_metadata_to_img(pbi, &ctx->img);
