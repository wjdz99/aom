...
aom_image_t *add_grain_if_needed(aom_image_t *img, aom_image_t **grain_buf_ptr,
                                 aom_film_grain_t *grain_params) {
  if (!grain_params->apply_grain) return img;

  aom_image_t *grain_img_buf = *grain_buf_ptr;
  ...
  // Save the (possibly updated) buffer back into our context
  *grain_buf_ptr = grain_img_buf;
    ...
    do {
      ...
      if (winterface->sync(worker)) {
        ...
        if (av1_get_raw_frame(frame_worker_data->pbi, *index, &sd,
          ...
          return add_grain_if_needed(img, &ctx->image_with_grain[*index],
                                     grain_params);
        }
      } else {
