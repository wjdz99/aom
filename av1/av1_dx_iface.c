  ...
  if (ctx->frame_worker != NULL) {
    ...
    if (winterface->sync(worker)) {
      ...
      if (av1_get_raw_frame(frame_worker_data->pbi, *index, &sd,
        ...
        if (pbi->ext_tile_debug && cm->tiles.single_tile_decoding &&
            pbi->dec_tile_row >= 0) {
          int tile_width, tile_height;
          av1_get_uniform_tile_size(cm, &tile_width, &tile_height);
          const int tile_row = AOMMIN(pbi->dec_tile_row, cm->tiles.rows - 1);
        ...
        if (pbi->ext_tile_debug && cm->tiles.single_tile_decoding &&
          ...
          int tile_width, tile_height;
          av1_get_uniform_tile_size(cm, &tile_width, &tile_height);
          const int tile_col = AOMMIN(pbi->dec_tile_col, cm->tiles.cols - 1);
