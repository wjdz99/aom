...
static aom_codec_err_t init_decoder(aom_codec_alg_priv_t *ctx) {
  ...
  ctx->buffer_pool->num_ref_frame_bufs = FRAME_BUFFERS;
  ctx->buffer_pool->frame_bufs =
      (RefCntBuffer *)aom_calloc(ctx->buffer_pool->num_ref_frame_bufs,
                                 sizeof(*ctx->buffer_pool->frame_bufs));
  if (ctx->buffer_pool->frame_bufs == NULL) return AOM_CODEC_MEM_ERROR;
