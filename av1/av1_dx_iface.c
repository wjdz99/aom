...
static aom_codec_err_t ctrl_get_dec_frame_info(aom_codec_alg_priv_t *ctx,
                                               va_list args) {
  SUBGOP_DATA *subgop_data = va_arg(args, SUBGOP_DATA *);
  if (!ctx->frame_worker) return AOM_CODEC_ERROR;
  AVxWorker *const worker = ctx->frame_worker;
  FrameWorkerData *const frame_worker_data = (FrameWorkerData *)worker->data1;
  AV1Decoder *const pbi = frame_worker_data->pbi;
  AV1_COMMON *const cm = &pbi->common;
  SubGOPStatsDec *subgop_stats = &pbi->subgop_stats;
  SUBGOP_STEP_DATA *step_data = subgop_data->step;
  int stat_count = subgop_stats->stat_count;
