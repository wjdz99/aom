...
static aom_codec_err_t ctrl_get_color_format(aom_codec_alg_priv_t *ctx,
                                             va_list args) {
  av1_color_format_t *const color_format = va_arg(args, av1_color_format_t *);
  AVxWorker *const worker = &ctx->frame_workers[ctx->next_output_worker_id];
  ...
  if (color_format) {
    if (worker) {
      FrameWorkerData *const frame_worker_data =
          (FrameWorkerData *)worker->data1;
      const AV1_COMMON *const cm = &frame_worker_data->pbi->common;
      color_format->subsampling_x = cm->subsampling_x;
      color_format->subsampling_y = cm->subsampling_y;
      return AOM_CODEC_OK;
    } else {
      return AOM_CODEC_ERROR;
    }
...
static aom_codec_ctrl_fn_map_t decoder_ctrl_maps[] = {
  ...
  { AV1D_GET_COLOR_FORMAT, ctrl_get_color_format },
