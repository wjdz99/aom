...
static aom_codec_err_t decoder_peek_si_internal(const uint8_t *data,
  ...
  do {
    ...
    if (data_size <= 0) break;  // exit if we're out of OBUs
  } while ((status = aom_read_obu_header_and_size(
                data, data_sz, si->is_annexb, &obu_header, &payload_size,
                &bytes_read)) == AOM_CODEC_OK);
