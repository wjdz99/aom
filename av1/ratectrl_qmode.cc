 ...
 *
 * \return std::vector<int> gf_intervals is returned to store the decided GF
 * group lengths.
 */
static std::vector<int> partition_gop_intervals(
  ...
  auto stats_in_loop_iter = start_pos;
    ...
    if (cur_last - cur_start <= rc_param.max_gop_show_frame_count &&
      ...
      for (int r = k_start + 1; r <= k_last; r++) {
        if (regions_list[r].type == SCENECUT_REGION &&
            regions_list[r].last - offset - cur_start >
      ...
      if (scenecut_idx != -1) {
        ...
        int is_minor_sc =
            (regions_list[scenecut_idx].avg_cor_coeff *
                 (1 - stats_list[order_index +
                                 regions_list[scenecut_idx].start - offset]
                              .noise_var /
                          regions_list[scenecut_idx].avg_intra_err) >
             0.6);
        cur_last = regions_list[scenecut_idx].last - offset - !is_minor_sc;
      } else {
        int is_last_analysed = (k_last == num_regions - 1) &&
                               (cur_last + offset == regions_list[k_last].last);
        ...
        if (!(is_last_analysed && not_enough_regions)) {
          ...
          double best_score = 0;
          int best_j = -1;
          const int first_frame = regions_list[0].start - offset;
          const int last_frame = regions_list[num_regions - 1].last - offset;
          ...
          for (int j = cur_start + min_shrink_int; j <= cur_last; j++) {
            ...
            base_score =
                (base_score + 1.0) * stats_list[order_index + j].cor_coeff;
            int this_reg =
                find_regions_index(regions_list, static_cast<int>(j) + offset);
          ...
          if (best_reg < num_regions - 1 && best_reg > 0) {
            if (regions_list[best_reg - 1].type == BLENDING_REGION &&
              ...
              if (best_j + offset == regions_list[best_reg].start &&
                  best_j + offset < regions_list[best_reg].last) {
                best_j += 1;
              } else if (best_j + offset == regions_list[best_reg].last &&
                         best_j + offset > regions_list[best_reg].start) {
                best_j -= 1;
              }
            }
          }
