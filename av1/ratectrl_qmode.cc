...
GopStructList AV1RateControlQMode::DetermineGopInfo(
  ...
  for (auto &stat : firstpass_info) {
    REGIONS region;
    int total_regions = 0;
    // TODO(jianj): MAX_FIRSTPASS_ANALYSIS_FRAMES should eventually be replaced
    // by the number of frames to the next KF.
    av1_identify_regions(&stat,
                         AOMMIN(static_cast<int>(round(stat.count)),
                                MAX_FIRSTPASS_ANALYSIS_FRAMES),
                         0, &region, &total_regions);
