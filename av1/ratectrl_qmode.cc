...
static double moddify_divisor(double divisor) {
...
static bool test_candidate_key(const FirstpassInfo &first_pass_info,
  ...
  bool test_this_candidate = false;
  ...
  if (this_stats.pcnt_inter < kVeryLowInterThreshold) {
    test_this_candidate = true;
  }

  if (detect_slide_transition(this_stats, last_stats, next_stats)) {
    test_this_candidate = true;
  }
  ...
  if (test_this_candidate) {
    ...
    for (i = 1; i <= frames_to_test_after_candidate_key; ++i) {
      ...
      double next_iiratio = (kBoostFactor * stats.intra_error /
                             moddify_divisor(stats.coded_error));
      ...
      // Question 3: Why do we need to divide 200 with unit_block_count?
      if ((stats.pcnt_inter < 0.05) || (next_iiratio < 1.5) ||
          (((stats.pcnt_inter - stats.pcnt_neutral) < 0.20) &&
           (next_iiratio < 3.0)) ||
          (boost_score_increment < 3.0) ||
          (stats.intra_error <
           (200.0 / static_cast<double>(first_pass_info.unit_block_count)))) {
        break;
      }
    }
