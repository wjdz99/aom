...
std::vector<RefFrameTable> AV1RateControlQMode::GetRefFrameTableList(
  ...
  int cur_gop_size = static_cast<int>(gop_struct.gop_frame_list.size());

  for (auto &lookahead_stat : lookahead_stats) {
    for (const GopFrame &gop_frame :
         lookahead_stat.gop_struct->gop_frame_list) {
      // Make a copy here since GopFrame in LookaheadStats is const.
      // Coding index of the GopFrame needs to be updated.
      GopFrame new_gop_frame(gop_frame);
      if (new_gop_frame.is_key_frame) {
        ref_frame_table.assign(rc_param_.ref_frame_table_size, new_gop_frame);
      } else if (new_gop_frame.update_ref_idx != -1) {
        assert(new_gop_frame.update_ref_idx <
               static_cast<int>(ref_frame_table.size()));
        new_gop_frame.coding_idx += cur_gop_size;
        ref_frame_table[new_gop_frame.update_ref_idx] = new_gop_frame;
      }
      ref_frame_table_list.push_back(ref_frame_table);
    }
  }
...
StatusOr<TplGopDepStats> ComputeTplGopDepStats(
  ...
  std::vector<TplFrameStats> tpl_frame_stats_list_with_lookahead(
      tpl_gop_stats.frame_stats_list);
  for (auto &lookahead_stat : lookahead_stats) {
    tpl_frame_stats_list_with_lookahead.insert(
        tpl_frame_stats_list_with_lookahead.end(),
        lookahead_stat.tpl_gop_stats->frame_stats_list.begin(),
        lookahead_stat.tpl_gop_stats->frame_stats_list.end());
  }
