 ...
 *
 * \return std::vector<int> gf_intervals is returned as a vector of decided GF
 * group lengths.
 */
      ...
      if (scenecut_idx != -1) {
        ...
        if (!(is_last_analysed && not_enough_regions)) {
          ...
          for (int j = cur_start + min_shrink_int; j <= cur_last; j++) {
            ...
            base_score =
                (base_score + 1.0) * stats_list[order_index + j].cor_coeff;
            int this_reg = find_regions_index(
                regions_list, static_cast<int>(j) + frames_since_key);
