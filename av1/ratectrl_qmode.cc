...
double tpl_frame_dep_stats_accumulate(const TplFrameDepStats &frame_dep_stats) {
  double sum = 0;
  const size_t unit_rows = frame_dep_stats.unit_stats.size();
  const size_t unit_cols = frame_dep_stats.unit_stats[0].size();
  for (size_t unit_row = 0; unit_row < unit_rows; ++unit_row) {
    for (size_t unit_col = 0; unit_col < unit_cols; ++unit_col) {
