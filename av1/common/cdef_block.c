...
void cdef_filter_fb(uint8_t *dst, int dstride, uint16_t *y, uint16_t *in,
  ...
  // If both primary and secondary strengths are 0, we still haven't
  // written anything to the destination yet, so we just copy the
  // input. This is necessary only for av1_cdef_search() and only
  // av1_cdef_search() sets dirinit.
#if CONFIG_CDEF_SINGLEPASS
