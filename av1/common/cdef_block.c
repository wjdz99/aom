...
void cccdef_filter_block_c(uint8_t *dst8, uint16_t *dst16, int dstride,
                           ...
                           int bit_depth, const int16_t *filter_coeff,
...
#if CCCDEF_LONG_TAP
      pixel[8] = in[curr_pos - 4];
      pixel[9] = in[curr_pos - 3];
      pixel[10] = in[curr_pos + 3];
      pixel[11] = in[curr_pos + 4];

#if VERTICAL_DIAGONAL_EXT == 1
      pixel[12] = in[curr_pos - 4 * stride];
      pixel[13] = in[curr_pos - 3 * stride];
      pixel[14] = in[curr_pos + 3 * stride];
      pixel[15] = in[curr_pos + 4 * stride];
#elif VERTICAL_DIAGONAL_EXT == 2
      pixel[12] = in[curr_pos - stride - 1];
      pixel[13] = in[curr_pos - stride + 1];
      pixel[14] = in[curr_pos + stride - 1];
      pixel[15] = in[curr_pos + stride + 1];
#endif
#endif
...
void av1_cccdef_filter_fb(uint8_t *dst8, uint16_t *dst16, int dstride,
                          ...
                          const uint8_t filter_strength_index,
  ...
  for (bi = 0; bi < cdef_count; bi++) {
    ...
    CHECKCDEF(direction_merge_mode != 0, " Directional merge mode is enabled ");
    if (direction_enable_flags[dir[by][bx]]) {
      const int16_t *filter_coeff_dir =
