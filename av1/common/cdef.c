...
static void cdef_prepare_fb(AV1_COMMON *cm, CdefBlockInfo *fb_info,
                            uint16_t **linebuf, const int *cdef_left,
                            const int fbc, const int fbr, uint8_t plane,
...
static INLINE void cdef_init_fb_col(MACROBLOCKD *xd,
                                    ...
                                    const int mbmi_cdef_strength, const int fbc,
                                    const int fbr, const uint8_t plane) {
...
static bool cdef_fb_col(AV1_COMMON *cm, MACROBLOCKD *xd, CdefBlockInfo *fb_info,
                        const int fbc, const int fbr, int *cdef_left,
                        uint16_t **linebuf, unsigned char *prev_row_cdef) {
  const CommonModeInfoParams *const mi_params = &cm->mi_params;
...
static INLINE void cdef_init_fb_row(CdefBlockInfo *fb_info, const int mi_rows,
                                    const int fbr) {
...
static void cdef_fb_row(AV1_COMMON *cm, MACROBLOCKD *xd, CdefBlockInfo *fb_info,
                        uint16_t **linebuf, const int fbr,
...
static void cdef_free(const int num_planes, unsigned char *row_cdef,
                      uint16_t **colbuf, uint16_t **linebuf) {
  aom_free(row_cdef);
  for (uint8_t plane = 0; plane < num_planes; plane++) {
...
void av1_cdef_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  cdef_free(av1_num_planes(cm), row_cdef, colbuf, linebuf);
}
