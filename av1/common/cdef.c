...
int sb_all_skip(const AV1_COMMON *const cm, int mi_row, int mi_col) {
  ...
  if (maxr > cm->mib_size_log2)
    maxr = cm->mib_size_log2;
  if (maxc > cm->mib_size_log2)
    maxc = cm->mib_size_log2;
#else
  if (maxr > MAX_MIB_SIZE)
    maxr = MAX_MIB_SIZE;
  if (maxc > MAX_MIB_SIZE)
    maxc = MAX_MIB_SIZE;
#endif
  ...
  for (sbr = 0; sbr < nvsb; sbr++) {
    ...
    for (sbc = 0; sbc < nhsb; sbc++) {
      ...
      for (pli = 0; pli < nplanes; pli++) {
        ...
        if (threshold == 0 && clpf_strength == 0)
          continue;
        od_dering(dst,
