...
void av1_cdef_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  for (fbr = 0; fbr < nvfb; fbr++) {
    ...
    for (fbc = 0; fbc < nhfb; fbc++) {
      ...
      if (fbr != nvfb - 1 &&
          (&cm->mi[mi_idx + (fb_mib_size - 1) * cm->mi_stride]))
        tile_bottom = cm->mi[mi_idx + (fb_mib_size - 1) * cm->mi_stride]
                          .mbmi.boundary_info &
                      TILE_BOTTOM_BOUNDARY;
      else
        tile_bottom = 1;

      if (fbc != nhfb - 1 && (&cm->mi[mi_idx + fb_mib_size - 1]))
        tile_right = cm->mi[mi_idx + fb_mib_size - 1].mbmi.boundary_info &
                     TILE_RIGHT_BOUNDARY;
      else
        tile_right = 1;
