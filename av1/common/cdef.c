    ...
    for (sbc = 0; sbc < nhsb; sbc++) {
      ...
      for (pli = 0; pli < nplanes; pli++) {
        ...
        if (!prev_row_dering[sbc]) {
          ...
          copy_nxm_16bit_to_16bit(&src[OD_FILT_HBORDER], OD_FILT_BSTRIDE,
        ...
        if (!prev_row_dering[sbc - 1]) {
          ...
          copy_nxm_16bit_to_16bit(src, OD_FILT_BSTRIDE, linebuf[pli] + coffset,
        ...
        if (!prev_row_dering[sbc + 1]) {
          ...
          copy_nxm_16bit_to_16bit(&src[nhbs + OD_FILT_HBORDER], OD_FILT_BSTRIDE,
