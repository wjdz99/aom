...
void av1_cdef_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  const int stride = (mi_params->mi_cols << MI_SIZE_LOG2) +
                     2 * CDEF_HBORDER;  // stride is for entire frame
  for (int pli = 0; pli < num_planes; pli++) {
    linebuf[pli] =
        aom_malloc(sizeof(*linebuf) * CDEF_VBORDER *
                   stride);  // number of lines for frame to store 3 lines
    colbuf[pli] =
        aom_malloc(sizeof(*colbuf) *
                   ((CDEF_BLOCKSIZE << mi_high_l2[pli]) + 2 * CDEF_VBORDER) *
                   CDEF_HBORDER);  // number of vertical lines 8 columns
  }
  for (int fbr = 0; fbr < nvfb; fbr++) {
    ...
    for (int fbc = 0; fbc < nhfb; fbc++) {
      ...
      nhb =
          AOMMIN(MI_SIZE_64X64,
                 mi_params->mi_cols -
                     MI_SIZE_64X64 * fbc);  // nummber of horizontal 4x4 blocks
                                            // within 64x64 filter block
      nvb =
          AOMMIN(MI_SIZE_64X64,
                 mi_params->mi_rows -
                     MI_SIZE_64X64 * fbr);  // nummber of vertical   4x4 blocks
                                            // within 64x64 filter block
      ...
      for (int pli = 0; pli < num_planes; pli++) {
        ...
        if (fbc == nhfb - 1)
          cend = hsize;  // frame right boundary
        else
          cend = hsize + CDEF_HBORDER;

        if (fbr == nvfb - 1)
          rend = vsize;  // frame bottom boundary
        else
        ...
        if (cm->seq_params.use_highbitdepth) {
          av1_cdef_filter_fb(
              ...
              sec_strength, damping, coeff_shift, 1);
        } else {
          av1_cdef_filter_fb(
              &xd->plane[pli]
                   .dst.buf[xd->plane[pli].dst.stride *
...
int extract_strength_from_index(int strength_idx) {
  int strength_values[4] = { 0, 1, 2, 4 };
  return strength_values[strength_idx];
}
void av1_cdef_cccdef_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  const int num_planes = av1_num_planes(cm);
#if CONFIG_CC_CDEF
  DECLARE_ALIGNED(16, uint16_t, src[3 * CDEF_INBUF_SIZE]);
#else
  DECLARE_ALIGNED(16, uint16_t, src[CDEF_INBUF_SIZE]);
#endif
  uint16_t *linebuf[3];
    ...
    for (int fbc = 0; fbc < nhfb; fbc++) {
      ...
      if (mi_params->mi_grid_base[MI_SIZE_64X64 * fbr * mi_params->mi_stride +
          ...
          (mi_params
           ...
           mi_params
                   ->mi_grid_base[MI_SIZE_64X64 * fbr * mi_params->mi_stride +
                                  MI_SIZE_64X64 * fbc]
                   ->cc_cdef_strength_index_fb[0] == -1 &&
           mi_params
                   ->mi_grid_base[MI_SIZE_64X64 * fbr * mi_params->mi_stride +
                                  MI_SIZE_64X64 * fbc]
                   ->cc_cdef_strength_index_fb[1] == -1)
#else
          mi_params
                  ->mi_grid_base[MI_SIZE_64X64 * fbr * mi_params->mi_stride +
                                 MI_SIZE_64X64 * fbc]
      ...
      for (int pli = 0; pli < num_planes; pli++) {
...
#if CONFIG_CC_CDEF
        int pli_offset = pli * CDEF_INBUF_SIZE;
#endif
        ...
        if (fbc == nhfb - 1) {
          /* On the last superblock column, fill in the right border with
             CDEF_VERY_LARGE to avoid filtering with the outside. */
          fill_rect(&src[pli_offset + cend + CDEF_HBORDER], CDEF_BSTRIDE,
                    rend + CDEF_VBORDER, hsize + CDEF_HBORDER - cend,
                    CDEF_VERY_LARGE);
        }
        if (fbr == nvfb - 1) {
          /* On the last superblock row, fill in the bottom border with
             CDEF_VERY_LARGE to avoid filtering with the outside. */
          fill_rect(&src[pli_offset + (rend + CDEF_VBORDER) * CDEF_BSTRIDE],
        ...
        /* Copy in the pixels we need from the current superblock for
           deringing.*/
        copy_sb8_16(cm,
                    &src[pli_offset + CDEF_VBORDER * CDEF_BSTRIDE +
                         CDEF_HBORDER + cstart],
                    CDEF_BSTRIDE, xd->plane[pli].dst.buf,
                    (MI_SIZE_64X64 << mi_high_l2[pli]) * fbr, coffset + cstart,
                    xd->plane[pli].dst.stride, rend, cend - cstart);
        if (!prev_row_cdef[fbc]) {
          copy_sb8_16(cm, &src[pli_offset + CDEF_HBORDER], CDEF_BSTRIDE,
                      xd->plane[pli].dst.buf,
                      (MI_SIZE_64X64 << mi_high_l2[pli]) * fbr - CDEF_VBORDER,
                      coffset, xd->plane[pli].dst.stride, CDEF_VBORDER, hsize);
        } else if (fbr > 0) {
          copy_rect(&src[pli_offset + CDEF_HBORDER], CDEF_BSTRIDE,
                    &linebuf[pli][coffset], stride, CDEF_VBORDER, hsize);
        } else {
          fill_rect(&src[pli_offset + CDEF_HBORDER], CDEF_BSTRIDE, CDEF_VBORDER,
                    hsize, CDEF_VERY_LARGE);
        }
        if (!prev_row_cdef[fbc - 1]) {
          copy_sb8_16(cm, &src[pli_offset], CDEF_BSTRIDE,
                      xd->plane[pli].dst.buf,
                      (MI_SIZE_64X64 << mi_high_l2[pli]) * fbr - CDEF_VBORDER,
                      coffset - CDEF_HBORDER, xd->plane[pli].dst.stride,
                      CDEF_VBORDER, CDEF_HBORDER);
        } else if (fbr > 0 && fbc > 0) {
          copy_rect(&src[pli_offset], CDEF_BSTRIDE,
                    &linebuf[pli][coffset - CDEF_HBORDER], stride, CDEF_VBORDER,
                    CDEF_HBORDER);
        } else {
          fill_rect(&src[pli_offset], CDEF_BSTRIDE, CDEF_VBORDER, CDEF_HBORDER,
                    CDEF_VERY_LARGE);
        }
        if (!prev_row_cdef[fbc + 1]) {
          copy_sb8_16(
              cm, &src[pli_offset + CDEF_HBORDER + (nhb << mi_wide_l2[pli])],
              CDEF_BSTRIDE, xd->plane[pli].dst.buf,
              (MI_SIZE_64X64 << mi_high_l2[pli]) * fbr - CDEF_VBORDER,
              coffset + hsize, xd->plane[pli].dst.stride, CDEF_VBORDER,
              CDEF_HBORDER);
        } else if (fbr > 0 && fbc < nhfb - 1) {
          copy_rect(&src[pli_offset + hsize + CDEF_HBORDER], CDEF_BSTRIDE,
                    &linebuf[pli][coffset + hsize], stride, CDEF_VBORDER,
                    CDEF_HBORDER);
        } else {
          fill_rect(&src[pli_offset + hsize + CDEF_HBORDER], CDEF_BSTRIDE,
                    CDEF_VBORDER, CDEF_HBORDER, CDEF_VERY_LARGE);
        }
        if (cdef_left) {
          /* If we deringed the superblock on the left then we need to copy in
             saved pixels. */
          copy_rect(&src[pli_offset], CDEF_BSTRIDE, colbuf[pli], CDEF_HBORDER,
                    rend + CDEF_VBORDER, CDEF_HBORDER);
        }
        /* Saving pixels in case we need to dering the superblock on the
                right. */
        copy_rect(colbuf[pli], CDEF_HBORDER, &src[pli_offset] + hsize,
        ...
        if (frame_top) {
          fill_rect(&src[pli_offset], CDEF_BSTRIDE, CDEF_VBORDER,
                    hsize + 2 * CDEF_HBORDER, CDEF_VERY_LARGE);
        }
        if (frame_left) {
          fill_rect(&src[pli_offset], CDEF_BSTRIDE, vsize + 2 * CDEF_VBORDER,
                    CDEF_HBORDER, CDEF_VERY_LARGE);
        }
        if (frame_bottom) {
          fill_rect(&src[pli_offset + (vsize + CDEF_VBORDER) * CDEF_BSTRIDE],
                    CDEF_BSTRIDE, CDEF_VBORDER, hsize + 2 * CDEF_HBORDER,
                    CDEF_VERY_LARGE);
        }
        if (frame_right) {
              ...
              &CONVERT_TO_SHORTPTR(
                  xd->plane[pli]
                      .dst.buf)[xd->plane[pli].dst.stride *
                                ...
                                (fbc * MI_SIZE_64X64 << mi_wide_l2[pli])],
              xd->plane[pli].dst.stride,
              &src[pli_offset + CDEF_VBORDER * CDEF_BSTRIDE + CDEF_HBORDER],
              xdec[pli], ydec[pli], dir, NULL, var, pli, dlist, cdef_count,
              level, sec_strength, damping, coeff_shift, 1);
        } else {
          av1_cdef_filter_fb(
              &xd->plane[pli]
                   .dst.buf[xd->plane[pli].dst.stride *
...
#if CONFIG_CC_CDEF
      // cross-component CDEF
      if (cm->seq_params.enable_cc_cdef && num_planes > 1) {
        // Convert CDEF direction to CC-CDEF directions
        for (int bi = 0; bi < cdef_count; bi++) {
          int by = dlist[bi].by;
          int bx = dlist[bi].bx;
          dir[by][bx] = cdefdir_to_cccdef_dir[dir[by][bx]];
        }

        for (int pli = 1; pli < num_planes; pli++) {
          uint8_t cccdef_enable =
              cdef_info->cc_cdef_info.cccdef_frame_enable_flag[pli - 1];

          if (cccdef_enable) {
            const int16_t *const filter_coeff =
                cdef_info->cc_cdef_info.filter_coeff[pli - 1];
            const int mbmi_cccdef_strength =
                mi_params
                    ->mi_grid_base[MI_SIZE_64X64 * fbr * mi_params->mi_stride +
                                   MI_SIZE_64X64 * fbc]
                    ->cc_cdef_strength_index_fb[pli - 1];

            uint8_t filter_strength =
                cdef_info->cc_cdef_info
                    .cccdef_strengths_index_array_frame[pli - 1]
                                                       [mbmi_cccdef_strength];
            const int direction_merge_mode = 0;
            const uint8_t *direction_enable_flags =
                &cdef_info->cc_cdef_info
                     .cccdef_frame_direction_enable_flag[pli - 1][0];

            if (cm->seq_params.use_highbitdepth) {
              av1_cccdef_filter_fb(
                  NULL,
                  &CONVERT_TO_SHORTPTR(
                      xd->plane[pli]
                          .dst
                          .buf)[xd->plane[pli].dst.stride *
                                    (MI_SIZE_64X64 * fbr << mi_high_l2[pli]) +
                                (fbc * MI_SIZE_64X64 << mi_wide_l2[pli])],
                  xd->plane[pli].dst.stride,
                  &src[0 + CDEF_VBORDER * CDEF_BSTRIDE + CDEF_HBORDER],
                  xdec[pli], ydec[pli], dir, dlist, cdef_count,
                  cm->seq_params.bit_depth, 0, filter_coeff, filter_strength,
                  direction_merge_mode, direction_enable_flags, 0);
            } else {
              av1_cccdef_filter_fb(
                  &xd->plane[pli]
                       .dst.buf[xd->plane[pli].dst.stride *
