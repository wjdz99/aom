...
int sb_all_skip(const AV1_COMMON *const cm, int mi_row, int mi_col) {
  ...
  maxr = AOMMIN(maxr, cm->mib_size >> CDEF_SB_SHIFT);
  maxc = AOMMIN(maxc, cm->mib_size >> CDEF_SB_SHIFT);
...
void av1_cdef_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  uint16_t *linebuf[3];
  uint16_t *colbuf[3];
  const int fb_mib_size = MAX_MIB_SIZE >> CDEF_SB_SHIFT;
  cdef_list
      dlist[(MAX_MIB_SIZE >> CDEF_SB_SHIFT) * (MAX_MIB_SIZE >> CDEF_SB_SHIFT)];
  ...
  nvfb = (cm->mi_rows + fb_mib_size - 1) / fb_mib_size;
  nhfb = (cm->mi_cols + fb_mib_size - 1) / fb_mib_size;
  ...
  for (fbr = 0; fbr < nvfb; fbr++) {
    for (pli = 0; pli < nplanes; pli++) {
      const int block_height =
          (fb_mib_size << mi_high_l2[pli]) + 2 * CDEF_VBORDER;
    ...
    for (fbc = 0; fbc < nhfb; fbc++) {
      ...
      int cstart = 0;
      curr_row_cdef[fbc] = 0;
      if (cm->mi_grid_visible[fb_mib_size * fbr * cm->mi_stride +
                              fb_mib_size * fbc] == NULL ||
          cm->mi_grid_visible[fb_mib_size * fbr * cm->mi_stride +
                              fb_mib_size * fbc]
      ...
      nhb = AOMMIN(fb_mib_size, cm->mi_cols - fb_mib_size * fbc);
      nvb = AOMMIN(fb_mib_size, cm->mi_rows - fb_mib_size * fbr);
      int tile_top, tile_left, tile_bottom, tile_right;
      int mi_idx = fb_mib_size * fbr * cm->mi_stride + fb_mib_size * fbc;
      ...
      if (fbr != nvfb - 1 &&
          cm->mi_grid_visible[mi_idx + fb_mib_size * cm->mi_stride])
        tile_bottom = cm->mi_grid_visible[mi_idx + fb_mib_size * cm->mi_stride]
                          ->mbmi.boundary_info &
                      TILE_ABOVE_BOUNDARY;
      else
        tile_bottom = 1;
      if (fbc != nhfb - 1 && cm->mi_grid_visible[mi_idx + fb_mib_size])
        tile_right =
            cm->mi_grid_visible[mi_idx + fb_mib_size]->mbmi.boundary_info &
            TILE_LEFT_BOUNDARY;
      else
        tile_right = 1;
      ...
      for (pli = 0; pli < nplanes; pli++) {
        ...
        coffset = fbc * fb_mib_size << mi_wide_l2[pli];
        if (fbc == nhfb - 1) {
        ...
        copy_sb8_16(cm,
                    ...
                    (fb_mib_size << mi_high_l2[pli]) * fbr, coffset + cstart,
                    xd->plane[pli].dst.stride, rend, cend - cstart);
        if (!prev_row_cdef[fbc]) {
          copy_sb8_16(cm, &src[CDEF_HBORDER], CDEF_BSTRIDE,
                      xd->plane[pli].dst.buf,
                      (fb_mib_size << mi_high_l2[pli]) * fbr - CDEF_VBORDER,
        ...
        copy_sb8_16(cm, &linebuf[pli][coffset], stride, xd->plane[pli].dst.buf,
                    (fb_mib_size << mi_high_l2[pli]) * (fbr + 1) - CDEF_VBORDER,
                    coffset, xd->plane[pli].dst.stride, CDEF_VBORDER, hsize);
              ...
              &CONVERT_TO_SHORTPTR(
                  xd->plane[pli]
                      .dst.buf)[xd->plane[pli].dst.stride *
                                ...
                                (fbc * fb_mib_size << mi_wide_l2[pli])],
