...
void av1_cdef_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  for (sbr = 0; sbr < nvsb; sbr++) {
    ...
    for (sbc = 0; sbc < nhsb; sbc++) {
      ...
      if (sbr != nvsb - 1 && (&cm->mi[mi_idx + (MAX_MIB_SIZE-1) * cm->mi_stride]))
        tile_bottom =
          cm->mi[mi_idx + (MAX_MIB_SIZE-1) * cm->mi_stride].mbmi.boundary_info &
      ...
      if (sbc != nhsb - 1 && (&cm->mi[mi_idx + MAX_MIB_SIZE - 1]))
        tile_right =
          cm->mi[mi_idx + MAX_MIB_SIZE - 1].mbmi.boundary_info &
          TILE_RIGHT_BOUNDARY;
      else
        tile_right = 1;
