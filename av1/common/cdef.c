...
void av1_cdef_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  uint16_t *linebuf[3];
  uint16_t colbuf[3][(MAX_SB_SIZE + 2 * CDEF_VBORDER) * CDEF_HBORDER];
    ...
    for (sbc = 0; sbc < nhsb; sbc++) {
      ...
      for (pli = 0; pli < nplanes; pli++) {
        ...
        copy_sb8_16(cm, &linebuf[pli][coffset], stride, xd->plane[pli].dst.buf,
                    (MAX_MIB_SIZE << mi_high_l2[pli]) * (sbr + 1) -
                        CDEF_VBORDER,
                    coffset, xd->plane[pli].dst.stride, CDEF_VBORDER, hsize);
        ...
        if (cm->use_highbitdepth) {
          cdef_filter_sb(
              ...
              sec_strength, pri_damping, sec_damping, coeff_shift, 1);
        } else {
#endif
          cdef_filter_sb(
              &xd->plane[pli]
              ...
              &src[CDEF_VBORDER * CDEF_BSTRIDE + CDEF_HBORDER], xdec[pli],
              ydec[pli], dir, NULL, var, pli, dlist, cdef_count, level,
              sec_strength, pri_damping, sec_damping, coeff_shift, 0);
