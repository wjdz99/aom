...
void av1_cdef_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  for (int fbr = 0; fbr < nvfb; fbr++) {
    ...
    for (int fbc = 0; fbc < nhfb; fbc++) {
      ...
      int mi_idx = mi_row * cm->mi_stride + mi_col;
      int mi_idx_br = (mi_row + MI_SIZE_64X64 - 1) * cm->mi_stride
          + (mi_col + MI_SIZE_64X64 - 1);
      // for the current filter block, it's top left corner mi structure (mi_tl)
      // is first accessed to check whether the top and left boundaries are
      // tile boundaries. Then bottom right mi structures (mi_br) is accessed
      // to check whether the bottom and right boundaries are tile boundaries.
      ...
      // then boundary_info should not be treated as tile boundaries. Also assume
      // CDEF filter block size is 64x64.
      MODE_INFO *const mi_tl = cm->mi + mi_idx;
      MODE_INFO *const mi_br = cm->mi + mi_idx_br;
      ...
      if (fbr != nvfb - 1 && mi_br)
        tile_bottom = mi_br->mbmi.boundary_info & TILE_BOTTOM_BOUNDARY;
      else
        tile_bottom = 1;

      if (fbc != nhfb - 1 && mi_br)
        tile_right = mi_br->mbmi.boundary_info & TILE_RIGHT_BOUNDARY;
      else
        tile_right = 1;
