...
void av1_cdef_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  for (fbr = 0; fbr < nvfb; fbr++) {
    ...
    for (sbc = 0; sbc < nhfb; sbc++) {
      ...
      int mi_idx = MI_SIZE_64X64 * fbr * cm->mi_stride + MI_SIZE_64X64 * sbc;
      ...
      if (sbc != nhfb - 1 && (&cm->mi[mi_idx + MI_SIZE_64X64 - 1]))
      ...
      clpf_strength = cm->cdef_strengths[mbmi_cdef_strength] % SEC_STRENGTHS;
      ...
      if ((level == 0 && clpf_strength == 0 && uv_level == 0 &&
          ...
          (cdef_count = sb_compute_cdef_list(
               cm, fbr * MI_SIZE_64X64, sbc * MI_SIZE_64X64, dlist,
      ...
      for (pli = 0; pli < nplanes; pli++) {
        ...
        if (cm->use_highbitdepth) {
          cdef_filter_fb(
              ...
              clpf_strength, sec_damping, pri_damping, coeff_shift, 0, 1);
        } else {
#endif
          cdef_filter_fb(
              &xd->plane[pli]
              ...
              &src[CDEF_VBORDER * CDEF_BSTRIDE + CDEF_HBORDER], xdec[pli],
              ydec[pli], dir, NULL, var, pli, dlist, cdef_count, level,
              clpf_strength, sec_damping, pri_damping, coeff_shift, 0, 0);
