...
// Loop filter with 8x8 granuity..
// When real time mode is used, the smallest block size and transform size
// is 8x8 for y plane, which allows us to apply loop filtering on a 8x8
// granuity instead of 4x4.
static void filter_block_plane_vert_8x8_granuity(
...
// is 8x8 for y plane, which allows us to apply loop filtering on a 8x8
// granuity instead of 4x4.
static void filter_block_plane_horz_8x8_granuity(
    const AV1_COMMON *const cm, const MACROBLOCKD *const xd, const int plane,
    const MACROBLOCKD_PLANE *const plane_ptr, const uint32_t mi_row,
    const uint32_t mi_col) {
  const uint32_t scale_horz = plane_ptr->subsampling_x;
...
void av1_filter_block_plane_vert(const AV1_COMMON *const cm,
  ...
  if (is_realtime && plane == 0) {
    return filter_block_plane_vert_8x8_granuity(cm, xd, plane, plane_ptr,
                                                mi_row, mi_col);
  }
...
void av1_filter_block_plane_horz(const AV1_COMMON *const cm,
                                 ...
                                 const uint32_t mi_row, const uint32_t mi_col) {
  if (is_realtime && plane == 0) {
    return filter_block_plane_horz_8x8_granuity(cm, xd, plane, plane_ptr,
                                                mi_row, mi_col);
  }
