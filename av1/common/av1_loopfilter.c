...
// Similar to set_lpf_parameters, but does so one row at a time to reduce calls
// to \ref get_transform_size and \ref av1_get_filter_level
static AOM_INLINE void set_lpf_parameters_row(
    AV1_DEBLOCKING_PARAMETERS *const params_buf, TX_SIZE *tx_buf,
    const AV1_COMMON *const cm, const MACROBLOCKD *const xd,
    const EDGE_DIR edge_dir, uint32_t x, const uint32_t y, const int plane,
    const struct macroblockd_plane *const plane_ptr, const uint32_t x_range) {
  ...
  const uint32_t scale_horz = plane_ptr->subsampling_x;
  const uint32_t scale_vert = plane_ptr->subsampling_y;
  const ptrdiff_t mode_step = (VERT_EDGE == edge_dir)
                                  ? (1 << scale_horz)
  ...
  while (x < x_range) {
    ...
    *tx_size = ts;
    ...
    const int advance_units = tx_size_wide_unit[ts];
...
// Similar to set_lpf_parameters, but does so one col at a time to reduce calls
// to \ref get_transform_size and \ref av1_get_filter_level
static AOM_INLINE void set_lpf_parameters_col(
    AV1_DEBLOCKING_PARAMETERS *const params_buf, TX_SIZE *tx_buf,
    const AV1_COMMON *const cm, const MACROBLOCKD *const xd,
    const EDGE_DIR edge_dir, const uint32_t x, uint32_t y, const int plane,
    const struct macroblockd_plane *const plane_ptr, const uint32_t y_range) {
  AV1_DEBLOCKING_PARAMETERS *params = params_buf;
  TX_SIZE *tx_size = tx_buf;
  ...
  while (y < y_range) {
    ...
    *tx_size = ts;
    ...
    const int advance_units = tx_size_high_unit[ts];
...
void av1_filter_block_plane_vert_rt(
  ...
  for (int y = 0; y < y_range; y += 2) {
    ...
    set_lpf_parameters_row(params_buf, tx_buf, cm, xd, VERT_EDGE, x_start,
...
void av1_filter_block_plane_vert_rt_chroma(
  ...
  for (int y = 0; y < y_range; y++) {
    if (y % 2 == 0 && (y + 1) < y_range) {
      ...
      // current and below rows must contain the same tx blocks. This is because
...
void av1_filter_block_plane_horz_rt(
  ...
  for (int x = 0; x < x_range; x += 2) {
    ...
    set_lpf_parameters_col(params_buf, tx_buf, cm, xd, HORZ_EDGE, curr_x,
