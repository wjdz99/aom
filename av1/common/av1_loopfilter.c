...
static int receive_filter_level(AV1_COMMON *const cm,
  ...
  const int filter_level =
      get_filter_level(cm, lfi_n, dir_idx, plane, mi_row, mi_col, mbmi);
#else
  (void)mi_row;
  (void)mi_col;
                      ...
                      default: {
                        ...
                        for (idx_4 = 1; idx_4 < 4;
                          ...
                          if (mi_4_col_offset >= max_cols ||
                            ...
                            continue;
                          build_y_masks(cm, lfi_n, mip, mode_info_stride, idx3,
                                        idy3, lfm, mi_row, mi_col);
                        }
                        break;
                      }
                    }
...
void av1_filter_block_plane_ss00_ver(AV1_COMMON *const cm,
  ...
  for (r = 0; r < cm->mib_size && mi_row + r < cm->mi_rows; r += 2) {
    ...
    uint32_t mask_16x16_l =
        (uint32_t)(mask_16x16->bits[index] >> shift) & bitmask;
    uint32_t mask_8x8_l = (uint32_t)(mask_8x8->bits[index] >> shift) & bitmask;
    uint32_t mask_4x4_l = (uint32_t)(mask_4x4->bits[index] >> shift) & bitmask;
...
// Disable filtering on the leftmost column.
      ...
      if (cm->use_highbitdepth)
        ...
        filter_selectively_vert_row2(
            plane->subsampling_x, dst->buf, dst->stride, mask_16x16_l,
            mask_8x8_l, mask_4x4_l, &cm->lf_info, &lfm->lfl_uv[r >> 1][0]);
