...
// TODO(luoyi): cleanup
#if 0
  static void print_bitmask(LOOP_FILTER_MASK *lf, int mi_row, int mi_col) {
    if (mi_row == 0 && mi_col == 0) {
      int i, j;
      for (j = 0; j < 1; j++)
        for (i = 0; i < 4; i++)
          printf("0x%016llx\n", (long long)lf->left_y[j].bits[i]);

      for (j = 0; j < 0; j++)
        for (i = 0; i < 4; i++)
          printf("0x%016llx\n", (long long)lf->above_y[j].bits[i]);
      printf("\n");
    }
  }

  static void print_level(LOOP_FILTER_MASK *lf, int mi_row, int mi_col) {
    if (mi_row == 0 && mi_col == 0) {

      long long *p = &lf->lfl_y[0][0];
...
void av1_loop_filter_rows(YV12_BUFFER_CONFIG *frame_buffer, AV1_COMMON *cm,
  ...
  for (mi_row = start; mi_row < stop; mi_row += cm->mib_size) {
    ...
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += cm->mib_size) {
      ...
      // print_bitmask(&lfm[0], mi_row, mi_col);
      // print_level(&lfm[0], mi_row, mi_col);
      for (plane = plane_start; plane < plane_end; ++plane) {
