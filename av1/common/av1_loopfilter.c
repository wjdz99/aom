      ...
      if (coord) {
        {
          ...
          if ((curr_level || pv_lvl) &&
            ...
            if (plane) {
              ...
              static const TX_SIZE tx_size_to_filter_length[TX_SIZES] = {
                4, 8, 14, 14, 14
              };
              params->filter_length = tx_size_to_filter_length[dim];
            }
    ...
    if (coord) {
      ...
      if (level && (!curr_skipped || pu_edge)) {
        ...
        if (plane) {
          ...
          static const TX_SIZE tx_size_to_filter_length[TX_SIZES] = { 4, 8, 14,
                                                                      14, 14 };
          params->filter_length = tx_size_to_filter_length[dim];
        }
...
void av1_filter_block_plane_vert_rt_chroma(
  ...
  for (int y = 0; y < y_range; y++) {
    ...
    for (int x = 0; x < x_range;) {
      ...
      filter_vert(p, dst_stride, params, min_height >= 8);
...
void av1_filter_block_plane_horz_rt_chroma(
  ...
  for (int x = 0; x < x_range; x++) {
    ...
    for (int y = 0; y < y_range;) {
      ...
      filter_horz(p, dst_stride, params, min_width >= 8);
