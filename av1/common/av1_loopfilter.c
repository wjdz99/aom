...
// Return TX_SIZE from av1_get_transform_size(), so it is plane and direction awared
...
static void av1_filter_block_plane_vert(
  ...
  for (int y = 0; y < y_range; y += row_step) {
    ...
    for (int x = 0; x < x_range; ) {
      ...
      tx_size = set_lpf_parameters(&params, ((ptrdiff_t)1 << scale_horz), cm, VERT_EDGE,
...
static void av1_filter_block_plane_horz(
  ...
  for (int x = 0; x < x_range; x += col_step) {
    ...
    for (int y = 0; y < y_range;) {
      ...
      tx_size = set_lpf_parameters(&params, (cm->mi_stride << scale_vert), cm, HORZ_EDGE,
                         curr_x, curr_y, plane, plane_ptr);
