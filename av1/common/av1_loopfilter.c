...
static void build_yuv_masks(AV1_COMMON *const cm, MODE_INFO **mi,
  ...
  build_masks(cm, mi, mode_info_stride, idx, idy, lfm, DO_YUV, br, bc);
}
...
static void setup_block_size(AV1_COMMON *const cm, MODE_INFO **mi,
  ...
  if (block_wide == cur_block_wide && block_high == cur_block_high) {
    if ((block_wide < 8 || block_high < 8) && position)
      build_y_masks(cm, mi, mi_stride, idx, idy, lfm, br, bc);
    else
      build_yuv_masks(cm, mi, mi_stride, idx, idy, lfm, br, bc);
