...
// ((idy >> MI_SIZE_LOG2) / 2) returns which uint64_t.
...
static int get_y_index_shift(int idx, int idy, int *y_index) {
  const int idy_unit = idy >> MI_SIZE_LOG2;
  const int idx_unit = idx >> MI_SIZE_LOG2;
  *y_index = idy_unit >> 2;

  const int rows = idy_unit % 4;
  const int stride_log2 = 4;
  const int shift = (rows << stride_log2) + idx_unit;
  return shift;
}
...
static int get_uv_index_shift(int idx, int idy, int ss_x, int ss_y) {
  const int stride_log2 = 3;
  const int idy_unit = (idy >> (MI_SIZE_LOG2 + ss_y));
  const int idx_unit = (idx >> (MI_SIZE_LOG2 + ss_x));
  const int shift = (idy_unit << stride_log2) + idx_unit;
  return shift;
}
...
static void check_loop_filter_masks(const LoopFilterMask *lfm) {
  ...
  check_mask_y(lfm->lfm_info.left_y);
  check_mask_y(lfm->lfm_info.above_y);
  check_mask_uv(lfm->lfm_info.left_u);
  check_mask_uv(lfm->lfm_info.above_u);
  check_mask_uv(lfm->lfm_info.left_v);
  check_mask_uv(lfm->lfm_info.above_v);
}
...
static void setup_masks(AV1_COMMON *const cm, int mi_row, int mi_col, int plane,
  ...
  int y_index;
  const int shift =
      plane ? get_uv_index_shift(idx, idy, subsampling_x, subsampling_y)
  ...
  for (dir = VERT_EDGE; dir <= HORZ_EDGE; ++dir) {
    ...
    if (is_edge) {
      ...
      if (dir == VERT_EDGE) {
        switch (plane) {
          case 0:
            lfm->lfm_info.left_y[min_tx_size].bits[y_index] |= (1 << shift);
            break;
          case 1: lfm->lfm_info.left_u[min_tx_size] |= (1 << shift); break;
          case 2: lfm->lfm_info.left_v[min_tx_size] |= (1 << shift); break;
          default: assert(plane <= 2);
        }
      } else {
        switch (plane) {
          case 0:
            lfm->lfm_info.above_y[min_tx_size].bits[y_index] |= (1 << shift);
            break;
          case 1: lfm->lfm_info.above_u[min_tx_size] |= (1 << shift); break;
          case 2: lfm->lfm_info.above_v[min_tx_size] |= (1 << shift); break;
          default: assert(plane <= 2);
        }
      }
    }
...
static void loop_filter_block_plane_vert(AV1_COMMON *const cm,
  ...
  switch (path) {
    case LF_PATH_420:
      ...
      break;
    case LF_PATH_444:
      av1_filter_block_plane_ss11_ver(cm, planes, pl, mi_row, mi_col);
      break;
    case LF_PATH_SLOW:
...
static void loop_filter_block_plane_horz(AV1_COMMON *const cm,
  ...
  switch (path) {
    case LF_PATH_420:
      ...
      break;
    case LF_PATH_444:
      av1_filter_block_plane_ss11_hor(cm, planes, pl, mi_row, mi_col);
      break;
    case LF_PATH_SLOW:
