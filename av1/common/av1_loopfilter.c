...
// Loop filter for real time mode.
// When real time mode is used, the smallest block size and transform size
// is 8x8, which allows us to apply loop filtering on a 8x8 granuity instead
// of 4x4.
static void filter_block_plane_vert_rt(const AV1_COMMON *const cm,
        ...
        case 4:
          if (use_highbitdepth) {
            aom_highbd_lpf_vertical_4_dual(
                ...
                params.hev_thr, params.mblim, params.lim, params.hev_thr,
                bit_depth);
          } else {
...
// is 8x8, which allows us to apply loop filtering on a 8x8 granuity instead
// of 4x4.
static void filter_block_plane_horz_rt(const AV1_COMMON *const cm,
                                       const MACROBLOCKD *const xd,
                                       const int plane,
                                       const MACROBLOCKD_PLANE *const plane_ptr,
                                       const uint32_t mi_row,
...
void av1_filter_block_plane_vert(const AV1_COMMON *const cm,
  ...
  if (is_realtime) {
    return filter_block_plane_vert_rt(cm, xd, plane, plane_ptr, mi_row, mi_col);
  }
...
void av1_filter_block_plane_horz(const AV1_COMMON *const cm,
                                 ...
                                 const uint32_t mi_row, const uint32_t mi_col) {
  if (is_realtime) {
    return filter_block_plane_horz_rt(cm, xd, plane, plane_ptr, mi_row, mi_col);
  }
...
static void loop_filter_rows(YV12_BUFFER_CONFIG *frame_buffer, AV1_COMMON *cm,
                             ...
                             int is_decoding,
#endif
                             int is_realtime, int plane_start, int plane_end) {
    ...
    if (cm->lf.combine_vert_horz_lf) {
      ...
      for (mi_row = start; mi_row < stop; mi_row += MAX_MIB_SIZE) {
        for (mi_col = col_start; mi_col < col_end; mi_col += MAX_MIB_SIZE) {
          ...
          av1_filter_block_plane_vert(cm, is_realtime, xd, plane, &pd[plane],
                                      mi_row, mi_col);
          ...
          if (mi_col - MAX_MIB_SIZE >= 0) {
            av1_setup_dst_planes(pd, cm->seq_params->sb_size, frame_buffer,
                                 ...
                                 plane + 1);
            av1_filter_block_plane_horz(cm, is_realtime, xd, plane, &pd[plane],
...
void av1_loop_filter_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  loop_filter_rows(frame, cm, xd, start_mi_row, end_mi_row,
                   ...
                   is_decoding,
#endif
                   is_realtime, plane_start, plane_end);
}
