...
static void set_lpf_parameters(
  ...
  assert(0 && "Not yet updated");
#endif  // CONFIG_EXT_PARTITION
  const int mi_row = (y >> MI_SIZE_LOG2) << scaleVert;
  const int mi_col = (x >> MI_SIZE_LOG2) << scaleHorz;
    ...
    if (coord) {
...
#endif  // CONFIG_LOOPFILTERING_ACROSS_TILES
      {
        ...
        if (tuEdge) {
          ...
          const int pvRow = (VERT_EDGE == edgeDir) ?
                            (mi_row) : (mi_row - (1 << scaleVert));
          const int pvCol = (VERT_EDGE == edgeDir) ?
                            (mi_col - (1 << scaleHorz)) : (mi_col);
