...
static void build_uv_masks(AV1_COMMON *const cm, MODE_INFO **mi, int plane,
  ...
  while (acc_size < tx_unit_h) {
    ...
    // In case of BLOCK_4X8/BLOCK_8X4 has two TX_4X4, the second TX_4X4 redo
    // UV mask that is the same as above TX_4X4
    single_edge = (FilterMaskUV)1 << shift_uv;
    if (plane == 1) {
      if (tx_w == TX_8X8) {
