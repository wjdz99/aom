...
static void check_loop_filter_masks(const LoopFilterMask *lfm, int plane) {
  ...
  if (plane == 0) {
    for (int i = 0; i < num_64x64; ++i) {
      ...
      // position.
      check_mask(lfm->lfm_info[i].left_y);
      check_mask(lfm->lfm_info[i].above_y);
    }
  } else if (plane == 1) {
    for (int i = 0; i < num_64x64; ++i) {
      check_mask(lfm->lfm_info[i].left_u);
      check_mask(lfm->lfm_info[i].above_u);
    }
  } else {
    for (int i = 0; i < num_64x64; ++i) {
      check_mask(lfm->lfm_info[i].left_v);
      check_mask(lfm->lfm_info[i].above_v);
    }
  }
}
