...
static AOM_FORCE_INLINE void set_lpf_parameters_for_line(
  ...
  {
    // Initialize the previous tx_size
    const int mi_row = scale_vert | ((y << scale_vert) >> MI_SIZE_LOG2);
    const int mi_col = scale_horz | ((x << scale_horz) >> MI_SIZE_LOG2);
    MB_MODE_INFO **mi =
        cm->mi_params.mi_grid_base + mi_row * cm->mi_params.mi_stride + mi_col;
...
static AOM_INLINE void filter_vert_chroma(uint8_t *u_dst, uint8_t *v_dst, int dst_stride,
                                   const AV1_DEBLOCKING_PARAMETERS *params,
                                   bool use_dual) {
  ...
  if (use_dual) {
    switch (params->filter_length) {
      // apply 4-tap filtering
      case 4:
        aom_lpf_vertical_4_dual(u_dst, dst_stride, u_limits->mblim, u_limits->lim,
                                u_limits->hev_thr, u_limits->mblim, u_limits->lim,
                                u_limits->hev_thr);
        aom_lpf_vertical_4_dual(v_dst, dst_stride, v_limits->mblim, v_limits->lim,
                                v_limits->hev_thr, v_limits->mblim, v_limits->lim,
                                v_limits->hev_thr);
        break;
      case 6:  // apply 6-tap filter for chroma plane only
        aom_lpf_vertical_6_dual(u_dst, dst_stride, u_limits->mblim, u_limits->lim,
                                u_limits->hev_thr, u_limits->mblim, u_limits->lim,
                                u_limits->hev_thr);
        aom_lpf_vertical_6_dual(v_dst, dst_stride, v_limits->mblim, v_limits->lim,
                                v_limits->hev_thr, v_limits->mblim, v_limits->lim,
                                v_limits->hev_thr);
        break;
      case 8:
...
void av1_filter_block_plane_vert_rt(
  ...
  for (int y = 0; y < y_range; y += 2) {
    ...
    set_lpf_parameters_for_line(params_buf, tx_buf, cm, xd, VERT_EDGE, x_start,
                                curr_y, plane, plane_ptr, x_end);
...
void av1_filter_block_plane_vert_rt_chroma(
  ...
  for (int y = 0; y < y_range; y++) {
    ...
    const uint32_t x_end =
        ((mi_col * MI_SIZE) >> scale_horz) + x_range * MI_SIZE;
    set_lpf_parameters_for_line_chroma(params_buf, tx_buf, cm, xd, VERT_EDGE, x_start,
                                curr_y, plane_ptr, x_end);
...
static AOM_INLINE void filter_horz_chroma(uint8_t *u_dst, uint8_t *v_dst, int dst_stride,
                                   const AV1_DEBLOCKING_PARAMETERS *params,
                                   bool use_dual) {
  const loop_filter_thresh *u_limits = params->uv_lfthr[0];
  const loop_filter_thresh *v_limits = params->uv_lfthr[1];
  if (use_dual) {
    switch (params->filter_length) {
      // apply 4-tap filtering
      case 4:
        aom_lpf_horizontal_4_dual(u_dst, dst_stride, u_limits->mblim, u_limits->lim,
                                  u_limits->hev_thr, u_limits->mblim, u_limits->lim,
                                  u_limits->hev_thr);
        aom_lpf_horizontal_4_dual(v_dst, dst_stride, v_limits->mblim, v_limits->lim,
                                  v_limits->hev_thr, v_limits->mblim, v_limits->lim,
                                  v_limits->hev_thr);
        break;
      case 6:  // apply 6-tap filter for chroma plane only
        aom_lpf_horizontal_6_dual(u_dst, dst_stride, u_limits->mblim, u_limits->lim,
                                  u_limits->hev_thr, u_limits->mblim, u_limits->lim,
                                  u_limits->hev_thr);
        aom_lpf_horizontal_6_dual(v_dst, dst_stride, v_limits->mblim, v_limits->lim,
                                  v_limits->hev_thr, v_limits->mblim, v_limits->lim,
                                  v_limits->hev_thr);
        break;
      case 8:
...
void av1_filter_block_plane_horz_rt(
  ...
  for (int x = 0; x < x_range; x += 2) {
    ...
    set_lpf_parameters_for_line(params_buf, tx_buf, cm, xd, HORZ_EDGE, curr_x,
                                y_start, plane, plane_ptr, y_end);
...
void av1_filter_block_plane_horz_rt_chroma(
  ...
  for (int x = 0; x < x_range; x++) {
    ...
    const uint32_t y_end =
        ((mi_row * MI_SIZE) >> scale_vert) + y_range * MI_SIZE;
    set_lpf_parameters_for_line_chroma(params_buf, tx_buf, cm, xd, HORZ_EDGE, curr_x,
                                y_start, plane_ptr, y_end);
