...
static void setup_masks(AV1_COMMON *const cm, const MB_MODE_INFO *const mbmi,
                        TX_SIZE y_tx_size, int x, int y, int mi_row, int mi_col,
                        LoopFilterMask *lfm) {
...
static void build_y_masks(AV1_COMMON *const cm, MODE_INFO **mi,
  ...
  const uint8_t filter_level =
      receive_filter_level(cm, lfi_n, 0, 0, 0, 0, mbmi);
...
static void setup_block_size(AV1_COMMON *const cm, MODE_INFO **mi,
                             ...
                             int idx, int idy, LoopFilterMask *lfm, int mi_row,
                             int mi_col, int br, int bc) {
  ...
  if (block_wide == cur_block_wide && block_high == cur_block_high) {
    ...
    for (i = 0; i < count; ++i) {
      ...
      setup_block_size(cm, mi_next, mi_stride, sub_w, sub_h, x, y, lfm, mi_row,
                       mi_col, br, bc);
    }
  }
}
