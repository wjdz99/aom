...
// Filter level can vary per MI
    ...
    switch (tx_wide) {
      case 64:
      case 32: tx_size_c = TX_32X32; break;
      case 16: tx_size_c = TX_16X16; break;
      case 8: tx_size_c = TX_8X8; break;
      default: tx_size_c = TX_4X4; assert(tx_wide == 4);
    }

    switch (tx_high) {
      case 64:
      case 32: tx_size_r = TX_32X32; break;
      case 16: tx_size_r = TX_16X16; break;
      case 8: tx_size_r = TX_8X8; break;
      default: tx_size_r = TX_4X4; assert(tx_high == 4);
    }

    memset(cm->top_txfm_context[pl] + ((mi_col + idx_c) << TX_UNIT_WIDE_LOG2),
           tx_size, AOMMAX(tx_size_wide_unit[tx_size],
                           mi_size_wide[BLOCK_8X8] << TX_UNIT_WIDE_LOG2));
    memset(cm->left_txfm_context[pl] +
               (((mi_row + idx_r) & MAX_MIB_MASK) << TX_UNIT_HIGH_LOG2),
           tx_size, AOMMAX(tx_size_high_unit[tx_size],
                           mi_size_high[BLOCK_8X8] << TX_UNIT_HIGH_LOG2));
#else
