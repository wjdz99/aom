...
static void setup_masks(const MB_MODE_INFO *const mbmi, TX_SIZE y_tx_size,
                        int x, int y, PlaneToWork plane, LoopFilterMask *lfm) {
...
static void setup_mask_from_tx_size(MODE_INFO **mi, int mi_stride,
                                    int max_tx_wide, int max_tx_high, int idx,
                                    int idy, PlaneToWork plane,
  ...
  if (tx_wide == max_tx_wide && tx_high == max_tx_high) {
    ...
    for (i = 0; i < 4; ++i) {
      ...
      setup_mask_from_tx_size(mi2, mi_stride, sub_tx_w, sub_tx_h, x, y, plane,
                              fm, br, bc);
    }
...
static void build_masks(AV1_COMMON *const cm, MODE_INFO **mi,
                        ...
                        LoopFilterMask *lfm, PlaneToWork plane, int br,
                        int bc) {
  ...
  if (!filter_level) {
    ...
    int i;
    for (i = 0; i < h; i++) memset(&fm->lfl_y[row + i][col], filter_level, w);
  }
  ...
  setup_mask_from_tx_size(mi, mode_info_stride, block_wide, block_high, idx,
                          idy, plane, lfm, br, bc);
}

// TODO(luoyi): 4:2:0 only
static void build_uv_masks(MODE_INFO **mi, int x, int y, LoopFilterMask *fm) {
  const MB_MODE_INFO *mbmi = &mi[0]->mbmi;
  const BLOCK_SIZE block_size = mbmi->sb_type;
...
static void setup_block_size(AV1_COMMON *const cm, MODE_INFO **mi,
                             ...
                             int idx, int idy, LoopFilterMask *lfm, int br,
                             int bc) {
