...
static void filter_selectively_vert_row2(int subsampling_factor, uint8_t *s,
  ...
  for (mask = mask_16x16_0 | mask_8x8_0 | mask_4x4_0 | mask_16x16_1 |
           mask_8x8_1 | mask_4x4_1; mask; mask >>= 1) {
...
static void filter_selectively_horiz2(
    uint8_t *s, int pitch, unsigned int mask_16x16, unsigned int mask_8x8,
    unsigned int mask_4x4, const loop_filter_info_n *lfi_n,
    const uint8_t *lfl) {
  unsigned int mask;
  int count;
    ...
    if (mask & 1) {
      if (mask_16x16 & 1) {
        if ((mask_16x16 & 3) == 3) {
          ...
          aom_lpf_horizontal_16(s, pitch, lfi->mblim, lfi->lim,
                                    lfi->hev_thr);
        }
      } else if (mask_8x8 & 1) {
        if ((mask_8x8 & 3) == 3) {
...
#if CONFIG_HIGHBITDEPTH
static void highbd_filter_selectively_horiz2(
    uint16_t *s, int pitch, unsigned int mask_16x16, unsigned int mask_8x8,
    unsigned int mask_4x4, const loop_filter_info_n *lfi_n, const uint8_t *lfl,
    int bd) {
  unsigned int mask;
  int count;
...
static void setup_masks(const MB_MODE_INFO *const mbmi, TX_SIZE y_tx_size,
  ...
  if (plane == DO_YUV) {
    // Do UV plane
    const TX_SIZE tx_uv_size = get_uv_tx_size(mbmi, y_tx_size);

    tx_width = txsize_horz_map[tx_uv_size];
    tx_height = txsize_vert_map[tx_uv_size];
...
static int receive_filter_level(AV1_COMMON *const cm,
  ...
  (void)plane;
#if CONFIG_LPF_SB
  const int filter_level = get_filter_level(cm, lfi_n, mi_row, mi_col, mbmi);
#else
...
static void setup_mask_intra(const MB_MODE_INFO *const mbmi, int block_wide,
                             int block_high, TX_SIZE tx_size, int idx, int idy,
                             PlaneToWork plane, LOOP_FILTER_MASK *lfm) {
  const int tx_wide = tx_size_wide[tx_size];
  const int tx_high = tx_size_high[tx_size];
  int x, y;
  for (y = idy; y < idy + block_high; y += tx_high) {
    for (x = idx; x < idx + block_wide; x += tx_wide) {
      setup_masks(mbmi, tx_size, x, y, plane, lfm);
    }
  }
}
...
static void build_masks(AV1_COMMON *const cm,
                        const loop_filter_info_n *const lfi_n, MODE_INFO **mi,
                        int mode_info_stride, int idx, int idy,
                        LOOP_FILTER_MASK *lfm, int mi_row, int mi_col,
                        PlaneToWork plane) {
  ...
  if (!filter_level) {
    ...
    const int shift_y = ((idy >> 3) << 3) + (idx >> 3);
  ...
  if (!is_inter_block(mbmi)) {
    setup_mask_intra(mbmi, block_wide, block_high, mbmi->tx_size, idx, idy,
                     DO_YUV, lfm);
  } else {
    ...
    setup_mask_from_tx(mi, mode_info_stride, block_wide, block_high, idx, idy,
                       plane, lfm, row_bound, col_bound);
  }
}

static void build_yuv_masks(AV1_COMMON *const cm,
                            const loop_filter_info_n *const lfi_n,
                            MODE_INFO **mi, int mode_info_stride, int idx,
                            int idy, LOOP_FILTER_MASK *lfm, int mi_row,
                            int mi_col) {
  build_masks(cm, lfi_n, mi, mode_info_stride, idx, idy, lfm, mi_row, mi_col,
              DO_YUV);
}
...
static void build_y_masks(AV1_COMMON *const cm,
                          const loop_filter_info_n *const lfi_n, MODE_INFO **mi,
                          int mode_info_stride, int idx, int idy,
                          LOOP_FILTER_MASK *lfm, int mi_row, int mi_col) {
  build_masks(cm, lfi_n, mi, mode_info_stride, idx, idy, lfm, mi_row, mi_col,
...
void av1_setup_mask(AV1_COMMON *const cm, const int mi_row, const int mi_col,
  ...
  const int offset_32[] = { 8, (mode_info_stride << 3) - 8, 8,
                            -(mode_info_stride << 3) - 8 };
  const int offset_16[] = { 4, (mode_info_stride << 2) - 4, 4,
                            -(mode_info_stride << 2) - 4 };
  const int offset_8[] = { 2, (mode_info_stride << 1) - 2, 2,
  ...
  switch (mip[0]->mbmi.sb_type) {
    case BLOCK_64X64:
      build_yuv_masks(cm, lfi_n, mip, mode_info_stride, 0, 0, lfm, mi_row,
                      mi_col);
      break;
    case BLOCK_64X32:
      build_yuv_masks(cm, lfi_n, mip, mode_info_stride, 0, 0, lfm, mi_row,
                      mi_col);
      mip2 = mip + (mode_info_stride << 3);
      if (max_rows - 8 <= 0) break;
      build_yuv_masks(cm, lfi_n, mip2, mode_info_stride, 0, 32, lfm, mi_row,
                      mi_col);
      break;
    case BLOCK_32X64:
      build_yuv_masks(cm, lfi_n, mip, mode_info_stride, 0, 0, lfm, mi_row,
                      mi_col);
      mip2 = mip + 8;
      if (max_cols - 8 <= 0) break;
      build_yuv_masks(cm, lfi_n, mip2, mode_info_stride, 32, 0, lfm, mi_row,
                      mi_col);
      break;
    default: {
      for (idx_32 = 0; idx_32 < 4; mip += offset_32[idx_32], ++idx_32) {
        const int mi_32_col_offset = (idx_32 & 1) << 3;
        const int mi_32_row_offset = (idx_32 >> 1) << 3;
        int idx = mi_32_col_offset << 2;
        int idy = mi_32_row_offset << 2;
        if (mi_32_col_offset >= max_cols || mi_32_row_offset >= max_rows)
          continue;
        switch (mip[0]->mbmi.sb_type) {
          case BLOCK_32X32:
            build_yuv_masks(cm, lfi_n, mip, mode_info_stride, idx, idy, lfm,
                            mi_row, mi_col);
            break;
          case BLOCK_32X16:
            build_yuv_masks(cm, lfi_n, mip, mode_info_stride, idx, idy, lfm,
                            mi_row, mi_col);
            if (mi_32_row_offset + 4 >= max_rows) continue;
            mip2 = mip + (mode_info_stride << 2);
            build_yuv_masks(cm, lfi_n, mip2, mode_info_stride, idx, idy + 16,
                            lfm, mi_row, mi_col);
            break;
          case BLOCK_16X32:
            build_yuv_masks(cm, lfi_n, mip, mode_info_stride, idx, idy, lfm,
                            mi_row, mi_col);
            if (mi_32_col_offset + 4 >= max_cols) continue;
            mip2 = mip + 4;
            build_yuv_masks(cm, lfi_n, mip2, mode_info_stride, idx + 16, idy,
                            lfm, mi_row, mi_col);
            break;
          default: {
            for (idx_16 = 0; idx_16 < 4; mip += offset_16[idx_16], ++idx_16) {
              const int mi_16_col_offset =
                  mi_32_col_offset + ((idx_16 & 1) << 2);
              const int mi_16_row_offset =
                  mi_32_row_offset + ((idx_16 >> 1) << 2);
              int idx1 = idx + ((idx_16 & 1) << 4);
              int idy1 = idy + ((idx_16 >> 1) << 4);
              if (mi_16_col_offset >= max_cols || mi_16_row_offset >= max_rows)
                continue;
              switch (mip[0]->mbmi.sb_type) {
                case BLOCK_16X16:
                  build_yuv_masks(cm, lfi_n, mip, mode_info_stride, idx1, idy1,
                                  lfm, mi_row, mi_col);
                  break;
                case BLOCK_16X8:
                  build_yuv_masks(cm, lfi_n, mip, mode_info_stride, idx1, idy1,
                                  lfm, mi_row, mi_col);
                  if (mi_16_row_offset + 2 >= max_rows) continue;
                  mip2 = mip + (mode_info_stride << 1);
                  build_yuv_masks(cm, lfi_n, mip2, mode_info_stride, idx1,
                                  idy1 + 8, lfm, mi_row, mi_col);
                  break;
                case BLOCK_8X16:
                  build_yuv_masks(cm, lfi_n, mip, mode_info_stride, idx1, idy1,
                                  lfm, mi_row, mi_col);
                  if (mi_16_col_offset + 2 >= max_cols) continue;
                  mip2 = mip + 2;
                  build_yuv_masks(cm, lfi_n, mip2, mode_info_stride, idx1 + 8,
                                  idy1, lfm, mi_row, mi_col);
                  break;
                default: {
                  for (idx_8 = 0; idx_8 < 4; mip += offset_8[idx_8], ++idx_8) {
                    const int mi_8_col_offset =
                        mi_16_col_offset + ((idx_8 & 1) << 1);
                    const int mi_8_row_offset =
                        mi_16_row_offset + ((idx_8 >> 1) << 1);
                    int idx2 = idx1 + ((idx_8 & 1) << 3);
                    int idy2 = idy1 + ((idx_8 >> 1) << 3);
                    ...
                    switch (mip[0]->mbmi.sb_type) {
                      case BLOCK_8X8:
                        build_yuv_masks(cm, lfi_n, mip, mode_info_stride, idx2,
                                        idy2, lfm, mi_row, mi_col);
                        break;
                      case BLOCK_8X4:
                        build_yuv_masks(cm, lfi_n, mip, mode_info_stride, idx2,
                                        idy2, lfm, mi_row, mi_col);
                        if (mi_8_row_offset >= max_rows) continue;
                        mip2 = mip + mode_info_stride;
                        build_y_masks(cm, lfi_n, mip2, mode_info_stride, idx2,
                                      idy2 + 4, lfm, mi_row, mi_col);
                        break;
                      case BLOCK_4X8:
                        build_yuv_masks(cm, lfi_n, mip, mode_info_stride, idx2,
                                        idy2, lfm, mi_row, mi_col);
                        if (mi_8_col_offset >= max_cols) continue;
                        mip2 = mip + 1;
                        build_y_masks(cm, lfi_n, mip2, mode_info_stride,
                                      idx2 + 4, idy2, lfm, mi_row, mi_col);
                        break;
                      default: {
                        build_yuv_masks(cm, lfi_n, mip, mode_info_stride, idx2,
                                        idy2, lfm, mi_row, mi_col);
                        mip += offset[0];
                        for (idx_4 = 1; idx_4 < 4; mip += offset[idx_4],
                                 ++idx_4) {
                          const int mi_4_col_offset =
                              mi_8_col_offset + (idx_4 & 1);
                          const int mi_4_row_offset =
                              mi_8_row_offset + (idx_4 >> 1);
                          int idx3 = idx2 + ((idx_4 & 1) << 2);
                          int idy3 = idy2 + ((idx_4 >> 1) << 2);
                          if (mi_4_col_offset >= max_cols ||
                              mi_4_row_offset >= max_rows)
                            continue;
                          build_y_masks(cm, lfi_n, mip, mode_info_stride,
                                        idx3, idy3, lfm, mi_row, mi_col);
                        }
                        break;
                      }
                    }
...
void av1_filter_block_plane_ss00_ver(AV1_COMMON *const cm,
  ...
  uint8_t *const dst0 = dst->buf;
  int r;
  const FilterMaskY *const mask_16x16 = &lfm->left_y[TX_16X16];
  const FilterMaskY *const mask_8x8 = &lfm->left_y[TX_8X8];
  const FilterMaskY *const mask_4x4 = &lfm->left_y[TX_4X4];
  ...
  for (r = 0; r < cm->mib_size && mi_row + r < cm->mi_rows; r += 2) {
    ...
    unsigned int mask_16x16_l = (mask_16x16->bits[index] >> shift) & bitmask;
    unsigned int mask_8x8_l = (mask_8x8->bits[index] >> shift) & bitmask;
    unsigned int mask_4x4_l = (mask_4x4->bits[index] >> shift) & bitmask;
...
// Disable filtering on the leftmost column.
    ...
    if (cm->use_highbitdepth)
      ...
      filter_selectively_vert_row2(
          ...
          mask_4x4_l, &cm->lf_info, &lfm->lfl_y[r][0]);
...
void av1_filter_block_plane_ss00_hor(AV1_COMMON *const cm,
  ...
  uint8_t *const dst0 = dst->buf;
  int r;
  const FilterMaskY *const mask_16x16 = &lfm->above_y[TX_16X16];
  const FilterMaskY *const mask_8x8 = &lfm->above_y[TX_8X8];
  const FilterMaskY *const mask_4x4 = &lfm->above_y[TX_4X4];
  ...
  for (r = 0; r < cm->mib_size && mi_row + r < cm->mi_rows; r++) {
    ...
    if (mi_row + r == 0) {
      ...
      mask_16x16_r = (mask_16x16->bits[index] >> shift) & 0xffff;
      mask_8x8_r = (mask_8x8->bits[index] >> shift) & 0xffff;
      mask_4x4_r = (mask_4x4->bits[index] >> shift) & 0xffff;
    }
    ...
    if (cm->use_highbitdepth)
      highbd_filter_selectively_horiz2(
          ...
          mask_4x4_r, &cm->lf_info, &lfm->lfl_y[r][0], (int)cm->bit_depth);
    else
#endif  // CONFIG_HIGHBITDEPTH
      filter_selectively_horiz2(dst->buf, dst->stride, mask_16x16_r, mask_8x8_r,
                                mask_4x4_r, &cm->lf_info, &lfm->lfl_y[r][0]);
...
void av1_filter_block_plane_ss11_ver(AV1_COMMON *const cm,
  ...
  FilterMaskUV mask_16x16 = lfm->left_uv[TX_16X16];
  FilterMaskUV mask_8x8 = lfm->left_uv[TX_8X8];
  FilterMaskUV mask_4x4 = lfm->left_uv[TX_4X4];
  ...
  for (r = 0; r < cm->mib_size && mi_row + r < cm->mi_rows; r += 4) {
    ...
    const int shift = r << 2;
    {
      unsigned int mask_16x16_l = (mask_16x16 >> shift) & 0xffff;
      unsigned int mask_8x8_l = (mask_8x8 >> shift) & 0xffff;
      unsigned int mask_4x4_l = (mask_4x4 >> shift) & 0xffff;
...
void av1_filter_block_plane_ss11_hor(AV1_COMMON *const cm,
  ...
  uint8_t *const dst0 = dst->buf;
  int r, c;
  FilterMaskUV mask_16x16 = lfm->above_uv[TX_16X16];
  FilterMaskUV mask_8x8 = lfm->above_uv[TX_8X8];
  FilterMaskUV mask_4x4 = lfm->above_uv[TX_4X4];
  ...
  for (r = 0; r < cm->mib_size && mi_row + r < cm->mi_rows; r += 2) {
    ...
    if (mi_row + r == 0) {
      ...
      mask_16x16_r = (mask_16x16 >> shift) & 0xffff;
      mask_8x8_r = (mask_8x8 >> shift) & 0xffff;
      mask_4x4_r = (mask_4x4 >> shift) & 0xffff;
    }
    ...
    if (cm->use_highbitdepth)
      highbd_filter_selectively_horiz2(
          CONVERT_TO_SHORTPTR(dst->buf), dst->stride, mask_16x16_r, mask_8x8_r,
          mask_4x4_r, &cm->lf_info, &lfm->lfl_uv[r >> 1][0],
          (int)cm->bit_depth);
    else
#endif  // CONFIG_HIGHBITDEPTH
      filter_selectively_horiz2(dst->buf, dst->stride, mask_16x16_r, mask_8x8_r,
...
void av1_loop_filter_rows(YV12_BUFFER_CONFIG *frame_buffer, AV1_COMMON *cm,
  ...
  for (mi_row = start; mi_row < stop; mi_row += cm->mib_size) {
    ...
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += cm->mib_size) {
      ...
      for (plane = plane_start; plane < plane_end; ++plane) {
        loop_filter_block_plane_ver(cm, planes, plane,
                                    mi + mi_col, mi_row, mi_col, path, &lfm);
        loop_filter_block_plane_hor(cm, planes, plane,
                                    mi + mi_col, mi_row, mi_col, path, &lfm);
      }
    }
  }
