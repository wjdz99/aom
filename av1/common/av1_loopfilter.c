...
static void build_y_masks(AV1_COMMON *const cm, MODE_INFO **mi,
  ...
  int i;
  for (i = 0; i < high; i++) {
    memset(&fm->lfl_y_hor[row + i][col], lpl_h, wide);
    memset(&fm->lfl_y_ver[row + i][col], lpl_v, wide);
  }
  ...
  top_skip += 2 * (cm->lf.neighbor_width + cm->lf.neighbor_height);
...
static void build_uv_masks(AV1_COMMON *const cm, MODE_INFO **mi, int plane,
  ...
  if (plane == 1) {
    left_uv = &lfm->left_u[tx_width];
    above_uv = &lfm->above_u[tx_height];
  } else {
    left_uv = &lfm->left_v[tx_width];
    above_uv = &lfm->above_v[tx_height];
  }
...
static void process_uv_level(AV1_COMMON *const cm, MODE_INFO **mi, int plane,
                             int idx, int idy, int mi_row, int mi_col,
                             LoopFilterMask *fm, int *lfl_flag) {
  const MB_MODE_INFO *mbmi = &mi[0]->mbmi;
  const BLOCK_SIZE block_size = mbmi->sb_type;
  ...
  if (plane == 1) {
    ...
    *lfl_flag = u_level[mi_row_pos] || lfl;
    ...
    if (!lfl && u_level[mi_row_pos])
      lfl = u_level[mi_row_pos];
    for (i = 0; i < high; i++) {
      memset(&lfm->lfl_u[row + i][col], lfl, wide);
    }
  }
  ...
  if (plane == 2) {
    *lfl_flag = v_level[mi_row_pos] || lfl;
    if (!lfl && u_level[mi_row_pos])
      lfl = u_level[mi_row_pos];
    for (i = 0; i < high; i++) {
      memset(&lfm->lfl_v[row + i][col], lfl, wide);
    }
  }
...
static void setup_block_size(AV1_COMMON *const cm, MODE_INFO **mi,
  ...
  if (block_wide == cur_block_wide && block_high == cur_block_high) {
    if (plane != 0) {
      int lflf = 0;
      process_uv_level(cm, mi, plane, idx, idy, mi_row, mi_col, lfm, &lflf);
      if (lflf)
        build_uv_masks(cm, mi, plane, idx, idy, mi_row, mi_col, lfm);
    } else {
  ...
  for (r = 0; r < cm->mib_size && mi_row + r < cm->mi_rows; r += 2) {
    ...
    if (cm->use_highbitdepth)
      ...
      filter_selectively_vert_row2(plane->subsampling_x, buf, dst->stride,
                                   ...
                                   &cm->lf_info, &mask->lfl_y_ver[r % mod][0]);
    ...
    if (sb_count > 1) {
      if (cm->use_highbitdepth)
        ...
        filter_selectively_vert_row2(
            ...
            mask2_4x4_l, &cm->lf_info, &mask2->lfl_y_ver[r % mod][0]);
    }
#endif
  ...
  for (r = 0; r < cm->mib_size && mi_row + r < cm->mi_rows; r++) {
    ...
    if (cm->use_highbitdepth)
      highbd_filter_selectively_horiz2(
          ...
          mask_4x4_r, &cm->lf_info, &mask->lfl_y_hor[r % mod][0],
          (int)cm->bit_depth);
    else
    ...
    if (sb_count > 1) {
      if (cm->use_highbitdepth)
        highbd_filter_selectively_horiz2(
            ...
            mask2_4x4_r, &cm->lf_info, &mask2->lfl_y_hor[r % mod][0],
            (int)cm->bit_depth);
      else
  ...
  for (r = 0; r < cm->mib_size && mi_row + r < cm->mi_rows; r += 4) {
    ...
    if (cm->use_highbitdepth)
      ...
      filter_selectively_vert_row2(
          ...
          mask_4x4_l, &cm->lf_info, &lfm->lfl_u[(r % mod) >> 1][0]);
...
// Disable filtering on the leftmost column.
    ...
    if (sb_count > 1) {
      if (cm->use_highbitdepth)
        ...
        filter_selectively_vert_row2(
            plane->subsampling_x, buf2, dst->stride, mask2_16x16_l, mask2_8x8_l,
            mask2_4x4_l, &cm->lf_info, &mask2->lfl_u[(r % mod) >> 1][0]);
    }
#endif
...
void av1_filter_block_plane_ss11_v_ver(AV1_COMMON *const cm,
  ...
  for (r = 0; r < cm->mib_size && mi_row + r < cm->mi_rows; r += 4) {
    ...
    if (cm->use_highbitdepth)
      ...
      filter_selectively_vert_row2(
          plane->subsampling_x, buf, dst->stride, mask_16x16_l, mask_8x8_l,
          mask_4x4_l, &cm->lf_info, &lfm->lfl_v[(r % mod) >> 1][0]);
...
// Disable filtering on the leftmost column.
    ...
    if (sb_count > 1) {
      if (cm->use_highbitdepth)
        ...
        filter_selectively_vert_row2(
            plane->subsampling_x, buf2, dst->stride, mask2_16x16_l, mask2_8x8_l,
            mask2_4x4_l, &cm->lf_info, &mask2->lfl_v[(r % mod) >> 1][0]);
    }
#endif
  ...
  for (r = 0; r < cm->mib_size && mi_row + r < cm->mi_rows; r += 2) {
    ...
    if (cm->use_highbitdepth)
      highbd_filter_selectively_horiz2(
          ...
          mask_4x4_a, &cm->lf_info, &mask->lfl_u[(r % mod) >> 1][0],
          (int)cm->bit_depth);
    else
    ...
    if (sb_count > 1) {
      if (cm->use_highbitdepth)
        highbd_filter_selectively_horiz2(
            ...
            mask2_4x4_a, &cm->lf_info, &mask2->lfl_u[(r % mod) >> 1][0],
            (int)cm->bit_depth);
      else
...
void av1_filter_block_plane_ss11_v_hor(AV1_COMMON *const cm,
  ...
  for (r = 0; r < cm->mib_size && mi_row + r < cm->mi_rows; r += 2) {
    ...
    if (cm->use_highbitdepth)
      highbd_filter_selectively_horiz2(
          CONVERT_TO_SHORTPTR(buf), dst->stride, mask_16x16_a, mask_8x8_a,
          mask_4x4_a, &cm->lf_info, &mask->lfl_v[(r % mod) >> 1][0],
          (int)cm->bit_depth);
    else
      filter_selectively_horiz2(buf, dst->stride, mask_16x16_a, mask_8x8_a,
                                mask_4x4_a, &cm->lf_info,
    ...
    if (sb_count > 1) {
      if (cm->use_highbitdepth)
        highbd_filter_selectively_horiz2(
            CONVERT_TO_SHORTPTR(buf), dst->stride, mask2_16x16_a, mask2_8x8_a,
            mask2_4x4_a, &cm->lf_info, &mask2->lfl_v[(r % mod) >> 1][0],
            (int)cm->bit_depth);
      else
        filter_selectively_horiz2(buf, dst->stride, mask2_16x16_a, mask2_8x8_a,
                                  mask2_4x4_a, &cm->lf_info,
