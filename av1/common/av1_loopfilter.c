...
static void build_masks(AV1_COMMON *const cm,
                        ...
                        LOOP_FILTER_MASK *lfm, PlaneToWork plane) {
...
static void build_yuv_masks(AV1_COMMON *const cm,
                            ...
                            int idy, LOOP_FILTER_MASK *lfm) {
  build_masks(cm, mi, mode_info_stride, idx, idy, lfm, DO_YUV);
}
...
static void build_y_masks(AV1_COMMON *const cm,
                          ...
                          int mode_info_stride, int idx, int idy,
                          LOOP_FILTER_MASK *lfm) {
  build_masks(cm, mi, mode_info_stride, idx, idy, lfm, DO_Y_ONLY);
}
...
static void setup_block_size(AV1_COMMON *const cm, MODE_INFO **mi, int mi_stride,
                             int block_wide, int block_high,
                             int idx, int idy, int position,
  ...
  if (block_wide == cur_block_wide && block_high == cur_block_high) {
    if ((block_wide < 8 || block_high < 8) && position)
      build_y_masks(cm, mi, mi_stride, idx, idy, lfm);
    else
      build_yuv_masks(cm, mi, mi_stride, idx, idy, lfm);
    ...
    for (i = 0; i < 4; ++i) {
      ...
      setup_block_size(cm, mi_next, mi_stride, sub_w, sub_h, x, y, i, lfm,
                       br, bc);
    }
...
void av1_setup_mask(AV1_COMMON *const cm, const int mi_row, const int mi_col,
  ...
  BLOCK_SIZE block_size = BLOCK_64X64;  //mi[0]->mbmi.sb_type;
  const int block_wide = block_size_wide[block_size];
  const int block_high = block_size_high[block_size];
  const int row_bound = cm->mi_rows - 1 - mi_row;
  const int col_bound = cm->mi_cols - 1 - mi_col;
  setup_block_size(cm, mi, mode_info_stride, block_wide, block_high, 0,
                   0, 0, lfm, row_bound, col_bound);
