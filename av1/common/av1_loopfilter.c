...
static void filter_block_plane_vert_dummy(
...
static void loop_filter_rows(YV12_BUFFER_CONFIG *frame_buffer, AV1_COMMON *cm,
  ...
  for (plane = plane_start; plane < plane_end; plane++) {
...
#if LOOP_FILTER_BITMASK
    // filter all vertical edges every 128x128
      ...
      for (mi_col = col_start; mi_col < col_end; mi_col += MAX_MIB_SIZE) {
        ...
        for (int y = 0; y < sb_num; ++y) {
          for (int x = 0; x < sb_num; ++x) {
            int row = mi_row + y * cm->seq_params.mib_size;
            int col = mi_col + x * cm->seq_params.mib_size;
            av1_setup_bitmask(cm, row, col, plane, pd[plane].subsampling_x,
    ...
    // filter all horizontal edges every 128x128
    for (mi_row = start; mi_row < stop; mi_row += MAX_MIB_SIZE) {
      for (mi_col = col_start; mi_col < col_end; mi_col += MAX_MIB_SIZE) {
        av1_setup_dst_planes(pd, cm->seq_params.sb_size, frame_buffer, mi_row,
                             mi_col, plane, plane + 1);
