...
LoopFilterMask *get_loop_filter_mask(const AV1_COMMON *const cm, int mi_row,
                                     int mi_col) {
  if (mi_row >= (cm->height >> MI_SIZE_LOG2) ||
      mi_col >= (cm->width >> MI_SIZE_LOG2))
    return NULL;
...
static void setup_tx_block_mask(AV1_COMMON *const cm, int mi_row, int mi_col,
  ...
  blk_row <<= ssy;
  blk_col <<= ssx;
  if (mi_row + blk_row >= (cm->height >> MI_SIZE_LOG2) ||
      mi_col + blk_col >= (cm->width >> MI_SIZE_LOG2))
    return;
...
static void setup_block_mask(AV1_COMMON *const cm, int mi_row, int mi_col,
  ...
  const int has_next_row =
      (mi_row + hbs < (cm->height >> MI_SIZE_LOG2)) & allow_sub8x8;
  const int has_next_col =
      (mi_col + hbs < (cm->width >> MI_SIZE_LOG2)) & allow_sub8x8;
  int i;
