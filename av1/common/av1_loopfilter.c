    ...
    if (mask & 1) {
      if (mask_16x16 & 1) {
        if ((mask_16x16 & 3) == 3) {
          ...
          // yet; use C code here now.
#if 1
          aom_lpf_horizontal_16_c(s, pitch, lfi->mblim, lfi->lim,
                                  lfi->hev_thr);
...
static void setup_masks(AV1_COMMON *const cm, const MB_MODE_INFO *const mbmi,
  ...
  if (lflf_v) {
    while (acc_h < tx_unit_h) {
      ...
      TX_SIZE choice_h = AOMMIN(tx_h, tx_high);
      ...
      if (choice_h < TX_32X32) {
        ...
        for (i = 0; i < 2; ++i) {
          mask[i] =
              left_txform_mask[choice_h].bits[i] & size_mask[block_size].bits[i];
          mask[i] <<= shift_y;
      ...
      acc_h += ntx_unit_h;
      y += (ntx_unit_h << MI_SIZE_LOG2);
      // Increment row position
      mi_row_loc += ntx_unit_h;
    }
  }
  ...
  if (lflf_h) {
    while (acc_w < tx_unit_w) {
      ...
      TX_SIZE choice_w = AOMMIN(tx_w, tx_wide);
      ...
      acc_w += ntx_unit_w;
      x += (ntx_unit_w << MI_SIZE_LOG2);
      // Increment col position
      mi_col_loc += ntx_unit_w;
    }
  }
...
void av1_loop_filter_rows(YV12_BUFFER_CONFIG *frame_buffer, AV1_COMMON *cm,
  ...
  for (mi_row = start; mi_row < stop; mi_row += cm->mib_size) {
    ...
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += cm->mib_size) {
      ...
      //if (plane_start == 0) print_luma(planes[0].dst.buf, planes[0].dst.stride);
