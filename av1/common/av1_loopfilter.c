...
static TX_SIZE get_y_tx_high(const MB_MODE_INFO *const mbmi, int mi_row,
  ...
  const TX_SIZE tx_high = txsize_vert_map[tx_size];
  return tx_high;
}
...
static void setup_masks(AV1_COMMON *const cm, MODE_INFO **mi, TX_SIZE y_tx_size,
  ...
  if (lflf_v) {
    ...
    while (acc_h < tx_unit_h) {
      ...
      int ntx_unit_h = tx_size_high_unit[ntx];
      ...
      if (acc_h > 0 && left_neighbor_not_available) {
        ...
        tx_h = get_y_tx_high(&left_mi[0]->mbmi, mi_row, mi_col);
        ntx_unit_h = tx_size_high_unit[tx_h];
      }
