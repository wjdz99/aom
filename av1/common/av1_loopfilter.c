...
static void set_lpf_parameters(
    AV1_DEBLOCKING_PARAMETERS *const params_ptr, const MODE_INFO **const mi_ptr,
    const ptrdiff_t mode_step, const AV1_COMMON *const cm,
    const EDGE_DIR edge_dir, const uint32_t x, const uint32_t y,
    const uint32_t width, const uint32_t height, const int plane,
    const struct macroblockd_plane *const plane_ptr, const uint32_t scale_horz,
    const uint32_t scale_vert) {
  // reset to initial values
  params_ptr->filter_length = 0;
  params_ptr->filter_length_internal = 0;
  ...
  const MB_MODE_INFO *mbmi = &mi_ptr[0]->mbmi;

  {
    const TX_SIZE ts =
        av1_get_transform_size(mi_ptr[0], edge_dir, mi_row, mi_col, plane,
    ...
    if (coord) {
...
#endif  // CONFIG_LOOPFILTERING_ACROSS_TILES
      {
        ...
        if (tu_edge) {
          const MODE_INFO *const mi_prev = *(mi_ptr - mode_step);
          const int pv_row =
          ...
          if ((curr_level || pv_lvl) &&
            ...
            if (TX_4X4 >= min_ts) {
              params_ptr->filter_length = 4;
            } else if (TX_8X8 == min_ts) {
              params_ptr->filter_length = 8;
            } else {
              params_ptr->filter_length = 16;
            ...
            params_ptr->filter_length =
                (TX_4X4 >= AOMMIN(ts, pv_ts)) ? (4) : (8);
      ...
      if (curr_level && !curr_skipped) {
        params_ptr->filter_length_internal = (TX_4X4 >= ts) ? (4) : (0);
      }
#endif

      // prepare common parameters
      if (params_ptr->filter_length || params_ptr->filter_length_internal) {
        const loop_filter_thresh *const limits = cm->lf_info.lfthr + level;
        params_ptr->lim = limits->lim;
        params_ptr->mblim = limits->mblim;
        params_ptr->hev_thr = limits->hev_thr;
      }
    }
  }
}
...
static void av1_filter_block_plane_vert(
    ...
    const MACROBLOCKD_PLANE *const plane_ptr, const MODE_INFO **mi_start_ptr,
    const uint32_t curr_x, const uint32_t curr_y) {
  ...
  for (int y = 0; y < (MAX_MIB_SIZE >> scale_vert); y += row_step) {
    ...
    for (int x = 0; x < (MAX_MIB_SIZE >> scale_horz); x += col_step) {
      ...
      const MODE_INFO **const mi_ptr =
          mi_start_ptr + (y << scale_vert) * cm->mi_stride + (x << scale_horz);
      AV1_DEBLOCKING_PARAMETERS params;
      memset(&params, 0, sizeof(params));

      set_lpf_parameters(&params, mi_ptr, ((ptrdiff_t)1 << scale_horz), cm,
                         VERT_EDGE, curr_x + x * MI_SIZE, curr_y + y * MI_SIZE,
                         width, height, plane, plane_ptr, scale_horz,
                         scale_vert);
...
static void av1_filter_block_plane_horz(
  ...
  for (int y = 0; y < (MAX_MIB_SIZE >> scale_vert); y += row_step) {
    ...
    for (int x = 0; x < (MAX_MIB_SIZE >> scale_horz); x += col_step) {
      ...
      AV1_DEBLOCKING_PARAMETERS params;
      memset(&params, 0, sizeof(params));
      set_lpf_parameters(&params, mi_ptr, (cm->mi_stride << scale_vert), cm,
                         HORZ_EDGE, curr_x + x * MI_SIZE, curr_y + y * MI_SIZE,
                         width, height, plane, plane_ptr, scale_horz,
                         scale_vert);
      switch (params.filter_length) {
        // apply 4-tap filtering
...
#endif  // CONFIG_UV_LVL
        const int32_t scale_horz = planes[plane_idx].subsampling_x;
        const int32_t scale_vert = planes[plane_idx].subsampling_y;
        av1_filter_block_plane_vert(cm, plane_idx, &planes[plane_idx],
                                    (const MODE_INFO **)(mi + mi_col),
                                    (mi_col * MI_SIZE) >> scale_horz,
                                    (mi_row * MI_SIZE) >> scale_vert);
      }
    }
  }
  ...
  for (mi_row = start; mi_row < stop; mi_row += MAX_MIB_SIZE) {
    ...
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += MAX_MIB_SIZE) {
      ...
      for (int plane_idx = 0; plane_idx < num_planes; plane_idx += 1) {
        ...
        const int32_t scale_horz = planes[plane_idx].subsampling_x;
        const int32_t scale_vert = planes[plane_idx].subsampling_y;
        av1_filter_block_plane_horz(cm, plane_idx, &planes[plane_idx],
                                    (const MODE_INFO **)(mi + mi_col),
                                    (mi_col * MI_SIZE) >> scale_horz,
                                    (mi_row * MI_SIZE) >> scale_vert);
      }
