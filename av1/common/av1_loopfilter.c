...
static void setup_masks(AV1_COMMON *const cm, const MB_MODE_INFO *const mbmi,
                        ...
                        int lflf_h, int lflf_v, LoopFilterMask *lfm) {
  ...
  FilterMaskY *left_y = &lfm->left_y[tx_w];
  FilterMaskY *above_y = &lfm->above_y[tx_h];
  ...
  if (lflf_v) {
    if (y_tx_size == TX_64X64) {
      ...
      mask[0] = left_txform_mask[tx_w].bits[0] & size_mask[block_size].bits[0];
      mask[0] <<= shift_y;
  ...
  // Top edge
  if (lflf_h) {
    mask[0] = above_txform_mask[tx_h].bits[0] & size_mask[block_size].bits[0];
    mask[0] <<= shift_y;
...
static void process_y_level(AV1_COMMON *const cm, MODE_INFO **mi, int idx,
  ...
  *lfl_v_flag = left_level[mi_row_pos] || lpl_v;
  ...
  int i;
  for (i = 0; i < high; i++) {
    memset(&fm->lfl_y_hor[row + i][col], lpl_h_f, wide);
    memset(&fm->lfl_y_ver[row + i][col], lpl_v_f, wide);
  }
}
...
static void build_uv_masks(AV1_COMMON *const cm, MODE_INFO **mi, int plane,
  ...
  if (plane == 1) {
    ...
    uint64_t mask[2];
    if (walk_on) {
      mask[0] = left_txform_mask_uv[tx_w] & size_mask_uv[block_size];
      mask[0] <<= shift_uv;
      *left_uv |= mask[0];
    }
    ...
    if (walk_on) {
      mask[0] = above_txform_mask_uv[tx_h] & size_mask_uv[block_size];
      mask[0] <<= shift_uv;
      *above_uv |= mask[0];
    }
