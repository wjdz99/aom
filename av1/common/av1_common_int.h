...
#if CONFIG_CC_CDEF
#if CCCDEF_LONG_TAP
#if VERTICAL_DIAGONAL_EXT
#define MAX_NUMBER_OF_CCCDEF_FILTER_COEFF 16
#else
#define MAX_NUMBER_OF_CCCDEF_FILTER_COEFF 12
#endif
#else
#define MAX_NUMBER_OF_CCCDEF_FILTER_COEFF 8
#endif

#define MAX_NEW_FILTERS_ENC_SEARCH 1
...
typedef struct {
  ...
  int16_t filter_coeff[2][MAX_NUMBER_OF_CCCDEF_FILTER_COEFF *
...
typedef struct {
  bool ccso_enable[2];
  int8_t filter_offset[2][CCSO_LUT_EXT_SIZE];
  uint8_t quant_idx[2];
...
static INLINE void av1_update_cccdef_filter_coeff_buffer(CdefInfo *cdef_info,
  ...
  int16_t *startp = &(coeff_buffer->buf[uv][write_position]);
...
#if CCCDEF_LONG_TAP
        const int16_t *def_filter_coeff =
            &defaultccdeffilter[uv][index][dir * 8];
#else
        const int16_t *def_filter_coeff =
...
#if CCCDEF_LONG_TAP
        memcpy(startp, def_filter_coeff, 8 * sizeof(def_filter_coeff[0]));
#else
...
static INLINE void copy_cccdef_filter_coeff_from_buffer(
    ...
    int16_t f[MAX_NUMBER_OF_CCCDEF_FILTER_COEFF], int uvplane, int dir) {
