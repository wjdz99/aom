...
static INLINE void av1_init_above_context(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  for (int i = 0; i < num_planes; ++i) {
    xd->above_context[i] = cm->above_contexts.entropy[i][tile_row];
  }
  xd->above_seg_context = cm->above_contexts.partition[tile_row];
  xd->above_txfm_context = cm->above_contexts.txfm[tile_row];
}
