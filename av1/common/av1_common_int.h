...
static INLINE int allow_tx_horz2_split(int use_inter_4way, 
                                       int use_intra_4way, 
                                       int is_inter, 
                                      TX_SIZE max_tx_size) {
  if (is_inter && !use_inter_4way) return 0;
  else if (!is_inter && !use_intra_4way) return 0;
  const int sub_txw = tx_size_wide[max_tx_size];
...
static INLINE int allow_tx_vert2_split(int use_inter_4way, 
                                       int use_intra_4way, 
                                       int is_inter, 
                                       TX_SIZE max_tx_size) {
  if (is_inter && !use_inter_4way) return 0;
...
static INLINE int use_tx_partition(int use_inter_4way,
                                   int use_intra_4way,
                                   int is_inter,
  ...
  const int allow_horz2 = allow_tx_horz2_split(use_inter_4way, use_intra_4way, is_inter, max_tx_size);
  const int allow_vert2 = allow_tx_vert2_split(use_inter_4way, use_intra_4way, is_inter, max_tx_size);
  switch (partition) {
