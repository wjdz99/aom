...
static INLINE void init_ibp_info_per_mode(
    uint16_t* weights[TX_SIZES_ALL][DIR_MODES_0_90],
    ...
    (uint16_t*)(aom_calloc(txw*txh, sizeof(uint16_t)));
    av1_dr_prediction_z1_info(weights[block_idx][mode_idx],
                              txw, txh, txw_log2, txh_log2,
                              dy, mode);
    return;
}
static INLINE void init_ibp_info(
    uint16_t* weights[TX_SIZES_ALL][DIR_MODES_0_90]) {
    for (TX_SIZE iblock = TX_4X4;
         iblock < TX_SIZES_ALL; iblock++) {
        ...
        for (int delta = -3; delta < 0; delta++) {
            init_ibp_info_per_mode(weights, iblock, V_PRED,
                                   delta, txw, txh,
                                   txw_log2, txh_log2);
            init_ibp_info_per_mode(weights, iblock, D67_PRED,
                                   delta, txw, txh,
                                   txw_log2, txh_log2);
            init_ibp_info_per_mode(weights, iblock, D45_PRED,
                                   delta, txw, txh,
                                   txw_log2, txh_log2);
        }
