...
static INLINE const struct scale_factors *get_ref_scale_factors_const(
    const AV1_COMMON *const cm, const MV_REFERENCE_FRAME ref_frame) {
#if !CONFIG_NEW_REF_SIGNALING
  const int converted_ref = cm->new_ref_frame_data.named_to_ranked_refs[ref_frame];
  //printf("%d\n", converted_ref);
  //const int map_idx = cm->new_ref_frame_data.named_to_ranked_refs[ref_frame];
  const int map_idx = get_ref_frame_map_idx(cm, converted_ref, 0);
//printf("conv %d idx %d n %d\n", converted_ref,map_idx, cm->new_ref_frame_data.n_total_refs);
//if (map_idx == INVALID_IDX)
//  printf("debug\n");
  const int map_idx2 = get_ref_frame_map_idx(cm, ref_frame, 1);
//printf("conv %d idx %d m2 %d\n", converted_ref,map_idx, map_idx2);
#else 
  const int map_idx = get_ref_frame_map_idx(cm, ref_frame, 1);
#endif  // CONFIG_NEW_REF_SIGNALING
