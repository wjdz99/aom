  ...
  for (tx_size = 0; tx_size < TX_SIZES; ++tx_size) {
    for (plane = 0; plane < PLANE_TYPES; ++plane) {
      for (ctx = 0; ctx < SIG_COEF_CONTEXTS; ++ctx) {
        ...
        fc->level_cdf[tx_size][plane][ctx][0] = AOM_ICDF(p);
        p += ((32768 - p) * fc->coeff_base[tx_size][plane][0][ctx]) >> 8;
        fc->level_cdf[tx_size][plane][ctx][1] = AOM_ICDF(p);
        p += ((32768 - p) * fc->coeff_base[tx_size][plane][1][ctx]) >> 8;
        fc->level_cdf[tx_size][plane][ctx][2] = AOM_ICDF(p);
        fc->level_cdf[tx_size][plane][ctx][3] = AOM_ICDF(32768);
        fc->level_cdf[tx_size][plane][ctx][4] = 0;
#else
