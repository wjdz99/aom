  ...
  if (xd->up_available) {
    ...
    do {  // for each mi in the above row
      ...
      if (is_neighbor_overlappable(above_mbmi)) {
        ...
        if ((i + mi_step - 1) * MI_SIZE < ((miw * MI_SIZE) / 2))
          continue;
        for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
          const struct macroblockd_plane *pd = &xd->plane[plane];
          ...
          if (i * MI_SIZE < ((miw * MI_SIZE) / 2)) {
            dst = &pd->dst.buf[((miw * MI_SIZE) / 2) >> pd->subsampling_x];
            tmp = &above[plane][((miw * MI_SIZE) / 2) >> pd->subsampling_x];
            bw = ((i + mi_step) * MI_SIZE - (miw * MI_SIZE) / 2) >>
                pd->subsampling_x;
          }
#if CONFIG_AOM_HIGHBITDEPTH
          if (is_hbd)
