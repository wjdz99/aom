  ...
  for (s = 0; s < NSMOOTHERS; s++) {
    ...
    for (i = 0; i < h; i += 2) {
      ...
      shift_copy(wedge_master_oblique_odd[s],
                 &wedge_mask_obl[s][1][WEDGE_OBLIQUE63][(i + 1) * stride], shift,
                 MASK_MASTER_SIZE);
    ...
    for (i = 0; i < h; i++) {
      for (j = 0; j < w; ++j) {
        ...
        const int msk = rint((1.0 + tanh(d / smoother_param[s])) * 32);
        wedge_mask_obl[s][1][WEDGE_OBLIQUE63][i * stride + j] = msk;
        const int mskx = rint((1.0 + tanh(x / smoother_param[s])) * 32);
