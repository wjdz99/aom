...
static void setup_obmc_mask_horizontal(uint8_t *mask, int mask_stride,
                                       const uint8_t *mask1d, int h, int w) {
  int r;
  uint8_t *pointer = mask;

  for (r = 0; r < h; ++r) {
    memcpy(pointer, mask1d, w);
    pointer += mask_stride;
  }
...
void av1_build_obmc_inter_prediction(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  for (i = 0; i < AOMMIN(xd->n8_h, cm->mi_rows - mi_row); i += mi_step) {
    ...
    for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
      ...
      setup_obmc_mask_horizontal(mask2d, MAX_SB_SIZE, mask[0], bh, bw);
