...
void realloc_tensor(TENSOR *tensor, int channels, int width, int height) {
...
void assign_tensor(TENSOR *tensor, float *buf[CNN_MAX_CHANNELS], int channels,
...
void swap_tensor(TENSOR *t1, TENSOR *t2) {
  TENSOR t = *t1;
  *t1 = *t2;
  *t2 = t;
}
...
void av1_cnn_predict_c(const float *input, int in_width, int in_height,
  ...
  for (int layer = 0; layer < cnn_config->num_layers; ++layer) {
    if (layer == 0) {  // First layer
      ...
      assign_tensor(&tensor1, (float **)&input, 1, in_width, in_height,
                    in_stride);
      if (cnn_config->num_layers == 1) {  // single layer case
        assert(cnn_config->layer_config[layer].out_channels == 1);
        assign_tensor(&tensor2, &output, 1, in_width, in_height, out_stride);
      ...
      av1_cnn_convolve((const float **)tensor1.buf, tensor1.width,
                       ...
                       &cnn_config->layer_config[layer], tensor2.buf,
                       tensor2.stride);
      ...
      if (layer < cnn_config->num_layers - 1) {  // Non-last layer
        ...
        assert(cnn_config->layer_config[layer].out_channels == 1);
        free_tensor(&tensor2);
        assign_tensor(&tensor2, &output, 1, o_width, o_height, out_stride);
      }
      av1_cnn_convolve((const float **)tensor1.buf, tensor1.width,
                       tensor1.height, tensor1.stride,
                       &cnn_config->layer_config[layer], tensor2.buf,
