...
void av1_cnn_process_layer_c(const float **input, int in_width, int in_height,
                             int in_stride,
                             const CNN_LAYER_CONFIG *layer_config,
                             const float **skip_buf, int skip_stride,
                             float **output, int out_stride) {
  if (!layer_config->deconvolve) {
    av1_cnn_convolve(input, in_width, in_height, in_stride, layer_config,
                     skip_buf, skip_stride, output, out_stride);

  } else {
    av1_cnn_deconvolve(input, in_width, in_height, in_stride, layer_config,
                       skip_buf, skip_stride, output, out_stride);
  }
}
...
void av1_cnn_predict_c(const float *input, int in_width, int in_height,
  ...
  for (int layer = 0; layer < cnn_config->num_layers; ++layer) {
    if (layer == 0) {  // First layer
      ...
      av1_cnn_process_layer((const float **)tensor1.buf, tensor1.width,
