  ...
  switch (layer_config->pad) {
    ...
    case PADDING_VALID:
      ...
      *out_height = (in_height - layer_config->filter_height +
                    ...
                    layer_config->skip_height;
      break;
    default: assert(0 && "Unknown padding type"); return;
  }
}
...
static void find_cnn_output_size(int in_width, int in_height,
                                 ...
                                 int *out_height) {
  int o_width = in_width + cnn_config->ext_width * 2;
  int o_height = in_height + cnn_config->ext_height * 2;
  for (int i = 0; i < cnn_config->num_layers; ++i) {
    find_layer_output_size(o_width, o_height, &cnn_config->layer_config[i],
                           &o_width, &o_height);
  }
  *out_width = o_width;
  *out_height = o_height;
