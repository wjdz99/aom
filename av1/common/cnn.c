...
void av1_cnn_convolve_c(const float **input, int in_width, int in_height,
  ...
  int filter_height_half = nn_layer_config->filter_height >> 1;
  int filter_width_half = nn_layer_config->filter_width >> 1;
  for (int i = 0; i < layer_config->out_channels; ++i) {
    for (int h = 0, u = 0; h < in_width; h += layer_config->skip_height, ++u) {
      for (int w = 0, v = 0; w < in_height;
        ...
        for (int k = 0; k < layer_config->in_channels; ++k)
          for (int l = -filter_height_half;
               l < -filter_height_half + layer_config->filter_height; ++l)
            for (int m = -filter_width_half;
                 m < -filter_width_half + layer_config->filter_width; ++m) {
...
void av1_cnn_predict_c(const float *input, int in_width, int in_height,
                       ...
                       float *output, int out_stride) {
  const float *buf1[CNN_MAX_CHANNLES];
  float *buf2[CNN_MAX_CHANNLES];
  ...
  for (int layer = 0; layer < cnn_config->num_layers; ++layer) {
    if (layer == 0) {  // First layer
      ...
      float *buf10 = buf1[0];
      for (int c = 0; c < cnn_config->layer_config[layer - 1].out_channels; ++c)
        buf1[c] = buf2[c];
      buf2[0] = buf10;
      ...
      av1_cnn_convolve_c(buf1, i_width, i_height, i_stride,
                         &cnn_config->layer_config[layer], buf2, o_stride);
    }
  }
  if (buf1[0]) free(buf1[0]);
}
