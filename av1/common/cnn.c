...
void av1_cnn_predict_c(const float **input, int in_width, int in_height,
  ...
  for (int layer = 0; layer < cnn_config->num_layers; ++layer) {
    ...
    if (cnn_config->layer_config[layer].bn_gamma) {
      assert(cnn_config->layer_config[layer].bn_gamma &&
             cnn_config->layer_config[layer].bn_beta &&
             cnn_config->layer_config[layer].bn_mean &&
             cnn_config->layer_config[layer].bn_std &&
             "Not all batchnorm parameters are valid");

      av1_cnn_batchnorm(tensor2[branch].buf, tensor2[branch].channels,
                        tensor2[branch].width, tensor2[branch].height,
                        tensor2[branch].stride,
                        cnn_config->layer_config[layer].bn_gamma,
                        cnn_config->layer_config[layer].bn_beta,
                        cnn_config->layer_config[layer].bn_mean,
                        cnn_config->layer_config[layer].bn_std);
    }
