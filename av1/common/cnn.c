...
static INLINE int check_tensor_equal_dims(TENSOR *t1, TENSOR *t2) {
  return (t1->width == t2->width && t1->height == t2->height);
}

static INLINE int check_tensor_equal_size(TENSOR *t1, TENSOR *t2) {
...
static int get_start_shift(int width, int stride) {
  const int mod = width % stride;
  return (mod ? mod - 1 : stride - 1) / 2;
}
...
void av1_cnn_convolve_c(const float **input, int in_width, int in_height,
  ...
  if (layer_config->maxpool &&
    ...
    switch (layer_config->pad) {
      case PADDING_SAME_ZERO:
        for (int i = 0; i < layer_config->out_channels; ++i) {
          for (int h = get_start_shift(in_height, layer_config->skip_height), u = 0;
               h < in_height; h += layer_config->skip_height, ++u) {
            for (int w = get_start_shift(in_width, layer_config->skip_width), v = 0;
