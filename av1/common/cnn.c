...
void av1_restore_cnn_plane_part(AV1_COMMON *cm, const CNN_CONFIG *cnn_config,
  ...
  if (cm->seq_params.use_highbitdepth) {
    ...
    switch (plane) {
      case AOM_PLANE_Y:
        offset = start_y * buf->y_stride + start_x;
        av1_restore_cnn(buf->y_buffer + offset, width, height, buf->y_stride,
                        cnn_config);
        break;
      case AOM_PLANE_U:
        offset = (start_y >> buf->subsampling_y) * buf->uv_stride +
        ...
        uv_width = width >> buf->subsampling_x;
        uv_height = height >> buf->subsampling_y;
        av1_restore_cnn(buf->u_buffer + offset, uv_width, uv_height,
      ...
      case AOM_PLANE_V:
        ...
        uv_width = width >> buf->subsampling_x;
        uv_height = height >> buf->subsampling_y;
        av1_restore_cnn(buf->v_buffer + offset, uv_width, uv_height,
                        buf->uv_stride, cnn_config);
        break;
      default: assert(0 && "Invalid plane index");
    }
