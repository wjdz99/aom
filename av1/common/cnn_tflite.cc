...
extern "C" int av1_restore_cnn_guided_img_tflite_highbd(
    ...
    int is_intra_only, const uint16_t *src, int src_stride, CRLCInfo *ci,
    int frameType) {
  // TODO(dandan): Change the code to get interpreter for guided CNN model.
  TfLiteDelegate *xnnpack_delegate = get_tflite_xnnpack_delegate(num_threads);
  std::unique_ptr<tflite::Interpreter> interpreter = get_tflite_interpreter(
      ...
      switch (plane) {
        case AOM_PLANE_Y:
          ...
          av1_restore_cnn_guided_img_tflite_highbd(
              ...
              num_threads, cm->seq_params.bit_depth, is_intra_only,
              CONVERT_TO_SHORTPTR(source_frame->y_buffer), source_frame->y_stride,
              &cm->crlc_info[0],
      ...
      switch (plane) {
        case AOM_PLANE_Y:
          ...
          av1_restore_cnn_guided_decode_img_tflite_highbd(
              ...
              cm->seq_params.bit_depth,is_intra_only, &cm->crlc_info[0],
              cm->crlc_info->crlc_unit_size);
          break;
        case AOM_PLANE_U:
          av1_restore_cnn_img_tflite_highbd(
