...
get_mv_precision(const MV mv, MvSubpelPrecision frame_precision) {
  ...
  if ((mv.row & 1) || (mv.col & 1)) precision = MV_SUBPEL_EIGHTH_PRECISION;
  else if ((mv.row & 3) || (mv.col & 3)) precision = MV_SUBPEL_QTR_PRECISION;
  else if ((mv.row & 7) || (mv.col & 7)) precision = MV_SUBPEL_HALF_PRECISION;
  else precision = MV_SUBPEL_NONE;
