...
static void scale_params(WarpedMotionParams *gm, WarpedMotionParams *gm_ref,
  ...
  for (int param_index = 0; param_index < n_params; param_index++) {
    ...
    //TODO
    param_value += (int32_t)(param_value * scale + scale_sign * 0.5);
...
static INLINE int get_gm_ref(WarpedMotionParams *gm_refs, WarpedMotionParams gm,
                             ...
                             int cur_ref_dist) {
  if (error_resilient || cur_ref_dist == INT_MAX) {
