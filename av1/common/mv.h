...
#define COMPANDED_INTMV_THRESH_QTR 16
#define COMPANDED_INTMV_THRESH_HALF 64
...
#if CONFIG_COMPANDED_MV
static INLINE MvSubpelPrecision get_companded_mv_precision(int diff) {
  const int sign = diff < 0;
  const int mag = sign ? -diff : diff;
  const int magint = ((mag - 1) >> 3);
#if CONFIG_FLEX_MVRES
  if (magint > COMPANDED_INTMV_THRESH_HALF) return MV_SUBPEL_HALF_PRECISION;
#endif  // CONFIG_FLEX_MVRES
  if (magint > COMPANDED_INTMV_THRESH_QTR) return MV_SUBPEL_QTR_PRECISION;
  return MV_SUBPEL_EIGHTH_PRECISION;
}
#endif  // CONFIG_COMPANDED_MV
