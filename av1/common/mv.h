...
static INLINE int_mv gm_get_motion_vector(const WarpedMotionParams *gm,
                                          int allow_hp, int width, int height) {
  int_mv res;
  ...
  xc = mat[2] * (width / 2) + mat[3] * (height / 2) + mat[0];
  yc = mat[4] * (width / 2) + mat[5] * (height / 2) + mat[1];

  int tx = (ROUND_POWER_OF_TWO_SIGNED(xc, shift) << scale) - ((width / 2) << 3);
  int ty =
      (ROUND_POWER_OF_TWO_SIGNED(yc, shift) << scale) - ((height / 2) << 3);
