...
void av1_update_boundary_info(const struct AV1Common *cm,
  ...
  for (row = mi_row; row < (mi_row + cm->mib_size); row++)
    for (col = mi_col; col < (mi_col + cm->mib_size); col++) {
      ...
      if (row == 0) mi->mbmi.boundary_info |= FRAME_ABOVE_BOUNDARY;
      if (col == 0) mi->mbmi.boundary_info |= FRAME_LEFT_BOUNDARY;
      if ((row + 1) >= cm->mi_rows)
        mi->mbmi.boundary_info |= FRAME_BOTTOM_BOUNDARY;
      if ((col + 1) >= cm->mi_cols)
        mi->mbmi.boundary_info |= FRAME_RIGHT_BOUNDARY;
    }
}
