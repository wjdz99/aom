...
void av1_setup_across_tile_boundary_info(const AV1_COMMON *const cm,
  ...
  if ((cm->tile_cols * cm->tile_rows > 1) && (!lpf_across_tiles_enabled)) {
    MODE_INFO *const mi =
        cm->mi + r * cm->tile_height * cm->mi_stride + c * cm->tile_width;
    MODE_INFO *mi_start;
    MODE_INFO *mi_end;
