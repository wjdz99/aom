...
int get_tile_size(int mi_frame_size, int log2_tile_num, int *ntiles) {
  ...
  // superblock size. To do this, shift right (and round up) to get the tile
  // size in max super-blocks and then shift left again to convert it to mi
  // units.
  const int max_sb_tile_size =
      ALIGN_POWER_OF_TWO(mi_frame_size, log2_tile_num + MAX_MIB_SIZE_LOG2);
  const int mi_tile_size = max_sb_tile_size << MAX_MIB_SIZE_LOG2;

  // The number of tiles is the ceiling of the frame size in mi units divided
  // by mi_size. This is at most 1 << log2_tile_num but might be strictly less
  // if max_sb_tile_size got rounded up significantly.
  if (ntiles) {
