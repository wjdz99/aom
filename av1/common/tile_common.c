...
void av1_setup_across_tile_boundary_info(const AV1_COMMON *const cm,
  ...
  if (cm->tile_cols * cm->tile_rows > 1) {
    ...
    assert((uintptr_t)mi_start < (uintptr_t)cm->mip + cm->mi_alloc_size);
    MODE_INFO *mi = 0;
    ...
    // Note:
    // This assert and above are used to guarantee MODE_INFO array accessing
    // is strictly inside allocated space.
    assert((uintptr_t) &
           (mi + col_diff - 1)->mbmi.boundary_info <
               (uintptr_t)cm->mip + cm->mi_alloc_size);
    for (col = 0; col < col_diff; ++col) {
