...
// If horizontal dependent tile is enabled, then the horizontal tile boundary
// is not treated as real tile boundary for loop filtering, only the horizontal
// tile group boundary is treated as tile boundary.
// Otherwise, tile group boundary is treated the same as tile boundary.Loop
// filtering operation is done based on the loopfilter_across_tiles_enabed
// flag for both tile boundary and tile group boundary.
