...
struct VP10Common;
void av1_default_coef_probs(struct VP10Common *cm);
void av1_adapt_coef_probs(struct VP10Common *cm);
...
extern const vpx_tree_index av1_coef_con_tree[TREE_SIZE(ENTROPY_TOKENS)];
extern const vpx_prob av1_pareto8_full[COEFF_PROB_MODELS][MODEL_NODES];

typedef vpx_prob av1_coeff_probs_model[REF_TYPES][COEF_BANDS][COEFF_CONTEXTS]
                                       [UNCONSTRAINED_NODES];

typedef unsigned int av1_coeff_count_model
    [REF_TYPES][COEF_BANDS][COEFF_CONTEXTS][UNCONSTRAINED_NODES + 1];

void av1_model_to_full_probs(const vpx_prob *model, vpx_prob *full);
