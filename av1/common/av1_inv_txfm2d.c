...
#if CONFIG_DATA_DRIVEN_TX
  // For DDTX, the stage_range argument is not required. Instead, we pass the
  // base qindex here.
  if (txfm_func_col == av1_iddt4 || txfm_func_col == av1_iddt8)
    stage_range_col[0] = base_qindex;
  if (txfm_func_row == av1_iddt4 || txfm_func_col == av1_iddt8)
    stage_range_row[0] = base_qindex;
#endif
...
#if CONFIG_DATA_DRIVEN_TX
  inv_txfm2d_add_c(input, output, stride, &cfg, txfm_buf, tx_size, base_qindex,
                   bd);
#else
...
#if CONFIG_DATA_DRIVEN_TX
  // for tx size >= 16, base_qindex is not really needed in inv_txfm2d_add_facade
  inv_txfm2d_add_facade(input, output, stride, txfm_buf, tx_type, TX_16X32, 0, bd);
#else
...
#if CONFIG_DATA_DRIVEN_TX
  inv_txfm2d_add_facade(input, output, stride, txfm_buf, tx_type, TX_32X16, 0, bd);
#else
...
#if CONFIG_DATA_DRIVEN_TX
  inv_txfm2d_add_facade(input, output, stride, txfm_buf, tx_type, TX_16X16, 0, bd);
#else
...
#if CONFIG_DATA_DRIVEN_TX
  inv_txfm2d_add_facade(input, output, stride, txfm_buf, tx_type, TX_32X32, 0,
                        bd);
#else
...
#if CONFIG_DATA_DRIVEN_TX
  inv_txfm2d_add_facade(mod_input, output, stride, txfm_buf, tx_type, TX_64X64, 0,
                        bd);
#else
...
#if CONFIG_DATA_DRIVEN_TX
  inv_txfm2d_add_facade(mod_input, output, stride, txfm_buf, tx_type, TX_64X32, 0,
                        bd);
#else
...
void av1_inv_txfm2d_add_32x64_c(const int32_t *input, uint16_t *output,
  ...
  DECLARE_ALIGNED(32, int, txfm_buf[64 * 32 + 64 + 64]);
#if CONFIG_DATA_DRIVEN_TX
  inv_txfm2d_add_facade(mod_input, output, stride, txfm_buf, tx_type, TX_32X64, 0,
                        bd);
#else
...
#if CONFIG_DATA_DRIVEN_TX
  inv_txfm2d_add_facade(mod_input, output, stride, txfm_buf, tx_type, TX_16X64, 0,
                        bd);
#else
...
void av1_inv_txfm2d_add_64x16_c(const int32_t *input, uint16_t *output,
  ...
  DECLARE_ALIGNED(32, int, txfm_buf[16 * 64 + 64 + 64]);
#if CONFIG_DATA_DRIVEN_TX
  inv_txfm2d_add_facade(mod_input, output, stride, txfm_buf, tx_type, TX_64X16, 0,
                        bd);
#else
