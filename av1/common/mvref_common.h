...
static INLINE int av1_nmv_ctx(const uint8_t ref_mv_count,
  ...
  if (ref_mv_stack[ref_mv_idx].weight >= REF_CAT_LEVEL &&
      ref_mv_count > 0) {
    if (abs(ref_mv_stack[ref_mv_idx].this_mv.as_mv.row -
            ref_mv_stack[ref_mv_idx].pred_mv.as_mv.row) <= 4 &&
        abs(ref_mv_stack[ref_mv_idx].this_mv.as_mv.col -
            ref_mv_stack[ref_mv_idx].pred_mv.as_mv.col) <= 4)
      return 2;
    else
      return 1;
  }
