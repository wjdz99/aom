...
static INLINE int av1_is_dv_valid(const MV dv, const TileInfo *const tile,
  ...
  const int active_row = mi_row * MI_SIZE + bh;
  if ((src_bottom_edge >> 3) >= active_row - INTRABC_ROW_DELAY) return 0;

  // The last 8 rows of the above superblock is invalid
  const int above_sb_row_bottom = mi_row * MI_SIZE;
  if ((src_bottom_edge >> 3) >= above_sb_row_bottom - INTRABC_ROW_DELAY &&
