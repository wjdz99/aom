...
static INLINE void av1_collect_neighbors_ref_counts_nrs(const AV1_COMMON *const cm,
  ...
  if (above_in_image && is_inter_block(above_mbmi)) {
    ...
    assert(convert_named_ref_to_named_ref_index(&cm->new_ref_frame_data,
                 above_mbmi->ref_frame[0]) == above_mbmi->ref_frame_nrs[0]);
    ref_counts[above_mbmi->ref_frame_nrs[0]]++;
    if (has_second_ref(above_mbmi)) {
      assert(convert_named_ref_to_named_ref_index(&cm->new_ref_frame_data,
                 above_mbmi->ref_frame[1]) == above_mbmi->ref_frame_nrs[1]);
  ...
  if (left_in_image && is_inter_block(left_mbmi)) {
    assert(convert_named_ref_to_named_ref_index(&cm->new_ref_frame_data,
                 left_mbmi->ref_frame[0]) == left_mbmi->ref_frame_nrs[0]);
    ref_counts[left_mbmi->ref_frame_nrs[0]]++;
    if (has_second_ref(left_mbmi)) {
      assert(convert_named_ref_to_named_ref_index(&cm->new_ref_frame_data,
                 left_mbmi->ref_frame[1]) == left_mbmi->ref_frame_nrs[1]);
