...
static INLINE int get_comp_index_context(const AV1_COMMON *const cm,
  ...
  const RefCntBuffer *const bck_buf =
      get_ref_frame_buf_const(cm, mbmi->ref_frame[0]);
  const RefCntBuffer *const fwd_buf =
      get_ref_frame_buf_const(cm, mbmi->ref_frame[1]);
