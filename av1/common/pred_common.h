...
static INLINE int av1_get_pred_context_seg_id(const MACROBLOCKD *xd,
  ...
  const int left_sip =
      (left_mi != NULL) ? left_mi->mbmi.seg_id_predicted[seg_cat_idx] : 0;
...
#if CONFIG_EXT_SEGMENT
static INLINE int av1_get_spatial_pred_context(const MACROBLOCKD *xd,
                                               SEG_CATEGORIES seg_cat_idx) {
  const MODE_INFO *const above_mi = xd->above_mi;
  const MODE_INFO *const left_mi = xd->left_mi;
  return (above_mi != NULL) && (left_mi != NULL) &&
         (above_mi->mbmi.segment_id[seg_cat_idx] ==
          left_mi->mbmi.segment_id[seg_cat_idx]);
}
static INLINE aom_prob
av1_get_pred_prob_seg_id(const struct segmentation_probs *segp,
                         const MACROBLOCKD *xd, SEG_CATEGORIES seg_cat_idx) {
