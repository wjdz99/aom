...
static INLINE int av1_get_pb_mv_precision_down_context(const AV1_COMMON *cm,
  ...
  const int above_down =
      above_mi ? above_mi->max_mv_precision - above_mi->pb_mv_precision : 0;
  const int left_down =
      left_mi ? left_mi->max_mv_precision - left_mi->pb_mv_precision : 0;
  assert(above_down >= 0);
  assert(left_down >= 0);
...
static INLINE aom_cdf_prob *av1_get_motion_mode_cdf(
    const MACROBLOCKD *xd, ALLOWED_MOTION_MODE_SET motion_mode_set,
    BLOCK_SIZE bsize, int *num_modes) {
  aom_cdf_prob *cdf;
  switch (motion_mode_set) {
    ...
    case ALLOW_OBMC_WARPED_CAUSAL:
      ...
      *num_modes = 3;
      break;
    default:
      assert(0 && "invalid motion mode set");
  }
  return cdf;
}
