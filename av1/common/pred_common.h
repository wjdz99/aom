...
static INLINE int get_comp_index_context(const RefBuffer *const frame_refs,
                                         const RefCntBuffer *const cur_frame,
                                         const BufferPool *const buffer_pool,
                                         const SequenceHeader *const seq_params,
                                         const MACROBLOCKD *xd) {
  MB_MODE_INFO *mbmi = xd->mi[0];
  int bck_idx = frame_refs[mbmi->ref_frame[0] - LAST_FRAME].idx;
  int fwd_idx = frame_refs[mbmi->ref_frame[1] - LAST_FRAME].idx;
  int bck_frame_index = 0, fwd_frame_index = 0;
  int cur_frame_index = cur_frame->cur_frame_offset;

  if (bck_idx >= 0)
    bck_frame_index = buffer_pool->frame_bufs[bck_idx].cur_frame_offset;

  if (fwd_idx >= 0)
    fwd_frame_index = buffer_pool->frame_bufs[fwd_idx].cur_frame_offset;
  int fwd = abs(get_relative_dist(&seq_params->order_hint_info, fwd_frame_index,
                                  cur_frame_index));
  int bck = abs(get_relative_dist(&seq_params->order_hint_info, cur_frame_index,
                                  bck_frame_index));
