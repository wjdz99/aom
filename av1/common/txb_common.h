...
static INLINE int get_nz_count(const tran_low_t *tcoeffs, int bwl,
                                  int height, int row, int col,
                                  TX_CLASS tx_class) {
  int count = 0;
  for (int idx = 0; idx < SIG_REF_OFFSET_NUM; ++idx) {
    const int ref_row = row + ( (tx_class == TX_CLASS_2D)?       sig_ref_offset[idx][0] : 
                                ( (tx_class == TX_CLASS_VERT)? sig_ref_offset_vert[idx][0] : 
                                                               sig_ref_offset_horiz[idx][0] ) );
    const int ref_col = col + ( (tx_class == TX_CLASS_2D)?       sig_ref_offset[idx][1] : 
