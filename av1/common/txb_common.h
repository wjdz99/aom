...
#define BASE_CONTEXT_MEG_POSITION_NUM 5
static const int base_meg_ref_offset[BASE_CONTEXT_MEG_POSITION_NUM][2] = {
  { 0, 1 }, { 0, 2 }, { 1, 0 }, { 1, 1 }, { 2, 0 }
};
...
static INLINE int get_level_mag(const uint8_t *const levels, const int bwl,
...
static INLINE int get_base_ctx(const uint8_t *const levels,
  ...
  const int count =
      get_level_count(levels, bwl, height, row, col, level_minus_1,
                      base_ref_offset, BASE_CONTEXT_POSITION_NUM);
  const int mag =
      get_level_mag(levels, bwl, height, row, col, base_meg_ref_offset,
                    BASE_CONTEXT_MEG_POSITION_NUM);
...
#define BR_CONTEXT_MEG_POSITION_NUM 3  // Base range coefficient context
...
static const int br_meg_ref_offset[BR_CONTEXT_MEG_POSITION_NUM][2] = {
  { 0, 1 }, { 1, 0 }, { 1, 1 }
};
...
static INLINE int get_br_ctx(const uint8_t *const levels,
  ...
  const int mag = get_level_mag(levels, bwl, height, row, col,
                                br_meg_ref_offset, BR_CONTEXT_MEG_POSITION_NUM);
