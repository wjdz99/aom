...
static INLINE int is_second_last_rtx(TX_SIZE tx_size, int proc_id) {
  int second_last = 0;
  if (tx_size_wide[tx_size] > tx_size_high[tx_size])
    if (proc_id == 0) second_last = 1;
  if (tx_size_wide[tx_size] < tx_size_high[tx_size])
    if (proc_id == 0) second_last = 1;
  if (tx_size_wide[tx_size] == tx_size_high[tx_size])
    if (proc_id == 2) second_last = 1;
