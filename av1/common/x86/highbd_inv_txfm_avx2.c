...
static void highbd_inv_txfm_add_32x32_avx2(const tran_low_t *input,
                                           uint8_t *dest, int stride,
                                           const TxfmParam *txfm_param) {
  const int bd = txfm_param->bd;
  const TX_TYPE tx_type = txfm_param->tx_type;
  const int32_t *src = av1_cast_to_int32(input);
  switch (tx_type) {
    case DCT_DCT:
...
void av1_highbd_inv_txfm_add_avx2(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_size) {
    case TX_32X32:
      highbd_inv_txfm_add_32x32_avx2(input, dest, stride, txfm_param);
      break;
    case TX_16X16:
