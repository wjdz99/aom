...
void av1_convolve_2d_avx2(const uint8_t *src, int src_stride,
  ...
  /* Horizontal filter */
  {
    ...
    for (i = 0; i < im_h; ++i) {
      for (j = 0; j < w; j += 16) {
        const __m256i data =
            _mm256_permute4x64_epi64(_mm256_loadu_si256((__m256i *)&src_ptr[i * src_stride + j]),_MM_SHUFFLE(2,1,1,0));

        // Filter even-index pixels
        const __m256i src_0 = _mm256_unpacklo_epi8(data, zero);
        const __m256i res_0 = _mm256_madd_epi16(src_0, coeff_01);
        const __m256i src_2 = _mm256_unpacklo_epi8(_mm256_srli_si256(data, 2), zero);
        const __m256i res_2 = _mm256_madd_epi16(src_2, coeff_23);
        const __m256i src_4 = _mm256_unpacklo_epi8(_mm256_srli_si256(data, 4), zero);
        const __m256i res_4 = _mm256_madd_epi16(src_4, coeff_45);
        const __m256i src_6 = _mm256_unpacklo_epi8(_mm256_srli_si256(data, 6), zero);
        ...
        res_even =
            _mm256_sra_epi32(_mm256_add_epi32(res_even, round_const), round_shift);

        // Filter odd-index pixels
        const __m256i src_1 = _mm256_unpacklo_epi8(_mm256_srli_si256(data, 1), zero);
        const __m256i res_1 = _mm256_madd_epi16(src_1, coeff_01);
        const __m256i src_3 = _mm256_unpacklo_epi8(_mm256_srli_si256(data, 3), zero);
        const __m256i res_3 = _mm256_madd_epi16(src_3, coeff_23);
        const __m256i src_5 = _mm256_unpacklo_epi8(_mm256_srli_si256(data, 5), zero);
        const __m256i res_5 = _mm256_madd_epi16(src_5, coeff_45);
        const __m256i src_7 = _mm256_unpacklo_epi8(_mm256_srli_si256(data, 7), zero);
        ...
        res_odd =
            _mm256_sra_epi32(_mm256_add_epi32(res_odd, round_const), round_shift);

        // Pack in the column order 0, 2, 4, 6, 1, 3, 5, 7
        __m256i res = _mm256_packs_epi32(res_even, res_odd);
        res = _mm256_packus_epi16(res, res);
        _mm_storel_epi64((__m128i *)&im_block[i * im_stride + j], _mm256_extractf128_si256(res,0));
        _mm_storel_epi64((__m128i *)&im_block[i * im_stride + j + 8], _mm256_extractf128_si256(res,1));
      }
    }
  }
      ...
      for (j = 0; j < w; j += 16) {
        ...
        const __m256i src_01 = _mm256_unpacklo_epi8(
            _mm256_inserti128_si256(
               _mm256_castsi128_si256(_mm_loadl_epi64((__m128i *)(data + 0 * im_stride))),
                   _mm_loadl_epi64((__m128i *)(data + 0 * im_stride + 8)), 1),
            _mm256_inserti128_si256(
               _mm256_castsi128_si256(_mm_loadl_epi64((__m128i *)(data + 1 * im_stride))),
                   _mm_loadl_epi64((__m128i *)(data + 1 * im_stride + 8)), 1));
        const __m256i src_23 = _mm256_unpacklo_epi8(
            _mm256_inserti128_si256(
               _mm256_castsi128_si256(_mm_loadl_epi64((__m128i *)(data + 2 * im_stride))),
                   _mm_loadl_epi64((__m128i *)(data + 2 * im_stride + 8)), 1),
            _mm256_inserti128_si256(
               _mm256_castsi128_si256(_mm_loadl_epi64((__m128i *)(data + 3 * im_stride))),
                   _mm_loadl_epi64((__m128i *)(data + 3 * im_stride + 8)), 1));
        const __m256i src_45 = _mm256_unpacklo_epi8(
            _mm256_inserti128_si256(
               _mm256_castsi128_si256(_mm_loadl_epi64((__m128i *)(data + 4 * im_stride))),
                   _mm_loadl_epi64((__m128i *)(data + 4 * im_stride + 8)), 1),
            _mm256_inserti128_si256(
               _mm256_castsi128_si256(_mm_loadl_epi64((__m128i *)(data + 5 * im_stride))),
                   _mm_loadl_epi64((__m128i *)(data + 5 * im_stride + 8)), 1));
        const __m256i src_67 = _mm256_unpacklo_epi8(
            _mm256_inserti128_si256(
               _mm256_castsi128_si256(_mm_loadl_epi64((__m128i *)(data + 6 * im_stride))),
                   _mm_loadl_epi64((__m128i *)(data + 6 * im_stride + 8)), 1),
            _mm256_inserti128_si256(
               _mm256_castsi128_si256(_mm_loadl_epi64((__m128i *)(data + 7 * im_stride))),
        ...
        if (do_average) {
          _mm_storeu_si128(p + 0,
                           _mm_add_epi32(_mm_loadu_si128(p + 0), _mm256_extractf128_si256(res_lo_round,0)));
          _mm_storeu_si128(p + 1,
                           _mm_add_epi32(_mm_loadu_si128(p + 1), _mm256_extractf128_si256(res_hi_round,0)));
          _mm_storeu_si128(p + 2,
                           _mm_add_epi32(_mm_loadu_si128(p + 2), _mm256_extractf128_si256(res_lo_round,1)));
          _mm_storeu_si128(p + 3,
                           _mm_add_epi32(_mm_loadu_si128(p + 3), _mm256_extractf128_si256(res_hi_round,1)));
        } else {
