...
void av1_convolve_2d_sr_avx2(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  __m256i coeffs_h12[6] = {}, coeffs_v12[6] = {};
  ...
  if (filter_params_x->taps == 12) {
    horiz_tap = 12;
  } else if (!(filter_x[0] | filter_x[1] | filter_x[6] | filter_x[7])) {
    horiz_tap = 4;
  ...
  if (filter_params_y->taps == 12) {
    vert_tap = 12;
  } else if (!(filter_y[0] | filter_y[1] | filter_y[6] | filter_y[7])) {
    vert_tap = 4;
  ...
  if (horiz_tap == 6)
    ...
    prepare_coeffs_12taps(filter_params_x, subpel_x_qn, coeffs_h12);
  else
    prepare_coeffs_lowbd(filter_params_x, subpel_x_qn, coeffs_h);
  ...
  if (vert_tap == 6)
    ...
    prepare_coeffs_12taps(filter_params_y, subpel_y_qn, coeffs_v12);
  else
    prepare_coeffs(filter_params_y, subpel_y_qn, coeffs_v);
