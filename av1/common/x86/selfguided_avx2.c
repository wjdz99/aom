...
static __m256i scan_32(__m256i x) {
  ...
  const __int32_t s = _mm256_extract_epi32(x04, 3);
  const __m128i s01 = _mm_set1_epi32(s);
  const __m256i s02 = _mm256_set_m128i(s01, _mm_setzero_si128());
...
static __m128i round_for_shift128(unsigned shift) {
  ...
  for (int i = 0; i < height; ++i) {
    ...
    for (int j = 0; j < width; j += 16) {
      ...
      if (highbd) {
        ...
        // Note that packing into 16 bits damages the order of the bits, so we
        // use a permute function to correct this
        ...
        // Note that each pack damages the order of the bits, so we use a
        // permute function to correct this
