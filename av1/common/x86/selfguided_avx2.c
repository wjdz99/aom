...
static void *memset_zero_aligned_avx(void *dest, const __m256i *zero, size_t count) {
  unsigned int i = 0;
  for (i = 0; i < (count & 0xffffffe0); i += 32) {
    _mm256_store_si256((__m256i *)((int *)dest + i), *zero);
    _mm256_store_si256((__m256i *)((int *)dest + i + 8), *zero);
    _mm256_store_si256((__m256i *)((int *)dest + i + 16), *zero);
    _mm256_store_si256((__m256i *)((int *)dest + i + 24), *zero);
  }
...
static void integral_images(const uint8_t *src, int src_stride, int width,
  ...
  memset_zero_aligned_avx(A, &zero, (width + 1));
  memset_zero_aligned_avx(B, &zero, (width + 1));
