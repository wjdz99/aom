...
static INLINE __m256i loadu2_avx2(__m128i *addr_hi, __m128i *addr_lo) {
  __m256i __v256 = _mm256_castsi128_si256(_mm_loadu_si128(addr_lo));
  return _mm256_insertf128_si256(__v256, _mm_loadu_si128(addr_hi), 1);
}
...
void av1_jnt_convolve_2d_avx2(const uint8_t *src, int src_stride, uint8_t *dst0,
  ...
  for (j = 0; j < w; j += 8) {
    /* Horizontal filter */
    {
      ...
      for (i = 0; i < im_h; i += 2) {
        const __m256i data =
            loadu2_avx2((__m128i *)(src_h + src_stride), (__m128i *)src_h);
        src_h += (src_stride << 1);
