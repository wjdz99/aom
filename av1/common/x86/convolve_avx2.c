...
void av1_convolve_y_sr_avx2(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  if (vert_tap == 6)
    ...
    prepare_coeffs_lowbd(filter_params_y, subpel_y_qn, coeffs);
      ...
      prepare_coeffs_12tap_avx2(filter_params_y, subpel_y_qn, coeffs);
    }
  ...
  if (vert_tap == 4) {
    ...
    for (j = 0; j < w; j += 16) {
      ...
      right_shift_const =
            _mm256_set1_epi32((1 << FILTER_BITS) >> 1);
      ...
      for (j = 0; j < w; j += 8) {
        ...
        for (i = 0; i < h; i += 2) {
          ...
          const __m256i res_lo = convolve_12tap_avx2(s, coeffs);
          ...
          if (w - j > 4) {
            const __m256i res_hi = convolve_12tap_avx2(s + 6, coeffs);
            ...
            const __m128i res_1 = _mm256_extracti128_si256(res_8b_lo, 1);
            if (w - j > 2) {
              *(uint32_t *) &dst[i * dst_stride + j] = (uint32_t) _mm_cvtsi128_si32(res_0);
              *(uint32_t *) &dst[i * dst_stride + j + dst_stride] = (uint32_t) _mm_cvtsi128_si32(res_1);
            } else {
              *(uint16_t *) &dst[i * dst_stride + j] = (uint16_t)_mm_cvtsi128_si32(res_0);
              *(uint16_t *) &dst[i * dst_stride + j + dst_stride] = (uint16_t)_mm_cvtsi128_si32(res_1);
            }
          }
          s[0] = s[1];
          s[1] = s[2];
...
void av1_convolve_x_sr_avx2(const uint8_t *src, int src_stride, uint8_t *dst,
    ...
    if (horiz_tap == 6)
      ...
      prepare_coeffs_12tap_avx2(filter_params_x, subpel_x_qn, coeffs);
    }
    ...
    if (horiz_tap == 4) {
      ...
      if (w <= 4) {
        for(i=0; i< h; i += 2){
          ...
          if (w > 2) {
            // 00 01 02 03
            *(uint32_t *) &dst[i * dst_stride] = (uint32_t) _mm_cvtsi128_si32(res_0);
            // 10 11 12 13
            *(uint32_t *) &dst[i * dst_stride + dst_stride] = (uint32_t) _mm_cvtsi128_si32(res_1);
          } else {
            // 00 01
            *(uint16_t *) &dst[i * dst_stride] = (uint16_t) _mm_cvtsi128_si32(res_0);
            // 10 11
            *(uint16_t *) &dst[i * dst_stride + dst_stride] = (uint16_t) _mm_cvtsi128_si32(res_1);
          }
        }
      } else {
        for(i = 0; i < h; i++){
          for(int j = 0; j < w; j += 8){
            ...
            const __m128i res_1 = _mm256_extracti128_si256(res_8b_lo, 1);
            *(uint32_t *) &dst[i * dst_stride + j] = (uint32_t) _mm_cvtsi128_si32(res_0);
            *(uint32_t *) &dst[i * dst_stride + j + 4] = (uint32_t) _mm_cvtsi128_si32(res_1);
          }
        }
      }
  }
