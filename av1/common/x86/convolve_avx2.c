...
void av1_convolve_y_sr_avx2(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  if (h <= 4) {
    __m256i coeffs[2], s[6];
    __m128i d[7];
    const int fo_vert = 1;
    const uint8_t *const src_ptr = src - fo_vert * src_stride;
    prepare_coeffs_lowbd_4tap(filter_params_y, subpel_y_q4, coeffs);
    for (j = 0; j < w; j += 16) {
      ...
      for (i = 0; i < h; i += 2) {
        const __m256i res_lo = convolve_lowbd_4tap(s, coeffs);
        /* rounding code */
        // shift by F - 1
        ...
        if (w - j > 8) {
          const __m256i res_hi = convolve_lowbd_4tap(s + 3, coeffs);
...
void av1_convolve_x_sr_avx2(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  if (w <= 4) {
    ...
    __m256i filt[2], coeffs[2];
    filt[0] = _mm256_load_si256((__m256i const *)(filt_global_avx2));
    filt[1] = _mm256_load_si256((__m256i const *)(filt_global_avx2 + 32));

    prepare_coeffs_lowbd_4tap(filter_params_x, subpel_x_q4, coeffs);
    for (i = 0; i < h; i += 2) {
      ...
      __m256i res_16b = convolve_lowbd_x_4tap(data, coeffs, filt);
