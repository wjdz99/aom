...
void av1_convolve_y_avx2(const uint8_t *src, int src_stride, uint8_t *dst0,
  ...
  const __m256i coeffs_y =
      _mm256_insertf128_si256(_mm256_castsi128_si256(coeffs_y8), coeffs_y8, 1);
...
void av1_convolve_x_avx2(const uint8_t *src, int src_stride, uint8_t *dst0,
  ...
  for (i = 0; i < h; ++i) {
    for (j = 0; j < w; j += 16) {
      ...
      const __m256i res_even = _mm256_add_epi32(_mm256_add_epi32(res_0, res_4),
      ...
      const __m256i res_odd = _mm256_add_epi32(_mm256_add_epi32(res_1, res_5),
      ...
      const __m256i res_lo_round =
          _mm256_sra_epi32(_mm256_add_epi32(res_lo, round_const), round_shift);
      const __m256i res_hi_round =
          _mm256_sra_epi32(_mm256_add_epi32(res_hi, round_const), round_shift);
