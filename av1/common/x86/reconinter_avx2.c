...
void av1_build_compound_diffwtd_mask_avx2(uint8_t *mask,
  ...
  if (4 == w) {
    ...
    do {
      __m128i s0A = xx_loadl_64(src0);
      __m128i s0B = xx_loadl_64(src0 + stride0);
      __m128i s0C = xx_loadl_64(src0 + stride0 * 2);
      __m128i s0D = xx_loadl_64(src0 + stride0 * 3);
      ...
      __m128i s1A = xx_loadl_64(src1);
      __m128i s1B = xx_loadl_64(src1 + stride1);
      __m128i s1C = xx_loadl_64(src1 + stride1 * 2);
      __m128i s1D = xx_loadl_64(src1 + stride1 * 3);
...
static INLINE __m128i calc_mask_d16(const __m128i *data_src0,
                                    const __m128i *data_src1,
                                    const __m128i *round_const,
                                    const __m128i *mask_base_16,
                                    const __m128i *clip_diff, int round) {
...
static INLINE __m256i calc_mask_d16_avx2(const __m256i *data_src0,
  ...
  __m256i diff_clamp = _mm256_min_epi16(diff_mask, *clip_diff);
  return diff_clamp;
}
...
static INLINE __m128i calc_mask_d16_inv(const __m128i *data_src0,
                                        const __m128i *data_src1,
                                        const __m128i *round_const,
                                        const __m128i *mask_base_16,
                                        const __m128i *clip_diff, int round) {
  ...
  const __m128i diff_const_16 = _mm_sub_epi16(*clip_diff, diff_clamp);
  return diff_const_16;
}
...
static INLINE void build_compound_diffwtd_mask_d16_avx2(
  ...
  if (w == 4) {
    do {
      ...
      const __m128i s1AB = _mm_unpacklo_epi64(s1A, s1B);
      const __m128i s1CD = _mm_unpacklo_epi64(s1C, s1D);
