...
static void hfilter(const uint8_t *src, int src_stride, int32_t *dst, int w,
  ...
  const uint64_t coeff_mask8b =
      (uint64_t)0xffffffffffffffff >> ((12 - ntaps) * 16);
  ...
  const __m128i round_shift = _mm_set1_epi64x(round);
  ...
  for (int x = 0; x < w; ++x, x_qn += x_step_qn) {
    ...
    const __m128d coeff8d8d = _mm_load1_pd((const double *)(filter + 8));
    const __m128i coeffhi =
        (__m128i)_mm_and_pd((__m128d)hicoeff_mask, coeff8d8d);
    const __m128i zero = (__m128i)_mm_setzero_ps();
...
static void hfilter8(const uint8_t *src, int src_stride, int32_t *dst, int w,
  ...
  for (int x = 0; x < w; ++x, x_qn += x_step_qn) {
    ...
    for (y = 0; y <= h - 4; y += 4) {
      ...
      const __m128i data08 = (__m128i)_mm_load_sd((double *)src0);
      const __m128i data18 = (__m128i)_mm_load_sd((double *)src1);
      const __m128i data28 = (__m128i)_mm_load_sd((double *)src2);
      const __m128i data38 = (__m128i)_mm_load_sd((double *)src3);
...
static void vfilter(const int32_t *src, int src_stride, int32_t *dst,
  ...
  const __m128i round_add = _mm_set1_epi32(round_add32);
  const __m128i round_shift = _mm_set1_epi64x(conv_params->round_1);
  ...
  for (int y = 0; y < h; ++y, y_qn += y_step_qn) {
    ...
    const __m128d coeff8d16 = _mm_load_sd((const double *)(filter + 8));
    const __m128i coeffhi16 =
        (__m128i)_mm_and_pd((__m128d)hicoeff_mask, coeff8d16);
...
static void highbd_hfilter(const uint16_t *src, int src_stride, int32_t *dst,
  ...
  for (int x = 0; x < w; ++x, x_qn += x_step_qn) {
    ...
    for (y = 0; y <= h - 4; y += 4) {
      ...
      const __m128i data01hi =
          (__m128i)_mm_shuffle_pd((__m128d)data0hi, (__m128d)data1hi, 0);
      const __m128i data23hi =
          (__m128i)_mm_shuffle_pd((__m128d)data2hi, (__m128d)data3hi, 0);
