...
// hfilter8.
static void hfilter(const uint8_t *src, int src_stride,
                    int32_t *dst, int w, int h,
  ...
  const uint64_t coeff_mask8b = (uint64_t)0xffffffffffffffff >> ((12 - ntaps) * 16);
  const __m128i hicoeff_mask = _mm_set_epi64x(coeff_mask8b, coeff_mask8b);

  int32_t round_add32 = (1 << round) / 2 + (1 << (bd + FILTER_BITS - 1));
  const __m128i round_add = _mm_set1_epi32(round_add32);
  const __m128i round_shift = _mm_set1_epi32(round);
  ...
  for (int x = 0; x < w; ++x, x_qn += x_step_qn) {
    ...
    // "hi" coefficients are arranged as 89ab89ab. For smaller filters, they
    // are masked out with hicoeff_mask.
    const __m128i coefflo = _mm_loadu_si128((__m128i *)filter);
    const __m128i coeff8d8d = _mm_load1_pd((const double *)(filter + 8));
    const __m128i coeffhi = _mm_and_pd(hicoeff_mask, coeff8d8d);
    ...
    for (y = 0; y <= h - 4; y += 4) {
      ...
      const __m128i shifted = _mm_sra_epi32(_mm_add_epi32(conv, round_add), round_shift);
...
static void hfilter8(const uint8_t *src, int src_stride,
                    int32_t *dst, int w, int h,
                    int subpel_x_qn, int x_step_qn,
                    const InterpFilterParams *filter_params, int round) {
  const int bd = 8;
  ...
  for (int x = 0; x < w; ++x, x_qn += x_step_qn) {
    ...
    for (y = 0; y <= h - 4; y += 4) {
      ...
      const __m128i data08 = _mm_load_sd((double *)src0);
      const __m128i data18 = _mm_load_sd((double *)src1);
      const __m128i data28 = _mm_load_sd((double *)src2);
      const __m128i data38 = _mm_load_sd((double *)src3);
...
static __m128i convolve_32(const int32_t *src,
                           __m128i coeff03, __m128i coeff47, __m128i coeff8d) {
...
static __m128i convolve_32_8(const int32_t *src, __m128i coeff03, __m128i coeff47) {
  const __m128i data03 = _mm_loadu_si128((__m128i *)src);
  const __m128i data47 = _mm_loadu_si128((__m128i *)(src + 4));
  const __m128i conv03 = _mm_mullo_epi32(data03, coeff03);
  const __m128i conv47 = _mm_mullo_epi32(data47, coeff47);
...
// general version, supporting 10 and 12 tap filters. For 8-tap filters, use
// vfilter8.
static void vfilter(const int32_t *src, int src_stride,
  ...
  const uint64_t coeff_mask8b =
      (ntaps == 8) ? 0 : (uint64_t)0xffffffffffffffff >> ((12 - ntaps) * 16);
  const __m128i hicoeff_mask = _mm_set_epi64x(coeff_mask8b, coeff_mask8b);

  int32_t round_add32 = (1 << conv_params->round_1) / 2 + (1 << offset_bits);
  const __m128i round_add = _mm_set1_epi32(round_add32);
  const __m128i round_shift = _mm_set1_epi32(conv_params->round_1);
  ...
  for (int y = 0; y < h; ++y, y_qn += y_step_qn) {
    ...
    const __m128i coeff8d16 = _mm_load_sd((const double *)(filter + 8));
    const __m128i coeffhi16 = _mm_and_pd(hicoeff_mask, coeff8d16);
...
// av1_convolve_2d_scale_sse4_1. This version only supports 8 tap filters.
static void vfilter8(const int32_t *src, int src_stride,
                     int32_t *dst, int dst_stride, int w, int h,
                     int subpel_y_qn, int y_step_qn,
                     const InterpFilterParams *filter_params,
                     const ConvolveParams *conv_params) {
...
void av1_convolve_2d_scale_sse4_1(const uint8_t *src, int src_stride,
  ...
  // horizontal filter
  if (xtaps == 8)
    hfilter8(src - fo_vert * src_stride, src_stride, tmp, w, im_h,
             subpel_x_qn, x_step_qn, filter_params_x, conv_params->round_0);
  else
    hfilter(src - fo_vert * src_stride, src_stride, tmp, w, im_h,
            subpel_x_qn, x_step_qn, filter_params_x, conv_params->round_0);

  // vertical filter (input is transposed)
  if (ytaps == 8)
    vfilter8(tmp, im_h, dst, dst_stride, w, h, subpel_y_qn, y_step_qn, filter_params_y,
             conv_params);
  else
    vfilter(tmp, im_h, dst, dst_stride, w, h, subpel_y_qn, y_step_qn, filter_params_y,
            conv_params);
}
