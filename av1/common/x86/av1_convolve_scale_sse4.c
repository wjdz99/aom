...
static void hfilter8(const uint8_t *src, int src_stride, int32_t *dst, int w,
  ...
  for (int x = 0; x < w; ++x, x_qn += x_step_qn) {
    ...
    for (y = 0; y <= h - 4; y += 4) {
      ...
      const __m128i data08 = _mm_castpd_si128(_mm_load_sd((double *)src0));
      const __m128i data18 = _mm_castpd_si128(_mm_load_sd((double *)src1));
      const __m128i data28 = _mm_castpd_si128(_mm_load_sd((double *)src2));
      const __m128i data38 = _mm_castpd_si128(_mm_load_sd((double *)src3));
...
static __m128i mm_shuffle0_si128(__m128i a, __m128i b) {
  __m128d ad = _mm_castps_si128(a);
  __m128d bd = _mm_castps_si128(b);
