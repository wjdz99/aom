...
static INLINE void ext_highbd_warp_8_horizontal_filter(
    const uint16_t *ref, __m128i *tmp, int stride, int32_t ix4, int32_t iy4,
    int32_t offset_x, int p_height, int height, int i,
    const int offset_bits_horiz, const int reduce_bits_horiz) {
  int k;
...
static INLINE void ext_highbd_4_filter_src_pixels(
    const __m128i *src, const __m128i *src2, __m128i *tmp, __m128i *coeff,
    const int offset_bits_horiz, const int reduce_bits_horiz, int k) {
  const __m128i src_1 = *src;
  const __m128i src2_1 = *src2;
...
static INLINE void ext_highbd_warp_4_horizontal_filter(
    const uint16_t *ref, __m128i *tmp, int stride, int32_t ix4, int32_t iy4,
  ...
  for (k = -5; k < 6; ++k) {
    ...
    ext_highbd_4_filter_src_pixels(&src, &src2, tmp, coeff, offset_bits_horiz,
                                   reduce_bits_horiz, k);
  }
}

void ext_highbd_warp_affine_8_sse4_1(const int32_t *mat, const uint16_t *ref,
                                     int width, int height, int stride,
                                     uint16_t *pred, int p_col, int p_row,
                                     int p_width, int p_height, int p_stride,
    ...
    for (j = 0; j < p_width; j += bsize_warp) {
      ...
      if (ix4 <= -7) {
        ...
        for (k = -7; k < AOMMIN(8, p_height - i); ++k) {
          ...
          highbd_filter_src_pixels(&src_padded, &src2_padded, tmp, coeff,
                                   offset_bits_horiz, reduce_bits_horiz, k);
        }
      } else {
        ext_highbd_warp_8_horizontal_filter(
            ref, tmp, stride, ix4, iy4, offs_x, p_height, height, i,
            offset_bits_horiz, reduce_bits_horiz);
      }
...
void ext_highbd_warp_affine_4_sse4_1(const int32_t *mat, const uint16_t *ref,
                                     int width, int height, int stride,
                                     uint16_t *pred, int p_col, int p_row,
                                     int p_width, int p_height, int p_stride,
                                     int subsampling_x, int subsampling_y,
    ...
    for (j = 0; j < p_width; j += bsize_warp) {
      ...
      if (ix4 <= -5) {
        ...
        for (k = -5; k < 6; ++k) {
          ...
          ext_highbd_4_filter_src_pixels(&src_padded, &src2_padded, tmp, coeff,
                                         offset_bits_horiz, reduce_bits_horiz,
                                         k);
        }
      } else {
        ext_highbd_warp_4_horizontal_filter(ref, tmp, stride, ix4, iy4, offs_x,
...
void av1_ext_highbd_warp_affine_sse4_1(const int32_t *mat, const uint16_t *ref,
  ...
  if (bsize_warp == 4) {
    ext_highbd_warp_affine_4_sse4_1(
        mat, ref, width, height, stride, pred, p_col, p_row, p_width, p_height,
        p_stride, subsampling_x, subsampling_y, bd, conv_params, bsize_warp);
  } else {
    assert(bsize_warp == 8);
    ext_highbd_warp_affine_8_sse4_1(
        mat, ref, width, height, stride, pred, p_col, p_row, p_width, p_height,
        p_stride, subsampling_x, subsampling_y, bd, conv_params, bsize_warp);
  }
}
