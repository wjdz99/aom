...
static INLINE void iidentity8_new_sse2(const __m128i *input, __m128i *output,
                                       int8_t cos_bit) {
  (void)cos_bit;
...
static INLINE __m128i lowbd_get_recon_8x8_sse2(const __m128i pred, __m128i res,
  ...
  if (fliplr) {
    ...
    // with flipped readding, for shuffle is usually expensive for low-end CPUs
...
static const transform_1d_sse2 lowbd_txfm8_1d_arr[] = {
...
inv_txfm_func inv_txfm_func_ls[TX_SIZES_ALL] = {
  NULL,                               // 4x4 transform
...
void av1_inv_txfm_add_sse2(const tran_low_t *dqcoeff, uint8_t *dst, int stride,
  ...
  inv_txfm_func inv_func = inv_txfm_func_ls[tx_type];
  if (inv_func != NULL)
