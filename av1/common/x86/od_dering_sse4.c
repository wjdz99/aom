...
void copy_nxm_8bit_to_16bit_sse4_1(uint16_t *dst, int dstride,
  ...
  for (i = 0; i < m; i++) {
    for (j = 0; j < (n & ~0x7); j += 8) {
      ...
      row = _mm_unpacklo_epi8(row, _mm_setzero_si128());
...
void copy_nxm_16bit_to_16bit_sse4_1(uint16_t *dst, int dstride,
