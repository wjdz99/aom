...
/* Horizontal sum of 8 16-bit unsigned values.*/
int32_t hsum_epi16(__m128i a) {
  __m128i lo, hi;
  // sign = _mm_cmplt_epi16(a, _mm_setzero_si128());
  lo = _mm_unpacklo_epi16(a, _mm_setzero_si128());
  lo = _mm_hadd_epi32(lo, lo);
  lo = _mm_hadd_epi32(lo, lo);

  hi = _mm_unpackhi_epi16(a, _mm_setzero_si128());
  hi = _mm_hadd_epi32(hi, hi);
  hi = _mm_hadd_epi32(hi, hi);
  return _mm_cvtsi128_si32(lo) + _mm_cvtsi128_si32(hi);
}
...
int od_filter_dering_direction_4x4_sse4_1(int16_t *y, int ystride,
  ...
  for (i = 0; i < 4; i += 2) {
    ...
    p = _mm_add_epi16(_mm_add_epi16(p, p), _mm_add_epi16(p, p));
    ...
    total_abs += hsum_epi16(_mm_abs_epi16(res));
