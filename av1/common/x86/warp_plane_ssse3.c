    ...
    for (j = 0; j < p_width; j += 8) {
      ...
      for (k = -7; k < AOMMIN(8, p_height - i); ++k) {
        ...
        if (ix4 <= -7) {
          ...
          const __m128i res = _mm_adds_epi16(_mm_adds_epi16(res_02, res_46),
                                             _mm_adds_epi16(res_13, res_57));
          const __m128i res_round = _mm_srai_epi16(
              _mm_adds_epi16(res, round_const), HORSHEAR_REDUCE_PREC_BITS);
          tmp[k + 7] = res_round;
        }
      }
      ...
      for (k = -4; k < AOMMIN(4, p_height - i - 4); ++k) {
        ...
        // Filter even-index pixels
        const __m128i tmp_0 = filter[(sy + 0 * gamma) >> WARPEDDIFF_PREC_BITS];
        const __m128i tmp_2 = filter[(sy + 2 * gamma) >> WARPEDDIFF_PREC_BITS];
        const __m128i tmp_4 = filter[(sy + 4 * gamma) >> WARPEDDIFF_PREC_BITS];
        const __m128i tmp_6 = filter[(sy + 6 * gamma) >> WARPEDDIFF_PREC_BITS];
        ...
        const __m128i tmp_1 = filter[(sy + 1 * gamma) >> WARPEDDIFF_PREC_BITS];
        const __m128i tmp_3 = filter[(sy + 3 * gamma) >> WARPEDDIFF_PREC_BITS];
        const __m128i tmp_5 = filter[(sy + 5 * gamma) >> WARPEDDIFF_PREC_BITS];
        const __m128i tmp_7 = filter[(sy + 7 * gamma) >> WARPEDDIFF_PREC_BITS];
        ...
        const __m128i res_8bit = _mm_packus_epi16(res_16bit, res_16bit);

        // Store, blending with 'pred' if needed
        __m128i *const p = (__m128i *)&pred[(i + k + 4) * p_stride + j];

        if (ref_frm) {
          const __m128i orig = _mm_loadl_epi64(p);
