...
// 1. Replace the SSE2 code inside with SSSE3 of much simpler instructions;
// 2. Rename the function to av1_round_shift_16bit_ssse3() and place it to a
//    separate file e.g. in av1_txfm_ssse3.h;
// 3. Make according needed changes for the new file created in item 2.
static INLINE void round_shift_16bit(__m128i *in, int size, int bit) {
  if (bit < 0) {
    bit = -bit;
    __m128i rounding = _mm_set1_epi32(1 << (bit - 1));
    for (int i = 0; i < size; ++i) {
      __m128i in_lo = _mm_unpacklo_epi16(in[i], in[i]);
      __m128i in_hi = _mm_unpackhi_epi16(in[i], in[i]);
      in_lo = _mm_slli_epi32(in_lo, 16);
      in_hi = _mm_slli_epi32(in_hi, 16);
      in_lo = _mm_srai_epi32(in_lo, 16);
      in_hi = _mm_srai_epi32(in_hi, 16);
      in_lo = _mm_add_epi32(in_lo, rounding);
      in_hi = _mm_add_epi32(in_hi, rounding);
      in_lo = _mm_srai_epi32(in_lo, bit);
      in_hi = _mm_srai_epi32(in_hi, bit);
      in[i] = _mm_packs_epi32(in_lo, in_hi);
    }
  } else if (bit > 0) {
    for (int i = 0; i < size; ++i) {
