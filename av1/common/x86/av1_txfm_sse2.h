...
static INLINE void store_rect_16bit_to_32bit(__m128i a, int32_t *b) {
  const __m128i sqrt = _mm_set1_epi16(NewSqrt2);
  const __m128i round = _mm_set1_epi32(1 << (NewSqrt2Bits - 1));
  __m128i a_lo, a_hi;
  a_lo = _mm_unpacklo_epi16(a, _mm_setzero_si128());
  a_hi = _mm_unpackhi_epi16(a, _mm_setzero_si128());
  a_lo = _mm_madd_epi16(a_lo, sqrt);
  a_hi = _mm_madd_epi16(a_hi, sqrt);
  a_lo = _mm_add_epi32(a_lo, round);
  a_hi = _mm_add_epi32(a_hi, round);
