...
static INLINE void get_8_level_counts_sse2(const uint8_t *const levels,
  ...
  level[0] = _mm_loadh_epi64(ls[1] - 1 * stride - 1, level[0]);
  level[1] = _mm_loadh_epi64(ls[1] - 1 * stride + 0, level[1]);
  level[2] = _mm_loadh_epi64(ls[1] - 1 * stride + 1, level[2]);
  level[3] = _mm_loadh_epi64(ls[1] + 0 * stride - 1, level[3]);
  level[4] = _mm_loadh_epi64(ls[1] + 0 * stride + 0, level[4]);
  level[5] = _mm_loadh_epi64(ls[1] + 0 * stride + 1, level[5]);
  level[0] = _mm_cmpgt_epi8(level[0], level_minus_1);
  level[1] = _mm_cmpgt_epi8(level[1], level_minus_1);
  level[2] = _mm_cmpgt_epi8(level[2], level_minus_1);
  level[3] = _mm_cmpgt_epi8(level[3], level_minus_1);
  ...
  do {
    ...
    level[6] = _mm_loadh_epi64(ls[1] + 1 * stride - 1, level[6]);
    level[7] = _mm_loadh_epi64(ls[1] + 1 * stride + 0, level[7]);
    level[8] = _mm_loadh_epi64(ls[1] + 1 * stride + 1, level[8]);
