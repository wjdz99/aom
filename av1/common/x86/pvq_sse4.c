 ...
 * pvq_search_rdo_double_c implementation but is not bit accurate.
 *
...
double pvq_search_rdo_double_sse4_1(const int16_t *xcoeff, int n, int k,
  ...
  if (pulses_left) {
    ...
    float lambda = (float)(0.5f * sqrt(xx) * pvq_norm_lambda / (1e-30 + g2));
    float delta_rate = 3.f / n;
    __m128 count = _mm_set_ps(3, 2, 1, 0);
    for (i = 0; i < n; i += 4) {
      _mm_storeu_ps(&lambda_delta_rate[i], _mm_mul_ps(count, _mm_set_ps1(lambda * delta_rate)));
