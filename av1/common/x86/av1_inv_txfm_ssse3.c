...
void av1_highbd_iwht4x4_16_add_ssse3(const tran_low_t *input, uint8_t *dest8,
  ...
  op[0] = _mm_loadu_si128((const __m128i *)(input + 0));
  op[1] = _mm_loadu_si128((const __m128i *)(input + 4));
  op[2] = _mm_loadu_si128((const __m128i *)(input + 8));
  op[3] = _mm_loadu_si128((const __m128i *)(input + 12));
  ...
  for (i = 0; i < 2; ++i) {
    ...
    if (i == 0) transpose_32bit_4x4(op, op);
  }

  for (i = 0; i < 4; i++) {
    tran_low_t vals[4];
    _mm_storeu_si128((__m128i *)vals, op[i]);

    range_check_value(vals[0], bd + 1);
    range_check_value(vals[1], bd + 1);
    range_check_value(vals[2], bd + 1);
    range_check_value(vals[3], bd + 1);
  }

  // Load uint16_t.
  dst[0] = _mm_loadl_epi64((__m128i const *)(dest + 0 * stride));
  dst[1] = _mm_loadl_epi64((__m128i const *)(dest + 1 * stride));
  dst[2] = _mm_loadl_epi64((__m128i const *)(dest + 2 * stride));
  dst[3] = _mm_loadl_epi64((__m128i const *)(dest + 3 * stride));
  ...
  switch (bd) {
    ...
    case 10:
      ...
      break;
    case 12:
      // This should not be needed for valid bitstreams but just in case.
  ...
  // Convert to int16_t.
  dst[0] = _mm_packs_epi32(dst[0], zero);
  dst[1] = _mm_packs_epi32(dst[1], zero);
  dst[2] = _mm_packs_epi32(dst[2], zero);
  dst[3] = _mm_packs_epi32(dst[3], zero);
