...
static void selfguided_restoration_1(uint8_t *src, int width, int height,
  ...
  for (i = 0; i < height; ++i) {
    ...
    // Note: The _mm_bslli_si128 call sets up a register containing
    ...
    __m128i sum_ = _mm_add_epi32(_mm_bslli_si128(b1, 4),
                                 _mm_add_epi32(b1, _mm_alignr_epi8(b2, b1, 4)));
    __m128i sum_sq_ = _mm_add_epi32(
        _mm_bslli_si128(a1, 4), _mm_add_epi32(a1, _mm_alignr_epi8(a2, a1, 4)));
...
static void selfguided_restoration_2(uint8_t *src, int width, int height,
  ...
  for (i = 0; i < height; ++i) {
    ...
    __m128i sum_ = _mm_add_epi32(
        _mm_add_epi32(
            _mm_add_epi32(_mm_bslli_si128(b1, 8), _mm_bslli_si128(b1, 4)),
            _mm_add_epi32(b1, _mm_alignr_epi8(b2, b1, 4))),
        _mm_alignr_epi8(b2, b1, 8));
    __m128i sum_sq_ = _mm_add_epi32(
        _mm_add_epi32(
            _mm_add_epi32(_mm_bslli_si128(a1, 8), _mm_bslli_si128(a1, 4)),
...
static void selfguided_restoration_3(uint8_t *src, int width, int height,
  ...
  for (i = 0; i < height; ++i) {
    ...
    __m128i sum_ =
        _mm_add_epi32(_mm_add_epi32(_mm_add_epi32(_mm_bslli_si128(b1, 12),
                                                  _mm_bslli_si128(b1, 8)),
                                    _mm_add_epi32(_mm_bslli_si128(b1, 4), b1)),
                      _mm_add_epi32(_mm_add_epi32(_mm_alignr_epi8(b2, b1, 4),
                                                  _mm_alignr_epi8(b2, b1, 8)),
                                    _mm_alignr_epi8(b2, b1, 12)));
    __m128i sum_sq_ =
        _mm_add_epi32(_mm_add_epi32(_mm_add_epi32(_mm_bslli_si128(a1, 12),
                                                  _mm_bslli_si128(a1, 8)),
                                    _mm_add_epi32(_mm_bslli_si128(a1, 4), a1)),
...
void apply_selfguided_restoration_sse4_1(uint8_t *dat, int width, int height,
  ...
  for (i = 0; i < height; ++i) {
    ...
    for (j = 0; j < width; j += 8) {
      ...
      const __m128i u_1 = _mm_cvtepu16_epi32(_mm_bsrli_si128(src, 8));
