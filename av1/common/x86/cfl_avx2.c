...
static INLINE __m256i fill_sum_epi32(__m256i l0) {
  l0 = _mm256_hadd_epi32(l0, l0);
  l0 = _mm256_permute4x64_epi64(l0, _MM_SHUFFLE(3, 1, 2, 0));
  l0 = _mm256_hadd_epi32(l0, l0);
  return _mm256_hadd_epi32(l0, l0);
}
...
static INLINE void subtract_average_avx2(int16_t *pred_buf, int width,
  ...
  do {
    ...
    if (width == 32) {
      ...
      sum = _mm256_add_epi32(sum, _mm256_addl_epi16(l1));
    }
  } while ((row += step) < end);
