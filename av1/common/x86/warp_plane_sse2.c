      ...
      for (k = -7; k < 8; ++k) {
        ...
        if (ix4 <= -7) {
          ...
          // i) the odd-index pixels, and ii) the even-index pixels
          {
            ...
            __m128i src_b = _mm_unpacklo_epi8(_mm_bsrli_si128(src, 1), zero);
            __m128i res_b = _mm_madd_epi16(src_b, coeff_b);
            __m128i src_c = _mm_unpacklo_epi8(_mm_bsrli_si128(src, 2), zero);
            __m128i res_c = _mm_madd_epi16(src_c, coeff_c);
            __m128i src_d = _mm_unpacklo_epi8(_mm_bsrli_si128(src, 3), zero);
            __m128i res_d = _mm_madd_epi16(src_d, coeff_d);
            __m128i src_e = _mm_unpacklo_epi8(_mm_bsrli_si128(src, 4), zero);
            __m128i res_e = _mm_madd_epi16(src_e, coeff_e);
            __m128i src_f = _mm_unpacklo_epi8(_mm_bsrli_si128(src, 5), zero);
            __m128i res_f = _mm_madd_epi16(src_f, coeff_f);
            __m128i src_g = _mm_unpacklo_epi8(_mm_bsrli_si128(src, 6), zero);
            __m128i res_g = _mm_madd_epi16(src_g, coeff_g);
            __m128i src_h = _mm_unpacklo_epi8(_mm_bsrli_si128(src, 7), zero);
