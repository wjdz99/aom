    ...
    for (j = 0; j < p_width; j += 8) {
      ...
      for (k = -7; k < AOMMIN(8, p_height - i); ++k) {
        ...
        if (ix4 <= -7) {
          ...
          const __m128i tmp_0 = *(__m128i const *)av1_get_warped_filter(
              (sx + 0 * alpha) >> WARPEDDIFF_PREC_BITS);
          const __m128i tmp_2 = *(__m128i const *)av1_get_warped_filter(
              (sx + 2 * alpha) >> WARPEDDIFF_PREC_BITS);
          const __m128i tmp_4 = *(__m128i const *)av1_get_warped_filter(
              (sx + 4 * alpha) >> WARPEDDIFF_PREC_BITS);
          const __m128i tmp_6 = *(__m128i const *)av1_get_warped_filter(
          ...
          const __m128i tmp_1 = *(__m128i const *)av1_get_warped_filter(
              (sx + 1 * alpha) >> WARPEDDIFF_PREC_BITS);
          const __m128i tmp_3 = *(__m128i const *)av1_get_warped_filter(
              (sx + 3 * alpha) >> WARPEDDIFF_PREC_BITS);
          const __m128i tmp_5 = *(__m128i const *)av1_get_warped_filter(
              (sx + 5 * alpha) >> WARPEDDIFF_PREC_BITS);
          const __m128i tmp_7 = *(__m128i const *)av1_get_warped_filter(
