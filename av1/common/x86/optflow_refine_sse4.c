  ...
  if (bw < 16) {
...
#endif
        xx_storeu_128(out1 + j, temp1);
        xx_storeu_128(out2 + j, temp2);
      }
    }
  } else {
    ...
    for (int i = 0; i < bh; i++) {
      ...
      for (int j = 0; j < bw; j = j + 16) {
        const __m128i src_buf1 = xx_loadu_128(inp1 + j);
        const __m128i src_buf2 = xx_loadu_128(inp2 + j);
...
#endif
        xx_storeu_128(out1 + j, res1);
        xx_storeu_128(out1 + j + 8, res2);
        xx_storeu_128(out2 + j, res3);
        xx_storeu_128(out2 + j + 8, res4);
      }
    }
  }
#else
  ...
  if (bw < 16) {
    for (int i = 0; i < bh; i++) {
      ...
      for (int j = 0; j < bw; j = j + 8) {
        ...
        xx_storeu_128(out1 + j, _mm_cvtepu8_epi16(src_buf));
      }
    }
  } else {
    const __m128i zero = _mm_setzero_si128();
    for (int i = 0; i < bh; i++) {
      ...
      int16_t *out1 = dst1 + i * bw;
      for (int j = 0; j < bw; j = j + 16) {
        const __m128i src_buf = xx_loadu_128(inp1 + j);
        xx_storeu_128(out1 + j, _mm_unpacklo_epi8(src_buf, zero));
        xx_storeu_128(out1 + j + 8, _mm_unpackhi_epi8(src_buf, zero));
      }
    }
  }
#endif
