...
void av1_wiener_convolve_add_src_avx2(const uint8_t *src, ptrdiff_t src_stride,
  ...
  coeffs_h[0] = yy_set_m128i(coeffs_01_128, coeffs_01_128);
  // coeffs [ f3 f2 f3 f2 f3 f2 f3 f2 ][ f3 f2 f3 f2 f3 f2 f3 f2 ]
  coeffs_h[1] = yy_set_m128i(coeffs_23_128, coeffs_23_128);
  // coeffs [ f5 f4 f5 f4 f5 f4 f5 f4 ][ f5 f4 f5 f4 f5 f4 f5 f4 ]
  coeffs_h[2] = yy_set_m128i(coeffs_45_128, coeffs_45_128);
  // coeffs [ f7 f6 f7 f6 f7 f6 f7 f6 ][ f7 f6 f7 f6 f7 f6 f7 f6 ]
  coeffs_h[3] = yy_set_m128i(coeffs_67_128, coeffs_67_128);

  const __m256i round_const_h32 = _mm256_set1_epi32(
    ...
    for (i = 0; i < im_h; i += 2) {
      ...
      if (i + 1 < im_h)
        data = _mm256_inserti128_si256(
            ...
            1);
      __m256i result[2];
      convolve_lowbd_x_tmp(data, coeffs_h, filt, result);

      result[0] = _mm256_sra_epi32(_mm256_add_epi32(result[0], round_const_h32),
                                   round_shift_h);
      result[1] = _mm256_sra_epi32(_mm256_add_epi32(result[1], round_const_h32),
                                   round_shift_h);
      ...
      __m256i data_[2];
      data_[0] = _mm256_cvtepi16_epi32(_mm256_extracti128_si256(data_0, 0));
      data_[1] = _mm256_cvtepi16_epi32(_mm256_extracti128_si256(data_0, 1));

      result[0] = _mm256_add_epi32(result[0], data_[0]);
      result[1] = _mm256_add_epi32(result[1], data_[1]);

      __m256i tmp_res = _mm256_packs_epi32(result[0], result[1]);
      tmp_res = _mm256_permute4x64_epi64(tmp_res, 0xd8);
      const __m256i res_clamped =
          _mm256_min_epi16(_mm256_max_epi16(tmp_res, clamp_low), clamp_high);
