...
void av1_build_compound_seg_mask_sse4_1(uint8_t *mask, SEG_MASK_TYPE mask_type,
                                        const uint8_t *src0, int stride0,
                                        const uint8_t *src1, int stride1, int h,
                                        int w) {
  ...
  if (4 == w) {
    ...
    do {
      int j = 0;
      do {
        const __m128i s0 = _mm_lddqu_si128((__m128i const *)(src0 + j));
        const __m128i s1 = _mm_lddqu_si128((__m128i const *)(src1 + j));
