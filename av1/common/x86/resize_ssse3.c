...
void av1_resize_and_extend_frame_ssse3(const YV12_BUFFER_CONFIG *src,
  ...
  // 2:1 horiz scale has normative only for BILINEAR.
  // TODO(marpan): Add normative case for EIGHTTAP 2:1 horiz.
  if (!has_normative_scaler ||
      (filter != BILINEAR && 2 * dst->y_crop_width == src->y_crop_width &&
       2 * dst->y_crop_height == src->y_crop_height)) {
