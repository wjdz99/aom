...
void av1_highbd_warp_affine_ssse3(int32_t *mat, uint16_t *ref, int width, int height,
                          int stride, uint16_t *pred, int p_col, int p_row,
                          int p_width, int p_height, int p_stride,
                          int subsampling_x, int subsampling_y, int bd, int ref_frm,
                          int32_t alpha, int32_t beta, int32_t gamma,
                          int32_t delta) {
#if HORSHEAR_REDUCE_PREC_BITS >= 5
  __m128i tmp[15];
#else
#error "SSE2 highbd warp filter is not yet implemented for HORSHEAR_REDUCE_PREC_BITS < 5"
#endif
  int i, j, k;
    ...
    for (j = 0; j < p_width; j += 8) {
      ...
      for (k = -7; k < AOMMIN(8, p_height - i); ++k) {
        ...
        if (ix4 <= -7) {
          ...
          __m128i res_2 = _mm_madd_epi16(_mm_alignr_epi8(src2, src, 4), coeff_2);
          __m128i res_4 = _mm_madd_epi16(_mm_alignr_epi8(src2, src, 8), coeff_4);
          __m128i res_6 = _mm_madd_epi16(_mm_alignr_epi8(src2, src, 12), coeff_6);
          ...
          __m128i res_1 = _mm_madd_epi16(_mm_alignr_epi8(src2, src, 2), coeff_1);
          __m128i res_3 = _mm_madd_epi16(_mm_alignr_epi8(src2, src, 6), coeff_3);
          __m128i res_5 = _mm_madd_epi16(_mm_alignr_epi8(src2, src, 10), coeff_5);
          __m128i res_7 = _mm_madd_epi16(_mm_alignr_epi8(src2, src, 14), coeff_7);
