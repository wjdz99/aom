...
static INLINE void warp_affine_horizontal(
    ...
    const int64_t y4, const int i, int16x8_t tmp[], const uint8x16_t indx) {
  const int bd = 8;
...
static const uint8_t iota_arr[] = { 0, 1, 2,  3,  4,  5,  6,  7,
...
static INLINE void av1_warp_affine_common(
  ...
  const uint8x16_t indx = vld1q_u8(iota_arr);
