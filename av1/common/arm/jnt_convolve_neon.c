...
static INLINE void dist_wtd_convolve_x_dist_wtd_avg_neon(
  ...
  if (w == 4 || h == 4) {
    do {
      ...
      uint16x4_t s0 = vget_low_s16(vreinterpretq_s16_u16(vmovl_u8(t0)));
      uint16x4_t s1 = vget_low_s16(vreinterpretq_s16_u16(vmovl_u8(t1)));
      uint16x4_t s2 = vget_low_s16(vreinterpretq_s16_u16(vmovl_u8(t2)));
      uint16x4_t s3 = vget_low_s16(vreinterpretq_s16_u16(vmovl_u8(t3)));
      uint16x4_t s4 = vget_high_s16(vreinterpretq_s16_u16(vmovl_u8(t0)));
      uint16x4_t s5 = vget_high_s16(vreinterpretq_s16_u16(vmovl_u8(t1)));
      uint16x4_t s6 = vget_high_s16(vreinterpretq_s16_u16(vmovl_u8(t2)));
      ...
      do {
        ...
        uint16x4_t s7 = vget_low_s16(vreinterpretq_s16_u16(vmovl_u8(t0)));
        uint16x4_t s8 = vget_low_s16(vreinterpretq_s16_u16(vmovl_u8(t1)));
        uint16x4_t s9 = vget_high_s16(vreinterpretq_s16_u16(vmovl_u8(t0)));
        uint16x4_t s10 = vget_high_s16(vreinterpretq_s16_u16(vmovl_u8(t1)));
      ...
      do {
        ...
        uint16x4_t s8 = vget_low_s16(vreinterpretq_s16_u16(vmovl_u8(t0)));

        uint16x4_t s1 = vext_s16(s0, s4, 1);  // a1 a2 a3 a4
        uint16x4_t s2 = vext_s16(s0, s4, 2);  // a2 a3 a4 a5
        uint16x4_t s3 = vext_s16(s0, s4, 3);  // a3 a4 a5 a6
        uint16x4_t s5 = vext_s16(s4, s8, 1);  // a5 a6 a7 a8
        uint16x4_t s6 = vext_s16(s4, s8, 2);  // a6 a7 a8 a9
        uint16x4_t s7 = vext_s16(s4, s8, 3);  // a7 a8 a9 a10
    ...
    do {
      ...
      do {
        t0 = vld1_u8(s);  // a8 a9 a10 a11 a12 a13 a14 a15
        uint16x8_t s8 = vreinterpretq_s16_u16(vmovl_u8(t0));

        uint16x8_t s1 = vextq_s16(s0, s8, 1);  // a1 a2 a3 a4 a5 a6 a7 a8
        uint16x8_t s2 = vextq_s16(s0, s8, 2);  // a2 a3 a4 a5 a6 a7 a8 a9
        uint16x8_t s3 = vextq_s16(s0, s8, 3);  // a3 a4 a5 a6 a7 a8 a9 a10
        uint16x8_t s4 = vextq_s16(s0, s8, 4);  // a4 a5 a6 a7 a8 a9 a10 a11
        uint16x8_t s5 = vextq_s16(s0, s8, 5);  // a5 a6 a7 a8 a9 a10 a11 a12
        uint16x8_t s6 = vextq_s16(s0, s8, 6);  // a6 a7 a8 a9 a10 a11 a12 a13
        uint16x8_t s7 = vextq_s16(s0, s8, 7);  // a7 a8 a9 a10 a11 a12 a13 a14
  ...
  if (w == 4 || h == 4) {
    do {
      ...
      do {
        ...
        int16x4_t d0 = convolve8_4_x(s0, s1, s2, s3, s4, s5, s6, s7, x_filter,
                                     vget_low_s16(round_offset_vec));
