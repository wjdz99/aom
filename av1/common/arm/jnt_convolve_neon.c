...
static INLINE void compute_avg_8x4(
  ...
  if (use_jnt_comp_avg) {
    ...
    const int32x4_t round_bits_vec = vdupq_n_s32(-(int32_t)round_bits);
    const int32x4_t dist_prec_bit_vec = vdupq_n_s32(-DIST_PRECISION_BITS);
    ...
    sum4 = vreinterpretq_u32_s32(
        vshlq_s32(vreinterpretq_s32_u32(sum4), dist_prec_bit_vec));
    sum5 = vreinterpretq_u32_s32(
        vshlq_s32(vreinterpretq_s32_u32(sum5), dist_prec_bit_vec));
    ...
    sum6 = vreinterpretq_u32_s32(
        vshlq_s32(vreinterpretq_s32_u32(sum6), dist_prec_bit_vec));
    sum7 = vreinterpretq_u32_s32(
        vshlq_s32(vreinterpretq_s32_u32(sum7), dist_prec_bit_vec));
    ...
    *t3 = vqmovn_u16(vreinterpretq_u16_s16(f3));
    ...
    f0 = vhaddq_s16(vreinterpretq_s16_u16(res0), vreinterpretq_s16_u16(d0));
    f1 = vhaddq_s16(vreinterpretq_s16_u16(res1), vreinterpretq_s16_u16(d1));
    f2 = vhaddq_s16(vreinterpretq_s16_u16(res2), vreinterpretq_s16_u16(d2));
    f3 = vhaddq_s16(vreinterpretq_s16_u16(res3), vreinterpretq_s16_u16(d3));

    f0 = vsubq_s16(f0, sub_const_vec);
    f1 = vsubq_s16(f1, sub_const_vec);
    f2 = vsubq_s16(f2, sub_const_vec);
    f3 = vsubq_s16(f3, sub_const_vec);
    ...
    do {
      ...
      do {
        ...
        if (conv_params->do_average) {
          ...
          res4 = vld1_u16(d);
          res5 = vld1_u16(d + dst_stride);
          res6 = vld1_u16(d + 2 * dst_stride);
          res7 = vld1_u16(d + 3 * dst_stride);
          ...
          vst1_u16((d + 1 * dst_stride), vreinterpret_u16_s16(d1));
          vst1_u16((d + 2 * dst_stride), vreinterpret_u16_s16(d2));
          vst1_u16((d + 3 * dst_stride), vreinterpret_u16_s16(d3));
        }
    ...
    do {
      ...
      do {
        ...
        if (conv_params->do_average) {
          ...
          d_tmp += dst_stride;
          res9 = vld1q_u16(d_tmp);
          d_tmp += dst_stride;
          res10 = vld1q_u16(d_tmp);
          d_tmp += dst_stride;
          res11 = vld1q_u16(d_tmp);
          d_tmp += dst_stride;
...
void av1_jnt_convolve_y_neon(const uint8_t *src, int src_stride, uint8_t *dst8,
  ...
  const uint16_t bck_offset = conv_params->bck_offset;
  const int use_jnt_comp_avg = conv_params->use_jnt_comp_avg;
  const int temp = (conv_params->round_1 - 1 - bits);
  ...
  if ((w == 4) || (h == 4)) {
    ...
    const int16x4_t round_offset64 = vdup_n_s16(round_offset);
    const int16x4_t round_offset_vec = vdup_n_s16(round_offset);
    const int16x4_t modified_vec = vdup_n_s16(-temp);
    uint8x8_t zero = vdup_n_u8(0);
    const int16x4_t zero_tmp = vreinterpret_s16_u8(zero);
    do {
      s = src_ptr;
      d = dst_ptr;
      d_u8 = dst_u8_ptr;
      ...
      do {
        ...
        if (conv_params->do_average) {
          ...
          vst1_u16((d), vreinterpret_u16_s16(d0));
          d += dst_stride;
          vst1_u16((d), vreinterpret_u16_s16(d1));
          d += dst_stride;
          vst1_u16((d), vreinterpret_u16_s16(d2));
          d += dst_stride;
          vst1_u16((d), vreinterpret_u16_s16(d3));
          d += dst_stride;
        }
    ...
    uint16x8_t res8, res9, res10, res11;
    const int16x8_t round_offset_vec = vdupq_n_s16(round_offset);
    const int16x8_t modified_vec = vdupq_n_s16(-temp);
    const int16x4_t round_offset64 = vdup_n_s16(round_offset);
    const int16x8_t zero_tmp = vdupq_n_s16(0);
    ...
    do {
      ...
      do {
        ...
        res4 = vaddq_s16(res4, round_offset_vec);
        res5 = convolve8_8x8_s16(s5, s6, s7, s8, s9, s10, s11, s12,
                                 y_filter_tmp, zero_tmp, modified_vec);
        res5 = vaddq_s16(res5, round_offset_vec);
        res6 = convolve8_8x8_s16(s6, s7, s8, s9, s10, s11, s12, s13,
                                 y_filter_tmp, zero_tmp, modified_vec);
        res6 = vaddq_s16(res6, round_offset_vec);
