...
static INLINE void convolve_x_sr_4tap_neon(const uint8_t *src_ptr,
  ...
  if (w == 4) {
    do {
      uint8x8_t t01[4];
      t01[0] = load_unaligned_u8(src_ptr + 0, (int)src_stride);
      t01[1] = load_unaligned_u8(src_ptr + 1, (int)src_stride);
      t01[2] = load_unaligned_u8(src_ptr + 2, (int)src_stride);
      t01[3] = load_unaligned_u8(src_ptr + 3, (int)src_stride);
