...
static INLINE void warp_affine_horizontal_neon(
    const uint16_t *ref, int width, int height, int stride, int p_width,
    int16_t alpha, int16_t beta, int iy4, int sx4, int ix4, int16x8_t tmp[],
    int bd, uint16x8_t indx0, uint16x8_t indx1) {
  const int round0 = (bd == 12) ? ROUND0_BITS + 2 : ROUND0_BITS;
...
static const uint16_t iota_arr[] = { 0, 1, 2,  3,  4,  5,  6,  7,
...
void av1_highbd_warp_affine_neon(const int32_t *mat, const uint16_t *ref,
  ...
  const uint16x8_t indx0 = vld1q_u16(iota_arr);
  const uint16x8_t indx1 = vld1q_u16(iota_arr + 8);
  ...
  for (int i = 0; i < p_height; i += 8) {
    for (int j = 0; j < p_width; j += 8) {
      ...
      warp_affine_horizontal_neon(ref, width, height, stride, p_width, alpha,
                                  beta, iy4, sx4, ix4, tmp, bd, indx0, indx1);
