...
static INLINE void load_filters(int16x8_t out[], int ofs, int stride) {
  int ofs0 = ROUND_POWER_OF_TWO(ofs + stride * 0, WARPEDDIFF_PREC_BITS);
  int ofs1 = ROUND_POWER_OF_TWO(ofs + stride * 1, WARPEDDIFF_PREC_BITS);
  int ofs2 = ROUND_POWER_OF_TWO(ofs + stride * 2, WARPEDDIFF_PREC_BITS);
  int ofs3 = ROUND_POWER_OF_TWO(ofs + stride * 3, WARPEDDIFF_PREC_BITS);
  int ofs4 = ROUND_POWER_OF_TWO(ofs + stride * 4, WARPEDDIFF_PREC_BITS);
  int ofs5 = ROUND_POWER_OF_TWO(ofs + stride * 5, WARPEDDIFF_PREC_BITS);
  int ofs6 = ROUND_POWER_OF_TWO(ofs + stride * 6, WARPEDDIFF_PREC_BITS);
  int ofs7 = ROUND_POWER_OF_TWO(ofs + stride * 7, WARPEDDIFF_PREC_BITS);
...
static INLINE void warp_affine_horizontal_neon(const uint16_t *ref, int width,
  ...
  const int reduce_bits_horiz =
      round0 + AOMMAX(bd + FILTER_BITS - round0 - 14, 0);
  ...
  if (ix4 <= -7) {
    ...
    for (int k = -7; k < 8; ++k) {
      int iy = clamp_iy(iy4 + k, height);
      int32_t dup_val = (1 << (bd + FILTER_BITS - reduce_bits_horiz - 1)) +
  ...
  for (int k = -7; k < 8; ++k) {
    ...
    res0 = vrshlq_s32(res0, vdupq_n_s32(-reduce_bits_horiz));
    res1 = vrshlq_s32(res1, vdupq_n_s32(-reduce_bits_horiz));
...
static INLINE void warp_affine_vertical_step_neon(
  ...
  if (!is_compound) {
    const int reduce_bits_vert = 2 * FILTER_BITS - reduce_bits_horiz;
  ...
  const int reduce_bits_vert = round1;
  sums[0] = vrshlq_s32(sums[0], vdupq_n_s32(-reduce_bits_vert));
  sums[1] = vrshlq_s32(sums[1], vdupq_n_s32(-reduce_bits_vert));
