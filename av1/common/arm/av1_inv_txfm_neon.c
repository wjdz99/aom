...
static INLINE void lowbd_write_buffer_8xn_neon(int16x8_t *in, uint8_t *output,
                                               int stride, int flipud,
                                               const int height) {
  int j = flipud ? (height - 1) : 0;
  const int step = flipud ? -1 : 1;
  int16x8_t temp_output[32];
  for (int i = 0; i < height; ++i, j += step) {
    temp_output[i] = vreinterpretq_s16_u16(vmovl_u8(vld1_u8(output)));
    temp_output[i] = vaddq_s16(temp_output[i], in[j]);
    vst1_u8(output, vqmovun_s16(temp_output[i]));
    output += stride;
  }
}
...
static INLINE void lowbd_write_buffer_16xn_neon(int16x8_t *in, uint8_t *output,
                                                int stride, int flipud,
                                                int height) {
...
static INLINE void inv_butterfly_lane_0_1_neon(const int16x8_t in0,
                                               const int16x8_t in1,
                                               const int16x4_t c, int16x8_t *t0,
                                               int16x8_t *t1,
  ...
  s0[0] = vrshlq_s32(s0[0], bits);
  s0[1] = vrshlq_s32(s0[1], bits);
  s1[0] = vrshlq_s32(s1[0], bits);
  s1[1] = vrshlq_s32(s1[1], bits);

  *t0 = vcombine_s16(vqmovn_s32(s0[0]), vqmovn_s32(s0[1]));
  *t1 = vcombine_s16(vqmovn_s32(s1[0]), vqmovn_s32(s1[1]));
}

static INLINE void inv_butterfly_lane_1_0_neon(const int16x8_t in0,
                                               const int16x8_t in1,
                                               const int16x4_t c, int16x8_t *t0,
                                               int16x8_t *t1,
                                               const int32x4_t bits) {
...
static INLINE void inv_butterfly_lane_2_3_neon(const int16x8_t in0,
                                               const int16x8_t in1,
                                               const int16x4_t c, int16x8_t *t0,
                                               int16x8_t *t1,
                                               const int32x4_t bits) {
...
static INLINE void inv_butterfly_neon(const int16x8_t in0, int16_t coef1,
                                      int16_t coef2, int16x8_t *t0,
                                      int16x8_t *t1, const int32x4_t bits) {
  ...
  s0_l = vrshlq_s32(s0_l, bits);
  s0_h = vrshlq_s32(s0_h, bits);
  s1_l = vrshlq_s32(s1_l, bits);
  s1_h = vrshlq_s32(s1_h, bits);

  *t0 = vcombine_s16(vmovn_s32(s0_l), vmovn_s32(s0_h));
  *t1 = vcombine_s16(vmovn_s32(s1_l), vmovn_s32(s1_h));
}

static INLINE void inv_butterfly_lane_3_2_neon(const int16x8_t in0,
                                               const int16x8_t in1,
                                               const int16x4_t c, int16x8_t *t0,
                                               int16x8_t *t1,
                                               const int32x4_t bits) {
...
static INLINE void inv_half_butterfly_neon(int16x8_t *const x,
                                           const int16x4_t c,
                                           const int32x4_t bits) {
  ...
  t0[0] = vrshlq_s32(t0[0], bits);
  t0[1] = vrshlq_s32(t0[1], bits);
  t1[0] = vrshlq_s32(t1[0], bits);
  t1[1] = vrshlq_s32(t1[1], bits);

  x[0] = vcombine_s16(vqmovn_s32(t0[0]), vqmovn_s32(t0[1]));
  x[1] = vcombine_s16(vqmovn_s32(t1[0]), vqmovn_s32(t1[1]));
}
...
static INLINE void iadst8_new_neon(int16x8_t *const in, int16x8_t *out,
  ...
  (void)bit;
  const int32x4_t shift_bits = vdupq_n_s32((int32_t)(-cos_bit));
  ...
  // Stage 2
  inv_butterfly_lane_0_1_neon(x[0], x[1], c0, &s0, &s1, shift_bits);
  inv_butterfly_lane_2_3_neon(x[2], x[3], c0, &s2, &s3, shift_bits);
  inv_butterfly_lane_0_1_neon(x[4], x[5], c1, &s4, &s5, shift_bits);
  inv_butterfly_lane_2_3_neon(x[6], x[7], c1, &s6, &s7, shift_bits);
  ...
  s2 = x[2];
  s3 = x[3];
  inv_butterfly_lane_2_3_neon(x[4], x[5], c2, &s4, &s5, shift_bits);
  inv_butterfly_lane_3_2_neon(x[7], x[6], c2, &s7, &s6, shift_bits);
  ...
  // stage 6
  inv_half_butterfly_neon(x + 2, c2, shift_bits);
  inv_half_butterfly_neon(x + 6, c2, shift_bits);
...
static INLINE void iadst8_low1_new_neon(int16x8_t *const in, int16x8_t *out,
  ...
  inv_butterfly_neon(x[1], cospi[60], cospi[4], &s0, &s1, shift_bits);
...
static INLINE void idct8_new_neon(int16x8_t *in, int16x8_t *out, int8_t cos_bit,
  ...
  // stage 2
  inv_butterfly_lane_0_1_neon(in[1], in[7], c0, &step1[7], &step1[4],
                              shift_bits);
  inv_butterfly_lane_2_3_neon(in[5], in[3], c0, &step1[6], &step1[5],
                              shift_bits);

  // stage 3
  inv_butterfly_lane_0_1_neon(in[0], in[4], c2, &step2[0], &step2[1],
                              shift_bits);
  inv_butterfly_lane_2_3_neon(in[2], in[6], c2, &step2[3], &step2[2],
                              shift_bits);
  ...
  inv_butterfly_lane_0_1_neon(step2[6], step2[5], c2, &step1[6], &step1[5],
                              shift_bits);
...
static const transform_8x8_neon
    lowbd_txfm_all_1d_zeros_w8x8_arr[TX_SIZES][ITX_TYPES_1D][4] = {
      {
          { NULL, NULL, NULL, NULL },
          { NULL, NULL, NULL, NULL },
          { NULL, NULL, NULL, NULL },
...
static INLINE void lowbd_inv_txfm2d_add_idtx_neon(const int32_t *input,
  ...
  const transform_8x8_neon row_txfm =
      lowbd_txfm_all_1d_zeros_w8x8_arr[txw_idx][hitx_1d_tab[tx_type]]
                                      [fun_idx_x];
  const transform_8x8_neon col_txfm =
      lowbd_txfm_all_1d_zeros_w8x8_arr[txw_idx][vitx_1d_tab[tx_type]]
                                      [fun_idx_y];
  ...
  for (int i = 0; i < buf_size_nonzero_h_div8; i++) {
    ...
    row_txfm(&a[i * 16], &a[i * 16], cos_bit_row, 0);
    av1_round_shift_array_16_neon(&a[i * 16], txfm_size_col, -shift[0]);
  }
  for (int j = 0; j < buf_size_w_div8; ++j) {
    col_txfm(&a[j * 16], &a[j * 16], cos_bit_col, 0);
    av1_round_shift_array_16_neon(&a[j * 16], txfm_size_row, -shift[1]);
  }
  if (txfm_size_col >= 16) {
    for (int i = 0; i < (txfm_size_col >> 4); i++) {
      lowbd_write_buffer_16xn_neon(&a[i * txfm_size_row * 2], output + 16 * i,
                                   stride, 0, txfm_size_row);
    }
  } else if (txfm_size_col == 8) {
    lowbd_write_buffer_8xn_neon(a, output, stride, 0, txfm_size_row);
  }
}
...
static INLINE void lowbd_inv_txfm2d_add_v_identity_neon(
  ...
  for (int j = 0; j < buf_size_w_div8; ++j) {
    col_txfm(&b[j * 16], &b[j * 16], cos_bit_col, 0);
    av1_round_shift_array_16_neon(&b[j * 16], txfm_size_row, -shift[1]);
  }
  if (txfm_size_col >= 16) {
    for (int i = 0; i < (txfm_size_col >> 4); i++) {
      lowbd_write_buffer_16xn_neon(&b[i * txfm_size_row * 2], output + 16 * i,
                                   stride, 0, txfm_size_row);
    }
  } else if (txfm_size_col == 8) {
    lowbd_write_buffer_8xn_neon(b, output, stride, 0, txfm_size_row);
  }
}
...
static INLINE void lowbd_inv_txfm2d_add_h_identity_neon(
  ...
  if (txfm_size_col >= 16) {
    ...
    lowbd_write_buffer_8xn_neon(a, output, stride, ud_flip, txfm_size_row);
  }
}
...
static INLINE void lowbd_inv_txfm2d_add_no_identity_neon(
  ...
  if (txfm_size_col >= 16) {
    ...
    lowbd_write_buffer_8xn_neon(b, output, stride, ud_flip, txfm_size_row);
  }
}
