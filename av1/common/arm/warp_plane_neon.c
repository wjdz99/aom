...
static const uint8_t warp_pad_left_neon[14][16] = {
...
static const uint8_t warp_pad_right_neon[14][16] = {
      ...
      if (ix4 <= -7) {
        ...
        for (k = -7; k < AOMMIN(8, p_height - i); ++k) {
          ...
          if (out_of_boundary_left >= 0) {
            ...
            ld_mask = warp_pad_left_neon[out_of_boundary_left];
            vec_dup = vdupq_n_u8(*(src + limit));
            mask_val = vld1q_u8(ld_mask);
            src_1 = vbslq_u8(mask_val, vec_dup, src_1);
          }
          if (out_of_boundary_right >= 0) {
            limit = 15 - (out_of_boundary_right + 1);
            ld_mask = warp_pad_right_neon[out_of_boundary_right];
            vec_dup = vdupq_n_u8(*(src + limit));
            mask_val = vld1q_u8(ld_mask);
            src_1 = vbslq_u8(mask_val, vec_dup, src_1);
          }
