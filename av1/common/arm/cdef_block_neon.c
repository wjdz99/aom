...
int cdef_find_dir_neon(const uint16_t *img, int stride, int32_t *var,
...
#else
  uint32x4_t max_cost = vpmaxq_u32(cost07, cost07);
  max_cost = vpmaxq_u32(max_cost, max_cost);
  best_cost = vgetq_lane_u32(max_cost, 0);
#endif
  uint8x16x2_t costs = { vreinterpretq_u8_u32(vceqq_u32(max_cost, cost03)),
                         vreinterpretq_u8_u32(vceqq_u32(max_cost, cost47)) };
  // idx = { 28, 24, 20, 16, 12, 8, 4, 0 };
  uint8x8_t idx = vreinterpret_u8_u64(vcreate_u64(0x0004080c1014181c));
  ...
  best_dir = aom_builtin_clzll(a) >> 3;
