...
void cdef_copy_rect8_8bit_to_16bit_neon(uint16_t *dst, int dstride,
  ...
  do {
    ...
    if (width > 16) {
      ...
      // Use vectors that overlap with the previous ones for the last elements
      // of the row, to avoid having to do scalar operations. This works
      // provided the src and dst buffers do not overlap.
      const uint8_t *src_tail_ptr = src_ptr + (width - 16);
      uint16_t *dst_tail_ptr = dst_ptr + (width - 16);
      uint8x16_t row = vld1q_u8(src_tail_ptr);
      uint8x16x2_t row_u16 = { { row, vdupq_n_u8(0) } };

      vst2q_u8((uint8_t *)(dst_tail_ptr), row_u16);
    } else if (width > 8) {
      uint8x8_t row = vld1_u8(src_ptr);
      vst1q_u16(dst_ptr, vmovl_u8(row));

      // Use vectors that overlap with the previous ones for the last elements
      // of the row, to avoid having to do scalar operations. This works
      // provided the src and dst buffers do not overlap.
      const uint8_t *src_tail_ptr = src_ptr + (width - 8);
      uint16_t *dst_tail_ptr = dst_ptr + (width - 8);
      row = vld1_u8(src_tail_ptr);
      vst1q_u16(dst_tail_ptr, vmovl_u8(row));
    } else {
...
void cdef_copy_rect8_16bit_to_16bit_neon(uint16_t *dst, int dstride,
  ...
  do {
    ...
    if (width > 8) {
      ...
      // of the row, to avoid having to do scalar operations. This works
      // provided the src and dst buffers do not overlap.
      const uint16_t *src_tail_ptr = src_ptr + (width - 8);
      uint16_t *dst_tail_ptr = dst_ptr + (width - 8);
      uint16x8_t row = vld1q_u16(src_tail_ptr);

      vst1q_u16(dst_tail_ptr, row);
    } else if (width > 4) {
      ...
      // of the row, to avoid having to do scalar operations. This works
      // provided the src and dst buffers do not overlap.
      const uint16_t *src_tail_ptr = src_ptr + (width - 4);
      uint16_t *dst_tail_ptr = dst_ptr + (width - 4);
      row = vld1_u16(src_tail_ptr);

      vst1_u16(dst_tail_ptr, row);
    } else {
      for (int w = 0; w < width; w++) {
        dst_ptr[w] = src_ptr[w];
      }
