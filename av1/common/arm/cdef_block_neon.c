...
static INLINE void filter_block_4x4(const int is_lowbd, void *dest, int dstride,
  ...
  for (int i = 0; i < height; i += 2) {
    ...
    uint16x8_t s = load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE, CDEF_BSTRIDE);
    max = min = s;
    ...
    if (enable_primary) {
      ...
      // Primary near taps
      tap[0] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE + po1, CDEF_BSTRIDE);
      tap[1] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE - po1, CDEF_BSTRIDE);
      ...
      // Primary far taps
      tap[2] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE + po2, CDEF_BSTRIDE);
      tap[3] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE - po2, CDEF_BSTRIDE);
    ...
    if (enable_secondary) {
      ...
      // Secondary near taps
      tap[0] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE + s1o1, CDEF_BSTRIDE);
      tap[1] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE - s1o1, CDEF_BSTRIDE);
      tap[2] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE + s2o1, CDEF_BSTRIDE);
      tap[3] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE - s2o1, CDEF_BSTRIDE);
      ...
      // Secondary far taps
      tap[4] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE + s1o2, CDEF_BSTRIDE);
      tap[5] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE - s1o2, CDEF_BSTRIDE);
      tap[6] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE + s2o2, CDEF_BSTRIDE);
      tap[7] =
          load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE - s2o2, CDEF_BSTRIDE);
    ...
    if (is_lowbd) {
      ...
      store_unaligned_u8_4x2(dst8 + i * dstride, dstride, res_128);
    } else {
      store_unaligned_u16_4x2(dst16 + i * dstride, dstride,
...
static INLINE void filter_block_8x8(const int is_lowbd, void *dest, int dstride,
  ...
  for (int i = 0; i < height; i++) {
    int16x8_t sum = vdupq_n_s16(0);
    uint16x8_t s = vld1q_u16(in + i * CDEF_BSTRIDE);
    max = min = s;
    ...
    if (enable_primary) {
      ...
      // Primary near taps
      tap[0] = vld1q_u16(in + i * CDEF_BSTRIDE + po1);
      tap[1] = vld1q_u16(in + i * CDEF_BSTRIDE - po1);
      int16x8_t p0 = constrain16(tap[0], s, pri_strength, pri_damping);
      int16x8_t p1 = constrain16(tap[1], s, pri_strength, pri_damping);
      ...
      // Primary far taps
      tap[2] = vld1q_u16(in + i * CDEF_BSTRIDE + po2);
      p0 = constrain16(tap[2], s, pri_strength, pri_damping);
      tap[3] = vld1q_u16(in + i * CDEF_BSTRIDE - po2);
      p1 = constrain16(tap[3], s, pri_strength, pri_damping);
    ...
    if (enable_secondary) {
      ...
      // Secondary near taps
      tap[0] = vld1q_u16(in + i * CDEF_BSTRIDE + s1o1);
      tap[1] = vld1q_u16(in + i * CDEF_BSTRIDE - s1o1);
      tap[2] = vld1q_u16(in + i * CDEF_BSTRIDE + s2o1);
      tap[3] = vld1q_u16(in + i * CDEF_BSTRIDE - s2o1);
      int16x8_t p0 = constrain16(tap[0], s, sec_strength, sec_damping);
      int16x8_t p1 = constrain16(tap[1], s, sec_strength, sec_damping);
      int16x8_t p2 = constrain16(tap[2], s, sec_strength, sec_damping);
      int16x8_t p3 = constrain16(tap[3], s, sec_strength, sec_damping);
      ...
      // Secondary far taps
      tap[4] = vld1q_u16(in + i * CDEF_BSTRIDE + s1o2);
      tap[5] = vld1q_u16(in + i * CDEF_BSTRIDE - s1o2);
      tap[6] = vld1q_u16(in + i * CDEF_BSTRIDE + s2o2);
      tap[7] = vld1q_u16(in + i * CDEF_BSTRIDE - s2o2);
      p0 = constrain16(tap[4], s, sec_strength, sec_damping);
      p1 = constrain16(tap[5], s, sec_strength, sec_damping);
      p2 = constrain16(tap[6], s, sec_strength, sec_damping);
      p3 = constrain16(tap[7], s, sec_strength, sec_damping);
    ...
    if (is_lowbd) {
      const uint8x8_t res_128 = vqmovun_s16(res);
      vst1_u8(dst8 + i * dstride, res_128);
    } else {
      vst1q_u16(dst16 + i * dstride, vreinterpretq_u16_s16(res));
    }
  }
}
...
static INLINE void copy_block_4xh(const int is_lowbd, void *dest, int dstride,
  ...
  for (int i = 0; i < height; i += 2) {
    const uint16x8_t row =
        load_unaligned_u16_4x2(in + i * CDEF_BSTRIDE, CDEF_BSTRIDE);
    if (is_lowbd) {
      const uint8x8_t res_128 = vqmovn_u16(row);
      store_unaligned_u8_4x2(dst8 + i * dstride, dstride, res_128);
    } else {
      store_unaligned_u16_4x2(dst16 + i * dstride, dstride, row);
    }
  }
}
...
static INLINE void copy_block_8xh(const int is_lowbd, void *dest, int dstride,
  ...
  for (int i = 0; i < height; i++) {
    const uint16x8_t row = vld1q_u16(in + i * CDEF_BSTRIDE);
    if (is_lowbd) {
      const uint8x8_t res_128 = vqmovn_u16(row);
      vst1_u8(dst8 + i * dstride, res_128);
    } else {
      vst1q_u16(dst16 + i * dstride, row);
    }
  }
}
