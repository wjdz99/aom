...
static INLINE void vldsubstq_s16(int16_t *dst, const uint16_t *src, int offset,
  ...
  vst1q_s16(dst + offset,
            vsubq_s16(vreinterpretq_s16_u16(vld1q_s16(src + offset)), sub));
}
...
static void cfl_luma_subsampling_420_lbd_neon(const uint8_t *input,
  ...
  do {
    if (width == 4) {
      ...
      int16x8x2_t sum;
      sum.val[0] = vshlq_n_s16(vaddq_u16(top_0, bot_0), 1);
...
static void cfl_luma_subsampling_444_lbd_neon(const uint8_t *input,
  ...
  do {
    if (width == 4) {
      ...
      vst1q_s16(pred_buf_q3 + 8, vshll_n_u8(vget_high_u8(top), 3));
      if (width == 32) {
...
static void cfl_luma_subsampling_444_hbd_neon(const uint16_t *input,
  ...
  do {
    if (width == 4) {
      ...
      const int16x8_t top = vld1q_u16(input);
      vst1q_u16(pred_buf_q3, vshlq_n_u16(top, 3));
    } else if (width == 16) {
      const uint16x8x2_t top = vld2q_u16(input);
      uint16x8x2_t results;
      results.val[0] = vshlq_n_s16(top.val[0], 3);
      results.val[1] = vshlq_n_s16(top.val[1], 3);
      vst2q_u16(pred_buf_q3, results);
    } else {
      const uint16x8x4_t top = vld4q_u16(input);
      uint16x8x4_t results;
      results.val[0] = vshlq_n_u16(top.val[0], 3);
      results.val[1] = vshlq_n_u16(top.val[1], 3);
      results.val[2] = vshlq_n_u16(top.val[2], 3);
      results.val[3] = vshlq_n_u16(top.val[3], 3);
      vst4q_u16(pred_buf_q3, results);
    }
    input += input_stride;
  } while ((pred_buf_q3 += CFL_BUF_LINE) < end);
}
