...
static INLINE uint8x8_t vldh_dup_u8(const uint8_t *input) {
  return vreinterpret_u8_u32(vld1_dup_u32((const uint32_t *)input));
}
...
static void cfl_luma_subsampling_420_lbd_neon(const uint8_t *input,
  ...
  do {
    if (width == 4) {
      ...
      *((uint32_t *)pred_buf_q3) = vreinterpret_u32_u16(vshl_n_u16(sum, 1))[0];
      // vst1_lane_u32(pred_buf_q3, vreinterpret_u32_u16(vshl_n_u16(sum, 1)),
      // 0);
    } else if (width == 8) {
