...
static INLINE void predict_w4(const int16_t *pred_buf_q3, const int16_t *end,
                              uint8_t *dst, int dst_stride, int alpha_q3,
                              int abs_alpha_q12) {
  ...
  do {
    ...
    scaled_luma = vadd_s16(scaled_luma, dc);
...
static INLINE uint8x8_t predict_w8(const int16_t *pred_buf_q3,
  ...
  scaled_luma = vaddq_s16(scaled_luma, dc);
  return vqmovn_u16(vqshluq_n_s16(scaled_luma, 0));
}
...
static INLINE uint8x16_t predict_w16(const int16_t *pred_buf_q3,
                                     ...
                                     int16x8_t dc) {
  const int16x8_t ac_q3 = vld1q_s16(pred_buf_q3);
  const int16x8_t ac_next_q3 = vld1q_s16(pred_buf_q3 + 8);
  const int16x8_t ac_sign = vsignq_s16(alpha_sign, ac_q3);
  const int16x8_t ac_next_sign = vsignq_s16(alpha_sign, ac_next_q3);
  int16x8_t scaled_luma = vqrdmulhq_n_s16(vabsq_s16(ac_q3), abs_alpha_q12);
  int16x8_t scaled_luma_next =
      vqrdmulhq_n_s16(vabsq_s16(ac_next_q3), abs_alpha_q12);
  scaled_luma = vsignq_s16(scaled_luma, ac_sign);
  scaled_luma_next = vsignq_s16(scaled_luma_next, ac_next_sign);
  scaled_luma = vaddq_s16(scaled_luma, dc);
  scaled_luma_next = vaddq_s16(scaled_luma_next, dc);
  uint16x8_t uscaled = vqshluq_n_s16(scaled_luma, 0);
#if __ARM_ARCH >= 8
  return vqmovun_high_s16(vqmovn_u16(uscaled), scaled_luma_next);
#else
  uint16x8_t uscaled_next = vqshluq_n_s16(scaled_luma_next, 0);
...
static INLINE void cfl_predict_lbd_neon(const int16_t *pred_buf_q3,
  ...
  if (width == 4) {
    predict_w4(pred_buf_q3, end, dst, dst_stride, alpha_q3, abs_alpha_q12);
    return;
  }
  ...
  do {
    if (width == 8) {
      vst1_u8(dst, predict_w8(pred_buf_q3, alpha_sign, abs_alpha_q12, dc));
    } else {
      vst1q_u8(dst, predict_w16(pred_buf_q3, alpha_sign, abs_alpha_q12, dc));
      if (width == 32) {
        vst1q_u8(dst + 16,
                 predict_w16(pred_buf_q3 + 16, alpha_sign, abs_alpha_q12, dc));
      }
    }
    dst += dst_stride;
  } while ((pred_buf_q3 += CFL_BUF_LINE) < end);
