...
static void vsthun_s16(uint8_t *dst, int16x4_t scaled_luma) {
  const uint16x4_t uscaled = vqshlu_n_s16(scaled_luma, 0);
...
static void vst1un_s16(uint8_t *dst, int16x8_t scaled_luma) {
  ...
  vst1_u8(dst, vqmovn_u16(uscaled));
}
...
static void vst1unq_s16(uint8_t *dst, int16x8_t scaled_luma,
  ...
  const uint16x8_t uscaled = vqshluq_n_s16(scaled_luma, 0);
#if __ARM_ARCH >= 8
  vst1q_u8(dst, vqmovun_high_s16(vqmovn_u16(uscaled), scaled_luma_next));
#else
  const uint16x8_t uscaled_next = vqshluq_n_s16(scaled_luma_next, 0);
  vst1q_u8(dst, vcombine_u8(vqmovn_u16(uscaled), vqmovn_u16(uscaled_next)));
#endif
}
