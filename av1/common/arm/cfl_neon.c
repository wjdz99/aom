...
static void cfl_luma_subsampling_420_lbd_neon(const uint8_t *input,
  ...
  do {
    if (width == 4) {
      const uint16x4_t top =
          vpaddl_u8(vreinterpret_u8_u32(vld1_dup_u32((uint32_t *)input)));
      const uint16x4_t sum =
          vpadal_u8(top, vreinterpret_u8_u32(
                             vld1_dup_u32((uint32_t *)(input + input_stride))));
      vst1_lane_u32(
          pred_buf_q3,
          vreinterpret_u32_s16(vshl_n_s16(vreinterpret_s16_u16(sum), 1)), 0);
    } else if (width == 8) {
