...
static INLINE void subtract_average_neon(int16_t *pred_buf, int width,
  ...
  do {
    ...
    if (width == 4) {
      const uint16x8_t c0 =
          vcombine_u16(vld1_u16(sum_buf), vld1_u16(sum_buf + CFL_BUF_LINE));
      const uint16x8_t c1 = vcombine_u16(vld1_u16(sum_buf + 2 * CFL_BUF_LINE),
  ...
  int16x4_t avg_16x4 =
      vreinterpret_s16_u16(vqrshrn_n_u32(sum_32x4, num_pel_log2));
