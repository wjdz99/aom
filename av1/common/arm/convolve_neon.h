...
static INLINE uint8x8_t wiener_convolve8_vert_h4_w8(
    const int16x8_t s0, const int16x8_t s1, const int16x8_t s2,
    const int16x8_t s3, const int16x8_t s4, const int16x8_t s5,
    const int16x8_t s6, const int16_t *filter_y, const int bd,
  ...
  int32x4_t round_bits = vdupq_n_s32(-round1_bits);
  ...
  sum0 =
      vmlal_n_s16(sum0, vget_low_s16(s3), (filter_y[3] + (1 << FILTER_BITS)));
  ...
  sum1 =
      vmlal_n_s16(sum1, vget_high_s16(s3), (filter_y[3] + (1 << FILTER_BITS)));
...
static INLINE uint16x8_t wiener_convolve8_horiz_h8_w8(
    const int16x8_t s0, const int16x8_t s1, const int16x8_t s2,
    const int16x8_t s3, const int16_t *filter_x, const int bd,
  ...
  int32x4_t round_bits = vdupq_n_s32(-round0_bits);
  ...
  s3_0 = vmull_n_s16(vget_low_s16(s3), (filter_x[3] + (1 << FILTER_BITS)));
  s3_1 = vmull_n_s16(vget_high_s16(s3), (filter_x[3] + (1 << FILTER_BITS)));
...
static INLINE uint16x4_t wiener_convolve8_horiz_h4_w8(
    const int16x4_t s0, const int16x4_t s1, const int16x4_t s2,
    const int16x4_t s3, const int16x4_t s4, const int16x4_t s5,
    const int16x4_t s6, const int16_t *filter_x, const int bd,
    const int round0_bits) {
  ...
  /*  s[3]*128 -- and filter coff max can be 128.
     ...
     hold the result. */
  s3_0 = vmull_n_s16(s3, (filter_x[3] + (1 << FILTER_BITS)));
  sum_0 = vaddq_s32(sum_0, s3_0);
