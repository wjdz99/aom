...
static INLINE void highbd_convolve_2d_y_sr_8tap_neon(
  ...
  if (w <= 4) {
    ...
    do {
      ...
      d0 = highbd_convolve8_4_srsub_s32_s16(s0, s1, s2, s3, s4, s5, s6, s7,
                                            y_filter, round1_shift_s32,
                                            offset_s32, correction_s32);
      d1 = highbd_convolve8_4_srsub_s32_s16(s1, s2, s3, s4, s5, s6, s7, s8,
                                            y_filter, round1_shift_s32,
                                            offset_s32, correction_s32);
      d2 = highbd_convolve8_4_srsub_s32_s16(s2, s3, s4, s5, s6, s7, s8, s9,
                                            y_filter, round1_shift_s32,
                                            offset_s32, correction_s32);
      d3 = highbd_convolve8_4_srsub_s32_s16(s3, s4, s5, s6, s7, s8, s9, s10,
                                            y_filter, round1_shift_s32,
                                            offset_s32, correction_s32);
    ...
    do {
      ...
      do {
        ...
        d0 = highbd_convolve8_8_srsub_s32_s16(s0, s1, s2, s3, s4, s5, s6, s7,
                                              y_filter, round1_shift_s32,
                                              offset_s32, correction_s32);
        d1 = highbd_convolve8_8_srsub_s32_s16(s1, s2, s3, s4, s5, s6, s7, s8,
                                              y_filter, round1_shift_s32,
                                              offset_s32, correction_s32);
        d2 = highbd_convolve8_8_srsub_s32_s16(s2, s3, s4, s5, s6, s7, s8, s9,
                                              y_filter, round1_shift_s32,
                                              offset_s32, correction_s32);
        d3 = highbd_convolve8_8_srsub_s32_s16(s3, s4, s5, s6, s7, s8, s9, s10,
                                              y_filter, round1_shift_s32,
                                              offset_s32, correction_s32);
...
static void highbd_convolve_add_src_horiz_hip(
  ...
  do {
    ...
    do {
      ...
      // Actually load the filters
      const int16_t *x_filter_ptr = x_filters[x_q4 & SUBPEL_MASK];
      const int16x8_t x_filter = vld1q_s16(x_filter_ptr);
      ...
      d0 = highbd_convolve8_horiz4x8_s32_s16(s0, s1, s2, s3, x_filter,
                                             shift_s32, rounding);
...
static void highbd_convolve_add_src_vert_hip(
  ...
  do {
    ...
    do {
      ...
      // calculate the indices anyway. Doing it in SIMD and then storing the
      // indices is faster than having to calculate the expression
      // &src_ptr[((x_q4 + 0*x_step_q4) >> SUBPEL_BITS)] 4 times
      // Ideally this should be a gather using the indices, but NEON does not
      // have that, so have to emulate
      ...
      // Actually load the filters
      const int16_t *const y_filter_ptr = y_filters[y_q4 & SUBPEL_MASK];
      const int16x8_t y_filter = vld1q_s16(y_filter_ptr);
...
void av1_highbd_wiener_convolve_add_src_neon(
  ...
  const InterpKernel *const x_filters_ptr = get_filter_base(filter_x);
  const int x0_q4 = get_filter_offset(filter_x, x_filters_ptr);

  const InterpKernel *const y_filters_ptr = get_filter_base(filter_y);
  const int y0_q4 = get_filter_offset(filter_y, y_filters_ptr);

  DECLARE_ALIGNED(16, uint16_t, im_block[WIENER_MAX_EXT_SIZE * MAX_SB_SIZE]);
  const int im_h = (((h - 1) * y_step_q4 + y0_q4) >> SUBPEL_BITS) + SUBPEL_TAPS;
  const int im_stride = MAX_SB_SIZE;
