  ...
  if (w <= 4) {
...
#else
      uint32x4_t src4;
      src4 = vaddw_u32(vdupq_n_u32((const uint32_t)src_ptr), src_idx_u32);
      int16_t *src4_ptr[4];
    ...
    do {
      ...
      do {
        ...
        vst1q_u64(tmp_ptr + 2, src4[1]);
#else
        uint32x4_t src4 = vaddq_u32(vdupq_n_u32((const uint32_t)s),
                                    vget_low_u32(src_idx_u32));
        int16_t *src4_ptr[4];
        uint32_t *tmp_ptr = (uint32_t *)&src4_ptr;
        vst1q_u64(tmp_ptr, src4);
#endif
        // Same for the filter vectors
        const int32x4_t filter_idx_s32 = vreinterpretq_s32_u32(
            vshrq_n_u32(vandq_u32(xqn_idx, subpel_mask), SCALE_EXTRA_BITS));
...
static INLINE void highbd_convolve_2d_y_scale_8tap_neon(
    ...
    const InterpFilterParams *filter_params, const int offset) {
...
void av1_highbd_convolve_2d_scale_neon(
    ...
    ConvolveParams *conv_params, int bd) {
  DECLARE_ALIGNED(16, uint16_t, im_block[MAX_SB_SIZE * MAX_SB_SIZE]);
  DECLARE_ALIGNED(16, uint16_t, im_block2[MAX_SB_SIZE * MAX_SB_SIZE]);
      ...
      if (conv_params->use_dist_wtd_comp_avg) {
        ...
        if (w <= 4) {
          for (int y = 0; y < h; ++y) {
            ...
            const int16x4_t d16 =
                vld1_s16((const int16_t *)(dst16) + y * dst16_stride);
            int32x4_t d0 = vaddl_s16(d16, s);
            d0 = vshrq_n_s32(d0, 1);
            d0 = vqrshlq_s32(vsubw_s16(d0, y_offset_correction_s16),
                             final_shift_s32);
            uint16x4_t d = vqmovun_s32(d0);
          ...
          for (int y = 0; y < h; ++y) {
            for (int x = 0; x < w; x += 8) {
              ...
              const int16x8_t d16 =
                  vld1q_s16((const int16_t *)(dst16) + y * dst16_stride + x);
              int32x4_t d0 = vaddl_s16(vget_low_s16(d16), vget_low_s16(s));
              int32x4_t d1 = vaddl_s16(vget_high_s16(d16), vget_high_s16(s));
              d0 = vshrq_n_s32(d0, 1);
              d1 = vshrq_n_s32(d1, 1);
              d0 = vqrshlq_s32(vsubw_s16(d0, y_offset_correction_s16),
                               final_shift_s32);
