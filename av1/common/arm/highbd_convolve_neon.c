...
void av1_highbd_convolve_2d_scale_neon(
  ...
  DECLARE_ALIGNED(16, uint16_t,
                  im_block2[(2 * MAX_SB_SIZE + MAX_FILTER_TAP) * MAX_SB_SIZE]);
      ...
      if (conv_params->use_dist_wtd_comp_avg) {
        ...
        if (w <= 4) {
          for (int y = 0; y < h; ++y) {
            const uint16x4_t s = vld1_u16(im_block2 + y * im_stride);
            const uint16x4_t d16 = vld1_u16(dst16 + y * dst16_stride);
            int32x4_t d0 = vreinterpretq_s32_u32(vaddl_u16(d16, s));
            d0 = vshrq_n_s32(d0, 1);
          ...
          for (int y = 0; y < h; ++y) {
            for (int x = 0; x < w; x += 8) {
              ...
              const uint16x8_t d16 = vld1q_u16(dst16 + y * dst16_stride + x);
              int32x4_t d0 = vreinterpretq_s32_u32(
                  vaddl_u16(vget_low_u16(d16), vget_low_u16(s)));
              int32x4_t d1 = vreinterpretq_s32_u32(
                  vaddl_u16(vget_high_u16(d16), vget_high_u16(s)));
              d0 = vshrq_n_s32(d0, 1);
              d1 = vshrq_n_s32(d1, 1);
              d0 = vsubw_s16(d0, y_offset_correction_s16);
