...
static AOM_INLINE void diffwtd_mask_neon(uint8_t *mask, const int inverse,
...
void av1_build_compound_diffwtd_mask_neon(uint8_t *mask,
  ...
  if (mask_type == DIFFWTD_38) {
    diffwtd_mask_neon(mask, 0, src0, src0_stride, src1, src1_stride, h, w);
  } else {  // mask_type == DIFFWTD_38_INV
    diffwtd_mask_neon(mask, 1, src0, src0_stride, src1, src1_stride, h, w);
  }
}
