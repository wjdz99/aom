...
// for the purpose round right shift.
#define NB_EVEN 5
#define NB_ODD 4

#define CALC_AB_FAST_INTERNAL_COMMON()                                       \
  /* a*n */                                                                  \
  ...
  p3 = vandq_u32(p3, q3);                                                    \
                                                                             \
  /* ROUND_POWER_OF_TWO(p * s, SGRPROJ_MTABLE_BITS); */                      \
  ...
  p3 = vmovl_u16(d3);                                                        \
                                                                             \
  p0 = vsubq_u32(sgrproj_sgr, p0);                                           \
  p1 = vsubq_u32(sgrproj_sgr, p1);                                           \
  p2 = vsubq_u32(sgrproj_sgr, p2);                                           \
  p3 = vsubq_u32(sgrproj_sgr, p3);                                           \
                                                                             \
...
#define CALC_AB_INTERNAL_COMMON()                                              \
  {                                                                            \
    /* a*n */                                                                  \
    s0 = vmulq_u32(s0, const_n_val);                                           \
    s1 = vmulq_u32(s1, const_n_val);                                           \
...
static INLINE void calc_ab_fast_internal_lbd(int32_t *A, uint16_t *A16,
  ...
  const uint32_t n = (2 * r + 1) * (2 * r + 1);
  const uint32x4_t const_n_val = vdupq_n_u32(n);
  const uint32x4_t sgrproj_sgr = vdupq_n_u32(SGRPROJ_SGR);
...
void apply_selfguided_restoration_neon(const uint8_t *dat8, int width,
  ...
  av1_selfguided_restoration_neon(dat8, width, height, stride, flt0, flt1,
...

  {
    ...
    do {
      w = width;
      count = 0;
      dst_ptr = dst8 + (rc << 2) * dst_stride;
      dst16_ptr = dst16 + (rc << 2) * dst_stride;
      do {
        load_s16_8x4((src_ptr + count), dgd16_stride, &s0, &s1, &s2, &s3);
        u0 = vshll_n_s16(vget_low_s16(s0), SGRPROJ_RST_BITS);
        u1 = vshll_n_s16(vget_low_s16(s1), SGRPROJ_RST_BITS);
        u2 = vshll_n_s16(vget_low_s16(s2), SGRPROJ_RST_BITS);
        u3 = vshll_n_s16(vget_low_s16(s3), SGRPROJ_RST_BITS);
        u4 = vshll_n_s16(vget_high_s16(s0), SGRPROJ_RST_BITS);
        u5 = vshll_n_s16(vget_high_s16(s1), SGRPROJ_RST_BITS);
        u6 = vshll_n_s16(vget_high_s16(s2), SGRPROJ_RST_BITS);
        u7 = vshll_n_s16(vget_high_s16(s3), SGRPROJ_RST_BITS);

        v0 = vshlq_n_s32(u0, SGRPROJ_PRJ_BITS);
        v1 = vshlq_n_s32(u1, SGRPROJ_PRJ_BITS);
        v2 = vshlq_n_s32(u2, SGRPROJ_PRJ_BITS);
        v3 = vshlq_n_s32(u3, SGRPROJ_PRJ_BITS);
        v4 = vshlq_n_s32(u4, SGRPROJ_PRJ_BITS);
        v5 = vshlq_n_s32(u5, SGRPROJ_PRJ_BITS);
        v6 = vshlq_n_s32(u6, SGRPROJ_PRJ_BITS);
        v7 = vshlq_n_s32(u7, SGRPROJ_PRJ_BITS);

        if (params->r[0] > 0) {
          load_s32_4x4((flt0 + count), width, &f00, &f01, &f02, &f03);
          load_s32_4x4((flt0 + count + 4), width, &f10, &f11, &f12, &f13);

          f00 = vsubq_s32(f00, u0);
          f01 = vsubq_s32(f01, u1);
          f02 = vsubq_s32(f02, u2);
          f03 = vsubq_s32(f03, u3);
          f10 = vsubq_s32(f10, u4);
          f11 = vsubq_s32(f11, u5);
          f12 = vsubq_s32(f12, u6);
          f13 = vsubq_s32(f13, u7);

          v0 = vmlaq_s32(v0, xq0_vec, f00);
          v1 = vmlaq_s32(v1, xq0_vec, f01);
          v2 = vmlaq_s32(v2, xq0_vec, f02);
          v3 = vmlaq_s32(v3, xq0_vec, f03);
          v4 = vmlaq_s32(v4, xq0_vec, f10);
          v5 = vmlaq_s32(v5, xq0_vec, f11);
          v6 = vmlaq_s32(v6, xq0_vec, f12);
          v7 = vmlaq_s32(v7, xq0_vec, f13);
        }

        if (params->r[1] > 0) {
          load_s32_4x4((flt1 + count), width, &f00, &f01, &f02, &f03);
          load_s32_4x4((flt1 + count + 4), width, &f10, &f11, &f12, &f13);
          f00 = vsubq_s32(f00, u0);
          f01 = vsubq_s32(f01, u1);
          f02 = vsubq_s32(f02, u2);
          f03 = vsubq_s32(f03, u3);
          ...
          v1 = vmlaq_s32(v1, xq1_vec, f01);
          v2 = vmlaq_s32(v2, xq1_vec, f02);
          v3 = vmlaq_s32(v3, xq1_vec, f03);
          v4 = vmlaq_s32(v4, xq1_vec, f10);
          v5 = vmlaq_s32(v5, xq1_vec, f11);
          v6 = vmlaq_s32(v6, xq1_vec, f12);
          v7 = vmlaq_s32(v7, xq1_vec, f13);
        }

        d0 = vqrshrn_n_s32(v0, SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);
        d1 = vqrshrn_n_s32(v1, SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);
        d2 = vqrshrn_n_s32(v2, SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);
        d3 = vqrshrn_n_s32(v3, SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);
        d4 = vqrshrn_n_s32(v4, SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);
        d5 = vqrshrn_n_s32(v5, SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);
        d6 = vqrshrn_n_s32(v6, SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);
        d7 = vqrshrn_n_s32(v7, SGRPROJ_PRJ_BITS + SGRPROJ_RST_BITS);

        r0 = vcombine_s16(d0, d4);
        r1 = vcombine_s16(d1, d5);
        r2 = vcombine_s16(d2, d6);
        r3 = vcombine_s16(d3, d7);

        // lower clipping
        r4 = vreinterpretq_u16_s16(vmaxq_s16(r0, zero));
        r5 = vreinterpretq_u16_s16(vmaxq_s16(r1, zero));
        r6 = vreinterpretq_u16_s16(vmaxq_s16(r2, zero));
        r7 = vreinterpretq_u16_s16(vmaxq_s16(r3, zero));
        ...
        if (highbd) {
          ...
          r6 = vminq_u16(r6, max);
          r7 = vminq_u16(r7, max);
          store_u16_8x4(dst16_ptr, dst_stride, r4, r5, r6, r7);
        } else {
          t0 = vqmovn_u16(r4);
          t1 = vqmovn_u16(r5);
          t2 = vqmovn_u16(r6);
          t3 = vqmovn_u16(r7);
          store_u8_8x4(dst_ptr, dst_stride, t0, t1, t2, t3);
        }
        w -= 8;
        count += 8;
        dst_ptr += 8;
