...
void aom_blend_a64_hmask_neon(uint8_t *dst, uint32_t dst_stride,
  ...
  if (w >= 16) {
    ...
    for (int i = 0; i < h; i += 2) {
      ...
      const uint8x8_t m = vreinterpret_u8_u32(vdup_lane_u32(
          vld1_lane_u32((uint32_t *)(mask), vreinterpret_u32_u8(m), 0), 0));
      const uint8x8_t max_minus_m = vsub_u8(vdup_64, m);
    ...
    for (int i = 0; i < h; i += 2) {
      ...
      __builtin_prefetch(src1 + 0 * src1_stride);
      __builtin_prefetch(src1 + 1 * src1_stride);
      const uint8x8_t m = vreinterpret_u8_u16(vdup_lane_u16(
          vld1_lane_u16((uint16_t *)(mask), vreinterpret_u16_u8(m), 0), 0));
      const uint8x8_t max_minus_m = vsub_u8(vdup_64, m);
