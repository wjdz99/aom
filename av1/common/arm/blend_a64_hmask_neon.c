...
void aom_blend_a64_hmask_neon(uint8_t *dst, uint32_t dst_stride,
  ...
  if (w >= 16) {
    ...
    for (int i = 0; i < h; i += 2) {
      ...
      load_unaligned_u8_4x2(src0, src0_stride, &tmp0);
      load_unaligned_u8_4x2(src1, src1_stride, &tmp1);
      res = vmull_u8(m, tmp0);
      res = vmlal_u8(res, max_minus_m, tmp1);
    ...
    for (int i = 0; i < h; i += 2) {
      ...
      __builtin_prefetch(src1 + 0 * src1_stride);
      __builtin_prefetch(src1 + 1 * src1_stride);
      load_unaligned_u8_2x2(src0, src0_stride, &tmp0);
      load_unaligned_u8_2x2(src1, src1_stride, &tmp1);
      res = vmull_u8(m, tmp0);
      res = vmlal_u8(res, max_minus_m, tmp1);
      const uint8x8_t result = vrshrn_n_u16(res, AOM_BLEND_A64_ROUND_BITS);
