...
void cfl_load(const CFL_CTX *const cfl, uint8_t *const output,
  ...
  // TODO(ltrudeau) adjusted to Chroma subsampling (hardcoded to 4:2:0)
  const int step = 2;
  const uint8_t *const pred = &cfl->y_pix[
    (step * (row * MAX_SB_SIZE + col)) << tx_size_wide_log2[0]];
  const int uv_width = step * ((col << tx_size_wide_log2[0]) + tx_blk_size);
  const int diff_width = (uv_width - cfl->y_width) / step;
  const int uv_height = step * ((row << tx_size_wide_log2[0]) + tx_blk_size);
  ...
  /*printf("%d [%d %d cfl:(%d %d) uv:(%d, %d) diff:(%d %d)] Load: ", tx_blk_size, row, col, cfl->y_width,
