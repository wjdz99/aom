...
void cfl_load(const CFL_CTX *const cfl, uint8_t *const output,
  ...
  const int uv_width = step * (row << tx_size_wide_log2[0]) + tx_blk_size;
  const int diff_width = (uv_width - cfl->y_width) / step;
  const int uv_height = step * (col << tx_size_wide_log2[0]) + tx_blk_size;
  ...
  if (diff_width > 0) {
    ...
    output_row_offset = 0;
    for (j = 0; j < tx_blk_size; j++) {
      last_pixel = output_row_offset + (tx_blk_size - (diff_width + 1));
      for (i = tx_blk_size - diff_width; i < tx_blk_size;  i++) {
  ...
  if (diff_height > 0) {
    const int last_row_offset =
      output_row_offset - ((diff_height + 1) * output_stride);
    output_row_offset = last_row_offset + output_stride;
...
void cfl_store(CFL_CTX *const cfl, uint8_t *const input, int input_stride,
  ...
  if (col == 0 && row == 0){
    ...
    cfl->y_width =
      OD_MAXI((col << tx_size_wide_log2[0]) + tx_blk_size, cfl->y_width);
    cfl->y_height =
      OD_MAXI((row << tx_size_wide_log2[0]) + tx_blk_size, cfl->y_height);
  }
}
#endif
