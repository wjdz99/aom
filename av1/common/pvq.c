...
void cfl_load(const CFL_CTX *const cfl, uint8_t *const output,
  ...
  const uint8_t *const pred = &cfl->y_pix[
    (step * (row * MAX_SB_SIZE + col)) << tx_off_log2];
  ...
  for (j = 0; j < tx_blk_size; j++) {
    for (i = 0; i < tx_blk_size;  i++) {
      output[output_row_offset + i] =
        (pred[step * (pred_row_offset + i)]
        + pred[step * (pred_row_offset + i) + 1]
        + pred[step * (pred_row_offset + i) + MAX_SB_SIZE]
        + pred[step * (pred_row_offset + i) + MAX_SB_SIZE + 1]
        )/4;
    }
...
void cfl_store(CFL_CTX *const cfl, uint8_t *const input, int input_stride,
  ...
  uint8_t *const pred = &cfl->y_pix[
    (row * MAX_SB_SIZE + col) << tx_off_log2];
  int pred_row_offset = 0;
