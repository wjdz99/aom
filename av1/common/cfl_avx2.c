...
static void cfl_luma_subsampling_420_lbd_avx2(const uint8_t *input,
  ...
  do {
    ...
    __m256i top_lo = _mm256_unpacklo_epi8(top, zeros);
    __m256i bot_lo = _mm256_unpacklo_epi8(bot, zeros);
    __m256i top_hi = _mm256_unpackhi_epi8(top, zeros);
    __m256i bot_hi = _mm256_unpackhi_epi8(bot, zeros);

    top_lo = _mm256_add_epi16(top_lo, bot_lo);
    top_hi = _mm256_add_epi16(top_hi, bot_hi);

    top_lo = _mm256_madd_epi16(top_lo, twos);
    top_hi = _mm256_madd_epi16(top_hi, twos);

    _mm256_storeu_si256((__m256i *)output_q3,
                        _mm256_packs_epi32(top_lo, top_hi));
    input += luma_stride;
