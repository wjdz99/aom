...
static INLINE void subtract_average_x(int16_t *pred_buf, int width, int height,
  ...
  __m256i *const pred_buf_i256 = (__m256i *)pred_buf;
  ...
  while (i < height) {
    ...
    if (width == 4) {
      ...
      sum = _mm256_add_epi32(
          sum, _mm256_add_epi32(_mm256_unpacklo_epi16(t0, zeros),
  ...
  __m256i avg_x16 = _mm256_set1_epi16(
      (_mm256_cvtsi256_si32(sum) + round_offset) >> num_pel_log2);
  ...
  for (i = 0; i < height; i++) {
    ...
    if (width == 32) {
      _mm256_storeu_si256(
          row + 1, _mm256_sub_epi16(_mm256_loadu_si256(row + 1), avg_x16));
    }
  }
}
...
cfl_subtract_average_fn get_subtract_average_fn_avx2(TX_SIZE tx_size) {
  static const cfl_subtract_average_fn sub_avg[TX_SIZES_ALL] = {
    ...
    subtract_average_8x8_avx2,    // 8x8
    subtract_average_16x16_avx2,  // 16x16
    subtract_average_32x32_avx2,  // 32x32
#if CONFIG_TX64X64
    cfl_subtract_average_null,    // 64x64 (invalid CFL size)
#endif                            // CONFIG_TX64X64
    subtract_average_4x8_avx2,    // 4x8
    subtract_average_8x4_avx2,    // 8x4
    subtract_average_8x16_avx2,   // 8x16
