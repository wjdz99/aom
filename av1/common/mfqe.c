...
// Dynamically allocate memory for the buffer to store the Gaussian blurred
// version of the given buffer representing a luma plane, then return the
// pointer to the new buffer.
static Y_BUFFER_CONFIG get_gaussian_blur(uint8_t *src, int stride, int h, int w,
                                         int high_bd, int bd) {
  // Allocate a new buffer to store the Gaussian blurred version of the plane.
  Y_BUFFER_CONFIG dst = { .stride = stride, .height = h, .width = w };
  dst.buffer = aom_memalign(32, sizeof(uint8_t) * stride * w);

  for (int col = 0; col < h; col += 32) {
    for (int row = 0; row < w; row += 32) {
      int w_size = AOMMIN(32, w - row);
      int h_size = AOMMIN(32, h - col);
      uint8_t *src_ptr = src + col * stride + row;
      uint8_t *dst_ptr = dst.buffer + col * stride + row;
...
static Y_BUFFER_CONFIG get_resize_plane(const uint8_t *src, int stride, int h,
  ...
  Y_BUFFER_CONFIG dst;
  dst.stride = stride * resize_factor;
  dst.height = h * resize_factor;
  dst.width = w * resize_factor;

  int num_buf_bytes = dst.stride * dst.width;
  dst.buffer = aom_memalign(32, sizeof(uint8_t) * num_buf_bytes);
  av1_resize_plane(src, h, w, stride, dst.buffer, dst.height, dst.width,
                   dst.stride);
  return dst;
}

static int64_t calc_sse(const uint8_t *a, int a_stride, const uint8_t *b,
...
static double get_mse_block(Y_BUFFER_CONFIG buf1, Y_BUFFER_CONFIG buf2,
                            ...
                            int16_t mb_row_2, int16_t mb_col_2, int bsize) {
  // Check if rows and columns are valid, return MSE_MAX if not.
  if ((mb_row_1 < 0) || (mb_col_1 < 0) || (mb_row_2 < 0) || (mb_col_2 < 0) ||
      (mb_row_1 >= buf1.height - bsize) || (mb_row_2 >= buf2.height - bsize) ||
      (mb_col_1 >= buf1.width - bsize) || (mb_col_2 >= buf2.width - bsize))
    return MSE_MAX;

  uint8_t *a = buf1.buffer + buf1.stride * mb_col_1 + mb_row_1;
  uint8_t *b = buf2.buffer + buf2.stride * mb_col_2 + mb_row_2;
  int64_t sse = calc_sse(a, buf1.stride, b, buf2.stride, bsize, bsize);

  double mse = ((double)sse) / (bsize * bsize);
  return mse;
}
...
void av1_apply_loop_mfqe(Y_BUFFER_CONFIG *tmp, RefCntBuffer *ref_frames[],
                         int bsize, int resize_factor, int high_bd, int bd) {
  ...
  Y_BUFFER_CONFIG tmp_sub = get_resize_plane(
      tmp_low.buffer, tmp->stride, tmp->height, tmp->width, resize_factor);
...
void av1_decode_restore_mfqe(AV1_COMMON *cm, int resize_factor, int bsize) {
