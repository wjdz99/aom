...
// block in the current frame, going through diamond search around the block.
...
static BLOCK_SIZE scale_block_8X8(int factor) {
  if (factor == 1) return BLOCK_8X8;
  if (factor == 2) return BLOCK_16X16;
  if (factor == 4) return BLOCK_32X32;
  if (factor == 8) return BLOCK_64X64;
  return BLOCK_8X8;
}

static BLOCK_SIZE scale_block_16X16(int factor) {
  if (factor == 1) return BLOCK_16X16;
  if (factor == 2) return BLOCK_32X32;
  if (factor == 4) return BLOCK_64X64;
  if (factor == 8) return BLOCK_128X128;
  return BLOCK_16X16;
}

// Scale up the given BLOCK_SIZE by the resizing factor.
static BLOCK_SIZE scale_block_size(BLOCK_SIZE bsize, int factor) {
  // Currently, only supports 8X8 and 16X16 blocks.
  assert(bsize == BLOCK_8X8 || bsize == BLOCK_16X16);

  if (bsize == BLOCK_8X8) return scale_block_8X8(factor);
  return scale_block_16X16(factor);
}
...
static void mfqe_alloc_buf(Y_BUFFER_CONFIG *buf, int stride, int h, int w,
  ...
  int buf_bytes = stride * (h + 2 * MFQE_PADDING_SIZE);
  buf->buffer_orig = aom_memalign(32, sizeof(uint8_t) * buf_bytes);
  buf->buffer = buf->buffer_orig + stride * MFQE_PADDING_SIZE;
}
