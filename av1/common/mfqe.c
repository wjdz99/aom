...
static void replace_block_alpha(Y_BUFFER_CONFIG tmp, Y_BUFFER_CONFIG refs_sub[],
  ...
  // Place the resized version of reference block into the swap block buffer.
  ...
  */
  av1_resize_plane(src, block_h * resize_factor, block_w * resize_factor,
                   src_stride, dst, block_h, block_w, dst_stride);
}
...
static void replace_block_alpha_highbd(Y_BUFFER_CONFIG tmp,
  ...
  */
 av1_highbd_resize_plane(src, block_h * resize_factor, block_w * resize_factor,
                         src_stride, dst, block_h, block_w, dst_stride, bd);
}
...
void av1_decode_restore_mfqe(AV1_COMMON *cm, int resize_factor,
  ...
  av1_apply_loop_mfqe(&cur_frame, ref_frames, bsize, resize_factor, high_bd,
