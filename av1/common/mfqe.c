...
static void replace_block_subpel(Y_BUFFER_CONFIG tmp,
                                 ...
                                 int resize_factor) {
  int16_t mb_row_ref = (mb_row + mvr->mv.row) * resize_factor;
  int16_t mb_col_ref = (mb_col + mvr->mv.col) * resize_factor;
...
static void replace_block_alpha(Y_BUFFER_CONFIG tmp, Y_BUFFER_CONFIG refs_sub[],
  ...
  av1_resize_plane(src, block_h * resize_factor, block_w * resize_factor,
                   src_stride, swap_block, block_h, block_w, dst_stride);

  uint8_t *swap_ptr = swap_block;
...
static void apply_loop_mfqe_lowbd(Y_BUFFER_CONFIG *tmp,
  ...
  const char file_name[256] = "data.yuv";
  FILE *f_data = fopen(file_name, "ab");

  fwrite(&tmp->height, 1, sizeof(tmp->height), f_data);
  fwrite(&tmp->width, 1, sizeof(tmp->width), f_data);

  for (int h = 0; h < tmp->height; ++h) {
    fwrite(&tmp->buffer[h * tmp->stride], 1, tmp->width, f_data);
  }
  ...
  for (int h = 0; h < tmp->height; ++h) {
    fwrite(&tmp_low.buffer[h * tmp->stride], 1, tmp->width, f_data);
  }

  fwrite(&tmp_sub.height, 1, sizeof(int), f_data);
  fwrite(&tmp_sub.width, 1, sizeof(int), f_data);

  for (int h = 0; h < tmp_sub.height; ++h) {
    fwrite(&tmp_sub.buffer[h * tmp_sub.stride], 1, tmp_sub.width, f_data);
  }
  ...
  fwrite(&tmp->height, 1, sizeof(int), f_data);
  fwrite(&tmp->width, 1, sizeof(int), f_data);
