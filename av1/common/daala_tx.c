...
/* This macro is used to avoid 16-bit overflows in 2D transforms that have 64
    or fewer coefficients.
   TODO: Based on future experimentation, we may decide to drop the +1 to avoid
    accumulating bias in the round trip 2D transforms. */
#define OD_AVERAGE(_a, _b) (((_a) + (_b) + 1) >> 1)
