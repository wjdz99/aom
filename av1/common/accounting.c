...
int aom_accounting_dictionary_lookup(AOMAccounting *accounting,
                                     const char *str) {
  int hash;
...
void aom_accounting_set_context(AOMAccounting *accounting, int16_t x,
                                int16_t y) {
  accounting->context.x = x;
  accounting->context.y = y;
}

void aom_accounting_record(AOMAccounting *accounting, const char *str,
                           uint32_t bits) {
...
void aom_accounting_dump(AOMAccounting *accounting) {
  ...
  for (i = 0; i < accounting->syms.num_syms; i++) {
    ...
    printf("%s %d, %d | %f\n", accounting->syms.dictionary.strs[sym->id],
           sym->context.x, sym->context.y, (float)sym->bits / 8.0);
  }
}