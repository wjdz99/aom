...
static void calc_delta_hbd4x2(v128 o, v128 a, v128 b, v128 c, v128 d, v128 e,
...
static void clpf_block4x2_hbd(const uint16_t *src, uint16_t *dst, int sstride,
                              int dstride, int x0, int y0, int sizey, int width,
                              int height, unsigned int strength) {
  ...
  for (y = 0; y < sizey; y += 2) {
    ...
    calc_delta_hbd4x2(o, a, b, c, d, e, f, dst, sp, sm, dstride);
    src += sstride * 2;
    dst += dstride * 2;
  }
}
...
void SIMD_FUNC(aom_clpf_block_hbd)(const uint16_t *src, uint16_t *dst,
  ...
  if ((sizex != 4 && sizex != 8) || ((sizey & 1) && sizex == 4)) {
    ...
    (sizex == 4 ? clpf_block4x2_hbd : clpf_block8x2_hbd)(
        src, dst, sstride, dstride, x0, y0, sizey, width, height, strength);
  }
}
#endif
