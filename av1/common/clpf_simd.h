...
static void clpf_block8(const uint8_t *src, uint8_t *dst, int sstride,
  ...
  for (y = 0; y < sizey; y += 2) {
    ...
    const v64 l3 = v64_load_aligned(src - (y != -y0) * sstride);
    const v64 l4 = v64_load_aligned(src + ((y != bottom) + 1) * sstride);
    v128 o = v128_from_v64(l1, l2);
    const v128 a =
        v128_from_v64(v64_load_aligned(src - 2 * (y != -y0) * sstride), l3);
...
static void clpf_block4(const uint8_t *src, uint8_t *dst, int sstride,
  ...
  for (y = 0; y < sizey; y += 4) {
    const uint32_t l0 = u32_load_aligned(src - 2 * (y != -y0) * sstride);
    const uint32_t l1 = u32_load_aligned(src - (y != -y0) * sstride);
...
SIMD_INLINE void clpf_block_hbd(const uint16_t *src, uint16_t *dst, int sstride,
  ...
  for (y = 0; y < sizey; y++) {
    ...
    const v128 a = v128_load_aligned(src - AOMMIN(y + y0, 2) * sstride);
    const v128 b = v128_load_aligned(src - (y != -y0) * sstride);
    const v128 g = v128_load_aligned(src + (y != bottom) * sstride);
    const v128 h = v128_load_aligned(src + AOMMIN(bottom - y, 2) * sstride);
    v128 c, d, e, f;

    if (x0) {
      c = v128_load_unaligned(src - 2);
    ...
    if (right) {
      e = v128_load_unaligned(src + 1);
      f = v128_load_unaligned(src + 2);
    } else {  // Right clipping
      e = v128_shuffle_8(o, v128_load_aligned(e_shuff));
      f = v128_shuffle_8(o, v128_load_aligned(f_shuff));
    }
