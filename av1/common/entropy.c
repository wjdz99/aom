...
#endif
#if CONFIG_VQ4X4
  RESET_CDF_COUNTER_STRIDE(fc->vq_gain_sym1_cdf, 4,
                           CDF_SIZE(VQ_GAIN_SYMBOLS_1));
  for (int i = 0; i < VQ_GAIN_SYMBOLS_1; i++)
    RESET_CDF_COUNTER_STRIDE(fc->vq_gain_sym2_cdf[i], 16,
                             CDF_SIZE(VQ_GAIN_SYMBOLS_2));
  RESET_CDF_COUNTER_STRIDE(fc->vq_shape_sym1_cdf, 4,
                           CDF_SIZE(VQ_SHAPE_SYMBOLS_1));
  for (int i = 0; i < VQ_SHAPE_SYMBOLS_1; i++)
    RESET_CDF_COUNTER_STRIDE(fc->vq_shape_sym2_cdf[i], 16,
