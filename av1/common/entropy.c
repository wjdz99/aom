...
static void build_token_cdfs(const aom_prob *pdf_model,
  ...
  int i, p, p1, p2, phead[6], scaleNZ, scaleEOB_1, scaleEOB_2p, scaleNEOB_1,
      scaleNEOB_2p, sum = 0;
  int scale_blocknz;
  ...
  // being non-zero
  scale_blocknz = blockz_model == NULL ? 256 : (256 - *blockz_model);
  ...
  scaleNZ = CDF_PROB_TOP - phead[1 + ZERO_TOKEN];
  ...
  scaleEOB_2p = scaleEOB_1 / 2;

  scaleNEOB_1 = CDF_PROB_TOP - scaleEOB_1;
  scaleNEOB_2p = CDF_PROB_TOP - scaleEOB_2p;
  if (scaleNZ == 0 || scaleNZ == CDF_PROB_TOP) abort();
  if (scaleEOB_1 == 0 || scaleEOB_1 == CDF_PROB_TOP) abort();
  if (scaleEOB_2p == 0 || scaleEOB_2p == CDF_PROB_TOP) abort();
  ...
  p1 = ROUND_POWER_OF_TWO(scaleNZ * p, 15);

  // Scale by the EOB factors
  phead[1 + ONE_TOKEN_EOB] = ROUND_POWER_OF_TWO(p1 * scaleEOB_1, 15);
  phead[1 + ONE_TOKEN_NEOB] = ROUND_POWER_OF_TWO(p1 * scaleNEOB_1, 15);

  // Probability token is 2 or more
  p2 = CDF_PROB_TOP - p1 - phead[1 + ZERO_TOKEN];

  phead[1 + TWO_TOKEN_PLUS_EOB] = ROUND_POWER_OF_TWO(p2 * scaleEOB_2p, 15);
  phead[1 + TWO_TOKEN_PLUS_NEOB] = ROUND_POWER_OF_TWO(p2 * scaleNEOB_2p, 15);

  // Now use block non-zerp prob to scale the values
  for (i = 1; i < 5; ++i) {
    phead[i] = (scale_blocknz * phead[i] + 128) >> 8;
  }
