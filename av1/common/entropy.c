...
#if CONFIG_EC_ADAPT
static void adapt_coef_cdfs(AV1_COMMON *cm, FRAME_CONTEXT *pre_fc,
                            TX_SIZE tx_size, int type) {
#if CONFIG_NEW_TOKENSET
...
void av1_adapt_coef_cdfs(AV1_COMMON *cm, FRAME_CONTEXT *pre_fc) {
  ...
  for (tx_size = 0; tx_size < TX_SIZES; tx_size++)
    for (t = 0; t < PLANE_TYPES; t++) adapt_coef_cdfs(cm, pre_fc, tx_size, t);
}
#endif
...
#define AVERAGE_TILE_CDFS(cname)                                              \
  for (i = 0; i < numt; ++i) cdf_ptr[i] = (aom_cdf_prob *)&ec_ctxs[i]->cname; \
  fc_cdf_ptr = (aom_cdf_prob *)&fc->cname;                                    \
  cdf_size = (int)sizeof(fc->cname) / sizeof(aom_cdf_prob);                   \
  av1_average_cdf(cdf_ptr, fc_cdf_ptr, cdf_size, numt);
