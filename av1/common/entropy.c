...
static void adapt_coef_probs(AV1_COMMON *cm, TX_SIZE tx_size,
  ...
  const av1_cbp_count_model *const cbp_counts =
      (const av1_cbp_count_model *)&cm->counts.cbp_count[tx_size][0];
#endif
  int i, j, k, l, m;
#if CONFIG_RECT_TX
  ...
  for (i = 0; i < PLANE_TYPES; ++i) {
    for (j = 0; j < REF_TYPES; ++j) {
      for (k = 0; k < CBP_CONTEXTS; ++k) {
        ...
        cbp_probs[i][j][k] = av1_merge_probs(pre_cbp_probs[i][j][k], branch_ct,
                                             count_sat, update_factor);
      }
    }
  }
