...
#define RESET_CDF_COUNTER(cname, nsymbs)                      \
  do {                                                        \
    ...
    int cdf_size = (int)sizeof(cname) / sizeof(aom_cdf_prob); \
...
void av1_reset_cdf_symbol_counters(FRAME_CONTEXT *fc) {
  ...
  RESET_CDF_COUNTER(fc->uv_mode_cdf[0], UV_INTRA_MODES - 1);
  RESET_CDF_COUNTER(fc->uv_mode_cdf[1], UV_INTRA_MODES);
  for (int i = 0; i < PARTITION_CONTEXTS; i++) {
    if (i < 4) {
      RESET_CDF_COUNTER(fc->partition_cdf[i], 4);
    } else if (i < 16) {
      RESET_CDF_COUNTER(fc->partition_cdf[i], 10);
    } else {
      RESET_CDF_COUNTER(fc->partition_cdf[i], 8);
    }
  }
  ...
  RESET_CDF_COUNTER(fc->tx_size_cdf[0], MAX_TX_DEPTH);
  ...
  RESET_CDF_COUNTER(fc->intra_ext_tx_cdf[1], 7);
  RESET_CDF_COUNTER(fc->intra_ext_tx_cdf[2], 5);
  RESET_CDF_COUNTER(fc->inter_ext_tx_cdf[1], 16);
  RESET_CDF_COUNTER(fc->inter_ext_tx_cdf[2], 12);
  RESET_CDF_COUNTER(fc->inter_ext_tx_cdf[3], 2);
