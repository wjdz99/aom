...
#if CONFIG_PVQ
static void av1_average_cxt(int *cxt_ptr[], int *fc_cxt_ptr,
                            int cxt_size, const int num_tiles) {
  int i, j;
  aom_cdf_prob last_val = 0;
...
#define AVERAGE_TILE_CXT(cname)                    \
  for (i = 0; i < num_tiles; ++i)                  \
    cxt_ptr[i] = (int *)&ec_ctxs[i]->cname;        \
  fc_cxt_ptr = (int *)&fc->cname;                  \
  cxt_size = (int)sizeof(fc->cname) / sizeof(int); \
  av1_average_cxt(cxt_ptr, fc_cxt_ptr, cxt_size, num_tiles);
...
void av1_average_tile_pvq_cdfs(FRAME_CONTEXT *fc, FRAME_CONTEXT *ec_ctxs[],
  ...
  for (j = 0; j < OD_NPLANES_MAX; j++)
    AVERAGE_TILE_CXT(pvq_context.model_dc[j])
  ...
  for (j = 0; j < 3; j++)
    AVERAGE_TILE_CDFS(pvq_context.pvq.pvq_param_model[j])
