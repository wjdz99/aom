...
void av1_reset_cdf_symbol_counters(FRAME_CONTEXT *fc) {
  ...
  RESET_CDF_COUNTER(fc->warp_rotation_flag_cdf, 2);
  RESET_CDF_COUNTER(fc->warp_rotation_degree_cdf, ROTATION_COUNT);
...
#if CONFIG_FLEX_MVRES
  for (int p = MV_SUBPEL_HALF_PRECISION; p < MV_SUBPEL_PRECISIONS; ++p) {
