...
static float get_gradient_hist_v3(const uint8_t *src, int src_stride, int rows,
                                  int cols, float *hist) {
  float total = 0.0f;
  float angle;
...
static float get_highbd_gradient_hist_v3(const uint8_t *src8, int src_stride,
...
static void generate_hog(const MACROBLOCKD *xd, float *hist) {
  ...
  if (is_cur_buf_hbd(xd)) {
    if (xd->above_mbmi) {
      total +=
          get_highbd_gradient_hist_v3(buf - lines * stride, stride, lines, cols,
                                      hist);
    }
    if (xd->left_mbmi) {
      total +=
          get_highbd_gradient_hist_v3(buf - lines, stride, rows, lines, hist);
    }
  } else {
    if (xd->above_mbmi) {
      total +=
          get_gradient_hist_v3(buf - lines * stride, stride, lines, cols, hist);
    }
    if (xd->left_mbmi) {
      total += get_gradient_hist_v3(buf - lines, stride, rows, lines, hist);
    }
  }
...
static int derive_intra_mode_from_hog_v2(const MACROBLOCKD *xd) {
...
int av1_get_derived_intra_mode(const MACROBLOCKD *xd, int bsize, int8_t *delta) {
  if (av1_enable_derived_intra_mode(xd, bsize)) {
    const int idx = derive_intra_mode_from_hog_v2(xd);
    int angle = angles[idx];
    if (angle < 36) angle += 180;
    const int mode = bin_to_mode[idx];
    *delta = (int8_t)((angle - mode_to_angle_map[mode]) / ANGLE_STEP);
    if (*delta < -9 || *delta > 9) {
      printf("\n error av1_get_derived_intra_mode, angle %d, mode %d, delta %d\n",
             angle, mode, *delta);
    }
    return mode;
  }
