...
static int get_bin_index_from_angle(int angle) {
  ...
  //return (angle + (BIN_WIDTH >> 1)) / BIN_WIDTH;
...
int av1_get_derived_intra_mode(const MACROBLOCKD *xd, int bsize, MB_MODE_INFO *mbmi) {
  if (av1_enable_derived_intra_mode(xd, bsize)) {
    ...
    for (int i = NUM_DERIVED_INTRA_MODES - 1; i > 0; --i) {
      ...
      //mbmi->derived_intra_angles[i] = mbmi->derived_intra_angles[0];
    }
