...
void init_txk_skip_array(const AV1_COMMON *cm, MB_MODE_INFO *mbmi, int mi_row,
                         int mi_col, BLOCK_SIZE bsize, uint8_t value,
                         FILE *fLog) {
  for (int plane = 0; plane < MAX_MB_PLANE; plane++) {
...
void update_txk_skip_array(const AV1_COMMON *cm, int mi_row, int mi_col,
                           int plane, int blk_row, int blk_col, TX_SIZE tx_size,
                           FILE *fLog) {
  blk_row *= 4;
  blk_col *= 4;
  int w = ((cm->width + MAX_SB_SIZE - 1) >> MAX_SB_SIZE_LOG2)
