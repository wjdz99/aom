...
void av1_foreach_transformed_block_interleave(
  ...
  const int max_4x4_w_y =
      num_4x4_w_y + (xd->mb_to_right_edge >= 0
                         ? 0
                         : xd->mb_to_right_edge >> (5 + pd_y->subsampling_x));
  const int max_4x4_h_y =
      num_4x4_h_y + (xd->mb_to_bottom_edge >= 0
                         ? 0
                         : xd->mb_to_bottom_edge >> (5 + pd_y->subsampling_y));
  const int extra_step_y = ((num_4x4_w_y - max_4x4_w_y) >> tx_log2_y) * step_y;

  const int max_4x4_w_c =
      num_4x4_w_c + (xd->mb_to_right_edge >= 0
                         ? 0
                         : xd->mb_to_right_edge >> (5 + pd_c->subsampling_x));
  const int max_4x4_h_c =
      num_4x4_h_c + (xd->mb_to_bottom_edge >= 0
                         ? 0
                         : xd->mb_to_bottom_edge >> (5 + pd_c->subsampling_y));
  ...
  while (tu_idx_y < tu_num_y) {
    ...
    offset_y = tu_idx_y * step_y + (tu_idx_y / tu_num_w_y) * extra_step_y;
    visit(0, offset_y, row_y, col_y, plane_bsize_y, tx_log2_y, arg);
    tu_idx_y++;
