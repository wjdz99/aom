...
void init_txk_skip_array(const AV1_COMMON *cm, MB_MODE_INFO *mbmi,
  ...
  for (int plane = 0; plane < MAX_MB_PLANE; plane++) {
    int w = ((cm->width + MAX_SB_SIZE - 1) >> MAX_SB_SIZE_LOG2) << MAX_SB_SIZE_LOG2;
    w >>= ((plane == 0) ? 0 : cm->seq_params.subsampling_x);
    int stride = (w + MIN_TX_SIZE - 1) >> MIN_TX_SIZE_LOG2;
  ...
  if (fLog) {
    ...
    if (value != 0) {
      fprintf(fLog,
        "\n\tSkipped TxBlock: row = %d, col = %d, blk_width = %d, blk_height = %d",
        row, col, w, h);
    } else {
      fprintf(fLog,
...
void update_txk_skip_array(const AV1_COMMON *cm, int mi_row, int mi_col,
  ...
  int x = (mi_col << MI_SIZE_LOG2) >> ((plane == 0) ? 0 : cm->seq_params.subsampling_x);
  int y = (mi_row << MI_SIZE_LOG2) >> ((plane == 0) ? 0 : cm->seq_params.subsampling_y);
  ...
  if (fLog) {
    fprintf(fLog,
      "\n\tSkipped TxBlock: row = %d, col = %d, tx_width = %d, tx_height = %d, plane = %d",
      ((mi_row << MI_SIZE_LOG2) + blk_row), ((mi_col << MI_SIZE_LOG2) + blk_col),
      tx_size_wide[tx_size], tx_size_high[tx_size], plane);
  }
}
