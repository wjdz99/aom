  ...
  if (sub_y == 0 && sub_x == 0) {
    ...
    for (int j = 0; j < height; j++) {
      for (int i = 0; i < width; i++) {
        ...
        output_q3[output_row_offset + i] = y_pix[pred_row_offset + i] << 3;
      }
...
void cfl_predict_block(MACROBLOCKD *const xd, uint8_t *dst, int dst_stride,
  ...
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
      ...
      dst[i] = clip_pixel(get_scaled_luma_q0(alpha_q3, y_pix_q3[i]) + dc_pred);
    }
    dst += dst_stride;
...
static INLINE void sub8x8_set_val(CFL_CTX *cfl, int row, int col, int val_high,
  ...
  assert((row + (val_high - 1)) * 2 + col + (val_wide - 1) < MAX_SUB8X8_VAL);
}
#endif  // CONFIG_DEBUG
