...
void cfl_predict_block(MACROBLOCKD *const xd, uint8_t *dst, int dst_stride,
  ...
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
      const int luma_diff_q10 = (((int)y_pix[i]) << 10) - avg_q10;
      const int scaled_luma_q13 = alpha_q3 * luma_diff_q10;

      const int pred = (scaled_luma_q13 + dc_pred_bias_q13) >> 13;
      assert(pred == (int)(alpha * (y_pix[i] - (avg_q10 / 1024.0)) +
                           (cfl->dc_pred_q7[plane - 1] / 128.0) + 0.5));
      // TODO(ltrudeau) Update for HBD
      assert(pred >= 0);
      assert(pred < 256);
      dst[i] = (uint8_t)pred;
    }
