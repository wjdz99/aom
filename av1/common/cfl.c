		...
		for (int i = 0; i < width; i++)
		{
			...
			if (cfl->use_up)
			{
				for (int k = 0; k < width; k++) //遍历上一行像素
				{
					...
					{
						if ((((i + 1) - rec_nb[k].positionx)*((i + 1) - rec_nb[k].positionx) + ((j + 1) - rec_nb[k].positiony)*((j + 1) - rec_nb[k].positiony)) < (((i + 1) - rec_nb[flag].positionx)*((i + 1) - rec_nb[flag].positionx) + ((j + 1) - rec_nb[flag].positiony)*((j + 1) - rec_nb[flag].positiony)))
						{
							flag = k;
						}
					}
			...
			if (cfl->use_left)
			{
				for (int k = 0; k < height; k++) //遍历左一列像素
				{
					...
					else if (abs(recon[i] - rec_nb[CFL_BUF_LINE + k].LumaPixel) == DValue)
					{
						if ((((i + 1) - rec_nb[CFL_BUF_LINE + k].positionx)*((i + 1) - rec_nb[CFL_BUF_LINE + k].positionx) + ((j + 1) - rec_nb[CFL_BUF_LINE + k].positiony)*((j + 1) - rec_nb[CFL_BUF_LINE + k].positiony)) < (((i + 1) - rec_nb[flag].positionx)*((i + 1) - rec_nb[flag].positionx) + ((j + 1) - rec_nb[flag].positiony)*((j + 1) - rec_nb[flag].positiony)))
						{
							flag = k + CFL_BUF_LINE;
						}
					}
...
static void cfl_load_search_res_lbd(const int16_t *search_res_cache, uint8_t *dst,
...
static void cfl_load_search_res_hbd(const int16_t *search_res_cache, uint16_t *dst,
	int dst_stride, int width, int height)
{
	...
	TX_SIZE tx_size, CFL_PRED_TYPE pred_plane)
{
	...
	if (is_cur_buf_hbd(xd))
	{
		...
		cfl_load_search_res_hbd(xd->cfl.search_res_cache[pred_plane], dst_16, dst_stride, width, height);
		return;
	}
	cfl_load_search_res_lbd(xd->cfl.search_res_cache[pred_plane], dst, dst_stride, width, height);
}
#else
...
static void cfl_store(CFL_CTX *cfl, const uint8_t *input, int input_stride,
  ...
  const int store_row = row << (tx_off_log2 - sub_y);//2-1
  const int store_col = col << (tx_off_log2 - sub_x);//2-1
...
void cfl_store_nb(MACROBLOCKD *const xd, BLOCK_SIZE bsize)
{
	...
	NeiborPix *recNb = cfl->recon_Neighbor;//保存相邻重建参考像素

	uint8_t *dst = &pd->dst.buf[0];
	const int dst_stride = pd->dst.stride;
	uint8_t *above_ref = dst - dst_stride;//上侧参考像素
	uint8_t *left_ref = dst - 1;//左侧参考像素
		...
		else
		{
			for (int i = 0; i < subWidth; i++)
			{
				recNb[i].LumaPixel = (above_ref[2 * i] + above_ref[2 * i + 1] + \
					(above_ref - ref_stride)[2 * i] + (above_ref - ref_stride)[2 * i + 1]) << 1;
				recNb[i].positionx = i;
				recNb[i].positiony = -1;
			}
		}
	...
	if (xd->chroma_left_available)
	{
		...
		else
		{
			for (int i = 0; i < subHeight; i++)
			{
				recNb[CFL_BUF_LINE + i].LumaPixel = (left_ref[0] + (left_ref + ref_stride)[0] +
					(left_ref - 1)[0] + (left_ref + ref_stride - 1)[0]) << 1;
				recNb[CFL_BUF_LINE + i].positionx = -1;
				recNb[CFL_BUF_LINE + i].positiony = i;
