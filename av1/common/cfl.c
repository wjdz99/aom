...
static INLINE void cfl_build_prediction(const int16_t *pred_buf_q3,
  ...
  if (cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    uint16_t *dst_16 = CONVERT_TO_SHORTPTR(dst);
    cfl_build_prediction_hbd(pred_buf_q3, dst_16, dst_stride, alpha_q3, dc_pred,
                             width, height, cur_buf->bit_depth);
  } else {
    cfl_build_prediction_lbd(pred_buf_q3, dst, dst_stride, alpha_q3, dc_pred,
                             width, height);
  }
}
#else
...
void cfl_predict_block(MACROBLOCKD *const xd, uint8_t *dst, int dst_stride,
  ...
  cfl_build_prediction(pred_buf_q3, dst, dst_stride, alpha_q3,
                       ...
                       tx_size_high[tx_size], xd->cur_buf);
}
...
static INLINE void sub8x8_set_val(CFL_CTX *cfl, int row, int col, int val_high,
  ...
  assert((row + (val_high - 1)) * 2 + col + (val_wide - 1) <
...
void cfl_store_tx(MACROBLOCKD *const xd, int row, int col, TX_SIZE tx_size,
  ...
  if (block_size_high[bsize] == 4 || block_size_wide[bsize] == 4) {
    ...
    assert((block_size_high[bsize] == 4 || !(col & 0x01)) &&
           (block_size_wide[bsize] == 4 || !(row & 0x01)));
