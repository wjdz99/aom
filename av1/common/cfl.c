...
static void cfl_compute_averages(CFL_CTX *cfl, TX_SIZE tx_size) {
  ...
  if (is_flat) {
    // When the block is flat, CfL turns into DC_PRED. To avoid this we change
    // the average. The average is changed such that, subtracting the
    // reconstructed luma pixel value from it will allow the smallest
    // code to offset the chroma DC_PRED by +/- 1.
    for (int i = 0; i < MAX_NUM_TXB; i++) {
      averages_q3[i] = AOMMAX(0, first_val - SMALLEST_ALPHA_ADDS_ONE);
    }
  }
