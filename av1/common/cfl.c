...
#if CONFIG_DEBUG
    sub8x8_set_val(cfl, row, col, tx_size);
    if (tx_size_wide[tx_size] < width)
      sub8x8_set_val(cfl, row, col + tx_size_wide_unit[tx_size], tx_size);
    if (tx_size_high[tx_size] < height)
      sub8x8_set_val(cfl, row + tx_size_high_unit[tx_size], col, tx_size);
#endif  // CONFIG_DEBUG
  }
