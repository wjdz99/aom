...
void cfl_init(CFL_CTX *cfl, AV1_COMMON *cm) {
  ...
  memset(cfl->sub8x8_val, 0, sizeof(uint8_t) * 4);
...
#if CONFIG_CHROMA_SUB8X8
static INLINE void sub8x8_adjust_offset(CFL_CTX *cfl, BLOCK_SIZE bsize,
...
#if CONFIG_DEBUG
    if (tx_size == TX_4X4) {
      cfl->sub8x8_val[row * 2 + col] = 1;
    } else if (tx_size == TX_4X8) {
      assert(row == 0);
      cfl->sub8x8_val[0 + col] = 1;
      cfl->sub8x8_val[2 + col] = 1;
    } else if (tx_size == TX_8X4) {
      assert(col == 0);
      cfl->sub8x8_val[row * 2 + 0] = 1;
      cfl->sub8x8_val[row * 2 + 1] = 1;
    }
#endif  // CONFIG_DEBUG
  }
#endif
...
void cfl_store_block(MACROBLOCKD *const xd, BLOCK_SIZE plane_bsize,
                     TX_SIZE tx_size) {
  CFL_CTX *const cfl = xd->cfl;
  struct macroblockd_plane *const pd = &xd->plane[AOM_PLANE_Y];
  int row = 0;
  ...
  if (plane_bsize < BLOCK_8X8) {
    sub8x8_adjust_offset(cfl, plane_bsize, &row, &col);
#if CONFIG_DEBUG
    if (plane_bsize == BLOCK_4X4) {
      cfl->sub8x8_val[row * 2 + col] = 1;
    } else if (plane_bsize == BLOCK_4X8) {
      assert(row == 0);
      cfl->sub8x8_val[0 + col] = 1;
      cfl->sub8x8_val[2 + col] = 1;
    } else if (plane_bsize == BLOCK_8X4) {
      assert(col == 0);
      cfl->sub8x8_val[row * 2 + 0] = 1;
      cfl->sub8x8_val[row * 2 + 1] = 1;
    }
...
#endif
  const int width =
      max_intra_block_width(xd, plane_bsize, AOM_PLANE_Y, tx_size);
  const int height =
      max_intra_block_height(xd, plane_bsize, AOM_PLANE_Y, tx_size);
...
void cfl_compute_parameters(MACROBLOCKD *const xd, TX_SIZE tx_size) {
  ...
  if (mbmi->sb_type < BLOCK_8X8) {
    assert(cfl->sub8x8_val[0] == 1);
    assert(cfl->sub8x8_val[1] == 1);
    assert(cfl->sub8x8_val[2] == 1);
    assert(cfl->sub8x8_val[3] == 1);
    memset(cfl->sub8x8_val, 0, sizeof(uint8_t) * 4);
  }
#endif  // CONFIG_DEBUG
#else
...
#if CONFIG_DEBUG
  if (mbmi->sb_type >= BLOCK_8X8) {
    if (cfl->y_width > cfl->uv_width << cfl->subsampling_x ||
        cfl->y_height > cfl->uv_height << cfl->subsampling_y)
      printf("%d <= %d  %d <= %d\n", cfl->y_width,
             cfl->uv_width << cfl->subsampling_x, cfl->y_height,
             cfl->uv_height << cfl->subsampling_y);
