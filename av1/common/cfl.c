...
void cfl_luma_subsampling_420_lbd_c(const uint8_t *input, int input_stride,
                                    int16_t *output_q3, int width, int height) {
  for (int j = 0; j < height; j += 2) {
    for (int i = 0; i < width; i += 2) {
      int bot = i + input_stride;
      output_q3[i >> 1] =
...
static void cfl_store(CFL_CTX *cfl, const uint8_t *input, int input_stride,
  ...
  const int store_height = tx_size_high[tx_size] >> sub_y;
  const int store_width = tx_size_wide[tx_size] >> sub_x;
  ...
  if (use_hbd) {
    ...
    get_subsample_hbd_fn(sub_x, sub_y, tx_size)(input_16, input_stride,
                                                pred_buf_q3);
    return;
  }
  get_subsample_lbd_fn(sub_x, sub_y, tx_size)(input, input_stride, pred_buf_q3);
}
