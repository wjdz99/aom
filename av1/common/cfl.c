...
void cfl_predict_block(MACROBLOCKD *const xd, uint8_t *dst, int dst_stride,
  ...
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
      const int luma_diff_q3 = (((int)y_pix[i]) << 3) - avg_q3;
      const int scaled_luma_q6 = alpha_q3 * luma_diff_q3;

      const int pred = (scaled_luma_q6 + dc_pred_bias_q6) >> 6;
      // TODO(ltrudeau) call clip_pixel_highbd when HBD is enabled.
      dst[i] = clip_pixel(pred);

      assert(pred == (int)(alpha * (y_pix[i] - (avg_q3 / 8.0)) +
