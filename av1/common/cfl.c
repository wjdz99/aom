...
void cfl_store_block(MACROBLOCKD *const xd, BLOCK_SIZE bsize, TX_SIZE tx_size) {
  ...
  if (block_size_high[bsize] == 4 || block_size_wide[bsize] == 4) {
    sub8x8_adjust_offset(cfl, &row, &col);
#if CONFIG_DEBUG
    const int unit_width = width >> tx_size_wide_log2[0];
    const int unit_height = height >> tx_size_high_log2[0];
    for (int i = 0; i < unit_height; i += tx_size_high_unit[tx_size])
      for (int j = 0; j < unit_width; j += tx_size_wide_unit[tx_size])
        sub8x8_set_val(cfl, row + i, col + j, tx_size);
#endif  // CONFIG_DEBUG
  }
