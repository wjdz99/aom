...
// Width of the sub8x8 validation buffer (in units of 4x4 blocks) with respect
// to chroma subsampling along x axis.
static const uint8_t val_block_size_wide[BLOCK_SIZES_ALL][2] = {
  { 0, 0 },  // BLOCK_2X2,
  { 0, 0 },  // BLOCK_2X4,
  { 0, 0 },  // BLOCK_4X2,
  { 1, 2 },  // BLOCK_4X4,
...
// Height of the sub8x8 validation buffer (in units of 4x4 blocks) with respect
// to chroma subsampling along y axis.
static const uint8_t val_block_size_high[BLOCK_SIZES_ALL][2] = {
  { 0, 0 },  // BLOCK_2X2,
  { 0, 0 },  // BLOCK_2X4,
  { 0, 0 },  // BLOCK_4X2,
  { 1, 2 },  // BLOCK_4X4,
...
static void sub8x8_set_val(CFL_CTX *cfl, int row, int col, TX_SIZE y_tx_size) {
  ...
  uint8_t *sub8x8_val = cfl->sub8x8_val + (row * CFL_SUB8X8_VAL_MI_SIZE + col);
  ...
  if (!is_first) {
    const uint8_t prev_store_counter = cfl->store_counter - 1;
    int found = 0;
...
void cfl_compute_parameters(MACROBLOCKD *const xd, TX_SIZE tx_size) {
  ...
  if (block_size_high[bsize] == 4 || block_size_wide[bsize] == 4) {
    ...
    const int val_wide = val_block_size_wide[bsize][cfl->subsampling_x];
    const int val_high = val_block_size_high[bsize][cfl->subsampling_y];
    assert(val_wide != 0);
    assert(val_high != 0);
