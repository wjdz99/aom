...
static INLINE void sum_above_row_lbd(const uint8_t *above_u,
                                     const uint8_t *above_v, int width,
...
static INLINE void sum_above_row_hbd(const uint16_t *above_u,
                                     const uint16_t *above_v, int width,
                                     int *out_sum_u, int *out_sum_v) {
  int sum_u = 0;
  int sum_v = 0;
  for (int i = 0; i < width; i++) {
...
static INLINE void sum_left_col_lbd(const uint8_t *left_u, int u_stride,
                                    const uint8_t *left_v, int v_stride,
...
#if CONFIG_HIGHBITDEPTH
static INLINE void sum_left_col_hbd(const uint16_t *left_u, int u_stride,
                                    const uint16_t *left_v, int v_stride,
                                    int height, int *out_sum_u,
                                    int *out_sum_v) {
  int sum_u = 0;
  int sum_v = 0;
...
static void cfl_dc_pred(MACROBLOCKD *xd, BLOCK_SIZE plane_bsize,
  ...
  const int num_pel = tx_width + tx_height;
...
#endif
        sum_above_row(xd, b_i, tx_width, &sum_u, &sum_v);
      } else {
        const int base = 128 << (xd->bd - 8);
        sum_u = tx_width * (base - 1);
        sum_v = tx_width * (base - 1);
      }
...
#endif
        sum_left_col(xd, b_j, tx_height, &sum_u, &sum_v);
      } else {
        const int base = 128 << (xd->bd - 8);
        sum_u += tx_height * (base + 1);
        sum_v += tx_height * (base + 1);
      }
...
static INLINE void cfl_build_prediction(const int16_t *pred_buf_q3,
                                        ...
                                        const int16_t *dc_pred, int use_hbd,
                                        int bit_depth) {
#if CONFIG_HIGHBITDEPTH
  if (use_hbd) {
...
void cfl_predict_block(MACROBLOCKD *const xd, uint8_t *dst, int dst_stride,
  ...
  cfl_build_prediction(pred_buf_q3, dst, dst_stride, tx_size_wide[tx_size],
                       tx_size_high[tx_size], alpha_q3, dc_pred, use_hbd,
                       xd->bd);
}
