...
void cfl_init(CFL_CTX *cfl, AV1_COMMON *cm) {
  ...
  static INLINE void cfl_luma_subsampling_420(
      const uint8_t *y_pix, uint8_t *output, int width, int height) {
  ...
  static INLINE void cfl_luma_subsampling_444(
      const uint8_t *y_pix, uint8_t *output, int width, int height) {
    for (int j = 0; j < height; j++) {
      for (int i = 0; i < width; i++) {
        output[i] = y_pix[i];
  ...
  static void cfl_load(CFL_CTX * cfl, int row, int col, int width, int height) {
    ...
    if (sub_y == 0 && sub_x == 0) {
      // TODO(ltrudeau) convert to uint16 to add HBD support
      const uint8_t *y_pix =
          cfl->y_pix + ((row * MAX_SB_SIZE + col) << off_log2);
...
// ..
  ...
  static void cfl_compute_averages(CFL_CTX * cfl, TX_SIZE tx_size) {
    ...
    assert(a <= MAX_NUM_TXB * MAX_NUM_TXB);
  }
...
#if CONFIG_DEBUG
  static INLINE void sub8x8_set_val(CFL_CTX * cfl, int row, int col,
                                    int val_high, int val_wide) {
