...
static const int seg_feature_data_signed[NUM_SEG_CATEGORIES][SEG_LVL_MAX] = { {0, 0}, {1, 1} };
...
int av1_extract_segment_id_from_map(int segment_id_in_map, SEG_CATEGORIES seg_cat_idx) {
  return seg_cat_idx == ACTIVE_SEG_IDX ? (segment_id_in_map & ((1 << MAX_LOG2_ACTIVE_SEGMENTS) - 1))
    : ((segment_id_in_map >> MAX_LOG2_ACTIVE_SEGMENTS) & ((1 << MAX_LOG2_QUALITY_SEGMENTS) - 1));
}
void av1_store_segment_id_into_map(int segment_id, uint8_t* segment_id_in_map, SEG_CATEGORIES seg_cat_idx) {
  if (seg_cat_idx == ACTIVE_SEG_IDX) {
    *segment_id_in_map = (*segment_id_in_map & ~((1 << MAX_LOG2_ACTIVE_SEGMENTS) - 1)) |
      (segment_id & ((1 << MAX_LOG2_ACTIVE_SEGMENTS) - 1));
  } else {
    *segment_id_in_map = (*segment_id_in_map & ((~((1 << MAX_LOG2_QUALITY_SEGMENTS) - 1)) << MAX_LOG2_ACTIVE_SEGMENTS)) |
      ((segment_id & ((1 << MAX_LOG2_QUALITY_SEGMENTS) - 1)) << MAX_LOG2_ACTIVE_SEGMENTS);
  }
}
...
const aom_tree_index av1_segment_tree[MAX_SEGMENTS - 1]
[TREE_SIZE(MAX_SEGMENTS)] = {
  { 0, -1, },
  { 0, 2, -1, -2, },
  { 2, 4, 0, -1, -2, -3, },
  { 2, 4, 0, -1, -2, 6, -3, -4, },
