...
const vpx_tree_index av1_mv_joint_tree[TREE_SIZE(MV_JOINTS)] = {
  -MV_JOINT_ZERO, 2, -MV_JOINT_HNZVZ, 4, -MV_JOINT_HZVNZ, -MV_JOINT_HNZVNZ
};

/* clang-format off */
const vpx_tree_index av1_mv_class_tree[TREE_SIZE(MV_CLASSES)] = {
  -MV_CLASS_0, 2,
  -MV_CLASS_1, 4,
  6, 8,
...
const vpx_tree_index av1_mv_class0_tree[TREE_SIZE(CLASS0_SIZE)] = {
  -0, -1,
};
...
void av1_adapt_mv_probs(VP10_COMMON *cm, int allow_hp) {
  int i, j;
  ...
  vpx_tree_merge_probs(av1_mv_joint_tree, pre_fc->joints, counts->joints,
                       fc->joints);
  ...
  for (i = 0; i < 2; ++i) {
    ...
    vpx_tree_merge_probs(av1_mv_class_tree, pre_comp->classes, c->classes,
                         comp->classes);
    vpx_tree_merge_probs(av1_mv_class0_tree, pre_comp->class0, c->class0,
                         comp->class0);
    ...
    for (j = 0; j < CLASS0_SIZE; ++j)
      vpx_tree_merge_probs(av1_mv_fp_tree, pre_comp->class0_fp[j],
                           c->class0_fp[j], comp->class0_fp[j]);

    vpx_tree_merge_probs(av1_mv_fp_tree, pre_comp->fp, c->fp, comp->fp);
...
void av1_init_mv_probs(VP10_COMMON *cm) { cm->fc->nmvc = default_nmv_context; }
