...
void av1_resize_and_extend_frame_c(const YV12_BUFFER_CONFIG *src,
  ...
  switch (filter) {
    case BILINEAR:
      kernel = av1_bilinear_filters;
      break;
    case EIGHTTAP_SMOOTH:
      kernel = av1_sub_pel_filters_8smooth;
      break;
    case EIGHTTAP_REGULAR:
      kernel = av1_sub_pel_filters_8;
      break;
    default:
      assert(false); // Other filters are not expected to be used here.
  }
    ...
    for (int y = 0; y < dst_h; y += 16) {
      ...
      for (int x = 0; x < dst_w; x += 16) {
        ...
        const uint8_t *src_ptr =
            src->buffers[i] + y * src_h / dst_h * src_stride + x * src_w / dst_w;
