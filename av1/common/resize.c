...
void av1_resize_and_extend_plane_c(const YV12_BUFFER_CONFIG *src,
  ...
  assert(plane_index < MAX_MB_PLANE);
  const PLANE_TYPE plane_type = plane_index > 0;
  const int src_w = src->crop_widths[plane_type];
  const int src_h = src->crop_heights[plane_type];
  const int dst_w = dst->crop_widths[plane_type];
  const int dst_h = dst->crop_heights[plane_type];
  const int src_stride = src->strides[plane_type];
  const int dst_stride = dst->strides[plane_type];
  const uint8_t *const src_buffer = src->buffers[plane_index];
  uint8_t *const dst_buffer = dst->buffers[plane_index];
