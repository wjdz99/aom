...
#define UPASCALE_PROC_UNIT 64  // Source step (roughly)
#define UPASCALE_PROC_UNIT_SCALE (UPASCALE_PROC_UNIT / SCALE_NUMERATOR)

static int get_upscale_convolve_params(int inlength, int outlength,
                                       int outstart, int32_t *x0_qn,
                                       int32_t *x_step_qn) {
  assert(outlength >= inlength);
  *x_step_qn = (((uint32_t)inlength << INTERP_PRECISION_BITS) + outlength / 2) /
               outlength;
  *x0_qn = (-((int32_t)(outlength - inlength) << (INTERP_PRECISION_BITS - 1)) +
            outlength / 2) /
               outlength +
           outstart * (*x_step_qn) + SUBPEL_INTERP_EXTRA_OFF;
  int off = (*x0_qn >> INTERP_PRECISION_BITS);
  *x0_qn = (int32_t)((uint32_t)(*x0_qn) & INTERP_PRECISION_MASK);
  return off;
}

static void convolve_horzonly_superres_c(const uint8_t *src, uint8_t *dst,
...
static void interpolate_normative_core(const uint8_t *const src, int inlength,
                                       uint8_t *dst, int outlength,
                                       int superres_denom,
                                       const int16_t *interp_filters,
                                       int interp_taps) {
  const int oproc_unit = UPASCALE_PROC_UNIT_SCALE * superres_denom;
  int32_t x_step_qn, x0_qn;
  const uint8_t *srcp = src;
  srcp +=
      get_upscale_convolve_params(inlength, outlength, 0, &x0_qn, &x_step_qn);
  for (int op = 0; op < outlength; op += oproc_unit) {
    int olen = AOMMIN(oproc_unit, outlength - op);
    convolve_horzonly_superres_c(srcp, dst + op, interp_filters, interp_taps,
                                 x0_qn, x_step_qn, olen);
    x0_qn += x_step_qn * olen;
    // Need to check whether ilen is always UPASCALE_PROC_UNIT.
    // In any case it should not deviate by more than +/- 1.
    const int ilen = (x0_qn >> INTERP_PRECISION_BITS);
    srcp += ilen;
    x0_qn = x0_qn & INTERP_PRECISION_MASK;
  }
}
...
static void interpolate_normative(const uint8_t *const input, int inlength,
                                  uint8_t *output, int outlength,
                                  int superres_denom) {
  uint8_t *intbuf_ = (uint8_t *)aom_malloc(
      sizeof(uint8_t) * (inlength + UPSCALE_NORMATIVE_TAPS + 2));
  uint8_t *intbuf = intbuf_ + UPSCALE_NORMATIVE_TAPS / 2 + 1;
  memcpy(intbuf, input, sizeof(*intbuf) * inlength);
  for (int k = 0; k < UPSCALE_NORMATIVE_TAPS / 2 + 1; ++k) {
    intbuf[-k - 1] = intbuf[0];
    intbuf[inlength + k] = intbuf[inlength - 1];
  }
  interpolate_normative_core(intbuf, inlength, output, outlength,
...
#if CONFIG_FRAME_SUPERRES && CONFIG_LOOP_RESTORATION
static void highbd_convolve_horzonly_superres_c(const uint16_t *src,
                                                uint16_t *dst,
                                                const int16_t *x_filters,
                                                int interp_taps, int x0_qn,
...
static void highbd_interpolate_normative_core(const uint16_t *const src,
  ...
  for (int op = 0; op < outlength; op += oproc_unit) {
    int olen = AOMMIN(oproc_unit, outlength - op);
    highbd_convolve_horzonly_superres_c(srcp, dst + op, interp_filters,
                                        interp_taps, x0_qn, x_step_qn, olen,
                                        bd);
    x0_qn += x_step_qn * olen;
    const int ilen = (x0_qn >> INTERP_PRECISION_BITS);
    srcp += ilen;
...
static void highbd_interpolate_normative(const uint16_t *const input,
                                         ...
                                         int superres_denom) {
  uint16_t *intbuf_ = (uint16_t *)aom_malloc(
      sizeof(uint16_t) * (inlength + UPSCALE_NORMATIVE_TAPS));
  uint16_t *intbuf = intbuf_ + UPSCALE_NORMATIVE_TAPS / 2;
  memcpy(intbuf, input, sizeof(*intbuf) * inlength);
  for (int k = 0; k < UPSCALE_NORMATIVE_TAPS / 2; ++k) {
    intbuf[-k - 1] = intbuf[0];
    intbuf[inlength + k] = intbuf[inlength - 1];
  }
  highbd_interpolate_normative_core(intbuf, inlength, output, outlength,
