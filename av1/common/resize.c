...
void av1_superres_upscale(AV1_COMMON *cm, BufferPool *const pool) {
  ...
  if (pool != NULL) {
    ...
    if (aom_realloc_frame_buffer(frame_to_show, cm->superres_upscaled_width, cm->superres_upscaled_height,
                                 cm->subsampling_x, cm->subsampling_y,
#ifdef CONFIG_HIGHBITDEPTH
                                 cm->use_highbitdepth,
#endif  // CONFIG_HIGHBITDEPTH
    ...
    if (aom_alloc_frame_buffer(frame_to_show, cm->superres_upscaled_width, cm->superres_upscaled_height,
                               cm->subsampling_x, cm->subsampling_y,
#ifdef CONFIG_HIGHBITDEPTH
                               cm->use_highbitdepth,
#endif  // CONFIG_HIGHBITDEPTH
  ...
  assert(frame_to_show->y_width == cm->superres_upscaled_width);
  assert(frame_to_show->y_height == cm->superres_upscaled_height);
  assert(copy_buffer.y_width != cm->superres_upscaled_width || copy_buffer.y_height != cm->superres_upscaled_height);
#if CONFIG_HIGHBITDEPTH
