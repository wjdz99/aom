...
YV12_BUFFER_CONFIG *av1_scale_if_required_fast(AV1_COMMON *cm,
  ...
  if (cm->mi_cols * MI_SIZE != unscaled->y_crop_width ||
      cm->mi_rows * MI_SIZE != unscaled->y_crop_height) {
...
void av1_superres_upscale(AV1_COMMON *cm, BufferPool *const pool) {
  ...
  assert(copy_buffer.y_crop_width != cm->superres_upscaled_width ||
         copy_buffer.y_crop_height != cm->superres_upscaled_height);
#if CONFIG_HIGHBITDEPTH
