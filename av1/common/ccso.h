...
void apply_ccso_filter(AV1_COMMON *cm, MACROBLOCKD *xd, const int plane,
                       ...
                       const int dst_stride, int8_t *filter_offset,
...
typedef void (*ccso_filter_block_func)(
    ...
    const int *dst_stride_idx, int y_uv_hori_scale, int y_uv_vert_scale,
    int pad_stride, int quant_step_size, int inv_quant_step, int *rec_idx,
    int maxval);
