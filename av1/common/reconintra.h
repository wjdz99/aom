...
static INLINE int av1_allow_intrabc(const FRAME_TYPE frame_type,
                                    const int allow_screen_content_tools,
                                    const int allow_intrabc) {
  // TODO(david.turner@argondesign.com) Pass this a CurrentFrame once
  // all the parameters are inside that struct.  Then change it to call
  // frame_is_intra_only(), passing that a CurrentFrame.
  return (frame_type == KEY_FRAME || frame_type == INTRA_ONLY_FRAME) &&
         allow_screen_content_tools && allow_intrabc;
}
