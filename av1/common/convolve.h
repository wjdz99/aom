...
static INLINE void av1_get_convolve_filter_params(
    InterpFilters interp_filters, int ref_idx, int avoid_1212,
    InterpFilterParams *params_x, InterpFilterParams *params_y) {
#if CONFIG_DUAL_FILTER
  InterpFilter filter_x =
      av1_extract_interp_filter(interp_filters, 1 + 2 * ref_idx);
  InterpFilter filter_y =
      av1_extract_interp_filter(interp_filters, 0 + 2 * ref_idx);
#else
  assert(ref_idx == 0);
