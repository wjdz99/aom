...
void av1_ext_highbd_warp_affine_c(const int32_t *mat, const uint16_t *ref,
  ...
  const int bsize_warp = av1_get_new_warp_bsize(mat);
  const int half_bsize_warp = (bsize_warp >> 1);
  for (int i = p_row; i < p_row + p_height; i += bsize_warp) {
    for (int j = p_col; j < p_col + p_width; j += bsize_warp) {
      // Calculate the center of this 4x4 block,
      // project to luma coordinates (if in a subsampled chroma plane),
      // apply the affine transformation,
      // then convert back to the original coordinates (if necessary)
      const int32_t src_x = (j + half_bsize_warp) << subsampling_x;
      const int32_t src_y = (i + half_bsize_warp) << subsampling_y;
      ...
      for (int k = -(half_bsize_warp + 3); k < (half_bsize_warp + 4); ++k) {
        const int iy = clamp(iy4 + k, 0, height - 1);

        for (int l = -half_bsize_warp; l < half_bsize_warp; ++l) {
          int ix = ix4 + l - 3;
          ...
          im_block[(k + (half_bsize_warp + 3)) * bsize_warp +
                   (l + half_bsize_warp)] = sum;
        }
      }
      ...
      for (int k = -half_bsize_warp;
           k < AOMMIN(half_bsize_warp, p_row + p_height - i - half_bsize_warp);
           ++k) {
        for (int l = -half_bsize_warp;
             l < AOMMIN(half_bsize_warp, p_col + p_width - j - half_bsize_warp);
             ++l) {
          int32_t sum = 1 << offset_bits_vert;
          for (int m = 0; m < 8; ++m) {
            sum += im_block[(k + m + half_bsize_warp) * bsize_warp +
                            (l + half_bsize_warp)] *
                   coeffs_y[m];
          }
          ...
          if (conv_params->is_compound) {
            CONV_BUF_TYPE *p =
                &conv_params->dst[(i - p_row + k + half_bsize_warp) *
                                      conv_params->dst_stride +
                                  (j - p_col + l + half_bsize_warp)];
            sum = ROUND_POWER_OF_TWO(sum, reduce_bits_vert);
            if (conv_params->do_average) {
              uint16_t *dst16 =
                  &pred[(i - p_row + k + half_bsize_warp) * p_stride +
                        (j - p_col + l + half_bsize_warp)];
              int32_t tmp32 = *p;
            ...
            uint16_t *p = &pred[(i - p_row + k + half_bsize_warp) * p_stride +
                                (j - p_col + l + half_bsize_warp)];
            sum = ROUND_POWER_OF_TWO(sum, reduce_bits_vert);
...
#if CONFIG_EXT_WARP
  if (!av1_is_affine_shear_allowed(alpha, beta, gamma, delta))
...
#if CONFIG_EXT_WARP
int av1_get_new_warp_bsize(const int32_t *mat) {
  int bsize_warp = 4;
  const int32_t dst_x =
      AOMMAX(0,
             AOMMAX(mat[2] * 4, AOMMAX(mat[3] * 4, mat[2] * 4 + mat[3] * 4))) -
      AOMMIN(0,
             AOMMIN(mat[2] * 4, AOMMIN(mat[3] * 4, mat[2] * 4 + mat[3] * 4)));
  const int32_t dst_y =
      AOMMAX(0,
             AOMMAX(mat[4] * 4, AOMMAX(mat[5] * 4, mat[4] * 4 + mat[5] * 4))) -
      AOMMIN(0,
             AOMMIN(mat[4] * 4, AOMMIN(mat[5] * 4, mat[4] * 4 + mat[5] * 4)));
  int32_t bounding_x4 = (dst_x >> WARPEDMODEL_PREC_BITS) + 11;
  int32_t bounding_y4 = (dst_y >> WARPEDMODEL_PREC_BITS) + 11;

  if ((bounding_x4 > 21) || (bounding_y4 > 21)) {
    bsize_warp = 8;
  }
  return bsize_warp;
}

void av1_ext_warp_affine_c(const int32_t *mat, const uint8_t *ref, int width,
                           int height, int stride, uint8_t *pred, int p_col,
                           int p_row, int p_width, int p_height, int p_stride,
                           int subsampling_x, int subsampling_y,
                           ConvolveParams *conv_params) {
  int16_t im_block[15 * 8];
  const int bd = 8;
      ...
      for (int k = -half_bsize_warp;
        ...
        for (int l = -half_bsize_warp;
          ...
          if (conv_params->is_compound) {
            ...
            uint8_t *p = &pred[(i - p_row + k + half_bsize_warp) * p_stride +
                               (j - p_col + l + half_bsize_warp)];
            sum = ROUND_POWER_OF_TWO(sum, reduce_bits_vert);
            assert(0 <= sum && sum < (1 << (bd + 2)));
