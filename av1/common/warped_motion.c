...
#define LS_STEP 8
#define LS_SUM(a) ((a)*4 + 2)
#define LS_PRODUCT(a, b) \
  ((a) * (b)*4 + ((a) + (b)) * 2 * LS_STEP + LS_STEP * LS_STEP)
#define LS_SQUARE(a) ((a) * (a)*4 + (a)*4 * LS_STEP + LS_STEP * LS_STEP)
...
static int find_affine_int(const int np, int *pts1, int *pts2, BLOCK_SIZE bsize,
  ...
  for (i = 0; i < np && n < LEAST_SQUARES_SAMPLES_MAX; ++i) {
    ...
    if (abs(sx - dx) < LEAST_SQUARES_MV_MAX &&
      ...
      A[0][0] += LS_SQUARE(sx);
      A[0][1] += LS_PRODUCT(sx, sy);
      A[1][1] += LS_SQUARE(sy);
      Bx[0] += LS_PRODUCT(sx, dx);
      Bx[1] += LS_PRODUCT(sy, dx);
      By[0] += LS_PRODUCT(sx, dy);
      By[1] += LS_PRODUCT(sy, dy);
      n++;
    }
  }
  int64_t Px[2], Py[2];
