...
static int64_t highbd_warp_error(
  ...
  int64_t gm_sumerr = 0;
  uint8_t *tmp = aom_malloc(p_width * p_height * sizeof(*tmp));
  if (!tmp) return INT64_MAX;

  highbd_warp_plane(wm, ref8, width, height, stride, tmp, p_col, p_row, p_width,
                    p_height, p_width, subsampling_x, subsampling_y, x_scale,
                    y_scale, bd, 0);

  gm_sumerr = highbd_frame_error(tmp, p_width, dst8, p_col, p_row, p_width,
