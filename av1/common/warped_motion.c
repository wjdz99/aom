...
void av1_warp_rotation(MB_MODE_INFO *mi, int8_t rotation, int mi_x, int mi_y) {
  ...
  const int cosine_val = cosine_values[rotation];
  ...
  if (mi->wm_params.wmtype == IDENTITY) {
    ...
    mi->wm_params.wmmat[0] =
        ROUND_POWER_OF_TWO_SIGNED((-mi_x * cosine_val) + (mi_y * sine_val) +
                                      (mi_x << SINE_PRECISION_BITS) +
                                      ((mv.col << SINE_PRECISION_BITS) / 8),
                                  precision_value);
    mi->wm_params.wmmat[1] =
        ROUND_POWER_OF_TWO_SIGNED((-mi_x * sine_val) - (mi_y * cosine_val) +
                                      (mi_y << SINE_PRECISION_BITS) +
                                      ((mv.row << SINE_PRECISION_BITS) / 8),
                                  precision_value);
    mi->wm_params.wmmat[2] = ROUND_POWER_OF_TWO(cosine_val, precision_value);
    mi->wm_params.wmmat[3] =
        ROUND_POWER_OF_TWO_SIGNED(-sine_val, precision_value);
    mi->wm_params.wmmat[4] =
        ROUND_POWER_OF_TWO_SIGNED(sine_val, precision_value);
    mi->wm_params.wmmat[5] = ROUND_POWER_OF_TWO(cosine_val, precision_value);
    ...
    mi->wm_params.wmmat[0] =
        ROUND_POWER_OF_TWO_SIGNED(
            (((matrix[2] * mi_y) - (matrix[3] * mi_x)) * sine_val) +
    ...
    mi->wm_params.wmmat[1] =
        ROUND_POWER_OF_TWO_SIGNED(
            (((matrix[4] * mi_y) - (matrix[5] * mi_x)) * sine_val) +
