...
static int find_affine_dbl(const int np, int *pts1, int *pts2,
  ...
  H[8] = 1;
  wm->wmtype = AFFINE;
  wm->wmmat[4] = (int32_t)lrint(H[4] * (1 << WARPEDMODEL_PREC_BITS));
  wm->wmmat[5] = (int32_t)lrint(H[5] * (1 << WARPEDMODEL_PREC_BITS));
  wm->wmmat[2] = (int32_t)lrint(H[2] * (1 << WARPEDMODEL_PREC_BITS));
  wm->wmmat[3] = (int32_t)lrint(H[3] * (1 << WARPEDMODEL_PREC_BITS));
  wm->wmmat[0] = (int32_t)lrint(H[0] * (1 << WARPEDMODEL_PREC_BITS));
  wm->wmmat[1] = (int32_t)lrint(H[1] * (1 << WARPEDMODEL_PREC_BITS));
...
int find_projection(const int np, int *pts1, int *pts2,
  ...
  switch (wm_params->wmtype) {
    case AFFINE:
      result = find_affine_int(np, pts1, pts2, wm_params);
      break;
    // case ROTZOOM: result = find_rotzoom_dbl(np, pts1, pts2, wm_params);
    // break;
