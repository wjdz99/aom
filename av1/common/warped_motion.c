...
int globalmv_rotation_allowed(const MACROBLOCKD *xd) {
  ...
  assert(xd->global_motion_nrs[mbmi->ref_frame_nrs[0]].invalid == 
         xd->global_motion[mbmi->ref_frame[0]].invalid);
  assert(xd->global_motion_nrs[mbmi->ref_frame_nrs[0]].wmtype == 
         xd->global_motion[mbmi->ref_frame[0]].wmtype);
  if (xd->global_motion_nrs[mbmi->ref_frame_nrs[0]].invalid) return 0;
  if (xd->global_motion_nrs[mbmi->ref_frame_nrs[0]].wmtype == IDENTITY) return 0;
#else
