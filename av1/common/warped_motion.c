...
static void warp_plane_old(WarpedMotionParams *wm, uint8_t *ref, int width,
  ...
  for (i = p_row; i < p_row + p_height; ++i) {
    for (j = p_col; j < p_col + p_width; ++j) {
      ...
      in[0] = j;
      in[1] = i;
      projectpoints(wm->wmmat, in, out, 1, 2, 2, subsampling_x,
                    subsampling_y);
      out[0] = ROUND_POWER_OF_TWO_SIGNED(out[0] * x_scale, 4);
      out[1] = ROUND_POWER_OF_TWO_SIGNED(out[1] * y_scale, 4);
      if (ref_frm)
...
static void warp_plane(WarpedMotionParams *wm, uint8_t *ref, int width,
  ...
  if (wm->wmtype == ROTZOOM || wm->wmtype == AFFINE) {
    ...
    if ((4*abs(alpha) + 7*abs(beta) > 1) || (4*abs(gamma) + 7*abs(delta) > 1)) {
      assert(0 && "Warped motion model is incompatible with new warp filter");
      warp_plane_old(wm, ref, width, height, stride, pred, p_col,
                     p_row, p_width, p_height, p_stride,
                     subsampling_x, subsampling_y, x_scale,
                     y_scale, ref_frm);
      return;
    }
