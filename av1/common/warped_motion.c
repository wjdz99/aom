...
static void highbd_warp_plane(const WarpedMotionParams *const wm,
                              const uint8_t *const ref8, int width, int height,
                              int stride, const uint8_t *const pred8, int p_col,
  ...
  if ((wm->wmtype == ROTZOOM || wm->wmtype == AFFINE) && x_scale == 16 &&
    ...
    const uint16_t *const ref = CONVERT_TO_SHORTPTR(ref8);
    const uint16_t *const pred = CONVERT_TO_SHORTPTR(pred8);
...
static double highbd_warp_erroradv(
    const WarpedMotionParams *const wm, const uint8_t *const ref8, int width,
    int height, int stride, const uint8_t *const dst8, int p_col, int p_row,
    int p_width, int p_height, int p_stride, int subsampling_x,
    int subsampling_y, int x_scale, int y_scale, int bd) {
