...
static int16_t resolve_divisor_gen(uint64_t D, int log2bytes, int16_t *shift) {
  static const uint64_t B[] = { 0x2,    0xC,        0xF0,
                                0xFF00, 0xFFFF0000, 0xFFFFFFFF00000000 };
  static const int S[] = { 1, 2, 4, 8, 16, 32 };
  int64_t e, f, x = D;
  *shift = 0;
  // First compute floor(log2(D)) into *shift
  for (int i = log2bytes - 1; i >= 0; --i) {
    if (x & B[i]) {
      x >>= S[i];
      *shift |= S[i];
    }
...
static int16_t resolve_divisor_64(uint64_t D, int16_t *shift) {
  return resolve_divisor_gen((uint64_t)D, 6, shift);
}

static int16_t resolve_divisor_32(uint32_t D, int16_t *shift) {
  return resolve_divisor_gen((uint64_t)D, 5, shift);
}
...
static int get_shear_params(WarpedMotionParams *wm, int32_t *alpha,
  ...
  *beta = mat[3];
#if APPROXIMATE_DIVISOR
  y = resolve_divisor_32(mat[2], &shift);
  v = ((int64_t)mat[4] << WARPEDMODEL_PREC_BITS) * y;
  *gamma = ROUND_POWER_OF_TWO_64(v, shift);
  v = ((int64_t)mat[3] * mat[4]) * y;
  *delta =
      mat[5] - ROUND_POWER_OF_TWO_64(v, shift) - (1 << WARPEDMODEL_PREC_BITS);
#else
...
static int find_affine_int(const int np, int *pts1, int *pts2,
  ...
  int64_t yDet;
  int16_t shift;
  yDet = resolve_divisor_64(Det, &shift) * sgnDet;
  // printf("yDet %d, shift %d Px %ld %ld %ld\n", yDet, shift, Px[0], Px[1],
  // Px[2]);
  if (shift > WARPEDMODEL_PREC_BITS) {
    shift -= WARPEDMODEL_PREC_BITS;
  } else {
    yDet <<= WARPEDMODEL_PREC_BITS;
  }
  v = Px[0] * (int64_t)yDet * sgnDet;
  wm->wmmat[2] = ROUND_POWER_OF_TWO_SIGNED_64(v, shift);
  v = Px[1] * (int64_t)yDet * sgnDet;
  wm->wmmat[3] = ROUND_POWER_OF_TWO_SIGNED_64(v, shift);
  v = Px[2] * (int64_t)yDet * sgnDet;
