...
static int find_affine_int(int np, int *pts1, int *pts2, BLOCK_SIZE bsize,
  ...
  // Note: In the vx, vy expressions below, the max value of 2nd and 3rd
  // terms are each (2^16 - 1) * (2^13 - 1). That leaves enough room for the
  // first term to ensure that the max value of vx, vy is less than 2^31 - 1.
  // Likewise, ther min value is  greater than -(2^31 - 1). So they safely
  // fit in 32 bits prior to clamping.
