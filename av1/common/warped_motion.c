...
static int64_t segmented_frame_error(const uint8_t *const ref, int stride,
                                     ...
                                     int segment_map_h) {
  // sarahparker
  assert(((p_width + (WARP_ERROR_BLOCK >> 1)) / WARP_ERROR_BLOCK) ==
         segment_map_w);
  assert(((p_height + (WARP_ERROR_BLOCK >> 1)) / WARP_ERROR_BLOCK) ==
         segment_map_h);
  int patch_w, patch_h;
  ...
  for (int i = 0; i < p_height; i += WARP_ERROR_BLOCK) {
    for (int j = 0; j < p_width; j += WARP_ERROR_BLOCK) {
      ...
      // model
      if (!segment_map[i * segment_map_w + j]) continue;
      ...
      sum_error +=
          frame_error(ref + j + i * p_stride, stride, dst + j + i * p_stride,
...
static int64_t warp_error(WarpedMotionParams *wm, const uint8_t *const ref,
  ...
  for (int i = p_row; i < p_row + p_height; i += WARP_ERROR_BLOCK) {
    for (int j = p_col; j < p_col + p_width; j += WARP_ERROR_BLOCK) {
      int seg_x = j / WARP_ERROR_BLOCK;
      int seg_y = i / WARP_ERROR_BLOCK;
      // Only compute the error if this block contains inliers from the motion
      // model
      if (!segment_map[i * segment_map_w + j]) continue;
      // avoid warping extra 8x8 blocks in the padded region of the frame
