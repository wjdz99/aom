...
void av1_warp_rotation(MB_MODE_INFO *mi, int8_t rotation, int center_x,
  ...
  if (mi->wm_params.wmtype == IDENTITY) {
    ...
    mi->wm_params.wmmat[0] =
        RIGHT_SIGNED_SHIFT((-center_x * cosine_val) + (center_y * sine_val) +
                               (center_x << SINE_PRECISION_BITS) +
                               (mv.col << (SINE_PRECISION_BITS - 3)),
                           -precision_value);
    mi->wm_params.wmmat[1] =
        RIGHT_SIGNED_SHIFT((-center_x * sine_val) - (center_y * cosine_val) +
                               (center_y << SINE_PRECISION_BITS) +
                               (mv.row << (SINE_PRECISION_BITS - 3)),
                           -precision_value);
    mi->wm_params.wmmat[2] = RIGHT_SIGNED_SHIFT(cosine_val, -precision_value);
    mi->wm_params.wmmat[3] = RIGHT_SIGNED_SHIFT(-sine_val, -precision_value);
    mi->wm_params.wmmat[4] = RIGHT_SIGNED_SHIFT(sine_val, precision_value);
    mi->wm_params.wmmat[5] = RIGHT_SIGNED_SHIFT(cosine_val, -precision_value);
