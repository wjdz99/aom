...
static INLINE int scaled_x(int val, const struct scale_factors *sf) {
  const int off = (sf->x_scale_fp - (1 << REF_SCALE_SHIFT)) / 2;
...
static INLINE int scaled_y(int val, const struct scale_factors *sf) {
  const int off = (sf->y_scale_fp - (1 << REF_SCALE_SHIFT)) / 2;
...
static int get_coarse_point_scale_factor(int other_size, int this_size) {
  ...
  // Hardware implementations can calculate scale factor in device driver
  // and use multiplication and shifting on hardware instead of division.
  return ((other_size << (SCALE_EXTRA_BITS + 4)) + this_size / 2) / this_size;
}
...
void av1_setup_scale_factors_for_frame(struct scale_factors *sf, int other_w,
  ...
  // sf->x_step_q4 = scaled_x(SUBPEL_SHIFTS, sf) - scaled_x(0, sf);
  // sf->y_step_q4 = scaled_y(SUBPEL_SHIFTS, sf) - scaled_y(0, sf);
