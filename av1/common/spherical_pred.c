...
void av1_get_pred_erp(int block_x, int block_y, int block_width,
  ...
  sphere_polar_to_carte(&block_polar, &blcok_v_prime);

  carte_vectors_cross_product(&block_v, &blcok_v_prime, &k);
  if (delta_phi != 0 || delta_theta != 0) {
    // k would be (0, 0, 0) if v and v_prime are the same
    normalize_carte_vector(&k);
  }

  double product = carte_vectors_dot_product(&block_v, &blcok_v_prime);
