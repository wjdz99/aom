...
static double cal_search_step_theta(double phi, double step_theta) {
  const double max_step_theta = 0.1 * PI;

  if (cos(phi) > 0.00001) {
    return AOMMIN(max_step_theta, 1 / cos(phi) * step_theta);
  } else {
    return max_step_theta;
  }
}
