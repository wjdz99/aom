...
void av1_init_new_ref_frame_map(AV1_COMMON *const cm, 
  ...
  for (int i = 0; i < REF_FRAMES; i++) {
    ...
    for (MV_REFERENCE_FRAME named_ref; named_ref <= ALTREF_FRAME; named_ref++) {
  ...
  cm->new_ref_frame_map.n_total_refs = n_ranked;
  int n_future = 0;
  int n_past = 0;
  for (int i = 0; i < n_ranked; i++) {
    cm->new_ref_frame_map.ref_frame_map[i] = scores[i].index;
    cm->new_ref_frame_map.named_ref_to_ref_frame_map[scores[i].named_ref] = i;
    if (scores[i].distance < 0) {
      cm->new_ref_frame_map.future_refs[n_future] = i;
      n_future++;
    } else {
      cm->new_ref_frame_map.past_refs[n_past] = i;
      n_past++;
    }
  }
  cm->new_ref_frame_map.n_past_refs = n_past;
  cm->new_ref_frame_map.n_future_refs = n_future;
}
