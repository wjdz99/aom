...
int av1_get_inter_mode_context(const MACROBLOCKD *xd) {
  ...
  if (has_above && has_left) {  // both edges available
    const int above_inter_comp_mode =
        is_inter_compound_mode(above_mbmi->mode) ||
        is_inter_singleref_comp_mode(above_mbmi->mode);
    const int left_inter_comp_mode =
        is_inter_compound_mode(left_mbmi->mode) ||
        is_inter_singleref_comp_mode(above_mbmi->mode);
    ...
    if (is_inter_compound_mode(edge_mbmi->mode) ||
        is_inter_singleref_comp_mode(edge_mbmi->mode))
      return 1;
