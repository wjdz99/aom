...
int av1_get_palette_cache(const MODE_INFO *above_mi, const MODE_INFO *left_mi,
  ...
  while (above_n > 0 && left_n > 0) {
    const unsigned int v_above = above_colors[above_idx];
    const unsigned int v_left = left_colors[left_idx];
    if (v_left < v_above) {
  ...
  while (above_n-- > 0) {
    const unsigned int val = above_colors[above_idx++];
    if (n == 0 || val != cache[n - 1]) cache[n++] = val;
  }
  while (left_n-- > 0) {
    const unsigned int val = left_colors[left_idx++];
    if (n == 0 || val != cache[n - 1]) cache[n++] = val;
  }
