    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      if (!has_second_ref(inter_mbmi)) {  // single pred
        if (!CHECK_LAST2_OR_GOLDEN(inter_mbmi->ref_frame[0]))
          ...
          pred_context =
              1 + 2 * (inter_mbmi->ref_frame[0] == LAST2_FRAME);
      } else {                              // comp pred
        if (has_uni_comp_refs(inter_mbmi) &&
            CHECK_LAST2_OR_GOLDEN(inter_mbmi->ref_frame[1]))
...
// TODO(zoeliu): To continue
        ...
        if (CHECK_LAST2_OR_GOLDEN(frfa) && CHECK_LAST2_OR_GOLDEN(frfl))
          pred_context = 4;
        else if (CHECK_LAST2_OR_GOLDEN(frfa) || CHECK_LAST2_OR_GOLDEN(frfl))
          pred_context = ;
        else
          pred_context = 2;
      } else if (l_sg || a_sg) {  // single/comp
...
#if CONFIG_LOWDELAY_COMPOUND || CONFIG_EXT_COMP_REFS  // No change to bitstream
