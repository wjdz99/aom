...
static void av1_prepare_filtering_parameters(AV1_DEBLOCKING_PARAMETERS * const pParams,
...
#endif // CONFIG_LOOPFILTERING_ACROSS_TILES
      {
        ...
        if (tuEdge) {
          ...
          const int32_t puEdge = 
              (coord & av1_prediction_masks[edgeDir][ss_size_lookup[ppCurr[0]->mbmi.sb_type][scaleHorz][scaleVert]]) ? (0) : (1);
...
static void av1_filter_block_plane_vert(const AV1_COMMON * const cm,
  ...
  for (int y = 0; y < (MAX_MIB_SIZE >> scaleVert); y += 1) {
    ...
    for (int x = 0; x < (MAX_MIB_SIZE >> scaleHorz); x += 1) {
      const MODE_INFO ** const pCurr = ppModeInfo + (y << scaleVert) * modeStride 
                                       + (x << scaleHorz);
      AV1_DEBLOCKING_PARAMETERS params;

      av1_prepare_filtering_parameters(&params,
                                       pCurr, ((ptrdiff_t) 1 << scaleHorz), cm, 
                                       VERT_EDGE,
