...
void update_tile_boundary_filter_mask(AV1_COMMON *const cm, const int mi_row,
                                      const int mi_col,
...
void av1_setup_mask(AV1_COMMON *const cm, const int mi_row, const int mi_col,
  ...
  if ((cm->loop_filter_across_tiles_enabled == 0) &&
      ((cm->tile_rows * cm->tile_cols) != 0)) {
    update_tile_boundary_filter_mask(cm, mi_row, mi_col, mode_info_stride, lfm);
  }
