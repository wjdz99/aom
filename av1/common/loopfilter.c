...
static uint8_t get_filter_level(const loop_filter_info_n *lfi_n,
  ...
  const int segment_id = AOMMIN(mbmi->segment_id[QUALITY_SEG_IDX], mbmi->segment_id_supertx[QUALITY_SEG_IDX]);
  assert(
    IMPLIES(supertx_enabled(mbmi), mbmi->segment_id_supertx[QUALITY_SEG_IDX] != MAX_QUALITY_SEGMENTS));
  assert(IMPLIES(supertx_enabled(mbmi),
    mbmi->segment_id_supertx[QUALITY_SEG_IDX] <= mbmi->segment_id[QUALITY_SEG_IDX]));
#else
