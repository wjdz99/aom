...
static void set_lpf_parameters(AV1_DEBLOCKING_PARAMETERS * const pParams,
  ...
  {
    const TX_SIZE ts = av1_get_transform_size(ppCurr[0], edgeDir,
        scaleHorz, scaleVert);
    const uint32_t currLevel = get_filter_level(&cm->lf_info,
        &ppCurr[0]->mbmi);
    const int currSkipped = ppCurr[0]->mbmi.skip &&
        is_inter_block(&ppCurr[0]->mbmi);
...
#endif  // CONFIG_LOOPFILTERING_ACROSS_TILES
      {
        const int32_t tuEdge = (coord & av1_transform_masks[edgeDir][ts]) ?
            (0) : (1);

        if (tuEdge) {
          const MODE_INFO * const pPrev = *(ppCurr - modeStep);
          const TX_SIZE pvTs = av1_get_transform_size(pPrev, edgeDir,
              scaleHorz, scaleVert);
          ...
          if ((currLevel || pvLvl) &&
              (!pvSkip || !currSkipped || puEdge)) {
            pParams->filterLength = (TX_4X4 >= min(ts, pvTs)) ? (4) : (8);
...
static void av1_filter_block_plane_vert(const AV1_COMMON *const cm,
  ...
  for (int y = 0; y < (MAX_MIB_SIZE >> scaleVert); y += 1) {
    ...
    for (int x = 0; x < (MAX_MIB_SIZE >> scaleHorz); x += 1) {
      ...
      set_lpf_parameters(
          &params, pCurr, ((ptrdiff_t)1 << scaleHorz), cm, VERT_EDGE,
          cuX + x * MI_SIZE, cuY + y * MI_SIZE, width, height, scaleHorz,
          scaleVert);
...
static void av1_filter_block_plane_horz(const AV1_COMMON *const cm,
  ...
  for (int y = 0; y < (MAX_MIB_SIZE >> scaleVert); y += 1) {
    ...
    for (int x = 0; x < (MAX_MIB_SIZE >> scaleHorz); x += 1) {
      ...
      set_lpf_parameters(&params, pCurr,
                         (modeStride << scaleVert), cm, HORZ_EDGE,
                         cuX + x * MI_SIZE, cuY + y * MI_SIZE,
                         width, height, scaleHorz, scaleVert);
    ...
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += MAX_MIB_SIZE) {
      ...
      for (int planeIdx = 0; planeIdx < num_planes; planeIdx += 1)
      {
        ...
        av1_filter_block_plane_vert(
            cm, planes + planeIdx, mi + mi_col, cm->mi_stride,
