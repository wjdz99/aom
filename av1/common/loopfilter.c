...
static
const uint32_t av1_transform_masks[NUM_EDGE_DIRS][TX_SIZES_ALL] =
{
  {
#if CONFIG_CB4X4
    2 - 1, // TX_2X2
...
static
void av1_prepare_filtering_parameters(AV1_DEBLOCKING_PARAMETERS * const pParams,
                                      const MODE_INFO ** const ppCurr, const ptrdiff_t modeStep,
  ...
  {
    const TX_SIZE currTxSize = av1_get_transform_size(ppCurr[0], edgeDir, scaleHorz, scaleVert);
    const uint32_t currLevel = get_filter_level(&cm->lf_info, &ppCurr[0]->mbmi);
    const int currSkipped = ppCurr[0]->mbmi.skip && is_inter_block(&ppCurr[0]->mbmi);
    ...
    if (coord) {
      ...
      if (av1_disable_loopfilter_on_tile_boundary(cm) &&
          (((VERT_EDGE == edgeDir) && (0 == (ppCurr[0]->mbmi.boundary_info & TILE_LEFT_BOUNDARY))) ||
           ((HORZ_EDGE == edgeDir) && (0 == (ppCurr[0]->mbmi.boundary_info & TILE_ABOVE_BOUNDARY)))))
#endif // CONFIG_LOOPFILTERING_ACROSS_TILES
      {
        ...
        if (tuEdge) {
          ...
          const int prevSkipped = pPrev->mbmi.skip && is_inter_block(&pPrev->mbmi);
          const int32_t puEdge = (coord & av1_prediction_masks[edgeDir][ss_size_lookup[ppCurr[0]->mbmi.sb_type][scaleHorz][scaleVert]]) ? (0) : (1);
          ...
          if ((currLevel || prevLevel) &&
            ...
            level = (currLevel) ? (currLevel) : (prevLevel);
          }
        }
      }
...
void av1_filter_block_plane_vert(const AV1_COMMON * const cm,
  ...
  const uint32_t miWidth = cm->mi_cols;
  const uint32_t miHeight = cm->mi_rows;
  ...
  for (int y = 0; y < (MAX_MIB_SIZE >> scaleVert); y += 1) {
    ...
    for (int x = 0; x < (MAX_MIB_SIZE >> scaleHorz); x += 1) {
      ...
      av1_prepare_filtering_parameters(&params,
                                       pCurr, ((ptrdiff_t) 1 << scaleHorz), cm, VERT_EDGE,
                                       cuX + x * MI_SIZE, cuY + y * MI_SIZE,
                                       width, height,
                                       scaleHorz, scaleVert);
      ...
      switch (params.filterLength) {
        ...
        case 8:
          aom_lpf_vertical_8(p, dstStride, params.mblim, params.lim, params.hev_thr);
          break;
...
void av1_filter_block_plane_horz(const AV1_COMMON * const cm,
  ...
  for (int y = 0; y < (MAX_MIB_SIZE >> scaleVert); y += 1) {
    ...
    for (int x = 0; x < (MAX_MIB_SIZE >> scaleHorz); x += 1) {
      ...
      av1_prepare_filtering_parameters(&params,
                                       pCurr, (modeStride << scaleVert), cm, HORZ_EDGE,
                                       cuX + x * MI_SIZE, cuY + y * MI_SIZE,
                                       width, height,
                                       scaleHorz, scaleVert);
      ...
      switch (params.filterLength) {
        ...
        // apply 8-tap filtering
        case 8:
          aom_lpf_horizontal_8(p, dstStride, params.mblim, params.lim, params.hev_thr);
          break;
...
void av1_loop_filter_rows(YV12_BUFFER_CONFIG *frame_buffer, AV1_COMMON *cm,
  ...
  const int num_planes = y_only ? 1 : MAX_MB_PLANE;
  int mi_row, mi_col;
#if (0 == CONFIG_PARALLEL_DEBLOCKING)
  enum lf_path path;
  LOOP_FILTER_MASK lfm;
  ...
  for (mi_row = start; mi_row < stop; mi_row += MAX_MIB_SIZE) {
    ...
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += MAX_MIB_SIZE) {
      ...
      for (int planeIdx = 0; planeIdx < num_planes; planeIdx += 1)
      {
        ...
        av1_filter_block_plane_vert(cm, planes + planeIdx,
                                    mi + mi_col, cm->mi_stride,
  ...
  for (mi_row = start; mi_row < stop; mi_row += MAX_MIB_SIZE) {
    ...
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += MAX_MIB_SIZE) {
      ...
      for (int planeIdx = 0; planeIdx < num_planes; planeIdx += 1)
      {
        ...
        av1_filter_block_plane_horz(cm, planes + planeIdx,
                                    mi + mi_col, cm->mi_stride,
                                    (mi_col * MI_SIZE) >> scaleHorz, (mi_row * MI_SIZE) >> scaleVert);
      }
    }
