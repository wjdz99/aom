...
void av1_filter_block_plane_non420_hor(AV1_COMMON *cm,
  ...
  //re-populate the filter mask for horizontal pass, it is the same as code in the av1_filter_block_plane_non420_ver
  for (r = 0; r < MAX_MIB_SIZE && mi_row + r < cm->mi_rows; r += row_step)  {
    unsigned int mask_16x16_c = 0;
    unsigned int mask_8x8_c = 0;
    unsigned int mask_4x4_c = 0;
...
void av1_filter_block_plane_ss11_hor(AV1_COMMON *const cm,
  ...
  //re-porpulate the filter level for uv, same as the code for vertical filter in av1_filter_block_plane_ss11_ver
  memset(lfm->lfl_uv, 0, sizeof(lfm->lfl_uv));
  for (r = 0; r < MAX_MIB_SIZE && mi_row + r < cm->mi_rows; r += 4) {
    if (plane->plane_type == 1) {
      for (c = 0; c < (MAX_MIB_SIZE >> 1); c++)  {
