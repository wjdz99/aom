...
void daala_inv_txfm_add(const tran_low_t *input_coeffs, void *output_pixels,
  ...
  if (txfm_param->lossless) {
    ...
    if (tx_type != DCT_DCT || txfm_param->eob > 1) {
      ...
      const int stages = tx_size_wide_log2[tx_size] +
        tx_size_high_log2[tx_size];
      const int shift = downshift + (stages >> 1) + 10;
      const int DC = ((stages & 1 ? input_coeffs[0] * 724 :
                       input_coeffs[0] * 1024) +
                      (1 << shift >> 1)) >> shift;
      ...
      if (txfm_param->is_hbd) {
        ...
        for (r = 0; r < rows; ++r)
          for (c = 0; c < cols; ++c)
            out16[r * output_stride + c] =
              highbd_clip_pixel_add(out16[r * output_stride + c], val,
                                    px_depth);
      } else {
        // Destination array is bytes
        uint8_t *out8 = (uint8_t *)output_pixels;
        for (r = 0; r < rows; ++r)
          for (c = 0; c < cols; ++c)
            out8[r * output_stride + c] =
              clip_pixel_add(out8[r * output_stride + c], val);
      }
    }
  }
}
