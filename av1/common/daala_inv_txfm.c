...
typedef void (*daala_itx)(const od_coeff *, int, od_coeff[]);
...
void daala_inv_txfm_add(const tran_low_t *input_coeffs,
                        void *output_pixels,
                        int output_stride,
  ...
  if (txfm_param->lossless) {
    ...
    if (hbd)
      av1_iwht4x4_add(input_coeffs, output_pixels, output_stride);
    else
    ...
    for (r = 0; r < row_n; ++r)
      for (c = 0; c < col_n; ++c)
        tmpsq[r*col_n+c] = input_coeffs[r*col_n+c] << upshift;
    input_coeffs = tmp;
#endif
    ...
    for (r = 0; r < row_n; ++r){
      if(row_flip)
        row_tx(tmpsq + (r+1)*col_n-1, -1, input_coeffs + r*col_n);
      else
        row_tx(tmpsq + r*col_n, 1, input_coeffs + r*col_n);
    }

    // Inverse-transform columns
    for (c = 0; c < col_n; ++c) {
      // Daala TX lowlevel only has output stride, construct temp
      // column vector for input
      for (r = 0; r < row_n; ++r)
        tmp[r] = tmpsq[r*col_n + c];
      if(col_flip)
        col_tx(tmpsq + (row_n-1) * col_n + c, -col_n, tmp);
      else
        col_tx(tmpsq + c, col_n, tmp);
    }
        ...
        for (c = 0; c < col_n; ++c)
          out16[r * output_stride + c] =
            highbd_clip_pixel_add(out16[r * output_stride + c],
               (*tmpsq_ptr++ + (1 << downshift >> 1)) >> downshift, px_depth);
    } else {
        ...
        for (c = 0; c < col_n; ++c)
          out8[r * output_stride + c] =
            clip_pixel_add(out8[r * output_stride + c],
             (*tmpsq_ptr++ + (1 << downshift >> 1)) >> downshift);
    }
  }
}
