...
void daala_inv_txfm_add(const tran_low_t *input_coeffs,
  ...
  if (txfm_param->lossless) {
    ...
    for (r = 0; r < rows; ++r)
      for (c = 0; c < cols; ++c)
        tmpsq[r*cols+c] = input_coeffs[r*cols+c] << upshift;
    input_coeffs = tmp;
#endif
    ...
    for (r = 0; r < rows; ++r){
      if(row_flip)
        row_tx(tmpsq + (r+1)*cols-1, -1, input_coeffs + r*cols);
      else
        row_tx(tmpsq + r*cols, 1, input_coeffs + r*cols);
    }
    ...
    for (c = 0; c < cols; ++c) {
      ...
      for (r = 0; r < rows; ++r)
        tmp[r] = tmpsq[r*cols + c];
      if(col_flip)
        col_tx(tmpsq + c + rows*cols-1, -cols, tmp);
      else
        col_tx(tmpsq + c, cols, tmp);
    }
