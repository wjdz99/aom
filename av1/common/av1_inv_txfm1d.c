...
void clamp_buf(int32_t *buf, int32_t size, int8_t bit) {
  // when signal range is expected to be smaller to 16 bits,
  // we clamp the input to 16 bits
  // This clamping allows us apply for 16-bit SIMD computation without
  // to worry about overflow due to quantizer noise
  if (bit <= 16) {
    bit = 16;
    const int32_t maxValue = (1LL << (bit - 1)) - 1;
    const int32_t minValue = -(1LL << (bit - 1));
