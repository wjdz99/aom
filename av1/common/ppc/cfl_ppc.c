...
static INLINE void subtract_average_vsx(const uint16_t *src_ptr,
int16_t *dst,
int width,
...
//  int16_t *dst = dst_ptr;
  ...
  do {
    ...
    if (width >= 16) {
      vec_vsx_st(vec_sub(vec_vsx_ld(OFF_1, dst), vec_avg), OFF_1,
                 dst);
    ...
    if (width == 32) {
      vec_vsx_st(vec_sub(vec_vsx_ld(OFF_2, dst), vec_avg), OFF_2,
                 dst);
      ...
      vec_vsx_st(vec_sub(vec_vsx_ld(OFF_3, dst), vec_avg), OFF_3,
                 dst);
