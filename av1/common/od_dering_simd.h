...
void SIMD_FUNC(copy_8x8_16bit_to_8bit)(uint8_t *dst, int dstride, uint16_t *src,
                                       int sstride) {
  int i;
  for (i = 0; i < 8; i++) {
...
void SIMD_FUNC(copy_4x4_16bit_to_8bit)(uint8_t *dst, int dstride, uint16_t *src,
                                       int sstride) {
  int i;
  for (i = 0; i < 4; i++) {
    v128 row = v128_load_unaligned(&src[i * sstride]);
    row = v128_pack_s16_u8(row, row);
    *((uint32_t *)&dst[i * dstride]) = v64_low_u32(row);
  }
}
...
void SIMD_FUNC(copy_4x4_16bit_to_16bit)(uint16_t *dst, int dstride,
  ...
  int i;
  for (i = 0; i < 4; i++) {
    v128 row = v64_load_unaligned(&src[i * sstride]);
    v64_store_unaligned(&dst[i * dstride], row);
  }
}
...
void SIMD_FUNC(copy_nxm_8bit_to_16bit)(uint16_t *dst, int dstride,
  ...
  for (i = 0; i < m; i++) {
    for (j = 0; j < (n & ~0x7); j += 8) {
      v128 row = v64_load_unaligned(&src[i * sstride + j]);
