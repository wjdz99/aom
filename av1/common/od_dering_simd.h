...
static INLINE v128 fold_mul_and_sum(v128 partiala, v128 partialb, v128 const1,
  ...
  tmp = partiala;
  partiala = v128_zip_16(partialb, partiala);
...
static INLINE v128 hsum4(v128 x0, v128 x1, v128 x2, v128 x3) {
  v128 t0, t1, t2, t3;
  t0 = v128_zip_32(x1, x0);
  t1 = v128_zip_32(x3, x2);
...
static INLINE void array_reverse_transpose_8x8(v128 *in, v128 *res) {
  const v128 tr0_0 = v128_zip_16(in[1], in[0]);
  const v128 tr0_1 = v128_zip_16(in[3], in[2]);
  const v128 tr0_2 = v128_ziphi_16(in[1], in[0]);
  const v128 tr0_3 = v128_ziphi_16(in[3], in[2]);
  const v128 tr0_4 = v128_zip_16(in[5], in[4]);
  const v128 tr0_5 = v128_zip_16(in[7], in[6]);
  const v128 tr0_6 = v128_ziphi_16(in[5], in[4]);
  const v128 tr0_7 = v128_ziphi_16(in[7], in[6]);

  const v128 tr1_0 = v128_zip_32(tr0_1, tr0_0);
  const v128 tr1_1 = v128_zip_32(tr0_5, tr0_4);
  const v128 tr1_2 = v128_ziphi_32(tr0_1, tr0_0);
  const v128 tr1_3 = v128_ziphi_32(tr0_5, tr0_4);
  const v128 tr1_4 = v128_zip_32(tr0_3, tr0_2);
  const v128 tr1_5 = v128_zip_32(tr0_7, tr0_6);
...
int SIMD_FUNC(od_filter_dering_direction_4x4)(uint16_t *y, int ystride,
  ...
  for (i = 0; i < 4; i += 2) {
    sum = v128_zero();
    row = v128_ziplo_64(v64_load_aligned(&in[(i + 1) * OD_FILT_BSTRIDE]),
                        v64_load_aligned(&in[i * OD_FILT_BSTRIDE]));

    /*p = in[i*OD_FILT_BSTRIDE + offset] - row*/
    tmp = v128_ziplo_64(v64_load_aligned(&in[(i + 1) * OD_FILT_BSTRIDE + off1]),
    ...
    /*p = in[i*OD_FILT_BSTRIDE + offset] - row*/
    tmp = v128_ziplo_64(v64_load_aligned(&in[(i + 1) * OD_FILT_BSTRIDE + off2]),
    ...
    v64_store_aligned(&y[i * ystride], res);
    v64_store_aligned(&y[(i + 1) * ystride], v128_ziphi_64(res, res));
  }
