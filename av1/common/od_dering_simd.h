...
void SIMD_FUNC(cdef_filter_block_4x4_8)(uint8_t *dst, int dstride,
  ...
  for (i = 0; i < 4; i += 2) {
    ...
    v64 res8 = v128_low_v64(v128_pack_s16_u8(res, res));
    u32_store_aligned(&dst[i * dstride], v64_high_u32(res8));
    u32_store_aligned(&dst[(i + 1) * dstride], v64_low_u32(res8));
  }
}
...
void SIMD_FUNC(cdef_filter_block_8x8_8)(uint8_t *dst, int dstride,
  ...
  for (i = 0; i < 8; i++) {
    ...
    res = v128_add_16(row, res);
    res = v128_min_s16(v128_max_s16(res, v128_zero()), v128_dup_16(max));
    v64_store_unaligned(&dst[i * dstride],
                        v128_low_v64(v128_pack_s16_u8(res, res)));
  }
}
