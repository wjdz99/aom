...
static INLINE __m128i sum_8x4_ssse3(int16_t *pred_buf) {
  const __m128i zeros = _mm_setzero_si128();
...
static int sum_block_8x4_ssse3(int16_t *pred_buf) {
  return horz_sum_ssse3(sum_8x4_ssse3(pred_buf));
}

static int sum_block_8x8_ssse3(int16_t *pred_buf) {
  return horz_sum_ssse3(_mm_add_epi32(
      sum_8x4_ssse3(pred_buf), sum_8x4_ssse3(pred_buf + 4 * CFL_BUF_LINE)));
}

static int sum_block_8x16_ssse3(int16_t *pred_buf) {
  __m128i r0 = sum_8x4_ssse3(pred_buf);
  __m128i r1 = sum_8x4_ssse3(pred_buf + 4 * CFL_BUF_LINE);
  __m128i r2 = sum_8x4_ssse3(pred_buf + 8 * CFL_BUF_LINE);
  __m128i r3 = sum_8x4_ssse3(pred_buf + 12 * CFL_BUF_LINE);
  return horz_sum_ssse3(
      _mm_add_epi32(_mm_add_epi32(r0, r1), _mm_add_epi32(r2, r3)));
}

static INLINE __m128i sum_16x2_ssse3(int16_t *pred_buf) {
  const __m128i zeros = _mm_setzero_si128();
  __m128i r0 = _mm_loadu_si128((__m128i *)pred_buf);
  __m128i r1 = _mm_loadu_si128((__m128i *)(pred_buf + 8));
  __m128i r2 = _mm_loadu_si128((__m128i *)(pred_buf + CFL_BUF_LINE));
  __m128i r3 = _mm_loadu_si128((__m128i *)(pred_buf + 8 + CFL_BUF_LINE));
  r0 = _mm_add_epi16(r0, r1);
  r2 = _mm_add_epi16(r2, r3);
  r0 = _mm_add_epi32(_mm_unpacklo_epi16(r0, zeros),
                     _mm_unpackhi_epi16(r0, zeros));
...
static INLINE __m128i sum_32x1_ssse3(int16_t *pred_buf) {
  const __m128i zeros = _mm_setzero_si128();
  __m128i r0 = _mm_loadu_si128((__m128i *)pred_buf);
  __m128i r1 = _mm_loadu_si128((__m128i *)(pred_buf + 8));
  __m128i r2 = _mm_loadu_si128((__m128i *)(pred_buf + 16));
  __m128i r3 = _mm_loadu_si128((__m128i *)(pred_buf + 24));
  r0 = _mm_add_epi16(r0, r1);
  r2 = _mm_add_epi16(r2, r3);
  r0 = _mm_add_epi32(_mm_unpacklo_epi16(r0, zeros),
                     _mm_unpackhi_epi16(r0, zeros));
