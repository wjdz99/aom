    ...
    for (int x = 0; x < pic_width_c; x += (1 << CCSO_BLK_SIZE)) {
      ...
      for (int yOff = 0; yOff < y_offset; yOff++) {
        for (int xOff = 0; xOff < x_offset; xOff++) {
          ...
          rec_yv_8[dst_stride_idx[yOff] + x + xOff] =
              clamp(offset_val + rec_yv_8[dst_stride_idx[yOff] + x + xOff], 0,
...
void ccso_filter_block_hbd_c(
  ...
  for (int yOff = 0; yOff < y_offset; yOff++) {
    for (int xOff = 0; xOff < x_offset; xOff++) {
      ...
      rec_uv_16[dst_stride_idx[yOff] + x + xOff] = clamp(
          offset_val + rec_uv_16[dst_stride_idx[yOff] + x + xOff], 0, maxval);
    }
  }
}
