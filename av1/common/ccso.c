...
void apply_ccso_filter(AV1_COMMON *cm, MACROBLOCKD *xd, const int plane,
  ...
  int y_uv_hori_scale = xd->plane[1].subsampling_x;
  int y_uv_vert_scale = xd->plane[1].subsampling_y;
    ...
    for (int x = 0; x < pic_width_c; x += (1 << CCSO_BLK_SIZE)) {
      ...
      for (int yOff = 0; yOff < y_offset; yOff++) {
        for (int xOff = 0; xOff < x_offset; xOff++) {
          cal_filter_support(
              rec_luma_idx,
              &temp_rec_y_buf[((ccso_stride_ext_idx[yOff] << y_uv_vert_scale) +
                               ((x + xOff) << y_uv_hori_scale)) +
                              pad_stride],
...
void ccso_filter_block_hbd_c(uint16_t *temp_rec_y_buf, uint16_t *rec_yuv_16,
                             ...
                             const int *dst_stride_idx, int y_uv_hori_scale,
                             int y_uv_vert_scale, int pad_stride,
                             int quant_step_size, int inv_quant_step,
                             int *rec_idx, int maxval) {
  int y_offset;
  int x_offset;
  if (y + (1 << CCSO_BLK_SIZE) >= pic_height_c)
    y_offset = pic_height_c - y;
  ...
  for (int yOff = 0; yOff < y_offset; yOff++) {
    for (int xOff = 0; xOff < x_offset; xOff++) {
      cal_filter_support(
          rec_luma_idx,
          &temp_rec_y_buf[((ccso_stride_idx[yOff] << y_uv_vert_scale) +
                           ((x + xOff) << y_uv_hori_scale)) +
                          pad_stride],
          quant_step_size, inv_quant_step, rec_idx);
      int offset_val = offset_buf[(rec_luma_idx[0] << 2) + rec_luma_idx[1]];
