...
void od_dering(uint8_t *dst, int dstride, uint16_t *y, uint16_t *in, int xdec, int ydec,
        ...
        for (iy = 0; iy < 1 << bsizey; iy++)
          for (ix = 0; ix < 1 << bsizex; ix++)
            y[(bi << (bsizex + bsizey)) + (iy << bsizex) + ix] =
                in[((by << bsizey) + iy) * OD_FILT_BSTRIDE + (bx << bsizex) +
                   ix];
      }
    } else {
    ...
    for (bi = 0; bi < dering_count; bi++) {
      ...
      if (!dst || hbd) {
        ...
        (!threshold || (dir[by][bx] < 4 && dir[by][bx]) ? aom_clpf_block_hbd
                                                        : aom_clpf_hblock_hbd)(
            dst ? (uint16_t *)dst + py * dstride + px : &y[bi << (bsizex + bsizey)],
