    ...
    for (bi = 0; bi < dering_count; bi++) {
      ...
      if (!dst || hbd) {
        ...
        (full ? aom_clpf_block_hbd : aom_clpf_hblock_hbd)(
            dst ? (uint16_t *)&dst[py * dstride + px] : y2, in2,
            dst && hbd ? dstride : bs, OD_FILT_BSTRIDE, bs, bs, clpf_strength,
            clpf_damping);
      } else {
        ...
        if (in[(py - 1) * OD_FILT_BSTRIDE + px - 1] == OD_DERING_VERY_LARGE ||
          ...
          (full ? aom_clpf_block_hbd : aom_clpf_hblock_hbd)(
              y2, in2, bs, OD_FILT_BSTRIDE, bs, bs, clpf_strength,
              clpf_damping);
          // Convert from 16 bit to 8 bit
          (bsize == 3 ? copy_8x8_16bit_to_8bit : copy_4x4_16bit_to_8bit)(
              &dst[py * dstride + px], dstride, y2, bs);
        } else {
          // No boundaries, use the faster 8 bit CLPF
          (full ? aom_clpf_block : aom_clpf_hblock)(
              &dst[py * dstride + px], in2, dstride, OD_FILT_BSTRIDE, bs, bs,
