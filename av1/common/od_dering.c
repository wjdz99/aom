...
void cdef_filter_sb(uint8_t *dst, int dstride, uint16_t *y, uint16_t *in,
  ...
  for (bi = 0; bi < cdef_count; bi++) {
    ...
    if (hbd)
      cdef_filter_block(
	  NULL, &((uint16_t *)dst)[(by << bsizey) * dstride + (bx << bsizex)],
          dstride, &in[(by * CDEF_BSTRIDE << bsizey) + (bx << bsizex)],
          (pli ? t : od_adjust_strength(t, var[by][bx])), s,
          t ? dir[by][bx] : 0, pri_damping, sec_damping, bsize,
          (256 << coeff_shift) - 1);
    else
      cdef_filter_block(
          dst ? &dst[(by << bsizey) * dstride + (bx << bsizex)] : 0,
          &y[bi << (bsizex + bsizey)], dst ? dstride : 1 << bsizex,
