...
void od_filter_dering_direction_8x8_c(uint16_t *y, int ystride,
  ...
  for (i = 0; i < 8; i++) {
    for (j = 0; j < 8; j++) {
      ...
      int A, B, C, D, E, F, G, H;
      if (dir != 2 && dir != 6) {
      A = in[(i - 2) * OD_FILT_BSTRIDE + j];
      B = in[(i - 1) * OD_FILT_BSTRIDE + j];
      C = in[i * OD_FILT_BSTRIDE + j - 2];
      D = in[i * OD_FILT_BSTRIDE + j - 1];
      E = in[i * OD_FILT_BSTRIDE + j + 1];
      F = in[i * OD_FILT_BSTRIDE + j + 2];
      G = in[(i + 1) * OD_FILT_BSTRIDE + j];
      H = in[(i + 2) * OD_FILT_BSTRIDE + j];
      } else {
      A = in[(i - 2) * OD_FILT_BSTRIDE + j - 2];
      B = in[(i - 1) * OD_FILT_BSTRIDE + j - 1];
      C = in[(i + 2) * OD_FILT_BSTRIDE + j - 2];
      D = in[(i + 1) * OD_FILT_BSTRIDE + j - 1];
      E = in[(i - 1) * OD_FILT_BSTRIDE + j + 1];
      F = in[(i - 2) * OD_FILT_BSTRIDE + j + 2];
      G = in[(i + 1) * OD_FILT_BSTRIDE + j + 1];
      H = in[(i + 2) * OD_FILT_BSTRIDE + j + 2];
      }
      sum += 1 * constrain(A - xx, clpf_strength, clpf_damping);
      sum += 3 * constrain(B - xx, clpf_strength, clpf_damping);
      sum += 1 * constrain(C - xx, clpf_strength, clpf_damping);
      sum += 3 * constrain(D - xx, clpf_strength, clpf_damping);
      sum += 3 * constrain(E - xx, clpf_strength, clpf_damping);
      sum += 1 * constrain(F - xx, clpf_strength, clpf_damping);
      sum += 3 * constrain(G - xx, clpf_strength, clpf_damping);
      sum += 1 * constrain(H - xx, clpf_strength, clpf_damping);
      for (k = 0; k < 3; k++) {
        int16_t p0;
        int16_t p1;
