...
void od_dering(uint8_t *dst, int dstride, uint16_t *y, uint16_t *in, int xdec,
  ...
  if (clpf_strength) {
    ...
    for (bi = 0; bi < dering_count; bi++) {
      ...
      const int py = by << mi_size_l2;
      const int px = bx << mi_size_l2;
      ...
      if (!dst || hbd) {
        ...
        (!threshold || (dir[by][bx] < 4 && dir[by][bx]) ? aom_clpf_block_hbd
            ...
            OD_FILT_BSTRIDE, bs, bs, clpf_strength << coeff_shift,
        ...
        if (in[(py - 1) * OD_FILT_BSTRIDE + px - 1] == OD_DERING_VERY_LARGE ||
          ...
          (!threshold || (dir[by][bx] < 4 && dir[by][bx])
               ? aom_clpf_block_hbd
               : aom_clpf_hblock_hbd)(
              y2, in2, dst && hbd ? dstride : bs, OD_FILT_BSTRIDE, bs, bs,
              clpf_strength << coeff_shift, clpf_damping + coeff_shift);
          (bsize == 3 ? copy_8x8_16bit_to_8bit
                      : copy_4x4_16bit_to_8bit)(dst2, dstride, y2, bs);
        } else {
          // No boundaries, use the faster 8 bit CLPF
          (!threshold || (dir[by][bx] < 4 && dir[by][bx]) ? aom_clpf_block
                                                          : aom_clpf_hblock)(
              dst2, in2, dstride, OD_FILT_BSTRIDE, bs, bs,
              clpf_strength << coeff_shift, clpf_damping + coeff_shift);
        }
      }
    }
