...
void od_filter_dering_direction_8x8_c(uint16_t *y, int ystride,
  ...
  damping &= 255;
  threshold &= 255;
  static const int taps[3] = { 3, 3 };
  static const int clpf_taps[3] = { 3, 1 };
  for (i = 0; i < 8; i++) {
    for (j = 0; j < 8; j++) {
      int16_t sum;
      int16_t xx;
      ...
      for (k = 0; k < 2; k++) {
        ...
        p0 = in[i * OD_FILT_BSTRIDE + j +
                OD_DIRECTION_OFFSETS_TABLE[(dir + 2) & 7][k]] -
             xx;
        p1 = in[i * OD_FILT_BSTRIDE + j -
                OD_DIRECTION_OFFSETS_TABLE[(dir + 2) & 7][k]] -
             xx;
        sum += clpf_taps[k] * constrain(p0, clpf_strength, clpf_damping);
        sum += clpf_taps[k] * constrain(p1, clpf_strength, clpf_damping);
        p0 = in[i * OD_FILT_BSTRIDE + j +
                OD_DIRECTION_OFFSETS_TABLE[(dir + 6) & 7][k]] -
             xx;
        p1 = in[i * OD_FILT_BSTRIDE + j -
                OD_DIRECTION_OFFSETS_TABLE[(dir + 6) & 7][k]] -
             xx;
...
void od_filter_dering_direction_4x4_c(uint16_t *y, int ystride,
  ...
  damping &= 255;
  threshold &= 255;
  static const int taps[2] = { 4, 3 };
  static const int clpf_taps[2] = { 3, 1 };
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      int16_t sum;
      int16_t xx;
...
void od_dering(uint8_t *dst, int dstride, uint16_t *y, uint16_t *in, int xdec,
  ...
  for (bi = 0; bi < dering_count; bi++) {
    ...
    (filter_dering_direction[bsize == BLOCK_8X8])(
        ...
        (pli ? t : od_adjust_thresh(t, var[by][bx])) | (s << 8), t ? dir[by][bx] : 0,
