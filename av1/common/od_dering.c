...
void od_filter_dering_direction_8x8_c(uint16_t *y, int ystride,
  ...
  static const int clpf_taps[3] = { 3*2, 2*2, 1*2 };
  for (i = 0; i < 8; i++) {
    for (j = 0; j < 8; j++) {
      int16_t sum;
      int16_t xx;
      ...
      for (k = 0; k < 3; k++) {
        ...
        if (k == 2) continue;
        p0 = in[i * OD_FILT_BSTRIDE + j + OD_DIRECTION_OFFSETS_TABLE[(dir + 4) & 7][k]] -
             xx;
        p1 = in[i * OD_FILT_BSTRIDE + j - OD_DIRECTION_OFFSETS_TABLE[(dir + 4) & 7][k]] -
             xx;
...
void od_filter_dering_direction_4x4_c(uint16_t *y, int ystride,
  ...
  static const int clpf_taps[2] = { 4*2, 1*2 };
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      int16_t sum;
      int16_t xx;
