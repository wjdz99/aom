...
const int cdef_directions[8]
                         [2] = {{-1 * CDEF_BSTRIDE + 1, -2 * CDEF_BSTRIDE + 2},
...
void copy_cdef_16bit_to_16bit(uint16_t *dst, int dstride, uint16_t *src,
                              cdef_list *list, int dering_count, int bsize) {
  int bi, bx, by;
...
static void copy_cdef_16bit_to_8bit(uint8_t *dst, int dstride,
                                    const uint16_t *src, cdef_list *list,
...
void cdef_filter_sb(uint8_t *dst, int dstride, uint16_t *y, uint16_t *in,
  ...
  for (bi = 0; bi < cdef_count; bi++) {
    ...
    by = list[bi].by;
    bx = list[bi].bx;
    cdef_filter_block(&y[bi << (bsizex + bsizey)], 1 << bsizex,
  ...
  if (dst) {
    if (hbd) {
      copy_cdef_16bit_to_16bit((uint16_t *)dst, dstride, y, list, cdef_count,
                               bsize);
    } else {
      copy_cdef_16bit_to_8bit(dst, dstride, y, list, cdef_count, bsize);
    }
  }
}
