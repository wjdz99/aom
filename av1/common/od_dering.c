...
void od_filter_dering_direction_c(uint16_t *y, int ystride,
  ...
  for (i = 0; i < 4 << (bsize == BLOCK_8X8); i++) {
	...
	max = AOMMAX(AOMMAX(AOMMAX(AOMMAX(AOMMAX(p0, p1), s0), s1), s2), s3);
	min = AOMMIN(AOMMIN(AOMMIN(AOMMIN(AOMMIN(p0, p1), s0), s1), s2), s3);
...
void od_dering(uint8_t *dst, int dstride, uint16_t *y, uint16_t *in, int xdec,
  ...
  for (bi = 0; bi < dering_count; bi++) {
    ...
    od_filter_dering_direction_c(
        ...
        (pli ? t : od_adjust_thresh(t, var[by][bx])), s,
        t ? dir[by][bx] : 0, dering_damping, clpf_damping,
	bsize);
  }
  if (dst) {
    if (hbd) {
