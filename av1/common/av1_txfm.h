...
#if CONFIG_MRC_TX
static INLINE void average1x5(uint16_t *input, uint16_t *output, int dim) {
...
static INLINE int get_mrc_diff_mask_inter(const int16_t *diff, int diff_stride,
  ...
  for (int i = 0; i < height; ++i) {
    ...
    average1x5(tmp_in, tmp_out, width);
    for (int j = 0; j < width; ++j) {
      out[j * MAX_SB_SIZE + i] = (uint16_t)(tmp_out[j] * norm);
    }
  }
  ...
  for (int i = 0; i < width; ++i) {
    ...
    average1x5(tmp_in, tmp_out, height);
    for (int j = 0; j < height; ++j) {
      mask[i + j * mask_stride] = (uint8_t)(tmp_out[j] * norm > thresh);
