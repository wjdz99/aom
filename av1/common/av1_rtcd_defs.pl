...
if (aom_config("CONFIG_AV1_ENCODER") eq "yes") {
  ...
  if (aom_config("CONFIG_REALTIME_ONLY") ne "yes") {
    add_proto qw/void av1_apply_temporal_filter_yuv/, "const struct yv12_buffer_config *ref_frame, const struct macroblockd *mbd, const BLOCK_SIZE block_size, const int mb_row, const int mb_col, const int strength, const int use_subblock, const int *blk_fw, const uint8_t *pred, uint32_t *accum, uint16_t *count";
