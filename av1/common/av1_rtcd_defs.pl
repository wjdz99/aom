...
if (aom_config("CONFIG_FRAME_SUPERRES") eq "yes") {
  if (aom_config("CONFIG_LOOP_RESTORATION") eq "yes") {
    add_proto qw/void av1_convolve_horiz_rs/, "const uint8_t *src, uint8_t *dst, int w, const int16_t *x_filters, int interp_taps, const int x0_qn, const int x_step_qn";
    if (aom_config("CONFIG_HIGHBITDEPTH") eq "yes") {
      add_proto qw/void av1_highbd_convolve_horiz_rs/, "const uint16_t *src, uint16_t *dst, int w, const int16_t *x_filters, int interp_taps, const int x0_qn, const int x_step_qn, int bd";
    }
  }
}
