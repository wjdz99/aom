...
if (aom_config("CONFIG_CCSO") eq "yes") {
	add_proto qw/void ccso_filter_block_hbd/, "uint16_t *temp_rec_y_buf, uint16_t *rec_yuv_16, int x, int y, int pic_width_c, int pic_height_c, int *rec_luma_idx, const int8_t *offset_buf, const int *ccso_stride_idx, const int *dst_stride_idx, int y_uv_hori_scale, int y_uv_vert_scale, int pad_stride, int quant_step_size, int inv_quant_step, int *rec_idx, int maxval";
}
