...
#ifndef IS_ENC
#error "IS_ENC must be defined for reconter_template.h."
#endif
...
AOM_INLINE void RENAME(build_one_inter_predictor)(
    uint8_t *dst, int dst_stride, const MV *src_mv,
    InterPredParams *inter_pred_params
#if !IS_ENC
    ,
    MACROBLOCKD *xd, int mi_x, int mi_y, int ref, uint8_t **mc_buf
#endif  //! IS_ENC
) {
...
static AOM_INLINE void RENAME(build_inter_predictors_sub8x8)(
...
static AOM_INLINE void RENAME(build_inter_predictors_8x8_and_bigger)(
    const AV1_COMMON *cm, MACROBLOCKD *xd, int plane, const MB_MODE_INFO *mi,
...
static AOM_INLINE void RENAME(build_inter_predictors)(
  ...
  if (is_sub8x8_inter(xd, plane, mi->bsize, is_intrabc_block(mi),
                      build_for_obmc)) {
    assert(bw < 8 || bh < 8);
    RENAME(build_inter_predictors_sub8x8)
    (cm, xd, plane, mi, mi_x, mi_y
#if !IS_ENC
     ,
     mc_buf
#endif  // !IS_ENC
    ...
    RENAME(build_inter_predictors_8x8_and_bigger)
    (cm, xd, plane, mi, build_for_obmc, bw, bh, mi_x, mi_y
#if !IS_ENC
     ,
     mc_buf
#endif  // !IS_ENC
