...
static void av1_restore_cnn_plane_Y_wrapper(
  ...
  if (qindex <= MIN_CNN_Q_INDEX) {
    ...
    av1_restore_cnn_plane(cm, &model_12, plane, thread_data);
  } else if (qindex < 108) {
    av1_restore_cnn_plane(cm, &model_22, plane, thread_data);
  } else if (qindex < 148) {
    av1_restore_cnn_plane(cm, &model_32, plane, thread_data);
  } else if (qindex < 192) {
    av1_restore_cnn_plane(cm, &model_43, plane, thread_data);
  } else if (qindex < 232) {
    av1_restore_cnn_plane(cm, &model_53, plane, thread_data);
  } else {
    av1_restore_cnn_plane(cm, &model_63, plane, thread_data);
  }
}
