...
static void loop_restoration_init(RestorationInternal *rst,
...
static void loop_restoration_rows(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  if (rsi[0].frame_restoration_type != RESTORE_NONE) {
    ...
    restore_func = restore_funcs[cm->rst_internal.rsi->frame_restoration_type];
#if CONFIG_AOM_HIGHBITDEPTH
    restore_func_highbd =
  ...
  if (!y_only) {
    ...
    assert(cm->rst_internal.rsi->frame_restoration_type == RESTORE_NONE);
    ...
    assert(rsi[2].frame_restoration_type == RESTORE_NONE);
    assert(cm->rst_internal.rsi->frame_restoration_type == RESTORE_NONE);
    if (cm->rst_internal.rsi->frame_restoration_type != RESTORE_NONE) {
      restore_func = restore_funcs[cm->rst_internal.rsi->frame_restoration_type];
#if CONFIG_AOM_HIGHBITDEPTH
...
void av1_loop_restoration_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  if (rsi->frame_restoration_type != RESTORE_NONE) {
    ...
    loop_restoration_init(&cm->rst_internal, cm->frame_type == KEY_FRAME,
                          cm->width, cm->height);
    loop_restoration_rows(frame, cm, start_mi_row, end_mi_row, y_only, rsi,
                          dst);
  }
}
