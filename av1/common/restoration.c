...
static void av1_selfguided_restoration_internal(int32_t *dgd, int width,
  ...
  const int vstep = 1 + (USE_HWFRIENDLY_SGR && (r > 1));
  ...
  // If vstep = 2, i.e. odd rows were skipped, interpolate vertically
  if (vstep == 2) {
    for (i = 1; i < height - 1; i += 2) {
      for (j = 0; j < width; ++j) {
        const int k = i * buf_stride + j;
        A[k] = (A[k - buf_stride] + A[k + buf_stride] + 1) / 2;
        B[k] = (B[k - buf_stride] + B[k + buf_stride] + 1) / 2;
      }
    }
    if ((height & 1) == 0) {
      for (j = 0; j < width; ++j) {
        const int k = i * buf_stride + j;
        A[k] = 2 * A[k - buf_stride] - A[k - 2 * buf_stride];
        B[k] = 2 * B[k - buf_stride] - B[k - 2 * buf_stride];
      }
    }
  }
  if (vstep == 2) {
    ...
    for (i = 0; i < height; ++i) {
      ...
      const int nb = 3;
      const int32_t a = 5 * A[k] + 3 * A[k + 1];
      const int32_t b = 5 * B[k] + 3 * B[k + 1];
    ...
    for (i = 0; i < height; ++i) {
      for (j = 1; j < width - 1; ++j) {
        ...
        const int l = i * stride + j;
        const int nb = 2;
        const int32_t a = 2 * A[k] + A[k - 1] + A[k + 1];
        const int32_t b = 2 * B[k] + B[k - 1] + B[k + 1];
        const int32_t v = a * dgd[l] + b;
        dgd[l] =
