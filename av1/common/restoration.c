...
static void av1_selfguided_restoration_fast_internal(
  ...
  for (i = 0; i < height + 1; i += 2) {
    if (i == height) {
      // For last row (if height is even) use best estimate from both
      // previous rows.
      for (j = -1; j < width + 1; ++j) {
        const int k = i * buf_stride + j;
      ...
      for (j = -1; j < width + 1; ++j) {
        const int k = i * buf_stride + j;
        A[k] = (A[k - buf_stride] + A[k + buf_stride] + 1) >> 1;
        B[k] = (B[k - buf_stride] + B[k + buf_stride] + 1) >> 1;
      }
    }
  }
...
#if CONFIG_FAST_SGR
  av1_selfguided_restoration_fast_internal(dgd32, width, height, dgd32_stride, flt1,
