...
static void apply_wiener_nonsep(const uint8_t *dgd, int width, int height,
                                ...
                                uint8_t *dst, int dst_stride, int32_t *buf) {
  for (int i = 0; i < height; ++i) {
    for (int j = 0; j < width; ++j) {
    ...
    for (int i = w; i < height - w; ++i) {
      for (int j = w; j < width - w; ++j) {
        for (int k = 0; k < num_pixels; ++k) {
          buf[(i-w)*crop_width + (j-w)] += filter[wiener_nsfilter[k][2]] * (
            dst[(i+wiener_nsfilter[k][0])*width + (j+wiener_nsfilter[k][1])]
              - dst[i*width + j]);
        }
        dst[i*width + j] = clip_pixel(
          buf[(i-w)*crop_width + (j+w)] >> wiener_nsbits);
      }
    }
  }
}
