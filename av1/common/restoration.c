...
static void loop_wiener_filter_tile(uint8_t *data, int tile_idx, int width,
  ...
  while (i < v_end) {
    int h = setup_processing_stripe_boundary(i, v_end, h_start, h_end, data,
                                             stride, rst, 0);
    if (h < 16)
      h = 16;  // TODO(olah): Is this correct? Why does h need to be at least 16
               // (see note below)?
#else
...
#if CONFIG_STRIPED_LOOP_RESTORATION
    restore_processing_stripe_boundary(i, v_end, h_start, h_end, data, stride,
                                       rst, 0);
    i += h;
#endif
  }
...
static void loop_wiener_filter_tile_highbd(uint16_t *data, int tile_idx,
  ...
  i = v_start;
  while (i < v_end) {
    int h = setup_processing_stripe_boundary(i, v_end, h_start, h_end,
                                             (uint8_t *)data, stride, rst, 1);
    if (h < 16)
      h = 16;  // TODO(olah): Is this correct? Why does h need to be at least 16
               // (see note below)?
#else
...
#if CONFIG_STRIPED_LOOP_RESTORATION
    restore_processing_stripe_boundary(i, v_end, h_start, h_end,
                                       (uint8_t *)data, stride, rst, 1);
    i += h;
#endif
  }
