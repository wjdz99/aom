...
static void GenDomainTxfmRFVtable() {
  ...
  for (i = 0; i < DOMAINTXFMRF_ITERS; ++i) {
    ...
    for (j = 0; j < DOMAINTXFMRF_PARAMS; ++j) {
      const double sigma_r = (double)domaintxfmrf_params[j] / DOMAINTXFMRF_SIGMA_SCALE;
...
static void apply_domaintxfmrf_hor(int iter, int param, uint8_t *img,
    ...
    *dp *= DOMAINTXFMRF_VTABLE_PREC;
    ...
    for (j = 1; j < width; ++j, dp++, ip++) {
      ...
      dp[0] = dp[0] * (DOMAINTXFMRF_VTABLE_PREC - v) +
          ((v * dp[-1] + DOMAINTXFMRF_VTABLE_PREC / 2) >> DOMAINTXFMRF_VTABLE_PRECBITS);
    }
    // right to left
    dp -= 2;
    ip -= 2;
...
static void apply_domaintxfmrf_ver(int iter, int param, uint8_t *img,
                                   int width, int height, int img_stride,
                                   int32_t *dat, int dat_stride) {
  int i, j;
  for (j = 0; j < width; ++j) {
    ...
    for (i = 1; i < height; ++i, dp += dat_stride, ip += img_stride) {
      const int v = domaintxfmrf_vtable[iter][param][abs(ip[0] - ip[-img_stride])];
      dp[0] = (dp[0] * (DOMAINTXFMRF_VTABLE_PREC - v) +
               (dp[-dat_stride] * v + DOMAINTXFMRF_VTABLE_PREC / 2)) >> DOMAINTXFMRF_VTABLE_PRECBITS;
    }
    // bottom to top
    dp -= 2 * dat_stride;
    ip -= 2 * img_stride;
    for (i = height - 2; i >= 0; --i, dp -= dat_stride, ip -= img_stride) {
      const int v =  domaintxfmrf_vtable[iter][param][abs(ip[img_stride] - ip[0])];
      dp[0] = (dp[0] * (DOMAINTXFMRF_VTABLE_PREC - v) +
               dp[dat_stride] * v + DOMAINTXFMRF_VTABLE_PREC / 2) >> DOMAINTXFMRF_VTABLE_PRECBITS;
    }
  }
...
static void apply_domaintxfmrf_reduce_prec(int32_t *dat, int width, int height,
  ...
  for (i = 0; i < height; ++i) {
    for (j = 0; j < width; ++j) {
      dat[i * dat_stride + j] = ROUND_POWER_OF_TWO_SIGNED(dat[i * dat_stride + j],
                                                          DOMAINTXFMRF_VTABLE_PRECBITS);
    }
  }
}

void av1_domaintxfmrf_restoration(
    uint8_t *dgd, int width, int height, int stride, int param) {
  ...
  for (t = 0; t < DOMAINTXFMRF_ITERS; ++t) {
    apply_domaintxfmrf_hor(t, param, dgd, width, height, stride,
                           dat, width);
    apply_domaintxfmrf_ver(t, param, dgd, width, height, stride,
                           dat, width);
...
static void loop_domaintxfmrf_filter_tile(
    uint8_t *data, int tile_idx, int width, int height, int stride,
    RestorationInternal *rst, void *tmpbuf) {
  const int tile_width = rst->tile_width >> rst->subsampling_x;
  const int tile_height = rst->tile_height >> rst->subsampling_y;
  int h_start, h_end, v_start, v_end;
...
static void apply_domaintxfmrf_hor_highbd(int iter, int param, uint16_t *img,
                                          int width, int height, int img_stride,
                                          int32_t *dat, int dat_stride, int bd) {
  ...
  for (i = 0; i < height; ++i) {
    ...
    *dp *= DOMAINTXFMRF_VTABLE_PREC;
    ...
    // left to right
    for (j = 1; j < width; ++j, dp++, ip++) {
      const int v = domaintxfmrf_vtable[iter][param][abs((ip[0] >> shift) - (ip[-1] >> shift))];
      dp[0] = dp[0] * (DOMAINTXFMRF_VTABLE_PREC - v) +
          ((v * dp[-1] + DOMAINTXFMRF_VTABLE_PREC / 2) >> DOMAINTXFMRF_VTABLE_PRECBITS);
    }
    // right to left
    ...
    ip -= 2;
    for (j = width - 2; j >= 0; --j, dp--, ip--) {
      const int v =  domaintxfmrf_vtable[iter][param][abs((ip[1] >> shift) - (ip[0] >> shift))];
      dp[0] = (dp[0] * (DOMAINTXFMRF_VTABLE_PREC - v) +
               v * dp[1] + DOMAINTXFMRF_VTABLE_PREC / 2) >> DOMAINTXFMRF_VTABLE_PRECBITS;
    }
  }
...
static void apply_domaintxfmrf_ver_highbd(int iter, int param, uint16_t *img,
  ...
  for (j = 0; j < width; ++j) {
    ...
    // top to bottom
    for (i = 1; i < height; ++i, dp += dat_stride, ip += img_stride) {
      const int v = domaintxfmrf_vtable[iter][param][abs((ip[0] >> shift) - (ip[-img_stride] >> shift))];
      dp[0] = (dp[0] * (DOMAINTXFMRF_VTABLE_PREC - v) +
               (dp[-dat_stride] * v + DOMAINTXFMRF_VTABLE_PREC / 2)) >> DOMAINTXFMRF_VTABLE_PRECBITS;
    }
    // bottom to top
    ...
    ip -= 2 * img_stride;
    for (i = height - 2; i >= 0; --i, dp -= dat_stride, ip -= img_stride) {
      const int v =  domaintxfmrf_vtable[iter][param][abs((ip[img_stride] >> shift) - (ip[0] >> shift))];
      dp[0] = (dp[0] * (DOMAINTXFMRF_VTABLE_PREC - v) +
               dp[dat_stride] * v + DOMAINTXFMRF_VTABLE_PREC / 2) >> DOMAINTXFMRF_VTABLE_PRECBITS;
    }
  }
...
void av1_domaintxfmrf_restoration_highbd(
    uint16_t *dgd, int width, int height, int stride, int param, int bit_depth) {
  int32_t dat[RESTORATION_TILEPELS_MAX];
  int i, j, t;
  for (i = 0; i < height; ++i) {
    for (j = 0; j < width; ++j) {
  ...
  for (t = 0; t < DOMAINTXFMRF_ITERS; ++t) {
    apply_domaintxfmrf_hor_highbd(t, param, dgd, width, height, stride,
                                  dat, width, bit_depth);
    apply_domaintxfmrf_ver_highbd(t, param, dgd, width, height, stride,
                                  dat, width, bit_depth);
    apply_domaintxfmrf_reduce_prec(dat, width, height, width);
  }
  for (i = 0; i < height; ++i) {
...
static void loop_domaintxfmrf_filter_tile_highbd(
    ...
    RestorationInternal *rst, int bit_depth, void *tmpbuf) {
  const int tile_width = rst->tile_width >> rst->subsampling_x;
  const int tile_height = rst->tile_height >> rst->subsampling_y;
  int h_start, h_end, v_start, v_end;
  (void)tmpbuf;
  ...
  av1_domaintxfmrf_restoration_highbd(
      data + h_start + v_start * stride,
      h_end - h_start, v_end - v_start, stride,
      rst->rsi->domaintxfmrf_info[tile_idx].sigma_r, bit_depth);
}

static void loop_domaintxfmrf_filter_highbd(uint8_t *data8, int width, int height,
                                            int stride, RestorationInternal *rst,
                                            uint8_t *tmpdata, int tmpstride,
                                            int bit_depth) {
  int tile_idx;
...
void av1_loop_restoration_rows(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  restore_func_highbd_type restore_funcs_highbd[RESTORE_TYPES] = {
    ...
    loop_wiener_filter_highbd, loop_domaintxfmrf_filter_highbd,
