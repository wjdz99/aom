...
static void loop_bilateral_filter_tile(uint8_t *data, int tile_idx, int width,
  ...
  for (subtile_idx = 0; subtile_idx < BILATERAL_SUBTILES; ++subtile_idx) {
    ...
    if (level < 0) {
      loop_copy_tile(data, tile_idx, subtile_idx, BILATERAL_SUBTILE_BITS, width,
                     height, stride, rst, dst, dst_stride);
      continue;
    }
    ...
    for (i = v_start; i < v_end; ++i) {
      memcpy(dst + i * dst_stride + h_start, tmpdata + i * tmpstride + h_start,
...
static void loop_wiener_filter_tile(uint8_t *data, int tile_idx, int width,
  ...
  // Note: This should really be a full-tile convolution, but here we do it
  // in blocks which match the size and alignment requirements of
  // (the SIMD versions of) aom_convolve8
...
static void loop_bilateral_filter_tile_highbd(uint16_t *data, int tile_idx,
  ...
  for (subtile_idx = 0; subtile_idx < BILATERAL_SUBTILES; ++subtile_idx) {
    ...
    if (level < 0) {
      loop_copy_tile_highbd(data, tile_idx, subtile_idx, BILATERAL_SUBTILE_BITS,
                            width, height, stride, rst, dst, dst_stride);
      continue;
    }
    wr_lut_ = (rst->keyframe ? bilateral_filter_coeffs_r_kf[level]
                             : bilateral_filter_coeffs_r[level]) +
...
static void loop_bilateral_filter_highbd(uint8_t *data8, int width, int height,
  ...
  for (tile_idx = 0; tile_idx < rst->ntiles; ++tile_idx) {
    loop_bilateral_filter_tile_highbd(data, tile_idx, width, height, stride,
                                      rst, tmpdata, tmpstride, bit_depth, dst,
                                      dst_stride);
  }
}
