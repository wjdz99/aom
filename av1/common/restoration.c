...
void apply_wiener_nonsep(const uint8_t *dgd, int width, int height, int stride,
  ...
  if (!is_uv || wienerns_uv_from_y_pixel == 0) {
    ...
    av1_convolve_nonsep_mask(dgd, width, height, stride, nsfilter, filter_, dst,
                             dst_stride, txskip_mask, mask_stride);
#else
...
#if CONFIG_WIENER_NONSEP_MASK
                               rui->txskip_mask + (j >> MIN_TX_SIZE_LOG2),
                               rui->mask_stride,
#endif  // CONFIG_WIENER_NONSEP_MASK
#if CONFIG_WIENER_NONSEP_CROSS_FILT
                               rui->luma + j, rui->luma_stride,
...
#if CONFIG_WIENER_NONSEP_MASK
  const uint8_t *txskip_mask_in_ru = rui->txskip_mask;
#endif  // CONFIG_WIENER_NONSEP_MASK
#if CONFIG_WIENER_NONSEP_CROSS_FILT
...
#if CONFIG_WIENER_NONSEP_MASK
  rui->txskip_mask = txskip_mask_in_ru;
#endif  // CONFIG_WIENER_NONSEP_MASK
#if CONFIG_WIENER_NONSEP_CROSS_FILT
  rui->luma = luma_in_ru;
#endif  // CONFIG_WIENER_NONSEP_CROSS_FILT
...
static void filter_frame_on_unit(const RestorationTileLimits *limits,
  ...
  int mask_idx_start =
      (limits->v_start >> MIN_TX_SIZE_LOG2) * ctxt->mask_stride +
      (limits->h_start >> MIN_TX_SIZE_LOG2);
  rsi->unit_info[rest_unit_idx].txskip_mask =
      &ctxt->txskip_mask[mask_idx_start];
#endif  // CONFIG_WIENER_NONSEP_MASK
#endif  // CONFIG_WIENER_NONSEP
...
static void foreach_rest_unit_in_planes(AV1LrStruct *lr_ctxt, AV1_COMMON *cm,
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    ctxt[plane].txskip_mask = &cm->tx_skip[plane][0];
#endif  // CONFIG_WIENER_NONSEP_MASK
#endif  // CONFIG_WIENER_NONSEP
