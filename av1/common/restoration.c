...
void av1_loop_restoration_filter_unit(
  ...
  uint8_t *data8_tl =
      data8 + (ptrdiff_t)limits->v_start * stride + limits->h_start;
  uint8_t *dst8_tl =
      dst8 + (ptrdiff_t)limits->v_start * dst_stride + limits->h_start;
...
static void save_deblock_boundary_lines(
  ...
  const ptrdiff_t src_stride = frame->strides[is_uv] << use_highbd;
