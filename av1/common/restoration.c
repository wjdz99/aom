...
int av1_loop_restoration_sb_is_rest_tl(const struct AV1Common *cm,
                                       int plane, int mi_row,
  ...
  const int ss_width = width >> (plane > 0 && cm->subsampling_x != 0);
  const int ss_height = height >> (plane > 0 && cm->subsampling_y != 0);

  int tile_w, tile_h, nhtiles, nvtiles;
  int ntiles =
    av1_get_rest_ntiles(ss_width, ss_height, cm->rst_info[0].restoration_tilesize,
