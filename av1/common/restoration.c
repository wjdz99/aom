...
#if CONFIG_WIENER_NONSEP_CROSS_FILT
const int wienerns_uv_pixel =
    wienerns_uv_from_uv_pixel + wienerns_uv_from_y_pixel;
#else
const int wienerns_uv_pixel = wienerns_uv_from_uv_pixel;
#endif  // CONFIG_WIENER_NONSEP_CROSS_FILT
const int wienerns_uv =
    sizeof(wienerns_coeff_uv) / sizeof(wienerns_coeff_uv[0]);
const int wienerns_yuv = wienerns_y + wienerns_uv;
...
void apply_wiener_nonsep(const uint8_t *dgd, int width, int height, int stride,
...
#if CONFIG_WIENER_NONSEP_CROSS_FILT
    is_uv ? wienerns_uv_pixel - wienerns_uv_from_uv_pixel : 0,
#else
    0,
#endif  // CONFIG_WIENER_NONSEP_CROSS_FILT
  ...
  if (!is_uv || wienerns_uv_pixel == wienerns_uv_from_uv_pixel) {
