...
int av1_alloc_restoration_struct(AV1_COMMON *cm,
  ...
  CHECK_MEM_ERROR(cm, rst_info->wiener_info,
                  (WienerInfo *)aom_memalign(
                      16, sizeof(*rst_info->wiener_info) * ntiles));
  memset(rst_info->wiener_info, 0, sizeof(*rst_info->wiener_info) * ntiles);
  CHECK_MEM_ERROR(cm, rst_info->sgrproj_info,
                  (SgrprojInfo *)aom_realloc(
