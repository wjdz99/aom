...
static void apply_wiener_nonsep(const uint8_t *dgd, int width, int height, 
  ...
  if ((w << 1) < width && (w << 1) < height) {
    ...
    for (int i = 0; i < sz; ++i) { tmpbuf[i] = 0; }

    for (int i = w; i < height - w; ++i) {
      for (int j = w; j < width - w; ++j) {
        for (int k = 0; k < num_pixels; ++k) {
          tmpbuf[(i - w)*crop_width + (j - w)] += filter[wiener_nsfilter[k][2]] * (
            dst[(i + wiener_nsfilter[k][0])*width + (j + wiener_nsfilter[k][1])] - dst[i*width + j]
          );
        }
        dst[i*width + j] = clip_pixel(tmpbuf[(i - w)*crop_width + (j + w)] >> wiener_nsbits);
      }
    }
  }
}

static void wiener_nonsep_filter_stripe(const RestorationUnitInfo *rui,
                                        int stripe_width, int stripe_height,
                                        int procunit_width, const uint8_t *src,
                                        int src_stride, uint8_t *dst, int dst_stride,
                                        int32_t *tmpbuf, int bit_depth) {
  (void)bit_depth;
  assert(bit_depth == 8);
  assert(sizeof(wiener_nsfilter)/sizeof(wiener_nsfilter[0]) <= WIENER_NONSEP_WIN);

  for (int j = 0; j < stripe_width; j += procunit_width) {
    int w = AOMMIN(procunit_width, stripe_width - j);
    apply_wiener_nonsep(src + j, w, stripe_height, src_stride, 
                        rui->wiener_nonsep_info.nsfilter, dst + j, dst_stride, tmpbuf);
  } 
}
