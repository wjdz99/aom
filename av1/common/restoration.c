...
static const uint16_t one_by_x[MAX_NELEM] = {
  65535, 32768, 21845, 16384, 13107, 10923,  9362,  8192,  7282,  6554,  5958,
  5461,   5041,  4681,  4369,  4096,  3855,  3641,  3449,  3277,  3121,  2979,
  2849,   2731,  2621,  2521,  2427,  2341,  2260,  2185,  2114,  2048,  1986,
  1928,   1872,  1820,  1771,  1725,  1680,  1638,  1598,  1560,  1524,  1489,
  1456,   1425,  1394,  1365,  1337,
};
#endif  // APPROXIMATE_SGR

void av1_selfguided_restoration(int32_t *dgd, int width, int height,
                                int stride, int bit_depth, int r, int eps,
                                int32_t *tmpbuf) {
  int32_t *A = tmpbuf;
  ...
  for (i = 0; i < height; ++i) {
    for (j = 0; j < width; ++j) {
      ...
      const int z = ((int64_t)s * (int64_t)p + (1 << (SGRPROJ_MTABLE_BITS - 1)))
          >> SGRPROJ_MTABLE_BITS;
      A[k] = x_by_xplus1[AOMMIN(z, 255)];
      B[k] = ((SGRPROJ_SGR - A[k]) * (int64_t)B[k] * (int64_t)one_by_x[n - 1] +
              (1 << (SGRPROJ_RECIP_BITS - 1))) >> SGRPROJ_RECIP_BITS;
#else
