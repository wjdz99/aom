...
static void setup_processing_stripe_boundary(
    ...
    int rsb_row, int use_highbd, int h, const AV1PixelRect *tile_rect,
    int loop_filter_across_tiles_v_enabled, uint8_t *data8, int data_stride,
...
static void restore_processing_stripe_boundary(
    ...
    int use_highbd, int h, const AV1PixelRect *tile_rect,
    int loop_filter_across_tiles_v_enabled, uint8_t *data8, int data_stride,
    int copy_above, int copy_below) {
  const int line_width =
      (limits->h_end - limits->h_start) + 2 * RESTORATION_EXTRA_HORZ;
...
void av1_loop_restoration_filter_unit(
  ...
  while (i < unit_h) {
    ...
    get_stripe_boundary_info(&remaining_stripes, tile_rect, ss_y,
                             loop_filter_across_tiles_h_enabled, &copy_above,
                             &copy_below);
    ...
    setup_processing_stripe_boundary(&remaining_stripes, rsb, rsb_row, highbd,
                                     h, tile_rect,
                                     loop_filter_across_tiles_v_enabled, data8,
...
static void filter_frame_on_unit(const RestorationTileLimits *limits,
  ...
  av1_loop_restoration_filter_unit(
      ...
      ctxt->cm->loop_filter_across_tiles_h_enabled, ctxt->ss_x, ctxt->ss_y,
      ctxt->highbd, ctxt->bit_depth, ctxt->data8, ctxt->data_stride, ctxt->dst8,
