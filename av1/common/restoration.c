...
void av1_alloc_restoration_struct(AV1_COMMON *cm, RestorationInfo *rsi,
  ...
  const int scaled_max_w = max_sb_w << MAX_MIB_SIZE_LOG2;
  const int scaled_max_h = max_sb_h << MAX_MIB_SIZE_LOG2;

  const int ss_x = is_uv && cm->subsampling_x;
  const int ss_y = is_uv && cm->subsampling_y;

  int max_tile_w = (scaled_max_w + ss_x) >> ss_x;
  int max_tile_h = (scaled_max_h + ss_y) >> ss_y;
#if CONFIG_FRAME_SUPERRES
  if (!av1_superres_unscaled(cm)) {
    av1_calculate_unscaled_superres_size(&max_tile_w, &max_tile_h,
  ...
  const int hpertile = (max_tile_w + (unit_size >> 1)) / unit_size;
  const int vpertile = (max_tile_h + (unit_size >> 1)) / unit_size;
...
// Which tile contains the superblock? Find that tile's bounds
#if CONFIG_MAX_TILE
  const int tile_row = get_tile_idx(cm->tile_row_start_sb, mi_row);
  const int tile_col = get_tile_idx(cm->tile_col_start_sb, mi_col);

  const int mi_top = cm->tile_row_start_sb[tile_row] << MAX_MIB_SIZE_LOG2;
  const int mi_left = cm->tile_col_start_sb[tile_col] << MAX_MIB_SIZE_LOG2;
#else
  ...
  *rcol1 = AOMMIN((mi_rel_col1 * mi_to_num + rnd) / (size * denom),
                  rsi->horz_units_per_tile);
  *rrow1 = AOMMIN((mi_rel_row1 * mi_to_num + rnd) / (size * denom),
