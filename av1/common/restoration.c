  ...
  for (i = 0; i < height; ++i) {
    ...
    for (j = 0; j < width2; j += 2) {
      ...
      if (j + 1 < width - 1) {  // odd col and not last
        ...
        const int nb = 6;
        const int32_t a = A[k] * 16 + (A[k - buf_stride] + A[k + buf_stride]) * 14 +
                          (A[k - 2] + A[k + 2]) * 4 +
                          (A[k - 2 - buf_stride] + A[k - 2 + buf_stride] +
                           A[k + 2 - buf_stride] + A[k + 2 + buf_stride]) *
                              3;
        const int32_t b = B[k] * 16 + (B[k - buf_stride] + B[k + buf_stride]) * 14 +
                          (B[k - 2] + B[k + 2]) * 4 +
        ...
        const int m = i * dst_stride + j + 1;
        const int nb = 6;
        const int32_t a = A[k] * 18 + (A[k - buf_stride] + A[k + buf_stride]) * 16 +
                          A[k - 2] * 6 +
                          (A[k - 2 - buf_stride] + A[k + 2 - buf_stride]) * 4;
        const int32_t b = B[k] * 18 + (B[k - buf_stride] + B[k + buf_stride]) * 16 +
                          B[k - 2] * 6 +
                          (B[k - 2 - buf_stride] + B[k + 2 - buf_stride]) * 4;
        const int32_t v = a * dgd[l] + b;
        dst[m] =
            ROUND_POWER_OF_TWO(v, SGRPROJ_SGR_BITS + nb - SGRPROJ_RST_BITS);
      }
