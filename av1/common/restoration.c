...
void av1_loop_restoration_filter_unit(const RestorationTileLimits *limits,
  ...
  if (unit_rtype == RESTORE_NONE) {
    copy_tile(unit_w, unit_h, data8_tl, stride, dst8, dst_stride, highbd);
    return;
  }
...
// Convolve the whole tile one stripe at a time
  ...
  while (i < unit_h) {
    ...
    stripe_filter(rui, unit_w, h, procunit_width, data8_tl + i * stride, stride,
                  dst8 + i * dst_stride, dst_stride, tmpbuf, bit_depth);
...
void av1_loop_restoration_filter_frame(YV12_BUFFER_CONFIG *frame,
  ...
  for (int plane = 0; plane < 3; ++plane) {
    ...
    for (int tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
      ...
      uint8_t *dst8 = dst->buffers[plane] +
                      limits.v_start * dst->strides[is_uv] + limits.h_start;
      ...
      av1_loop_restoration_filter_unit(
          ...
          frame->buffers[plane], frame->strides[is_uv], dst8,
