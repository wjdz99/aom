...
void apply_wiener_nonsep(const uint8_t *dgd, int width, int height, int stride,
  ...
  if (!is_uv || wienerns_uv_pixel == wienerns_uv_inter_pixel) {
    const NonsepFilterConfig nsfilter = {
      wienerns_prec_bits, is_uv ? wienerns_uv_pixel : wienerns_y_pixel,
      is_uv ? wienerns_config_uv : wienerns_config_y,
    };
    av1_convolve_nonsep(dgd, width, height, stride, &nsfilter, filter, dst,
                        dst_stride);
    return;
  }
...
void apply_wiener_nonsep_highbd(const uint8_t *dgd8, int width, int height,
                                int stride, const int16_t *filter, uint8_t *dst8,
  ...
  if (!is_uv || wienerns_uv_pixel == wienerns_uv_inter_pixel) {
    ...
    av1_convolve_nonsep_highbd(dgd8, width, height, stride, &nsfilter, filter,
