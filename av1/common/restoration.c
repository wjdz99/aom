...
static int setup_processing_stripe_boundary( int y0, int v_end, int h_start, int h_end, uint8_t *data, int stride, RestorationInternal *rst, int use_highbd) {
  ...
  boundary_above_buf += ((stripe_index-1)*2*boundary_stride+RESTORATION_EXTRA_HORZ) << use_highbd;
  boundary_below_buf += (stripe_index*2*boundary_stride+RESTORATION_EXTRA_HORZ) << use_highbd;
  ...
  for(i=0; i<2; i++) {
    ...
    if ( y>=0 && y<y0 && y>=y0-2) {
      ...
      uint8_t *new_data = boundary_above_buf + ((i*boundary_stride+x0) << use_highbd);
      //printf("above %3d %3d: %08x %08x : %08x %08x\n", y, x0, ((uint32_t*)p)[0], ((uint32_t*)p)[1], ((uint32_t*)new_data)[0], ((uint32_t*)new_data)[1]);
      // Save old pixels
  ...
  for(i=0; i<2; i++) {
    ...
    if ( y<v_end+2) {
      uint8_t *p = data + ((y*stride + x0)<<use_highbd);
      uint8_t *new_data = boundary_below_buf + ((i*boundary_stride+x0) << use_highbd);
...
static void restore_processing_stripe_boundary( int y0, int v_end, int h_start, int h_end, uint8_t *data, int stride, RestorationInternal *rst, int use_highbd) {
...
static void loop_wiener_filter_tile(uint8_t *data, int tile_idx, int width,
  ...
  av1_get_rest_tile_limits(tile_idx, 0, 0, rst->nhtiles, rst->nvtiles,
                           ...
                           0, 0, &h_start, &h_end, &v_start, &v_end);
#if CONFIG_STRIPED_LOOP_RESTORATION
  (void)procunit_height;
  i=v_start;
  while(i < v_end) {
    int h = setup_processing_stripe_boundary( i, v_end, h_start, h_end, data, stride, rst, 0);
    if (h<16)
        h=16;   // TODO(olah): Is this correct? Why does h need to be at least 16 (see note below)?
#else
...
#if CONFIG_STRIPED_LOOP_RESTORATION
    restore_processing_stripe_boundary( i, v_end, h_start, h_end, data, stride, rst, 0);
    i+=h;
#endif
  }
}
...
static void loop_wiener_filter_tile_highbd(uint16_t *data, int tile_idx,
  ...
  i=v_start;
  while(i < v_end) {
    int h = setup_processing_stripe_boundary( i, v_end, h_start, h_end, (uint8_t*)data, stride, rst, 1);
    if (h<16)
        h=16;   // TODO(olah): Is this correct? Why does h need to be at least 16 (see note below)?
#else
  // Convolve the whole tile (done in blocks here to match the requirements
...
#if CONFIG_STRIPED_LOOP_RESTORATION
    restore_processing_stripe_boundary( i, v_end, h_start, h_end, (uint8_t*)data, stride, rst, 1);
    i+=h;
#endif
  }
}
...
static void extend_line(uint8_t *buf,
                        int width,
                        int extend,
...
void av1_loop_restoration_save_boundary_lines(YV12_BUFFER_CONFIG *frame,
  ...
  int src_width, src_height, src_stride, stripe_height, stripe_offset, stripe_y, yy;
  ...
  for (p = 0; p < MAX_MB_PLANE; ++p) {
    ...
    if (use_highbitdepth) {
      src_buf = CONVERT_TO_BYTEPTR(src_buf);
    }
#endif
    ...
    // Loop over stripes
    for(stripe_y=stripe_offset; stripe_y<src_height; stripe_y+=stripe_height) {
      ...
      for(yy=0; yy<2; yy++) {
        if (stripe_y+yy < src_height) {
          ...
          extend_line(boundary_above_buf, src_width, RESTORATION_EXTRA_HORZ, use_highbitdepth);
      ...
      for(yy=2; yy<4; yy++) {
        if (stripe_y+yy < src_height) {
          ...
          extend_line(boundary_below_buf, src_width, RESTORATION_EXTRA_HORZ, use_highbitdepth);
          src_buf += src_stride << use_highbitdepth;
