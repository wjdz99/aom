    ...
    for (w = 0; w < x_mis; w++) {
      ...
      for (int idx = 0; idx < 2; ++idx) {
        ...
        if (ref_frame > INTRA_FRAME) {
          uint8_t ref_idx = cm->ref_frame_side[ref_frame];
...
static int motion_field_projection(AV1_COMMON *cm, MV_REFERENCE_FRAME ref_frame,
  ...
  int cur_rf_index[INTER_REFS_PER_FRAME + 1] = { 0 };
  int ref_rf_idx[INTER_REFS_PER_FRAME + 1] = { 0 };
  int cur_offset[INTER_REFS_PER_FRAME + 1] = { 0 };
  int ref_offset[INTER_REFS_PER_FRAME + 1] = { 0 };
...
void av1_setup_motion_field(AV1_COMMON *cm) {
  ...
  for (int ref_frame = LAST_FRAME; ref_frame <= INTER_REFS_PER_FRAME;
    ...
    if (frame_index > cur_frame_index) cm->ref_frame_side[ref_frame] = 1;
  }
  ...
  if (lst_buf_idx >= 0) {
    ...
    if (!is_lst_overlay) {
      motion_field_projection(cm, LAST_FRAME, ref_stamp, 1);
      //      motion_field_projection(cm, LAST_FRAME, ref_stamp, 2);
    }
