...
int appendNonCausalSamples(const AV1_COMMON *cm, MACROBLOCKD *xd, int mi_row,
                           ...
                           int *has_no_causal) {
  MB_MODE_INFO *const mbmi0 = &(xd->mi[0]->mbmi);
  BLOCK_SIZE bsize = mbmi0->sb_type;
  int ref_frame = mbmi0->ref_frame[0];
  int i, mi_step = 1, np;
  int global_offset_c = mi_col * MI_SIZE;
  int global_offset_r = mi_row * MI_SIZE;
  ...
  np = sortSamples_w_mv(pts_mv, &mbmi0->mv[0].as_mv, pts, pts_inref, np);

  // test if we can have causal projection
  if (np == 0) {
    *has_no_causal = 1;
  } else {
    if (find_projection(mbmi0->num_proj_ref[0], pts, pts_inref, bsize,
                        mbmi0->mv[0].as_mv.row, mbmi0->mv[0].as_mv.col,
                        &mbmi0->wm_params[0], mi_row, mi_col)) {
      *has_no_causal = 1;
    } else {
      *has_no_causal = 0;
    }
  }
  // remove this condition because we will search if ncwm for
  // simple translation now
...
int try_noncausal_warp(const AV1_COMMON *cm, MACROBLOCKD *xd, int mi_row,
  ...
  int has_no_causal_warp;

  mbmi->wm_params[0].wmtype = DEFAULT_WMTYPE;

  mbmi->num_proj_ref[0] = appendNonCausalSamples(
      cm, xd, mi_row, mi_col, pts0, pts_inref0, pts_mv0, &has_no_causal_warp);
  ...
  if (find_projection(mbmi->num_proj_ref[0], pts0, pts_inref0, bsize,
    ...
    *mbmi = backup_mbmi;
    ...
    if (!has_no_causal_warp) {
      c_diff = try_projection_mtx(&backup_mbmi.wm_params[0], mi_row, mi_col,
                                  mbmi->mv[0].as_mv.col, mbmi->mv[0].as_mv.row,
                                  bsize);
