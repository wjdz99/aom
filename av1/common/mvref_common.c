...
#if CONFIG_TMV
  const int frame_mvs_stride = (cm->mi_cols + 1) >> 1;
  MV_REF *frame_mvs =
      cm->cur_frame->mvs + ((mi_row & 0xfffe) >> 1) * frame_mvs_stride + ((mi_col & 0xfffe) >> 1);
#else
  const int frame_mvs_stride = cm->mi_cols;
  MV_REF *frame_mvs =
      cm->cur_frame->mvs + (mi_row & 0xfffe) * frame_mvs_stride + (mi_col & 0xfffe);
#endif  // CONFIG_TMV
  int w, h;
  x_mis = (x_mis + 1) >> 1;
  y_mis = (y_mis + 1) >> 1;
...
static int add_col_ref_mv(const AV1_COMMON *cm,
  ...
  const MV_REF *prev_frame_mvs =
      prev_frame_mvs_base + ((blk_row + 1) >> 1) * prev_frame_mvs_stride + ((blk_col + 1) >> 1);
  POSITION mi_pos;
  int ref, idx;
  int coll_blk_count = 0;
...
static void setup_ref_mv_list(const AV1_COMMON *cm, const MACROBLOCKD *xd,
...
#if CONFIG_TMV
  const int prev_frame_mvs_stride = (cm->mi_cols + 1) >> 1;
  const int tmi_row = mi_row & 0xfffe;
  const int tmi_col = mi_col & 0xfffe;
  const MV_REF *const prev_frame_mvs_base =
      cm->use_prev_frame_mvs
          ? cm->prev_frame->mvs + (tmi_row >> 1) * prev_frame_mvs_stride + (tmi_col >> 1)
          : NULL;
#else
  ...
  const MV_REF *const prev_frame_mvs_base =
      cm->use_prev_frame_mvs
          ? cm->prev_frame->mvs + (((mi_row >> 1) << 1) + 1) * prev_frame_mvs_stride +
                ((mi_col >> 1) << 1) + 1
          : NULL;
#else
  const MV_REF *const prev_frame_mvs_base =
  ...
  *refmv_count = 0;
  ...
  for (blk_row = 0; blk_row < xd->n8_h; ++blk_row) {
    for (blk_col = 0; blk_col < xd->n8_w; ++blk_col) {
      // prev_frame_mvs_base is not used here, tpl_mvs is used.
...
#endif
  {
    ...
    // spatial resolution.
    int tpl_sample_pos[5][2] = { { -2, xd->n8_w },
                                 { 0, xd->n8_w },
                                 { xd->n8_h, xd->n8_w },
                                 { xd->n8_h, 0 },
                                 { xd->n8_h, -2 } };
    ...
    for (blk_row = 0; blk_row < xd->n8_h; blk_row += mi_step) {
      for (blk_col = 0; blk_col < xd->n8_w; blk_col += mi_step) {
        // mi_step is 2 or 4, so blk_row and blk_col are always even numbers.
#if CONFIG_TMV
        coll_blk_count += add_col_ref_mv(
            cm, prev_frame_mvs_base, prev_frame_mvs_stride, xd, tmi_row, tmi_col, ref_frame, blk_row,
            blk_col, refmv_count, ref_mv_stack, mode_context);
#else
        coll_blk_count += add_col_ref_mv(
            cm, prev_frame_mvs_base, prev_frame_mvs_stride, xd, mi_row, mi_col, ref_frame, blk_row,
            blk_col, refmv_count, ref_mv_stack, mode_context);
#endif  // CONFIG_TMV
      }
    }
    ...
    for (i = 0; i < 5; ++i) {
      ...
      blk_col = tpl_sample_pos[i][1];
#if CONFIG_TMV
      coll_blk_count += add_col_ref_mv(cm, prev_frame_mvs_base, prev_frame_mvs_stride, xd, tmi_row,
                                       tmi_col, ref_frame, blk_row, blk_col,
                                       refmv_count, ref_mv_stack, mode_context);
#else
      coll_blk_count += add_col_ref_mv(cm, prev_frame_mvs_base, prev_frame_mvs_stride, xd, mi_row,
                                       mi_col, ref_frame, blk_row, blk_col,
                                       refmv_count, ref_mv_stack, mode_context);
#endif  // CONFIG_TMV
    }
...
static void find_mv_refs_idx(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  const MV_REF *const prev_frame_mvs =
      cm->use_prev_frame_mvs && inside
          ? cm->prev_frame->mvs + (tmi_row >> 1) * ((cm->mi_cols + 1) >> 1) + (tmi_col >> 1)
          : NULL;
#else
#if CONFIG_MV_COMPRESS
