...
static uint8_t scan_row_mbmi(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  for (i = 0; i < xd->n8_w && *refmv_count < MAX_REF_MV_STACK_SIZE;) {
    ...
    if (is_inside(tile, mi_col, mi_row, cm->mi_rows, cm->dependent_horz_tiles, &mi_pos)) {
#else
...
#if CONFIG_DEPENDENT_HORZTILES
  if (!is_inside(&xd->tile, mi_col, mi_row, cm->mi_rows, cm->dependent_horz_tiles, &mi_pos)) return coll_blk_count;
#else
...
#if CONFIG_DEPENDENT_HORZTILES
    if (is_inside(tile, mi_col, mi_row, cm->mi_rows, cm->dependent_horz_tiles, mv_ref)) {
#else
