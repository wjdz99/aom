...
static int add_tpl_ref_mv(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (rf[1] == NONE_FRAME) {
    ...
    int cur_offset_0 = get_relative_dist(&cm->seq_params.order_hint,
    ...
    int cur_offset_1 = get_relative_dist(&cm->seq_params.order_hint,
...
void av1_setup_frame_sign_bias(AV1_COMMON *cm) {
  ...
  for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
    const int buf_idx = cm->frame_refs[ref_frame - LAST_FRAME].idx;
    if (cm->seq_params.order_hint.enable_order_hint && buf_idx != INVALID_IDX) {
      const int ref_frame_offset =
          cm->buffer_pool->frame_bufs[buf_idx].cur_frame_offset;
      cm->ref_frame_sign_bias[ref_frame] =
          (get_relative_dist(&cm->seq_params.order_hint, ref_frame_offset,
...
static int motion_field_projection(AV1_COMMON *cm,
  ...
  int start_to_current_frame_offset = get_relative_dist(
      &cm->seq_params.order_hint, start_frame_offset, cur_frame_offset);

  for (MV_REFERENCE_FRAME rf = LAST_FRAME; rf <= INTER_REFS_PER_FRAME; ++rf) {
    ref_offset[rf] =
        get_relative_dist(&cm->seq_params.order_hint, start_frame_offset,
...
void av1_setup_motion_field(AV1_COMMON *cm) {
  ...
  if (!cm->seq_params.order_hint.enable_order_hint) return;
  ...
  for (int ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ref_frame++) {
    ...
    if (get_relative_dist(&cm->seq_params.order_hint, order_hint,
                          cur_order_hint) > 0)
  ...
  if (get_relative_dist(&cm->seq_params.order_hint,
...
void av1_setup_skip_mode_allowed(AV1_COMMON *cm) {
  ...
  if (!cm->seq_params.order_hint.enable_order_hint || frame_is_intra_only(cm) ||
    ...
    if (get_relative_dist(&cm->seq_params.order_hint, ref_offset,
      ...
      if (ref_frame_offset[0] == -1 ||
          get_relative_dist(&cm->seq_params.order_hint, ref_offset,
                            ref_frame_offset[0]) > 0) {
        ref_frame_offset[0] = ref_offset;
        ref_idx[0] = i;
      }
    } else if (get_relative_dist(&cm->seq_params.order_hint, ref_offset,
                                 cur_frame_offset) > 0) {
      // Backward reference
...
void av1_set_frame_refs(AV1_COMMON *const cm, int lst_map_idx,
  ...
  assert(cm->seq_params.order_hint.enable_order_hint);
  assert(cm->seq_params.order_hint.order_hint_bits_minus_1 >= 0);
  const int cur_frame_offset = (int)cm->frame_offset;
  const int cur_frame_sort_idx =
      1 << cm->seq_params.order_hint.order_hint_bits_minus_1;
  ...
  for (int i = 0; i < REF_FRAMES; ++i) {
    ...
    ref_frame_info[i].sort_idx =
        (offset == -1)
            ? -1
            : cur_frame_sort_idx + get_relative_dist(&cm->seq_params.order_hint,
