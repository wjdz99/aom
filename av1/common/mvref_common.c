...
void av1_get_mv_refs_adj(REF_MV_INFO *ref_mv_info, int ref_frame,
  ...
  *ref_mv_count_adj = 0;
  ...
  for (int i = 0; i < ref_mv_count_orig; ++i) {
    ...
    lower_mv_precision(&ref_mv_stack_adj[*ref_mv_count_adj].this_mv.as_mv, precision);
    if (is_compound) {
      lower_mv_precision(&ref_mv_stack_adj[*ref_mv_count_adj].comp_mv.as_mv, precision);
    }
    ...
    if (is_compound) {
      for (k = 0; k < *ref_mv_count_adj; ++k) {
        if (ref_mv_stack_adj[*ref_mv_count_adj].this_mv.as_int ==
            ...
            ref_mv_stack_adj[*ref_mv_count_adj].comp_mv.as_int == ref_mv_stack_adj[k].comp_mv.as_int)
          break;
      }
    } else {
      for (k = 0; k < *ref_mv_count_adj; ++k) {
        if (ref_mv_stack_adj[*ref_mv_count_adj].this_mv.as_int == ref_mv_stack_adj[k].this_mv.as_int)
          break;
      }
    }
