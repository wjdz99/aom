...
int findSamples(const AV1_COMMON *cm, MACROBLOCKD *xd, int mi_row, int mi_col,
  ...
  // TODO(yunqingwang): remove duplicate code.
  // scan the above rows
  if (up_available) {
    for (n = 0; n < bs; n++) {
      ...
      if (xd->n8_w <= n8_w) {
        ...
        if (mbmi->ref_frame[0] == ref_frame &&
          ...
          int cc_offset = col_offset * MI_SIZE + AOMMAX(bw, MI_SIZE) / 2 - 1;
      ...
      for (i = 0; i < AOMMIN(xd->n8_w, cm->mi_cols - mi_col); i += mi_step) {
        ...
        if (mbmi->ref_frame[0] == ref_frame &&
          ...
          int bw = block_size_wide[mbmi->sb_type];
          int bh = block_size_high[mbmi->sb_type];
          int cr_offset = -n * MI_SIZE - AOMMAX(bh, MI_SIZE) / 2 - 1;
    ...
    for (n = 0; n < bs; n++) {
      ...
      if (xd->n8_h <= n8_h) {
        ...
        if (mbmi->ref_frame[0] == ref_frame &&
          ...
          int bw = block_size_wide[mbmi->sb_type];
          int bh = block_size_high[mbmi->sb_type];
          int cr_offset = row_offset * MI_SIZE + AOMMAX(bh, MI_SIZE) / 2 - 1;
      ...
      for (i = 0; i < AOMMIN(xd->n8_h, cm->mi_rows - mi_row); i += mi_step) {
        ...
        if (mbmi->ref_frame[0] == ref_frame &&
          ...
          int cc_offset = -n * MI_SIZE - AOMMAX(bw, MI_SIZE) / 2 - 1;
          int x = cc_offset + global_offset_c;
          int y = cr_offset + global_offset_r;
