...
static AOM_INLINE void setup_ref_mv_list(
  ...
  *refmv_count = 0;
  ...
  bool one_part_dbscan = false;
  // Clustering for 2 Parts Seperately
  if (two_parts_dbscan) {
    // DBSCAN Parameters
  ...
  else if(one_part_dbscan){
    // DBSCAN Parameters (Clustering as One Part)
    ...
    if ((*refmv_count) > MAX_MV_REF_CANDIDATES) {
      mv_dbscan(ref_mv_stack, 0, (*refmv_count), min_points, dist_threshold,
