...
void av1_setup_skip_mode_allowed(AV1_COMMON *const cm) {
  ...
  if (cm->is_skip_mode_allowed && cm->use_ref_frame_mvs) {
    const int tpl_frame_buf_idx = ref_buf_idx[1];
    const int tpl_frame_ref_offsets[INTER_REFS_PER_FRAME] = {
      frame_bufs[tpl_frame_buf_idx].lst_frame_offset,
      frame_bufs[tpl_frame_buf_idx].lst2_frame_offset,
      frame_bufs[tpl_frame_buf_idx].lst3_frame_offset,
      frame_bufs[tpl_frame_buf_idx].gld_frame_offset,
      frame_bufs[tpl_frame_buf_idx].bwd_frame_offset,
      frame_bufs[tpl_frame_buf_idx].alt2_frame_offset,
      frame_bufs[tpl_frame_buf_idx].alt_frame_offset
    };

    for (int i = 0; i < INTER_REFS_PER_FRAME; ++i) {
      if (tpl_frame_ref_offsets[i] == ref_frame_offset[0]) {
...
static void av1_setup_mvs_for_skip_mode(const AV1_COMMON *cm,
                                        const MACROBLOCKD *xd, MODE_INFO *mi,
                                        MB_MODE_INFO *mbmi, int mi_row,
                                        int mi_col, int_mv nearest_mv[2],
  ...
  if (cm->tpl_frame_ref0_idx != INVALID_IDX) {
    ...
    for (i = 0; i < TEMPORAL_CANDIDATES; ++i) {
      ...
      if (tpl_mvs_row >= 0 && tpl_mvs_row < mvs_rows && tpl_mvs_col >= 0 &&
        ...
        // Check whether the temporal candidate block has its forward reference
        ...
        get_mv_projection(&candidate_mv.as_mv, tpl_frame_mv[0], cur_to_ref0,
                          ref1_to_ref0);
        const int_mv rf_mv[2] = { candidate_mv, mv_ref->mv[1] };
        SKIP_MODE_MV_LIST_ADD(rf_mv, mv_list, mv_list_count, 0, bw, bh, xd);
      }
    }
  }
