...
static int compare_ref_frame_info(const void *arg_a, const void *arg_b) {
  ...
  return (info_a->offset < info_b->offset)
             ? -1
             : ((info_a->offset > info_b->offset) ? 1 : 0);
}
...
void av1_set_frame_refs(AV1_COMMON *const cm, int lst_map_idx,
  ...
  for (ref_idx = 0; ref_idx < (INTER_REFS_PER_FRAME - 2); ref_idx++) {
    ...
    while (ref_frame_info[fwd_end_idx].map_idx == lst_map_idx ||
  ...
  // Assign all the remaining frame(s) to the earliest reference frame.
