...
#define PARTIALL_REF_MV_STACK_SIZE 8
...
#if CONFIG_EXT_COMP_REFMV
    int_mv
        partial_ref_mv_stack[][PARTIALL_REF_MV_STACK_SIZE],
    uint16_t
        partial_ref_mv_weight[][PARTIALL_REF_MV_STACK_SIZE],
#endif  // CONFIG_EXT_COMP_REFMV
      ...
      for (ref = 0; ref < 2; ++ref) {
        ...
        for (int i = 0; i < 2; ++i) {
          ...
          for (int j = 0; j < PARTIALL_REF_MV_STACK_SIZE; ++j) {
            if (partial_ref_mv_stack[i][j].as_int ==
                this_mv.as_int ||
...
#if CONFIG_EXT_COMP_REFMV
  int_mv
  partial_ref_mv_stack[2][PARTIALL_REF_MV_STACK_SIZE];
  uint16_t partial_ref_mv_weight[2]
                                          [PARTIALL_REF_MV_STACK_SIZE];
...
#if CONFIG_EXT_COMP_REFMV
                  partial_ref_mv_stack,
                  partial_ref_mv_weight,
#endif  // CONFIG_EXT_COMP_REFMV
