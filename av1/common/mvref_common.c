...
static void clamp_ext_compound_mv(const AV1_COMMON *const cm, 
                                  int_mv *mv, 
...
static void scale_mv(const int_mv this_refmv, int this_ref, int r1_dist,
  ...
  const float col_round =
      (((r1_dist < 0) != (r2_dist < 0)) && (this_refmv.as_mv.col > 0)) ? -0.5
                                                                       : 0.5;
  int32_t mv_row = 
      (int32_t)((float)this_refmv.as_mv.row * ratio + row_round);
  int32_t mv_col = 
      (int32_t)((float)this_refmv.as_mv.col * ratio + col_round);
  scaled_mv->as_mv.row =
    (int16_t)clamp(mv_row, INT16_MIN, INT16_MAX);
      //(int16_t)((float)this_refmv.as_mv.row * ratio + row_round);
  scaled_mv->as_mv.col =
    (int16_t)clamp(mv_col, INT16_MIN, INT16_MAX);
