...
static int find_mv_refs_idx(const AV1_COMMON *cm, const MACROBLOCKD *xd,
...
void av1_find_mv_refs(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (rf[1] != NONE)
    zeromv[1].as_int = gm_get_motion_vector(&cm->global_motion[rf[1]]).as_int;
  else
    zeromv[1].as_int = 0;
#else
...
#endif  // CONFIG_REF_MV
  {
    int count = find_mv_refs_idx(cm, xd, mi, ref_frame, mv_ref_list, -1, mi_row,
                                 mi_col, sync, data, mode_context);
    for (idx = count; idx < MAX_MV_REF_CANDIDATES; ++idx)
      mv_ref_list[idx].as_int = zeromv[0].as_int;
  }
  ...
  /* Note: If global motion is enabled, then we want to set the ALL_ZERO flag
     ...
     * Encoder decides to throw away global motion (eg, because it is never
...
void av1_append_sub8x8_mvs_for_idx(const AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  *ref_mv_count = 0;
  ...
  count = find_mv_refs_idx(cm, xd, mi, mi->mbmi.ref_frame[ref], mv_list, block,
                           mi_row, mi_col, NULL, NULL, NULL);
  for (idx = count; idx < MAX_MV_REF_CANDIDATES; ++idx) mv_list[idx].as_int = 0;
