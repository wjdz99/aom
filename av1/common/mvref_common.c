...
static void scan_row_mbmi(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  for (i = 0; i < end_mi;) {
    ...
    add_ref_mv_candidate(
        ...
        ref_mv_weight, gm_mv_candidates, cm->global_motion, col_offset + i,
        len * weight
#if CONFIG_EXT_NEW_REFMV
        ,
        ref_location_stack, location_count, xd, row_offset, col_offset + i
#endif
...
static void scan_col_mbmi(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  for (i = 0; i < end_mi;) {
    ...
    add_ref_mv_candidate(
        candidate, rf, refmv_count, ref_match_count, newmv_count, ref_mv_stack,
        ref_mv_weight, gm_mv_candidates, cm->global_motion, col_offset,
        len * weight
#if CONFIG_EXT_NEW_REFMV
        ,
        ref_location_stack, location_count, xd, row_offset + i, col_offset
#endif
...
static int_mv calc_affine_mv(LOCATION_INFO *source_points,
  ...
  const int64_t mv_col = roundf(my_projected_x - my_point.x);
  const int64_t mv_row = roundf(my_projected_y - my_point.y);
