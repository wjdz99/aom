...
static void clamp_scaled_mv(const AV1_COMMON *const cm, int_mv *mv, int mi_row,
                            int mi_col, int bw, int bh) {
  const int mi_width = bw >> 2;
  const int mi_height = bh >> 2;
...
void av1_get_scaled_mv(const AV1_COMMON *const cm, const int_mv refmv,
  ...
  clamp_scaled_mv(cm, scaled_mv, mi_row, mi_col, bw, bh);
}
