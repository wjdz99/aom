...
#define PARTIALLY_MATCHED_REF_MV_STACK_SIZE 8
...
#if CONFIG_EXT_COMP_REFMV
    CANDIDATE_MV *partially_matched_ref_mv_stack,
    uint16_t partially_matched_ref_mv_weight[][PARTIALLY_MATCHED_REF_MV_STACK_SIZE],
#endif  // CONFIG_EXT_COMP_REFMV
  ...
  if (rf[1] == NONE_FRAME) {
    ...
    if (candidate->ref_frame[0] != rf[0] || candidate->ref_frame[1] != rf[1]) {
      for (ref = 0; ref < 2; ++ref) {
        ...
        if (cand_ref == rf[0]) {
          for (int i = 0; i < PARTIALLY_MATCHED_REF_MV_STACK_SIZE; ++i) {
            if (partially_matched_ref_mv_stack[i].this_mv.as_int == this_mv.as_int ||
                partially_matched_ref_mv_weight[0][i] == 0) {
              partially_matched_ref_mv_stack[i].this_mv = this_mv;
              partially_matched_ref_mv_weight[0][i] += weight;
              break;
            }
          }
        }
        if (cand_ref == rf[1]) {
          for (int i = 0; i < PARTIALLY_MATCHED_REF_MV_STACK_SIZE; ++i) {
            if (partially_matched_ref_mv_stack[i].comp_mv.as_int == this_mv.as_int ||
                partially_matched_ref_mv_weight[1][i] == 0) {
              partially_matched_ref_mv_stack[i].comp_mv = this_mv;
              partially_matched_ref_mv_weight[1][i] += weight;
              break;
            }
          }
        }
...
#endif  // CONFIG_EXT_REFMV || CONFIG_ENHANCED_WARPED_MOTION
#if CONFIG_EXT_COMP_REFMV
        partially_matched_ref_mv_stack, partially_matched_ref_mv_weight,
#endif  // CONFIG_EXT_COMP_REFMV
...
#if CONFIG_EXT_COMP_REFMV
    for (int i = 0; i < PARTIALLY_MATCHED_REF_MV_STACK_SIZE; ++i) {
      if (partially_matched_ref_mv_stack[i].this_mv.as_int == this_refmv.as_int ||
          partially_matched_ref_mv_weight[0][i] == 0) {
        partially_matched_ref_mv_stack[i].this_mv = this_refmv;
        partially_matched_ref_mv_weight[0][i] += 2 * weight_unit;
        break;
      }
    }
    for (int i = 0; i < PARTIALLY_MATCHED_REF_MV_STACK_SIZE; ++i) {
      if (partially_matched_ref_mv_stack[i].comp_mv.as_int == comp_refmv.as_int ||
          partially_matched_ref_mv_weight[1][i] == 0) {
        partially_matched_ref_mv_stack[i].comp_mv = comp_refmv;
        partially_matched_ref_mv_weight[1][i] += 2 * weight_unit;
        break;
      }
    }
#endif
...
static void setup_ref_mv_list(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  *refmv_count = 0;
...
#if CONFIG_EXT_COMP_REFMV
  CANDIDATE_MV partially_matched_ref_mv_stack[PARTIALLY_MATCHED_REF_MV_STACK_SIZE];
  uint16_t partially_matched_ref_mv_weight[2][PARTIALLY_MATCHED_REF_MV_STACK_SIZE];
  av1_zero(partially_matched_ref_mv_weight);
#endif  // CONFIG_EXT_COMP_REFMV
  ...
  if (rf[1] > INTRA_FRAME && *refmv_count < MAX_REF_MV_STACK_SIZE) {
    int_mv buf[2][PARTIALLY_MATCHED_REF_MV_STACK_SIZE];
    int count[2] = {0};
    for (int i = 0; i < 2; ++i) {
      int j = 0;
      // Sort the candidates based on their weights in descending order.
      for (;; ++j) {
        int best = -1, max_weight = 0;
        for (int k = 0; k < PARTIALLY_MATCHED_REF_MV_STACK_SIZE; ++k) {
          if (partially_matched_ref_mv_weight[i][k] > max_weight) {
            max_weight = partially_matched_ref_mv_weight[i][k];
            best = k;
          }
        }
        if (best < 0) break;
        buf[i][j] = i ? partially_matched_ref_mv_stack[best].comp_mv :
                    partially_matched_ref_mv_stack[best].this_mv;
        partially_matched_ref_mv_weight[i][best] = 0;
      }
      count[i] = j;
    }
    ...
    if (count[0] > 0 && count[1] > 0) {
      //
      for (int i = 0; i < AOMMIN(max_num, count[0]) && *refmv_count < MAX_REF_MV_STACK_SIZE; ++i) {
        for (int j = 0; j < AOMMIN(max_num, count[1]) && *refmv_count < MAX_REF_MV_STACK_SIZE; ++j) {
