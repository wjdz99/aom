...
static uint8_t scan_row_mbmi(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  for (i = col_offset; i < end_mi && *refmv_count < MAX_REF_MV_STACK_SIZE;) {
    const MODE_INFO *const candidate_mi = candidate_mi0[i];
    const MB_MODE_INFO *const candidate = &candidate_mi->mbmi;
    int len = AOMMIN(xd->n8_w, mi_size_wide[candidate->sb_type]);
    if (use_step_16) len = AOMMAX(n8_w_16, len);
    else if (abs(row_offset) > 1) len = AOMMAX(len, n8_w_8);
    newmv_count += add_ref_mv_candidate(
        candidate_mi, candidate, rf, refmv_count, ref_mv_stack,
        cm->allow_high_precision_mv, len, block, i);
    i += len;
  }
...
static uint8_t scan_col_mbmi(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  for (i = row_offset; i < end_mi && *refmv_count < MAX_REF_MV_STACK_SIZE;) {
    const MODE_INFO *const candidate_mi =
        xd->mi[i * xd->mi_stride + col_offset];
    const MB_MODE_INFO *const candidate = &candidate_mi->mbmi;
    int len = AOMMIN(xd->n8_h, mi_size_high[candidate->sb_type]);
    if (use_step_16) len = AOMMAX(n8_h_16, len);
    else if (abs(col_offset) > 1) len = AOMMAX(len, n8_h_8);
    newmv_count += add_ref_mv_candidate(
        candidate_mi, candidate, rf, refmv_count, ref_mv_stack,
...
static void setup_ref_mv_list(const AV1_COMMON *cm, const MACROBLOCKD *xd,
...
#endif
  const int row_adj = (xd->mi[0]->mbmi.sb_type < BLOCK_8X8) && (mi_row & 0x01);
  const int col_adj = (xd->mi[0]->mbmi.sb_type < BLOCK_8X8) && (mi_col & 0x01);
  ...
  *refmv_count = 0;
  ...
  if (max_row_offset) {
    max_row_offset = find_valid_row_offset(tile, mi_row, cm->mi_rows, cm,
                                           max_row_offset);
    // row_offset = -1;
    if (abs(max_row_offset) >= 1)
      newmv_count += scan_row_mbmi(cm, xd, mi_col, block, rf,
  ...
  if (max_col_offset) {
    ...
    if (abs(max_col_offset) >= 1)
      newmv_count += scan_col_mbmi(cm, xd, mi_row, block, rf,
                                   -1, ref_mv_stack, refmv_count);
  }
  ...
  for (idx = 2; idx <= MVREF_ROWS; ++idx) {
    if (max_row_offset) {
      ...
      if (abs(row_offset) <= abs(max_row_offset))
        scan_row_mbmi(cm, xd, mi_col, block, rf, row_offset,
    ...
    if (max_col_offset) {
      ...
      if (abs(col_offset) <= abs(max_col_offset))
        scan_col_mbmi(cm, xd, mi_row, block, rf, col_offset,
                      ref_mv_stack, refmv_count);
    }
  }
