...
xd->mi[0]->mbmi.sb_type, mi_row,
        mi_col, xd->plane[0].subsampling_x, xd->plane[0].subsampling_y);
#else
...
static uint8_t scan_col_mbmi(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  for (i = 0; i < end_mi && *refmv_count < MAX_REF_MV_STACK_SIZE;) {
    ...
    newmv_count += add_ref_mv_candidate(
        ...
        gm_mv_candidates, cm->global_motion,
#endif  // CONFIG_GLOBAL_MOTION && USE_CUR_GM_REFMV
        col_offset, weight, cm->cur_frame_mv_precision_level, xd->mi[0]->mbmi.sb_type, mi_row,
        mi_col, xd->plane[0].subsampling_x, xd->plane[0].subsampling_y);
#else
    newmv_count += add_ref_mv_candidate(candidate_mi, candidate, rf,
                                        refmv_count, ref_mv_stack,
...
static uint8_t scan_blk_mbmi(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (is_inside(tile, mi_col, mi_row, cm->mi_rows, cm, &mi_pos) &&
    ...
    newmv_count += add_ref_mv_candidate(
        ...
        gm_mv_candidates, cm->global_motion,
#endif  // CONFIG_GLOBAL_MOTION && USE_CUR_GM_REFMV
        mi_pos.col, 2, cm->cur_frame_mv_precision_level, xd->mi[0]->mbmi.sb_type, mi_row,
        mi_col, xd->plane[0].subsampling_x, xd->plane[0].subsampling_y);
#else
    newmv_count += add_ref_mv_candidate(candidate_mi, candidate, rf,
                                        refmv_count, ref_mv_stack,
...
static void setup_ref_mv_list(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  *refmv_count = 0;
  ...
  if (abs(max_row_offset) >= 1)
    newmv_count +=
        scan_row_mbmi(cm, xd, mi_row, mi_col, block, rf, -1, ref_mv_stack, refmv_count,
#if CONFIG_GLOBAL_MOTION && USE_CUR_GM_REFMV
                      gm_mv_candidates,
#endif  // CONFIG_GLOBAL_MOTION && USE_CUR_GM_REFMV
  ...
  if (abs(max_col_offset) >= 1)
    newmv_count +=
        scan_col_mbmi(cm, xd, mi_row, mi_col, block, rf, -1, ref_mv_stack, refmv_count,
#if CONFIG_GLOBAL_MOTION && USE_CUR_GM_REFMV
                      gm_mv_candidates,
#endif  // CONFIG_GLOBAL_MOTION && USE_CUR_GM_REFMV
