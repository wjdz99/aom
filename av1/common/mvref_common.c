...
static AOM_INLINE void add_ref_mv_candidate(
  ...
  if (rf[1] == NONE_FRAME) {
    ...
    for (ref = 0; ref < 2; ++ref) {
      if (candidate->ref_frame[ref] == rf[0]) {
        ...
        if (same_side) {
          ...
          const int is_gm_block = is_global_mv_block(candidate,
            gm_params[candidate->ref_frame[ref]].wmtype);
          const int_mv cand_refmv = is_gm_block ? gm_mv_candidates[0] :
                                                  get_block_mv(candidate, ref);
          const MvSubpelPrecision fr_mv_precision =
    ...
    if (candidate_ref_idx0 != -1 &&
        candidate_ref_idx1 != -1) {
      int_mv this_refmv[2];
      const int is_gm_block =
        is_global_mv_block(candidate,
                           gm_params[rf[candidate_ref_idx0]].wmtype);
...
static AOM_INLINE void scan_row_mbmi(
 ...
 #if CONFIG_SMVP_IMPROVEMENT
    , int add_more_mvs, SINGLE_MV_CANDIDATE *single_mv,
    uint8_t *single_mv_count, CANDIDATE_MV *derived_mv_stack,
    uint16_t *derived_mv_weight, uint8_t *derived_mv_count
 #endif
     ) {
