...
static void add_ref_mv_candidate(
  ...
  if (rf[1] == NONE_FRAME) {
    // single reference frame
    uint8_t *newmv_count_ptr =
        (newmv_count == NULL) ? NULL : &newmv_count[rf[0]];
    single_add(candidate_mi, candidate, rf, &refmv_count[rf[0]],
               &ref_match_count[rf[0]], newmv_count_ptr, ref_mv_stack[rf[0]],
               use_hp, len,
#if USE_CUR_GM_REFMV
    ...
    uint8_t *newmv_count_ptr =
        (newmv_count == NULL) ? NULL : &newmv_count[ref_frame];
...
static int add_col_ref_mv(const AV1_COMMON *cm,
  ...
  for (ref = 0; ref < 2; ++ref) {
    if (prev_frame_mvs->ref_frame[ref] == ref_frame) {
      ...
      for (idx = 0; idx < refmv_count[ref_frame]; ++idx)
        if (this_refmv.as_int == ref_mv_stack[ref_frame][idx].this_mv.as_int)
          break;
