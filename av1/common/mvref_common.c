...
void av1_setup_frame_sign_bias(AV1_COMMON *cm) {
  ...
  for (int ref_frame = 0; ref_frame < cm->new_ref_frame_data.n_future_refs; ++ref_frame) {
  ...
  for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
    ...
    if (cm->new_ref_frame_data.n_total_refs > 0) { 
      int ranked_ref = convert_named_ref_to_ranked_ref_index(&cm->new_ref_frame_data, ref_frame);
      (void)ranked_ref; 
      assert(cm->ref_frame_sign_bias_nrs[ranked_ref] == cm->ref_frame_sign_bias[ref_frame]);
    }
#endif  // CONFIG_NEW_REF_SIGNALING
  }
}
...
#if CONFIG_NEW_REF_SIGNALING
  for (int ref_frame = 0; ref_frame < cm->new_ref_frame_data.n_past_refs; ref_frame++) {
