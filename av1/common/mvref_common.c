...
static AOM_INLINE bool check_rmb_cand(
    ...
    int refmv_count,
    int is_comp, int mi_row, int mi_col, int block_width, int block_height,
    int frame_width, int frame_height) {
  // Check if the MV candidate is already existing in the ref MV stack.
  int existing = 0;
  for (int i = 0; i < refmv_count; ++i) {
    if (ref_mv_stack[i].this_mv.as_int ==
        cand_mv.this_mv.as_int &&
        (!is_comp || ref_mv_stack[i].comp_mv.as_int ==
            cand_mv.comp_mv.as_int)) {
      existing = 1;
      break;
    }
  }
  ...
  *refmv_count = 0;
  ...
  if (*refmv_count < MAX_REF_MV_STACK_SIZE && ref_frame != INTRA_FRAME) {
    ...
    const CANDIDATE_MV *queue_left =
        ref_mv_bank_left->rmb_buffer[ref_frame];
    ...
    const int start_idx_above =
        ref_mv_bank_above->rmb_start_idx[ref_frame];
