...
static void setup_ref_mv_list(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  *refmv_count = 0;
  ...
  if (*refmv_count < MAX_REF_MV_STACK_SIZE && ref_frame != INTRA_FRAME) {
    const NL_REF_MV_INFO *nl_ref_mv_info_left = xd->nl_ref_mv_info_left_pt ?
        xd->nl_ref_mv_info_left_pt : &xd->nl_ref_mv_info_left;
    const CANDIDATE_MV* queue_left = nl_ref_mv_info_left->nl_ref_mv_queue[ref_frame];
    const int count_left = nl_ref_mv_info_left->nl_ref_mv_count[ref_frame];
    const int start_idx_left = nl_ref_mv_info_left->nl_ref_mv_start_idx[ref_frame];

    const int sb_col = xd->mi_col / cm->seq_params.mib_size;
    const NL_REF_MV_INFO *nl_ref_mv_info_above = xd->nl_ref_mv_info_above_pt ?
        &xd->nl_ref_mv_info_above_pt[sb_col] : &xd->nl_ref_mv_info_above[sb_col];
    const int count_above = nl_ref_mv_info_above->nl_ref_mv_count[ref_frame];
    const CANDIDATE_MV* queue_above = nl_ref_mv_info_above->nl_ref_mv_queue[ref_frame];
    const int start_idx_above = nl_ref_mv_info_above->nl_ref_mv_start_idx[ref_frame];
...
#if 0
    for (int i = 0; i < count; ++i) {
      const int idx = (start_idx_left + count - 1 - i) % NON_LOCAL_REF_MV_SIZE;
      int existing = 0;
      for (int j = 0; j < *refmv_count; ++j) {
        if (ref_mv_stack[j].this_mv.as_int == queue[idx].this_mv.as_int &&
            (!is_comp || ref_mv_stack[j].comp_mv.as_int == queue[idx].comp_mv.as_int)) {
          existing = 1;
          break;
        }
      }
      if (existing) continue;
      ref_mv_stack[*refmv_count] = queue[idx];
      ref_mv_weight[*refmv_count] = 1;
      if (++(*refmv_count) == MAX_REF_MV_STACK_SIZE) break;
    }
#endif
  }
