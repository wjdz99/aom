...
void av1_find_mv_refs_nrs(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  printf("{%d}[%d, %d](%d): count %d\n", cm->current_frame.order_hint,
         mi_row, mi_col, mi->sb_type, ref_mv_count[ref_frame_nrs]);
...
void av1_find_mv_refs(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  /*
  printf("{%d}[%d, %d](%d): count %d\n", cm->current_frame.order_hint,
         mi_row, mi_col, mi->sb_type, ref_mv_count[ref_frame]);
...
void av1_setup_motion_field(AV1_COMMON *cm) {
  ...
  for (int ref_frame = 0;
       ref_frame < cm->new_ref_frame_data.n_past_refs; ref_frame++) {
    const int index = cm->new_ref_frame_data.past_refs[ref_frame];
  ...
  for (int ref_frame = 0;
       ref_frame < cm->new_ref_frame_data.n_future_refs;
       ref_frame++) {
    const int index = cm->new_ref_frame_data.future_refs[ref_frame];
    if (!is_ref_overlay_nrs(cm, index)) {
