...
static void add_ref_mv_candidate(
  ...
  if (rf[1] == NONE_FRAME) {
    single_add(candidate_mi, candidate, rf, &refmv_count[rf[0]],
               &ref_match_count[rf[0]], &newmv_count[rf[0]],
    ...
    compound_add(candidate_mi, candidate, rf, &refmv_count[ref_frame],
                 &ref_match_count[ref_frame], &newmv_count[ref_frame],
...
static void setup_ref_mv_list(
  ...
  memcpy(nearest_match, ref_match_count, sizeof(uint8_t) * MODE_CTX_REF_FRAMES);
  memcpy(nearest_refmv_count, refmv_count,
         sizeof(uint8_t) * MODE_CTX_REF_FRAMES);
  ...
  for (int idx = 2; idx <= MVREF_ROWS; ++idx) {
    ...
    if (abs(row_offset) <= abs(max_row_offset) &&
      ...
      scan_row_mbmi(cm, xd, mi_row, mi_col, rf, row_offset, ref_mv_stack,
                    refmv_count, ref_match_count, newmv_count,
#if USE_CUR_GM_REFMV
                    gm_mv_candidates,
#endif  // USE_CUR_GM_REFMV
                    max_row_offset, &processed_rows);
  ...
  if (rf[1] > NONE_FRAME) {
    ...
    //         ++idx) {
    //      mv_ref_list[rf[0]][idx].as_int = ref_mv_stack[idx].this_mv.as_int;
    //      mv_ref_list[rf[1]][idx].as_int = ref_mv_stack[idx].comp_mv.as_int;
    //    }
...
void av1_find_mv_refs(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  //    find_mv_refs_idx(cm, xd, mi, ref_frame, mv_ref_list[rf[1]], mi_row,
  //    mi_col, sync,
  //                     data, compound_mode_context, zeromv[0], *ref_mv_count);
  //  }
}
