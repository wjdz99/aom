...
static int add_tpl_ref_mv(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (rf[1] == NONE_FRAME) {
    ...
    const RefCntBuffer *buf_0 =
        get_ref_frame_buf_const(cm, FWD_RF_OFFSET(rf[0]) + 1);
    ...
    const RefCntBuffer *buf_1 =
        get_ref_frame_buf_const(cm, FWD_RF_OFFSET(rf[1]) + 1);
...
static int motion_field_projection(AV1_COMMON *cm,
  ...
  const RefCntBuffer *start_frame_buf =
      get_ref_frame_buf_const(cm, FWD_RF_OFFSET(start_frame) + 1);
...
void av1_setup_skip_mode_allowed(AV1_COMMON *cm) {
  ...
  for (int i = 0; i < INTER_REFS_PER_FRAME; ++i) {
    const RefCntBuffer *buf = get_ref_frame_buf_const(cm, i + 1);
    if (!buf) continue;
