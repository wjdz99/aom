...
static void setup_ref_mv_list(
  ...
  *refmv_count = 0;
  ...
  if (abs(max_row_offset) >= 1)
    scan_row_mbmi(cm, xd, mi_row, mi_col, rf, -1, ref_mv_stack,
                  ref_mv_weight, refmv_count,
                  &row_match_count, &newmv_count, gm_mv_candidates,
                  max_row_offset, &processed_rows);
  // Scan the first left column mode info. col_offset = -1;
  if (abs(max_col_offset) >= 1)
    scan_col_mbmi(cm, xd, mi_row, mi_col, rf, -1, ref_mv_stack,
                  ref_mv_weight, refmv_count,
  ...
  while (len > 0) {
    ...
    for (int idx = 1; idx < len; ++idx) {
      if (ref_mv_weight[idx - 1] < ref_mv_weight[idx]) {
        ...
        uint16_t tmp_ref_mv_weight = ref_mv_weight[idx - 1];
  ...
  if (rf[1] > NONE_FRAME) {
    ...
    if (*refmv_count < MAX_MV_REF_CANDIDATES) {
      ...
      if (*refmv_count) {
        ...
        if (comp_list[0][0].as_int ==
            ref_mv_stack[0].this_mv.as_int &&
            comp_list[0][1].as_int ==
                ref_mv_stack[0].comp_mv.as_int) {
    ...
    *refmv_count < MAX_MV_REF_CANDIDATES;) {
    ...
    if (mv_ref_list != NULL) {
      ...
      for (int idx = 0;
           idx < AOMMIN(MAX_MV_REF_CANDIDATES, *refmv_count); ++idx) {
        mv_ref_list[idx].as_int =
...
void av1_find_mv_refs(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  setup_ref_mv_list(cm, xd, ref_frame, &ref_mv_count[ref_frame],
                    ...
                    mv_ref_list[ref_frame], zeromv, mi_row, mi_col,
                    mode_context);
}
