...
void av1_setup_frame_sign_bias(AV1_COMMON *cm) {
  ...
  for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
    RefCntBuffer *buf =
        cm->current_frame.frame_refs[ref_frame - LAST_FRAME].buf;
    if (cm->seq_params.order_hint_info.enable_order_hint && buf) {
...
void av1_setup_motion_field(AV1_COMMON *cm) {
  ...
  if (!cm->seq_params.order_hint_info.enable_order_hint) return;
  ...
  RefCntBuffer *ref_buf[INTER_REFS_PER_FRAME];
  int ref_order_hint[INTER_REFS_PER_FRAME];

  for (int ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ref_frame++) {
    const int ref_idx = ref_frame - LAST_FRAME;
    RefCntBuffer *buf = cm->current_frame.frame_refs[ref_idx].buf;
    int order_hint = 0;
    ...
    if (get_relative_dist(&cm->seq_params.order_hint_info, order_hint,
                          cur_order_hint) > 0)
  ...
  if (ref_buf[LAST_FRAME - LAST_FRAME]) {
  ...
  if (ref_stamp >= 0 && ref_buf[LAST2_FRAME - LAST_FRAME])
...
void av1_setup_skip_mode_allowed(AV1_COMMON *cm) {
  ...
  for (int i = 0; i < INTER_REFS_PER_FRAME; ++i) {
    RefCntBuffer *buf = cm->current_frame.frame_refs[i].buf;
    if (!buf) continue;
    ...
    } else if (get_relative_dist(&cm->seq_params.order_hint_info,
                                 ref_order_hint, cur_order_hint) > 0) {
      // Backward reference
  ...
  if (ref_idx[0] != INVALID_IDX && ref_idx[1] != INVALID_IDX) {
    ...
    cm->current_frame.skip_mode_info.ref_frame_idx_0 =
        AOMMIN(ref_idx[0], ref_idx[1]);
    cm->current_frame.skip_mode_info.ref_frame_idx_1 =
        AOMMAX(ref_idx[0], ref_idx[1]);
