...
static void add_ref_mv_candidate(
    ...
    uint16_t
        partially_matched_ref_mv_weight[][PARTIALLY_MATCHED_REF_MV_STACK_SIZE],
#endif  // CONFIG_EXT_COMP_REFMV
      ...
      for (ref = 0; ref < 2; ++ref) {
        ...
        if (cand_ref == rf[0]) {
          for (int i = 0; i < PARTIALLY_MATCHED_REF_MV_STACK_SIZE; ++i) {
            if (partially_matched_ref_mv_stack[i].this_mv.as_int ==
                    this_mv.as_int ||
        ...
        if (cand_ref == rf[1]) {
          for (int i = 0; i < PARTIALLY_MATCHED_REF_MV_STACK_SIZE; ++i) {
            if (partially_matched_ref_mv_stack[i].comp_mv.as_int ==
                    this_mv.as_int ||
  ...
  *refmv_count = 0;
...
#if CONFIG_EXT_COMP_REFMV
  CANDIDATE_MV
      partially_matched_ref_mv_stack[PARTIALLY_MATCHED_REF_MV_STACK_SIZE];
  uint16_t partially_matched_ref_mv_weight[2]
                                          [PARTIALLY_MATCHED_REF_MV_STACK_SIZE];
...
#if CONFIG_EXT_COMP_REFMV
                  partially_matched_ref_mv_stack,
                  partially_matched_ref_mv_weight,
#endif  // CONFIG_EXT_COMP_REFMV
  ...
  if (cm->allow_ref_frame_mvs) {
    ...
    for (int blk_row = 0; blk_row < blk_row_end; blk_row += step_h) {
      for (int blk_col = 0; blk_col < blk_col_end; blk_col += step_w) {
        int ret = add_tpl_ref_mv(
            cm, xd, mi_row, mi_col, ref_frame, ref_mv_info, blk_row, blk_col,
#if CONFIG_EXT_COMP_REFMV
            partially_matched_ref_mv_stack, partially_matched_ref_mv_weight,
#endif  // CONFIG_EXT_COMP_REFMV
  ...
  if (rf[1] > INTRA_FRAME && *refmv_count < MAX_REF_MV_STACK_SIZE) {
    ...
    if (count[0] > 0 && count[1] > 0) {
      //
      for (int i = 0; i < AOMMIN(max_num, count[0]) &&
                      *refmv_count < MAX_REF_MV_STACK_SIZE;
           ++i) {
        for (int j = 0; j < AOMMIN(max_num, count[1]) &&
                        *refmv_count < MAX_REF_MV_STACK_SIZE;
             ++j) {
