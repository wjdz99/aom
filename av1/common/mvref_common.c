...
static void motion_field_fwd_projection(AV1_COMMON *cm, int ref_stamp) {
  ...
  for (int blk_row = 0; blk_row < mvs_rows && !is_lst_overlay; ++blk_row) {
    for (int blk_col = 0; blk_col < mvs_cols; ++blk_col) {
      ...
      if (ref_frame[0] <= GOLDEN_FRAME && ref_frame[0] > INTRA_FRAME) {
        ...
        get_mv_projection(&this_mv.as_mv, fwd_mv, cur_to_lst, ref_frame_offset);
...
static void motion_field_bwd_projection(AV1_COMMON *cm,
                                        MV_REFERENCE_FRAME ref_frame,
                                        int ref_stamp) {
  TPL_MV_REF *tpl_mvs_base = cm->tpl_mvs;
...
void av1_setup_motion_field(AV1_COMMON *cm) {
  ...
  if (lst_buf_idx >= 0) {
    motion_field_fwd_projection(cm, ref_stamp);
    --ref_stamp;
  }

  if (bwd_frame_index > cur_frame_index) {
    motion_field_bwd_projection(cm, BWDREF_FRAME, ref_stamp);
    --ref_stamp;
  }

  if (alt2_frame_index > cur_frame_index) {
    motion_field_bwd_projection(cm, ALTREF2_FRAME, ref_stamp);
    --ref_stamp;
  }

  if (alt_frame_index > cur_frame_index && ref_stamp >= 0)
    motion_field_bwd_projection(cm, ALTREF_FRAME, ref_stamp);
}
#endif  // CONFIG_MFMV
