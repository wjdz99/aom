...
static void scale_mv(const int_mv this_refmv, int this_ref, int r1_dist,
  ...
  // integer
  const float row_round = this_refmv.as_mv.row < 0 ? -0.5 : 0.5;
  const float col_round = this_refmv.as_mv.col < 0 ? -0.5 : 0.5;
  scaled_mv->as_mv.row =
...
static void scan_row_mbmi(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  for (i = 0; i < end_mi;) {
    ...
    if (rf[1] != NONE_FRAME) {
      int_mv scaled_mv;
      av1_get_scaled_mv(cm, ref_mv_stack[2].this_mv, 0, rf, 1, &scaled_mv);
    }
    ///////////
