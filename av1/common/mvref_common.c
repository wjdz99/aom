...
static void scan_row_mbmi(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  for (i = 0; i < end_mi;) {
    ...
    add_ref_mv_candidate(
        candidate, ref_frame, ref_mv_info, rf, refmv_count, ref_match_count,
  ...
  *refmv_count = 0;
  ...
  if (abs(max_row_offset) >= 1)
    scan_row_mbmi(cm, xd, mi_row, mi_col, ref_frame, ref_mv_info, rf, -1,
                  ref_mv_stack, ref_mv_weight, refmv_count, &row_match_count,
                  &newmv_count, gm_mv_candidates,
#if CONFIG_EXT_REFMV
  ...
  if (abs(max_col_offset) >= 1)
    scan_col_mbmi(cm, xd, mi_row, mi_col, ref_frame, ref_mv_info, rf, -1,
                  ref_mv_stack, ref_mv_weight, refmv_count, &col_match_count,
                  &newmv_count, gm_mv_candidates,
#if CONFIG_EXT_REFMV
                  ref_location_stack, &location_count,
#endif
  ...
  if (has_tr)
    scan_blk_mbmi(cm, xd, mi_row, mi_col, ref_frame, ref_mv_info, rf, -1,
                  xd->n4_w, ref_mv_stack, ref_mv_weight, &row_match_count,
                  &newmv_count,
#if CONFIG_EXT_REFMV
                  ref_location_stack, &location_count,
#endif
  ...
  if (cm->allow_ref_frame_mvs) {
      ...
      for (int blk_col = 0; blk_col < blk_col_end; blk_col += step_w) {
        int ret =
            add_tpl_ref_mv(cm, xd, mi_row, mi_col, ref_frame, ref_mv_info,
                           blk_row, blk_col, gm_mv_candidates, refmv_count,
    ...
    for (int i = 0; i < 3 && allow_extension; ++i) {
      ...
      add_tpl_ref_mv(cm, xd, mi_row, mi_col, ref_frame, ref_mv_info, blk_row,
                     blk_col, gm_mv_candidates, refmv_count, ref_mv_stack,
                     ref_mv_weight, mode_context);
    }
  }
  ...
  scan_blk_mbmi(cm, xd, mi_row, mi_col, ref_frame, ref_mv_info, rf, -1, -1,
                ref_mv_stack, ref_mv_weight, &row_match_count,
                &dummy_newmv_count,
#if CONFIG_EXT_REFMV
                ref_location_stack, &location_count,
#endif
  ...
  for (int idx = 2; idx <= MVREF_ROW_COLS; ++idx) {
    ...
    if (abs(row_offset) <= abs(max_row_offset) &&
      ...
      scan_row_mbmi(cm, xd, mi_row, mi_col, ref_frame, ref_mv_info, rf,
                    row_offset, ref_mv_stack, ref_mv_weight, refmv_count,
    ...
    if (abs(col_offset) <= abs(max_col_offset) &&
      ...
      scan_col_mbmi(cm, xd, mi_row, mi_col, ref_frame, ref_mv_info, rf,
                    col_offset, ref_mv_stack, ref_mv_weight, refmv_count,
...
void av1_get_mv_refs_adj(REF_MV_INFO *ref_mv_info, int ref_frame,
  ...
  *count_adj = 0;
  ...
  for (int i = 0; i < ref_mv_count_orig; ++i) {
    ...
    lower_mv_precision(&stack_adj[*count_adj].this_mv.as_mv,
                       precision);
    if (is_compound) {
      lower_mv_precision(&stack_adj[*count_adj].comp_mv.as_mv,
                         precision);
    }
    ...
    if (is_compound) {
      for (k = 0; k < *count_adj; ++k) {
        if (stack_adj[*count_adj].this_mv.as_int ==
            ...
            stack_adj[*count_adj].comp_mv.as_int ==
                stack_adj[k].comp_mv.as_int)
          break;
      }
    } else {
      for (k = 0; k < *count_adj; ++k) {
