...
static int add_tpl_ref_mv(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (rf[1] == NONE_FRAME) {
    ...
    RefCntBuffer *buf_0 = cm->frame_refs[FWD_RF_OFFSET(rf[0])].buf;
    ...
    int cur_offset_0 = get_relative_dist(&cm->seq_params.order_hint_info,
                                         cur_frame_index, frame0_index);
    RefCntBuffer *buf_1 = cm->frame_refs[FWD_RF_OFFSET(rf[1])].buf;
...
void av1_setup_motion_field(AV1_COMMON *cm) {
  ...
  for (int ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ref_frame++) {
    ...
    RefCntBuffer *buf = cm->frame_refs[ref_idx].buf;
    int order_hint = 0;
...
void av1_setup_skip_mode_allowed(AV1_COMMON *cm) {
  ...
  for (int i = 0; i < INTER_REFS_PER_FRAME; ++i) {
    RefCntBuffer *buf = cm->frame_refs[i].buf;
    if (!buf) continue;
