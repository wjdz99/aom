...
static int_mv get_scaled_mv(const int_mv this_refmv, int this_ref, int r1_dist, int r2_dist) {
  ...
  scaled_mv.as_mv.row = (int16_t)((double)this_refmv.as_mv.row * ratio);
  scaled_mv.as_mv.col = (int16_t)((double)this_refmv.as_mv.col * ratio);
//if (scaled_mv.as_mv.row != this_refmv.as_mv.row ||
//    scaled_mv.as_mv.col != this_refmv.as_mv.col)
  //  printf("%d %d, %d %d, %f\n", scaled_mv.as_mv.row, scaled_mv.as_mv.col, this_refmv.as_mv.row, this_refmv.as_mv.col, ratio);
//scaled_mv.as_mv.row = this_refmv.as_mv.row + 1;// + 0.5);
//scaled_mv.as_mv.col = this_refmv.as_mv.col + 1;// + 0.5);
  return scaled_mv;
  //return this_refmv;
}
    ...
    for (ref = 0; ref < 2; ++ref) {
      if (candidate->ref_frame[ref] == rf[0]) {
        ...
        const int_mv this_refmv = is_gm_block
                                      ? gm_mv_candidates[0] // sarahparker why is this 0 and not ref?
    ...
    for (ref = 0; ref < 2; ++ref) {
      if (candidate->ref_frame[ref] == rf[ref]) {
        ...
        const int_mv this_refmv = is_gm_block
                                      ...
                                      : get_sub_block_mv(candidate, ref, col);
        for (index = 0; index < *refmv_count; ++index) {
          if ((ref == 0 && (ref_mv_stack[index].this_mv.as_int == this_refmv.as_int)) ||
             (ref == 1 && (ref_mv_stack[index].comp_mv.as_int == this_refmv.as_int))) {
            ref_mv_weight[index] += weight;
            break;
          }
        }
        ...
        //  printf("debug\n");
          const int_mv scaled_refmv = get_scaled_mv(this_refmv, ref, r0_dist, r1_dist);
          lower_mv_precision(&scaled_refmv.as_mv, cm->mv_precision);
          if (ref == 0) {
            ref_mv_stack[index].this_mv = this_refmv;
            ref_mv_stack[index].comp_mv = scaled_refmv;
          } else {
            ref_mv_stack[index].this_mv = scaled_refmv;
...
void av1_find_mv_refs(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (mi_row == 50 && mi_col == 72 && bsize == BLOCK_32X8 /*&& mi->mode == 18*/ && ref_frame == 7 && cm->cur_frame->order_hint == 24)
