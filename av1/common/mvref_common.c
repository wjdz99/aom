  ...
  *refmv_count = 0;
  ...
  if (abs(max_col_offset) >= 1)
    scan_col_mbmi(cm, xd,
                  ...
                  mi_col,
#endif  // CONFIG_EXT_RECUR_PARTITIONS
                  mi_row, rf, -1, ref_mv_stack, ref_mv_weight, refmv_count,
  ...
  if (*refmv_count > refmv_count_max_ctx) *refmv_count = refmv_count_max_ctx;
  ...
  if (rf[1] > NONE_FRAME) {
    ...
    if (*refmv_count < cm->features.max_drl_bits + 1) {
      int stack_idx;
      for (stack_idx = 0; stack_idx < *refmv_count; ++stack_idx) {
        const int_mv stack_mv = ref_mv_stack[stack_idx].this_mv;
