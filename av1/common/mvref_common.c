...
#endif
                                    BLOCK_SIZE bsize,
                                    int mi_row, int mi_col, int subsampling_x,
                                    int subsampling_y) {
  int index = 0, ref;
  int newmv_count = 0;
#if CONFIG_CB4X4
    ...
    for (ref = 0; ref < 2; ++ref) {
      if (candidate->ref_frame[ref] == rf[0]) {
        ...
        if (candidate->motion_mode == WARPED_CAUSAL) {
          ...
          this_refmv.as_mv.col = ROUND_POWER_OF_TWO_SIGNED(out[0] + out[2], SCALING_FCT + 1);
          this_refmv.as_mv.row = ROUND_POWER_OF_TWO_SIGNED(out[1] + out[3], SCALING_FCT + 1);
...
static void setup_ref_mv_list(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  *refmv_count = 0;
  ...
  if (abs(max_row_offset) >= 1)
    newmv_count += scan_row_mbmi(cm, xd, mi_row, mi_col, block, rf, -1, ref_mv_stack,
                                 refmv_count, max_row_offset, &processed_rows);
  // Scan the first left column mode info. col_offset = -1;
  if (abs(max_col_offset) >= 1)
    newmv_count += scan_col_mbmi(cm, xd, mi_row, mi_col, block, rf, -1, ref_mv_stack,
