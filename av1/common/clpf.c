...
void aom_clpf_cdef(const uint8_t *src, const uint16_t *dering, uint8_t *dst, int sstride,
                   int fstride, int dstride, int x0, int y0, int sizex, int sizey,
                   unsigned int strength, BOUNDARY_TYPE bt,
                   unsigned int damping) {
  int x, y;
  const int xmin = x0 - !(bt & TILE_LEFT_BOUNDARY) * 2;
  ...
  for (y = y0; y < y0 + sizey; y++) {
    for (x = x0; x < x0 + sizex; x++) {
      ...
      const int delta =
          av1_clpf_sample(X, A, B, C, D, E, F, G, H, strength, damping);
      dst[y * dstride + x] = clamp((int)dering[y * fstride + x] + delta, 0, 255);
    }
  }
}
