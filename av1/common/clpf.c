...
int av1_clpf_sample(int X, int A, int B, int C, int D, int E, int F, int G, int H, int I, int J, int K, int L, int b, unsigned int bd) {
  int delta =
    4 * constrain(A - X, b, bd) + 1 * constrain(B - X, b, bd) +
    3 * constrain(C - X, b, bd) + 3 * constrain(D - X, b, bd) +
    1 * constrain(E - X, b, bd) + 4 * constrain(F - X, b, bd) +
    0 * constrain(G - X, b, bd) + 0 * constrain(H - X, b, bd) +
    0 * constrain(I - X, b, bd) + 0 * constrain(J - X, b, bd) +
    0 * constrain(K - X, b, bd) + 0 * constrain(L - X, b, bd);
