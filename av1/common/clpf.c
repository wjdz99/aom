...
int av1_clpf_sample(int X, int A, int B, int C, int D, int E, int F, int G, int H, int I, int J, int K, int L, int b) {
  int delta =
    9 * constrain(A - X, b) + 2 * constrain(B - X, b) +
    9 * constrain(C - X, b) + 9 * constrain(D - X, b) +
    2 * constrain(E - X, b) + 9 * constrain(F - X, b) +
    2 * constrain(G - X, b) + 2 * constrain(H - X, b) +
    5 * constrain(I - X, b) + 5 * constrain(J - X, b) +
    5 * constrain(K - X, b) + 5 * constrain(L - X, b);
