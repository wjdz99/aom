...
int av1_clpf_frame(const YV12_BUFFER_CONFIG *orig_dst,
  ...
  const int width = cm->mi_cols * bs;
  const int height = cm->mi_rows * bs;
  int xpos, ypos;
  const int sstride = rec->y_stride;
  int dstride = orig_dst->y_stride;
  const int num_fb_hor = (width + (1 << fb_size_log2) - bs) >> fb_size_log2;
  const int num_fb_ver = (height + (1 << fb_size_log2) - bs) >> fb_size_log2;
  int block_index = 0;
  uint8_t *cache = NULL;
