...
int av1_clpf_frame(
  ...
  int c, k, l, m, n;
  const int subx = comp && frame->subsampling_x;
  const int suby = comp && frame->subsampling_y;
  const int bs = (subx || suby) ? 4 : 8;
  const int bslog = get_msb(bs);
  int width = comp ? frame->uv_crop_width : frame->y_crop_width;
  int height = comp ? frame->uv_crop_height : frame->y_crop_height;
  int xpos, ypos;
  const int sstride = comp ? frame->uv_stride : frame->y_stride;
  int dstride = bs;
  ...
  uint8_t *src_buffer =
      comp ? (comp == 1 ? frame->u_buffer : frame->v_buffer) : frame->y_buffer;
  uint8_t *dst_buffer =
