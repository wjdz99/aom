...
static INLINE int get_dq_profile_from_ctx(int qindex, int is_inter,
                                          PLANE_TYPE plane_type) {
  // intra/inter, Y/UV, ctx, qrange
  static const int def_dq_profile_lookup[REF_TYPES][PLANE_TYPES][QUANT_RANGES] =
      {
        {
            // intra
            { 2, 1 },  // Y
  ...
  return def_dq_profile_lookup[is_inter][plane_type][qindex_to_qrange(qindex)];
}
