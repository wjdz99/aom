...
static INLINE int allow_warp(const MODE_INFO *const mi,
                             ...
                             int build_for_obmc,
#endif  // CONFIG_MOTION_VAR
                             WarpedMotionParams *final_wm_params) {

  const MB_MODE_INFO *const mbmi = &mi->mbmi;
  set_default_warp_params(final_wm_params);
  (void)mbmi;
  (void)final_wm_params;
...
// Only global motion configured
  ...
  if (warp_types->global_warp_allowed) {
    memcpy(final_wm_params, gm_params, sizeof(WarpedMotionParams));
    return 1;
  }
...
// Only warped motion configured
  ...
  if (mbmi->motion_mode == WARPED_CAUSAL && warp_types->local_warp_allowed) {
    memcpy(final_wm_params, &mbmi->wm_params[0], sizeof(WarpedMotionParams));
    return 1;
  }
...
// Motion var and global motion configured
  ...
  // TODO(sarahparker) only warp if !build_for_obmc, this will be included
  // in a followup patch
  (void)build_for_obmc;
  if (warp_types->global_warp_allowed) {
    memcpy(final_wm_params, gm_params, sizeof(WarpedMotionParams));
...
// Motion var and warped motion configured
  ...
  if (warp_types->local_warp_allowed) {
    if ((build_for_obmc && WARP_NEIGHBORS_WITH_OBMC) || (!build_for_obmc)) {
      memcpy(final_wm_params, &mbmi->wm_params[0], sizeof(WarpedMotionParams));
      return 1;
    }
  }
...
// Make sure the selected motion mode is valid for this configuration
  ...
  if (do_warp) {
    ...
    av1_warp_plane(&final_wm_params,
#if CONFIG_AOM_HIGHBITDEPTH
