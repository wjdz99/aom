...
void av1_convolve_horiz_scale(const uint8_t *src, int src_stride, uint8_t *dst,
                              int dst_stride, int w, int h,
                              const InterpFilterParams filter_params,
                              const int subpel_x_q8, int x_step_q8,
                              ConvolveParams *conv_params) {
  int x, y;
  int filter_size = filter_params.taps;
  assert(conv_params->round == CONVOLVE_OPT_ROUND);
  src -= filter_size / 2 - 1;
  for (y = 0; y < h; ++y) {
    int x_q8 = subpel_x_q8;
    for (x = 0; x < w; ++x) {
      const uint8_t *const src_x = &src[x_q8 >> SCALE_SUBPEL_BITS];
      const int x_filter_idx = (x_q8 & SCALE_SUBPEL_MASK) >> SCALE_EXTRA_BITS;
...
void av1_convolve_vert_scale(const uint8_t *src, int src_stride, uint8_t *dst,
                             int dst_stride, int w, int h,
                             const InterpFilterParams filter_params,
                             const int subpel_y_q8, int y_step_q8,
                             ConvolveParams *conv_params) {
  int x, y;
  int filter_size = filter_params.taps;
  assert(conv_params->round == CONVOLVE_OPT_ROUND);
  src -= src_stride * (filter_size / 2 - 1);
  for (x = 0; x < w; ++x) {
    int y_q8 = subpel_y_q8;
    for (y = 0; y < h; ++y) {
      const uint8_t *const src_y =
          &src[(y_q8 >> SCALE_SUBPEL_BITS) * src_stride];
      const int y_filter_idx = (y_q8 & SCALE_SUBPEL_MASK) >> SCALE_EXTRA_BITS;
...
void av1_convolve_horiz_facade_scale(const uint8_t *src, int src_stride,
                                     ...
                                     ConvolveParams *conv_params) {
  assert(conv_params->round == CONVOLVE_OPT_ROUND);
  // TODO(afergs): Make high precision versions of aom_convolve8_horiz_c and
  //               aom_convolve8_avg_horiz_c for scaling
  av1_convolve_horiz_scale(src, src_stride, dst, dst_stride, w, h,
                           filter_params, subpel_x_q8, x_step_q8, conv_params);
}
...
void av1_convolve_vert_facade_scale(const uint8_t *src, int src_stride,
                                    ...
                                    ConvolveParams *conv_params) {
  assert(conv_params->round == CONVOLVE_OPT_ROUND);
  // TODO(afergs): Make high precision versions of aom_convolve8_vert_c and
  //               aom_convolve8_avg_vert_c for scaling
  av1_convolve_vert_scale(src, src_stride, dst, dst_stride, w, h, filter_params,
                          subpel_y_q8, y_step_q8, conv_params);
}
...
static void convolve_scale_helper(const uint8_t *src, int src_stride,
                                  ...
                                  const int subpel_y_q8, int y_step_q8,
                                  ConvolveParams *conv_params,
                                  ConvolveFunc convolve_horiz_scale,
                                  ConvolveFunc convolve_vert_scale) {
  int ignore_horiz = x_step_q8 == SCALE_SUBPEL_SHIFTS && subpel_x_q8 == 0;
  int ignore_vert = y_step_q8 == SCALE_SUBPEL_SHIFTS && subpel_y_q8 == 0;
#if CONFIG_DUAL_FILTER
  InterpFilterParams filter_params_x =
      av1_get_interp_filter_params(interp_filter[1 + 2 * conv_params->ref]);
  InterpFilterParams filter_params_y =
  ...
  assert(w <= MAX_BLOCK_WIDTH);
  assert(h <= MAX_BLOCK_HEIGHT);
  assert(y_step_q8 <= (MAX_STEP << SCALE_EXTRA_BITS));
  assert(x_step_q8 <= (MAX_STEP << SCALE_EXTRA_BITS));
  ...
  if (ignore_horiz && ignore_vert) {
...
#endif
    assert(filter_params.taps <= MAX_FILTER_TAP);
    convolve_horiz_scale(src, src_stride, dst, dst_stride, w, h, filter_params,
                         subpel_x_q8, x_step_q8, conv_params);
  } else if (ignore_horiz) {
#if CONFIG_DUAL_FILTER
    filter_params = filter_params_y;
#endif
    ...
    if (filter_params_y.taps < filter_params_x.taps) {
      ...
      filter_size = filter_params_x.taps;
      intermediate_width =
          (((w - 1) * x_step_q8 + subpel_x_q8) >> SCALE_SUBPEL_BITS) +
          filter_size;
      assert(intermediate_width <= max_intermediate_size);

      assert(filter_params.taps <= MAX_FILTER_TAP);

      convolve_vert_scale(src - (filter_size / 2 - 1), src_stride, temp,
                          temp_stride, intermediate_width, h, filter_params,
                          subpel_y_q8, y_step_q8, &temp_conv_params);

      filter_params = filter_params_x;
      assert(filter_params.taps <= MAX_FILTER_TAP);
      convolve_horiz_scale(temp + (filter_size / 2 - 1), temp_stride, dst,
                           dst_stride, w, h, filter_params, subpel_x_q8,
                           x_step_q8, conv_params);
    } else {
#endif  // CONFIG_DUAL_FILTER && USE_EXTRA_FILTER
      int intermediate_height;
      int temp_stride = MAX_SB_SIZE;
    ...
    filter_size = filter_params.taps;
#endif
      intermediate_height =
          (((h - 1) * y_step_q8 + subpel_y_q8) >> SCALE_SUBPEL_BITS) +
          filter_size;
      assert(intermediate_height <= max_intermediate_size);
      (void)max_intermediate_size;
      ...
      convolve_horiz_scale(src - src_stride * (filter_size / 2 - 1), src_stride,
                           temp, temp_stride, w, intermediate_height,
                           filter_params, subpel_x_q8, x_step_q8,
                           &temp_conv_params);
      ...
      convolve_vert_scale(temp + temp_stride * (filter_size / 2 - 1),
...
void av1_convolve_scale(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  convolve_scale_helper(src, src_stride, dst, dst_stride, w, h, interp_filter,
                        subpel_x_q8, x_step_q8, subpel_y_q8, y_step_q8,
