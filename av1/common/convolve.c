      ...
      //  printf("a");
        if (conv_params->write) {
          int r =
               ...
               (1 << (offset_bits - conv_params->round_1 - 1)));
          // 1 << 12, 1 << 11
          if (conv_params->p1[y * w + x] < r)
            conv_params->p1[y * w + x] = 0;
          else
            conv_params->p1[y * w + x] -= r;

          if (res < r)
            conv_params->p2[y * w + x] = 0;
          else
            conv_params->p2[y * w + x] = res - r;
        }
        //////////////////////////////////////
  ...
  for (int y = 0; y < h; ++y) {
    for (int x = 0; x < w; ++x) {
      ...
      //  printf("");
        //////////////////////////////////
        if (conv_params->write) {
          if (conv_params->p1[y * w + x] < round_offset)
            conv_params->p1[y * w + x] = 0;
          else
            conv_params->p1[y * w + x] -= round_offset;

          if (res < round_offset)
            conv_params->p2[y * w + x] = 0;
          else
            conv_params->p2[y * w + x] = res - round_offset;
        }
        ////////////////////////////////
        int32_t tmp = dst16[y * dst16_stride + x];
        if (conv_params->use_dist_wtd_comp_avg) {
        ...
        if (conv_params->write) conv_params->p1[y * w + x] = res;
        dst16[y * dst16_stride + x] = res;
      }
    }
  }
