...
static void convolve_2d_for_intrabc(const uint8_t *src, int src_stride,
  ...
  if (subpel_x_q4 != 0 && subpel_y_q4 != 0) {
    av1_convolve_2d_sr_c(src, src_stride, dst, dst_stride, w, h, &filter_params,
                         &filter_params, 0, 0, conv_params);
  } else if (subpel_x_q4 != 0) {
    av1_convolve_x_sr_c(src, src_stride, dst, dst_stride, w, h, &filter_params,
                        &filter_params, 0, 0, conv_params);
  } else {
    av1_convolve_y_sr_c(src, src_stride, dst, dst_stride, w, h, &filter_params,
                        &filter_params, 0, 0, conv_params);
  }
}
