...
void av1_convolve_horiz_scale(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  for (y = 0; y < h; ++y) {
    int x_q4 = subpel_x_q4;
    for (x = 0; x < w; ++x) {
      const uint8_t *const src_x = &src[x_q4 >> SCALE_SUBPEL_BITS];
      const int x_filter_idx = (x_q4 & SCALE_SUBPEL_MASK) >> SCALE_EXTRA_BITS;
  ...
  for (x = 0; x < w; ++x) {
    ...
    for (y = 0; y < h; ++y) {
      const uint8_t *const src_y =
          &src[(y_q4 >> SCALE_SUBPEL_BITS) * src_stride];
      const int y_filter_idx = (y_q4 & SCALE_SUBPEL_MASK) >> SCALE_EXTRA_BITS;
...
static void convolve_scale_helper(const uint8_t *src, int src_stride,
                                  ...
                                  ConvolveFunc convolve_horiz,
                                  ConvolveFunc convolve_vert) {
  int ignore_horiz = x_step_q4 == SCALE_SUBPEL_SHIFTS && subpel_x_q4 == 0;
  int ignore_vert = y_step_q4 == SCALE_SUBPEL_SHIFTS && subpel_y_q4 == 0;
#if CONFIG_DUAL_FILTER
  InterpFilterParams filter_params_x =
      av1_get_interp_filter_params(interp_filter[1 + 2 * conv_params->ref]);
  InterpFilterParams filter_params_y =
  ...
  assert(w <= MAX_BLOCK_WIDTH);
  assert(h <= MAX_BLOCK_HEIGHT);
  assert(y_step_q4 <= (MAX_STEP << SCALE_EXTRA_BITS));
  assert(x_step_q4 <= (MAX_STEP << SCALE_EXTRA_BITS));
  ...
  if (ignore_horiz && ignore_vert) {
    ...
    if (filter_params_y.taps < filter_params_x.taps) {
      ...
      filter_size = filter_params_x.taps;
      intermediate_width =
          (((w - 1) * x_step_q4 + subpel_x_q4) >> SCALE_SUBPEL_BITS) +
          filter_size;
      assert(intermediate_width <= max_intermediate_size);
...
#endif
      intermediate_height =
          (((h - 1) * y_step_q4 + subpel_y_q4) >> SCALE_SUBPEL_BITS) +
          filter_size;
      assert(intermediate_height <= max_intermediate_size);
      (void)max_intermediate_size;
