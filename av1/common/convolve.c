...
void av1_convolve_2d_facade(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  if (scaled) {
    ...
    if (subpel_x_q4 == 0 && subpel_y_q4 == 0) {
      av1_jnt_convolve_2d_copy(src, src_stride, dst,
                               dst_stride, w, h,
                               &filter_params_x, &filter_params_y,
                               subpel_x_q4, subpel_y_q4, conv_params);
    } else if (subpel_x_q4 == 0) {
      // place holder
      av1_jnt_convolve_2d(src, src_stride, dst,
                          dst_stride, w, h, &filter_params_x,
  ...
  if (scaled) {
    ...
    // av1_convolve_2d.
    if (subpel_x_q4 == 0 && subpel_y_q4 == 0) {
      av1_convolve_2d_copy(src, src_stride, dst,
                           dst_stride, w, h, &filter_params_x,
                           &filter_params_y, subpel_x_q4, subpel_y_q4,
                           conv_params);
    } else if (subpel_x_q4 == 0) {
      av1_convolve_y(src, src_stride, dst,
                     dst_stride, w, h, &filter_params_x,
                     &filter_params_y, subpel_x_q4, subpel_y_q4, conv_params);
    } else if (subpel_y_q4 == 0) {
      av1_convolve_x(src, src_stride, dst,
                     dst_stride, w, h, &filter_params_x,
                     &filter_params_y, subpel_x_q4, subpel_y_q4, conv_params);
    } else {
      // subpel_x_q4 != 0 && subpel_y_q4 != 0
      av1_convolve_2d(src, src_stride, dst,
                      dst_stride, w, h, &filter_params_x,
                      &filter_params_y, subpel_x_q4, subpel_y_q4,
                      conv_params);
    }
