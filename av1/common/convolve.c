...
void av1_highbd_convolve_2d_facade(
    ...
    int h, int_interpfilters interp_filters, const int subpel_x_qn,
    int x_step_q4, const int subpel_y_qn, int y_step_q4, int scaled,
    ConvolveParams *conv_params, const struct scale_factors *sf, int is_intrabc,
    int bd, const InterPredExt *ext) {
  assert(IMPLIES(is_intrabc, !scaled));
  (void)x_step_q4;
  (void)y_step_q4;
  (void)dst_stride;
  ...
  if (is_intrabc && (subpel_x_qn != 0 || subpel_y_qn != 0)) {
    ...
    highbd_convolve_2d_for_intrabc(src, src_stride, dst, dst_stride, new_w,
                                   new_h, subpel_x_qn, subpel_y_qn, conv_params,
                                   bd);
    return;
  }
  ...
  if (scaled) {
    ...
    av1_highbd_convolve_2d_scale(src, src_stride, dst, dst_stride, new_w, new_h,
                                 filter_params_x, filter_params_y, subpel_x_qn,
                                 x_step_q4, subpel_y_qn, y_step_q4, conv_params,
                                 bd);
  } else {
