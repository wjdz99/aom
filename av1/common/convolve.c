...
void av1_convolve_horiz_scale(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  for (y = 0; y < h; ++y) {
    ...
    for (x = 0; x < w; ++x) {
      ...
      const int16_t *x_filter = av1_get_interp_filter_subpel_kernel(
          filter_params, (x_q4 & SCALE_SUBPEL_MASK) >> SCALE_EXTRA_BITS);
      int k, sum = 0;
      for (k = 0; k < filter_size; ++k) sum += src_x[k] * x_filter[k];
...
void av1_convolve_vert_scale(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  for (x = 0; x < w; ++x) {
    ...
    for (y = 0; y < h; ++y) {
      ...
      const int16_t *y_filter = av1_get_interp_filter_subpel_kernel(
          filter_params, (y_q4 & SCALE_SUBPEL_MASK) >> SCALE_EXTRA_BITS);
      int k, sum = 0;
      for (k = 0; k < filter_size; ++k)
        sum += src_y[k * src_stride] * y_filter[k];
...
static void convolve_helper_scale(const uint8_t *src, int src_stride,
                                  uint8_t *dst, int dst_stride, int w, int h,
#if CONFIG_DUAL_FILTER
                                  const InterpFilter *interp_filter,
#else
                                  ...
                                  ConvolveFunc convolve_horiz,
                                  ConvolveFunc convolve_vert) {
  int ignore_horiz = x_step_q4 == 1024 && subpel_x_q4 == 0;
  int ignore_vert = y_step_q4 == 1024 && subpel_y_q4 == 0;
#if CONFIG_DUAL_FILTER
  InterpFilterParams filter_params_x =
      av1_get_interp_filter_params(interp_filter[1 + 2 * conv_params->ref]);
  InterpFilterParams filter_params_y =
  ...
  assert(w <= MAX_BLOCK_WIDTH);
  assert(h <= MAX_BLOCK_HEIGHT);
  assert(y_step_q4 <= 1024 * 2);
  assert(x_step_q4 <= 1024 * 2);
...
void av1_convolve_scale(const uint8_t *src, int src_stride, uint8_t *dst,
                        ...
                        const int subpel_y_q4, int y_step_q4,
                        ConvolveParams *conv_params) {
  convolve_helper_scale(src, src_stride, dst, dst_stride, w, h, interp_filter,
