...
void av1_convolve_2d(const uint8_t *src, int src_stride, CONV_BUF_TYPE *dst,
  ...
  for (y = 0; y < im_h; ++y) {
    for (x = 0; x < w; ++x) {
      ...
      im_block[y * im_stride + x] = ROUND_POWER_OF_TWO_SIGNED(sum, conv_params->round_0);
    }
  }
  ...
  for (y = 0; y < h; ++y) {
    for (x = 0; x < w; ++x) {
      ...
      dst[y * dst_stride + x] += ROUND_POWER_OF_TWO_SIGNED(sum, conv_params->round_1);
    }
  }
}
...
void av1_convolve_2d_facade(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  av1_convolve_2d(src, src_stride, conv_params->dst, conv_params->dst_stride, w, h, &filter_params_x, &filter_params_y, subpel_x_q4, subpel_y_q4, conv_params);
}
