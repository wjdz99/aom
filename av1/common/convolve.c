  ...
  for (y = 0; y < h; ++y) {
    ...
    for (x = 0; x < w; ++x) {
      ...
      const int x_filter_idx =
          AOMMIN(ROUND_POWER_OF_TWO(x_q4 & SCALE_SUBPEL_MASK, SCALE_EXTRA_BITS),
                 SUBPEL_SHIFTS - 1);
...
void av1_convolve_vert_scale(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  for (x = 0; x < w; ++x) {
    ...
    for (y = 0; y < h; ++y) {
      ...
      const int y_filter_idx =
          AOMMIN(ROUND_POWER_OF_TWO(y_q4 & SCALE_SUBPEL_MASK, SCALE_EXTRA_BITS),
                 SUBPEL_SHIFTS - 1);
...
void av1_convolve_horiz_facade_scale(const uint8_t *src, int src_stride,
  ...
  if (filter_params.taps == SUBPEL_TAPS) {
    const int16_t *filter_x = av1_get_interp_filter_subpel_kernel(
        filter_params, subpel_x_q4 >> SCALE_EXTRA_BITS);
    if (conv_params->do_average == 0)
...
void av1_convolve_vert_facade_scale(const uint8_t *src, int src_stride,
  ...
  if (filter_params.taps == SUBPEL_TAPS) {
    const int16_t *filter_y = av1_get_interp_filter_subpel_kernel(
        filter_params, subpel_y_q4 >> SCALE_EXTRA_BITS);
    if (conv_params->do_average == 0) {
      aom_convolve8_vert_scale(src, src_stride, dst, dst_stride, NULL, -1,
                               filter_y, y_step_q4, w, h);
    } else {
      aom_convolve8_avg_vert_scale(src, src_stride, dst, dst_stride, NULL, -1,
                                   filter_y, y_step_q4, w, h);
    }
  } else {
    av1_convolve_vert_scale(src, src_stride, dst, dst_stride, w, h,
                            filter_params, subpel_y_q4, y_step_q4, conv_params);
  }
}
...
static void convolve_scale_helper(const uint8_t *src, int src_stride,
  ...
  assert(w <= MAX_BLOCK_WIDTH);
  assert(h <= MAX_BLOCK_HEIGHT);
  assert(y_step_q4 <= SCALE_SUBPEL_SHIFTS * 2);
  assert(x_step_q4 <= SCALE_SUBPEL_SHIFTS * 2);
