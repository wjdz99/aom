...
#define TRANSPOSE(dst, dst_stride, src, src_stride, w, h)          \
  {                                                                \
    int r, c;                                                      \
    for (r = 0; r < (h); ++r)                                      \
      for (c = 0; c < (w); ++c)                                    \
...
void av1_convolve_2d_facade(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  if (filter_params_y.taps < filter_params_x.taps) {
    ...
    TRANSPOSE(tr_src, tr_src_stride, src - fo_vert * src_stride - fo_horiz,
              src_stride, w + filter_params_x.taps - 1,
              h + filter_params_y.taps - 1);
    TRANSPOSE(tr_dst, tr_dst_stride, conv_params->dst, conv_params->dst_stride,
              w, h);
    av1_convolve_2d(tr_src + fo_vert * tr_src_stride + fo_horiz, tr_src_stride,
                    tr_dst, tr_dst_stride, h, w, &filter_params_y,
                    &filter_params_x, subpel_y_q4, subpel_x_q4, conv_params);
    TRANSPOSE(conv_params->dst, conv_params->dst_stride, tr_dst, tr_dst_stride,
              h, w);
