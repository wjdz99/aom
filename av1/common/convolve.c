...
#if CONFIG_LATESHIFT
void av1_convolve_horiz_full_c(const uint8_t *src, int src_stride,
                               uint32_t *dst, int dst_stride, int w, int h,
                               const InterpFilterParams filter_params,
...
void av1_convolve_vert_full_c(const uint32_t *src, int src_stride, uint8_t *dst,
                              int dst_stride, int w, int h,
                              const InterpFilterParams filter_params,
                              const int subpel_y_q4, int y_step_q4, int avg) {
  int x, y;
  ...
  for (x = 0; x < w; ++x) {
    int y_q4 = subpel_y_q4;
    for (y = 0; y < h; ++y) {
      const uint32_t *const src_y = &src[(y_q4 >> SUBPEL_BITS) * src_stride];
      const int16_t *y_filter = av1_get_interp_filter_subpel_kernel(
          filter_params, y_q4 & SUBPEL_MASK);
      int k, sum = 0;
      for (k = 0; k < filter_size; ++k)
      ...
      if (avg) {
        dst[y * dst_stride] = ROUND_POWER_OF_TWO(
            dst[y * dst_stride] +
                clip_pixel(ROUND_POWER_OF_TWO(sum, FILTER_BITS * 2)),
            1);
      } else {
        dst[y * dst_stride] =
            clip_pixel(ROUND_POWER_OF_TWO(sum, FILTER_BITS * 2));
...
#if CONFIG_LATESHIFT
void av1_convolve_horiz_facade_full(const uint8_t *src, int src_stride,
                                    uint32_t *dst, int dst_stride, int w, int h,
                                    const InterpFilterParams filter_params,
                                    const int subpel_x_q4, int x_step_q4) {
  av1_convolve_horiz_full_c(src, src_stride, dst, dst_stride, w, h,
                            filter_params, subpel_x_q4, x_step_q4);
}

void av1_convolve_vert_facade_full(const uint32_t *src, int src_stride,
                                   uint8_t *dst, int dst_stride, int w, int h,
                                   const InterpFilterParams filter_params,
                                   const int subpel_y_q4, int y_step_q4,
                                   int avg) {
  av1_convolve_vert_full_c(src, src_stride, dst, dst_stride, w, h,
...
#if CONFIG_LATESHIFT
      av1_convolve_horiz_facade_full(src - src_stride * (filter_size / 2 - 1),
                                     src_stride, temp, temp_stride, w,
                                     intermediate_height, filter_params,
                                     subpel_x_q4, x_step_q4);
#else
...
#if CONFIG_LATESHIFT
      av1_convolve_vert_facade_full(
          temp + temp_stride * (filter_size / 2 - 1), temp_stride, dst,
          dst_stride, w, h, filter_params, subpel_y_q4, y_step_q4, ref_idx);
#else
...
#if CONFIG_LATESHIFT
void av1_highbd_convolve_horiz_full_c(const uint16_t *src, int src_stride,
                                      uint32_t *dst, int dst_stride, int w, int h,
                                      const InterpFilterParams filter_params,
                                      const int subpel_x_q4, int x_step_q4) {
  int x, y;
...
void av1_highbd_convolve_vert_full_c(const uint32_t *src, int src_stride,
  ...
  for (x = 0; x < w; ++x) {
    ...
    for (y = 0; y < h; ++y) {
      ...
      if (avg) {
        dst[y * dst_stride] = ROUND_POWER_OF_TWO(
            dst[y * dst_stride] +
                clip_pixel_highbd(ROUND_POWER_OF_TWO(sum, FILTER_BITS * 2), bd),
            1);
      } else {
        dst[y * dst_stride] =
            clip_pixel_highbd(ROUND_POWER_OF_TWO(sum, FILTER_BITS * 2), bd);
...
#if CONFIG_LATESHIFT
void av1_highbd_convolve_horiz_facade_full(const uint8_t *src8, int src_stride,
                                           uint32_t *dst, int dst_stride, int w,
                                           int h,
                                           const InterpFilterParams filter_params,
                                           const int subpel_x_q4, int x_step_q4) {
  uint16_t *src = CONVERT_TO_SHORTPTR(src8);
  av1_highbd_convolve_horiz_full_c(src, src_stride, dst, dst_stride, w, h,
                                   filter_params, subpel_x_q4, x_step_q4);
}

void av1_highbd_convolve_vert_facade_full(const uint32_t *src, int src_stride,
                                          uint8_t *dst8, int dst_stride, int w,
                                          int h,
                                          const InterpFilterParams filter_params,
                                          const int subpel_y_q4, int y_step_q4,
  ...
  av1_highbd_convolve_vert_full_c(src, src_stride, dst, dst_stride, w, h,
                                  filter_params, subpel_y_q4, y_step_q4, avg, bd);
}
#endif
...
void av1_highbd_convolve(const uint8_t *src8, int src_stride, uint8_t *dst8,
  ...
  if (ignore_horiz && ignore_vert) {
    ...
    DECLARE_ALIGNED(16, uint16_t,
                    temp[((MAX_SB_SIZE * 2 + 16) + 16) * MAX_SB_SIZE]);
#endif
#if !CONFIG_LATESHIFT || CONFIG_DUAL_FILTER
...
#if CONFIG_LATESHIFT
      av1_highbd_convolve_horiz_facade_full(
          src8 - src_stride * (filter_size / 2 - 1), src_stride, temp,
...
#if CONFIG_LATESHIFT
      av1_highbd_convolve_vert_facade_full(
          temp + temp_stride * (filter_size / 2 - 1), temp_stride, dst8,
