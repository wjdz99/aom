...
void aom_cdf_init_q15(uint16_t *cdf, int ncdfs, int cdf_size) {
  ...
  int j;
  for (i = 0; i < ncdfs; i++) {
    for (j = 0; j < cdf_size - CONFIG_EC_ADAPT; j++)
      cdf[i*cdf_size + j] = (j + 1)*32768/(cdf_size - CONFIG_EC_ADAPT);

#if CONFIG_EC_ADAPT
    cdf[i*cdf_size + cdf_size - 1] = 0;
#endif
  }
}
 ...
 */
void generic_model_update(int *ex_q16, int x, int xs, int id,
 int integration) {
