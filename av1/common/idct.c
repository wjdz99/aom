...
int idx_selfloop_wrt_mode(PREDICTION_MODE mode, int is_col) {
  ...
  switch (mode) {
    ...
    case SMOOTH_H_PRED:
#endif
    case H_PRED: return is_col ? 0 : 3;
    // prediction is good for V direction: large SL for col only
#if CONFIG_SMOOTH_HV
    case SMOOTH_V_PRED:
#endif
    case V_PRED: return is_col ? 3 : 0;
#if LGT_SL_INTRA
...
void get_lgt4_from_pred(const TxfmParam *txfm_param, int is_col,
  ...
  // Each lgt4mtx_arr[k][i] corresponds to a line graph with a self-loop in
  ...
  if (mode == DC_PRED || mode == SMOOTH_PRED) {
    // DC_PRED: do not use break point, since 1) is_left_available and
    // is_top_available are not known by txfm_param for now, so accessing
    ...
    // directional modes closer to vertical
    if (!is_col)
      get_discontinuity_2d(dst, stride, 4, 0, &bp, ntx);
  } else if (mode == D135_PRED || mode == D153_PRED || mode == D207_PRED) {
    // directional modes closer to horizontal
    if (is_col)
      get_discontinuity_2d(dst, stride, 4, 1, &bp, ntx);
  } else if (mode > TM_PRED) {
    // inter
...
void get_lgt8_from_pred(const TxfmParam *txfm_param, int is_col,
  ...
  if (mode == DC_PRED || mode == SMOOTH_PRED) {
    ...
    if (!is_col)
      get_discontinuity_2d(dst, stride, 8, 0, &bp, ntx);
  } else if (mode == D135_PRED || mode == D153_PRED || mode == D207_PRED) {
    if (is_col)
      get_discontinuity_2d(dst, stride, 8, 1, &bp, ntx);
  } else if (mode > TM_PRED) {
    // inter
