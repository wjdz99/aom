...
int get_inv_lgt4(transform_1d tx_orig, const TXFM_PARAM *txfm_param,
...
int get_inv_lgt8(transform_1d tx_orig, const TXFM_PARAM *txfm_param,
                 const tran_high_t *lgtmtx[], int ntx) {
  // inter/intra split
...
static void init_txfm_param(const MACROBLOCKD *xd, TX_SIZE tx_size,
                            TX_TYPE tx_type, int eob, TXFM_PARAM *txfm_param) {
...
// TODO(kslu) Change input arguments to TXFM_PARAM, which contains mode,
