...
int get_discontinuity(int arr[], int n) {
  ...
  for (int i = 1; i < n; ++i) {
    ...
    if (i == 1)
      ...
      temp = abs(arr[n - 1] - arr[n - 2]);
    else
      temp = abs(arr[i] - (arr[i + 1] + arr[i - 1]) / 2);
#else
...
void get_lgt4_from_pred(const TxfmParam *txfm_param, int is_col,
  ...
  if (!is_col) {
    ...
    bp = get_discontinuity(arr, 4);
    switch (mode) {
      case DC_PRED:
#if LGT_ALL_MODE_PRED
        if (bp == -1) return;
        for (int i = 0; i < ntx; ++i)
      ...
      case SMOOTH_V_PRED:
        ...
        for (int i = 0; i < ntx; ++i) {
          ...
          bp = get_discontinuity(arr, 4);
          if (bp != -1) lgtmtx[i] = lgt4mtx_arr[get_weight_idx(i, ntx)][bp - 1];
        }
#else
    ...
    switch (mode) {
      ...
      case SMOOTH_V_PRED:
        ...
        for (int i = 0; i < ntx; ++i) {
          ...
          bp = get_discontinuity(arr, 8);
          if (bp != -1) lgtmtx[i] = lgt8mtx_arr[get_weight_idx(i, ntx)][bp - 1];
        }
#else
        if (bp == -1) return;
        for (int i = 0; i < ntx; ++i)
...
#if CONFIG_LGT
    if (tx_type == LGT2D)
      ilgt16up(tmp[i], out[i], lgtmtx_col[i]);
    else
#endif
...
void av1_iht16x4_64_add_c(const tran_low_t *input, uint8_t *dest, int stride,
  ...
  if (tx_type == LGT2D) {
    ...
    get_lgt16up_from_pred(txfm_param, 0, lgtmtx_col, 4);
  }
#endif
...
#if CONFIG_LGT
    if (tx_type == LGT2D)
      ilgt16up(input, outtmp, lgtmtx_row[i]);
    else
#endif
...
void av1_iht32x8_256_add_c(const tran_low_t *input, uint8_t *dest, int stride,
  ...
  const tran_high_t *lgtmtx_row[8];
  if (tx_type == LGT2D) {
    get_lgt4_from_pred(txfm_param, 1, lgtmtx_col, 32);
    get_lgt16up_from_pred(txfm_param, 0, lgtmtx_row, 8);
  }
#endif
...
#endif  // CONFIG_MRC_TX
#if CONFIG_LGT
    case LGT2D: assert(0 && "Invalid tx type for tx size"); break;
#endif  // CONFIG_LGT
    default: assert(0); break;
  }
}
...
#if CONFIG_LGT
    case LGT2D:
      assert(0 && "Invalid tx type for tx size");
      break;
#endif  // CONFIG_LGT
    default: assert(0); break;
  }
