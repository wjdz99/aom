...
#if CONFIG_TXMG
static void copy_16bit_block_from_frame_buffer(uint16_t *dst, int dst_stride,
                                               const uint8_t *src,
                                               int src_stride, int w, int h,
                                               const MACROBLOCKD *xd) {
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    for (int r = 0; r < h; ++r)
      memcpy(dst + r * dst_stride, CONVERT_TO_SHORTPTR(src) + r * src_stride,
             w * sizeof(uint16_t));
  } else {
    for (int r = 0; r < h; ++r)
      for (int c = 0; c < w; ++c)
...
static void copy_16bit_block_to_frame_buffer(uint8_t *dst, int dst_stride,
                                             const uint16_t *src,
                                             int src_stride, int w, int h,
                                             const MACROBLOCKD *xd) {
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    for (int r = 0; r < h; ++r)
      memcpy(CONVERT_TO_SHORTPTR(dst) + r * dst_stride, src + r * src_stride,
             w * sizeof(uint16_t));
  } else {
    for (int r = 0; r < h; ++r)
      for (int c = 0; c < w; ++c)
...
void av1_inverse_transform_block(const MACROBLOCKD *xd,
  ...
  copy_16bit_block_from_frame_buffer(tmp, tmp_stride, dst, stride, w, h, xd);

  av1_highbd_inv_txfm_add(dqcoeff, CONVERT_TO_BYTEPTR(tmp), tmp_stride,
                          &txfm_param);

  copy_16bit_block_to_frame_buffer(dst, stride, tmp, tmp_stride, w, h, xd);
