...
static void highbd_inv_txfm_add_16x16(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    ...
    case H_FLIPADST:
      ...
      break;
    case IDTX:
      highbd_inv_idtx_add_c(input, dest, stride, 16, tx_type, bd); break;
#endif  // CONFIG_EXT_TX
    default: assert(0); break;
  }
}
...
static void highbd_inv_txfm_add_32x32(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
    case H_FLIPADST:
      highbd_iht32x32_1024_add_c(input, dest, stride, tx_type, bd); break;
    case IDTX:
      highbd_inv_idtx_add_c(input, dest, stride, 32, tx_type, bd); break;
#endif  // CONFIG_EXT_TX
    default: assert(0); break;
  }
}
...
static void highbd_inv_txfm_add_64x64(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
    case H_FLIPADST:
      highbd_iht64x64_4096_add_c(input, dest, stride, tx_type, bd); break;
    case IDTX:
      highbd_inv_idtx_add_c(input, dest, stride, 64, tx_type, bd); break;
#endif  // CONFIG_EXT_TX
    default: assert(0); break;
  }
}
...
void av1_highbd_inv_txfm_add(const tran_low_t *input, uint8_t *dest, int stride,
  ...
  switch (tx_size) {
#if CONFIG_TX64X64
    case TX_64X64:
      highbd_inv_txfm_add_64x64(input, dest, stride, eob, bd, tx_type); break;
#endif  // CONFIG_TX64X64
    case TX_32X32:
      highbd_inv_txfm_add_32x32(input, dest, stride, eob, bd, tx_type); break;
    case TX_16X16:
      highbd_inv_txfm_add_16x16(input, dest, stride, eob, bd, tx_type); break;
    case TX_8X8:
      highbd_inv_txfm_add_8x8(input, dest, stride, eob, bd, tx_type); break;
    case TX_4X8:
    ...
    case TX_8X4:
      ...
      break;
    case TX_8X16:
      highbd_inv_txfm_add_8x16(input, dest, stride, eob, bd, tx_type); break;
    case TX_16X8:
      highbd_inv_txfm_add_16x8(input, dest, stride, eob, bd, tx_type); break;
    case TX_16X32:
      highbd_inv_txfm_add_16x32(input, dest, stride, eob, bd, tx_type); break;
    case TX_32X16:
      highbd_inv_txfm_add_32x16(input, dest, stride, eob, bd, tx_type); break;
    case TX_4X4:
      // this is like av1_short_idct4x4 but has a special case around eob<=1
      // which is significant (not just an optimization) for the lossless
      // case.
