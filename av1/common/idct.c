...
static void init_txfm_param(const MACROBLOCKD *xd, int plane, TX_SIZE tx_size,
  ...
  txfm_param->intra_mode = (plane == AOM_PLANE_Y) ? mbmi->mode : get_uv_mode(mbmi->uv_mode);
  if ((txfm_param->intra_mode < PAETH_PRED) &&!(mbmi->filter_intra_mode_info.use_filter_intra)) {
...
#if CONFIG_IST
  MB_MODE_INFO* const mbmi = xd->mi[0];
  PREDICTION_MODE intra_mode = (plane == AOM_PLANE_Y) ? mbmi->mode : get_uv_mode(mbmi->uv_mode);
...
#if CONFIG_IST
void inv_stxfm(tran_low_t* src, tran_low_t* dst, const PREDICTION_MODE mode, const int8_t stx_idx, const int size) {
...
void av1_inv_stxfm(tran_low_t* coeff, TxfmParam* txfm_param) {
  ...
  if ((width >= 4 && height >= 4) && txfm_param->stx_type)
  {
    ...
    {
      *tmp++ = src[scan_order_out[i]];
    }

    inv_stxfm(buf0, buf1, intra_mode, (int8_t)(stx_type - 1), sbSize);
