...
static void init_txfm_param(const MACROBLOCKD *xd, int plane, TX_SIZE tx_size,
  ...
  txfm_param->tx_type = (tx_type & 0x0f);
  ...
  if ((txfm_param->intra_mode < PAETH_PRED) &&
    ...
    if ((sbSize == 4) && (eob > (IST_4x4_HEIGHT - 1)))
      ...
      ist_eob = 0;
    if (ist_eob) txfm_param->sec_tx_type = (tx_type >> 4);
  }
#else
...
void av1_inv_stxfm(tran_low_t *coeff, TxfmParam *txfm_param) {
  ...
  if ((width >= 4 && height >= 4) && stx_type) {
    ...
    if (transpose) {
      ...
      scan_order_out = (sbSize == 4) ? stx_scan_orders_4x4[log2width - 2]
                                     : stx_scan_orders_8x8[log2width - 2];
    }
    inv_stxfm(buf0, buf1, mode_t, stx_type - 1, sbSize);
    tmp = buf1;
    src = coeff;
    for (int r = 0; r < sbSize * sbSize; r++) {
