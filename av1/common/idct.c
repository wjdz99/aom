...
#if CONFIG_EXT_TX && !CONFIG_DAALA_TX
...
void av1_iht4x16_64_add_c(const tran_low_t *input, uint8_t *dest, int stride,
  ...
  for (i = 0; i < n4; ++i) {
    ...
    tran_low_t temp_in[4];
    for (j = 0; j < n; j++) temp_in[j] = input[j] * 2;
    IHT_4x16[tx_type].rows(temp_in, outtmp);
#else
#if CONFIG_LGT
    if (use_lgt_row)
      ilgt4(input, outtmp, lgtmtx_row[i]);
...
void av1_iht16x4_64_add_c(const tran_low_t *input, uint8_t *dest, int stride,
  ...
  for (i = 0; i < n; ++i) {
    ...
    for (j = 0; j < n4; j++) temp_in[j] = input[j] * 2;
    IHT_16x4[tx_type].rows(temp_in, outtmp);
#else
...
void av1_iht8x32_256_add_c(const tran_low_t *input, uint8_t *dest, int stride,
  ...
  for (i = 0; i < n4; ++i) {
    ...
    IHT_8x32[tx_type].rows(temp_in, outtmp);
#else
#if CONFIG_LGT
    if (use_lgt_row)
      ilgt8(input, outtmp, lgtmtx_row[i]);
...
void av1_iht32x8_256_add_c(const tran_low_t *input, uint8_t *dest, int stride,
  ...
  for (i = 0; i < n; ++i) {
    ...
    for (j = 0; j < n4; j++) temp_in[j] = input[j] * 4;
    IHT_32x8[tx_type].rows(temp_in, outtmp);
#else
