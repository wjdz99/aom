...
void av1_loop_filter_frame_mt(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  filter_y = (cm->lf.filter_level[0] || cm->lf.filter_level[1]);
  planes_to_lf[0] = (filter_y && plane_start <= 0 && 0 < plane_end);
  planes_to_lf[1] =
      (filter_y && cm->lf.filter_level_u && plane_start <= 1 && 1 < plane_end);
  planes_to_lf[2] =
      (filter_y && cm->lf.filter_level_v && plane_start <= 2 && 2 < plane_end);
  ...
  if (num_workers > 1) {
    ...
    // Directly filter in the main thread. Filter top rows of all planes first,
    // in case the output can be partially reconstructed row by row.
    int mi_row, plane, dir;
    for (mi_row = start_mi_row; mi_row < end_mi_row; mi_row += MAX_MIB_SIZE) {
