...
void av1_loop_filter_frame_mt(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  int filter_at_least_y, planes_to_lf[3];

  // Whether to filter at least the luma plane or no plane at all.
  filter_at_least_y = (cm->lf.filter_level[0] || cm->lf.filter_level[1]);
  // For each luma and chroma plane, whether to filter it or not.
  planes_to_lf[0] = (filter_at_least_y && plane_start <= 0 && 0 < plane_end);
  planes_to_lf[1] = (filter_at_least_y && cm->lf.filter_level_u &&
                     plane_start <= 1 && 1 < plane_end);
  planes_to_lf[2] = (filter_at_least_y && cm->lf.filter_level_v &&
                     plane_start <= 2 && 2 < plane_end);
  // Early exit.
