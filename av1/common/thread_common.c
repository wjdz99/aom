...
static void set_loop_restoration_done(AV1LrSync *const lr_sync,
  ...
  for (int plane = 0; plane < MAX_MB_PLANE; ++plane) {
    ...
    while (y0 < plane_h) {
      int remaining_h = plane_h - y0;
      int h = (remaining_h < ext_size) ? remaining_h : unit_size;
      lr_sync_write(lr_sync, i, hnum_rest_units - 1, hnum_rest_units, plane);
      y0 += h;
      i++;
    }
  }
...
static int loop_restoration_row_worker(void *arg1, void *arg2) {
  ...
  if (setjmp(error_info->jmp)) {
    ...
    pthread_mutex_unlock(job_mutex_);
#endif
    // In case of loop restoration row-multithreading, the worker on an SB row
    // waits for the completion of the filtering of the top-right and
    // bottom-right SBs. Hence, in case a thread (main/worker) encounters an
    // error, update that filtering of every SB row in the frame is complete in
