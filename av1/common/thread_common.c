  ...
  if (plane == 0) {
    ...
    switch (path) {
      ...
      case LF_PATH_444:
        ...
        break;
      case LF_PATH_SLOW:
        av1_filter_block_plane_non420_ver(cm, &planes[plane], mi,
                                          mi_row, mi_col);
        break;
    }
  }
  ...
  if (plane == 0) {
    ...
    switch (path) {
      ...
      case LF_PATH_444:
        ...
        break;
      case LF_PATH_SLOW:
        av1_filter_block_plane_non420_hor(cm, &planes[plane], mi,
                                          mi_row, mi_col);
        break;
    }
  }
...
static int loop_filter_row_worker(AV1LfSync *const lf_sync,
  ...
  for (mi_row = lf_data->start; mi_row < lf_data->stop;
    ...
    for (mi_col = 0; mi_col < lf_data->cm->mi_cols; mi_col += MAX_MIB_SIZE) {
      ...
      for (plane = 0; plane < num_planes; ++plane) {
        loop_filter_block_plane_ver(lf_data->cm, lf_data->planes, plane, mi + mi_col,
                                    mi_row, mi_col, path, &lfm);
        loop_filter_block_plane_hor(lf_data->cm, lf_data->planes, plane, mi + mi_col, 
                                    mi_row, mi_col, path, &lfm);
      }
