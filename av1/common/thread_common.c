...
static INLINE void thread_loop_filter_rows(
  ...
  if (dir == 0) {
    ...
    for (mi_col = 0; mi_col < cm->mi_params.mi_cols; mi_col += MAX_MIB_SIZE) {
      ...
      if (do_extend_border) {
        const YV12_BUFFER_CONFIG *const ybf = frame_buffer;
