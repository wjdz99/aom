...
static void set_cdef_init_fb_row_done(AV1CdefSync *const cdef_sync, int nvfb) {
  // In case of cdef row-multithreading, the worker on a filter block row (fbr)
  // waits for the line buffers (top and bottom) copy of the above row. Hence,
  // in case a thread (main/worker) encounters an error before copying of the
  // line buffers, update that line buffer copy is complete in order to avoid
  // dependent workers waiting indefinitely.
