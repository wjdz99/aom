...
#if CONFIG_BLOCK_ADAPT_SCAN
static INLINE void set_scan_context(AV1_COMMON *cm, MACROBLOCKD *xd, int mi_row,
                                    int mi_col, SCAN_CONTEXT scan_context) {
  int col = mi_col >> cm->mib_size_log2;
  (void)mi_row;
...
static INLINE void get_scan_context(AV1_COMMON *cm, MACROBLOCKD *xd, int mi_row,
                                    int mi_col, SCAN_CONTEXT **left,
                                    SCAN_CONTEXT **above) {
  int col = mi_col >> cm->mib_size_log2;
  (void)mi_row;
...
#if CONFIG_BLOCK_ADAPT_SCAN
  av1_zero_array(
      cm->above_scan_context + 2 * (mi_col_start >> cm->mib_size_log2),
      2 * (aligned_width >> cm->mib_size_log2));
#endif
}
