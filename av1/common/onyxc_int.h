...
typedef struct AV1Common {
  ...
  PARTITION_CONTEXT *above_seg_context[MAX_TILE_ROWS];
  ENTROPY_CONTEXT *above_context[MAX_TILE_ROWS][MAX_MB_PLANE];
  TXFM_CONTEXT *above_txfm_context[MAX_TILE_ROWS];
...
static INLINE void av1_init_above_context(AV1_COMMON *cm, MACROBLOCKD *xd, const int tile_row) {
  const int num_planes = av1_num_planes(cm);
  for (int i = 0; i < num_planes; ++i) {
    xd->above_context[i] = cm->above_context[tile_row][i];
  }
...
static INLINE void av1_zero_above_context(AV1_COMMON *const cm,
  ...
  av1_zero_array(cm->above_context[tile_row][0] + offset_y, width_y);
  if (num_planes > 1) {
    if (cm->above_context[tile_row][1] && cm->above_context[tile_row][2]) {
      av1_zero_array(cm->above_context[tile_row][1] + offset_uv, width_uv);
      av1_zero_array(cm->above_context[tile_row][2] + offset_uv, width_uv);
    }
    else {
  ...
  memset(cm->above_txfm_context[tile_row] + (mi_col_start << TX_UNIT_WIDE_LOG2),
