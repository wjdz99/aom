...
typedef struct AV1Common {
  ...
  // (inter) reference frame type to the corresponding reference info.
  ...
  // Index to the next free frame buffer, in the buffer pool 'cm->buffer_pool'.
  int new_fb_idx;
...
static INLINE void assign_buffer(RefCntBuffer *bufs, int *ptr, int new_idx) {
  ...
  if (old_idx >= 0) {
    ...
    // One less pointer to the buffer at 'old_idx', so decrease ref count.
    --bufs[old_idx].ref_count;
  }

  *ptr = new_idx;
  // One more pointer to the buffer at 'new_idx', so increase ref count.
