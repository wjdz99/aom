...
static INLINE int is_chroma_reference_helper(int mi_row, int mi_col, int bw, int bh,
...
static INLINE void set_mi_row_col(MACROBLOCKD *xd, const TileInfo *const tile,
  ...
  const int chroma_ref = is_chroma_reference_helper(mi_row, mi_col, bw, bh, ss_x, ss_y);
  if (chroma_ref) {
    ...
    // TODO(now): Correct or 0 offset?
    const int is_3rd_horz3_16x16_partition = (mi_row & 1) && (bw == 4) && (bh == 1);
    const int base_mi_row_offset = is_3rd_horz3_16x16_partition ? -2 : - (mi_row & ss_y);

    const int is_3rd_vert3_16x16_partition = (mi_col & 1) && (bw == 1) && (bh == 4);
    const int base_mi_col_offset = is_3rd_vert3_16x16_partition ? -2 : - (mi_col & ss_x);
  ...
  if (bsize >= BLOCK_8X8) {
    ...
    switch (partition) {
      ...
      case PARTITION_HORZ_3: {
        ...
        update_partition_context(xd, mi_row + quarter_step, mi_col, bsize3, bsize3);
        update_partition_context(xd, mi_row + 3 * quarter_step, mi_col, subsize, subsize);
        break;
      }
      case PARTITION_VERT_3: {
        const BLOCK_SIZE bsize3 = get_partition_subsize(bsize, PARTITION_VERT);
        update_partition_context(xd, mi_row, mi_col, subsize, subsize);
        update_partition_context(xd, mi_row, mi_col + quarter_step, bsize3, bsize3);
        update_partition_context(xd, mi_row, mi_col + 3 * quarter_step, subsize, subsize);
        break;
      }
