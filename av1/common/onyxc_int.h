...
static INLINE void set_plane_n4(MACROBLOCKD *const xd, int mi_row, int mi_col,
  ...
  for (int i = 0; i < num_planes; i++) {
    struct macroblockd_plane *const pd = &xd->plane[i];
    const BLOCK_SIZE plane_bsize = get_plane_block_size(mi_row, mi_col, bsize,
        pd->subsampling_x, pd->subsampling_y);
    assert(IMPLIES((pd->subsampling_x == 0 && pd->subsampling_y == 0) ||
                   (pd->subsampling_x == 1 && pd->subsampling_y == 1),
                   plane_bsize != BLOCK_INVALID));
    pd->width = block_size_wide[plane_bsize];
    pd->height = block_size_high[plane_bsize];
    assert(pd->width >= 4);
    assert(pd->height >= 4);
  }
}
