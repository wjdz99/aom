...
static INLINE int frame_might_allow_ref_frame_mvs(const AV1_COMMON *cm) {
  return !cm->error_resilient_mode &&
    cm->seq_params.order_hint.enable_ref_frame_mvs &&
    cm->seq_params.order_hint.enable_order_hint && !frame_is_intra_only(cm);
}
...
// TODO(david.turner@argondesign.com): Replace all instances of
// av1_num_planes() with av1_num_planes_from_seq() to reduce dependency on
// AV1_COMMON
static INLINE int av1_num_planes_from_seq(const SequenceHeader *const
    seq_params) { return seq_params->monochrome ? 1 : MAX_MB_PLANE;
}
