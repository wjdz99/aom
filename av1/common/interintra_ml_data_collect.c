...
// first 8 bits.
static void copy_value(uint8_t *dst, int dst_offset, uint8_t *src,
...
static void copy_intrapred_lshape(IIMLPlaneInfo *info, MACROBLOCK *const x) {
  ...
  av1_interintra_ml_data_collect_copy_intrapred_lshape(
      ...
      info->border, info->bd, is_cur_buf_hbd(xd));
}
...
void av1_interintra_ml_data_collect_copy_intrapred_lshape(
  ...
  for (int j = 0; j < border; ++j) {
    for (int i = 0; i < border + width; ++i) {
      copy_value(dst, info_i++, src, j * stride + i, bitdepth, is_src_hbd);
    }
  }
...
static void copy_interpred(IIMLPlaneInfo *info, const AV1_COMP *const cpi,
  ...
  av1_enc_build_border_inter_predictor(cm, xd, xd->mi_row, xd->mi_col, NULL,
                                       bsize, info->plane, info->plane);

  int info_i = 0;
  for (int j = -1 * info->border; j < info->height; ++j) {
    for (int i = -1 * info->border; i < info->width; ++i) {
      copy_value(info->interpred, info_i++, interpred, j * dst_stride + i,
...
static void process_block(IIMLPlaneInfo *info, const AV1_COMP *const cpi,
                          MACROBLOCK *const x, BLOCK_SIZE bsize, int plane,
                          int mi_row, int mi_col, int border) {
  MACROBLOCKD *const xd = &x->e_mbd;
  ...
  info = malloc(sizeof(IIMLPlaneInfo));
  assert(info != NULL);
  ...
  assert(border >= 4);  // Hardcoded for now.
  info->border = 4;
  info->x = (xd->mi_col * MI_SIZE) >> pd->subsampling_x;
  info->y = (xd->mi_row * MI_SIZE) >> pd->subsampling_y;
  info->frame_order_hint = cm->seq_params.order_hint_info;
  ...
  copy_interpred(info, cpi, x, border);
}
...
void write_buffer_and_advance(uint8_t **buf, uint8_t *src, size_t size,
  ...
  for (size_t i = 0; i < size; ++i) {
    uint8_t b1 = (uint8_t)(0xff & (src16[i] >> 8));
    uint8_t b2 = (uint8_t)(0xff & src16[i]);
...
void av1_interintra_ml_data_collect(const AV1_COMP *const cpi,
  ...
  assert(INFO_U == NULL);
  assert(INFO_V == NULL);
  process_block(INFO_Y, cpi, x, bsize, AOM_PLANE_Y, mi_row, mi_col, border);
  process_block(INFO_U, cpi, x, bsize, AOM_PLANE_U, mi_row, mi_col, border);
  process_block(INFO_V, cpi, x, bsize, AOM_PLANE_V, mi_row, mi_col, border);
  assert(INFO_Y != NULL);
  assert(INFO_U != NULL);
  assert(INFO_V != NULL);
}
...
void av1_interintra_ml_data_collect_finalize() {
  ...
  // May need a check here that INFO_Y != NULL, returning otherwise.
  assert(INFO_Y != NULL);
  av1_interintra_ml_data_collect_serialize(INFO_Y, &buf, &buf_size);
  size_t written = fwrite(buf, buf_size, sizeof(*buf), FP);
  assert(written == buf_size);
  free(buf);

  assert(INFO_U != NULL);
  av1_interintra_ml_data_collect_serialize(INFO_U, &buf, &buf_size);
  written = fwrite(buf, buf_size, sizeof(*buf), FP);
  assert(written == buf_size);
  free(buf);
