...
void av1_adapt_inter_frame_probs(AV1_COMMON *cm) {
  ...
  for (i = 0; i < REF_CONTEXTS; i++)
    for (j = 0; j < (FWD_REFS - 1); j++)
      fc->single_fwdref_prob[i][j] = mode_mv_merge_probs(
          pre_fc->single_fwdref_prob[i][j], counts->single_fwdref[i][j]);
  for (i = 0; i < REF_CONTEXTS; i++)
    for (j = 0; j < (BWD_REFS - 1); j++)
      fc->single_bwdref_prob[i][j] = mode_mv_merge_probs(
          pre_fc->single_bwdref_prob[i][j], counts->single_bwdref[i][j]);
#else
  for (i = 0; i < REF_CONTEXTS; i++)
    fc->comp_ref_prob[i] =
