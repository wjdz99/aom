...
const vpx_prob av1_kf_y_mode_prob[INTRA_MODES][INTRA_MODES][INTRA_MODES - 1] =
    { {
          // above = dc
...
const vpx_prob av1_kf_uv_mode_prob[INTRA_MODES][INTRA_MODES - 1] = {
...
const vpx_tree_index av1_intra_mode_tree[TREE_SIZE(INTRA_MODES)] = {
  -DC_PRED,   2,          /* 0 = DC_NODE */
  -TM_PRED,   4,          /* 1 = TM_NODE */
  -V_PRED,    6,          /* 2 = V_NODE */
  8,          12,         /* 3 = COM_NODE */
...
const vpx_tree_index av1_inter_mode_tree[TREE_SIZE(INTER_MODES)] = {
  -INTER_OFFSET(ZEROMV), 2, -INTER_OFFSET(NEARESTMV), 4, -INTER_OFFSET(NEARMV),
  -INTER_OFFSET(NEWMV)
};

const vpx_tree_index av1_partition_tree[TREE_SIZE(PARTITION_TYPES)] = {
...
const vpx_tree_index av1_switchable_interp_tree[TREE_SIZE(
    SWITCHABLE_FILTERS)] = { -EIGHTTAP, 2, -EIGHTTAP_SMOOTH, -EIGHTTAP_SHARP };

void av1_adapt_inter_frame_probs(VP10_COMMON *cm) {
  int i, j;
  ...
  for (i = 0; i < INTER_MODE_CONTEXTS; i++)
    vpx_tree_merge_probs(av1_inter_mode_tree, pre_fc->inter_mode_probs[i],
                         counts->inter_mode[i], fc->inter_mode_probs[i]);

  for (i = 0; i < BLOCK_SIZE_GROUPS; i++)
    vpx_tree_merge_probs(av1_intra_mode_tree, pre_fc->y_mode_prob[i],
                         counts->y_mode[i], fc->y_mode_prob[i]);

#if !CONFIG_MISC_FIXES
  for (i = 0; i < INTRA_MODES; ++i)
    vpx_tree_merge_probs(av1_intra_mode_tree, pre_fc->uv_mode_prob[i],
                         counts->uv_mode[i], fc->uv_mode_prob[i]);

  for (i = 0; i < PARTITION_CONTEXTS; i++)
    vpx_tree_merge_probs(av1_partition_tree, pre_fc->partition_prob[i],
...
void av1_adapt_intra_frame_probs(VP10_COMMON *cm) {
  int i;
  FRAME_CONTEXT *fc = cm->fc;
  const FRAME_CONTEXT *pre_fc = &cm->frame_contexts[cm->frame_context_idx];
  const FRAME_COUNTS *counts = &cm->counts;
  ...
  for (i = TX_4X4; i < EXT_TX_SIZES; ++i) {
    ...
    for (j = 0; j < TX_TYPES; ++j)
      vpx_tree_merge_probs(av1_ext_tx_tree, pre_fc->intra_ext_tx_prob[i][j],
                           counts->intra_ext_tx[i][j],
                           fc->intra_ext_tx_prob[i][j]);
  }
  for (i = TX_4X4; i < EXT_TX_SIZES; ++i) {
    vpx_tree_merge_probs(av1_ext_tx_tree, pre_fc->inter_ext_tx_prob[i],
  ...
  if (cm->seg.temporal_update) {
    ...
    vpx_tree_merge_probs(av1_segment_tree, pre_fc->seg.tree_probs,
...
void av1_setup_past_independence(VP10_COMMON *cm) {
