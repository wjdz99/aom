...
static void init_mode_probs(FRAME_CONTEXT *fc) {
...
#if CONFIG_INTRA_ENTROPY
  NN_CONFIG_EM *intra_y_mode = &fc->intra_y_mode;
  av1_zero(*intra_y_mode);
  intra_y_mode->lr = intra_y_mode_lr;
  intra_y_mode->num_hidden_layers = 0;
  intra_y_mode->input_layer.num_sparse_inputs = EM_NUM_Y_SPARSE_FEATURES;
  intra_y_mode->input_layer.num_dense_inputs = EM_NUM_Y_DENSE_FEATURES;
  memcpy(intra_y_mode->input_layer.sparse_input_size,
         intra_y_sparse_feat_sizes, sizeof(intra_y_sparse_feat_sizes));
  intra_y_mode->input_layer.num_outputs = INTRA_MODES;
  intra_y_mode->input_layer.activation = ACTN_NONE;
  ...
  for (int sparse_idx = 0;
       sparse_idx < intra_y_mode->input_layer.num_sparse_inputs;
       sparse_idx++) {
    const int arr_size =
        intra_y_mode->input_layer.sparse_input_size[sparse_idx] *
        intra_y_mode->input_layer.num_outputs;
    av1_copy_array(intra_y_mode_sprse_weights[sparse_idx],
                   intra_y_mode_input_layer_sparse_weights[sparse_idx], arr_size);
  }
  ...
  intra_uv_mode->input_layer.num_sparse_inputs = EM_NUM_UV_SPARSE_FEATURES;
  intra_uv_mode->input_layer.num_dense_inputs = 0;
  memcpy(intra_uv_mode->input_layer.sparse_input_size,
         intra_uv_sparse_feat_sizes, sizeof(intra_uv_sparse_feat_sizes));
  intra_uv_mode->input_layer.num_outputs = UV_INTRA_MODES;
  intra_uv_mode->input_layer.activation = ACTN_NONE;
  ...
  for (int sparse_idx = 0;
       sparse_idx < intra_uv_mode->input_layer.num_sparse_inputs;
       sparse_idx++) {
    const int arr_size =
        intra_uv_mode->input_layer.sparse_input_size[sparse_idx] *
        intra_uv_mode->input_layer.num_outputs;
    av1_copy_array(intra_uv_mode_sprse_weights[sparse_idx],
                   intra_uv_mode_input_layer_sparse_weights[sparse_idx], arr_size);
  }

  if (intra_uv_mode->input_layer.num_dense_inputs > 0) {
    memcpy(fc->intra_y_mode_input_layer_dense_weights,
