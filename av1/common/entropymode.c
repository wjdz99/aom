...
const aom_tree_index av1_seg_id_spatial_prediction_tree[TREE_SIZE(
    SEG_ID_SPATIAL_PREDICTIONS)] = {
  -SEG_ID_COPY_LEFT, 2, -SEG_ID_SPATIAL_UNPREDICTED, -SEG_ID_COPY_ABOVE
};
static const aom_prob default_seg_id_heterogeneous_spatial_prediction_tree_probs
    [HETEROGENEOUS_SPATIAL_PRED_PROBS] = { 180, 80 };
static const aom_prob default_seg_id_harmonic_spatial_prediction_tree_probs
    [HARMONIC_SPATIAL_PRED_PROBS] = { 60 };
#endif
static void init_mode_probs(FRAME_CONTEXT *fc) {
  ...
  av1_copy(fc->seg[ACTIVE_SEG_IDX].heterogeneous_spatial_pred_probs,
           default_seg_id_heterogeneous_spatial_prediction_tree_probs);
  av1_copy(fc->seg[ACTIVE_SEG_IDX].harmonic_spatial_pred_probs,
           default_seg_id_harmonic_spatial_prediction_tree_probs);
  ...
  av1_copy(fc->seg[QUALITY_SEG_IDX].heterogeneous_spatial_pred_probs,
           default_seg_id_heterogeneous_spatial_prediction_tree_probs);
  av1_copy(fc->seg[QUALITY_SEG_IDX].harmonic_spatial_pred_probs,
           default_seg_id_harmonic_spatial_prediction_tree_probs);
...
#if CONFIG_EXT_SEGMENT
  av1_tree_to_cdf(av1_seg_id_spatial_prediction_tree,
                  fc->seg[ACTIVE_SEG_IDX].heterogeneous_spatial_pred_probs,
                  fc->seg[ACTIVE_SEG_IDX].heterogeneous_spatial_pred_cdf);
  // this conversion will be re-do after num seg is determined.
  // For now, just default is used
  av1_tree_to_cdf(av1_segment_tree[MAX_SEGMENTS - 2],
                  fc->seg[ACTIVE_SEG_IDX].tree_probs,
                  fc->seg[ACTIVE_SEG_IDX].tree_cdf);

  av1_tree_to_cdf(av1_seg_id_spatial_prediction_tree,
                  fc->seg[QUALITY_SEG_IDX].heterogeneous_spatial_pred_probs,
                  fc->seg[QUALITY_SEG_IDX].heterogeneous_spatial_pred_cdf);
  // this conversion will be re-do after num seg is determined.
  // For now, just default is used
  av1_tree_to_cdf(av1_segment_tree[MAX_SEGMENTS - 2],
                  fc->seg[QUALITY_SEG_IDX].tree_probs,
                  fc->seg[QUALITY_SEG_IDX].tree_cdf);
...
void av1_set_mode_cdfs(struct AV1Common *cm) {
  ...
  if (cm->seg[ACTIVE_SEG_IDX].enabled && cm->seg[ACTIVE_SEG_IDX].update_map) {
    av1_tree_to_cdf(av1_segment_tree[cm->seg[ACTIVE_SEG_IDX].num_seg - 2],
                    cm->fc->seg[ACTIVE_SEG_IDX].tree_probs,
                    cm->fc->seg[ACTIVE_SEG_IDX].tree_cdf);
    av1_tree_to_cdf(
        av1_seg_id_spatial_prediction_tree,
        cm->fc->seg[ACTIVE_SEG_IDX].heterogeneous_spatial_pred_probs,
  ...
  if (cm->seg[QUALITY_SEG_IDX].enabled && cm->seg[QUALITY_SEG_IDX].update_map) {
    av1_tree_to_cdf(av1_segment_tree[cm->seg[QUALITY_SEG_IDX].num_seg - 2],
                    cm->fc->seg[QUALITY_SEG_IDX].tree_probs,
                    cm->fc->seg[QUALITY_SEG_IDX].tree_cdf);
    av1_tree_to_cdf(
        av1_seg_id_spatial_prediction_tree,
        cm->fc->seg[QUALITY_SEG_IDX].heterogeneous_spatial_pred_probs,
...
void av1_adapt_intra_frame_probs(AV1_COMMON *cm) {
  ...
  if (cm->seg[ACTIVE_SEG_IDX].temporal_update) {
    for (i = 0; i < PREDICTION_PROBS; i++)
      fc->seg[ACTIVE_SEG_IDX].pred_probs[i] =
          av1_mode_mv_merge_probs(pre_fc->seg[ACTIVE_SEG_IDX].pred_probs[i],
                                  counts->seg[ACTIVE_SEG_IDX].temp_pred[i]);
  }
  fc->seg[ACTIVE_SEG_IDX].harmonic_spatial_pred_probs[0] =
      av1_mode_mv_merge_probs(
          pre_fc->seg[ACTIVE_SEG_IDX].harmonic_spatial_pred_probs[0],
          counts->seg[ACTIVE_SEG_IDX].harmonic_spatial_pred);

  aom_tree_merge_probs(
      av1_seg_id_spatial_prediction_tree,
      pre_fc->seg[ACTIVE_SEG_IDX].heterogeneous_spatial_pred_probs,
      counts->seg[ACTIVE_SEG_IDX].heterogeneous_spatial_pred,
      fc->seg[ACTIVE_SEG_IDX].heterogeneous_spatial_pred_probs);

  aom_tree_merge_probs(av1_segment_tree[cm->seg[ACTIVE_SEG_IDX].num_seg - 2],
                       pre_fc->seg[ACTIVE_SEG_IDX].tree_probs,
                       counts->seg[ACTIVE_SEG_IDX].tree_mispred,
                       fc->seg[ACTIVE_SEG_IDX].tree_probs);

  if (cm->seg[QUALITY_SEG_IDX].temporal_update) {
    for (i = 0; i < PREDICTION_PROBS; i++)
      fc->seg[QUALITY_SEG_IDX].pred_probs[i] =
          av1_mode_mv_merge_probs(pre_fc->seg[QUALITY_SEG_IDX].pred_probs[i],
                                  counts->seg[QUALITY_SEG_IDX].temp_pred[i]);
  }
  fc->seg[QUALITY_SEG_IDX].harmonic_spatial_pred_probs[0] =
      av1_mode_mv_merge_probs(
          pre_fc->seg[QUALITY_SEG_IDX].harmonic_spatial_pred_probs[0],
          counts->seg[QUALITY_SEG_IDX].harmonic_spatial_pred);

  aom_tree_merge_probs(
      av1_seg_id_spatial_prediction_tree,
      pre_fc->seg[QUALITY_SEG_IDX].heterogeneous_spatial_pred_probs,
      counts->seg[QUALITY_SEG_IDX].heterogeneous_spatial_pred,
      fc->seg[QUALITY_SEG_IDX].heterogeneous_spatial_pred_probs);

  aom_tree_merge_probs(av1_segment_tree[cm->seg[QUALITY_SEG_IDX].num_seg - 2],
                       pre_fc->seg[QUALITY_SEG_IDX].tree_probs,
                       counts->seg[QUALITY_SEG_IDX].tree_mispred,
                       fc->seg[QUALITY_SEG_IDX].tree_probs);
#else
