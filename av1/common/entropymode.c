...
#if CONFIG_EXT_SEGMENT
static const aom_cdf_prob default_seg_tree_cdf[MAX_SEGMENTS - 1]
  ...
  { 2185, 4369, 6554, 8738, 10923, 13107, 15292, 17476, 19661, 21845, 24030,
    26214, 28399, 30583, 32768, 0},
  // 16 segments
  { 2048, 4096, 6144, 8192, 10240, 12288, 14336, 16384, 18432, 20480, 22528,
    24576, 26624, 28672, 30720, 32768, 0 },
};
static const aom_cdf_prob default_seg_id_spatial_prediction_tree_cdf
[SPATIAL_PRED_CTX][CDF_SIZE(SPATIAL_PRED_PROBS + 1)] =
{ { 23040, 26080, 32768, 0},{ 23040, 32768, 32768, 0} };
#else
...
#if CONFIG_EXT_SEGMENT
void init_segtree_cdf(FRAME_CONTEXT *fc, const int num_active_seg,
                             const int num_quality_seg) {
  av1_copy(fc->seg[ACTIVE_SEG_IDX].tree_cdf,
           default_seg_tree_cdf[num_active_seg - 2]);
  av1_copy(fc->seg[QUALITY_SEG_IDX].tree_cdf,
           default_seg_tree_cdf[num_quality_seg - 2]);
}
#endif
#if CONFIG_EXT_SEGMENT
static void init_mode_probs(FRAME_CONTEXT *fc, struct AV1Common *cm) {
#else
...
#else
  av1_copy(fc->seg[ACTIVE_SEG_IDX].spatial_pred_cdf,
           default_seg_id_spatial_prediction_tree_cdf);
  av1_copy(fc->seg[QUALITY_SEG_IDX].spatial_pred_cdf,
           default_seg_id_spatial_prediction_tree_cdf);
  av1_copy(fc->seg[ACTIVE_SEG_IDX].tree_cdf,
           default_seg_tree_cdf[AOMMAX(0, cm->seg[ACTIVE_SEG_IDX].num_seg - 2)]);
  av1_copy(fc->seg[QUALITY_SEG_IDX].tree_cdf,
           default_seg_tree_cdf[AOMMAX(0, cm->seg[QUALITY_SEG_IDX].num_seg - 2)]);
#endif
#else
...
void av1_set_mode_cdfs(struct AV1Common *cm) {
  ...
  if (cm->seg[ACTIVE_SEG_IDX].enabled && cm->seg[ACTIVE_SEG_IDX].update_map) {
    ...
    cm->fc->seg[QUALITY_SEG_IDX].spatial_pred_cdf[1][1] = 32768;
  }
