...
typedef struct FrameContexts {
  AomProb y_mode_prob[BLOCK_SIZE_GROUPS][INTRA_MODES - 1];
  AomProb uv_mode_prob[INTRA_MODES][INTRA_MODES - 1];
#if CONFIG_EXT_PARTITION_TYPES
  AomProb partition_prob[PARTITION_CONTEXTS][EXT_PARTITION_TYPES - 1];
#else
  AomProb partition_prob[PARTITION_CONTEXTS][PARTITION_TYPES - 1];
#endif
  Av1CoeffProbsModel coef_probs[TX_SIZES][PLANE_TYPES];
#if CONFIG_NEW_TOKENSET
  CoeffCdfModel coef_tail_cdfs[TX_SIZES][PLANE_TYPES];
  CoeffCdfModel coef_head_cdfs[TX_SIZES][PLANE_TYPES];
  AomProb blockzero_probs[TX_SIZES][PLANE_TYPES][REF_TYPES][BLOCKZ_CONTEXTS];
#elif CONFIG_EC_MULTISYMBOL
  CoeffCdfModel coef_cdfs[TX_SIZES][PLANE_TYPES];
#endif  // CONFIG_NEW_TOKENSET
  AomProb switchable_interp_prob[SWITCHABLE_FILTER_CONTEXTS]
                                [SWITCHABLE_FILTERS - 1];
#if CONFIG_ADAPT_SCAN
// TODO(angiebird): try AomProb
#if CONFIG_CB4X4
  ...
  ScanOrder sc[TX_SIZES_ALL][TX_TYPES];

  int16_t eob_threshold[TX_SIZES_ALL][TX_TYPES][EOB_THRESHOLD_NUM];
#endif  // CONFIG_ADAPT_SCAN

#if CONFIG_LV_MAP
  AomProb txb_skip[TX_SIZES][TXB_SKIP_CONTEXTS];
  AomProb nz_map[TX_SIZES][PLANE_TYPES][SIG_COEF_CONTEXTS];
  AomProb eob_flag[TX_SIZES][PLANE_TYPES][EOB_COEF_CONTEXTS];
  AomProb dc_sign[PLANE_TYPES][DC_SIGN_CONTEXTS];
  AomProb coeff_base[TX_SIZES][PLANE_TYPES][NUM_BASE_LEVELS]
                    [COEFF_BASE_CONTEXTS];
  AomProb coeff_lps[TX_SIZES][PLANE_TYPES][LEVEL_CONTEXTS];
#endif

  AomProb newmv_prob[NEWMV_MODE_CONTEXTS];
  AomProb zeromv_prob[ZEROMV_MODE_CONTEXTS];
  AomProb refmv_prob[REFMV_MODE_CONTEXTS];
  AomProb drl_prob[DRL_MODE_CONTEXTS];

  AomProb inter_mode_probs[INTER_MODE_CONTEXTS][INTER_MODES - 1];
#if CONFIG_EXT_INTER
  AomProb inter_compound_mode_probs[INTER_MODE_CONTEXTS]
                                   [INTER_COMPOUND_MODES - 1];
#if CONFIG_COMPOUND_SINGLEREF
  AomProb inter_singleref_comp_mode_probs[INTER_MODE_CONTEXTS]
                                         [INTER_SINGLEREF_COMP_MODES - 1];
#endif  // CONFIG_COMPOUND_SINGLEREF
  AomProb compound_type_prob[BLOCK_SIZES][COMPOUND_TYPES - 1];
  AomProb interintra_prob[BLOCK_SIZE_GROUPS];
  AomProb interintra_mode_prob[BLOCK_SIZE_GROUPS][INTERINTRA_MODES - 1];
  AomProb wedge_interintra_prob[BLOCK_SIZES];
#endif  // CONFIG_EXT_INTER
#if CONFIG_MOTION_VAR || CONFIG_WARPED_MOTION
  AomProb motion_mode_prob[BLOCK_SIZES][MOTION_MODES - 1];
#if CONFIG_MOTION_VAR && CONFIG_WARPED_MOTION
  AomProb obmc_prob[BLOCK_SIZES];
#endif  // CONFIG_MOTION_VAR && CONFIG_WARPED_MOTION
#endif  // CONFIG_MOTION_VAR || CONFIG_WARPED_MOTION
  AomProb intra_inter_prob[INTRA_INTER_CONTEXTS];
  AomProb comp_inter_prob[COMP_INTER_CONTEXTS];
  AomProb single_ref_prob[REF_CONTEXTS][SINGLE_REFS - 1];
#if CONFIG_EXT_REFS
  AomProb comp_ref_prob[REF_CONTEXTS][FWD_REFS - 1];
  AomProb comp_bwdref_prob[REF_CONTEXTS][BWD_REFS - 1];
#else
  AomProb comp_ref_prob[REF_CONTEXTS][COMP_REFS - 1];
#endif  // CONFIG_EXT_REFS
#if CONFIG_EXT_INTER && CONFIG_COMPOUND_SINGLEREF
  AomProb comp_inter_mode_prob[COMP_INTER_MODE_CONTEXTS];
#endif  // CONFIG_EXT_INTER && CONFIG_COMPOUND_SINGLEREF
  AomProb tx_size_probs[MAX_TX_DEPTH][TX_SIZE_CONTEXTS][MAX_TX_DEPTH];
#if CONFIG_VAR_TX
  AomProb txfm_partition_prob[TXFM_PARTITION_CONTEXTS];
#endif
  AomProb skip_probs[SKIP_CONTEXTS];
  NmvContext nmvc[NMV_CONTEXTS];
#if CONFIG_INTRABC
  NmvContext ndvc;
#endif
  int initialized;
#if CONFIG_EXT_TX
  AomProb inter_ext_tx_prob[EXT_TX_SETS_INTER][EXT_TX_SIZES][TX_TYPES - 1];
  AomProb intra_ext_tx_prob[EXT_TX_SETS_INTRA][EXT_TX_SIZES][INTRA_MODES]
                           [TX_TYPES - 1];
#else
  AomProb intra_ext_tx_prob[EXT_TX_SIZES][TX_TYPES][TX_TYPES - 1];
  AomProb inter_ext_tx_prob[EXT_TX_SIZES][TX_TYPES - 1];
#endif  // CONFIG_EXT_TX
#if CONFIG_SUPERTX
  AomProb supertx_prob[PARTITION_SUPERTX_CONTEXTS][TX_SIZES];
#endif  // CONFIG_SUPERTX
  struct SegmentationProbs seg;
#if CONFIG_EXT_INTRA
#if CONFIG_INTRA_INTERP
  AomProb intra_filter_probs[INTRA_FILTERS + 1][INTRA_FILTERS - 1];
#endif  // CONFIG_INTRA_INTERP
#endif  // CONFIG_EXT_INTRA
#if CONFIG_FILTER_INTRA
  AomProb filter_intra_probs[PLANE_TYPES];
#endif  // CONFIG_FILTER_INTRA
#if CONFIG_GLOBAL_MOTION
  AomProb global_motion_types_prob[GLOBAL_TRANS_TYPES - 1];
#endif  // CONFIG_GLOBAL_MOTION
#if CONFIG_LOOP_RESTORATION
  AomProb switchable_restore_prob[RESTORE_SWITCHABLE_TYPES - 1];
#endif  // CONFIG_LOOP_RESTORATION
#if CONFIG_EC_MULTISYMBOL
  AomCdfProb y_mode_cdf[BLOCK_SIZE_GROUPS][CDF_SIZE(INTRA_MODES)];
  AomCdfProb uv_mode_cdf[INTRA_MODES][CDF_SIZE(INTRA_MODES)];
#if CONFIG_EXT_PARTITION_TYPES
  AomCdfProb partition_cdf[PARTITION_CONTEXTS][CDF_SIZE(EXT_PARTITION_TYPES)];
#else
  AomCdfProb partition_cdf[PARTITION_CONTEXTS][CDF_SIZE(PARTITION_TYPES)];
#endif
  AomCdfProb switchable_interp_cdf[SWITCHABLE_FILTER_CONTEXTS]
                                  [CDF_SIZE(SWITCHABLE_FILTERS)];
  AomCdfProb inter_mode_cdf[INTER_MODE_CONTEXTS][CDF_SIZE(INTER_MODES)];
  /* Keep track of kf_y_cdf here, as this makes handling
     multiple copies for adaptation in tiles easier */
  AomCdfProb kf_y_cdf[INTRA_MODES][INTRA_MODES][CDF_SIZE(INTRA_MODES)];
  AomCdfProb tx_size_cdf[MAX_TX_DEPTH][TX_SIZE_CONTEXTS]
  ...
  AomCdfProb inter_ext_tx_cdf[EXT_TX_SETS_INTER][EXT_TX_SIZES]
                             [CDF_SIZE(TX_TYPES)];
#else
  AomCdfProb intra_ext_tx_cdf[EXT_TX_SIZES][TX_TYPES][CDF_SIZE(TX_TYPES)];
  AomCdfProb inter_ext_tx_cdf[EXT_TX_SIZES][CDF_SIZE(TX_TYPES)];
#endif  // CONFIG_EXT_TX
#if CONFIG_EXT_INTRA && CONFIG_INTRA_INTERP
  AomCdfProb intra_filter_cdf[INTRA_FILTERS + 1][CDF_SIZE(INTRA_FILTERS)];
#endif  // CONFIG_EXT_INTRA && CONFIG_INTRA_INTERP
#endif  // CONFIG_EC_MULTISYMBOL
#if CONFIG_DELTA_Q
  AomProb delta_q_prob[DELTA_Q_PROBS];
#if CONFIG_EXT_DELTA_Q
  AomProb delta_lf_prob[DELTA_LF_PROBS];
#endif
#endif
#if CONFIG_PVQ
...
#endif  // CONFIG_PVQ
#if CONFIG_CFL
  AomCdfProb cfl_alpha_cdf[CFL_ALPHABET_SIZE];
#endif
} FrameContext;
...
typedef struct FrameCounts {
...
#endif
  Av1CoeffCountModel coef[TX_SIZES][PLANE_TYPES];
...
#if CONFIG_EC_MULTISYMBOL
  Av1BlockzCountModel blockz_count[TX_SIZES][PLANE_TYPES];
#endif
  ...
  NmvContextCounts mv[NMV_CONTEXTS];
#if CONFIG_INTRABC
  NmvContextCounts dv;
#endif
#if CONFIG_DELTA_Q
...
extern const AomProb av1_kf_y_mode_prob[INTRA_MODES][INTRA_MODES]
                                       [INTRA_MODES - 1];
#if CONFIG_EC_MULTISYMBOL
// CDF version of 'av1_kf_y_mode_prob'.
extern const AomCdfProb av1_kf_y_mode_cdf[INTRA_MODES][INTRA_MODES]
                                         [CDF_SIZE(INTRA_MODES)];
#endif

#if CONFIG_PALETTE
extern const AomProb av1_default_palette_y_mode_prob[PALETTE_BLOCK_SIZES]
                                                    [PALETTE_Y_MODE_CONTEXTS];
extern const AomProb av1_default_palette_uv_mode_prob[PALETTE_UV_MODE_CONTEXTS];
extern const AomProb av1_default_palette_y_size_prob[PALETTE_BLOCK_SIZES]
                                                    [PALETTE_SIZES - 1];
extern const AomProb av1_default_palette_uv_size_prob[PALETTE_BLOCK_SIZES]
                                                     [PALETTE_SIZES - 1];
extern const AomProb av1_default_palette_y_color_index_prob
    [PALETTE_SIZES][PALETTE_COLOR_INDEX_CONTEXTS][PALETTE_COLORS - 1];
extern const AomProb av1_default_palette_uv_color_index_prob
    [PALETTE_SIZES][PALETTE_COLOR_INDEX_CONTEXTS][PALETTE_COLORS - 1];
#endif  // CONFIG_PALETTE

extern const AomTreeIndex av1_intra_mode_tree[TREE_SIZE(INTRA_MODES)];
extern const AomTreeIndex av1_inter_mode_tree[TREE_SIZE(INTER_MODES)];
#if CONFIG_EC_MULTISYMBOL
...
#if CONFIG_EXT_INTER
extern const AomTreeIndex av1_interintra_mode_tree[TREE_SIZE(INTERINTRA_MODES)];
...
#endif  // CONFIG_COMPOUND_SINGLEREF
extern const AomTreeIndex av1_compound_type_tree[TREE_SIZE(COMPOUND_TYPES)];
#endif  // CONFIG_EXT_INTER
extern const AomTreeIndex av1_partition_tree[TREE_SIZE(PARTITION_TYPES)];
#if CONFIG_EXT_PARTITION_TYPES
extern const AomTreeIndex
...
#if CONFIG_PALETTE
extern const AomTreeIndex av1_palette_size_tree[TREE_SIZE(PALETTE_SIZES)];
extern const AomTreeIndex
    av1_palette_color_index_tree[PALETTE_SIZES][TREE_SIZE(PALETTE_COLORS)];
#endif  // CONFIG_PALETTE
extern const AomTreeIndex av1_tx_size_tree[MAX_TX_DEPTH][TREE_SIZE(TX_SIZES)];
#if CONFIG_EXT_INTRA && CONFIG_INTRA_INTERP
extern const AomTreeIndex av1_intra_filter_tree[TREE_SIZE(INTRA_FILTERS)];
#endif  // CONFIG_EXT_INTRA && CONFIG_INTRA_INTERP
#if CONFIG_EXT_TX
extern const AomTreeIndex av1_ext_tx_inter_tree[EXT_TX_SETS_INTER]
                                               [TREE_SIZE(TX_TYPES)];
extern const AomTreeIndex av1_ext_tx_intra_tree[EXT_TX_SETS_INTRA]
                                               [TREE_SIZE(TX_TYPES)];
#else
extern const AomTreeIndex av1_ext_tx_tree[TREE_SIZE(TX_TYPES)];
#endif  // CONFIG_EXT_TX
#if CONFIG_MOTION_VAR || CONFIG_WARPED_MOTION
extern const AomTreeIndex av1_motion_mode_tree[TREE_SIZE(MOTION_MODES)];
#endif  // CONFIG_MOTION_VAR || CONFIG_WARPED_MOTION
