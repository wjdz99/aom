...
typedef struct {
  ...
  // restoration units in (one row of) the top-left tile in the frame. The data
  // in unit_info is laid out with units_per_tile entries for each tile, which
  // have stride horz_units_per_tile.
  //
  // If there are several tiles, the tiles on the right or bottom edge might
  // actually be slightly smaller, but the data in unit_info is laid out as if
  // all tiles are of full size.
  int units_per_tile;
