...
typedef struct {
  ...
  // restoration units in (one row of) the largest tile in the frame.
  //
  ...
  /*!
   * Whether optimized lr can be used for speed in cases of
   * no cdef and no superres, or if trial runs are used on the
   * encode side.
   */
  int optimized_lr;
} RestorationInfo;
