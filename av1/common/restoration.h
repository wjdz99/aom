...
typedef struct {
  ...
  // Temporary buffers to save/restore 2 lines above/below the restoration stripe
  // Allow for filter margin to left and right
  uint16_t tmp_save_above[2][RESTORATION_TILESIZE_MAX + 2*RESTORATION_EXTRA_HORZ];
  uint16_t tmp_save_below[2][RESTORATION_TILESIZE_MAX + 2*RESTORATION_EXTRA_HORZ];
#endif
} RestorationInternal;
...
static INLINE void av1_get_rest_tile_limits(
    ...
    int tile_width, int tile_height, int im_width, int im_height,
#if CONFIG_STRIPED_LOOP_RESTORATION
    int subsampling_y,
#endif
    int clamp_h, int clamp_v, int *h_start, int *h_end, int *v_start, int *v_end) {
  ...
  *v_start -= RESTORATION_TILE_OFFSET>>subsampling_y;
  if (*v_start<0)
    *v_start = 0;
  if (*v_end<im_height)
    *v_end -= RESTORATION_TILE_OFFSET>>subsampling_y;
#endif
}
