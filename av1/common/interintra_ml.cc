...
void load_inputs(tflite::Interpreter *interpreter, INTERINTRA_MODE mode,
  ...
  for (int j = 0; j < INTERINTRA_ML_BORDER; ++j) {
    for (int i = 0; i < bw + INTERINTRA_ML_BORDER; ++i) {
      intra_top_input[interpreter_offset++] = intra_pred[i + j * intra_stride];
    }
  }
  ...
  for (int j = INTERINTRA_ML_BORDER; j < INTERINTRA_ML_BORDER + bh; ++j) {
    for (int i = 0; i < INTERINTRA_ML_BORDER; ++i) {
      intra_left_input[interpreter_offset++] = intra_pred[i + j * intra_stride];
    }
  }
...
void copy_to_output(tflite::Interpreter *interpreter, BLOCK_SIZE bsize,
  ...
  for (int j = 0; j < bh; ++j) {
    for (int i = 0; i < bw; ++i) {
      comp_pred[i + j * comp_stride] = output[i + j * bw];
    }
  }
}
