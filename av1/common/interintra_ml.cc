...
void load_inputs(tflite::Interpreter *interpreter, BLOCK_SIZE bsize,
  ...
  int *inter_input = interpreter->typed_input_tensor<int>(0);
  ...
  int *intra_input = interpreter->typed_input_tensor<int>(1);
...
void av1_combine_interintra_ml(BLOCK_SIZE plane_bsize, uint8_t *comp_pred,
  ...
  load_inputs(interpreter, plane_bsize, inter_pred, inter_stride, intra_pred,
              intra_stride);
...
void av1_combine_interintra_ml_highbd(BLOCK_SIZE plane_bsize,
