...
tflite::ErrorReporter *get_or_init_reporter() {
  ...
  static tflite::ErrorReporter *reporter_ = nullptr;
...
void load_inputs(tflite::Interpreter *interpreter, INTERINTRA_MODE mode,
  ...
  for (int j = 0; j < bh + INTERINTRA_ML_BORDER; ++j) {
    std::copy(inter_pred + j * inter_stride,
              inter_pred + j * inter_stride + bw + INTERINTRA_ML_BORDER,
  ...
  for (int j = 0; j < INTERINTRA_ML_BORDER; ++j) {
    std::copy(intra_pred + j * intra_stride,
              intra_pred + j * intra_stride + bw + INTERINTRA_ML_BORDER,
  ...
  for (int j = INTERINTRA_ML_BORDER; j < INTERINTRA_ML_BORDER + bh; ++j) {
    std::copy(intra_pred + j * intra_stride,
              intra_pred + j * intra_stride + INTERINTRA_ML_BORDER,
