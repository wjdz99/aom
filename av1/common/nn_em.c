...
void av1_nn_sparse_forward_c(FC_INPUT_LAYER_EM *layer, const int *input) {
  ...
  for (int sparse_idx = 0; sparse_idx < layer->num_sparse_inputs;
    ...
    const float *weight_ptr =
        sparse_weights[sparse_idx] + input[sparse_idx] * output_size;
...
void av1_nn_input_forward(FC_INPUT_LAYER_EM *layer, const int *sparse_features,
  ...
  const float *bias = layer->bias;
  memcpy(output, bias, output_size * sizeof(*bias));

  // Handle sparse layer
  if (has_sparse) {
    av1_nn_sparse_forward(layer, sparse_features);
  }
...
static void nn_fc_input_backward(const int *sparse_features,
  ...
  // Handle bias
  float *db = layer->db;
  memcpy(db, dY_fc, num_out * sizeof(*dY_fc));
  ...
  if (has_sparse) {
    for (int s_idx = 0; s_idx < num_sparse; s_idx++) {
      ...
      memcpy(&dW_sparse[s_idx][non_zero_idx * num_out], dY_fc,
             num_out * sizeof(*dY_fc));
    }
  }
