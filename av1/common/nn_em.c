...
static void nn_fc_backward(const float *X, float *dX_out, FC_LAYER_EM *layer) {
  ...
  av1_copy_array(db, dY_fc, layer->num_outputs);
...
static void nn_fc_input_backward(const int *sparse_features,
  ...
  // Handle bias
  float *db = layer->db;
  av1_copy_array(db, dy_fc, num_out);
  ...
  if (has_sparse) {
    for (int s_idx = 0; s_idx < num_sparse; s_idx++) {
      ...
      av1_copy_array(&dw_sparse[s_idx][non_zero_idx * num_out], dy_fc, num_out);
    }
  }
...
static void update_input_layer(NN_CONFIG_EM *nn_config, float mu) {
  ...
  if (has_dense) {
    ...
    adapt(w_dense, dw_dense, mu, num_dense * num_out);
  }
}
