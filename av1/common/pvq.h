...
struct od_pvq_codeword_ctx {
  ...
  uint16_t            pvq_k1_cdf[12][16 + CONFIG_EC_ADAPT];
  uint16_t            pvq_split_cdf[22*7][8 + CONFIG_EC_ADAPT];
};
...
struct od_pvq_adapt_ctx {
  ...
  uint16_t        pvq_gaintheta_cdf[2*OD_TXSIZES*PVQ_MAX_PARTITIONS][16 + CONFIG_EC_ADAPT];
  uint16_t        pvq_skip_dir_cdf[2*(OD_TXSIZES-1)][7 + CONFIG_EC_ADAPT];
};
