...
static INLINE void free_cdef_linebuf_conditional(
  ...
  for (int plane = 0; plane < MAX_MB_PLANE; plane++) {
    if (new_linebuf_size[plane] != cdef_info->prev_linebuf_size[plane]) {
...
static INLINE void free_cdef_bufs_conditional(AV1_COMMON *const cm,
  ...
  CdefInfo *cdef_info = &cm->cdef_info;
  if (new_srcbuf_size != cdef_info->prev_srcbuf_size) {
    aom_free(*srcbuf);
    *srcbuf = NULL;
  }
  for (int plane = 0; plane < MAX_MB_PLANE; plane++) {
    if (new_colbuf_size[plane] != cdef_info->prev_colbuf_size[plane]) {
...
static INLINE void alloc_cdef_linebuf(AV1_COMMON *const cm,
  ...
  CdefInfo *cdef_info = &cm->cdef_info;
  for (int plane = 0; plane < MAX_MB_PLANE; plane++) {
    if (linebuf[plane] == NULL && cdef_info->prev_linebuf_size[plane] != 0) {
      CHECK_MEM_ERROR(cm, linebuf[plane],
                      aom_malloc(cdef_info->prev_linebuf_size[plane]));
    }
  }
}
...
static INLINE void alloc_cdef_bufs(AV1_COMMON *const cm, uint16_t **colbuf,
                                   uint16_t **srcbuf) {
  CdefInfo *cdef_info = &cm->cdef_info;
  if (*srcbuf == NULL && cdef_info->prev_srcbuf_size != 0)
    CHECK_MEM_ERROR(cm, *srcbuf, aom_memalign(16, cdef_info->prev_srcbuf_size));

  for (int plane = 0; plane < MAX_MB_PLANE; plane++) {
    if (colbuf[plane] == NULL && cdef_info->prev_colbuf_size[plane] != 0) {
      CHECK_MEM_ERROR(cm, colbuf[plane],
                      aom_malloc(cdef_info->prev_colbuf_size[plane]));
    }
  }
}
...
void av1_alloc_cdef_buffers(AV1_COMMON *const cm,
  ...
  int is_num_workers_changed = cdef_info->allocated_num_workers != num_workers;
  ...
  // Calculate size of src, line and column buffers
  const int is_cdef_enabled =
      cm->seq_params.enable_cdef && !cm->tiles.large_scale;
  new_srcbuf_size =
      sizeof(*cdef_info->srcbuf) * CDEF_INBUF_SIZE * is_cdef_enabled;
  for (int plane = 0; plane < num_planes; plane++) {
    const int shift = plane == AOM_PLANE_Y ? 0 : cm->seq_params.subsampling_x;
    const int luma_stride =
        ALIGN_POWER_OF_TWO(cm->mi_params.mi_cols << MI_SIZE_LOG2, 4);
    const int stride = luma_stride >> shift;
    new_linebuf_size[plane] = sizeof(*cdef_info->linebuf) * num_bufs *
                              (CDEF_VBORDER << 1) * stride * is_cdef_enabled;

    const int block_height =
        (CDEF_BLOCKSIZE << (MI_SIZE_LOG2 - shift)) * 2 * CDEF_VBORDER;
    new_colbuf_size[plane] = sizeof(*cdef_info->colbuf[plane]) * block_height *
                             CDEF_HBORDER * is_cdef_enabled;
  }
  ...
  // num_workers
  if (is_num_workers_changed && *cdef_worker != NULL) {
    for (int idx = cdef_info->allocated_num_workers - 1; idx >= 1; idx--) {
      free_cdef_bufs((*cdef_worker)[idx].colbuf, &(*cdef_worker)[idx].srcbuf);
    }
  } else if (num_workers > 1 && *cdef_worker != NULL) {
    // Free src and column buffers for remaining workers in case of reallocation
    for (int idx = num_workers - 1; idx >= 1; idx--) {
  ...
  // Updating buffer sizes to cdef_info
  cdef_info->prev_srcbuf_size = new_srcbuf_size;
  av1_copy(cdef_info->prev_colbuf_size, new_colbuf_size);
  av1_copy(cdef_info->prev_linebuf_size, new_linebuf_size);
  ...
  alloc_cdef_bufs(cm, cdef_info->colbuf, &cdef_info->srcbuf);
  alloc_cdef_linebuf(cm, cdef_info->linebuf);
