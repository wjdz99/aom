...
int levels_to_id(const int lev[DERING_REFINEMENT_LEVELS], const int str[CLPF_REFINEMENT_LEVELS]) {
  return str[1]+CLPF_STRENGTHS*(str[0]+CLPF_STRENGTHS*(lev[3] + DERING_STRENGTHS*(lev[2] + DERING_STRENGTHS*(lev[1] + DERING_STRENGTHS*lev[0]))));
}

void id_to_levels(int lev[DERING_REFINEMENT_LEVELS], int str[CLPF_REFINEMENT_LEVELS], int id) {
  lev[0] = id/(DERING_STRENGTHS*DERING_STRENGTHS*DERING_STRENGTHS*CLPF_STRENGTHS*CLPF_STRENGTHS);
  id -= lev[0]*(DERING_STRENGTHS*DERING_STRENGTHS*DERING_STRENGTHS*CLPF_STRENGTHS*CLPF_STRENGTHS);
  lev[1] = id/(DERING_STRENGTHS*DERING_STRENGTHS*CLPF_STRENGTHS*CLPF_STRENGTHS);
  id -= lev[1]*(DERING_STRENGTHS*DERING_STRENGTHS*CLPF_STRENGTHS*CLPF_STRENGTHS);
  lev[2] = id/(DERING_STRENGTHS*CLPF_STRENGTHS*CLPF_STRENGTHS);
  id -= lev[2]*(DERING_STRENGTHS*CLPF_STRENGTHS*CLPF_STRENGTHS);
  lev[3] = id/(CLPF_STRENGTHS*CLPF_STRENGTHS);
  id -= lev[3]*(CLPF_STRENGTHS*CLPF_STRENGTHS);
  str[0] = id/CLPF_STRENGTHS;
  id -= str[0]*(CLPF_STRENGTHS);
  str[1] = id;
}
...
void av1_dering_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
  ...
  for (sbr = 0; sbr < nvsb; sbr++) {
    ...
    for (sbc = 0; sbc < nhsb; sbc++) {
      ...
      int nhb, nvb;
      int cstart = 0;
      BOUNDARY_TYPE boundary_type = cm->mi_grid_visible[MAX_MIB_SIZE * sbr * cm->mi_stride +
                                                        MAX_MIB_SIZE * sbc]->mbmi.boundary_info;
      if (!dering_left) cstart = -OD_FILT_HBORDER;
      nhb = AOMMIN(MAX_MIB_SIZE, cm->mi_cols - MAX_MIB_SIZE * sbc);
      nvb = AOMMIN(MAX_MIB_SIZE, cm->mi_rows - MAX_MIB_SIZE * sbr);
      level = cm->dering_signal ? cm->dering_signal : dering_level_table[lev[cm->mi_grid_visible[MAX_MIB_SIZE * sbr * cm->mi_stride +
                                            MAX_MIB_SIZE * sbc]
                            ->mbmi.dering_gain]];
      clpf_strength = cm->clpf_signal ? cm->clpf_signal : str[cm->mi_grid_visible[MAX_MIB_SIZE * sbr * cm->mi_stride +
					      MAX_MIB_SIZE * sbc]->mbmi.clpf_strength];
      ...
      for (pli = 0; pli < nplanes; pli++) {
        ...
        od_dering(
            ...
            dec[pli], dir, pli, dlist, dering_count, threshold, clpf_strength, clpf_damping, coeff_shift, boundary_type);
#if CONFIG_AOM_HIGHBITDEPTH
        if (cm->use_highbitdepth) {
