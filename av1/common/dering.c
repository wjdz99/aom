...
void id_to_levels(int lev[DERING_REFINEMENT_LEVELS],
  ...
  // Pack tables
  int j;
  for (i = j = 1; i < DERING_REFINEMENT_LEVELS && j < DERING_REFINEMENT_LEVELS; i++)
    if (lev[j - 1] == lev[j]) {
      memmove(&lev[j - 1], &lev[j],
              (DERING_REFINEMENT_LEVELS - j) * sizeof(*lev));
    } else
      j++;
  for (i = j = 1; i < CLPF_REFINEMENT_LEVELS && j < DERING_REFINEMENT_LEVELS; i++)
    if (str[j - 1] == str[j]) {
...
void av1_dering_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
                      MACROBLOCKD *xd, uint32_t global_level) {
  int r, c;
  int sbr, sbc;
  int nhsb, nvsb;
  ...
  for (sbr = 0; sbr < nvsb; sbr++) {
    ...
    for (sbc = 0; sbc < nhsb; sbc++) {
      ...
      level = !pli || chroma_dering ? dering_level_table
          [lev[cm->mi_grid_visible[MAX_MIB_SIZE * sbr * cm->mi_stride +
                                   MAX_MIB_SIZE * sbc]
               ->mbmi.dering_gain]] : 0;
      clpf_strength = pli == 1 ? cm->clpf_strength_u : (pli == 2 ? cm->clpf_strength_v :
