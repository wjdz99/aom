...
uint64_t levels_to_id(const int lev[DERING_REFINEMENT_LEVELS],
...
void id_to_levels(int lev[DERING_REFINEMENT_LEVELS],
                  int str[CLPF_REFINEMENT_LEVELS], uint64_t id) {
  int i;
  ...
  // Pack tables
  for (i = 1; i < DERING_REFINEMENT_LEVELS && lev[i] >= 0; i++)
    if (lev[i - 1] == lev[i]) {
      memmove(&lev[i - 1], &lev[i],
              (DERING_REFINEMENT_LEVELS - i) * sizeof(*lev));
      lev[DERING_REFINEMENT_LEVELS - 1] = -1;
      i--;
    }
  for (i = 1; i < CLPF_REFINEMENT_LEVELS && str[i] >= 0; i++)
    if (str[i - 1] == str[i]) {
      memmove(&str[i - 1], &str[i],
              (CLPF_REFINEMENT_LEVELS - i) * sizeof(*str));
      str[CLPF_REFINEMENT_LEVELS - 1] = -1;
      i--;
    }
}
...
void cdef_get_bits(const int *lev, const int *str, int *dering_bits,
                   int *clpf_bits) {
  // printf("%d %d %d %d  %d %d\n", lev[0], lev[1], lev[2], lev[3], str[0],
  // str[1]);
  for (*dering_bits = 0;
       *dering_bits < DERING_REFINEMENT_LEVELS && lev[*dering_bits] >= 0;
       (*dering_bits)++)
    ;
  *dering_bits = get_msb(*dering_bits);
  for (*clpf_bits = 0;
       *clpf_bits < CLPF_REFINEMENT_LEVELS && str[*clpf_bits] >= 0;
       (*clpf_bits)++)
    ;
  *clpf_bits = get_msb(*clpf_bits);
  // printf("%d %d\n", *dering_bits, *clpf_bits);
}
...
void av1_dering_frame(YV12_BUFFER_CONFIG *frame, AV1_COMMON *cm,
                      MACROBLOCKD *xd, uint64_t global_level) {
  int r, c;
  int sbr, sbc;
  int nhsb, nvsb;
