...
void FilterIntraPredictor_C(void *const dest, ptrdiff_t stride,
                            ...
                            const void *const left_column, const int mode,
                            const int width, const int height) {
  const uint8_t *const top = (const uint8_t *)top_row;
  const uint8_t *const left = (const uint8_t *)left_column;

  assert(width <= 32 && height <= 32);

  uint8_t buffer[3][33];  // cache 2 rows + top & left boundaries
  ...
  do {
    ...
    int x = 1;
    do {
      const uint8_t p0 = buffer[row0][x - 1];  // top-left
      const uint8_t p1 = buffer[row0][x + 0];  // top 0
      const uint8_t p2 = buffer[row0][x + 1];  // top 1
      const uint8_t p3 = buffer[row0][x + 2];  // top 2
      const uint8_t p4 = buffer[row0][x + 3];  // top 3
      const uint8_t p5 = buffer[1][x - 1];     // left 0
      const uint8_t p6 = buffer[row2][x - 1];  // left 1
      for (int i = 0; i < 8; ++i) {
        const int xoffset = i & 0x03;
        const int yoffset = (i >> 2) * ystep;
        const int value = av1_filter_intra_taps[mode][i][0] * p0 +
                          av1_filter_intra_taps[mode][i][1] * p1 +
                          av1_filter_intra_taps[mode][i][2] * p2 +
                          av1_filter_intra_taps[mode][i][3] * p3 +
                          av1_filter_intra_taps[mode][i][4] * p4 +
                          av1_filter_intra_taps[mode][i][5] * p5 +
                          av1_filter_intra_taps[mode][i][6] * p6;
        buffer[1 + yoffset][x + xoffset] = (uint8_t)clip_pixel(
            ROUND_POWER_OF_TWO_SIGNED(value, FILTER_INTRA_SCALE_BITS));
      }
      x += 4;
    } while (x < width);
...
#if CONFIG_AV1_HIGHBITDEPTH
void HighbdFilterIntraPredictor_C(void *const dest, ptrdiff_t stride,
                                  const void *const top_row,
                                  const void *const left_column, const int mode,
                                  const int width, const int height,
                                  const int bitdepth) {
  const int kMaxPixel = (1 << bitdepth) - 1;
  const uint16_t *const top = (const uint16_t *)top_row;
  const uint16_t *const left = (const uint16_t *)left_column;

  assert(width <= 32 && height <= 32);

  uint16_t buffer[3][33];  // cache 2 rows + top & left boundaries
  memcpy(buffer[0], &top[-1], (width + 1) * sizeof(top[0]));
  ...
  do {
    ...
    int x = 1;
    do {
      const uint16_t p0 = buffer[row0][x - 1];  // top-left
      const uint16_t p1 = buffer[row0][x + 0];  // top 0
      const uint16_t p2 = buffer[row0][x + 1];  // top 1
      const uint16_t p3 = buffer[row0][x + 2];  // top 2
      const uint16_t p4 = buffer[row0][x + 3];  // top 3
      const uint16_t p5 = buffer[1][x - 1];     // left 0
      const uint16_t p6 = buffer[row2][x - 1];  // left 1
      for (int i = 0; i < 8; ++i) {
        const int xoffset = i & 0x03;
        const int yoffset = (i >> 2) * ystep;
        const int value = av1_filter_intra_taps[mode][i][0] * p0 +
                          ...
                          av1_filter_intra_taps[mode][i][5] * p5 +
                          av1_filter_intra_taps[mode][i][6] * p6;
        buffer[1 + yoffset][x + xoffset] = (uint16_t)clamp(
            ROUND_POWER_OF_TWO_SIGNED(value, FILTER_INTRA_SCALE_BITS), 0,
            kMaxPixel);
      }
      x += 4;
    } while (x < width);
...
static void highbd_filter_intra_predictor(uint16_t *dst, ptrdiff_t stride,
  ...
  const int bw = tx_size_wide[tx_size];
  const int bh = tx_size_high[tx_size];
  HighbdFilterIntraPredictor_C(dst, stride, above, left, mode, bw, bh, bd);
}
#endif  // CONFIG_AV1_HIGHBITDEPTH
