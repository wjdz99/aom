...
#if CONFIG_MRLS
  const int max_base_y = (bw + bh - 1 + mrl_index * 2) << upsample_left;
#else
...
static void build_intra_predictors_high(
...
#if CONFIG_MRLS
  int mrl_index = (plane == PLANE_TYPE_Y && is_inter_block(xd->mi[0]) == 0)
                      ? xd->mi[0]->mrl_index
                      : 0;
  ...
  if (need_left) {
    ...
    const int num_left_pixels_needed =
        txhpx + (need_bottom ? txwpx : 3) + mrl_index * 2;
#else
...
static void build_intra_predictors(const MACROBLOCKD *xd, const uint8_t *ref,
  ...
  int i;
#if CONFIG_MRLS
  uint8_t mrl_index = (plane == PLANE_TYPE_Y && is_inter_block(xd->mi[0]) == 0)
                          ? xd->mi[0]->mrl_index
                          : 0;
  const int above_mrl_idx = is_sb_boundary ? 0 : mrl_index;
