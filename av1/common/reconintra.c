...
void av1_predict_intra_block_facade(const AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  const int angle_delta =
      cm->seq_params.enable_orip
          ? ((mbmi->angle_delta[plane != AOM_PLANE_Y] == ANGLE_DELTA_FOR_FILTER)
                 ? 0
  ...
  if (plane != AOM_PLANE_Y && mbmi->uv_mode == UV_CFL_PRED) {
    ...
    if (cfl->dc_pred_is_cached[pred_plane] == 0) {
      ...
      av1_predict_intra_block(
          ...
          use_palette, filter_intra_mode, dst, dst_stride, dst, dst_stride,
          blk_col, blk_row, plane, disable_intra_pred_filter_for_hor_ver_mode);
#else
...
void av1_apply_orip_4x4subblock(uint8_t *dst, ptrdiff_t stride, TX_SIZE tx_size,
...
#if !REPLACE_MEMCPY_WITH_ASSIGNMENT
  int num_top_ref = 5;
  int num_left_ref = 4;
#endif
...
#if REPLACE_MEMCPY_WITH_ASSIGNMENT
  ref_samples_sb_row[0] = above[-1];
  ref_samples_sb_row[1] = above[0];
  ref_samples_sb_row[2] = above[1];
  ref_samples_sb_row[3] = above[2];
  ref_samples_sb_row[4] = above[3];
  ref_samples_sb_row[5] = left[0];
  ref_samples_sb_row[6] = left[1];
  ref_samples_sb_row[7] = left[2];
  ref_samples_sb_row[8] = left[3];
  // initialize references for the column
  if (num_vertical_sb > 1) {
    ref_samples_sb_col[0] = left[3];
    ref_samples_sb_col[1] = dst[3 * stride];
    ref_samples_sb_col[2] = dst[3 * stride + 1];
    ref_samples_sb_col[3] = dst[3 * stride + 2];
    ref_samples_sb_col[4] = dst[3 * stride + 3];
    ref_samples_sb_col[5] = left[4];
    ref_samples_sb_col[6] = left[5];
    ref_samples_sb_col[7] = left[6];
    ref_samples_sb_col[8] = left[7];
  }
#else
...
#if REPLACE_MEMCPY_WITH_ASSIGNMENT
    ref_samples_sb_row[0] = above[c_sb - 1];
    ref_samples_sb_row[1] = above[c_sb];
    ref_samples_sb_row[2] = above[c_sb + 1];
    ref_samples_sb_row[3] = above[c_sb + 2];
    ref_samples_sb_row[4] = above[c_sb + 3];
#else
...
#if REPLACE_MEMCPY_WITH_ASSIGNMENT
    ref_samples_sb_row[5] = left_ref_tmp_for_next_sb[0];
    ref_samples_sb_row[6] = left_ref_tmp_for_next_sb[1];
    ref_samples_sb_row[7] = left_ref_tmp_for_next_sb[2];
    ref_samples_sb_row[8] = left_ref_tmp_for_next_sb[3];
#else
  ...
  if (num_vertical_sb > 1) {
    for (int m = 1; m < num_vertical_sb; m++) {
...
#if REPLACE_MEMCPY_WITH_ASSIGNMENT
      ref_samples_sb_col[5] = left[r_sb];
      ref_samples_sb_col[6] = left[r_sb + 1];
      ref_samples_sb_col[7] = left[r_sb + 2];
      ref_samples_sb_col[8] = left[r_sb + 3];
      top_ref_tmp_for_next_sb[0] = dst[(r_sb + 3) * stride];
      top_ref_tmp_for_next_sb[1] = dst[1 + (r_sb + 3) * stride];
      top_ref_tmp_for_next_sb[2] = dst[2 + (r_sb + 3) * stride];
      top_ref_tmp_for_next_sb[3] = dst[3 + (r_sb + 3) * stride];
#else
      ...
      ref_samples_sb_col[2] = top_ref_tmp_for_next_sb[1];
      ref_samples_sb_col[3] = top_ref_tmp_for_next_sb[2];
      ref_samples_sb_col[4] = top_ref_tmp_for_next_sb[3];
#else
