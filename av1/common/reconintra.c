...
static void second_dr_predictor(uint8_t *dst, ptrdiff_t stride, TX_SIZE tx_size,
  ...
  if (angle > 0 && angle < 90) {
    ...
    av1_dr_prediction_z1(dst, stride, bw, bh, above, left,
                         upsample_above, dx, dy, 0);
#else
    av1_dr_prediction_z1(dst, stride, bw, bh, above, left,
                         upsample_above, dx, dy);
#endif
  }
}
#endif
...
static void build_intra_predictors_high(
  ...
  if (is_dr_mode) {
    ...
    if (!disable_edge_filter) {
      ...
      int filt_type_above = ab ? is_smooth(ab, plane) : 0;
      int filt_type_left = le ? is_smooth(le, plane) : 0;
#else
      ...
      upsample_above =
          av1_use_intra_edge_upsample(txwpx, txhpx, cur_angle, filt_type_above);
#else
      upsample_above =
...
#if CONFIG_IBP
      cur_angle = (p_angle < 90) ? p_angle : (p_angle - 180);
      upsample_left =
          av1_use_intra_edge_upsample(txhpx, txwpx, cur_angle, filt_type_left);
#else
      upsample_left =
