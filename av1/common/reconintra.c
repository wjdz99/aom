...
#define ADAPT_FILTER_INTRA_DEFINE_3_TAP_ACCUM_FUNC(func_name, tap1, tap2, \
  ...
  void func_name(const uint16_t *src, int stride, int w, int h,           \
    ...
    for (int i = 0; i < h; i++) {                                         \
      for (int j = 0; j < w; j++) {                                       \
        int x = src[i * stride + j];                                      \
        int v1 = ADAPT_FILTER_INTRA_GET_SRC_VAL_##tap1;                   \
        int v2 = ADAPT_FILTER_INTRA_GET_SRC_VAL_##tap2;                   \
        int v3 = ADAPT_FILTER_INTRA_GET_SRC_VAL_##tap3;                   \
        dst_buf[0] += v1 * v1;                                            \
        dst_buf[1] += v1 * v2;                                            \
        dst_buf[2] += v2 * v2;                                            \
        dst_buf[3] += v1 * v3;                                            \
  ...
  void func_name(const uint16_t *src, int stride, int w, int h,           \
    ...
    for (int i = 0; i < h; i++) {                                         \
      for (int j = 0; j < w; j++) {                                       \
        ...
        int v2 = ADAPT_FILTER_INTRA_GET_SRC_VAL_##tap2;                   \
        int v3 = ADAPT_FILTER_INTRA_GET_SRC_VAL_##tap3;                   \
        int v4 = ADAPT_FILTER_INTRA_GET_SRC_VAL_##tap4;                   \
        dst_buf[0] += v1 * v1;                                            \
        dst_buf[1] += v1 * v2;                                            \
        dst_buf[2] += v2 * v2;                                            \
        dst_buf[3] += v1 * v3;                                            \
...
static const int
    adapt_filter_intra_thick_hor[TX_SIZES_ALL][ADAPT_FILTER_INTRA_MODES] = {
...
static const int
    adapt_filter_intra_thick_ver[TX_SIZES_ALL][ADAPT_FILTER_INTRA_MODES] = {
...
static const int
    adapt_filter_intra_top_right_offs[TX_SIZES_ALL][ADAPT_FILTER_INTRA_MODES] =
        {
...
static const int adapt_filter_intra_bottom_left_offs
...
static void adapt_filter_intra_accumulate_stats(
  ...
  const int up_offs =
      AOMMIN(adapt_filter_intra_thick_ver[tx_size][mode], px_row) - 1;
  const int left_offs =
      AOMMIN(adapt_filter_intra_thick_hor[tx_size][mode], px_col) - 1;
  const int top_right_offs = adapt_filter_intra_top_right_offs[tx_size][mode];
  const int bottom_left_offs =
      adapt_filter_intra_bottom_left_offs[tx_size][mode];
  ...
  if (adapt_filter_intra_top_allowed[mode] &&
    ...
    int extra_offs = (n_left_px > 0 || bottom_left_offs <= 0)
    ...
    int extra_offs = (n_top_px > 0 || top_right_offs <= 0)
...
static void adapt_filter_intra_solve_3x3(const int64_t *coefs,
  ...
  int64_t base_det = lhs[0] * a[3] - lhs[1] * a[1] + lhs[3] * a[0];
  assert(base_det != 0);
  int64_t det1 = rhs[0] * a[3] + lhs[1] * a[5] - lhs[3] * a[4];
  int64_t det2 = -lhs[0] * a[5] - rhs[0] * a[1] + lhs[3] * a[2];
  int64_t det3 = lhs[0] * a[4] - lhs[1] * a[2] + rhs[0] * a[0];
...
static void adapt_filter_intra_solve_4x4(const int64_t *coefs,
  ...
  int64_t base_det = a[0] * b[7] + a[7] * b[0] + a[2] * b[4] + a[4] * b[2] -
                     a[5] * b[1] - a[1] * b[5];
  assert(base_det != 0);
  int64_t det1 = a[5] * b[8] + a[8] * b[5] - a[6] * b[7] - a[7] * b[6] -
                 a[4] * b[9] - a[9] * b[4];
  int64_t det2 = a[1] * b[9] + a[9] * b[1] + a[3] * b[7] + a[7] * b[3] -
                 a[2] * b[8] - a[8] * b[2];
  int64_t det3 = a[2] * b[6] + a[6] * b[2] - a[0] * b[9] - a[9] * b[0] -
                 a[3] * b[5] - a[5] * b[3];
  int64_t det4 = a[0] * b[8] + a[8] * b[0] + a[3] * b[4] + a[4] * b[3] -
                 a[6] * b[1] - a[1] * b[6];
