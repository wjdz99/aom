...
static void upsample_intra_edge(uint8_t *p, int sz) {
  // interpolate half-sample positions for 4x4 block edge
...
#if CONFIG_INTRA_EDGE_UPSAMPLE
    if (upsample_above) {
      upsample_intra_edge_high(above_row, txwpx + (need_right ? txhpx : 0), xd->bd);
    }
    if (upsample_left) {
      upsample_intra_edge_high(left_col, txhpx + (need_bottom ? txwpx : 0), xd->bd);
    }
#endif
  }
#endif
...
#if CONFIG_INTRA_EDGE_UPSAMPLE
    if (upsample_above) {
      upsample_intra_edge(above_row, txwpx + (need_right ? txhpx : 0));
    }
    if (upsample_left) {
      upsample_intra_edge(left_col, txhpx + (need_bottom ? txwpx : 0));
    }
#endif
  }
#endif
