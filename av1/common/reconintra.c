...
DECLARE_ALIGNED(16, const int16_t,
                filter_intra_taps_4x2procunit[FILTER_INTRA_MODES][8][8]) = {
  {
      { -6, 10, 0, 0, 0, 12, 0, 0 },
      { -5, 2, 10, 0, 0, 9, 0, 0 },
      { -3, 1, 1, 10, 0, 7, 0, 0 },
...
void filter_intra_predictor_c(uint8_t *dst, ptrdiff_t stride, TX_SIZE tx_size,
                              const uint8_t *above, const uint8_t *left,
                              int mode) {
  int r, c;
...
static void highbd_filter_intra_predictor(uint16_t *dst, ptrdiff_t stride,
  ...
  for (r = 0; r < bh; ++r) {
    memcpy(dst, &buffer[r + 1][1], bw * sizeof(uint16_t));
    dst += stride;
  }
}
...
#if CONFIG_FILTER_INTRA
  if (use_filter_intra) {
    filter_intra_predictor(dst, dst_stride, tx_size, above_row, left_col,
                           filter_intra_mode);
    return;
  }
#endif  // CONFIG_FILTER_INTRA
