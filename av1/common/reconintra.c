...
void av1_predict_intra_block(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    build_intra_predictors_high(
        xd, ref, ref_stride, dst, dst_stride, mode, angle_delta, filter_intra_mode,
        tx_size,
#if CONFIG_INTRA_EDGE2
        disable_edge_filter,
#endif  // CONFIG_INTRA_EDGE2
  ...
  build_intra_predictors(xd, ref, ref_stride, dst, dst_stride, mode, angle_delta,
                         filter_intra_mode, tx_size,
#if CONFIG_INTRA_EDGE2
                         disable_edge_filter,
#endif  // CONFIG_INTRA_EDGE2
                         have_top ? AOMMIN(txwpx, xr + txwpx) : 0,
...
void av1_predict_intra_block_facade(const AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  const FILTER_INTRA_MODE filter_intra_mode =
      (plane == AOM_PLANE_Y && mbmi->filter_intra_mode_info.use_filter_intra) ?
          mbmi->filter_intra_mode_info.filter_intra_mode : FILTER_INTRA_MODES;
