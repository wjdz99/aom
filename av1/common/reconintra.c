...
void set_uv_mode_and_delta_angle(MB_MODE_INFO *mi) {
  if (mi->joint_uv_mode < NON_DIRECTIONAL_MODES_COUNT) {
    mi->uv_mode = mi->joint_uv_mode;
    mi->angle_delta[PLANE_TYPE_UV] = 0;
  } else if (mi->joint_uv_mode == CHROMA_MODE_COUNT - 1) {
    mi->uv_mode = UV_CFL_PRED;
    mi->angle_delta[PLANE_TYPE_UV] = 0;
  } else {
    mi->uv_mode = (mi->joint_uv_mode - NON_DIRECTIONAL_MODES_COUNT) / 7 +
                  NON_DIRECTIONAL_MODES_COUNT;
    mi->angle_delta[PLANE_TYPE_UV] =
        (mi->joint_uv_mode - NON_DIRECTIONAL_MODES_COUNT) % 7 - 3;
  }
  if (mi->uv_mode != UV_CFL_PRED)
    mi->uv_mode = reordered_nominal_mode[mi->uv_mode];
}
...
void get_uv_intra_prediction_mode_set(MB_MODE_INFO *mi) {
  const PREDICTION_MODE luma_mode = av1_get_block_joint_mode(mi);
  const int is_directional_luma_mode =
      luma_mode >= NON_DIRECTIONAL_MODES_COUNT ? 1 : 0;
  int isModeSelectedList[CHROMA_MODE_COUNT];

  int i;
  int selectedModeIdx = 0;
  for (i = 0; i < CHROMA_MODE_COUNT; i++) {
    isModeSelectedList[i] = -1;
    mi->uv_intra_mode_list[i] = -1;
  }

  // put cfl mode into the mode list
  mi->uv_intra_mode_list[selectedModeIdx++] = CHROMA_MODE_COUNT - 1;
  isModeSelectedList[CHROMA_MODE_COUNT - 1] = 1;

  if (is_directional_luma_mode) {
    mi->uv_intra_mode_list[selectedModeIdx++] = luma_mode;
    isModeSelectedList[luma_mode] = 1;
  }

  // put non-directional modes into the mode list
  for (i = 0; i < NON_DIRECTIONAL_MODES_COUNT; ++i) {
    if (isModeSelectedList[i] == -1) {
      mi->uv_intra_mode_list[selectedModeIdx++] = i;
      isModeSelectedList[i] = 1;
    }
  }

  // fill the remaining list with default modes
  for (i = 0; i < SECOND_MODE_COUNT && selectedModeIdx < UV_INTRA_MODES; i++) {
    if (isModeSelectedList[defaultModeList[i] + NON_DIRECTIONAL_MODES_COUNT] ==
        -1) {
      mi->uv_intra_mode_list[selectedModeIdx++] =
          defaultModeList[i] + NON_DIRECTIONAL_MODES_COUNT;
      isModeSelectedList[defaultModeList[i] + NON_DIRECTIONAL_MODES_COUNT] = 1;
    }
  }
