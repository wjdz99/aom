...
int av1_intra_bottom_unavailable(const MACROBLOCKD *xd, int plane) {
  ...
  TX_SIZE tx_size = max_txsize_rect_lookup[plane_bsize];
  const int txwpx = tx_size_wide[tx_size];
  const int txhpx = tx_size_high[tx_size];
  ...
  const int yd =
      (xd->mb_to_bottom_edge >> (3 + pd->subsampling_y)) + hpx - txhpx;
  return yd < 0 ? -1 * yd : 0;
}
...
int av1_intra_right_unavailable(const MACROBLOCKD *xd, int plane,
  ...
  const int xr =
      (xd->mb_to_right_edge >> (3 + pd->subsampling_x)) + wpx - txwpx;
  return xr < 0 ? -1 * xr : 0;
}
...
static void extend_intra_border_rows(const uint8_t *ref, int ref_stride,
  ...
  for (int j = 0; j < border - top_part; ++j) {
    ...
    av1_bd_memset(dst + copyable, dst[copyable - 1], right_unavailable_cols,
                  is_hbd);
    dst += dst_stride;
  }
