...
static int overwrite_ref_row(int buf_flags, int block_width, uint8_t *ref_row,
                             const uint8_t *dst_row, uint8_t *tmp_row,
                             int matching_strides) {
...
static void restore_ref_row(int buf_flags, int block_width, uint8_t *ref_row,
                            const uint8_t *tmp_row) {
  int row_bytes = block_width;
#if CONFIG_HIGHBITDEPTH
  if (buf_flags & YV12_FLAG_HIGHBITDEPTH) {
    row_bytes *= 2;
...
static int overwrite_ref_col(int buf_flags, int block_height, uint8_t *ref_row,
                             int ref_stride, const uint8_t *dst_row,
                             int dst_stride, uint8_t *tmp_row) {
...
static void restore_ref_col(int buf_flags, int block_height, uint8_t *ref_row,
                            int ref_stride, const uint8_t *tmp_row) {
#if CONFIG_HIGHBITDEPTH
  if (buf_flags & YV12_FLAG_HIGHBITDEPTH) {
  ...
  if (block_width < block_height) {
    ...
    while (next_row_idx < block_height) {
      ...
      const int needs_restore =
          overwrite_ref_row(xd->cur_buf->flags, block_width, last_ref_row,
                            last_dst_row, tmp, ref_stride == dst_stride);

      const uint8_t *next_ref_row = ref + next_row_idx * ref_stride;
      uint8_t *next_dst_row = dst + next_row_idx * ref_stride;
      ...
      if (needs_restore)
        restore_ref_row(xd->cur_buf->flags, block_width, last_ref_row, tmp);
    ...
    while (next_col_idx < block_width) {
      ...
      const int needs_restore =
          overwrite_ref_col(xd->cur_buf->flags, block_height, last_ref_col,
                            ref_stride, last_dst_col, dst_stride, tmp);
      ...
      if (needs_restore)
        restore_ref_col(xd->cur_buf->flags, block_height, last_ref_col,
                        ref_stride, tmp);
