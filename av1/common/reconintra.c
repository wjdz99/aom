...
static void highbd_build_non_directional_intra_predictors(
  ...
  assert(n_top_px >= 0);
  assert(n_left_px >= 0);
  assert(mode == DC_PRED || mode == SMOOTH_PRED || mode == SMOOTH_H_PRED ||
         mode == SMOOTH_V_PRED || mode == PAETH_PRED);
...
void av1_predict_intra_block(const MACROBLOCKD *xd, BLOCK_SIZE sb_size,
  ...
  const int top_px = have_top ? AOMMIN(txwpx, xr + txwpx) : 0;
  const int left_px = have_left ? AOMMIN(txhpx, yd + txhpx) : 0;
  if (!use_filter_intra && !is_dr_mode) {
#if CONFIG_AV1_HIGHBITDEPTH
    if (is_hbd) {
      highbd_build_non_directional_intra_predictors(ref, ref_stride, dst,
                                                    dst_stride, mode, tx_size,
                                                    top_px, left_px, xd->bd);
      return;
    }
#endif  // CONFIG_AV1_HIGHBITDEPTH
    build_non_directional_intra_predictors(ref, ref_stride, dst, dst_stride,
                                           mode, tx_size, top_px, left_px);
    return;
  }
  ...
  if (is_hbd) {
    highbd_build_directional_and_filter_intra_predictors(
        ...
        tx_size, disable_edge_filter, top_px, top_right_px, left_px,
        bottom_left_px, intra_edge_filter_type, xd->bd);
    return;
  }
#endif
  build_directional_and_filter_intra_predictors(
      ref, ref_stride, dst, dst_stride, mode, p_angle, filter_intra_mode,
      tx_size, disable_edge_filter, top_px, top_right_px, left_px,
      bottom_left_px, intra_edge_filter_type);
}
