...
static void build_intra_predictors_high(
  ...
  if (is_dr_mode) {
    ...
    if (apply_sub_block_based_refinement_filter) {
      av1_apply_sub_block_based_intra_boundary_filter_both_direction_4x4subblock_hbd(
          dst, dst_stride, tx_size, above_row, left_col, mode, xd->bd);
    }
#endif
...
static void build_intra_predictors(const MACROBLOCKD *xd, const uint8_t *ref,
  ...
  if (is_dr_mode) {
    ...
    if (apply_sub_block_based_refinement_filter) {
      av1_apply_sub_block_based_intra_boundary_filter_both_direction_4x4subblock(
          dst, dst_stride, tx_size, above_row, left_col, mode);
    }
#endif
    return;
  }
...
void av1_apply_sub_block_based_intra_boundary_filter_both_direction_4x4subblock(
    uint8_t *dst, ptrdiff_t stride, TX_SIZE tx_size, const uint8_t *above,
    const uint8_t *left, PREDICTION_MODE mode) {
...
void av1_apply_sub_block_based_intra_boundary_filter_both_direction_4x4subblock_hbd(
    uint16_t *dst, ptrdiff_t stride, TX_SIZE tx_size, const uint16_t *above,
    const uint16_t *left, PREDICTION_MODE mode, int bd) {
  const int bw = tx_size_wide[tx_size];
  const int bh = tx_size_high[tx_size];
