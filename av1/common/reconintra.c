...
static int get_filt_type(const MACROBLOCKD *xd) {
  ...
  ab = xd->up_available ? &xd->mi[-xd->mi_stride]->mbmi : 0;
  le = xd->left_available ? &xd->mi[-1]->mbmi : 0;

  int ab_sm = ab ? is_smooth(ab) : 0;
  int le_sm = le ? is_smooth(le) : 0;
