...
#define INIT_ALL_SIZES(p, type)           \
  ...
  INIT_ALL_SIZES(ibp_dc_pred[0][1], ibp_top);
  INIT_ALL_SIZES(ibp_dc_pred[1][0], ibp_left);
  ...
  INIT_ALL_SIZES(ibp_dc_pred_high[0][1], highbd_ibp_top);
  INIT_ALL_SIZES(ibp_dc_pred_high[1][0], highbd_ibp_left);
...
static void second_dr_predictor(uint8_t *dst, ptrdiff_t stride, TX_SIZE tx_size,
  ...
  if (angle > 0 && angle < 90) {
      ...
      int dx = 1;
      av1_dr_prediction_z3(dst, stride, bw, bh, above,
                           left, upsample_left, dx, dy);
  } else if (angle > 180 && angle < 270) {
      int dx =  second_dr_intra_derivative[270-angle];
      int dy = 1;
      av1_dr_prediction_z1(dst, stride, bw, bh, above,
                           left, upsample_above, dx, dy);
  }
}
#endif
// Directional prediction, zone 1: 0 < angle < 90
...
static void highbd_second_dr_predictor(uint16_t *dst, ptrdiff_t stride,
  ...
  if (angle > 0 && angle < 90) {
    int dy = second_dr_intra_derivative[angle];
    int dx = 1;
    av1_highbd_dr_prediction_z3(dst, stride, bw, bh,
                                above, left, upsample_left, dx, dy, bd);
  } else if (angle > 180 && angle < 270) {
    int dx =  second_dr_intra_derivative[270-angle];
    int dy = 1;
    av1_highbd_dr_prediction_z1(dst, stride, bw, bh,
                                above, left, upsample_above, dx, dy, bd);
  }
}
#endif
...
#if CONFIG_IBP
static void filter_intra_edge_corner_separate(
  uint8_t *p_above, uint8_t *p_left,
  int upsample_above, int upsample_left) {
  const int kernel[3] = { 5, 6, 5 };
  ...
  int s = (p_left[0] * kernel[0]) + (p_above[-1] * kernel[1]) +
          (p_above[0] * kernel[2]);
  s = (s + 8) >> 4;
  if (upsample_above) p_above[-1] = s;
  if (upsample_left)  p_left[-1] = s;
}
#endif
...
#if CONFIG_IBP
static void filter_intra_edge_corner_high_separate(
  uint16_t *p_above, uint16_t *p_left,
  int upsample_ab, int upsample_le) {
  const int kernel[3] = { 5, 6, 5 };

  int s = (p_left[0] * kernel[0]) + (p_above[-1] * kernel[1]) +
          (p_above[0] * kernel[2]);
  s = (s + 8) >> 4;
  if (upsample_ab) p_above[-1] = s;
  if (upsample_le) p_left[-1] = s;
}
#endif
...
void av1_highbd_ibp_dr_prediction_z1_c(
    uint16_t* weights, uint16_t *dst, ptrdiff_t stride,
    uint16_t* second_pred, ptrdiff_t second_stride,
    int bw, int bh) {
    int r, c;
    for (r = 0; r < bh; ++r) {
        for (c = 0; c < bw; ++c) {
            dst[c] = (dst[c]*weights[c]+second_pred[c]*
                      (1024-weights[c])+512)>>10;
        }
        weights += bw;
        dst += stride;
...
void av1_highbd_ibp_dr_prediction_z3_c(
    ...
    for (c = 0; c < bw; ++c) {
        ...
        for (r = 0; r < bh; ++r) {
            tmp_dst[0] = (tmp_dst[0]*weights[r]+
                          tmp_second[0]*(1024-weights[r])+
                          512)>>10;
            tmp_dst += stride;
...
void av1_ibp_dr_prediction_z1_c(
    uint16_t* weights, uint8_t *dst, ptrdiff_t stride,
    uint8_t* second_pred, ptrdiff_t second_stride,
    int bw, int bh) {
    int r, c;
    for (r = 0; r < bh; ++r) {
        for (c = 0; c < bw; ++c) {
            dst[c] = (dst[c]*weights[c]+
                      second_pred[c]*(1024-weights[c])+
                      512)>>10;
        }
        weights += bw;
        dst += stride;
...
void av1_ibp_dr_prediction_z3_c(
    uint16_t* weights, uint8_t *dst, ptrdiff_t stride,
    uint8_t* second_pred, ptrdiff_t second_stride,
    int bw, int bh) {
    int r, c;
...
static int32_t transpose_tx_size[19] = {
    TX_4X4,
    TX_8X8,
    TX_16X16,
    TX_32X32,
...
static void check_reference_activity_high(
    const MB_MODE_INFO *neighbor, uint16_t* ref,
    int ref_number, int *filt_type, int *upsample_flag,
    int plane, int thresh_type, int thresh_filter) {
    if (neighbor == NULL) {
        (*filt_type) = 0;
        (*upsample_flag) = 1;
        return;
    ...
    int max_gra = 0;
    int avg_gra = 0;
    int num = ref_number - 2;
    for (int t = 0; t < num; t++) {
        int gra = abs(ref[t]*2 - ref[t-1]- ref[t+1]);
        max_gra = (gra > max_gra) ? gra : max_gra;
        avg_gra += gra;
    }
    if (num > 0)
        avg_gra = (avg_gra + (num>>1))/num;
    (*filt_type) = (is_smooth(neighbor, plane) &&
                    (max_gra < (avg_gra*thresh_type)))
                   ? 1 : 0;
    (*upsample_flag) = ((!is_smooth(neighbor, plane)) &&
                        (max_gra >= (avg_gra*thresh_filter)))
                       ? 0 : 1;
    return;
}
static void check_reference_activity(
    const MB_MODE_INFO *neighbor, uint8_t* ref,
    int ref_number, int *filt_type,
    int *upsample_flag, int plane,
    int thresh_type, int thresh_filter) {
    if (neighbor == NULL) {
        (*filt_type) = 0;
        (*upsample_flag) = 1;
        return;
...
static void build_intra_predictors_high(
...
#if CONFIG_IBP
    , uint16_t* const ibp_weights[TX_SIZES_ALL][DIR_MODES_0_90]
    , int thresh_type, int thresh_filter
#endif
    ) {
  int i;
...
#if CONFIG_IBP
    if (is_dr_mode) need_bottom =  (p_angle < 90) || (p_angle > 180);
#else
...
#if CONFIG_IBP
    if (is_dr_mode) need_right = (p_angle < 90) || (p_angle > 180);
#else
  ...
  if (is_dr_mode) {
    ...
    if (!disable_edge_filter) {
#if CONFIG_IBP
        const int need_right = (p_angle < 90) || (p_angle > 180);
        const int need_bottom = (p_angle < 90) || (p_angle > 180);
        int filt_type_above = 0;
        int filt_type_left = 0;
#else
        ...
        if (p_angle != 90 && p_angle != 180) {
#if CONFIG_IBP
            const MB_MODE_INFO *ab = (plane == 0) ?
                                     xd->above_mbmi: xd->chroma_above_mbmi;
            check_reference_activity_high(ab, above_row,
                                          top_number, &filt_type_above,
                                          &upsample_above, plane,
                                          thresh_type, thresh_filter);
            const MB_MODE_INFO *le = (plane == 0) ?
                                     xd->left_mbmi : xd->chroma_left_mbmi;
            check_reference_activity_high(le, left_col,
                                          left_number, &filt_type_left,
                                          &upsample_left, plane,
                                          thresh_type, thresh_filter);
...
#if CONFIG_IBP
                    filter_intra_edge_corner_high_separate(above_row, left_col,
                                                           upsample_above,
                                                           upsample_left);
#else
...
#if CONFIG_IBP
                if (need_above  && n_top_px > 0 && upsample_above) {
                    int cur_angle = (p_angle > 180) ?
                                    (p_angle - 180 - 90) : (p_angle - 90);
                    const int strength = intra_edge_filter_strength(
                                                txwpx, txhpx, cur_angle,
                                                filt_type_above);
#else
...
#endif
                    const int n_px = n_top_px + ab_le +
                                     (need_right ? txhpx : 0);
                    av1_filter_intra_edge_high(above_row - ab_le,
                                               n_px, strength);
                }
#if CONFIG_IBP
                if (need_left && n_left_px > 0 && upsample_left) {
                    int cur_angle = (p_angle < 90) ?
                                    p_angle : (p_angle - 180);
                    const int strength = intra_edge_filter_strength(
                                                txhpx, txwpx, cur_angle,
                                                filt_type_left);
#else
                if (need_left && n_left_px > 0) {
                    const int strength = intra_edge_filter_strength(
                                         txhpx, txwpx, p_angle - 180,
                                         filt_type);
#endif
                    const int n_px = n_left_px + ab_le +
                                     (need_bottom ? txwpx : 0);
                    av1_filter_intra_edge_high(left_col - ab_le,
                                               n_px, strength);
                }
            }
#if CONFIG_IBP
            if (upsample_above) {
                int cur_angle = (p_angle > 180) ?
                                (p_angle - 180 - 90) : (p_angle - 90);
                upsample_above = av1_use_intra_edge_upsample(
                                    txwpx, txhpx, cur_angle,
                                    filt_type_above);
#else
                upsample_above = av1_use_intra_edge_upsample(
    ...
    if ((p_angle > 0) && (p_angle < 90)) {
        ...
        uint16_t* weights = ibp_weights[tx_size][mode_index];
        highbd_second_dr_predictor(second_pred, txwpx,
                                   tx_size, above_row, left_col,
                                   upsample_above, upsample_left,
                                   p_angle, xd->bd);
        av1_highbd_ibp_dr_prediction_z1_c(weights, dst,
                                          dst_stride,
                                          second_pred,
                                          txwpx, txwpx, txhpx);
    }
    if (p_angle > 180 && p_angle < 270) {
        ...
        uint16_t* weights = ibp_weights[transpose_tsize]
                                       [mode_index];
        highbd_second_dr_predictor(second_pred, txwpx,
                                   tx_size, above_row, left_col,
                                   upsample_above, upsample_left,
                                   p_angle, xd->bd);
        av1_highbd_ibp_dr_prediction_z3_c(weights, dst,
                                          dst_stride,
                                          second_pred,
                                          txwpx, txwpx, txhpx);
    }
  ...
  if (mode == DC_PRED) {
#if CONFIG_IBP
      if ((plane > 0) && (xd->mi[0]->uv_mode == UV_CFL_PRED)) {
#endif
...
static void build_intra_predictors(const MACROBLOCKD *xd, const uint8_t *ref,
                                   ...
                                   int plane
#if CONFIG_IBP
                                   , uint16_t* const ibp_weights[TX_SIZES_ALL]
                                                            [DIR_MODES_0_90]
                                   , int thresh_type, int thresh_filter
#endif
                                   ) {
  ...
  if (is_dr_mode) {
      ...
      if (!disable_edge_filter) {
          ...
          if (p_angle != 90 && p_angle != 180) {
              ...
              const MB_MODE_INFO *ab = (plane == 0) ?
                                       xd->above_mbmi:
                                       xd->chroma_above_mbmi;
              check_reference_activity(ab, above_row, top_number,
                                       &filt_type_above,
                                       &upsample_above, plane,
                                       thresh_type, thresh_filter);
              const MB_MODE_INFO *le = (plane == 0) ?
                                       xd->left_mbmi :
                                       xd->chroma_left_mbmi;
              check_reference_activity(le, left_col, left_number,
                                       &filt_type_left,
                                       &upsample_left, plane,
                                       thresh_type, thresh_filter);
              if (upsample_above || upsample_left) {
                  ...
                  if (need_above && need_left && (txwpx + txhpx >= 24)) {
#if CONFIG_IBP
                      filter_intra_edge_corner_separate(above_row,
                                                        left_col,
                                                        upsample_above,
                                                        upsample_left);
#else
...
#if CONFIG_IBP
              if (upsample_above) {
                  int cur_angle = (p_angle > 180) ? (p_angle - 180 - 90) :
                                                    (p_angle - 90);
                  upsample_above = av1_use_intra_edge_upsample(
                                   txwpx, txhpx, cur_angle,
                                   filt_type_above);
              ...
              if (upsample_left) {
                  int cur_angle = (p_angle < 90) ? p_angle : (p_angle - 180);
                  upsample_left = av1_use_intra_edge_upsample(
                                  txhpx, txwpx, cur_angle, filt_type_left);
#else
                  upsample_left = av1_use_intra_edge_upsample(
                                  txhpx, txwpx, p_angle - 180, filt_type);
#endif
      ...
      if ((p_angle > 0) && (p_angle < 90)) {
         ...
         av1_ibp_dr_prediction_z1_c(weights, dst, dst_stride,
                                    second_pred, txwpx, txwpx, txhpx);
      }
      if (p_angle > 180 && p_angle < 270) {
         int mode_index = angle_to_mode_index[270 - p_angle];
         int transpose_tsize = transpose_tx_size[tx_size];
         uint16_t* weights = ibp_weights[transpose_tsize][mode_index];
         second_dr_predictor(second_pred, txwpx, tx_size, above_row, left_col,
                             upsample_above, upsample_left, p_angle);
         av1_ibp_dr_prediction_z3_c(weights, dst, dst_stride,
                                    second_pred, txwpx, txwpx, txhpx);
      }
#endif
      return;
  ...
  if (mode == DC_PRED) {
      ...
      if ((plane > 0) && (xd->mi[0]->uv_mode == UV_CFL_PRED)) {
#endif
        dc_pred[n_left_px > 0][n_top_px > 0][tx_size](
                dst, dst_stride, above_row, left_col);
#if CONFIG_IBP
      } else {
        ibp_dc_pred[n_left_px > 0][n_top_px > 0][tx_size](
                dst, dst_stride, above_row, left_col);
      }
#endif
  } else {
