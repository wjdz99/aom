...
static int overwrite_ref_row(int matching_strides, int buf_flags, int block_width, const uint8_t *dst_row, uint8_t *ref_row, uint8_t *tmp_row) {
...
static void restore_ref_row(int buf_flags, int block_width, const uint8_t *tmp_row, uint8_t *ref_row) {
  int row_bytes = block_width;
#if CONFIG_HIGHBITDEPTH
  if (buf_flags & YV12_FLAG_HIGHBITDEPTH) {
    row_bytes *= 2;
...
static int overwrite_ref_col(int buf_flags, int block_height, const uint8_t *dst_row,
                             int dst_stride, uint8_t *ref_row,
                             int ref_stride, uint8_t *tmp_row) {
...
static void restore_ref_col(int buf_flags, int block_height, const uint8_t *tmp_row, uint8_t *ref_row,
                            int ref_stride) {
#if CONFIG_HIGHBITDEPTH
  if (buf_flags & YV12_FLAG_HIGHBITDEPTH) {
  ...
  if (block_width < block_height) {
    ...
    while (next_row_idx < block_height) {
      ...
      const int needs_restore =
        overwrite_ref_row(ref_stride == dst_stride, xd->cur_buf->flags, block_width, last_dst_row, last_ref_row,
                          tmp);
    ...
    while (next_col_idx < block_width) {
      ...
      const int needs_restore =
          overwrite_ref_col(xd->cur_buf->flags, block_height, last_dst_col, dst_stride, last_ref_col,
                            ref_stride, tmp);
