...
static void copy_img(const YV12_BUFFER_CONFIG *src, YV12_BUFFER_CONFIG *dst) {
  copy_plane(src->y_buffer, src->y_stride, dst->y_buffer, dst->y_stride,
...
void av1_setup_butteraugli_source(AV1_COMP *cpi) {
  ...
  zero_img(cpi->source);
  copy_img(&resized_source, cpi->source);
...
void av1_restore_butteraugli_source(AV1_COMP *cpi) {
  ...
  set_mb_butteraugli_rdmult_scaling(cpi, &cpi->common.cur_frame->buf);
