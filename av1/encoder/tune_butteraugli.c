  ...
  if (write_file) {
    ...
    for (int row = 0; row < num_rows; ++row) {
      for (int col = 0; col < num_cols; ++col) {
        ...
        uint8_t *blk_src_buf_u = source->u_buffer +
                                 row_offset_y / 2 * source->uv_stride +
                                 col_offset_y / 2;
        uint8_t *blk_recon_buf_u = recon->u_buffer +
                                   row_offset_y / 2 * recon->uv_stride +
                                   col_offset_y / 2;
        uint8_t *blk_src_buf_v = source->v_buffer +
                                 row_offset_y / 2 * source->uv_stride +
                                 col_offset_y / 2;
        uint8_t *blk_recon_buf_v = recon->v_buffer +
                                   row_offset_y / 2 * recon->uv_stride +
                                   col_offset_y / 2;
        uint8_t *recon_dst_u = recon_block.u_buffer;
        uint8_t *src_dst_u = source_block.u_buffer;
        uint8_t *recon_dst_v = recon_block.v_buffer;
        uint8_t *src_dst_v = source_block.v_buffer;

        // Copy block from source frame.
        for (int i = 0; i < block_h / 2; ++i) {
          for (int j = 0; j < block_w / 2; ++j) {
            src_dst_u[j] = blk_src_buf_u[j];
            recon_dst_u[j] = blk_recon_buf_u[j];
            src_dst_v[j] = blk_src_buf_v[j];
            recon_dst_v[j] = blk_recon_buf_v[j];
          }
          blk_src_buf_u += source->uv_stride;
          blk_recon_buf_u += recon->uv_stride;
          src_dst_u += source_block.uv_stride;
          recon_dst_u += recon_block.uv_stride;
          blk_src_buf_v += source->uv_stride;
          blk_recon_buf_v += recon->uv_stride;
          src_dst_v += source_block.uv_stride;
          recon_dst_v += recon_block.uv_stride;
        }
