...
static AOM_INLINE void prepare_enc_workers(AV1_COMP *cpi, AVxWorkerHook hook,
  ...
  for (int i = num_workers - 1; i >= 0; i--) {
    ...
    const int num_planes = av1_num_planes(&cpi->common);
    for (int plane = 0; plane < num_planes; plane++) {
      const int sb_size = (MAX_SB_SIZE >> cm->seq_params.subsampling_x) *
                          (MAX_SB_SIZE >> cm->seq_params.subsampling_y);
      CHECK_MEM_ERROR(
          cm, thread_data->td->mb.plane[plane].src_diff,
          (int16_t *)aom_memalign(
              32, sizeof(thread_data->td->mb.plane[plane].src_diff) * sb_size));
    }
