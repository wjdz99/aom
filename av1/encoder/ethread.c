...
static AOM_INLINE void prepare_fpmt_workers(AV1_PRIMARY *ppi,
  ...
  struct aom_internal_error_info *backup_cm_error;
  ...
  while (i < num_workers) {
    ...
    backup_cm_error = cm->error;
    // At this stage, cm->error.setjmp = 0 for all the frame contexts
    // (ppi->parallel_cpi[frame_idx]). Hence, to facilitate the error reporting
    // of all the frame contexts, point the corresponding cm->error to a local
    // aom_internal_error_info which has a local setjmp target. Restore the
    // original cm->error before returning from the function.
    cm->error = &error;
    if (setjmp(error.jmp)) {
      error.setjmp = 0;
      cm->error = backup_cm_error;
