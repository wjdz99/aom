...
void av1_row_mt_sync_read_dummy(AV1EncRowMTSync *const enc_row_mt_sync, int r,
                                int c) {
  (void)enc_row_mt_sync;
  (void)r;
  (void)c;
  return;
}

void av1_row_mt_sync_write_dummy(AV1EncRowMTSync *const enc_row_mt_sync, int r,
...
void av1_row_mt_sync_read(AV1EncRowMTSync *const enc_row_mt_sync, int r,
                          int c) {
#if CONFIG_MULTITHREAD
  const int nsync = enc_row_mt_sync->sync_range;

  if (r) {
    pthread_mutex_t *const mutex = &enc_row_mt_sync->mutex_[r - 1];
    pthread_mutex_lock(mutex);

    while (c > enc_row_mt_sync->cur_col[r - 1] - nsync) {
      pthread_cond_wait(&enc_row_mt_sync->cond_[r - 1], mutex);
    }
...
void av1_row_mt_sync_write(AV1EncRowMTSync *const enc_row_mt_sync, int r, int c,
                           const int cols) {
#if CONFIG_MULTITHREAD
  const int nsync = enc_row_mt_sync->sync_range;
  int cur;
  ...
  if (sig) {
    pthread_mutex_lock(&enc_row_mt_sync->mutex_[r]);

    enc_row_mt_sync->cur_col[r] = cur;

    pthread_cond_signal(&enc_row_mt_sync->cond_[r]);
    pthread_mutex_unlock(&enc_row_mt_sync->mutex_[r]);
  }
#else
  (void)enc_row_mt_sync;
  (void)r;
...
void av1_row_mt_sync_mem_alloc(AV1EncRowMTSync *const enc_row_mt_sync,
                               AV1_COMMON *const cm, int rows) {
#if CONFIG_MULTITHREAD
  int i;

  CHECK_MEM_ERROR(cm, enc_row_mt_sync->mutex_,
                  aom_malloc(sizeof(*enc_row_mt_sync->mutex_) * rows));
  if (enc_row_mt_sync->mutex_) {
    for (i = 0; i < rows; ++i) {
      pthread_mutex_init(&enc_row_mt_sync->mutex_[i], NULL);
    }
  }

  CHECK_MEM_ERROR(cm, enc_row_mt_sync->cond_,
                  aom_malloc(sizeof(*enc_row_mt_sync->cond_) * rows));
  if (enc_row_mt_sync->cond_) {
    for (i = 0; i < rows; ++i) {
      pthread_cond_init(&enc_row_mt_sync->cond_[i], NULL);
    }
  }
#endif  // CONFIG_MULTITHREAD

  CHECK_MEM_ERROR(cm, enc_row_mt_sync->cur_col,
                  aom_malloc(sizeof(*enc_row_mt_sync->cur_col) * rows));

  enc_row_mt_sync->rows = rows;
  // Set up nsync.
  enc_row_mt_sync->sync_range = 1;
}

// Deallocate row based multi-threading synchronization related mutex and data
void av1_row_mt_sync_mem_dealloc(AV1EncRowMTSync *const enc_row_mt_sync) {
  if (enc_row_mt_sync != NULL) {
#if CONFIG_MULTITHREAD
    int i;

    if (enc_row_mt_sync->mutex_ != NULL) {
      for (i = 0; i < enc_row_mt_sync->rows; ++i) {
        pthread_mutex_destroy(&enc_row_mt_sync->mutex_[i]);
      }
      aom_free(enc_row_mt_sync->mutex_);
    }
    if (enc_row_mt_sync->cond_ != NULL) {
      for (i = 0; i < enc_row_mt_sync->rows; ++i) {
        pthread_cond_destroy(&enc_row_mt_sync->cond_[i]);
      }
      aom_free(enc_row_mt_sync->cond_);
    }
#endif  // CONFIG_MULTITHREAD
    aom_free(enc_row_mt_sync->cur_col);

    // clear the structure as the source of this call may be dynamic change
    // in tiles in which case this call will be followed by an _alloc()
    // which may fail.
    av1_zero(*enc_row_mt_sync);
  }
}
...
static int get_next_job(AV1_COMP *const cpi, int *current_mi_row,
                        int cur_tile_id) {
  AV1EncRowMTSync *const enc_row_mt_sync =
      &cpi->tile_data[cur_tile_id].enc_row_mt_sync;
  const int mi_row_end = cpi->tile_data[cur_tile_id].tile_info.mi_row_end;
  const int mib_size = cpi->common.seq_params.mib_size;

  if (enc_row_mt_sync->current_mi_row < mi_row_end) {
    *current_mi_row = enc_row_mt_sync->current_mi_row;
    enc_row_mt_sync->num_threads_working++;
    enc_row_mt_sync->current_mi_row += mib_size;
    return 1;
  }
  return 0;
}
...
static AOM_INLINE void switch_tile_and_get_next_job(AV1_COMP *const cpi,
  ...
  AV1EncRowMTInfo *const frame_row_mt_info = &cpi->frame_row_mt_info;
  ...
  for (int tile_row = 0; tile_row < tile_rows; tile_row++) {
    for (int tile_col = 0; tile_col < tile_cols; tile_col++) {
      ...
      AV1EncRowMTSync *const enc_row_mt_sync = &this_tile->enc_row_mt_sync;
      ...
      int num_threads_working = enc_row_mt_sync->num_threads_working;

      if (num_threads_working < theoretical_limit_on_threads) {
        int num_mis_to_encode =
            this_tile->tile_info.mi_row_end - enc_row_mt_sync->current_mi_row;
  ...
  if (tile_id == -1) {
    *end_of_frame = 1;
    ...
    // Update the current tile id to the next tile id that will be processed,
    // which will be the least processed tile.
    *cur_tile_id = tile_id;
    get_next_job(cpi, current_mi_row, tile_id);
  }
}
...
static int enc_row_mt_worker_hook(void *arg1, void *unused) {
  ...
  int cur_tile_id = frame_row_mt_info->thread_id_to_tile_id[thread_id];
  (void)unused;
...
#if CONFIG_MULTITHREAD
    pthread_mutex_lock(frame_row_mt_info->mutex_);
#endif
...
#if CONFIG_MULTITHREAD
    pthread_mutex_unlock(frame_row_mt_info->mutex_);
#endif
...
static AOM_INLINE void create_enc_workers(AV1_COMP *cpi, int num_workers) {
  ...
  const int sb_mi_size = av1_get_sb_mi_size(cm);
#if CONFIG_MULTITHREAD
  AV1EncRowMTInfo *const enc_row_mt = &cpi->frame_row_mt_info;
#endif
...
void av1_encode_tiles_row_mt(AV1_COMP *cpi) {
  ...
  const int tile_cols = cm->tiles.cols;
  const int tile_rows = cm->tiles.rows;
  int *thread_id_to_tile_id = frame_row_mt_info->thread_id_to_tile_id;
  uint8_t cdf_update_mode = cpi->oxcf.cdf_update_mode;
  int num_workers = 0;
  ...
  for (int row = 0; row < tile_rows; row++) {
    for (int col = 0; col < tile_cols; col++) {
      TileInfo tile_info = cpi->tile_data[row * cm->tiles.cols + col].tile_info;
  ...
  if (frame_row_mt_info->allocated_tile_cols != tile_cols ||
      frame_row_mt_info->allocated_tile_rows != tile_rows ||
      frame_row_mt_info->allocated_sb_rows != max_sb_rows) {
    av1_row_mt_mem_dealloc(cpi);
  ...
  for (int tile_row = 0; tile_row < tile_rows; tile_row++) {
    for (int tile_col = 0; tile_col < tile_cols; tile_col++) {
      ...
      memset(enc_row_mt_sync->cur_col, -1,
             sizeof(*enc_row_mt_sync->cur_col) * max_sb_rows);
      enc_row_mt_sync->current_mi_row = this_tile->tile_info.mi_row_start;
      enc_row_mt_sync->num_threads_working = 0;
