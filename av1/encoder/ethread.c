...
void av1_row_mt_sync_read_dummy(AV1EncRowMTSync *const row_mt_sync, int r,
                                int c) {
  (void)row_mt_sync;
  (void)r;
  (void)c;
...
void av1_row_mt_sync_write_dummy(AV1EncRowMTSync *const row_mt_sync, int r,
...
void av1_row_mt_sync_read(AV1EncRowMTSync *const row_mt_sync, int r, int c) {
...
void av1_row_mt_sync_write(AV1EncRowMTSync *const row_mt_sync, int r, int c,
                           const int cols) {
#if CONFIG_MULTITHREAD
  const int nsync = row_mt_sync->sync_range;
  int cur;
...
void av1_row_mt_sync_mem_alloc(AV1EncRowMTSync *const row_mt_sync,
...
void av1_row_mt_sync_mem_dealloc(AV1EncRowMTSync *const row_mt_sync) {
  if (row_mt_sync != NULL) {
#if CONFIG_MULTITHREAD
    int i;
...
static int get_next_job(AV1_COMP *const cpi, int *current_mi_row,
                        int cur_tile_id) {
  AV1EncRowMTSync *const row_mt_sync = &cpi->tile_data[cur_tile_id].row_mt_sync;
  const int mi_row_end = cpi->tile_data[cur_tile_id].tile_info.mi_row_end;
  const int mib_size = cpi->common.seq_params.mib_size;

  if (row_mt_sync->current_mi_row < mi_row_end) {
    *current_mi_row = row_mt_sync->current_mi_row;
    row_mt_sync->num_threads_working++;
    row_mt_sync->current_mi_row += mib_size;
    return 1;
  }
  return 0;
}
...
static AOM_INLINE void switch_tile_and_get_next_job(AV1_COMP *const cpi,
  ...
  for (int tile_row = 0; tile_row < tile_rows; tile_row++) {
    for (int tile_col = 0; tile_col < tile_cols; tile_col++) {
      ...
      AV1EncRowMTSync *const row_mt_sync = &this_tile->row_mt_sync;
      ...
      if (num_threads_working < theoretical_limit_on_threads) {
        int num_mis_to_encode =
            this_tile->tile_info.mi_row_end - row_mt_sync->current_mi_row;
...
void av1_encode_tiles_row_mt(AV1_COMP *cpi) {
  ...
  for (int tile_row = 0; tile_row < tile_rows; tile_row++) {
    for (int tile_col = 0; tile_col < tile_cols; tile_col++) {
      ...
      memset(row_mt_sync->cur_col, -1,
             sizeof(*row_mt_sync->cur_col) * max_sb_rows);
      row_mt_sync->current_mi_row = this_tile->tile_info.mi_row_start;
