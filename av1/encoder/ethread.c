...
void av1_cdef_mse_calc_frame_mt(MultiThreadInfo *mt_info,
                                CdefSearchCtx *cdef_search_ctx) {
  AV1CdefSync *cdef_sync = &mt_info->cdef_sync;
  const int num_workers = mt_info->num_workers;
  const AVxWorkerInterface *const winterface = aom_get_worker_interface();
  AVxWorker *workers = mt_info->workers;

  cdef_reset_job_info(cdef_sync);
  // Set up cdef filter thread data.
  for (int i = 0; i < num_workers; ++i) {
    AVxWorker *const worker = &workers[i];
    worker->hook = cdef_filter_block_worker_hook;
    worker->data1 = cdef_sync;
    worker->data2 = cdef_search_ctx;
    // Start cdef filtering
    if (i == num_workers - 1) {
      winterface->execute(worker);
    } else {
      winterface->launch(worker);
  ...
  // Wait till all blocks are finished
  for (int i = 0; i < num_workers; ++i) {
