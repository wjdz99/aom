...
static AOM_INLINE int compute_num_enc_row_mt_workers(AV1_COMP *cpi) {
  AV1_COMMON *const cm = &cpi->common;
  TileInfo tile_info;
  const int tile_cols = cm->tiles.cols;
  const int tile_rows = cm->tiles.rows;
...
static AOM_INLINE int compute_num_enc_tile_mt_workers(AV1_COMP *cpi) {
  AV1_COMMON *const cm = &cpi->common;
  const int tile_cols = cm->tiles.cols;
  const int tile_rows = cm->tiles.rows;
  return AOMMIN(cpi->oxcf.max_threads, tile_cols * tile_rows);
}
...
static AOM_INLINE int compute_num_enc_workers(AV1_COMP *cpi) {
  ...
  if (cpi->oxcf.row_mt && (cpi->oxcf.max_threads > 1))
    return compute_num_enc_row_mt_workers(cpi);
  else
    return compute_num_enc_tile_mt_workers(cpi);
}
...
void av1_encode_tiles_row_mt(AV1_COMP *cpi) {
  ...
  int max_sb_rows = compute_max_sb_rows(cpi);
