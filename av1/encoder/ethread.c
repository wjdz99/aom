...
// stage
static AOM_INLINE void compute_max_sb_rows(AV1_COMP *cpi, int *max_sb_rows,
                                           int *max_sb_cols) {
  AV1_COMMON *const cm = &cpi->common;
  const int tile_cols = cm->tiles.cols;
  const int tile_rows = cm->tiles.rows;
...
void av1_encode_tiles_row_mt(AV1_COMP *cpi) {
  ...
  compute_max_sb_rows(cpi, &max_sb_rows, &max_sb_cols);
...
void av1_fp_encode_tiles_row_mt(AV1_COMP *cpi) {
  ...
  if (enc_row_mt->allocated_tile_cols != tile_cols ||
    ...
    av1_row_mt_mem_dealloc(cpi);
    row_mt_mem_alloc(cpi, max_mb_rows, 0, 0);
  }
