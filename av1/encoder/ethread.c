...
static AOM_INLINE void restore_workers_after_fpmt(AV1_PRIMARY *ppi,
  ...
  PrimaryMultiThreadInfo *const p_mt_info = &ppi->p_mt_info;
  int num_workers = p_mt_info->number_of_primary_workers_prepared;
...
prepare_fpmt_workers(AV1_PRIMARY *ppi, AV1_COMP_DATA *first_cpi_data,
  ...
  while (i < num_workers) {
    ...
    i += mt_info->num_workers;
    ppi->p_mt_info.number_of_primary_workers_prepared = i;
...
int av1_compress_parallel_frames(AV1_PRIMARY *const ppi,
  ...
  if (prepare_fpmt_workers(ppi, first_cpi_data, get_compressed_data_hook,
                           frames_in_parallel_set)) {
    aom_internal_error(&ppi->error, AOM_CODEC_MEM_ERROR,
                       "Allocation failed in prepare_fpmt_workers");
  }
