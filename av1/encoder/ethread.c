...
static AOM_INLINE int get_next_job_allintra(
    AV1EncAllIntraRowMultiThreadSync *const row_mt_sync, const int mi_row_end,
    int *current_mi_row, int mib_size) {
  if (row_mt_sync->next_mi_row < mi_row_end) {
    *current_mi_row = row_mt_sync->next_mi_row;
    row_mt_sync->num_threads_working++;
...
static void wiener_var_sync_mem_alloc(
    ...
    AV1_COMMON *const cm, const int rows) {
#if CONFIG_MULTITHREAD
  int i;

  CHECK_MEM_ERROR(cm, intra_row_mt_sync->mutex_,
                  aom_malloc(sizeof(*intra_row_mt_sync->mutex_) * rows));
  if (intra_row_mt_sync->mutex_) {
    for (i = 0; i < rows; ++i) {
      pthread_mutex_init(&intra_row_mt_sync->mutex_[i], NULL);
    }
  }

  CHECK_MEM_ERROR(cm, intra_row_mt_sync->cond_,
                  aom_malloc(sizeof(*intra_row_mt_sync->cond_) * rows));
  if (intra_row_mt_sync->cond_) {
    for (i = 0; i < rows; ++i) {
      pthread_cond_init(&intra_row_mt_sync->cond_[i], NULL);
    }
  }
#endif  // CONFIG_MULTITHREAD

  CHECK_MEM_ERROR(
      cm, intra_row_mt_sync->num_finished_cols,
      aom_malloc(sizeof(*intra_row_mt_sync->num_finished_cols) * rows));

  intra_row_mt_sync->rows = rows;
  // Set up nsync.
  intra_row_mt_sync->sync_range = 1;
}

// Deallocate row based multi-threading synchronization related mutex and data
static void wiener_var_sync_mem_dealloc(
    AV1EncAllIntraRowMultiThreadSync *intra_row_mt_sync) {
  if (intra_row_mt_sync != NULL) {
#if CONFIG_MULTITHREAD
    int i;

    if (intra_row_mt_sync->mutex_ != NULL) {
      for (i = 0; i < intra_row_mt_sync->rows; ++i) {
        pthread_mutex_destroy(&intra_row_mt_sync->mutex_[i]);
      }
      aom_free(intra_row_mt_sync->mutex_);
    }
    if (intra_row_mt_sync->cond_ != NULL) {
      for (i = 0; i < intra_row_mt_sync->rows; ++i) {
        pthread_cond_destroy(&intra_row_mt_sync->cond_[i]);
      }
      aom_free(intra_row_mt_sync->cond_);
    }
#endif  // CONFIG_MULTITHREAD
    aom_free(intra_row_mt_sync->num_finished_cols);

    // clear the structure as the source of this call may be dynamic change
    // in tiles in which case this call will be followed by an _alloc()
    // which may fail.
    av1_zero(*intra_row_mt_sync);
  }
}

static AOM_INLINE void prepare_wiener_var_workers(
    AV1_COMP *const cpi, AVxWorkerHook hook,
    AV1EncAllIntraRowMultiThreadSync *intra_row_mt_sync,
  ...
  for (int i = num_workers - 1; i >= 0; i--) {
    ...
    worker->hook = hook;
    worker->data1 = thread_data;
    worker->data2 = intra_row_mt_sync;
...
static int cal_mb_wiener_var_hook(void *arg1, void *arg2) {
  EncWorkerData *const thread_data = (EncWorkerData *)arg1;
  AV1EncAllIntraRowMultiThreadSync *const intra_row_mt_sync =
      (AV1EncAllIntraRowMultiThreadSync *)arg2;
  AV1_COMP *const cpi = thread_data->cpi;
  MACROBLOCK *x = &thread_data->td->mb;
  MACROBLOCKD *xd = &x->e_mbd;
  ...
  while (has_jobs) {
    ...
    av1_calc_mb_wiener_var_row(cpi, intra_row_mt_sync, x, xd, current_mi_row,
                               src_diff, coeff, qcoeff, dqcoeff,
...
void av1_calc_mb_wiener_var_mt(AV1_COMP *cpi, int num_workers,
  ...
  AV1_COMMON *const cm = &cpi->common;
  MultiThreadInfo *const mt_info = &cpi->mt_info;
  // On Linux x86-64, intra_row_mt_sync is only 48 bytes, so we can allocate
  // it as a local variable in this function.
  AV1EncAllIntraRowMultiThreadSync intra_row_mt_sync;

  // TODO(chengchen): the memory usage could be improved.
  const int mi_rows = cm->mi_params.mi_rows;
  wiener_var_sync_mem_alloc(&intra_row_mt_sync, cm, mi_rows);
  ...
  prepare_wiener_var_workers(cpi, cal_mb_wiener_var_hook, &intra_row_mt_sync,
                             num_workers);
  launch_workers(mt_info, num_workers);
  sync_enc_workers(mt_info, cm, num_workers);

  wiener_var_sync_mem_dealloc(&intra_row_mt_sync);
}
