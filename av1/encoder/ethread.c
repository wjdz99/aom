...
static void init_tile_pack_bs_params(AV1_COMP *const cpi, uint8_t *const dst,
                                     ...
                                     PackBSParams *pack_bs_params_arr,
  ...
  for (tile_idx = 0; tile_idx < num_tiles; tile_idx++) {
    TileInfo const *tile_info = &cpi->tile_data[tile_idx].tile_info;
    ...
    tile_count_in_tg++;
    if (tile_count_in_tg == tg_size || tile_idx == (num_tiles - 1))
  ...
  size_t max_buf_size = cpi->available_bs_size / 2 - 1;
...
static void prepare_pack_bs_workers(AV1_COMP *const cpi,
  ...
  for (int i = num_workers - 1; i >= 0; i--) {
    ...
    thread_data->start = i;
    thread_data->thread_id = i;
    // TODO(Cherma): Replace error_info accessed from MACROBLOCK structure with
    // AV1_COMMON
    thread_data->td->mb.error_info = &cpi->common.error;
  ...
  // Sort tiles in descending order based on tile area. Sorting is not required
  // if number of tile is less than or same as number of workers.
  if (num_tiles > num_workers)
...
static void accumulate_pack_bs_data(
    AV1_COMP *const cpi, PackBSParams *pack_bs_params_arr, uint8_t *const dst,
    uint32_t *total_size, const FrameHeaderInfo *fh_info,
