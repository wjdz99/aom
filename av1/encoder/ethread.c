...
static AOM_INLINE void prepare_wiener_var_workers(AV1_COMP *const cpi,
  ...
  for (int i = num_workers - 1; i >= 0; i--) {
    ...
    if (thread_data->td != &cpi->td) {
      thread_data->td->mb = cpi->td.mb;
      av1_alloc_mb_wiener_var_pred_buf(cpi, thread_data->td);
    }
  }
}
...
static void set_mb_wiener_var_calc_done(AV1_COMP *const cpi) {
  ...
  // indefinitely.
  for (int mi_row = 0, mt_thread_id = 0; mi_row < cpi->frame_info.mi_rows;
...
static int cal_mb_wiener_var_hook(void *arg1, void *unused) {
  ...
  while (has_jobs) {
    ...
    // When mb_wiener_mt_exit is set to true, other workers need not pursue any
    // further jobs.
    if (mb_wiener_mt_exit) return 1;
    if (!has_jobs) break;
...
static void dealloc_mb_wiener_data(AV1_COMP *cpi, int num_workers) {
...
void av1_calc_mb_wiener_var_mt(AV1_COMP *cpi, int num_workers,
  ...
  launch_workers(mt_info, num_workers);
  sync_enc_workers(mt_info, cm, num_workers);
  dealloc_mb_wiener_data(cpi, num_workers);
}
