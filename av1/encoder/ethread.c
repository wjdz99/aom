...
void av1_encode_tiles_mt(AV1_COMP *cpi) {
  ...
  if (mt_info->enc_mt_buf_init_done == 0) {
    ...
    // TODO(nithya): Check if this min check is needed anymore.
...
void av1_encode_tiles_row_mt(AV1_COMP *cpi) {
  ...
  if (mt_info->enc_mt_buf_init_done == 0) {
    ...
    // TODO(nithya): Check if the min is needed anymore.
    num_workers = AOMMIN(num_workers, mt_info->num_workers);
  }
...
void av1_mc_flow_dispenser_mt(AV1_COMP *cpi) {
  ...
  // TODO(nithya): Check if the min is needed now.
  int num_workers =
...
//TODO(nithya): Increase the no. of workers based on tf mb rows for speed-up.
...
static AOM_INLINE int compute_gm_workers(const AV1_COMP *cpi) {
  ...
  assert(num_gm_workers <= max_num_workers);
