...
void av1_encode_tiles_row_mt(AV1_COMP *cpi) {
  ...
  // TODO(ravi.chaudhary@ittiam.com):Currently the percentage of post-processing
  // stage in encoder is quiet low, so limiting the number of threads to the
  // theoretical limit in row-mt does not have much impact on post-processing
  // multi-threading stage. Need to revisit this when post-processing time
  // starts shooting up.
