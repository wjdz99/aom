...
static AOM_INLINE int get_sb_rows(AV1_COMMON *cm) {
...
static void row_mt_mem_alloc(AV1_COMP *cpi, int max_rows, int max_cols,
  ...
  const int sb_rows = get_sb_rows(cm);
  CHECK_MEM_ERROR(
...
static void loop_filter_mt_opt_init(AV1_COMP *cpi) {
  AV1_COMMON *cm = &cpi->common;
  ...
  if (!use_loopfilter || disable_lf(cpi) || (method == LPF_PICK_MINIMAL_LPF)) {
    lf->filter_level[PLANE_TYPE_Y] = 0;
    lf->filter_level[PLANE_TYPE_UV] = 0;
    return;
  }
  ...
  if ((lf->filter_level[0] || lf->filter_level[1]) &&
    ...
    loop_filter_frame_mt_init(cm, start_mi_row, end_mi_row, planes_to_lf,
                              cpi->mt_info.num_mod_workers[MOD_ENC],
...
void av1_encode_tiles_row_mt(AV1_COMP *cpi) {
  ...
  if (cpi->sf.rt_sf.lpf_mt_opt) loop_filter_mt_opt_init(cpi);
