...
void av1_row_mt_sync_write(AV1EncRowMultiThreadSync *row_mt_sync, int r, int c,
  ...
  if (sig) {
    ...
    // When a thread encounters an error, num_finished_cols[row] is set to max
    // cols. In this case, the maximum operation here ensures that
    // num_finished_cols[row] is not overwritten with a smaller value thus
...
static int cal_mb_wiener_var_hook(void *arg1, void *unused) {
  ...
  while (has_jobs) {
    ...
    pthread_mutex_lock(enc_row_mt_mutex);
#endif
    has_jobs = enc_row_mt->mb_wiener_mt_exit
                   ? 0
