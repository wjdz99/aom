...
static void set_firstpass_encoding_done(AV1_COMP *cpi) {
  AV1_COMMON *const cm = &cpi->common;
  AV1EncRowMultiThreadInfo *const enc_row_mt = &cpi->mt_info.enc_row_mt;
  const int tile_cols = cm->tiles.cols;
  const int tile_rows = cm->tiles.rows;
  const int fp_block_size = cpi->fp_block_size;
  const int unit_height = mi_size_high[fp_block_size];
  ...
  for (int tile_row = 0; tile_row < tile_rows; ++tile_row) {
    for (int tile_col = 0; tile_col < tile_cols; ++tile_col) {
      ...
      for (int mi_row = tile->mi_row_start; mi_row < tile->mi_row_end;
           mi_row += unit_height) {
        int unit_row = mi_row >> unit_height_log2;
        int unit_row_in_tile =
            unit_row - (tile->mi_row_start >> unit_height_log2);
...
static int fp_enc_row_mt_worker_hook(void *arg1, void *unused) {
...
#endif
    set_firstpass_encoding_done(cpi);
    return 0;
  }
  error_info->setjmp = 1;
  ...
  const int unit_height = mi_size_high[fp_block_size];
  int end_of_frame = 0;
  bool firstpass_mt_exit = false;
  while (1) {
    int current_mi_row = -1;
#if CONFIG_MULTITHREAD
    pthread_mutex_lock(enc_row_mt_mutex_);
#endif
    firstpass_mt_exit = enc_row_mt->firstpass_mt_exit;
...
#if !CONFIG_REALTIME_ONLY
static AOM_INLINE void dealloc_thread_data_src_diff_buf(AV1_COMP *cpi,
                                                        int num_workers) {
  for (int i = num_workers - 1; i >= 0; i--) {
