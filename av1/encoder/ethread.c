...
int av1_compute_num_enc_workers(AV1_COMP *cpi, int max_workers) {
  ...
  if (cpi->oxcf.row_mt && (cpi->oxcf.max_threads > 1))
    num_workers =
        compute_num_enc_row_mt_workers(&cpi->common, cpi->oxcf.max_threads);
  else
    num_workers =
        compute_num_enc_tile_mt_workers(&cpi->common, cpi->oxcf.max_threads);

  return AOMMIN(num_workers, max_workers);
}
