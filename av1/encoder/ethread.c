...
void av1_row_mt_sync_read_dummy(SyncData *const sync, int tile_index, int r,
                                int c) {
  (void)sync;
  (void)tile_index;
  (void)r;
...
void av1_row_mt_sync_write_dummy(SyncData *const sync, int tile_index, int r,
...
void av1_row_mt_sync_read(SyncData *const sync, int tile_index, int r, int c) {
#if CONFIG_MULTITHREAD
  SyncData *const tile_row_mt_sync = &sync[tile_index];
...
void av1_row_mt_sync_write(SyncData *const sync, int tile_index, int r, int c,
                           const int cols) {
#if CONFIG_MULTITHREAD
  SyncData *const tile_row_mt_sync = &sync[tile_index];
  const int nsync = tile_row_mt_sync->sync_range;
...
void av1_row_mt_sync_mem_alloc(SyncData *const sync, AV1_COMMON *const cm,
                               int rows) {
#if CONFIG_MULTITHREAD
  int i;

  CHECK_MEM_ERROR(cm, sync->mutex_, aom_malloc(sizeof(*sync->mutex_) * rows));
  if (sync->mutex_) {
    for (i = 0; i < rows; ++i) {
      pthread_mutex_init(&sync->mutex_[i], NULL);
    }
  }

  CHECK_MEM_ERROR(cm, sync->cond_, aom_malloc(sizeof(*sync->cond_) * rows));
  if (sync->cond_) {
    for (i = 0; i < rows; ++i) {
      pthread_cond_init(&sync->cond_[i], NULL);
    }
  }
#endif  // CONFIG_MULTITHREAD

  CHECK_MEM_ERROR(cm, sync->cur_col, aom_malloc(sizeof(*sync->cur_col) * rows));
...
void av1_row_mt_sync_mem_dealloc(SyncData *const sync) {
  if (sync != NULL) {
#if CONFIG_MULTITHREAD
    int i;

    if (sync->mutex_ != NULL) {
      for (i = 0; i < sync->rows; ++i) {
        pthread_mutex_destroy(&sync->mutex_[i]);
      }
      aom_free(sync->mutex_);
    }
    if (sync->cond_ != NULL) {
      for (i = 0; i < sync->rows; ++i) {
        pthread_cond_destroy(&sync->cond_[i]);
      }
      aom_free(sync->cond_);
    }
#endif  // CONFIG_MULTITHREAD
    aom_free(sync->cur_col);
...
static int get_next_job(AV1_COMP *const cpi, int *current_mi_row,
                        int cur_tile_id) {
  SyncData *const sync = &cpi->enc_row_mt.sync[cur_tile_id];
  const int mi_row_end = cpi->tile_data[cur_tile_id].tile_info.mi_row_end;
  const int mib_size = cpi->common.seq_params.mib_size;

  if (sync->current_mi_row < mi_row_end) {
    *current_mi_row = sync->current_mi_row;
    sync->num_threads_working++;
    sync->current_mi_row += mib_size;
    return 1;
  }
  return 0;
}
...
static AOM_INLINE void switch_tile_and_get_next_job(AV1_COMP *const cpi,
  ...
  for (int tile_row = 0; tile_row < tile_rows; tile_row++) {
    for (int tile_col = 0; tile_col < tile_cols; tile_col++) {
      ...
      int num_threads_working = sync->num_threads_working;

      if (num_threads_working < theoretical_limit_on_threads) {
        int num_mis_to_encode = tile_info.mi_row_end - sync->current_mi_row;
...
static int enc_row_mt_worker_hook(void *arg1, void *unused) {
  ...
  while (1) {
    ...
    SyncData *const sync = &enc_row_mt->sync[cur_tile_id];
...
void av1_encode_tiles_row_mt(AV1_COMP *cpi) {
  ...
  for (int tile_row = 0; tile_row < tile_rows; tile_row++) {
    for (int tile_col = 0; tile_col < tile_cols; tile_col++) {
      ...
      memset(sync->cur_col, -1, sizeof(*sync->cur_col) * max_sb_rows);
      sync->current_mi_row = this_tile->tile_info.mi_row_start;
      sync->num_threads_working = 0;
