...
static void set_firstpass_encode_done(AV1_COMP *cpi) {
  ...
  // thread (main/worker) encounters an error, update that the firstpass
  // processing of every firstpass row in the frame is complete in order to
  ...
  for (int tile_row = 0; tile_row < tile_rows; ++tile_row) {
    for (int tile_col = 0; tile_col < tile_cols; ++tile_col) {
      ...
      AV1EncRowMultiThreadSync *const row_mt_sync = &tile_data->row_mt_sync;
      const int unit_cols_in_tile = av1_get_unit_cols_in_tile(tile, fp_block_size);
...
static int fp_enc_row_mt_worker_hook(void *arg1, void *unused) {
  ...
  AV1_COMMON *const cm = &cpi->common;
  const int cur_tile_id = enc_row_mt->thread_id_to_tile_id[thread_id];
