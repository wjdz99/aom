 ...
 * \param[in]    use_scaled_ref_frame     Flag to indicate if scaled reference
 *                                        frame is used.
 *
...
static INLINE void find_predictors(
  ...
  const YV12_BUFFER_CONFIG *yv12 =
      scaled_ref ? scaled_ref : get_ref_frame_yv12_buf(cm, ref_frame);
  ...
  mbmi->num_proj_ref = 1;
  if (scaled_ref) *use_scaled_ref_frame = 1;
}
