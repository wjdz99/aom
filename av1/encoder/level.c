...
// We assume time t to be valid iff t >= 0.0. So INVALID_TIME can be defined
// as anything less than 0.
#define INVALID_TIME (-1.0)

static void free_buffer(DECODER_MODEL *const decoder_model, int idx) {
  assert(idx >= 0 && idx < BUFFER_POOL_MAX_SIZE);
  FRMAE_BUFFER *const this_buffer = &decoder_model->frame_buffer_pool[idx];
  this_buffer->decoder_ref_count = 0;
  this_buffer->player_ref_count = 0;
  this_buffer->decode_index = -1;
...
static int get_free_buffer(DECODER_MODEL *const decoder_model) {
  for (int i = 0; i < BUFFER_POOL_MAX_SIZE; ++i) {
    const FRMAE_BUFFER *const this_buffer =
...
static double time_to_decode_frame(const AV1_COMMON *const cm,
  ...
  if ( frame_type == KEY_FRAME || frame_type == INTRA_ONLY_FRAME ) {
    luma_samples = upscaled_width * height;
  } else {
...
static void start_decode_at_removal_time(DECODER_MODEL *const decoder_model,
                                         double removal_time) {
  for (int i = 0; i < BUFFER_POOL_MAX_SIZE; ++i) {
    FRMAE_BUFFER *const this_buffer = &decoder_model->frame_buffer_pool[i];
...
double time_next_buffer_is_free(const DECODER_MODEL *const decoder_model) {
  ...
  double buf_free_time = INVALID_TIME;
  for (int i = 0; i < BUFFER_POOL_MAX_SIZE; ++i) {
    const FRMAE_BUFFER *const this_buffer =
        &decoder_model->frame_buffer_pool[i];
    if (this_buffer->decoder_ref_count == 0 ) {
      ...
      if (presentation_time >= 0.0 &&
          (buf_free_time < 0.0 || presentation_time < buf_free_time)) {
        buf_free_time = presentation_time;
      }
    }
  }
  return buf_free_time;
}
...
void av1_decoder_model_print_status(const DECODER_MODEL *const decoder_model) {
  printf("\n status %d, num_frame %3d, num_decoded_frame %3d, "
         ...
         decoder_model->status,
         decoder_model->num_frame,
         decoder_model->num_decoded_frame,
         decoder_model->num_shown_frame,
         decoder_model->current_time,
         frames_in_buffer_pool(decoder_model),
         decoder_model->initial_presentation_delay,
         decoder_model->coded_frame_interval_queue.total_interval);
  for (int i = 0; i < 10; ++i) {
    const FRMAE_BUFFER *const this_buffer =
        &decoder_model->frame_buffer_pool[i];
...
void av1_decoder_model_init(const AV1_COMP *const cpi, AV1_LEVEL level,
  ...
  decoder_model->status = DECODER_MODLE_OK;
  decoder_model->level = level;

  const AV1_COMMON *const cm = &cpi->common;
  const SequenceHeader *const seq_params = &cm->seq_params;
  decoder_model->bit_rate =
      get_max_bitrate(av1_level_defs + level, seq_params->tier[op_index],
                      seq_params->profile);
  ...
  decoder_model->low_delay_mode = 0;
  ...
  decoder_model->coded_frame_interval_queue.total_interval = 0.0;
  decoder_model->coded_frame_interval_queue.head = 0;
  decoder_model->coded_frame_interval_queue.size = 0;
...
void av1_decoder_model_process_frame(const AV1_COMP *const cpi,
  ...
  if (!decoder_model || decoder_model->status != DECODER_MODLE_OK) return;
  ...
  if (show_existing_frame) {
    ...
    const double buffer_delay =
        (decoder_model->encoder_buffer_delay +
            decoder_model->decoder_buffer_delay) / 90000.0;
    ...
    // Smooth buffer underflows if the last bit arrives after the removal time.
    if (decoder_model->last_bit_arrival_time > removal_time &&
        !decoder_model->low_delay_mode) {
      decoder_model->status = SMOOTH_BUFFER_UNDERFLOW;
      return;
    }
    // Reset the coded bits for the next DFG.
    decoder_model->coded_bits = 0;

    // Check if the smooth buffer overflows.
    CODED_FRMAE_INTERVAL_QUEUE *const queue =
        &decoder_model->coded_frame_interval_queue;
    if (queue->size >= CODED_FRMAE_INTERVAL_QUEUE_SIZE) {
      assert(0);
    }
    ...
    while (queue->buf[queue->head].removal_time <= last_bit_arrival_time &&
      ...
      if (queue->buf[queue->head].removal_time - first_bit_arrival_time +
          queue->total_interval > 1.0) {
        decoder_model->status = SMOOTH_BUFFER_OVERFLOW;
        return;
      }
      queue->total_interval -=
          queue->buf[queue->head].last_bit_arrival_time -
          queue->buf[queue->head].first_bit_arrival_time;
      queue->head = (queue->head + 1) % CODED_FRMAE_INTERVAL_QUEUE_SIZE;
      --queue->size;
    }
    // Push current DFG into the queue.
    const int queue_index =
        (queue->head + queue->size++) % CODED_FRMAE_INTERVAL_QUEUE_SIZE;
    ...
    // The smooth buffer can hold at most "bit_rate" bits, which is equivalent
    // to 1 second of total interval.
    ...
    start_decode_at_removal_time(decoder_model, removal_time);
    decoder_model->current_time =
    ...
    {
      FRMAE_BUFFER *const this_buffer =
          &decoder_model->frame_buffer_pool[cfbi];
      this_buffer->frame_type = current_frame->frame_type;
      this_buffer->order_hint = current_frame->order_hint;
      this_buffer->decode_index = decoder_model->num_decoded_frame;
    }
