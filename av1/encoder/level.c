...
static int is_in_current_op(int current_operating_point,
                            int temporal_layer_id, int spatial_layer_id) {
  if (!current_operating_point) return 1;

  return ((current_operating_point >> temporal_layer_id) & 1) &&
      ((current_operating_point >> (spatial_layer_id + 8)) & 1);
}
...
void av1_update_level_info(AV1_COMP *cpi, size_t size, int64_t ts_start,
  ...
  for (int i = 0; i < seq->operating_points_cnt_minus_1 + 1; ++i) {
    if (!is_in_current_op(seq->operating_point_idc[i], cm->temporal_layer_id,
                          cm->spatial_layer_id))
      continue;
