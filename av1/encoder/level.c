...
static const char *level_fail_messages[TARGET_LEVEL_FAIL_IDS] = {
  ...
  "The picture height is too large.", "Too many column tiles are used.",
  "Too many tiles are used.",
};

static void check_level_constraints(AV1_COMP *cpi, AV1LevelSpec *level_spec) {
  AV1_COMMON *const cm = &cpi->common;

  if (cpi->target_seq_level_idx > LEVEL_DISABLED &&
      cpi->target_seq_level_idx < LEVEL_END) {
    // Check level conformance
    // TODO(kyslov@) implement all constraints
    const AV1LevelSpec *target_level_spec =
        av1_level_defs + cpi->target_seq_level_idx;
    const int target_level_major = 2 + (cpi->target_seq_level_idx >> 2);
    const int target_level_minor = cpi->target_seq_level_idx & 3;
...
void av1_update_level_info(AV1_COMP *cpi, size_t size, int64_t ts_start,
  ...
  const uint32_t frame_compressed_size = (size > 129 ? size - 128 : 1);
  const uint32_t frame_uncompressed_size =
      (luma_pic_size * pic_size_profile_factor) >> 3;
  const uint32_t compression_ratio =
      (frame_uncompressed_size + (frame_uncompressed_size >> 1)) /
  ...
  // TODO(kyslov@) These are needed for further level atat calculations
