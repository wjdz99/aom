...
static void get_tile_stats(const AV1_COMP *const cpi,
                           int *max_tile_size,
...
void av1_update_level_info(AV1_COMP *cpi, size_t size, int64_t ts_start,
  ...
  get_tile_stats(cpi, &max_tile_size, &min_cropped_tile_width,
                 &min_cropped_tile_height, &tile_width_valid);
  ...
  for (int i = 0; i < seq->operating_points_cnt_minus_1 + 1; ++i) {
    ...
    level_spec->max_tiles = AOMMAX(level_spec->max_tile_cols, tiles);
    ...
    level_spec->tile_width_valid &= tile_width_valid;
