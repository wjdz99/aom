...
static AOM_INLINE void update_b_sep_sym(int wiener_win, int64_t **Mc,
  ...
  // of Hc. Noting that the caller wiener_decompose_sep_sym() initializes a[...]
  // to a range smaller than 16 bits, the scale_threshold is set as below for
  // convenience. (The largest scale_threshold is expected to depend on
  // bit-depth and maximum restoration-unit size.)
  int32_t max_a_l = 0;
  for (int l = 0; l < wiener_win; ++l) {
    const int32_t abs_a_l = abs(a[l]);
    if (abs_a_l > max_a_l) max_a_l = abs_a_l;
  }
  const int scale_threshold = 2 * WIENER_TAP_SCALE_FACTOR;
