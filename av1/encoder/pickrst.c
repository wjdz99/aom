...
static int64_t calc_sgrproj_err(const RestSearchCtxt *rsc,
  ...
  int dat_stride = rsc->dgd_stride;
  int src_stride = rsc->src_stride;
  ...
  if (limits != NULL) {
    ...
    VECTOR_FOR_EACH(current_unit_stack, listed_unit) {
      ...
      dat8 = rsc->dgd_buffer + old_limits.v_start * rsc->dgd_stride +
             old_limits.h_start;
      const uint8_t *old_src8 = rsc->src_buffer +
                                old_limits.v_start * rsc->src_stride +
                                old_limits.h_start;
      width = old_limits.h_end - old_limits.h_start;
      height = old_limits.v_end - old_limits.v_start;
      flt_stride = ((width + 7) & ~7) + 8;
      err +=
          compute_sgrproj_err(dat8, width, height, dat_stride, old_src8,
                              src_stride, use_highbitdepth, bit_depth, pu_width,
                              pu_height, ep, flt0, flt1, flt_stride, exqd);
    }
  }
