...
// the function values
static double compute_score(int wiener_win, double *M, double *H, InterpKernel vfilt,
                            InterpKernel hfilt) {
...
static int count_wiener_bits(int wiener_win,
...
// Construct a (WIENER_HALFWIN)-pixel border around the frame
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    quantize_sym_filter(wiener_win, vfilterd, rsi[plane].wiener_info[tile_idx].vfilter);
    quantize_sym_filter(wiener_win, hfilterd, rsi[plane].wiener_info[tile_idx].hfilter);

    // Filter score computes the value of the function x'*A*x - x'*b for the
    // learned filter and compares it against identity filer. If there is no
    // reduction in the function, the filter is reverted back to identity
    score = compute_score(wiener_win, M, H, rsi[plane].wiener_info[tile_idx].vfilter,
    ...
    err = finer_tile_search_wiener(src, cpi, rsi, 4, plane, tile_idx,
                                   partial_frame, dst_frame);
    bits =
        count_wiener_bits(wiener_win,
                          &rsi[plane].wiener_info[tile_idx], &ref_wiener_info)
        << AV1_PROB_COST_SHIFT;
...
static double search_switchable_restoration(
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    for (r = 1; r < RESTORE_SWITCHABLE_TYPES; r++) {
      ...
      if (r == RESTORE_WIENER)
        tilebits +=
            count_wiener_bits((plane == AOM_PLANE_Y ? WIENER_WIN : WIENER_WIN - 2),
