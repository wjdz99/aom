...
static AOM_FORCE_INLINE void compute_stats_highbd(
    ...
    void *Mp, void *Hp, aom_bit_depth_t bit_depth) {
  int i, j, k, l;
  const int wiener_win2 = wiener_win * wiener_win;
  const int wiener_halfwin = (wiener_win >> 1);
  const uint16_t *src = CONVERT_TO_SHORTPTR(src8);
  ...
  if (bit_depth == AOM_BITS_12) bit_depth_divider = 16;
  else if (bit_depth == AOM_BITS_10) bit_depth_divider = 4;
  ...
  for (i = v_start; i < v_end; i++) {
    for (j = h_start; j < h_end; j++) {
      ...
      assert(idx == wiener_win2);
      for (k = 0; k < wiener_win2; ++k) {
        M[k] += Y[k] * X / bit_depth_divider;
        for (l = k; l < wiener_win2; ++l) {
          // H is a symmetric matrix, so we only need to fill out the upper
          // triangle here. We can copy it down to the lower triangle outside
          // the (i, j) loops.
          H[k * wiener_win2 + l] += Y[k] * Y[l] / bit_depth_divider;
        }
      }
    }
  }
