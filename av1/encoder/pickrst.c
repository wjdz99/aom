...
static void search_selfguided_restoration(uint8_t *dat8, int width, int height,
                                          ...
                                          int pu_width, int pu_height, int *eps,
                                          int *xqd, int32_t *rstbuf) {
  int32_t *flt1 = rstbuf;
  int32_t *flt2 = flt1 + RESTORATION_TILEPELS_MAX;
  int ep, bestep = 0;
  int64_t err, besterr = -1;
  int exqd[2], bestxqd[2] = { 0, 0 };
  int flt1_stride = width + RESTORATION_BORDER_HORZ * 2;
  int flt2_stride = width + RESTORATION_BORDER_HORZ * 2;
  ...
  for (ep = 0; ep < SGRPROJ_PARAMS; ep++) {
    ...
    get_proj_subspace(src8, width, height, src_stride, dat8, dat_stride,
                      bit_depth, flt1, flt1_stride, flt2, flt2_stride, exq);
    aom_clear_system_state();
    encode_xq(exq, exqd);
    err = finer_search_pixel_proj_error(
        src8, width, height, src_stride, dat8, dat_stride, bit_depth, flt1,
        flt1_stride, flt2, flt2_stride, 2, exqd);
