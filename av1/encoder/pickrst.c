...
static int64_t get_pixel_proj_error2(uint8_t *src8, uint8_t *src82,
                                     int width, int height,
                                     int src_stride, uint8_t *dat8, uint8_t *dat82,
                                     int dat_stride, int bit_depth,
                                     int32_t *flt1, int32_t *flt12, int flt1_stride,
                                     int32_t *flt2, int32_t *flt22, int flt2_stride, int *xqd) {
  int i, j;
  int64_t err = 0;
  int xq[2];
  decode_xq(xqd, xq);
...
static void get_proj_subspace2(uint8_t *src8, uint8_t *src82, int width, int height,
                               int src_stride, uint8_t *dat8, uint8_t *dat82,
                               int dat_stride, int bit_depth,
                               int32_t *flt1, int32_t *flt12, int flt1_stride,
...
static void search_selfguided_restoration2(uint8_t *dat8, uint8_t *dat82,
  ...
  int64_t err, besterr = -1;
  int exqd[2], bestxqd[2] = { 0, 0 };
  int32_t *flt12 = (int32_t *)aom_malloc(2 * RESTORATION_TILEPELS_MAX * sizeof(*flt12));
...
#endif  // CONFIG_HIGHBITDEPTH
    aom_clear_system_state();
    get_proj_subspace2(src8, src82, width, height, src_stride, dat8, dat82, dat_stride,
                       bit_depth, flt1, flt12, width, flt2, flt22, width, exq);
    aom_clear_system_state();
    encode_xq(exq, exqd);
    err =
        get_pixel_proj_error2(src8, src82, width, height, src_stride, dat8, dat82, dat_stride,
                             bit_depth, flt1, flt12, width, flt2, flt22, width, exqd);
    if (besterr == -1 || err < besterr) {
      bestep = ep;
      besterr = err;
      bestxqd[0] = exqd[0];
...
static double search_sgrproj(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
  ...
  SgrprojInfo *sgrproj_info2 = plane == 3 ? info[cplane + 1].sgrproj_info : NULL;
  ...
  const int ntiles =
      av1_get_rest_ntiles(width, height, cm->rst_info[cplane].restoration_tilesize,
                          &tile_width, &tile_height, &nhtiles, &nvtiles);
  SgrprojInfo ref_sgrproj_info;
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    err = sse_restoration_tile(src, cm->frame_to_show, cm, h_start,
                               h_end - h_start, v_start, v_end - v_start, comp_plane);
    ...
    if (plane == 3) {
      search_selfguided_restoration2(
          ...
          dgd_buffer2 + v_start * dgd_stride + h_start,
          h_end - h_start,
      ...
      memcpy(&rsi[cplane + 1].sgrproj_info[tile_idx], &rsi[cplane].sgrproj_info[tile_idx],
...
static double search_sgrproj(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
                             ...
                             double *best_tile_cost,
                             YV12_BUFFER_CONFIG *dst_frame) {
  SgrprojInfo *sgrproj_info = info[plane].sgrproj_info;
  double err, cost_norestore, cost_sgrproj;
  int bits;
  MACROBLOCK *x = &cpi->td.mb;
  AV1_COMMON *const cm = &cpi->common;
  ...
  const int ntiles =
      av1_get_rest_ntiles(width, height, cm->rst_info[plane].restoration_tilesize,
                          &tile_width, &tile_height, &nhtiles, &nvtiles);
  SgrprojInfo ref_sgrproj_info;
  set_default_sgrproj(&ref_sgrproj_info);
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    av1_get_rest_tile_limits(tile_idx, 0, 0, nhtiles, nvtiles, tile_width,
                             ...
                             &v_start, &v_end);
    err = sse_restoration_tile(src, cm->frame_to_show, cm, h_start,
                               h_end - h_start, v_start, v_end - v_start, 1 << plane);
    // #bits when a tile is not restored
    bits = av1_cost_bit(RESTORE_NONE_SGRPROJ_PROB, 0);
    cost_norestore = RDCOST_DBL(x->rdmult, x->rddiv, (bits >> 4), err);
    best_tile_cost[tile_idx] = DBL_MAX;
...
static int64_t finer_tile_search_wiener(const YV12_BUFFER_CONFIG *src,
                                        ...
                                        int tile_idx, int partial_frame,
                                        YV12_BUFFER_CONFIG *dst_frame) {
...
static double search_wiener(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
                            ...
                            RestorationType *type, double *best_tile_cost,
                            YV12_BUFFER_CONFIG *dst_frame) {
  WienerInfo *wiener_info = info[plane].wiener_info;
  AV1_COMMON *const cm = &cpi->common;
  RestorationInfo *rsi = cpi->rst_search;
  int64_t err;
  int bits;
...
static double search_norestore(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
                               ...
                               double *best_tile_cost,
                               YV12_BUFFER_CONFIG *dst_frame) {
  RestorationInfo *rsi = &info[plane];
  int64_t err;
  double cost_norestore;
  int bits;
  MACROBLOCK *x = &cpi->td.mb;
...
static double search_switchable_restoration(
  ...
  const int ntiles = av1_get_rest_ntiles(
      width, height, cm->rst_info[cplane].restoration_tilesize, NULL, NULL, NULL,
      NULL);
  SgrprojInfo ref_sgrproj_info;
  set_default_sgrproj(&ref_sgrproj_info);
  WienerInfo ref_wiener_info;
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    for (r = 1; r < RESTORE_SWITCHABLE_TYPES; r++) {
      ...
      if (cost < best_cost) {
        ...
        if (plane == 3)
          rsi[1].restoration_type[tile_idx] = r;
        best_cost = cost;
      }
    }
...
void av1_pick_filter_restoration(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
  ...
  for (int plane = AOM_PLANE_Y; plane <= AOM_PLANE_V; ++plane) {
    ...
    for (r = 0; r < RESTORE_SWITCHABLE_TYPES; ++r) {
      ...
      cost_restore[r] =
          search_restore_fun[r](src, cpi, method == LPF_PICK_FROM_SUBIMAGE,
                                plane_sent, cm->rst_info, restore_types[r],
    ...
    if (plane_sent == 3) cm->rst_info[plane + 1].frame_restoration_type = best_restore;
    if (best_restore != RESTORE_SWITCHABLE) {
      ...
      if (plane_sent == 3)
        memcpy(cm->rst_info[plane + 1].restoration_type, restore_types[best_restore],
               nt * sizeof(restore_types[best_restore][0]));
    }
  }
