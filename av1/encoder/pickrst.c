...
static int64_t compute_sse(uint8_t *dgd, int width, int height, int dgd_stride,
  ...
  for (i = 0; i < height; ++i) {
    for (j = 0; j < width; ++j) {
      const int diff = (int)dgd[i * dgd_stride + j] - (int)src[i * src_stride + j];
      sse += diff * diff;
    }
  }
  return sse;
...
#if CONFIG_AOM_HIGHBITDEPTH
static int64_t compute_sse_highbd(uint16_t *dgd, int width, int height, int dgd_stride,
...
static void search_domaintxfmrf_restoration(uint8_t *dgd8, int width, int height,
                                            int dgd_stride, uint8_t *src8,
                                            int src_stride, int bit_depth,
                                            int *sigma_r) {
  ...
  if (bit_depth == 8) {
    ...
    // Second Phase
    best_p0 = best_p;
    for (p = best_p0 - second_p_range; p <= best_p0 + second_p_range; p += second_p_step) {
    ...
    // Third Phase
    best_p0 = best_p;
    for (p = best_p0 - third_p_range; p <= best_p0 + third_p_range; p += third_p_step) {
      if (p < 0 || p == best_p || p >= DOMAINTXFMRF_PARAMS) continue;
      for (i = 0; i < height; ++i) {
        memcpy(&tmp[i * width], &dgd[i * dgd_stride], width * sizeof(dgd[0]));
      }
    ...
    for (p = first_p_step / 2; p < DOMAINTXFMRF_PARAMS; p += first_p_step) {
      ...
      av1_domaintxfmrf_restoration_highbd(tmp, width, height, width, p, bit_depth);
...
static double search_domaintxfmrf(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    if (cost_domaintxfmrf >= cost_norestore) {
      ...
      best_tile_cost[tile_idx] = RDCOST_DBL(
          x->rdmult, x->rddiv,
          (bits + cpi->switchable_restore_cost[RESTORE_DOMAINTXFMRF]) >> 4, err);
    }
    rsi.domaintxfmrf_info[tile_idx].level = 0;
  }
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    bits +=
        av1_cost_bit(RESTORE_NONE_DOMAINTXFMRF_PROB, domaintxfmrf_info[tile_idx].level);
...
void av1_pick_filter_restoration(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
  ...
  cm->rst_info.domaintxfmrf_info = (DomaintxfmrfInfo *)aom_realloc(
      cm->rst_info.domaintxfmrf_info, sizeof(*cm->rst_info.domaintxfmrf_info) * ntiles);
