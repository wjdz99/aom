...
static int compute_quantized_wienerns_filter(const uint8_t *dgd,
  ...
  for (int i = v_beg; i < v_end; ++i) {
    for (int j = h_beg; j < h_end; ++j) {
      ...
      if (rui->txskip_mask[((i - v_beg) >> MIN_TX_SIZE_LOG2) *
                               rui->mask_stride +
                           ((j - h_beg) >> MIN_TX_SIZE_LOG2)])
        continue;
...
static void search_wiener_nonsep(const RestorationTileLimits *limits,
  ...
  int mask_idx_start = (limits->v_start >> MIN_TX_SIZE_LOG2) * rui.mask_stride +
                       (limits->h_start >> MIN_TX_SIZE_LOG2);
  rui.txskip_mask = &rsc->cm->tx_skip[rui.plane][mask_idx_start];
