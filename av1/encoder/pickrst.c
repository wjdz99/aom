...
static int linsolve_wiener(int n, int64_t *A, int stride, int64_t *b,
  ...
  for (int k = 0; k < n - 1; k++) {
    ...
    // b/278065963 and b/259173819: The multiplies
    ...
    // scaler_c: Pick as low as possible while maintaining scaler_c >= 1 >> 7.
    //   Needed for A[(i + 1) * stride + j] and b[i + 1].
    int64_t max_abs_akj = 0;
    for (int j = 0; j < n; j++) {
      const int64_t abs_akj = abs(A[k * stride + j]);
