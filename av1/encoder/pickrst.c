...
#if CONFIG_RST_MERGECOEFFS || CONFIG_FORCE_WIENER
...
#endif  // CONFIG_RST_MERGECOEFFS || CONFIG_FORCE_WIENER
...
static void search_wiener(const RestorationTileLimits *limits,
  ...
  for (int index = 0; index < WIENER_WIN2 * WIENER_WIN2; index++) {
    H_AVG[index] = (H_AVG[index]) / (current_unit_stack->size + 1);
  }
  // generate new filter
  ...
  for (Iterator listed_unit = aom_vector_begin((current_unit_stack)),
    ...
    // first unit in stack has larger unit_bits bc the merged coeffs are linked
    // to it
    int64_t unit_bits;
