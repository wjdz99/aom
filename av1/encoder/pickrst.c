...
static double search_switchable_restoration(
  ...
  rsi[plane].frame_restoration_type = RESTORE_SWITCHABLE;
  bits = frame_level_restore_bits[rsi[plane].frame_restoration_type]
         << AV1_PROB_COST_SHIFT;
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    for (r = 1; r < RESTORE_SWITCHABLE_TYPES; r++) {
      ...
      int tilebits = 0;
      if (r == RESTORE_WIENER)
        tilebits += count_wiener_bits(&rsi[plane].wiener_info[tile_idx],
                                      &ref_wiener_info);
      else if (r == RESTORE_SGRPROJ)
        tilebits += count_sgrproj_bits(&rsi[plane].sgrproj_info[tile_idx],
                                       &ref_sgrproj_info);
      ...
      if (cost < best_cost) {
        rsi[plane].restoration_type[tile_idx] = r;
        best_cost = cost;
      }
    }
    if (rsi[plane].restoration_type[tile_idx] == RESTORE_WIENER)
      memcpy(&ref_wiener_info, &rsi[plane].wiener_info[tile_idx],
             sizeof(ref_wiener_info));
    else if (rsi[plane].restoration_type[tile_idx] == RESTORE_SGRPROJ)
      memcpy(&ref_sgrproj_info, &rsi[plane].sgrproj_info[tile_idx],
             sizeof(ref_sgrproj_info));
    if (force_restore_type != 0)
      assert(rsi[plane].restoration_type[tile_idx] == force_restore_type ||
             rsi[plane].restoration_type[tile_idx] == RESTORE_NONE);
