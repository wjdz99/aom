...
static int64_t try_restoration_tile(const YV12_BUFFER_CONFIG *src,
  ...
  ntiles = av1_get_rest_ntiles(width, height, &tile_width,
  ...
  av1_get_rest_tile_limits(tile_idx, subtile_idx, subtile_bits, nhtiles,
                           nvtiles, tile_width, tile_height, width,
                           height, 0, 0, &h_start, &h_end, &v_start,
                           &v_end);
...
/*
static double search_wiener_uv_single(const YV12_BUFFER_CONFIG *src,
                                      AV1_COMP *cpi,
                                      int filter_level, int partial_frame,
...
static double search_wiener_uv(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
  ...
  bits = 0;
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    bits += av1_cost_bit(RESTORE_NONE_WIENER_PROB,
                         type[tile_idx] != RESTORE_NONE);
  ...
  aom_yv12_copy_u(&cpi->last_frame_uf, cm->frame_to_show);
  aom_yv12_copy_v(&cpi->last_frame_uf, cm->frame_to_show);
  return info->frame_restoration_type == RESTORE_WIENER ?
      cost_wiener_frame : cost_norestore_frame;
}
...
// searched level, filt_guess = q * 0.316206 + 3.87252
  ...
  search_wiener_uv(src, cpi, lf->filter_level, method == LPF_PICK_FROM_SUBIMAGE,
                   ...
                   &cpi->trial_frame_rst);
  /*
  search_wiener_uv_single(src, cpi, lf->filter_level,
