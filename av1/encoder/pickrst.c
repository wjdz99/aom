      ...
      for (int sb_row = sb_row_start; sb_row < sb_row_end; sb_row++) {
        ...
        for (int sb_col = sb_col_start; sb_col < sb_col_end; sb_col++) {
          ...
          int has_lr_info = av1_loop_restoration_corners_in_sb(
              cm, is_uv, mi_row, mi_col, sb_size, &rcol0, &rcol1, &rrow0,
              &rrow1);
...
void av1_pick_filter_restoration(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi) {
  ...
  // (i.e., from foreach_rest_unit_in_plane() 1.5 times the
  ...
  for (int plane = plane_start; plane <= plane_end; ++plane) {
    ...
    // Set frame_restoration_type to anything other than RESTORE_NONE.
    // This bypasses an early exit in av1_loop_restoration_corners_in_sb(),
    // which would otherwise not give us the proper LR unit indices for
    // RESTORE_NONE.
