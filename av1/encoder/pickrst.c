...
static SgrprojInfo search_selfguided_restoration(
  ...
  if (disable_sgr_ep_pruning) {
    ...
    static const int seed_ep[] = { 0, 3, 6, 9 };
    static const int group_2_ep[] = { 10, 10, 11, 11, 11, 12, 12, 12, 12, 13 };
    int bestep_ref = 0;
    int nearpt = -1;
    int nextpt = 0;
    int start_ep_group3 = 14;
    for (idx = 0; idx < 4; idx++) {
      ep = seed_ep[idx];
      int64_t err;
      compute_sgrproj_err(dat8, width, height, dat_stride, src8, src_stride,
    ...
    bestep_ref = bestep;
    for (idx = 0; idx < 4; idx++) {
      // idx < 2 means evaluate two ep from group 2 else 2 ep from group 3
      if (idx < 2) {
        // ep 0 and 9 have one closer point in group 2
        if ((bestep_ref == 0 || bestep_ref == 9) && nextpt) continue;
        ep = group_2_ep[bestep_ref] + nextpt++;
      } else {
        ep = start_ep_group3++;
      }
      int64_t err;
