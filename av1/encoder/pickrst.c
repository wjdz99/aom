...
static void smull_saturate(int64_t x, int64_t y, int64_t *sum) {
  if(!__builtin_smull_overflow(x, y, sum))
    return;
  if(x < 0 && y > 0)
    *sum = INT64_MIN;
  else if(x > 0 && y < 0)
...
static void saddl_saturate(int64_t x, int64_t y, int64_t *sum) {
  if(!__builtin_saddl_overflow(x, y, sum))
    return;
  if(x > 0 && y > 0)
    *sum = INT64_MAX;
  else if(x < 0 && y < 0)
...
#if CONFIG_INTEGERIZE_WIENER
static uint64_t find_average_i(const uint8_t *src, int h_start, int h_end,
...
// Redeclare integerized version of this function to avoid using the SIMD
// parallelized versions of av1_compute_stats
void av1_compute_stats_i(int wiener_win, const uint8_t *dgd, const uint8_t *src,
                         int h_start, int h_end, int v_start, int v_end,
  ...
  const uint64_t avg =
      find_average_i(dgd, h_start, h_end, v_start, v_end, dgd_stride);
  ...
  for (i = v_start; i < v_end; i++) {
    for (j = h_start; j < h_end; j++) {
      ...
      assert(idx == wiener_win2);
      for (k = 0; k < wiener_win2; ++k) {
        // Do M[k] += Y[k] * X; but with saturating multiply and add
        int64_t tmp = 0;
        smull_saturate(Y[k], X, &tmp);
        saddl_saturate(M[k], tmp, &M[k]);
        for (l = k; l < wiener_win2; ++l) {
          // H is a symmetric matrix, so we only need to fill out the upper
          // triangle here. We can copy it down to the lower triangle outside
          // the (i, j) loops.
...
static void update_a_sep_sym(int wiener_win, double **Mc, double **Hc,
  ...
  for (i = 0; i < wiener_win; i++) {
    for (j = 0; j < wiener_win; ++j) {
      ...
      int64_t tmp = 0;
      smull_saturate(Mc[i][j], b[i], &tmp);
      tmp /= WIENER_TAP_SCALE_FACTOR;
      saddl_saturate(A[jj], tmp, &A[jj]);
#else
      A[jj] += Mc[i][j] * b[i];
#endif  // CONFIG_INTEGERIZE_WIENER
    }
...
//      A[ii] += Mc[i][j] * a[j] / WIENER_TAP_SCALE_FACTOR;
      int64_t tmp = 0;
      smull_saturate(Mc[i][j], a[j], &tmp);
      tmp /= WIENER_TAP_SCALE_FACTOR;
      saddl_saturate(A[ii], tmp, &A[ii]);
#else
      A[ii] += Mc[i][j] * a[j];
#endif  // CONFIG_INTEGERIZE_WIENER
    }
...
#if !CONFIG_INTEGERIZE_WIENER  // Disable highbd
  const AV1_COMMON *const cm = rsc->cm;
...
#endif  // disable highbd
#if CONFIG_INTEGERIZE_WIENER
    av1_compute_stats_i(wiener_win, rsc->dgd_buffer, rsc->src_buffer,
                      limits->h_start, limits->h_end, limits->v_start,
                      limits->v_end, rsc->dgd_stride, rsc->src_stride, M, H);
#else  // Disable highbd
