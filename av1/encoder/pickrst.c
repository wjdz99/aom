...
// the row and column parameters are not valid indices, return DBL_MAX.
static double get_mse_block(Y_BUFFER_CONFIG buf1, Y_BUFFER_CONFIG buf2,
                            ...
                            int block_size) {
  // Check if rows and columns are valid, return DBL_MAX if not.
...
#if CONFIG_MFQE_RESTORATION
  int use_mfqe = 0;
  // In-Loop Multi-Frame Quality Enhancement.
  if (!cm->seq_params.use_highbitdepth)
