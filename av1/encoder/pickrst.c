...
static int linsolve_wiener(int n, int64_t *A, int stride, int64_t *b,
  ...
  for (int i = n - 1; i >= 0; i--) {
    ...
    for (int j = i + 1; j <= n - 1; j++) {
      c += A[i * stride + j] * x[j] / WIENER_FILT_STEP;
    }
    // Store filter taps x in scaled form
    x[i] = WIENER_FILT_STEP * (b[i] - c) / A[i * stride + i];
  }
...
static void update_a_sep_sym(int wiener_win, double **Mc, double **Hc,
...
#if CONFIG_INTEGERIZE_WIENER
      A[jj] += Mc[i][j] * b[i] / WIENER_FILT_STEP;
#else
      A[jj] += Mc[i][j] * b[i];
#endif  // CONFIG_INTEGERIZE_WIENER
    }
      ...
      for (k = 0; k < wiener_win; ++k)
        for (l = 0; l < wiener_win; ++l) {
          ...
          B[ll * wiener_halfwin1 + kk] +=
              ...
              * b[i] / WIENER_FILT_STEP * b[j] / WIENER_FILT_STEP;
#else
          B[ll * wiener_halfwin1 + kk] +=
  ...
  if (linsolve_wiener(wiener_halfwin1 - 1, B, wiener_halfwin1, A, S)) {
    S[wiener_halfwin1 - 1] = WIENER_FILT_STEP;
...
static void update_b_sep_sym(int wiener_win, double **Mc, double **Hc,
  ...
  for (i = 0; i < wiener_win; i++) {
    ...
    for (j = 0; j < wiener_win; j++) {
#if CONFIG_INTEGERIZE_WIENER
      A[ii] += Mc[i][j] * a[j] / WIENER_FILT_STEP;
#else
      A[ii] += Mc[i][j] * a[j];
#endif  // CONFIG_INTEGERIZE_WIENER
    }
      ...
      for (k = 0; k < wiener_win; ++k) {
        for (l = 0; l < wiener_win; ++l) {
          ...
          B[jj * wiener_halfwin1 + ii] +=
              ...
              * a[k] / WIENER_FILT_STEP * a[l] / WIENER_FILT_STEP;
#else
          B[jj * wiener_halfwin1 + ii] +=
...
#if CONFIG_INTEGERIZE_WIENER
static void finalize_sym_filter(int wiener_win, int16_t *f, InterpKernel fi) {
  int i;
  const int wiener_halfwin = (wiener_win >> 1);
...
#if CONFIG_INTEGERIZE_WIENER
  int16_t vfilter[WIENER_WIN], hfilter[WIENER_WIN];
#else
