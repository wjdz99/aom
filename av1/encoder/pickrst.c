...
// Basic data to maintain the in-loop data export. Manipulate using
...
typedef struct {
  ...
  // File name to save the data under. This is what needs to be fed into
  // kickoff through --keep_stats=...
...
static bool export_context_initialize(int num_rows_luma, int num_cols_luma) {
  ...
  fwrite(&num_rows_luma, sizeof(int), 1, export_file);
  fwrite(&num_cols_luma, sizeof(int), 1, export_file);
  fclose(export_file);
...
static bool export_context_export(const uint8_t *frame, int stride) {
  ...
  for (int r = 0; r < export_context.num_rows_luma; ++r) {
    for (int c = 0; c < export_context.num_cols_luma; ++c) {
      const float pixel_value = frame[r * stride + c];
      fwrite(&pixel_value, sizeof(float), 1, export_file);
    }
  }
  fclose(export_file);
  return true;
...
void av1_pick_filter_restoration(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi) {
...
#if CONFIG_SAVE_IN_LOOP_DATA
  // TODO: Add the high-bit-detph path.
  ...
  if (exporting_this_frame) {
    if (!export_context_is_initialized()) {
      ...
      export_context_initialize(num_rows_luma, num_cols_luma);
    }
    export_context_register_as_exported(absolute_poc);

    // Export original.
    assert(export_context_export(src->buffers[AOM_PLANE_Y], src->strides[0]));

    // Export decoded frame before loop reconstruction.
    assert(export_context_export(pre_lr_decoded->buffers[AOM_PLANE_Y],
                                 pre_lr_decoded->strides[0]));

    assert(export_context_export_qstep(cpi));
  }
  ...
  if (exporting_this_frame) {
    ...
    // Fill tmp_buffer with pre-lr decoded frame.
    ...
    // Export.
    assert(export_context_export(tmp_buffer->buffers[AOM_PLANE_Y],
                                 tmp_buffer_stride));
  }
#endif  // CONFIG_SAVE_IN_LOOP_DATA
