  ...
  for (int plane = AOM_PLANE_Y; plane <= AOM_PLANE_V; ++plane) {
    ...
    for (r = 0; r < RESTORE_SWITCHABLE_TYPES; ++r) {
      ...
      cost_restore[r] = search_restore_fun[r](
          src, cpi, method == LPF_PICK_FROM_SUBIMAGE, plane_sent, cm->rst_info,
    ...
    if (plane_sent == 3)
      cm->rst_info[plane + 1].frame_restoration_type = best_restore;
    if (best_restore != RESTORE_SWITCHABLE) {
      ...
      if (plane_sent == 3)
        memcpy(cm->rst_info[plane + 1].restoration_type,
               restore_types[best_restore],
               nt * sizeof(restore_types[best_restore][0]));
    }
  }
  /*
