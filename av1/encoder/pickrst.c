...
static int64_t finer_tile_search_wiener(const YV12_BUFFER_CONFIG *src,
  ...
  int64_t err, err2;
  int tap_min[] = {
    WIENER_FILT_TAP0_MINV, WIENER_FILT_TAP1_MINV, WIENER_FILT_TAP2_MINV
  };
  int tap_max[] = {
    WIENER_FILT_TAP0_MAXV, WIENER_FILT_TAP1_MAXV, WIENER_FILT_TAP2_MAXV
  };
...
static double search_wiener_uv(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    err = finer_tile_search_wiener(src, cpi, rsi, plane, tile_idx, partial_frame,
                                   dst_frame);
    bits =
