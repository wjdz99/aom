...
// Comparator function for sorting RefCntBuffer based on base_qindex.
...
// Returns the sum of squared errors for the given blocks in two buffers. If
// the rows and columns are not valid indices, return DBL_MAX;
static double get_mse_block(Y_BUFFER_CONFIG buf1, Y_BUFFER_CONFIG buf2,
  ...
  if ((mb_row_1 < 0) || (mb_col_1 < 0) || (mb_row_2 < 0) || (mb_col_2 < 0) ||
      (mb_row_1 >= buf1.height - block_size) ||
      (mb_row_2 >= buf2.height - block_size) ||
      (mb_col_1 >= buf1.width - block_size) ||
      (mb_col_2 >= buf2.width - block_size))
    return DBL_MAX;
  ...
  double v1;
  double v2;
  int divisor = block_size * block_size;
...
static double get_mse_frame(uint8_t *buf1, uint8_t *buf2, int stride1,
  ...
  for (int row = 0; row < w; ++row) {
    for (int col = 0; col < h; ++col) {
      p1 = buf1[stride1 * col + row];
      p2 = buf2[stride2 * col + row];
      v1 = (double)p1;
      v2 = (double)p2;
      mse += (v1 - v2) * (v1 - v2);
    }
...
// Perform initial grid search to obtain the reference motion vector.
static void full_pixel_grid_search(MV_MFQE *mvr, int16_t mb_row, int16_t mb_col,
  ...
  for (int ref_index = 0; ref_index < 3; ++ref_index) {
    for (int point = 0; point < 13; ++point) {
      ...
      mb_row_ref = mb_row + mvr->mv.row + dr * block_size;
      mb_col_ref = mb_col + mvr->mv.col + dc * block_size;

      this_mse = get_mse_block(cur, refs[ref_index], mb_row, mb_col, mb_row_ref,
                               mb_col_ref, block_size);

      if (this_mse < best_mse) {
        best_mse = this_mse;
        mvr_best.mv.row = mvr->mv.row + dr * block_size;
        mvr_best.mv.col = mvr->mv.col + dc * block_size;
...
static void full_pixel_search(MV_MFQE *mvr, int16_t mb_row, int16_t mb_col,
  ...
  full_pixel_grid_search(&mvr_init, mb_row, mb_col, cur, refs, block_size);
  ...
  for (int dr = -search_size; dr <= search_size; ++dr) {
    for (int dc = -search_size; dc <= search_size; ++dc) {
      ...
      this_mse = get_mse_block(cur, refs[mvr_init.ref_index], mb_row, mb_col,
                               mb_row_ref, mb_col_ref, block_size);
...
static void replace_block_subpel(Y_BUFFER_CONFIG tmp,
                                 ...
                                 int16_t mb_row, int16_t mb_col, int block_size,
                                 int scale, ConvolveParams *conv_params,
  ...
  av1_convolve_2d_facade(src, src_stride, dst, dst_stride, block_size,
                         block_size, block_size, block_size, *interp_filters,
                         mvr->subpel_x_qn, 0, mvr->subpel_y_qn, 0, 0,
                         conv_params, 0);
}

static void apply_loop_mfqe(YV12_BUFFER_CONFIG *cur, Y_BUFFER_CONFIG *tmp,
                            RefCntBuffer *ref_frames[], int block_size,
                            int scale, int high_bd, int bd) {
  int cur_buf_bytes = cur->y_width * cur->y_stride;
  Y_BUFFER_CONFIG tmp_low = get_gaussian_blur(
      cur->y_buffer, cur->y_stride, cur->y_height, cur->y_width, high_bd, bd);
  Y_BUFFER_CONFIG tmp_sub = get_resize_plane(
      tmp_low.buffer, cur->y_stride, cur->y_height, cur->y_width, scale);

  Y_BUFFER_CONFIG refs_low[3];
  Y_BUFFER_CONFIG refs_sub[3];
  ...
  int16_t num_rows = cur->y_height / block_size;
  int16_t num_cols = cur->y_width / block_size;

  for (int16_t mb_row = 0; mb_row < num_rows; ++mb_row) {
    for (int16_t mb_col = 0; mb_col < num_cols; ++mb_col) {
      mvr = kZeroMvMFQE;
      full_pixel_search(&mvr, mb_row, mb_col, tmp_low, refs_low, block_size);

      if (!mvr.valid) continue;
      sub_pixel_search(&mvr, mb_row, mb_col, tmp_sub, refs_sub, block_size,
                       scale);
      replace_block_subpel(*tmp, refs_sub, &mvr, mb_row, mb_col, block_size,
                           scale, &conv_params, &interp_filters);
    }
  }

  // Free dynamically allocated memory in Y_BUFFER_CONFIG;
...
// Apply multi-frame quality enhancement to frame, then return use_mfqe.
static int search_rest_mfqe(const YV12_BUFFER_CONFIG *src,
                            YV12_BUFFER_CONFIG *cur, AV1_COMMON *cm) {
  int use_mfqe = 0;
  int frame_bytes = cur->y_width * cur->y_stride;

  // Buffer for the copy of current frame to restore using MFQE.
  uint8_t *tmpbuf = (uint8_t *)aom_memalign(32, sizeof(uint8_t) * frame_bytes);
  copy_single_plane_lowbd(cur->y_buffer, tmpbuf, cur->y_stride, cur->y_stride,
                          cur->y_height, cur->y_width);

  Y_BUFFER_CONFIG tmp;
  tmp.buffer = tmpbuf;
  tmp.stride = cur->y_stride;
  tmp.height = cur->y_height;
  tmp.width = cur->y_width;
  ...
  int resize_scale = 8;
  int block_size = 8;
  apply_loop_mfqe(cur, &tmp, ref_frames, block_size, resize_scale,
                  cm->seq_params.use_highbitdepth, cm->seq_params.bit_depth);
...
void av1_pick_filter_restoration(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi) {
  ...
  if (!cm->seq_params.use_highbitdepth)
    use_mfqe = search_rest_mfqe(src, &cm->cur_frame->buf, cm);
