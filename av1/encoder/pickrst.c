...
#if CONFIG_RST_MERGECOEFFS
RestSearchCtxt switchable_update_refs(Vector *path, RestSearchCtxt *rsc,
                                      bool collect_stats) {
  ...
  VECTOR_FOR_EACH(path, listed_unit) {
    ...
    if (visited_rtype == RESTORE_NONE) continue;
...
static void search_switchable(const RestorationTileLimits *limits,
  ...
  // Get restoration type, SSE, and bits - would need to set rsc equal
  *rsc = switchable_update_refs(&best_path, rsc, true);
  ...
  //   double this_cost =
  //       min_cost_type_path(0, rest_unit_idx * max_out + i + 1, max_out,
  //       graph,
  ...
  // // Get restoration type, SSE, and bits - would need to set rsc equal
  // switchable_update_refs(&bb_path, rsc, true);
  // free(graph);
  // aom_vector_destroy(&best_path);
