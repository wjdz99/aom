...
static void search_sgrproj(const RestorationTileLimits *limits,
  ...
  if (rui_temp.sgrproj_info.ep < 10) {
    cost_merge *= (1 + DUAL_SGR_PENALTY_MULT * rsc->sf->dual_sgr_penalty_level);
  }
...
static void search_wiener_nonsep(const RestorationTileLimits *limits,
  ...
  if (compute_quantized_wienerns_filter(
    ...
    double cost_nomerge = RDCOST_DBL(x->rdmult, bits_nomerge >> 4,
                                     rusi->sse[RESTORE_WIENER_NONSEP]);
    RestorationType rtype =
        (cost_nomerge < cost_none) ? RESTORE_WIENER_NONSEP : RESTORE_NONE;
    bits_nomerge = (cost_nomerge < cost_none) ? bits_nomerge : bits_none;
    cost_nomerge = (cost_nomerge < cost_none) ? cost_nomerge : cost_none;
