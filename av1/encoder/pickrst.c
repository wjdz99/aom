...
static SgrprojInfo search_selfguided_restoration(
  ...
  int shouldbreak = 0;
  int ep_map[] = { 5, 4, 6, 0, 1, 2, 3, 7, 8, 9, 10, 11, 12, 13, 14, 15 };
  int ep_mapgrp2[] = { 10, 10, 11, 11, 12, 12, 13, 13, 13, 13 };
  int ep_mapgrp3[] = { 14, 14, 14, 14, 14, 14, 14, 15,
                       15, 15, 15, 15, 15, 15, 16, 16 };
  for (int idx = 0; idx < SGRPROJ_PARAMS;) {
    if (!disable_sgr_ep_pruning) {
      if (shouldbreak == 2) break;
      if (idx == 3) idx = bestep == 6 ? 7 : 3;
      ep = ep_map[idx];
    } else
      ep = idx;
#else
...
#if prune_sgr_ep
    if ((idx == 6 || idx == 9) && !disable_sgr_ep_pruning) {
      idx = ep_mapgrp2[bestep];
    } else if (idx > 9 && !disable_sgr_ep_pruning) {
      shouldbreak++;
      idx = ep_mapgrp3[bestep];
    } else
      idx++;
#endif
  }
