...
static void dump_frame_buffer(const YV12_BUFFER_CONFIG *buf,
                              FILE *file, int base_qindex) {
  if (!(buf && file)) return;
  // Skip if upsampling is used to encode the frame.
  if (!(buf->y_width == buf->y_crop_width
        && buf->y_height == buf->y_crop_height)) return;

  // Write out header information for YV12_BUFFER_CONFIG.
  fwrite(&base_qindex, 1, sizeof(int), file);
  fwrite(&buf->y_width, 1, sizeof(int), file);
  fwrite(&buf->y_height, 1, sizeof(int), file);
...
static void dump_frame_data(const YV12_BUFFER_CONFIG *src,
  ...
  for (int i = 0; i < 7; ++i) {
    ...
    if (ref) {
      fwrite(&ref->order_hint, 1, sizeof(int), f_data);
