...
static double search_sgrproj(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
                             ...
                             double *best_tile_cost,
                             YV12_BUFFER_CONFIG *dst_frame) {
  const int cplane = (plane == 3 ? AOM_PLANE_U : plane);
  const int comp_plane = (plane == 3 ? 6 : 1 << plane);
  SgrprojInfo *sgrproj_info = info[cplane].sgrproj_info;
  SgrprojInfo *sgrproj_info2 =
      plane == 3 ? info[cplane + 1].sgrproj_info : NULL;
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    rsi[cplane].restoration_type[tile_idx] = RESTORE_NONE;
    if (plane == 3) rsi[cplane + 1].restoration_type[tile_idx] = RESTORE_NONE;
  }
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    if (plane == 3) rsi[cplane + 1].restoration_type[tile_idx] = type[tile_idx];
  }
...
static int64_t finer_tile_search_wiener(const YV12_BUFFER_CONFIG *src,
                                        ...
                                        YV12_BUFFER_CONFIG *dst_frame) {
  const int cplane = (plane == 3 ? AOM_PLANE_U : plane);
  const int comp_plane = plane == 3 ? plane << 1 : 1 << plane;
...
static double search_wiener(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
  ...
  WienerInfo *wiener_info2 = plane == 3 ? info[cplane + 1].wiener_info : NULL;
  AV1_COMMON *const cm = &cpi->common;
...
// Construct a (WIENER_HALFWIN)-pixel border around the frame
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    if (cost_wiener >= cost_norestore) {
      ...
      if (plane == 3)
        memcpy(&wiener_info2[tile_idx], &rsi[cplane].wiener_info[tile_idx],
               sizeof(wiener_info[tile_idx]));
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    rsi[cplane].restoration_type[tile_idx] = type[tile_idx];
    if (plane == 3) {
      memcpy(&rsi[cplane + 1].wiener_info[tile_idx], &wiener_info[tile_idx],
             sizeof(wiener_info[tile_idx]));
...
static double search_switchable_restoration(
  ...
  if (plane == 3) rsi[cplane + 1].frame_restoration_type = RESTORE_SWITCHABLE;
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    for (r = 1; r < RESTORE_SWITCHABLE_TYPES; r++) {
      ...
      if (cost < best_cost) {
        ...
        if (plane == 3) rsi[cplane + 1].restoration_type[tile_idx] = r;
        best_cost = cost;
      }
    }
