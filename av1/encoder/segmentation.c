...
static void count_segs(const AV1_COMMON *cm, MACROBLOCKD *xd,
                       ...
                       unsigned (*pred_seg_counts)[SPATIAL_PRED_PROBS + 1],
                       unsigned (*temporal_predictor_count)[2],
                       unsigned (*t_unpred_seg_counts)[MAX_SEGMENTS],
                       SEG_CATEGORIES seg_cat_idx, int bw, int bh, int mi_row,
                       int mi_col) {
  int segment_id;
...

  {
    ...
    if (!seg_id_skip) {
      ...
      if (!pred_flag) {
        ...
        spatial_pred_idc = left_mi && segment_id == left_mi->mbmi.segment_id[seg_cat_idx]
          ? SEG_ID_COPY_LEFT
          : SEG_ID_SPATIAL_UNPREDICTED;
        pred_seg_counts[0][spatial_pred_idc]++;
        if (spatial_pred_idc == SEG_ID_SPATIAL_UNPREDICTED) {
          t_unpred_seg_counts[0][segment_id]++;
        }
#else
#if CONFIG_EXT_SEGMENT_NOSPATIAL
        spatial_pred_idc = SEG_ID_SPATIAL_UNPREDICTED;
#else
        ...
        if (spatial_seg_pred_ctx) {
          spatial_pred_idc = segment_id == left_mi->mbmi.segment_id[seg_cat_idx]
                                 ? SEG_ID_COPY_LEFT
                                 : SEG_ID_SPATIAL_UNPREDICTED;
          pred_seg_counts[spatial_seg_pred_ctx][spatial_pred_idc]++;
        } else {
          if (left_mi)
            spatial_pred_idc =
...
// TODO(Fangwen) CONFIG_EXT_SEGMENT does not work with
// CONFIG_EXT_PARTITION_TYPES yet
#if CONFIG_EXT_PARTITION_TYPES
  if (bsize == BLOCK_8X8)
...
#if CONFIG_EXT_SEGMENT
               pred_seg_counts, temporal_predictor_count, t_unpred_seg_counts,
               seg_cat_idx,
#else
...
#if CONFIG_EXT_SEGMENT
                    pred_seg_counts, temporal_predictor_count,
                    t_unpred_seg_counts, seg_cat_idx,
#else
...
#if CONFIG_EXT_SEGMENT
          count_segs_sb(cm, xd, &tile_info, mi, spatial_predictor_counts,
                        temporal_predictor_count, unpred_seg_counts,
                        seg_cat_idx, mi_row, mi_col, cm->sb_size);
#else
...
#if CONFIG_EXT_SEGMENT
  {
    ...
    for (i = 0; i < SPATIAL_PRED_CTX; ++i) {
      av1_tree_probs_from_distribution(av1_seg_id_spatial_prediction_tree,
                                       branch_ct, spatial_predictor_counts[i]);

      for (j = 0; j < SPATIAL_PRED_PROBS; j++) {
        count0 = branch_ct[j][0];
        count1 = branch_ct[j][1];

        spatial_pred_prob[i][j] = get_binary_prob(count0, count1);
        av1_prob_diff_update_savings_search(
            branch_ct[i], segp->spatial_pred_probs[i][j],
            &spatial_pred_prob[i][j], DIFF_UPDATE_PROB, probwt);

        // Add in the predictor signaling cost
        t_pred_cost += count0 * av1_cost_zero(spatial_pred_prob[i][j]) +
                       count1 * av1_cost_one(spatial_pred_prob[i][j]);
      }
    }
    ...
    av1_copy(cm->counts.seg[seg_cat_idx].spatial_pred,
             spatial_predictor_counts);
  }
#else
