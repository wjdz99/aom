...
static void count_segs(const AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  {
    int i, j;
    int seg_map_minb_size = 1 << (cm->seg[seg_cat_idx].seg_map_minb_size_log2_minus3);
    ...
    const int seg_id_skip = (xd->mi[0]->mbmi.ref_frame > INTRA_FRAME ||
      !av1_is_segfeature_enabled(&cm->seg[QUALITY_SEG_IDX], QUALITY_SEG_LVL_ALT_LF)
      )
      && xd->mi[0]->mbmi.skip ? 1 : 0;
    ...
    if (xd->seg_id_is_done[seg_cat_idx] ||
      (!xd->seg_id_is_done[seg_cat_idx] && seg_id_skip && !last_block_seg_minb))
      return;
    if (!xd->seg_id_is_done[seg_cat_idx] && seg_id_skip && last_block_seg_minb) {
    ...
    if (!seg_id_skip) {
      ...
      if (cm->seg[seg_cat_idx].temporal_update) {        
        ...
        /*if (seg_cat_idx == QUALITY_SEG_IDX)
          printf("(%d,%d), tmpctx:%d, tmp_id_pred:%d\n", mi_row, mi_col, pred_context, pred_flag);*/
      }
      if (!pred_flag) {
        const int spatial_seg_pred_ctx = top_mi && left_mi &&
          top_mi->mbmi.segment_id[seg_cat_idx] == left_mi->mbmi.segment_id[seg_cat_idx] ? 1 : 0;
        if (spatial_seg_pred_ctx) {
          spatial_pred_idc = segment_id == left_mi->mbmi.segment_id[seg_cat_idx] ? SEG_ID_COPY_LEFT : SEG_ID_SPATIAL_UNPREDICTED;
          harmonic_pred_seg_counts[0][spatial_pred_idc]++;
          /*if (seg_cat_idx == QUALITY_SEG_IDX)
            printf("(%d,%d), spatialctx:%d, spatial_id_pred:%d\n", mi_row, mi_col, spatial_seg_pred_ctx, spatial_pred_idc);*/
        }
        else {
          if (left_mi)
            spatial_pred_idc = segment_id == left_mi->mbmi.segment_id[seg_cat_idx] ? SEG_ID_COPY_LEFT : SEG_ID_SPATIAL_UNPREDICTED;
          if (top_mi && spatial_pred_idc == SEG_ID_SPATIAL_UNPREDICTED)
            spatial_pred_idc = segment_id == top_mi->mbmi.segment_id[seg_cat_idx] ? SEG_ID_COPY_ABOVE : SEG_ID_SPATIAL_UNPREDICTED;
        ...
        if (spatial_pred_idc == SEG_ID_SPATIAL_UNPREDICTED) {
          ...
          /*if (seg_cat_idx == QUALITY_SEG_IDX)
            printf("(%d,%d), spatialctx:%d, spatial_id_pred:%d, seg_id:%d\n", mi_row, mi_col, spatial_seg_pred_ctx, spatial_pred_idc, segment_id);*/
        }
      }
      for (i = 0; i < AOMMIN(AOMMAX(mi_size_high[xd->mi[0]->mbmi.sb_type], seg_map_minb_size),
        cm->mi_rows - (mi_row & ~(seg_map_minb_size - 1))); i++) {
...
void av1_choose_segmap_coding_method(AV1_COMMON *cm, MACROBLOCKD *xd) {
  ...
  if (!frame_is_intra_only(cm) && !cm->error_resilient_mode) cm->seg[seg_cat_idx].temporal_update = 1;
  else 
