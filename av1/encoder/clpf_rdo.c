...
int av1_clpf_decision(int k, int l, const YV12_BUFFER_CONFIG *rec,
  ...
  *res = sum1 < sum0 + (sum0 >> 8);  // Slightly favour filtering
  return *res;
}
...
void av1_clpf_test_frame(const YV12_BUFFER_CONFIG *rec,
  ...
  const int bs = MI_SIZE >> (plane != AOM_PLANE_Y);
  ...
  for (j = 0; j < 4; j++) {
    ...
    double lambda = lambda_square[av1_get_qindex(&cm->seg, 0, cm->base_qindex)];
