...
void aom_clpf_detect_multi_c(const uint8_t *rec, const uint8_t *org,
  ...
  for (y = y0; y < y0 + 8; y++) {
    for (x = x0; x < x0 + 8; x++) {
      ...
      int E = rec[y * rstride + AOMMIN(width - 1, x + 2)];
      int F = rec[AOMMIN(height - 1, y + 1) * rstride + x];
      int delta1 = av1_clpf_sample(X, A, B, C, D, E, F, 4);
      int delta2 = av1_clpf_sample(X, A, B, C, D, E, F, 8);
      int delta3 = av1_clpf_sample(X, A, B, C, D, E, F, 16);
      int F1 = X + delta1;
      int F2 = X + delta2;
      int F3 = X + delta3;
      sum[0] += (O - X) * (O - X);
...
void aom_clpf_detect_multi_hbd_c(const uint16_t *rec, const uint16_t *org,
  ...
  for (y = y0; y < y0 + 8; y++) {
    for (x = x0; x < x0 + 8; x++) {
      ...
      int delta2 = av1_clpf_sample(X, A, B, C, D, E, F, 4);
      int delta3 = av1_clpf_sample(X, A, B, C, D, E, F, 16);
      int F1 = X + delta1;
      int F2 = X + delta2;
      int F3 = X + delta3;
      ...
      if (!cm->mi_grid_visible[ypos / bs * cm->mi_stride + xpos / bs]
        ...
        if (cm->use_highbitdepth) {
          aom_clpf_detect_hbd_c(CONVERT_TO_SHORTPTR(rec->y_buffer),
                                ...
                                rec->y_crop_width, rec->y_crop_height, &sum0,
                                &sum1, strength);
        } else {
...
#if CONFIG_AOM_HIGHBITDEPTH
        if (cm->use_highbitdepth) {
          aom_clpf_detect_multi_hbd_c(
              CONVERT_TO_SHORTPTR(rec->y_buffer),
              CONVERT_TO_SHORTPTR(org->y_buffer), rec->y_stride, org->y_stride,
              xpos, ypos, rec->y_crop_width, rec->y_crop_height, sum);
        } else {
