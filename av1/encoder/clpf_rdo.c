...
int av1_clpf_decision(int k, int l, const YV12_BUFFER_CONFIG *rec,
  ...
  const int subx = comp && rec->subsampling_x;
  const int suby = comp && rec->subsampling_y;
  uint8_t *rec_buffer =
...
static int clpf_rdo(int y, int x, const YV12_BUFFER_CONFIG *rec,
  ...
  if (!comp && fb_size_log2 > (unsigned int)get_msb(MAX_FB_SIZE) - 3) {
    ...
    res[i][1] = !i ? res[i][1] : AOMMIN(sum1 + res[i][0], res[i][1]);
    res[i][2] = !i ? res[i][2] : AOMMIN(sum2 + res[i][0], res[i][2]);
    res[i][3] = !i ? res[i][3] : AOMMIN(sum3 + res[i][0], res[i][3]);
...
void av1_clpf_test_frame(const YV12_BUFFER_CONFIG *rec,
  ...
  if (comp)  // Slightly favour unfiltered chroma
    sums[0][0] -= sums[0][0] >> 7;
