...
void aom_clpf_detect_c(const uint8_t *rec, const uint8_t *org, int rstride,
  ...
  for (y = y0; y < y0 + size; y++) {
    for (x = x0; x < x0 + size; x++) {
      ...
      int delta = av1_clpf_sample(X, A, B, C, D, E, F, G, H, I, J, K, L, strength);
      int Y = X + delta;
      *sum0 += (O - X) * (O - X);
      *sum1 += (O - Y) * (O - Y);
    }
...
void aom_clpf_detect_multi_c(const uint8_t *rec, const uint8_t *org,
  ...
  for (y = y0; y < y0 + size; y++) {
    for (x = x0; x < x0 + size; x++) {
      ...
      int delta1 = av1_clpf_sample(X, A, B, C, D, E, F, G, H, I, J, K, L, 1);
      int delta2 = av1_clpf_sample(X, A, B, C, D, E, F, G, H, J, K, L, 1, 2);
      int delta3 = av1_clpf_sample(X, A, B, C, D, E, F, G, H, J, K, L, 1, 4);
      int F1 = X + delta1;
      int F2 = X + delta2;
      int F3 = X + delta3;
      sum[0] += (O - X) * (O - X);
