...
int av1_clpf_decision(int k, int l, const YV12_BUFFER_CONFIG *rec,
  ...
  const int subx = plane && rec->subsampling_x;
  const int suby = plane && rec->subsampling_y;
  uint8_t *rec_buffer =
      plane ? (plane == 1 ? rec->u_buffer : rec->v_buffer) : rec->y_buffer;
  uint8_t *org_buffer =
      plane ? (plane == 1 ? org->u_buffer : org->v_buffer) : org->y_buffer;
  int rec_width = plane ? rec->uv_crop_width : rec->y_crop_width;
  int rec_height = plane ? rec->uv_crop_height : rec->y_crop_height;
  int rec_stride = plane ? rec->uv_stride : rec->y_stride;
  int org_stride = plane ? org->uv_stride : org->y_stride;
  for (m = 0; m < h; m++) {
    for (n = 0; n < w; n++) {
...
static int clpf_rdo(int y, int x, const YV12_BUFFER_CONFIG *rec,
  ...
  if (!plane && fb_size_log2 > (unsigned int)get_msb(MAX_FB_SIZE) - 3) {
...
void av1_clpf_test_frame(const YV12_BUFFER_CONFIG *rec,
  ...
  int width = plane ? rec->uv_crop_width : rec->y_crop_width;
  int height = plane ? rec->uv_crop_height : rec->y_crop_height;
  ...
  if (plane)  // Slightly favour unfiltered chroma
    sums[0][0] -= sums[0][0] >> 7;
