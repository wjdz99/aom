...
static void detect_clpf(const uint8_t *rec, const uint8_t *org, int x0, int y0,
                        ...
                        int *sum1, unsigned int strength, int size, int comp) {
  int x, y;
  for (y = y0; y < y0 + size; y++) {
    for (x = x0; x < x0 + size; x++) {
      ...
      int Y = X + delta;
      *sum0 += comp ? abs(O - X) : (O - X) * (O - X);
      *sum1 += comp ? abs(O - Y) : (O - Y) * (O - Y);
    }
  }
}
...
static void detect_multi_clpf(const uint8_t *rec, const uint8_t *org, int x0,
  ...
  for (y = y0; y < y0 + size; y++) {
    for (x = x0; x < x0 + size; x++) {
      ...
      int F2 = X + delta2;
      int F3 = X + delta3;
      sum[0] += comp ? abs(O - X) : (O - X) * (O - X);
      sum[1] += comp ? abs(O - F1) : (O - F1) * (O - F1);
      sum[2] += comp ? abs(O - F2) : (O - F2) * (O - F2);
      sum[3] += comp ? abs(O - F3) : (O - F3) * (O - F3);
    }
  }
}
    ...
    for (n = 0; n < w; n++) {
      ...
      if (!cm->mi_grid_visible[ypos / MAX_MIB_SIZE * cm->mi_stride +
        ...
        detect_clpf(rec_buffer, org_buffer, xpos, ypos, rec_width, rec_height,
                    org_stride, rec_stride, &sum0, &sum1, strength, size, comp);
    }
  }
  *res = sum1 < sum0;
  return *res;
    ...
    for (n = 0; n < w; n++) {
      ...
      if (!cm->mi_grid_visible[ypos / MAX_MIB_SIZE * cm->mi_stride +
        ...
        detect_multi_clpf(rec_buffer, org_buffer, xpos, ypos, rec_width,
                          rec_height, org_stride, rec_stride, sum, size, comp);
        filtered = 1;
      }
    }
  }
...
void av1_clpf_test_frame(const YV12_BUFFER_CONFIG *rec,
  ...
  for (j = 0; j < 4; j++) {
    ...
    int c, cost = (int)((1.2 * lambda * (sums[j][0] + 2 + 2 * (j > 0)) + 0.5));
    for (c = 0; c < 4; c++)
