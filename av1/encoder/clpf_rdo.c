...
static void clpf_rdo(const YV12_BUFFER_CONFIG *rec,
                     ...
                     unsigned int block_size, int w,
                     int h, int64_t res[4], int plane) {
  int m, n;
  int sum[4];
  ...
  for (m = 0; m < h; m++) {
	...
	if (cm->use_highbitdepth) {
	  aom_clpf_detect_multi_hbd(
            ...
            sum, block_size, cm->bit_depth, damping);
        } else {
...
void av1_clpf_test_frame(const YV12_BUFFER_CONFIG *rec,
  ...
  clpf_rdo(rec, org, cm, bs, width >> bslog,
