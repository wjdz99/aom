...
void aom_clpf_detect_c(const uint8_t *rec, const uint8_t *org, int x0, int y0,
...
void aom_clpf_detect_multi_c(const uint8_t *rec, const uint8_t *org, int x0,
  ...
  for (m = 0; m < h; m++) {
    for (n = 0; n < w; n++) {
      ...
      if (!cm->mi_grid_visible[ypos / bs * cm->mi_stride + xpos / bs]
               ->mbmi.skip)
        aom_clpf_detect(rec->y_buffer, org->y_buffer, xpos, ypos,
  ...
  for (m = 0; m < h; m++) {
    for (n = 0; n < w; n++) {
      ...
      if (!cm->mi_grid_visible[ypos / MI_BLOCK_SIZE * cm->mi_stride +
                               xpos / MI_BLOCK_SIZE]
               ->mbmi.skip) {
        aom_clpf_detect_multi(rec->y_buffer, org->y_buffer, xpos, ypos,
