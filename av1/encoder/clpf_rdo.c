...
static int clpf_rdo(int y, int x, const YV12_BUFFER_CONFIG *rec,
  ...
  if (plane == AOM_PLANE_Y &&
    ...
    filtered = fb_size_log2-- == 7;  // Always signal for fb_size == 128
  ...
  for (m = 0; m < h; m++) {
    for (n = 0; n < w; n++) {
      ...
      if (!cm->mi_grid_visible[(ypos << suby) / MI_SIZE * cm->mi_stride +
        ...
        aom_clpf_detect_multi(rec_buffer, org_buffer, rec_stride, org_stride,
                              xpos, ypos, rec_width, rec_height, sum + 4,
                              block_size);
    }
  }
...
void av1_clpf_test_frame(const YV12_BUFFER_CONFIG *rec,
  ...
  // When fb_size == 128, include skip blocks in the result.
  if (plane == AOM_PLANE_Y) {
    sums[1][1] = sums[1][5] + sums[0][0] - sums[1][4];
    sums[1][2] = sums[1][6] + sums[0][0] - sums[1][4];
    sums[1][3] = sums[1][7] + sums[0][0] - sums[1][4];
  }
