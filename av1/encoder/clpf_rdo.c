...
void aom_clpf_detect_multi_c(const uint8_t *rec, const uint8_t *org,
  ...
  for (y = y0; y < y0 + size; y++) {
    for (x = x0; x < x0 + size; x++) {
      ...
      const int G = rec[AOMMIN(height - 1, y + 1) * rstride + x];
      const int H = rec[AOMMIN(height - 1, y + 2) * rstride + x];
      const int delta1 = av1_clpf_sample(X, B, B, C, D, E, F, G, G, 1, dmp);
      const int delta2 = av1_clpf_sample(X, B, B, C, D, E, F, G, G, 2, dmp);
      const int delta3 = av1_clpf_sample(X, B, B, C, D, E, F, G, G, 4, dmp);
...
void aom_clpf_detect_multi_hbd_c(const uint16_t *rec, const uint16_t *org,
  ...
  for (y = y0; y < y0 + size; y++) {
    for (x = x0; x < x0 + size; x++) {
      ...
      const int H = rec[AOMMIN(height - 1, y + 2) * rstride + x] >> shift;
      const int delta1 =
          av1_clpf_sample(X, B, B, C, D, E, F, G, G, 1, dmp - shift);
      const int delta2 =
          av1_clpf_sample(X, B, B, C, D, E, F, G, G, 2, dmp - shift);
      const int delta3 =
          av1_clpf_sample(X, B, B, C, D, E, F, G, G, 4, dmp - shift);
      const int F1 = X + delta1;
      const int F2 = X + delta2;
      const int F3 = X + delta3;
      sum[0] += (O - X) * (O - X);
