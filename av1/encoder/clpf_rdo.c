...
static int clpf_rdo(int y, int x, const YV12_BUFFER_CONFIG *rec,
  ...
  if (fb_size_log2 > log2i(MAX_FB_SIZE) - 3) {
    int w1, h1, w2, h2, i, sum1, sum2, sum3, oldfiltered;

    fb_size_log2--;
    w1 = min(1 << (fb_size_log2 - bslog), w);
    h1 = min(1 << (fb_size_log2 - bslog), h);
    w2 = min(w - (1 << (fb_size_log2 - bslog)), w >> 1);
    h2 = min(h - (1 << (fb_size_log2 - bslog)), h >> 1);
    i = log2i(MAX_FB_SIZE) - fb_size_log2;
    sum1 = res[i][1];
    sum2 = res[i][2];
    sum3 = res[i][3];
    oldfiltered = res[i][0];
...
void clpf_test_frame(const YV12_BUFFER_CONFIG *rec,
  ...
  const int bs = MI_BLOCK_SIZE;
  int fb_size_log2 = log2i(MAX_FB_SIZE);

  memset(sums, 0, sizeof(sums));

  for (k = 0; k<(height + (1 << fb_size_log2) - bs)>> fb_size_log2; k++) {
    for (l = 0; l<(width + (1 << fb_size_log2) - bs)>> fb_size_log2; l++) {
