...
void av1_cdef_search(AV1_COMP *cpi) {
  ...
  if (cdef_control == CDEF_ADAPTIVE && cpi->oxcf.mode == ALLINTRA &&
    ...
    for (int j = 0; j < cdef_info->nb_cdef_strengths; j++) {
      ...
      int new_pri_luma_strength = (luma_strength / CDEF_SEC_STRENGTHS) >> 1;
      int new_sec_luma_strength = (luma_strength % CDEF_SEC_STRENGTHS) >> 1;
      int new_pri_chroma_strength = (chroma_strength / CDEF_SEC_STRENGTHS) >> 1;
      int new_sec_chroma_strength = (chroma_strength % CDEF_SEC_STRENGTHS) >> 1;
