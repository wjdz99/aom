...
// Fill the required neighboring regions of the block when it falls on frame
// boundary
static void fill_borders_for_fbs_on_frame_boundary(
    uint16_t *inbuf, int hsize, int vsize, bool is_fb_on_frm_left_boundary,
    bool is_fb_on_frm_right_boundary, bool is_fb_on_frm_top_boundary,
    bool is_fb_on_frm_bottom_boundary) {
  ...
  if (is_fb_on_frm_bottom_boundary) {
    ...
    const int buf_offset = (vsize + CDEF_VBORDER) * CDEF_BSTRIDE;
    fill_rect(&inbuf[buf_offset + CDEF_HBORDER], CDEF_BSTRIDE, CDEF_VBORDER,
  ...
  if (is_fb_on_frm_bottom_boundary || is_fb_on_frm_right_boundary) {
    ...
    fill_rect(&inbuf[buf_offset + hsize + CDEF_HBORDER], CDEF_BSTRIDE,
              CDEF_VBORDER, CDEF_HBORDER, CDEF_VERY_LARGE);
  }
  if (is_fb_on_frm_top_boundary) {
    // Fill top region of the block
    fill_rect(&inbuf[CDEF_HBORDER], CDEF_BSTRIDE, CDEF_VBORDER, hsize,
              CDEF_VERY_LARGE);
  }
  ...
  if (is_fb_on_frm_top_boundary || is_fb_on_frm_right_boundary) {
    ...
    fill_rect(&inbuf[hsize + CDEF_HBORDER], CDEF_BSTRIDE, CDEF_VBORDER,
  ...
  if (is_fb_on_frm_left_boundary) {
    ...
    fill_rect(&inbuf[buf_offset], CDEF_BSTRIDE, vsize, CDEF_HBORDER,
              CDEF_VERY_LARGE);
  }
  if (is_fb_on_frm_right_boundary) {
    const int buf_offset = CDEF_VBORDER * CDEF_BSTRIDE;
    // Fill right region of the block
    fill_rect(&inbuf[buf_offset + hsize + CDEF_HBORDER], CDEF_BSTRIDE, vsize,
              CDEF_HBORDER, CDEF_VERY_LARGE);
  }
}
...
void av1_cdef_mse_calc_block(CdefSearchCtx *cdef_search_ctx, int fbr, int fbc,
  ...
  for (int pli = 0; pli < cdef_search_ctx->num_planes; pli++) {
    ...
    const int ysize = (nvb << mi_high_l2[pli]) +
                      CDEF_VBORDER * (!is_fb_on_frm_bottom_boundary) + yoff;
    const int xsize = (nhb << mi_wide_l2[pli]) +
                      CDEF_HBORDER * (!is_fb_on_frm_right_boundary) + xoff;
    ...
    fill_borders_for_fbs_on_frame_boundary(
        inbuf, nhb << mi_wide_l2[pli], nvb << mi_high_l2[pli],
        is_fb_on_frm_left_boundary, is_fb_on_frm_right_boundary,
        is_fb_on_frm_top_boundary, is_fb_on_frm_bottom_boundary);
