...
static INLINE uint64_t mse_8x8_16bit(uint16_t *dst, int dstride,
...
static INLINE uint64_t mse_4x4_16bit(uint16_t *dst, int dstride,
                                     uint16_t *src, int sstride) {
  uint64_t sum = 0;
  int i, j;
  for (i = 0; i < 4; i++) {
      ...
      for (pli = 0; pli < nplanes; pli++) {
        ...
        for (gi = 0; gi < TOTAL_STRENGTHS; gi++) {
          ...
          od_dering(tmp_dst, in, dec[pli], dir, &dirinit, var, pli, dlist,
                    ...
                    coeff_shift);
          mse[pli][sb_count][gi] = compute_dering_mse(ref_coeff[pli] +
              (sbr * MAX_MIB_SIZE << bsize[pli])*stride[pli] +
              (sbc * MAX_MIB_SIZE << bsize[pli]), stride[pli],
              tmp_dst, dlist, dering_count, bsize[pli], coeff_shift);
          sb_index[sb_count] =
