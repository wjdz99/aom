...
static void copy_sb16_16_8bit(uint16_t *dst, int dstride, const void *src,
  ...
  for (r = 0; r < vsize; r++)
    for (c = 0; c < hsize; c++) dst[r * dstride + c] = base[r * sstride + c];
}
...
static INLINE uint64_t mse_8x8_16bit(void *dst, int dstride, uint16_t *src,
                                     int sstride, int use_highbd) {
  uint64_t sum = 0;
  int i, j;
  if (use_highbd) {
    uint16_t *dst_buff = dst;
    for (i = 0; i < 8; i++) {
      for (j = 0; j < 8; j++) {
        int e = dst_buff[i * dstride + j] - src[i * sstride + j];
        sum += e * e;
      }
    }
  } else {
    ...
    for (i = 0; i < 8; i++) {
      for (j = 0; j < 8; j++) {
        int e = (uint16_t)dst_buff[i * dstride + j] - src[i * sstride + j];
        sum += e * e;
      }
    }
  }
...
static INLINE uint64_t mse_4x4_16bit(void *dst, int dstride, uint16_t *src,
                                     int sstride, int use_highbd) {
  uint64_t sum = 0;
  int i, j;
  if (use_highbd) {
  ...
  if (use_highbd) {
    ...
    if (bsize == BLOCK_8X8) {
      for (bi = 0; bi < cdef_count; bi++) {
        ...
        bx = dlist[bi].bx;
        if (pli == 0) {
          sum += mse_8x8_16bit(&dst_buff[(by << 3) * dstride + (bx << (3))],
                               dstride, &src[bi << (3 + 3)], 8, use_highbd);
        } else {
          sum += mse_8x8_16bit(&dst_buff[(by << 3) * dstride + (bx << (3))],
                               dstride, &src[bi << (3 + 3)], 8, use_highbd);
        }
      ...
      for (bi = 0; bi < cdef_count; bi++) {
        by = dlist[bi].by;
        bx = dlist[bi].bx;
        sum += mse_4x4_16bit(&dst_buff[(by << 3) * dstride + (bx << (2))],
                             dstride, &src[bi << (3 + 2)], 4, use_highbd);
        sum += mse_4x4_16bit(&dst_buff[((by << 3) + 4) * dstride + (bx << (2))],
      ...
      for (bi = 0; bi < cdef_count; bi++) {
        by = dlist[bi].by;
        bx = dlist[bi].bx;
        sum += mse_4x4_16bit(&dst_buff[(by << 2) * dstride + (bx << (3))],
                             dstride, &src[bi << (2 + 3)], 8, use_highbd);
        sum += mse_4x4_16bit(&dst_buff[(by << 2) * dstride + (bx << (3)) + 4],
                             dstride, &src[(bi << (2 + 3)) + 4], 8, use_highbd);
      }
    } else {
      ...
      for (bi = 0; bi < cdef_count; bi++) {
        by = dlist[bi].by;
        bx = dlist[bi].bx;
        sum += mse_4x4_16bit(&dst_buff[(by << 2) * dstride + (bx << (2))],
                             dstride, &src[bi << (2 + 2)], 4, use_highbd);
      }
    }
  } else {
    uint8_t *dst_buff = dst;
        ...
        for (int gi = 0; gi < total_strengths; gi++) {
          ...
          if (cm->seq_params.use_highbitdepth) {
            curr_mse = compute_cdef_dist(
                &(CONVERT_TO_SHORTPTR(ref_buffer)[row * ref_stride + col]),
                ref_stride, tmp_dst, dlist, cdef_count, bsize[pli], coeff_shift,
                pli, cm->seq_params.use_highbitdepth);
          } else {
            curr_mse = compute_cdef_dist(&(ref_buffer)[row * ref_stride + col],
                                         ref_stride, tmp_dst, dlist, cdef_count,
                                         bsize[pli], coeff_shift, pli,
