      ...
      for (int pli = 0; pli < num_planes; pli++) {
        ...
        for (int gi = 0; gi < total_strengths; gi++) {
          ...
          av1_cdef_filter_fb(
              ...
              sec_strength + (sec_strength == 3), damping, coeff_shift, 1);
#else
          av1_cdef_filter_fb(
              NULL, tmp_dst, CDEF_BSTRIDE, in, xdec[pli], ydec[pli], dir,
              &dirinit, var, pli, dlist, cdef_count, pri_strength,
...
#if CONFIG_CC_CDEF
void av1_cc_cdef_reset(AV1_COMMON *cm) {
  ...
  for (int plane = 0; plane < 2; plane++) {
    for (int dir = 0; dir < MAX_NUMBER_OF_DIRECTIONS; dir++) {
      ...
      cm->cdef_info.cc_cdef_info.cccdef_frame_filter_idx_in_buf[plane][dir] = 0;
    }
  }
}

void av1_cc_cdef_generate_filter_coeff(AV1_COMMON *cm,
                                       const YV12_BUFFER_CONFIG *frame,
                                       const YV12_BUFFER_CONFIG *ref,
                                       MACROBLOCKD *xd,
                                       CDEF_PICK_METHOD pick_method,
                                       int16_t **new_filter_tmp_buf,
                                       int16_t num_new_filters_test) {
  const int num_planes = av1_num_planes(cm);
  ...
  for (int fbr = 0; fbr < nvfb; ++fbr) {
    for (int fbc = 0; fbc < nhfb; ++fbc) {
      ...
      /* We avoid filtering the pixels for which some of the pixels to
                                      average are outside the frame. We could
         change the filter instead, but it would add special cases for any
  ...
  for (int test_filter_idx = 0; test_filter_idx < num_new_filters_test;
       test_filter_idx++) {
    int idx = test_filter_idx * MAX_NUMBER_OF_DIRECTIONS *
              MAX_NUMBER_OF_CCCDEF_FILTER_COEFF;
    int16_t *dst0 = &new_filter_tmp_buf[0][idx];
    // process Cb
    get_frame_filter_stats(cm, filter_block_stats[0], frame_stats[0], 0,
                           test_filter_idx);
    derive_filter_coeffs_from_stats(frame_stats[0], dst0, 0);

    // Process Cr
    int16_t *dst1 = &new_filter_tmp_buf[1][idx];
    get_frame_filter_stats(cm, filter_block_stats[1], frame_stats[1], 0,
                           test_filter_idx);
    derive_filter_coeffs_from_stats(frame_stats[1], dst1, 0);
  }
...
void av1_cc_cdef_search_filter_coeff(AV1_COMMON *cm,
                                     const YV12_BUFFER_CONFIG *frame,
                                     const YV12_BUFFER_CONFIG *ref,
                                     MACROBLOCKD *xd,
                                     CDEF_PICK_METHOD pick_method, int rdmult,
                                     int key_freq_max, int key_freq_min,
                                     int16_t **new_filter_tmp_buf,
                                     int16_t num_new_filters_test) {
  const int num_planes = av1_num_planes(cm);
  ...
  uint64_t frame_distortion[MAX_NUMBER_OF_DIRECTIONS]
                           [2 * (MAX_NUMBER_OF_TEMPORAL_FILTERS +
                                 MAX_NEW_FILTERS_ENC_SEARCH)];
  ...
  for (int dir = 0; dir < number_of_directions; dir++) {
    ...
    number_of_filter_sets[0][dir] =
        (num_new_filters_test + coeff_buffer->curr_number_of_filter_sets[0]);
    number_of_filter_sets[1][dir] =
        (num_new_filters_test + coeff_buffer->curr_number_of_filter_sets[1]);
      ...
      for (int pli = 1; pli < num_planes; pli++) {
        ...
        for (int dir = 0; dir < number_of_directions; dir++) {
          ...
          for (int filter_set = 0;
               ...
               filter_set++) {
            const uint8_t use_history =
                (filter_set < coeff_buffer->curr_number_of_filter_sets[pli - 1])
                    ? 1
                    : 0;
            int new_filter_idx =
                filter_set - coeff_buffer->curr_number_of_filter_sets[pli - 1];
            CHECKCDEF(!use_history && new_filter_idx < 0,
                      " index computation is not correct");

            if (new_filter_idx < 0) new_filter_idx = 0;

            const int16_t *const filter_coeff =
                (!use_history)
                    ? &new_filter_tmp_buf[pli - 1][new_filter_idx *
                                                   num_filter_coeff *
            ...
            const int single_direction_coeff = (use_history) ? 1 : 0;
  ...
  for (int uv = 0; uv < 2; uv++) {
    for (int dir = 0; dir < number_of_directions; dir++) {
      ...
      int best_filter_set_idx =
          coeff_buffer->curr_number_of_filter_sets[uv];  // initially set the
                                                         // new filer as best
      ...
      for (int filter_set = 0; filter_set <= (number_of_filter_sets[uv][dir]);
        ...
        const uint8_t use_history =
            (filter_set < coeff_buffer->curr_number_of_filter_sets[uv]) ? 1 : 0;

        if (filter_set != number_of_filter_sets[uv][dir]) {
          frame_bits++;  // 1 bit to signal signal new filter flag
          if (!use_history) {
            int new_filter_idx =
                filter_set - coeff_buffer->curr_number_of_filter_sets[uv];
            CHECKCDEF(new_filter_idx < 0, " The index is not correct ");

            const int16_t *const filter_coeff =
                &new_filter_tmp_buf[uv][(new_filter_idx * num_filter_coeff *
                                         number_of_directions) +
                                        (dir * num_filter_coeff)];
      ...
      int16_t *filter_coeff_dir =
          &new_filter_tmp_buf[uv]
                             [(0 * num_filter_coeff * number_of_directions) +
                              (dir * num_filter_coeff)];
      ...
      if (force_new_filter)
        best_filter_set_idx = (coeff_buffer->curr_number_of_filter_sets[uv]);

      if (!at_least_one_coeff_nonzero)
        best_filter_set_idx =
            number_of_filter_sets[uv][dir];  // filtering is OFF
      ...
      if (best_filter_set_idx == number_of_filter_sets[uv][dir]) {
        ...
        memcpy(dst, src,
               num_filter_coeff *
                   sizeof(cdef_info->cc_cdef_info.filter_coeff[uv][0]));
...
void av1_cc_cdef_search(const YV12_BUFFER_CONFIG *frame,
  ...
  int16_t num_new_filters_test = MAX_NEW_FILTERS_ENC_SEARCH;
  int16_t *new_filter_tmp_buf[2];

  new_filter_tmp_buf[0] = aom_malloc(
      num_new_filters_test * number_of_directions * num_filter_coeff *
      sizeof(cdef_info->cc_cdef_info.filter_coeff[0][0]));
  new_filter_tmp_buf[1] = aom_malloc(
      num_new_filters_test * number_of_directions * num_filter_coeff *
      sizeof(cdef_info->cc_cdef_info.filter_coeff[1][0]));

  av1_cc_cdef_generate_filter_coeff(cm, frame, ref, xd, pick_method,
                                    new_filter_tmp_buf, num_new_filters_test);

  av1_cc_cdef_search_filter_coeff(cm, frame, ref, xd, pick_method, rdmult,
  ...
  aom_free(new_filter_tmp_buf[0]);
  aom_free(new_filter_tmp_buf[1]);
...
void av1_get_filter_statstics_64x64(
  ...
  for (bi = 0; bi < cdef_count; bi++) {
    ...
    CHECKCDEF(dir[by][bx] >= MAX_NUMBER_OF_DIRECTIONS,
...
#if CCCDEF_LONG_TAP
      diffs[8] = (double)in[coll_luma_pos - 4];
      diffs[9] = (double)in[coll_luma_pos - 3];
      diffs[10] = (double)in[coll_luma_pos + 3];
      diffs[11] = (double)in[coll_luma_pos + 4];

#if VERTICAL_DIAGONAL_EXT == 1
      diffs[12] = (double)in[coll_luma_pos - 4 * instride];
      diffs[13] = (double)in[coll_luma_pos - 3 * instride];
      diffs[14] = (double)in[coll_luma_pos + 3 * instride];
      diffs[15] = (double)in[coll_luma_pos + 4 * instride];
#elif VERTICAL_DIAGONAL_EXT == 2
      diffs[12] = (double)in[coll_luma_pos - instride - 1];
      diffs[13] = (double)in[coll_luma_pos - instride + 1];
      diffs[14] = (double)in[coll_luma_pos + instride - 1];
      diffs[15] = (double)in[coll_luma_pos + instride + 1];
#endif
#endif
...
void get_frame_filter_stats(
  ...
  for (int fbr = 0; fbr < nvfb; ++fbr) {
    for (int fbc = 0; fbc < nhfb; ++fbc) {
      ...
      if (test_filter_idx == 1) {
        if (fbr >= (nvfb >> 1) || fbc >= (nhfb >> 1)) {
          fb_count++;
          continue;
        }
      } else if (test_filter_idx == 2) {
        if (fbr >= (nvfb >> 1) || fbc < (nhfb >> 1)) {
          fb_count++;
          continue;
        }
      } else if (test_filter_idx == 3) {
        if (fbr < (nvfb >> 1) || fbc > (nhfb >> 1)) {
          fb_count++;
          continue;
        }
      } else if (test_filter_idx == 4) {
        if (fbr < (nvfb >> 1) || fbc < (nhfb >> 1)) {
          fb_count++;
          continue;
        }
...
void derive_filter_coeffs_from_stats(
    ...
    int16_t f[MAX_NUMBER_OF_CCCDEF_FILTER_COEFF * MAX_NUMBER_OF_DIRECTIONS],
    int direction_merge) {
...
int derive_ccdef_filter_coefficients(
  ...
  if (matrix_decomposition(auto_correlation, upper_matrix, number_of_coeff)) {
    ...
    if (!success) {
      memset(filter_coefficients, 0,
             sizeof(filter_coefficients[0]) * number_of_coeff);
      return 0;
    }
...
int get_ccdef_coeff_golomb_length(int16_t coeff) {
  int x = coeff + 1;
  int i = x;
  int length = 0;
...
int get_ccdef_coeff_total_bit_length(const int16_t *const coeff,
