...
static INLINE bool should_calc_error_using_8bit_data(
    ...
    int tot_count, uint8_t use_highbitdepth, BLOCK_SIZE plane_bsize) {
...
void av1_cdef_mse_calc_block(CdefSearchCtx *cdef_search_ctx, int fbr, int fbc,
  ...
  const int cdef_count =
      av1_cdef_compute_sb_list(mi_params, fbr * MI_SIZE_64X64,
                               fbc * MI_SIZE_64X64, dlist, &tot_count, bs);
  ...
  for (int pli = 0; pli < cdef_search_ctx->num_planes; pli++) {
    ...
    for (int gi = 0; gi < cdef_search_ctx->total_strengths; gi++) {
      ...
      if (should_calc_error_using_8bit_data(
              &pd, row, col, cdef_count, tot_count,
        ...
        if (pri_strength == 0 && sec_strength == 0) {
          unused = cpi->ppi->fn_ptr[plane_bsize].vf(
              &ref_buffer[pli]
                         [get_offset_from_fullmv(&this_mv, ref_stride[pli])],
          ...
          av1_cdef_filter_fb(
              tmp_dst8, NULL, CDEF_BSTRIDE, in, cdef_search_ctx->xdec[pli],
          ...
          unused = cpi->ppi->fn_ptr[plane_bsize].vf(
              &ref_buffer[pli]
                         [get_offset_from_fullmv(&this_mv, ref_stride[pli])],
              ref_stride[pli], tmp_dst8, CDEF_BSTRIDE, &curr_uint_mse);
        }
        (void)unused;
