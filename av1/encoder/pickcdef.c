...
static uint64_t compute_cdef_dist_highbd(void *dst, int dstride, uint16_t *src,
  ...
  int bi, bx, by;
  uint16_t *dst_buff =
      &(CONVERT_TO_SHORTPTR((uint8_t *)dst)[row * dstride + col]);
  if (bsize == BLOCK_8X8) {
    for (bi = 0; bi < cdef_count; bi++) {
      by = dlist[bi].by;
      bx = dlist[bi].bx;
      sum += mse_wxh_16bit_highbd(&dst_buff[(by << 3) * dstride + (bx << 3)],
                                  dstride, &src[bi << (3 + 3)], 8, 8, 8);
    }
  } else if (bsize == BLOCK_4X8) {
    for (bi = 0; bi < cdef_count; bi++) {
      by = dlist[bi].by;
      bx = dlist[bi].bx;
      sum += mse_wxh_16bit_highbd(&dst_buff[(by << 3) * dstride + (bx << 2)],
                                  dstride, &src[bi << (3 + 2)], 4, 4, 4);
      sum +=
          mse_wxh_16bit_highbd(&dst_buff[((by << 3) + 4) * dstride + (bx << 2)],
                               dstride, &src[(bi << (3 + 2)) + 4 * 4], 4, 4, 4);
    }
  } else if (bsize == BLOCK_8X4) {
    for (bi = 0; bi < cdef_count; bi++) {
      by = dlist[bi].by;
      bx = dlist[bi].bx;
      sum += mse_wxh_16bit_highbd(&dst_buff[(by << 2) * dstride + (bx << 3)],
                                  dstride, &src[bi << (2 + 3)], 8, 4, 4);
      sum +=
          mse_wxh_16bit_highbd(&dst_buff[(by << 2) * dstride + (bx << 3) + 4],
                               dstride, &src[(bi << (2 + 3)) + 4], 8, 4, 4);
    }
  } else {
    ...
    for (bi = 0; bi < cdef_count; bi++) {
      by = dlist[bi].by;
      bx = dlist[bi].bx;
      sum += mse_wxh_16bit_highbd(&dst_buff[(by << 2) * dstride + (bx << 2)],
                                  dstride, &src[bi << (2 + 2)], 4, 4, 4);
    }
  }
...
static uint64_t compute_cdef_dist(void *dst, int dstride, uint16_t *src,
  ...
  uint64_t sum = 0;
  int bi, bx, by;
  uint8_t *dst_buff = &((uint8_t *)dst)[row * dstride + col];
  if (bsize == BLOCK_8X8) {
    for (bi = 0; bi < cdef_count; bi++) {
      by = dlist[bi].by;
      bx = dlist[bi].bx;
      sum += mse_wxh_16bit(&dst_buff[(by << 3) * dstride + (bx << 3)], dstride,
                           &src[bi << (3 + 3)], 8, 8, 8);
    }
  } else if (bsize == BLOCK_4X8) {
    for (bi = 0; bi < cdef_count; bi++) {
      by = dlist[bi].by;
      bx = dlist[bi].bx;
      sum += mse_wxh_16bit(&dst_buff[(by << 3) * dstride + (bx << 2)], dstride,
                           &src[bi << (3 + 2)], 4, 4, 4);
      sum += mse_wxh_16bit(&dst_buff[((by << 3) + 4) * dstride + (bx << 2)],
                           dstride, &src[(bi << (3 + 2)) + 4 * 4], 4, 4, 4);
    }
  } else if (bsize == BLOCK_8X4) {
    for (bi = 0; bi < cdef_count; bi++) {
      by = dlist[bi].by;
      bx = dlist[bi].bx;
      sum += mse_wxh_16bit(&dst_buff[(by << 2) * dstride + (bx << 3)], dstride,
                           &src[bi << (2 + 3)], 8, 4, 4);
      sum += mse_wxh_16bit(&dst_buff[(by << 2) * dstride + (bx << 3) + 4],
                           dstride, &src[(bi << (2 + 3)) + 4], 8, 4, 4);
    }
  } else {
    assert(bsize == BLOCK_4X4);
    for (bi = 0; bi < cdef_count; bi++) {
      by = dlist[bi].by;
      bx = dlist[bi].bx;
      sum += mse_wxh_16bit(&dst_buff[(by << 2) * dstride + (bx << 2)], dstride,
                           &src[bi << (2 + 2)], 4, 4, 4);
    }
  }
  return sum >> 2 * coeff_shift;
    ...
    for (int fbc = 0; fbc < nhfb; ++fbc) {
      ...
      for (int pli = 0; pli < num_planes; pli++) {
        ...
        for (int gi = 0; gi < total_strengths; gi++) {
          ...
          uint64_t curr_mse;
          curr_mse = compute_cdef_dist_fn(ref_buffer, ref_stride, tmp_dst,
                                          dlist, cdef_count, bsize[pli],
                                          coeff_shift, row, col);
          if (pli < 2)
