...
void av1_cdef_search(struct AV1_COMP *cpi, const int is_screen_content) {
  ...
  int rtc_ext_rc = cpi->rc.rtc_external_ratectrl;
  if (rtc_ext_rc) {
  ...
  if (pick_method == CDEF_PICK_FROM_Q) {
    av1_pick_cdef_from_qp(cm, cpi->sf.rt_sf.skip_cdef_sb, is_screen_content);
    return;
  }
  const CommonModeInfoParams *const mi_params = &cm->mi_params;
  ...
  if (!cpi->cdef_search_ctx)
    CHECK_MEM_ERROR(cm, cpi->cdef_search_ctx,
                    aom_malloc(sizeof(CdefSearchCtx)));
  ...
  if (cpi->mt_info.num_workers > 1) {
    av1_cdef_mse_calc_frame_mt(cm, &cpi->mt_info);
  } else {
