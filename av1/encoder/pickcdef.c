...
// search method
static INLINE void get_cdef_filter_strength(CDEF_PICK_METHOD pick_method,
                                            int *pri_strength,
                                            int *sec_strength,
                                            int strength_idx) {
  ...
  switch (pick_method) {
    ...
    case CDEF_FAST_SEARCH_LVL3:
      ...
      *sec_strength = secconv_lvl3[sec_idx];
      break;
    default: assert(0 && "Invalid CDEF pick method");
  }
}
...
void av1_cdef_search(YV12_BUFFER_CONFIG *frame, const YV12_BUFFER_CONFIG *ref,
    ...
    for (int fbc = 0; fbc < nhfb; ++fbc) {
      ...
      for (int pli = 0; pli < num_planes; pli++) {
        ...
        for (int gi = 0; gi < total_strengths; gi++) {
          ...
          get_cdef_filter_strength(pick_method, &pri_strength, &sec_strength,
                                   gi);
  ...
  if (fast) {
    for (int j = 0; j < cdef_info->nb_cdef_strengths; j++) {
      ...
      int pri_strength, sec_strength;
      // get luma primary and secondary filter strength
      get_cdef_filter_strength(pick_method, &pri_strength, &sec_strength,
                               luma_strength);
      cdef_info->cdef_strengths[j] =
          pri_strength * CDEF_SEC_STRENGTHS + sec_strength;

      // get chroma primary and secondary filter strength
      get_cdef_filter_strength(pick_method, &pri_strength, &sec_strength,
                               chroma_strength);
