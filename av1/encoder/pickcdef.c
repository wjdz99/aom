...
static uint64_t compute_cdef_dist_highbd(void *dst, int dstride, uint16_t *src,
  ...
  const int src_stride = block_size_wide[bsize];
  const int width = block_size_wide[bsize];
  const int height = block_size_high[bsize];
  const int width_log2 = MI_SIZE_LOG2 + mi_size_wide_log2[bsize];
  const int height_log2 = MI_SIZE_LOG2 + mi_size_wide_log2[bsize];
      ...
      for (int pli = 0; pli < num_planes; pli++) {
        ...
        for (int gi = 0; gi < total_strengths; gi++) {
          ...
          const uint64_t curr_mse = compute_cdef_dist_fn(
              ref_buffer, ref_stride, tmp_dst, dlist, cdef_count, bsize[pli],
              coeff_shift, row, col);
          if (pli < 2)
