...
static INLINE bool should_calc_error_using_8bit_data(
    ...
    int tot_8x8_count, uint8_t use_highbitdepth, BLOCK_SIZE plane_bsize) {
  return (row + block_size_high[plane_bsize] <= pd->dst.height &&
          col + block_size_wide[plane_bsize] <= pd->dst.width &&
          cdef_count == tot_8x8_count && !use_highbitdepth);
}
...
static INLINE uint64_t get_filt_error(
    ...
    int cdef_count, int tot_8x8_count, int pli, int coeff_shift,
    BLOCK_SIZE bs) {
  uint64_t curr_mse;
  const AV1_COMP *const cpi = (AV1_COMP *)cdef_search_ctx->cpi;
  ...
  const bool calc_error_using_8bit_data = should_calc_error_using_8bit_data(
      pd, row, col, cdef_count, tot_8x8_count,
  ...
  if (calc_error_using_8bit_data) {
    ...
    const buf_offset = get_offset_from_fullmv(&this_mv, ref_stride);
...
void av1_cdef_mse_calc_block(CdefSearchCtx *cdef_search_ctx, int fbr, int fbc,
  ...
  const int cdef_count =
      av1_cdef_compute_sb_list(mi_params, fbr * MI_SIZE_64X64,
                               fbc * MI_SIZE_64X64, dlist, &tot_8x8_count, bs);
  ...
  for (int pli = 0; pli < cdef_search_ctx->num_planes; pli++) {
    ...
    for (int gi = 0; gi < cdef_search_ctx->total_strengths; gi++) {
      ...
      const uint64_t curr_mse = get_filt_error(
          ...
          tot_8x8_count, pli, coeff_shift, bs);
      if (pli < 2)
