...
// Returns if error can be calculated using 8-bit pixel data
static INLINE bool calc_error_using_8bit_data(
    const struct macroblockd_plane *pd, int pri_strength, int sec_strength,
    int row, int col, int cdef_count, int tot_count, uint8_t use_highbitdepth,
    BLOCK_SIZE plane_bsize) {
  return (pri_strength == 0 && sec_strength == 0 &&
          row + block_size_high[plane_bsize] <= pd->dst.height &&
          col + block_size_wide[plane_bsize] <= pd->dst.width &&
          cdef_count == tot_count && use_highbitdepth);
}
...
void av1_cdef_mse_calc_block(CdefSearchCtx *cdef_search_ctx, int fbr, int fbc,
  ...
  const int cdef_count =
      av1_cdef_compute_sb_list(mi_params, fbr * MI_SIZE_64X64,
                               fbc * MI_SIZE_64X64, dlist, bs, &tot_count);
  ...
  for (int pli = 0; pli < cdef_search_ctx->num_planes; pli++) {
    ...
    for (int gi = 0; gi < cdef_search_ctx->total_strengths; gi++) {
      ...
      get_cdef_filter_strengths(cdef_search_ctx->pick_method, &pri_strength,
                                &sec_strength, gi);
      AV1_COMP *cpi = (AV1_COMP *)cdef_search_ctx->cpi;
      const BLOCK_SIZE plane_bsize =
          get_plane_block_size(bs, pd.subsampling_x, pd.subsampling_y);
      if (calc_error_using_8bit_data(
              &pd, pri_strength, sec_strength, row, col, cdef_count, tot_count,
              cpi->common.seq_params->use_highbitdepth, plane_bsize)) {
        unsigned int curr_uint_mse;
        int unused;
        FULLPEL_MV this_mv = { row, col };
        unused = cpi->ppi->fn_ptr[plane_bsize].vf(
            &ref_buffer[pli][get_offset_from_fullmv(&this_mv, ref_stride[pli])],
            ref_stride[pli], get_buf_from_fullmv(&pd.dst, &this_mv),
            pd.dst.stride, &curr_uint_mse);
        curr_mse = curr_uint_mse;
      } else {
        av1_cdef_filter_fb(
            NULL, tmp_dst, CDEF_BSTRIDE, in, cdef_search_ctx->xdec[pli],
            cdef_search_ctx->ydec[pli], dir, &dirinit, var, pli, dlist,
            cdef_count, pri_strength, sec_strength + (sec_strength == 3),
...
// Inputs:
//   cpi: Pointer to top level encoder structure
...
// Returns:
//   Nothing will be returned. Contents of cdef_search_ctx will be modified.
static AOM_INLINE void cdef_params_init(AV1_COMP *cpi,
...
void av1_cdef_search(AV1_COMP *cpi, MACROBLOCKD *xd,
                     const int is_screen_content) {
  const CDEF_CONTROL cdef_control = cpi->oxcf.tool_cfg.cdef_control;
  assert(cdef_control != CDEF_NONE);
  const int non_reference_frame = cpi->rtc_ref.non_reference_frame;
  ...
  const CDEF_PICK_METHOD pick_method = cpi->sf.lpf_sf.cdef_pick_method;

  if (pick_method == CDEF_PICK_FROM_Q) {
    const int skip_cdef_feature = cpi->sf.rt_sf.skip_cdef_sb;
    pick_cdef_from_qp(cm, skip_cdef_feature, is_screen_content);
    return;
  }
  MultiThreadInfo *const mt_info = &cpi->mt_info;
  const YV12_BUFFER_CONFIG *const frame = &cm->cur_frame->buf;
  const YV12_BUFFER_CONFIG *const ref = cpi->source;
  const int rdmult = cpi->td.mb.rdmult;
  const CommonModeInfoParams *const mi_params = &cm->mi_params;
  ...
  cdef_params_init(cpi, frame, ref, cm, xd, &cdef_search_ctx, pick_method);
