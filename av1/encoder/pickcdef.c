...
#define REDUCED_PRI_STRENGTHS 4
#define REDUCED_TOTAL_STRENGTHS (REDUCED_PRI_STRENGTHS * CDEF_SEC_STRENGTHS)
#define TOTAL_STRENGTHS (CDEF_PRI_STRENGTHS * CDEF_SEC_STRENGTHS)

//static int priconv[REDUCED_PRI_STRENGTHS] =
//    { 0, 1, 2, 3, 4, 7, 12, 25 };
static int priconv[REDUCED_PRI_STRENGTHS] = { 0, 2, 9, 19 };
...
static uint64_t joint_strength_search(int *best_lev, int nb_strengths,
  ...
  if (!fast) {
    for (i = 0; i < 4 * nb_strengths; i++) {
      ...
      best_tot_mse = search_one(best_lev, nb_strengths - 1, mse, sb_count, fast);
    }
  }
  return best_tot_mse;
}
...
static uint64_t joint_strength_search_dual(int *best_lev0, int *best_lev1,
  ...
  /* Greedy search: add one strength options at a time. */
  for (i = 0; i < nb_strengths; i++) {
    best_tot_mse = search_one_dual(best_lev0, best_lev1, i, mse, sb_count, fast);
  }
  /* Trying to refine the greedy search by reconsidering each
     already-selected option. */
  for (i = 0; i < 4 * nb_strengths; i++) {
    ...
    best_tot_mse =
      search_one_dual(best_lev0, best_lev1, nb_strengths - 1, mse, sb_count, fast);
  }
  return best_tot_mse;
}
...
void av1_cdef_search(YV12_BUFFER_CONFIG *frame, const YV12_BUFFER_CONFIG *ref,
  ...
  uint64_t tot_mse;
  int sb_count;
  int nvfb = (cm->mi_rows + fb_mib_size - 1) / fb_mib_size;
  int nhfb = (cm->mi_cols + fb_mib_size - 1) / fb_mib_size;
  ...
  for (fbr = 0; fbr < nvfb; ++fbr) {
    for (fbc = 0; fbc < nhfb; ++fbc) {
      ...
      cm->mi_grid_visible[fb_mib_size * fbr * cm->mi_stride + fb_mib_size * fbc]
          ->mbmi.cdef_strength = -1;
      if (sb_all_skip(cm, fbr * fb_mib_size, fbc * fb_mib_size)) continue;
      cdef_count = sb_compute_cdef_list(cm, fbr * fb_mib_size,
                                        fbc * fb_mib_size, dlist, 1);
      for (pli = 0; pli < nplanes; pli++) {
        ...
        for (gi = 0; gi < total_strengths; gi++) {
          ...
          curr_mse = compute_cdef_dist(
              ref_coeff[pli] +
                  (fbr * fb_mib_size << mi_high_l2[pli]) * stride[pli] +
          ...
          sb_index[sb_count] =
              fb_mib_size * fbr * cm->mi_stride + fb_mib_size * fbc;
        }
      }
      sb_count++;
    }
  ...
  for (i = 0; i <= 3; i++) {
    ...
    if (nplanes >= 3)
      ...
      tot_mse =
        joint_strength_search(best_lev0, nb_strengths, mse[0], sb_count, fast);
