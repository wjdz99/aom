...
/*
  ...
  The array "params_by_motion" should be length 8 * num_motions, where the first
  2 slots in each group of 8 parameters are the translation parameters in (row,
  col) order, and the remaining slots correspond to values in the transformation
  matrix of the corresponding motion model. They are arranged in "params" such
  that values on the tx-matrix diagonal have odd numbered indices so the
  folowing matrix: A | B C | D would produce params = [trans row, trans col, B,
  A, C, D].
  "num_inliers" should be length "num_motions", and will be populated with the
  number of inlier feature points for each motion. Params corresponding to
  motions with 0 inliers should be ignored.
*/
