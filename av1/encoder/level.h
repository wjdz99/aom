...
typedef struct {
  ...
  // the smooth buffer. Removal time is essentially the time when the decoding
  // of the frame starts.
  double removal_time;
} CODED_FRMAE_INTERVAL;

#define CODED_FRMAE_INTERVAL_QUEUE_SIZE 64
typedef struct {
  int head;
  int size;
  double total_interval;
  CODED_FRMAE_INTERVAL buf[CODED_FRMAE_INTERVAL_QUEUE_SIZE];
} CODED_FRMAE_INTERVAL_QUEUE;

enum {
  RESOURCE_MODE = 0,  // Resource availability mode.
  SCHEDULE_MODE       // Decoding schedule mode.
} UENUM1BYTE(DECODER_MODLE_MODE);
...
enum {
  ...
  DISPLAY_FRAME_LATE,
  SMOOTH_BUFFER_UNDERFLOW,
  SMOOTH_BUFFER_OVERFLOW
} UENUM1BYTE(DECODER_MODLE_ERROR);

#define BUFFER_POOL_MAX_SIZE 10
typedef struct {
  DECODER_MODLE_ERROR status;
  DECODER_MODLE_MODE mode;
  int low_delay_mode;
  AV1_LEVEL level;
  double bit_rate;
  ...
  FRMAE_BUFFER frame_buffer_pool[BUFFER_POOL_MAX_SIZE];
  CODED_FRMAE_INTERVAL_QUEUE coded_frame_interval_queue;
