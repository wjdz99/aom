...
#if CONFIG_FLEX_REFS
static int64_t raw_motion_error_stdev(int *raw_motion_err_list,
                                      int raw_motion_err_counts) {
  int64_t sum_raw_err = 0;
  int64_t raw_err_avg = 0;
  int64_t raw_err_stdev = 0;
  if (raw_motion_err_counts == 0)
    ...
    raw_err_avg = (int64_t)(sum_raw_err / raw_motion_err_counts);
    for (int jj = 0; jj < raw_motion_err_counts; jj++) {
      raw_err_stdev += (raw_motion_err_list[jj] - raw_err_avg) *
                       (raw_motion_err_list[jj] - raw_err_avg);
    }
    ...
    raw_err_stdev = (int64_t)sqrt(raw_err_stdev / raw_motion_err_counts);
...
#if CONFIG_FLEX_REFS
    fps.raw_error_stdev = (double)raw_err_stdev;
#endif  // CONFIG_FLEX_REFS
...
static void define_gf_group(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  avg_raw_err_stdev /= non_zero_stdev_count;
