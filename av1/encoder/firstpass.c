...
#if USE_GF16_MULTI_LAYER
#if USE_GF16_DEBUG
  // NOTE(zoeliu): For debug on GF16
  printf("\nDerived baseline_gf_interval=%d, forced it to be 16.\n\n",
         rc->baseline_gf_interval);
  rc->baseline_gf_interval = 16;
#endif  // USE_GF16_DEBUG
#endif  // USE_GF16_MULTI_LAYER
...
static void configure_buffer_updates(AV1_COMP *cpi) {
  ...
  if (rc->baseline_gf_interval == 16) {
    ...
    printf("\nFrame=%d, GF_index=%d:\n", cpi->common.current_video_frame,
           gf_group->index);
    printf(
        "Before CONFIGURE: lst_fb_idx=%d, lst2_fb_idx=%d, lst3_fb_idx=%d, "
        "gld_fb_idx=%d, bwd_fb_idx=%d, alt2_fb_idx=%d, alt_fb_idx=%d, "
        "ext_fb_idx=%d\n",
        cpi->lst_fb_idxes[LAST_FRAME - LAST_FRAME],
        cpi->lst_fb_idxes[LAST2_FRAME - LAST_FRAME],
        cpi->lst_fb_idxes[LAST3_FRAME - LAST_FRAME], cpi->gld_fb_idx,
        cpi->bwd_fb_idx, cpi->alt2_fb_idx, cpi->alt_fb_idx, cpi->ext_fb_idx);

    // Print out the info stored in gf_group
    printf("gf_group->update_type=%d, ",
           gf_group->update_type[gf_group->index]);
    // == ref_fb_idx_map ==
    printf("gf_group->ref_fb_idx_map:");
    for (int ref_idx = 0; ref_idx < REF_FRAMES; ++ref_idx)
      printf(" %d", gf_group->ref_fb_idx_map[gf_group->index][ref_idx]);
    printf(" , ");
    printf("gf_group->refresh_idx=%d, gf_group->refresh_flag=%d\n",
           gf_group->refresh_idx[gf_group->index],
           gf_group->refresh_flag[gf_group->index]);
#endif  // USE_GF16_DEBUG
    ...
    // NOTE(zoeliu): For debug
    printf(
        "After CONFIGURE: lst_fb_idx=%d, lst2_fb_idx=%d, lst3_fb_idx=%d, "
        "gld_fb_idx=%d, bwd_fb_idx=%d, alt2_fb_idx=%d, alt_fb_idx=%d, "
        "ext_fb_idx=%d\n",
        cpi->lst_fb_idxes[LAST_FRAME - LAST_FRAME],
        cpi->lst_fb_idxes[LAST2_FRAME - LAST_FRAME],
        cpi->lst_fb_idxes[LAST3_FRAME - LAST_FRAME], cpi->gld_fb_idx,
        cpi->bwd_fb_idx, cpi->alt2_fb_idx, cpi->alt_fb_idx, cpi->ext_fb_idx);
    printf("refresh_fb_idx=%d\n", cpi->refresh_fb_idx);
    printf(
        "refresh_last_frame=%d, refresh_golden_frame=%d, "
        "refresh_bwd_ref_frame=%d, refresh_alt2_ref_frame=%d, "
        "refresh_alt_ref_frame=%d\n",
        cpi->refresh_last_frame, cpi->refresh_golden_frame,
        cpi->refresh_bwd_ref_frame, cpi->refresh_alt2_ref_frame,
        cpi->refresh_alt_ref_frame);
    printf(
        "is_bwd_ref_frame=%d, is_last_bipred_frame=%d, "
        "is_bipred_frame=%d, is_src_frame_alt_ref=%d, "
        "is_src_frame_ext_arf=%d\n",
        cpi->rc.is_bwd_ref_frame, cpi->rc.is_last_bipred_frame,
        cpi->rc.is_bipred_frame, cpi->rc.is_src_frame_alt_ref,
