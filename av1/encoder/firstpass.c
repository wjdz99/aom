...
#define KF_ABS_ZOOM_THRESH 24.0
#define ARF_ABS_ZOOM_THRESH 12.0
...
#define BASELINE_ERR_PER_MB 16000.0
...
#define KF_BASELINE_ERR_PER_MB 16000.0
static double calc_kf_frame_boost(AV1_COMP *cpi,
  ...
  const double lq = av1_convert_qindex_to_q(
      cpi->rc.avg_frame_qindex[INTER_FRAME], cpi->common.bit_depth);
  const double boost_q_correction = AOMMIN((0.5 + (lq * 0.015)), 2.0);
  const double active_area = calculate_active_area(cpi, this_frame);
...
static void find_next_key_frame(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  for (i = 0; i < (rc->frames_to_key - 1); ++i) {
    ...
    if (i <= KF_BOOST_SCAN_MAX_FRAMES) {
      ...
      // printf("zeromo %g zm_factor %g boost_score %g\n",
      // zero_motion_accumulator,
      //        zm_factor, boost_score);
