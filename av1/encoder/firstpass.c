...
aom_codec_err_t av1_firstpass_info_init(FIRSTPASS_INFO *firstpass_info,
  ...
  if (ext_stats_buf == NULL) {
    ...
    firstpass_info->curr_index = 0;
    firstpass_info->stats_count = 0;
    firstpass_info->curr_stats_count_future = 0;
    firstpass_info->curr_stats_count_past = 0;
    ...
    firstpass_info->curr_stats_count_future = firstpass_info->stats_count;
    firstpass_info->curr_stats_count_past = 0;
    av1_zero(firstpass_info->total_stats);
...
aom_codec_err_t av1_firstpass_info_move_curr(FIRSTPASS_INFO *firstpass_info) {
  assert(firstpass_info->curr_stats_count_future +
             firstpass_info->curr_stats_count_past ==
         firstpass_info->stats_count);
  if (firstpass_info->curr_stats_count_future > 1) {
    firstpass_info->curr_index =
        (firstpass_info->curr_index + 1) % firstpass_info->stats_buf_size;
    --firstpass_info->curr_stats_count_future;
    ++firstpass_info->curr_stats_count_past;
    return AOM_CODEC_OK;
  } else {
    return AOM_CODEC_ERROR;
...
aom_codec_err_t av1_firstpass_info_move_curr_and_pop(
  ...
  aom_codec_err_t ret = av1_firstpass_info_move_curr(firstpass_info);
...
const FIRSTPASS_STATS *av1_firstpass_info_peek(
  ...
  if (offset_from_curr >= -firstpass_info->curr_stats_count_past &&
      offset_from_curr < firstpass_info->curr_stats_count_future) {
    const int index = (firstpass_info->curr_index + offset_from_curr) %
                      firstpass_info->stats_buf_size;
...
int av1_firstpass_info_future_count(const FIRSTPASS_INFO *firstpass_info,
  ...
  if (offset_from_curr < firstpass_info->curr_stats_count_future) {
    return firstpass_info->curr_stats_count_future - offset_from_curr;
  }
  return 0;
}
...
int av1_firstpass_info_past_count(const FIRSTPASS_INFO *firstpass_info,
                                  int offset_from_curr) {
  if (offset_from_curr >= -firstpass_info->curr_stats_count_past) {
    return offset_from_curr + firstpass_info->curr_stats_count_past;
  }
  return 0;
}
