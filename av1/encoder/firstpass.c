...
static int construct_multi_layer_gf_structure(AV1_COMP *cpi,
  ...
  cpi->gf_params[frame_ind][0] = OVERLAY_UPDATE;  // update_type
  cpi->gf_params[frame_ind][1] = 0;               // arf_offset
  cpi->gf_params[frame_ind][2] = 0;               // arf_pos_in_gf
  cpi->gf_params[frame_ind][3] = 0;               // arf_update_idx
  ++frame_ind;

  // ALT0
  cpi->gf_params[frame_ind][0] = ARF_UPDATE;       // update_type
  cpi->gf_params[frame_ind][1] = gf_interval - 1;  // arf_offset
  cpi->gf_params[frame_ind][2] = 0;                // arf_pos_in_gf
  cpi->gf_params[frame_ind][3] = 0;                // arf_update_idx
  ++frame_ind;

  // set parameters for the rest of the frames
  set_multi_layer_params(cpi, 0, gf_interval, &frame_ind, 0);
