...
static int firstpass_intra_prediction(
  ...
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, xd->mi[0]->ref_frame_nrs[0]) == xd->mi[0]->ref_frame[0]);
...
#if CONFIG_NEW_REF_SIGNALING
  xd->mi[0]->ref_frame_nrs[0] = convert_named_ref_to_ranked_ref_index(&cm->new_ref_frame_data, xd->mi[0]->ref_frame[0]);
  xd->mi[0]->ref_frame_nrs[1] = convert_named_ref_to_ranked_ref_index(&cm->new_ref_frame_data, xd->mi[0]->ref_frame[1]);
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, xd->mi[0]->ref_frame_nrs[0]) == xd->mi[0]->ref_frame[0]);
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, xd->mi[0]->ref_frame_nrs[1]) == xd->mi[0]->ref_frame[1]);
#endif  // CONFIG_NEW_REF_SIGNALING
