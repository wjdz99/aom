...
static void define_gf_group(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  if (allow_alt_ref && (i < cpi->oxcf.lag_in_frames) &&
    ...
    // If this gf is too long and we do not have enough frame left, and
    // then we have to reduce the current gf length
  ...
  if (rc->source_alt_ref_pending) {
    // If this gf is too long and we do not have enough frame left, and
    // then we have to reduce the boost factor on current alt ref
    if (rc->frames_to_key - i < 9 && i > 10) {
