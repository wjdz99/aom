...
unsigned char gf_multi_layer_params[MAX_CUSTOMIZED_GF_FRAMES][GF_FRAME_PARAMS];

void check_frame_params(int gf_interval, int frame_nums) {
...
static void set_multi_layer_params(int l, int r, int *frame_ind, int arf_ind) {
  if (r - l == 2) {
    // leave node, not a look-ahead frame
    gf_multi_layer_params[*frame_ind][0] = LAST_BIPRED_UPDATE;  // update_type
    gf_multi_layer_params[*frame_ind][1] = 0;                   // arf_offset
    gf_multi_layer_params[*frame_ind][2] = 0;                   // arf_pos_in_gf
    gf_multi_layer_params[*frame_ind][3] = arf_ind;  // arf_update_idx
    ++(*frame_ind);
  } else {
    int m = (l + r) / 2;
    int arf_pos_in_gf = *frame_ind;

    gf_multi_layer_params[*frame_ind][0] = INTNL_ARF_UPDATE;  // update_type
    gf_multi_layer_params[*frame_ind][1] = m - l - 1;         // arf_offset
    gf_multi_layer_params[*frame_ind][2] = 0;                 // arf_pos_in_gf
    gf_multi_layer_params[*frame_ind][3] = 1;                 // arf_update_idx
    ++(*frame_ind);

    // left nodes
    set_multi_layer_params(l, m, frame_ind, 1);

    // for overlay frames, we need to record the position of its corresponding
    // arf frames for bit allocation
    gf_multi_layer_params[*frame_ind][0] = INTNL_OVERLAY_UPDATE;  // update_type
    gf_multi_layer_params[*frame_ind][1] = 0;                     // arf_offset
    gf_multi_layer_params[*frame_ind][2] = arf_pos_in_gf;  // arf_pos_in_gf
    gf_multi_layer_params[*frame_ind][3] = 1;              // arf_update_idx
    ++(*frame_ind);

    // right nodes
    set_multi_layer_params(m, r, frame_ind, arf_ind);
  }
}

static int construct_multi_layer_gf_structure(const int gf_interval) {
  int frame_ind = 0;

  // At the beginning of each GF group it will be a key or overlay frame,
  gf_multi_layer_params[frame_ind][0] = OVERLAY_UPDATE;
  gf_multi_layer_params[frame_ind][1] = 0;
  gf_multi_layer_params[frame_ind][2] = 0;
  gf_multi_layer_params[frame_ind][3] = 0;
  ++frame_ind;

  // ALT0
  gf_multi_layer_params[frame_ind][0] = ARF_UPDATE;
  gf_multi_layer_params[frame_ind][1] = gf_interval - 1;
  gf_multi_layer_params[frame_ind][2] = 0;
  gf_multi_layer_params[frame_ind][3] = 0;
  ++frame_ind;

  // Construct the rest frames
  set_multi_layer_params(0, gf_interval, &frame_ind, 0);

  // check_frame_params(gf_interval, frame_ind);
...
void configure_buffer_updates_firstpass(AV1_COMP *cpi,
