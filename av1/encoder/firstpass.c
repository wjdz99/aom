...
#if CONFIG_NEW_REF_SIGNALING
  const MV_REFERENCE_FRAME_NRS last_frame_ = get_closest_pastcur_ref_index(cm);
  const MV_REFERENCE_FRAME_NRS golden_frame_ = get_best_past_ref_index(cm);
  const YV12_BUFFER_CONFIG *const last_frame = get_ref_frame_yv12_buf_nrs(
      cm, last_frame_ == INVALID_IDX ? 0 : last_frame_);
  const YV12_BUFFER_CONFIG *golden_frame = get_ref_frame_yv12_buf_nrs(
      cm, golden_frame_ == INVALID_IDX ? 0 : golden_frame_);
#else
