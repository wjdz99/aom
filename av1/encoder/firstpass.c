...
void check_frame_params(AV1_COMP *cpi, int gf_interval, int frame_nums) {
  ...
  for (int i = 0; i <= frame_nums; ++i) {
    fprintf(fid, "%s %d %d %d\n", str[cpi->gf_params[i][0]],
            cpi->gf_params[i][1], cpi->gf_params[i][2], cpi->gf_params[i][3]);
  }
  fflush(fid);
}
...
static void set_multi_layer_params(AV1_COMP *cpi, int l, int r, int *frame_ind,
                                   int arf_ind) {
  if (r - l == 2) {
    ...
    cpi->gf_params[*frame_ind][0] = INTNL_ARF_UPDATE;  // update_type
    cpi->gf_params[*frame_ind][1] = m - l - 1;         // arf_offset
    cpi->gf_params[*frame_ind][2] = 0;                 // arf_pos_in_gf
    cpi->gf_params[*frame_ind][3] = 1;                 // arf_update_idx
    ++(*frame_ind);

    // set parameters for frames displayed before this frame
    set_multi_layer_params(cpi, l, m, frame_ind, 1);

    // for overlay frames, we need to record the position of its corresponding
    // arf frames for bit allocation
    cpi->gf_params[*frame_ind][0] = INTNL_OVERLAY_UPDATE;  // update_type
    cpi->gf_params[*frame_ind][1] = 0;                     // arf_offset
    cpi->gf_params[*frame_ind][2] = arf_pos_in_gf;         // arf_pos_in_gf
    cpi->gf_params[*frame_ind][3] = 1;                     // arf_update_idx
    ++(*frame_ind);

    // set parameters for frames displayed after this frame
    set_multi_layer_params(cpi, m, r, frame_ind, arf_ind);
  }
}

static int construct_multi_layer_gf_structure(AV1_COMP *cpi,
                                              const int gf_interval) {
  int frame_index = 0;

  // At the beginning of each GF group it will be a key or overlay frame,
  cpi->gf_params[frame_index][0] = OVERLAY_UPDATE;  // update_type
  cpi->gf_params[frame_index][1] = 0;               // arf_offset
  cpi->gf_params[frame_index][2] = 0;               // arf_pos_in_gf
  cpi->gf_params[frame_index][3] = 0;               // arf_update_idx
  ++frame_index;

  // ALT0
  cpi->gf_params[frame_index][0] = ARF_UPDATE;       // update_type
  cpi->gf_params[frame_index][1] = gf_interval - 1;  // arf_offset
  cpi->gf_params[frame_index][2] = 0;                // arf_pos_in_gf
  cpi->gf_params[frame_index][3] = 0;                // arf_update_idx
  ++frame_index;

  // set parameters for the rest of the frames
  set_multi_layer_params(cpi, 0, gf_interval, &frame_index, 0);

  // check_frame_params(cpi, gf_interval, frame_ind);
...
void define_customized_gf_group_structure(AV1_COMP *cpi) {
  ...
  const int gf_update_frames =
      construct_multi_layer_gf_structure(cpi, rc->baseline_gf_interval);
  int frame_index;
  ...
  for (frame_index = 0; frame_index < gf_update_frames; ++frame_index) {
    ...
    if (frame_index == 0) {
      ...
      gf_group->update_type[frame_index] =
          cpi->gf_params[frame_index][param_idx];
    }
...
// It is an example of how to define a GF stucture manually.
// The function will result in exactly the same GF group structure as
