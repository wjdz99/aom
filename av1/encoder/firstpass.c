...
static void intra_pred_and_calc_error(int plane, int block, int blk_row,
  ...
  av1_predict_intra_block(
      ...
      src, src_stride, dst, dst_stride, blk_col, blk_row, plane);

  av1_subtract_txb(x, plane, plane_bsize, blk_col, blk_row, tx_size);
}

static void intra_predict_block_plane_first_pass(
  ...
  av1_foreach_transformed_block_in_plane(xd, plane_bsize, plane,
                                         intra_pred_and_calc_error, &args);

  // copy source data to recon buffer, as the recon buffer will used as a
...
static int firstpass_intra_prediction(
  ...
  if (cpi->sf.fp_sf.disable_recon)
    intra_predict_block_plane_first_pass(seq_params, x, bsize);
  else
