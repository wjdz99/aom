...
static void first_pass_motion_search(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  tmp_err = cpi->diamond_search_sad(x, &cpi->ss_cfg[0], &ref_mv_full, &tmp_mv,
  ...
  while (n < further_steps) {
    ...
    if (num00) {
      ...
      tmp_err = cpi->diamond_search_sad(x, &cpi->ss_cfg[0], &ref_mv_full,
                                        &tmp_mv, step_param + n, x->sadperbit16,
