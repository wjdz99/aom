...
static void intra_pred_and_calc_error_luma(int plane, int block, int blk_row,
...
static void intra_predict_block_luma_plane_first_pass(
  ...
  av1_foreach_transformed_block_in_plane(xd, plane_bsize, plane,
                                         intra_pred_and_calc_error_luma, &args);
...
static int firstpass_intra_prediction(
  ...
  if (cpi->sf.fp_sf.disable_recon)
    intra_predict_block_luma_plane_first_pass(seq_params, x, bsize);
  else
