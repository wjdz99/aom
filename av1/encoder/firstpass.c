...
static void configure_buffer_updates(AV1_COMP *cpi) {
  ...
  switch (twopass->gf_group.update_type[twopass->gf_group.index]) {
...
#if CONFIG_ALTREF2
      if (cpi->num_extra_arfs == MAX_EXT_ARFS &&
          twopass->gf_group.arf_update_idx[twopass->gf_group.index] > 0) {
      }
#else   // !CONFIG_ALTREF2
      if (cpi->num_extra_arfs) {
    ...
    case INTNL_ARF_UPDATE:
      ...
      if (cpi->num_extra_arfs == MAX_EXT_ARFS &&
          twopass->gf_group.arf_update_idx[twopass->gf_group.index] > 0) {
        // 1. Set up the indexes for its backward references.
        const int tmp = cpi->bwd_fb_idx;
        cpi->bwd_fb_idx = cpi->alt2_fb_idx;
        cpi->arf_map[0] = tmp;

        // 2. Set up its own update index.
        cpi->alt2_fb_idx = cpi->arf_map[2];
      }
      break;
#endif  // CONFIG_ALTREF2
#endif  // CONFIG_EXT_REFS
