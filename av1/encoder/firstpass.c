...
static double get_distribution_av_err(AV1_COMP *cpi, TWO_PASS *const twopass) {
  (void)cpi;
...
void av1_first_pass(AV1_COMP *cpi, const struct lookahead_entry *source) {
  ...
  int intra_count_low = 0;
  int intra_count_high = 0;
...
// well tested.
    ...
    twopass->bpm_factor = AOMMAX(0.25, AOMMIN(4.0, twopass->bpm_factor));
#endif
    printf("last_group_rate_err = %g\n", last_group_rate_err);
    // Try and pick a max Q that will be high enough to encode the
    // content at the given rate.
    for (q = rc->best_quality; q < rc->worst_quality; ++q) {
      const double factor =
          calc_correction_factor(av_err_per_mb, ERR_DIVISOR, q);
      const double ofactor = factor * speed_term * cpi->twopass.bpm_factor * noise_factor;
      const int bits_per_mb = av1_rc_bits_per_mb(
          INTER_FRAME, q,
          factor * speed_term * cpi->twopass.bpm_factor * noise_factor,
          cpi->common.bit_depth);
      printf("q %d factor %g bits_per_mb %d/%d\n", q, ofactor, bits_per_mb, target_norm_bits_per_mb);
...
static double calc_frame_boost(AV1_COMP *cpi, const FIRSTPASS_STATS *this_frame,
  ...
  return AOMMIN(frame_boost, GF_MAX_BOOST * boost_q_correction);
}
...
static void define_gf_group(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  if ((cpi->oxcf.rc_mode != AOM_Q) && (rc->baseline_gf_interval > 1)) {
    ...
    printf("twopass->active_worst_quality = %d / %d\n", twopass->active_worst_quality, tmp_q);
#if ALWAYS_ADJUST_BPM
  ...
  printf("gf_arf_bits %d gf_bits %d %d %d\n", gf_arf_bits,
         twopass->gf_group.bit_allocation[0],
         twopass->gf_group.bit_allocation[1],
