...
static int firstpass_inter_prediction(
  ...
  // TODO(any): The unscaled last source might be different dimension as the
  // current source.
...
void av1_first_pass(AV1_COMP *cpi, const int64_t ts_duration) {
  ...
  if (!frame_is_intra_only(cm)) {
    ...
    last_frame = av1_get_scaled_ref_frame(cpi, LAST_FRAME);
    golden_frame = av1_get_scaled_ref_frame(cpi, GOLDEN_FRAME);
  }
  if (!last_frame) {
    last_frame = get_ref_frame_yv12_buf(cm, LAST_FRAME);
  }
