...
#define MIN_ARF_GF_BOOST 500
#define MIN_KF_TOT_BOOST 2000
#define MAX_KF_TOT_BOOST 7000
#define KF_MAX_FRAME_BOOST 84.0
#define KF_MIN_FRAME_BOOST 84.0
#define GF_MAX_FRAME_BOOST 96.0

#define KF_ABS_ZOOM_THRESH 8.0
#define ARF_ABS_ZOOM_THRESH 4.0
...
static int fp_estimate_block_noise(MACROBLOCK *x, BLOCK_SIZE bsize) {
  ...
  const int width = mi_size_wide[bsize] * 4;
  const int height = mi_size_high[bsize] * 4;
  int w, h;
...
#define KF_BASELINE_ERR_PER_MB 15000.0
static double calc_kf_frame_boost(AV1_COMP *cpi,
  ...
  // Underlying boost factor is based on inter error ratio.
  frame_boost =
      (KF_BASELINE_ERR_PER_MB * active_area) /
      DOUBLE_DIVIDE_CHECK(this_frame->coded_error + 0.75 * *sr_accumulator);
...
static void find_next_key_frame(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  const double kf_max_frame_boost = AOMMIN(
      AOMMAX(rc->frames_to_key * 0.8, KF_MIN_FRAME_BOOST), KF_MAX_FRAME_BOOST);
