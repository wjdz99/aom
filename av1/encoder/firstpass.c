...
void av1_first_pass_row(AV1_COMP *cpi, ThreadData *td, TileDataEnc *tile_data,
  ...
  int mb_row_in_tile = mb_row - (tile->mi_row_start >> 2);
  int mb_col_start = tile->mi_col_start >> 2;
  ...
  av1_setup_src_planes(x, cpi->source, mb_row << 2, tile->mi_col_start,
                       num_planes, fp_block_size);
  ...
  for (int mb_col = mb_col_start; mb_col < (tile->mi_col_end >> 2); ++mb_col) {
...
void av1_first_pass(AV1_COMP *cpi, const int64_t ts_duration) {
  ...
  for (int mb_row = (tile->mi_row_start >> 2); mb_row < (tile->mi_row_end >> 2);
       ++mb_row) {
