...
static void update_partition_cdfs_and_counts(MACROBLOCKD *xd, int blk_col,
  ...
  if (allow_horz && allow_vert) {
    ...
    const int split4_ctx = is_inter ? 
        txfm_partition_split4_inter_context(
    ...
    if (is_inter) ++counts->inter_4way_txfm_partition[is_rect][split4_ctx][split4_partition];
    else ++counts->intra_4way_txfm_partition[is_rect][split4_ctx][split4_partition];
    ...
    if (((split4_partition == TX_PARTITION_VERT) && allow_vert4) ||
      ...
      if (allow_update_cdf) {
        aom_cdf_prob *split2_rect_cdf = is_inter ?  
                   xd->tile_ctx->inter_2way_rect_txfm_partition_cdf :
                   xd->tile_ctx->intra_2way_rect_txfm_partition_cdf;
        update_cdf(split2_rect_cdf, has_split, 2);
      }
#if CONFIG_ENTROPY_STATS
      if (is_inter) ++counts->inter_2way_rect_txfm_partition[has_split];
      else ++counts->intra_2way_rect_txfm_partition[has_split];
#endif  // CONFIG_ENTROPY_STATS
    }
    ...
    if (allow_update_cdf) {
      aom_cdf_prob *split2_cdf = is_inter ?  
                 xd->tile_ctx->inter_2way_txfm_partition_cdf :
                 xd->tile_ctx->intra_2way_txfm_partition_cdf;
      update_cdf(split2_cdf, has_first_split, 2);
    }
#if CONFIG_ENTROPY_STATS
    if (is_inter) ++counts->inter_2way_txfm_partition[has_first_split];
    else ++counts->intra_2way_txfm_partition[has_first_split];
#endif  // CONFIG_ENTROPY_STATS
    ...
    if (has_first_split && (allow_horz4 || allow_vert4)) {
      ...
      if (allow_update_cdf) {
        aom_cdf_prob *split2_rect_cdf = is_inter ?  
                   xd->tile_ctx->inter_2way_rect_txfm_partition_cdf :
                   xd->tile_ctx->intra_2way_rect_txfm_partition_cdf;
        update_cdf(split2_rect_cdf,
                   has_second_split, 2);
      }
#if CONFIG_ENTROPY_STATS
      if (is_inter) ++counts->inter_2way_txfm_partition[has_second_split];
      else ++counts->intra_2way_txfm_partition[has_second_split];
#endif  // CONFIG_ENTROPY_STATS
    }
