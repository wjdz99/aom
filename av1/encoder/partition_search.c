...
static void update_drl_index_stats(FRAME_CONTEXT *fc, FRAME_COUNTS *counts,
  ...
  const int range = AOMMIN(mbmi_ext->ref_mv_count[ref_frame_type] - 1, 2);
...
static void update_stats(const AV1_COMMON *const cm, ThreadData *td) {
        ...
        if (!seg_ref_active && inter_block) {
          ...
          if (mbmi->ref_frame[1] != INTRA_FRAME) {
            if (motion_allowed >= OBMC_CAUSAL) {
              td->rd_counts
                  .obmc_used[bsize][mbmi->motion_mode == OBMC_CAUSAL]++;
            }
            if (motion_allowed == WARPED_CAUSAL) {
  ...
  static void encode_sb(const AV1_COMP *const cpi, ThreadData *td,
    ...
    if (!dry_run) {
      ...
      const int num_splittable_sub_blocks =
          partition == PARTITION_SPLIT ? 4 : 0;
    ...
    switch (partition) {
      ...
      case PARTITION_HORZ_4:
        for (int i = 0; i < SUB_PARTITIONS_PART4; ++i) {
          ...
          encode_b(cpi, tile_data, td, tp, this_mi_row, mi_col, dry_run,
      ...
      case PARTITION_VERT_4:
        for (int i = 0; i < SUB_PARTITIONS_PART4; ++i) {
          ...
          encode_b(cpi, tile_data, td, tp, mi_row, this_mi_col, dry_run,
  ...
  */
  void av1_rd_use_partition(AV1_COMP * cpi, ThreadData * td,
    ...
    if ((cpi->sf.part_sf.partition_search_type == VAR_BASED_PARTITION &&
      ...
      for (int i = 0; i < SUB_PARTITIONS_SPLIT; i++) {
        ...
        pick_sb_modes(cpi, tile_data, x, mi_row + y_idx, mi_col + x_idx,
                      ...
                      pc_tree->split[i]->none, invalid_rdc, PICK_MODE_RD);
  ...
  */
  void av1_nonrd_use_partition(
      AV1_COMP * cpi, ThreadData * td, TileDataEnc * tile_data,
      MB_MODE_INFO * *mib, TokenExtra * *tp, int mi_row, int mi_col,
      BLOCK_SIZE bsize, PC_TREE *pc_tree, PARTITION_TREE *ptree) {
    AV1_COMMON *const cm = &cpi->common;
    const CommonModeInfoParams *const mi_params = &cm->mi_params;
    TileInfo *const tile_info = &tile_data->tile_info;
    MACROBLOCK *const x = &td->mb;
    ...
    switch (partition) {
      case PARTITION_NONE:
        pc_tree->none = av1_alloc_pmc(cm, bsize, &td->shared_coeff_buf);
        if (cpi->sf.rt_sf.nonrd_check_partition_split &&
            do_slipt_check(bsize) && !frame_is_intra_only(cm)) {
      ...
      case PARTITION_VERT:
        ...
        if (mi_col + hbs < mi_params->mi_cols && bsize > BLOCK_8X8) {
          ...
          encode_b_nonrd(cpi, tile_data, td, tp, mi_row, mi_col + hbs, 0,
                         subsize, PARTITION_VERT, pc_tree->vertical[1], NULL);
        }
        break;
      case PARTITION_HORZ:
        for (int i = 0; i < SUB_PARTITIONS_RECT; ++i) {
        ...
        if (mi_row + hbs < mi_params->mi_rows && bsize > BLOCK_8X8) {
          ...
          encode_b_nonrd(cpi, tile_data, td, tp, mi_row + hbs, mi_col, 0,
                         subsize, PARTITION_HORZ, pc_tree->horizontal[1], NULL);
        }
        break;
      case PARTITION_SPLIT:
        for (int i = 0; i < SUB_PARTITIONS_SPLIT; ++i) {
  ...
  static int rd_try_subblock(
      ...
      TokenExtra **tp, int is_last, int mi_row, int mi_col, BLOCK_SIZE subsize,
      RD_STATS best_rdcost, RD_STATS *sum_rdc, PARTITION_TYPE partition,
    ...
    av1_rd_stats_subtraction(x->rdmult, &best_rdcost, sum_rdc,
                             &rdcost_remaining);
    RD_STATS this_rdc;
  ...
  static bool rd_test_partition3(
      ...
      PICK_MODE_CONTEXT *ctxs[SUB_PARTITIONS_AB], int mi_row, int mi_col,
  ...
  static void init_partition_search_state_params(
      ...
      PartitionSearchState *part_search_state, int mi_row, int mi_col,
      BLOCK_SIZE bsize) {
    MACROBLOCKD *const xd = &x->e_mbd;
    const AV1_COMMON *const cm = &cpi->common;
  ...
  static void rd_pick_rect_partition(
      ...
      const int idx, int mi_row, int mi_col, BLOCK_SIZE bsize,
    ...
    pick_sb_modes(cpi, tile_data, x, mi_row, mi_col,
                  &part_search_state->this_rdc, partition_type, bsize,
                  cur_partition_ctx, best_remain_rdcost, PICK_MODE_RD);
  ...
  static AOM_INLINE int is_rect_part_allowed(
      ...
      active_edge_info *active_edge, RECT_PART_TYPE rect_part,
      const int mi_pos) {
    PartitionBlkParams blk_params = part_search_state->part_blk_params;
  ...
  static void rectangular_partition_search(
    ...
    for (RECT_PART_TYPE i = HORZ; i < NUM_RECT_PARTS; i++) {
      ...
      if (sum_rdc->rdcost < best_rdc->rdcost && is_not_edge_block[i]) {
        ...
        av1_update_state(cpi, td, cur_ctx[i][sub_part_idx][0],
                         blk_params.mi_row, blk_params.mi_col,
                         blk_params.subsize, DRY_RUN_NORMAL);
        ...
        sub_part_idx = 1;
        rd_pick_rect_partition(cpi, tile_data, x, cur_ctx[i][sub_part_idx][0],
                               part_search_state, best_rdc, 1,
                               mi_pos_rect[i][sub_part_idx][0],
                               mi_pos_rect[i][sub_part_idx][1],
  ...
  static void rd_pick_ab_part(
      ...
      const BLOCK_SIZE ab_subsize[SUB_PARTITIONS_AB],
      const int ab_mi_pos[SUB_PARTITIONS_AB][2],
      const PARTITION_TYPE part_type) {
    const AV1_COMMON *const cm = &cpi->common;
    PartitionBlkParams blk_params = part_search_state->part_blk_params;
...
#endif
    av1_restore_context(cm, x, x_ctx, mi_row, mi_col, bsize,
                        av1_num_planes(cm));
  }
  ...
  static void rd_pick_4partition(
    ...
    set_mi_pos_partition4(inc_step, mi_pos, blk_params.mi_row,
                          blk_params.mi_col);
#if CONFIG_COLLECT_PARTITION_STATS
  ...
  static void prune_4_partition_using_split_info(
      AV1_COMP *const cpi, MACROBLOCK *x,
      PartitionSearchState *part_search_state,
  ...
  static void set_none_partition_params(
      const AV1_COMMON *const cm, ThreadData *td, MACROBLOCK *x,
      PC_TREE *pc_tree, PartitionSearchState *part_search_state,
      RD_STATS *best_remain_rdcost, RD_STATS *best_rdc, int *pt_cost) {
    PartitionBlkParams blk_params = part_search_state->part_blk_params;
    RD_STATS partition_rdcost;
    // Set PARTITION_NONE context.
    if (pc_tree->none == NULL)
      pc_tree->none =
          av1_alloc_pmc(cm, blk_params.bsize, &td->shared_coeff_buf);
  ...
  static void prune_partitions_after_none(
  ...
  static void prune_partitions_after_split(
    ...
    if (cpi->sf.part_sf.ml_early_term_after_part_split_level &&
      ...
      av1_ml_early_term_after_split(
          cpi, x, sms_tree, bsize, best_rdc->rdcost, part_none_rd,
          part_split_rd, part_search_state->split_rd, mi_row, mi_col,
          &part_search_state->terminate_partition_search);
    }
  ...
  static void none_partition_search(
      ...
      RD_SEARCH_MACROBLOCK_CONTEXT *x_ctx,
      PartitionSearchState *part_search_state, RD_STATS *best_rdc,
      unsigned int *pb_source_variance, int64_t *none_rd,
  ...
  static void split_partition_search(
    ...
    for (idx = 0;
         idx < SUB_PARTITIONS_SPLIT && sum_rdc.rdcost < best_rdc->rdcost;
         ++idx) {
  ...
  */
  bool av1_rd_pick_partition(
      AV1_COMP *const cpi, ThreadData *td, TileDataEnc *tile_data,
      TokenExtra **tp, int mi_row, int mi_col, BLOCK_SIZE bsize,
      RD_STATS *rd_cost, RD_STATS best_rdc, PC_TREE *pc_tree,
  ...
  BEGIN_PARTITION_SEARCH:
    ...
    if (!part_search_state.terminate_partition_search &&
      ...
      const int inc_step[NUM_PART4_TYPES] = {
        mi_size_high[blk_params.bsize] / 4, 0
      };
    ...
    if (!part_search_state.terminate_partition_search &&
      ...
      const int inc_step[NUM_PART4_TYPES] = {
        0, mi_size_wide[blk_params.bsize] / 4
      };
