...
static void prune_4_way_partition_search(
    ...
    const PartitionSearchState *part_search_state, RD_STATS *best_rdc,
...
static void set_none_partition_params(
    const AV1_COMP *const cpi, ThreadData *td, MACROBLOCK *x, PC_TREE *pc_tree,
    const PartitionSearchState *part_search_state, RD_STATS *best_remain_rdcost,
  ...
  if (pc_tree->none == NULL)
    pc_tree->none =
        av1_alloc_pmc(cpi, blk_params->bsize, &td->shared_coeff_buf);
  if (!pc_tree->none)
    aom_internal_error(x->e_mbd.error_info, AOM_CODEC_MEM_ERROR,
                       "Failed to allocate PICK_MODE_CONTEXT");
