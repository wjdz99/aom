...
static void pick_sb_modes(AV1_COMP *const cpi, TileDataEnc *tile_data,
  ...
  if (rd_cost->rate != INT_MAX &&
    ...
    av1_update_state(cpi, &cpi->td, ctx, mi_row, mi_col, bsize, DRY_RUN_NORMAL);
    ...
    for (int row = mi_row; row < mi_row + mi_high; row += row_step) {
      for (int col = mi_col; col < mi_col + mi_wide; col += col_step) {
        ...
        for (int plane = 0; plane < num_planes; ++plane) {
          ...
          //     RDCOST(tpl_frame->base_rdmult, this_stats->mc_dep_rate,
          //            this_stats->mc_dep_dist);
          double intra_cost = this_stats->recrf_dist << RDDIV_BITS;
          double mc_dep_cost =
...
static int rd_try_subblock(AV1_COMP *const cpi, ThreadData *td,
  ...
  av1_rd_stats_subtraction(x->sb_rdmult, &best_rdcost, sum_rdc,
                           &rdcost_remaining);
  RD_STATS this_rdc;
