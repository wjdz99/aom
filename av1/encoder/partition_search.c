...
static void update_partition_cdfs_and_counts(MACROBLOCKD *xd, 
                                  int blk_col, int blk_row, 
                                  TX_SIZE max_tx_size, int allow_update_cdf, 
                                  FRAME_COUNTS *counts) {
  (void)counts;
  MB_MODE_INFO *mbmi = xd->mi[0];
  const int txb_size_index = av1_get_txb_size_index(mbmi->sb_type, blk_row, blk_col);
  ...
  ++counts->inter_2way_rect_txfm_partition[has_split];
      ...
      const int has_first_split = partition != TX_PARTITION_NONE;
       if (allow_update_cdf) {
      update_cdf(xd->tile_ctx->inter_2way_txfm_partition_cdf,
           has_first_split, 2);
      }
#if CONFIG_ENTROPY_STATS
...
static void update_txfm_count(MACROBLOCK *x, MACROBLOCKD *xd,
  ...
  update_partition_cdfs_and_counts(xd,
                        blk_col, blk_row, tx_size, allow_update_cdf, counts);
