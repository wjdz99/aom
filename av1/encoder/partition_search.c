...
bool av1_rd_pick_partition(AV1_COMP *const cpi, ThreadData *td,
  ...
  // the recursive function av1_rd_pick_partition() can reach. To implement
  // this: only PARTITION_NONE is allowed if the current node equals
  // min_sq_part, only PARTITION_SPLIT is allowed if the current node exceeds
  // max_sq_part.
...
BEGIN_PARTITION_SEARCH:
  ...
  for (int idx = 0;
       ...
       is_partition_valid(bsize, PARTITION_HORZ) &&
#endif  // CONFIG_EXT_RECUR_PARTITIONS
       (do_rectangular_split || av1_active_h_edge(cpi, mi_row, mi_step_h)) &&
       !is_gt_max_sq_part;
       idx++) {
    ...
    av1_rd_pick_partition(cpi, td, tile_data, tp, mi_row, mi_col, subsize,
                          max_sq_part, min_sq_part, &this_rdc,
...
#if CONFIG_EXT_RECUR_PARTITIONS
      av1_rd_pick_partition(cpi, td, tile_data, tp, mi_row + mi_step_h, mi_col,
  ...
  for (int idx = 0;
       ...
       is_partition_valid(bsize, PARTITION_VERT) &&
#endif  // CONFIG_EXT_RECUR_PARTITIONS
       (do_rectangular_split || av1_active_v_edge(cpi, mi_col, mi_step_w)) &&
       !is_gt_max_sq_part;
       idx++) {
    ...
    av1_rd_pick_partition(cpi, td, tile_data, tp, mi_row, mi_col, subsize,
                          max_sq_part, min_sq_part, &this_rdc,
                          best_remain_rdcost, pc_tree->vertical[0], NULL, NULL,
                          multi_pass_mode);
#else
    ...
    if (sum_rdc.rdcost < best_rdc.rdcost && has_cols) {
      ...
      av1_update_state(cpi, td, pc_tree->vertical[0], mi_row, mi_col, subsize,
                       1);
      av1_encode_superblock(cpi, tile_data, td, tp, DRY_RUN_NORMAL, subsize,
                            NULL);
#endif  // !CONFIG_EXT_RECUR_PARTITIONS
      ...
      av1_rd_stats_subtraction(x->rdmult, &best_rdc, &sum_rdc,
                               &best_remain_rdcost);
#if CONFIG_EXT_RECUR_PARTITIONS
      av1_rd_pick_partition(cpi, td, tile_data, tp, mi_row, mi_col + mi_step_w,
                            subsize, max_sq_part, min_sq_part, &this_rdc,
                            best_remain_rdcost, pc_tree->vertical[1], NULL,
                            NULL, multi_pass_mode);
#else
  ...
  for (int idx = 0;
       ...
       (do_rectangular_split || av1_active_v_edge(cpi, mi_row, mi_step_h)) &&
       !is_gt_max_sq_part;
       idx++) {
