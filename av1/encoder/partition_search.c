  ...
  for (RECT_PART_TYPE i = HORZ; i < NUM_RECT_PARTS; i++) {
    ...
    if (i == HORZ) {
      ...
      av1_rd_stats_subtraction(x->rdmult, best_rdc, sum_rdc,
                               &best_remain_rdcost);
      av1_rd_pick_partition(
          cpi, td, tile_data, tp, blk_params.mi_row, blk_params.mi_col,
...
#if CONFIG_SDP
                            (ptree_luma && ptree_luma->partition == PARTITION_VERT ? ptree_luma->sub_tree[0] : NULL;
#endif  // CONFIG_SDP
                            NULL);
...
#if CONFIG_SDP
                             (ptree_luma && ptree_luma->partition == PARTITION_VERT ? ptree_luma->sub_tree[1] : NULL;
#endif  // CONFIG_SDP
                              NULL);
      }
      av1_rd_cost_update(x->rdmult, &this_rdc);
...
static INLINE void search_partition_horz_3(PartitionSearchState *search_state,
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    SUBBLOCK_RDO_DATA rdo_data = {
      NULL, pc_tree->horizontal3[i],
#if CONFIG_SDP
      ptree_luma ? ptree_luma->sub_tree[i] : NULL;
#endif  // CONFIG_SDP
    NULL, this_mi_row, mi_col, subblock_sizes[i], PARTITION_HORZ_3, i == 2, 1
  };
...
static INLINE void search_partition_vert_3(PartitionSearchState *search_state,
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    SUBBLOCK_RDO_DATA rdo_data = {
      NULL, pc_tree->vertical3[i],
#if CONFIG_SDP
      ptree_luma ? ptree_luma->sub_tree[i] : NULL;
#endif  // CONFIG_SDP
    NULL, mi_row, this_mi_col, subblock_sizes[i], PARTITION_VERT_3, i == 2, 1
  };
  if (!rd_try_subblock_new(cpi, td, tile_data, tp, &rdo_data, *best_rdc,
                           &sum_rdc, multi_pass_mode)) {
    av1_invalid_rd_stats(&sum_rdc);
