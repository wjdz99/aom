...

  {
    ...
    av1_setup_src_planes(x, cpi->source, mi_row, mi_col, 1, bsize);
    {
      ...
      for (i = 0; i < 4; ++i) {
        ...
        features[feature_idx] =
            (var_ratio - means[feature_idx]) / sqrt(vars[feature_idx]);
        feature_idx++;
      }
    }
...
void av1_nonrd_pick_partition(AV1_COMP *cpi, ThreadData *td,
  ...
  if (partition_none_allowed) {
    ...
    if (this_rdc.rate != INT_MAX) {
      ...
      // for (k = 0; k<depth; k++)
      //   printf(" ");
      // printf ("mi_row %d, mi_col %d, BS %d, NONE, rdcost %ld, part cost
      // %d\n", mi_row, mi_col, bsize, this_rdc.rdcost,
  ...
  if (do_split) {
    ...
    // for (k = 0; k<depth; k++)
    //     printf(" ");
    // printf ("mi_row %d, mi_col %d, BS %d, SPLIT, rdcost %ld,\n", mi_row,
    // mi_col, bsize, sum_rdc.rdcost);
