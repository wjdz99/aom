...
static AOM_INLINE int keep_cdef_for_current(MB_MODE_INFO *const mi) {
  if (mi->mode < INTRA_MODES || mi->mode == NEWMV) {
    return 1;
  } else {
    return 0;
  }
...
* \param[in]    pc_tree   Pointer to the PC_TREE node holding the picked
partitions and mode info for the current block
* \param[in]    skip_cdef_large_mv   An array with flags for each superblock to
skip cdef
*
* \return Nothing is returned. The pc_tree struct is modified to store the
