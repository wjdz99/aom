...
#if ENABLE_SDP_SIGNALING && !ENABLE_SDP_ENCODER
  mbmi->sb_type[PLANE_TYPE_UV] = bsize;
#else
        ...
        if (cm->tree_type == CHROMA_PART &&
          ...
          for (int y_district = 0; y_district < 2; y_district++) {
            for (int x_district = 0; x_district < 2; x_district++) {
              ...
              for (int r = 0; r < y_mis_half; ++r) {
                for (int c = 0; c < x_mis_half; ++c) {
                  int y_pos = y_district * y_mis_half + r;
                  int x_pos = x_district * x_mis_half + c;
