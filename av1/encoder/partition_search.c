...
static void update_txfm_count(MACROBLOCK *x, MACROBLOCKD *xd,
  ...
  (void)depth;
  (void)counts;
  const int split4_ctx = txfm_partition_split4_inter_context(xd->above_txfm_context + blk_col,
    ...
    if (txfm_params->tx_mode_search_type == TX_MODE_SELECT &&
      ...
      if (is_inter) {
        ...
        if (block_signals_txsize(bsize)) {
          ...
          const TX_PARTITION_TYPE split4_partition = 
            get_split4_partition(mbmi->partition_type[0]);
          if (tile_data->allow_update_cdf)
            update_cdf(xd->tile_ctx->intra_4way_txfm_partition_cdf[is_rect][tx_size_ctx],
                       split4_partition, 4);
#if CONFIG_ENTROPY_STATS
          ++td->counts->intra_4way_txfm_partition[is_rect][tx_size_ctx][split4_partition];
#endif
#else  // CONFIG_NEW_TX_PARTITION
