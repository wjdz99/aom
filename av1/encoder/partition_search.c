...
static void encode_b_nonrd(const AV1_COMP *const cpi, TileDataEnc *tile_data,
  ...
  if (!dry_run) {
//    if(mi_row >=32 && mi_col >=48)
//    printf("\n !!!!  (%d;%d;)    %d; %d;  \n", mi_row  , mi_col, x->cb_offset[PLANE_TYPE_Y],
//                   (1 << num_pels_log2_lookup[cpi->common.seq_params->sb_size]));
  ...
  switch (partition) {
    ...
    case PARTITION_SPLIT:
      ...
      if (cpi->sf.rt_sf.nonrd_check_partition_merge_mode &&
        ...
        // merge_mode features.
        if ((mi_row + bs <= mi_params->mi_rows) && (mi_col + bs <= mi_params->mi_cols)) {
          MB_MODE_INFO **b0 = mib;
          MB_MODE_INFO **b1 = mib + hbs;
          MB_MODE_INFO **b2 = mib + hbs * mi_params->mi_stride;
          MB_MODE_INFO **b3 = mib + hbs * mi_params->mi_stride + hbs;

          int further_split = b0[0]->bsize < subsize || b1[0]->bsize < subsize || b2[0]->bsize < subsize || b3[0]->bsize < subsize;

//          if (further_split)
//          printf("\n (%d;%d)    %d;   %d;%d;%d;%d;  \n ", mi_row, mi_col, subsize, b0[0]->bsize, b1[0]->bsize , b2[0]->bsize , b3[0]->bsize);

          int no_skip = !b0[0]->skip_txfm || !b1[0]->skip_txfm || !b2[0]->skip_txfm ||
              !b3[0]->skip_txfm;
          int compound = (b0[0]->ref_frame[1] != b1[0]->ref_frame[1] ||
              b0[0]->ref_frame[1] != b2[0]->ref_frame[1] ||
              b0[0]->ref_frame[1] != b3[0]->ref_frame[1] || b0[0]->ref_frame[1] > NONE_FRAME);

          int different_ref = (b0[0]->ref_frame[0] != b1[0]->ref_frame[0] ||
              b0[0]->ref_frame[0] != b2[0]->ref_frame[0] ||
              b0[0]->ref_frame[0] != b3[0]->ref_frame[0] || b0[0]->ref_frame[0] == INTRA_FRAME);  // no intra

          int different_mode = (b0[0]->mode != b1[0]->mode || b0[0]->mode != b2[0]->mode ||
              b0[0]->mode != b3[0]->mode);
  ...
  //        GLOBALMV,
  //        NEWMV,
          int unsupport_mode = (b0[0]->mode == NEWMV || b0[0]->mode == NEARMV);

          int unsupport_motion_mode = (b0[0]->motion_mode != b1[0]->motion_mode || b0[0]->motion_mode != b2[0]->motion_mode ||
              b0[0]->motion_mode != b3[0]->motion_mode || b0[0]->motion_mode != SIMPLE_TRANSLATION);

          int diffent_filter = (b0[0]->interp_filters.as_int != b1[0]->interp_filters.as_int || b0[0]->interp_filters.as_int != b2[0]->interp_filters.as_int ||
              b0[0]->interp_filters.as_int != b3[0]->interp_filters.as_int);

          // no palette!!!   /////

          // skip block?
          //if (b0[0]->current_qindex != b1[0]->current_qindex || b0[0]->current_qindex != b2[0]->current_qindex   ||  b0[0]->current_qindex != b3[0]->current_qindex  ) continue;

          // Not needed for skip block?
          int different_seg = (b0[0]->segment_id != b1[0]->segment_id || b0[0]->segment_id != b2[0]->segment_id   ||  b0[0]->segment_id != b3[0]->segment_id  );

          int different_mv = (b0[0]->mv[0].as_int != b1[0]->mv[0].as_int ||
              b0[0]->mv[0].as_int != b2[0]->mv[0].as_int ||
              b0[0]->mv[0].as_int != b3[0]->mv[0].as_int);

          int skip_direct_merging = further_split || no_skip || compound || different_ref || different_mode || unsupport_mode || unsupport_motion_mode
          ...
          if (!skip_direct_merging) {
            ...
            // Update contexts.
            av1_set_offsets_without_segment_id(cpi, &tile_data->tile_info, x, mi_row, mi_col, this_mi[0]->bsize);
            ...
            get_ref_frame_use_mask(cpi, x, this_mi[0], mi_row, mi_col, this_mi[0]->bsize, gf_temporal_ref,
            ...
            //if (/*cm->current_frame.frame_number > 140 &&*/ xd->mi_row == 40 && xd->mi_col == 8) {
//              printf("\n mv:   %d, %d, %d;     bs: %d;  skip:%d;   mv:%d;%d;    sub mv:%d;%d; \n ", cm->current_frame.frame_number, xd->mi_row, xd->mi_col,
//                     bsize, this_mi[0]->skip_txfm, frame_mv[NEARESTMV][1].as_mv.row, frame_mv[NEARESTMV][1].as_mv.col,
//                     b0[0]->mv[0].as_mv.row, b0[0]->mv[0].as_mv.col);
            //}

             int continue_merging = 1;
             if (frame_mv[NEARESTMV][1].as_mv.row != b0[0]->mv[0].as_mv.row || frame_mv[NEARESTMV][1].as_mv.col != b0[0]->mv[0].as_mv.col)
               continue_merging = 0;
  ...
  //                         this_mi[0]->bsize, force_skip_low_temp_var, skip_pred_mv);
            ...
            // if found nearestmv is a different mv, continue?????
  ...
  //////////////////////////////////
                ...
                update_ext_partition_context(xd, mi_row, mi_col, this_subsize, this_mi[0]->bsize, this_mi[0]->partition);
  ...
  //    const int bh = mi_size_high[bsize];    xd->height
      ...
      xd->above_txfm_context = cm->above_contexts.txfm[tile_info->tile_row] + mi_col;
...
//                    (subsampling_x + subsampling_y)));
           ...
           //update_cb_offsets(x, this_mi[0]->bsize, subsampling_x, subsampling_y);
            ...
            if (/*cm->current_frame.frame_number > 140 &&*/ mi_row == 40 && mi_col == 8) {
//              struct macroblockd_plane *const pd = &xd->plane[0];
//               struct buf_2d *const dst_buf = &pd->dst;
//               uint8_t *const dst = dst_buf->buf;
//
...
//
//               //if(bsize == 12 && xd->mi_row == 0 && xd->mi_col == 0)
//               val = dst[0 * dst_buf->stride + 4];

//              printf("\n dec   %d, %d, %d;     bs: %d;  skip:%d;   val: %d;    %p; \n ", cm->current_frame.frame_number, xd->mi_row, xd->mi_col,
//                     bsize, mbmi->skip_txfm,  val, &dst[0 * dst_buf->stride + 4] );
              printf("\n par:   %d, %d, %d;     bs: %d; mode: %d;  skip:%d;  mv:%d;%d; \n ", cm->current_frame.frame_number, mi_row, mi_col,
                     this_mi[0]->bsize, this_mi[0]->mode, this_mi[0]->skip_txfm, this_mi[0]->mv[0].as_mv.row, this_mi[0]->mv[0].as_mv.col );
