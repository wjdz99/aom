...
static void write_partition_tree(AV1_COMP *const cpi,
                                 ...
                                 const int mi_col) {
  char filename[128];
  sprintf(filename, "partition_tree_%d", cpi->sb_counter);
  ++cpi->sb_counter;
...
static void verify_write_partition_tree(const AV1_COMP *const cpi,
                                        ...
                                        const int mi_row, const int mi_col) {
  char filename[128];
  sprintf(filename, "verify_partition_tree_%d", cpi->sb_counter - 1);
  FILE *pfile = fopen(filename, "w");
  fprintf(pfile, "%d,%d,%d,%d,%d,", cpi->common.show_frame,
          cpi->common.showable_frame, cpi->common.show_existing_frame,
          cpi->gf_group.update_type[cpi->gf_group.index],
...
static int64_t rd_search_for_fixed_partition(
  ...
  const BLOCK_SIZE subsize = get_partition_subsize(bsize, PARTITION_SPLIT);
  const int subblock_mi_row[4] = { 0, 0, mi_size_high[subsize],
                                   mi_size_high[subsize] };
  const int subblock_mi_col[4] = { 0, mi_size_wide[subsize], 0,
                                   mi_size_wide[subsize] };
  const int inc_step_horz_4[NUM_PART4_TYPES] = { mi_size_high[bsize] / 4, 0 };
  const int inc_step_vert_4[NUM_PART4_TYPES] = { 0, mi_size_wide[bsize] / 4 };
  ...
  switch (partition) {
    ...
    case PARTITION_HORZ_4:
      rd_pick_4partition(cpi, td, tile_data, tp, x, &x_ctx, pc_tree,
                         ...
                         inc_step_horz_4, PARTITION_HORZ_4);
      break;
    case PARTITION_VERT_4:
      rd_pick_4partition(cpi, td, tile_data, tp, x, &x_ctx, pc_tree,
                         pc_tree->vertical4, &part_search_state, &best_rdc,
                         inc_step_vert_4, PARTITION_VERT_4);
      break;
    case PARTITION_SPLIT:
      rdcost = 0;
      for (int idx = 0; idx < SUB_PARTITIONS_SPLIT; ++idx) {
        if (mi_row + subblock_mi_row[idx] >= cm->mi_params.mi_rows ||
            mi_col + subblock_mi_col[idx] >= cm->mi_params.mi_cols) {
          continue;
        }
        rdcost += rd_search_for_fixed_partition(
            cpi, td, tile_data, tp, sms_tree->split[idx],
            mi_row + subblock_mi_row[idx], mi_col + subblock_mi_col[idx],
            subsize, pc_tree->split[idx]);
      }
      break;
    default: assert("invalid partition type."); exit(0);
  }
