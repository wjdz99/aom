...
static void split_partition_search(
  ...
  *part_split_rd = sum_rdc.rdcost;
  ...
  // 2) Current block size not same as super-block size as the final encode
...
static AOM_INLINE bool do_dry_run_encode_for_current_block(
  ...
  // max_partition_size is same as immediate split of super block, then avoid
  ...
  *rd_cost = best_rdc;
  ...
  if (part_search_state.found_best_partition) {
    if (bsize == cm->seq_params->sb_size) {
      ...
      pc_tree_dealloc = 1;
    } else if (do_dry_run_encode_for_current_block(cm->seq_params->sb_size,
                                                   x->sb_enc.max_partition_size,
                                                   pc_tree->index, bsize)) {
