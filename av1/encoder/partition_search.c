...
static void encode_b(const AV1_COMP *const cpi, TileDataEnc *tile_data,
  ...
  if (!dry_run) {
    x->mbmi_ext_frame->cb_offset[PLANE_TYPE_Y] = x->cb_offset[PLANE_TYPE_Y];
    x->mbmi_ext_frame->cb_offset[PLANE_TYPE_UV] = x->cb_offset[PLANE_TYPE_UV];
...
void av1_rd_use_partition(AV1_COMP *cpi, ThreadData *td, TileDataEnc *tile_data,
  ...
  if (do_recon) {
    if (bsize == cm->seq_params.sb_size) {
      ...
      x->cb_offset[PLANE_TYPE_Y] = 0;
      x->cb_offset[PLANE_TYPE_UV] = 0;
