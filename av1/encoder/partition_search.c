...
static void rectangular_partition_search(
  ...
  // mi_pos_rect[NUM_RECT_PARTS][part_idx][0]: mi_row postion of HORZ and VERT
  //                                    partition types for ith sub-partition.
  // mi_pos_rect[NUM_RECT_PARTS][part_idx][1]: mi_col postion of HORZ and VERT
  //                                    partition types for ith sub-partition.
...
static void set_4_part_ctx_and_rdcost(
  ...
  for (PART4_TYPES subpart_idx = 0; subpart_idx < SUB_PARTITIONS_PART4; ++subpart_idx
    cur_part_ctx[subpart_idx] = av1_alloc_pmc(cm, subsize, &td->shared_coeff_buf);
}
