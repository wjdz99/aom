...
void av1_rd_use_partition(AV1_COMP *cpi, ThreadData *td, TileDataEnc *tile_data,
  ...
  if (pc_tree->none == NULL) {
    pc_tree->none = av1_alloc_pmc(cm, mi_row, mi_col, bsize, pc_tree,
  ...
  for (int i = 0; i < SUB_PARTITIONS_SPLIT; ++i) {
    int x_idx = (i & 1) * hbs;
    int y_idx = (i >> 1) * hbs;
    pc_tree->split[i] = av1_alloc_pc_tree_node(mi_row + y_idx, mi_col + x_idx,
  ...
  switch (partition) {
    ...
    case PARTITION_SPLIT:
      for (int i = 0; i < SUB_PARTITIONS_SPLIT; ++i) {
        int x_idx = (i & 1) * hbs;
        int y_idx = (i >> 1) * hbs;
        pc_tree->split[i] = av1_alloc_pc_tree_node(mi_row + y_idx,
          mi_col + x_idx, subsize, pc_tree, PARTITION_SPLIT, i, i == 3, ss_x,
          ss_y);
      }
          ...
          for (int i = 0; i < SUB_PARTITIONS_SPLIT; i++) {
            ...
            if (pc_tree->split[i]->none == NULL) {
              pc_tree->split[i]->none =
                  ...
                  pc_tree->split[i], PARTITION_NONE, i, ss_x, ss_y,
                  &td->shared_coeff_buf);
            }
            pc_tree->split[i]->partitioning = PARTITION_NONE;
  ...
  for (RECT_PART_TYPE i = HORZ; i < NUM_RECT_PARTS; i++) {
    ...
    for (int j = 0; j < SUB_PARTITIONS_RECT; j++) {
      if (cur_ctx[i][j][0] == NULL) {
        cur_ctx[i][j][0] =
            av1_alloc_pmc(cm, mi_pos_rect[i][j][0], mi_pos_rect[i][j][1],
              blk_params.subsize, pc_tree, partition_type, j,
  ...
  for (AB_PART_TYPE ab_part_type = 0; ab_part_type < NUM_AB_PARTS;
    ...
    for (int i = 0; i < SUB_PARTITIONS_AB; i++) {
      ...
      cur_part_ctxs[ab_part_type][i] =
          av1_alloc_pmc(cm, ab_mi_pos[ab_part_type][i][0],
            ab_mi_pos[ab_part_type][i][1], ab_subsize[ab_part_type][i],
            pc_tree, part_type, i, part_search_state->ss_x,
...
static void set_4_part_ctx_and_rdcost(
  ...
  for (PART4_TYPES i = 0; i < SUB_PARTITIONS_PART4; ++i) {
    cur_part_ctx[i] = av1_alloc_pmc(cm, mi_pos[i][0], mi_pos[i][1], subsize,
...
static void set_none_partition_params(const AV1_COMMON *const cm,
  ...
  if (pc_tree->none == NULL)
    pc_tree->none = av1_alloc_pmc(cm, blk_params.mi_row, blk_params.mi_col,
...
static void split_partition_search(
  ...
  for (idx = 0; idx < SUB_PARTITIONS_SPLIT && sum_rdc.rdcost < best_rdc->rdcost;
    ...
    if (pc_tree->split[idx] == NULL) {
      pc_tree->split[idx] = av1_alloc_pc_tree_node(mi_row + y_idx,
        mi_col + x_idx, subsize, pc_tree, PARTITION_SPLIT, idx, idx == 3,
        part_search_state->ss_x, part_search_state->ss_y);
    }    
