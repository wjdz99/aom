...
static int64_t rd_search_for_fixed_partition(
  ...
  switch (partition) {
    ...
    case PARTITION_SPLIT:
      rdcost = 0;
      for (int idx = 0; idx < SUB_PARTITIONS_SPLIT; ++idx) {
        const int next_mi_row = idx < 2 ? mi_row : mi_row + mi_size_high[subsize];
        const int next_mi_col = idx % 2 == 0 ? mi_col : mi_col + mi_size_wide[subsize];
        ...
        rdcost += rd_search_for_fixed_partition(
            ...
            next_mi_row, next_mi_col, subsize, pc_tree->split[idx]);
      }
      break;
