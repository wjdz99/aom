...
static int ml_predict_var_paritioning(AV1_COMP *cpi, MACROBLOCK *x,
...

  {
    ...
    features[feature_idx] = (float)(logf((float)(dc_q * dc_q) / 256.0f + 1.0f) -
                                    means[feature_idx]) /
    ...
    av1_setup_src_planes(x, cpi->source, mi_row, mi_col, 1, bsize);
    {
      ...
      features[feature_idx] =
          (float)(logf((float)var + 1.0f) - means[feature_idx]) /
          sqrt(vars[feature_idx]);
      feature_idx++;
      for (i = 0; i < 4; ++i) {
        ...
        features[feature_idx] =
            (float)(var_ratio - means[feature_idx]) / sqrt(vars[feature_idx]);
        feature_idx++;
      }
    }
