...
static void process_first_pass_stats(AV1_COMP *cpi,
  ...
  if (cpi->oxcf.rc_mode != AOM_Q && current_frame->frame_number == 0 &&
    ...
    if (cpi->lap_enabled && cpi->oxcf.rc_mode == AOM_VBR) {
      /*
       * Accumulate total_stats using available limited no.of stats,
...
void av1_init_single_pass_lap(AV1_COMP *cpi) {
  ...
  if (cpi->oxcf.rc_mode == AOM_Q) {
    ...
    twopass->stats_buf_ctx->total_left_stats = NULL;
    twopass->stats_buf_ctx->total_stats = NULL;
  } else {
    CHECK_MEM_ERROR(cm, twopass->stats_buf_ctx->total_left_stats,
                    aom_calloc(1, sizeof(FIRSTPASS_STATS)));
    CHECK_MEM_ERROR(cm, twopass->stats_buf_ctx->total_stats,
                    aom_calloc(1, sizeof(FIRSTPASS_STATS)));
    av1_twopass_zero_stats(twopass->stats_buf_ctx->total_left_stats);
    av1_twopass_zero_stats(twopass->stats_buf_ctx->total_stats);
    twopass->total_left_stats = twopass->stats_buf_ctx->total_left_stats;
    twopass->total_stats = twopass->stats_buf_ctx->total_stats;
  }
