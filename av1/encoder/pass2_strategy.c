...
static void find_next_key_frame(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame) {
   ...
   * Forcing to key_freq since all frame stats are not available
   * in lap enabled single pass, rc->frames_to_key
   * can point to end of stats which might not be the actual
   * key frame if scenecut is not detected within the available stats.
   */
