...
static int get_twopass_worst_quality(AV1_COMP *cpi, const double av_frame_err,
  ...
  if (av_target_bandwidth <= 0) {
    ...
    const double av_err_per_mb = av_frame_err / (1 - inactive_zone);
...
static double calc_frame_boost(const RATE_CONTROL *rc,
  ...
  frame_boost =
      AOMMAX(baseline_err_per_mb(frame_info) * active_area,
...
#define LAST_ALR_BOOST_FACTOR 0.2f
  ...
  if (is_final_pass)
    twopass->kf_group_error_left -= gf_stats.gf_group_err;
