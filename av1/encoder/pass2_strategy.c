...
static int find_next_scenecut(const FIRSTPASS_STATS *const stats_start,
  ...
  for (int i = first; i <= last; i++) {
    ...
    if (max_prev_ratio < 0.001 && max_next_ratio < 0.001) {
      ...
      double max_sr_fr_ratio = max_sr / stats_start[i].coded_error;
...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (gf_group->index == gf_group->size) {
    ...
    if (rc->frames_since_key == 0 ||
      ...
      rest_frames = AOMMIN(rest_frames, twopass->stats_buf_ctx->stats_in_end -
                                            stats_start + offset);
      rc->frames_till_regions_update = rest_frames;

      identify_regions(stats_start, rc->frames_till_regions_update - offset,
                       offset, rc->regions, &rc->num_regions, rc->cor_coeff);
    }
