...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (gf_group->index == gf_group->size) {
    ...
    /*
     * If last frame is OVERLAY_UDPATE or GF_UDPATE then it is good quality
     * frames. This flag is an indicator to not set another good quality frame
     * consecutively in next gop.
     */
    cpi->gf_state.arf_gf_boost_lst =
        gf_group->update_type[gf_group->size - 1] == OVERLAY_UPDATE ||
        gf_group->update_type[gf_group->size - 1] == GF_UPDATE;
