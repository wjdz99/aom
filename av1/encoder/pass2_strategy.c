...
static void define_gf_group(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame,
  ...
  if (oxcf->pass == 0) {
    if (cpi->oxcf.aq_mode == CYCLIC_REFRESH_AQ) {
      ...
      if (rc->max_gf_interval > MAX_GF_INTERVAL)
...
//  if (rc->baseline_gf_interval > cpi->lookahead->sz + 1)
//    rc->baseline_gf_interval = cpi->lookahead->sz + 1;
//  printf("int %d look %d kf %d\n", rc->baseline_gf_interval,
//         cpi->lookahead->sz, rc->frames_to_key);
    rc->gfu_boost = DEFAULT_GF_BOOST;
    rc->constrained_gf_group =
        (rc->baseline_gf_interval >= rc->frames_to_key) ? 1 : 0;
    // Should we use the alternate reference frame.
    struct lookahead_entry *arf =
        av1_lookahead_peek(cpi->lookahead, rc->baseline_gf_interval - 1);
    const int use_alt_ref =
        allow_alt_ref && (rc->baseline_gf_interval < cpi->oxcf.lag_in_frames) &&
        arf != NULL && (cpi->oxcf.gf_max_pyr_height > MIN_PYRAMID_LVL);
    ...
    for (int cur_index = 0; cur_index < gf_group->size; ++cur_index) {
      const FRAME_UPDATE_TYPE cur_update_type =
          gf_group->update_type[cur_index];
...
static void setup_target_rate(AV1_COMP *cpi, FRAME_TYPE frame_type) {
  ...
  if (cpi->oxcf.pass == 0) {
    rc_set_frame_target(cpi, target_rate, cpi->common.width,
                        cpi->common.height);
  } else {
...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (cpi->oxcf.pass == 2) {
    ...
    twopass->active_worst_quality = 126;
    twopass->baseline_active_worst_quality = 126;//rc->worst_quality;
  }
