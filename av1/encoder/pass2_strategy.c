...
double layer_fraction[MAX_ARF_LAYERS + 1] = {1.0, 0.60, 0.55, 0.60, 0.60, 1.0, 1.0};
...
void av1_twopass_postencode_update(AV1_COMP *cpi) {
  ...
  if (!rc->is_src_frame_alt_ref) {
    ...
    for (i = pyramid_level; i <= MAX_ARF_LAYERS; ++i) {
       ...
       //   AOMMAX(rc->active_best_quality[pyramid_level], cpi->common.base_qindex);
       //}
    }
  }
