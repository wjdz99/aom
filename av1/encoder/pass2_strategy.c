        ...
        if (scenecut_idx != -1) {
          ...
          int is_minor_sc =
              (regions[scenecut_idx].avg_cor_coeff *
                   (1 - p_rc->noise_var[regions[scenecut_idx].start] /
          ...
          if (!(is_last_analysed && not_enough_regions)) {
            ...
            for (int j = cur_start + 1; j < cur_start + min_shrink_int; j++) {
              base_score = (base_score + 1.0) * p_rc->cor_coeff[j + offset];
            }
            ...
            for (int j = cur_start + min_shrink_int; j <= cur_last; j++) {
              ...
              for (int n = j + 1; n <= j + 3 && n <= last_frame; n++) {
                temp_accu_coeff *= p_rc->cor_coeff[n + offset];
                this_score +=
                    temp_accu_coeff * (1 - p_rc->noise_var[n + offset] /
...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (cpi->gf_frame_index == gf_group->size) {
    ...
    if (rc->frames_since_key == 0 || rc->frames_since_key == 1 ||
      ...
      p_rc->regions_offset = rc->frames_since_key;
      ...
      identify_regions(stats_start, rest_frames - offset, offset, p_rc->regions,
                       &p_rc->num_regions, p_rc->cor_coeff, p_rc->noise_var);
    }
    ...
    if (max_gop_length > 16 && oxcf->algo_cfg.enable_tpl_model &&
      ...
      int this_idx = rc->frames_since_key + p_rc->gf_intervals[p_rc->cur_gf_index] -
