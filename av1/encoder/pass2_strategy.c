...
int av1_calc_arf_boost(const TWO_PASS *twopass, const RATE_CONTROL *rc,
  ...
  // if all forward frame stats are not available, we will extrapolate forward
  // boost using backward frames' boost.
  if (i != f_frames) {
  ...
  // Extrapolate the boost by doubling the backward boost score.
  if (extrapolate_f_boost) arf_boost *= 2;
...
#define REDUCE_GF_LENGTH_BY 1
  ...
  if (use_alt_ref) {
    const int forward_frames = ((rc->frames_to_key - 1) - i >= i - 1)
                                   ? i - 1
                                   : AOMMAX(0, (rc->frames_to_key - 1) - i);
