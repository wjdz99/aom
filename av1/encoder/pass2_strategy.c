...
static void twopass_update_bpm_factor(AV1_COMP *cpi, int rate_err_tol) {
  ...
  // const double max_adj = AOMMAX(0.25, (double)(100 - rate_err_tol) / 200.0);
  ...
  if (rc->vbr_bits_off_target && rc->total_actual_bits > 0) {
    ...
    rate_err_factor = AOMMAX(min_adj, AOMMIN(max_adj, rate_err_factor));
  ...
  if ((rate_err_factor < 1.0 && err_estimate > 0) ||
    ...
    twopass->bpm_factor = AOMMAX(min_adj, AOMMIN(max_adj, twopass->bpm_factor));
  }
}
