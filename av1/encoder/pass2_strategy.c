...
// Calculate the number bits to assign to boosted frames in a group.
...
// Calculate the boost factor based on the number bits assigned, i.e. the
...
static int adjust_boost_bits_for_target_level(AV1_COMP *const cpi,
  ...
  for (int index = 0; index < seq_params->operating_points_cnt_minus_1 + 1;
    ...
    const double level_bitrate_limit =
        av1_get_max_bitrate_for_level(target_level,
                                      seq_params->tier[0], seq_params->profile);
    ...
    if (frame_type == 0) {
      ...
      if (bits_assigned > level_enforced_max_arf_bits) {
        ...
        bits_assigned =
            calculate_boost_bits(rc->baseline_gf_interval, rc->gfu_boost,
                                 group_bits);
      }
    } else {
      assert(0);
...
static void find_next_key_frame(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  kf_bits =
      adjust_boost_bits_for_target_level(cpi, kf_bits, twopass->kf_group_bits,
                                         0);
