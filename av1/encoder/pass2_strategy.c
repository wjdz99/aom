...
static int detect_transition_to_still(TWO_PASS *const twopass,
                                      int min_gf_interval, int frame_interval,
                                      int still_interval,
...
static void accumulate_next_frame_stats(
    ...
    TWO_PASS *const twopass, int flash_detected, int frames_since_key,
    int cur_idx, int can_disable_arf, int min_gf_interval,
    GF_GROUP_STATS *gf_stats) {
...
static void average_gf_stats(int total_frame, const FIRSTPASS_STATS *last_stat,
                             GF_GROUP_STATS *gf_stats) {
  if (total_frame) {
      ...
      if (cur_last - cur_start <= active_max_gf_interval) {
        ...
        for (n = cur_start - before_pad; n <= cur_last + after_pad; n++) {
          if (start_pos + n - 1 > twopass->stats_buf_ctx->stats_in_end) {
            ...
            assert(after_pad >= 0);
            break;
          } else if (start_pos + n - 1 < twopass->stats_buf_ctx->stats_in_start) {
