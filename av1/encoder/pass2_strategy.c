...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (rc->frames_till_gf_update_due == 0) {
    ...
    if (rc->intervals_till_gf_calculate_due == 0) {
      calculate_gf_length(cpi, max_gop_length, max_gop_length);
    }
    ...
    if (rc->gf_intervals[rc->cur_gf_index] - 1 > 16) {
      ...
      int temp = rc->gf_intervals[rc->cur_gf_index];
      rc->gf_intervals[rc->cur_gf_index] = 32;
      define_gf_group(cpi, &this_frame, frame_params, max_gop_length, 0);
      printf("32: %d; ", rc->baseline_gf_interval);
      if (!av1_tpl_setup_stats(cpi, 1, frame_params, frame_input)) {
        ...
        rc->gf_intervals[rc->cur_gf_index] = temp;
      }
    } else if (max_gop_length > 16) {
