...
static void define_gf_group(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame,
 ...
 // if (*frame_update_type == LF_UPDATE) *frame_update_type = GF_UPDATE;
    ...
    struct lookahead_entry *arf = av1_lookahead_peek(cpi->lookahead,
   ...
   /*
    allocate_gf_group_bits(cpi, gf_group_bits, gf_group_error_left, gf_arf_bits,
                           frame_params);
                           */
    //for (int ind = 0; ind < gf_group->total
    ////////////////
    return;
  }
...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (rc->frames_till_gf_update_due > 0) {
    ...
    if (cpi->oxcf.pass == 0) {
      ...
      rc_set_frame_target(cpi, target_rate, cpi->common.width, cpi->common.height);
