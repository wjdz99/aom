...
static void calculate_gf_length(AV1_COMP *cpi, int max_gop_length,
  ...
  // Special cases - when KEY frame is the only frame in subgop.
  int is_single_key_subgop =
  ...
  if (has_no_stats_stage(cpi)) {
    for (i = 0; i < MAX_NUM_GF_INTERVALS; i++) {
      ...
      if (curr_frame_type == KEY_FRAME &&
          rc->gf_intervals[i] >= rc->frames_to_key && !is_single_key_subgop)
  ...
  while (count_cuts < max_intervals + 1) {
    ...
    if (i >= rc->frames_to_key) {
      ...
      // or scenecut or application forced key, also if the curr_frame_type is
      // KEY_FRAME, which is the only frame in subgop, then gf-interval will
      // account for KEY_FRAME.
      if (curr_frame_type == KEY_FRAME && !is_single_key_subgop)
...
static void define_gf_group_pass0(AV1_COMP *cpi, FRAME_TYPE curr_frame_type) {
  ...
  // To introduce forward kf, baseline_gf_interval needs to point to KEY frame
