...
static void get_gradient(const double *values, int start, int last,
  ...
  for (int i = start; i <= last; i++) {
    ...
    grad[i] = (values[next] - values[prev]) / 2.0;
  }
}
...
static void get_region_stats(const FIRSTPASS_STATS *stats, const int *is_flash,
  ...
  for (k = 0; k < num_regions; k++) {
    if (regions[k].type != STABLE_REGION &&
      ...
      if (count) regions[k].avg_noise_var /= (double)count;
      analyze_region(stats, k, regions, coeff);
    }
  }
}
...
static int find_stable_regions(const double *filt_intra_err,
                               const double *filt_coded_err,
  ...
  for (i = this_start; i <= this_last; i++) {
    ...
    double mean_intra = 0, var_intra = 0;
    double mean_coded = 0, var_coded = 0;
    for (j = -HALF_WIN; j <= HALF_WIN; j++) {
      int idx = AOMMIN(AOMMAX(i + j, this_start), this_last);
      mean_intra += filt_intra_err[idx] / WINDOW_SIZE;
      var_intra += filt_intra_err[idx] * filt_intra_err[idx] / WINDOW_SIZE;
      mean_coded += filt_coded_err[idx] / WINDOW_SIZE;
      var_coded += filt_coded_err[idx] * filt_coded_err[idx] / WINDOW_SIZE;
    }

    int is_intra_stable = (var_intra / (mean_intra * mean_intra) < 1.01);
    int is_coded_stable = (var_coded / (mean_coded * mean_coded) < 1.02 &&
...
static void cleanup_regions(REGIONS *regions, int *num_regions) {
  int k = 1;
  while (k < *num_regions) {
    if (regions[k - 1].type == regions[k].type ||
...
static void adjust_unstable_region_bounds(const FIRSTPASS_STATS *stats,
  ...
  for (k = 0; k < *num_regions; k++) {
    ...
    if (k > 0) {
      ...
      if (counti > 0) {
        avg_intra_err /= (double)counti;
        avg_coded_err /= (double)counti;
        avg_coeff /= (double)counti;
        ...
        for (j = lasti + 1; j <= regions[k].last; j++) {
          ...
          int coded_small = stats[j].coded_error / avg_intra_err < 0.05;
          int coeff_close =
    ...
    if (k < *num_regions - 1) {
      ...
      if (counti > 0) {
        ...
        for (j = starti - 1; j >= regions[k].start; j--) {
          ...
          int coded_small = stats[j + 1].coded_error / avg_intra_err < 0.05;
          int coeff_close =
  ...
  while (k < *num_regions && (*num_regions) > 1) {
    if (regions[k].type == STABLE_REGION &&
        ((k > 0 &&  // previous regions
          (regions[k].avg_coded_err > regions[k - 1].avg_coded_err * 0.9 ||
           regions[k].avg_cor_coeff < regions[k - 1].avg_cor_coeff * 0.99)) ||
...
static void find_blending_regions(const FIRSTPASS_STATS *stats,
  ...
  while (k < *num_regions) {
    ...
    for (i = regions[k].start; i <= regions[k].last; i++) {
      ...
      if (is_flash[i] || is_flash[i - 1]) continue;
      double grad = stats[i].intra_error - stats[i - 1].intra_error;
      int large_change = fabs(grad) / stats[i].intra_error > 0.05;
      int this_dir = 0;
      if (large_change) {
        this_dir = (grad > 0) ? 1 : -1;
      }
  ...
  while (k < *num_regions) {
    ...
    if (regions[k - 1].type == BLENDING_REGION &&
      ...
      if (to_merge) {
        ...
        // These a possibly two separate blending regions. Mark the boundary
    ...
    if (is_short_blending ||
      ...
      double next_diff =
          (k < *num_regions - 1)
              ? fabs(regions[k].avg_cor_coeff - regions[k + 1].avg_cor_coeff)
              : 1;
      // merge == 0 means to merge with previous.
...
// Offset is the offset from the last keyframe to the first frame here. It
// should be at least 1 because we assume every frame would have valid
// coded_err.
static void identify_regions(const FIRSTPASS_STATS *const stats_start,
                             int total_frames, int offset, REGIONS *regions,
                             int *total_regions, double *coeff) {
  int k;
  ...
  do {
    ...
    int num_regions =
        find_stable_regions(filt_intra_err, filt_coded_err, grad_coded,
    ...
    // find_blending_regions(stats_start, filt_intra_err, this_start, this_last,
    //                       temp_regions, &num_regions);
    ...
    // All regions are finalized. Get final stats.
    get_region_stats(stats_start, is_flash, temp_regions, coeff, num_regions);

    // copy the regions in the scenecut group
    for (k = 0; k < num_regions; k++) {
      regions[k + cur_region] = temp_regions[k];
      regions[k + cur_region].start += offset;
      regions[k + cur_region].last += offset;
    }
    ...
    if (next_scenecut > -1) {
      ...
      regions[cur_region].start = offset + next_scenecut;
      regions[cur_region].last = offset + next_scenecut;
      cur_region++;
  ...
  *total_regions = cur_region;
  ...
  for (int i = 0; i < total_frames; i++) {
    printf(
        "%d: intra: %.2f, filt intra: %.2f"
        ", coded: %.2f, filt_coded: %.2f, sr: %.2f, ratio: %.2f, coeff: "
        "%.2f\n",
        i + offset, stats_start[i].intra_error, filt_intra_err[i],
        stats_start[i].coded_error, filt_coded_err[i],
        stats_start[i].sr_coded_error,
        (stats_start[i].sr_coded_error) /
            AOMMAX(stats_start[i].coded_error, stats_start[i - 1].coded_error),
        coeff[i]);
  }
  printf("\n");

  printf("\n");
  for (k = 0; k < cur_region; k++) {
    printf(
        "region type: %d, from %d to %d, coeff: %.2f, noise: %.2f, intra : "
        "%.2f, coded: %.2f, ratio: %.2f\n",
        regions[k].type, regions[k].start, regions[k].last,
        regions[k].avg_cor_coeff, regions[k].avg_noise_var,
        regions[k].avg_intra_err, regions[k].avg_coded_err,
        regions[k].avg_sr_fr_ratio);
  }
  printf("\n");
}
