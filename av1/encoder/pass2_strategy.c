...
static void analyze_region(const FIRSTPASS_STATS *stats, int k,
  ...
  for (i = regions[k].start; i <= regions[k].last; i++) {
    ...
    regions[k].avg_cor_coeff +=
        stats[i].cor_coeff / (double)(regions[k].last - regions[k].start + 1);
    regions[k].avg_noise_var +=
        stats[i].noise_var / (double)(regions[k].last - regions[k].start + 1);
  }
}
          ...
          if (!(is_last_analysed && not_enough_regions)) {
            ...
            for (int j = cur_start + min_shrink_int; j <= cur_last; j++) {
              ...
              for (int n = j + 1; n <= j + 3 && n <= last_frame; n++) {
                ...
                this_score +=
                    temp_accu_coeff *
                    (1 - stats[n].noise_var / regions[this_reg].avg_intra_err);
                count_f++;
              }
              // preceding frames
              temp_accu_coeff = 1.0;
...
static void estimate_noise(FIRSTPASS_STATS *first_stats,
  ...
  for (this_stats = first_stats; this_stats < first_stats + 2; this_stats++) {
...
void av1_init_second_pass(AV1_COMP *cpi) {
  ...
  mark_flashes(cpi->twopass.stats_buf_ctx->stats_in_start,
               cpi->twopass.stats_buf_ctx->stats_in_end);
  estimate_noise(cpi->twopass.stats_buf_ctx->stats_in_start,
                 cpi->twopass.stats_buf_ctx->stats_in_end);
  estimate_coeff(cpi->twopass.stats_buf_ctx->stats_in_start,
                 cpi->twopass.stats_buf_ctx->stats_in_end);
