...
static void allocate_gf_group_bits(GF_GROUP *gf_group, RATE_CONTROL *const rc,
  ...
  for (int idx = 2; idx <= MAX_ARF_LAYERS; ++idx) {
    arf_depth_bits[idx] = calculate_boost_bits(
        rc->baseline_gf_interval - total_arfs - total_overlays - arf_depth_count[idx],
  ...
  int normal_frames = rc->baseline_gf_interval - total_arfs - total_overlays;

  fprintf(stderr, "normal_frames = %d\n", normal_frames);
...
//      clamp(target_frame_size, 0, AOMMIN(max_bits, (int)total_group_bits));
  ...
  for (int idx = frame_index; idx < gf_group_size; ++idx) {
    ...
    fprintf(stderr, "idx = %d, frame budget = %d\n", idx, gf_group->bit_allocation[idx]);
  }
...
#define REDUCE_GF_LENGTH_BY 1
  ...
  if (use_alt_ref) {
    ...
    gf_group->max_layer_depth_allowed = 5; // cpi->oxcf.gf_max_pyr_height;
  } else {
...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (rc->frames_till_gf_update_due == 0) {
    ...
    if (!av1_tpl_setup_stats(cpi, 1, frame_params, frame_input)) max_gop_length = 16;
    calculate_gf_length(cpi, max_gop_length);
