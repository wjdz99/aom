...
static int define_kf_interval(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame,
  ...
  if (num_frames_to_detect_scenecut == 0) return num_frames_to_next_key;
  ...
  if (!scenecut_detected) frames_to_key = num_frames_to_next_key;
...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (rc->frames_till_gf_update_due == 0) {
    ...
    if (cpi->rc.enable_scenecut_detection)
      num_frames_to_detect_scenecut = MAX_GF_INTERVAL + 1;
    else
