...
static void calculate_gf_length(AV1_COMP *cpi, int max_gop_length,
  ...
  if (has_no_stats_stage(cpi)) {
    for (i = 0; i < MAX_NUM_GF_INTERVALS; i++) {
       ...
       * Simillarly if frames_to_key is 1 due to scenecut or application
       * forced key, also if the curr_frame_type == KEY_FRAME, which is the
       * only frame in subgop, then gf-interval will account for KEY_FRAME
       */
