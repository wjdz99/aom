...
static int test_candidate_kf(AV1_COMP *cpi, TWO_PASS *twopass,
  ...
  if (IMPLIES(rc_mode == AOM_Q, frame_count_so_far >= 3) &&
    ...
    /*
     * In first pass-lap enabled scenario, key frame is decided based on its
     * primary criteria only. Disabling future frames prediction for this
     * scenario, due to limited no.of frame stats availability.
     * */
    if (cpi->lap_enabled) {
...
static int define_kf_interval(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame,
  ...
  if (cpi->lap_enabled) {
    ...
    if (!scenecut_detected) frames_to_key = rc->frames_to_key;
  }
