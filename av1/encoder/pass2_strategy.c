...
static void rank_frames_by_ref_use(AV1_COMP *cpi,
  ...
  for (int frame_index = 0; frame_index < rc->baseline_gf_interval + 1; ++frame_index) {
#if RC_USE_RANKING
    if (use_ranking)
      gf_group->rc_boost_level[frame_index] = level[ranking[frame_index]];
    else
