...
static INLINE int detect_gf_cut(AV1_COMP *cpi, int i, int cur_start,
  ...
  if (!flash_detected) {
    ...
    if (detect_transition_to_still(cpi, i - cur_start, 5, loop_decay_rate,
  ...
  if (i - cur_start >= active_min_gf_interval &&
      // If possible don't break very close to a kf
      (rc->frames_to_key - i >= rc->min_gf_interval) &&
      ((i - cur_start) & 0x01) && !flash_detected &&
  ...
  if (((i - cur_start) >= active_max_gf_interval + 1) &&
...
#define GF_SHRINK_OUTPUT 1  // print output for gf length decision
int determine_high_err_gf(double *errs, int *is_high, double *si, int len,
                          double *ratio
#if GF_SHRINK_OUTPUT
                          ,
                          int gf_start, int gf_end, int before_pad
#endif
) {
  ...
  double mean = 0, mean_low = 0, mean_high = 0, prev_mean_low = 0,
         prev_mean_high = 0;
  ...
  *ratio = mean_high / (mean_low + 0.00001);
  ...
  if (reset) {
    memset(is_high, 0, sizeof(int) * len);
    memset(si, 0, sizeof(double) * len);
  }
  return reset;
}
  ...
  while (count_cuts < max_intervals + 1) {
    ...
    if (cut_here) {
      ...
      if (cur_last - cur_start <= active_max_gf_interval) {
        ...
        int len = before_pad + after_pad + cur_last - cur_start + 1;

        reset = determine_high_err_gf(errs, is_high, si, len, &ratio
#if GF_SHRINK_OUTPUT
                                      ,
                                      cur_start, cur_last, before_pad
#endif
        );
...
static void define_gf_group_pass0(AV1_COMP *cpi,
  ...
  if (cpi->oxcf.aq_mode == CYCLIC_REFRESH_AQ)
    ...
    rc->baseline_gf_interval = rc->gf_intervals[rc->cur_gf_interval];
...
static void define_gf_group(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame,
  ...
  while (i < rc->gf_intervals[rc->cur_gf_interval]) {
    ++i;
