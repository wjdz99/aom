...
static void process_first_pass_stats(AV1_COMP *cpi,
  ...
  if (current_frame->frame_number == 0) {
    ...
    if (use_ml_model_to_decide_flat_gop(rc_cfg) && can_disable_altref(gf_cfg) &&
      ...
      //     "model.\n");
      // Some first pass statistics like tr_coded_error, pcnt_third_ref,
      // frame_avg_wavelet_energy are invalid, i.e., these stats calculations
      // are skipped in the first pass of encoding. As these stats are required
