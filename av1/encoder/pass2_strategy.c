...
#define LAST_ALR_BOOST_FACTOR 0.2f
  ...
  frame_params->show_frame =
      !(gf_group->update_type[cpi->cur_gf_index] == ARF_UPDATE ||
        gf_group->update_type[cpi->cur_gf_index] == INTNL_ARF_UPDATE);
...
static void process_first_pass_stats(AV1_COMP *cpi,
  ...
  if (cpi->oxcf.rc_cfg.mode != AOM_Q && current_frame->frame_number == 0 &&
      cpi->cur_gf_index == 0 && cpi->twopass.stats_buf_ctx->total_stats &&
...
static void setup_target_rate(AV1_COMP *cpi) {
  ...
  int target_rate = gf_group->bit_allocation[cpi->cur_gf_index];
...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  const int update_type = gf_group->update_type[cpi->cur_gf_index];
  frame_params->frame_type = gf_group->frame_type[cpi->cur_gf_index];

  if (cpi->cur_gf_index < gf_group->size && !(frame_flags & FRAMEFLAGS_KEY)) {
    assert(cpi->cur_gf_index < gf_group->size);
  ...
  if (is_stat_consumption_stage(cpi)) {
    if (cpi->cur_gf_index < gf_group->size || rc->frames_to_key == 0)
  ...
  int is_overlay_forward_kf =
      rc->frames_to_key == 0 &&
      gf_group->update_type[cpi->cur_gf_index] == OVERLAY_UPDATE;
  ...
  if (cpi->cur_gf_index == gf_group->size) {
    assert(cpi->common.current_frame.frame_number == 0 ||
           cpi->cur_gf_index == gf_group->size);
    const FIRSTPASS_STATS *const start_position = twopass->stats_in;
    ...
    if (gf_group->update_type[cpi->cur_gf_index] != ARF_UPDATE &&
        rc->frames_since_key > 0)
      process_first_pass_stats(cpi, &this_frame);
...
void av1_twopass_postencode_update(AV1_COMP *cpi) {
  ...
  if (!rc->is_src_frame_alt_ref) {
    const int pyramid_level = cpi->ppi->gf_group.layer_depth[cpi->cur_gf_index];
    int i;
