...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (cpi->gf_frame_index == gf_group->size) {
    ...
    if (max_gop_length > 16 && oxcf->algo_cfg.enable_tpl_model &&
      ...
      int is_last_scenecut = p_rc->gf_intervals[p_rc->cur_gf_index] >= rc->frames_to_key;
      // TODO(angiebird): Figure out why this_region and next_region are -1 in unit
      // test like AltRefFramePresenceTestLarge
      if(this_region != -1) {
        is_last_scenecut |= p_rc->regions[this_region].type == SCENECUT_REGION;
      }
      if(next_region != -1) {
        is_last_scenecut |= p_rc->regions[next_region].type == SCENECUT_REGION;
      }
