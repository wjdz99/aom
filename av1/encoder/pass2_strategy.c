      ...
      if (rc->gf_intervals[rc->cur_gf_index] > 16) {
        ...
        if (gop_length_decision_method != 2) {
          ...
          if (gop_length_decision_method == 1) {
            // Check if tpl stats based on base layer ARF, (base+1) layer ARF,
            // (base+2) layer ARF can decide the GOP length.
            int tpl_gop_length_decision =
                av1_tpl_setup_stats(cpi, 2, frame_params, frame_input);

            if (tpl_gop_length_decision != 2) {
              do_complete_tpl = 0;
              shorten_gf_interval = !tpl_gop_length_decision;
            }
          }
          if (do_complete_tpl) {
            // Decide GOP length based on complete tpl stats.
            shorten_gf_interval =
          ...
          // GOP length is decided based on GF boost and tpl stats based on
          // base layer ARF, (base+1) layer ARF
          shorten_gf_interval =
              (rc->gfu_boost <
