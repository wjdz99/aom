...
static double PID_control_rc_factor(const int64_t this_gop_bits,
  ...
  // The original rc_factor control.
#if !USE_PID_CONTROL
  double rc_factor = 1.0;
  if (target_bandwidth > 0) {
    int rate_error;
    rate_error = (int)((rc->vbr_bits_off_target * 100) / target_bandwidth);
  ...
  return rc_factor;
#endif  // USE_PID_CONTROL

  double rc_factor = 1.0;
  if (this_gop_bits > 0) {
    double rate_error = 0;
    const int64_t this_gop_target_bits = this_gop_target;
    if (last_gop_bits > 0) {
      // PID controller.
      ...
      const double proportional_error =
          (double)(this_gop_target_bits - this_gop_bits) / (double)target_bandwidth;
      ...
      rate_error =
          fclamp(propotional_factor * proportional_error + integral_factor * integral_error + derivative_factor * derivative_error, -1.0, 1.0);
    } else {
      ...
      const double integral_error =
          (double)vbr_bits_off_target / (double)target_bandwidth;
      rate_error =
          fclamp(propotional_factor * proportional_error + integral_factor * integral_error, -1.0, 1.0);
    }
    if (rate_error > 0) {
...
#define LAST_ALR_BOOST_FACTOR 0.2f
  ...
  if ((cpi->oxcf.rc_mode != AOM_Q) && (rc->baseline_gf_interval > 1)) {
    ...
    const double rc_factor = PID_control_rc_factor(rc->this_gop_bits,
                                                   rc->last_gop_bits,
                                                   rc->this_gop_target,
