...
static void calculate_gf_length(AV1_COMP *cpi, int max_gop_length,
  ...
  fprintf(stderr, "frames since key %d, to key %d\n",
    rc->frames_since_key, rc->frames_to_key);

  i = (rc->frames_since_key == 0);
  max_intervals = 1; // cpi->lap_enabled ? 1 : max_intervals;
  int count_cuts = 1;
  ...
  while (count_cuts < max_intervals + 1) {
    ...
    if (i >= rc->frames_to_key) {
      ...
      count_cuts++;
      fprintf(stderr, "reach end of kf group\n");
      break;
    }
  ...
  for (int n = 1; n < count_cuts; n++) {
    ...
    fprintf(stderr, "n = %d, cut_pos (%d, %d)\n", n, cut_pos[n], cut_pos[n-1]);
  }
  rc->cur_gf_index = 0;
...
void av1_gop_bit_allocation(const AV1_COMP *cpi, RATE_CONTROL *const rc,
  ...
  fprintf(stderr, "\n\n baseline gf interval = %d\n\n", rc->baseline_gf_interval);
  fprintf(stderr, "gfu boost = %d\n", rc->gfu_boost);
  for (int idx = 0; idx < gf_group->size; ++idx)
    fprintf(stderr, "idx = %d, bits = %d\n", idx, gf_group->bit_allocation[idx]);
}
...
//      process_first_pass_stats(cpi, &this_frame);
  ...
  fprintf(stderr, "this frame index = %f\n", this_frame.frame);
