...
static void find_next_key_frame(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  boost_score = get_kf_boost_score(cpi, kf_raw_err, &zero_motion_accumulator,
                                   &sr_accumulator, start_position, 0);
  ...
  if (cpi->lap_enabled) {
    if (cpi->oxcf.rc_mode == AOM_Q)
      ...
      // TODO(Aasaipriya): Estimate kf boost using average stats for AOM_Q
      boost_score =
          get_kf_boost_score(cpi, kf_raw_err, &zero_motion_accumulator,
                             &sr_accumulator, start_position, 1);
      rc->kf_boost += (int)boost_score;
    }
  }
