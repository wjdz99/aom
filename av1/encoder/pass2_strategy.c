...
static void twopass_update_bpm_factor(TWO_PASS *twopass, int err_pct, int us, int os) {
  // Overall is there systemic undershoot or overshoot outside permitted levels
  if ((err_pct >= us) && (-err_pct >= os)) {
    // Based on recent history adjust expectations of bits per macroblock.
    double last_group_rate_err =
      (double)twopass->rolling_arf_group_actual_bits /
      DOUBLE_DIVIDE_CHECK((double)twopass->rolling_arf_group_target_bits);

    last_group_rate_err = AOMMAX(0.5, AOMMIN(1.5, last_group_rate_err));
    last_group_rate_err = 1.0 + ((last_group_rate_err - 1.0) / 5.0);

    // Is the last GOP error making the total error worse or better? Only make
    // an adjustment if things are getting worse.
    if ((last_group_rate_err < 1.0 && err_pct > 0) ||
        (last_group_rate_err > 1.0 && err_pct < 0)) {
      twopass->bpm_factor *= last_group_rate_err;
      twopass->bpm_factor = AOMMAX(0.25, AOMMIN(1.75, twopass->bpm_factor));
    }
  }
}
...
static int get_twopass_worst_quality(AV1_COMP *cpi, const double av_frame_err,
  ...
  if (av_target_bandwidth <= 0) {
    ...
    twopass_update_bpm_factor(&cpi->twopass, rc->rate_error_estimate,
        rc_cfg->under_shoot_pct, rc_cfg->over_shoot_pct );
...
void av1_twopass_postencode_update(AV1_COMP *cpi) {
  ...
  //rc->vbr_bits_off_target += rc->this_frame_target - rc->projected_frame_size;
