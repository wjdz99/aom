...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (cpi->gf_frame_index == gf_group->size) {
    ...
    if (max_gop_length > 16 && oxcf->algo_cfg.enable_tpl_model &&
      ...
      // unit test like AltRefFramePresenceTestLarge
      if (this_region != -1 && p_rc->regions[this_region].type == SCENECUT_REGION) {
        is_last_scenecut = 1;
      }
      if (next_region != -1 && p_rc->regions[next_region].type == SCENECUT_REGION) {
        is_last_scenecut = 1;
      }
