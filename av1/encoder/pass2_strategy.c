...
#define REDUCE_GF_LENGTH_BY 1
  ...
  if (use_alt_ref) {
    ...
    set_baseline_gf_interval(cpi, i, active_max_gf_interval);
...
static int define_kf_interval(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame,
                              double *kf_group_err, int total_frames_to_check) {
  TWO_PASS *const twopass = &cpi->twopass;
  RATE_CONTROL *const rc = &cpi->rc;
  const AV1EncoderConfig *const oxcf = &cpi->oxcf;
  ...
  const int lap_delay = cpi->lookahead->max_sz;  // provides lag in frame count
  ...
  if (total_frames_to_check == 0 ||
      (cpi->lap_enabled && lap_delay < MAX_LAP_BUFFERS &&
       total_frames_to_check == MAX_GF_INTERVAL + 1)) {
  ...
  num_frames_to_check = AOMMIN(total_frames_to_check, num_frames_to_check);

  // Call at find_next_key_frame of lag<35 and LAP - accumulate kf_grp_err
  // properly
  ...
  if (cpi->lap_enabled && !scenecut_detected)
    frames_to_key = num_frames_to_app_forced_key;
...
static void find_next_key_frame(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  if (cpi->lap_enabled && frames_to_key == -1)
...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (rc->frames_till_gf_update_due == 0) {
    ...
    const FIRSTPASS_STATS *const start_position = twopass->stats_in;
    int total_frames_to_check, frames_to_key;
    if (cpi->lap_enabled)
      total_frames_to_check = MAX_GF_INTERVAL + 1;
    else
      total_frames_to_check = 0;
    frames_to_key =
        define_kf_interval(cpi, &this_frame, NULL, total_frames_to_check);
    reset_fpf_position(twopass, start_position);
    if (!(cpi->lap_enabled && frames_to_key == -1))
