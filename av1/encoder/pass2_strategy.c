...
void av1_get_second_pass_params(AV1_COMP *cpi,
  ...
  if (cpi->gf_frame_index == gf_group->size) {
    ...
    if (cpi->third_pass_ctx && oxcf->pass == AOM_RC_THIRD_PASS) {
      if (!cpi->third_pass_ctx->file && oxcf->two_pass_output) {
        cpi->third_pass_ctx->file = oxcf->two_pass_output;
      }
      if (cpi->third_pass_ctx->file) {
        int gf_len;
        const int order_hint_bits =
            cpi->common.seq_params->order_hint_info.order_hint_bits_minus_1 + 1;
        int ret = av1_set_gop_third_pass(cpi->third_pass_ctx, gf_group,
                                         order_hint_bits, &gf_len);
        if (ret == 0) {
          p_rc->cur_gf_index = 0;
          p_rc->gf_intervals[0] = gf_len;
          need_gf_len = 0;
        } else {
          // Something went wrong with decoding the two-pass stream. Continue
          // with regular encoding.
          fprintf(stderr,
                  "Failed to decode two-pass bitstream, continuing with "
                  "regular encoding.\n");
