...
static int64_t calculate_total_gf_group_bits(AV1_COMP *cpi,
  ...
  fprintf(stderr, "gf group err = %lf, kf left = %d\n",
    gf_group_err, twopass->kf_group_error_left);

  fprintf(stderr, "total gf bits = %d, kfg bits = %d\n", 
    total_group_bits, twopass->kf_group_bits);
...
static void allocate_gf_group_bits(GF_GROUP *gf_group, RATE_CONTROL *const rc,
  ...
  base_frame_bits =
    (int)(total_group_bits / rc->baseline_gf_interval);
...
//   }
  ...
  while (i < rc->gf_intervals[rc->cur_gf_index]) {
    ...
    if (EOF == input_stats(twopass, &next_frame)) {
      fprintf(stderr, "EOF  fpf index = %f\n", next_frame.frame);
      break;
    }
    
    fprintf(stderr, "fpf index = %f\n", next_frame.frame);
