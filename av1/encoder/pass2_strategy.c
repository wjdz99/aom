...
static double calculate_modified_err_sub(const FRAME_INFO *frame_info,
...
static double calculate_modified_err(const FRAME_INFO *frame_info,
  ...
  return calculate_modified_err_sub(
...
static int detect_transition_to_still(
    const FIRSTPASS_INFO *firstpass_info, int next_stats_index,
    const int min_gf_interval, const int frame_interval, int still_interval,
    const double loop_decay_rate, const double last_decay_rate) {
  ...
  if (frame_interval > min_gf_interval && loop_decay_rate >= 0.999 &&
    ...
    still_interval = AOMMIN(still_interval, stats_left);
    int j;
    // Look ahead a few frames to see if static condition persists...
    for (j = 0; j < still_interval; ++j) {
      const FIRSTPASS_STATS *stats =
          av1_firstpass_info_peak(firstpass_info, next_stats_index + j);
...
static INLINE int detect_gf_cut(AV1_COMP *cpi, int frame_index, int cur_start,
  ...
  if (!flash_detected) {
    ...
    // TODO(angiebird): avoid using twopass_frame.stats_in
    int index =
...
static int test_candidate_kf(const FIRSTPASS_INFO *firstpass_info,
  ...
  const FIRSTPASS_STATS *last_stats =
      av1_firstpass_info_peak(firstpass_info, this_stats_index - 1);
  const FIRSTPASS_STATS *this_stats =
      av1_firstpass_info_peak(firstpass_info, this_stats_index);
  ...
  if (scenecut_mode == ENABLE_SCENECUT_MODE_1) {
    if (firstpass_info->stats_count < 3) {
      return 0;
    } else {
  ...
  if (IMPLIES(rc_mode == AOM_Q, frame_count_so_far >= 3) &&
    ...
    for (i = 1; i <= frames_to_test_after_candidate_key; ++i) {
      ...
      const FIRSTPASS_STATS *local_next_frame =
          av1_firstpass_info_peak(firstpass_info, this_stats_index + i);
      double next_iiratio =
 ...
 *
 * \param[in]    cpi              Top-level encoder structure
 * \param[in]    firstpass_info   struct for firstpass stats
...
static int define_kf_interval(AV1_COMP *cpi,
  ...
  const int num_mbs = (oxcf->resize_cfg.resize_mode != RESIZE_NONE)
                          ...
                          : cpi->common.mi_params.MBs;
  const FIRSTPASS_STATS *this_stats =
      av1_firstpass_info_peak(firstpass_info, 0);
  while (frames_to_key < firstpass_info->stats_count &&
    ...
    if (kf_group_err != NULL) {
      *kf_group_err += calculate_modified_err_sub(
    ...
    this_stats = av1_firstpass_info_peak(firstpass_info, frames_to_key);

    // Provided that we are not at the end of the file...
    if ((cpi->ppi->p_rc.enable_scenecut_detection > 0) && kf_cfg->auto_key &&
        frames_to_key + 1 < firstpass_info->stats_count) {
      const FIRSTPASS_STATS *next_stats =
          av1_firstpass_info_peak(firstpass_info, frames_to_key + 1);
...
static void find_next_key_frame(AV1_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  frames_to_key = define_kf_interval(cpi, &twopass->firstpass_info,
                                     &kf_group_err, kf_cfg->key_freq_max);
