...
#define SPARSE_MTX_CHECK_ALLOC(error_info, lval, expr)                        \
  do {                                                                        \
    lval = (expr);                                                            \
    if (!lval) {                                                              \
      if (error_info) {                                                       \
 ...
 * err_info: struct to store mem error
 *
 * Output:
...
int av1_init_sparse_mtx(const int *rows, const int *cols, const double *values,
  ...
  SPARSE_MTX_CHECK_ALLOC(err_info, sm->row_pos,
                         aom_calloc(num_elem, sizeof(*sm->row_pos)));
  SPARSE_MTX_CHECK_ALLOC(err_info, sm->col_pos,
                         aom_calloc(num_elem, sizeof(*sm->col_pos)));
  SPARSE_MTX_CHECK_ALLOC(err_info, sm->value,
                         aom_calloc(num_elem, sizeof(*sm->value)));
...
int av1_init_combine_sparse_mtx(const SPARSE_MTX *sm1, const SPARSE_MTX *sm2,
  ...
  SPARSE_MTX_CHECK_ALLOC(err_info, sm->row_pos,
                         aom_calloc(sm->n_elem, sizeof(*sm->row_pos)));
  SPARSE_MTX_CHECK_ALLOC(err_info, sm->col_pos,
                         aom_calloc(sm->n_elem, sizeof(*sm->col_pos)));
  SPARSE_MTX_CHECK_ALLOC(err_info, sm->value,
                         aom_calloc(sm->n_elem, sizeof(*sm->value)));
...
int av1_bi_conjugate_gradient_sparse(const SPARSE_MTX *A, const double *b,
  ...
  // initialize
  SPARSE_MTX_CHECK_ALLOC(err_info, r, aom_calloc(bl, sizeof(*r)));
  SPARSE_MTX_CHECK_ALLOC(err_info, r_hat, aom_calloc(bl, sizeof(*r_hat)));
  SPARSE_MTX_CHECK_ALLOC(err_info, p, aom_calloc(bl, sizeof(*p)));
  SPARSE_MTX_CHECK_ALLOC(err_info, p_hat, aom_calloc(bl, sizeof(*p_hat)));
  SPARSE_MTX_CHECK_ALLOC(err_info, Ap, aom_calloc(bl, sizeof(*Ap)));
  SPARSE_MTX_CHECK_ALLOC(err_info, p_hatA, aom_calloc(bl, sizeof(*p_hatA)));
  SPARSE_MTX_CHECK_ALLOC(err_info, x_hat, aom_calloc(bl, sizeof(*x_hat)));
...
int av1_jacobi_sparse(const SPARSE_MTX *A, const double *b, int bl, double *x,
  ...
  SPARSE_MTX_CHECK_ALLOC(err_info, diags, aom_calloc(bl, sizeof(*diags)));
  SPARSE_MTX_CHECK_ALLOC(err_info, Rx, aom_calloc(bl, sizeof(*Rx)));
  SPARSE_MTX_CHECK_ALLOC(err_info, x_last, aom_calloc(bl, sizeof(*x_last)));
  SPARSE_MTX_CHECK_ALLOC(err_info, x_cur, aom_calloc(bl, sizeof(*x_cur)));
...
int av1_steepest_descent_sparse(const SPARSE_MTX *A, const double *b, int bl,
  ...
  SPARSE_MTX_CHECK_ALLOC(err_info, d, aom_calloc(bl, sizeof(*d)));
  SPARSE_MTX_CHECK_ALLOC(err_info, Ax, aom_calloc(bl, sizeof(*Ax)));
  SPARSE_MTX_CHECK_ALLOC(err_info, Ad, aom_calloc(bl, sizeof(*Ad)));
