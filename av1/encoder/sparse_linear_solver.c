...
#define SPARSE_MTX_CHECK_ALLOC(error_info, lval, expr, failexpr) \
  do {                                                           \
    lval = (expr);                                               \
    if (!lval) {                                                 \
      (failexpr);                                                \
...
int av1_bi_conjugate_gradient_sparse(const SPARSE_MTX *A, const double *b,
  ...
  // initialize
  SPARSE_MTX_CHECK_ALLOC(
      err_info, r, aom_calloc(bl, sizeof(*r)),
      free_solver_local_buf(r, r_hat, p, p_hat, Ap, p_hatA, x_hat));
  SPARSE_MTX_CHECK_ALLOC(
      err_info, r_hat, aom_calloc(bl, sizeof(*r_hat)),
      free_solver_local_buf(r, r_hat, p, p_hat, Ap, p_hatA, x_hat));
  SPARSE_MTX_CHECK_ALLOC(
      err_info, p, aom_calloc(bl, sizeof(*p)),
      free_solver_local_buf(r, r_hat, p, p_hat, Ap, p_hatA, x_hat));
  SPARSE_MTX_CHECK_ALLOC(
      err_info, p_hat, aom_calloc(bl, sizeof(*p_hat)),
      free_solver_local_buf(r, r_hat, p, p_hat, Ap, p_hatA, x_hat));
  SPARSE_MTX_CHECK_ALLOC(
      err_info, Ap, aom_calloc(bl, sizeof(*Ap)),
      free_solver_local_buf(r, r_hat, p, p_hat, Ap, p_hatA, x_hat));
  SPARSE_MTX_CHECK_ALLOC(
      err_info, p_hatA, aom_calloc(bl, sizeof(*p_hatA)),
      free_solver_local_buf(r, r_hat, p, p_hat, Ap, p_hatA, x_hat));
  SPARSE_MTX_CHECK_ALLOC(
      err_info, x_hat, aom_calloc(bl, sizeof(*x_hat)),
      free_solver_local_buf(r, r_hat, p, p_hat, Ap, p_hatA, x_hat));
...
int av1_jacobi_sparse(const SPARSE_MTX *A, const double *b, int bl, double *x,
                      struct aom_internal_error_info *err_info) {
  double *diags = NULL, *Rx = NULL, *x_last = NULL, *x_cur = NULL, *tempx = NULL;
  double resi2;

  SPARSE_MTX_CHECK_ALLOC(
      err_info, diags, aom_calloc(bl, sizeof(*diags)),
      free_solver_local_buf(diags, Rx, x_last, x_cur, NULL, NULL, NULL));
  SPARSE_MTX_CHECK_ALLOC(
      err_info, Rx, aom_calloc(bl, sizeof(*Rx)),
      free_solver_local_buf(diags, Rx, x_last, x_cur, NULL, NULL, NULL));
  SPARSE_MTX_CHECK_ALLOC(
      err_info, x_last, aom_calloc(bl, sizeof(*x_last)),
      free_solver_local_buf(diags, Rx, x_last, x_cur, NULL, NULL, NULL));
  SPARSE_MTX_CHECK_ALLOC(
      err_info, x_cur, aom_calloc(bl, sizeof(*x_cur)),
      free_solver_local_buf(diags, Rx, x_last, x_cur, NULL, NULL, NULL));
...
int av1_steepest_descent_sparse(const SPARSE_MTX *A, const double *b, int bl,
  ...
  SPARSE_MTX_CHECK_ALLOC(
      err_info, d, aom_calloc(bl, sizeof(*d)),
      free_solver_local_buf(d, Ax, Ad, NULL, NULL, NULL, NULL));
  SPARSE_MTX_CHECK_ALLOC(
      err_info, Ax, aom_calloc(bl, sizeof(*Ax)),
      free_solver_local_buf(d, Ax, Ad, NULL, NULL, NULL, NULL));
  SPARSE_MTX_CHECK_ALLOC(
      err_info, Ad, aom_calloc(bl, sizeof(*Ad)),
      free_solver_local_buf(d, Ax, Ad, NULL, NULL, NULL, NULL));
