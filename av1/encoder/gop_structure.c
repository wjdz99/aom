...
static void set_multi_layer_params(GF_GROUP *const gf_group, int l, int r,
  ...
  const int num_frames_to_process = r - l - 1;  // Excluding both 'l' and 'r'.
...
static INLINE unsigned char max_pyramid_height_from_width(int pyramid_width) {
...
static void define_pyramid_gf_group_structure(
  ...
  // Count extra arfs used and set rate factor levels.
  // Note: We skip the key-frame in this loop, as it's rate factor is already
  // assigned, and it cannot be an internal arf frame.
  cpi->num_extra_arfs = 0;
  for (int frame_index = key_frame; frame_index < gf_update_frames;
       ++frame_index) {
