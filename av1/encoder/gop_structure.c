...
static INLINE void queue_enqueue(int *queue, int new_item, int *queue_size,
                                 int max_queue_size) {
...
static INLINE void init_ref_frame_idx_array(int *ref_idx) {
  for (int i = 0; i < INTER_REFS_PER_FRAME; ++i) ref_idx[i] = -1;
}
...
static void set_gop_ref_frame_map(GF_GROUP *const gf_group) {
  for (int frame_idx = 0; frame_idx <= gf_group->size; ++frame_idx)
    init_ref_frame_idx_array(gf_group->ref_frame_gop_idx[frame_idx]);
  ...
  for (int frame_idx = 1; frame_idx <= gf_group->size; ++frame_idx) {
    ...
    switch (gf_group->update_type[frame_idx]) {
      ...
      case INTNL_OVERLAY_UPDATE:
        ...
        queue_enqueue(lst_queue, int_arf_idx, &lst_queue_size, 3);
        break;
