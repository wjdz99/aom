...
static int construct_multi_layer_gf_structure(
  ...
  // Rest of the frames.
  set_multi_layer_params(twopass, gf_group, rc, frame_info, 
                         cur_frame_index, gf_interval,
  ...
  if (use_altref) {
    ...
    for (; frame_index <= gf_interval; ++frame_index) {
      ...
      gf_group->refbuf_state[frame_index] = REFBUF_UPDATE;
      gf_group->max_layer_depth =
          AOMMAX(gf_group->max_layer_depth, 2);
      ++cur_frame_index;
    }
  }
  return frame_index;
...
void av1_gop_setup_structure(AV1_COMP *cpi,
  ...
  const FRAME_UPDATE_TYPE first_frame_update_type =
      key_frame ? KF_UPDATE
                : rc->source_alt_ref_active || 1 ? OVERLAY_UPDATE : GF_UPDATE;
