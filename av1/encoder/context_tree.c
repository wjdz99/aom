...
void av1_free_pc_tree_recursive(PC_TREE *pc_tree, int num_planes, int keep_best,
  ...
  if (!keep_best || (partition != PARTITION_SPLIT)) {
    // printf("pointer NULL:%d %d %d %d %d\n",
    // pc_tree->block_size, pc_tree->split[0] == NULL, pc_tree->split[1] ==
    // NULL, pc_tree->split[2] == NULL, pc_tree->split[3] == NULL);

    for (int i = 0; i < 4; ++i) {
      // printf("free split %d\n", i);
      if (pc_tree->split[i] != NULL) {
        printf("real free split %d\n", i);
        av1_free_pc_tree_recursive(pc_tree->split[i], num_planes, keep_best, 0);
