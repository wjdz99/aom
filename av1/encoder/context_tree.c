...
PICK_MODE_CONTEXT *av1_alloc_pmc(const AV1_COMMON *cm, BLOCK_SIZE bsize,
  ...
  /*  printf("ALLOC: [%d %d] %dx%d, p%d\n", mi_row, mi_col,
     block_size_wide[bsize], block_size_high[bsize], partition);*/
...
void av1_free_pmc(PICK_MODE_CONTEXT *ctx, int num_planes) {
  if (ctx == NULL) return;
  /*
     printf("FREE: [%d %d] %dx%d, p%d\n",
     ctx->mi_row, ctx->mi_col, block_size_wide[ctx->bsize],
     block_size_high[ctx->bsize], ctx->partition);*/
...
PC_TREE *av1_alloc_pc_tree_node(BLOCK_SIZE bsize, int mi_row, int mi_col) {
