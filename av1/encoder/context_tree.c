...
PC_TREE *same_block_from_different_partition(PC_TREE *pc_tree, PC_TREE *target);

static PC_TREE *look_for_same_block_helper(PC_TREE *cur, PC_TREE *target) {
  ...
  if (current_bsize == target_bsize) {
    ...
    if (mi_size_wide[current_bsize] >= mi_size_wide[target_bsize] &&
      ...
      return same_block_from_different_partition(cur, target);
    } else {
      return NULL;
    }
  }
...
PC_TREE *same_block_from_different_partition(PC_TREE *pc_tree,
                                             PC_TREE *target) {
  if (pc_tree == NULL || pc_tree == target) return NULL;

  PC_TREE *result;
  result = look_for_same_block_helper(pc_tree->split[0], target);
  if (result) return result;
  result = look_for_same_block_helper(pc_tree->horizontal[0], target);
  if (result) return result;
  result = look_for_same_block_helper(pc_tree->vertical[0], target);
  if (result) return result;
  result = look_for_same_block_helper(pc_tree->horizontal3[0], target);
  if (result) return result;
  result = look_for_same_block_helper(pc_tree->vertical3[0], target);
  if (result) return result;

  return NULL;
}

PC_TREE *av1_look_for_searched_block(PC_TREE *pc_tree) {
  if (!pc_tree) return 0;

  // Find the biggest possible common parent node
  PC_TREE *current = pc_tree;
  while (current->index == 0 && current->parent) {
    current = current->parent;
  }

  // Starting from the parent node, search down
  return same_block_from_different_partition(current, pc_tree);
}
