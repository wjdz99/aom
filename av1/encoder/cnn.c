  ...
  if (layer_config->maxpool &&
    ...
    switch (layer_config->pad) {
      case PADDING_SAME_ZERO:
        av1_maxpool_one_padding_zero(input, in_width, in_height, in_stride,
                                     layer_config, output, out_stride, cstep,
                                     filter_height_half, filter_width_half);
        break;
      case PADDING_SAME_REPLICATE:
        av1_maxpool_one_padding_replicate(
            input, in_width, in_height, in_stride, layer_config, output,
            out_stride, cstep, filter_height_half, filter_width_half);
        break;
      case PADDING_VALID:
        av1_maxpool_one_padding_valid(input, in_width, in_height, in_stride,
                                      layer_config, output, out_stride, cstep);
        break;
    ...
    if (layer_config->filter_height == 1 && layer_config->filter_width == 1) {
      av1_element_wise_convole(input, in_width, in_height, in_stride,
    ...
    switch (layer_config->pad) {
      case PADDING_SAME_ZERO: {
        av1_maxpool_zero_padding_zero(
            input, in_width, in_height, in_stride, layer_config, output,
            out_stride, start_idx, cstep, filter_height_half, filter_width_half,
            ii_shift, jj_shift, channel_step);
        break;
      }
      case PADDING_SAME_REPLICATE: {
        av1_maxpool_zero_padding_replicate(
            input, in_width, in_height, in_stride, layer_config, output,
            out_stride, start_idx, cstep, ii_shift, jj_shift, channel_step);
        break;
      }
      case PADDING_VALID: {
        av1_maxpool_zero_padding_valid(input, in_width, in_height, in_stride,
