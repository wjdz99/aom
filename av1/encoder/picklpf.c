...
static int64_t try_filter_frame(const YV12_BUFFER_CONFIG *sd,
  ...
  if (cm->use_highbitdepth) {
    filt_err = aom_highbd_get_sse_plane(sd, cm->frame_to_show, plane);
  } else {
  ...
  aom_yv12_copy_plane(&cpi->last_frame_uf, cm->frame_to_show, plane);
#else
#if CONFIG_HIGHBITDEPTH
  if (cm->use_highbitdepth) {
...
#if CONFIG_UV_LVL
  aom_yv12_copy_plane(cm->frame_to_show, &cpi->last_frame_uf, plane);
#else
