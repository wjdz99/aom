...
// range.
  ...
  if (cpi->common.current_video_frame < 5) {
    FILE *fp = fopen("lf_data_128_blocks_with_border.txt", "a");
    ...
    assert(num_sse_blocks_ver*num_sse_blocks_hor < 500);
    // 500 should be enough up to FullHD resolution with current SSE block size
    int64_t err_vals[(MAX_LOOP_FILTER + 1)*500];
    int sse_block_idx;

#define SSE_BORDER_SZ 2 // Estimate SSE in a bit smaller subregion to limit
                        // possible influence of source values outside of the
                        // the current block on loop filtering result
    ...
    for (int i = 0; i <= MAX_LOOP_FILTER; ++i) {
      ...
      buf1.y_crop_width = buf2.y_crop_width = sse_block_size - 2*SSE_BORDER_SZ;
      buf1.y_crop_height = buf2.y_crop_height = sse_block_size - 2*SSE_BORDER_SZ;
      sse_block_idx = 0;
      for(int start_row = 0; start_row < height - sse_block_size; start_row += sse_stride)
        ...
        {
          buf1.y_buffer = buf1_y + (start_row + SSE_BORDER_SZ)*buf1.y_stride + start_col + SSE_BORDER_SZ;
          buf2.y_buffer = buf2_y + (start_row + SSE_BORDER_SZ)*buf2.y_stride + start_col + SSE_BORDER_SZ;
