...
#define SUBBLOCKS_IN_OCTILE (SUBBLOCKS_IN_SB / 8)
  ...
  // and next octile. Make sure superblock indexes are always within bounds.
  assert(octile >= 1 && octile <= 8);
  const int middle_index = octile * SUBBLOCKS_IN_OCTILE - 1;
  const int lower_index = AOMMAX(0, middle_index - SUBBLOCKS_IN_OCTILE);
  const int upper_index =
      AOMMIN(SUBBLOCKS_IN_SB - 1, middle_index + SUBBLOCKS_IN_OCTILE);

  // Weigh the three variances in a 2:1:1 ratio. This allows for smoother
  // delta-q transitions among superblocks with mixed-variance features.
  const unsigned int variance =
      ((variances[middle_index] * 2) + variances[lower_index] +
       variances[upper_index]) /
      4;
