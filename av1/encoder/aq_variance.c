...
#define VAR_BOOST_SUBBLOCKS_IN_SB_DIM 8
#define VAR_BOOST_SUBBLOCK_SIZE 8
...
// Comparer used by av1_get_block_variance_boost() to order subblock variances
// from smallest to largest.
static int variance_comp_unsigned_int(const void *a, const void *b) {
...
unsigned int av1_get_block_variance_boost(const AV1_COMP *cpi,
  ...
  for (int subb_i = 0; subb_i < VAR_BOOST_SUBBLOCKS_IN_SB_DIM; subb_i++) {
    int i = subb_i * VAR_BOOST_SUBBLOCK_SIZE;
    for (int subb_j = 0; subb_j < VAR_BOOST_SUBBLOCKS_IN_SB_DIM; subb_j++) {
      int j = subb_j * VAR_BOOST_SUBBLOCK_SIZE;
      if (is_cur_buf_hbd(xd)) {
        variances[subb_i * VAR_BOOST_SUBBLOCKS_IN_SB_DIM + subb_j] =
  ...
  qsort(&variances, 64, sizeof(unsigned int), variance_comp_unsigned_int);

  // Take the 8x8 variance value in the specified octile.
  assert(octile >= 1 && octile <= 8);
  const unsigned int variance = variances[octile * 8 - 1];
