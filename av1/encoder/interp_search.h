...
/*!\brief Struct holding data used to speed up the motion mode search.
 * \param[in]    motion_mode_cand A motion_mode_candidate struct to store motion mode information used in a speed feature to search motion modes other than SIMPLE_TRANSLATION only on winning candidates.
 */
typedef struct {
  /*!
  ...
  */
  ...
  /*!
  * Holds an estimated entropy cost for picking the current reference frame. This is used
  * to compute an rd estimate.
  */
  int ref_frame_cost;
  /*!
  * Holds an estimated entropy cost for picking single or compound reference. This is used
  * to compute an rd estimate.
  */
  int single_comp_cost;
  /*!
  * Pointer to the first element in a 3D array holding rd's of SIMPLE_TRANSLATION used
  * to prune out the motion mode search in single ref modes used to determine compound 
  * ref modes. The full structure is: 
