...
void av1_configure_buffer_updates(AV1_COMP *const cpi,
  ...
  const ExtRefreshFrameFlagsInfo *const refresh_frame =
  ...
  if (refresh_frame->refresh_frame_flags_pending &&
      (!is_stat_generation_stage(cpi))) {
    frame_params->refresh_golden_frame = refresh_frame->golden_frame;
    frame_params->refresh_alt_ref_frame = refresh_frame->alt_ref_frame;
    frame_params->refresh_bwd_ref_frame = refresh_frame->bwd_ref_frame;
  }
...
static INLINE int is_frame_droppable(
    const SVC *const svc, const ExtRefreshFrameFlagsInfo *const refresh_frame) {
  ...
  else if (refresh_frame->refresh_frame_flags_pending)
...
int av1_get_refresh_frame_flags(const AV1_COMP *const cpi,
  ...
  if (is_frame_droppable(svc, refresh_frame)) return 0;
...
int av1_encode_strategy(AV1_COMP *const cpi, size_t *const size,
  ...
  *time_end = source->ts_end;
  ...
  if (!is_stat_generation_stage(cpi)) {
    ...
    if (!ext_flags->refresh_frame.refresh_frame_flags_pending) {
