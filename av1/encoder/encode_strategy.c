...
static void adjust_frame_rate(AV1_COMP *cpi, int64_t ts_start, int64_t ts_end) {
  ...
  if (ts_start == cpi->time_stamps.first_ever) {
    this_duration = ts_end - ts_start;
    step = 1;
  } else {
    int64_t last_duration =
        cpi->time_stamps.last_end_seen - cpi->time_stamps.last_start_seen;

    this_duration = ts_end - cpi->time_stamps.last_end_seen;
  ...
  if (this_duration) {
    if (step) {
      ...
      const double interval =
          AOMMIN((double)(ts_end - cpi->time_stamps.first_ever), 10000000.0);
  ...
  cpi->time_stamps.last_start_seen = ts_start;
  cpi->time_stamps.last_end_seen = ts_end;
}
...
int av1_encode_strategy(AV1_COMP *const cpi, size_t *const size,
  ...
  *time_end = source->ts_end;
  if (source->ts_start < cpi->time_stamps.first_ever) {
    ...
    cpi->time_stamps.last_end_seen = source->ts_start;
  }
