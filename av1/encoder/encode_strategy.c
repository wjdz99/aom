...
/*
static void add_ref_to_slot(MapIdxPair *ref, int *const remapped_ref_idx) {
...
static int get_ref_frames_subgop(AV1_COMP *const cpi) {
...
//int max_level_ind = 0;
  ...
  if (golden_frame < 0) {
    ...
    printf("ADD GOLD %d, frame %d\n", buffer_map[golden_frame].disp_order, -1);
  }
  ...
  int oldest_ind = -1;
  int last3_ind = -1;
  int use_low_level_last3 = n_past_high_level < 4 && n_bufs > ALTREF_FRAME;
  ...
  if (n_bufs >= ALTREF_FRAME) {
    ...
    printf("ADD L3 %d, frame %d\n", buffer_map[last3_ind].disp_order, -1);
  }
  ...
  for (int i = 0; i < n_min_level_refs; i++) {
    ...
    printf("ADD L1 %d, frame %d\n", cur_buf->disp_order, cur_frame_assign);
    cur_buf->used = 1;
    cur_frame_assign--;
  }
...
//  printf("ASSIGN\n");
    int done = get_ref_frames_subgop(cpi); 
    if (done) return;
//}
