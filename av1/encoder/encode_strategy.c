...
int av1_encode_strategy(AV1_COMP *const cpi, size_t *const size,
  ...
  *frame_flags = (source->flags & AOM_EFLAG_FORCE_KF) ? FRAMEFLAGS_KEY : 0;
  ...
  // This is used in rtc encoding.
  if (is_psnr_calc_enabled(cpi) && cpi->sf.rt_sf.use_rtc_tf && cpi->common.current_frame.frame_type!= KEY_FRAME) {
    assert(cpi->orig_source.buffer_alloc_sz);
