...
int av1_encode_strategy(AV1_COMP *const cpi, size_t *const size,
  ...
  // For SVC: set the last_source to the source_last_ref, source frame of
  // LAST reference (base TL0).
  if (cpi->ppi->use_svc) {
  ...
  *size = frame_results.size;
  ...
  // refresh of LAST reference (base temporal layer). Copy for the top
  // spatial enhancement layer only so all spatial layers of the next
  // superframe have last_source to be aligned with previous TL0 superframe. 
  if (cpi->ppi->use_svc &&
      cpi->svc.spatial_layer_id == cpi->svc.number_spatial_layers - 1 &&
      cpi->svc.temporal_layer_id == 0) {
    aom_yv12_copy_y(cpi->unscaled_source, &cpi->svc.source_last_ref);
    aom_yv12_copy_u(cpi->unscaled_source, &cpi->svc.source_last_ref);
    aom_yv12_copy_v(cpi->unscaled_source, &cpi->svc.source_last_ref);
  }
