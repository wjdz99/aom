...
static struct lookahead_entry *setup_arf_frame(
  ...
  *code_arf = 0;
  ...
  struct lookahead_entry *source =
      av1_lookahead_peek(cpi->lookahead, arf_src_index, ENC_STAGE);
...
static int is_forced_keyframe_pending(struct lookahead_ctx *lookahead,
  ...
  for (int i = 0; i <= up_to_index; i++) {
    const struct lookahead_entry *e =
        av1_lookahead_peek(lookahead, i, ENC_STAGE);
    if (e == NULL) {
  ...
  *code_arf = 0;
  ...
  if (!source) {
    ...
    if (cm->current_frame.frame_number > 0) {
      *last_source = av1_lookahead_peek(cpi->lookahead, -1, ENC_STAGE);
    }
    // Read in the source frame.
    source = av1_lookahead_pop(cpi->lookahead, *flush, ENC_STAGE);
...
static int allow_show_existing(const AV1_COMP *const cpi,
  ...
  const struct lookahead_entry *lookahead_src =
      av1_lookahead_peek(cpi->lookahead, 0, ENC_STAGE);
