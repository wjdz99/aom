...
static INLINE void set_frame_refresh_flags(
    FrameRefreshFlags *const frame_refresh_flags, int refresh_gf,
    int refresh_bwdref, int refresh_arf) {
  frame_refresh_flags->golden_frame = refresh_gf;
  frame_refresh_flags->bwd_ref_frame = refresh_bwdref;
  frame_refresh_flags->alt_ref_frame = refresh_arf;
}

void av1_configure_buffer_updates(AV1_COMP *const cpi,
                                  FrameRefreshFlags *const frame_refresh_flags,
  ...
  switch (type) {
    case KF_UPDATE:
      set_frame_refresh_flags(frame_refresh_flags, 1, 1, 1);
      break;

    case LF_UPDATE:
      set_frame_refresh_flags(frame_refresh_flags, 0, 0, 0);
      break;

    case GF_UPDATE:
      set_frame_refresh_flags(frame_refresh_flags, 1, 0, 0);
      break;
    ...
    case ARF_UPDATE:
      ...
      set_frame_refresh_flags(frame_refresh_flags, 0, 0, 1);
      break;
    ...
    case INTNL_ARF_UPDATE:
      set_frame_refresh_flags(frame_refresh_flags, 0, 1, 0);
      break;
  ...
  if (ext_refresh_frame_flags->update_pending &&
    ...
    set_frame_refresh_flags(frame_refresh_flags,
                            ext_refresh_frame_flags->golden_frame,
                            ext_refresh_frame_flags->bwd_ref_frame,
                            ext_refresh_frame_flags->alt_ref_frame);

  if (force_refresh_all) set_frame_refresh_flags(frame_refresh_flags, 1, 1, 1);
}
  ...
  if (source != NULL) {
    ...
    if (arf_src_index == rc->frames_to_key) {
      ...
      if (oxcf->arnr_max_frames > 0) {
        ...
        av1_configure_buffer_updates(cpi, &frame_params->frame_refresh_flags,
                                     frame_update_type, 0);
        *code_arf =
...
static void update_frame_flags(
  ...
  if (frame_refresh_flags->golden_frame == 1) {
    *frame_flags |= FRAMEFLAGS_GOLDEN;
  } else {
    *frame_flags &= ~FRAMEFLAGS_GOLDEN;
  }

  if (frame_refresh_flags->alt_ref_frame == 1) {
    *frame_flags |= FRAMEFLAGS_ALTREF;
  } else {
    *frame_flags &= ~FRAMEFLAGS_ALTREF;
  }

  if (frame_refresh_flags->bwd_ref_frame == 1) {
    *frame_flags |= FRAMEFLAGS_BWDREF;
  } else {
    *frame_flags &= ~FRAMEFLAGS_BWDREF;
  }
...
int av1_encode_strategy(AV1_COMP *const cpi, size_t *const size,
  ...
  *time_end = source->ts_end;
  ...
  if (!is_stat_generation_stage(cpi)) {
    ...
    // flags
    update_frame_flags(&cpi->common, &cpi->frame_refresh_flags, frame_flags);
    if (!ext_flags->refresh_frame.update_pending) {
      int ref_map_index =
