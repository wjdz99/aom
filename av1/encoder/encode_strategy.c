...
static void set_unmapped_ref(RefBufMapData *buffer_map, int n_bufs,
                             int update_arf, int min_level, int cur_frame_disp) {
  int max_dist = 0;
  int unmapped_idx = -1;
  ...
  printf("UNMAPPED %d\n", buffer_map[unmapped_idx].disp_order);
...
static void get_ref_frames_subgop(
  ...
  set_unmapped_ref(buffer_map, n_bufs, update_arf, min_level, cur_frame_disp);
