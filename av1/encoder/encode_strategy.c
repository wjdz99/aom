...
static int get_current_frame_ref_type(
  ...
  switch (cpi->ppi->gf_group.layer_depth[cpi->cur_gf_index]) {
    case 0: return 0;
    case 1: return 1;
    case MAX_ARF_LAYERS:
...
static int choose_primary_ref_frame(
  ...
  // cpi->ppi->gf_group.layer_depth[cpi->cur_gf_index], which also controls
...
static struct lookahead_entry *choose_frame_source(
  ...
  int src_index = gf_group->arf_src_offset[cpi->cur_gf_index];
...
static int denoise_and_encode(AV1_COMP *const cpi, uint8_t *const dest,
  ...
  if (apply_filtering) {
    ...
    int arf_src_index = gf_group->arf_src_offset[cpi->cur_gf_index];
  ...
  if (allow_tpl == 0) {
    ...
    if (cpi->cur_gf_index == 0) av1_init_tpl_stats(&cpi->tpl_data);
  } else {
...
int av1_encode_strategy(AV1_COMP *const cpi, size_t *const size,
    ...
    if (oxcf->kf_cfg.fwd_kf_enabled &&
      ...
      frame_params.show_existing_frame =
          (cpi->show_existing_alt_ref &&
           gf_group->update_type[cpi->cur_gf_index] == OVERLAY_UPDATE) ||
          gf_group->update_type[cpi->cur_gf_index] == INTNL_OVERLAY_UPDATE;
    }
    frame_params.show_existing_frame &= allow_show_existing(cpi, *frame_flags);

    // Reset show_existing_alt_ref decision to 0 after it is used.
    if (gf_group->update_type[cpi->cur_gf_index] == OVERLAY_UPDATE) {
  ...
  *frame_flags = (source->flags & AOM_EFLAG_FORCE_KF) ? FRAMEFLAGS_KEY : 0;
  ...
  if (cpi->oxcf.speed >= 5 && cm->number_spatial_layers == 1 &&
    ...
    av1_set_reference_structure_one_pass_rt(cpi, cpi->cur_gf_index == 0);
#else
  ...
  FRAME_UPDATE_TYPE frame_update_type =
      get_frame_update_type(gf_group, cpi->cur_gf_index);
  ...
  if (!is_stat_generation_stage(cpi)) {
    ...
    frame_params.order_offset = gf_group->arf_src_offset[cpi->cur_gf_index];
