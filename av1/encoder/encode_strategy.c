...
// TODO(bohanli): check if we should add tpl in av1_encoder().
static int denoise_and_encode(AV1_COMP *const cpi, uint8_t *const dest,
  ...
  if (frame_params->frame_type == KEY_FRAME) {
    ...
    // ARF
    apply_filtering = oxcf->arnr_max_frames > 0;
    if (apply_filtering) {
  ...
  if (frame_params->frame_type == KEY_FRAME) {
    ...
    // Do tpl after ARF is filtered, or if no ARF, at the first of GF group.
    int which_frame_tpl = AOMMAX(cpi->gf_group.arf_index, 0);
    allow_tpl = allow_tpl && cpi->gf_group.index == which_frame_tpl;
  }
  if (allow_tpl) {
    av1_configure_buffer_updates(cpi, &frame_params->refresh_frame,
                                 get_frame_update_type(&cpi->gf_group), 0);
