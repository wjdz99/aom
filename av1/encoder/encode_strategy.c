  ...
  *time_end = source->ts_end;
  ...
  if (!frame_params.show_existing_frame) {
    ...
    if (gf_cfg->lag_in_frames > 0 && !is_stat_generation_stage(cpi)) {
      ...
      // If a group has no arf, then do it at the first frame.
      // But if it starts with a key frame then for now it is done in
      // function denoise_and_encode.
      // TODO(bohanli): Unify it with arf generation.
      if (which_frame_tpl < 0 && gf_group->update_type[0] != KF_UPDATE) {
        which_frame_tpl = 0;
      }
      if (cpi->gf_group.index == which_frame_tpl && oxcf->enable_tpl_model) {
        av1_configure_buffer_updates(cpi, &frame_params.refresh_frame,
                                     frame_update_type, 0);
