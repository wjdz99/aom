...
static void get_ref_frames_subgop(AV1_COMP *const cpi) {
  ...
  const int cur_frame_disp =
      cpi->common.current_frame.frame_number + order_offset;
   int cur_pyr_level = 
        get_true_pyr_level(gf_group.layer_depth[gf_group.index], 
                           cur_frame_disp, 
                           gf_group.max_layer_depth);
//const SubGOPStepCfg *step_gop_cfg = get_subgop_step(&gf_group, gf_group.index);
//assert(step_gop_cfg != NULL);
//const FRAME_TYPE_CODE type_code = step_gop_cfg->type_code;

  for (int i = 0; i < REF_FRAMES; ++i) remapped_ref_idx[i] = INVALID_IDX;

  MapIdxPair buffer_map[REF_FRAMES];
  int n_bufs = 0;
  ...
  for (int map_idx = 0; map_idx < REF_FRAMES; map_idx++) {
    ...
    const int reference_frame_level = buf->pyramid_level;
      //get_true_pyr_level(
      //buf->pyramid_level, frame_order, cpi->gf_group.max_layer_depth);
    printf("(%d %d) ", reference_frame_level, frame_order);
  ...
  qsort(buffer_map, n_bufs, sizeof(buffer_map[0]),
        compare_map_idx_pair_asc);

  int min_level_ref_idxs[REF_FRAMES] = { 0 };
...
//int max_level_ind = 0;
  ...
  // Find various necessary information about what frames are available
  for (int i = n_bufs - 1; i >= 0; i--) {

    // Record all frames that are at the lowest pyramid level
    if (buffer_map[i].pyr_level == min_level) {
      min_level_ref_idxs[n_min_level_refs] = i;
      n_min_level_refs++;
      // Find the golden frame, which is the closest past frame at the lowest level
      if (buffer_map[i].disp_order < cur_frame_disp && golden_frame < 0) {
        golden_frame = i; 
      }
    } 
...
/*
    // Find the furthest frame at the highest level. This frame will be excluded
    // from being added to the reference slot if we have 8 reference buffers
    if (buffer_map[i].pyr_level == max_level) { 
      int diff = abs(buffer_map[i].disp_order - cur_frame_disp);
...
*/
    // Keep track of where the frames change from being past frames and future frames
...
/*
  // Exclude the furthest frame at the highest level from being added to a named slot
  // if there are more buffers than slots
  if (n_bufs > ALTREF_FRAME) buffer_map[max_level_ind].used = 1;
*/

  // Assign the found golden frame
  if (golden_frame < 0) {
    // TODO(sarahparker)
  } else {
    remapped_ref_idx[GOLDEN_FRAME - LAST_FRAME] = buffer_map[golden_frame].map_idx;
    buffer_map[golden_frame].used = 1;
  }

  FRAME_UPDATE_TYPE frame_update_type = get_frame_update_type(&gf_group);
  const int update_arf = frame_update_type == ARF_UPDATE && cur_pyr_level == min_level && 
                         n_min_level_refs > 2;
  if (update_arf) printf("UPDATE ARF\n");
  int oldest_ind = -1;
  int last3_ind = -1;
  int use_low_level_last3 = n_past_high_level < 4;
  // TODO(sarahparker) clean this up
  for (int i = 0; i < n_bufs; i++) {
    if (buffer_map[i].disp_order == cur_frame_disp && buffer_map[i].pyr_level != min_level) {
      buffer_map[i].used = 1;
      remapped_ref_idx[BWDREF_FRAME - LAST_FRAME] = buffer_map[i].map_idx;
    } else if (n_bufs > ALTREF_FRAME && update_arf && oldest_ind < 0 && buffer_map[i].pyr_level == min_level) {
      oldest_ind = i;
    } else if (n_bufs > ALTREF_FRAME && i != golden_frame && oldest_ind < 0 && buffer_map[i].pyr_level != min_level) {
      oldest_ind = i;
    } else if (last3_ind < 0 && (buffer_map[i].pyr_level != min_level || (use_low_level_last3 && buffer_map[i].pyr_level == min_level))) {
      last3_ind = i; 
    }
  }
//if (last3_ind > cur_frame_disp && n_min_level_refs > 2) {
//  for (int i = 0; i < n_bufs; i++) {
//    if (buffer_map[i].pyr_level == min_level && buffer_map[i].disp_order < cur_frame_disp) {
//    last3_ind = i; 
//    }
//  }
//}

  // Keep oldest frame in 8th buffer slot
  if (n_bufs > ALTREF_FRAME) {
    buffer_map[oldest_ind].used = 1;
  }
  // Map LAST3 FRAME
  if (n_bufs >= ALTREF_FRAME) {
    buffer_map[last3_ind].used = 1;
    remapped_ref_idx[LAST3_FRAME - LAST_FRAME] = buffer_map[last3_ind].map_idx;
  }
  
  printf("OLDEST %d\n", buffer_map[oldest_ind].disp_order);

  // Assign low level frames first
  int cur_frame_assign = ALTREF_FRAME;
  for (int i = 0; i < n_min_level_refs; i++) {
    int buf_idx = min_level_ref_idxs[i]; 
    MapIdxPair *cur_buf = &buffer_map[buf_idx];
    if (cur_buf->used) continue;
    if (cur_frame_assign == GOLDEN_FRAME) cur_frame_assign--;
    remapped_ref_idx[cur_frame_assign - LAST_FRAME] = cur_buf->map_idx;
    cur_buf->used = 1;
    cur_frame_assign--;
  }

  // Place remaining past frames
  int buf_map_idx = closest_past_ref;
  for (int frame = LAST_FRAME; frame < REF_FRAMES; frame++) {
    ...
    if (buffer_map[buf_map_idx].used) break;
    remapped_ref_idx[frame - LAST_FRAME] = buffer_map[buf_map_idx].map_idx;
    printf("ADD PAST %d, frame %d\n", buffer_map[buf_map_idx].disp_order, frame);
    buffer_map[buf_map_idx].used = 1;
  }
