...
static int get_ref_frames_subgop(
  ...
  const int cur_frame_disp =
      cpi->common.current_frame.frame_number + order_offset;
  int cur_pyr_level =
      get_true_pyr_level(gf_group.layer_depth[gf_group.index], cur_frame_disp,
  ...
  if (n_bufs < ALTREF_FRAME) return 0;
  ...
  const int update_arf = frame_update_type == ARF_UPDATE &&
                         cur_pyr_level == min_level && n_min_level_refs > 2;
...
void av1_get_ref_frames(AV1_COMP *const cpi, RefBufferStack *ref_buffer_stack,
  ...
  if (!cpi->oxcf.algo_cfg.enable_tpl_model &&
    ...
    int n = get_ref_frames_subgop(cpi, ref_frame_map_pairs);
    if (n) return;
  }
