...
void av1_configure_buffer_updates(AV1_COMP *const cpi,
  ...
  if (cpi->ext_refresh_frame_flags_pending && (!is_analysis_stage(cpi))) {
...
static int denoise_and_encode(AV1_COMP *const cpi, uint8_t *const dest,
  ...
  if (oxcf->lag_in_frames > 0 && !is_analysis_stage(cpi) &&
...
int av1_encode_strategy(AV1_COMP *const cpi, size_t *const size,
...
#if !CONFIG_REALTIME_ONLY
    if (flush && is_analysis_stage(cpi) && !cpi->twopass.first_pass_done) {
  ...
  *time_end = source->ts_end;
  ...
  if (!is_analysis_stage(cpi)) set_ext_overrides(cpi, &frame_params);
