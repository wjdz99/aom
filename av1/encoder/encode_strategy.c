...
static int compare_map_idx_pair_asc(const void *a, const void *b) {
  if (((RefBufMapData *)a)->disp_order == ((RefBufMapData *)b)->disp_order) {
    return 0;
  } else if (((const RefBufMapData *)a)->disp_order > ((const RefBufMapData *)b)->disp_order) {
    return 1;
  } else {
    return -1;
  }
...
static void add_ref_to_slot(RefBufMapData *ref, int *const remapped_ref_idx, int frame) {
  remapped_ref_idx[frame - LAST_FRAME] = ref->map_idx;
  ref->used = 1;
}

static void set_unmapped_ref(RefBufMapData *buffer_map, int n_bufs, int update_arf, int min_level) {
  if (n_bufs <= ALTREF_FRAME) return;
  for (int i = 0; i < n_bufs; i++) {
    if (buffer_map[i].used) continue;
    if (buffer_map[i].pyr_level != min_level || 
        (update_arf && buffer_map[i].pyr_level == min_level)) {
      buffer_map[i].used = 1;
      return;
    }
  }
  assert(0 && "Unmapped reference not found");
}
...
static void get_ref_frames_subgop(AV1_COMP *const cpi, 
  ...
  for (int i = n_bufs - 1; i >= 0; i--) {
    ...
    // Keep track of where the frames change from being past frames to future frames
  ...
  set_unmapped_ref(buffer_map, n_bufs, update_arf, min_level);


  // Map LAST3_FRAME
  if (n_bufs >= ALTREF_FRAME) {
    const int use_low_level_last3 = n_past_high_level < 4 && n_bufs > ALTREF_FRAME;
