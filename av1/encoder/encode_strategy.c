...
static int denoise_and_encode(AV1_COMP *const cpi, uint8_t *const dest,
  ...
  } else if (get_frame_update_type(&cpi->ppi->gf_group, cpi->gf_frame_index) ==
             ARF_UPDATE) {
...
static void get_ref_frames(AV1_COMP *const cpi, int cur_frame_disp,
                           RefFrameMapPair ref_frame_map_pairs[REF_FRAMES]) {
...
#if FP_ENABLED
  get_ref_frames(cpi, cur_frame_disp, ref_frame_map_pairs);
  return;
#endif  // FP_ENABLED
