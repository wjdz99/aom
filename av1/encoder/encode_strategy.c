...
static void disable_low_ranking_refs(int *ref_frame_flags, 
  ...
  printf("BEFORE: ");
    ...
    printf("LAST %d ", last_allowed);
    printf("LAST2 %d ", last2_allowed);
    printf("LAST3 %d ", last3_allowed);
    printf("GLD %d ", golden_allowed);
    printf("BWD %d ", bwdref_allowed);
    printf("ARF2 %d ", altref2_allowed);
    printf("ARF %d ", altref_allowed);
  ...
  printf("\n\nAFTER: ");
    ...
    last_allowed = (*ref_frame_flags & av1_ref_frame_flag_list[LAST_FRAME]) != 0;
    last2_allowed = (*ref_frame_flags & av1_ref_frame_flag_list[LAST2_FRAME]) != 0;
    last3_allowed = (*ref_frame_flags & av1_ref_frame_flag_list[LAST3_FRAME]) != 0;
    golden_allowed = (*ref_frame_flags & av1_ref_frame_flag_list[GOLDEN_FRAME]) != 0;
    bwdref_allowed = (*ref_frame_flags & av1_ref_frame_flag_list[BWDREF_FRAME]) != 0;
    altref2_allowed = (*ref_frame_flags & av1_ref_frame_flag_list[ALTREF2_FRAME]) != 0;
    altref_allowed = (*ref_frame_flags & av1_ref_frame_flag_list[ALTREF_FRAME]) != 0;
    printf("LAST %d ", last_allowed);
    printf("LAST2 %d ", last2_allowed);
    printf("LAST3 %d ", last3_allowed);
    printf("GLD %d ", golden_allowed);
  ...
  if (!is_stat_generation_stage(cpi)) {
    ...
    if (!is_stat_generation_stage(cpi)) {
      int n_ranked = rank_ref_frames(cm, cur_frame_disp, cur_pyr_level,
                                     ranking);
      printf("n_ranked %d\n", n_ranked);
      int n_refs_allowed = AOMMAX(n_ranked - 1, 4);
      printf("n_allowed %d\n", n_refs_allowed);
