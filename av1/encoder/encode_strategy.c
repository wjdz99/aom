    ...
    if (gf_group->update_type[cpi->gf_frame_index] == ARF_UPDATE) {
      for (int i = 0; i < 8; i++) {
        if ( i < 7) {
          ...
          cpi->ppi->rtc_ref.reference[i] = cpi->ppi->rtc_ref.reference_arf[cpi->svc.spatial_layer_id][i];
          cpi->ppi->rtc_ref.ref_idx[i] = cpi->ppi->rtc_ref.ref_idx_arf[cpi->svc.spatial_layer_id][i];
        }
        if (cpi->svc.spatial_layer_id == 0) {
          cpi->ppi->rtc_ref.refresh_tmp[i] = cpi->ppi->rtc_ref.refresh[i];
        }
        cpi->ppi->rtc_ref.refresh[i] = cpi->ppi->rtc_ref.refresh_arf[cpi->svc.spatial_layer_id][i];
      }
