...
int av1_encode_strategy(AV1_COMP *const cpi, size_t *const size,
  ...
  *time_end = source->ts_end;
  ...
  if (!is_stat_generation_stage(cpi)) {
    RefCntBuffer *ref_frames[INTER_REFS_PER_FRAME];
    ...
    for (int i = LAST_FRAME; i <= ALTREF_FRAME; ++i) {
      ref_frames[i - 1] = get_ref_frame_buf(cm, i);
      ref_frame_buf[i - 1] =
          ref_frames[i - 1] != NULL ? &ref_frames[i - 1]->buf : NULL;
    }
    // Work out which reference frame slots may be used.
    frame_params.ref_frame_flags =
        get_ref_frame_flags(ref_frame_buf, cpi->ext_ref_frame_flags,
