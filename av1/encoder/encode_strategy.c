...
int av1_encode_strategy(AV1_COMP *const cpi, size_t *const size,
  ...
  *size = frame_results.size;
  ...
  // refresh of LAST reference (index 0).
  if (cpi->svc.number_temporal_layers > 1 &&
      cpi->svc.number_spatial_layers == 1 &&
      cpi->ppi->rtc_ref.refresh[cpi->ppi->rtc_ref.ref_idx[0]]) {
    aom_yv12_copy_y(cpi->unfiltered_source, &cpi->svc.source_last_ref);
    aom_yv12_copy_u(cpi->unfiltered_source, &cpi->svc.source_last_ref);
    aom_yv12_copy_v(cpi->unfiltered_source, &cpi->svc.source_last_ref);
  }
