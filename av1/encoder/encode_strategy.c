...
static int get_refresh_frame_flags_subgop_cfg(
    const AV1_COMP *const cpi, const EncodeFrameParams *const frame_params,
    const RefBufferStack *const ref_buffer_stack, int refresh_mask,
    int free_fb_index) {
...
int av1_get_refresh_frame_flags(const AV1_COMP *const cpi,
  ...
  if (cpi->subgop_config_set.num_configs > 0 &&
      ...
      && gop_cfg_index == 0) {
    return get_refresh_frame_flags_subgop_cfg(
        cpi, frame_params, ref_buffer_stack, refresh_mask, free_fb_index);
  }
  switch (frame_update_type) {
    case KF_UPDATE:
    case GF_UPDATE:
