  ...
  for (int i = n_bufs - 1; i >= 0; i--) {
    ...
    if (buffer_map[i].pyr_level == min_level) {
      ...
      if (buffer_map[i].disp_order < cur_frame_disp && golden_frame < 0) {
        printf("golden %d, leve %d\n", buffer_map[i].disp_order, min_level);
        golden_frame = i; 
      }
    } 
...
*/
  ...
  for (int i = 0; i < n_bufs; i++) {
    if (update_arf && oldest_ind < 0 && buffer_map[i].pyr_level == min_level) {
      oldest_ind = i;
      break;
    } else if (i != golden_frame && oldest_ind < 0 && buffer_map[i].pyr_level != min_level) {
      oldest_ind = i;
      break;
    }
  }
  ...
  if (n_bufs > ALTREF_FRAME) buffer_map[oldest_ind].used = 1;
  ...
  for (int frame = LAST_FRAME; frame < REF_FRAMES; frame++) {
    ...
    printf("past %d\n", buffer_map[buf_map_idx].disp_order);
