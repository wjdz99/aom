    ...
    for (int i = ref_buffer_stack->lst_stack_size - 1; i >= 0; --i) {
      if (ref_buffer_stack->lst_stack[i] == ref_map_index) {
        for (int idx = i; idx < ref_buffer_stack->lst_stack_size - 1; ++idx)
          ref_buffer_stack->lst_stack[idx] = ref_buffer_stack->lst_stack[idx + 1];
    ...
    for (int i = ref_buffer_stack->gld_stack_size - 1; i >= 0; --i) {
      if (ref_buffer_stack->gld_stack[i] == ref_map_index) {
        for (int idx = i; idx < ref_buffer_stack->gld_stack_size - 1; ++idx)
          ref_buffer_stack->gld_stack[idx] = ref_buffer_stack->gld_stack[idx + 1];
  ...
  AV1_COMMON *const cm = &cpi->common;
    ...
    switch (frame_update_type) {
      case KEY_FRAME:
        ...
        stack_push(ref_buffer_stack->gld_stack, &ref_buffer_stack->gld_stack_size, ref_map_index);
        break;
      case GF_UPDATE:
      ...
      case LF_UPDATE:
        update_arf_stack(cpi, ref_map_index, ref_buffer_stack);
        stack_push(ref_buffer_stack->lst_stack, &ref_buffer_stack->lst_stack_size, ref_map_index);
        break;
      case ARF_UPDATE:
      case INTNL_ARF_UPDATE:
        update_arf_stack(cpi, ref_map_index, ref_buffer_stack);
        stack_push(ref_buffer_stack->arf_stack, &ref_buffer_stack->arf_stack_size, ref_map_index);
        break;
      case OVERLAY_UPDATE:
        if (cpi->preserve_arf_as_gld || cm->show_existing_frame) {
          ref_map_index = stack_pop(ref_buffer_stack->arf_stack, &ref_buffer_stack->arf_stack_size);
          stack_push(ref_buffer_stack->gld_stack, &ref_buffer_stack->gld_stack_size, ref_map_index);
        } else {
          stack_pop(ref_buffer_stack->arf_stack, &ref_buffer_stack->arf_stack_size);
          update_arf_stack(cpi, ref_map_index, ref_buffer_stack);
...
static int get_free_ref_map_index(const AV1_COMP *cpi, const RefBufferStack *ref_buffer_stack) {
  (void)cpi;
  for (int idx = 0; idx < REF_FRAMES; ++idx) {
    int is_free = 1;
    for (int i = 0; i < ref_buffer_stack->arf_stack_size; ++i)
      if (ref_buffer_stack->arf_stack[i] == idx) is_free = 0;
    for (int i = 0; i < ref_buffer_stack->lst_stack_size; ++i)
      if (ref_buffer_stack->lst_stack[i] == idx) is_free = 0;
    for (int i = 0; i < ref_buffer_stack->gld_stack_size; ++i)
      if (ref_buffer_stack->gld_stack[i] == idx) is_free = 0;
  ...
  switch (frame_update_type) {
    ...
    case GF_UPDATE:
      if (free_fb_index != INVALID_IDX) {
        ...
        if (ref_buffer_stack->gld_stack_size)
          refresh_mask = 1 << ref_buffer_stack->gld_stack[ref_buffer_stack->gld_stack_size - 1];
        else
          refresh_mask = 1 << ref_buffer_stack->lst_stack[ref_buffer_stack->lst_stack_size - 1];
      }
      break;
    case LF_UPDATE:
      if (free_fb_index != INVALID_IDX) {
    ...
    case OVERLAY_UPDATE:
      if (!cpi->preserve_arf_as_gld) {
        if (free_fb_index != INVALID_IDX) {
          ...
          refresh_mask = 1 << ref_buffer_stack->arf_stack[ref_buffer_stack->arf_stack_size - 1];
        } else {
          refresh_mask = 1 << ref_buffer_stack->lst_stack[ref_buffer_stack->lst_stack_size - 1];
        }
      }
...
void av1_get_ref_frames(AV1_COMP *const cpi,
  ...
  if (arf_stack_size)
    cm->remapped_ref_idx[ALTREF_FRAME - LAST_FRAME] = ref_buffer_stack->arf_stack[arf_stack_size - 1];

  if (arf_stack_size > 1)
    cm->remapped_ref_idx[BWDREF_FRAME - LAST_FRAME] = ref_buffer_stack->arf_stack[0];

  if (arf_stack_size > 2)
    cm->remapped_ref_idx[ALTREF2_FRAME - LAST_FRAME] = ref_buffer_stack->arf_stack[1];

  if (lst_stack_size)
    cm->remapped_ref_idx[LAST_FRAME - LAST_FRAME] = ref_buffer_stack->lst_stack[0];

  if (lst_stack_size > 1)
    cm->remapped_ref_idx[LAST2_FRAME - LAST_FRAME] = ref_buffer_stack->lst_stack[1];

  if (gld_stack_size)
    cm->remapped_ref_idx[GOLDEN_FRAME - LAST_FRAME] = ref_buffer_stack->gld_stack[gld_stack_size - 1];

  if (gld_stack_size > 1) {
    if (arf_stack_size <= 1)
      cm->remapped_ref_idx[BWDREF_FRAME - LAST_FRAME] = ref_buffer_stack->gld_stack[0];
    else
      cm->remapped_ref_idx[LAST3_FRAME - LAST_FRAME] = ref_buffer_stack->gld_stack[0];
  }
  ...
  for (int idx = ALTREF_FRAME - LAST_FRAME; idx >= 0; --idx) {
    ...
    for (int i = 0; i < ref_buffer_stack->arf_stack_size && ref_map_index == INVALID_IDX; ++i) {
      int ref_idx = 0;
      for (ref_idx = 0; ref_idx <= ALTREF_FRAME - LAST_FRAME; ++ref_idx)
        if (ref_buffer_stack->arf_stack[i] == cm->remapped_ref_idx[ref_idx]) break;
    ...
    for (int i = 0; i < ref_buffer_stack->gld_stack_size && ref_map_index == INVALID_IDX; ++i) {
      int ref_idx = 0;
      for (ref_idx = 0; ref_idx <= ALTREF_FRAME - LAST_FRAME; ++ref_idx)
        if (ref_buffer_stack->gld_stack[i] == cm->remapped_ref_idx[ref_idx]) break;
    ...
    for (int i = 0; i < ref_buffer_stack->lst_stack_size && ref_map_index == INVALID_IDX; ++i) {
      int ref_idx = 0;
      for (ref_idx = 0; ref_idx <= ALTREF_FRAME - LAST_FRAME; ++ref_idx)
        if (ref_buffer_stack->lst_stack[i] == cm->remapped_ref_idx[ref_idx]) break;
    ...
    else cm->remapped_ref_idx[idx] = ref_buffer_stack->gld_stack[0];
  }
...
int av1_encode_strategy(AV1_COMP *const cpi, size_t *const size,
  ...
  *time_end = source->ts_end;
  ...
  if (oxcf->pass == 0 || oxcf->pass == 2) {
    ...
    int ref_map_index = av1_get_refresh_ref_frame_map(cm->current_frame.refresh_frame_flags);
    av1_update_ref_frame_map(cpi, frame_update_type, ref_map_index, &cpi->ref_buffer_stack);
  }
