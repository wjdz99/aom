...
static void tree2tok(struct Av1Token *tokens, const AomTreeIndex *tree, int i,
                     int v, int l) {
  v += v;
  ++l;

  do {
    const AomTreeIndex j = tree[i++];
    if (j <= 0) {
      tokens[-j].value = v;
      tokens[-j].len = l;
    } else {
...
void av1_tokens_from_tree(struct Av1Token *tokens, const AomTreeIndex *tree) {
  tree2tok(tokens, tree, 0, 0, 0);
}

static unsigned int convert_distribution(unsigned int i, AomTree tree,
