...
static INLINE int av1_get_start_shift_convolve(int width, int filt_width,
                                               int stride) {
...
// "PADDING_SAME_ZERO"
static INLINE void av1_maxpool_one_padding_zero(
    ...
    const int cstep, const int filter_height_half,
...
static INLINE void av1_maxpool_one_padding_replicate(
    const float **input, int in_width, int in_height, int in_stride,
    const CNN_LAYER_CONFIG *layer_config, float **output, int out_stride,
    const int cstep, const int filter_height_half,
    const int filter_width_half) {
...
// CNNConvolve specific to maxpool set as 1 and padding equal to "PADDING_VALID"
static INLINE void av1_maxpool_one_padding_valid(
    const float **input, int in_width, int in_height, int in_stride,
    const CNN_LAYER_CONFIG *layer_config, float **output, int out_stride,
    const int cstep) {
  for (int i = 0; i < layer_config->out_channels; ++i) {
...
// equals 1
static INLINE void av1_element_wise_convole(
    const float **input, int in_width, int in_height, int in_stride,
    const CNN_LAYER_CONFIG *layer_config, float **output, int out_stride,
    int start_idx, int step) {
  const int start_h = av1_get_start_shift_convolve(
      in_height, layer_config->filter_height, layer_config->skip_height);
  const int start_w =
      av1_get_start_shift_convolve(in_width, layer_config->filter_width,
                                   layer_config->skip_width) +
...
// "PADDING_SAME_ZERO"
static INLINE void av1_maxpool_zero_padding_zero(
    const float **input, int in_width, int in_height, int in_stride,
    const CNN_LAYER_CONFIG *layer_config, float **output, int out_stride,
    int start_idx, const int cstep, const int filter_height_half,
    const int filter_width_half, const int ii_shift, const int jj_shift,
    const int channel_step) {
  const int start_h = av1_get_start_shift_convolve(
      in_height, layer_config->filter_height, layer_config->skip_height);
  const int start_w = av1_get_start_shift_convolve(
      in_width, layer_config->filter_width, layer_config->skip_width);
...
// CNNConvolve specific to maxpool set as 0 and padding equal to
// "PADDING_SAME_REPLICATE"
static INLINE void av1_maxpool_zero_padding_replicate(
    const float **input, int in_width, int in_height, int in_stride,
    const CNN_LAYER_CONFIG *layer_config, float **output, int out_stride,
  ...
  // computation.
  const int start_h =
      av1_get_start_shift_convolve(in_height, layer_config->filter_height,
                                   layer_config->skip_height) -
      ii_shift;
  const int start_w =
      av1_get_start_shift_convolve(in_width, layer_config->filter_width,
...
// CNNConvolve specific to maxpool set as 0 and padding equal to "PADDING_VALID"
static INLINE void av1_maxpool_zero_padding_valid(
    const float **input, int in_width, int in_height, int in_stride,
    const CNN_LAYER_CONFIG *layer_config, float **output, int out_stride,
