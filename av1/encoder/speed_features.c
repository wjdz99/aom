...
// TODO(kyslov) Re-enable when AV1 models are trained
  ...
  if (cpi->ppi->use_svc) {
    ...
    // base temporal layers, and only on base spatial layer if highest resolution
    // is above 640x360. Use better mv search on TL1 if the motion is high
    // (i.e., avg_frame_low_motion is low).
