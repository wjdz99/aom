...
// for resolution index 'j' for speed setting 'i+1'.
// i = 0: speed 1, i = 1: speed 2.
// j = 0: lower than 720p resolution, j = 1: 720p or larger resolution.
// Currently invoked only for speed 1 and 2.
static int ms_qthresh[2][2][2] = { { { 170, 50 }, { MAXQ, 200 } },
                                   { { 170, 40 }, { 200, 40 } } };
...
void av1_set_speed_features_qindex_dependent(AV1_COMP *cpi, int speed) {
  ...
  if (cpi->oxcf.mode == GOOD && ((speed == 1) || (speed == 2))) {
    if (!is_stat_generation_stage(cpi)) {
      ...
      // quantizers.
      const int speed_index = speed - 1;
      const int qindex_thresh1 = ms_qthresh[speed_index][is_720p_or_larger][0];
      const int qindex_thresh2 = ms_qthresh[speed_index][is_720p_or_larger][1];
