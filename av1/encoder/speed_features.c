...
// Predict DC block level used during transform type search is indexed using the
// following
// First index  : Speed feature 'dc_blk_pred_level' (0 to 3)
// Second index : Mode evaluation type (DEFAULT_EVAL, MODE_EVAL and
// WINNER_MODE_EVAL).
//
// predict_dc_levels[][] takes values from 0 to 2 as below.
// These values indicate the aggressiveness of predicting a block as transform
// skip or DC only.
// 0 : No early skip block or DC only block prediction
// 1 : Early prediction of skip block based on residual mean and variance
// 2 : Early prediction of skip block or DC only block based on residual mean
// and variance
