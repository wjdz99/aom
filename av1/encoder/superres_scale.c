...
static double get_energy_by_q2_thresh(const GF_GROUP *gf_group,
                                      const RATE_CONTROL *rc, int index) {
  // TODO(now): Return keyframe thresh * factor based on frame type / pyramid
  // level.
  if (gf_group->update_type[index] == ARF_UPDATE) {
    return SUPERRES_ENERGY_BY_Q2_THRESH_ARFFRAME;
  } else if (gf_group->update_type[index] == KF_UPDATE) {
...
static uint8_t get_superres_denom_for_qindex(const AV1_COMP *cpi, int qindex,
  ...
  if (gf_group->update_type[cpi->cur_gf_index] != KF_UPDATE &&
      gf_group->update_type[cpi->cur_gf_index] != ARF_UPDATE) {
    return SCALE_NUMERATOR;
  }
  if (gf_group->update_type[cpi->cur_gf_index] == KF_UPDATE && !sr_kf) {
    return SCALE_NUMERATOR;
  }
  if (gf_group->update_type[cpi->cur_gf_index] == ARF_UPDATE && !sr_arf) {
    return SCALE_NUMERATOR;
  }
  ...
  const double energy_by_q2_thresh =
      get_energy_by_q2_thresh(gf_group, &cpi->rc, cpi->cur_gf_index);
