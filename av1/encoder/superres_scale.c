  ...
  switch (cpi->superres_mode) {
    ...
    case AOM_SUPERRES_QTHRESH: {
      ...
      if (oxcf->rc_mode == AOM_VBR || oxcf->rc_mode == AOM_CQ)
        av1_set_target_rate(cpi, frame_dim_cfg->width, frame_dim_cfg->height);

      // Now decide the use of superres based on 'q'.
      int bottom_index, top_index;
      const int q = av1_rc_pick_q_and_bounds(
          cpi, &cpi->rc, frame_dim_cfg->width, frame_dim_cfg->height,
...
static size_params_type calculate_next_size_params(AV1_COMP *cpi) {
  ...
  const FrameDimensionCfg *const frame_dim_cfg = &oxcf->frame_dim_cfg;
  size_params_type rsz = { frame_dim_cfg->width, frame_dim_cfg->height,
                           SCALE_NUMERATOR };
  ...
  if (resize_pending_params->width && resize_pending_params->height) {
    ...
    rsz.resize_width = frame_dim_cfg->width;
    rsz.resize_height = frame_dim_cfg->height;
    av1_calculate_scaled_size(&rsz.resize_width, &rsz.resize_height,
                              resize_denom);
  }
  ...
  if (!validate_size_scales(oxcf->resize_cfg.resize_mode, cpi->superres_mode,
                            frame_dim_cfg->width, frame_dim_cfg->height, &rsz))
