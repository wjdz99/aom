...
static void set_prob_tokens(int prob_head[ENTROPY_TOKENS],
  ...
  prob_head[ZERO_TOKEN] =
      AOMMIN(255, (prob_tmp[ZERO_TOKEN] * (1 << 8)) / prob_block_nz);
  prob_head[ONE_TOKEN] = AOMMIN(
      255,
      ((/*prob_tmp[ONE_TOKEN_EOB] +*/ prob_tmp[ONE_TOKEN_NEOB]) * (1 << 8) +
       offset) /
          prob_block_nz);
  prob_head[TWO_TOKEN] = AOMMIN(
      255, ((/*prob_tmp[TWO_TOKEN_PLUS_EOB] +*/ prob_tmp[TWO_TOKEN_PLUS_NEOB]) *
                (1 << 8) +
            offset) /
               prob_block_nz);
  ...
  prob_head[EOB_TOKEN] = AOMMIN(
      255,
      ((prob_tmp[ONE_TOKEN_EOB] + prob_tmp[TWO_TOKEN_PLUS_EOB]) * (1 << 8) +
       offset) /
          scale);
