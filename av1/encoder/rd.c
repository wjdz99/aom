...
void av1_update_rd_mode_costs(RD_COSTS *rdc, const FRAME_CONTEXT *const fc) {
  ...
  for (i = 0; i < BLOCK_SIZE_GROUPS; ++i)
    av1_cost_tokens_from_cdf(rdc->mb_mode[i], fc->adapted_cdfs.y_mode_cdf[i],
                             av1_intra_mode_inv);

  for (i = 0; i < INTRA_MODES; ++i)
    av1_cost_tokens_from_cdf(rdc->intra_uv_mode[i],
  ...
  for (i = 0; i < MAX_TX_DEPTH; ++i)
    for (j = 0; j < TX_SIZE_CONTEXTS; ++j)
      av1_cost_tokens_from_cdf(rdc->tx_size[i][j],
  ...
  for (i = TX_4X4; i < EXT_TX_SIZES; ++i) {
    av1_cost_tokens_from_cdf(rdc->inter_tx_type[i],
  ...
  for (i = 0; i < INTRA_FILTERS + 1; ++i)
    av1_cost_tokens_from_cdf(rdc->intra_filter[i],
