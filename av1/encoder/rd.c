...
void av1_fill_mode_rates(AV1_COMMON *const cm, ModeCosts *mode_costs,
  ...
  for (i = 0; i < Y_MODE_CONTEXTS; ++i) {
    // y mode costs
    av1_cost_tokens_from_cdf(mode_costs->y_primary_flag_cost[i],
                             fc->y_mode_set_cdf[i], NULL);
    ...
    av1_cost_tokens_from_cdf(mode_costs->y_third_mode_costs[i],
                             fc->y_third_mode_cdf[i], NULL);
  }
