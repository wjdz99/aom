    ...
    for (int plane = 0; plane < PLANE_TYPES; ++plane) {
      ...
      for (int ctx = 0; ctx < 2; ++ctx) {
        ...
        switch (eob_multi_size) {
          case 0:
            pcdf = &fc->eob_flag_cdf16[plane][ctx][0];
            av1_cost_tokens_from_cdf(pcost->eob_cost[ctx], pcdf, NULL);
            break;
          case 1:
            pcdf = &fc->eob_flag_cdf32[plane][ctx][0];

            av1_cost_tokens_from_cdf(pcost->eob_cost[ctx], pcdf, NULL);
            break;
          case 2:
            pcdf = &fc->eob_flag_cdf64[plane][ctx][0];

            av1_cost_tokens_from_cdf(pcost->eob_cost[ctx], pcdf, NULL);
            break;
          case 3:
            pcdf = &fc->eob_flag_cdf128[plane][ctx][0];

            av1_cost_tokens_from_cdf(pcost->eob_cost[ctx], pcdf, NULL);
            break;
          case 4:
            pcdf = &fc->eob_flag_cdf256[plane][ctx][0];

            av1_cost_tokens_from_cdf(pcost->eob_cost[ctx], pcdf, NULL);
            break;
          case 5:
            pcdf = &fc->eob_flag_cdf512[plane][ctx][0];

            av1_cost_tokens_from_cdf(pcost->eob_cost[ctx], pcdf, NULL);
            break;
          case 6:
          default:
            pcdf = &fc->eob_flag_cdf1024[plane][ctx][0];
