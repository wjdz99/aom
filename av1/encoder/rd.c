...
// Mapping of entropy cost update frequency from the encoder's control time
// settings of type COST_UPDATE_TYPE to sfs of type INTERNAL_COST_UPDATE_TYPE.
static INTERNAL_COST_UPDATE_TYPE
    av1_map_to_internal_cost_upd[NUM_COST_UPDATE_TYPES] = {
      INTERNAL_COST_UPD_SB, INTERNAL_COST_UPD_SBROW, INTERNAL_COST_UPD_TILE,
      INTERNAL_COST_UPD_OFF
    };

// Populates sfs based on control time settings (of type COST_UPDATE_TYPE) and
// expected sf settings (of type INTERNAL_COST_UPDATE_TYPE) by considering the
// worst case frequencies. The populated/updated sfs are used for cost updates
// in the encoder.
