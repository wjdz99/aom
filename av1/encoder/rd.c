...
void av1_fill_mode_rates(AV1_COMMON *const cm, MACROBLOCK *x,
  ...
  if (!frame_is_intra_only(cm)) {
    ...
    for (i = 0; i < DRL0_SINGLE_MODE_CONTEXTS; i++) {
      av1_cost_tokens_from_cdf(x->drl0_single_mode_cost[i],
                               fc->drl0_single_cdf[i], NULL);
    }
    for (i = 0; i < DRL0_COMPOUND_MODE_CONTEXTS; i++) {
      av1_cost_tokens_from_cdf(x->drl0_compound_mode_cost[i],
                               fc->drl0_compound_cdf[i], NULL);
    }
