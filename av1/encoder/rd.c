...
#if CONFIG_VQ4X4
  av1_cost_tokens_from_cdf(x->vq_gain_sym1_costs,
                           fc->vq_gain_sym1_cdf, NULL);
  for (i = 0; i < NUM_GAIN_SYMBOL1; i++)
    av1_cost_tokens_from_cdf(x->vq_gain_sym2_costs[i],
                             fc->vq_gain_sym2_cdf[i], NULL);
  av1_cost_tokens_from_cdf(x->vq_codeword_1_costs,
                           fc->vq_codeword_1_cdf, NULL);
  for (i = 0; i < NUM_CW_SYMBOL1; i++)
    av1_cost_tokens_from_cdf(x->vq_codeword_2_costs[i],
                             fc->vq_codeword_2_cdf[i], NULL);
#endif
