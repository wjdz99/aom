...
void av1_fill_mode_rates(AV1_COMMON *const cm, ModeCosts *mode_costs,
  ...
  if (!frame_is_intra_only(cm)) {
    ...
    for (i = 0; i < DRL_MODE_CONTEXTS; ++i) {
      av1_cost_tokens_from_cdf(mode_costs->drl_mode_cost[0][i], fc->drl_cdf[0][i],
                               NULL);
      av1_cost_tokens_from_cdf(mode_costs->drl_mode_cost[1][i], fc->drl_cdf[1][i],
                               NULL);
      av1_cost_tokens_from_cdf(mode_costs->drl_mode_cost[2][i], fc->drl_cdf[2][i],
                               NULL);
    }
#else
