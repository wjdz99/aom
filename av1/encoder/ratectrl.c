...
static int rc_pick_q_and_bounds_two_pass(const AV1_COMP *cpi, int width,
  ...
  if (frame_is_intra_only(cm)) {
    ...
    // worst allowed quality as well. This insures that even on hard sections we
    // dont clamp the Q at the same value for arf frames and leaf (non arf) frames.
    // This is important to the TPL model which assumes Q drops with each arf level.
    if (!(rc->is_src_frame_alt_ref) &&
        (cpi->refresh_golden_frame || cpi->refresh_alt_ref_frame ||
         is_intrl_arf_boost)) {
      active_worst_quality =
