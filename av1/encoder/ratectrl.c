...
static int adjust_q_cbr(const AV1_COMP *cpi, int q, int active_worst_quality,
  ...
  // limit on max_delta_down. Only for high speeds levels for now to
  // avoid bdrate regression.
  if (cpi->is_screen_content_type && cpi->sf.rt_sf.check_scene_detection &&
      cpi->oxcf.speed > 8 && rc->frame_source_sad == 0 && q > 35 &&
    ...
    const int qp_bias = 6;
    int qp_delta = 4;
    if (p_rc->buffer_level > (p_rc->optimal_buffer_level << 1)) {
      qp_delta = 16;
      q = q - qp_bias;
    } else if (p_rc->buffer_level > ((3 * p_rc->optimal_buffer_level) >> 1)) {
      qp_delta = 10;
      q = q - (qp_bias >> 1);
    } else if (p_rc->buffer_level > ((5 * p_rc->optimal_buffer_level) >> 2))
      qp_delta = 6;
