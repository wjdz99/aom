...
void av1_rc_update_rate_correction_factors(AV1_COMP *cpi, int width,
  ...
  double rate_correction_factor = get_rate_correction_factor(cpi, width,
                                                             height);
...
void av1_rc_postencode_update(AV1_COMP *cpi, uint64_t bytes_used) {
  ...
  rc->this_frame_target /= resize_rate_factor(cpi, cm->width, cm->height);
  if (cm->frame_type != KEY_FRAME) {
  ...
  /*
  rc->this_frame_target =
      (int)(rc->this_frame_target / resize_rate_factor(cpi, cm->width, cm->height));
      */
}
