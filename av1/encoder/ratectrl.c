...
static int rc_pick_q_and_bounds_two_pass(const AV1_COMP *cpi, int *bottom_index,
  ...
  if (!av1_frame_unscaled(cpi) && !frame_is_kf_gf_arf(cpi)) {
  ...
  *height = oxcf->height;
  ...
  if (oxcf->resize_mode == RESIZE_FIXED) {
    ...
    *height = oxcf->scaled_frame_height;
    return;
  } else if (oxcf->resize_mode == RESIZE_DYNAMIC) {
    // NOTE: RESIZE_DYNAMIC defaults to random for now.
