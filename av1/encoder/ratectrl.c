...
static int get_active_best_quality_q_mode(
    const int cq_level, const int bit_depth, const GF_GROUP *gf_group,
    const int gf_index, const int is_overlay_frame, int gfu_boost,
    int arf_boost_factor, int arf_q) {
  const int is_intrl_arf_boost =
      gf_group->update_type[gf_index] == INTNL_ARF_UPDATE;
  int is_leaf_frame =
      !(gf_group->update_type[gf_index] == ARF_UPDATE ||
  ...
  int active_best_quality =
      get_gf_active_quality_no_rc(gfu_boost, cq_level, bit_depth);
