...
static void rc_scene_detection_onepass_rt(AV1_COMP *cpi,
  ...
  int last_src_width;
  int last_src_height;
  if (cpi->oxcf.frm_dim_cfg.width != cm->render_width ||
      cpi->oxcf.frm_dim_cfg.height != cm->render_height || unscaled_src == NULL ||
  ...
  if (src_width == last_src_width && src_height == last_src_height) {
    ...
    if ((cpi->oxcf.frm_dim_cfg.width == cm->render_width) &&
        (cpi->oxcf.frm_dim_cfg.height == cm->render_height)) {
