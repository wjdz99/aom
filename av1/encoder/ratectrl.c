...
unsigned int is_scroll_motion(const AV1_COMP *cpi, uint8_t *src_buf,
                              uint8_t *last_src_buf, int src_stride,
                              int ref_stride, BLOCK_SIZE bsize, int pos_col,
                              int pos_row, int *best_intmv_col,
                              int *best_intmv_row) {
  const AV1_COMMON *const cm = &cpi->common;
...
static void rc_scene_detection_onepass_rt(AV1_COMP *cpi,
  ...
  if (cpi->svc.temporal_layer_id == 0) {
    ...
    if (cpi->oxcf.tune_cfg.content == AOM_CONTENT_SCREEN &&
      ...
      // Compute fast coarse/global motion for 128x128 superblock centered at
      // middle
      /// of frames, to determine if motion is scroll.
      ...
      unsigned int y_sad = is_scroll_motion(
