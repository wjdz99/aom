...
// For 1 pass real-time mode: set the frame_type, GF update and
// and set the target frame size. For 1 layer (non-SVC) this will
// also set the reference structure (GF and ALT reference/refresh),
// for SVC some update/resets are done (update_temporal_layer_framerate
// and restore_layer_context).
void av1_get_one_pass_rt_params(AV1_COMP *cpi,
  ...
  // Check for scene change, for 1 spatial layer case for now.
  if (cm->number_spatial_layers == 1 && cpi->sf.rt_sf.check_scene_detection)
