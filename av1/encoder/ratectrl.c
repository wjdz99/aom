...
int av1_rc_bits_per_mb(const AV1_COMP *cpi, FRAME_TYPE frame_type, int qindex,
  ...
  if (frame_type != KEY_FRAME && accurate_estimate) {
    ...
    //printf("\n cpi->rc.rc_1_frame: %d; ratio: %d; (int)(ratio * sse_sqrt): %d;   ", cpi->rc.rc_1_frame, ratio, (int)(ratio * sse_sqrt));

    int max_thr = 900000;
    if (cpi->rc.frames_since_key > 3) {
      if (cpi->rc.frames_since_key > 8) max_thr = 170000;
      else  max_thr = 300000;
    }
...
//
//    //printf("\n %d; %d; %d;     \n", p_rc->last_kf_qindex, q,  rc->best_quality );
//  }
