...
static int adjust_q_cbr(const AV1_COMP *cpi, int q,
  ...
  // On resize frame: if resolution has increased push q closer to the
  // active_worst to avoid excess overshoot.
  if (cpi->resize_pending_width && cpi->resize_pending_height &&
      cm->prev_frame && (cm->width * cm->height >
...
void av1_get_one_pass_rt_params(AV1_COMP *cpi,
  ...
  int target;
  int resize_pending = (cpi->resize_pending_width &&
      cpi->resize_pending_height && (cm->width != cpi->resize_pending_width ||
