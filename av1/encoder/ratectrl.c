 ...
 *
 * \return Nothing is returned. Instead the settings for the prediction structure
 * are set in \c cpi-ext_flags. And the buffer slot index (for each of 7 references)
 * and refresh flags (for each of the 8 slots) are set in \c cpi->svc.ref_idx[]
 * and \c cpi->svc.refresh[].
 */
