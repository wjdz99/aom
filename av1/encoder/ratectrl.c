  ...
  const int max_delta_down =
      (cpi->oxcf.tune_cfg.content == AOM_CONTENT_SCREEN)
          ...
          : (cpi->sf.hl_sf.accurate_bit_estimate
                 ? AOMMIN(
                       16,
                       AOMMAX(1, (rc->q_1_frame * (rc->rc_1_frame + 3)) >> 5))
...
static void set_rate_correction_factor(AV1_COMP *cpi, int is_encode_stage,
  ...
  if (cpi->common.current_frame.frame_type == KEY_FRAME) {
    ...
    // this start value. Exclude forced key frame case since it is not tested
    // yet.
...
void av1_rc_update_rate_correction_factors(AV1_COMP *cpi, int is_encode_stage,
  ...
  if (correction_factor > 0.0) {
    if (cpi->is_screen_content_type) {
      ...
      const double coeff = cpi->sf.hl_sf.accurate_bit_estimate ? 0.01 : 0.75;
      adjustment_limit =
          0.25 + coeff * AOMMIN(0.5, fabs(log10(correction_factor)));
    }
  } else {
...
//                                                pre_ystride, &sse);
        variance(src_y, src_ystride, pre_y,
                           pre_ystride, 64, 64, &sse, &sum);
      ...
      uint8_t src[64 * 64] = { 0 };
        ...
        for (int i = 0; i < 64; i+=4) {
          for (int j = 0; j < 64; j+=4) {
            const unsigned int avg = aom_avg_4x4(src_y + i * src_ystride + j, src_ystride);
  ...
  for (int sbi_row = 0; sbi_row < sb_rows; ++sbi_row) {
    for (int sbi_col = 0; sbi_col < sb_cols; ++sbi_col) {
      ...
      uint8_t pre[16 * 16] = { 0 };
        ...
        for (int i = 0; i <  16; ++i) {
          for (int j = 0; j < 16; ++j) {
            src[i * 16 + j] = aom_avg_4x4(src_y + (i << 2) * src_ystride + (j << 2), src_ystride);
          }
        }
        for (int i = 0; i <  16; ++i) {
          for (int j = 0; j < 16; ++j) {
            pre[i * 16 + j] = aom_avg_4x4(pre_y + (i << 2) * pre_ystride + (j << 2), pre_ystride);
          }
        }

      //cpi->ppi->fn_ptr[bsize].vf(src_y, src_ystride, pre_y, pre_ystride, &sse);
      //cpi->ppi->fn_ptr[BLOCK_16X16].vf(src, 16, pre, 16, &sse);
      variance(src, 16, pre, 16, 16, 16, &sse, &sum);
