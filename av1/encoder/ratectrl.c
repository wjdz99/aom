...
void av1_set_reference_structure_one_pass_rt(AV1_COMP *cpi, int gf_update) {
  ...
  if (!gld_fixed_slot) {
    ...
    // When golden is not long-term reference with fixed slot but a reference
    // with a moving slot lagging behind last (i.e., gld_fixed_slot = 0),
    // we should disable the gf_refresh_based_on_qp feature.
