...
int av1_q_mode_get_q_index(int base_q_index, int gf_update_type, int height,
                           int arf_q) {
 ...
 */
void av1_q_mode_compute_gop_q_indices(AV1_COMP *cpi, int base_q_index) {
  AV1_COMMON *cm = &cpi->common;
  GF_GROUP *gf_group = &cpi->ppi->gf_group;
  const PRIMARY_RATE_CONTROL *const p_rc = &cpi->ppi->p_rc;

  int gfu_boost = p_rc->gfu_boost;
  int bit_depth = cm->seq_params->bit_depth;
  float_t arf_boost_factor = p_rc->arf_boost_factor;
  int arf_q =
