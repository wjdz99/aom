...
int av1_rc_bits_per_mb(const AV1_COMP *cpi, FRAME_TYPE frame_type, int qindex,
  ...
  if (cpi->oxcf.mode == REALTIME && frame_type != KEY_FRAME &&
      ...
      AOMMIN(cm->width, cm->height) <=
          720) {  // cm->current_frame.frame_number > 100
    ...
    const float var_over_q2 = (float)(cpi->src_var << BPER_MB_NORMBITS) /
                              ((float)ac_q_step * ac_q_step) /
                              (float)cm->mi_params.MBs;
    ...
    const int target_bits_per_mb =
        (int)(((uint64_t)rc->this_frame_target << BPER_MB_NORMBITS) /
              cm->mi_params.MBs);

    return bits_offset + target_bits_per_mb;
  }
