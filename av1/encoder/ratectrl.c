 ...
 */
void av1_q_mode_compute_gop_q_indices(AV1_COMP *cpi,
  ...
  int arf_boost_factor = p_rc->arf_boost_factor;
  int arf_q = av1_get_arf_q_index(base_q_index,
                                  gfu_boost,
                                  bit_depth,
                                  arf_boost_factor);
  
  cm->current_frame.frame_type = frame_params->frame_type;
  for (int gf_index = cpi->gf_frame_index;
       gf_index < gf_group->size;
       ++gf_index) {
    ...
    gf_group->q_val[gf_index] =
        av1_q_mode_get_q_index(base_q_index,
                               gf_group,
                               gf_index,
                               arf_q)
  }
