...
static void get_intra_q_and_bounds_with_stats(const AV1_COMP *cpi, int width,
...
static int rc_pick_q_and_bounds_with_stats(const AV1_COMP *cpi, int width,
  ...
  if (frame_is_intra_only(cm)) {
    ...
    get_intra_q_and_bounds_with_stats(cpi, width, height, &active_best_quality,
                                      &active_worst_quality, cq_level,
                                      is_fwd_kf);
#ifdef STRICT_RC
...
int av1_rc_pick_q_and_bounds(const AV1_COMP *cpi, RATE_CONTROL *rc, int width,
  ...
  // with rc_pick_q_and_bounds_with_stats().
  const GF_GROUP *gf_group = &cpi->gf_group;
  if ((cpi->oxcf.rc_mode != AOM_Q ||
       gf_group->update_type[gf_index] == ARF_UPDATE) &&
    ...
    q = rc_pick_q_and_bounds_with_stats(cpi, width, height, gf_index,
