...
static void av1_scene_detection_onepass(AV1_COMP *cpi) {
  ...
  if (src_width == last_src_width && src_height == last_src_height) {
    ...
    int sb_size_by_mb = (cm->seq_params.sb_size == BLOCK_128X128)
                            ? (cm->seq_params.mib_size >> 1)
                            : cm->seq_params.mib_size;
    int sb_cols = (num_mi_cols + sb_size_by_mb - 1) / sb_size_by_mb;
    int sb_rows = (num_mi_rows + sb_size_by_mb - 1) / sb_size_by_mb;
      ...
      for (int sbi_col = 0; sbi_col < sb_cols; ++sbi_col) {
        ...
        if (full_sampling ||
          ...
          tmp_sad = cpi->fn_ptr[bsize].sdf(src_y, src_ystride, last_src_y,
                                           last_src_ystride);
          if (cpi->sf.rt_sf.check_light_change) {
            ...
            } else if (sse < avg_source_sse_threshold) {
              x->content_state_sb = kLowSad;
            } else if (sse > avg_source_sse_threshold_high) {
    ...
    if (cpi->sf.rt_sf.check_light_change && num_samples > 0 &&
