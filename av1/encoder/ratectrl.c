      ...
      if (!cpi->refresh_alt_ref_frame) {
        ...
        if (gf_group->rf_level[gf_group->index] == GF_ARF_LOW) {
          ...
          if (gf_group->update_type[gf_group->index] == INTNL_ARF_UPDATE) {
            // === ALTREF2 ===
            // Lower down cq_level by 9/16 of the delta between cq_level and
            // 'gf_active_quality'.
            active_best_quality =
                cq_level -
                ((AOMMAX(0, (cq_level - rc->gf_active_quality)) * 9 + 8) >> 4);
          } else {
            // === BWDREF ===
            // Lower down cq_level by 3/8 of the delta between cq_level and
            // 'gf_active_quality'.
            active_best_quality =
                cq_level -
                ((AOMMAX(0, (cq_level - rc->gf_active_quality)) * 3 + 4) >> 3);
          }
#else
...
if (gf_group->update_type[gf_group->index] == BIPRED_UPDATE ||
  ...
  // Boost up cq_level by 1/8 of the delta between cq_level and
  // 'gf_active_quality'.
  active_best_quality = cq_level +
      ((AOMMAX(0, (cq_level - rc->gf_active_quality)) + 4) >> 3);
else
 */
#endif  // CONFIG_EXT_REFS
      active_best_quality = cq_level;
