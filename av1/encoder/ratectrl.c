...
void av1_rc_get_one_pass_vbr_params(AV1_COMP *cpi) {
  ...
  if (!cpi->refresh_alt_ref_frame &&
    ...
    if (sframe_enabled) {
      if (altref_enabled) {
        if (sframe_mode ==
            1) {  // sframe_mode= 1 : insert sframe if it matches altref.
          if (cm->current_video_frame % sframe_dist == 0 &&
              cpi->refresh_alt_ref_frame && cm->frame_type != KEY_FRAME &&
              cm->current_video_frame != 0) {
            cm->is_sframe = 1;
        ...
        } else {  // sframe_mode !=1: if sframe will be inserted at the
                  // closest coming altref frame
          if (cm->current_video_frame % sframe_dist == 0 &&
