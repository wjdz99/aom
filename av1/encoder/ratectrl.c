...
static int adjust_q_cbr(const AV1_COMP *cpi, int q, int active_worst_quality,
  ...
  // for current frame is low (~half of optimal). For such frames,
  // if the source_sad is non-zero, relax the max_delta_up and clamp
  // applied below.
  int overshoot_buffer_low =
      (cpi->rc.rc_1_frame == -1 && rc->frame_source_sad > 1000 &&
       p_rc->buffer_level < (p_rc->optimal_buffer_level << 1));
  int max_delta_down;
  int max_delta_up = (overshoot_buffer_low) ? 100: 20;
...
static void rc_scene_detection_onepass_rt(AV1_COMP *cpi,
  ...
  if (cpi->oxcf.tune_cfg.content != AOM_CONTENT_SCREEN) {
    min_thresh = (cm->width * cm->height <= 320 * 240) ? 50000 : 100000;
  }
  const BLOCK_SIZE bsize = BLOCK_64X64;
  ...
  uint64_t tmp_sad = 0;
  int num_samples = 0;
  const int thresh = (cm->width * cm->height <= 320 * 240) ? 4 : 6;
