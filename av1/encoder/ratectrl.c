...
static unsigned int estimate_scroll_motion(
  ...
  int search_size_width = (cpi->rc.frame_source_sad > 20000) ? 512 : 192;
...
static int fast_detection_scroll(
  ...
  uint8_t *src_y;
  uint8_t *last_src_y;
  // Only allow for 8 bit-depth for now.
...
static void rc_scene_detection_onepass_rt(AV1_COMP *cpi,
  ...
  int scroll_detected_frame_level = 0;
  int scroll_tested = 0;
  ...
  if (rc->high_source_sad) {
    ...
    if (cpi->oxcf.tune_cfg.content == AOM_CONTENT_SCREEN &&
        cpi->svc.temporal_layer_id == 0 && unscaled_src->y_width >= 1280 &&
        unscaled_src->y_height >= 720) {
      scroll_detected_frame_level = fast_detection_scroll(
  ...
  if (cpi->svc.temporal_layer_id == 0 && rc->drop_count_consec < 3) {
    ...
    if (cpi->oxcf.speed >= 11 &&
      ...
      if (scroll_tested && scroll_detected_frame_level) {
        cpi->rc.high_motion_content_screen_rtc = 0;
      } else {
