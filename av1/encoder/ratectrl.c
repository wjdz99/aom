...
static int adjust_q_cbr(const AV1_COMP *cpi, int q) {
  ...
  const AV1_COMMON *const cm = &cpi->common;
  // If resolution changes or avg_frame_bandwidth changes, then
  // target bits per macroblock will change.
  int change_target_bits_mb = cm->prev_frame &&
      (cm->width != cm->prev_frame->width ||
       cm->height != cm->prev_frame->height ||
       rc->avg_frame_bandwidth != rc->prev_avg_frame_bandwidth);
  // Apply some control/clamp to QP under certain conditions.
  if (cm->current_frame.frame_type != KEY_FRAME && !cpi->use_svc &&
      !change_target_bits_mb && (!cpi->oxcf.gf_cbr_boost_pct ||
    ...
    if (rc->rc_1_frame * rc->rc_2_frame == -1 &&
      ...
      // If previous frame had overshoot and the current q needs to increase
      // above the clamped value, reduce the clamp for faster reaction to
      // overshoot.
      if (rc->rc_1_frame == -1 && q > qclamp)
        q = (q + qclamp) >> 1;
      else
        q = qclamp;
    }
    // Limit the decrease in Q from previous frame.
    if (rc->q_1_frame - q > 16) q = rc->q_1_frame - 16;
  }
...
int av1_rc_regulate_q(const AV1_COMP *cpi, int target_bits_per_frame,
  ...
  if (cpi->oxcf.rc_mode == AOM_CBR) return adjust_q_cbr(cpi, q);
