...
static double resize_rate_factor(const FrameDimensionCfg *const frame_dim_cfg,
                                 int width, int height) {
  return (double)(frame_dim_cfg->width * frame_dim_cfg->height) /
         (width * height);
}
...
void av1_rc_init(const AV1EncoderConfig *oxcf, int pass, RATE_CONTROL *rc) {
  ...
  if (rc->min_gf_interval == 0)
    rc->min_gf_interval = av1_rc_get_default_min_gf_interval(
        oxcf->frame_dim_cfg.width, oxcf->frame_dim_cfg.height,
        oxcf->init_framerate);
...
static double get_rate_correction_factor(const AV1_COMP *cpi, int width,
  ...
  rcf *= resize_rate_factor(&cpi->oxcf.frame_dim_cfg, width, height);
...
static void set_rate_correction_factor(AV1_COMP *cpi, double factor, int width,
  ...
  factor /= resize_rate_factor(&cpi->oxcf.frame_dim_cfg, width, height);
  ...
  if (av1_frame_scaled(cm))
    rc->this_frame_target =
        (int)(rc->this_frame_target *
              resize_rate_factor(&cpi->oxcf.frame_dim_cfg, width, height));
...
void av1_rc_postencode_update(AV1_COMP *cpi, uint64_t bytes_used) {
  ...
  if (av1_frame_scaled(cm))
    rc->this_frame_target = (int)(rc->this_frame_target /
                                  resize_rate_factor(&cpi->oxcf.frame_dim_cfg,
                                                     cm->width, cm->height));
  if (current_frame->frame_type != KEY_FRAME) {
  ...
  /*
  rc->this_frame_target =
      (int)(rc->this_frame_target / resize_rate_factor(&cpi->oxcf.frame_dim_cfg,
  cm->width, cm->height));
      */
}
