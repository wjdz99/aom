...
static int adjust_rtc_keyframe(const AV1_COMP *cpi, int enumerator) {
  const RATE_CONTROL *rc = &cpi->rc;
  // Don't adjust ig most of the image is flat.
  ...
  if (rc->last_encoded_size_keyframe == 0 ||
    ...
    // Use stats from last encoded keyframe.
...
int av1_rc_bits_per_mb(const AV1_COMP *cpi, FRAME_TYPE frame_type, int qindex,
  ...
  if (cpi->oxcf.rc_cfg.mode == AOM_CBR && frame_type != KEY_FRAME &&
    ...
    enumerator = AOMMIN(AOMMAX((int)(ratio * sse_sqrt), 20000), 170000);
             ...
             cpi->sf.rt_sf.rc_adjust_keyframe &&
             cpi->oxcf.rc_cfg.max_intra_bitrate_pct > 0 &&
             cpi->oxcf.rc_cfg.max_intra_bitrate_pct < 1000 &&
             cpi->svc.spatial_layer_id == 0) {
    enumerator = adjust_rtc_keyframe(cpi, enumerator);
  }
