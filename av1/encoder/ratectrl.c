...
static int adjust_q_cbr(const AV1_COMP *cpi, int q, int active_worst_quality) {
  ...
  if (cm->current_frame.frame_type != KEY_FRAME && !cpi->use_svc &&
      ...
      (!cpi->oxcf.gf_cbr_boost_pct || !(frame_refresh_flags->alt_ref_frame ||
                                        frame_refresh_flags->golden_frame))) {
...
static double get_rate_correction_factor(const AV1_COMP *cpi, int width,
  ...
  if (cpi->common.current_frame.frame_type == KEY_FRAME) {
    ...
    if ((frame_refresh_flags->alt_ref_frame ||
         frame_refresh_flags->golden_frame) &&
...
static int calc_active_worst_quality_one_pass_vbr(const AV1_COMP *cpi) {
  ...
  if (cpi->common.current_frame.frame_type == KEY_FRAME) {
    ...
    if (!rc->is_src_frame_alt_ref && (frame_refresh_flags->golden_frame ||
                                      frame_refresh_flags->bwd_ref_frame ||
  ...
  if (cpi->oxcf.rc_mode != AOM_Q) {
    if (frame_is_intra_only(cm) ||
        (!rc->is_src_frame_alt_ref &&
         (frame_refresh_flags->golden_frame || is_intrl_arf_boost ||
          frame_refresh_flags->alt_ref_frame))) {
      active_best_quality -=
...
static int get_active_best_quality(const AV1_COMP *const cpi,
  ...
  const int is_leaf_frame =
      !(frame_refresh_flags->golden_frame ||
        frame_refresh_flags->alt_ref_frame || is_intrl_arf_boost);
  ...
  if (rc_mode == AOM_Q && !frame_refresh_flags->alt_ref_frame &&
      !is_intrl_arf_boost) {
    return cq_level;
  }
...
static void update_golden_frame_stats(AV1_COMP *cpi) {
  ...
  if (cpi->frame_refresh_flags.golden_frame || rc->is_src_frame_alt_ref) {
    rc->frames_since_golden = 0;
...
void av1_rc_postencode_update(AV1_COMP *cpi, uint64_t bytes_used) {
  ...
  if (current_frame->frame_type == KEY_FRAME) {
    ...
    if ((cpi->use_svc && cpi->oxcf.rc_mode == AOM_CBR) ||
        (!rc->is_src_frame_alt_ref &&
         !(frame_refresh_flags->golden_frame || is_intrnl_arf ||
           frame_refresh_flags->alt_ref_frame))) {
  ...
  if ((qindex < rc->last_boosted_qindex) ||
      ...
      (!rc->constrained_gf_group &&
       (frame_refresh_flags->alt_ref_frame || is_intrnl_arf ||
        (frame_refresh_flags->golden_frame && !rc->is_src_frame_alt_ref)))) {
  ...
  if (is_altref_enabled(cpi) && frame_refresh_flags->alt_ref_frame &&
      (current_frame->frame_type != KEY_FRAME))
