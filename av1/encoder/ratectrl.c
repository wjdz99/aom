...
static void vbr_rate_correction(AV1_COMP *cpi, int *this_frame_target) {
  ...
  if (frame_window > 0) {
    const int64_t max_delta = AOMMIN(
        llabs((vbr_bits_off_target / frame_window)),
    ...
    frame_target = (frame_target < INT_MAX) ? frame_target : INT_MAX;
  }
  ...
  if (!frame_is_kf_gf_arf(cpi) &&
    ...
    if (fast_extra_bits > 0) {
      ...
      // local undershoot and clamp to max allowed.
  ...
  *this_frame_target = (int)((frame_target < INT_MAX)
                             ? frame_target : INT_MAX);
}
