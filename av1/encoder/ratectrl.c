...
void av1_rc_init(const AV1EncoderConfig *oxcf, int pass, RATE_CONTROL *rc) {
  ...
  rc->resize_count = 0;
  rc->resize_scale_den = 1;
  rc->resize_scale_num = 1;
}
...
static void dynamic_resize_one_pass_cbr(AV1_COMP *cpi) {
  ...
  int min_width = (160 * 4) / 3;
  int min_height = (90 * 4) / 3;
  int down_size_on = 1;
  ...
  if (resize_action != NO_RESIZE) {
    ...
    if (resize_action == DOWN_THREEFOUR || resize_action == UP_THREEFOUR) {
      rc->resize_scale_num = 3;
      rc->resize_scale_den = 4;
    } else if (resize_action == DOWN_ONEHALF) {
      rc->resize_scale_num = 1;
      rc->resize_scale_den = 2;
    } else {  // UP_ORIG or anything else/
      rc->resize_scale_num = 1;
      rc->resize_scale_den = 1;
    }
    resize_width =
        resize_width * rc->resize_scale_num / rc->resize_scale_den;
    resize_height =
        resize_height * rc->resize_scale_num / rc->resize_scale_den;
    tot_scale_change = (rc->resize_scale_den * rc->resize_scale_den) /
                       (rc->resize_scale_num * rc->resize_scale_num);
    ...
    qindex = av1_rc_regulate_q(
        cpi, target_bits_per_frame, rc->best_quality, active_worst_quality,
...
void av1_get_one_pass_rt_params(AV1_COMP *cpi,
  ...
  if (cpi->oxcf.resize_cfg.resize_mode == RESIZE_DYNAMIC) {
    ...
    resize_pending_params->width = cpi->oxcf.frm_dim_cfg.width *
        rc->resize_scale_num / rc->resize_scale_den;
