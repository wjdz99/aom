...
void av1_rc_postencode_update(AV1_COMP *cpi, uint64_t bytes_used) {
  ...
  printf("frame_number, QP, frame_size, per-frame bandwidth ** references used ** golded/alt_ref refreshed: %d %d %d %d %d %d %d %d %d \n",
    cm->current_frame.frame_number, qindex, rc->projected_frame_size, rc->avg_frame_bandwidth, 
    (cpi->ref_frame_flags & AOM_LAST_FLAG) > 0, (cpi->ref_frame_flags & AOM_GOLD_FLAG) > 0, (cpi->ref_frame_flags & AOM_ALT_FLAG) > 0, 
    cpi->refresh_frame.golden_frame, cpi->refresh_frame.alt_ref_frame);
  ...
  if (!resize_pending && !rc->high_source_sad) {
    ...
    if (gf_update_changed) {
      ...
      for (unsigned int i = 0; i < INTER_REFS_PER_FRAME; i++) {
        ...
        refresh_mask |= rtc_ref->refresh[ref_frame_map_idx] << ref_frame_map_idx;
      }
...
void av1_set_reference_structure_one_pass_rt(AV1_COMP *cpi, int gf_update) {
  ...
  for (unsigned int i = 0; i < INTER_REFS_PER_FRAME; i++)
    cm->remapped_ref_idx[i] = rtc_ref->ref_idx[i];
}
...
void av1_get_one_pass_rt_params(AV1_COMP *cpi,
  ...
  if (set_key_frame(cpi, frame_flags)) {
    ...
    if (cpi->ppi->use_svc) {
      ...
      // If the user is setting the reference structure with set_ref_frame_config
      // and did not set any references, set the frame type to Intra-only.
