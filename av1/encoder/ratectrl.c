...
static int rc_pick_q_and_bounds_one_pass_cbr(const AV1_COMP *cpi, int width,
  ...
  *bottom_index = active_best_quality;
  ...
  if (cm->current_frame.frame_type == KEY_FRAME && !rc->this_key_frame_forced &&
    ...
    int qdelta = 0;
    aom_clear_system_state();
    qdelta = av1_compute_qdelta_by_rate(&cpi->rc, cm->current_frame.frame_type,
  ...
  if (frame_is_intra_only(cm)) {
    ...
    if (oxcf->rc_mode == AOM_Q) {
      ...
      const int delta_qindex = av1_compute_qdelta(
          rc, q_val,
          q_val *
              delta_rate[cm->current_frame.frame_number % FIXED_GF_INTERVAL],
          bit_depth);
      active_best_quality = AOMMAX(qindex + delta_qindex, rc->best_quality);
    } else {
      // Use the lower of active_worst_quality and recent/average Q.
      active_best_quality = (cm->current_frame.frame_number > 1)
  ...
  *bottom_index = active_best_quality;
  ...
  // Limit Q range for the adaptive loop.
  {
    ...
    if (cm->current_frame.frame_type == KEY_FRAME &&
        !rc->this_key_frame_forced && !(cm->current_frame.frame_number == 0)) {
      qdelta =
          av1_compute_qdelta_by_rate(&cpi->rc, cm->current_frame.frame_type,
                                     active_worst_quality, 2.0, bit_depth);
    } else if (!rc->is_src_frame_alt_ref &&
               (cpi->refresh_golden_frame || cpi->refresh_alt_ref_frame)) {
      qdelta =
...
void av1_rc_postencode_update(AV1_COMP *cpi, uint64_t bytes_used) {
  ...
  if (cm->current_frame.frame_type == KEY_FRAME) rc->last_kf_qindex = qindex;
  ...
  if (cm->current_frame.frame_type == KEY_FRAME) rc->frames_since_key = 0;
  // if (cm->current_frame.frame_number == 1 && cm->show_frame)
  /*
  rc->this_frame_target =
    ...
    if (sframe_enabled) {
      if (altref_enabled) {
        if (sframe_mode == 1) {
          ...
          if (cm->current_frame.frame_number % sframe_dist == 0 &&
              cm->current_frame.frame_type != KEY_FRAME &&
              cm->current_frame.frame_number != 0 &&
              cpi->refresh_alt_ref_frame) {
