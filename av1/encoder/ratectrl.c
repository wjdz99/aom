...
void av1_set_reference_structure_one_pass_rt(AV1_COMP *cpi, int gf_update) {
  ...
  if (cpi->sf.rt_sf.sad_based_adp_altref_lag) {
    ...
    const uint64_t th_frame_sad[4][3] = {
      { 25000, 22000, 18000 },  // HDRES CPU 9
      { 30000, 27000, 25000 },  // MIDRES CPU 9
      { 33000, 30000, 27000 },  // HDRES CPU10
      { 40000, 35000, 30000 }   // MIDRES CPU 10
    };
    int th_idx = cpi->sf.rt_sf.sad_based_adp_altref_lag - 1;
    assert(th_idx < 4);
    if (rc->avg_source_sad > th_frame_sad[th_idx][2])
      lag_alt = 3;
    else if (rc->avg_source_sad > th_frame_sad[th_idx][1])
      lag_alt = 4;
    else if (rc->avg_source_sad > th_frame_sad[th_idx][0])
      lag_alt = 5;
  }
