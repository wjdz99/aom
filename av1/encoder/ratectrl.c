...
static void set_reference_structure_one_pass_rt(AV1_COMP *cpi) {
  ...
  // where ALT_REF always behind LAST by lag_alt frames, and GOLDEN is
  // either updated on LAST with period baseline_gf_interval (fixed slot)
  // or always behind LAST by baseline_gf_interval (moving slot).
  int gld_fixed_slot = 1;
  unsigned int gld_interval = cpi->rc.baseline_gf_interval;
  unsigned int lag_alt = 2;
  int last_idx = 0;
  ...
  // For golden: fixed index or lag,
  if (gld_fixed_slot) {
    gld_idx = 6;
  } else {
    // Moving index for alt_ref, lag behind current by 3 frames.
    if (cm->current_frame.frame_number > gld_interval)
      gld_idx =
          ((cm->current_frame.frame_number - gld_interval) % 6);
  }
  // Moving index for alt_ref, lag behind LAST by lag_alt frames.
  if (cm->current_frame.frame_number > lag_alt)
    alt_ref_idx = ((cm->current_frame.frame_number - lag_alt) % 6);
