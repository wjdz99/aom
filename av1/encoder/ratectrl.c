...
static void resize_reset_rc(AV1_COMP *cpi, int resize_width,
                            int resize_height, int prev_width,
                            int prev_height) {
  RATE_CONTROL *const rc = &cpi->rc;
  SVC *const svc = &cpi->svc;
  LAYER_CONTEXT *lc = NULL;
  double tot_scale_change = 1.0;
  int target_bits_per_frame = rc->this_frame_target;
  int active_worst_quality = rc->worst_quality;
  int qindex = rc->worst_quality;
  int tl = 0;
  ...
  target_bits_per_frame = (int)rc->this_frame_target;
  ...
  if (tot_scale_change >= 1.0) {
    ...
    if (qindex <= 110 * rc->last_q[INTER_FRAME] / 100)
