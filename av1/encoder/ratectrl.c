...
int av1_post_encode_drop_cbr(AV1_COMP *cpi, size_t *size) {
  PRIMARY_RATE_CONTROL *const p_rc = &cpi->ppi->p_rc;
  ...
  // or the QP is already at max, then no need to drop.
  if (!cpi->rc.high_source_sad && new_buffer_level < 0 &&
      frame_size > 8 * (unsigned int)cpi->rc.avg_frame_bandwidth &&
      cpi->common.quant_params.base_qindex < cpi->rc.worst_quality) {
    *size = 0;
