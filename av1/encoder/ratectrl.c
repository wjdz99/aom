...
static double get_rate_correction_factor(const AV1_COMP *cpi, int width,
  ...
  if (cpi->common.current_frame.frame_type == KEY_FRAME) {
    ...
    const RATE_FACTOR_LEVEL rf_lvl =
        get_rate_factor_level(&cpi->ppi->gf_group, cpi->cur_gf_index);
...
int av1_frame_type_qdelta(const AV1_COMP *cpi, int q) {
  const GF_GROUP *const gf_group = &cpi->ppi->gf_group;
  const RATE_FACTOR_LEVEL rf_lvl =
      get_rate_factor_level(gf_group, cpi->cur_gf_index);
  const FRAME_TYPE frame_type = gf_group->frame_type[cpi->cur_gf_index];
  const int arf_layer = AOMMIN(gf_group->layer_depth[cpi->cur_gf_index], 6);
...
static int rc_pick_q_and_bounds(const AV1_COMP *cpi, int width, int height,
  ...
  if (oxcf->q_cfg.use_fixed_qp_offsets) {
    return get_q_using_fixed_offsets(oxcf, rc, gf_group, cpi->cur_gf_index,
                                     cq_level, bit_depth);
  }
...
void av1_get_one_pass_rt_params(AV1_COMP *cpi,
  ...
  if ((!cpi->use_svc && rc->frames_to_key == 0) ||
    ...
    gf_group->update_type[cpi->cur_gf_index] = KF_UPDATE;
    gf_group->frame_type[cpi->cur_gf_index] = KEY_FRAME;
    gf_group->refbuf_state[cpi->cur_gf_index] = REFBUF_RESET;
    if (cpi->use_svc) {
    ...
    gf_group->update_type[cpi->cur_gf_index] = LF_UPDATE;
    gf_group->frame_type[cpi->cur_gf_index] = INTER_FRAME;
    gf_group->refbuf_state[cpi->cur_gf_index] = REFBUF_UPDATE;
    if (cpi->use_svc) {
      LAYER_CONTEXT *lc = &svc->layer_context[layer];
      lc->is_key_frame =
  ...
  if (cpi->oxcf.rc_cfg.mode == AOM_CBR) {
    if (frame_params->frame_type == KEY_FRAME) {
      ...
      target = av1_calc_pframe_target_size_one_pass_cbr(
          cpi, gf_group->update_type[cpi->cur_gf_index]);
    }
  } else {
    if (frame_params->frame_type == KEY_FRAME) {
