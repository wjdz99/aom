...
static void dynamic_resize_one_pass_cbr(AV1_COMP *cpi) {
  ...
  if (resize_action != NO_RESIZE) {
    ...
    resize_pending_params->width =
        cpi->oxcf.frm_dim_cfg.width * resize_scale_num / resize_scale_den;
    resize_pending_params->height =
        cpi->oxcf.frm_dim_cfg.height * resize_scale_num / resize_scale_den;
    ...
    if (resize_action < 0 &&
        qindex > 130 * cm->quant_params.base_qindex / 100) {
