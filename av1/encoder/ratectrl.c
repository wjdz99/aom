...
static int get_minq_index(double maxq, double x3, double x2, double x1,
  ...
  return av1_find_qindex(minqtarget, bit_depth, 0, bit_depth == AOM_BITS_8 ? QINDEX_RANGE_UNEXT - 1 : QINDEX_RANGE - 1);
#else
...
#if CONFIG_EXTQUANT_HBD
  for (i = 0;
       i < (bit_depth == AOM_BITS_8 ? QINDEX_RANGE_UNEXT : QINDEX_RANGE);
       i++) {
#else
