...
static int od_codedquantizer_to_quantizer(int q) { return q; }

static int od_quantizer_to_codedquantizer(int q) { return q; }
...
static void od_enc_rc_reset(od_enc_ctx *enc) {
  int64_t npixels;
  int64_t ibpp;
  enc->rc.bits_per_frame =
      (int64_t)(enc->rc.target_bitrate / enc->state.info.framerate);
  ...
  npixels = enc->state.frame_width * (int64_t)enc->state.frame_height;
...
int od_enc_rc_resize(od_enc_ctx *enc) {
  ...
  if (enc->state.cur_time == 0) {
    ...
    enc->rc.reservoir_max =
        enc->rc.bits_per_frame * enc->rc.reservoir_frame_delay;
    enc->rc.reservoir_target = ((enc->rc.reservoir_max + 1) >> 1) +
                               ((enc->rc.bits_per_frame + 2) >> 2) *
                                   OD_MINI(enc->state.info.keyframe_rate,
                                           enc->rc.reservoir_frame_delay);
    ...
    if (idt < OD_MINI(enc->rc.inter_p_delay, enc->rc.frame_count[OD_P_FRAME])) {
...
int od_enc_rc_init(od_enc_ctx *enc, int64_t bitrate, int delay_ms) {
  od_rc_state *rc;
  if (enc->state.info.framerate <= 0) return OD_EINVAL;
  rc = &enc->rc;
  ...
  if (bitrate > 0) {
    ...
    rc->reservoir_frame_delay =
        (int)OD_MINI((delay_ms / 1000.0f) * enc->state.info.framerate, 256.0f);
    rc->drop_frames = 1;
    rc->cap_overflow = 1;
    rc->cap_underflow = 0;
    rc->twopass_state = 0;
...
int od_frame_type(daala_enc_ctx *enc, int64_t coding_frame_count,
                  int *is_golden, int *is_altref, int64_t *ip_count) {
  int frame_type;
  ...
  *is_golden = (*ip_count % enc->input_queue.goldenframe_rate) == 0 ||
               frame_type == OD_I_FRAME;
  *is_altref = (*ip_count % enc->input_queue.altref_rate) == 0 ||
               frame_type == OD_I_FRAME;
  return frame_type;
}
...
static int frame_type_count(od_enc_ctx *enc, int nframes[OD_FRAME_NSUBTYPES]) {
  ...
  for (i = 0; i < reservoir_frame_delay; i++) {
    ...
    int is_altref;
    int64_t dummy;
    frame_type = od_frame_type(enc, enc->curr_coding_order + i, &is_golden,
                               &is_altref, &dummy);
    switch (frame_type) {
      case OD_I_FRAME: {
...
static int quality_to_quantizer(int quality, int bit_depth) {
  if (quality < 96) /* Linear region for low quantizers */
    ...
    return (int)av1_convert_qindex_to_q(quality, bit_depth) + 60;
}
...
int od_enc_rc_select_quantizers_and_lambdas(od_enc_ctx *enc,
  ...
  if (!enc->input_queue.end_of_input) {
    ...
    closed_form_type =
        od_frame_type(enc, enc->curr_coding_order, &closed_form_golden,
                      &closed_form_altref, &closed_form_ip_count);
  ...
  /*Quantizer selection sticks to the codable, lossy portion of the quantizer
    range.*/
  lossy_quantizer_min = od_codedquantizer_to_quantizer(1);
  lossy_quantizer_max =
  ...
  if (enc->rc.target_bitrate <= 0) {
    ...
    if (enc->quality == 0) {
      ...
      if (enc->quality == -1) {
        ...
        enc->rc.base_quantizer =
            quality_to_quantizer(enc->quality, enc->bit_depth);
      }

      if (!is_golden_frame) {
        if (enc->quality <
            96) { /* Low quantizers (below quality == 24) are linear */
        ...
        } else { /* Ported from the old RC system, for non-linear higher Qs */
          const double delta_rate[] = { 0.50, 1.0,  0.85, 1.0,  0.70,
                                        1.0,  0.85, 1.0,  0.50, 1.0,
                                        0.85, 1.0,  0.70, 1.0,  0.85,
                                        1.0,  0.50, 1.0,  0.85, 1.0,
                                        0.70, 1.0,  0.85, 1.0,  0.50 };
          const int delta_qindex = av1_compute_qdelta(
              enc->alt_rc, enc->rc.base_quantizer,
              enc->rc.base_quantizer *
                  delta_rate[enc->ip_frame_count %
                             enc->input_queue.goldenframe_rate],
              enc->input_queue.goldenframe_rate);
      ...
      log_quantizer =
          od_blog64(enc->rc.base_quantizer) - OD_Q57(OD_COEFF_SHIFT);
    ...
    /*Downgrade the delta frame rate to correspond to the recent drop count
      ...
      In the event this is extended later, the drop tracking watches all
       frame types.*/
    nframes[OD_I_FRAME] =
        od_rc_scale_drop(&enc->rc, OD_I_FRAME, nframes[OD_I_FRAME]);
    nframes[OD_P_FRAME] =
        od_rc_scale_drop(&enc->rc, OD_P_FRAME, nframes[OD_P_FRAME]);
    ...
    rate_bias =
        (enc->rc.rate_bias / (enc->state.cur_time + 1000)) * reservoir_frames;
    ...
    while (qlo < qhi) {
      ...
      for (i = 0; i < OD_FRAME_NSUBTYPES; i++) {
        ...
        bits +=
            nframes[i] * od_bexp64(enc->rc.log_scale[i] + enc->rc.log_npixels -
                                   (log_quantizer >> 6) * enc->rc.exp[i]);
      }
    ...
    if (clamp) {
      base_quantizer = OD_CLAMPI(
          (enc->rc.base_quantizer * 0x0CCCD + 0x8000) >> 16, base_quantizer,
    ...
    if (enc->rc.cap_overflow) {
      ...
      log_scale_pixels = enc->rc.log_scale[frame_subtype] + enc->rc.log_npixels;
    ...
    if (enc->quality == -1) {
      ...
      /*Compute the maximum number of bits we can use in the next frame.
        ...
        This may not be enough for keyframes or sudden changes in
         complexity.*/
      log_hard_limit =
          od_blog64(enc->rc.reservoir_fullness + (enc->rc.bits_per_frame >> 1));
  ...
  enc->mv_rdo_lambda = OD_MAXI(
      ((2320000 + (((1 << OD_COEFF_SHIFT) - 1) >> 1)) >> OD_COEFF_SHIFT) *
              enc->target_quantizer >>
          (22 - OD_LAMBDA_SCALE),
      40);
  ...
  /*The PVQ RDO lambda is used for RDO calculations involving unquantized
     data.*/
  enc->pvq_rdo_lambda =
      OD_PVQ_LAMBDA * enc->target_quantizer * enc->target_quantizer;
  ...
  enc->dering_lambda =
      0.67 * OD_PVQ_LAMBDA * enc->target_quantizer * enc->target_quantizer;
  *bottom_idx = enc->target_quantizer;
  *top_idx = lossy_quantizer_max;
  return enc->target_quantizer;
}

int od_enc_rc_update_state(od_enc_ctx *enc, int64_t bits, int is_golden_frame,
  ...
  if (enc->rc.target_bitrate > 0) {
    ...
    if (bits <= 0) {
      ...
      log_scale =
          OD_MINI(log_bits - enc->rc.log_npixels + log_qexp, OD_Q57(16));
    }
