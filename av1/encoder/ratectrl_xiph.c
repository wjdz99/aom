...
int od_enc_rc_init(od_rc_state *rc, int64_t bitrate, int delay_ms) {
  ...
  if (bitrate > 0) {
    ...
    rc->reservoir_frame_delay =
        (int)OD_MINI((delay_ms / 1000.0f) * rc->framerate, 256.0f);
    rc->drop_frames = 1;
    rc->cap_overflow = 1;
    rc->cap_underflow = 0;
    rc->twopass_state = 0;
...
static int frame_type_count(od_rc_state *rc, int nframes[OD_FRAME_NSUBTYPES]) {
  ...
  /*Don't count past the end of the stream (once we know where end-of-stream
     is).*/
  reservoir_frame_delay = rc->end_of_input ? enc->input_queue.input_size + 1
                                           : rc->reservoir_frame_delay;
#endif
  for (i = 0; i < reservoir_frame_delay; i++) {
    ...
    int is_altref;
    int64_t dummy;
    frame_type = od_frame_type(rc, rc->curr_coding_order + i, &is_golden,
                               &is_altref, &dummy);
    switch (frame_type) {
      case OD_I_FRAME: {
...
int od_enc_rc_select_quantizers_and_lambdas(od_rc_state *rc,
  ...
  if (!rc->end_of_input) {
    ...
    closed_form_type =
        od_frame_type(rc, rc->curr_coding_order, &closed_form_golden,
                      &closed_form_altref, &closed_form_ip_count);
    OD_UNUSED(closed_form_type);
    OD_ASSERT(closed_form_type == frame_type);
    OD_ASSERT(closed_form_ip_count == rc->ip_frame_count);
  ...
  if (rc->target_bitrate <= 0) {
    ...
    /*Can't use the OD_LOSSLESS macro, as it uses rc.quantizer to intuit,
      and we've not set it yet.*/
    if (rc->quality == 0) {
      /*Lossless coding requested.*/
      rc->base_quantizer = 0;
      rc->target_quantizer = 0;
      rc->coded_quantizer = 0;
      rc->quantizer = 0;
    } else {
      int quantizer;
      ...
      if (rc->quality == -1) {
        ...
        rc->base_quantizer = quality_to_quantizer(10, rc->bit_depth);
      } else {
        rc->base_quantizer = quality_to_quantizer(rc->quality, rc->bit_depth);
      }
      ...
      if (!is_golden_frame) {
        if (rc->quality <
          ...
          int dist_to_golden = rc->ip_frame_count % pattern_rate;
          ...
          const int delta_qindex = av1_compute_qdelta(
              rc->alt_rc, rc->base_quantizer,
              rc->base_quantizer *
                  delta_rate[rc->ip_frame_count % rc->goldenframe_rate],
              rc->goldenframe_rate);
          rc->base_quantizer += delta_qindex;
        }
      }
      ...
      log_quantizer =
          (int64_t)od_quantizer_to_codedquantizer(rc->base_quantizer) << 33;
      log_quantizer *= mqp_Q12[frame_subtype];
      log_quantizer += dqp_Q45[frame_subtype];
      rc->coded_quantizer =
          OD_CLAMPI(1, log_quantizer >> 45, OD_N_CODED_QUANTIZERS - 1);
      rc->quantizer = od_codedquantizer_to_quantizer(rc->coded_quantizer);
    }
  } else {
    int clamp;
    int reservoir_frames;
    ...
    rate_bias = (rc->rate_bias / (rc->cur_time + 1000)) * reservoir_frames;
    ...
    /*If there's a quality specified, it's used to select the
      ...
      Otherwise we can use up to and including the coarsest codable
       quantizer.*/
    if (rc->quality > 0)
      qhi = quality_to_quantizer(rc->quality, rc->bit_depth);
    else
    ...
    /*Determine actual quantizer to code/use from the current ideal
       quantizer.*/
    rc->coded_quantizer = od_quantizer_to_codedquantizer(rc->target_quantizer);
    rc->quantizer = od_codedquantizer_to_quantizer(rc->coded_quantizer);
  }
  *bottom_idx = rc->target_quantizer;
