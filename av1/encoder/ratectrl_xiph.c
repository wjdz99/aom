  ...
  if (rc->target_bitrate <= 0) {
    ...
    if (rc->quality == 0) {
      ...
      if (rc->periodic_boosts) {
        const double delta_rate[] = { 0.50, 1.0,  0.85, 1.0,  0.70, 1.0,  0.85,
                                      1.0,  0.50, 1.0,  0.85, 1.0,  0.70, 1.0,
                                      0.85, 1.0,  0.50, 1.0,  0.85, 1.0,  0.70,
                                      1.0,  0.85, 1.0,  0.50 };
        const int delta_qindex = av1_compute_qdelta(
            alt_rc, rc->base_quantizer,
            rc->base_quantizer *
                delta_rate[rc->cur_frame % rc->goldenframe_rate],
            rc->goldenframe_rate);
        double nb = ((delta_qindex * 0.5f + rc->prev_frame_boost) / 2.0f) -
                    rc->prev_frame_boost * 0.8;
        nb *= OD_PERIODIC_BOOST_STRENGTH;
        rc->prev_frame_boost = nb;
