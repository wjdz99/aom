...
int od_enc_rc_select_quantizers_and_lambdas(od_rc_state *rc,
  ...
  /*Quantizer selection sticks to the codable, lossy portion of the quantizer
    range.*/
  lossy_quantizer_min = av1_convert_qindex_to_q(rc->minq, rc->bit_depth);
  lossy_quantizer_max = av1_convert_qindex_to_q(rc->maxq, rc->bit_depth);
  ...
  if (rc->target_bitrate <= 0) {
    ...
    if (rc->quality == 0) {
      ...
      if (rc->quality == -1) {
        ...
        rc->base_quantizer = av1_convert_qindex_to_q(10, rc->bit_depth);
      } else {
        rc->base_quantizer =
            av1_convert_qindex_to_q(rc->quality, rc->bit_depth);
      }
      ...
      log_quantizer = (int64_t)od_bexp64(rc->base_quantizer) << 33;
      log_quantizer *= mqp_Q12[frame_subtype];
      log_quantizer += dqp_Q45[frame_subtype];
      rc->coded_quantizer = OD_CLAMPI(1, log_quantizer >> 45, MAXQ);
      rc->quantizer = od_blog64(rc->coded_quantizer);
    }
  } else {
    int clamp;
    int reservoir_frames;
    ...
    /*If there's a quality specified, it's used to select the
      ...
      Otherwise we can use up to and including the coarsest codable
       quantizer.*/
    if (rc->quality > 0)
      qhi = av1_convert_qindex_to_q(rc->quality, rc->bit_depth);
    else
    ...
    /*Determine actual quantizer to code/use from the current ideal
       quantizer.*/
    rc->coded_quantizer = od_bexp64(rc->target_quantizer);
    rc->quantizer = od_blog64(rc->coded_quantizer);
  }
