...
int od_codedquantizer_to_quantizer(int cq) {
  ...
  return cq < OD_N_CODED_QUANTIZERS
             ? (OD_CODED_QUANTIZER_MAP_Q4[cq] << OD_COEFF_SHIFT >> 4)
...
int od_enc_rc_init(od_rc_state *rc, long bitrate, int delay_ms) {
...
static int quality_to_quantizer(int quality, int bit_depth) {
  return (int)av1_convert_qindex_to_q(quality, bit_depth);
}
...
int od_enc_rc_select_quantizers_and_lambdas(od_rc_state *rc,
  ...
  *top_idx = lossy_quantizer_max;
  return av1_qindex_from_ac(rc->target_quantizer, rc->bit_depth);
}

int od_enc_rc_update_state(od_rc_state *rc, long bits, int is_golden_frame,
