...
static void select_tx_partition_type(
  ...
  for (TX_PARTITION_TYPE type = 0; type < TX_PARTITION_TYPES; ++type) {
    ...
    if (!use_tx_partition(cpi->common.seq_params.use_inter_4way_tx_split,
                          cpi->common.seq_params.use_intra_4way_tx_split,
                          is_inter_block(mbmi), type, max_tx_size))
      continue;
    ...
    if (max_tx_size > TX_4X4) {
      ...
      partition_rd_stats.rate +=
          inter_tx_partition_cost(x, is_rect, type, tx_above + blk_col,
                                  tx_left + blk_row, mbmi->sb_type, max_tx_size,
                                  cpi->common.seq_params.use_inter_4way_tx_split);
    }
    ...
    tx_size_rate =
        is_inter
            ? inter_tx_partition_cost(
                  x, is_rect, 0, xd->above_txfm_context, xd->left_txfm_context,
                  mbmi->sb_type, max_tx_size,
                  cpi->common.seq_params.use_inter_4way_tx_split)
            : tx_size_cost(x, bs, cpi->common.seq_params.use_intra_4way_tx_split,
                           tx_size);
