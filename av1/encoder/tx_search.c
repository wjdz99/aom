...
int64_t av1_uniform_txfm_yrd(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  if (tx_select) {
    ...
    const TX_SIZE max_tx_size = max_txsize_rect_lookup[bs];
    const int is_rect = is_rect_tx(max_tx_size);
       inter_tx_partition_cost(is_rect, 0,
        xd->above_txfm_context, xd->left_txfm_context, mbmi->sb_type, max_tx_size);
    }
    tx_size_rate = is_inter ?  inter_tx_partition_cost(is_rect, 0,
        xd->above_txfm_context, xd->left_txfm_context, mbmi->sb_type, max_tx_size) // sarahparker max_tx_size?
