...
#if USE_SUBSAMPLE
    for (int j = 0; j < num_to_average; j += 2) {
#else
...
void get_xsx_from_filter(const int16_t *diff, int stride, int len,
                         int num_to_average, int num_operators,
                         const int *operators[], const int *wops, int *xsx,
                         const int *nes, const int sc_fac, int is_col_tx) {
  *xsx = 0;
  int temp[16] = { 0 }, y[16] = { 0 };
  int ids = 0, idt = 0;
  const int *operator= NULL;
#if USE_SUBSAMPLE
  for (int l = 0; l < num_to_average; l += 2) {
#else
  for (int l = 0; l < num_to_average; l++) {
#endif
    for (int j = 0; j < len; j++) y[j] = 0;
    for (int k = 0; k < num_operators; k++) {
      operator= operators[k];
      ...
      for (int i = 0; i < nes[k]; i++) {
        ...
        idt = operator[2 * i + 1];
        if (is_col_tx) {
          temp[ids] += diff[idt * stride + l];
          temp[idt] += diff[ids * stride + l];
        } else {
          temp[ids] += diff[l * stride + idt];
          temp[idt] += diff[l * stride + ids];
        }
      }
      for (int j = 0; j < len; j++) y[j] += wops[k] * temp[j];
    }
    // times x in the left
    for (int i = 0; i < len; i++)
      *xsx += y[i] * (is_col_tx ? diff[i * stride + l] : diff[l * stride + i]);
  }
  *xsx /= (num_to_average * sc_fac);
}
...
#if USE_FILTER_OPERATION
    get_xsx_from_filter(diff, stride, bw, bh, nbd, Bds[bwidx], wds[bwidx],
                        &xlx_h[DCT_1D], nedges[bwidx], soc[bwidx], 0);
    get_xsx_from_filter(diff, stride, bh, bw, nbd, Bds[bhidx], wds[bhidx],
                        &xlx_v[DCT_1D], nedges[bhidx], soc[bhidx], 1);
#else
