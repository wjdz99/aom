...
static INLINE int64_t av1_block_error_qm(const tran_low_t *coeff,
                                         ...
                                         intptr_t block_size,
                                         const QUANT_PARAM *quant_param,
                                         const SCAN_ORDER *scan_order,
                                         int64_t *ssz) {
  int64_t out_dist = 0;
  *ssz = 0;
  for (intptr_t i = 0; i < block_size; i++) {
    int64_t weight = quant_param->qmatrix[scan_order->scan[i]];
    int64_t dd = coeff[i] - dqcoeff[i];
    dd *= weight;
    int64_t cc = coeff[i];
    cc *= weight;
    out_dist += (dd * dd + (1 << (2 * AOM_QM_BITS - 1))) >> (2 * AOM_QM_BITS);
    *ssz += (cc * cc + (1 << (2 * AOM_QM_BITS - 1))) >> (2 * AOM_QM_BITS);
  }
  return out_dist;
}
...
#endif
  {
    if (quant_param->qmatrix == NULL ||
        !x->txfm_search_params.use_qm_dist_metric) {
      *out_dist = av1_block_error(coeff, dqcoeff, buffer_length, &this_sse);
    } else {
      *out_dist = av1_block_error_qm(coeff, dqcoeff, buffer_length, quant_param,
                                     scan_order, &this_sse);
    }
  }
...
uint16_t prune_txk_type_separ(const AV1_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  for (idx = 0; idx < 4; ++idx) {
    ...
    dist_block_tx_domain(x, plane, block, tx_size, &quant_param, scan_order,
                         &dist, &sse);
...
int64_t av1_estimate_txfm_yrd(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  for (int blk_row = 0; blk_row < max_blocks_high && !args.incomplete_exit;
    ...
    for (int blk_col = 0; blk_col < max_blocks_wide; blk_col += txw_unit) {
      ...
      const SCAN_ORDER *const scan_order =
          get_scan(txfm_param.tx_size, txfm_param.tx_type);
      dist_block_tx_domain(x, 0, i, tx_size, &quant_param, scan_order,
                           &this_rd_stats.dist, &this_rd_stats.sse);
