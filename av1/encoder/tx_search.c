...
static void ml_predict_tx_partition(const AV1_COMP *cpi, MACROBLOCK *x, 
  ...
  aom_clear_system_state();
  int int_score = 0;
  const int ml_threshold = cpi->sf.tx_sf.tx_type_search.ml_tx_split_thresh;
  const float comparison_threshold = 0.1f;
  ...
  for (TX_PARTITION_TYPE type = TX_PARTITION_SPLIT; type < TX_PARTITION_HORZ4; ++type) {
    if (!use_tx_partition(type, max_tx_size)) {
      skip_partition_mask[type] = 1;
      continue;
    }

    if (x->e_mbd.bd == 8 && nn_config && ml_threshold >= 0 && 
      ...
      if (split_score < -ml_threshold)  {
        skip_partition_mask[type] = 1; 
        continue;
      }
    }
    // TODO(sarahparker) implement more features for rectangular partitions
    if (is_rect) continue;
    if (type == TX_PARTITION_HORZ) {
      if (fabs(features[0] - features[4]) > comparison_threshold &&
          fabs(features[2] - features[6]) > comparison_threshold) skip_partition_mask[type] = 1;
    } else if (type == TX_PARTITION_VERT) { 
      if (fabs(features[0] - features[2]) > comparison_threshold &&
          fabs(features[4] - features[6]) > comparison_threshold) skip_partition_mask[type] = 1;
