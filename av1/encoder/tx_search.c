...
static AOM_INLINE int get_mean_dev_features(const int16_t *data, int stride,
  ...
  // For feature extraction, square blocks are split into 4 square sub-blocks
  // and rectangular blocks are split into 2 square/rectangular sub-blocks.
  // Hence the number of sub-blocks should be at least 2.
  assert(num_sub_blks > 1);
