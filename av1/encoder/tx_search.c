...
static void select_tx_partition_type(
  ...
  for (TX_PARTITION_TYPE type = 0; type < TX_PARTITION_TYPES; ++type) {
    ...
    if (max_tx_size > TX_4X4) {
      ...
      partition_rd_stats.rate += x->mode_costs.txfm_partition_cost[is_rect][ctx_0][type];
    }
    ...
    for (int r = 0; r < tx_size_high_unit[max_tx_size]; r += bsh) {
      for (int c = 0; c < tx_size_wide_unit[max_tx_size]; c += bsw, ++blk_idx) {
        ...
        try_tx_block_no_split(cpi, x, offsetr, offsetc, cur_block, sub_tx, 0,
                              ...
                              ref_best_rd - tmp_rd, ftxs_mode, 
                              rd_info_node, &no_split);
  ...
  for (int r = 0; r < tx_size_high_unit[max_tx_size]; r += bsh) {
    for (int c = 0; c < tx_size_wide_unit[max_tx_size]; c += bsw) {
      ...
      set_blk_skip(x->txfm_search_info.blk_skip, 0, offsetr * bw + offsetc, full_blk_skip[cur_partition]);
      block += sub_step;
      cur_partition++;
    }
  }
...
static void choose_tx_size_type_from_rd(const AV1_COMP *const cpi,
  ...
  if (!tx_select) chosen_tx_size = tx_size_from_tx_mode(bs, txfm_params->tx_mode_search_type);
...
#endif
    if (!cpi->oxcf.txfm_cfg.enable_tx64 && txsize_sqr_up_map[cur_tx_size] == TX_64X64)
      continue;

    RD_STATS this_rd_stats;
    cur_rd = av1_uniform_txfm_yrd(cpi, x, &this_rd_stats, ref_best_rd, bs, cur_tx_size,
                      FTXS_NONE, 0);
