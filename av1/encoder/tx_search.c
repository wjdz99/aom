                                     ...
                                     TX_SIZE tx_size, void *arg) 
{
  ...
  if (!is_inter) 
  {
    ...
    av1_predict_intra_block_facade(cm, xd, plane, blk_col, blk_row, tx_size);//预测值

    av1_subtract_txb(x, plane, plane_bsize, blk_col, blk_row, tx_size);//残差
  }
  ...
  const int blk_idx = blk_row * (block_size_wide[plane_bsize] >> MI_SIZE_LOG2) + blk_col;
  if (plane == 0)
  ...
  if (is_inter) 
  {
    const int64_t no_skip_rd = RDCOST(x->rdmult, this_rd_stats.rate, this_rd_stats.dist);
...
int64_t av1_uniform_txfm_yrd(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  MB_MODE_INFO *const mbmi = xd->mi[0];
  const int is_inter = is_inter_block(mbmi);
  const int tx_select = x->tx_mode_search_type == TX_MODE_SELECT && block_signals_txsize(mbmi->sb_type);
  int tx_size_rate = 0;
  if (tx_select) 
  {
    const int ctx = txfm_partition_context(xd->above_txfm_context, xd->left_txfm_context, mbmi->sb_type, tx_size);
...
{//传进来是current_rd=0
  ...
  av1_foreach_transformed_block_in_plane(xd, plane_bsize, plane, block_rd_txfm, &args);
