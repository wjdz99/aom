...
static INLINE int is_intra_hash_match(
    ...
    TXB_RD_INFO **intra_txb_rd_info, const int tx_type_map_idx,
...
static void search_tx_type(const AV1_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  const int is_inter = is_inter_block(mbmi);
  const int use_intra_txb_hash = cpi->sf.tx_sf.use_intra_txb_hash &&
  ...
  unsigned int block_mse_q8;
  int64_t block_sse =
      pixel_diff_dist(x, plane, blk_row, blk_col, plane_bsize,
