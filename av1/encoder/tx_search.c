...
static void ml_predict_intra_tx_depth_prune(MACROBLOCK *x, int blk_row,
  ...
  // 2) When transform and current block sizes do not match
  // 3) When operating bit-depth is not 8-bit
  ...
  if (score <= intra_tx_prune_thresh[0])
    txfm_params->nn_prune_mask_for_intra_tx_depths |= TX_PRUNE_SPLIT;
  else if (score > intra_tx_prune_thresh[1])
    txfm_params->nn_prune_mask_for_intra_tx_depths |= TX_PRUNE_LARGEST;
}
...
static AOM_INLINE void choose_tx_size_type_from_rd(const AV1_COMP *const cpi,
...
#if !CONFIG_REALTIME_ONLY
    if (txfm_params->nn_prune_mask_for_intra_tx_depths & TX_PRUNE_SPLIT) break;
  ...
  // consumption of prune mask
  txfm_params->enable_nn_prune_intra_tx_depths = 0;
  txfm_params->nn_prune_mask_for_intra_tx_depths = 0;
#endif
}
  ...
  if (!is_inter) {
    ...
    if (txfm_params->enable_nn_prune_intra_tx_depths) {
      ml_predict_intra_tx_depth_prune(x, blk_row, blk_col, plane_bsize,
                                      tx_size);
      if (txfm_params->nn_prune_mask_for_intra_tx_depths & TX_PRUNE_LARGEST) {
