...
static int skip_trellis_opt_based_on_satd(
    MACROBLOCK *x, int plane, int block, int qstep, TxfmParam *txfm_param,
  ...
  const int satd = RIGHT_SIGNED_SHIFT(aom_satd(coeff_ptr, n_coeffs), shift);

  const int skip_block_coeff_opt =
      ((uint64_t)satd >
       (uint64_t)coeff_opt_satd_threshold * qstep * sqrt_tx_pixels_2d[tx_size]);

  return skip_block_coeff_opt;
}
...
static void search_tx_type(const AV1_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  for (int idx = 0; idx < TX_TYPES; ++idx) {
    ...
    if (!skip_trellis) {
      skip_trellis_based_on_satd[tx_type] =
          skip_trellis_opt_based_on_satd(x, plane, block, qstep, &txfm_param,
                                         txfm_params->coeff_opt_satd_threshold);
      av1_setup_quant(tx_size, !skip_trellis_based_on_satd[tx_type],
                      skip_trellis_based_on_satd[tx_type]
                          ? (USE_B_QUANT_NO_TRELLIS ? AV1_XFORM_QUANT_B
                                                    : AV1_XFORM_QUANT_FP)
                          : AV1_XFORM_QUANT_FP,
