...
#if CONFIG_DSPL_RESIDUAL && CONFIG_DEBUG
...
static int find_tx_size_rd_records(MACROBLOCK *x, BLOCK_SIZE bsize,
  ...
  TXB_RD_RECORD *rd_records_table[4] = {
    txfm_info->txb_rd_record_8X8[dspl_type],
    txfm_info->txb_rd_record_16X16[dspl_type],
    txfm_info->txb_rd_record_32X32[dspl_type],
    txfm_info->txb_rd_record_64X64[dspl_type]
  };
#else
...
#if CONFIG_DSPL_RESIDUAL
  // TODO(singhprakhar): See if DSPL_XY can be used to optimize this
  param.dspl_type = DSPL_NONE;
#endif
...
#if CONFIG_DSPL_RESIDUAL
    TXB_RD_INFO_NODE *rd_info_node, DSPL_TYPE dspl_type);
#else
...
#if CONFIG_DSPL_RESIDUAL
  av1_inverse_transform_block(xd, dqcoeff, plane, tx_type, tx_size, dspl_type,
                              dst, dst_stride, eob, reduced_tx_set);
#else
...
static INLINE void recon_intra(const AV1_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  assert(IMPLIES(!is_inter, dspl_type == DSPL_NONE));
#endif
  ...
  if (!is_inter && best_eob &&
...
#if CONFIG_DSPL_RESIDUAL
      av1_setup_xform(cm, x, tx_size, best_tx_type, mbmi->dspl_type,
                      &txfm_param_intra);
#else
    ...
    inverse_transform_block_facade(x, plane, block, blk_row, blk_col,
                                   x->plane[plane].eobs[block], dspl_type,
...
#if CONFIG_DSPL_RESIDUAL
  av1_setup_xform(cm, x, tx_size, DCT_DCT, DSPL_BAD, &txfm_param);
#else
...
static void search_tx_type(const AV1_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  if (use_intra_txb_hash) {
    ...
    if (within_border &&
      ...
      recon_intra(cpi, x, plane, block, blk_row, blk_col, plane_bsize, tx_size,
                  txb_ctx, skip_trellis, best_tx_type, dspl_type, 1, &rate_cost,
                  best_eob);
#else
      recon_intra(cpi, x, plane, block, blk_row, blk_col, plane_bsize, tx_size,
...
#if CONFIG_DSPL_RESIDUAL
  av1_setup_xform(cm, x, tx_size, DCT_DCT, dspl_type, &txfm_param);
#else
  av1_setup_xform(cm, x, tx_size, DCT_DCT, &txfm_param);
#endif
  av1_setup_quant(tx_size, !skip_trellis,
...
#if CONFIG_DSPL_RESIDUAL && CONFIG_DSPL_ASSERT
  assert(IMPLIES(dspl_type == DSPL_XY, is_inter));
#endif
  ...
  for (int idx = 0; idx < TX_TYPES; ++idx) {
    ...
    if ((rand() % 100 == 1) && tx_type == DCT_DCT && is_inter) {
      const int block_offset = BLOCK_OFFSET(block);
      tran_low_t *const coeff = p->coeff + block_offset;
      tran_low_t *const dqcoeff = p->dqcoeff + block_offset;
      const int diff_stride = block_size_wide[plane_bsize];
      ...
      const uint8_t *src = &x->plane[plane].src.buf[src_offset << MI_SIZE_LOG2];
      const uint8_t *dst =
          &xd->plane[plane].dst.buf[src_offset << MI_SIZE_LOG2];

      // Inverse
      DECLARE_ALIGNED(32, uint16_t, recon[MAX_SB_SQUARE]);
      DECLARE_ALIGNED(32, int16_t, recon_diff[MAX_SB_SQUARE]);

      aom_convolve_copy(dst, dst_stride, (uint8_t *)recon, txw, txw, txh);
      av1_inverse_transform_block(xd, dqcoeff, plane, tx_type, tx_size,
                                  dspl_type, (uint8_t *)recon, txw, eob,
                                  cpi->common.features.reduced_tx_set_used);
      av1_inverse_transform_block_diff(
          xd, dqcoeff, plane, tx_type, tx_size, recon_diff, txw, eob,
          cpi->common.features.reduced_tx_set_used);

      FILE *f = fopen("transforms.txt", "a");
      fprintf(f, "dspl_type = %s, tx_size = (%d, %d), eob = %d\n",
              dspl_type_to_str(dspl_type), txh, txw, eob);
      print_array2d_uint8_t(src, txw, txh, src_stride, "src", f);
      print_array2d_int16_t(src_diff, txw, txh, diff_stride, "src_diff", f);
      print_array2d_int32_t(coeff, txw, txh, txw, "coeff", f);
      print_array2d_int32_t(dqcoeff, txw, txh, txw, "dqcoeff", f);
      print_array2d_int16_t(recon_diff, txw, txh, txw, "recon_diff", f);
      print_array2d_uint8_t((uint8_t *)recon, txw, txh, txw, "recon", f);
      print_array2d_uint8_t(dst, txw, txh, dst_stride, "dst", f);
      fclose(f);
    }
#endif
...
#if CONFIG_DSPL_RESIDUAL
        this_rd_stats.dist =
            dist_block_px_domain(cpi, x, plane, plane_bsize, block, blk_row,
...
#if CONFIG_DSPL_RESIDUAL
  recon_intra(cpi, x, plane, block, blk_row, blk_col, plane_bsize, tx_size,
              txb_ctx, skip_trellis, best_tx_type, dspl_type, 0, &rate_cost,
              best_eob);
#else
  recon_intra(cpi, x, plane, block, blk_row, blk_col, plane_bsize, tx_size,
...
#if CONFIG_DSPL_RESIDUAL && CONFIG_DSPL_STATS
  if (plane == 0 && is_inter) {
    block_dspl_txfm_stats[tx_size].ptrt_counts = 1;
    block_dspl_txfm_stats[tx_size].ptrt_avg_rate = best_rd_stats->rate;
    block_dspl_txfm_stats[tx_size].ptrt_avg_sig_rate =
        x->mode_costs.dspl_type_cost[dspl_type];
    block_dspl_txfm_stats[tx_size].ptrt_avg_dist = best_rd_stats->dist;
    block_dspl_txfm_stats[tx_size].ptrt_avg_sse = best_rd_stats->sse;
    block_dspl_txfm_stats[tx_size].ptrt_avg_src_var = x->source_variance;
    block_dspl_txfm_stats[tx_size].ptrt_avg_rdcost = best_rd;
    block_dspl_txfm_stats[tx_size].ptrt_avg_rdmult = x->rdmult;
    av1_accumulate_dspl_txfm_stats(&block_dspl_txfm_all_stats[tx_size],
                                   &block_dspl_txfm_stats[tx_size], 1);
  }
#endif
}
...
static AOM_INLINE void tx_type_rd(
    const AV1_COMP *cpi, MACROBLOCK *x, TX_SIZE tx_size, int blk_row,
    int blk_col, int block, int plane_bsize, TXB_CTX *txb_ctx,
    RD_STATS *rd_stats, FAST_TX_SEARCH_MODE ftxs_mode, int64_t ref_rdcost,
#if CONFIG_DSPL_RESIDUAL
    TXB_RD_INFO *rd_info_array, DSPL_TYPE dspl_type) {
#else
...
static AOM_INLINE void try_tx_block_no_split(
...
#if CONFIG_DSPL_RESIDUAL
    TxCandidateInfo *no_split, DSPL_TYPE dspl_type) {
#else
  ...
  tx_type_rd(cpi, x, tx_size, blk_row, blk_col, block, plane_bsize, &txb_ctx,
             ...
             rd_info_node != NULL ? rd_info_node->rd_info_array : NULL, dspl_type);
#else
  tx_type_rd(cpi, x, tx_size, blk_row, blk_col, block, plane_bsize, &txb_ctx,
             rd_stats, ftxs_mode, ref_best_rd,
...
static AOM_INLINE void try_tx_block_split(
    ...
    FAST_TX_SEARCH_MODE ftxs_mode, TXB_RD_INFO_NODE *rd_info_node,
#if CONFIG_DSPL_RESIDUAL
    RD_STATS *split_rd_stats, DSPL_TYPE dspl_type) {
#else
  ...
  for (int r = 0, blk_idx = 0; r < txb_height; r += sub_txb_height) {
    for (int c = 0; c < txb_width; c += sub_txb_width, ++blk_idx) {
      ...
      select_tx_block(
          ...
          (rd_info_node != NULL) ? rd_info_node->children[blk_idx] : NULL, dspl_type);
#else
      select_tx_block(
          cpi, x, offsetr, offsetc, block, sub_txs, depth + 1, plane_bsize, ta,
          tl, tx_above, tx_left, &this_rd_stats, no_split_rd / nblks,
...
static AOM_INLINE void select_tx_block(
...
#if CONFIG_DSPL_RESIDUAL && CONFIG_DSPL_STATS
  DSPL_STATS tmp_dspl_txfm_stats[TX_SIZES_ALL],
      no_split_dspl_txfm_stats[TX_SIZES_ALL],
      split_dspl_txfm_stats[TX_SIZES_ALL];
  av1_copy(tmp_dspl_txfm_stats, block_dspl_txfm_stats);
#endif
  ...
  if (try_no_split) {
    ...
    try_tx_block_no_split(cpi, x, blk_row, blk_col, block, tx_size, depth,
                          ...
                          ftxs_mode, rd_info_node, &no_split, dspl_type);
#if CONFIG_DSPL_STATS
    av1_copy(no_split_dspl_txfm_stats, block_dspl_txfm_stats);
#endif
#else
    try_tx_block_no_split(cpi, x, blk_row, blk_col, block, tx_size, depth,
                          plane_bsize, ta, tl, ctx, rd_stats, ref_best_rd,
  ...
  if (try_split) {
    ...
    try_tx_block_split(cpi, x, blk_row, blk_col, block, tx_size, depth,
                       ...
                       rd_info_node, &split_rd_stats, dspl_type);
#if CONFIG_DSPL_STATS
    av1_copy(split_dspl_txfm_stats, block_dspl_txfm_stats);
#endif
#else
    try_tx_block_split(cpi, x, blk_row, blk_col, block, tx_size, depth,
                       plane_bsize, ta, tl, tx_above, tx_left, ctx, no_split.rd,
...
#if CONFIG_DSPL_RESIDUAL && CONFIG_DSPL_STATS
  av1_copy(block_dspl_txfm_stats, tmp_dspl_txfm_stats);
#endif
  ...
  if (no_split.rd < split_rd_stats.rdcost) {
    ...
    set_blk_skip(x->txfm_search_info.blk_skip, 0, blk_row * bw + blk_col,
                 rd_stats->skip_txfm);
#if CONFIG_DSPL_RESIDUAL && CONFIG_DSPL_STATS
    av1_accumulate_dspl_txfm_stats(block_dspl_txfm_stats,
                                   no_split_dspl_txfm_stats, TX_SIZES_ALL);
#endif
  } else {
    *rd_stats = split_rd_stats;
...
#if CONFIG_DSPL_RESIDUAL && CONFIG_DSPL_STATS
    av1_accumulate_dspl_txfm_stats(block_dspl_txfm_stats, split_dspl_txfm_stats,
                                   TX_SIZES_ALL);
#endif
  }
}
...
static AOM_INLINE void choose_largest_tx_size(const AV1_COMP *const cpi,
...
#if CONFIG_DSPL_RESIDUAL
  mbmi->dspl_type = is_inter_block(mbmi) ? DSPL_BAD : DSPL_NONE;
#endif
  ...
  av1_txfm_rd_in_plane(x, cpi, rd_stats, ref_best_rd,
                       ...
                       FTXS_NONE, skip_trellis, skip_dspl_search);
#else
  av1_txfm_rd_in_plane(x, cpi, rd_stats, ref_best_rd,
                       AOMMIN(no_skip_txfm_rd, skip_txfm_rd), AOM_PLANE_Y, bs,
                       mbmi->tx_size, cpi->sf.rd_sf.use_fast_coef_costing,
...
static AOM_INLINE void choose_smallest_tx_size(const AV1_COMP *const cpi,
  ...
  const int skip_trellis = 0;
#if CONFIG_DSPL_RESIDUAL
  const int skip_dspl_search = 1;  // Can't downsample with 4x4 transforms
...
#if CONFIG_DSPL_RESIDUAL
    rd[depth] =
        av1_uniform_txfm_yrd(cpi, x, &this_rd_stats, ref_best_rd, bs, tx_size,
                             FTXS_NONE, skip_trellis, skip_dspl_search);
#else
...
#if CONFIG_DSPL_RESIDUAL
  DSPL_TYPE dspl_type = args->skip_dspl_search ? DSPL_NONE : DSPL_BAD;
#endif
...
int64_t av1_uniform_txfm_yrd(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  mbmi->tx_size = tx_size;
#if CONFIG_DSPL_RESIDUAL
  if (skip_dspl_search) mbmi->dspl_type = DSPL_NONE;
  av1_txfm_rd_in_plane(x, cpi, rd_stats, ref_best_rd,
...
#if CONFIG_DSPL_RESIDUAL
    tx_type_rd(cpi, x, tx_size, blk_row, blk_col, block, plane_bsize, &txb_ctx,
               rd_stats, ftxs_mode, ref_best_rd, NULL, DSPL_BAD);
#else
    tx_type_rd(cpi, x, tx_size, blk_row, blk_col, block, plane_bsize, &txb_ctx,
...
void av1_pick_recursive_tx_size_type_yrd(const AV1_COMP *cpi, MACROBLOCK *x,
...
#if CONFIG_DSPL_RESIDUAL
  int64_t best_rd = INT64_MAX;
  DSPL_TYPE best_dspl_type = DSPL_BAD;
  TX_SIZE best_tx_size = TX_INVALID;
  ...
  RD_STATS dspl_rd_stats[DSPL_END];
  for (DSPL_TYPE dspl_type = DSPL_NONE; dspl_type < DSPL_END; ++dspl_type)
    av1_invalid_rd_stats(&dspl_rd_stats[dspl_type]);
  DSPL_STATS dspl_txfm_stats[DSPL_END][TX_SIZES_ALL],
      dspl_txfm_all_stats[DSPL_END][TX_SIZES_ALL];
#endif

  const int16_t *orig_qtx = x->plane[0].quant_QTX;
  ...
  for (DSPL_TYPE dspl_type = DSPL_NONE; dspl_type < DSPL_END; ++dspl_type) {
    ...
    if (ref_best_rd != INT64_MAX && within_border &&
      ...
      found_rd_info[dspl_type] = find_tx_size_rd_records(
          x, bsize, dspl_type, matched_rd_info[dspl_type]);
    }
...
#if CONFIG_DSPL_ASSERT && !CONFIG_DSPL_STATS
    const int64_t dspl_rd2 = select_tx_size_and_type(
        cpi, x, &dummy_rd_stats, bsize, ref_best_rd,
        found_rd_info[dspl_type] ? matched_rd_info[dspl_type] : NULL,
        dspl_type);

    const int64_t dspl_rd3 = select_tx_size_and_type(
        cpi, x, &dummy_rd_stats, bsize, ref_best_rd,
        found_rd_info[dspl_type] ? matched_rd_info[dspl_type] : NULL,
        dspl_type);

    assert(dspl_rd == dspl_rd2 && dspl_rd2 == dspl_rd3);
#endif
...
#if CONFIG_DSPL_STATS
    dspl_rd_stats[dspl_type] = dummy_rd_stats;
    dspl_rd_stats[dspl_type].rdcost = dspl_rd;
    av1_copy(dspl_txfm_stats[dspl_type], block_dspl_txfm_stats);
    av1_copy(dspl_txfm_all_stats[dspl_type], block_dspl_txfm_all_stats);
#endif
  }

#if CONFIG_DSPL_STATS
  bool do_update_dspl_stats = true;
  for (DSPL_TYPE dspl_type = DSPL_NONE; dspl_type < DSPL_END; ++dspl_type) {
    if (dspl_type > DSPL_NONE &&
        (block_size_wide[bsize] < DSPL_MIN_PARTITION_SIDE ||
         block_size_high[bsize] < DSPL_MIN_PARTITION_SIDE))
      continue;
    if (dspl_rd_stats[dspl_type].rdcost == INT64_MAX) {
      do_update_dspl_stats = false;
      break;
    }
  }

  // Update stats
  if (do_update_dspl_stats && !rd_stats->skip_txfm) {
    for (DSPL_TYPE dspl_type = DSPL_NONE; dspl_type < DSPL_END; ++dspl_type) {
      if (dspl_type > DSPL_NONE &&
          (block_size_wide[bsize] < DSPL_MIN_PARTITION_SIDE ||
           block_size_high[bsize] < DSPL_MIN_PARTITION_SIDE))
      ...
      av1_accumulate_dspl_txfm_stats(
          agg_dspl_txfm_stats[qindex][bsize][best_dspl_type][dspl_type],
          dspl_txfm_stats[dspl_type], TX_SIZES_ALL);
      av1_accumulate_dspl_txfm_stats(
          agg_dspl_txfm_all_stats[qindex][bsize][best_dspl_type][dspl_type],
          dspl_txfm_all_stats[dspl_type], TX_SIZES_ALL);
    }
  }
#endif
  ...
  if (best_rd != INT64_MAX) {
    ...
    assert(rd == best_rd);
    assert(mbmi->dspl_type != DSPL_BAD && mbmi->tx_size != TX_INVALID);
    assert(mbmi->dspl_type == best_dspl_type && mbmi->tx_size == best_tx_size);
#endif
  }

  // Restore original quantizer
  av1_setup_dspl_quantizer(cpi, x, mbmi->segment_id, DSPL_NONE);
  assert(x->plane[0].quant_QTX == orig_qtx);
#else
  // Pre-compute residue hashes (transform block level) and find existing or
  // add new RD records to store and reuse rate and distortion values to speed
  // up TX size/type search.
...
int av1_txfm_uvrd(const AV1_COMP *const cpi, MACROBLOCK *x, RD_STATS *rd_stats,
  ...
  const int skip_trellis = 0;
#if CONFIG_DSPL_RESIDUAL
  // TODO(singhprakhar): consider adding dspl search for chroma blocks
  const int skip_dspl_search = 1;
#endif
...
#if CONFIG_DSPL_RESIDUAL
  args.skip_dspl_search = skip_dspl_search;
#endif
...
int av1_txfm_search(const AV1_COMP *cpi, MACROBLOCK *x, BLOCK_SIZE bsize,
...
#if CONFIG_DSPL_RESIDUAL && CONFIG_DSPL_ASSERT
  int choose_skip_txfm_y = rd_stats->skip_txfm;
#endif
  ...
  if (choose_skip_txfm) {
    ...
    mbmi->skip_txfm = 0;
#if CONFIG_DSPL_RESIDUAL && CONFIG_DSPL_ASSERT
    // TODO(singhprakhar): if we chose to skip transforming Y but are forced to
    // due to UV, we can still pick optimal dspl_type. However, currently,
    // dspl_type is only chosen if it got picked as non skip for Y plane.
    assert(IMPLIES(choose_skip_txfm_y, mbmi->dspl_type == DSPL_NONE));
#endif
  }
