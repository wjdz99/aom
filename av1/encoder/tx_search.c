...
static AOM_INLINE void try_tx_block_split(
  ...
  const int r_end = AOMMIN(txb_height, max_blocks_high - blk_row);
  const int c_end = AOMMIN(txb_width, max_blocks_wide - blk_col);
  for (int r = 0, blk_idx = 0; r < r_end; r += sub_txb_height) {
    const int offsetr = blk_row + r;
    for (int c = 0; c < c_end; c += sub_txb_width, ++blk_idx) {
      assert(blk_idx < 4);
