...
static uint16_t prune_operators(MACROBLOCK *x, BLOCK_SIZE bsize,
  ...
  fprintf(stderr, "(%d, %d) ", prune_thr_daf, prune_thr_i);
#endif
  ...
  fprintf(stderr, "(%d, %d) bitmask=", prune_thr_daf, prune_thr_i);
...
static uint16_t prune_laplacian(MACROBLOCK *x, BLOCK_SIZE bsize,
  ...
  for (int t = DCT_1D; t < TX_TYPES_1D; t++)
  ...
  // compute thresholds and bitmask
#if USE_2D_SCORES
  ...
  int prune_thr_1 = (int)((double)sum_xlx_daf * TAU_DAF);
  int prune_thr_2 = (int)((double)sum_xlx_dafi * TAU_I);
  for (int vtx = DCT_1D; vtx < TX_TYPES_1D; vtx++) {
    for (int htx = DCT_1D; htx < TX_TYPES_1D; htx++) {
