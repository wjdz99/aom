...
static int INLINE av1_check_newmv_joint_nonzero(const AV1_COMMON *cm,
                                                MACROBLOCK *const x) {
  MACROBLOCKD *xd = &x->e_mbd;
  MB_MODE_INFO *mbmi = xd->mi[0];
  const PREDICTION_MODE this_mode = mbmi->mode;
  MvSubpelPrecision mv_precision = mbmi->max_mv_precision;
#if CONFIG_FLEX_MVRES && !CONFIG_SB_FLEX_MVRES
  if (is_flex_mv_precision_active(cm, this_mode, mbmi->max_mv_precision))
    mv_precision = av1_get_mbmi_mv_precision(cm, mbmi);
#endif  // CONFIG_FLEX_MVRES && !CONFIG_SB_FLEX_MVRES
  if (this_mode == NEW_NEWMV) {
    int_mv ref_mv_0 = av1_get_ref_mv(x, 0);
    int_mv ref_mv_1 = av1_get_ref_mv(x, 1);
    lower_mv_precision(&ref_mv_0.as_mv, mv_precision);
    lower_mv_precision(&ref_mv_1.as_mv, mv_precision);
