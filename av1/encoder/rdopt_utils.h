...
static INLINE int check_txfm_eval(MACROBLOCK *const x, BLOCK_SIZE bsize,
  ...
  // (iii) when called from compound_type_rd since best skip rd is computed
  // after and skip rd is computed before interpolation filter search
  const int scale[4][2] = {
    { INT_MAX, INT_MAX }, { 3, 16 }, { 3, 15 }, { 2, 11 }
  };
  int aggr_factor =
      AOMMAX(1, ((MAXQ - x->qindex) * 2 + QINDEX_RANGE / 2) >> QINDEX_BITS);
  if (best_skip_rd >
      (x->source_variance << (num_pels_log2_lookup[bsize] + RDDIV_BITS)))
    aggr_factor *= scale[level][0];

  int64_t rd_thresh =
      (best_skip_rd == INT64_MAX)
          ? best_skip_rd
          : rdy ? (int64_t)(best_skip_rd * aggr_factor * scale[level][1] >> 3)
