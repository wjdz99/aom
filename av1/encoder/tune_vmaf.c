...
void av1_vmaf_neg_preprocessing(AV1_COMP *const cpi,
  ...
  const int layer_depth =
      AOMMIN(gf_group->layer_depth[cpi->cur_gf_index], MAX_ARF_LAYERS - 1);
...
static AOM_INLINE void get_neighbor_frames(const AV1_COMP *const cpi,
  ...
  const int src_index =
      cm->show_frame != 0 ? 0 : gf_group->arf_src_offset[cpi->cur_gf_index];
...
int av1_get_vmaf_base_qindex(const AV1_COMP *const cpi, int current_qindex) {
  ...
  if (cm->show_frame == 0) {
    const int src_index = gf_group->arf_src_offset[cpi->cur_gf_index];
