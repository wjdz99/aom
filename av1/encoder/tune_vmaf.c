...
static AOM_INLINE double cal_approx_vmaf(const AV1_COMP *const cpi,
  ...
  aom_calc_vmaf_at_index(vmaf_context, cpi->vmaf_info.vmaf_model, source,
...
static double find_best_frame_unsharp_amount_loop(
    const AV1_COMP *const cpi, VmafContext *vmaf_context, int *vmaf_cal_index,
  ...
  do {
    ...
    approx_vmaf = cal_approx_vmaf(cpi, vmaf_context, vmaf_cal_index,
...
static double find_best_frame_unsharp_amount(const AV1_COMP *const cpi,
  ...
  if (unsharp_amount_start <= step_size) {
    unsharp_amount = find_best_frame_unsharp_amount_loop(
        cpi, vmaf_context, &vmaf_cal_index, source, blurred, &sharpened, 0.0,
        baseline_variance, 0.0, step_size, max_loop_count, max_filter_amount);
  } else {
    double a0 = unsharp_amount_start - step_size, a1 = unsharp_amount_start;
    double v0, v1;
    unsharp(cpi, source, blurred, &sharpened, a0);
    v0 = cal_approx_vmaf(cpi, vmaf_context, &vmaf_cal_index, baseline_variance,
                         source, &sharpened);
    unsharp(cpi, source, blurred, &sharpened, a1);
    v1 = cal_approx_vmaf(cpi, vmaf_context, &vmaf_cal_index, baseline_variance,
                         source, &sharpened);
    if (fabs(v0 - v1) < 0.01) {
      unsharp_amount = a0;
    } else if (v0 > v1) {
      unsharp_amount = find_best_frame_unsharp_amount_loop(
          cpi, vmaf_context, &vmaf_cal_index, source, blurred, &sharpened, v0,
          baseline_variance, a0, -step_size, max_loop_count, max_filter_amount);
    } else {
      unsharp_amount = find_best_frame_unsharp_amount_loop(
          cpi, vmaf_context, &vmaf_cal_index, source, blurred, &sharpened, v1,
          baseline_variance, a1, step_size, max_loop_count, max_filter_amount);
    }
  }
...
void av1_set_mb_vmaf_rdmult_scaling(AV1_COMP *cpi) {
  ...
  for (int row = 0; row < num_rows; ++row) {
    for (int col = 0; col < num_cols; ++col) {
      ...
      double vmaf;
      aom_calc_vmaf_at_index(vmaf_context, cpi->vmaf_info.vmaf_model,
                             &resized_source, &recon, bit_depth, index, &vmaf);
...
static AOM_INLINE double cal_approx_score(
  ...
  aom_calc_vmaf_at_index(vmaf_context, cpi->vmaf_info.vmaf_model, src,
...
void av1_update_vmaf_curve(AV1_COMP *cpi) {
  ...
  aom_init_vmaf_context(
      &vmaf_context, cpi->vmaf_info.vmaf_model,
      cpi->oxcf.tune_cfg.tuning == AOM_TUNE_VMAF_NEG_MAX_GAIN);
  aom_calc_vmaf_at_index(vmaf_context, cpi->vmaf_info.vmaf_model, source, recon,
                         bit_depth, 0, &base_score);
