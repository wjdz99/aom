...
void unsharp_rect(const uint8_t *source, int source_stride,
...
double find_best_frame_unsharp_amount(const AV1_COMP *const cpi,
...
void vmaf_preprocessing(const AV1_COMP *const cpi,
  ...
  double src_vmaf;
  aom_calc_vmaf(cpi->oxcf.vmaf_model_path, source, source, &src_vmaf);
  const double best_frame_unsharp_amount =
      find_best_frame_unsharp_amount(cpi, source, &blurred, src_vmaf);
  ...
  double *best_unsharp_amounts =
      aom_malloc(sizeof(double) * num_cols * num_rows);
  memset(best_unsharp_amounts, 0, sizeof(double) * num_cols * num_rows);
