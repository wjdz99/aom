...
void av1_vmaf_blk_preprocessing(AV1_COMP *const cpi,
  ...
  aom_alloc_frame_buffer(
      ...
      cpi->oxcf.border_in_pixels, cm->features.byte_alignment);
  aom_alloc_frame_buffer(
      &source_extended, width, height, ss_x, ss_y, cm->seq_params.use_highbitdepth,
      cpi->oxcf.border_in_pixels, cm->features.byte_alignment);
  ...
  aom_alloc_frame_buffer(
      &source_block, block_w, block_h, ss_x, ss_y, cm->seq_params.use_highbitdepth,
      cpi->oxcf.border_in_pixels, cm->features.byte_alignment);
  aom_alloc_frame_buffer(
      &blurred_block, block_w, block_h, ss_x, ss_y, cm->seq_params.use_highbitdepth,
      cpi->oxcf.border_in_pixels, cm->features.byte_alignment);
...
void av1_set_mb_vmaf_rdmult_scaling(AV1_COMP *cpi) {
  ...
  aom_alloc_frame_buffer(
      &resized_source, y_width / resize_factor, y_height / resize_factor,
      ss_x, ss_y, cm->seq_params.use_highbitdepth, cpi->oxcf.border_in_pixels,
      cm->features.byte_alignment);
  ...
  YV12_BUFFER_CONFIG blurred;
  memset(&blurred, 0, sizeof(blurred));
  aom_alloc_frame_buffer(&blurred, resized_y_width, resized_y_height,
                         ss_x, ss_y, cm->seq_params.use_highbitdepth,
                         cpi->oxcf.border_in_pixels,
                         cm->features.byte_alignment);
  ...
  aom_alloc_frame_buffer(&recon, resized_y_width, resized_y_height,
                         ss_x, ss_y, cm->seq_params.use_highbitdepth,
                         cpi->oxcf.border_in_pixels,
                         cm->features.byte_alignment);
...
static double calc_vmaf_motion_score(const AV1_COMP *const cpi,
  ...
  aom_alloc_frame_buffer(
      &blurred_cur, y_width, y_height, ss_x, ss_y,
      cm->seq_params.use_highbitdepth, cpi->oxcf.border_in_pixels,
      cm->features.byte_alignment);
  aom_alloc_frame_buffer(
      &blurred_last, y_width, y_height, ss_x, ss_y,
      cm->seq_params.use_highbitdepth, cpi->oxcf.border_in_pixels,
      cm->features.byte_alignment);
  aom_alloc_frame_buffer(
      &blurred_next, y_width, y_height, ss_x, ss_y,
      cm->seq_params.use_highbitdepth, cpi->oxcf.border_in_pixels,
      cm->features.byte_alignment);
...
static double find_best_frame_unsharp_amount_neg(
  ...
  aom_alloc_frame_buffer(
      &recon_sharpened, width, height, ss_x, ss_y,
      cm->seq_params.use_highbitdepth, cpi->oxcf.border_in_pixels,
      cm->features.byte_alignment);
  aom_alloc_frame_buffer(
      &src_sharpened, width, height, ss_x, ss_y,
      cm->seq_params.use_highbitdepth, cpi->oxcf.border_in_pixels,
      cm->features.byte_alignment);
  aom_alloc_frame_buffer(
      &recon_blurred, width, height, ss_x, ss_y,
      cm->seq_params.use_highbitdepth, cpi->oxcf.border_in_pixels,
      cm->features.byte_alignment);
  aom_alloc_frame_buffer(
      &src_blurred, width, height, ss_x, ss_y,
      cm->seq_params.use_highbitdepth, cpi->oxcf.border_in_pixels,
      cm->features.byte_alignment);
