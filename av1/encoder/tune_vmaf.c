...
static int update_frame(float *ref_data, float *main_data, float *temp_data,
  ...
  if (row < 0 && col < 0) {
    ...
    const int prev_row = col - 1 < 0 ? row - 1 : row;
    if (prev_row >= 0) {
      const int prev_col = col - 1 < 0 ? num_cols - 1 : col - 1;
      const int row_offset = prev_row * block_h;
      const int col_offset = prev_col * block_w;
      ...
      unsharp_rect_float(ref_buf, stride, /*unused*/ (uint8_t *)ref_buf, stride,
                         main_buf, stride, block_width, block_height, 0.0f);
    }
...
void av1_set_mb_vmaf_rdmult_scaling(AV1_COMP *cpi) {
  ...
  gaussian_blur(cpi, &fake_recon, &blurred);
  ...
  for (int row = 0; row < num_rows; ++row) {
    for (int col = 0; col < num_cols; ++col) {
      ...
      const double mse = image_mse_c(y_buffer, y_stride, fake_recon.y_buffer,
