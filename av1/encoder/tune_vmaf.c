...
static double frame_average_variance(const AV1_COMP *const cpi,
  ...
  const ThreadData *const td = &cpi->td;
  ...
  for (row = 0; row < frame->y_height / block_h; ++row) {
    for (col = 0; col < frame->y_width / block_w; ++col) {
      ...
      if (use_hbd) {
        var +=
            av1_high_get_sby_perpixel_variance(cpi, &buf, block_size, xd->bd);
      } else {
...
static double find_best_frame_unsharp_amount(const AV1_COMP *const cpi,
  ...
  do {
    ...
    loop_count++;
    // printf("%f, %f, %f\n", unsharp_amount, approx_vmaf, best_vmaf);
...
void av1_vmaf_blk_preprocessing(const AV1_COMP *const cpi,
  ...
  YV12_BUFFER_CONFIG source_block;
  ...
  for (int row = 0; row < num_rows; ++row) {
    for (int col = 0; col < num_cols; ++col) {
      ...
      unsharp_rect(frame_src_buf, source->y_stride, frame_src_buf,
                   source->y_stride, source_block.y_buffer,
                   source_block.y_stride, block_width, block_height, 0.0);

      YV12_BUFFER_CONFIG blurred_block;
      memset(&blurred_block, 0, sizeof(blurred_block));
      aom_alloc_frame_buffer(&blurred_block, block_w, block_h, 1, 1,
                             cm->seq_params.use_highbitdepth,
                             cpi->oxcf.border_in_pixels, cm->byte_alignment);
      // Copy block from blurred frame.
      unsharp_rect(frame_blurred_buf, blurred.y_stride, frame_blurred_buf,
                   blurred.y_stride, blurred_block.y_buffer,
                   blurred_block.y_stride, block_width, block_height, 0.0);
