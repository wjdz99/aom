...
typedef struct REAL_TIME_SPEED_FEATURES {
  ...
  // A flag used in RTC case to control frame_refs_short_signaling. Note that the final decision is made
  // in check_frame_refs_short_signaling(). The flag can only be turned on
  // when res < 360p and speed >= 9, in which case only LAST and GOLDEN ref frames are used now.
