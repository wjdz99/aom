...
typedef struct INTRA_MODE_SPEED_FEATURES {
  ...
  // best possible Chroma to Luma (CFL) parameter. Then we do a full RD search
  // near the best possible parameter. The search range is set here. 1: Fastest
  // mode. 3: Default mode that provides good speedup witout losing compression
  // performance at speed 0. 33: Exhaustive rd search (33 == CFL_MAGS_SIZE).
  // This mode should only be used for debugging purpose.
