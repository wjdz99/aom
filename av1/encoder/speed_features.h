...
/*!\enum INTERNAL_COST_UPD_TYPE
 * \brief This enum decides internally how often to update the entropy costs
 */
typedef enum {
  INT_COST_UPD_OFF,       /*!< Turn off cost updates. */
  INT_COST_UPD_SBROW_SET, /*!< Update every set of rows of height 256 pixels. */
  INT_COST_UPD_SBROW,     /*!< Update every sb rows inside a tile. */
  INT_COST_UPD_SB,        /*!< Update every sb. */
...
typedef struct INTER_MODE_SPEED_FEATURES {
  ...
  INTERNAL_COST_UPD_TYPE mv_cost_upd_level;
...
typedef struct INTRA_MODE_SPEED_FEATURES {
  ...
  INTERNAL_COST_UPD_TYPE dv_cost_upd_level;
