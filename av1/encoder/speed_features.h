...
typedef struct INTER_MODE_SPEED_FEATURES {
  ...
  // Prune reference frames based on temporal distance and pred MV SAD.
  // 0 no pruning
  // 1 prune reference frames which are not at a nearest distance from the
  //   current frame in past/future direction and with large pred_mv_sad.
