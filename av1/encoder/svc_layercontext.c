...
void av1_svc_update_buffer_slot_refreshed(AV1_COMP *const cpi) {
  SVC *const svc = &cpi->svc;
  RTC_REF *const rtc_ref = &cpi->ppi->rtc_ref;
  const unsigned int current_frame = (cpi->ppi->use_svc) ?
  ...
  if (cpi->common.current_frame.frame_type == KEY_FRAME) {
    ...
    for (unsigned int i = 0; i < INTER_REFS_PER_FRAME; i++) {
      ...
      if (cpi->ppi->rtc_ref.refresh[ref_frame_map_idx]) {
        ...
        rtc_ref->buffer_spatial_layer[ref_frame_map_idx] = svc->spatial_layer_id;
      }
    }
  }
}
...
int av1_svc_get_min_ref_dist(const AV1_COMP *cpi) {
  ...
  const unsigned int current_frame_num = (cpi->ppi->use_svc) ?
                           cpi->svc.current_superframe :
                           cpi->common.current_frame.frame_number;
  for (unsigned int i = 0; i < INTER_REFS_PER_FRAME; i++) {
...
void av1_svc_set_reference_was_previous(AV1_COMP *cpi) {
  ...
  const unsigned int current_frame = (cpi->ppi->use_svc) ?
                                      cpi->svc.current_superframe :
                                      cpi->common.current_frame.frame_number;
  rtc_ref->reference_was_previous_frame = 1;
  if (current_frame > 0) {
    rtc_ref->reference_was_previous_frame = 0;
    for (unsigned int i = 0; i < INTER_REFS_PER_FRAME; i++) {
      if (rtc_ref->reference[i]) {
        int ref_frame_map_idx = rtc_ref->ref_idx[i];
        if (rtc_ref->buffer_time_index[ref_frame_map_idx] ==
            current_frame - 1)
          rtc_ref->reference_was_previous_frame = 1;
      }
    }
