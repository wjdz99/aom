...
void av1_svc_set_last_source(AV1_COMP *const cpi, EncodeFrameInput *frame_input,
  ...
  if (cpi->svc.number_spatial_layers == 1) {
    // For temporal layers only: if the LAST reference (index 0) is not
    // the previous frame set the last_source to the source corresponding
  ...
  } else if (cpi->svc.number_spatial_layers > 1) {
    // For spatial layers: always set the last_source to the source of the
    // last TL0.
