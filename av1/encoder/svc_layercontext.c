...
void av1_set_svc_fixed_mode(AV1_COMP *const cpi) {
  ...
  if (svc->spatial_layer_id > 0 && (!svc->ksvc_fixed_mode ||
  ...
  if (svc->temporal_layer_id == 0) {
    ...
    if (svc->spatial_layer_id == 0) {
      ...
      // Set buffer_idx for LAST to slot 1, GOLDEN (and all other refs) to slot 0.
      // Update slot 1 (LAST).
      for (i = 0; i < INTER_REFS_PER_FRAME; i++) svc->ref_idx[i] = 0;
      svc->ref_idx[SVC_LAST_FRAME] = 1;
      svc->refresh[1] = 1;
    } else if (svc->spatial_layer_id == 2) {
      // Set buffer_idx for LAST to slot 2, GOLDEN (and all other refs) to slot 1.
      // Update slot 2 (LAST).
      for (i = 0; i < INTER_REFS_PER_FRAME; i++) svc->ref_idx[i] = 1;
      svc->ref_idx[SVC_LAST_FRAME] = 2;
      svc->refresh[2] = 1;
    }
  } else if (svc->temporal_layer_id == 2 &&
             (superframe_cnt - 1) % 4 == 0) {
