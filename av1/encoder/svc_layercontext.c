...
void av1_svc_set_last_source(AV1_COMP *const cpi, EncodeFrameInput *frame_input,
  ...
  if (cpi->svc.spatial_layer_id == 0) {
    ...
    // For spatial enhancement layers: the previous source (prev_source) corresponds
    // to the lower spatial layer (which is the same source so we can't use that),
    // so always set the last_source to the source of the last TL0.
    if (cpi->svc.current_superframe > 0)
      frame_input->last_source = &cpi->svc.source_last_TL0;
    else
