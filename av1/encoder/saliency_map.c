...
static INLINE void upscale_map(const double *input, int in_level, int out_level,
  ...
  for (int level = in_level; level > out_level; level--) {
    ...
    for (int i = 0; i < h_upscale; ++i) {
      for (int j = 0; j < w_upscale; ++j) {
        ii = clamp((i >> 1), 0, cur_height - 1);
        jj = clamp((j >> 1), 0, cur_width - 1);
...
static void center_surround_diff(const double *input[9], int height[9],
                                 int width[9],
  ...
  for (int k = 2; k < 5; k++) {
    ...
    upscale_map(input[k + 3], k + 3, k, height, width, output[j]->buf);
    ...
    upscale_map(input[k + 4], k + 4, k, height, width, output[j + 1]->buf);
...
static void center_surround_diff_rgb(const double *input_1[9],
  ...
  for (int k = 2; k < 5; k++) {
    ...
    upscale_map(input_2[k + 3], k + 3, k, height, width, output[j]->buf);
    ...
    upscale_map(input_2[k + 4], k + 4, k, height, width, output[j + 1]->buf);
...
// differences.
static void gaussian_pyramid(const double *src, int width, int height,
  ...
  center_surround_diff((const double **)gaussian_map, pyr_height, pyr_width,
                       dst);
...
static void get_feature_map_rgb(double *cr, double *cg, double *cb, int width,
  ...
  gaussian_pyramid_rgb(rg_mat, gr_mat, width, height, rg_map);
  gaussian_pyramid_rgb(by_mat, yb_mat, width, height, by_map);
...
static void get_feature_map_orientation(const double *intensity, int width,
                                        int height,
  ...
  for (int j = 0; j < 4; j++) {
    center_surround_diff((const double **)tempGaborOutput[j], pyr_height,
                         pyr_width, tmp[j]);
  }
...
static void normalize_fm(saliency_feature_map *input[6], int width, int height,
...
static saliency_feature_map *normalized_map(saliency_feature_map *input[6],
  ...
  normalize_fm(input, width, height, num_fm, n_input);
