...
static AOM_INLINE int inter_tx_partition_cost(const MACROBLOCK *const x,
  ...
  if (allow_horz && allow_vert) {
    ...
    cost += x->mode_costs
      .inter_4way_txfm_partition_cost[is_rect][split4_ctx_0][split4_partition];
    cost += (((split4_partition == TX_PARTITION_VERT) && allow_vert2) || 
    ...
    cost += 1 + (has_first_split && (allow_horz2 || allow_vert2));
  } else {
