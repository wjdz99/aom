...
int ColorCountComp(const void *a, const void *b) {
  const struct ColorCount *color_count1 = (const struct ColorCount *)a;
  const struct ColorCount *color_count2 = (const struct ColorCount *)b;
  return color_count1->count > color_count2->count;
}

static void FindTopColors(const int *const count_buf, int bit_depth,
                          int n_colors, int *top_colors) {
  // Top color array, serving as a priority queue if more than n_colors are
  // found.
  struct ColorCount top_color_counts[PALETTE_MAX_SIZE];
  int n_color_count = 0;
  for (int i = 0; i < (1 << bit_depth); ++i) {
    if (count_buf[i] > 0) {
      if (n_color_count < n_colors) {
        ...
        ++n_color_count;
        if (n_color_count == n_colors) {
          qsort(top_color_counts, n_colors, sizeof(struct ColorCount),
                ColorCountComp);
        }
      } else {
        ...
        if (count_buf[i] > top_color_counts[n_colors - 1].count) {
          ...
          memmove(top_color_counts + j + 1, top_color_counts + j,
                  (n_colors - j - 1) * sizeof(struct ColorCount));
...
void av1_rd_pick_palette_intra_sby(
  ...
  if (colors_threshold > 1 && colors_threshold <= 64) {
    ...
    FindTopColors(count_buf, bit_depth, AOMMIN(colors, PALETTE_MAX_SIZE),
                  top_colors);
