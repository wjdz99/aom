...
int av1_index_color_cache(const unsigned int *color_cache, int n_cache,
  ...
  if (n_cache <= 0) {
    int i;
    for (i = 0; i < n_colors; ++i) out_cache_colors[i] = colors_in[i];
    return n_colors;
  }
  ...
  int i, j;
  for (i = 0; i < n_cache && n_in_cache < n_colors; ++i) {
