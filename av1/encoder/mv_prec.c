...
static AOM_INLINE void keep_one_mv_stat(MV_STATS *mv_stats, const MV *ref_mv,
  ...
  const MV lp_diff =
      use_hp ? (MV){ (diff.row / 2) * 2, (diff.col / 2) * 2 } : diff;
...
static AOM_INLINE int get_smart_mv_prec(AV1_COMP *cpi, const MV_STATS *mv_stats,
  ...
  float features[MV_PREC_FEATURE_SIZE] = {
    ...
    mv_stats->inter_count,
    mv_stats->intra_count,
    mv_stats->default_mvs,
    mv_stats->mv_joint_count[0],
    mv_stats->mv_joint_count[1],
    mv_stats->mv_joint_count[2],
    mv_stats->mv_joint_count[3],
    mv_stats->last_bit_zero,
    mv_stats->last_bit_nonzero,
  ...
  if (is_720p_or_larger) {
    ml_mean = av1_mv_prec_mean_hdres;
    ml_std = av1_mv_prec_std_hdres;
    nn_config = &av1_mv_prec_dnn_config_hdres;
  } else if (is_480p_or_larger) {
    ml_mean = av1_mv_prec_mean_midres;
    ml_std = av1_mv_prec_std_midres;
    nn_config = &av1_mv_prec_dnn_config_midres;
  } else {
    ml_mean = av1_mv_prec_mean_lowres;
    ml_std = av1_mv_prec_std_lowres;
    nn_config = &av1_mv_prec_dnn_config_lowres;
  }
...
void av1_pick_and_set_high_precision_mv(AV1_COMP *cpi, int qindex) {
  ...
  if (cpi->sf.high_precision_mv_usage == QTR_ONLY) {
    use_hp = 0;
           ...
           frame_allows_smart_mv(cpi) && cpi->mv_stats.valid) {
