...
static AOM_INLINE int keep_one_comp_stat(MV_STATS *mv_stats, int comp_val,
  ...
  const int use_hp =
      cpi->common.features.fr_mv_precision > MV_SUBPEL_QTR_PRECISION;
  int r_idx = 0;
...
void av1_pick_and_set_high_precision_mv(AV1_COMP *cpi, int qindex) {
  ...
  MvSubpelPrecision prec = MV_SUBPEL_QTR_PRECISION + use_hp;
...
#if CONFIG_FLEX_MVRES
  cpi->common.features.use_sb_mv_precision = 1;
