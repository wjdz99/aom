...
typedef struct {
  ...
  int nhfb;
  /*!
   * Pointer to the mean squared error between the CDEF filtered block
   * and the source block. mse[i][j][k] stores the MSE of the ith plane
   * (i=0 corresponds to Y-plane, i=1 corresponds to U and V planes),
   * jth block and kth strength index.
   */
 ...
 * \param[in]      xd               Pointer to common current coding block
 *                                  structure
 * \param[in]      cdef_search_ctx  Pointer to the structure containing
...
void av1_cdef_search(const YV12_BUFFER_CONFIG *frame,
                     ...
                     MACROBLOCKD *xd, CdefSearchCtx *cdef_search_ctx,
