...
typedef struct {
  ...
  int nhfb;
  /*!
   * Stores MSE corresponds to each block and strength. mse[i]
   * indicates the MSE for ith plane (i=0 corresponds to Y-plane,
   * i=1 corresponds to U and V planes).
   */
...
//   respectively.
static INLINE int cdef_sb_skip(CdefSearchCtx *cdef_search_ctx, int fbr,
                               int fbc) {
