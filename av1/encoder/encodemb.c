    ...
    if (x == 0) {
      ...
      if (i < eob - 1) {
        ...
        if (! disjoint_token_cache_upd) {
          ...
          const int next_bits0 = av1_get_coeff_token_cost(
              token_next, eob_val_next, 0, head_token_costs[band_next][ctx_next0],
              tail_token_costs[band_next][ctx_next0]);

          token_cache[rc] = av1_pt_energy_class[t1];
          const int ctx_next1 = get_coef_context(nb, token_cache, i + 1);
          const int next_bits1 = av1_get_coeff_token_cost(
              token_next, eob_val_next, 0, head_token_costs[band_next][ctx_next1],
