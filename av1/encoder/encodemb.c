...
static int cfl_compute_alpha_ind(MACROBLOCK *const x, const CFL_CTX *const cfl,
  ...
  for (int c = 1; c < CFL_ALPHABET_SIZE; c++) {
    ...
    for (int sign_u = cfl_alpha_codes[c][CFL_PRED_U] == 0.0;
         sign_u < CFL_SIGNS; sign_u++) {
...
#error "CfL rate estimation requires ec_adapt."
      ...
      for (int c = 0; c < CFL_ALPHABET_SIZE; c++) {
        ...
        int prob_den = ec_ctx->cfl_alpha_cdf[CFL_ALPHABET_SIZE - 1];
      ...
      mbmi->cfl_alpha_ind =
          cfl_compute_alpha_ind(x, cfl, plane_bsize, cfl_costs, mbmi->cfl_alpha_signs);
    }
  }
