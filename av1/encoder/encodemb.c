...
static inline void cfl_update_costs(CFL_CTX *cfl, FRAME_CONTEXT *ec_ctx) {
  ...
  aom_cdf_prob p15 = AOM_ICDF(ec_ctx->cfl_alpha_cdf[0]);
  cfl->costs[0] = av1_cost_symbol(p15);

  aom_cdf_prob prev_cdf = AOM_ICDF(ec_ctx->cfl_alpha_cdf[0]);
  for (int c = 1; c < CFL_ALPHABET_SIZE; c++) {
    int sign_bit_cost = (cfl_alpha_codes[c][CFL_PRED_U] != 0) +
                        (cfl_alpha_codes[c][CFL_PRED_V] != 0);
    p15 = AOM_ICDF(ec_ctx->cfl_alpha_cdf[c]) - prev_cdf;
    prev_cdf = AOM_ICDF(ec_ctx->cfl_alpha_cdf[c]);
    cfl->costs[c] = av1_cost_symbol(p15) + av1_cost_literal(sign_bit_cost);
  }
}
