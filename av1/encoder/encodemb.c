...
int av1_optimize_b(const AV1_COMMON *cm, MACROBLOCK *mb, int plane, int block,
  ...
  TX_TYPE tx_type = get_tx_type(plane_type, xd, block_4x4, tx_size);
...
static void encode_block(int plane, int block, int blk_row, int blk_col,
  ...
  *a = *l = !x->pvq_skip[plane];
  ...
  inv_txfm_param.tx_type = get_tx_type(pd->plane_type, xd, block_4x4, tx_size);
...
void av1_encode_block_intra(int plane, int block, int blk_row, int blk_col,
  ...
  PLANE_TYPE plane_type = (plane == 0) ? PLANE_TYPE_Y : PLANE_TYPE_UV;
  const int block_4x4 = (tx_size == TX_4X8 && block == 2) ? 1 : block;
  const TX_TYPE tx_type = get_tx_type(plane_type, xd, block_4x4, tx_size);
  ...
  src_diff =
      &p->src_diff[(blk_row * diff_stride + blk_col) << tx_size_wide_log2[0]];
  mode = (plane == 0) ? get_y_mode(xd->mi[0], block_4x4) : mbmi->uv_mode;
