...
#if CONFIG_COLLECT_PRED_RES
    if (dry_run == OUTPUT_ENABLED && mbmi->tx_size == TX_32X32 &&
        plane_bsize == BLOCK_32X32) {
      ...
      const uint8_t frameid = cm->cur_frame->order_hint;
      const uint8_t pl = plane;
      const uint8_t mirow = mi_row;
      const uint8_t micol = mi_col;
      ...
      const uint8_t qindex = cm->base_qindex;
      ...
      if (plane == 0) {
        fprintf(ft,
                "[INTER BLOCK] frame %d mi_row %d mi_col %d\n"
                "mode %d ref0_order %d ref1_order %d mv0 (%d,%d) mv1 (%d,%d)\n"
                "base_qindex %d motion_mode %d comp_type %d\n"
#if COLLECT_INTER_TXTYPE
                "block_size %dx%d tx_size %dx%d tx_type:\n\n",
#else
                "block_size %dx%d tx_size %dx%d\n",
#endif
                frameid, mi_row, mi_col, mbmi->mode, i0, i1, mv0_row, mv0_col,
                mv1_row, mv1_col, cm->base_qindex, mm, ct, cols, rows, tw, th);
...
#if CONFIG_COLLECT_PRED_RES
    if (args->dry_run == OUTPUT_ENABLED && tx_size == TX_32X32 &&
        plane_bsize == BLOCK_32X32) {
      ...
      const uint8_t frameid = cm->cur_frame->order_hint;
      const uint8_t mirow = xd->mi_row;
      const uint8_t micol = xd->mi_col;
