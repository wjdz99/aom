...
/* ===================================================== */
  ...
  int64_t error_array[1025];
  memset((void *)error_array, 0, sizeof(int64_t)*1025);
  int blk_size = 16 << (tx_size << 1);
  {
    int64_t accu_error = 0;
    error_array[blk_size] = 0;
    // this loop can start at eob without affecting RD cost comparison
    for (i = (eob < blk_size)? eob: eob-1; i >= 0; i--)
    {
      const int rc= scan[i];
      int dx = (coeff[rc]) * mul;
#if CONFIG_AOM_HIGHBITDEPTH
...
#endif  // CONFIG_AOM_HIGHBITDEPTH
      accu_error += (int64_t)dx * dx;
      error_array[i] = accu_error;
      assert(error_array[i]>= error_array[i+1]);

      if (i < eob)
      {
        //rate cost of the extra bits (excluding the cost of the token) at scan position i.
        tokens[i][0].rate = av1_get_token_cost(qcoeff[rc], &t0, cat6_bits);
        tokens[i][0].token = t0;
        tokens[i][0].qc = qcoeff[rc];
  ...
  int64_t best_block_rd_cost = RDCOST(rdmult, rddiv, rate0, error_array[0]);
  ...
  for (i = 0; i < eob; i++)
  {
    ...
    for (j = 0; j < max_j; j++)
    {
      ...
      /* Determine the cost of choosin cx[j] */
      ...
      if (cx[j] != 0)
      {
        ...
        eob_cost1 =  RDCOST(rdmult, rddiv, (accu_rate+rate0),
                            (accu_error+ dist[j]+ error_array[i+1]) );
        //eob_cost1 = (rate0+accu_rate)*rdmult*1./512 +
        //             (error_array[i+1]+dist[j]+accu_error)*1.*(1<<rddiv);
    ...
    tokens[i][0].rate = av1_get_token_cost(cx[best_j], &t0, cat6_bits);
    tokens[i][0].token = t0;
...
// to an odd number.
    ...
    accu_error += dist[best_j];

    tokens[i][1].rate = av1_get_token_cost(cx[best_eob_j], &t0, cat6_bits);
    tokens[i][1].token = t0;
