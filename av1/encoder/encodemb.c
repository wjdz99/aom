...
static int optimize_b_greedy(const AV1_COMMON *cm, MACROBLOCK *mb, int plane,
  ...
  aom_cdf_prob(*head_token_cdfs)[COEFF_CONTEXTS][CDF_SIZE(ENTROPY_TOKENS)] =
      xd->tile_ctx->coef_head_cdfs[txsize_sqr_map[tx_size]][plane_type][ref];
  aom_cdf_prob(*tail_token_cdfs)[COEFF_CONTEXTS][CDF_SIZE(ENTROPY_TOKENS)] =
      xd->tile_ctx->coef_tail_cdfs[txsize_sqr_map[tx_size]][plane_type][ref];
  ...
  rate0 = av1_get_coeff_token_cost(
      BLOCK_Z_TOKEN, 1, 1, head_token_cdfs[0][ctx0], tail_token_cdfs[0][ctx0]);
  ...
  for (i = 0; i < eob; i++) {
    ...
    if (x == 0) {
      ...
      rate0 = av1_get_coeff_token_cost(token, eob_val, is_first,
                                       head_token_cdfs[band_cur][ctx_cur],
                                       tail_token_cdfs[band_cur][ctx_cur]);
      accu_rate += rate0;
      ...
      if (i < default_eob - 1) {
        ...
        if (token_next != EOB_TOKEN) {
          next_bits0 = av1_get_coeff_token_cost(
              token_next, eob_val_next, 0, head_token_cdfs[band_next][ctx_next],
              tail_token_cdfs[band_next][ctx_next]);
        }
      ...
      next_eob_bits0 = av1_get_coeff_token_cost(
          t0, eob_v, is_first, head_token_cdfs[band_cur][ctx_cur],
          tail_token_cdfs[band_cur][ctx_cur]);
      eob_cost0 =
      ...
      if (x_a != 0) {
        next_eob_bits1 = av1_get_coeff_token_cost(
          t1, eob_v, is_first, head_token_cdfs[band_cur][ctx_cur],
          tail_token_cdfs[band_cur][ctx_cur]);
