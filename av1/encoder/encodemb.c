...
void av1_vec_quant(MACROBLOCK *x, int plane, int blk_row, int blk_col,
  ...
  int16_t qgain = vq_gain_sqrt_quant(gain_sq);
  ...
  if (qgain == 0) {
    ...
    for (int cw = 0; cw < NUM_CODEWORDS; ++cw) {
      ...
      //   src_diff = gain * x
      //   codeword4x4 = 2^8 * y
      //   corr = dot(codeword4x4, src_diff)
      //        = 2^8 * g * dot(x, y)
      for (int r = 0; r < txb_h; ++r)
        for (int c = 0; c < txb_w; ++c)
          corr +=
              codeword4x4[cw][r * txb_w + c] * src_diff[r * diff_stride + c];
  ...
  // Say g is the gain and Q(g) is the quantized gain, then
  // sse = || g*x - Q(g)*y ||^2
