...
int av1_optimize_b(const struct AV1_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  return av1_optimize_txb_new(cpi, x, plane, block, tx_size, dspl_type, tx_type,
...
void av1_xform(MACROBLOCK *x, int plane, int block, int blk_row, int blk_col,
  ...
  assert(DSPL_NO_TXFM <= txfm_param->dspl_type &&
         txfm_param->dspl_type < DSPL_END);

  assert(IMPLIES(plane == 0, txfm_param->dspl_type == DSPL_NO_TXFM ||
                                 (tx_size_wide[tx_size] >= 8 &&
  ...
  if (plane > 0 || txfm_param->dspl_type == DSPL_NO_TXFM)
...
void av1_quant(MACROBLOCK *x, int plane, int block, TxfmParam *txfm_param,
               QUANT_PARAM *qparam) {
  assert(txfm_param->tx_size == qparam->tx_size);
  assert(txfm_param->tx_size != TX_INVALID);
  assert(txfm_param->dspl_type == qparam->dspl_type);

  assert(DSPL_NO_TXFM <= txfm_param->dspl_type &&
         txfm_param->dspl_type < DSPL_END);

  assert(IMPLIES(plane == 0, txfm_param->dspl_type == DSPL_NO_TXFM ||
                                 (tx_size_wide[txfm_param->tx_size] >= 8 &&
                                  tx_size_high[txfm_param->tx_size] >= 8)));
...
static void encode_block(int plane, int block, int blk_row, int blk_col,
  ...
  if (!is_blk_skip(x->txfm_search_info.blk_skip, plane,
    ...
    if (quant_param.use_optimize_b && do_trellis) {
      ...
      av1_optimize_b(args->cpi, x, plane, block, tx_size, mbmi->dspl_type,
                     tx_type, &txb_ctx, args->cpi->sf.rd_sf.trellis_eob_fast,
                     &dummy_rate_cost);
    }
...
static void encode_block_pass1(int plane, int block, int blk_row, int blk_col,
  ...
  // TODO(singhprakhar): check if we need to consider DSPL_TXFM here
...
void av1_encode_sb(const struct AV1_COMP *cpi, MACROBLOCK *x, BLOCK_SIZE bsize,
  ...
  int original_skip_txfm = mbmi->skip_txfm;
  mbmi->skip_txfm = 1;
  ...
  // TODO(singhprakhar): This is a temporary fix for skip being overwritten
  // invertigate to see why the assert fails without this
  if (original_skip_txfm == 0 && mbmi->skip_txfm == 1)
...
void av1_encode_block_intra(int plane, int block, int blk_row, int blk_col,
  ...
  assert(args->dspl_type == DSPL_NO_TXFM);
  ...
  if (plane == 0 && is_blk_skip(x->txfm_search_info.blk_skip, plane,
    ...
    *eob = 0;
    ...
    av1_setup_xform(cm, x, tx_size, tx_type, dspl_type, &txfm_param);
    av1_setup_quant(tx_size, use_trellis, quant_idx, cpi->oxcf.quant_b_adapt,
                    dspl_type, &quant_param);
    av1_setup_qmatrix(&cm->quant_params, xd, plane, tx_size, tx_type,
                      &quant_param);
    ...
    if (quant_param.use_optimize_b && do_trellis) {
      TXB_CTX txb_ctx;
      get_txb_ctx(plane_bsize, tx_size, plane, a, l, &txb_ctx);
      av1_optimize_b(args->cpi, x, plane, block, tx_size, dspl_type, tx_type,
                     &txb_ctx, args->cpi->sf.rd_sf.trellis_eob_fast,
                     &dummy_rate_cost);
    }
    if (do_dropout) {
      av1_dropout_qcoeff(x, plane, block, tx_size, tx_type,
...
void av1_encode_intra_block_plane(const struct AV1_COMP *cpi, MACROBLOCK *x,
  ...
  struct encode_b_args arg = {
    cpi, x,  NULL,    &(xd->mi[0]->skip_txfm), DSPL_NO_TXFM,
