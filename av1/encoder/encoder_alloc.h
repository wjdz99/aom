...
static AOM_INLINE void setup_tpl_buffers(AV1_COMP *const cpi) {
  AV1_COMMON *const cm = &cpi->common;
  ...
  // If lag_in_frames <= 1, TPL module is not invoked.
  if (cpi->oxcf.gf_cfg.lag_in_frames <= 1) return;

  // TODO(any): Explore the allocation of tpl buffers based on lag in frames.
