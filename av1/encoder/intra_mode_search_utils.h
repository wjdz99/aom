 ...
 * \callergraph
 */
static void av1_quick_txfm(int use_hardamard, TX_SIZE tx_size, int is_hbd,
                           int bd, const int16_t *src_diff,
...
static int64_t intra_model_rd(const AV1_COMMON *cm, MACROBLOCK *const x,
  ...
  for (row = 0; row < max_blocks_high; row += stepr) {
    for (col = 0; col < max_blocks_wide; col += stepc) {
      ...
      av1_quick_txfm(use_hardamard, tx_size, is_cur_buf_hbd(xd), xd->bd,
