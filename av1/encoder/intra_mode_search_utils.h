...
static AOM_INLINE void generate_hog_superblock(MACROBLOCK *const x,
                                               BLOCK_SIZE sb_size,
                                               PLANE_TYPE plane) {
  pixel_level_gradient_info *hog_info = x->hog_info + plane * MAX_SB_SQUARE;
...
static AOM_INLINE void get_generated_hog_data(const MACROBLOCK *x, int rows,
  ...
  const pixel_level_gradient_info *hog_info_block =
  ...
  for (int r = 1; r < rows - 1; ++r) {
    for (int c = 1; c < cols - 1; ++c) {
      ...
      if (is_dx_zero) {
        ...
        const int8_t idx = hog_info_block[c + r * hog_stride].hist_bin_idx;
        assert(idx >= 0 && idx < BINS);
