...
static void av1_quick_txfm(int use_hardamard, TX_SIZE tx_size,
                           const int16_t *src_diff, ptrdiff_t src_stride,
                           tran_low_t *coeff) {
  if (use_hardamard) {
    switch (tx_size) {
    ...
    // TODO(angiebird): don't we need to support high bitdepth here?
...
static int64_t intra_model_rd(const AV1_COMMON *cm, MACROBLOCK *const x,
                              ...
                              TX_SIZE tx_size, int use_hardamard) {
  ...
  for (row = 0; row < max_blocks_high; row += stepr) {
    for (col = 0; col < max_blocks_wide; col += stepc) {
      ...
      // Here we use p->src_diff and p->coeff as temporary buffer for prediction
      // residue and transform coefficients. The buffers are only used in this
      // for loop, therefore we don't need to properly add offset to the
      // buffers.
      av1_subtract_block(
      ...
      av1_quick_txfm(use_hardamard, tx_size, p->src_diff,
...
static AOM_INLINE int model_intra_yrd_and_prune(const AV1_COMP *const cpi,
  ...
  const int use_hardamard = 1;
  const int64_t this_model_rd =
      intra_model_rd(cm, x, plane, bsize, tx_size, use_hardamard);
