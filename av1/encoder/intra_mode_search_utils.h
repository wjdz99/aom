...
// Caching of source variance for 4x4 sub-blocks is allowed only if any of the
// following conditions are satisfied:
// (1) Encoding mode is ALLINTRA with the sf partition_search_type is set to
//     SEARCH_PARTITION
// (2) Encoding mode is ALLINTRA with sf partition_search_type set to
//     VAR_BASED_PARTITION and INTRA_RD_VAR_THRESH > 0
//
// Caching of source variance for 4x4 sub-blocks is not required in the
// following cases:
// (1) The current encoding mode is not ALLINTRA
// (2) When sf partition_search_type is VAR_BASED_PARTITION with
//     INTRA_RD_VAR_THRESH <= 0
// (3) When sf use_nonrd_pickmode is set as 1 with hybird_intra_pick_mode set to
//     0
static AOM_INLINE bool is_src_var_for_4x4_sub_blks_caching_enabled(
    const AV1_COMP *const cpi) {
  const SPEED_FEATURES *const sf = &cpi->sf;
...
// Initialize the members of Blk4x4VarInfo structure to -1 at the start
// of every superblock.
static AOM_INLINE void init_src_var_info_of_4x4_sub_blks(
    const AV1_COMP *const cpi, Blk4x4VarInfo *src_var_info_of_4x4_sub_blks,
  ...
  for (int i = 0; i < mi_count_in_sb; i++) {
    src_var_info_of_4x4_sub_blks[i].var = -1;
    src_var_info_of_4x4_sub_blks[i].log_var = -1.0;
  }
}
