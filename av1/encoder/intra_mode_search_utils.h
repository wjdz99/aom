...
static AOM_INLINE void compute_gradient_info_sb(MACROBLOCK *const x,
                                                BLOCK_SIZE sb_size,
                                                PLANE_TYPE plane) {
  pixel_level_gradient_info *grad_info_sb =
  ...
  for (int r = 1; r < sb_height - 1; ++r) {
    for (int c = 1; c < sb_width - 1; ++c) {
      ...
      grad_info_sb[r * sb_width + c].abs_dx_abs_dy_sum = abs(dx) + abs(dy);
...
static AOM_INLINE void produce_gradients_for_sb(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  // In case of inter frames or when partition_search_type is not
  // SEARCH_PARTITION, SB level caching of gradient data may not help in speedup
  // due to early intra mode / partition gating. Gradient data is computed at
  // block level in such cases.
...
static AOM_INLINE void generate_hog_using_gradient_cache(const MACROBLOCK *x,
  ...
  const pixel_level_gradient_info *grad_info_blk = x->pixel_gradient_info +
                                                   plane * MAX_SB_SQUARE +
...
static INLINE void collect_hog_data(const MACROBLOCK *x, BLOCK_SIZE bsize,
  ...
  // If gradient data is already generated at SB level, reuse the same.
