...
// for LBD encode.
static AOM_INLINE void compute_gradient_info_sb_lowbd(MACROBLOCK *const x,
                                                      BLOCK_SIZE sb_size,
                                                      PLANE_TYPE plane) {
...
// Computes and stores pixel level gradient information of a given superblock
// for HBD encode.
static AOM_INLINE void compute_gradient_info_sb_highbd(MACROBLOCK *const x,
                                                       BLOCK_SIZE sb_size,
                                                       PLANE_TYPE plane) {
  PixelLevelGradientInfo *grad_info_sb =
      x->pixel_gradient_info + plane * MAX_SB_SQUARE;
...
static AOM_INLINE void compute_gradient_info_sb(MACROBLOCK *const x,
  ...
  if (is_cur_buf_hbd(&x->e_mbd)) {
    compute_gradient_info_sb_highbd(x, sb_size, plane);
    return;
  }
#endif  // CONFIG_AV1_HIGHBITDEPTH
  compute_gradient_info_sb_lowbd(x, sb_size, plane);
}
