...
int av1_dering_search(YV12_BUFFER_CONFIG *frame, const YV12_BUFFER_CONFIG *ref,
  ...
  best_tot_mse = (uint64_t)1 << 63;
  {
    ...
    for (l0=0;l0<DERING_STRENGTHS;l0++) {
      int l1;
      lev[0] = l0;
      for (l1=l0+DERING_REDUCED;l1<DERING_STRENGTHS;l1++) {
        int l2;
        lev[1] = l1;
        for (l2=l1+DERING_REDUCED;l2<DERING_STRENGTHS;l2++) {
          int l3;
          lev[2] = l2;
          for (l3=l2+DERING_REDUCED;l3<DERING_STRENGTHS;l3++) {
	    int cs0;
	    lev[3] = l3;
	    for (cs0=0; cs0 < CLPF_STRENGTHS; cs0++) {
	      int cs1;
	      str[0] = cs0;
	      for (cs1=cs0+DERING_REDUCED; cs1 < CLPF_STRENGTHS; cs1++) {
		double tot_mse = 0;
		str[1] = cs1;
...
#if !DERING_REDUCED
		// Add the bit cost
		int dering_values = 0, clpf_values = 0;
		for (i=1;i<DERING_REFINEMENT_LEVELS;i++)
		  dering_values += lev[i] != lev[i-1];
		for (i=1;i<CLPF_REFINEMENT_LEVELS;i++)
		  clpf_values += str[i] != str[i-1];
		tot_mse += sb_count * lambda * (log2[dering_values] + log2[clpf_values]);
#endif
