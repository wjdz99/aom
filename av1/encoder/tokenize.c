...
static void cost_coeffs_b(int plane, int block, int blk_row, int blk_col,
  ...
  const TX_TYPE tx_type = get_tx_type(type, xd, blk_row, blk_col, block, tx_size);
  const SCAN_ORDER *const scan_order = get_scan(cm, tx_size, tx_type, mbmi);
  const int rate = av1_cost_coeffs(cpi, x, plane, blk_row, blk_col, block, tx_size, scan_order,
