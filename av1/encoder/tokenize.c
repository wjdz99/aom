...
static int cost_and_tokenize_map_sb(const MACROBLOCK *const x, int plane,
  ...
  int color_cost[PALETTE_SIZES][PALETTE_COLOR_INDEX_CONTEXTS][PALETTE_COLORS];
  ...
  av1_get_block_dimensions(bsize, plane, xd, &plane_block_width, NULL, &rows,
                           &cols);
  memcpy(color_cost, plane ? x->palette_uv_color_cost : x->palette_y_color_cost,
         sizeof(color_cost));
  ...
  for (int i = 0; i < rows; ++i) {
    for (int j = (i == 0 ? 1 : 0); j < cols; ++j) {
      ...
      if (calc_rate) {
        this_rate += color_cost[n - PALETTE_MIN_SIZE][color_ctx][color_new_idx];
      } else {
