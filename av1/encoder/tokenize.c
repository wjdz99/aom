...
static INLINE void add_token(
    TOKENEXTRA **t, aom_cdf_prob (*tail_cdf)[ENTROPY_TOKENS + 1],
    aom_cdf_prob (*head_cdf)[ENTROPY_TOKENS + 1], int is_eob,
...
#if CONFIG_NEW_TOKENSET
  aom_cdf_prob(
      *const coef_head_cdfs)[COEFF_CONTEXTS][ENTROPY_TOKENS + 1] =
      ec_ctx->coef_head_cdfs[tx_size][type][ref];
  aom_cdf_prob(
      *const coef_tail_cdfs)[COEFF_CONTEXTS][ENTROPY_TOKENS + 1] =
...
#if CONFIG_EC_MULTISYMBOL
  aom_cdf_prob(
      *const coef_cdfs)[COEFF_CONTEXTS][ENTROPY_TOKENS + 1] =
