...
typedef struct color_map_param {
  int rows;
  int cols;
  int n_colors;
  int plane_width;
...
static int cost_and_tokenize_map(ColorMapParam *param,
...
static void get_palette_params(const MACROBLOCK *const x, int plane,
  ...
  params->color_cost = plane ? &x->palette_uv_color_cost
                             : &x->palette_y_color_cost;
  params->n_colors = pmi->palette_size[plane];
  av1_get_block_dimensions(bsize, plane, xd, &params->plane_width, NULL, &params->rows,
                           &params->cols);
}
...
static void get_color_map_params(const MACROBLOCK *const x, int plane,
  ...
  switch (type) {
    case PALETTE_MAP:
      get_palette_params(x, plane, bsize, params);
      break;
#if CONFIG_MRC_TX
    case PALETTE_MAP:
      get_mrc_params(x, plane, bsize, params);
      break;
#endif  // CONFIG_MRC_TX
...
int av1_cost_color_map(const MACROBLOCK *const x, int plane,
                       BLOCK_SIZE bsize, COLOR_MAP_TYPE type) {
