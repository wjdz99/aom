...
static void tokenize_b(int plane, int block, int blk_row, int blk_col,
  ...
  *tp = t;
  ...
  // update_scan_count_facade(). The update behavior should be the same because
  // update_scan_count_facade() only cares if coefficients are zero or not.
  update_scan_count_facade((AV1_COMMON *)cm, tx_size, tx_type, qcoeff, c);
#endif
