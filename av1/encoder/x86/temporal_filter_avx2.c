...
static AOM_INLINE __m256 approx_exp(__m256 y) {
...
static void apply_temporal_filter(
  ...
  if (tf_wgt_calc_lvl == 0) {
    ...
    subblock_mses[0] = _mm256_set1_pd(subblock_mses_scaled[0]);
    subblock_mses[1] = _mm256_set1_pd(subblock_mses_scaled[1]);
    subblock_mses[2] = _mm256_set1_pd(subblock_mses_scaled[2]);
    subblock_mses[3] = _mm256_set1_pd(subblock_mses_scaled[3]);
    ...
    for (int i = 0; i < block_height; i++) {
      ...
      for (int j = 0; j < block_width; j += 8) {
        ...
        // 2);
        const int subblock_idx = y_blk_raster_offset + (j >= block_width / 2);
        const __m256d blk_error = subblock_mses[subblock_idx];
        ...
        const __m256 exp_result = approx_exp(minus_scaled_diff_ps);
