...
int64_t av1_get_pixel_proj_error_avx2(const uint8_t *src8, int width,
  ...
  if (!use_highbitdepth) {
    ...
    if (params->r0 > 0 && params->r1 > 0) {
      ...
      for (int i = 0; i < height; ++i) {
        for (int j = 0; j < n16; ++j) {
          __m256i dl = _mm256_cvtepu16_epi32(
              _mm_loadu_si128((const __m128i *)(&dat[j * 16 + 0])));
          __m256i dh = _mm256_cvtepu16_epi32(
              _mm_loadu_si128((const __m128i *)(&dat[j * 16 + 8])));
          __m256i sl = _mm256_cvtepu16_epi32(
              _mm_loadu_si128((const __m128i *)(&src[j * 16 + 0])));
          __m256i sh = _mm256_cvtepu16_epi32(
              _mm_loadu_si128((const __m128i *)(&src[j * 16 + 8])));
