...
static INLINE void read_coeff(const tran_low_t *coeff, int offset, __m128i *c0,
                              __m128i *c1) {
...
static INLINE void write_zero(tran_low_t *qcoeff, int offset) {
  const __m128i zero = _mm_setzero_si128();
  tran_low_t *addr = qcoeff + offset;
#if CONFIG_HIGHBITDEPTH
