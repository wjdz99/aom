...
static INLINE void quantize(const int16_t *iscan_ptr,
  ...
  const __m128i qcoeff0_sh = _mm_slli_epi16(qcoeff0, 1);
  const __m128i qcoeff1_sh = _mm_slli_epi16(qcoeff1, 1);
  const __m128i mask0 = _mm_or_si128(_mm_cmpgt_epi16(qcoeff0_sh, *thr),
                                     _mm_cmpeq_epi16(qcoeff0_sh, *thr));
  const __m128i mask1 = _mm_or_si128(_mm_cmpgt_epi16(qcoeff1_sh, *thr),
                                     _mm_cmpeq_epi16(qcoeff1_sh, *thr));
...
void av1_quantize_fp_sse2(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  const __m128i thr0 = dequant0;
  __m128i eob = _mm_setzero_si128();

  quantize(iscan_ptr, coeff_ptr, n_coeffs, qcoeff_ptr, dqcoeff_ptr, &round0,
           &round1, &quant0, &quant1, &dequant0, &dequant1, &thr0, &eob);

  n_coeffs += 8 * 2;
  const __m128i thr1 = dequant1;

  // AC only loop
  while (n_coeffs < 0) {
    quantize(iscan_ptr, coeff_ptr, n_coeffs, qcoeff_ptr, dqcoeff_ptr, &round1,
             &round1, &quant1, &quant1, &dequant1, &dequant1, &thr1, &eob);
    n_coeffs += 8 * 2;
  }
