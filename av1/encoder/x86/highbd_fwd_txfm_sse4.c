...
void av1_fwd_txfm2d_64x32_sse4_1(const int16_t *input, int32_t *coeff,
  ...
  for (int i = 0; i < num_row; i++) {
    col_txfm_16x16_rounding((in + i * 64), -shift[1]);
  }
  transpose_8nx8n(in, outcoef128, txfm_size_col, txfm_size_row);
...
void av1_fwd_txfm2d_16x64_sse4_1(const int16_t *input, int32_t *coeff,
  ...
  for (int i = 0; i < 64; i += 4) {
    load_buffer_4x4(input + (i + 0) * stride, in + (i + 0) * 4, 4, ud_flip,
                    lr_flip, shift[0]);
    load_buffer_4x4(input + (i + 1) * stride, in + (i + 1) * 4, 4, ud_flip,
                    lr_flip, shift[0]);
    load_buffer_4x4(input + (i + 2) * stride, in + (i + 2) * 4, 4, ud_flip,
                    lr_flip, shift[0]);
    load_buffer_4x4(input + (i + 3) * stride, in + (i + 3) * 4, 4, ud_flip,
                    lr_flip, shift[0]);
  }

  for (int i = 0; i < 4; i++) {
    av1_fdct64_new_sse4_1(in + i, outcoeff128 + i, bitcol, 4, 4);
  }
  ...
  transpose_8nx8n(in, outcoeff128, 32, 16);
  memset(coeff + 16 * 32, 0,
         16 * 32 * sizeof(*coeff)); /*TODO : Needs to be removed*/
  (void)bd;
}
...
void av1_fwd_txfm2d_64x16_sse4_1(const int16_t *input, int32_t *coeff,
  ...
  // col tranform
  for (int i = 0; i < 16; i++) {
    load_buffer_4x4(input + 0 + i * stride, in + 0 + i * 16, 4, ud_flip,
                    lr_flip, shift[0]);
    load_buffer_4x4(input + 16 + i * stride, in + 4 + i * 16, 4, ud_flip,
                    lr_flip, shift[0]);
    load_buffer_4x4(input + 32 + i * stride, in + 8 + i * 16, 4, ud_flip,
                    lr_flip, shift[0]);
    load_buffer_4x4(input + 48 + i * stride, in + 12 + i * 16, 4, ud_flip,
                    lr_flip, shift[0]);
  }

  fdct16x16_sse4_1(in, outcoeff128, bitcol, 16);
  col_txfm_16x16_rounding(outcoeff128, -shift[1]);
  col_txfm_16x16_rounding(outcoeff128 + 64, -shift[1]);
  col_txfm_16x16_rounding(outcoeff128 + 128, -shift[1]);
  col_txfm_16x16_rounding(outcoeff128 + 192, -shift[1]);
  ...
  transpose_8nx8n(in, outcoeff128, 16, 32);
  (void)bd;
}
