...
void fwd_stxfm_sse4_1(tran_low_t *src, tran_low_t *dst,
                      const PREDICTION_MODE mode, const int8_t stx_idx,
                      const int size) {
  ...
  for (int j = 0; j < reduced_height; j++) {
    ...
    coef = 0;
    __m128i tmpSum = _mm_setzero_si128();  // Return vector of type __m128i with
                                           // all elements set to zero.
    for (int i = 0; i < reduced_width; i += 4) {
      __m128i tmpBlk = _mm_loadu_si128(
          (__m128i *)(srcPtr + i));  // Load 128-bits of integer data from
                                     // memory into tmpBlk
      __m128i tmpT = _mm_cvtepi16_epi32(_mm_loadl_epi64(
          (__m128i *)(kernel_tmp +
                      i)));  // Load 64-bit integer;Sign extend packed 16-bit
                             // integers to packed 32-bit integers, and store the
                             // results in tmpT
