...
// Shuffle mask used to rearrange weights corresponds to layer 1 and layer 2.
...
// order shown below to match source pixels. Basically this mask replicate
...
// and fill the registers appropriately to match source pixel mapping.
static INLINE void prepare_weights_for_2x2_concolve(
  ...
  shuffle_weight[0] = _mm256_castps128_ps256(_mm_loadu_ps(weight));
  shuffle_weight[1] =
      _mm256_permutevar8x32_ps(shuffle_weight[0], weight_mask[1]);
  shuffle_weight[0] =
      _mm256_permutevar8x32_ps(shuffle_weight[0], weight_mask[0]);
}
...
static INLINE void perform_convolve_for_8h_2x2_block(
    const float *input_ptr, int in_stride, int out_pos, __m256 *weight,
    __m256 *out_accum, __m256i shuffle_output_mask) {
  __m256 load_src[4];
  ...
  out_accum[out_pos] = _mm256_add_ps(out_accum[out_pos], load_src[0]);
}

// Do convolution on 8(4 horizontal x 2 vertical) 2x2 blocks.
static INLINE void perform_convolve_for_4hx2v_2x2_block(
    const float *input_ptr, int in_stride, int out_pos, __m256 *weight,
    __m256 *out_accum, __m256i shuffle_output_mask) {
  __m256 load_src[4];
  // Load input into source registers.
  ...
  load_src[2] = _mm256_loadu_ps(input_ptr + (in_stride << 1));
...
static INLINE void cnn_convolve_no_maxpool_padding_valid_layer1_avx2(
    ...
    const CNN_LAYER_CONFIG *const layer_config, float **output, int out_stride,
    int start_idx, const int cstep, const int channel_step,
    __m256i *weight_mask, const __m256i shuffle_output_mask) {
  for (int i = start_idx; i < layer_config->out_channels; i += channel_step) {
    ...
    for (int k = 0; k < layer_config->in_channels; ++k) {
      ...
      // which is why the load of weights happens once for each in-channel.
      prepare_weights_for_2x2_concolve(layer_config, off, cstep, shuffle_weight,
                                       weight_mask);
      ...
      for (int h = 0, u = 0; h < in_height - layer_config->filter_height + 1;
        ...
        perform_convolve_for_8h_2x2_block(input_ptr, in_stride, u,
                                          shuffle_weight, out_accum,
                                          shuffle_output_mask);
      }
    }
  ...
  for (int i = start_idx; i < layer_config->out_channels; i += channel_step) {
    ...
    for (int k = 0; k < layer_config->in_channels; ++k) {
      ...
      for (int h = 0, u = 0; h < in_height - layer_config->filter_height + 1;
        ...
        perform_convolve_for_4hx2v_2x2_block(input_ptr, in_stride, u,
                                             shuffle_weight, out_accum,
                                             shuffle_output_mask);
      }
    }
...
// AVX2 variant of cnn_no_maxpool_padding_valid(), when filter_width and
// filter_height are equal to 2.
// As per the config layer set by av1_intra_mode_cnn_partition_cnn_config,
...
void cnn_convolve_no_maxpool_padding_valid_2x2_avx2(
  ...
  const __m256i shuffle_output_mask =
  ...
  // This case of in_width and in_height equal to 16 is corresponds to layer 1.
  ...
  if (in_width == 16 && in_height == 16) {
    cnn_convolve_no_maxpool_padding_valid_layer1_avx2(
        ...
        start_idx, cstep, channel_step, weight_mask, shuffle_output_mask);
  } else if (in_width == 8 && in_height == 8) {
    // This case of in_width and in_height equal to 8 is corresponds to layer 2.
    ...
    // which is why calling C path for layer >= 3.
...
// As per the current encoder, cnn_convolve function gets called for
// block size equal to 64x64. cnn_convolve() uses layer config values
// set by av1_intra_mode_cnn_partition_cnn_config. The following are few
// details related to each layer config parameters.
