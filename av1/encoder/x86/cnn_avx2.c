...
static INLINE void prepare_weights_for_5x5_conv(
    __m256 load_weight[], __m256i weight_mask_0, __m256i weight_mask_1,
    const CNN_LAYER_CONFIG *layer_config, int off, int cstep) {
  float weight[5][5];
  for (int i = 0; i < 5; ++i) {
    for (int j = 0; j < 5; ++j) {
...
#define PERFORM_CONVOLVE_FOR_1_5X5_BLOCK(blk0, w, accum0, input, in_stride) \
  {                                                                         \
    __m256 load_src[5];                                                     \
...
void cnn_convlove_5x5_avx2(const float **input, int in_width, int in_height,
  ...
  for (int i = start_idx; i < layer_config->out_channels; i += channel_step) {
    ...
    for (int k = 0; k < layer_config->in_channels; ++k) {
      ...
      prepare_weights_for_5x5_conv(shuffle_weight, weight_mask_0, weight_mask_1,
                                   layer_config,
                                   k * layer_config->out_channels + i, cstep);
