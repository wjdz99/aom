...
static INLINE void prepare_weights_for_5x5_concolve(
    const CNN_LAYER_CONFIG *const layer_config, int in_ch, int out_ch,
    float weight[5][8], const int cstep, __m256 *shuffle_weight,
    const __m256i weight_mask_0, const __m256i weight_mask_1) {
  int off = in_ch * layer_config->out_channels + out_ch;
...
static void cnn_no_maxpool_padding_valid_convolve_5x5_avx2(
  ...
  const int min_width_for_3_5x5_blk =
  ...
  for (int i = start_idx; i < layer_config->out_channels; i += channel_step) {
    ...
    for (int k = 0; k < layer_config->in_channels; ++k) {
      ...
      prepare_weights_for_5x5_concolve(layer_config, k, i, weight, cstep,
      ...
      for (int h = 0, u = 0; h < in_height - layer_config->filter_height + 1;
        ...
        while (rem_width >= min_width_for_3_5x5_blk) {
  ...
  if (layer_config->maxpool &&
    ...
    switch (layer_config->pad) {
      case PADDING_SAME_ZERO:
        cnn_maxpool_padding_zero(input, in_width, in_height, in_stride,
                                 layer_config, output, out_stride, cstep,
                                 filter_width_half, filter_height_half);
        break;
      case PADDING_SAME_REPLICATE:
        cnn_maxpool_padding_replicate(input, in_width, in_height, in_stride,
                                      layer_config, output, out_stride, cstep,
                                      filter_width_half, filter_height_half);
        break;
      case PADDING_VALID:
        cnn_maxpool_padding_valid(input, in_width, in_height, in_stride,
                                  layer_config, output, out_stride, cstep);
        break;
    ...
    if (layer_config->filter_height == 1 && layer_config->filter_width == 1) {
      cnn_element_wise_convolve(input, in_width, in_height, in_stride,
    ...
    switch (layer_config->pad) {
      ...
      case PADDING_SAME_REPLICATE: {
        cnn_no_maxpool_padding_replicate(
            input, in_width, in_height, in_stride, layer_config, output,
      ...
      case PADDING_VALID: {
        if (layer_config->filter_width == 5 &&
          ...
          cnn_no_maxpool_padding_valid_convolve_5x5_avx2(
              input, in_width, in_height, in_stride, layer_config, output,
              out_stride, start_idx, cstep, channel_step);
        } else {
          cnn_no_maxpool_padding_valid(input, in_width, in_height, in_stride,
