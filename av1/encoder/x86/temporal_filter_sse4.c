...
void av1_apply_temporal_filter_yuv_sse4_1(
  ...
  const int mb_size = block_height * block_width;
  ...
  const uint8_t *u_pre = pred + mb_size;
  const uint8_t *v_pre = pred + mb_size * 2;
  uint32_t *y_accum = accum;
  uint32_t *u_accum = accum + mb_size;
  uint32_t *v_accum = accum + mb_size * 2;
  uint16_t *y_count = count;
  uint16_t *u_count = count + mb_size;
  uint16_t *v_count = count + mb_size * 2;
