...
static void transpose_8x8_avx2_stride(const __m256i *in, __m256i *out,
...
static INLINE void round_shift_16xn_avx2(__m256i *in, int size, int bit,
                                         int stride) {
  if (bit < 0) {
    bit = -bit;
...
typedef void (*transform_1d_avx2)(__m256i *in, __m256i *out,
                                  ...
                                  int outstride);
static void av1_fdct64_new_stage_10_to_11_avx2(__m256i *x9, __m256i *output,
                                               int8_t cos_bit,
                                               const int stride1) {
  ...
  int startidx = 0 * stride1;
  int endidx = 63 * stride1;
  // stage 11
...
static void av1_fdct64_new_stage_4_to_9_avx2(__m256i *x3, __m256i *x9,
                                             int8_t cos_bit) {
  const int32_t *cospi = cospi_arr(cos_bit);
  const __m256i __rounding = _mm256_set1_epi32(1 << (cos_bit - 1));
...
static void av1_fdct64_new_avx2(__m256i *input, __m256i *output, int8_t cos_bit,
  ...
  __m256i x9[64];
  av1_fdct64_new_stage_4_to_9_avx2(x3, x9, cos_bit);

  av1_fdct64_new_stage_10_to_11_avx2(x9, output, cos_bit, outstride);
}
void av1_fwd_txfm2d_64x64_avx2(const int16_t *input, int32_t *output,
  ...
  const transform_1d_avx2 col_txfm = av1_fdct64_new_avx2;
  const transform_1d_avx2 row_txfm = av1_fdct64_new_avx2;
  ...
  for (int i = 0; i < width_div16; i++) {
    load_buffer_16xn_avx2(input + (i << 4), &buf0[i << 1], stride, height,
                          width_div8);
    round_shift_16xn_avx2(&buf0[i << 1], height, shift[0], width_div8);
    round_shift_16xn_avx2(&buf0[(i << 1) + 1], height, shift[0], width_div8);
    ...
    round_shift_16xn_avx2(&buf0[i << 1], height, shift[1], width_div8);
    round_shift_16xn_avx2(&buf0[(i << 1) + 1], height, shift[1], width_div8);
  }

  for (r = 0; r < height; r += 8) {
    for (c = 0; c < width_div8; c++) {
      transpose_8x8_avx2_stride(&buf0[r * width_div8 + c],
                                &buf1[c * 8 * width_div8 + r / 8], width_div8,
                                width_div8);
    }
  }
  ...
  for (int i = 0; i < 2; i++) {
    ...
    round_shift_16xn_avx2(&buf0[i << 1], (height >> 1), shift[2], width_div16);
    round_shift_16xn_avx2(&buf0[(i << 1) + 1], (height >> 1), shift[2],
                          width_div16);
  }

  for (r = 0; r < (height >> 1); r += 8) {
    for (c = 0; c < width_div16; c++) {
      transpose_8x8_avx2_stride(&buf0[r * width_div16 + c],
                                &buf1[c * 8 * width_div16 + r / 8], width_div16,
                                width_div16);
    }
  }
