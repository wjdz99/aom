...
void fdct64_new_sse2(const __m128i *input, __m128i *output, int8_t cos_bit) {
  const int32_t *cospi = cospi_arr(cos_bit);
  const __m128i __rounding = _mm_set1_epi32(1 << (cos_bit - 1));
...
void fadst16_new_sse2(const __m128i *input, __m128i *output, int8_t cos_bit) {
  const int32_t *cospi = cospi_arr(cos_bit);
  const __m128i __zero = _mm_setzero_si128();
  const __m128i __rounding = _mm_set1_epi32(1 << (cos_bit - 1));
...
void av1_fwd_txfm2d_16x16_sse2(const int16_t *input, int32_t *output,
  ...
  if (col_txfm != NULL && row_txfm != NULL) {
    for (int i = 0; i < 2; i++) {
      if (ud_flip) {
        load_buffer_16bit_to_16bit_flip(input + 8 * i, stride, buf0, buf_size);
      } else {
        load_buffer_16bit_to_16bit(input + 8 * i, stride, buf0, buf_size);
      }
      round_shift_16bit(buf0, buf_size, shift[0]);
      col_txfm(buf0, buf0, cos_bit_col);
      round_shift_16bit(buf0, buf_size, shift[1]);
      transpose_16bit_8x8(buf0, buf1 + 8 * i);
      transpose_16bit_8x8(buf0 + 8, buf1 + 8 * i + 16);
    }
    ...
    for (int i = 0; i < 2; i++) {
      ...
      if (lr_flip) {
        buf = buf0;
        flip_buf_sse2(buf1 + 16 * i, buf, 16);
      } else {
        buf = buf1 + 16 * i;
      }
      row_txfm(buf, buf, cos_bit_row);
      round_shift_16bit(buf, buf_size, shift[2]);
      transpose_16bit_8x8(buf, buf);
      store_buffer_16bit_to_32bit_8x8(buf, output + 8 * buf_size * i, buf_size);
      transpose_16bit_8x8(buf + 8, buf + 8);
      store_buffer_16bit_to_32bit_8x8(buf + 8, output + 8 * buf_size * i + 8,
                                      buf_size);
    }
  } else {
