...
static void set_segment_id_supertx(const AV1_COMP *const cpi,
  ...
  const int miw =
      AOMMIN(mi_size_wide[bsize], cm->mi_cols - mi_col);
  const int mih =
      AOMMIN(mi_size_high[bsize], cm->mi_rows - mi_row);
...
#if CONFIG_SUPERTX
    abort_flag = (sum_rdc.rdcost >= best_rd && (bsize > BLOCK_8X8 || unify_bsize)) ||
...
static void extend_dir(const AV1_COMP *const cpi, ThreadData *td,
  ...
  if (dir == 0 || dir == 1) {  // lower and upper
    extend_bsize = (mi_width == mi_size_wide[BLOCK_8X8] || bsize < BLOCK_8X8 || xss < yss)
                       ? BLOCK_8X8
                       : BLOCK_16X8;
    ...
    for (j = 0; j < mi_height + ext_offset; j += high_unit)
      for (i = 0; i < mi_width + ext_offset; i += wide_unit)
        predict_b_extend(cpi, td, tile, block, mi_row, mi_col,
                         mi_row_pred + j, mi_col_pred + i,
                         mi_row_top, mi_col_top, dst_buf, dst_stride,
                         top_bsize, extend_bsize, dry_run, b_sub8x8, 1);
  } else if (dir == 2 || dir == 3) {  // left and right
    extend_bsize = (mi_height == mi_size_high[BLOCK_8X8] || bsize < BLOCK_8X8 || yss < xss)
                       ? BLOCK_8X8
                       : BLOCK_8X16;
#if CONFIG_CB4X4
    if (bsize < BLOCK_8X8) {
    ...
    mi_row_pred = mi_row + ((dir == 4 || dir == 6) ? mi_height : -(mi_height + ext_offset));
    mi_col_pred = mi_col + ((dir == 6 || dir == 7) ? mi_width : -(mi_width + ext_offset));
