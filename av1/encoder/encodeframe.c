...
void save_results(MACROBLOCK *const x, BLOCK_SIZE bsize,
  ...
  for (i = 0; i < MAX_MB_PLANE; i++) {
    ...
    int width = 4 * num_4x4_blocks_wide_lookup[bs];
    const uint8_t *src = pd->dst.buf;
    uint8_t *dst = (uint8_t *)ctx->recon_pix[i];
#if CONFIG_AOM_HIGHBITDEPTH
    if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH)
  ...
  // AQ modes 2 and 3 change the segment id before reencoding
  ctx->mic.mbmi.segment_id = xd->mi[0]->mbmi.segment_id;
}
...
void restore_results(MACROBLOCK *const x, BLOCK_SIZE bsize,
  ...
  for (i = 0; i < MAX_MB_PLANE; i++) {
    ...
    const int height = 4 * num_4x4_blocks_high_lookup[bs];
    int width = 4 * num_4x4_blocks_wide_lookup[bs];
    const uint8_t *src = (uint8_t *)ctx->recon_pix[i];
  ...
  switch (partition) {
    ...
    case PARTITION_HORZ:
      ...
      if (last_part_rdc.rate != INT_MAX && bsize >= BLOCK_8X8 &&
        ...
        update_state(cpi, td, ctx_h, subsize);
    ...
    case PARTITION_VERT:
      ...
      if (last_part_rdc.rate != INT_MAX && bsize >= BLOCK_8X8 &&
        ...
        av1_rd_cost_init(&tmp_rdc);
        update_state(cpi, td, ctx_v, subsize);
...
static void rd_pick_partition(const AV1_COMP *const cpi, ThreadData *td,
  ...
  if (partition_vert_allowed &&
    ...
    if (sum_rdc.rdcost < best_rdc.rdcost && !force_vert_split &&
        bsize > BLOCK_8X8) {
      update_state(cpi, td, &pc_tree->vertical[0], subsize);
