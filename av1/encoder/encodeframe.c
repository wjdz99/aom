...
static int rd_try_subblock(AV1_COMP *const cpi, ThreadData *td,
                           ...
                           PARTITION_TYPE partition,
                           const PICK_MODE_CONTEXT *const prev_ctx,
...
static bool rd_test_partition3(
    ...
    PICK_MODE_CONTEXT *ctxs[3], const PICK_MODE_CONTEXT *const ctx, int mi_row,
    int mi_col, BLOCK_SIZE bsize, PARTITION_TYPE partition, int mi_row0,
    int mi_col0, BLOCK_SIZE subsize0, int mi_row1, int mi_col1,
...
typedef struct {
  ...
  int horz_ctx_is_ready;
  int vert_ctx_is_ready;
...
static void init_partition_search_state_params(
  ...
  blk_params->bsize_1d = block_size_wide[bsize];
  ...
  blk_params->bsize2 = blk_params->subsize;
  ...
  part_search_state->partition_cost =
      mode_costs->partition_cost[part_search_state->pl];
  ...
  // Chroma subsampling.
  part_search_state->xss = x->e_mbd.plane[1].subsampling_x;
  part_search_state->yss = x->e_mbd.plane[1].subsampling_y;
  ...
  part_search_state->partition_horz_allowed =
      ...
      get_plane_block_size(get_partition_subsize(bsize, PARTITION_HORZ),
                           part_search_state->xss,
                           part_search_state->yss) != BLOCK_INVALID;
  ...
  part_search_state->cur_none_rd = 0;
  ...
  for (int i = 0; i < 2; i++) part_search_state->split_ctx_is_ready[i] = 0;
  part_search_state->horz_ctx_is_ready = 0;
  part_search_state->vert_ctx_is_ready = 0;
...
static bool rd_pick_partition(AV1_COMP *const cpi, ThreadData *td,
  ...
  if (!(blk_params.has_rows && blk_params.has_cols)) {
    assert(blk_params.bsize_at_least_8x8 && part_search_state.pl >= 0);
    const aom_cdf_prob *partition_cdf =
        cm->fc->partition_cdf[part_search_state.pl];
...
BEGIN_PARTITION_SEARCH:
  ...
  if (x->must_find_valid_partition) {
    part_search_state.do_square_split =
        blk_params.bsize_at_least_8x8 &&
        (blk_params.bsize_1d > blk_params.min_partition_size_1d);
    part_search_state.partition_none_allowed =
        blk_params.has_rows && blk_params.has_cols &&
        (blk_params.bsize_1d >= blk_params.min_partition_size_1d);
    part_search_state.partition_horz_allowed =
  ...
  if (pc_tree->none == NULL)
    pc_tree->none = av1_alloc_pmc(cm, bsize, &td->shared_coeff_buf);
  PICK_MODE_CONTEXT *ctx_none = pc_tree->none;
  if ((blk_params.bsize_1d <= blk_params.min_partition_size_1d) &&
      blk_params.has_rows && blk_params.has_cols)
  ...
  if (part_search_state.partition_none_allowed) {
    ...
    part_search_state.cur_none_rd = part_search_state.this_rdc.rdcost;
  ...
  if ((!part_search_state.terminate_partition_search &&
      ...
      if (idx <= 1 && (bsize <= BLOCK_8X8 ||
        ...
        if (pmi->palette_size[0] == 0 && pmi->palette_size[1] == 0) {
          if (mbmi->uv_mode != UV_CFL_PRED)
            part_search_state.split_ctx_is_ready[idx] = 1;
        }
      }
    }
#if CONFIG_COLLECT_PARTITION_STATS
    ...
    if (reached_last_index &&
      ...
      if (cpi->sf.part_sf.less_rectangular_check_level == 2 || idx <= 2) {
        const int partition_none_valid = part_search_state.cur_none_rd > 0;
        const int partition_none_better =
            part_search_state.cur_none_rd < part_search_state.sum_rdc.rdcost;
        part_search_state.do_rectangular_split &=
  ...
  if (!cpi->sf.part_sf.ml_early_term_after_part_split_level &&
    ...
    av1_ml_prune_rect_partition(
        cpi, x, bsize, best_rdc.rdcost, part_search_state.cur_none_rd,
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    if (part_search_state.sum_rdc.rdcost < best_rdc.rdcost &&
      ...
      if (pmi->palette_size[0] == 0 && pmi->palette_size[1] == 0) {
        if (mbmi->uv_mode != UV_CFL_PRED)
          part_search_state.horz_ctx_is_ready = 1;
      }
      update_state(cpi, td, ctx_h, mi_row, mi_col, subsize, 1);
      encode_superblock(cpi, tile_data, td, tp, DRY_RUN_NORMAL, subsize, NULL);
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    if (part_search_state.sum_rdc.rdcost < best_rdc.rdcost &&
      ...
      if (pmi->palette_size[0] == 0 && pmi->palette_size[1] == 0) {
        if (mbmi->uv_mode != UV_CFL_PRED)
          part_search_state.vert_ctx_is_ready = 1;
      }
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    pc_tree->horizontala[0] =
        av1_alloc_pmc(cm, blk_params.bsize2, &td->shared_coeff_buf);
    ...
    // PARTITION_SPLIT.
    if (part_search_state.split_ctx_is_ready[0]) {
      ...
      if (part_search_state.split_ctx_is_ready[1]) {
...
#if CONFIG_COLLECT_PARTITION_STATS
    {
      ...
      tmp_sum_rdc.rate = x->partition_cost[blk_params.pl][PARTITION_HORZ_A];
    ...
    part_search_state.found_best_partition |= rd_test_partition3(
        ...
        blk_params.bsize2, mi_row, blk_params.mi_col_edge, blk_params.bsize2,
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    if (part_search_state.horz_ctx_is_ready) {
...
#if CONFIG_COLLECT_PARTITION_STATS
    {
      RD_STATS tmp_sum_rdc;
      av1_init_rd_stats(&tmp_sum_rdc);
      tmp_sum_rdc.rate = x->partition_cost[blk_params.pl][PARTITION_HORZ_B];
      tmp_sum_rdc.rdcost = RDCOST(x->rdmult, tmp_sum_rdc.rate, 0);
      if (best_rdc.rdcost - tmp_sum_rdc.rdcost >= 0) {
    ...
    part_search_state.found_best_partition |= rd_test_partition3(
        ...
        subsize, blk_params.mi_row_edge, mi_col, blk_params.bsize2,
        blk_params.mi_row_edge, blk_params.mi_col_edge, blk_params.bsize2);
  ...
  if (!part_search_state.terminate_partition_search &&
...
#if CONFIG_COLLECT_PARTITION_STATS
    {
      RD_STATS tmp_sum_rdc;
      av1_init_rd_stats(&tmp_sum_rdc);
      tmp_sum_rdc.rate = x->partition_cost[blk_params.pl][PARTITION_VERT_A];
      tmp_sum_rdc.rdcost = RDCOST(x->rdmult, tmp_sum_rdc.rate, 0);
      if (best_rdc.rdcost - tmp_sum_rdc.rdcost >= 0) {
    ...
    part_search_state.found_best_partition |= rd_test_partition3(
        ...
        blk_params.bsize2, blk_params.mi_row_edge, mi_col, blk_params.bsize2,
        mi_row, blk_params.mi_col_edge, subsize);
#if CONFIG_COLLECT_PARTITION_STATS
    if (partition_timer_on) {
      aom_usec_timer_mark(&partition_timer);
      int64_t time = aom_usec_timer_elapsed(&partition_timer);
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    if (part_search_state.vert_ctx_is_ready) {
...
#if CONFIG_COLLECT_PARTITION_STATS
    {
      RD_STATS tmp_sum_rdc;
      av1_init_rd_stats(&tmp_sum_rdc);
      tmp_sum_rdc.rate = x->partition_cost[blk_params.pl][PARTITION_VERT_B];
      tmp_sum_rdc.rdcost = RDCOST(x->rdmult, tmp_sum_rdc.rate, 0);
      if (!frame_is_intra_only(cm) &&
    ...
    part_search_state.found_best_partition |= rd_test_partition3(
        ...
        subsize, mi_row, blk_params.mi_col_edge, blk_params.bsize2,
        blk_params.mi_row_edge, blk_params.mi_col_edge, blk_params.bsize2);
#if CONFIG_COLLECT_PARTITION_STATS
    if (partition_timer_on) {
      aom_usec_timer_mark(&partition_timer);
  ...
  if (blk_params.bsize_1d < (blk_params.min_partition_size_1d << 2)) {
