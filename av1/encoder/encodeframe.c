...
static void init_partition_search_state_params(
  ...
  blk_params->split_part_size = blk_params->subsize;
  ...
  part_search_state->pl_context_ondex =
  ...
  part_search_state->partition_cost =
      mode_costs->partition_cost[part_search_state->pl_context_ondex];
...
static bool rd_pick_partition(AV1_COMP *const cpi, ThreadData *td,
  ...
  if (!(blk_params.has_rows && blk_params.has_cols)) {
    assert(blk_params.bsize_at_least_8x8 &&
           part_search_state.pl_context_ondex >= 0);
    const aom_cdf_prob *partition_cdf =
        cm->fc->partition_cdf[part_search_state.pl_context_ondex];
...
BEGIN_PARTITION_SEARCH:
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    pc_tree->horizontala[0] =
        av1_alloc_pmc(cm, blk_params.split_part_size, &td->shared_coeff_buf);
...
#if CONFIG_COLLECT_PARTITION_STATS
    {
      ...
      tmp_sum_rdc.rate = x->partition_cost[part_search_state.pl_context_ondex]
                                          [PARTITION_HORZ_A];
    ...
    part_search_state.found_best_partition |= rd_test_partition3(
        ...
        blk_params.split_part_size, mi_row, blk_params.mi_col_edge,
        blk_params.split_part_size, blk_params.mi_row_edge, mi_col, subsize);
#if CONFIG_COLLECT_PARTITION_STATS
    if (partition_timer_on) {
      aom_usec_timer_mark(&partition_timer);
      int64_t time = aom_usec_timer_elapsed(&partition_timer);
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    part_search_state.found_best_partition |= rd_test_partition3(
        ...
        subsize, blk_params.mi_row_edge, mi_col, blk_params.split_part_size,
        blk_params.mi_row_edge, blk_params.mi_col_edge,
        blk_params.split_part_size);
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    part_search_state.found_best_partition |= rd_test_partition3(
        ...
        blk_params.split_part_size, blk_params.mi_row_edge, mi_col,
        blk_params.split_part_size, mi_row, blk_params.mi_col_edge, subsize);
#if CONFIG_COLLECT_PARTITION_STATS
    if (partition_timer_on) {
      aom_usec_timer_mark(&partition_timer);
      int64_t time = aom_usec_timer_elapsed(&partition_timer);
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    part_search_state.found_best_partition |= rd_test_partition3(
        ...
        subsize, mi_row, blk_params.mi_col_edge, blk_params.split_part_size,
        blk_params.mi_row_edge, blk_params.mi_col_edge,
        blk_params.split_part_size);
#if CONFIG_COLLECT_PARTITION_STATS
    if (partition_timer_on) {
