...
static AOM_INLINE void encode_nonrd_sb(AV1_COMP *cpi, ThreadData *td,
  ...
  const BLOCK_SIZE sb_size = cm->seq_params->sb_size;
  PC_TREE *const pc_root = cpi->rt_pc_root;
...
static AOM_INLINE void encode_frame_internal(AV1_COMP *cpi) {
  ...
  const int use_nonrd_mode = cpi->sf.rt_sf.use_nonrd_pick_mode;
  cpi->rt_pc_root =
      use_nonrd_mode ? av1_alloc_pc_tree_node(cm->seq_params->sb_size) : NULL;
  ...
  av1_free_pc_tree_recursive(cpi->rt_pc_root, av1_num_planes(cm), 0, 0);
