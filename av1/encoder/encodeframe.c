 ...
 * \param[out]    num_planes     Number of image planes (e.g.
 * Y,U,V)
 *
...
void init_quantizers(AV1_COMP *cpi, ThreadData *td,
                     const TileDataEnc *tile_data,
                     SIMPLE_MOTION_DATA_TREE *sms_tree, RD_STATS *rd_cost,
...
int sb_qp_sweep(AV1_COMP *const cpi, ThreadData *td, TileDataEnc *tile_data,
  ...
  for (int sweep_qp_delta = start; sweep_qp_delta <= end;
       sweep_qp_delta += step) {
    init_quantizers(cpi, td, tile_data, sms_tree, &cur_rdc, mi_row, mi_col,
                    sweep_qp_delta);
...
static AOM_INLINE void encode_rd_sb(AV1_COMP *cpi, ThreadData *td,
  ...
  if (sf->part_sf.partition_search_type == VAR_BASED_PARTITION) {
    ...
    FRAME_CONTEXT *org_tile_ctx = NULL;

    if (cpi->oxcf.sb_qp_sweep) {
      sb_org_stats = (SB_FIRST_PASS_STATS *)malloc(sizeof(SB_FIRST_PASS_STATS));
      if (!sb_org_stats) return;
      av1_backup_sb_state(sb_org_stats, cpi, td, tile_data, mi_row, mi_col);

      // Don't copy in row_mt case, otherwise run into data race. No behavior
      // change in row_mt case.
      if (cpi->sf.inter_sf.inter_mode_rd_model_estimation == 1) {
        org_tile_ctx = (FRAME_CONTEXT *)malloc(sizeof(FRAME_CONTEXT));
        if (!org_tile_ctx) return;
    ...
    if (cpi->oxcf.sb_qp_sweep &&
      ...
      init_quantizers(cpi, td, tile_data, sms_root, &dummy_rdc, mi_row, mi_col,
                      best_qp_diff);
