...
BEGIN_PARTITION_SEARCH:
  ...
  // 1:4 partitions search stage.
  int part4_search_allowed[NUM_PART4_TYPES] = { 1, 1 };

  // Dsiable partition4 search flags for width less than twice the minimum
  // width.
  ...
  // PARTITION_HORZ_4
  assert(
      IMPLIES(!part_cfg->enable_rect_partitions, !part4_search_allowed[HORZ4]));
  if (!part_search_state.terminate_partition_search &&
  ...
  // PARTITION_VERT_4
  assert(
      IMPLIES(!part_cfg->enable_rect_partitions, !part4_search_allowed[VERT4]));
  if (!part_search_state.terminate_partition_search &&
