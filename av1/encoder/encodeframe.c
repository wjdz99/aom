...
static AOM_INLINE void prune_partitions_after_split(
    ...
    PartitionSearchState *part_search_state, RD_STATS *best_rdc,
    int64_t part_none_rd, int64_t part_split_rd, BLOCK_SIZE bsize) {
  const AV1_COMMON *const cm = &cpi->common;
  PartitionBlkParams blk_params = part_search_state->part_blk_params;
  const int mi_row = blk_params.mi_row;
  const int mi_col = blk_params.mi_col;
...
static AOM_INLINE void prune_partitions_based_on_none_part(
    AV1_COMP *const cpi, MACROBLOCK *x, SIMPLE_MOTION_DATA_TREE *sms_tree,
    PICK_MODE_CONTEXT *ctx_none, PartitionSearchState *part_search_state,
    RD_STATS *best_rdc, unsigned int *pb_source_variance, BLOCK_SIZE bsize) {
  const AV1_COMMON *const cm = &cpi->common;
  MACROBLOCKD *const xd = &x->e_mbd;
  PartitionBlkParams blk_params = part_search_state->part_blk_params;
  const CommonModeInfoParams *const mi_params = &cm->mi_params;
...
static AOM_INLINE void none_partition_search(
    ...
    int64_t *none_rd, int64_t *part_none_rd, BLOCK_SIZE bsize) {
  const AV1_COMMON *const cm = &cpi->common;
  PartitionBlkParams blk_params = part_search_state->part_blk_params;
  RD_STATS *this_rdc = &part_search_state->this_rdc;
  const int mi_row = blk_params.mi_row;
    ...
    *part_none_rd = this_rdc->rdcost;
    if (this_rdc->rdcost < best_rdc->rdcost) {
      *best_rdc = *this_rdc;
      ...
      prune_partitions_based_on_none_part(cpi, x, sms_tree, pc_tree->none,
                                          part_search_state, best_rdc,
                                          pb_source_variance, bsize);
    }
  }
  restore_context(x, x_ctx, mi_row, mi_col, bsize, av1_num_planes(cm));
}
...
BEGIN_PARTITION_SEARCH:
  ...
  prune_partitions_after_split(cpi, x, sms_tree, &part_search_state, &best_rdc,
                               part_none_rd, part_split_rd, bsize);
