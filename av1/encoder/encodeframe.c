      ...
      if (inter_block) {
        ...
        if (has_second_ref(mbmi)) {
          ...
          if (mbmi->sb_type >= BLOCK_8X8 &&
            ...
            const int bit0 = (ref_third == ALTREF_FRAME || ref_third == BWDREF_FRAME);
...
#if CONFIG_COMP_TRIPRED
      YV12_BUFFER_CONFIG *cfg = (ref < 2) ?
          get_ref_frame_buffer(cpi, mbmi->ref_frame[ref]) :
          get_ref_frame_buffer(cpi, mbmi->ref_frame_third);
      const struct scale_factors *const sf = (ref < 2) ?
          &xd->block_refs[ref]->sf : &xd->block_ref_third->sf;
#else
