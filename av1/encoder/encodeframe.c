...
static void update_stats(const AV1_COMMON *const cm, ThreadData *td, int mi_row,
  ...
  if (cm->delta_q_present_flag && (bsize != BLOCK_64X64 || !mbmi->skip) &&
    ...
    if (cm->delta_lf_present_flag) {
      const int dlf = (mbmi->current_delta_lf - xd->prev_delta_lf) / cm->delta_lf_res;
      ...
      xd->prev_delta_lf = mbmi->current_delta_lf;
    }
#endif
  }
#else
...
#if CONFIG_EXT_DELTA_Q
  if (cm->delta_lf_present_flag)
    if (mi_row == tile_info->mi_row_start) xd->prev_delta_lf = 0;
#endif
#endif
  ...
  for (mi_col = tile_info->mi_col_start; mi_col < tile_info->mi_col_end;
    ...
    if (cpi->oxcf.deltaq_mode != NO_DELTA_Q) {
      ...
      if (cpi->oxcf.deltaq_mode == DELTA_Q_LF) {
        ...
        int current_delta_lf = offset_qindex / 2;
        current_delta_lf =
          ((current_delta_lf + cm->delta_lf_res / 2) & lfmask);
        //pre-set the delta lf for loop filter. Note that this value is set
        //before mi is assigned for each block in current superblock
        for (j = 0; j < AOMMIN(cm->mib_size, cm->mi_rows - mi_row); j++) {
          for (k = 0; k < AOMMIN(cm->mib_size, cm->mi_cols - mi_col); k++) {
            cm->mi[(mi_row + j) * cm->mi_stride + (mi_col + k)].mbmi.current_delta_lf = current_delta_lf;
          }
        }        
      }
    }
