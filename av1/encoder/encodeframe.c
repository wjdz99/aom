...
// Whether to allow lower model search if the erroradv test
// fails for a higher model
#define DO_LOWER_MODEL_SEARCH 0
...
#if DO_LOWER_MODEL_SEARCH
              if (cm->global_motion[frame].wmtype <= TRANSLATION) {
                set_default_gmparams(&cm->global_motion[frame]);
              } else {
                // Not enough advantage in using a global model. Try lower models.
                do {
                  erroradvantage = refine_integerized_param(
                      &cm->global_motion[frame],
                      cm->global_motion[frame].wmtype - 1,
#if CONFIG_AOM_HIGHBITDEPTH
                      xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH, xd->bd,
#endif  // CONFIG_AOM_HIGHBITDEPTH
                      ref_buf->y_buffer, ref_buf->y_width, ref_buf->y_height,
                ...
                } while (cm->global_motion[frame].wmtype > TRANSLATION);
              }
#else
              set_default_gmparams(&cm->global_motion[frame]);
#endif  // DO_LOWER_MODEL_SEARCH
            }
          }
        }
        aom_clear_system_state();
