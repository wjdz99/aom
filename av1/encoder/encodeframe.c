...
unsigned int av1_get_sbuv_perpixel_variance(const AV1_COMP *cpi,
                                            const MACROBLOCK *x,
  ...
  const struct buf_2d *puv_src = &x->plane[plane].src;
  unsigned int var, sse;
  ...
  if (is_cur_buf_hbd(xd)) {
    ...
    const uint16_t *high_var_offs[3] = { AV1_HIGH_VAR_OFFS_8,
                                         AV1_HIGH_VAR_OFFS_10,
                                         AV1_HIGH_VAR_OFFS_12 };
    var = cpi->ppi->fn_ptr[uv_bsize].vf(
        puv_src->buf, puv_src->stride,
        CONVERT_TO_BYTEPTR(high_var_offs[off_index]), 0, &sse);
  } else {
    var = cpi->ppi->fn_ptr[uv_bsize].vf(puv_src->buf, puv_src->stride,
                                        AV1_VAR_OFFS, 0, &sse);
  }
#else
  var = cpi->ppi->fn_ptr[uv_bsize].vf(puv_src->buf, puv_src->stride,
                                      AV1_VAR_OFFS, 0, &sse);
#endif
  return ROUND_POWER_OF_TWO(var, num_pels_log2_lookup[uv_bsize]);
}
