...
static int skin_sb_split(AV1_COMP *cpi, BLOCK_SIZE bsize, int mi_row,
                         int mi_col) {
  const CommonModeInfoParams *const mi_params = &cpi->common.mi_params;
  // Avoid checking superblocks on/near boundary and avoid low resolutions.
  // Note superblock may still pick 64X64 if y_sad is very small
  // (i.e., y_sad < cpi->vbp_threshold_sad) below. For now leave this as is.
  if ((mi_col >= 8 && mi_col + 8 < mi_params->mi_cols && mi_row >= 8 &&
       mi_row + 8 < mi_params->mi_rows)) {
    int num_skin = 0;
    ...
    const int fac = (bsize == BLOCK_8X8) ? 2 : 4;
    int num_blks = 0;
    // Loop through the 8x8 sub-blocks.
    int i, j;
    for (i = 0; i < ymis; i += fac) {
      for (j = 0; j < xmis; j += fac) {
...
static AOM_INLINE void encode_nonrd_sb(AV1_COMP *cpi, ThreadData *td,
  ...
  if (cpi->sf.rt_sf.use_skinmap_detection && !seg_skip &&
    ...
    av1_compute_skin_sb(cpi, BLOCK_16X16, mi_row, mi_col,
                        x->content_state_sb.source_sad_nonrd <= kLowSad);
    x->sb_is_skin = skin_sb_split(cpi, BLOCK_16X16, mi_row, mi_col);
  }
