...
static AOM_INLINE void rectangular_partition_search(
  ...
  int is_not_edge_block[NUM_RECT_PARTS] = { blk_params.has_rows,
                                            blk_params.has_cols };
  ...
  for (RECT_PART_TYPE i = HORZ; i < NUM_RECT_PARTS; i++) {
    ...
    if (sum_rdc->rdcost < best_rdc->rdcost && is_not_edge_block[i]) {
      ...
      update_state(cpi, td, cur_ctx[i][sub_part_idx][0], blk_params.mi_row,
                   blk_params.mi_col, blk_params.subsize, 1);
...
BEGIN_PARTITION_SEARCH:
  ...
  // PARTITION_HORZ_A
  if (!part_search_state.terminate_partition_search &&
	part_search_state.partition_rect_allowed[HORZ] && horza_partition_allowed) {
  ...
  // PARTITION_HORZ_B
  if (!part_search_state.terminate_partition_search &&
      part_search_state.partition_rect_allowed[HORZ] && horzb_partition_allowed) {
    ...
    part_search_state.found_best_partition |= rd_test_partition3(
        ...
        mi_row, mi_col, bsize, PARTITION_HORZ_B, mi_row, mi_col,
        blk_params.subsize, blk_params.mi_row_edge, mi_col,
        blk_params.split_bsize2, blk_params.mi_row_edge, blk_params.mi_col_edge,
  ...
  // PARTITION_VERT_A
  if (!part_search_state.terminate_partition_search &&
      part_search_state.partition_rect_allowed[VERT] && verta_partition_allowed) {
  ...
  // PARTITION_VERT_B
  if (!part_search_state.terminate_partition_search &&
      part_search_state.partition_rect_allowed[VERT] && vertb_partition_allowed) {
    ...
    part_search_state.found_best_partition |= rd_test_partition3(
        ...
        mi_col, bsize, PARTITION_VERT_B, mi_row, mi_col, blk_params.subsize,
        mi_row, blk_params.mi_col_edge, blk_params.split_bsize2,
