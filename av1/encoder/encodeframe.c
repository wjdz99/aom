...
static AOM_INLINE void encode_rd_sb(AV1_COMP *cpi, ThreadData *td,
  ...
  if (sf->rt_sf.var_part_based_on_sse) {
    if (!cpi->sf.rt_sf.check_scene_detection || cpi->rc.frame_source_sad > 0)
      av1_source_content_sb(cpi, x, mi_row, mi_col);
    else
      x->content_state_sb.source_sad_set2 = kZeroSad;
  }
...
static AOM_INLINE void encode_sb_row(AV1_COMP *cpi, ThreadData *td,
  ...
  for (int mi_col = tile_info->mi_col_start, sb_col_in_tile = 0;
    ...
    x->content_state_sb.source_sad_set1 = kMedSad;
    x->content_state_sb.source_sad_set2 = kMedSad;
