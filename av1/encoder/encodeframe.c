...
static AOM_INLINE int is_calc_src_content_needed(AV1_COMP *cpi,
  ...
  // TODO(yunqing): for 128x128 sb size, need to consider 4 64x64 results.
  if (cpi->src_sad_blk_64x64 != NULL && AOMMIN(cm->width, cm->height) < 360) {
    ...
    // The threshold is determined based on kLowSad and kHighSad threshold and test results.
    const uint64_t thresh_low = 15000;
    const uint64_t thresh_high = 35000;
    const uint64_t blk_sad = cpi->src_sad_blk_64x64[sbi_col + sbi_row * sb_cols];

    if (blk_sad > thresh_low) {
      if (blk_sad > thresh_high)
      x->content_state_sb.source_sad_nonrd = kHighSad;
      do_calc_src_content = 0;
    }
    //printf("\n   sad: %ld; %d;       ", cpi->src_sad_blk_64x64[sbi_col + sbi_row * sb_cols], do_calc_src_content);
  }
