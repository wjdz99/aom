...
static AOM_INLINE void encode_nonrd_sb(AV1_COMP *cpi, ThreadData *td,
  ...
  if (cm->current_frame.frame_type != KEY_FRAME) {
    ...
    for (int k = 0; k < 4; ++k) {
      ...
      for (int i = 0; i < AOMMIN(sb_mis, cm->mi_params.mi_rows - mi_row);
           i += step) {
        for (int j = 0; j < AOMMIN(sb_mis, cm->mi_params.mi_cols - mi_col);
             j += step) {
          ...
          if (b[0].mv[0].as_int == b[1].mv[0].as_int &&
              ...
              b[0].mv[1].as_int == b[1].mv[1].as_int &&
            ...
            for (int y = 0; y < step; y++) {
              for (int x_idx = 0; x_idx < step; x_idx++) {
                if ((xd->mb_to_right_edge >> (3 + MI_SIZE_LOG2)) + step >
                        x_idx &&
                    (xd->mb_to_bottom_edge >> (3 + MI_SIZE_LOG2)) + step > y) {
