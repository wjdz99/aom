...
// Can be removed as the sum_rdc is local variable.
#if 1
  av1_rd_cost_update(x->rdmult, &sum_rdc);
...
// AB partition context setting and initialize params.
static AOM_INLINE void init_ab_part_params(const AV1_COMMON *cm,
                                           PICK_MODE_CONTEXT *dst_ctxs[3],
                                           ThreadData *td,
                                           const BLOCK_SIZE ab_subsize[3]) {
  // Set AB partition context.
  dst_ctxs[0] = av1_alloc_pmc(cm, ab_subsize[0], &td->shared_coeff_buf);
  dst_ctxs[1] = av1_alloc_pmc(cm, ab_subsize[1], &td->shared_coeff_buf);
  dst_ctxs[2] = av1_alloc_pmc(cm, ab_subsize[2], &td->shared_coeff_buf);

  // Set rd mode as not ready.
  dst_ctxs[0]->rd_mode_is_ready = 0;
  dst_ctxs[1]->rd_mode_is_ready = 0;
  dst_ctxs[2]->rd_mode_is_ready = 0;
}
...
BEGIN_PARTITION_SEARCH:
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    blk_params.subsize = get_partition_subsize(bsize, PARTITION_HORZ_A);
    const BLOCK_SIZE ab_subsize[SUB_PARTITIONS_AB] = { blk_params.split_bsize2,
    ...
    init_ab_part_params(cm, pc_tree->horizontala, td, ab_subsize);
    ...
    rd_pick_ab_part(cpi, td, tile_data, tp, x, &x_ctx, pc_tree,
                    pc_tree->horizontala, &part_search_state, &best_rdc,
                    ab_subsize, ab_mi_pos, PARTITION_HORZ_A);
  }
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    blk_params.subsize = get_partition_subsize(bsize, PARTITION_HORZ_B);
    const BLOCK_SIZE ab_subsize[SUB_PARTITIONS_AB] = {
      blk_params.subsize, blk_params.split_bsize2, blk_params.split_bsize2
    };
    const int ab_mi_pos[SUB_PARTITIONS_AB][2] = {
      { mi_row, mi_col },
      { blk_params.mi_row_edge, mi_col },
    ...
    init_ab_part_params(cm, pc_tree->horizontalb, td, ab_subsize);
    ...
    rd_pick_ab_part(cpi, td, tile_data, tp, x, &x_ctx, pc_tree,
                    pc_tree->horizontalb, &part_search_state, &best_rdc,
                    ab_subsize, ab_mi_pos, PARTITION_HORZ_B);
  }
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    blk_params.subsize = get_partition_subsize(bsize, PARTITION_VERT_A);
    const BLOCK_SIZE ab_subsize[SUB_PARTITIONS_AB] = { blk_params.split_bsize2,
                                                       blk_params.split_bsize2,
                                                       blk_params.subsize };
    const int ab_mi_pos[SUB_PARTITIONS_AB][2] = {
    ...
    init_ab_part_params(cm, pc_tree->verticala, td, ab_subsize);
    ...
    rd_pick_ab_part(cpi, td, tile_data, tp, x, &x_ctx, pc_tree,
                    pc_tree->verticala, &part_search_state, &best_rdc,
                    ab_subsize, ab_mi_pos, PARTITION_VERT_A);
  }
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    blk_params.subsize = get_partition_subsize(bsize, PARTITION_VERT_B);
    const BLOCK_SIZE ab_subsize[SUB_PARTITIONS_AB] = {
      blk_params.subsize, blk_params.split_bsize2, blk_params.split_bsize2
    };
    const int ab_mi_pos[SUB_PARTITIONS_AB][2] = {
    ...
    init_ab_part_params(cm, pc_tree->verticalb, td, ab_subsize);
    ...
    rd_pick_ab_part(cpi, td, tile_data, tp, x, &x_ctx, pc_tree,
                    pc_tree->verticalb, &part_search_state, &best_rdc,
                    ab_subsize, ab_mi_pos, PARTITION_VERT_B);
  }
