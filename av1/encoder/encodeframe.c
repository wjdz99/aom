...
static AOM_INLINE void encode_frame_internal(AV1_COMP *cpi) {
  ...
  if (!is_stat_generation_stage(cpi) && av1_use_hash_me(cpi) &&
    ...
    int k, j;
    bool error = false, mem_alloc_failed = false;

    for (k = 0; k < 2; k++) {
      for (j = 0; j < 2; j++) {
        block_hash_values[k][j] =
            (uint32_t *)aom_malloc(sizeof(uint32_t) * pic_width * pic_height);
      ...
      for (j = 0; j < 3; j++) {
        is_block_same[k][j] =
            (int8_t *)aom_malloc(sizeof(int8_t) * pic_width * pic_height);
        if (!is_block_same[k][j]) {
          mem_alloc_failed = true;
          goto Error;
        }
      }
  ...
  Error:
    ...
    if (mem_alloc_failed) {
      aom_internal_error(
          ...
          "Error allocating block_hash_values & is_block_same buffers");
    }
    if (error) {
      aom_internal_error(cm->error, AOM_CODEC_MEM_ERROR,
