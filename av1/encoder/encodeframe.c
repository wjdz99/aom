...
static AOM_INLINE int do_not_wait_for_top_right_sb(const AV1_COMP *const cpi) {
...
static AOM_INLINE void encode_sb_row(AV1_COMP *cpi, ThreadData *td,
  ...
  for (int mi_col = tile_info->mi_col_start, sb_col_in_tile = 0;
    ...
    (*(enc_row_mt->sync_read_ptr))(
        row_mt_sync, sb_row,
        sb_col_in_tile - do_not_wait_for_top_right_sb(cpi));
  ...
  for (tile_row = 0; tile_row < tile_rows; ++tile_row) {
    for (tile_col = 0; tile_col < tile_cols; ++tile_col) {
      ...
      tile_data->allow_update_cdf = tile_data->allow_update_cdf &&
                                    ...
                                    !do_not_wait_for_top_right_sb(cpi);
