...
static AOM_INLINE void split_partition_search(
  ...
  // blk_params.subsize = get_partition_subsize(blk_params.bsize,
  // PARTITION_SPLIT); assert(blk_params.subsize <= BLOCK_LARGEST);
  for (int i = 0; i < SPLIT_SUB_PARTITIONS; ++i) {
    if (pc_tree->split[i] == NULL)
      pc_tree->split[i] = av1_alloc_pc_tree_node(
          get_partition_subsize(blk_params.bsize, PARTITION_SPLIT));
    pc_tree->split[i]->index = i;
  }
  ...
  for (idx = 0; idx < SPLIT_SUB_PARTITIONS && sum_rdc.rdcost < best_rdc->rdcost;
    ...
    if (!rd_pick_partition(
            cpi, td, tile_data, tp, mi_row + y_idx, mi_col + x_idx,
            get_partition_subsize(blk_params.bsize, PARTITION_SPLIT),
