...
/*!\brief Calculate the SAD at superblock level using SAD stat
 *
 * \ingroup partition_search
 * \callgraph
 * \callergraph
 */
static int64_t get_sb_source_sad(AV1_COMP *cpi, MACROBLOCK *const x, int mi_row,
                                 int mi_col) {
  ...
  int64_t curr_sb_sad = -1;
  const int64_t *const src_sad_blk_64x64_data =
...
static AOM_INLINE bool is_calc_src_content_needed(AV1_COMP *cpi,
                                                  MACROBLOCK *const x,
                                                  int mi_row, int mi_col) {
  const int64_t curr_sb_sad = get_sb_source_sad(cpi, x, mi_row, mi_col);
  if (curr_sb_sad == 0) {
  ...
  if (cpi->src_sad_blk_64x64 != NULL && AOMMIN(cm->width, cm->height) < 360) {
    ...
    // test results.
    const int64_t thresh_low = 15000;
    const int64_t thresh_high = 40000;
    const int64_t blk_sad = cpi->src_sad_blk_64x64[sbi_col + sbi_row * sb_cols];
