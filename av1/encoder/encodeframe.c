...
static AOM_INLINE int is_ab_part_allowed(
    PartitionSearchState *part_search_state, const int ab_partitions_allowed[4],
    const int ab_part_type) {
...
static AOM_INLINE void ab_partitions_search(
    ...
    RD_STATS *best_rdc, const int ab_partitions_allowed[NUM_AB_PARTS]) {
  const AV1_COMMON *const cm = &cpi->common;
  PartitionBlkParams blk_params = part_search_state->part_blk_params;
  const int mi_row = blk_params.mi_row;
  const int mi_col = blk_params.mi_col;
  ...
  PICK_MODE_CONTEXT **mode_srch_ctx[NUM_AB_PARTS][2] = {
    { &pc_tree->split[0]->none, &pc_tree->split[1]->none },
    { &pc_tree->horizontal[0], NULL },
...
BEGIN_PARTITION_SEARCH:
  ...
  const int ab_partitions_allowed[4] = { horza_partition_allowed,
  ...
  ab_partitions_search(cpi, td, tile_data, tp, x, &x_ctx, pc_tree,
                       &part_search_state, &best_rdc, ab_partitions_allowed);
