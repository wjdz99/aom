...
static AOM_INLINE void ab_partitions_search(
  ...
  av1_prune_ab_partitions(
      ...
      part_search_state->partition_rect_allowed[HORZ],
      part_search_state->partition_rect_allowed[VERT], ab_partitions_allowed);
  ...
  for (AB_PART_TYPE ab_part_type = 0; ab_part_type < NUM_AB_PARTS;
    ...
    if (ab_part_type == 0) {
      mode_srch_ctx[0][0] = &pc_tree->split[0]->none;
      mode_srch_ctx[0][1] = &pc_tree->split[1]->none;
    }
    if (ab_part_type == 2) mode_srch_ctx[2][0] = &pc_tree->split[0]->none;
