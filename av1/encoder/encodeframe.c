...
static AOM_INLINE void set_mi_pos_partition4(const int inc_step[2],
                                             int mi_pos[4][2], const int mi_row,
                                             const int mi_col) {
  for (int i = 0; i < 4; i++) {
    mi_pos[i][0] = mi_row + i * inc_step[0];
    mi_pos[i][1] = mi_col + i * inc_step[1];
  }
}
...
static AOM_INLINE void rd_pick_4partition(
    ...
    PartitionSearchState *part_search_state, RD_STATS *best_rdc,
    const int inc_step[2], PARTITION_TYPE partition_type) {
  const AV1_COMMON *const cm = &cpi->common;
  PartitionBlkParams blk_params = part_search_state->part_blk_params;
  // mi positions needed for horz4 and vert4 partition types.
  int mi_pos_check[2] = { cm->mi_params.mi_rows, cm->mi_params.mi_cols };
...
BEGIN_PARTITION_SEARCH:
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    const int inc_step[2] = { mi_size_high[blk_params.bsize] / 4, 0 };
  ...
  if (!part_search_state.terminate_partition_search &&
    ...
    const int inc_step[2] = { 0, mi_size_wide[blk_params.bsize] / 4 };
