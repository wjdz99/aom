...
static INLINE MvSubpelPrecision get_mv_ref_precision(
    const MV mv, const MV ref, MvSubpelPrecision frame_precision) {
  MV ref_ = ref;
  lower_mv_precision(&ref_, frame_precision);
  const MV diff = { (int16_t)(mv.row - ref_.row),
                    (int16_t)(mv.col - ref_.col) };
  return get_mv_precision(diff);
}

static INLINE MvSubpelPrecision
get_mv_ref_precision2(const MV mv, const MV ref, const MV mv2, const MV ref2,
  ...
  const MV diff = { (int16_t)(mv.row - ref_.row),
                    (int16_t)(mv.col - ref_.col) };
  MV ref2_ = ref2;
  lower_mv_precision(&ref2_, frame_precision);
  const MV diff2 = { (int16_t)(mv2.row - ref2_.row),
                     (int16_t)(mv2.col - ref2_.col) };
  return get_mv_precision2(diff, diff2);
}
#endif  // CONFIG_FLEX_MVRES
