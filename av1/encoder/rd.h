...
static INLINE void av1_merge_rd_stats(RD_STATS *rd_stats_dst,
  ...
  rd_stats_dst->sane_check &= rd_stats_src->sane_check;
#endif
}
...
static INLINE void av1_add_reg_stat(RD_STATS *rd_stats, int eob, int64_t rd,
  ...
  assert(rd_stats->sane_check);

  // Size of TX block and SB
  int block_width_mi = mi_size_wide[bsize];
  int block_height_mi = mi_size_high[bsize];
  int crop_width_mi = mi_size_wide[crop_bsize];
  int crop_height_mi = mi_size_high[crop_bsize];

  // Get how much the tx_block in in each quadrant of the macroblock
  int height_in_0 = AOMMIN(block_height_mi/2 - blk_row, crop_height_mi);
  int height_not_in_0 = crop_height_mi - height_in_0;
  int width_in_0 = AOMMIN(block_width_mi/2 - blk_col, crop_width_mi);
  int width_not_in_0 = crop_width_mi - width_in_0;
  ...
  // being truncated.
  int scaling_factor = MAX_MIB_SIZE * MAX_MIB_SIZE /
      (crop_width_mi * crop_height_mi);

  // Update the stats
  rd_stats->eob = eob;
  rd_stats->eob_0 = eob * (height_in_0 * width_in_0) *
      scaling_factor;
  rd_stats->eob_1 = eob * (height_in_0 * width_not_in_0) *
      scaling_factor;
  rd_stats->eob_2 = eob * (height_not_in_0 * width_in_0) *
      scaling_factor;
  rd_stats->eob_3 = eob * (height_not_in_0 * width_not_in_0) *
      scaling_factor;

  rd_stats->rd = rd;
  rd_stats->rd_0 = rd * (height_in_0 * width_in_0) *
      scaling_factor;
  rd_stats->rd_1 = rd * (height_in_0 * width_not_in_0) *
      scaling_factor;
  rd_stats->rd_2 = rd * (height_not_in_0 * width_in_0) *
      scaling_factor;
  rd_stats->rd_3 = rd * (height_not_in_0 * width_not_in_0) *
      scaling_factor;

  rd_stats->y_sse = sse;
  rd_stats->sse_0 = sse * (height_in_0 * width_in_0) *
      scaling_factor;
  rd_stats->sse_1 = sse * (height_in_0 * width_not_in_0) *
      scaling_factor;
  rd_stats->sse_2 = sse * (height_not_in_0 * width_in_0) *
      scaling_factor;
  rd_stats->sse_3 = sse * (height_not_in_0 * width_not_in_0) *
      scaling_factor;
}

static INLINE void av1_reg_stat_skipmode_update(RD_STATS *rd_stats, int rdmult){
  assert(rd_stats->sane_check);
...
static INLINE void av1_copy_reg_stat(RD_STATS *rd_stats_dst,
  ...
  assert(rd_stats_dst->sane_check);
  assert(rd_stats_src->sane_check);
...
static INLINE void av1_unpack_reg_stat(RD_STATS *rd_stats,
                                       int *eob, int *eob_0, int *eob_1,
                                       int *eob_2, int *eob_3,
...
static INLINE void av1_set_reg_stat(RD_STATS *rd_stats,
                                    int eob, int eob_0, int eob_1,
                                    int eob_2, int eob_3,
                                    int64_t rd, int64_t rd_0,
                                    int64_t rd_1, int64_t rd_2,
