...
static INLINE void av1_add_reg_stat(RD_STATS *rd_stats, int eob, int64_t rd,
  ...
  if (crop_width_mi <= block_width_mi/2 && crop_height_mi <= block_width_mi/2){
    // The transform block lies completely in a quadrant.
    const int factor =  (crop_width_mi * crop_height_mi) * scaling_factor;
    const int r_eob = eob * factor, r_rd = rd * factor, r_sse = sse * factor;;
  ...
  } else if (crop_width_mi == block_width_mi/2) {
    // The tranform block is a vertical block half the size of predition block
    const int factor =  (crop_width_mi * crop_height_mi) * scaling_factor / 2;
    const int r_eob = eob * factor, r_rd = rd * factor, r_sse = sse * factor;;
  ...
  } else if (crop_height_mi == block_height_mi/2) {
    ...
    const int factor =  (crop_width_mi * crop_height_mi) * scaling_factor / 4;
    const int r_eob = eob * factor, r_rd = rd * factor, r_sse = sse * factor;;
