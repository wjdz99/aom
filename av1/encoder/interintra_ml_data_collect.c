...
static int calc_intra_border(const AV1_COMP *const cpi, MACROBLOCK *const x,
                             BLOCK_SIZE bsize) {
  MACROBLOCKD *const xd = &x->e_mbd;
  ...
  for (int plane = AOM_PLANE_Y; plane <= AOM_PLANE_V; ++plane) {
    border = std::min(border, av1_intra_top_available(xd, plane));
    border = std::min(border, av1_intra_left_available(xd, plane));
...
bool av1_interintra_ml_data_collect_valid(const AV1_COMP *const cpi,
  ...
  const int inter_border =
      av1_calc_border(xd, AOM_PLANE_Y, false /* build for obmc */);
  const int intra_border = calc_intra_border(cpi, x, bsize);
  return inter_border >= BORDER_SIZE && intra_border >= BORDER_SIZE && !x->skip;
}
