...
void av1_interintra_ml_data_collect(const AV1_COMP *const cpi,
  ...
  assert(INFO_U == NULL);
  assert(INFO_V == NULL);
  INFO_Y = init_plane_info(cpi, x, bsize, AOM_PLANE_Y, border);
  INFO_U = init_plane_info(cpi, x, bsize, AOM_PLANE_U, border);
  INFO_V = init_plane_info(cpi, x, bsize, AOM_PLANE_V, border);
  assert(INFO_Y != NULL);
  assert(INFO_U != NULL);
  assert(INFO_V != NULL);
}
