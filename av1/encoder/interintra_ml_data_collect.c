...
void av1_interintra_ml_data_collect(const AV1_COMP *const cpi,
  ...
  assert(INFO_U == NULL);
  assert(INFO_V == NULL);
  process_block(INFO_Y, cpi, x, bsize, AOM_PLANE_Y, border);
  process_block(INFO_U, cpi, x, bsize, AOM_PLANE_U, border);
  process_block(INFO_V, cpi, x, bsize, AOM_PLANE_V, border);
  assert(INFO_Y != NULL);
  assert(INFO_U != NULL);
  assert(INFO_V != NULL);
}
