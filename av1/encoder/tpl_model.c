...
static AOM_INLINE void mode_estimation(AV1_COMP *cpi, ThreadData *td,
  ...
  TplBuffers *const tpl_tmp_buffers = &td->tpl_tmp_buffers;
...
// a row
void av1_mc_flow_dispenser_row(AV1_COMP *cpi, ThreadData *td, MACROBLOCK *x,
  ...
  for (int mi_col = 0, tplb_col_in_tile = 0; mi_col < mi_params->mi_cols;
    ...
    mode_estimation(cpi, td, x, mi_row, mi_col, bsize, tx_size, &tpl_stats);
...
static AOM_INLINE void mc_flow_dispenser(AV1_COMP *cpi) {
  ...
  for (int mi_row = 0; mi_row < mi_params->mi_rows; mi_row += mi_height) {
    ...
    av1_mc_flow_dispenser_row(cpi, td, x, mi_row, bsize, tx_size);
  }
}
...
int av1_tpl_setup_stats(AV1_COMP *cpi, int gop_eval,
  ...
  if (!tpl_alloc_temp_buffers(tpl_tmp_buffers,
