...
static AOM_INLINE void mc_flow_dispenser(AV1_COMP *cpi, int frame_idx,
  ...
  for (idx = 0; idx < INTER_REFS_PER_FRAME; ++idx) {
    ref_frame[idx] =
        cpi->tpl_data.tpl_frame[tpl_frame->ref_map_index[idx]].rec_picture;
    src_frame[idx] =
        cpi->tpl_data.tpl_frame[tpl_frame->ref_map_index[idx]].gf_picture;
  }
...
static AOM_INLINE void init_gop_frames_for_tpl(
  ...
  *pframe_qindex = 0;
  ...
  for (int i = 0; i < REF_FRAMES; ++i) {
    if (frame_params.frame_type == KEY_FRAME || gop_eval) {
      cpi->tpl_data.tpl_frame[-i - 1].gf_picture = NULL;
      cpi->tpl_data.tpl_frame[-1 - 1].rec_picture = NULL;
      cpi->tpl_data.tpl_frame[-i - 1].frame_display_index = 0;
    } else {
      cpi->tpl_data.tpl_frame[-i - 1].gf_picture = &cm->ref_frame_map[i]->buf;
      cpi->tpl_data.tpl_frame[-i - 1].rec_picture = &cm->ref_frame_map[i]->buf;
      cpi->tpl_data.tpl_frame[-i - 1].frame_display_index =
  ...
  *tpl_group_frames = cur_frame_idx;
  ...
  for (gf_index = cur_frame_idx; gf_index <= gop_length; ++gf_index) {
    TplDepFrame *tpl_frame = &cpi->tpl_data.tpl_frame[gf_index];
    ...
    if (frame_update_type != OVERLAY_UPDATE &&
        frame_update_type != INTNL_OVERLAY_UPDATE) {
      tpl_frame->rec_picture = &cpi->tpl_data.tpl_rec_pool[process_frame_count];
      tpl_frame->tpl_stats_ptr =
          cpi->tpl_data.tpl_stats_pool[process_frame_count];
      ++process_frame_count;
    }
...
static AOM_INLINE void init_tpl_stats(AV1_COMP *cpi) {
  for (int frame_idx = 0; frame_idx < MAX_LAG_BUFFERS; ++frame_idx) {
    TplDepFrame *tpl_frame = &cpi->tpl_data.tpl_stats_buffer[frame_idx];
    memset(cpi->tpl_data.tpl_stats_pool[frame_idx], 0,
...
int av1_tpl_setup_stats(AV1_COMP *cpi, int gop_eval,
  ...
  for (int frame_idx = gf_group->index; frame_idx < tpl_gf_group_frames;
    ...
    aom_extend_frame_borders(cpi->tpl_data.tpl_frame[frame_idx].rec_picture,
                             av1_num_planes(cm));
  }
...
void av1_tpl_rdmult_setup(AV1_COMP *cpi) {
  ...
  const TplDepFrame *const tpl_frame = &cpi->tpl_data.tpl_frame[tpl_idx];
