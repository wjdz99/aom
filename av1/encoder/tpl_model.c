...
static void init_gop_frames_for_tpl(AV1_COMP *cpi, GF_PICTURE *gf_picture,
  ...
  *tpl_group_frames = 0;
  ...
  for (i = 0; i < 7; ++i) gf_picture[1].ref_frame[i] = -1;
  ...
  for (frame_idx = 2; frame_idx < MAX_LAG_BUFFERS; ++frame_idx) {
    ...
    for (i = 0; i < 7; ++i) gf_picture[frame_idx].ref_frame[i] = -1;
    ...
    if (lst_queue_size > 0) {
      for (int idx = lst_queue_size - 1; idx >= 0; --idx) {
        int ref_frame_type_idx = LAST_FRAME + (lst_queue_size - 1 - idx);
