...
// Take neighbor MVs into account when traversing through blocks.
static INLINE int_mv optimize_for_neighbor_mvs(const TplDepFrame *tpl_frame,
  ...
  double current_mv_magnitude =
      sqrt(pow(current_mv.as_mv.row, 2.0) + pow(current_mv.as_mv.col, 2.0));

  // Retrieve the up and left neighbors.
  double up_error = DBL_MAX;
  int_mv up_mv;
  if (row - step >= 0) {
    tpl_stats = &tpl_frame->tpl_stats_ptr[av1_tpl_ptr_pos(
        row - step, col, tpl_stride, right_shift)];
    up_mv = tpl_stats->mv[tpl_stats->ref_frame_index[0]];
    up_mv.as_mv.row = current_mv.as_mv.row - up_mv.as_mv.row;
    up_mv.as_mv.col = current_mv.as_mv.col - up_mv.as_mv.col;
    up_error =
        sqrt(pow(abs(up_mv.as_mv.row), 2.0) + pow(abs(up_mv.as_mv.col), 2.0));
  }

  double left_error = DBL_MAX;
  int_mv left_mv;
  if (col - step >= 0) {
    tpl_stats = &tpl_frame->tpl_stats_ptr[av1_tpl_ptr_pos(
        row, col - step, tpl_stride, right_shift)];
    left_mv = tpl_stats->mv[tpl_stats->ref_frame_index[0]];
    left_mv.as_mv.row = current_mv.as_mv.row - left_mv.as_mv.row;
    left_mv.as_mv.col = current_mv.as_mv.col - left_mv.as_mv.col;
    left_error = sqrt(pow(abs(left_mv.as_mv.row), 2.0) +
                      pow(abs(left_mv.as_mv.col), 2.0));
  }
...
double av1_tpl_compute_frame_mv_entropy(const TplDepFrame *tpl_frame,
  ...
  for (int row = 0; row < tpl_frame->mi_rows; row += step) {
    for (int col = 0; col < tpl_frame->mi_cols; col += step) {
      int_mv mv = optimize_for_neighbor_mvs(tpl_frame, row, col, step,
