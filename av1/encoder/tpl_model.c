  ...
  *tpl_group_frames = cur_frame_idx;
  ...
  for (gf_index = cur_frame_idx; gf_index < gop_length; ++gf_index) {
    ...
    if (gf_index == cur_frame_idx) {
      ...
      if (buf == NULL && gop_eval) {
        fprintf(stderr, "look ahead index = %d, arf offset = %d, anc offset = %d\n",
          lookahead_index, gf_group->arf_src_offset[gf_index], anc_frame_offset);
...
int av1_tpl_setup_stats(AV1_COMP *cpi, int gop_eval,
  ...
  // factors.
  return (beta[0] >= beta[1] + 0.7) && beta[0] > 7.0;
}
