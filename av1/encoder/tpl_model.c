...
#define MC_FLOW_BSIZE32 0

#if MC_FLOW_BSIZE32
#define MC_FLOW_NUM_PELS 1024  // 32x32
#else
#define MC_FLOW_NUM_PELS 256   // Assume 16x16
#endif  // MC_FLOW_BSIZE32
...
static void mode_estimation(AV1_COMP *cpi, MACROBLOCK *x, MACROBLOCKD *xd,
  ...
  for (rf_idx = 0; rf_idx < 7; ++rf_idx) {
    ...
    inter_cost_weighted = (int64_t)(
        (double)inter_cost * (delta_q < 0 ? 0.25 + 0.75 * exp(delta_q / 10)
                                          : 4.0 - 3.0 * exp(-delta_q / 10)) +
        0.5);
