...
// This function will process the row level operations of mc_flow_dispenser
static AOM_INLINE void mc_flow_row(AV1_COMP *cpi, MACROBLOCK *x, int mi_row,
...
static AOM_INLINE void mc_flow_dispenser(AV1_COMP *cpi) {
  ...
  for (int mi_row = 0; mi_row < mi_params->mi_rows; mi_row += mi_height) {
    ...
    mc_flow_row(cpi, x, mi_row, bsize, tx_size);
  }
}
