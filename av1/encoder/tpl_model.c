...
void get_tpl_valid_list(const TplParams *tpl_data, int gop_size, int* valid_list) {
  for(int i = 0; i < gop_size; ++i) {
    valid_list[i] = av1_tpl_stats_ready(tpl_data, i);
  }
}
...
int av1_q_mode_estimate_base_q(const GF_GROUP *gf_group,
  ...
  double q_max_estimate = av1_estimate_gop_bitrate(q_index_list, stats_valid_list, gf_group->size,
                                                   txfm_stats_list, NULL);
  av1_q_mode_compute_gop_q_indices(gf_frame_index, q_min, arf_qstep_ratio,
                                   bit_depth, gf_group, q_index_list);
  double q_min_estimate = av1_estimate_gop_bitrate(q_index_list, stats_valid_list, gf_group->size,
                                                   txfm_stats_list, NULL);
  ...
  while (true) {
    ...
    double estimate = av1_estimate_gop_bitrate(q_index_list, stats_valid_list, gf_group->size,
                                               txfm_stats_list, NULL);
  ...
  av1_q_mode_compute_gop_q_indices(gf_frame_index, q, arf_qstep_ratio,
                                   bit_depth, gf_group, q_index_list);
  av1_estimate_gop_bitrate(q_index_list, stats_valid_list, gf_group->size, txfm_stats_list,
...
void av1_vbr_rc_update_q_index_list(VBR_RATECTRL_INFO *vbr_rc_info,
  ...
  if (gf_frame_index == 0) {
    ...
    get_tpl_valid_list(tpl_data, gf_group->size, stats_valid_list);
    av1_q_mode_estimate_base_q(
        gf_group, tpl_data->txfm_stats_list, stats_valid_list, gop_bit_budget, gf_frame_index,
