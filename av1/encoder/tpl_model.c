...
static AOM_INLINE void mode_estimation(AV1_COMP *cpi, MACROBLOCK *x, int mi_row,
  ...
  if (best_cmp_rf_idx >= 0) {
    tpl_stats->recrf_dist = 0;
    tpl_stats->recrf_rate = 0;
    ...
    tpl_stats->recrf_dist += tpl_stats->cmp_recrf_dist[0];
    tpl_stats->recrf_rate += tpl_stats->cmp_recrf_rate[0];
    ...
    tpl_stats->recrf_dist += tpl_stats->cmp_recrf_dist[1];
    tpl_stats->recrf_rate += tpl_stats->cmp_recrf_rate[1];    
    ...
    if (!is_inter_mode(best_mode)) {
      ...
      tpl_stats->srcrf_rate = tpl_stats->recrf_rate;
    }
  } else {
    ref_frame_ptr[0] =
        best_mode == NEW_NEWMV
  ...
  if (best_rf_idx >= 0 || best_cmp_rf_idx >= 0) {
...
static AOM_INLINE void tpl_model_update_b(TplParams *const tpl_data, int mi_row,
  ...
  int64_t recrf_dist = is_compound ? tpl_stats_ptr->cmp_recrf_dist[ref]
                                   : tpl_stats_ptr->recrf_dist;
  int64_t recrf_rate = is_compound ? tpl_stats_ptr->cmp_recrf_rate[ref]
                                   : tpl_stats_ptr->recrf_rate;

  int64_t cur_dep_dist = recrf_dist - tpl_stats_ptr->srcrf_dist;
  int64_t mc_dep_dist =
      (int64_t)(tpl_stats_ptr->mc_dep_dist *
                ((double)(recrf_dist - tpl_stats_ptr->srcrf_dist) /
                 tpl_stats_ptr->recrf_dist));
  int64_t delta_rate = recrf_rate - tpl_stats_ptr->srcrf_rate;
  int64_t mc_dep_rate = delta_rate_cost(tpl_stats_ptr->mc_dep_rate, recrf_dist,
...
static AOM_INLINE void tpl_model_store(TplDepStats *tpl_stats_ptr, int mi_row,
  ...
  for (int idy = 0; idy < mi_height; idy += step) {
    ...
    for (int idx = 0; idx < mi_width; idx += step) {
      ...
      tpl_ptr->cmp_recrf_dist[0] = src_stats->cmp_recrf_dist[0] / div;
      tpl_ptr->cmp_recrf_dist[1] = src_stats->cmp_recrf_dist[1] / div;
      tpl_ptr->cmp_recrf_rate[0] = src_stats->cmp_recrf_rate[0] / div;
      tpl_ptr->cmp_recrf_rate[1] = src_stats->cmp_recrf_rate[1] / div;
