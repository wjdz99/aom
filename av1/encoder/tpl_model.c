...
static AOM_INLINE void tpl_model_update_b(TplParams *const tpl_data, int mi_row,
  ...
  int64_t d[2] = {
    tpl_stats_ptr->recrf_dist - tpl_stats_ptr->cmp_recrf_dist[1],
    tpl_stats_ptr->recrf_dist - tpl_stats_ptr->cmp_recrf_dist[0],
  };

  double dist_scale = is_compound ? (double)d[ref] / (d[0] + d[1] + 1) : 1.0;
  int64_t cur_dep_dist =
      (int64_t)((tpl_stats_ptr->recrf_dist - srcrf_dist) * dist_scale);
