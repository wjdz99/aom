...
static AOM_INLINE int tpl_use_multithread(const AV1_COMP *cpi) {
  return cpi->mt_info.num_workers > 1;
}
...
static AOM_INLINE void tpl_stats_update_abs_coeff_mean(
  ...
  printf("\n %d, ", tpl_txfm_stats->txfm_block_count);
  for (int i = 0; i < tpl_frame->coeff_num; ++i) {
    ...
    printf("%.4f, ", tpl_frame->abs_coeff_sum[i]);
  }
}
...
int av1_tpl_setup_stats(AV1_COMP *cpi, int gop_eval,
  ...
  for (int frame_idx = gf_group->index; frame_idx < tpl_gf_group_frames;
    ...
    tpl_stats_update_abs_coeff_mean(&cpi->tpl_data, &cpi->td.tpl_txfm_stats);
