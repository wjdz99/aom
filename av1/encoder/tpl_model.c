...
static AOM_INLINE void tpl_model_update_b(TplParams *const tpl_data, int mi_row,
  ...
  // if (is_compound) {
  //   fprintf(stderr, "cmp dist (%lld, %lld), rec dist %lld, src dist %lld\n",
  //   tpl_stats_ptr->cmp_recrf_dist[0],tpl_stats_ptr->cmp_recrf_dist[1],
  ...
  int64_t cur_dep_dist =
      is_compound ? AOMMAX(0, tpl_stats_ptr->recrf_dist -
                                  tpl_stats_ptr->cmp_recrf_dist[!ref])
                  : tpl_stats_ptr->recrf_dist - tpl_stats_ptr->srcrf_dist;

  double prop_factor =
      is_compound
          ? (double)(AOMMAX(0, tpl_stats_ptr->recrf_dist -
                                   tpl_stats_ptr->cmp_recrf_dist[!ref])) /
                tpl_stats_ptr->recrf_dist
          : (double)(recrf_dist - tpl_stats_ptr->srcrf_dist) / recrf_dist;
  int64_t mc_dep_dist = (int64_t)(tpl_stats_ptr->mc_dep_dist * prop_factor);
  int64_t delta_rate =
      is_compound ? AOMMAX(0, tpl_stats_ptr->recrf_rate -
                                  tpl_stats_ptr->cmp_recrf_rate[!ref])
                  : tpl_stats_ptr->recrf_rate - tpl_stats_ptr->srcrf_rate;

  int64_t mc_dep_rate =
      delta_rate_cost(tpl_stats_ptr->mc_dep_rate, tpl_stats_ptr->recrf_dist,
                      is_compound ? tpl_stats_ptr->cmp_recrf_dist[!ref]
