...
static AOM_INLINE void init_gop_frames_for_tpl(
  ...
  AV1_COMMON *cm = &cpi->common;
  int cur_frame_idx = cpi->cur_gf_index;
  *pframe_qindex = 0;
...
int av1_tpl_setup_stats(AV1_COMP *cpi, int gop_eval,
  ...
  for (int gf_index = cpi->cur_gf_index; gf_index < gf_group->size;
       ++gf_index) {
  ...
  for (int frame_idx = cpi->cur_gf_index; frame_idx < tpl_gf_group_frames;
       ++frame_idx) {
  ...
  for (int frame_idx = tpl_gf_group_frames - 1; frame_idx >= cpi->cur_gf_index;
       --frame_idx) {
    if (gf_group->update_type[frame_idx] == INTNL_OVERLAY_UPDATE ||
        gf_group->update_type[frame_idx] == OVERLAY_UPDATE)
      continue;
