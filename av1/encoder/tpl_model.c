...
static void mode_estimation(AV1_COMP *cpi, MACROBLOCK *x, MACROBLOCKD *xd,
  ...
  tpl_stats->ref_disp_frame_index = cpi->gf_group.frame_disp_idx[idx];
...
#error "Invalid block size for tpl model"
  ...
  const int tpl_cur_idx = cpi->gf_group.frame_disp_idx[gf_group->index];
  ...
  for (int idx = gf_group->index + 1; idx < cpi->tpl_gf_group_frames; ++idx) {
    const int tpl_future_idx = cpi->gf_group.frame_disp_idx[idx];
    ...
    for (int ridx = 0; ridx < INTER_REFS_PER_FRAME; ++ridx) {
      ...
      const int tpl_ref_idx = cpi->gf_group.frame_disp_idx[ref_idx];
