...
static AOM_INLINE void mode_estimation_nrs(AV1_COMP *cpi, MACROBLOCK *x, int mi_row,
...
static AOM_INLINE void init_mc_flow_dispenser_nrs(AV1_COMP *cpi, int frame_idx,
  ...
  int ref_frame_flags_base = get_ref_frame_flags(&cpi->sf, ref_frames_ordered,
  ...
  *pframe_qindex = 0;
  ...
  for (int i = 0; i < INTER_REFS_PER_FRAME_NRS; ++i) {
    if (frame_params.frame_type == KEY_FRAME || gop_eval) {
      ...
      assert(cpi->tpl_data.tpl_frame[-1 - 1].rec_picture == tpl_data->tpl_frame[-1 - 1].rec_picture);
    } else {
      ...
      assert(cpi->tpl_data.tpl_frame[-i - 1].rec_picture == tpl_data->tpl_frame[-i - 1].rec_picture);
    }
    assert(cpi->tpl_data.tpl_frame[-i - 1].gf_picture == tpl_data->tpl_frame[-i - 1].gf_picture);
    assert(cpi->tpl_data.tpl_frame[-i - 1].frame_display_index == tpl_data->tpl_frame[-i - 1].frame_display_index);
  ...
  *tpl_group_frames = cur_frame_idx;
  ...
  for (gf_index = cur_frame_idx; gf_index < gop_length; ++gf_index) {
    ...
    tpl_frame->frame_display_index =
        frame_display_index + cm->current_frame.frame_number - anc_frame_offset;
    assert(tpl_data->tpl_frame[gf_index].frame_display_index == cpi->tpl_data.tpl_frame[gf_index].frame_display_index);
    ...
    int refresh_frame_map_index = av1_get_refresh_ref_frame_map_nrs(refresh_mask);
  ...
  for (;
    ...
    assert(tpl_frame->gf_picture == cpi->tpl_data.tpl_frame[gf_index].gf_picture);
    ...
    tpl_frame->frame_display_index =
        frame_display_index + cm->current_frame.frame_number - anc_frame_offset;
    assert(tpl_frame->frame_display_index == cpi->tpl_data.tpl_frame[gf_index].frame_display_index);
...
void av1_tpl_setup_stats(AV1_COMP *cpi, int gop_eval,
  ...
  init_gop_frames_for_tpl_nrs(cpi, frame_params, gf_group, gop_eval,
                          &tpl_gf_group_frames_nrs, frame_input, &pframe_qindex);
