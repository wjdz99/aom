...
double av1_tpl_get_qstep_ratio(const TplParams *tpl_data, int gf_frame_index) {
  if (av1_tpl_stats_ready(tpl_data, gf_frame_index) == 0) {
    return 1;
  }
...
void av1_vbr_rc_update_q_index_list(VBR_RATECTRL_INFO *vbr_rc_info,
  ...
  if (gf_frame_index == 0) {
    // We only estimate leaf_qindex when gf_frame_index == 0 to avoid decision
    // conflict
    ...
    // This could happen when this is ARF in the first GOP
    // We will reuse the leaf_qindex with updated qstep_ratio
