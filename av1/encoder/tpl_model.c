...
static AOM_INLINE void init_mc_flow_dispenser(AV1_COMP *cpi, int frame_idx,
  ...
  cpi->tpl_data.frame_idx = frame_idx;
  tpl_reset_src_ref_frames(&cpi->tpl_data);
  av1_tile_init(&xd->tile, cm, 0, 0);

  // Setup scaling factor
  av1_setup_scale_factors_for_frame(
      &cpi->tpl_data.sf, this_frame->y_crop_width, this_frame->y_crop_height,
      this_frame->y_crop_width, this_frame->y_crop_height);

  xd->cur_buf = this_frame;

  for (idx = 0; idx < INTER_REFS_PER_FRAME; ++idx) {
    cpi->tpl_data.ref_frame[idx] =
        tpl_data->tpl_frame[tpl_frame->ref_map_index[idx]].rec_picture;
    cpi->tpl_data.src_ref_frame[idx] =
  ...
  for (idx = 0; idx < INTER_REFS_PER_FRAME; ++idx) {
    if ((ref_frame_flags & (1 << idx)) == 0) {
      cpi->tpl_data.ref_frame[idx] = NULL;
    }
  }
  ...
  xd->block_ref_scale_factors[0] = &cpi->tpl_data.sf;
...
static AOM_INLINE void mc_flow_dispenser(AV1_COMP *cpi) {
  AV1_COMMON *cm = &cpi->common;
  TplParams *const tpl_data = &cpi->tpl_data;
  TplDepFrame *tpl_frame = &tpl_data->tpl_frame[cpi->tpl_data.frame_idx];
  ...
  for (int mi_row = 0; mi_row < mi_params->mi_rows; mi_row += mi_height) {
    ...
    for (int mi_col = 0; mi_col < mi_params->mi_cols; mi_col += mi_width) {
      ...
      mode_estimation(cpi, x, xd, &cpi->tpl_data.sf, cpi->tpl_data.frame_idx,
                      mi_row, mi_col, bsize, tx_size, cpi->tpl_data.ref_frame,
                      cpi->tpl_data.src_ref_frame, &tpl_stats);
