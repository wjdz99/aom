...
int av1_tpl_setup_stats(AV1_COMP *cpi, int gop_eval,
  ...
  // When gop_eval is set to 2, tpl stats calculation is done for base layer
  // ARF, (base+1) layer ARF and (base+2) layer ARF. When gop_eval is set to 3,
  // tpl stats calculation is limited to base layer ARF and (base+1) layer ARF.
  ...
  if (approx_gop_eval) {
    if (gop_eval == 2) {
      if (beta[0] > 1.6 && beta[0] >= beta[1] + 0.4)
        ...
        return 2;  // Cannot decide the gop length, so redo the
