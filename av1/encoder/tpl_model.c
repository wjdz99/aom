...
static AOM_INLINE void mode_estimation(AV1_COMP *cpi,
  ...
  tpl_stats->nz_coeff_count = 1;
  for (int i = 0; i < 256; ++i) {
    if (qcoeff[i]) ++tpl_stats->nz_coeff_count;
  }
...
int av1_tpl_setup_stats(AV1_COMP *cpi, int gop_eval,
  ...
  for (int frame_idx = cpi->gf_frame_index;
       frame_idx < cpi->gf_frame_index + 1; //  gop_length;
       ++frame_idx) {
    if (gf_group->update_type[frame_idx] == INTNL_OVERLAY_UPDATE ||
        gf_group->update_type[frame_idx] == OVERLAY_UPDATE)
      continue;
  ...
  // tpl_data->norm = exp((intra_cost_base - mc_dep_cost_base) / cbcmp_base);
  tpl_data->norm = cbcmp_base / blk_count;
...
double av1_tpl_get_frame_importance(const TplParams *tpl_data,
  ...
  fprintf(stderr, "cbcmp = %f, norm = %f, ratio = %f\n", cbcmp_base / blk_count, tpl_data->norm,
          (cbcmp_base / blk_count) / tpl_data->norm);
  return exp(((mc_dep_cost_base - intra_cost_base) / cbcmp_base) * ((cbcmp_base / blk_count) / tpl_data->norm));
