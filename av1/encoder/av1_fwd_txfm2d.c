...
#if CONFIG_MODEDEP_TX && SEP_TX_DEBUG
  // debug
...
static INLINE void fwd_nonsep_txfm2d(const int16_t *input, int32_t *output,
  ...
  const int32_t *nstx = nstx_arr(tx_type, tx_size, mode, bd);
...
#if MODEDEP_TX_DEBUG_BLOCK
  // debug
  fprintf(stderr, "tx_type %d mode %d bd %d ud_flip %d lr_flip %d\n", tx_type,
          mode, bd, ud_flip, lr_flip);
    ...
    for (ct = 0; ct < txw; ++ct) {
      ...
      for (rp = 0; rp < txh; ++rp) {
        for (cp = 0; cp < txw; ++cp) {
          ...
          buf[l] += round_shift(nstx[l * tx_stride + kt] * input[kp], 1);
#if MODEDEP_TX_DEBUG_MULT
          fprintf(stderr, "(%d,%d,%d)[%d,%d,%d]", l, tx_stride, kt,
                  nstx[l * tx_stride + kt], input[kp], buf[l]);
#endif
        }
#if MODEDEP_TX_DEBUG_MULT
        fprintf(stderr, "\n");
...
void av1_fwd_txfm2d_4x4_c(const int16_t *input, int32_t *output, int stride,
  ...
  if (tx_type > H_FLIPADST) {
    fwd_nonsep_txfm2d(input, output, stride, txfm_buf, tx_type, mode, TX_4X4,
                      bd);
  } else {
#endif
    TXFM_2D_FLIP_CFG cfg;
...
#if CONFIG_MODEDEP_TX
  if (tx_type > H_FLIPADST) {
    fwd_nonsep_txfm2d(input, output, stride, txfm_buf, tx_type, mode, TX_8X8,
                      bd);
  } else {
#endif
    TXFM_2D_FLIP_CFG cfg;
