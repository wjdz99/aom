...
static int64_t estimate_yrd_for_sb(const AV1_COMP *const cpi, BLOCK_SIZE bs,
  ...
  const int64_t rd =
      ...
      av1_estimate_txfm_yrd(cpi, x, rd_stats, ref_best_rd, bs,
                           max_txsize_rect_lookup[bs], FTXS_NONE, skip_trellis);
...
int av1_compound_type_rd(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  *rd = INT64_MAX;
  ...
  for (int i = 0; i < valid_type_count; i++) {
    ...
    if (args->cmp_mode[ref_frame] >= 0 &&
        args->cmp_mode[ref_frame] != COMPOUND_WEDGE) {
    ...
    if (cur_type < COMPOUND_WEDGE) {
      ...
      if (need_mask_search) {
        if (this_mode == NEW_NEWMV && best_rd_cur < INT64_MAX) {
