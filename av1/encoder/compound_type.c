  ...
  *rd = INT64_MAX;
  ...
  for (int i = 0; i < valid_type_count; i++) {
    ...
    if (cur_type < COMPOUND_WEDGE) {
      ...
      if (mode_rd / 2 < ref_best_rd) {
        // Reuse data if matching record is found
        if (comp_rate[cur_type] == INT_MAX || 1) {
          if (have_newmv_in_inter_mode(this_mode)) {
            ...
            tmp_rate_mv = av1_interinter_compound_motion_search(cpi, x, tmp_mv,
                                                                    bsize, this_mode);
            mbmi->mv[0] = tmp_mv[0];
            mbmi->mv[1] = tmp_mv[1];
          }
          ...
          if (cpi->sf.inter_sf.txfm_rd_gate_level) {
            ...
            int64_t skip_rd = RDCOST(x->rdmult, rs2 + tmp_rate_mv, (sse_y << 4));
          ...
          best_rd_cur = RDCOST(x->rdmult, rs2 + tmp_rate_mv + comp_rate[cur_type],
                               comp_dist[cur_type]);
          // Recalculate model rdcost with the updated rate
          comp_model_rd_cur =
              RDCOST(x->rdmult, rs2 + tmp_rate_mv + comp_model_rate[cur_type],
