  ...
  *rd = INT64_MAX;
  ...
  for (int i = 0; i < valid_type_count; i++) {
    ...
    if (cur_type < COMPOUND_WEDGE) {
      ...
      for (int wedge_mask = 0; wedge_mask < wedge_mask_size && need_mask_search; ++wedge_mask) {
      ...
      for (int mask_index = 0; mask_index < 2 && need_mask_search; ++mask_index) {
        // hard coded number for diff wtd
        int mask_value = mask_index == 0 ? 38 : 26;
        memset(xd->seg_mask, mask_value, sizeof(xd->seg_mask));
        tmp_rate_mv = *rate_mv;
        mbmi->interinter_comp.mask_type = mask_index == 0 ? DIFFWTD_38 : DIFFWTD_38_INV;
        if (have_newmv_in_inter_mode(this_mode))
          tmp_rate_mv = av1_interinter_compound_motion_search(cpi, x, cur_mv,
                                                              bsize, this_mode);
        av1_enc_build_inter_predictor(cm, xd, mi_row, mi_col, orig_dst, bsize,
      ...
      if (need_mask_search) {
        ...
        int mask_value =
            mbmi->interinter_comp.mask_type == 0 ? 38 : 26;
        memset(xd->seg_mask, mask_value, sizeof(xd->seg_mask));
