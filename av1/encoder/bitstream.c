...
static struct av1_token single_ref0_encodings[INTER_REFS_PER_FRAME];
...
#if CONFIG_EXT_COMP_REFS
  av1_tokens_from_tree(single_ref0_encodings, av1_single_or_comp_ref0_tree);
#endif  // CONFIG_EXT_COMP_REFS
...
static void write_ref0_frame(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  const aom_prob *const single_ref0_probs = cm->fc->single_ref0_probs[ref0_ctx];

  av1_write_token(w, av1_single_or_comp_ref0_tree, single_ref0_probs,
                  &single_ref0_encodings[INTER_REF_OFFSET(ref0_frame)]);
}
#endif  // CONFIG_EXT_COMP_REFS
  ...
  if (frame_is_intra_only(cm)) {
    ...
    if (cm->reference_mode != COMPOUND_REFERENCE) {
      ...
      for (i = 0; i < REF0_CONTEXTS; i++) {
        prob_diff_update(
            av1_single_or_comp_ref0_tree, fc->single_ref0_probs[i],
            counts->single_ref0[i], INTER_REFS_PER_FRAME, probwt, header_bc);
      }
#else
