  ...
  switch (type) {
    ...
    case ROTZOOM:
      aom_write_primitive_symmetric(
          w, (params->wmmat[2] >> GM_ALPHA_PREC_DIFF) -
                 (1 << GM_ALPHA_PREC_BITS),
          GM_ABS_ALPHA_BITS);
      aom_write_primitive_symmetric(
          w, (params->wmmat[3] >> GM_ALPHA_PREC_DIFF),
          GM_ABS_ALPHA_BITS);
      if (type == AFFINE) {
        aom_write_primitive_symmetric(
            w, (params->wmmat[4] >> GM_ALPHA_PREC_DIFF),
            GM_ABS_ALPHA_BITS);
        aom_write_primitive_symmetric(
            w, (params->wmmat[5] >> GM_ALPHA_PREC_DIFF) -
                   (1 << GM_ALPHA_PREC_BITS),
            GM_ABS_ALPHA_BITS);
      }
    ...
    case TRANSLATION:
      aom_write_primitive_symmetric(
          w, (params->wmmat[0] >> GM_TRANS_PREC_DIFF),
          GM_ABS_TRANS_BITS);
      aom_write_primitive_symmetric(
          w, (params->wmmat[1] >> GM_TRANS_PREC_DIFF),
          GM_ABS_TRANS_BITS);
      break;
    default: assert(0);
  }
