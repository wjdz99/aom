...
static void write_tx_partition(MACROBLOCKD *xd, const MB_MODE_INFO *mbmi,
  ...
  if (allow_horz && allow_vert) {
    ...
    aom_write_symbol(w, split4_partition, 
                     ec_ctx->inter_4way_txfm_partition_cdf[is_rect][split4_ctx], 4);
    if ((split4_partition == TX_PARTITION_VERT) || 
        (split4_partition  == TX_PARTITION_HORZ)) {
      const int further_split = (partition == TX_PARTITION_VERT4) || 
                                (partition == TX_PARTITION_HORZ4);
      aom_write_bit(w, further_split);
    }
  } else if (allow_horz || allow_vert) {
    const int has_first_split = partition != TX_PARTITION_NONE;
    aom_write_bit(w, has_first_split);
    if (has_first_split) {
      const int has_second_split = 
        (partition == TX_PARTITION_VERT4) || (partition == TX_PARTITION_HORZ4);
      aom_write_bit(w, has_second_split);
    }
  } else {
