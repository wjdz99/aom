...
#if !CONFIG_TXK_SEL
#if CONFIG_VAR_TX
  const TX_SIZE max_tx_size = max_txsize_rect_lookup[xd->mi[0]->mbmi.sb_type];
  const TX_SIZE tx_size = is_inter ?
        AOMMAX(sub_tx_size_map[max_tx_size], mbmi->min_tx_size) : mbmi->tx_size;
#else
  const TX_SIZE tx_size = mbmi->tx_size;
#endif  // CONFIG_VAR_TX
