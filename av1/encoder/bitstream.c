  ...
  for (size_t i = 0; i < arr->sz; i++) {
    ...
    if (current_metadata && current_metadata->payload) {
      ...
      if ((cm->current_frame.frame_type == KEY_FRAME &&
        ...
        const bool is_layer_specific =
            (current_metadata->insert_flag & 0x10) != 0;
        obu_header_size = av1_write_obu_header(
            &cpi->ppi->level_params, &cpi->frame_header_count, OBU_METADATA,
            cm->seq_params->has_nonzero_operating_point_idc, is_layer_specific,
            obu_extension_header, dst);
...
int av1_pack_bitstream(AV1_COMP *const cpi, uint8_t *dst, size_t dst_size,
  ...
  if (cm->show_frame) {
    const size_t bytes_written =
        av1_write_metadata_array(cpi, data, data_size, obu_extension_header);
    data += bytes_written;
    data_size -= bytes_written;
  }
