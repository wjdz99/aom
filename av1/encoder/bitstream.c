...
#endif
{
  ...
  struct aom_write_bit_buffer saved_wb2;
  int saved_offset;
#endif
  size_t total_size = 0;
  ...
  for (tile_row = 0; tile_row < tile_rows; tile_row++) {
    for (tile_col = 0; tile_col < tile_cols; tile_col++) {
      ...
      if (tile_count >= tg_size) {
        ...
        // header
        aom_wb_write_literal(&saved_wb2, tile_idx - tile_count, n_log2_tiles);
        aom_wb_write_literal(&saved_wb2, tile_count - 1, n_log2_tiles);
        saved_wb2.bit_offset = saved_offset + 8 * total_size;
  ...
  if (n_log2_tiles) {
    aom_wb_write_literal(&saved_wb2, (1<<n_log2_tiles) - tile_count, n_log2_tiles);
    aom_wb_write_literal(&saved_wb2, tile_count - 1, n_log2_tiles);
  }
#endif
