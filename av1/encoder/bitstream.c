...
static AOM_INLINE void write_tile_info_max_tile(
  ...
  aom_wb_write_bit(wb, cm->tiles.uniform_spacing);

  if (cm->tiles.uniform_spacing) {
    // Uniform spaced tiles with power-of-two number of rows and columns
    // tile columns
    int ones = cm->tiles.log2_cols - cm->tiles.min_log2_cols;
    while (ones--) {
      aom_wb_write_bit(wb, 1);
    }
    if (cm->tiles.log2_cols < cm->tiles.max_log2_cols) {
      aom_wb_write_bit(wb, 0);
    }

    // rows
    ones = cm->tiles.log2_rows - cm->tiles.min_log2_rows;
    while (ones--) {
      aom_wb_write_bit(wb, 1);
    }
    if (cm->tiles.log2_rows < cm->tiles.max_log2_rows) {
      aom_wb_write_bit(wb, 0);
    }
  } else {
    ...
    for (i = 0; i < cm->tiles.cols; i++) {
      size_sb = cm->tiles.col_start_sb[i + 1] - cm->tiles.col_start_sb[i];
      wb_write_uniform(wb, AOMMIN(width_sb, cm->tiles.max_width_sb),
                       size_sb - 1);
      width_sb -= size_sb;
    }
    assert(width_sb == 0);
    ...
    // rows
    for (i = 0; i < cm->tiles.rows; i++) {
      size_sb = cm->tiles.row_start_sb[i + 1] - cm->tiles.row_start_sb[i];
      wb_write_uniform(wb, AOMMIN(height_sb, cm->tiles.max_height_sb),
                       size_sb - 1);
      height_sb -= size_sb;
    }
    assert(height_sb == 0);
...
static int remux_tiles(const AV1_COMMON *const cm, uint8_t *dst,
  ...
  *tile_col_size_bytes = tcsb;
  ...
  if (cm->tiles.large_scale) {
    ...
    for (tile_col = 0; tile_col < cm->tiles.cols; tile_col++) {
      ...
      if (tile_col < cm->tiles.cols - 1) {
        ...
        tile_col_size -= (4 - tsb) * cm->tiles.rows;
  ...
  const int n_tiles = cm->tiles.cols * cm->tiles.rows;
  int n;
...
static uint32_t write_tiles_in_tg_obus(AV1_COMP *const cpi, uint8_t *const dst,
  ...
  const int n_log2_tiles = cm->tiles.log2_rows + cm->tiles.log2_cols;
  ...
  *largest_tile_id = 0;
  ...
  if (cm->tiles.large_scale) {
    ...
    for (tile_col = 0; tile_col < tile_cols; tile_col++) {
      ...
      for (tile_row = 0; tile_row < tile_rows; tile_row++) {
        ...
        mode_bc.allow_update_cdf = !cm->tiles.large_scale;
        ...
        if (have_tiles) {
          ...
          const int tile_copy_mode = ((AOMMAX(cm->tiles.width, cm->tiles.height)
                                       << MI_SIZE_LOG2) <= 256)
                                         ? 1
                                         : 0;
  ...
  if (have_tiles) {
    ...
    aom_wb_overwrite_literal(saved_wb, *largest_tile_id,
                             cm->tiles.log2_cols + cm->tiles.log2_rows);
