...
#define FRAME_REFS_SHORT_SIGNALING 0
      ...
      int first_ref_map_idx = 0;
      int num_references = 0;
      for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
        if (cpi->ppi->rtc_ref.reference[ref_frame - 1] == 1) num_references++;
      }
      // For the case of single reference with set_ref_frame_config:
      // for a reference not used reset the ref_map_idx to the first reference
      // used.
