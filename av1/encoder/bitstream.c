...
void av1_write_tx_type(const AV1_COMMON *const cm, const MACROBLOCKD *xd,
...
#endif
      if ((((tx_type & 0xf) == DCT_DCT) || ((tx_type & 0xf) == ADST_ADST)) && ((intra_dir < PAETH_PRED) && !(mbmi->filter_intra_mode_info.use_filter_intra)))
      {
  ...
  else if (!is_inter)
  {
    ...
    if ((ht == max_ht) && (wd == max_wd))
    {
      if ((intra_dir < PAETH_PRED) && !(mbmi->filter_intra_mode_info.use_filter_intra))
      {
          aom_write_symbol(
            w, stx_flag,
            ec_ctx->stx_cdf[square_tx_size],
