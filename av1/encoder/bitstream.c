...
static AOM_INLINE void write_ref_frames(const AV1_COMMON *cm,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    ...
    if ((mbmi->ref_frame[0] ==  ALTREF2_FRAME && mbmi->ref_frame[1]== ALTREF_FRAME) ||
        (mbmi->ref_frame[0] ==  LAST2_FRAME && mbmi->ref_frame[1] == LAST3_FRAME) ||
        (mbmi->ref_frame[0] ==  LAST2_FRAME && mbmi->ref_frame[1] == GOLDEN_FRAME) ||
        (mbmi->ref_frame[0] ==  LAST3_FRAME && mbmi->ref_frame[1] == GOLDEN_FRAME) ||
        (mbmi->ref_frame[0] ==  BWDREF_FRAME && mbmi->ref_frame[1] == ALTREF2_FRAME)) printf("HERE\n");
        ...
        if (is_future) {
          ...
          if (bit1) {
            ...
            assert(mbmi->ref_frame[0] == ALTREF2_FRAME);
