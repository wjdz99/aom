...
static int get_refresh_mask(AV1_COMP *cpi) {
  ...
  refresh_mask |= (cpi->refresh_last_frame
                   << cpi->remapped_reference_idx[LAST_REF_FRAMES - 1]);
#if USE_SYMM_MULTI_LAYER
  refresh_mask |= (cpi->new_bwdref_update_rule == 1)
                      ? (cpi->refresh_bwd_ref_frame
                         << cpi->remapped_reference_idx[EXTREF_FRAME - 1])
                      : (cpi->refresh_bwd_ref_frame
                         << cpi->remapped_reference_idx[BWDREF_FRAME - 1]);
#else
  refresh_mask |= (cpi->refresh_bwd_ref_frame
                   << cpi->remapped_reference_idx[BWDREF_FRAME - 1]);
#endif
  refresh_mask |= (cpi->refresh_alt2_ref_frame
                   << cpi->remapped_reference_idx[ALTREF2_FRAME - 1]);
  ...
  if (av1_preserve_existing_gf(cpi)) {
    ...
    if (cpi->preserve_arf_as_gld) {
      ...
      return refresh_mask | (cpi->refresh_golden_frame
                             << cpi->remapped_reference_idx[ALTREF_FRAME - 1]);
    }
  } else {
    const int arf_idx = cpi->remapped_reference_idx[ALTREF_FRAME - 1];
    return refresh_mask |
           (cpi->refresh_golden_frame
            << cpi->remapped_reference_idx[GOLDEN_FRAME - 1]) |
