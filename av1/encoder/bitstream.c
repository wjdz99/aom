...
static void write_cdef(AV1_COMMON *cm, aom_writer *w, int skip,
  ...
  if (!(mi_row & (cm->mib_size - 1)) && !(mi_col & (cm->mib_size - 1)) &&  // Top left?
    ...
    if (cm->sb_size == BLOCK_128X128) {
      mbmi = &cm->mi_grid_visible[mi_row * cm->mi_stride + mi_col + cm->mib_size / 2]->mbmi;
      aom_write_literal(w, mbmi->cdef_strength, cm->cdef_bits);
      mbmi = &cm->mi_grid_visible[(mi_row + cm->mib_size / 2) * cm->mi_stride + mi_col]->mbmi;
      aom_write_literal(w, mbmi->cdef_strength, cm->cdef_bits);
      mbmi = &cm->mi_grid_visible[(mi_row + cm->mib_size / 2) * cm->mi_stride + mi_col + cm->mib_size / 2]->mbmi;
      aom_write_literal(w, mbmi->cdef_strength, cm->cdef_bits);
    }
#endif
  }
