...
static AOM_INLINE void write_mrl_index(FRAME_CONTEXT *ec_ctx, uint8_t mrl_index,
                                       aom_writer *w, uint8_t mrls_flag,
                                       PREDICTION_MODE mode) {
  if (mrls_flag == 0 || av1_is_directional_mode(mode) == 0) return;
  aom_write_symbol(w, mrl_index, ec_ctx->mrl_index_cdf, MRL_LINE_COUNT);
}
#endif
...
#if CONFIG_MRLS
    // Signaling reference line index
    write_mrl_index(ec_ctx, mbmi->mrl_index, w, cm->seq_params.enable_mrls,
                    mode);
#endif
#if CONFIG_SDP
  }
