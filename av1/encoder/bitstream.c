...
int enable_pack_bitstream_mt(const TileDataEnc *tile_data, int num_tiles,
                             int num_workers) {
  uint64_t total_abs_sum = 0;
  for (int i = 0; i < num_tiles; i++) {
    total_abs_sum += tile_data[i].abs_sum_level;
  }
  const int num_work_sq = num_workers * num_workers;
  const uint64_t thresh = 50;
  return (AOMMIN(num_workers, num_tiles) > 1 &&
          ((total_abs_sum > (num_work_sq * thresh) / (num_workers - 1))));
}
