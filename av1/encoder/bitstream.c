...
static uint32_t av1_write_metadata_itut_t35_obu_header(aom_metadata_t *metadata,
                                                       uint8_t *const dst) {
  size_t coded_obu_size = 0;
    ...
    if (currentMetadata && currentMetadata->payload) {
      ...
      switch (type) {
        ...
        case OBU_METADATA_TYPE_ITUT_T35:
          ...
          obu_payload_size = av1_write_metadata_itut_t35_obu_header(
          ...
          if (av1_write_uleb_obu_size(obu_header_size, obu_payload_size, dst) ==
              AOM_CODEC_OK) {
            dst += (size_t)obu_header_size + (size_t)obu_payload_size +
                   length_field_size;
            total_bytes_written +=
...
int av1_pack_bitstream(AV1_COMP *const cpi, uint8_t *dst, size_t *size,
  ...
  // write metadata obu before the frame obus that have the show_frame flag set
