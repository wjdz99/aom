...
static int write_skip(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_SKIP)) {
    ...
    vpx_write(w, skip, av1_get_skip_prob(cm, xd));
    return skip;
  }
}

static void update_skip_probs(AV1_COMMON *cm, vpx_writer *w,
                              FRAME_COUNTS *counts) {
  int k;
...
static void update_switchable_interp_probs(AV1_COMMON *cm, vpx_writer *w,
                                           FRAME_COUNTS *counts) {
  int j;
...
static void update_ext_tx_probs(AV1_COMMON *cm, vpx_writer *w) {
...
static void write_ref_frames(const AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    ...
    if (cm->reference_mode == REFERENCE_MODE_SELECT) {
      vpx_write(w, is_compound, av1_get_reference_mode_prob(cm, xd));
    } else {
    ...
    if (is_compound) {
      ...
      vpx_write(w, bit0, av1_get_pred_prob_single_ref_p1(cm, xd));
      if (bit0) {
        const int bit1 = mbmi->ref_frame[0] != GOLDEN_FRAME;
        vpx_write(w, bit1, av1_get_pred_prob_single_ref_p2(cm, xd));
      }
    }
  }
}
...
static void pack_inter_mode_mvs(AV1_COMP *cpi, const MODE_INFO *mi,
  ...
  if (seg->update_map) {
    if (seg->temporal_update) {
      ...
      vpx_prob pred_prob = av1_get_pred_prob_seg_id(segp, xd);
  ...
  if (!segfeature_active(seg, segment_id, SEG_LVL_REF_FRAME))
    vpx_write(w, is_inter, av1_get_intra_inter_prob(cm, xd));
...
static void update_coef_probs_common(vpx_writer *const bc, AV1_COMP *cpi,
                                     TX_SIZE tx_size,
...
static void update_coef_probs(AV1_COMP *cpi, vpx_writer *w) {
...
#if CONFIG_MISC_FIXES
static void update_seg_probs(AV1_COMP *cpi, vpx_writer *w) {
...
static void update_txfm_probs(AV1_COMMON *cm, vpx_writer *w,
                              FRAME_COUNTS *counts) {
...
static void write_sync_code(struct vpx_write_bit_buffer *wb) {
  vpx_wb_write_literal(wb, AV1_SYNC_CODE_0, 8);
  vpx_wb_write_literal(wb, AV1_SYNC_CODE_1, 8);
  vpx_wb_write_literal(wb, AV1_SYNC_CODE_2, 8);
}
...
static void write_bitdepth_colorspace_sampling(
    AV1_COMMON *const cm, struct vpx_write_bit_buffer *wb) {
