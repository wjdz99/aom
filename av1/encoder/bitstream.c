  ...
  if (seg->enabled && seg->update_map) {		
    ...
    if (spatial_pred_ctx) {
      ...
      aom_write_symbol(w, spatial_segment_id_predicted,
                       segp->heterogeneous_spatial_pred_cdf,
...
static void write_modes_sb(AV1_COMP *const cpi, const TileInfo *const tile,
  ...
  if (partition != PARTITION_NONE && supertx_enabled && pack_token) {
    ...
    set_mi_row_col(xd, tile, mi_row, bsh, mi_col, bsw, cm->mi_rows,
                   cm->mi_cols);
#if CONFIG_EXT_SEGMENT
    assert(IMPLIES(!cm->seg[ACTIVE_SEG_IDX].enabled, mbmi->segment_id_supertx[ACTIVE_SEG_IDX] == 0));
    assert(mbmi->segment_id_supertx[ACTIVE_SEG_IDX] < MAX_ACTIVE_SEGMENTS);

    skip = write_skip(cm, xd, mbmi->segment_id_supertx[ACTIVE_SEG_IDX], xd->mi[0], w);
#else
...
static void update_seg_probs(AV1_COMP *cpi, aom_writer *w) {
  ...
  if (cm->seg[ACTIVE_SEG_IDX].update_map) {
    ...
    av1_cond_prob_diff_update(
        w, &cm->fc->seg[ACTIVE_SEG_IDX].harmonic_spatial_pred_probs[0],
        cm->counts.seg[ACTIVE_SEG_IDX].harmonic_spatial_pred, probwt);
    prob_diff_update(
        av1_seg_id_spatial_prediction_tree,
        cm->fc->seg[ACTIVE_SEG_IDX].heterogeneous_spatial_pred_probs,
        cm->counts.seg[ACTIVE_SEG_IDX].heterogeneous_spatial_pred,
        HETEROGENEOUS_SPATIAL_PRED_PROBS + 1, probwt, w);
