...
static void write_drl_idx(FRAME_CONTEXT *ec_ctx, int16_t mode_ctx,
  ...
  assert(mbmi->ref_mv_idx < mbmi_ext_frame->ref_mv_count[ref_frame_type]);
  assert(mbmi->ref_mv_idx < MAX_DRL_BITS + 1);
  int range =
      AOMMIN(mbmi_ext_frame->ref_mv_count[ref_frame_type] - 1, MAX_DRL_BITS);
  for (int idx = 0; idx < range; ++idx) {
    aom_cdf_prob *drl_cdf = av1_get_drl_cdf(mode_ctx, ec_ctx, mbmi->mode,
                                            mbmi_ext_frame->weight, idx);
...
#if CONFIG_NEW_INTER_MODES
        write_drl_idx(ec_ctx, mode_ctx, mbmi, mbmi_ext_frame, w);
#else
