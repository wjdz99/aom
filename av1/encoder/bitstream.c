...
static void write_segment_id(aom_writer *w, const struct segmentation *seg,
                             struct segmentation_probs *segp, const int8_t segment_id,
    ...
    MODE_INFO* top_mi = xd->mi[-(((-xd->mb_to_top_edge >> (MI_SIZE_LOG2 + 3)) & (seg_map_minb_size - 1)) + 1) * cm->mi_stride
      - ((-xd->mb_to_left_edge >> (MI_SIZE_LOG2 + 3)) & (seg_map_minb_size - 1))];
    MODE_INFO* left_mi = xd->mi[-((-xd->mb_to_top_edge >> (MI_SIZE_LOG2 + 3)) & (seg_map_minb_size - 1)) * cm->mi_stride
      - (((-xd->mb_to_left_edge >> (MI_SIZE_LOG2 + 3)) & (seg_map_minb_size - 1)) + 1)];
	if (seg_id_written) xd->seg_id_is_done[seg_cat_idx] = 0;
	if (seg_id_written || !xd->seg_id_is_done[seg_cat_idx]) {
		const int spatial_pred_ctx = top_mi && left_mi &&
          top_mi->mbmi.segment_id[seg_cat_idx] == left_mi->mbmi.segment_id[seg_cat_idx] ? 1 : 0;
		if (seg->temporal_update) {
                  if (!skip_seg_id_write || seg->skip_seg_id_disabled) {
                    const int pred_flag =
                        mi->mbmi.seg_id_predicted[seg_cat_idx];
                    const int pred_context =
                        (top_mi ? top_mi->mbmi.seg_id_predicted[seg_cat_idx]
                                : 0) +
                        (left_mi ? left_mi->mbmi.seg_id_predicted[seg_cat_idx]
                                 : 0);
                    aom_prob pred_prob = segp->pred_probs[pred_context];
                    aom_write(w, pred_flag, pred_prob);
                    if (!pred_flag) {
                      write_temp_unpred_segment_id(
                          w, seg, segp, segment_id,
...
#if CONFIG_EXT_SEGMENT
  const int8_t * segment_id = &mbmi->segment_id[0];
#else
  const int segment_id = mbmi->segment_id;
#endif
  const BLOCK_SIZE bsize = mbmi->sb_type;
...
static void encode_segmentation(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  if (seg[QUALITY_SEG_IDX]->enabled) {
    ...
    if (seg[QUALITY_SEG_IDX]->update_map) {
      ...
      aom_wb_write_bit(wb, seg[QUALITY_SEG_IDX]->skip_seg_id_disabled);
      if (seg[QUALITY_SEG_IDX]->temporal_update &&
          !seg[QUALITY_SEG_IDX]->skip_seg_id_disabled) {
        aom_wb_write_bit(wb, seg[QUALITY_SEG_IDX]->skip_seg_id_replacement);
      }
    }
