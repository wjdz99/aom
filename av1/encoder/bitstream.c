        ...
        for (row = 0; row < num_4x4_h; row += bkh) {
          for (col = 0; col < num_4x4_w; col += bkw) {
            ...
            if (palette_size_plane > 0 && plane <= 1) {
              pack_palette_tokens(
                  w, tok, palette_size_plane,
                  AOMMIN(cols - col * (bkw << tx_size_wide_log2[0]),
                         (bkw << tx_size_wide_log2[0])) *
                          AOMMIN(rows - row * (bkh << tx_size_wide_log2[0]),
                                 (bkh << tx_size_wide_log2[0])) -
                      ((row == 0 && col == 0) ? 1 : 0));
            }
#endif
      ...
      for (row = 0; row < num_4x4_h; row += bkh) {
        for (col = 0; col < num_4x4_w; col += bkw) {
          if (!is_inter_block(mbmi) && palette_size_plane > 0 && plane <= 1) {
            pack_palette_tokens(
                w, tok, palette_size_plane,
                AOMMIN(cols - (col << tx_size_wide_log2[0]),
                       (bkw << tx_size_wide_log2[0])) *
                        AOMMIN(rows - (row << tx_size_high_log2[0]),
                               (bkh << tx_size_wide_log2[0])) -
                    ((row == 0 && col == 0) ? 1 : 0));
          }
          pack_mb_tokens(w, tok, tok_end, cm->bit_depth, tx, &token_stats);
