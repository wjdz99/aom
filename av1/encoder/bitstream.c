...
#if !CONFIG_EC_MULTISYMBOL
static void write_intra_mode_kf(aom_writer *w, PREDICTION_MODE mode,
    ...
    if (bsize >= BLOCK_8X8 || unify_bsize) {
      ...
      for (idy = 0; idy < 2; idy += num_4x4_h) {
        for (idx = 0; idx < 2; idx += num_4x4_w) {
          ...
          write_intra_mode(ec_ctx, 0, b_mode, w);
        }
      }
    }
#if CONFIG_CB4X4
...
static void write_mb_modes_kf(AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (bsize >= BLOCK_8X8 || unify_bsize) {
    ...
    aom_write_symbol(w, av1_intra_mode_ind[mbmi->mode],
                     ...
                     INTRA_MODES);
#else
    write_intra_mode_kf(w, mbmi->mode,
    ...
    for (idy = 0; idy < 2; idy += num_4x4_h) {
      for (idx = 0; idx < 2; idx += num_4x4_w) {
        ...
        aom_write_symbol(w, av1_intra_mode_ind[mi->bmi[block].as_mode],
                         ...
                         INTRA_MODES);
#else
        write_intra_mode_kf(w, mi->bmi[block].as_mode,
