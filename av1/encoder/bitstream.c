...
#if CONFIG_OBU
  cm->frame_type = cm->intra_only ? INTRA_ONLY_FRAME : cm->frame_type;  // intra-only
...
static uint32_t write_temporal_delimiter_obu(uint8_t *const dst) { return 0; }
...
static uint32_t write_frame_header_obu(AV1_COMP *cpi, uint8_t *const dst) {
  ...
  // write the tile length code  (TODO: go through all tiles)
  aom_wb_write_literal(&wb, 3, 2);
...
static uint32_t write_tiles_in_tg_obus(AV1_COMP *const cpi, uint8_t *const dst,
                                       uint8_t *const frame_header_loc,
...
void av1_pack_bitstream(AV1_COMP *const cpi, uint8_t *dst, size_t *size) {
  ...
  AV1_COMMON *const cm = &cpi->common;
  struct aom_write_bit_buffer wb = { dst, 0 };
  uint32_t total_size = 0, obu_size;
  ...
  obu_size = write_obu_header(OBU_FRAME_HEADER, 0, data + 4);
  ...
  if (cm->show_existing_frame) {
    ...
    //  obu
    data_size = write_tiles_in_tg_obus(cpi, data, frame_header_location,
                                       frame_header_size, &max_tile_size,
                                       &max_tile_col_size);
  }
