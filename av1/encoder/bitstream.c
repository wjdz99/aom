...
//printf("ENC horz %d vert %d horz2 %d vert2 %d max_tx %d tx %d part %d\n", allow_horz, allow_vert, 
//        allow_horz2, allow_vert2, max_tx_size, tx_size, partition);
  if (allow_horz && allow_vert) {
    ...
    if (((split4_partition == TX_PARTITION_VERT) && allow_vert2) ||
      ...
      aom_write_symbol(w, has_split, ec_ctx->inter_2way_rect_txfm_partition_cdf, 2);
    }
  } else if (allow_horz || allow_vert) {
    const int has_first_split = partition != TX_PARTITION_NONE;
    aom_write_symbol(w, has_first_split, ec_ctx->inter_2way_txfm_partition_cdf, 2);
    if (has_first_split && (allow_horz2 || allow_vert2)) {
      ...
      aom_write_symbol(w, has_second_split, ec_ctx->inter_2way_rect_txfm_partition_cdf, 2);
    }
  } else {
