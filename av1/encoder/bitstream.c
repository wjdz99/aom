...
static void write_tx_partition(MACROBLOCKD *xd, const MB_MODE_INFO *mbmi,
  ...
  if (tx_size < max_tx_size)
      assert(sub_tx_size_map[max_tx_size] == tx_size);
  const TX_PARTITION_TYPE partition =
      tx_size != max_tx_size ? TX_PARTITION_SPLIT : TX_PARTITION_NONE;
  //printf("IN tx size %d max tx %d part %d\n", tx_size, max_tx_size, partition);
  printf("IN %d\n", partition);
  aom_write_symbol(w, partition, ec_ctx->txfm_partition_cdf[ctx], TX_PARTITION_TYPES);
    ...
    for (int row = 0; row < tx_size_high_unit[max_tx_size]; row += bsh)
      for (int col = 0; col < tx_size_wide_unit[max_tx_size]; col += bsw) {
        ...
        txfm_partition_update(xd->above_txfm_context + offsetc,
                          xd->left_txfm_context + offsetr, tx_size, tx_size);
