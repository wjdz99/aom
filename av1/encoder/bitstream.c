    ...
    if (cm->buffer_removal_delay_present) {
      for (int op_num = 0; op_num < cm->operating_points_decoder_model_cnt; op_num++)
      {
        if ( ((cm->op_params[op_num].decoder_model_operating_point_idc >> cm->temporal_layer_id) & 0x1 &&
          ...
          aom_wb_write_literal(wb, (uint32_t) cm->op_frame_timing[op_num].buffer_removal_delay, cm->buffer_model.buffer_removal_delay_length);
          cm->op_frame_timing[op_num].buffer_removal_delay++;
        }
      }
    }
...
#if CONFIG_BUFFER_MODEL
    aom_wb_write_bit(&wb,
                     cm->decoder_model_info_present);
    if (cm->decoder_model_info_present) write_decoder_model_info(cm, &wb);
  }
#endif

#if CONFIG_BUFFER_MODEL
  aom_wb_write_literal(&wb, cm->operating_points_decoder_model_cnt, 5);
  for (int op_num = 0; op_num < cm->operating_points_decoder_model_cnt; ++op_num){
    aom_wb_write_literal(&wb, cm->op_params[op_num].decoder_model_operating_point_idc, 12);
    aom_wb_write_bit(&wb,
                     cm->op_params[op_num].display_model_param_present_flag);
    if (cm->op_params[op_num].display_model_param_present_flag)
      aom_wb_write_literal(&wb, cm->op_params[op_num].initial_display_delay - 1, 4);
    if (cm->decoder_model_info_present){
    aom_wb_write_bit(&wb,
