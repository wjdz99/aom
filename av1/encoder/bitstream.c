...
int calc_pack_bs_mt_workers(const TileDataEnc *tile_data, int num_tiles,
  ...
  const int setup_time_const_worker = 5;
  const int tile_job_disp_time_const = 1;
  int ideal_num_workers = 1;
  const float job_disp_time_const = (float)num_tiles * tile_job_disp_time_const;
  float max_sum = 0.0;
  ...
  for (int num_workers = avail_workers; num_workers > 1; num_workers--) {
    ...
    const float setup_time_const = (float)num_workers * setup_time_const_worker;
