      ...
      for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
        ...
        if (!current_frame->frame_refs_short_signaling) {
          if (cpi->ppi->rtc_ref.set_ref_frame_config &&
            ...
            // if any of the other references are not used set their
            // ref_map_idx to the first used reference.
            // For now only do this for !rtc_external_ratectrl until the
            // failing test in external_rtc_ratectrl is resolved.
