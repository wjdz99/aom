...
static AOM_INLINE void write_intra_luma_mode(MACROBLOCKD *const xd,
  ...
  if (mode_idx >= FIRST_MODE_COUNT + SECOND_MODE_COUNT + THIRD_MODE_COUNT * 2)
    primary_flag++;
  aom_write_symbol(w, primary_flag, ec_ctx->y_mode_set_cdf[context],
                   INTRA_MODE_SETS);
  if (primary_flag == 0) {
    aom_write_symbol(w, mode_idx, ec_ctx->y_first_mode_cdf[context],
                     FIRST_MODE_COUNT);
  } else if (primary_flag == 1) {
    aom_write_symbol(w, mode_idx - FIRST_MODE_COUNT,
                     ec_ctx->y_second_mode_cdf[context], SECOND_MODE_COUNT);
  } else {
    aom_write_symbol(w,
                     mode_idx - FIRST_MODE_COUNT - SECOND_MODE_COUNT -
                         (primary_flag - 2) * THIRD_MODE_COUNT,
                     ec_ctx->y_third_mode_cdf[context], THIRD_MODE_COUNT);
  }
  if (mbmi->joint_y_mode < NON_DIRECTIONAL_MODES_COUNT)
    assert(mbmi->joint_y_mode == mbmi->mode_idx);
}
...
static AOM_INLINE void write_intra_uv_mode(MACROBLOCKD *const xd,
  ...
  aom_write_symbol(w, uv_mode_idx, ec_ctx->uv_first_mode_cdf[context],
                   UV_INTRA_MODES);
}
#endif  // CONFIG_INTRA_MODE_CODING
