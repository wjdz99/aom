...
static void write_inter_mode(aom_writer *w, PREDICTION_MODE mode,
                             ...
                             int is_compound,
#endif  // CONFIG_REF_MV && CONFIG_EXT_INTER && !CONFIG_COMPOUND_SINGLEREF
...
#define IS_NEWMV_MODE(mode) ((mode) == NEWMV || (mode) == NEWFROMNEARMV)
#else
...
#if CONFIG_EXT_INTER && !CONFIG_COMPOUND_SINGLEREF
  if (!is_compound && IS_NEWMV_MODE(mode))
...
static void pack_inter_mode_mvs(AV1_COMP *cpi, const MODE_INFO *mi,
  ...
  if (!is_inter) {
    ...
    // TODO(zoeliu): To work with more compound modes for the single ref case
    if (!segfeature_active(seg, segment_id, SEG_LVL_REF_FRAME)) {
#if 0
      if (!is_compound && bsize >= BLOCK_8X8 && is_inter_compound_mode(mode) &&
          mode != NEAREST_NEARMV) {
...
#endif  // 0
      // NOTE: Handle single ref comp mode (not supported for sub8x8)
...
#if CONFIG_EXT_INTER && CONFIG_COMPOUND_SINGLEREF
    if (!has_second_ref(&m->mbmi) && m->mbmi.sb_type >= BLOCK_8X8 &&
        is_inter_compound_mode(m->mbmi.mode)) {
...
#define FRAME_TO_CHECK 2
      if (cm->current_video_frame == FRAME_TO_CHECK &&
        ...
        for (ref = 0; ref < 1 + is_comp_ref; ++ref) {
          // Block mvs
          mv[ref].as_mv = (bsize < BLOCK_8X8) ? m->bmi[0].as_mv[ref].as_mv
                                              : m->mbmi.mv[ref].as_mv;
        }
        ...
        printf(
            ...
            cm->current_video_frame, mi_row, mi_col, mbmi->mode, bsize, b_mode,
