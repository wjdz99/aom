...
static void write_modes_sb(AV1_COMP *const cpi, const TileInfo *const tile,
  ...
  if (subsize < BLOCK_8X8 && !unify_bsize) {
    writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col);
  } else {
    switch (partition) {
      case PARTITION_NONE:
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col);
      ...
      case PARTITION_HORZ:
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col);
        if (mi_row + hbs < cm->mi_rows)
          writes_mode_b(cpi, tile, w, tok, tok_end, mi_row + hbs, mi_col);
        break;
      case PARTITION_VERT:
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col);
        if (mi_col + hbs < cm->mi_cols)
          writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col + hbs);
        break;
      case PARTITION_SPLIT:
      ...
      case PARTITION_HORZ_A:
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col);
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row + qbs, mi_col);
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row + hbs, mi_col);
        break;
      case PARTITION_HORZ_B:
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col);
        ...
        if (mi_row + 3 * qbs < cm->mi_rows)
          writes_mode_b(cpi, tile, w, tok, tok_end, mi_row + 3 * qbs, mi_col);
        break;
      case PARTITION_VERT_A:
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col);
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col + qbs);
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col + hbs);
        break;
      case PARTITION_VERT_B:
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col);
        ...
        if (mi_col + 3 * qbs < cm->mi_cols)
          writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col + 3 * qbs);
        break;
#else
      case PARTITION_HORZ_A:
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col);
      ...
      case PARTITION_HORZ_B:
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col);
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row + hbs, mi_col);
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row + hbs, mi_col + hbs);
        break;
      case PARTITION_VERT_A:
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, mi_col);
        writes_mode_b(cpi, tile, w, tok, tok_end, mi_row + hbs, mi_col);
      ...
      case PARTITION_HORZ_4:
        for (i = 0; i < 4; ++i) {
          ...
          writes_mode_b(cpi, tile, w, tok, tok_end, this_mi_row, mi_col);
        }
        break;
      case PARTITION_VERT_4:
        for (i = 0; i < 4; ++i) {
          ...
          writes_mode_b(cpi, tile, w, tok, tok_end, mi_row, this_mi_col);
        }
        break;
#endif  // CONFIG_EXT_PARTITION_TYPES
      default: assert(0);
...
static void write_modes(AV1_COMP *const cpi, const TileInfo *const tile,
  ...
  for (mi_row = mi_row_start; mi_row < mi_row_end; mi_row += cm->mib_size) {
    ...
    for (mi_col = mi_col_start; mi_col < mi_col_end; mi_col += cm->mib_size) {
      write_modes_sb(cpi, tile, w, tok, tok_end, 0, mi_row, mi_col,
                     cm->sb_size);
#if CONFIG_MOTION_VAR && NC_MODE_INFO
