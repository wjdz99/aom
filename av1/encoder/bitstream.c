...
static uint32_t write_sequence_header_obu(AV1_COMP *cpi, uint8_t *const dst,
                                          uint8_t enhancement_layer_count) {
  AV1_COMMON *const cm = &cpi->common;
  struct aom_write_bit_buffer wb = { dst, 0 };
  uint32_t size = 0;
  ...
  if (cm->seq_params.reduced_still_picture_hdr) {
    ...
    uint8_t operating_points_minus1_cnt = enhancement_layer_count;
...
int av1_pack_bitstream(AV1_COMP *const cpi, uint8_t *dst, size_t *size) {
  ...
  const uint8_t enhancement_layer_count = cm->enhancement_layer_count;
