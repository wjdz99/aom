...
#if 0
#define FRAME_TO_EXAMINE 1
    if (cm->current_video_frame == FRAME_TO_EXAMINE) {
      ...
      if (mbmi->sb_type < BLOCK_8X8) {
        ...
        mv_row[0] = mbmi->mv[0].as_mv.row;
        mv_col[0] = mbmi->mv[0].as_mv.col;
        if (is_compound) {
          mv_row[1] = mbmi->mv[1].as_mv.row;
          mv_col[1] = mbmi->mv[1].as_mv.col;
        } else {
          mv_row[1] = -1;
          mv_col[1] = -1;
        }
