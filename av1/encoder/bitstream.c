...
#if CONFIG_MRLS
static AOM_INLINE void write_mrl_index(FRAME_CONTEXT *ec_ctx, uint8_t mrlIdx,
                                       aom_writer *w) {
  aom_write_symbol(w, mrlIdx, ec_ctx->mrl_index_cdf, MRL_LINE_NUMBER);
}
#endif
...
static AOM_INLINE void write_intra_prediction_modes(AV1_COMP *cpi,
  ...
  if (xd->tree_type != CHROMA_PART) {
    ...
    if (av1_is_directional_mode(mode)) {
      write_mrl_index(ec_ctx, mbmi->mrlIdx, w);
    }
#endif
#if CONFIG_SDP
  }
