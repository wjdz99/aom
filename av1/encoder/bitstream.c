...
#if CONFIG_CCSO
static AOM_INLINE void write_ccso(AV1_COMMON *cm, MACROBLOCKD *const xd, aom_writer *w) {
  if (cm->features.coded_lossless) return;
  if (cm->features.allow_intrabc) return;
  const CommonModeInfoParams *const mi_params = &cm->mi_params;
  const int mi_row = xd->mi_row;
  const int mi_col = xd->mi_col;
  const int m = ~((1 << (CCSO_BLK_SIZE + xd->plane[1].subsampling_x - MI_SIZE_LOG2)) - 1);
  const MB_MODE_INFO *mbmi =
    mi_params->mi_grid_base[(mi_row & m) * mi_params->mi_stride + (mi_col & m)];

  if (!(mi_row & ((1 << CCSO_BLK_SIZE >> 1) - (MI_SIZE_LOG2 - xd->plane[1].subsampling_y))) &&
    !(mi_col & ((1 << CCSO_BLK_SIZE >> 1) - (MI_SIZE_LOG2 - xd->plane[1].subsampling_x))) && cm->ccso_info.ccso_enable[0]){
    aom_write_bit(w, mbmi->ccso_blk_u == 0 ? 0 : 1);
    xd->ccso_blk_u = mbmi->ccso_blk_u;
  }

  if (!(mi_row & ((1 << CCSO_BLK_SIZE >> 1) - (MI_SIZE_LOG2 - xd->plane[2].subsampling_y))) &&
    !(mi_col & ((1 << CCSO_BLK_SIZE >> 1) - (MI_SIZE_LOG2 - xd->plane[2].subsampling_x))) && cm->ccso_info.ccso_enable[1]) {
      ...
      for (int d0 = 0; d0 < 3; d0++) {
        for (int d1 = 0; d1 < 3; d1++) {
          ...
          for (int offset_idx = 0; offset_idx < 8; offset_idx++) {
            if (cm->ccso_info.filter_offset[plane][lut_idx_ext] == ccso_offset[offset_idx]) {
              aom_wb_write_literal(wb, offset_idx, (CCSO_DYNAMIC_RANGE + 1));
              break;
            }
          }
        }
