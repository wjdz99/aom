...
static void write_q_segment_id(const AV1_COMMON *cm,
  ...
  if ((mi_row - MAX_MIB_SIZE) >= 0 && (mi_col - MAX_MIB_SIZE) >= 0)
    prev_ul = get_segment_id(cm, cm->q_seg_map, bsize,
                             mi_row - MAX_MIB_SIZE, mi_col - MAX_MIB_SIZE);
  if ((mi_row - MAX_MIB_SIZE) >= 0)
    prev_u = get_segment_id(cm, cm->q_seg_map, bsize,
                            mi_row - MAX_MIB_SIZE, mi_col - 0);
  if ((mi_row - MAX_MIB_SIZE) >= 0)
    prev_l = get_segment_id(cm, cm->q_seg_map, bsize, mi_row - 0,
                            mi_col - MAX_MIB_SIZE);

  int cdf_num = pick_q_seg_cdf(prev_ul, prev_u, prev_l);

  // TODO: figure out of median is better
  ...
  aom_write_symbol(w, coded_id, pred_cdf, seg->q_lvls);
#else
...
#if CONFIG_Q_SEGMENTATION
  if (!skip)
    write_q_segment_id(cm, mbmi, w, seg, segp, bsize, mi_row, mi_col);
#endif
...
#if CONFIG_Q_SEGMENTATION
static void encode_q_segmentation(AV1_COMMON *cm, struct aom_write_bit_buffer *wb)
{
  int i;
  ...
  for (i = 0; i < seg->q_lvls; i++) {
    const int val = get_segdata(seg, i, SEG_LVL_ALT_Q);
