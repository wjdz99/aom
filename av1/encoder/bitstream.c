...
static void write_tile_info(const AV1_COMMON *const cm,
  ...
  int tile_enable_flag = cm->tile_cols > 1 || cm->tile_rows > 1 ? 1 : 0;
  assert(cm->tile_cols <= 64);
  assert(cm->tile_rows <= 64);
  aom_wb_write_bit(wb, tile_enable_flag);
  if (tile_enable_flag) {
    ...
    if (!cm->uniform_spacing_flag && cm->tile_cols > 1) {
      for (int i = 0; i < cm->tile_cols; i++) {
        aom_wb_write_literal(wb, (ALIGN_POWER_OF_TWO(cm->tile_widths[i],
                             cm->mib_size_log2) >> cm->mib_size_log2) - 1, 6);
      }
    }
    if (!cm->uniform_spacing_flag && cm->tile_rows > 1) {
      for (int i = 0; i < cm->tile_rows; i++) {
        aom_wb_write_literal(wb, (ALIGN_POWER_OF_TWO(cm->tile_heights[i],
                             cm->mib_size_log2) >> cm->mib_size_log2) - 1, 6);
      }
    }
  }
