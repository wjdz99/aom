...
static void pack_inter_mode_mvs(AV1_COMP *cpi, const MODE_INFO *mi,
  ...
  if (!is_inter) {
    ...
    if (// cm->current_video_frame == FRAME_TO_CHECK &&
        cm->current_video_frame == 3 &&
        // mi_row == 1 && mi_col == 0 &&
        // m->mbmi.interinter_compound == COMPOUND_TRIPRED &&
        cm->show_frame == 1) {
      // For sub8x8, simply dump out the first sub8x8 block info
      const PREDICTION_MODE b_mode =
          (bsize < BLOCK_8X8) ? mi->bmi[0].as_mode : -1;
      const int mv_x = (bsize < BLOCK_8X8) ? mi->bmi[0].as_mv[0].as_mv.row
                                           : mbmi->mv[0].as_mv.row;
      const int mv_y = (bsize < BLOCK_8X8) ? mi->bmi[0].as_mv[0].as_mv.col
                                           : mbmi->mv[0].as_mv.col;

      // static int first_compound_tripred = 0;
      // if (mbmi->interinter_compound == COMPOUND_TRIPRED &&
      //     !first_compound_tripred) {
      printf(
          "=== ENCODER ===: Before pack_inter_mode_mvs(): "
          ...
          "is_last_bipred_frame=%d, is_bipred_frame=%d\n",
          cm->current_video_frame, mi_row, mi_col, mode, segment_id, bsize,
          b_mode, mv_x, mv_y, mbmi->ref_frame[0], mbmi->ref_frame[1],
          mbmi->interinter_compound,
          mbmi->interinter_compound == COMPOUND_TRIPRED ?
          mbmi->mv_third.as_mv.row : 0,
          mbmi->interinter_compound == COMPOUND_TRIPRED ?
          mbmi->mv_third.as_mv.col : 0,
          mbmi->interinter_compound == COMPOUND_TRIPRED ?
          mbmi->ref_frame_third : 0,
          cpi->rc.is_last_bipred_frame, cpi->rc.is_bipred_frame);
      // first_compound_tripred = 1;
      // }
    }
#endif  // 0
    ...
    if (bsize < BLOCK_8X8) {
      ...
      //               non-sub8x8 blocks
      if (is_compound && mbmi->interinter_compound == COMPOUND_TRIPRED) {
