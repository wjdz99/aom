...
static void write_modes_sb(AV1_COMP *const cpi, const TileInfo *const tile,
  ...
  if (subsize < BLOCK_8X8 && !unify_bsize) {
    writes_mode_b(cpi, tile, w, tok, tok_end, supertx_enabled, mi_row, mi_col);
  } else {
    switch (partition) {
      case PARTITION_NONE:
      ...
      case PARTITION_VERT:
        ...
        break;
      case PARTITION_SPLIT:
        write_modes_sb(cpi, tile, w, tok, tok_end, supertx_enabled, mi_row,
                       mi_col, subsize);
        write_modes_sb(cpi, tile, w, tok, tok_end, supertx_enabled, mi_row,
                       mi_col + hbs, subsize);
        write_modes_sb(cpi, tile, w, tok, tok_end, supertx_enabled,
      ...
      case PARTITION_HORZ_A:
        writes_mode_b(cpi, tile, w, tok, tok_end, supertx_enabled, mi_row,
                      mi_col);
        writes_mode_b(cpi, tile, w, tok, tok_end, supertx_enabled, mi_row + qbs,
                      mi_col);
        writes_mode_b(cpi, tile, w, tok, tok_end, supertx_enabled, mi_row + hbs,
                      mi_col);
        break;
      case PARTITION_HORZ_B:
        writes_mode_b(cpi, tile, w, tok, tok_end, supertx_enabled, mi_row,
