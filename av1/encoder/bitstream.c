...
static struct av1_token seg_id_spatial_prediction_encodings[SEG_ID_SPATIAL_PREDICTIONS];
static struct av1_token segment_tree_encodings[MAX_SEGMENTS - 1][MAX_SEGMENTS];
#endif
...
#if CONFIG_EXT_TX || CONFIG_PALETTE || CONFIG_EXT_SEGMENT
  int s;
...
#endif  // CONFIG_LOOP_RESTORATION
#if CONFIG_EXT_SEGMENT
  av1_tokens_from_tree(seg_id_spatial_prediction_encodings,
	  av1_seg_id_spatial_prediction_tree);
  for (s = 0; s < MAX_SEGMENTS - 1; s++) {
    av1_tokens_from_tree(segment_tree_encodings[s],
      av1_segment_tree[s]);
  }
#endif
...
#if CONFIG_EXT_SEGMENT
static void write_temp_unpred_segment_id(aom_writer *w, const struct segmentation *seg,
                                         struct segmentation_probs *segp, int segment_id, 
                                         int spatial_segment_id_predicted, 
                                         int spatial_pred_ctx) {
	if (seg->enabled && seg->update_map) {		
#if CONFIG_EC_MULTISYMBOL
		if (spatial_pred_ctx) {
			assert(spatial_segment_id_predicted < SEG_ID_COPY_ABOVE);
			aom_write(w, spatial_segment_id_predicted, segp->harmonic_spatial_pred_probs[0]);
		}
		else {
			aom_write_symbol(w, spatial_segment_id_predicted, segp->heterogeneous_spatial_pred_cdf, HETEROGENEOUS_SPATIAL_PRED_PROBS + 1);
		}
		if(!spatial_segment_id_predicted)	aom_write_symbol(w, segment_id, segp->tree_cdf, seg->num_seg);
#else
		if (spatial_pred_ctx) {
			assert(spatial_segment_id_predicted < SEG_ID_COPY_ABOVE);
			aom_write(w, spatial_segment_id_predicted, segp->harmonic_spatial_pred_probs[0]);
			...
			av1_write_token(w, av1_seg_id_spatial_prediction_tree,
				segp->heterogeneous_spatial_pred_probs,
				&seg_id_spatial_prediction_encodings[spatial_segment_id_predicted]);			
		}
		if (!spatial_segment_id_predicted)	
            av1_write_token(w, av1_segment_tree[seg->num_seg - 2],
                            segp->tree_probs,
                            &segment_tree_encodings[seg->num_seg - 2][segment_id]);          
#endif
	}
}
static void write_segment_id(aom_writer *w, const struct segmentation *seg,
                             struct segmentation_probs *segp, const uint8_t segment_id,
                             MACROBLOCKD *const xd, AV1_COMMON *const cm, 
                             MODE_INFO *mi, int skip_seg_id_write, int seg_cat_idx) {
	int seg_map_minb_size = 1 << (seg->seg_map_minb_size_log2_minus3);
	const int seg_id_written = !((-xd->mb_to_left_edge >> (MI_SIZE_LOG2 + 3)) & (seg_map_minb_size - 1)) &&
		!((-xd->mb_to_top_edge >> (MI_SIZE_LOG2 + 3)) & (seg_map_minb_size - 1));
    const int pred_segment_id =
      get_segment_id(cm, cm->last_frame_seg_map, mi->mbmi.sb_type, seg_cat_idx,
                     (-xd->mb_to_top_edge >> (MI_SIZE_LOG2 + 3)), 
                     (-xd->mb_to_left_edge >> (MI_SIZE_LOG2 + 3)));
	if (seg_id_written) xd->seg_id_is_done[seg_cat_idx] = 0;
	if (seg_id_written || !xd->seg_id_is_done[seg_cat_idx]) {
		int above_segment_id = xd->above_mi ? xd->above_mi->mbmi.segment_id[seg_cat_idx] : 0;
		int left_segment_id = xd->left_mi ? xd->left_mi->mbmi.segment_id[seg_cat_idx] : 0;
		const int spatial_pred_ctx = av1_get_spatial_pred_context(xd, seg_cat_idx);
		if (seg->temporal_update) {
			if (!skip_seg_id_write) {
				const int pred_flag = mi->mbmi.seg_id_predicted[seg_cat_idx];
				aom_prob pred_prob = av1_get_pred_prob_seg_id(segp, xd, seg_cat_idx);
				aom_write(w, pred_flag, pred_prob);
                if (!pred_flag) {
                  //re-derive seg_id_spatial_predicted signal due to neighboring skip block
                  mi->mbmi.seg_id_spatial_predicted[seg_cat_idx] =
                    xd->left_mi && xd->left_mi->mbmi.segment_id[seg_cat_idx] == segment_id ?
                    SEG_ID_COPY_LEFT :
                    xd->above_mi && xd->above_mi->mbmi.segment_id[seg_cat_idx] == segment_id ?
                    SEG_ID_COPY_ABOVE : SEG_ID_SPATIAL_UNPREDICTED;
                  write_temp_unpred_segment_id(w, seg, segp, segment_id,
                                               mi->mbmi.seg_id_spatial_predicted[seg_cat_idx],
                                               spatial_pred_ctx);
                }
				xd->seg_id_is_done[seg_cat_idx] = 1;
            } else {
              mi->mbmi.seg_id_predicted[seg_cat_idx] = 1;//treated as temporal predicted
              mi->mbmi.segment_id[seg_cat_idx] = pred_segment_id;
            }
		}
		else {
			if (!skip_seg_id_write) {
              mi->mbmi.seg_id_spatial_predicted[seg_cat_idx] =
                ...
                xd->above_mi && xd->above_mi->mbmi.segment_id[seg_cat_idx] == segment_id ?
                SEG_ID_COPY_ABOVE : SEG_ID_SPATIAL_UNPREDICTED;
              write_temp_unpred_segment_id(w, seg, segp, segment_id, mi->mbmi.seg_id_spatial_predicted[seg_cat_idx], spatial_pred_ctx);
				xd->seg_id_is_done[seg_cat_idx] = 1;
            } else {
              mi->mbmi.seg_id_spatial_predicted[seg_cat_idx] = xd->left_mi ? SEG_ID_COPY_LEFT :
                                                               xd->above_mi ? SEG_ID_COPY_ABOVE : 
                                                               SEG_ID_SPATIAL_UNPREDICTED;
              mi->mbmi.segment_id[seg_cat_idx] = xd->left_mi ? xd->left_mi->mbmi.segment_id[seg_cat_idx] :
                                                 xd->above_mi ? xd->above_mi->mbmi.segment_id[seg_cat_idx] :
                                                 0;
            }
		}
	}
...
#if CONFIG_EXT_SEGMENT
  if (segfeature_active(&cm->seg[ACTIVE_SEG_IDX], segment_id, ACTIVE_SEG_LVL_REF_FRAME)) {
    assert(!is_compound);
    assert(mbmi->ref_frame[0] ==
      get_segdata(&cm->seg[ACTIVE_SEG_IDX], segment_id, ACTIVE_SEG_LVL_REF_FRAME));
#else
...
static void pack_inter_mode_mvs(AV1_COMP *cpi,
#if CONFIG_EXT_SEGMENT
  MODE_INFO *mi,
#else
  const MODE_INFO *mi,
...
#endif
#if CONFIG_EXT_SEGMENT
  const struct segmentation *const seg[NUM_SEG_CATEGORIES] = { &cm->seg[0], &cm->seg[1] };
  struct segmentation_probs *const segp[NUM_SEG_CATEGORIES] = { &cm->fc->seg[0], &cm->fc->seg[1] };
#else
...
#if CONFIG_EXT_SEGMENT
  const uint8_t * segment_id = &mbmi->segment_id[0];
#else
  const int segment_id = mbmi->segment_id;
#endif
  const BLOCK_SIZE bsize = mbmi->sb_type;
  ...
  if (seg[ACTIVE_SEG_IDX]->update_map) {
    write_segment_id(w, seg[ACTIVE_SEG_IDX], segp[ACTIVE_SEG_IDX],
                     segment_id[ACTIVE_SEG_IDX], xd, cm, mi, 0, ACTIVE_SEG_IDX);
  }
#if CONFIG_SUPERTX
  if (supertx_enabled)
    skip = mbmi->skip;
  else
    skip = write_skip(cm, xd, segment_id[ACTIVE_SEG_IDX], mi, w);
#else
  skip = write_skip(cm, xd, segment_id[ACTIVE_SEG_IDX], mi, w);
#endif  // CONFIG_SUPERTX
#else
...
#if CONFIG_EXT_SEGMENT
  if (!segfeature_active(seg[ACTIVE_SEG_IDX], segment_id[ACTIVE_SEG_IDX], ACTIVE_SEG_LVL_REF_FRAME))
    aom_write(w, is_inter, av1_get_intra_inter_prob(cm, xd));
  if (seg[QUALITY_SEG_IDX]->update_map) {
    int seg_id_skip = (is_inter ||
      !av1_is_segfeature_enabled(seg[QUALITY_SEG_IDX], QUALITY_SEG_LVL_ALT_LF)
      )
      && skip;
    write_segment_id(w, seg[QUALITY_SEG_IDX], segp[QUALITY_SEG_IDX], 
                           segment_id[QUALITY_SEG_IDX], xd, cm, mi, seg_id_skip, QUALITY_SEG_IDX);
  }
#else
...
#endif  // CONFIG_SUPERTX
#if CONFIG_EXT_SEGMENT
      !(is_inter && skip) && !xd->lossless[segment_id[QUALITY_SEG_IDX]]) {
#else
...
#if CONFIG_EXT_SEGMENT
    if (!segfeature_active(seg[ACTIVE_SEG_IDX], segment_id[ACTIVE_SEG_IDX], ACTIVE_SEG_LVL_SKIP)) {
#else
...
static void write_mb_modes_kf(AV1_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (seg[ACTIVE_SEG_IDX]->update_map) 
    write_segment_id(w, seg[ACTIVE_SEG_IDX], segp[ACTIVE_SEG_IDX], 
                     mbmi->segment_id[ACTIVE_SEG_IDX], xd, cm, mi, 0, ACTIVE_SEG_IDX);
#if CONFIG_DELTA_Q
  skip = write_skip(cm, xd, mbmi->segment_id[ACTIVE_SEG_IDX], mi, w);
  if (cm->delta_q_present_flag) {
  ...
  if (seg[QUALITY_SEG_IDX]->update_map) {
    int segment_id_skip =
      ...
      && skip;
    write_segment_id(w, seg[QUALITY_SEG_IDX], segp[QUALITY_SEG_IDX],
      mbmi->segment_id[QUALITY_SEG_IDX], xd, cm, mi, segment_id_skip, QUALITY_SEG_IDX);
  }
...
static void encode_segmentation(AV1_COMMON *cm, MACROBLOCKD *xd,
                                struct aom_write_bit_buffer *wb) {
  int i, j;
  const struct segmentation *seg[NUM_SEG_CATEGORIES] = { (&cm->seg[0]), (&cm->seg[1]) };

  aom_wb_write_bit(wb, seg[ACTIVE_SEG_IDX]->enabled);
  aom_wb_write_bit(wb, seg[QUALITY_SEG_IDX]->enabled);
  if (!seg[ACTIVE_SEG_IDX]->enabled && !seg[QUALITY_SEG_IDX]->enabled) return;

  // Active segmentation map
  if (seg[ACTIVE_SEG_IDX]->enabled) {
    if (!frame_is_intra_only(cm) && !cm->error_resilient_mode) {
      aom_wb_write_bit(wb, seg[ACTIVE_SEG_IDX]->update_map);
    }
    else {
      assert(seg[ACTIVE_SEG_IDX]->update_map == 1);
    }
    if (seg[ACTIVE_SEG_IDX]->update_map) {
      // Select the coding strategy (temporal or spatial)      
      av1_choose_segmap_coding_method(cm, xd, ACTIVE_SEG_IDX);

      // Write out the chosen coding method.
      if (!frame_is_intra_only(cm) && !cm->error_resilient_mode) {
        aom_wb_write_bit(wb, seg[ACTIVE_SEG_IDX]->temporal_update);
      }
      else {
        assert(seg[ACTIVE_SEG_IDX]->temporal_update == 0);
      }
      aom_wb_write_literal(wb, seg[ACTIVE_SEG_IDX]->num_seg, 2);
      aom_wb_write_literal(wb, seg[ACTIVE_SEG_IDX]->seg_map_minb_size_log2_minus3, 2);
    }

    // Segmentation data
    aom_wb_write_bit(wb, seg[ACTIVE_SEG_IDX]->update_data);
    if (seg[ACTIVE_SEG_IDX]->update_data) {
      //aom_wb_write_bit(wb, seg[ACTIVE_SEG_IDX]->abs_delta); //not applicable for active segments
      for (i = 0; i < seg[ACTIVE_SEG_IDX]->num_seg; i++) {
        for (j = 0; j < ACTIVE_SEG_LVL_MAX; j++) {
          const int active = segfeature_active(seg[ACTIVE_SEG_IDX], i, j);
          aom_wb_write_bit(wb, active);
          if (active) {
            const int data = get_segdata(seg[ACTIVE_SEG_IDX], i, j);
            const int data_max = av1_seg_feature_data_max(j, ACTIVE_SEG_IDX);
  ...
  // Quality segmentation map
  if (seg[QUALITY_SEG_IDX]->enabled) {
    if (!frame_is_intra_only(cm) && !cm->error_resilient_mode) {
      aom_wb_write_bit(wb, seg[QUALITY_SEG_IDX]->update_map);
    }
    else {
      assert(seg[QUALITY_SEG_IDX]->update_map == 1);
    }
    if (seg[QUALITY_SEG_IDX]->update_map) {
      // Select the coding strategy (temporal or spatial)
      //spatial prediction is not accurated here 
      //due to skip info is not considered here
      //So the decision on temporal or spatial is not always optimal
      av1_choose_segmap_coding_method(cm, xd, QUALITY_SEG_IDX);

      // Write out the chosen coding method.
      if (!frame_is_intra_only(cm) && !cm->error_resilient_mode) {
        aom_wb_write_bit(wb, seg[QUALITY_SEG_IDX]->temporal_update);
      }
      else {
        assert(seg[QUALITY_SEG_IDX]->temporal_update == 0);
      }
      aom_wb_write_literal(wb, seg[QUALITY_SEG_IDX]->num_seg, 4);
      aom_wb_write_literal(wb, seg[QUALITY_SEG_IDX]->seg_map_minb_size_log2_minus3, 2);
    }

    // Segmentation data
    aom_wb_write_bit(wb, seg[QUALITY_SEG_IDX]->update_data);
    if (seg[QUALITY_SEG_IDX]->update_data) {
      aom_wb_write_bit(wb, seg[QUALITY_SEG_IDX]->abs_delta); //not applicable for active segments
      for (i = 0; i < seg[QUALITY_SEG_IDX]->num_seg; i++) {
        for (j = 0; j < QUALITY_SEG_LVL_MAX; j++) {
          const int active = segfeature_active(seg[QUALITY_SEG_IDX], i, j);
          aom_wb_write_bit(wb, active);
          if (active) {
            const int data = get_segdata(seg[QUALITY_SEG_IDX], i, j);
            const int data_max = av1_seg_feature_data_max(j, QUALITY_SEG_IDX);
...
static void update_seg_probs(AV1_COMP *cpi, aom_writer *w) {
  ...
  if ((!cm->seg[ACTIVE_SEG_IDX].enabled || !cm->seg[ACTIVE_SEG_IDX].update_map) 
      && (!cm->seg[QUALITY_SEG_IDX].enabled || !cm->seg[QUALITY_SEG_IDX].update_map))
    return;

  int i;
  if (cm->seg[ACTIVE_SEG_IDX].update_map) {
    if (cm->seg[ACTIVE_SEG_IDX].temporal_update) {
      for (i = 0; i < PREDICTION_PROBS; i++)
        av1_cond_prob_diff_update(w, &cm->fc->seg[ACTIVE_SEG_IDX].pred_probs[i],
          cm->counts.seg[ACTIVE_SEG_IDX].temp_pred[i], probwt);
    }
    av1_cond_prob_diff_update(w, &cm->fc->seg[ACTIVE_SEG_IDX].harmonic_spatial_pred_probs[0],
      cm->counts.seg[ACTIVE_SEG_IDX].harmonic_spatial_pred, probwt);
    prob_diff_update(av1_seg_id_spatial_prediction_tree, cm->fc->seg[ACTIVE_SEG_IDX].heterogeneous_spatial_pred_probs,
      cm->counts.seg[ACTIVE_SEG_IDX].heterogeneous_spatial_pred, HETEROGENEOUS_SPATIAL_PRED_PROBS + 1, probwt, w);
    prob_diff_update(av1_segment_tree[cm->seg[ACTIVE_SEG_IDX].num_seg - 2], cm->fc->seg[ACTIVE_SEG_IDX].tree_probs,
      cm->counts.seg[ACTIVE_SEG_IDX].tree_mispred, cm->seg[ACTIVE_SEG_IDX].num_seg, probwt, w);
  }
  if (cm->seg[QUALITY_SEG_IDX].update_map) {
    if (cm->seg[QUALITY_SEG_IDX].temporal_update) {
      for (i = 0; i < PREDICTION_PROBS; i++)
        av1_cond_prob_diff_update(w, &cm->fc->seg[QUALITY_SEG_IDX].pred_probs[i],
          cm->counts.seg[QUALITY_SEG_IDX].temp_pred[i], probwt);
    }
    av1_cond_prob_diff_update(w, &cm->fc->seg[QUALITY_SEG_IDX].harmonic_spatial_pred_probs[0],
      cm->counts.seg[QUALITY_SEG_IDX].harmonic_spatial_pred, probwt);
    prob_diff_update(av1_seg_id_spatial_prediction_tree, cm->fc->seg[QUALITY_SEG_IDX].heterogeneous_spatial_pred_probs,
      cm->counts.seg[QUALITY_SEG_IDX].heterogeneous_spatial_pred, HETEROGENEOUS_SPATIAL_PRED_PROBS + 1, probwt, w);
    prob_diff_update(av1_segment_tree[cm->seg[QUALITY_SEG_IDX].num_seg - 2], cm->fc->seg[QUALITY_SEG_IDX].tree_probs,
      cm->counts.seg[QUALITY_SEG_IDX].tree_mispred, cm->seg[QUALITY_SEG_IDX].num_seg, probwt, w);
  }
#else
...
#endif
#if CONFIG_EXT_SEGMENT
  if (!cm->seg[QUALITY_SEG_IDX].enabled && xd->lossless[0])
#else
