...
#if CONFIG_FRAME_SIZE
  if (!found) {
    int frame_size_override = 1; //Allways equal to 1 in write_frame_size_with_refs()
...
static void write_uncompressed_header_frame(AV1_COMP *cpi,
  ...
  int frame_size_override_flag =
    (cm->width != cm->seq_params.max_frame_width || cm->height != cm->seq_params.max_frame_height);
