...
static void write_palette_colors_uv(const PALETTE_MODE_INFO *const pmi,
  ...
  // U channel colors.
  int min_bits_u = 0;
  bits = av1_get_palette_delta_bits_u(pmi, bit_depth, &min_bits_u);
  ...
  bits = av1_get_palette_delta_bits_v(pmi, bit_depth, &zero_count, &min_bits_v);
  const int rate_using_delta =
      2 + bit_depth + (bits + 1) * (n - 1) - zero_count;
  ...
  if (rate_using_delta < rate_using_raw) {  // delta encoding
    ...
    for (i = 1; i < n; ++i) {
      ...
      if (delta <= max_val - delta) {
        aom_write_literal(w, delta, bits);
        aom_write_bit(w, sign_bit);
      } else {
        aom_write_literal(w, max_val - delta, bits);
        aom_write_bit(w, !sign_bit);
      }
    }
