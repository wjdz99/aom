...
static const struct av1_token inter_ref0_encodings[INTER_REFS_PER_FRAME];
    ...
    if (cm->reference_mode != COMPOUND_REFERENCE) {
      for (i = 0; i < REF0_CONTEXTS; i++) {
        for (j = 0; j < (INTER_REFS_PER_FRAME - 1); j++) {
          av1_cond_prob_diff_update(header_bc, &fc->inter_ref0_prob[i][j],
                                    counts->inter_ref0[i][j], probwt);
        }
      }
    }
#else
