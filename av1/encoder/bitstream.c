...
#if CONFIG_DAALA_EC
  /* This horrible hack is because the four TX_TYPES are not consecutive, e.g.,
      0, 1, 2, 3, when doing an in-order traversal of the av1_ext_tx_tree
      structure. */
...
static void pack_inter_mode_mvs(AV1_COMP *cpi, const MODE_INFO *mi,
...
#if CONFIG_DAALA_EC
      od_ec_encode_cdf_q15(&w->ec, mbmi->interp_filter,
  ...
  if (mbmi->tx_size < TX_32X32 && cm->base_qindex > 0 && !mbmi->skip &&
    ...
    if (is_inter) {
#if CONFIG_DAALA_EC
      od_ec_encode_cdf_q15(&w->ec, av1_ext_tx_ind[mbmi->tx_type],
...
#if CONFIG_DAALA_EC
    od_ec_encode_cdf_q15(&w->ec, p, cm->fc->partition_cdf[ctx],
                         PARTITION_TYPES);
#else
