...
static void write_mb_interp_filter(AV1_COMP *cpi, const MACROBLOCKD *xd,
  ...
  if (cm->interp_filter == SWITCHABLE) {
    ...
    for (int dir = 0; dir < 2; ++dir) {
      if (dir && !enable_dual_filter) return;
    ...
    if (av1_CRLC_corners_in_sb(cm, 0, mi_row, mi_col, bsize, &rcol0, &rcol1,
      ...
      for (int rrow = rrow0; rrow < rrow1; ++rrow) {
        for (int rcol = rcol0; rcol < rcol1; ++rcol) {
          ...
          if (cm->use_guided_cnn && cm->use_full_crlc == 0) {
...
static void write_filter_crlc(MACROBLOCKD *xd, int QP, const CRLCInfo *ci,
  ...
  for (int i = 0; i < ci->units_per_tile; i++) {
    ...
    aom_write_primitive_refsubexpfin(wb, 16, 1, 1, b_a0);
    aom_write_primitive_refsubexpfin(wb, 16, 1, 1, b_a1);
  }
}
#endif  // CONFIG_CNN_CRLC_GUIDED
...
#if CONFIG_CNN_CRLC_GUIDED
static void crlc_guided_write_coeffs(AV1_COMMON *const cm, MACROBLOCKD *xd,
                                     const RestorationUnitInfo *rui,
                                     aom_writer *const w,
                                     FRAME_COUNTS *counts) {
...
#if CONFIG_CNN_CRLC_GUIDED
    aom_wb_write_bit(wb, cm->use_guided_cnn);
#endif  // CONFIG_CNN_CRLC_GUIDED
  } else {
    assert(!cm->use_cnn);
  }
