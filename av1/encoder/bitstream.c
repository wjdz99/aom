...
static void encode_restoration_mode(AV1_COMMON *cm,
  ...
  for (int p = plane_start; p < num_planes; ++p) {
    ...
    aom_wb_write_bit(wb, cm->frame_contains_merge[p]);
...
static void write_wiener_nsfilter(MACROBLOCKD *xd, int is_uv,
  ...
  if (write_merge) {
    const int equal =
        check_wienerns_eq(is_uv, wienerns_info, ref_wienerns_info);
    aom_write_symbol(wb, equal, xd->tile_ctx->merged_param_cdf, 2);
    if (equal) {
      return;
    }
  ...
  if (frame_rtype == RESTORE_SWITCHABLE) {
    ...
    switch (unit_rtype) {
      case RESTORE_WIENER:
        ...
        break;
      case RESTORE_SGRPROJ:
        write_sgrproj_filter(xd, &rui->sgrproj_info, ref_sgrproj_info,
#if CONFIG_RST_MERGECOEFFS
                             cm->frame_contains_merge[plane],
#endif  // CONFIG_RST_MERGECOEFFS
                             w);
