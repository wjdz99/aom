...
static AOM_INLINE void write_intra_y_mode_kf(MACROBLOCKD *const xd,
                                             aom_writer *w) {
...
static void write_tx_partition_intra(const MACROBLOCKD *xd, aom_writer *w,
  ...
  if (block_signals_txsize(bsize)) {
    ...
    if (allow_horz && allow_vert) {
      ...
      aom_write_symbol(
          w, split4_partition,
          ec_ctx->intra_4way_txfm_partition_cdf[is_rect][split4_ctx], 4);
      if (((split4_partition == TX_PARTITION_VERT) && allow_vert2) ||
          ((split4_partition == TX_PARTITION_HORZ) && allow_horz2)) {
        const int has_split = (partition == TX_PARTITION_HORZ4) ||
                              (partition == TX_PARTITION_VERT4);
        aom_write_symbol(w, has_split,
                         ec_ctx->intra_2way_rect_txfm_partition_cdf, 2);
      }
    } else if (allow_horz || allow_vert) {
      const int has_first_split = partition != TX_PARTITION_NONE;
      aom_write_symbol(w, has_first_split,
                       ec_ctx->intra_2way_txfm_partition_cdf, 2);
      if (has_first_split && (allow_horz2 || allow_vert2)) {
        const int has_second_split = (partition == TX_PARTITION_VERT4) ||
                                     (partition == TX_PARTITION_HORZ4);
        aom_write_symbol(w, has_second_split,
    ...
    default:
      ...
      if (mbmi->motion_mode == WARPED_CAUSAL) {
        aom_write_symbol(w, mbmi->rot_flag,
                         xd->tile_ctx->warp_rotation_flag_cdf[mbmi->sb_type],
                         2);
        if (mbmi->rot_flag) {
          aom_write_symbol(w, (mbmi->rotation + ROTATION_RANGE) / ROTATION_STEP,
                           xd->tile_ctx->warp_rotation_degree_cdf,
                           ROTATION_COUNT);
        }
      }
...
static AOM_INLINE void write_mb_interp_filter(AV1_COMMON *const cm,
  ...
  if (!av1_is_interp_needed(xd)) {
    ...
    // In optical flow refinement, use the sharp filter instead of the frame
    // level interpolation filter
...
static AOM_INLINE void write_intra_y_mode_nonkf(const MACROBLOCKD *xd,
                                                aom_writer *w) {
  FRAME_CONTEXT *frame_ctx = xd->tile_ctx;
  const MB_MODE_INFO *mbmi = xd->mi[0];
  const BLOCK_SIZE bsize = mbmi->sb_type;
...
static AOM_INLINE void write_intra_prediction_modes(AV1_COMP *cpi,
  ...
  // Y mode.
  if (is_keyframe) {
    write_intra_y_mode_kf(xd, w);
  } else {
    write_intra_y_mode_nonkf(xd, w);
  }
  ...
  if (!cm->seq_params.monochrome && xd->is_chroma_ref) {
    const UV_PREDICTION_MODE uv_mode = mbmi->uv_mode;
    write_intra_uv_mode(ec_ctx, xd, is_cfl_allowed(xd), w);
