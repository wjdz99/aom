...
static void write_q_segment_id(const AV1_COMMON *cm,
  ...
  if (!seg->q_lvls || mbmi->skip) {
    set_q_segment_id(cm, cm->q_seg_map, mbmi->sb_type, mi_row, mi_col, 0);
    return;
  }
  ...
  if (above && left)
    prev_ul = get_segment_id(cm, cm->q_seg_map, BLOCK_4X4, mi_row - 1, mi_col - 1);
  ...
  set_q_segment_id(cm, cm->q_seg_map, bsize, mi_row, mi_col, mbmi->q_segment_id);
}
#endif
