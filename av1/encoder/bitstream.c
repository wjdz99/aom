...
static void write_modes_sb(AV1_COMP *cpi, const TileInfo *const tile,
  ...
  if (bsize == BLOCK_64X64 && cm->clpf_blocks && cm->clpf_strength_y &&
    ...
    if (cm->clpf_blocks[tl] >= 0) aom_write_literal(w, cm->clpf_blocks[tl], 1);

    if (mi_col + MI_SIZE / 2 < cm->mi_cols && cm->clpf_blocks[tr] != CLPF_NOFLAG)
      aom_write_literal(w, cm->clpf_blocks[tr], 1);

    if (mi_row + MI_SIZE / 2 < cm->mi_rows && cm->clpf_blocks[bl] != CLPF_NOFLAG)
      aom_write_literal(w, cm->clpf_blocks[bl], 1);

    if (mi_row + MI_SIZE / 2 < cm->mi_rows &&
        mi_col + MI_SIZE / 2 < cm->mi_cols && cm->clpf_blocks[br] != CLPF_NOFLAG)
