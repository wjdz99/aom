...
static void encode_clpf(const AV1_COMMON *cm,
  ...
  if (cm->clpf_strength) {
    ...
    if (cm->clpf_size) {
      int i;
      // TODO?: The number of bits to transmit could be implicitly
      // deduced if transmitted after the filter block or after the
      // frame (when it's known whether the block is all skip and
      // implicitly unfiltered).  And the bits do not have 50%
      // probability, so a more efficient coding is possible.
      aom_wb_write_literal(wb, cm->clpf, clpf_maxbits(cm));
      for (i = 0; i < cm->clpf; i++) {
