...
static void write_segment_id(const AV1_COMMON *cm,
  ...
  if ((mi_row - MAX_MIB_SIZE) >= 0 && (mi_col - MAX_MIB_SIZE) >= 0)
    prev_ul = get_segment_id(cm, cm->current_frame_seg_map, bsize,
                             mi_row - MAX_MIB_SIZE, mi_col - MAX_MIB_SIZE);
  if ((mi_row - MAX_MIB_SIZE) >= 0)
    prev_u = get_segment_id(cm, cm->current_frame_seg_map, bsize,
                            mi_row - MAX_MIB_SIZE, mi_col - 0);
  if ((mi_row - MAX_MIB_SIZE) >= 0)
    prev_l = get_segment_id(cm, cm->current_frame_seg_map, bsize, mi_row - 0,
                            mi_col - MAX_MIB_SIZE);
