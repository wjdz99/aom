...
static AOM_INLINE void write_ccso(AV1_COMMON *cm, MACROBLOCKD *const xd,
  ...
  const MB_MODE_INFO *mbmi =
      mi_params->mi_grid_base[(mi_row & m_row) * mi_params->mi_stride +
                              (mi_col & m_col)];

  if (!(mi_row &
        ((1 << CCSO_BLK_SIZE >> (MI_SIZE_LOG2 - xd->plane[1].subsampling_y)) -
         1)) &&
      !(mi_col &
        ((1 << CCSO_BLK_SIZE >> (MI_SIZE_LOG2 - xd->plane[1].subsampling_x)) -
         1)) &&
  ...
  if (!(mi_row &
        ((1 << CCSO_BLK_SIZE >> (MI_SIZE_LOG2 - xd->plane[2].subsampling_y)) -
         1)) &&
      !(mi_col &
        ((1 << CCSO_BLK_SIZE >> (MI_SIZE_LOG2 - xd->plane[2].subsampling_x)) -
         1)) &&
