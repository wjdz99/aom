...
static void write_intra_y_mode_kf(MACROBLOCKD *const xd,
  ...
  av1_get_if_y_mode_cdf_ml(xd, cdf);
  ...
  const uint64_t *above_hist =
      av1_block_mode(xd->above_mbmi, &above, &above_var);
  const uint64_t *left_hist =
      av1_block_mode(xd->left_mbmi, &left, &left_var);
  const uint64_t *al_hist =
      av1_block_mode(xd->aboveleft_mbmi, &aboveleft, &al_var);
...
static void write_intra_uv_mode(const MACROBLOCKD *xd,
                                FRAME_CONTEXT *frame_ctx,
  ...
  int64_t y_var[2];
  const uint64_t *above_hist = av1_block_mode(xd->above_mbmi, y_mode1, y_var);
  const uint64_t *left_hist = av1_block_mode(xd->left_mbmi, y_mode1 + 1,
                                             y_var + 1);
  FILE *fp = fopen("intra_data.csv", "a");
  if (fp) {
    fprintf(fp,
