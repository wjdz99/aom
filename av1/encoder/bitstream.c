...
#if CONFIG_NCOBMC_ADAPT_WEIGHT
static void write_ncobmc_adapt_weight_mode(const AV1_COMMON *cm,
                                           const MODE_INFO *mi, aom_writer *w) {
  const MB_MODE_INFO *mbmi = &mi->mbmi;
  ADAPT_OVERLAP_BLOCK ao_block = adapt_overlap_block_lookup[mbmi->sb_type];
  if (intrpl_mode_allowed(mbmi->sb_type) == NO_OVERLAP) return;

  av1_write_token(w, av1_intrpl_mode_tree,
                  cm->fc->intrpl_mode_prob[mbmi->motion_mode][ao_block],
...
#if CONFIG_NCOBMC_ADAPT_WEIGHT
    write_ncobmc_adapt_weight_mode(cm, mi, w);
#endif
#endif  // CONFIG_MOTION_VAR || CONFIG_WARPED_MOTION
...
static uint32_t write_compressed_header(AV1_COMP *cpi, uint8_t *data) {
  ...
  if (frame_is_intra_only(cm)) {
    ...
    for (i = 0; i < MOTION_MODES; ++i) {
      for (j = ADAPT_OVERLAP_BLOCK_8X8; j < ADAPT_OVERLAP_BLOCKS; ++j) {
