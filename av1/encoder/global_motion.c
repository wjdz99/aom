...
int compute_global_motion_feature_based(
  ...
  // For motions whose number of inliers is too low, reset num_inliers
  // to 0 so they are ignored.
  for (int motion_i = 0; motion_i < num_motions; ++motion_i) {
    if (num_inliers_by_motion[motion_i] <
        MIN_INLIER_PROB * num_correspondences) {
      num_inliers_by_motion[motion_i] = 0;
    }
  }

  // Return true if any one of the motions has inliers.
  for (int motion_i = 0; motion_i < num_motions; ++motion_i) {
    if (num_inliers_by_motion[motion_i] > 0) return 1;
  }
  return 0;
}
