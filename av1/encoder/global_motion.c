...
static INLINE int64_t get_erroradv_threshold(double scaling_factor,
                                             int64_t erroradv_threshold) {
  return erroradv_threshold < (int64_t)((INT64_MAX / scaling_factor) + 0.5)
...
int64_t av1_refine_integerized_param(
  ...
  for (i = 0; i < n_refinements; i++, step >>= 1) {
    int64_t error_adv_tr =
        get_erroradv_threshold(tr_factors[i], erroradv_threshold);
    for (p = 0; p < n_params; ++p) {
      ...
      *param = add_param_offset(p, curr_param, -step);
      step_error = av1_warp_error(
          ...
          AOMMIN(best_error, error_adv_tr), segment_map, segment_map_stride);
