...
int compute_global_motion_feature_based(
  ...
  int num_correspondences, num_required_correspondences;
  int *correspondences;
  ...
  num_required_correspondences = MIN_INLIER_PROB * num_correspondences;
  for (i = 0; i < num_motions; ++i) {
    if (num_inliers_by_motion[i] < num_required_correspondences) {
