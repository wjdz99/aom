...
static uint16_t setup_interp_filter_search_mask(AV1_COMP *cpi) {
  ...
  if (cm->current_frame.frame_type == KEY_FRAME || frame_update_type == ARF_UPDATE || frame_update_type == GF_UPDATE)
    return mask;
  ...
  int mul = QINDEX_RANGE * 8 / (cm->base_qindex + 1);
  mul = clamp(mul, 10, 100);
  for (InterpFilter ifilter = EIGHTTAP_REGULAR; ifilter <= MULTITAP_SHARP;
       ++ifilter) {
    int last_score = get_interp_filter_selected(cm, nearest_ref, ifilter) * mul;
    if (ref_total[nearest_ref] && last_score <= ref_total[nearest_ref]) {
      int filter_score = 0;
      for (MV_REFERENCE_FRAME ref = LAST_FRAME; ref <= ALTREF_FRAME; ++ref) {
        if (ref != nearest_ref) {
          filter_score += get_interp_filter_selected(cm, ref, ifilter) * (mul >> 1);
        }
      }
