...
void av1_change_config(struct AV1_COMP *cpi, AV1EncoderConfig *oxcf) {
  AV1_COMMON *const cm = &cpi->common;
...
#if 1
      int level;
      av1_get_seq_level_idx(cpi, &level);
      printf("\n level is %d\n", level);
#endif
    }
#endif  // CONFIG_INTERNAL_STATS
...
static void recode_loop_update_q(
  ...
  // Max bandwidth enforced for current frame in order to avoid bitrate spike
  // and meet the target level.
  int level_forced_max_bandwidth = 0;
  // TODO(huisu@): currently only works for the 0th operating point, so scalable
  // coding is not supported yet.
  if (cpi->target_seq_level_idx[0] < SEQ_LEVELS) {
    const int layer_depth = cpi->gf_group.layer_depth[cpi->gf_group.index];
    int bandwidth_multiplier = 12;
    switch (layer_depth) {
      case 0: bandwidth_multiplier = 64; break;
      case 1: bandwidth_multiplier = 40; break;
      case 2: bandwidth_multiplier = 24; break;
    }
    level_forced_max_bandwidth =
        (rc->avg_frame_bandwidth * bandwidth_multiplier) >> 3;
  }

  const int min_cr = cpi->oxcf.min_cr;
  if (min_cr > 0 && level_forced_max_bandwidth > 0) {
    aom_clear_system_state();
    const double compression_ratio =
        av1_get_compression_ratio(cm, rc->projected_frame_size >> 3);
    // Compression ratio corresponding to the level_forced_max_bandwidth.
    const double level_forced_cr =
        av1_get_compression_ratio(cm, level_forced_max_bandwidth >> 3);
    const double target_cr = AOMMAX(min_cr / 100.0, level_forced_cr);
