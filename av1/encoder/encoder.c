...
static void set_or_save_encoding_params_for_screen_content(AV1_COMP *cpi,
  ...
  if (pass == 0) {
    ...
    cpi->sf.always_this_block_size = BLOCK_32X32;
  } else {
...
static void determine_sc_tools_with_encoding(AV1_COMP *cpi, const int q_orig) {
  ...
  if (cpi->sf.use_nonrd_pick_mode || cpi->oxcf.fwd_kf_enabled ||
  ...
  const int partition_search_type_orig = cpi->sf.partition_search_type;
  ...
  cpi->sf.partition_search_type = partition_search_type_orig;
  cpi->sf.always_this_block_size = fixed_partition_block_size_orig;
}
