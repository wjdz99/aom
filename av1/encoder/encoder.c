...
static int encode_with_recode_loop(AV1_COMP *cpi, size_t *size, uint8_t *dest) {
  ...
  do {
    ...
    MACROBLOCK *const x = &cpi->td.mb;
    if (cpi->sf.inclined_min_partition_size > x->min_partition_size) {
      int q_squared = q * q;
      ...
      const int num_mbs = (cpi->oxcf.resize_mode != RESIZE_NONE)
                              ...
                              : cpi->common.MBs;
      aom_clear_system_state();
      double mb_coded_error = this_frame_stats->coded_error / (double)num_mbs;
      double mb_intra_error = this_frame_stats->intra_error / (double)num_mbs;
