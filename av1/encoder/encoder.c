...
int av1_convert_sect5obus_to_annexb(uint8_t *buffer, size_t *input_size,
  ...
  size_t output_size = 0, total_bytes_read = 0, remaining_size = *input_size;
  uint8_t *buff_ptr = buffer;

  if (precede_frame_sz) {
    //  Preliminary Annex-B-prime code
    size_t length_of_size = aom_uleb_size_in_bytes((uint64_t)remaining_size);
    size_t coded_obu_size;
    memmove(buff_ptr + length_of_size, buff_ptr, *input_size);
    if (aom_uleb_encode((uint64_t)length_of_size, sizeof(remaining_size),
                        buff_ptr, &coded_obu_size) != 0) {
      return AOM_CODEC_ERROR;
    }
  }
  ...
  while (total_bytes_read < *input_size) {
    ...
    size_t length_of_payload_size, length_of_obu_size;
