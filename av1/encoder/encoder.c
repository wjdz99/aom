...
static int encode_with_and_without_superres(AV1_COMP *cpi, size_t *size,
  ...
  for (int denom = SCALE_NUMERATOR + 1; denom <= 2 * SCALE_NUMERATOR; ++denom) {
    ...
    const double this_rdcost =
        RDCOST_DBL(rdmult, this_rate, this_sse, cm->seq_params.bit_depth);
  ...
  const double proj_rdcost1 =
      RDCOST_DBL(rdmult, rate1, sse1, cm->seq_params.bit_depth);
#endif  // SUPERRES_RECODE_ALL_RATIOS
  const double proj_rdcost2 =
      RDCOST_DBL(rdmult, rate2, sse2, cm->seq_params.bit_depth);
