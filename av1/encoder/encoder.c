...
static int enc_alloc_mi(AV1_COMMON *cm) {
  ...
  if (cm->mi_alloc_size < alloc_mi_size) {
    cm->free_mi(cm);
...
static void alloc_context_buffers_ext(AV1_COMP *cpi) {
  ...
  cpi->mi_ext_alloc_size = cm->mi_alloc_rows * cm->mi_alloc_cols;

  dealloc_context_buffers_ext(cpi);
  CHECK_MEM_ERROR(
      cm, cpi->mbmi_ext_base,
      aom_calloc(cpi->mi_ext_alloc_size, sizeof(*cpi->mbmi_ext_base)));
}
...
static void update_frame_size(AV1_COMP *cpi) {
  ...
  cm->set_mb_mi(cm, cm->width, cm->height);
  av1_init_context_buffers(cm);
  av1_init_macroblockd(cm, xd, NULL);

  const int new_alloc_mi_size = cm->mi_alloc_rows * cm->mi_alloc_cols;
  if (new_alloc_mi_size > cpi->mi_ext_alloc_size) {
    dealloc_context_buffers_ext(cpi);
    CHECK_MEM_ERROR(cm, cpi->mbmi_ext_base,
                    aom_calloc(new_alloc_mi_size, sizeof(*cpi->mbmi_ext_base)));
  }
  cpi->mi_ext_alloc_size = new_alloc_mi_size;
  memset(cpi->mbmi_ext_base, 0,
         new_alloc_mi_size * sizeof(*cpi->mbmi_ext_base));
  set_tile_info(cpi);
}
