...
int av1_receive_raw_frame(AV1_COMP *cpi, aom_enc_frame_flags_t frame_flags,
  ...
  if (cpi->oxcf.noise_level > 0 &&
    ...
    if (cpi->oxcf.mode == ALLINTRA) {
      ...
      // quality based on an estimated noise level in the source, but only if the
      // noise level is set ont he command line to > 0.
      cpi->oxcf.noise_level = (float)(av1_estimate_noise_from_single_plane(
          sd, 0, cm->seq_params->bit_depth) - 0.1);
