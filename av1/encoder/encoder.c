...
static void alloc_raw_frame_buffers(AV1_COMP *cpi) {
  ...
  if (!cpi->lookahead)
    cpi->lookahead = av1_lookahead_init(
        ...
        oxcf->lag_in_frames, cm->border_in_pixels);
  if (!cpi->lookahead)
  ...
  if (aom_realloc_frame_buffer(
          ...
          seq_params->use_highbitdepth, cm->border_in_pixels,
...
static void alloc_util_frame_buffers(AV1_COMP *cpi) {
  ...
  if (aom_realloc_frame_buffer(
          ...
          seq_params->subsampling_y, seq_params->use_highbitdepth,
          cm->border_in_pixels, cm->byte_alignment, NULL, NULL, NULL))
    aom_internal_error(&cm->error, AOM_CODEC_MEM_ERROR,
...
static void init_config(struct AV1_COMP *cpi, AV1EncoderConfig *oxcf) {
  ...
  cm->border_in_pixels = (oxcf->resize_mode || oxcf->superres_mode)
      ...
      if (ref->y_crop_width != cm->width || ref->y_crop_height != cm->height) {
        ...
        if (force_scaling || new_fb->buf.y_crop_width != cm->width ||
          ...
          if (aom_realloc_frame_buffer(
                  ...
                  cm->seq_params.use_highbitdepth, cm->border_in_pixels,
                  cm->byte_alignment, NULL, NULL, NULL)) {
            if (force_scaling) {
