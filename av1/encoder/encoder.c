...
static void loopfilter_frame(AV1_COMP *cpi, AV1_COMMON *cm) {
  ...
  if (!is_lossless_requested(&cpi->oxcf)) {
    ...
    if (strength_y) {
      ...
      cm->clpf_size = fb_size_log2 ? fb_size_log2 - MAX_FB_SIZE_LOG2 + 3 : CLPF_NOSIZE;
      av1_clpf_frame(frame, cpi->Source, cm, cm->clpf_size != CLPF_NOSIZE, strength_y,
                     4 + cm->clpf_size, AOM_PLANE_Y, av1_clpf_decision);
    }
    if (strength_u) {
