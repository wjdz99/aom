...
size_params_type av1_calculate_next_size_params(AV1_COMP *cpi) {
  ...
  int resize_num = calculate_next_resize_scale(cpi);
  rsz.resize_width = cpi->oxcf.width;
  rsz.resize_height = cpi->oxcf.height;
  av1_calculate_scaled_size(&rsz.resize_width, &rsz.resize_height, resize_num);
#if CONFIG_FRAME_SUPERRES
...
size_params_type av1_calculate_next_size_params_pending_resize(AV1_COMP *cpi) {
  const AV1EncoderConfig *oxcf = &cpi->oxcf;
  size_params_type rsz = {
    0,
    0,
...
static void setup_frame_size(AV1_COMP *cpi) {
  ...
  if (cpi->resize_pending_width && cpi->resize_pending_height) {
    rsz = av1_calculate_next_size_params_pending_resize(cpi);
  } else {
