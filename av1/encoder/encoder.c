...
static uint8_t calculate_next_resize_scale(const AV1_COMP *cpi) {
  ...
  const AV1EncoderConfig *oxcf = &cpi->oxcf;
  if (is_first_pass(cpi)) return SCALE_NUMERATOR;
...
static size_params_type calculate_next_size_params(AV1_COMP *cpi) {
  ...
  if (is_first_pass(cpi)) return rsz;
...
static int encode_frame_to_data_rate(AV1_COMP *cpi, size_t *size,
  ...
  if (!is_first_pass(cpi) && cpi->common.allow_screen_content_tools &&
  ...
  if (!is_first_pass(cpi) && cpi->need_to_clear_prev_hash_table) {
...
#endif  // CONFIG_INTERNAL_STATS
#if CONFIG_SPEED_STATS
  if (!is_first_pass(cpi) && !cm->show_existing_frame) {
