...
static int setup_interp_filter_search_mask(AV1_COMP *cpi) {
  ...
  for (ifilter = EIGHTTAP_REGULAR; ifilter <= MULTITAP_SHARP; ++ifilter) {
    if ((cpi->interp_filter_selected[LAST_FRAME][ifilter] * 10 <=
         ref_total[LAST_FRAME]) &&
        ((cpi->interp_filter_selected[LAST2_FRAME][ifilter] +
          cpi->interp_filter_selected[LAST3_FRAME][ifilter] +
          cpi->interp_filter_selected[GOLDEN_FRAME][ifilter] +
          cpi->interp_filter_selected[BWDREF_FRAME][ifilter] +
          cpi->interp_filter_selected[ALTREF2_FRAME][ifilter] +
          cpi->interp_filter_selected[arf_idx][ifilter]) *
             30 <=
