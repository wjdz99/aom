...
static void loopfilter_frame(AV1_COMP *cpi, AV1_COMMON *cm) {
  ...
  if (!is_lossless_requested(&cpi->oxcf)) {
    ...
    if (!hq) {
      ...
      clpf_test_frame(&cpi->last_frame_uf, cpi->Source, cm, &strength,
                      &fb_size_log2);

      strength = 1;
      fb_size_log2 = 0;
      if (!fb_size_log2) fb_size_log2 = log2i(MAX_FB_SIZE);
      ...
      if (!strength)  // Better to disable for the whole frame?
        ...
        cm->clpf_size =
            fb_size_log2 ? fb_size_log2 - log2i(MAX_FB_SIZE) + 3 : 0;
