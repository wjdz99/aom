...
static void set_additional_frame_flags(AV1_COMMON *const cm,
                                       unsigned int *frame_flags) {
  *frame_flags |= (frame_is_intra_only(cm) ? FRAMEFLAGS_INTRAONLY : 0);
  *frame_flags |= (frame_is_sframe(cm) ? FRAMEFLAGS_SWITCH : 0);
  *frame_flags |= (cm->error_resilient_mode ? FRAMEFLAGS_ERROR_RESILIENT : 0);
}
