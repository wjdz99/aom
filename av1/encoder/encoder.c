...
static void generate_psnr_packet(AV1_COMP *cpi) {
  ...
  static YV12_BUFFER_CONFIG *get_av1_ref_frame_buffer(
    ...
    return ref_frame == NONE_FRAME ? NULL
...
#endif  // CONFIG_EXT_REFS
        ref_cnt_fb(pool->frame_bufs, &cm->ref_frame_map[arf_idx],
                   cm->new_fb_idx);
      ...
      if (cpi->ref_frame_flags & ref_mask[ref_frame - 1]) {
        ...
        if (ref->y_crop_width != cm->width ||
          ...
          if (force_scaling || new_fb_ptr->buf.y_crop_width != cm->width ||
            ...
            if (aom_realloc_frame_buffer(
                    &new_fb_ptr->buf, cm->width, cm->height, cm->subsampling_x,
                    cm->subsampling_y, AOM_BORDER_IN_PIXELS, cm->byte_alignment,
                    NULL, NULL, NULL))
              aom_internal_error(&cm->error, AOM_CODEC_MEM_ERROR,
                                 "Failed to allocate frame buffer");
            av1_resize_and_extend_frame(ref, &new_fb_ptr->buf);
...
#define COUPLED_CHROMA_FROM_LUMA_RESTORATION 0
  ...
  static void set_frame_size(AV1_COMP * cpi, int width, int height) {
    ...
    alloc_util_frame_buffers(
        cpi);  // TODO(afergs): Remove? Gets called anyways.
    ...
    for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
      ...
      if (buf_idx != INVALID_IDX) {
        YV12_BUFFER_CONFIG *const buf =
            &cm->buffer_pool->frame_bufs[buf_idx].buf;
        ref_buf->buf = buf;
#if CONFIG_HIGHBITDEPTH
  ...
  static void loopfilter_frame(AV1_COMP * cpi, AV1_COMMON * cm) {
    ...
    if (cm->rst_info[0].frame_restoration_type != RESTORE_NONE ||
      ...
      av1_loop_restoration_frame(cm->frame_to_show, cm, cm->rst_info, 7, 0,
                                 NULL);
    }
#endif  // CONFIG_LOOP_RESTORATION
...
#define DUMP_RECON_FRAMES 0
  ...
  static void dump_filtered_recon_frames(AV1_COMP * cpi) {
    ...
    // --- U ---
    for (h = 0; h < (cm->height >> 1); ++h) {
      fwrite(&recon_buf->u_buffer[h * recon_buf->uv_stride], 1,
             (cm->width >> 1), f_recon);
    }
    // --- V ---
    for (h = 0; h < (cm->height >> 1); ++h) {
      fwrite(&recon_buf->v_buffer[h * recon_buf->uv_stride], 1,
             (cm->width >> 1), f_recon);
    }
...
// TODO(yaowu): unify these two versions into one.
...
#endif
        {
          const double frame_blockiness = av1_get_blockiness(
        ...
        if (cpi->b_calculate_consistency) {
...
#endif
          {
            const double this_inconsistency = aom_get_ssim_metrics(
                orig->y_buffer, orig->y_stride, recon->y_buffer,
                recon->y_stride, orig->y_width, orig->y_height, cpi->ssim_vars,
                &cpi->metrics, 1);
  ...
  int av1_get_compressed_data(AV1_COMP * cpi, unsigned int *frame_flags,
    ...
    bitstream_queue_set_frame_write(cm->current_video_frame * 2 +
                                    cm->show_frame);
#endif
    ...
    if (arf_src_index) {
      ...
      if ((source = av1_lookahead_peek(cpi->lookahead, arf_src_index)) !=
          NULL) {
...
// Produce the filtered ARF frame.
    ...
    if (brf_src_index) {
      ...
      if ((source = av1_lookahead_peek(cpi->lookahead, brf_src_index)) !=
          NULL) {
        cm->show_frame = 0;
        cm->intra_only = 0;
    ...
    if (source) {
      ...
      cpi->unscaled_last_source =
          last_source != NULL ? &last_source->img : NULL;
