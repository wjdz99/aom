...
#if CONFIG_NEW_QUANT && SEARCH_NEW_QUANT
  int best_dq_type = 0;
  double best_dq_rd = (double)INT64_MAX;
  for (int dq_type = 1; dq_type <= QUANT_PROFILES; dq_type++) {
    cm->dq_type = dq_type == QUANT_PROFILES ? best_dq_type : dq_type;
#endif  // CONFIG_NEW_QUANT && SEARCH_NEW_QUANT
    do {
      aom_clear_system_state();
