...
static void loopfilter_frame(AV1_COMP *cpi, AV1_COMMON *cm) {
  ...
  const int using_lossless =
      is_lossless_requested(&cpi->oxcf) || cm->all_lossless;

  struct loopfilter *lf = &cm->lf;
  int no_loopfilter = 0;
  int no_restoration = !cpi->oxcf.using_restoration;

  if (using_lossless || cm->large_scale_tile) {
    no_loopfilter = 1;
    no_restoration = 1;
  }

  int no_cdef = 0;
  if (using_lossless || !cpi->oxcf.using_cdef || cm->large_scale_tile) {
    no_cdef = 1;
  }
