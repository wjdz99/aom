...
static void init_config(struct AV1_COMP *cpi, AV1EncoderConfig *oxcf) {
  ...
  cm->decoder_model_info_present = oxcf->decoder_model_info_present;
  cm->operating_points_decoder_model_cnt = oxcf->operating_points_decoder_model_cnt;
  if (oxcf->decoder_model_info_present) {
    ...
    set_dec_model_op_parameters(&cm->op_params[0], &cm->buffer_model,
                                &cm->timing_info, oxcf->target_bandwidth, 0);
    for (int op_num = 0; op_num < cm->operating_points_decoder_model_cnt; ++op_num)
      cm->op_frame_timing[0].buffer_removal_delay_flag =
          oxcf->op_frame_timing[0].buffer_removal_delay_flag;
  }
#endif
  cm->width = oxcf->width;
