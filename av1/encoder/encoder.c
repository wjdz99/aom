...
int av1_get_compressed_data(AV1_COMP *cpi, unsigned int *frame_flags,
  ...
  *size = 0;
  ...
  // Some scenarious disallow the use of show_existing_frame
  // show_existing_frame is currently only supported in second-pass
  cm->show_existing_frame &= allow_show_existing(cpi);
  if (cm->show_existing_frame) assert(oxcf->pass == 2);
