...
static void configure_static_seg_features(AV1_COMP *cpi) {
  ...
  } else if (cpi->frame_refresh_flags.alt_ref_frame) {
...
void av1_change_config(struct AV1_COMP *cpi, const AV1EncoderConfig *oxcf) {
  ...
  FrameRefreshFlags *const frame_refresh_flags = &cpi->frame_refresh_flags;
  ...
  frame_refresh_flags->golden_frame = 0;
  frame_refresh_flags->bwd_ref_frame = 0;
...
AV1_COMP *av1_create_compressor(AV1EncoderConfig *oxcf, BufferPool *const pool,
  ...
  cpi->frame_refresh_flags.alt_ref_frame = 0;
...
static uint16_t setup_interp_filter_search_mask(AV1_COMP *cpi) {
  ...
  if (cpi->last_frame_type == KEY_FRAME ||
      cpi->frame_refresh_flags.alt_ref_frame)
    return mask;
...
static void dump_filtered_recon_frames(AV1_COMP *cpi) {
  ...
  printf(
      ...
      cpi->frame_refresh_flags.alt_ref_frame, recon_buf->y_stride,
...
int av1_encode(AV1_COMP *const cpi, uint8_t *const dest,
  ...
  memcpy(&cpi->frame_refresh_flags, &frame_params->frame_refresh_flags,
         sizeof(FrameRefreshFlags));
