...
static void loopfilter_frame(AV1_COMP *cpi, AV1_COMMON *cm) {
  ...
  if (!is_lossless_requested(&cpi->oxcf)) {
    ...
    av1_clpf_test_frame(frame, cpi->Source, cm, &strength_y, &fb_size_log2, 0);
    av1_clpf_test_frame(frame, cpi->Source, cm, &strength_u, &fb_size_log2, 1);
    av1_clpf_test_frame(frame, cpi->Source, cm, &strength_v, &fb_size_log2, 2);
    ...
    if (strength_y) {
      ...
      cm->clpf_numblocks = av1_clpf_frame(
          ...
          4 + cm->clpf_size, cm->clpf_blocks, 0, av1_clpf_decision);
    }
    if (strength_u) {
      cm->clpf_strength_u = strength_u - (strength_u == 4);
      av1_clpf_frame(frame, NULL, cm, 0, strength_u, 4, NULL, 1, NULL);
    }
    if (strength_v) {
      cm->clpf_strength_v = strength_v - (strength_v == 4);
      av1_clpf_frame(frame, NULL, cm, 0, strength_v, 4, NULL, 2, NULL);
    }
  }
#endif
#if CONFIG_DERING
