...
static void save_rate_control(AV1_COMP *cpi) {
  CODING_CONTEXT *const cc = &cpi->coding_context;
  cc->rc = cpi->rc;
}
...
static void restore_rate_control(AV1_COMP *cpi) {
  CODING_CONTEXT *const cc = &cpi->coding_context;
  cpi->rc = cc->rc;
}
...
static int encode_with_recode_loop_and_filter(AV1_COMP *cpi, size_t *size,
  ...
  if (rdcost != NULL) {
    ...
    const int64_t rate = (bits << 5);  // To match scale.
...
static int encodes_with_and_without_superres(AV1_COMP *cpi, size_t *size,
  ...
  if (proj_rdcost1 < proj_rdcost2) {
    ...
    if (!frame_is_intra_only(&cpi->common)) release_scaled_references(cpi);
    double proj_rdcost3;
    err = encode_with_recode_loop_and_filter(cpi, size, dest, 1, &proj_rdcost3,
                                             largest_tile_id);
    assert(proj_rdcost1 == proj_rdcost3);
  } else {
