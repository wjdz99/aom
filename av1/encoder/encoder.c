...
static int big_rate_miss_high_threshold(const AV1_COMP *const cpi) {
  const RATE_CONTROL *const rc = &cpi->rc;
  // Note: special case for overlay frames, because for these frames
  // rc->this_frame_target is set to 0.
  return rc->is_src_frame_alt_ref
             ? rc->max_frame_bandwidth
             : frame_is_kf_gf_arf(cpi) ? rc->this_frame_target * 3 / 2
...
static int big_rate_miss(const AV1_COMP *const cpi) {
  ...
  // Also, ignore if we are NOT currently writing to the bitstream.
  if (rc->is_src_frame_alt_ref || !cpi->pack_bitstream) {
    return 0;
  } else {
