...
static void setup_frame_size(AV1_COMP *cpi) {
  ...
  cm->superres_scale_numerator = av1_calculate_next_superres_scale(cpi, encode_width, encode_width);
  av1_calculate_superres_size(cm, &encode_width, &encode_height);
  printf("superres encode dim %d %d\n", encode_width, encode_height);
#endif  // CONFIG_FRAME_SUPERRES
...
static void superres_post_encode(AV1_COMP *cpi) {
  ...
  // used.
  if (av1_superres_resize_unscaled(cpi)) {  // TODO(afergs): Check if this does anything.
    ...
    assert(cpi->unscaled_source->y_crop_height != cm->superres_upscaled_height ||
           cpi->unscaled_source->y_crop_width != cm->superres_upscaled_width);
#if CONFIG_HIGHBITDEPTH
    ...
    if (cpi->last_source != NULL) {
      ...
      if (aom_realloc_frame_buffer(
          &cpi->scaled_last_source, cm->superres_upscaled_width,
      ...
      if (cpi->unscaled_last_source != NULL) {
        assert(cpi->scaled_last_source.y_crop_width == cm->superres_upscaled_width);
        assert(cpi->scaled_last_source.y_crop_height == cm->superres_upscaled_height);
        assert(cpi->unscaled_last_source->y_crop_height !=
               cm->superres_upscaled_height ||
               cpi->unscaled_last_source->y_crop_width !=
               cm->superres_upscaled_width);
#if CONFIG_HIGHBITDEPTH
        av1_resize_and_extend_frame(cpi->unscaled_last_source,
                                    &cpi->scaled_last_source, (int)cm->bit_depth);
#else
        av1_resize_and_extend_frame(cpi->unscaled_last_source,
...
static void encode_without_recode_loop(AV1_COMP *cpi) {
  ...
  assert(cm->width == cpi->scaled_source.y_width);
  assert(cm->height == cpi->scaled_source.y_height);
