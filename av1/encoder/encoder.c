...
int av1_get_compressed_data(AV1_COMP *cpi, unsigned int *frame_flags,
  ...
  *size = 0;
  ...
  if (oxcf->pass != 1 && cpi->common.allow_screen_content_tools)
  {
    ...
    for (i = 0; i < 2; i++) {
      for (j = 0; j < 2; j++) {
        uiBlockHashValues[i][j] = malloc(sizeof(uint32_t)*picWidth*picHeight);
      }

      for (j = 0; j < 3; j++) {
        bIsBlockSame[i][j] = malloc(sizeof(int8_t)*picWidth*picHeight);
      }
    }

    hash_table_create(&cm->cur_frame->hash_table);
    generate_block_2x2_hash_value(&cm->cur_frame->hash_table, cpi->source, uiBlockHashValues[0], bIsBlockSame[0]);
    generate_block_hash_value(&cm->cur_frame->hash_table, cpi->source, 4, 4, uiBlockHashValues[0], uiBlockHashValues[1], bIsBlockSame[0], bIsBlockSame[1]);
    generate_block_hash_value(&cm->cur_frame->hash_table, cpi->source, 8, 8, uiBlockHashValues[1], uiBlockHashValues[0], bIsBlockSame[1], bIsBlockSame[0]);
    add_to_hash_map_by_row_with_precal_data(&cm->cur_frame->hash_table, uiBlockHashValues[0], bIsBlockSame[0][2], picWidth, picHeight, 8, 8);
    generate_block_hash_value(&cm->cur_frame->hash_table, cpi->source, 16, 16, uiBlockHashValues[0], uiBlockHashValues[1], bIsBlockSame[0], bIsBlockSame[1]);
    add_to_hash_map_by_row_with_precal_data(&cm->cur_frame->hash_table, uiBlockHashValues[1], bIsBlockSame[1][2], picWidth, picHeight, 16, 16);
    generate_block_hash_value(&cm->cur_frame->hash_table, cpi->source, 32, 32, uiBlockHashValues[1], uiBlockHashValues[0], bIsBlockSame[1], bIsBlockSame[0]);
    add_to_hash_map_by_row_with_precal_data(&cm->cur_frame->hash_table, uiBlockHashValues[0], bIsBlockSame[0][2], picWidth, picHeight, 32, 32);
    generate_block_hash_value(&cm->cur_frame->hash_table, cpi->source, 64, 64, uiBlockHashValues[0], uiBlockHashValues[1], bIsBlockSame[0], bIsBlockSame[1]);
    add_to_hash_map_by_row_with_precal_data(&cm->cur_frame->hash_table, uiBlockHashValues[1], bIsBlockSame[1][2], picWidth, picHeight, 64, 64);

    for (i = 0; i < 2; i++) {
      for (j = 0; j < 2; j++) {
        free(uiBlockHashValues[i][j]);
      }

      for (j = 0; j < 3; j++) {
        free(bIsBlockSame[i][j]);
      }
    }
  }
