...
static int encode_without_recode(AV1_COMP *cpi) {
  ...
  if (!frame_is_intra_only(cm) &&
    ...
    scale_references(cpi, filter_scaler, phase_scaler, 1);
...
static int encode_with_recode_loop(AV1_COMP *cpi, size_t *size, uint8_t *dest) {
  ...
  do {
    ...
    if (!frame_is_intra_only(cm)) {
      ...
      scale_references(cpi, EIGHTTAP_REGULAR, 0, 0);
    }
#if CONFIG_TUNE_VMAF
    if (oxcf->tune_cfg.tuning == AOM_TUNE_VMAF_WITH_PREPROCESSING ||
        oxcf->tune_cfg.tuning == AOM_TUNE_VMAF_WITHOUT_PREPROCESSING ||
...
static int encode_frame_to_data_rate(AV1_COMP *cpi, size_t *size,
  ...
  if (oxcf->tune_cfg.tuning == AOM_TUNE_SSIM) set_mb_ssim_rdmult_scaling(cpi);
