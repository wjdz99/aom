...
#if CONFIG_AMVR
  if (cpi->common.allow_high_precision_mv&& cur_frame_mv_precision_level == 0) {
#else
...
#if CONFIG_AMVR
int is_integer_mv(AV1_COMP *cpi, const YV12_BUFFER_CONFIG *cur_picture,
  ...
  for (int i = 0; i + block_size <= pic_height; i += block_size) {
    for (int j = 0; j + block_size <= pic_width; j += block_size) {
      ...
      if (av1_hash_is_horizontal_perfect(cur_picture, block_size, x_pos, y_pos)
       || av1_hash_is_vertical_perfect(cur_picture, block_size, x_pos, y_pos)) {
        S++;
        continue;
      }

      av1_get_block_hash_value(cur_picture->y_buffer + y_pos*stride_cur + x_pos,
                               stride_cur, block_size,
                               &hash_value_1, &hash_value_2);
  ...
  *size = 0;
  ...
  if (oxcf->pass != 1 && cpi->common.allow_screen_content_tools) {
    if (cpi->common.seq_mv_precision_level == 2) {
      struct lookahead_entry* previous_entry = cpi->lookahead->buf + cpi->previsous_index;
      cpi->common.cur_frame_mv_precision_level =
        is_integer_mv(cpi, cpi->source, &previous_entry->img,
                      cpi->previsou_hash_table);
    }
    else {
      cpi->common.cur_frame_mv_precision_level =
