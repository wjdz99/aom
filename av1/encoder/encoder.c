...
static void setup_frame(AV1_COMP *cpi) {
  ...
  if (cm->current_frame.frame_type == KEY_FRAME && cm->show_frame) {
    ...
    const RefCntBuffer *primary_ref_buf = get_primary_ref_frame_buf(cm);
    if (!primary_ref_buf) {
      av1_setup_past_independence(cm);
      cm->seg.update_map = 1;
...
void av1_change_config(struct AV1_COMP *cpi, const AV1EncoderConfig *oxcf) {
  ...
  if (!cpi->seq_params_locked) {
    ...
    if (seq_params->operating_points_cnt_minus_1 + 1 >
        MAX_NUM_OPERATING_POINTS) {
      aom_internal_error(
          &cm->error, AOM_CODEC_UNSUP_BITSTREAM,
          "Encoder does not support %d decoder model operating points",
...
static void update_reference_frames(AV1_COMP *cpi) {
  ...
  if (av1_preserve_existing_gf(cpi)) {
    ...
    cm->remapped_ref_idx[ALTREF_FRAME - 1] =
        get_ref_frame_map_idx(cm, GOLDEN_FRAME);
    cm->remapped_ref_idx[GOLDEN_FRAME - 1] = tmp;
    ...
    cm->remapped_ref_idx[LAST_FRAME - 1] =
        get_ref_frame_map_idx(cm, bwdref_to_show);
    ...
    if (cpi->new_bwdref_update_rule == 1) {
      ...
      // spared space
      cm->remapped_ref_idx[EXTREF_FRAME - 1] = last3_remapped_idx;
    } else {
#endif
      cm->remapped_ref_idx[bwdref_to_show - 1] = last3_remapped_idx;
#if USE_SYMM_MULTI_LAYER
    }
#endif
    ...
    if (cpi->refresh_bwd_ref_frame) {
      ...
      if (cpi->new_bwdref_update_rule) {
        ...
        int tmp = get_ref_frame_map_idx(cm, EXTREF_FRAME);
        assign_frame_buffer_p(&cm->ref_frame_map[tmp], cm->cur_frame);

        rshift_bwd_ref_frames(cpi);
        cm->remapped_ref_idx[BWDREF_FRAME - 1] = tmp;
      } else {
#endif  // USE_SYMM_MULTI_LAYER
        assign_frame_buffer_p(
  ...
  if (cpi->refresh_last_frame) {
    ...
    shift_last_ref_frames(cpi);
    cm->remapped_ref_idx[LAST_FRAME - 1] = last3_remapped_idx;
    ...
    if (cpi->rc.is_last_bipred_frame) {
      ...
      cm->remapped_ref_idx[BWDREF_FRAME - 1] = last3_remapped_idx;
...
static void scale_references(AV1_COMP *cpi) {
  ...
  for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
    ...
    if (cpi->ref_frame_flags & ref_mask[ref_frame - 1]) {
      ...
      const YV12_BUFFER_CONFIG *const ref = get_ref_frame_buffer(cm, ref_frame);
...
static void setup_frame_offset(AV1_COMP *cpi, EncodeInputFrame *eif) {
  // TODO(jsh): remove references to as many AV1_COMP members as possible once
  // done
  AV1_COMMON *cm = &cpi->common;
  if (eif->show_frame == 0) {
    ...
    eif->order_hint = cm->current_frame.order_hint =
        eif->frame_number + arf_offset;
  } else {
    eif->order_hint = cm->current_frame.order_hint = eif->frame_number;
  }
  eif->order_hint = cm->current_frame.order_hint %=
...
static int encode_with_recode_loop(AV1_COMP *cpi, size_t *size, uint8_t *dest,
  ...
  do {
    ...
    if (!get_primary_ref_frame_buf(cm)) {
...
static int get_ref_frame_flags(const AV1_COMP *cpi) {
  const RefCntBuffer *last_buf =
      get_ref_frame_buf_const(&cpi->common, LAST_FRAME);
  const RefCntBuffer *last2_buf =
      get_ref_frame_buf_const(&cpi->common, LAST2_FRAME);
  const RefCntBuffer *last3_buf =
      get_ref_frame_buf_const(&cpi->common, LAST3_FRAME);
  const RefCntBuffer *golden_buf =
      get_ref_frame_buf_const(&cpi->common, GOLDEN_FRAME);
  const RefCntBuffer *bwd_buf =
      get_ref_frame_buf_const(&cpi->common, BWDREF_FRAME);
  const RefCntBuffer *alt2_buf =
      get_ref_frame_buf_const(&cpi->common, ALTREF2_FRAME);
  const RefCntBuffer *alt_buf =
      get_ref_frame_buf_const(&cpi->common, ALTREF_FRAME);
...
static void dump_filtered_recon_frames(AV1_COMP *cpi) {
  ...
  for (int ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
    RefCntBuffer *buf = get_ref_frame_buf_const(cm, ref_frame);
    const int ref_offset = buf ? (int)buf->order_hint : -1;
...
int av1_get_compressed_data(AV1_COMP *cpi, unsigned int *frame_flags,
  ...
  // held.
  if (cm->cur_frame) --cm->cur_frame->ref_count;
  int new_fb_idx = get_free_fb(cm);
...
int av1_get_last_show_frame(AV1_COMP *cpi, YV12_BUFFER_CONFIG *frame) {
  if (!cpi->last_show_frame_buf) return -1;
