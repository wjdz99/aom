...
#ifdef OUTPUT_STATS
extern double improve_rate;
extern int64_t ada_ncobmc_count;
extern double ip_rate[BLOCK_SIZES][MAX_NCOBMC_MODES];
extern double ip_dist[BLOCK_SIZES][MAX_NCOBMC_MODES];
extern int ncobmc_counts[BLOCK_SIZES][MAX_NCOBMC_MODES];
#endif
...
#endif
#ifdef OUTPUT_STATS
  for (int blk = 0; blk < BLOCK_SIZES; ++blk) {
    for (int mm = 0; mm < MAX_NCOBMC_MODES; ++mm) {
      ncobmc_counts[blk][mm] = 0;
      ip_dist[blk][mm] = 0;
      ip_rate[blk][mm] = 0;
    }
  }
...
#endif
#ifdef OUTPUT_YUV_REC
  yuv_rec_file = fopen("./tmp/raw/rec.yuv", "wb");
#endif
  ...
  if (cm->current_video_frame > 0) {
    ...
    if (cpi->oxcf.pass != 1) {
...
#ifdef OUTPUT_STATS
      {
        ...
        for (t = 0; t < ADAPT_OVERLAP_BLOCKS; ++t) {
          fprintf(stdout, "[%d]: ", t);
          for (int mm = 0; mm < MAX_NCOBMC_MODES - 1; ++mm) {
            fprintf(stdout, "%d ", aggregate_fc.ncobmc_mode[t][mm]);
          }
          fprintf(stdout, "\n");
        }

        fprintf(stdout, "average improve rate = %1.5f\n",
                improve_rate / ada_ncobmc_count);

        for (int blk = BLOCK_8X8; blk <= BLOCK_32X32; ++blk) {
          for (int mm = 0; mm < MAX_NCOBMC_MODES - 1; ++mm) {
            fprintf(stdout, "{%d %d}: ", blk, mm);
            if (ncobmc_counts[blk][mm] == 0) ++ncobmc_counts[blk][mm];
            fprintf(stdout, "[%d %1.5f %1.5f]\n", ncobmc_counts[blk][mm],
                    ip_rate[blk][mm] / ncobmc_counts[blk][mm],
                    ip_dist[blk][mm] / ncobmc_counts[blk][mm]);
          }
          fprintf(stdout, "\n");
        }
      }
