...
static BLOCK_SIZE select_sb_size(const AV1_COMP *const cpi) {
  ...
  if (cpi->svc.number_spatial_layers > 1) {
    ...
    return AOMMIN(oxcf->frame_dim_cfg.width, oxcf->frame_dim_cfg.height) > 480
               ? BLOCK_128X128
               : BLOCK_64X64;
  }
...
void av1_init_seq_coding_tools(SequenceHeader *seq, AV1_COMMON *cm,
  ...
  const FrameDimensionCfg *const frame_dim_cfg = &oxcf->frame_dim_cfg;
  ...
  seq->max_frame_width = frame_dim_cfg->forced_max_frame_width
                             ? frame_dim_cfg->forced_max_frame_width
                             : frame_dim_cfg->width;
  seq->max_frame_height = frame_dim_cfg->forced_max_frame_height
                              ? frame_dim_cfg->forced_max_frame_height
                              : frame_dim_cfg->height;
  seq->num_bits_width =
  ...
  set_bitstream_level_tier(seq, cm, frame_dim_cfg->width, frame_dim_cfg->height,
                           oxcf->init_framerate);
...
static void init_config(struct AV1_COMP *cpi, AV1EncoderConfig *oxcf) {
  ...
  cm->width = oxcf->frame_dim_cfg.width;
  cm->height = oxcf->frame_dim_cfg.height;
...
void av1_change_config(struct AV1_COMP *cpi, const AV1EncoderConfig *oxcf) {
  ...
  const FrameDimensionCfg *const frame_dim_cfg = &cpi->oxcf.frame_dim_cfg;
  ...
  if (frame_dim_cfg->render_width > 0 && frame_dim_cfg->render_height > 0) {
    cm->render_width = frame_dim_cfg->render_width;
    cm->render_height = frame_dim_cfg->render_height;
  } else {
    cm->render_width = frame_dim_cfg->width;
    cm->render_height = frame_dim_cfg->height;
  }
  cm->width = frame_dim_cfg->width;
  cm->height = frame_dim_cfg->height;
...
#define MBFP(BT, MCSDF, MCSVF)  \
  ...
  cm->superres_upscaled_width = oxcf->frame_dim_cfg.width;
  cm->superres_upscaled_height = oxcf->frame_dim_cfg.height;
...
static void init_motion_estimation(AV1_COMP *cpi) {
  ...
  const int y_stride_src = ((cpi->oxcf.frame_dim_cfg.width != cm->width ||
                             cpi->oxcf.frame_dim_cfg.height != cm->height) ||
...
int av1_set_internal_size(AV1EncoderConfig *const oxcf,
  ...
  resize_pending_params->width = (hs - 1 + oxcf->frame_dim_cfg.width * hr) / hs;
  resize_pending_params->height =
      (vs - 1 + oxcf->frame_dim_cfg.height * vr) / vs;
