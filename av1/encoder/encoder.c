...
static void superres_post_encode(AV1_COMP *cpi) {
  ...
  av1_superres_upscale(cm);

  // If regular resizing is occurring the source will need to be downscaled to
  // match the upscaled superres resolution. Otherwise the original source can
  // be used.
  if (av1_resize_unscaled(cpi)) {  // TODO(afergs): Check if this does anything.
    ...
    // Do downscale. Note that cm->(width|height) has been updated by
    ...
    if (cpi->last_source != NULL) {
      ...
      if (!aom_realloc_frame_buffer(
        ...
        aom_internal_error(
            ...
            "upscaling");
      cpi->unscaled_last_source = av1_scale_if_required(
...
/* quasi-random initialization of current_frame_id for a key frame */
  ...
  if (cm->superres_scale_numerator != 12) cm->superres_pending = 1;
  cm->superres_scale_numerator = 12;
#endif  // CONFIG_FRAME_SUPERRES
