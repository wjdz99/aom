...
static int encode_with_recode_loop(AV1_COMP *cpi, size_t *size, uint8_t *dest) {
  ...
  do {
    ...
    if (do_dummy_pack) {
      ...
      printf("\n%d %d %d\n", cpi->ppi->gf_group.update_type[cpi->gf_frame_index], cpi->gf_frame_index, cpi->ppi->gf_group.size);
      if (cpi->gf_frame_index + 2 >= cpi->ppi->gf_group.size) {
        VBR_RATECTRL_INFO info = cpi->vbr_rc_info;
        for (int i = 0; i < cpi->ppi->gf_group.size; i++) {
          printf("i: %d, \test_bitrate: %f, est_mv_bitrate: %f, act_bitrate: %d, act_mv_bitrate: %d, \tq: %d, update_type: %d\n",
                 i,
