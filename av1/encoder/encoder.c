...
static INLINE void shift_last_ref_frames(AV1_COMP *cpi) {
  ...
  for (int ref_frame = LAST3_FRAME; ref_frame > LAST_FRAME; --ref_frame) {
    ...
    cpi->remapped_ref_idx[ref_idx] = get_ref_frame_map_idx(cpi, ref_idx);
...
static void update_reference_frames(AV1_COMP *cpi) {
  ...
  if (av1_preserve_existing_gf(cpi)) {
    ...
    get_ref_frame_map_idx(cpi, GOLDEN_FRAME) = tmp;
    ...
    get_ref_frame_map_idx(cpi, LAST_FRAME) =
        get_ref_frame_map_idx(cpi, bwdref_to_show);
    ...
    if (cpi->new_bwdref_update_rule == 1) {
      ...
      // spared space
      get_ref_frame_map_idx(cpi, EXTREF_FRAME) = last3_remapped_idx;
    } else {
#endif
      get_ref_frame_map_idx(cpi, bwdref_to_show) = last3_remapped_idx;
#if USE_SYMM_MULTI_LAYER
    }
#endif
    ...
    if (cpi->refresh_bwd_ref_frame) {
      ...
      if (cpi->new_bwdref_update_rule) {
        ...
        get_ref_frame_map_idx(cpi, BWDREF_FRAME) = tmp;
      } else {
#endif  // USE_SYMM_MULTI_LAYER
        assign_frame_buffer(
            pool->frame_bufs,
  ...
  if (cpi->refresh_last_frame) {
    ...
    shift_last_ref_frames(cpi);
    get_ref_frame_map_idx(cpi, LAST_FRAME) = last3_remapped_idx;
    ...
    if (cpi->rc.is_last_bipred_frame) {
      ...
      get_ref_frame_map_idx(cpi, BWDREF_FRAME) = last3_remapped_idx;
