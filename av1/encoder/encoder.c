...
static int encode_without_recode(AV1_COMP *cpi) {
  ...
  // rtc only
  if (is_psnr_calc_enabled(cpi) && cpi->sf.rt_sf.use_rtc_tf  && cm->current_frame.frame_type != KEY_FRAME) {
    const int num_planes = av1_num_planes(cm);
    const SequenceHeader *seq_params = cm->seq_params;

    if(cpi->orig_source.buffer_alloc_sz == 0 || cpi->last_source->y_width != cpi->source->y_width || cpi->last_source->y_height != cpi->source->y_height) {
  // Allocate a source buffer to store the true source for psnr calculation.
      if (aom_alloc_frame_buffer(
          &cpi->orig_source, cpi->oxcf.frm_dim_cfg.width, cpi->oxcf.frm_dim_cfg.height, seq_params->subsampling_x,
              seq_params->subsampling_y, seq_params->use_highbitdepth,
