...
int av1_denoise_2d(AV1_COMMON *const cm,
  ...
  if (!cm->flat_block_finder) {
    ...
    aom_flat_block_finder_init(cm->flat_block_finder, block_size, cm->bit_depth, use_highbd);
  }
  if (!cm->film_grain_table) {
    cm->film_grain_table = aom_malloc(sizeof(*cm->film_grain_table));
  ...
  fprintf(stdout, "Num flat blocks: %d (bit_depth: %d, highbd: %d/%d)\n", num_flat,
          (int)cm->bit_depth, use_highbd, !CONFIG_LOWBITDEPTH);
  if (num_flat == 0) {
  ...
  if (!cm->noise_model) {
    ...
    const aom_noise_model_params_t params = {AOM_NOISE_SHAPE_SQUARE, 3, cm->bit_depth,
                                             use_highbd };
  ...
  if (have_noise_model) {
    ...
    memcpy(raw_data[1], denoised[1], (strides[1] * sd->uv_height) << use_highbd);
    memcpy(raw_data[2], denoised[2], (strides[2] * sd->uv_height) << use_highbd);
  }
...
int av1_get_compressed_data(AV1_COMP *cpi, unsigned int *frame_flags,
  ...
  fprintf(stderr, "Outputting seed: %d %d show: %d %d source:%d\n",
          cm->film_grain_params.random_seed,
          cm->film_grain_params_present,
