...
int combine_prior_with_tpl_boost(int prior_boost, int tpl_boost,
                                 int frames_to_key) {
  frames_to_key = AOMMIN(frames_to_key, 100);
  frames_to_key = AOMMAX(frames_to_key, 16);
  int boost =
      (frames_to_key * prior_boost + (100 - frames_to_key) * tpl_boost) / 100;
  return boost;
}
  ...
  if (tpl_frame->is_valid) {
    ...
    if (mc_dep_cost_base == 0) {
      ...
      if (is_frame_arf_and_tpl_eligible(cpi)) {
        ...
        const int gfu_boost =
            get_gfu_boost_from_r0(cpi->rd.r0, cpi->rc.frames_to_key);
        // printf("old gf boost %d new gf boost %d [%d]\n", cpi->rc.gfu_boost,
        //        gfu_boost, cpi->rc.frames_to_key);
