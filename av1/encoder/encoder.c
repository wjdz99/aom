...
static void scale_and_extend_frame(const YV12_BUFFER_CONFIG *src,
  ...
  assert(planes <= 3);
  assert((src->uv_width < src->y_width) == (dst->uv_width < dst->y_width));
  assert((src->uv_height < src->y_height) == (dst->uv_height < dst->y_height));
  for (y = 0; y < dst_h; y += 16) {
    for (x = 0; x < dst_w; x += 16) {
      for (i = 0; i < planes; ++i) {
        int ss_x = i == 0 ? 0 : src->uv_width < src->y_width;
        int ss_y = i == 0 ? 0 : src->uv_height < src->y_height;
