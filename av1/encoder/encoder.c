...
static int get_ref_frame_flags(const AV1_COMP *cpi) {
  ...
  // TODO(yunqing): After av1_apply_encoding_flags() is called,
  // cpi->ref_frame_flags might be adjusted according to external encoder flags.
  // Need to ensure that there is not conflict between external reference frame
  // flags and internal decision made in this function.
...
void av1_apply_encoding_flags(AV1_COMP *cpi, aom_enc_frame_flags_t flags) {
  // TODO(yunqing): For what references to use, external encoding flags should
  // be consistent with internal reference frame selection. Currently, LAST
  // FRAME has highest priority to be used. Suggest to disable GF & ARF first.
