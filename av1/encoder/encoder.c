...
static void cdef_restoration_frame(AV1_COMP *cpi, AV1_COMMON *cm,
  ...
  uint16_t *rec_yuv[3];
  uint16_t *ext_rec_y;
  int use_ccso = !cm->features.coded_lossless && !cm->tiles.large_scale && cm->seq_params.enable_ccso;
  const int num_planes = av1_num_planes(cm);
  av1_setup_dst_planes(xd->plane, cm->seq_params.sb_size, &cm->cur_frame->buf, 0, 0, 0,
    num_planes);
  ...
  if (use_ccso) {
    ...
    for (int pli = 0; pli < 1; pli++) {
      rec_yuv[pli] = aom_malloc(sizeof(*rec_yuv) * xd->plane[0].dst.height * ccso_stride);
      ...
      for (int r = 0; r < pic_height; ++r) {
        for (int c = 0; c < pic_width; ++c) {
          if (cm->seq_params.use_highbitdepth) {
            ...
            rec_yuv[pli][r * ccso_stride + c] = xd->plane[pli].dst.buf[r * dst_stride + c];
          }
        }
      }
    }
  ...
  if (use_ccso) {
    ...
    ccso_search(&cm->cur_frame->buf, cpi->source, cm, xd,
      cpi->td.mb.rdmult, ext_rec_y, rec_yuv);
