...
static void set_frame_size(AV1_COMP *cpi, int width, int height) {
  ...
  // cm->ctx_width = cm->width;
  // cm->ctx_height = cm->height;
}
...
static void superres_post_encode(AV1_COMP *cpi) {
  ...
  if (av1_resize_unscaled(cpi)) {  // TODO(afergs): Check if this does anything.
    ...
    if (aom_realloc_frame_buffer(&cpi->scaled_source, cm->superres_upscaled_width, cm->superres_upscaled_height,
                                 cm->subsampling_x, cm->subsampling_y,
#if CONFIG_HIGHBITDEPTH
                                 cm->use_highbitdepth,
#endif  // CONFIG_HIGHBITDEPTH
    ...
    assert(cpi->scaled_source.y_width == cm->superres_upscaled_width);
    assert(cpi->scaled_source.y_height == cm->superres_upscaled_height);
    assert(cpi->unscaled_source->y_height != cm->superres_upscaled_height ||
           cpi->unscaled_source->y_width != cm->superres_upscaled_width);
#if CONFIG_HIGHBITDEPTH
    av1_resize_and_extend_frame(cpi->unscaled_source, &cpi->scaled_source, (int)cm->bit_depth);
#else
    ...
    if (cpi->last_source != NULL) {
      ...
      if (!aom_realloc_frame_buffer(
              &cpi->scaled_last_source, cm->superres_upscaled_width, cm->superres_upscaled_height,
              cm->subsampling_x, cm->subsampling_y,
#if CONFIG_HIGHBITDEPTH
              cm->use_highbitdepth,
#endif  // CONFIG_HIGHBITDEPTH
        ...
        aom_internal_error(
            ...
            "upscaling");
      assert(cpi->scaled_last_source.y_width == cm->superres_upscaled_width);
      assert(cpi->scaled_last_source.y_height == cm->superres_upscaled_height);
      assert(cpi->unscaled_last_source->y_height != cm->superres_upscaled_height ||
             cpi->unscaled_last_source->y_width != cm->superres_upscaled_width);
#if CONFIG_HIGHBITDEPTH
      av1_resize_and_extend_frame(cpi->unscaled_last_source, &cpi->scaled_last_source, (int)cm->bit_depth);
#else
      av1_resize_and_extend_frame(cpi->unscaled_last_source, &cpi->scaled_last_source);
#endif  // CONFIG_HIGHBITDEPTH
...
/* quasi-random initialization of current_frame_id for a key frame */
  ...
  // int new_num = cm->current_video_frame < 2 ? rand() % 9 + 8 : 16;
