...
static void init_buffer_indices(AV1_COMP *cpi) {
  ...
  for (fb_idx = 0; fb_idx < REF_FRAMES; ++fb_idx)
    cpi->remapped_reference_idx[fb_idx] = fb_idx;
  cpi->rate_index = 0;
  cpi->rate_size = 0;
  cpi->cur_poc = -1;
}
...
static void check_show_existing_frame(AV1_COMP *cpi) {
  ...
  if (cm->show_existing_frame == 1) {
    ...
    if (cpi->new_bwdref_update_rule == 0) {
      ...
      cpi->existing_fb_idx_to_show = cpi->remapped_reference_idx[0];
#if USE_SYMM_MULTI_LAYER
    }
#endif
    ...
    cpi->existing_fb_idx_to_show =
        (next_frame_update_type == OVERLAY_UPDATE)
            ...
            : cpi->remapped_reference_idx[bwdref_to_show - 1];
#if USE_SYMM_MULTI_LAYER
    if (cpi->new_bwdref_update_rule == 0)
#endif
...
static INLINE void shift_last_ref_frames(AV1_COMP *cpi) {
  ...
  for (ref_frame = LAST_REF_FRAMES - 1; ref_frame > 0; --ref_frame) {
    cpi->remapped_reference_idx[ref_frame] =
        cpi->remapped_reference_idx[ref_frame - 1];
...
static INLINE void rshift_bwd_ref_frames(AV1_COMP *cpi) {
  ...
  for (int i = 2; i > 0; --i) {
    ...
    cpi->remapped_reference_idx[ordered_bwd[i]] =
        cpi->remapped_reference_idx[ordered_bwd[i - 1]];
  }
}
...
static void update_reference_frames(AV1_COMP *cpi) {
  ...
  if ((cm->frame_type == KEY_FRAME && cm->show_frame) || frame_is_sframe(cm)) {
    for (int ref_frame = 0; ref_frame < REF_FRAMES; ++ref_frame) {
      ref_cnt_fb(pool->frame_bufs,
                 &cm->ref_frame_map[cpi->remapped_reference_idx[ref_frame]],
                 cm->new_fb_idx);
    }
    return;
  }
  ...
  if (av1_preserve_existing_gf(cpi)) {
    ...
    if (!cpi->preserve_arf_as_gld) {
      ref_cnt_fb(
          pool->frame_bufs,
          &cm->ref_frame_map[cpi->remapped_reference_idx[ALTREF_FRAME - 1]],
          cm->new_fb_idx);
    }

    tmp = cpi->remapped_reference_idx[ALTREF_FRAME - 1];
    cpi->remapped_reference_idx[ALTREF_FRAME - 1] =
        cpi->remapped_reference_idx[GOLDEN_FRAME - 1];
    cpi->remapped_reference_idx[GOLDEN_FRAME - 1] = tmp;
    ...
    const int tmp = cpi->remapped_reference_idx[LAST_REF_FRAMES - 1];
    shift_last_ref_frames(cpi);

    cpi->remapped_reference_idx[LAST_FRAME - 1] =
        cpi->remapped_reference_idx[bwdref_to_show - 1];
    ...
    if (cpi->new_bwdref_update_rule == 1) {
      ...
      // spared space
      cpi->remapped_reference_idx[EXTREF_FRAME - 1] = tmp;
    } else {
#endif
      cpi->remapped_reference_idx[bwdref_to_show - 1] = tmp;
#if USE_SYMM_MULTI_LAYER
    }
#endif
    ...
    if (cpi->refresh_alt_ref_frame) {
      int arf_idx = cpi->remapped_reference_idx[ALTREF_FRAME - 1];
    ...
    if (cpi->refresh_golden_frame) {
      ref_cnt_fb(
          pool->frame_bufs,
          &cm->ref_frame_map[cpi->remapped_reference_idx[GOLDEN_FRAME - 1]],
          cm->new_fb_idx);
    ...
    if (cpi->refresh_bwd_ref_frame) {
      ...
      if (cpi->new_bwdref_update_rule) {
        ...
        int tmp = cpi->remapped_reference_idx[EXTREF_FRAME - 1];
        ref_cnt_fb(pool->frame_bufs, &cm->ref_frame_map[tmp], cm->new_fb_idx);

        rshift_bwd_ref_frames(cpi);
        cpi->remapped_reference_idx[BWDREF_FRAME - 1] = tmp;
      } else {
#endif  // USE_SYMM_MULTI_LAYER
        ref_cnt_fb(
            pool->frame_bufs,
            &cm->ref_frame_map[cpi->remapped_reference_idx[BWDREF_FRAME - 1]],
            cm->new_fb_idx);
#if USE_SYMM_MULTI_LAYER
      }
#endif
    ...
    if (cpi->refresh_alt2_ref_frame) {
      ref_cnt_fb(
          pool->frame_bufs,
          &cm->ref_frame_map[cpi->remapped_reference_idx[ALTREF2_FRAME - 1]],
          cm->new_fb_idx);
  ...
  if (cpi->refresh_last_frame) {
    ...
    //      |                     |                     |
    //      v                     v                     v
    // remapped_reference_idx[0],   ..., remapped_reference_idx[2],   ...,
    // remapped_reference_idx[ALTREF_FRAME-1]
    //      |                     |                     |
    //      v                     v                     v
    ...
    //      |                |                |
    //      v                v                v
    // remapped_reference_idx[2],   remapped_reference_idx[0],
    // remapped_reference_idx[1]
    int tmp;

    ref_cnt_fb(
        pool->frame_bufs,
        &cm->ref_frame_map[cpi->remapped_reference_idx[LAST_REF_FRAMES - 1]],
        cm->new_fb_idx);

    tmp = cpi->remapped_reference_idx[LAST_REF_FRAMES - 1];

    shift_last_ref_frames(cpi);
    cpi->remapped_reference_idx[0] = tmp;
...
static int get_ref_frame_flags(const AV1_COMP *cpi) {
  ...
  const int last2_is_last = map[cpi->remapped_reference_idx[1]] ==
                            map[cpi->remapped_reference_idx[0]];
  const int last3_is_last = map[cpi->remapped_reference_idx[2]] ==
                            map[cpi->remapped_reference_idx[0]];
  const int gld_is_last = map[cpi->remapped_reference_idx[GOLDEN_FRAME - 1]] ==
                          map[cpi->remapped_reference_idx[0]];
  const int bwd_is_last = map[cpi->remapped_reference_idx[BWDREF_FRAME - 1]] ==
                          map[cpi->remapped_reference_idx[0]];
  const int alt2_is_last =
      map[cpi->remapped_reference_idx[ALTREF2_FRAME - 1]] ==
      map[cpi->remapped_reference_idx[0]];
  const int alt_is_last = map[cpi->remapped_reference_idx[ALTREF_FRAME - 1]] ==
                          map[cpi->remapped_reference_idx[0]];

  // No.2 Priority: ALTREF_FRAME
  const int last2_is_alt = map[cpi->remapped_reference_idx[1]] ==
                           map[cpi->remapped_reference_idx[ALTREF_FRAME - 1]];
  const int last3_is_alt = map[cpi->remapped_reference_idx[2]] ==
                           map[cpi->remapped_reference_idx[ALTREF_FRAME - 1]];
  const int gld_is_alt = map[cpi->remapped_reference_idx[GOLDEN_FRAME - 1]] ==
                         map[cpi->remapped_reference_idx[ALTREF_FRAME - 1]];
  const int bwd_is_alt = map[cpi->remapped_reference_idx[BWDREF_FRAME - 1]] ==
                         map[cpi->remapped_reference_idx[ALTREF_FRAME - 1]];
  const int alt2_is_alt = map[cpi->remapped_reference_idx[ALTREF2_FRAME - 1]] ==
                          map[cpi->remapped_reference_idx[ALTREF_FRAME - 1]];

  // No.3 Priority: LAST2_FRAME
  const int last3_is_last2 = map[cpi->remapped_reference_idx[2]] ==
                             map[cpi->remapped_reference_idx[1]];
  const int gld_is_last2 = map[cpi->remapped_reference_idx[GOLDEN_FRAME - 1]] ==
                           map[cpi->remapped_reference_idx[1]];
  const int bwd_is_last2 = map[cpi->remapped_reference_idx[BWDREF_FRAME - 1]] ==
                           map[cpi->remapped_reference_idx[1]];
  ...
  const int gld_is_last3 = map[cpi->remapped_reference_idx[GOLDEN_FRAME - 1]] ==
                           map[cpi->remapped_reference_idx[2]];
  const int bwd_is_last3 = map[cpi->remapped_reference_idx[BWDREF_FRAME - 1]] ==
                           map[cpi->remapped_reference_idx[2]];
  ...
  const int bwd_is_gld = map[cpi->remapped_reference_idx[BWDREF_FRAME - 1]] ==
                         map[cpi->remapped_reference_idx[GOLDEN_FRAME - 1]];
  const int alt2_is_gld = map[cpi->remapped_reference_idx[ALTREF2_FRAME - 1]] ==
                          map[cpi->remapped_reference_idx[GOLDEN_FRAME - 1]];

  // No.6 Priority: BWDREF_FRAME
  const int alt2_is_bwd = map[cpi->remapped_reference_idx[ALTREF2_FRAME - 1]] ==
                          map[cpi->remapped_reference_idx[BWDREF_FRAME - 1]];
