...
static INLINE int do_extend_borders_with_lpf_mt(AV1_COMMON *cm) {
  return (!is_cdef_used(cm) && !is_restoration_used(cm) &&
          !av1_superres_scaled(cm));
}
    ...
    if (cm->rst_info[0].frame_restoration_type != RESTORE_NONE ||
      ...
      if (num_workers > 1)
        av1_loop_restoration_filter_frame_mt(
            ...
            &mt_info->lr_row_sync, &cpi->lr_ctxt, 1);
      else
...
static void loopfilter_frame(AV1_COMP *cpi, AV1_COMMON *cm) {
  ...
  if ((lf->filter_level[0] || lf->filter_level[1]) &&
    ...
    int do_extend_border = do_extend_borders_with_lpf_mt(cm);
    av1_loop_filter_frame_mt(
        &cm->cur_frame->buf, cm, xd, 0, num_planes, 0, mt_info->workers,
        num_workers, &mt_info->lf_row_sync, lpf_opt_level, do_extend_border);
  }
#if CONFIG_COLLECT_COMPONENT_TIMING
...
static int encode_with_recode_loop_and_filter(AV1_COMP *cpi, size_t *size,
  ...
  for (int plane = 0; plane < av1_num_planes(cm); ++plane) {
    if (cm->extend_border_mt[plane] == 0) {
      YV12_BUFFER_CONFIG *ybf = &cm->cur_frame->buf;
