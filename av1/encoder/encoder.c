...
static void config_target_level(AV1_COMP *const cpi, AV1_LEVEL target_level,
  ...
  // Adjust target bitrate to be no larger than 85% of level limit.
  const BITSTREAM_PROFILE profile = seq_params->profile;
  const double level_bitrate_limit =
      av1_get_max_bitrate_for_level(target_level, tier, profile);
  const int64_t max_bitrate = (int64_t)(level_bitrate_limit * 0.85);
  oxcf->target_bandwidth = AOMMIN(oxcf->target_bandwidth, max_bitrate);
  cpi->rc.level_imposed_max_bandwidth = (int)level_bitrate_limit;

  // Adjust max over-shoot percentage.
  const double target_bitrate = (double)oxcf->target_bandwidth;
  const int max_over_shoot_pct =
      (int)((level_bitrate_limit * 0.95 - target_bitrate) * 100.0 /
          target_bitrate);
  oxcf->over_shoot_pct = AOMMIN(oxcf->over_shoot_pct, max_over_shoot_pct);
...
void av1_change_config(struct AV1_COMP *cpi, const AV1EncoderConfig *oxcf) {
  ...
  if (cpi->target_seq_level_idx[0] < SEQ_LEVELS) {
    ...
    cpi->rc.level_imposed_max_bandwidth = 0;
  }
...
int av1_get_compressed_data(AV1_COMP *cpi, unsigned int *frame_flags,
  ...
  *size = 0;
  ...
  if (cpi->keep_level_stats && oxcf->pass != 1) {
    ...
    printf("\n level is %d, target bitrate %ld, bitrate %d\n",
           level, cpi->oxcf.target_bandwidth,
           cpi->level_info[0]->level_stats.max_bitrate);
  }
