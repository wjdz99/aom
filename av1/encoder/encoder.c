...
//AV1_COMP *cpi_0 = aom_malloc(sizeof(*cpi));
//uint8_t *dest_0 = aom_malloc(368640);
//size_t size_0 = *size;
//memcpy(cpi_0, cpi, sizeof(*cpi));
//memcpy(dest_0, dest, 368640);
...
//  frcount++;
  ...
  for (int dq_type = 3; dq_type <= DQ_TYPES; dq_type++) {
//  if (frcount == 5 && dq_type < DQ_TYPES)
//    printf("debug\n");
    cm->dq_type = dq_type == DQ_TYPES ? 3 : 2; //best_dq_type : dq_type;
#endif  // CONFIG_NEW_QUANT && SEARCH_NEW_QUANT
    do {
      aom_clear_system_state();
          ...
          if (rc->projected_frame_size > rc->this_frame_target) {
            ...
            if (undershoot_seen || loop_at_this_size > 1) {
              ...
              while (q < q_low && retries < 10) {
                av1_rc_update_rate_correction_factors(cpi, cm->width,
                                                      cm->height);
                q = av1_rc_regulate_q(cpi, rc->this_frame_target, bottom_index,
                                      AOMMAX(q_high, top_index), cm->width,
                                      cm->height);
