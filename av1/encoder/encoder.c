...
int av1_get_compressed_data(AV1_COMP *cpi, unsigned int *frame_flags,
...
#if CONFIG_XIPHRC
      struct aom_codec_cx_pkt pkt;
      uint8_t buf[OD_RC_2PASS_SUMMARY_SZ];
      int stats_size = od_enc_rc_2pass_out(&cpi->od_rc, buf, 1);
      pkt.data.twopass_stats.buf = buf;
      pkt.data.twopass_stats.sz = stats_size;
  ...
  *size = 0;
  ...
  if (oxcf->pass == 2) {
#if CONFIG_XIPHRC
    int ret = od_enc_rc_2pass_in(&cpi->od_rc);
    if (ret < 0) assert(0 && "2 pass input failure!");
  }
#else
    av1_rc_get_second_pass_params(cpi);
  } else if (oxcf->pass == 1) {
...
#if CONFIG_XIPHRC
  if (oxcf->pass == 1) {
    size_t tmp, stats_size;
    struct aom_codec_cx_pkt pkt;
    uint8_t buf[OD_RC_2PASS_SUMMARY_SZ];
    Pass0Encode(cpi, &tmp, dest, frame_flags);
    stats_size = od_enc_rc_2pass_out(&cpi->od_rc, buf, 0);
    pkt.data.twopass_stats.buf = buf;
    pkt.data.twopass_stats.sz = stats_size;
    pkt.kind = AOM_CODEC_STATS_PKT;
    aom_codec_pkt_list_add(cpi->output_pkt_list, &pkt);
