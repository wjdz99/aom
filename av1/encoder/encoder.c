...
static MvSubpelPrecision determine_frame_mv_precision(const AV1_COMP *cpi,
  ...
  if (buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    for (int i = 0; i < height; i += MV_PREC_DET_BLK_SIZE) {
      for (int j = 0; j < width; j += MV_PREC_DET_BLK_SIZE) {
        const uint16_t *src = src16 + i * stride + j;
        int64_t gx = 0, gy = 0, g;
        for (int y = 0; y < 16; ++y) {
          for (int x = 0; x < 16; ++x) {
            gx += abs(src[-stride + 1] - src[-stride - 1] + src[1] -
                      src[-1] * 2 + src[stride + 1] - src[stride - 1]);
            gy += abs(src[stride - 1] - src[-stride - 1] + src[stride] -
                      src[-stride] * 2 + src[stride + 1] - src[-stride + 1]);
          }
        }
        g = gx * gx + gy * gy;
  ...
  // printf("frac_edge_blks = %f, q = %d, prod = %f\n",
  //        frac_edge_blks, q, frac_edge_blks * q);
  if (frac_edge_blks * q >= MV_PREC_DET_THRESH)
  ...
  *q = av1_rc_pick_q_and_bounds(cpi, cm->width, cm->height, bottom_index,
  ...
  if (!frame_is_intra_only(cm)) {
    if (!cpi->common.cur_frame_force_integer_mv) {
      MvSubpelPrecision precision = determine_frame_mv_precision(cpi, *q);
      set_mv_precision(cpi, precision, cpi->common.cur_frame_force_integer_mv);
    }
    // set_mv_precision(cpi, (*q) < HIGH_PRECISION_MV_QTHRESH ?
    //                  MV_SUBPEL_EIGHTH_PRECISION : MV_SUBPEL_QTR_PRECISION,
