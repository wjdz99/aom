    ...
    if (!is_stat_generation_stage(cpi)) {
      ...
      if (cpi->b_calculate_psnr) {
        ...
        if ((cpi->source->flags & YV12_FLAG_HIGHBITDEPTH) &&
          ...
          SNPRINT(headings,
                  "\tAVPsnr_H\tGLPsnr_H\tAVPsnrP_H\tGLPsnrP_H"
                  "\tAVPsnrY_H\tAPsnrCb_H\tAPsnrCr_H");
          SNPRINT2(results, "\t%7.3f",
...
static void generate_psnr_packet(AV1_COMP *cpi) {
  ...
  if ((cpi->source->flags & YV12_FLAG_HIGHBITDEPTH) &&
      (in_bit_depth < bit_depth)) {
    for (i = 0; i < 4; ++i) {
      pkt.data.psnr_hbd.samples[i] = psnr.samples_hbd[i];
      pkt.data.psnr_hbd.sse[i] = psnr.sse_hbd[i];
      pkt.data.psnr_hbd.psnr[i] = psnr.psnr_hbd[i];
    }
  }
#endif
