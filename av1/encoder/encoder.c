...
static double get_kf_boost_projection_factor(int frames_count) {
  double factor = sqrt((double)frames_count);
  factor = AOMMIN(factor, 10.0);
  factor = AOMMAX(factor, 4.0);
  factor = (int)rint((75.0 + 14.0 * factor));
  return factor;
}
