...
static void superres_post_encode(AV1_COMP *cpi) {
  ...
  if (cm->current_video_frame == 1 && !cm->show_frame) {
    int i;
    char fname[64];
    sprintf(fname, "Enc_presuperres%d_%d.y", cm->current_video_frame, cm->show_frame);
    FILE *fp = fopen(fname, "wb");
    for (i = 0; i < cm->frame_to_show->y_crop_height; i++)
      fwrite(cm->frame_to_show->y_buffer + cm->frame_to_show->y_stride * i,
             cm->frame_to_show->y_crop_width, 1, fp);
    fclose(fp);
  }
      ...
      if (cpi->unscaled_last_source != NULL) {
        if (aom_realloc_frame_buffer(
          ...
          aom_internal_error(
              ...
              "upscaling");
        assert(cpi->scaled_last_source.y_crop_width ==
               cm->superres_upscaled_width);
        assert(cpi->scaled_last_source.y_crop_height ==
               cm->superres_upscaled_height);
               cm->superres_upscaled_height ||
