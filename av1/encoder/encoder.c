    ...
    if (cpi->active_map.enabled) {
      for (i = 0; i < cpi->common.mi_rows * cpi->common.mi_cols; ++i)
        if (seg_map[i] == AM_SEGMENT_ID_ACTIVE)
          seg_map[i] = active_map[i] == AM_SEGMENT_ID_INACTIVE
                           ? seg->num_seg - 1
    ...
    if (cpi->active_map.enabled) {
      ...
      for (r = 0; r < mi_rows; ++r) {
        for (c = 0; c < mi_cols; ++c) {
          ...
          new_map_16x16[(r >> 1) * cols + (c >> 1)] |=
#if CONFIG_EXT_SEGMENT
              av1_extract_segment_id_from_map(seg_map_8x8[r * mi_cols + c],
                                              ACTIVE_SEG_IDX) !=
              cpi->common.seg[ACTIVE_SEG_IDX].num_seg - 1;
#else
...
static void configure_static_seg_features(AV1_COMP *cpi) {
  ...
  if (cm->frame_type == KEY_FRAME) {
    ...
    if (quality_seg->enabled) {
      ...
      av1_set_segdata(quality_seg, 1, QUALITY_SEG_LVL_ALT_Q, qi_delta - 2,
                      QUALITY_SEG_IDX);
      av1_set_segdata(quality_seg, 1, QUALITY_SEG_LVL_ALT_LF, -2,
                      QUALITY_SEG_IDX);
    ...
    if (rc->frames_since_golden == 0) {
      ...
      if (rc->source_alt_ref_active) {
        ...
        if (high_q || (cpi->static_mb_pct == 100)) {
          av1_set_segdata(active_seg, 1, ACTIVE_SEG_LVL_REF_FRAME, ALTREF_FRAME,
                          ACTIVE_SEG_IDX);
      ...
      av1_set_segdata(active_seg, 0, ACTIVE_SEG_LVL_REF_FRAME, ALTREF_FRAME,
                      ACTIVE_SEG_IDX);
...
static void update_reference_segmentation_map(AV1_COMP *cpi,
  ...
  for (row = 0; row < cm->mi_rows; row++) {
    ...
    for (col = 0; col < cm->mi_cols; col++, mi_8x8++, cache++) {
      av1_store_segment_id_into_map(mi_8x8[0]->mbmi.segment_id[seg_cat_idx],
                                    &cache[0], seg_cat_idx);
    }
...
// exclusive.
  ...
  if (cpi->oxcf.aq_mode == VARIANCE_AQ ||
      cpi->oxcf.aq_mode == REPEAT_VARIANCE_AQ) {
#else
  if (cpi->oxcf.aq_mode == VARIANCE_AQ) {
#endif
...
// Reset the loop filter deltas and segmentation map.
    ...
    switch (oxcf->aq_mode) {
      ...
      default:
        ...
        cm->seg[QUALITY_SEG_IDX].seg_map_minb_size_log2_minus3 = 0;
    }
    if (cpi->active_map.enabled) cm->seg[ACTIVE_SEG_IDX].num_seg = 2;
    cm->seg[ACTIVE_SEG_IDX].seg_map_minb_size_log2_minus3 =
        oxcf->min_active_seg_unit_size;
    cm->seg[ACTIVE_SEG_IDX].skip_seg_id_disabled = 0;
    cm->seg[ACTIVE_SEG_IDX].skip_seg_id_replacement = 0;
    cm->seg[QUALITY_SEG_IDX].skip_seg_id_disabled =
        oxcf->skip_quality_seg_id_disabled;
    cm->seg[QUALITY_SEG_IDX].skip_seg_id_replacement =
        oxcf->skip_quality_seg_id_replacement;
#endif
...
// off.
  ...
  if (cm->seg[ACTIVE_SEG_IDX].update_map)
    update_reference_segmentation_map(cpi, ACTIVE_SEG_IDX);
  if (cm->seg[QUALITY_SEG_IDX].enabled)
    update_reference_segmentation_map(cpi, QUALITY_SEG_IDX);
#else
...
static int frame_is_reference(const AV1_COMP *cpi) {
  ...
  return cm->frame_type == KEY_FRAME || cpi->refresh_last_frame ||
         ...
         cm->seg[ACTIVE_SEG_IDX].update_data ||
         cm->seg[QUALITY_SEG_IDX].update_map ||
         cm->seg[QUALITY_SEG_IDX].update_data;
#else
