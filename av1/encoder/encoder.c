...
void av1_alloc_obmc_buffers(OBMCBuffer *obmc_buffer, AV1_COMMON *cm) {
  CHECK_MEM_ERROR(cm, obmc_buffer->wsrc_buf,
                  (int32_t *)aom_memalign(
                      16, MAX_SB_SQUARE * sizeof(*obmc_buffer->wsrc_buf)));
  CHECK_MEM_ERROR(cm, obmc_buffer->mask_buf,
                  (int32_t *)aom_memalign(
                      16, MAX_SB_SQUARE * sizeof(*obmc_buffer->mask_buf)));
  CHECK_MEM_ERROR(
      cm, obmc_buffer->above_pred_buf,
      (uint8_t *)aom_memalign(16, MAX_MB_PLANE * MAX_SB_SQUARE *
                                      sizeof(*obmc_buffer->above_pred_buf)));
  CHECK_MEM_ERROR(
      cm, obmc_buffer->left_pred_buf,
      (uint8_t *)aom_memalign(16, MAX_MB_PLANE * MAX_SB_SQUARE *
                                      sizeof(*obmc_buffer->left_pred_buf)));
}

void av1_release_obmc_buffers(OBMCBuffer *obmc_buffer) {
  aom_free(obmc_buffer->mask_buf);
  aom_free(obmc_buffer->above_pred_buf);
  aom_free(obmc_buffer->left_pred_buf);
  aom_free(obmc_buffer->wsrc_buf);

  obmc_buffer->mask_buf = NULL;
  obmc_buffer->above_pred_buf = NULL;
  obmc_buffer->left_pred_buf = NULL;
  obmc_buffer->wsrc_buf = NULL;
}
