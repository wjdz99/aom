...
static int encode_with_recode_loop(AV1_COMP *cpi, size_t *size, uint8_t *dest) {
  ...
  const int passes_to_determine_screen_content_type =
      is_screen_content_type_orig_decision ? 1 : (is_key_frame ? 3 : 1);
  const int q_orig = q;
  ...
  printf("\n Encoding a frame:");
#endif
  for (int pass = 0; pass < passes_to_determine_screen_content_type; ++pass) {
    q = set_or_save_encoding_params_for_screen_content(
        cpi, passes_to_determine_screen_content_type, pass,
        q_for_screen_content_quick_run, q_orig, partition_search_type_orig,
    ...
    do {
      ...
      if (loop) printf("\n Recoding:");
#endif
      // Do not allow recoding when we test encoding with and without
      // screen content tools.
      if (passes_to_determine_screen_content_type > 1 && pass < 2) loop = 0;
    } while (loop);
