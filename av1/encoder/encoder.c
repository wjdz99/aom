...
static int encode_with_recode_loop_and_filter(AV1_COMP *cpi, size_t *size,
  ...
  if (cpi->ppi->rtc_ref.set_ref_frame_config &&
      cpi->oxcf.gf_cfg.lag_in_frames > 0 && cpi->svc.spatial_layer_id > 0 &&
...
int av1_receive_raw_frame(AV1_COMP *cpi, aom_enc_frame_flags_t frame_flags,
  ...
  if (cpi->svc.spatial_layer_id == 0 || cpi->oxcf.gf_cfg.lag_in_frames == 0) {
...
void av1_post_encode_updates(AV1_COMP *const cpi,
  ...
  if (ppi->rtc_ref.set_ref_frame_config && cpi->oxcf.gf_cfg.lag_in_frames > 0 &&
      cpi->svc.number_spatial_layers > 1) {
    GF_GROUP *gf_group = &cpi->ppi->gf_group;
