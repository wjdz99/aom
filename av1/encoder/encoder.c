...
static uint8_t calculate_next_resize_scale(const AV1_COMP *cpi) {
  ...
  const AV1EncoderConfig *oxcf = &cpi->oxcf;
  if (is_analysis_stage(cpi)) return SCALE_NUMERATOR;
...
static size_params_type calculate_next_size_params(AV1_COMP *cpi) {
  ...
  if (is_analysis_stage(cpi)) return rsz;
...
static int encode_frame_to_data_rate(AV1_COMP *cpi, size_t *size,
  ...
  if (!is_analysis_stage(cpi) && cpi->common.allow_screen_content_tools &&
      !frame_is_intra_only(cm)) {
  ...
  if (!is_analysis_stage(cpi) && cpi->need_to_clear_prev_hash_table) {
...
int av1_get_compressed_data(AV1_COMP *cpi, unsigned int *frame_flags,
  ...
  *size = 0;
  ...
  if (cpi->b_calculate_psnr) {
    if (cm->show_existing_frame ||
        (!is_analysis_stage(cpi) && cm->show_frame)) {
      generate_psnr_packet(cpi);
    }
  }

  if (cpi->keep_level_stats && !is_analysis_stage(cpi)) {
...
#endif  // CONFIG_INTERNAL_STATS
#if CONFIG_SPEED_STATS
  if (!is_analysis_stage(cpi) && !cm->show_existing_frame) {
