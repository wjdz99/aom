...
static int encode_without_recode(AV1_COMP *cpi) {
  ...
  if (cm->current_frame.frame_number == 0 &&
    ...
    if (aom_alloc_frame_buffer(
              &cpi->svc.source_base_tl0, cm->width, cm->height,
 ...
 if (cpi->svc.number_temporal_layers > 1 && cpi->svc.temporal_layer_id == 0) {
    aom_yv12_copy_y(cpi->source, &cpi->svc.source_base_tl0);
    aom_yv12_copy_u(cpi->source, &cpi->svc.source_base_tl0);
    aom_yv12_copy_v(cpi->source, &cpi->svc.source_base_tl0);
 }
