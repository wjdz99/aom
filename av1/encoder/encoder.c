...
static void update_reference_frames(AV1_COMP *cpi) {
  ...
  if (av1_preserve_existing_gf(cpi)) {
    ...
    if (cpi->refresh_alt_ref_frame) {
      ...
      assign_buffer(pool->frame_bufs, &cm->ref_frame_map[arf_idx],
                    cm->new_fb_idx);
    ...
    if (cpi->refresh_bwd_ref_frame) {
      ...
      if (cpi->new_bwdref_update_rule) {
        ...
        assign_buffer(pool->frame_bufs, &cm->ref_frame_map[tmp],
                      cm->new_fb_idx);
  ...
  if (cpi->refresh_last_frame) {
    ...
    //      |                     |                     |
    //      v                     v                     v
    // remapped_ref_idx[0],   ..., remapped_ref_idx[2],   ...,
    // remapped_ref_idx[ALTREF_FRAME-1]
    //      |                     |                     |
    //      v                     v                     v
    ...
    //      |                |                |
    //      v                v                v
    // remapped_ref_idx[2],   remapped_ref_idx[0],
    // remapped_ref_idx[1]
    assign_buffer(pool->frame_bufs,
