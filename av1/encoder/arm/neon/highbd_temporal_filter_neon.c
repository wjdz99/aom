...
#define CASE_ALIGN_VECTORS_32(a, b, offset) \
  case offset: return vextq_u32((a), (b), (offset)); break;

static INLINE uint32x4_t valign(uint32x4_t l, uint32x4_t r, int offset) {
  switch (offset) {
    case 0:
      return l;
      break;
      CASE_ALIGN_VECTORS_32(l, r, 1);
      CASE_ALIGN_VECTORS_32(l, r, 2);
      CASE_ALIGN_VECTORS_32(l, r, 3);
    case 4: return r; break;
    default: return l; break;
  }
}

#undef CASE_ALIGN_VECTORS_32
...
static uint32_t sum_kernel5x5_mask_double(const uint32x4_t vsrc[5][2],
                                          const uint32x4_t mask1,
                                          const uint32x4_t mask2) {
  uint32x4_t vsums = vandq_u32(vsrc[0][0], mask1);
  vsums = vaddq_u32(vsums, vandq_u32(vsrc[0][1], mask2));
  vsums = vaddq_u32(vsums, vandq_u32(vsrc[1][0], mask1));
  vsums = vaddq_u32(vsums, vandq_u32(vsrc[1][1], mask2));
  vsums = vaddq_u32(vsums, vandq_u32(vsrc[2][0], mask1));
  vsums = vaddq_u32(vsums, vandq_u32(vsrc[2][1], mask2));
  vsums = vaddq_u32(vsums, vandq_u32(vsrc[3][0], mask1));
  vsums = vaddq_u32(vsums, vandq_u32(vsrc[3][1], mask2));
  vsums = vaddq_u32(vsums, vandq_u32(vsrc[4][0], mask1));
  vsums = vaddq_u32(vsums, vandq_u32(vsrc[4][1], mask2));
  return horizontal_add_u32x4(vsums);
}
...
static void highbd_apply_temporal_filter(
  ...
  const uint32x4_t kffff = vdupq_n_u32(0xffffffff);
  const uint32x4_t k0000 = vdupq_n_u32(0);
  const uint32x4_t k3110 = { 0, 1, 1, 3 };
  const uint32x4_t k2111 = { 1, 1, 1, 2 };
  const uint32x4_t k1112 = { 2, 1, 1, 1 };
  const uint32x4_t k0113 = { 3, 1, 1, 0 };

  uint32x4_t vmask1[4], vmask2[4];
  for (int i = 0; i < 4; i++) {
    vmask1[i] = valign(k0000, kffff, 4 - i);
    vmask2[i] = valign(kffff, k0000, 4 - i - 1);
  }
  ...
  do {
    ...
    do {
      ...
      acc_5x5_neon[row][col - half_window + 0] =
          sum_kernel5x5_mask_double(vsrc, vmask1[0], vmask2[0]);
      acc_5x5_neon[row][col - half_window + 1] =
          sum_kernel5x5_mask_double(vsrc, vmask1[1], vmask2[1]);
      acc_5x5_neon[row][col - half_window + 2] =
          sum_kernel5x5_mask_double(vsrc, vmask1[2], vmask2[2]);
      acc_5x5_neon[row][col - half_window + 3] =
          sum_kernel5x5_mask_double(vsrc, vmask1[3], vmask2[3]);
