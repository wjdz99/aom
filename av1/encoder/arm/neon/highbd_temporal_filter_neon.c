...
static INLINE void load_and_pad(const uint32_t *src, uint32x4_t *dstvec,
  ...
  if (col == 0) {
    ...
    vtmp1 = vsetq_lane_u32(lane2, vtmp2, 1);
...
static INLINE uint32x4_t valign(uint32x4_t r, uint32x4_t l, int offset) {
  switch (offset) {
    case 0:
      return l;
      break;
...
static void highbd_apply_temporal_filter(
  ...
  for (uint32_t col = 0; col < block_width; col += 4) {
    ...
    const uint32x4_t vmask = vdupq_n_u32(0xffffffff);
    for (unsigned int row = 0; row < block_height; row++) {
      for (int i = 0; i < 4; i++) {
        uint32x4_t vsum = vzero;
        for (int j = 0; j < 5; j++) {
          const uint32x4_t vmask1 = valign(vzero, vmask, j % 4);
          const uint32x4_t vmask2 = valign(vmask, vzero, (j % 4) + 1);
          const uint32x4_t vsrc1 = vandq_u32(vsrc[j][0], vmask1);
          const uint32x4_t vsrc2 = vandq_u32(vsrc[j][1], vmask2);
#if AOM_ARCH_AARCH64
          vsum = vpaddq_u32(vsum, vsrc1);
          vsum = vpaddq_u32(vsum, vsrc2);
#else
          const uint32x4_t vtmp = vaddq_u32(vsrc1, vsrc2);
          const uint32x2_t lo =
              vpadd_u32(vget_low_u32(vsum), vget_high_u32(vsum));
          const uint32x2_t hi =
              vpadd_u32(vget_low_u32(vtmp), vget_high_u32(vtmp));
          vsum = vcombine_u32(lo, hi);
#endif
        }
      ...
      if (row <= block_height - 4) {
        ...
        load_and_pad(src, vsrc[4], col, block_width);
        src += SSE_STRIDE;
      } else {
