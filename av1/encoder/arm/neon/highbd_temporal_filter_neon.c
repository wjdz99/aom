...
double av1_highbd_estimate_noise_from_single_plane_neon(const uint16_t *src,
  ...
  do {
    ...
    while (w - 1 >= 8) {
      ...
      ga = vrshlq_u16(ga, vdupq_n_u16(8 - bitdepth));
    ...
    if (w - 1 >= 4) {
      ...
      ga = vrshl_s16(ga, vdup_n_s16(8 - bitdepth));
      ...
      uint32x4_t v_u32 = vmovl_u16(vrshl_u16(v, vdup_n_u16(8 - bitdepth)));

      acc = vpadalq_u32(acc, v_u32);
      // Add -1 for each lane where the gradient is under the threshold.
      count = vaddw_s16(count, thresh_u16);
