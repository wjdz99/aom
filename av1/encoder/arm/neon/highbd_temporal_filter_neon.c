...
static void highbd_apply_temporal_filter(
  ...
  uint32_t acc_5x5_neon[BW];

  const uint32x4_t _0001 = { 0, 0, 0, 1 };
  const uint32x4_t _1233 = { 3, 3, 2, 1 };
  const uint32x4_t _0013 = { 0, 0, 1, 3 };

  uint32x4_t vsrc[3][2];

  const uint32_t *src = frame_sse;

  // For the first column, replicate the first element twice to the left
  vsrc[0][0] = vld1q_u32(src);
  vsrc[0][1] = vld1q_u32(src + 4);
  src += SSE_STRIDE;
  vsrc[1][0] = vld1q_u32(src);
  vsrc[1][1] = vld1q_u32(src + 4);
  src += SSE_STRIDE;
  vsrc[2][0] = vld1q_u32(src);
  vsrc[2][1] = vld1q_u32(src + 4);

  uint32_t lane2 = vgetq_lane_u32(vsrc[0][0], 2);
  vsrc[0][0] = vsetq_lane_u32(lane2, vsrc[0][0], 0);
  vsrc[0][0] = vsetq_lane_u32(lane2, vsrc[0][0], 1);
  lane2 = vgetq_lane_u32(vsrc[1][0], 2);
  vsrc[1][0] = vsetq_lane_u32(lane2, vsrc[1][0], 0);
  vsrc[1][0] = vsetq_lane_u32(lane2, vsrc[1][0], 1);
  lane2 = vgetq_lane_u32(vsrc[2][0], 2);
  vsrc[2][0] = vsetq_lane_u32(lane2, vsrc[2][0], 0);
  vsrc[2][0] = vsetq_lane_u32(lane2, vsrc[2][0], 1);

  uint32x4_t vsum = vmlaq_u32(vsrc[1][1], vsrc[0][0], _1233);
  vsum = vmlaq_u32(vsum, vsrc[0][1], _0013);
  vsum = vmlaq_u32(vsum, vsrc[1][0], _0001);
  uint32_t sum = horizontal_add_u32x4(vsum);
  vsum = vdupq_n_u32(sum);

  vst1q_u32(&acc_5x5_neon[0], vsum);
  ...
  do {
    ...
    vsum = vmlaq_u32(vsrc[1][1], vsrc[0][0], _1233);
    vsum = vmlaq_u32(vsum, vsrc[0][1], _0013);
    vsum = vmlaq_u32(vsum, vsrc[1][0], _0001);
    sum = horizontal_add_u32x4(vsum);
    vsum = vdupq_n_u32(sum);

    vst1q_u32(&acc_5x5_neon[col], vsum);
    col += 4;
  ...
  uint32_t lane5 = vgetq_lane_u32(vsrc[0][1], 1);
  vsrc[0][1] = vsetq_lane_u32(lane5, vsrc[0][1], 2);
  vsrc[0][1] = vsetq_lane_u32(lane5, vsrc[0][1], 3);
  lane5 = vgetq_lane_u32(vsrc[1][1], 1);
  vsrc[1][1] = vsetq_lane_u32(lane5, vsrc[1][1], 2);
  vsrc[1][1] = vsetq_lane_u32(lane5, vsrc[1][1], 3);
  lane5 = vgetq_lane_u32(vsrc[2][1], 1);
  vsrc[2][1] = vsetq_lane_u32(lane5, vsrc[2][1], 2);
  vsrc[2][1] = vsetq_lane_u32(lane5, vsrc[2][1], 3);
  ...
  vst1q_u32(&acc_5x5_neon[block_width - 4], vsum);
  ...
  if (tf_wgt_calc_lvl == 0) {
    for (unsigned int i = 0, k = 0; i < block_height; i++) {
      for (unsigned int j = 0; j < block_width; j++, k++) {
        ...
        const uint32_t diff_sse = acc_5x5_neon[j] + luma_sse_sum[i * BW + j];
