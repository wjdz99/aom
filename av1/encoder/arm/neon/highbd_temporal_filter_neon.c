  ...
  for (uint32_t col = 0; col < block_width; col += 4) {
    ...
    for (unsigned int row = 0; row < block_height; row++) {
      for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < 5; j++) {
          uint32x4_t vmask1 = valign(vzero, vmask, j % 4);
          uint32x4_t vmask2 = valign(vmask, vzero, (j % 4) + 1);
          vsum = vpaddq_u32(vsum, vandq_u32(vsrc[j][0], vmask1));
          vsum = vpaddq_u32(vsum, vandq_u32(vsrc[j][1], vmask2));
        }
...
void av1_highbd_apply_temporal_filter_neon(
  ...
  //  assert(block_size == BLOCK_32X32 && "Only support 32x32 block with
  //  Neon!");
