...
void av1_compute_stats_sve(int wiener_win, const uint8_t *dgd,
  ...
  uint8_t avg = find_average_sve(dgd_start, dgd_stride, width, height);
  int downsample_factor =
  ...
  int dgd_avg_stride = ((width + 2 * wiener_halfwin) & ~7) + 8;
  int src_avg_stride = (width & ~7) + 8;
  ...
  int downsample_height = height / downsample_factor;
