...
void av1_quantize_fp_64x64_neon(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  int16x8_t coeff = load_tran_low_to_s16q(&coeff_ptr[0]);
  int16x8_t abs = vabsq_s16(coeff);
  int16x8x2_t absl = vzipq_s16(abs, zero);
  int16x8x2_t dequantl = vzipq_s16(dequant, zero);
  uint16x8_t check = vcombine_u16(
      vmovn_u32(vcgeq_s32(vshlq_n_s32(vreinterpretq_s32_s16(absl.val[0]), 3),
                          vreinterpretq_s32_s16(dequantl.val[0]))),
      vmovn_u32(vcgeq_s32(vshlq_n_s32(vreinterpretq_s32_s16(absl.val[1]), 3),
                          vreinterpretq_s32_s16(dequantl.val[1]))));
  uint64_t nz_check = vget_lane_u64(vreinterpret_u64_u8(vmovn_u16(check)), 0);
  if (nz_check) {
    const int16x8_t coeff_sign = vshrq_n_s16(coeff, 15);
    const int16x8_t v_iscan = vld1q_s16(&iscan[0]);
  ...
  for (int i = 8; i < n_coeffs; i += 8) {
    coeff = load_tran_low_to_s16q(&coeff_ptr[i]);
    abs = vabsq_s16(coeff);
    absl = vzipq_s16(abs, zero);
    dequantl = vzipq_s16(dequant, zero);
    check = vcombine_u16(
        vmovn_u32(vcgeq_s32(vshlq_n_s32(vreinterpretq_s32_s16(absl.val[0]), 3),
                            vreinterpretq_s32_s16(dequantl.val[0]))),
        vmovn_u32(vcgeq_s32(vshlq_n_s32(vreinterpretq_s32_s16(absl.val[1]), 3),
