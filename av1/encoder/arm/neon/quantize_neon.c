...
void av1_quantize_fp_32x32_neon(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  int16x8_t zero = veorq_s16(zero, zero);
  int16x8_t v_eobmax_76543210 = vceqq_s16(zero, zero);
  i = 0;
  ...
  int16x8_t coeff = load_tran_low_to_s16q(&coeff_ptr[i]);

  int16x8_t abs = vabsq_s16(coeff);
  int16x8_t check = vcgeq_s16(abs, vshrq_n_s16(dequant, 2));
  uint64_t nz_check = vget_lane_u64(vreinterpret_u64_s8(vmovn_s16(check)), 0);
  if (nz_check) {
    ...
    coeff_nz_mask =
        vbslq_s16(check, dqcoeff_temp, load_tran_low_to_s16q(&dqcoeff_ptr[i]));
    ...
    int16x8_t vtmp_mask = vcgtq_s16(abs, zero);
    const int16x8_t v_nz_mask = vandq_s16(vtmp_mask, check);
    check = vandq_s16(v_nz_mask, vcgtq_s16(v_iscan, v_eobmax_76543210));
...
void av1_quantize_fp_64x64_neon(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  int16x8_t abs = vabsq_s16(coeff);
  int16x8_t check =
      vcombine_s16(vmovn_s32(vcgeq_s32(vshlq_n_s32(vzip1q_s16(abs, zero), 3),
                                       vzip1q_s16(dequant, zero))),
                   vmovn_s32(vcgeq_s32(vshlq_n_s32(vzip2q_s16(abs, zero), 3),
                                       vzip2q_s16(dequant, zero))));
  uint64_t nz_check = vget_lane_u64(vreinterpret_u64_s8(vmovn_s16(check)), 0);
  if (nz_check) {
    const int16x8_t coeff_sign = vshrq_n_s16(coeff, 15);
    const int16x8_t v_iscan = vld1q_s16(&iscan[i]);
    ...
    quant = vsetq_lane_s16(quant_ptr[0], quant, 0);
    abs = vqaddq_s16(abs, round);
    int16x8_t temp = vorrq_s16(
        vshlq_n_s16(vqdmulhq_s16(abs, quant), 1),
        vreinterpretq_s16_u16(vshrq_n_u16(vmulq_s16(abs, quant), 14)));
    int16x8_t qcoeff_temp = vsubq_s16(veorq_s16(temp, coeff_sign), coeff_sign);

    abs = vreinterpretq_s16_u16(vshlq_u16(
        vreinterpretq_s16_u16(vmulq_s16(temp, dequant)), v_log_scale));
...
void aom_quantize_b_neon(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  int non_zero_count = (int)n_coeffs, eob = -1;
...

  {
    ...
    int16x8_t vcond = vcgeq_s16(v_abs, vzbins);
    uint64_t nz_check = vget_lane_u64(vreinterpret_u64_s8(vmovn_s16(vcond)), 0);
    if (nz_check) {
      ...
      int16x8_t vtmp_mask = vcgtq_s16(vtmp2, zero);
      const int16x8_t v_nz_mask = vandq_s16(vtmp_mask, vcond);
      int16x8_t v_iscan = vld1q_s16(&iscan[0]);
      vcond = vandq_s16(v_nz_mask, vcgtq_s16(v_iscan, v_eobmax_76543210));
      v_eobmax_76543210 = vbslq_s16(vcond, v_iscan, v_eobmax_76543210);
    }
    vzbins = vsetq_lane_s16(zbins[1], vzbins, 0);
  }

  for (int i = 8; i < non_zero_count; i += 8) {
    int16x8_t v_coeff = load_tran_low_to_s16q(&coeff_ptr[i]);
    int16x8_t v_coeff_sign = vshrq_n_s16(v_coeff, 15);
    int16x8_t v_abs = vabsq_s16(v_coeff);
    int16x8_t vcond = vcgeq_s16(v_abs, vzbins);
    ...
    if (nz_check) {
      int16x8_t vtmp = vqaddq_s16(v_abs, vround);
      uint16x8_t vtmp2 = vsraq_n_s16(vtmp, vqdmulhq_s16(vtmp, vquant), 1);
...
static void aom_quantize_b_helper_32x32_neon(
  ...
  const int16x8_t zero = veorq_s16(zero, zero);
  int16x8_t v_eobmax_76543210 = vceqq_s16(zero, zero);
  ...
  int16x8_t vdequant = vreinterpretq_u16_s16(vdupq_n_s16(dequant_ptr[1]));
  int16x8_t vquant = vdupq_n_s16(quant_ptr[1]);
  int16x8_t vquant_shift = vdupq_n_s16(quant_shift_ptr[1]);
  {
    int16x8_t v_coeff = load_tran_low_to_s16q(&coeff_ptr[0]);
    ...
    uint64_t nz_check = vget_lane_u64(vreinterpret_u64_s8(vmovn_s16(vcond)), 0);
    if (nz_check) {
      vround = vsetq_lane_s16(ROUND_POWER_OF_TWO(round_ptr[0], log_scale),
                              vround, 0);
      vquant = vsetq_lane_s16(quant_ptr[0], vquant, 0);
      vdequant = vsetq_lane_s16(dequant_ptr[0], vdequant, 0);
      vquant_shift = vsetq_lane_s16(quant_shift_ptr[0], vquant_shift, 0);
      ...
      vzbins = vsetq_lane_s16(zbins[1], vzbins, 0);
      vround = vsetq_lane_s16(ROUND_POWER_OF_TWO(round_ptr[1], log_scale),
                              vround, 0);
      vquant = vsetq_lane_s16(quant_ptr[1], vquant, 0);
      vdequant = vsetq_lane_s16(dequant_ptr[1], vdequant, 0);
      vquant_shift = vsetq_lane_s16(quant_shift_ptr[1], vquant_shift, 0);
