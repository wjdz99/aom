...
static INLINE int64_t div_shift_s64(int64_t x, int power) {
  return (x < 0 ? x + (1 << power) - 1 : x) >> power;
}

// The M matrix is accumulated in a bitdepth-depedent number of steps to
...
static INLINE void highbd_compute_stats_win7_neon(
  ...
  do {
    ...
    while (j >= 2) {
      ...
      // The remainining last (49th) elements of `dgd - avg`.
    ...
    if (width & 1) {
      ...
      // The remainining last (49th) element of `dgd - avg`.
      DGD_AVG0[48] = dgd_ptr[6] - avg;
...
static void highbd_compute_stats_win5_neon(const uint16_t *dgd,
  ...
  do {
    ...
    while (j >= 2) {
      ...
      // The remainining last (25th) elements of `dgd - avg`.
    ...
    if (width & 1) {
      ...
      // The remainining last (25th) element of `dgd - avg`.
      DGD_AVG0[24] = dgd_ptr[4] - avg;
      DGD_AVG1[24] = dgd_ptr[5] - avg;
