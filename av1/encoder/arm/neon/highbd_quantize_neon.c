...
static INLINE uint16x4_t quantize_4(
  ...
  const int32x4_t v_abs_coeff_scaled =
      vshlq_n_s32(v_abs_coeff, (1 + log_scale));
...
#endif
  const int64x2_t v_abs_qcoeff_lo_sh = vshrq_n_s64(v_abs_qcoeff_lo, shift);
  const int64x2_t v_abs_qcoeff_hi_sh = vshrq_n_s64(v_abs_qcoeff_hi, shift);
  ...
  const int64x2_t v_abs_dqcoeff_lo_sh =
      vshrq_n_s64(v_abs_dqcoeff_lo, log_scale);
  const int64x2_t v_abs_dqcoeff_hi_sh =
      vshrq_n_s64(v_abs_dqcoeff_hi, log_scale);
...
static INLINE int16x8_t get_max_lane_eob(const int16_t *iscan,
  ...
  const int16x8_t v_nz_iscan =
      vbslq_s16(v_mask, v_iscan_plus1, vdupq_n_s16(0));
...
void av1_highbd_quantize_fp_neon(
  ...
  v_round_s32 = vdupq_laneq_s32(v_round_s32, 1);
  v_quant_s32 = vdupq_laneq_s32(v_quant_s32, 1);
  v_dequant_s32 = vdupq_laneq_s32(v_dequant_s32, 1);
