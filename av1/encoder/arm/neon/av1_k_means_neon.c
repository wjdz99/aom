...
static int32x4_t k_means_horizontal_sum_neon(const int16x8_t a) {
...
void av1_calc_indices_dim1_neon(const int16_t *data, const int16_t *centroids,
  ...
  for (int i = 0; i < n; i += 8) {
    ...
    for (int j = 1; j < k; ++j) {
      ...
      ind = vorrq_u16(vandq_u16(vmvnq_u16(cmp), ind), vandq_u16(cmp, ind1));
    }
    if (total_dist) {
...
#if defined(__aarch64__)
      const int32x4_t sum32_tmp = vmlal_high_s16(l, dist_min, dist_min);
#else
      const int32x4_t h =
          vmull_s16(vget_high_s16(dist_min), vget_high_s16(dist_min));
      const int32x4_t sum32_tmp = vaddq_s32(l, h);
#endif
      // Convert to 64 bit and add to sum.
      const int64x2_t sum_tmp =
          vaddl_s32(vget_low_s32(sum32_tmp), vget_high_s32(sum32_tmp));
      sum = vaddq_s64(sum, sum_tmp);
    }
...
#else
    *total_dist = vadd_s64(vget_low_s64(sum), vget_high_s64(sum));
#endif
  }
}
...
void av1_calc_indices_dim2_neon(const int16_t *data, const int16_t *centroids,
  ...
  for (int i = 0; i < n; i += 8) {
    for (int l = 0; l < 2; ++l) {
      ...
      int32x4_t dist_min = k_means_horizontal_sum_neon(d1);

      for (int j = 1; j < k; ++j) {
        // Compute the distance to the centroid.
        d1 = vsubq_s16(in, cents[j]);
        const int32x4_t dist = k_means_horizontal_sum_neon(d1);
        // Compare to the minimal one.
        const uint32x4_t cmp = vcgtq_s32(dist_min, dist);
        dist_min = vminq_s32(dist_min, dist);
        const uint32x4_t ind1 = vdupq_n_u32(j);
        ind[l] =
            vorrq_u32(vandq_u32(vmvnq_u32(cmp), ind[l]), vandq_u32(cmp, ind1));
      }
      if (total_dist) {
        // Convert to 64 bit and add to sum.
        const int64x2_t sum_tmp =
            vaddl_s32(vget_low_s32(dist_min), vget_high_s32(dist_min));
        sum = vaddq_s64(sum, sum_tmp);
      }
      data += 8;
    }
