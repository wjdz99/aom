...
DECLARE_ALIGNED(16, static const uint16_t, load_pad[4][16]) = {
...
static void apply_temporal_filter(
  ...
  const uint16x8x2_t pad_tbl0 = vld1q_u16_x2(load_pad[0]);
  const uint16x8x2_t pad_tbl1 = vld1q_u16_x2(load_pad[1]);
  const uint16x8x2_t pad_tbl2 = vld1q_u16_x2(load_pad[2]);
  const uint16x8x2_t pad_tbl3 = vld1q_u16_x2(load_pad[3]);
  ...
  // the sliding window (similar to load_pad declared above) using pre-defined
  ...
  // Perform filtering.
  do_filtering(frame, stride, acc_5x5_neon, luma_sse_sum, inv_num_ref_pixels,
               block_height, block_width, subblock_mses, weight_factor,
               inv_factor, decay_factor, d_factor, accumulator, count,
               tf_wgt_calc_lvl, bd);
}
