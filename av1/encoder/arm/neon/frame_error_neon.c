...
const uint16_t static error_measure_lut[256] = {
...
int64_t av1_calc_frame_error_neon(const uint8_t *const ref, int ref_stride,
  ...
  do {
    ...
    do {
      ...
      uint32_t abs0 = vgetq_lane_s32(abs_v, 0);
      uint32_t abs1 = vgetq_lane_s32(abs_v, 1);
      uint32_t abs2 = vgetq_lane_s32(abs_v, 2);
      uint32_t abs3 = vgetq_lane_s32(abs_v, 3);
