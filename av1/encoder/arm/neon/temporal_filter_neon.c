...
DECLARE_ALIGNED(16, static const uint8x16_t, mask[2]) = {
...
static INLINE void get_abs_diff(const uint8_t *frame1, const uint32_t stride1,
  ...
  for (uint32_t i = 0; i < block_height; i++) {
    for (uint32_t j = 0; j < block_width; j += 16) {
...
static void apply_temporal_filter(
  ...
  for (uint32_t col = 0; col < block_width; col += 4) {
    ...
    for (int i = 2; i < 5; i++) {
      ...
      vsrc[i][0] = vandq_u8(s, mask[0]);
      vsrc[i][1] = vandq_u8(s, mask[1]);
      src += SSE_STRIDE;
    }
    ...
    for (unsigned int row = 0; row < block_height; row++) {
      ...
      if (row <= block_height - 4) {
        ...
        uint8x16_t s = load_and_pad(src, col, block_width);
        vsrc[4][0] = vandq_u8(s, mask[0]);
        vsrc[4][1] = vandq_u8(s, mask[1]);
        src += SSE_STRIDE;
      } else {
...
DECLARE_ALIGNED(16, static const uint16x8_t, mask[4]) = {
  ...
  for (uint32_t col = 0; col < block_width; col += 4) {
    ...
    for (unsigned int row = 0; row < block_height; row++) {
      for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < 5; j++) {
          vsum = vpadalq_u16(vsum, vandq_u16(vsrc[j], mask[i]));
        }
