...
void av1_update_inter_mode_stats(FRAME_CONTEXT *fc, FRAME_COUNTS *counts,
  ...
  const int16_t simode_ctx = GLOBALMV_MODE_CONTEXTS * newmv_ctx + zeromv_ctx;
#if CONFIG_ENTROPY_STATS
  ++counts->inter_single_mode[simode_ctx][mode - SINGLE_INTER_MODE_START];
#endif
  update_cdf(fc->inter_single_mode_cdf[simode_ctx],
             mode - SINGLE_INTER_MODE_START, INTER_MODES);
#else
