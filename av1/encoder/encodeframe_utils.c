...
void av1_sum_intra_stats(const AV1_COMMON *const cm, FRAME_COUNTS *counts,
  ...
  if (xd->tree_type != CHROMA_PART) {
    ...
    int primary_flag = mode_idx < FIRST_MODE_COUNT ? 0 : 1;
    if (mode_idx >= FIRST_MODE_COUNT + SECOND_MODE_COUNT) primary_flag++;
    if (mode_idx >= FIRST_MODE_COUNT + SECOND_MODE_COUNT + THIRD_MODE_COUNT)
      primary_flag++;
    if (mode_idx >= FIRST_MODE_COUNT + SECOND_MODE_COUNT + THIRD_MODE_COUNT * 2)
      primary_flag++;
    update_cdf(fc->y_mode_set_cdf[context], primary_flag, INTRA_MODE_SETS);
    if (primary_flag == 0) {
      update_cdf(fc->y_first_mode_cdf[context], mode_idx, FIRST_MODE_COUNT);
    } else if (primary_flag == 1) {
      update_cdf(fc->y_second_mode_cdf[context], mode_idx - FIRST_MODE_COUNT,
                 SECOND_MODE_COUNT);
    } else {
      update_cdf(fc->y_third_mode_cdf[context],
                 mode_idx - FIRST_MODE_COUNT - SECOND_MODE_COUNT -
                     THIRD_MODE_COUNT * (primary_flag - 2),
                 THIRD_MODE_COUNT);
    }
#else
  if (intraonly) {
  ...
  if (xd->tree_type != LUMA_PART) {
    ...
    update_cdf(fc->uv_first_mode_cdf[uv_context], uv_mode_idx, UV_INTRA_MODES);
#else
...
void av1_avg_cdf_symbols(FRAME_CONTEXT *ctx_left, FRAME_CONTEXT *ctx_tr,
  ...
  AVERAGE_CDF(ctx_left->y_mode_set_cdf, ctx_tr->y_mode_set_cdf,
              INTRA_MODE_SETS);
  AVERAGE_CDF(ctx_left->y_first_mode_cdf, ctx_tr->y_first_mode_cdf,
              FIRST_MODE_COUNT);
  AVERAGE_CDF(ctx_left->y_second_mode_cdf, ctx_tr->y_second_mode_cdf,
              SECOND_MODE_COUNT);
  AVERAGE_CDF(ctx_left->y_third_mode_cdf, ctx_tr->y_third_mode_cdf,
              THIRD_MODE_COUNT);

  AVERAGE_CDF(ctx_left->uv_first_mode_cdf, ctx_tr->uv_first_mode_cdf,
              UV_INTRA_MODES);
#else
