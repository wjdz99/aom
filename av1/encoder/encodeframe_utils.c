...
int av1_get_hier_tpl_rdmult(const AV1_COMP *const cpi, MACROBLOCK *const x,
  ...
  const int tpl_idx = cpi->cur_gf_index;
  ...
  if (!is_frame_tpl_eligible(gf_group, cpi->cur_gf_index)) return deltaq_rdmult;
...
int av1_get_rdmult_delta(AV1_COMP *cpi, BLOCK_SIZE bsize, int mi_row,
  ...
  if (!is_frame_tpl_eligible(gf_group, cpi->cur_gf_index)) return orig_rdmult;

  if (cpi->cur_gf_index >= MAX_TPL_FRAME_IDX) return orig_rdmult;
...
int av1_get_q_for_deltaq_objective(AV1_COMP *const cpi, BLOCK_SIZE bsize,
  ...
  if (!is_frame_tpl_eligible(gf_group, cpi->cur_gf_index)) return base_qindex;

  if (cpi->cur_gf_index >= MAX_TPL_FRAME_IDX) return base_qindex;
