...
static int skip_cost_update(const SequenceHeader *seq_params,
                            ...
                            const int mi_col,
                            INTERNAL_COST_UPD_TYPE upd_level) {
  if (upd_level == INT_COST_UPD_SB) return 0;
  if (upd_level == INT_COST_UPD_OFF) return 1;

  // upd_level is at most as frequent as each sb_row in a tile.
  if (mi_col != tile_info->mi_col_start) return 1;

  if (upd_level == INT_COST_UPD_SBROW_SET) {
    ...
    // When upd_level = INT_COST_UPD_SBROW_SET, the cost update happens once
    // for 2, 4 sb rows for sb size 128, sb size 64 respectively. However, as
    // the update will not be equally spaced in smaller resolutions making it
    // equally spaced by calculating (mv_num_rows_cost_update) the number of
