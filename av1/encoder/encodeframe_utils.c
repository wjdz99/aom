...
void av1_source_content_sb(AV1_COMP *cpi, MACROBLOCK *x, int mi_row,
  ...
  // weight based on abs_diff and nb's diff
  if (tmp_variance <= threshold && nmean2 <= 15) {
    // In-place temporal filter
    ...
    uint8_t h = block_size_high[bsize];
    uint8_t w = block_size_wide[bsize];
    const int h_left = cpi->source->heights[0] - (mi_row << 2);
    const int w_left = cpi->source->widths[0] - (mi_col << 2);
    h = h_left < h ? h_left : h;
    w = w_left < w ? w_left : w;

    //printf("\n %d; %d;   %d; %d;   %d;%d;   %d; %d; \n ", cm->mi_params.mi_rows, cm->mi_params.mi_cols, mi_row, mi_col, h,w, cpi->source->widths[0], cpi->source->widths[1]);
