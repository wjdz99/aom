...
void av1_source_content_sb(AV1_COMP *cpi, MACROBLOCK *x, int offset, int mi_row, int mi_col) {
  ...
  // 3 planes, weight based on abs_diff and nb's diff, extend borders
  if (tmp_variance <= threshold && nmean2 <= 10) {
    ...
    for (int plane = 0; plane < av1_num_planes(cm); ++plane) {
      ...
      int src_stride = cpi->source->strides[plane != 0];
      uint8_t *last_src = cpi->last_source->buffers[plane];
      int last_src_stride = cpi->last_source->strides[plane != 0];
      src += src_stride * ((mi_row << 2) >> cpi->source->subsampling_y) + ((mi_col << 2) >> cpi->source->subsampling_x);
      last_src += last_src_stride *  ((mi_row << 2) >> cpi->source->subsampling_y) + ((mi_col << 2) >> cpi->source->subsampling_x);

    for (int i = 0; i < (h >> cpi->source->subsampling_y); ++i) {
      for (int j = 0; j < (w >> cpi->source->subsampling_x); ++j) {
    ...
    //aom_extend_frame_borders(cpi->source, av1_num_planes(cm));   not needed
    }
