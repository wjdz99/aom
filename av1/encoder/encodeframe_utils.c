...
static int is_skip_mv_cost_update(AV1_COMP *cpi,
  ...
  if (cpi->sf.inter_sf.mv_cost_upd_level > 0) {
    ...
    if (cpi->sf.inter_sf.mv_cost_upd_level == 2) {
      ...
      aom_clear_system_state();
      const double tile_height_in_sbs = (double)tile_height / sb_size;
      const int update_freq_sb_rows =
          sb_size_update_freq_map[sb_size != MAX_SB_SIZE];
      const double num_updates_per_tile =
          ceil(tile_height_in_sbs / update_freq_sb_rows);
      const int num_sb_rows_for_update =
          (int)ceil(tile_height_in_sbs / num_updates_per_tile);
      if ((sb_row % num_sb_rows_for_update) != 0) return 1;
    }
  }
  return 0;
}
