...
#define MV_COST_UPDATE_FREQ_SB_ROWS 4
// Checks for skip status of mv cost update.
static int is_skip_mv_cost_update(AV1_COMP *cpi,
  ...
  if (cpi->sf.inter_sf.mv_cost_upd_level > 0) {
    ...
    if (cpi->sf.inter_sf.mv_cost_upd_level == 2) {
      ...
      // When mv_cost_upd_level = 2, the cost update happens once for 4 sb
      // rows. However, as the update will not be equally spaced in smaller
      // resolutions making it equally spaced by calculating
      // (mv_num_rows_cost_update) the number of rows after which the cost
      // update should happen.
      aom_clear_system_state();
      const double num_mv_cost_updates_per_tile =
          ceil((double)tile_height / sb_size / MV_COST_UPDATE_FREQ_SB_ROWS);
      const int num_sb_rows_for_mv_cost_update =
          (int)ceil(tile_height / num_mv_cost_updates_per_tile / sb_size);
      if ((sb_row % num_sb_rows_for_mv_cost_update) != 0) return 1;
    }
  }
  return 0;
}
...
void av1_set_cost_upd_freq(AV1_COMP *cpi, ThreadData *td,
  ...
  switch (cpi->oxcf.cost_upd_freq.mv) {
    ...
    case COST_UPD_SBROW:  // SB row level in tile
      ...
      AOM_FALLTHROUGH_INTENDED;
    case COST_UPD_SB:  // SB level
      // Checks skip status of mv cost update.
      if (is_skip_mv_cost_update(cpi, tile_info, mi_row, mi_col)) break;
