...
static void process_tpl_stats_frame(AV1_COMP *cpi) {
  ...
  assert(IMPLIES(gf_group->size > 0, cpi->cur_gf_index < gf_group->size));
  ...
  if (tpl_frame->is_valid) {
    ...
    if (mc_dep_cost_base == 0) {
      ...
      if (is_frame_tpl_eligible(gf_group, cpi->cur_gf_index)) {
        if (cpi->lap_enabled) {
...
void av1_set_size_dependent_vars(AV1_COMP *cpi, int *q, int *bottom_index,
  ...
  *q = av1_rc_pick_q_and_bounds(cpi, &cpi->rc, cm->width, cm->height,
                                cpi->cur_gf_index, bottom_index, top_index);
...
void av1_dump_filtered_recon_frames(AV1_COMP *cpi) {
  ...
  printf(
      ...
      current_frame->frame_number, cpi->cur_gf_index,
      cpi->gf_group.update_type[cpi->cur_gf_index], current_frame->order_hint,
