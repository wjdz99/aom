...
/* Note the read_token_after and readline helper functions are same
   to what is defined in subgop.c as static functins. This needs to be unified <TB> */
static char *qStep_read_token_after(char *str, const char *delim, char **saveptr) {
...
int process_qStep_config_from_file(const char *paramfile, AV1EncoderConfig *oxcf)
{
    ...
    while (qStep_readline(line, linesize, fp)) {
        if (qStep_read_token_after(line, "qStepMode:", NULL)) {
            ...
            assert(qStepMode == 0 || qStepMode == 1 ||  qStepMode == 2 || qStepMode == 3);
            oxcf->q_cfg.qStep_mode  = qStepMode;
        }

        if(oxcf->q_cfg.qStep_mode == 1) {
            int num_transition_interval_minus1 = 0;
            if (qStep_read_token_after(line, "num_transition_interval_minus1:", NULL)) {
                str = line;
                num_transition_interval_minus1 = atoi(qStep_read_token_after(str, "num_transition_interval_minus1:", &str));
            ...
            if (qStep_read_token_after(line,"num_q_step_periods_minus1:", NULL)) {
                str = line;
                num_qStep_levels_minus1 = atoi(qStep_read_token_after(str, "num_q_step_periods_minus1:", &str));
            ...
            if (qStep_read_token_after(line,"num_table_templates_minus1:", NULL)) {
                str = line;
                num_table_templates_minus1 = atoi(qStep_read_token_after(str, "num_table_templates_minus1:", &str));
            ...
            if (qStep_read_token_after(line, "num_entries_in_table_minus1:", NULL)) {
                int idx=0;
                str = line;
                token = qStep_read_token_after(str, "num_entries_in_table_minus1:", &str);
                char *token1 = strtok(token, ",");
                while(token1){
            ...
            for(int kk = 0; kk <= oxcf->q_cfg.num_table_templates_minus1; kk++) {
                char buffer [100];
                sprintf(buffer,"qsteps_level_period_%d:",kk);
...
void update_qstep_parameters(struct AV1_COMP *cpi,
    ...
    if((seq_params->qStep_mode == 0) || (seq_params->qStep_mode == 1)) {
        ...
        for (int idx = 0; idx <= seq_params->num_qStep_intervals; idx++) {
            seq_params->num_qsteps_in_interval[idx] = oxcf->q_cfg.num_qsteps_in_interval[idx];
        }
        ...
        seq_params->num_table_templates_minus1  = oxcf->q_cfg.num_table_templates_minus1;
        for (int idx = 0; idx <= seq_params->num_table_templates_minus1; idx++) {
            seq_params->num_entries_in_table_minus1[idx]  = oxcf->q_cfg.num_entries_in_table_minus1[idx];
            for (int i = 0; i <= seq_params->num_entries_in_table_minus1[idx]; i++) {
                seq_params->qSteps_level_in_table[idx][i] = oxcf->q_cfg.qSteps_level_in_table[idx][i];
            }
        }
        seq_params->num_qStep_intervals  = oxcf->q_cfg.num_qStep_intervals;
        for (int idx = 0; idx <= seq_params->num_qStep_intervals; idx++) {
            seq_params->template_table_idx[idx] = oxcf->q_cfg.template_table_idx[idx];
            seq_params->table_start_region_idx[idx] = oxcf->q_cfg.table_start_region_idx[idx];
            seq_params->num_qsteps_in_table[idx] = oxcf->q_cfg.num_qsteps_in_table[idx];
        }
    }
}
#endif
