...
void apply_active_map(AV1_COMP *cpi) {
...
void set_size_dependent_vars(AV1_COMP *cpi, int *q, int *bottom_index,
                             int *top_index) {
  AV1_COMMON *const cm = &cpi->common;
...
uint16_t setup_interp_filter_search_mask(AV1_COMP *cpi) {
...
void determine_sc_tools_with_encoding(AV1_COMP *cpi, const int q_orig) {
  AV1_COMMON *const cm = &cpi->common;
  const AV1EncoderConfig *const oxcf = &cpi->oxcf;
...
int recode_loop_test_global_motion(WarpedMotionParams *const global_motion,
...
void finalize_encoded_frame(AV1_COMP *const cpi) {
  AV1_COMMON *const cm = &cpi->common;
...
int is_integer_mv(const YV12_BUFFER_CONFIG *cur_picture,
...
void set_mb_ssim_rdmult_scaling(AV1_COMP *cpi) {
...
void save_all_coding_context(AV1_COMP *cpi) {
  save_cur_buf(cpi);
