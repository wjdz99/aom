 ...
 * either 0/CR_SEGMENT_ID_BASE (no refresh) or to 1/CR_SEGMENT_ID_BOOST1 (refresh)
 * for each superblock, prior to encoding.
 *
 * \ingroup cyclic_refresh
 * \callgraph
 * \callergraph
 ...
 * check if we should reset the segment_id (based on mode/motion/skip selected for
 * that block) and update the cyclic_refresh map and segmentation map.
 *
 * \ingroup cyclic_refresh
 * \callgraph
 * \callergraph
 ...
 * \param[in[   rate      Block rate from pickmode
 * \param[in[   dist      Block dist from pickmode
 * \params[in]  skip      Skip flag set from picmode
 *
