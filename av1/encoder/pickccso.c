...
int best_filter_offset[2][CCSO_LUT_EXT_SIZE] = { 0 };
int final_filter_offset[2][CCSO_LUT_EXT_SIZE] = { 0 };
int chroma_error[CCSO_LUT_EXT_SIZE] = { 0 };
int chroma_count[CCSO_LUT_EXT_SIZE] = { 0 };
...
void derive_blk_md(AV1_COMMON *cm, MACROBLOCKD *xd, uint64_t *unfiltered_dist,
  ...
  const int ccso_nvfb = ((mi_params->mi_rows >> xd->plane[1].subsampling_x) +
                         (1 << CCSO_BLK_SIZE >> 2) - 1) /
                        (1 << CCSO_BLK_SIZE >> 2);
  const int ccso_nhfb = ((mi_params->mi_cols >> xd->plane[1].subsampling_y) +
                         (1 << CCSO_BLK_SIZE >> 2) - 1) /
                        (1 << CCSO_BLK_SIZE >> 2);
...
void compute_total_error(MACROBLOCKD *xd, uint16_t *ext_rec_luma,
  ...
  for (int y = 0; y < pic_height_c; y += (1 << CCSO_BLK_SIZE)) {
    for (int x = 0; x < pic_width_c; x += (1 << CCSO_BLK_SIZE)) {
      ...
      for (int yOff = 0; yOff < y_offset; yOff++) {
        for (int xOff = 0; xOff < x_offset; xOff++) {
          cal_filter_support(
              rec_luma_idx,
              &ext_rec_luma[((ccso_stride_idx[yOff] + x + xOff) << y_uv_scale) +
                            pad_stride],
    ...
    ext_rec_luma += (ccso_stride << (CCSO_BLK_SIZE + 1));
...
void derive_ccso_filter(AV1_COMMON *cm, const int plane, MACROBLOCKD *xd,
  ...
  const CommonModeInfoParams *const mi_params = &cm->mi_params;
  const int ccso_nvfb =
      ((mi_params->mi_rows >> xd->plane[plane].subsampling_x) +
       (1 << CCSO_BLK_SIZE >> 2) - 1) /
      (1 << CCSO_BLK_SIZE >> 2);
  const int ccso_nhfb =
      ((mi_params->mi_cols >> xd->plane[plane].subsampling_y) +
       (1 << CCSO_BLK_SIZE >> 2) - 1) /
      (1 << CCSO_BLK_SIZE >> 2);
  ...
  const uint16_t *org_uv = org_yuv[plane];
  const uint16_t *rec_uv = rec_yuv[plane];
  ...
  int total_filter_support = CCSO_FILTER_SUPPORT_EXT ? 8 : 1;
  ...
  for (int ext_filter_support = 0; ext_filter_support < total_filter_support;
    ...
    for (int quant_idx = 0; quant_idx < total_quant_idx; quant_idx++) {
      ...
      while (keep_training) {
        ...
        if (ccso_enable) {
          ...
          memcpy(temp_rec_yuv_buf, rec_yuv[plane],
                 sizeof(*temp_rec_yuv_buf) * xd->plane[0].dst.height *
                     ccso_stride);
          compute_total_error(xd, ext_rec_yuv[0], org_uv, temp_rec_yuv_buf,
        ...
        apply_ccso_filter_hbd(cm, xd, -1, ext_rec_yuv[0], temp_rec_yuv_buf,
...
void ccso_search(YV12_BUFFER_CONFIG *frame, const YV12_BUFFER_CONFIG *ref,
  ...
  for (int pli = 0; pli < num_planes; pli++) {
    ...
    ext_rec_yuv[pli] =
        aom_malloc(sizeof(*ext_rec_yuv) *
                   ...
                   (ccso_stride + (CCSO_PADDING_SIZE << 1)));
  ...
  derive_ccso_filter(cm, AOM_PLANE_U, xd, org_yuv, ext_rec_yuv, rec_yuv,
                     rdmult);
  derive_ccso_filter(cm, AOM_PLANE_V, xd, org_yuv, ext_rec_yuv, rec_yuv,
                     rdmult);
  for (int pli = 0; pli < num_planes; pli++) {
