  ...
  for (int y_sb = 0; y_sb < ccso_nvfb; y_sb++) {
    for (int x_sb = 0; x_sb < ccso_nhfb; x_sb++) {
      ...
      for (int cur_filter_control = 0; cur_filter_control < 2;
        ...
        const unsigned long long rd_cost = RDCOST(rdmult, rate, ssd * 16);
    ...
    for (int x = 0; x < pic_width_c; x += (1 << CCSO_BLK_SIZE)) {
      ...
      for (int yOff = 0; yOff < y_offset; yOff++) {
        for (int xOff = 0; xOff < x_offset; xOff++) {
          cal_filter_support(
              rec_luma_idx,
              &ext_rec_luma[((ccso_stride_ext_idx[yOff] << y_uv_vert_scale) +
          ...
          chroma_error[(rec_luma_idx[0] << 2) + rec_luma_idx[1]] +=
              ...
              rec_uv_16[ccso_stride_idx[yOff] + x + xOff];
...
void derive_ccso_filter(AV1_COMMON *cm, const int plane, MACROBLOCKD *xd,
  ...
  const unsigned long long best_unfiltered_cost =
  ...
  const int total_filter_support = CCSO_FILTER_SUPPORT_EXT ? 6 : 1;
  const int total_quant_idx = CCSO_QUANT_STEP ? 4 : 1;
    ...
    for (int quant_idx = 0; quant_idx < total_quant_idx; quant_idx++) {
      ...
      while (keep_training) {
        ...
        if (ccso_enable) {
          ...
          const unsigned long long cur_total_cost =
