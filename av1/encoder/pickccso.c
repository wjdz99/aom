    ...
    for (int quant_idx = 0; quant_idx < total_quant_idx; quant_idx++) {
      ...
      while (keep_training) {
        ...
        if (ccso_enable) {
          ...
          memcpy(temp_rec_yuv_buf, rec_uv,
...
void ccso_search(YV12_BUFFER_CONFIG *frame, const YV12_BUFFER_CONFIG *ref,
  ...
  for (int pli = 0; pli < num_planes; pli++) {
    ...
    memset(org_yuv[pli], 0,
           sizeof(*org_yuv) * xd->plane[0].dst.height * ccso_stride);
  ...
  derive_ccso_filter(cm, AOM_PLANE_U, xd, org_yuv[AOM_PLANE_U], ext_rec_y,
                     rec_yuv[AOM_PLANE_U], rdmult);
  derive_ccso_filter(cm, AOM_PLANE_V, xd, org_yuv[AOM_PLANE_V], ext_rec_y,
                     rec_yuv[AOM_PLANE_V], rdmult);
  for (int pli = 0; pli < num_planes; pli++) {
