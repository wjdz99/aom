...
typedef void (*daala_ftx)(od_coeff[], int, const od_coeff *, int, int);

static daala_ftx tx_map[TX_SIZES][TX_TYPES] = {
  //  4-point transforms
  { od_bin_fdct4, od_bin_fdst4,
    od_bin_fdst4_flip, od_bin_fidtx4 },

  //  8-point transforms
  { od_bin_fdct8, od_bin_fdst8,
    od_bin_fdst8_flip, od_bin_fidtx8 },

  //  16-point transforms
  { od_bin_fdct16, od_bin_fdst16,
    od_bin_fdst16_flip, od_bin_fidtx16 },

  //  32-point transforms
  { od_bin_fdct32, od_bin_fdst32,
    od_bin_fdst32_flip, od_bin_fidtx32 },
...
void daala_fwd_txfm(const int16_t *input_pixels,
  ...
  if (txfm_param->lossless) {
    ...
    daala_ftx col_tx = tx_map[col_idx][htx_tab[tx_type]];
    ...
    // Transform columns
    for (c = 0; c < col_n; ++c)
      col_tx(output_coeffs + c, col_n, (od_coeff *)input_pixels,
             input_stride, upshift);

    // Transform rows
    for (r = 0; r < row_n; ++r)
      row_tx(output_coeffs + r*col_n, 1, output_coeffs + r*col_n, 1, 0);

    // The below shifting is temporary while we're testing against existing
    // behavior, which includes a post-shift.
    // Thus it's implemented as a separate, tacked-on operation.
#if 1
    // up-3, down-1 compatability mode with av1_get_tx_scale
