...
void daala_fwd_txfm(const int16_t *input_pixels, tran_low_t *output_coeffs,
  ...
  assert(tx_type <= (int)TX_TYPES_1D);
...
// General TX case
    ...
    for (c = 0; c < col_n; ++c) {
      ...
      for (r = 0; r < row_n; ++r) output_coeffs[r * col_n + c] = tmp[r];
    }

    // Transform rows
    for (r = 0; r < row_n; ++r)
      if (row_flip)
        row_tx(output_coeffs + r * col_n, output_coeffs + (r + 1) * col_n - 1,
               -1);
      else
    ...
    for (r = 0; r < row_n; ++r)
      for (c = 0; c < col_n; ++c)
        output_coeffs[r * col_n + c] =
            (output_coeffs[r * col_n + c] + (1 << (downshift - 1))) >>
            downshift;
#endif
  }
}
