      ...
      for (int j = 0; j < 4; j++) {
        ...
        if (!is_key_frame) {
          ...
          if (vt->split[m].split[i].split[j].part_variances.none.variance >
            ...
            // Check for split to 8x8 if superblock sad is above threshold,
            // and overall frame content is not too large. Only for low-resoln
            // and prefer_large_partition_blocks >= 3.
          } else if (cpi->sf.rt_sf.prefer_large_partition_blocks >= 3 &&
                     low_res && blk_sad > 15000 &&
                     ...
                     vt->split[m]
                             ...
                             .part_variances.none.variance > thresholds[2]
                                                                 << 1) {
            // We have 16x16 variance (based on average) above threshold,
            // so compute the minmax over the 8x8 sub-blocks, and if above
            // threshold, force split to 8x8 block for this 16x16 block.
            // TODO(marpan): Check if 8x8 variance based on 4x4 downsampling
            // is better than minmax for detection moving boundary.
