...
static void setup_planes(AV1_COMP *cpi, MACROBLOCK *x, unsigned int *y_sad,
...
#if CONFIG_NEW_REF_SIGNALING
  if (!cpi->use_svc && (cpi->common.ref_frame_flags_nrs & (1 << golden_frame)) &&
      cpi->sf.rt_sf.use_nonrd_pick_mode) {
    yv12_g = get_ref_frame_yv12_buf_nrs(cm, golden_frame);
    if (yv12_g && yv12_g != yv12) {
      av1_setup_pre_planes(xd, 0, yv12_g, mi_row, mi_col,
                           get_ref_scale_factors_nrs(cm, golden_frame), num_planes,
                           NULL);
  ...
  av1_setup_pre_planes(xd, 0, yv12, mi_row, mi_col,
                       get_ref_scale_factors_nrs(cm, last_frame), num_planes, NULL);
#else
