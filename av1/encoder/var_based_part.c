...
static AOM_INLINE void chroma_check(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  // higher resolutions, which tend to have more camera noise. Since this
  // may be used to skip compound mode in nonrd pickmode, which is generally
  // more effective for higher resolutions, better to be more conservative.
  int fac_uv =
      (cpi->common.width * cpi->common.height >= RESOLUTION_1080P) ? 3 : 5;
