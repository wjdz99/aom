...
static AOM_INLINE void chroma_check(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  int shift = 3;
  // High resolutions have more camera noise, so use lower threshold (more
  // conservative in setting color flag)).
  int fac_uv =
      (cpi->common.width * cpi->common.height >= RESOLUTION_1080P) ? 2 : 4;
