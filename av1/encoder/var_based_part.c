...
// Possible states for the force_split variable while evaluating variance based
// partitioning.
enum {
  // Evaluate all partition types
  EVAL_NONE_RECT_SPLIT = 0,
...
static int set_vt_partitioning(AV1_COMP *cpi, MACROBLOCKD *const xd,
  ...
  if (mi_col + bs_width_check <= tile->mi_col_end &&
      ...
      force_split == CHOOSE_NONE) {
    set_block_size(cpi, mi_row, mi_col, bsize);
    return 1;
  }
  if (force_split == CHOOSE_SPLIT) return 0;
...
static void fill_variance_tree_leaves(
  ...
  for (int m = 0; m < num_64x64_blocks; m++) {
    ...
    const int m2 = m << 2;
    force_split[m + 1] = EVAL_NONE_RECT_SPLIT;

    for (int i = 0; i < 4; i++) {
      const int x32_idx = x64_idx + ((i & 1) << 5);
      const int y32_idx = y64_idx + ((i >> 1) << 5);
      const int i2 = (m2 + i) << 2;
      force_split[5 + m2 + i] = EVAL_NONE_RECT_SPLIT;
      avg_16x16[m][i] = 0;
      maxvar_16x16[m][i] = 0;
      ...
      for (int j = 0; j < 4; j++) {
        ...
        force_split[split_index] = EVAL_NONE_RECT_SPLIT;
        ...
        if (!is_key_frame) {
          ...
          if (vt->split[m].split[i].split[j].part_variances.none.variance >
            ...
            // levels).
            force_split[split_index] = CHOOSE_SPLIT;
            force_split[5 + m2 + i] = CHOOSE_SPLIT;
            force_split[m + 1] = CHOOSE_SPLIT;
            force_split[0] = CHOOSE_SPLIT;
...
static AOM_INLINE PART_EVAL_STATUS
get_part_eval_based_on_sub_blk_var(VP16x16 *const vtemp, int64_t threshold16) {
  int max_8x8_var = 0, min_8x8_var = INT_MAX;
  for (int k = 0; k < 4; k++) {
    get_variance(&vtemp->split[k].part_variances.none);
    int this_8x8_var = vtemp->split[k].part_variances.none.variance;
  ...
  // only PARTITION_NONE.
  return ((max_8x8_var - min_8x8_var) > (threshold16 << 2)) ? CHOOSE_SPLIT
                                                            : CHOOSE_NONE;
}
...
int av1_choose_var_based_partitioning(AV1_COMP *cpi, const TileInfo *const tile,
  ...
  force_split[0] = EVAL_NONE_RECT_SPLIT;
  ...
  for (m = 0; m < num_64x64_blocks; ++m) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      // (64x64) level.
      if (force_split[5 + m2 + i] == EVAL_NONE_RECT_SPLIT) {
    ...
    if (force_split[1 + m] == EVAL_NONE_RECT_SPLIT) {
      ...
      if (!is_key_frame &&
          ...
          (noise_level >= kMedium || cpi->ppi->use_svc ||
           cpi->sf.rt_sf.force_large_partition_blocks)) {
        force_split[1 + m] = CHOOSE_SPLIT;
        force_split[0] = CHOOSE_SPLIT;
      }
      avg_64x64 += var_64x64;
    }
    if (is_small_sb) force_split[0] = CHOOSE_SPLIT;
  }

  if (force_split[0] == EVAL_NONE_RECT_SPLIT) {
