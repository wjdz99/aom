...
static AOM_INLINE void chroma_check(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  for (i = 1; i <= 2; ++i) {
    ...
    if (uv_sad_g > (y_sad_g >> 2))
      x->color_sensitivity_sb_g[i - 1] = 1;
    else
      x->color_sensitivity_sb_g[i - 1] = 0;
  }
}
...
int av1_choose_var_based_partitioning(AV1_COMP *cpi, const TileInfo *const tile,
  ...
  uv_sad[0] = 0;
  uv_sad[1] = 0;
  chroma_check(cpi, x, bsize, y_sad_last, y_sad_g, is_key_frame, zero_motion, uv_sad);
