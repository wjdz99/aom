...
static AOM_INLINE void set_vbp_thresholds(AV1_COMP *cpi, int64_t thresholds[],
  ...
  if (cpi->sf.rt_sf.prefer_large_partition_blocks >= 5) {
    double weight;
    const int win = 20;
  ...
  } else if (cpi->sf.rt_sf.prefer_large_partition_blocks >= 4) {
    thresholds[3] <<= 2;
    thresholds[1] <<= 1;
    thresholds[2] <<= (source_sad_nonrd == kLowSad) ? 2 : 1;
  } else if (cpi->sf.rt_sf.prefer_large_partition_blocks >= 3) {
    thresholds[3] = (3 * thresholds[3]) >> 1;
    thresholds[1] <<= 1;
    thresholds[2] = (source_sad_nonrd == kLowSad) ? (3 * thresholds[2])
                                                  : (thresholds[2] << 1);
  } else if (cpi->sf.rt_sf.prefer_large_partition_blocks >= 2) {
    tune_thresh_based_on_qindex_window(current_qindex, QINDEX_LARGE_BLOCK_THR2,
                                       thresholds);
