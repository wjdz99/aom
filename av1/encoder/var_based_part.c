...
int av1_choose_var_based_partitioning(AV1_COMP *cpi, const TileInfo *const tile,
  ...
  thresh_exit_part = (cm->seq_params->sb_size == BLOCK_64X64) ? 5000 : 10000;
  // If the superblock is completely static (zero source sad) and
  // the y_sad is very small, take sb_size partition and exit, and
  // force zeromv_last skip mode for nonrd_pickmode.
  // Only do this when the cyclic refresh is applied, and only on
  // the base segment (so the qp-boosted segment can still contnue
  // cleaning/ramping up the quality).
