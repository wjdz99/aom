        ...
        if (!is_screen ||
          ...
          *y_sad = av1_int_pro_motion_estimation(
          ...
          if (is_screen) {
            ...
            // Select when motion is either horiz or vert, to capture scroll,
            // and above some threshold.
            int motion_scroll =
                (abs(mi->mv[0].as_mv.col) >= 16 && mi->mv[0].as_mv.row == 0) ||
                (abs(mi->mv[0].as_mv.row) >= 16 && mi->mv[0].as_mv.col == 0);
            if (motion_scroll && *y_sad < (y_sad_zero >> 1) &&
                *y_sad < thresh_sad) {
              x->sb_me_partition = 1;
