...
static AOM_INLINE void evaluate_neighbour_mv(AV1_COMP *cpi, MACROBLOCK *x,
                                             unsigned int *y_sad,
                                             const bool is_small_sb) {
  MACROBLOCKD *xd = &x->e_mbd;
...
static void setup_planes(AV1_COMP *cpi, MACROBLOCK *x, unsigned int *y_sad,
  ...
  if (use_last_ref) {
    ...
    // If previous found MV is non-zero, test zero MV.
    ...
    // already, so only non-zero MVs are tested here.
    if (cpi->sf.rt_sf.estimate_motion_for_var_based_partition == 2 &&
        (xd->up_available || xd->left_available))
      evaluate_neighbour_mv(cpi, x, y_sad, is_small_sb);
