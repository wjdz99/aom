...
static void fill_variance_tree_leaves(
  ...
  // For temporal filtering or temporal denoiser enabled: since the source is
  // modified we need to avoid 4x4 avg along superblock boundary, since
  // simd codec will load 8 pixels for 4x4 avg and so can access source
  // data outside superblock (while its being modified by temporal filter).
#if CONFIG_AV1_TEMPORAL_DENOISING
  temporal_denoising |= cpi->oxcf.noise_sensitivity;
#endif
  if (temporal_denoising && cpi->oxcf.max_threads > 1)
    border_offset_4x4 = 4;
