...
void av1_set_variance_partition_thresholds(AV1_COMP *cpi, int q,
  ...
  if (sf->part_sf.partition_search_type != VAR_BASED_PARTITION) {
    ...
    set_vbp_thresholds(cpi, cpi->vbp_info.vbp_thresholds, q, content_state);
    // The threshold below is not changed locally.
    cpi->vbp_info.vbp_threshold_minmax = 15 + (q >> 3);
  }
}
...
int av1_choose_var_based_partitioning(AV1_COMP *cpi, const TileInfo *const tile,
  ...
  AV1_COMMON *const cm = &cpi->common;
  MACROBLOCKD *xd = &x->e_mbd;
  const int64_t *const vbp_thresholds = cpi->vbp_info.vbp_thresholds;
      ...
      for (j = 0; j < 4; j++) {
        ...
        if (!is_key_frame) {
          ...
          if (vt->split[m].split[i].split[j].part_variances.none.variance >
            ...
            int thresh_minmax = (int)cpi->vbp_info.vbp_threshold_minmax;
