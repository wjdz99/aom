...
static AOM_INLINE void set_vbp_thresholds(AV1_COMP *cpi, int64_t thresholds[],
  ...
  if (is_key_frame) {
    ...
    // Increase partition thresholds for noisy content. Apply only for superblocks
    // where sumdiff is low, as we assume the sumdiff of superblock whose only change
    // is due to noise will be low (i..e, noise will average out over large block).
    if (content_lowsumdiff &&
        cpi->noise_estimate.enabled && (cm->width * cm->height > 640 * 480) &&
        cm->current_frame.frame_number > 100) {
