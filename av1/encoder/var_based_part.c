...
static int set_vt_partitioning(AV1_COMP *cpi, MACROBLOCK *const x,
  ...
  if (0 && cm->seq_params->sb_size == BLOCK_64X64) {
  ...
  if (bsize == bsize_min) {
    ...
    if (mi_col + (block_width >> shw) < tile->mi_col_end &&
        mi_row + (block_height >> shh) < tile->mi_row_end &&
    ...
    if (mi_row + (block_height >> shh) < tile->mi_row_end &&
        mi_col + (block_width >> sh2w) < tile->mi_col_end) {
    ...
    if (mi_col + (block_width >> shw) < tile->mi_col_end &&
        mi_row + (block_height >> sh2h) < tile->mi_row_end) {
