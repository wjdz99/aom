...
static AOM_INLINE void set_vbp_thresholds(AV1_COMP *cpi, int64_t thresholds[],
  ...
  if (cpi->sf.rt_sf.prefer_large_partition_blocks >= 6) {
    double weight;
    const int win = 20;
    ...
    const int ag_idx = cpi->sf.rt_sf.prefer_large_partition_blocks - 4;
    const int fac = (!ag_idx && source_sad_nonrd != kLowSad) ? 1 : 2;
