...
static AOM_INLINE PART_EVAL_STATUS get_part_eval_based_on_sub_blk_var(
    VP16x16 *var_16x16_ptr, int64_t threshold16) {
  int max_8x8_var = 0, min_8x8_var = INT_MAX;
  for (int k = 0; k < 4; k++) {
    get_variance(&var_16x16_ptr->split[k].part_variances.none);
    int this_8x8_var = var_16x16_ptr->split[k].part_variances.none.variance;
