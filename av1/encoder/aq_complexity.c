...
#if CONFIG_EXT_SEGMENT
    memset(cpi->segmentation_map, DEFAULT_AQ2_SEG << MAX_LOG2_ACTIVE_SEGMENTS,
...
// Default segment "Q" feature is disabled so it defaults to the baseline Q.
    ...
    for (segment = 0; segment < AQ_C_SEGMENTS; ++segment) {
      ...
      if ((cm->base_qindex + qindex_delta) > 0) {
        ...
        av1_set_segdata(seg, segment, QUALITY_SEG_LVL_ALT_Q, qindex_delta,
                        QUALITY_SEG_IDX);
#else
  ...
  for (y = 0; y < ymis; y++) {
    for (x = 0; x < xmis; x++) {
      ...
      av1_store_segment_id_into_map(
          segment, &cpi->segmentation_map[mi_offset + y * cm->mi_cols + x],
          QUALITY_SEG_IDX);
#else
