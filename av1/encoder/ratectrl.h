...
void av1_rc_init(const struct VP10EncoderConfig *oxcf, int pass,
                  RATE_CONTROL *rc);
...
void av1_rc_get_one_pass_vbr_params(struct VP10_COMP *cpi);
void av1_rc_get_one_pass_cbr_params(struct VP10_COMP *cpi);

// Post encode update of the rate control parameters based
// on bytes used
void av1_rc_postencode_update(struct VP10_COMP *cpi, uint64_t bytes_used);
// Post encode update of the rate control parameters for dropped frames
void av1_rc_postencode_update_drop_frame(struct VP10_COMP *cpi);

// Updates rate correction factors
// Changes only the rate correction factors in the rate control structure.
void av1_rc_update_rate_correction_factors(struct VP10_COMP *cpi);

// Decide if we should drop this frame: For 1-pass CBR.
// Changes only the decimation count in the rate control structure
int av1_rc_drop_frame(struct VP10_COMP *cpi);

// Computes frame size bounds.
void av1_rc_compute_frame_size_bounds(const struct VP10_COMP *cpi,
                                       int this_frame_target,
                                       int *frame_under_shoot_limit,
                                       int *frame_over_shoot_limit);

// Picks q and q bounds given the target for bits
int av1_rc_pick_q_and_bounds(const struct VP10_COMP *cpi, int *bottom_index,
                              int *top_index);

// Estimates q to achieve a target bits per frame
int av1_rc_regulate_q(const struct VP10_COMP *cpi, int target_bits_per_frame,
...
int av1_rc_clamp_iframe_target_size(const struct VP10_COMP *const cpi,
                                     int target);
int av1_rc_clamp_pframe_target_size(const struct VP10_COMP *const cpi,
                                     int target);
// Utility to set frame_target into the RATE_CONTROL structure
// This function is called only from the av1_rc_get_..._params() functions.
void av1_rc_set_frame_target(struct VP10_COMP *cpi, int target);
...
int av1_frame_type_qdelta(const struct VP10_COMP *cpi, int rf_level, int q);

void av1_rc_update_framerate(struct VP10_COMP *cpi);

void av1_rc_set_gf_interval_range(const struct VP10_COMP *const cpi,
                                   RATE_CONTROL *const rc);

void av1_set_target_rate(struct VP10_COMP *cpi);

int av1_resize_one_pass_cbr(struct VP10_COMP *cpi);
