...
static int cfl_rd_pick_alpha(MACROBLOCK *const x, const AV1_COMP *const cpi,
  ...
  // Evaluate alpha parameter of each plane.
  ...
  if (est_best_cfl_idx[0] == CFL_INDEX_ZERO &&
    ...
    // Set invalid CfL parameters here and return since cfl index is zero for
    // both the planes which implies CfL mode is not applicable.
  ...
  // Compute the rd cost of each plane using the evaluated alpha parameters.
