...
int av1_search_palette_mode(IntraModeSearchState *intra_search_state,
  ...
  av1_rd_pick_palette_intra_sby(
      (AV1_COMP *)cpi, x, bsize, intra_mode_cost[DC_PRED], &best_mbmi_palette,
      best_palette_color_map, &best_rd_palette, &rd_stats_y.rate, NULL,
      &rd_stats_y.dist, &rd_stats_y.skip_txfm, NULL, ctx, best_blk_skip,
      best_tx_type_map);
...
int64_t av1_rd_pick_intra_sby_mode(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  if (try_palette) {
    av1_rd_pick_palette_intra_sby(
        (AV1_COMP *)cpi, x, bsize, bmode_costs[DC_PRED], &best_mbmi,
        best_palette_color_map, &best_rd, rate, rate_tokenonly, distortion,
        skippable, &beat_best_rd, ctx, ctx->blk_skip, ctx->tx_type_map);
  }
