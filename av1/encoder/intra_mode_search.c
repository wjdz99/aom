...
int prune_intra_y_mode(int64_t this_model_rd, int64_t *best_model_rd,
  ...
  for (int i = 0; i < top_intra_model_count_allowed; i++) {
    if (this_model_rd < top_intra_model_rd[i]) {
      for (int j = top_intra_model_count_allowed - 1; j > i; j--) {
  ...
  if (top_intra_model_rd[top_intra_model_count_allowed - 1] != INT64_MAX &&
      this_model_rd >
          thresh_top * top_intra_model_rd[top_intra_model_count_allowed - 1])
    return 1;
