...
// Computes mapped variance for source and reconstructed blocks.
static void compute_mapped_var(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  for (int i = 0; i < bh; i += MI_SIZE) {
    ...
    for (int j = 0; j < bw; j += MI_SIZE) {
      ...
      Blk4x4VarInfo *blk_4x4_var_info =
          &x->src_var_info_of_4x4_sub_blks[mi_offset];
      int src_var = blk_4x4_var_info->var;
      double log_src_var = blk_4x4_var_info->log_var;
      ...
      // log(1+var), if the values present in src_var_info_of_4x4_sub_blks are
      // invalid. Reuse the same if it is readily available with valid values.
      if (src_var < 0) {
        // Compute and store source variance and log(1 + src_var).
        src_var = av1_calc_normalized_variance(
            cpi->ppi->fn_ptr[BLOCK_4X4].vf,
            x->plane[0].src.buf + i * x->plane[0].src.stride + j,
            x->plane[0].src.stride, is_hbd);
        blk_4x4_var_info->var = src_var;
        log_src_var = log(1.0 + src_var / 16.0);
        blk_4x4_var_info->log_var = log_src_var;
      } else {
        // When source variance is already calculated and available for
        // retrieval, check if log(1+var) is also available. If it is available
        // then, retrieve from buffer. Else, calculate the same and store to the
        // buffer.
        if (log_src_var < 0) {
          log_src_var = log(1.0 + src_var / 16.0);
          blk_4x4_var_info->log_var = log_src_var;
