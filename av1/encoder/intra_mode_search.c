...
// Update the intra model yrd and prune the current iimode if the new estimate
...
{//cfl_rd_pick_alpha ->txfm_rd_in_plane ->av1_foreach_transformed_block_in_plane 
 // ->block_rd_txfm ->av1_predict_intra_block_facade ->cfl_predict_block
  MACROBLOCKD *const xd = &x->e_mbd;
  MB_MODE_INFO *const mbmi = xd->mi[0];
  const MACROBLOCKD_PLANE *pd = &xd->plane[AOM_PLANE_U];
  const BLOCK_SIZE plane_bsize = get_plane_block_size(mbmi->sb_type, pd->subsampling_x, pd->subsampling_y);
  ...
  xd->cfl.use_search_res_cache = 1;
#else
  xd->cfl.use_dc_pred_cache = 1;
#endif

  const int64_t mode_rd = RDCOST(x->rdmult,
  ...
  for (int plane = 0; plane < CFL_PRED_PLANES; plane++) 
  {
    ...
    {//1、2
      const int8_t joint_sign = PLANE_SIGN_TO_JOINT_SIGN(plane, CFL_SIGN_ZERO, i);//当前符号为0
      if (i == CFL_SIGN_NEG) 
      {
      ...
      best_rd_uv[joint_sign][plane] = RDCOST(x->rdmult, rd_stats.rate + alpha_rate, rd_stats.dist);
#if CONFIG_DEBUG
      ...
      for (int c = 0; c < CFL_ALPHABET_SIZE; c++) 
      {
        ...
        for (int i = 0; i < CFL_SIGNS; i++) 
        {
          ...
          if (i == 0) //另一个分量符号为0的情况
          {
            ...
            mbmi->cfl_alpha_signs = joint_sign;
            av1_txfm_rd_in_plane( x, cpi, &rd_stats, best_rd, 0, plane + 1, plane_bsize, tx_size,
                cpi->sf.rd_sf.use_fast_coef_costing, FTXS_NONE, skip_trellis);
            if (rd_stats.rate == INT_MAX) break;
          }
          const int alpha_rate = x->cfl_cost[joint_sign][plane][c];
          int64_t this_rd = RDCOST(x->rdmult,  rd_stats.rate + alpha_rate, rd_stats.dist);
...
#if CONFIG_CFL_SEARCH_VERSION_1
  xd->cfl.use_search_res_cache = 0;
  xd->cfl.search_res_is_cached[0] = 0;
  xd->cfl.search_res_is_cached[1] = 0;
#else
...
int64_t av1_rd_pick_intra_sby_mode(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  if (cpi->sf.winner_mode_sf.enable_multiwinner_mode_process) {
    ...
    if (best_mbmi.palette_mode_info.palette_size[0] > 0) 
    {
      ...
      memcpy(color_map_dst, color_map_src,block_width * block_height * sizeof(*color_map_src));
    }
  } 
  else 
  {
