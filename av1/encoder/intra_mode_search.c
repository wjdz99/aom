...
int get_uv_mode_cost(MB_MODE_INFO *mbmi, const ModeCosts mode_costs) {
  ...
  return mode_costs.uv_first_mode_costs[uv_context][mbmi->uv_mode_idx];
}
#endif  // CONFIG_INTRA_MODE_CODING
...
int64_t av1_rd_pick_intra_sbuv_mode(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  for (int mode_idx = 0; mode_idx < UV_INTRA_MODES; ++mode_idx) {
    ...
    mbmi->joint_uv_mode = mbmi->uv_intra_mode_list[mode_idx];
    set_uv_mode_and_delta_angle(mbmi);
...
int64_t av1_handle_intra_mode(IntraModeSearchState *intra_search_state,
  ...
  int primary_flag = mbmi->mode_idx < FIRST_MODE_COUNT ? 0 : 1;
  if (mbmi->mode_idx >= FIRST_MODE_COUNT + SECOND_MODE_COUNT) primary_flag++;
  if (mbmi->mode_idx >= FIRST_MODE_COUNT + SECOND_MODE_COUNT + THIRD_MODE_COUNT)
    primary_flag++;
  if (mbmi->mode_idx >=
      FIRST_MODE_COUNT + SECOND_MODE_COUNT + THIRD_MODE_COUNT * 2)
    primary_flag++;
  mode_cost += x->mode_costs.y_primary_flag_cost[context][primary_flag];
  if (mbmi->mode_idx < FIRST_MODE_COUNT) {
    mode_cost += x->mode_costs.y_first_mode_costs[context][mbmi->mode_idx];
  } else if (mbmi->mode_idx < FIRST_MODE_COUNT + SECOND_MODE_COUNT) {
    mode_cost +=
        x->mode_costs
            .y_second_mode_costs[context][mbmi->mode_idx - FIRST_MODE_COUNT];
  } else {
    mode_cost +=
        x->mode_costs
            .y_third_mode_costs[context][mbmi->mode_idx - FIRST_MODE_COUNT -
                                         SECOND_MODE_COUNT -
  ...
  if (num_planes > 1) {
    ...
    if (intra_search_state->rate_uv_intra == INT_MAX) {
      ...
      intra_search_state->joint_uv_mode = mbmi->joint_uv_mode;
#endif
    ...
    mbmi->joint_uv_mode = intra_search_state->joint_uv_mode;
#endif
    if (try_palette) {
      pmi->palette_size[1] = intra_search_state->pmi_uv.palette_size[1];
      memcpy(pmi->palette_colors + PALETTE_MAX_SIZE,
...
int64_t av1_rd_pick_intra_sby_mode(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  for (int mrl_idx = 0; mrl_idx < (enable_mrls_flag ? MRL_LINE_NUMBER : 1);
    ...
    for (int mode_idx = INTRA_MODE_START; mode_idx < LUMA_MODE_COUNT;
      ...
      mode_costs = 0;
      int primary_flag = mbmi->mode_idx < FIRST_MODE_COUNT ? 0 : 1;
      if (mbmi->mode_idx >= FIRST_MODE_COUNT + SECOND_MODE_COUNT)
        primary_flag++;
      if (mbmi->mode_idx >=
          FIRST_MODE_COUNT + SECOND_MODE_COUNT + THIRD_MODE_COUNT)
        primary_flag++;
      if (mbmi->mode_idx >=
          FIRST_MODE_COUNT + SECOND_MODE_COUNT + THIRD_MODE_COUNT * 2)
        primary_flag++;
      mode_costs += x->mode_costs.y_primary_flag_cost[context][primary_flag];
      if (mode_idx < FIRST_MODE_COUNT) {
        mode_costs += x->mode_costs.y_first_mode_costs[context][mode_idx];
      } else if (mbmi->mode_idx < FIRST_MODE_COUNT + SECOND_MODE_COUNT) {
        mode_costs +=
            x->mode_costs.y_second_mode_costs[context][mbmi->mode_idx -
                                                       FIRST_MODE_COUNT];
      } else {
        mode_costs +=
            x->mode_costs
                .y_third_mode_costs[context]
  ...
  // Searches palette
#if CONFIG_INTRA_MODE_CODING
  mode_costs = x->mode_costs.y_primary_flag_cost[context][DC_PRED];
