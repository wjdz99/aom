...
static double intra_rd_variance_factor(const AV1_COMP *cpi, MACROBLOCK *x,
  ...
  // For non-positive threshold values, the comparison of mapped source and
  // reconstructed variances with threshold evaluates to false
  // (mapped_src_var < threshold/mapped_rec_var < threshold) as these metrics
  // are greater than 0. Hence further calculations are skipped.
