...
static void cfl_idx_to_sign_and_alpha(int cfl_idx, CFL_SIGN_TYPE *cfl_sign,
  ...
  if (cfl_linear_idx == 0) {
    ...
    *cfl_alpha = 0;
  } else if (cfl_linear_idx > 0) {
    *cfl_sign = CFL_SIGN_POS;
    *cfl_alpha = abs(cfl_linear_idx) - 1;
  } else {
    *cfl_sign = CFL_SIGN_NEG;
    *cfl_alpha = abs(cfl_linear_idx) - 1;
  }
}

int64_t cfl_compute_rd(const AV1_COMP *const cpi, MACROBLOCK *x, int plane,
                       TX_SIZE tx_size, BLOCK_SIZE plane_bsize, int cfl_idx,
  ...
  if (fast_mode) {
    int use_hardamard = 0;
    int64_t cfl_cost =
        intra_model_rd(cm, x, plane, plane_bsize, tx_size, use_hardamard);
    return cfl_cost;
  } else {
...
void cfl_pick_plane_parameter(const AV1_COMP *const cpi, MACROBLOCK *x,
                              int plane, TX_SIZE tx_size, int cfl_search_range,
                              int *cfl_rate_arr, int64_t *cfl_dist_arr) {
  MACROBLOCKD *const xd = &x->e_mbd;
  ...
  if (cfl_search_range < CFL_INDEX_MAX) {
    ...
    for (int si = 0; si < 2; ++si) {
      ...
      for (int i = 1; i < CFL_INDEX_MAX; ++i) {
        ...
        int cfl_cost = cfl_compute_rd(cpi, x, plane, tx_size, plane_bsize,
  ...
  RD_STATS rd_stats;
  int64_t best_cfl_cost = cfl_compute_rd(cpi, x, plane, tx_size, plane_bsize,
                                         start_cfl_idx, fast_mode, &rd_stats);
...
static int cfl_rd_pick_alpha(MACROBLOCK *const x, const AV1_COMP *const cpi,
  ...
  for (int ui = 0; ui < CFL_INDEX_MAX; ++ui) {
    for (int vi = 0; vi < CFL_INDEX_MAX; ++vi) {
      ...
      int cfl_alpha_idx = (cfl_alpha_u << CFL_ALPHABET_SIZE_LOG2) + cfl_alpha_v;
  ...
  if (best_rd < ref_best_rd) {
    ...
    mbmi->cfl_alpha_idx = best_alpha_idx;
...
int64_t av1_rd_pick_intra_sbuv_mode(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  for (int mode_idx = 0; mode_idx < UV_INTRA_MODES; ++mode_idx) {
    ...
    if (mode == UV_CFL_PRED) {
      ...
      int cfl_search_range = CFL_INDEX_MAX;
      cfl_alpha_rate = cfl_rd_pick_alpha(x, cpi, uv_tx_size, best_rd,
                                         cfl_search_range, &cfl_token_rate);
    ...
    if (mode == UV_CFL_PRED) {
      ...
      if (!xd->lossless[mbmi->segment_id]) {
        assert(cfl_token_rate == tokenonly_rd_stats.rate);
      }
#endif  // CONFIG_DEBUG
    }
    this_rd = RDCOST(x->rdmult, this_rate, tokenonly_rd_stats.dist);
