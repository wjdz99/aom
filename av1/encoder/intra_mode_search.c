 ...
 */
static void cfl_rd_pick_alpha(MACROBLOCK *const x, const AV1_COMP *const cpi,
  ...
  best_rd_stats->rdcost = INT64_MAX;
  for (int ui = 0; ui < CFL_MAGS_SIZE; ++ui) {
    ...
    for (int vi = 0; vi < CFL_MAGS_SIZE; ++vi) {
      ...
      int rate = cfl_rate_arr_u[ui] + cfl_rate_arr_v[vi];
      rate += mode_costs->cfl_cost[joint_sign][CFL_PRED_U][cfl_alpha_u];
      rate += mode_costs->cfl_cost[joint_sign][CFL_PRED_V][cfl_alpha_v];
      int64_t this_rd = RDCOST(x->rdmult, rate, dist);
      if (this_rd < best_rd_stats->rdcost) {
        best_rd_stats->rate = rate;
        best_rd_stats->dist = dist;
        best_rd_stats->rdcost = this_rd;
        *best_cfl_alpha_idx =
...
int64_t av1_rd_pick_intra_sbuv_mode(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  for (int mode_idx = 0; mode_idx < UV_INTRA_MODES; ++mode_idx) {
    ...
    if (mode == UV_CFL_PRED) {
      ...
      cfl_rd_pick_alpha(x, cpi, uv_tx_size, best_rd,
