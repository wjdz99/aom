...
void RENAME(av1_calc_indices2)(const int *data, const int *centroids,
                               uint8_t *indices, int n, int k) {
  int i, j;
  for (i = 0; i < n; ++i) {
...
static int64_t RENAME(calc_total_dist2)(const int *data, const int *centroids,
  ...
  //return (dist >= INT_MAX) ? INT_MAX : (int)dist;
  return dist;
}

void RENAME(av1_k_means2)(const int *data, int *centroids, uint8_t *indices,
                          int n, int k, int max_itr) {
  int i;
  int64_t this_dist;
  ...
  RENAME(av1_calc_indices2)(data, centroids, indices, n, k);
