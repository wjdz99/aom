...
static INLINE int is_mv_match_with_global_mv(
    const MB_MODE_INFO_EXT *mbmi_ext, int ref_idx, int ref_mv_idx,
    const uint8_t ref_frame_type, const MV_REFERENCE_FRAME *ref_frame) {
  int_mv mv;

  if (ref_idx == 0)
    mv = mbmi_ext->ref_mv_stack[ref_frame_type][ref_mv_idx].this_mv;
  else
    mv = mbmi_ext->ref_mv_stack[ref_frame_type][ref_mv_idx].comp_mv;

  if (mv.as_int == mbmi_ext->global_mvs[ref_frame[ref_idx]].as_int) return 1;

  return 0;
}

static INLINE int get_this_mv(int_mv *this_mv, PREDICTION_MODE this_mode,
                              int ref_idx, int ref_mv_idx, int skip_repeat_mv,
  ...
  if (single_mode == NEWMV) {
    ...
    if (skip_repeat_mv) {
      ...
      int ref_mv_count = AOMMIN(USABLE_REF_MV_STACK_SIZE,
                                mbmi_ext->ref_mv_count[ref_frame_type]);
      for (int mv_idx = 0; mv_idx < ref_mv_count; mv_idx++) {
        is_repeat_mv = is_mv_match_with_global_mv(mbmi_ext, ref_idx, mv_idx,
                                                  ref_frame_type, ref_frame);
        if (is_repeat_mv) return 0;
      }
    }
    ...
    if (ref_mv_offset < mbmi_ext->ref_mv_count[ref_frame_type]) {
      ...
      if (skip_repeat_mv) {
        ...
        if (!mbmi_ext->ref_mv_count[ref_frame_type])
          ...
          is_repeat_mv = is_mv_match_with_global_mv(mbmi_ext, ref_idx, 0,
                                                    ref_frame_type, ref_frame);
        if (is_repeat_mv) return 0;
      }
      *this_mv = mbmi_ext->global_mvs[ref_frame[ref_idx]];
...
static INLINE int build_cur_mv(int_mv *cur_mv, PREDICTION_MODE this_mode,
  ...
  for (int i = 0; i < is_comp_pred + 1; ++i) {
    int_mv this_mv;
    this_mv.as_int = INVALID_MV;
    ret = get_this_mv(&this_mv, this_mode, i, mbmi->ref_mv_idx, skip_repeat_mv,
...
static int64_t handle_inter_mode(AV1_COMP *const cpi, TileDataEnc *tile_data,
  ...
  for (int ref_mv_idx = 0; ref_mv_idx < ref_set; ++ref_mv_idx) {
    ...
    if (!build_cur_mv(cur_mv, this_mode, cm, x,
                      cpi->sf.inter_sf.skip_repeated_non_newmv)) {
      continue;
    }
