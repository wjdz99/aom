...
static INLINE int check_repeat_ref_mv(const MB_MODE_INFO_EXT *mbmi_ext,
  ...
  if (single_mode == NEARESTMV) {
    ...
    if (mbmi_ext->ref_mv_count[ref_frame_type] < 1) return 1;
  } else if (single_mode == GLOBALMV) {
    if (mbmi_ext->ref_mv_count[ref_frame_type] <= 1) return 1;
  }

  int ref_mv_count =
      AOMMIN(USABLE_REF_MV_STACK_SIZE, mbmi_ext->ref_mv_count[ref_frame_type]);
  // Check GLOBALMV match with NEARESTMV or NEARMVs
