...
// Check ref frame of current block matches with given block.
static INLINE void match_ref_frame(const MB_MODE_INFO *mbmi,
                                   const MV_REFERENCE_FRAME *ref_frames,
                                   int *is_ref_match) {
  if (is_inter_block(mbmi)) {
    is_ref_match[0] |= ref_frames[0] == mbmi->ref_frame[0];
    is_ref_match[0] |= ref_frames[1] == mbmi->ref_frame[0];
    if (has_second_ref(mbmi)) {
      is_ref_match[1] |= ref_frames[0] == mbmi->ref_frame[1];
      is_ref_match[1] |= ref_frames[1] == mbmi->ref_frame[1];
    }
  }
}

// Prune compound mode using ref frame of neighbor blocks.
static int compound_skip_using_neighbor_refs(
  ...
  // Check ref frame of this block matches with left neighbor.
  if (xd->left_available)
    match_ref_frame(xd->left_mbmi, ref_frames, is_ref_match);

  // Check ref frame of this block matches with above neighbor.
  if (xd->up_available)
