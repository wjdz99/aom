  ...
  for (THR_MODES midx = THR_MODE_START; midx < THR_MODE_END; ++midx) {
    ...
    const MV_REFERENCE_FRAME second_ref_frame = ref_frames[1];
        ...
        assert(convert_named_ref_to_ranked_ref_index(&cm->new_ref_frame_data,
               ref_frame) == mbmi->ref_frame_nrs[0]);
...
#if CONFIG_NEW_REF_SIGNALING
  const unsigned int intra_ref_frame_cost = ref_costs_single[INTRA_FRAME_NRS];
#else
...
#if CONFIG_NEW_REF_SIGNALING
  int last_index = convert_named_ref_to_ranked_ref_index(&cm->new_ref_frame_data, LAST_FRAME);
