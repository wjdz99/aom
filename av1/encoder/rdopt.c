...
/*static const int scale_factor_txfm_gate[QINDEX_RANGE] = {
  1024, 1019, 1013, 1008, 1003, 997, 992, 987, 981, 976, 971, 965, 960, 955,
  949,  944,  939,  933,  928,  923, 917, 912, 907, 901, 896, 891, 885, 880,
  875,  869,  864,  859,  853,  848, 843, 837, 832, 827, 821, 816, 811, 805,
  800,  795,  789,  784,  779,  773, 768, 763, 757, 752, 747, 741, 736, 731,
  725,  720,  715,  709,  704,  699, 693, 688, 683, 677, 672, 667, 661, 656,
  651,  645,  640,  635,  629,  624, 619, 613, 608, 603, 597, 592, 587, 581,
  576,  571,  565,  560,  555,  549, 544, 539, 533, 528, 523, 517, 512, 507,
  501,  496,  491,  485,  480,  475, 469, 464, 459, 453, 448, 443, 437, 432,
  427,  421,  416,  411,  405,  400, 395, 389, 384, 379, 373, 368, 363, 357,
  352,  347,  341,  336,  331,  325, 320, 315, 309, 304, 299, 293, 288, 283,
  277,  272,  267,  261,  256,  251, 245, 240, 235, 229, 224, 219, 213, 208,
  203,  197,  192,  187,  181,  176, 171, 165, 160, 155, 149, 144, 139, 133,
  128,  123,  117,  112,  107,  101, 96,  91,  85,  80,  75,  69,  64,  64,
...
static INLINE int check_txfm_eval(const AV1_COMP *cpi, MACROBLOCK *const x,
                                  BLOCK_SIZE bsize, int64_t best_skip_rd,
                                  int64_t sse_cost) {
  int eval_txfm = 1;
  int64_t sse_thresh = best_skip_rd;
  /*sse_thresh = (sse_thresh == INT64_MAX) ? sse_thresh :
          ((sse_thresh * scale_factor_txfm_gate[x->qindex]) >> 6);*/
  const int max_qs =
      av1_dc_quant_QTX(MAXQ, 0, cpi->common.seq_params.bit_depth) >> 3;
  int64_t max_qs_sq = (max_qs * max_qs);
  // Obtain the normalized Qscale
  const int qs =
      av1_dc_quant_QTX(x->qindex, 0, cpi->common.seq_params.bit_depth) >> 3;
  int64_t var_thresh = max_qs_sq / (qs * qs);
  double scale = 1.0;
  if (x->source_variance && x->source_variance < var_thresh)
    scale = 2.0;  // var_thresh / x->source_variance;
  else if (!x->source_variance)
    scale = 10.0;
  else if (best_skip_rd > (x->source_variance
                           << (num_pels_log2_lookup[bsize] + RDDIV_BITS + 1)))
    scale = 2.0;
  sse_thresh = (best_skip_rd == INT64_MAX) ? best_skip_rd
                                           : (int64_t)(best_skip_rd * scale);
  if (sse_cost > sse_thresh) eval_txfm = 0;
  return eval_txfm;
}
    ...
    *mbmi = base_mbmi;
    ...
    if (!do_tx_search) {
      ...
      *best_est_rd = AOMMIN(*best_est_rd, rd_stats->rdcost);
      ...
      int eval_txfm = 1;
      int64_t sse_cost = INT64_MAX;
      if (cpi->sf.inter_sf.txfm_gate_level) {
        int64_t curr_sse = get_sse(cpi, x);
        sse_cost = RDCOST(x->rdmult, rd_stats->rate, curr_sse);
        eval_txfm = check_txfm_eval(cpi, x, bsize, *ref_skip_rd, sse_cost);
      }
      if (eval_txfm) {
    ...
    for (int j = 0; j < inter_modes_info->num; ++j) {
      ...
      *mbmi = inter_modes_info->mbmi_arr[data_idx];
      ...
      if (cpi->sf.inter_sf.txfm_gate_level) {
        ...
        sse_cost = RDCOST(x->rdmult, mode_rate, curr_sse);
        eval_txfm =
            check_txfm_eval(cpi, x, bsize, search_state.best_skip_rd, sse_cost);
      }
      if (eval_txfm) {
        if (!av1_txfm_search(cpi, tile_data, x, bsize, &rd_stats, &rd_stats_y,
        ...
        if (rd_stats.rdcost < search_state.best_rd) {
          update_search_state(&search_state, rd_cost, ctx, &rd_stats,
                              ...
                              txfm_search_done);
          search_state.best_skip_rd = sse_cost;
        }
      }
    }
  }
