...
void av1_rd_pick_inter_mode(struct AV1_COMP *cpi, struct TileDataEnc *tile_data,
  ...
  for (THR_MODES midx = mode_start; midx < mode_end; ++midx) {
    ...
    if (is_low_temp_var && !comp_pred && ref_frame != LAST_FRAME) {
      const int above_use_last =
          (xd->above_mbmi != NULL) ? xd->above_mbmi->ref_frame[0] == LAST_FRAME
                                   : 0;
      const int left_use_last = (xd->left_mbmi != NULL)
                                    ? xd->left_mbmi->ref_frame[0] == LAST_FRAME
                                    : 0;
      if (above_use_last && left_use_last) continue;
    }
