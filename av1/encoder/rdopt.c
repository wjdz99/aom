...
static INLINE int check_txfm_eval(const AV1_COMP *cpi, MACROBLOCK *const x,
                                  ...
                                  int64_t skip_rd) {
  int eval_txfm = 1;
  // Derive aggressiveness factor for bypassing the transform search
  // Be more conservative for (i) low quantizers (ii) regions where prediction
  // is poor
  int aggr_factor =
    ...
    *mbmi = base_mbmi;
    ...
    if (!do_tx_search) {
      ...
      *best_est_rd = AOMMIN(*best_est_rd, rd_stats->rdcost);
      ...
      if (cpi->sf.inter_sf.enable_txfm_gate) {
    ...
    for (int j = 0; j < inter_modes_info->num; ++j) {
      ...
      *mbmi = inter_modes_info->mbmi_arr[data_idx];
      ...
      if (cpi->sf.inter_sf.enable_txfm_gate) {
        ...
        eval_txfm =
            check_txfm_eval(cpi, x, bsize, search_state.best_skip_rd, skip_rd);
      }
      if (eval_txfm) {
        if (!av1_txfm_search(cpi, tile_data, x, bsize, &rd_stats, &rd_stats_y,
