...
// This is only called if the original mode search only estimated RD 
// for the transform search should be computed.
static void tx_search_best_inter_cands(AV1_COMP *cpi, TileDataEnc *tile_data, 
                MACROBLOCK *x, int64_t best_rd_so_far, 
                BLOCK_SIZE bsize,
                struct buf_2d yv12_mb[REF_FRAMES][MAX_MB_PLANE],
...
void av1_rd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  if (do_tx_search != 1) {
    ...
    // top mode candidates
    tx_search_best_inter_cands(cpi, tile_data, x, best_rd_so_far, 
                               bsize, yv12_mb, mi_row, mi_col, &search_state, 
                               rd_cost, ctx);
  }
#if CONFIG_COLLECT_COMPONENT_TIMING
