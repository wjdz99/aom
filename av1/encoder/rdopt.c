...
static void tx_search_best_inter_candidates(
  ...
  *yrd = INT64_MAX;
  ...
  if (cpi->sf.inter_sf.limit_inter_mode_cands) {
    // The maximum allowed inter mode candidates if a newmv mode gets evaluated
    // is limited to max_allowed_cands + 1.
  ...
  for (int j = 0; j < num_inter_mode_cands; ++j) {
    ...
    *mbmi = inter_modes_info->mbmi_arr[data_idx];
    ...
    // If a newmv mode was evaluated early, limit the number of candidates to
    // max_allowed_cands + 1.
