...
static INLINE int refine_mv_for_newmv(const AV1_COMP *const cpi,
                                      MACROBLOCK *const x, BLOCK_SIZE bsize,
                                      const int *pts0, const int *pts_inref0,
                                      int total_samples, int tmp_rate2,
                                      int rate2_nocoeff, int rate_mv0) {
  ...
  if (mv0.as_int != mbmi->mv[0].as_int) {
    ...
    int tmp_rate_mv = av1_mv_bit_cost(&mbmi->mv[0].as_mv, &ref_mv.as_mv,
    ...
    if (mbmi->motion_mode == SIMPLE_TRANSLATION && !is_interintra_mode) {
      ...
      for (int rot = -ROTATION_RANGE; rot <= ROTATION_RANGE;
        ...
        if (!av1_find_projection(mbmi->num_proj_ref, pts, pts_inref, bsize,
          ...
          if (have_newmv_in_inter_mode(this_mode)) {
            // Refine MV for NEWMV mode
            tmp_rate2 = refine_mv_for_newmv(cpi, x, bsize, pts0, pts_inref0,
