...
static INLINE int64_t compute_best_interintra_mode(
    const AV1_COMP *const cpi, MB_MODE_INFO *mbmi, MACROBLOCKD *xd,
    MACROBLOCK *const x, int skip_smooth_intra,
    const int *const interintra_mode_cost, const BUFFER_SET *orig_dst,
    uint8_t *intrapred, const uint8_t *tmp_buf, BLOCK_SIZE bsize,
  ...
  const int bw = block_size_wide[bsize];
  for (*index = 0; *index < INTERINTRA_MODES; ++(*index)) {
    if (skip_smooth_intra && (INTERINTRA_MODE)*index == II_SMOOTH_PRED)
      continue;
    mbmi->interintra_mode = (INTERINTRA_MODE)*index;
    int rmode = interintra_mode_cost[mbmi->interintra_mode];
    av1_build_intra_predictors_for_interintra(cm, xd, bsize, 0, orig_dst,
                                              intrapred, bw);
    av1_combine_interintra(xd, bsize, 0, tmp_buf, bw, intrapred, bw);
    model_rd_sb_fn[MODELRD_TYPE_INTERINTRA](cpi, bsize, x, xd, 0, 0, &rate,
                                            &dist, &skip_txfm_sb, &skip_sse_sb,
                                            NULL, NULL, NULL);
...
static int handle_inter_intra_mode(const AV1_COMP *const cpi,
  ...
  const int skip_smooth_intra =
      (!cpi->oxcf.enable_smooth_intra || cpi->sf.intra_sf.disable_smooth_intra);
  ...
  if (try_smooth_interintra) {
    ...
    if (cpi->sf.inter_sf.reuse_inter_intra_mode == 0 ||
      ...
      best_interintra_rd_nowedge = compute_best_interintra_mode(
          cpi, mbmi, xd, x, skip_smooth_intra, interintra_mode_cost, orig_dst,
          intrapred, tmp_buf, bsize, &best_interintra_mode,
          best_interintra_rd_nowedge, &idx);
    ...
    if (idx == 0 || best_interintra_mode != INTERINTRA_MODES - 1) {
  ...
  if (try_wedge_interintra) {
    ...
    if (!cpi->sf.inter_sf.fast_interintra_wedge_search) {
      ...
      if (best_interintra_mode == INTERINTRA_MODES) {
        ...
        int idx = 0;
        best_interintra_rd_wedge = compute_best_interintra_mode(
            cpi, mbmi, xd, x, 0, interintra_mode_cost, orig_dst, intrapred,
            tmp_buf, bsize, &best_interintra_mode, best_interintra_rd_wedge,
            &idx);
        args->inter_intra_mode[mbmi->ref_frame[0]] = best_interintra_mode;
        mbmi->interintra_mode = best_interintra_mode;
