...
static AOM_INLINE void init_neighbor_pred_buf(
  ...
  if (is_hbd) {
    ...
    args->above_pred_buf[0] = CONVERT_TO_BYTEPTR(obmc_buffer->above_pred_buf);
    args->above_pred_buf[1] = CONVERT_TO_BYTEPTR(obmc_buffer->above_pred_buf +
                                                 (MAX_SB_SQUARE >> 1) * len);
    args->above_pred_buf[2] =
        CONVERT_TO_BYTEPTR(obmc_buffer->above_pred_buf + MAX_SB_SQUARE * len);
    args->left_pred_buf[0] = CONVERT_TO_BYTEPTR(obmc_buffer->left_pred_buf);
    args->left_pred_buf[1] = CONVERT_TO_BYTEPTR(obmc_buffer->left_pred_buf +
                                                (MAX_SB_SQUARE >> 1) * len);
    args->left_pred_buf[2] =
        CONVERT_TO_BYTEPTR(obmc_buffer->left_pred_buf + MAX_SB_SQUARE * len);
  } else {
    args->above_pred_buf[0] = obmc_buffer->above_pred_buf;
    args->above_pred_buf[1] =
        obmc_buffer->above_pred_buf + (MAX_SB_SQUARE >> 1);
    args->above_pred_buf[2] = obmc_buffer->above_pred_buf + MAX_SB_SQUARE;
    args->left_pred_buf[0] = obmc_buffer->left_pred_buf;
    args->left_pred_buf[1] = obmc_buffer->left_pred_buf + (MAX_SB_SQUARE >> 1);
    args->left_pred_buf[2] = obmc_buffer->left_pred_buf + MAX_SB_SQUARE;
  }
}
...
static INLINE void calc_target_weighted_pred_above(
  ...
  int32_t *wsrc = ctxt->obmc_buffer->wsrc_buf + (rel_mi_col * MI_SIZE);
  int32_t *mask = ctxt->obmc_buffer->mask_buf + (rel_mi_col * MI_SIZE);
...
static INLINE void calc_target_weighted_pred_left(
  ...
  int32_t *wsrc = ctxt->obmc_buffer->wsrc_buf + (rel_mi_row * MI_SIZE * bw);
  int32_t *mask = ctxt->obmc_buffer->mask_buf + (rel_mi_row * MI_SIZE * bw);
...
static AOM_INLINE void calc_target_weighted_pred(
  ...
  int32_t *mask_buf = obmc_buffer->mask_buf;
  int32_t *wsrc_buf = obmc_buffer->wsrc_buf;
