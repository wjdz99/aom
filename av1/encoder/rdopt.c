...
static void rd_pick_palette_intra_sby(
  ...
  if (colors > 1 && colors <= 64) {
    ...
    int start_n = AOMMIN(colors, PALETTE_MAX_SIZE), end_n = PALETTE_MIN_SIZE;

    // Perform top color palette search from start_n
    int top_color_winner = perform_top_color_palette_search(
    ...
    if (colors == PALETTE_MIN_SIZE) {
      ...
      int k_means_winner = perform_k_means_palette_search(
