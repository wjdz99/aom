...
static AOM_INLINE void inter_modes_info_push(
    InterModesInfo *inter_modes_info, int mode_rate, int64_t sse, int64_t sse_y,
    int64_t rd, RD_STATS *rd_cost, RD_STATS *rd_cost_y, RD_STATS *rd_cost_uv,
  ...
  inter_modes_info->sse_y_arr[num] = sse_y;
...
static int64_t get_sse(const AV1_COMP *cpi, const MACROBLOCK *x,
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    cpi->fn_ptr[bs].vf(p->src.buf, p->src.stride, pd->dst.buf, pd->dst.stride,
                       &sse);
    total_sse += sse;
    if (!plane && sse_y) *sse_y = ROUND_POWER_OF_TWO(sse, (xd->bd - 8) * 2);
  }
  total_sse <<= 4;
  return total_sse;
}
    ...
    if (!do_tx_search) {
      ...
      *best_est_rd = AOMMIN(*best_est_rd, rd_stats->rdcost);
      if (cm->current_frame.reference_mode == SINGLE_REFERENCE) {
        if (!is_comp_pred) {
          assert(curr_sse >= 0);
          inter_modes_info_push(inter_modes_info, mode_rate, curr_sse, sse_y,
    ...
    for (int j = 0; j < inter_modes_info->num; ++j) {
      ...
      *mbmi = inter_modes_info->mbmi_arr[data_idx];
      ...
      if (cpi->sf.inter_sf.txfm_rd_gate_level) {
        ...
        int64_t curr_sse_y = inter_modes_info->sse_y_arr[data_idx];
        skip_rd = RDCOST(x->rdmult, mode_rate, curr_sse);
        skip_rdy = RDCOST(x->rdmult, mode_rate, curr_sse_y);
        int eval_txfm =
      ...
      if (rd_stats.rdcost < search_state.best_rd) {
        ...
        search_state.best_skip_rd[1] = skip_rdy;
      }
    }
  }
#if CONFIG_COLLECT_COMPONENT_TIMING
