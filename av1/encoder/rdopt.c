...
static AOM_INLINE void store_coding_context(
  ...
  const int mode_index = get_prediction_mode_idx(
            mode, ref_frames[0], ref_frames[1]);
...
static AOM_INLINE int prune_modes_based_on_tpl_stats(
    PruneInfoFromTpl *inter_cost_info_from_tpl, const MV_REFERENCE_FRAME *refs, int ref_mv_idx,
...
static int skip_repeated_newmv(
    ...
    HandleInterModeArgs *args, int drl_cost, const MV_REFERENCE_FRAME *refs, int_mv *cur_mv,
...
static void tx_search_best_inter_candidates(
  ...
  store_winner_mode_stats(&cpi->common, x, mbmi, NULL, NULL, NULL, NULL,
...
void av1_rd_pick_inter_mode_sb(struct AV1_COMP *cpi,
  ...
  for (int j = 0; j < intra_mode_num; ++j) {
    ...
    MV_REFERENCE_FRAME refs[2] = { intra_mode_ref_frames[j][0], intra_mode_ref_frames[j][1] };

    if (refs[0] != INTRA_FRAME) printf("mode %d, ref0 %d ref1 %d\n", this_mode, refs[0], refs[1]);
  ...
  *mbmi = search_state.best_mbmode;
  ...
  store_coding_context(x, ctx, search_state.best_mbmode.mode, 
                       refs,
...
void av1_rd_pick_inter_mode_sb_seg_skip(const AV1_COMP *cpi,
  ...
  MV_REFERENCE_FRAME refs[2] = 
            { LAST_FRAME, -1 } ;
  store_coding_context(x, ctx, GLOBALMV, refs, best_pred_diff, 0);
#else
