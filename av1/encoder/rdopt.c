...
#else
  const int drl_cost =
      get_drl_cost(mbmi, mbmi_ext, x->mode_costs.drl_mode_cost, ref_frame_type);
#endif  // CONFIG_NEW_INTER_MODES
...
#else
  const int drl_cost =
      get_drl_cost(mbmi, mbmi_ext, mode_costs->drl_mode_cost, ref_frame_type);
#endif  // CONFIG_NEW_INTER_MODES
...
#if CONFIG_NEW_INTER_MODES
  int_mv dv_ref = av1_find_best_ref_mv_from_stack(0, mbmi_ext, ref_frame, 0);
