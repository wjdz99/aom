...
static int64_t motion_mode_rd(
  ...
  if (features->switchable_motion_mode) {
    ...
    if (last_motion_mode_allowed != last_motion_mode_allowed2)
      printf("debug\n");
    const MOTION_MODE last_motion_mode_allowed3 = motion_mode_allowed_nrs(
        xd->global_motion_nrs, xd, mbmi, features->allow_warped_motion);
    // TODO(sarahparker) Temporary assert
    const MOTION_MODE last_motion_mode_allowed4 = motion_mode_allowed(
        xd->global_motion, xd, mbmi, features->allow_warped_motion);
  ...
  if (x->mbmi_ext->ref_mv_count[ref_frame_type] == UINT8_MAX) {
    ...
    MB_MODE_INFO_EXT *mbmi_ext = x->mbmi_ext;
      ...
      if (ref_frame_type < REF_FRAMES) { 
        ref_frame_nrs = convert_named_ref_to_ranked_ref_index(&cm->new_ref_frame_data, ref_frame_type);
        // TODO(sarahparker) Temporary assert
        assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data,
                                               ref_frame_nrs) ==
         ref_frame_type);
      }
    av1_find_mv_refs(cm, xd, mbmi, ref_frame_type, ref_frame_nrs, mbmi_ext->ref_mv_count,
                     xd->ref_mv_stack, xd->weight, NULL, mbmi_ext->global_mvs,
                     mbmi_ext->mode_context);
#else
...
static AOM_INLINE void set_params_rd_pick_inter_mode(
  ...
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data,
                                               ref_frame_nrs) ==
         ref_frame);
      setup_buffer_ref_mvs_inter(cpi, x, ref_frame, ref_frame_nrs, bsize, yv12_mb);
#else
