...
#if CONFIG_NEW_REF_SIGNALING
      search_state->best_mbmode.ref_frame_nrs[0] = convert_named_ref_to_ranked_ref_index(&cm->new_ref_frame_data, search_state->best_mbmode.ref_frame[0]);
  search_state->best_mbmode.ref_frame_nrs[1] = convert_named_ref_to_ranked_ref_index(&cm->new_ref_frame_data, search_state->best_mbmode.ref_frame[1]);
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, search_state->best_mbmode.ref_frame_nrs[0]) == search_state->best_mbmode.ref_frame[0]);
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, search_state->best_mbmode.ref_frame_nrs[1]) == search_state->best_mbmode.ref_frame[1]);
#endif  // CONFIG_NEW_REF_SIGNALING
  ...
  for (THR_MODES midx = THR_MODE_START; midx < THR_MODE_END; ++midx) {
    ...
    const MV_REFERENCE_FRAME second_ref_frame = ref_frames[1];
        ...
        if (skip_inter_mode(cpi, x, bsize, ref_frame_rd,
          ...
          continue;
#if CONFIG_NEW_REF_SIGNALING
  if (convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, mbmi->ref_frame_nrs[0]) != mbmi->ref_frame[0])
    printf("here\n");
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, mbmi->ref_frame_nrs[0]) == mbmi->ref_frame[0]);
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, mbmi->ref_frame_nrs[1]) == mbmi->ref_frame[1]);
#endif  // CONFIG_NEW_REF_SIGNALING
