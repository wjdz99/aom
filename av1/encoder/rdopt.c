...
static void choose_largest_tx_size(VP10_COMP *cpi, MACROBLOCK *x, int *rate,
  ...
  vpx_prob skip_prob = av1_get_skip_prob(cm, xd);
...
static int64_t encode_inter_mb_segment(VP10_COMP *cpi, MACROBLOCK *x,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    fwd_txm4x4 = xd->lossless[mi->mbmi.segment_id] ? av1_fwht4x4 : vpx_fdct4x4;
  }
#else
  fwd_txm4x4 = xd->lossless[mi->mbmi.segment_id] ? av1_fwht4x4 : vpx_fdct4x4;
#endif  // CONFIG_VPX_HIGHBITDEPTH
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    vpx_subtract_block(height, width, av1_raster_block_offset_int16(
...
static void estimate_ref_frame_costs(const VP10_COMMON *cm,
  ...
  if (seg_ref_active) {
    ...
    *comp_mode_p = 128;
    ...
    vpx_prob intra_inter_p = av1_get_intra_inter_prob(cm, xd);
    ...
    if (cm->reference_mode != COMPOUND_REFERENCE) {
      vpx_prob ref_single_p1 = av1_get_pred_prob_single_ref_p1(cm, xd);
      vpx_prob ref_single_p2 = av1_get_pred_prob_single_ref_p2(cm, xd);
    ...
    if (cm->reference_mode != SINGLE_REFERENCE) {
      vpx_prob ref_comp_p = av1_get_pred_prob_comp_ref_p(cm, xd);
      unsigned int base_cost = av1_cost_bit(intra_inter_p, 1);
...
void av1_rd_pick_inter_mode_sub8x8(VP10_COMP *cpi, TileDataEnc *tile_data,
                                    MACROBLOCK *x, int mi_row, int mi_col,
                                    RD_COST *rd_cost, BLOCK_SIZE bsize,
                                    PICK_MODE_CONTEXT *ctx,
                                    int64_t best_rd_so_far) {
