...
static AOM_INLINE int ref_match_found_in_nb_blocks(MB_MODE_INFO *cur_mbmi,
  ...
  for (int i = 0; i < (is_cur_comp_pred ? 2 : 1); i++) {
    if (cur_ref_frames[i] == (nb_ref_frames[0] || nb_ref_frames[1])) return 1;
  }
  return 0;
}

static INLINE int find_ref_match_in_above_nbs(const AV1_COMMON *cm,
                                              MACROBLOCKD *xd) {
...
static INLINE int find_ref_match_in_left_nbs(const AV1_COMMON *cm,
                                             MACROBLOCKD *xd) {
...
#define TPL_THRESH_DIV_FACTOR 2
...
static AOM_INLINE int prune_modes_based_on_tpl_stats(
  ...
  static const int thresh_to_prune_ref_mv_idx[MAX_REF_MV_SEARCH] = {
    3, TPL_THRESH_DIV_FACTOR, TPL_THRESH_DIV_FACTOR
  };
  ...
  int threshold = is_globalmv ? TPL_THRESH_DIV_FACTOR
                              : thresh_to_prune_ref_mv_idx[ref_mv_idx];

  if (cur_inter_cost > ((threshold * best_inter_cost) / TPL_THRESH_DIV_FACTOR))
    return 1;
  return 0;
}
...
static int64_t handle_inter_mode(
  ...
  if (prune_based_on_tpl) {
    ref_match_found_in_above_nb = find_ref_match_in_above_nbs(cm, xd);
    ref_match_found_in_left_nb = find_ref_match_in_left_nbs(cm, xd);
  }
