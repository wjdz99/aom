...
static void store_winner_mode_stats(const AV1_COMMON *const cm, MACROBLOCK *x,
  ...
  // mode in Inter frames. Update rd stats if the support added
  if (!frame_is_intra_only(cm) && !is_palette_mode && rd_cost && rd_cost_y &&
      rd_cost_uv) {
    const MACROBLOCKD *xd = &x->e_mbd;
    const int skip_ctx = av1_get_skip_context(xd);
...
static AOM_INLINE void refine_winner_mode_tx(
  ...
  MACROBLOCKD *const xd = &x->e_mbd;
  MB_MODE_INFO *const mbmi = xd->mi[0];
  int64_t best_rd = RDCOST(x->rdmult, rd_cost->rate, rd_cost->dist);
  const int num_planes = av1_num_planes(cm);
