...
static AOM_INLINE int prune_modes_based_on_tpl_stats(
  ...
  // ref_mv_idx 0. 'thresh_index' 0, 1, 2 corresponds to ref_mv indices 0, 1
  // and 2. thresh_index 3 corresponds to GLOBALMV/GLOBAL_GLOBALMV
  const int thresh_to_prune[MAX_REF_MV_SEARCH + 1] = { 3, tpl_thresh_factor,
                                                       tpl_thresh_factor,
                                                       tpl_thresh_factor };
...
static int64_t handle_inter_mode(
  ...
  for (int ref_mv_idx = 0; ref_mv_idx < ref_set; ++ref_mv_idx) {
    ...
    if (prune_based_on_tpl && !ref_match_found_in_above_nb &&
