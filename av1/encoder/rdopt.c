...
// Update 'ref_combo' mask to disable all inter references except 1st.
static AOM_INLINE void disable_inter_references_except_first_nrs(
    bool ref_combo[REF_FRAMES_NRS][REF_FRAMES_NRS + 1]) {
  disable_reference_nrs(1, ref_combo);
  disable_reference_nrs(2, ref_combo);
  disable_reference_nrs(3, ref_combo);
  disable_reference_nrs(4, ref_combo);
  disable_reference_nrs(5, ref_combo);
  disable_reference_nrs(6, ref_combo);
}
...
static int inter_mode_search_order_independent_skip(
    ...
    const MV_REFERENCE_FRAME *ref_frame) {
#if CONFIG_NEW_REF_SIGNALING
  if (mask_says_skip_nrs(mode_skip_mask, ref_frame, mode)) {
    return 1;
  }
#else
