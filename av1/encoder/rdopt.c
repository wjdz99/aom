 ...
 */
static inline int prune_zero_mv_with_sse(
...
static inline void init_neighbor_pred_buf(
...
static inline void calculate_cost_from_tpl_data(
...
static inline int get_block_temp_var(const AV1_COMP *cpi,
                                         const MACROBLOCK *x,
                                         BLOCK_SIZE bsize) {
  const AV1_COMMON *const cm = &cpi->common;
  const SPEED_FEATURES *const sf = &cpi->sf;
