 ...
 * \param[in]    rd_stats          Struct to keep track of the overall RD
 *                                 information.
 ...
 * \param[in]    rd_buffers        CompoundTypeRdBuffers struct to hold all
 *                                 allocated buffers for the compound
 *                                 predictors and masks in the compound type
 *                                 search.
 * \param[in]    best_est_rd       Estimated RD for motion mode search if
 ...
 *                                 full transform search on the best candidates.
 * \param[in]    inter_modes_info  InterModesInfo struct used to hold inter mode
 ...
 * \param[in]    motion_mode_cand  A motion_mode_candidate struct to store
 *                                 motion mode information used in a speed
 *                                 feature to search motion modes other than
 *                                 SIMPLE_TRANSLATION only on winning
 *                                 candidates.
 * \param[in]    skip_rd           An array of length 2, where skip_rd[0] is the
 ...
 *
 * \return The RD cost for the mode being serached.
 * Additionally, the following structures are modified to hold all related
 * information to the current mode: MB_MODE_INFO struct, rd_stats, rd_buffers,
 * best_est_rd,  inter_modes_info, motion_mode_cand,  skip_rd.
 */
