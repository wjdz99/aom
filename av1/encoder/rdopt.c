...
// Check NEARESTMV, NEARMV, GLOBALMV ref mvs for duplicate
static INLINE int check_repeat_ref_mv(const MB_MODE_INFO_EXT *mbmi_ext,
  ...
  if (single_mode == NEARESTMV) {
    ...
    // when ref_mv_count = 1, NEARMV is same as GLOBALMV, hence skip this NEARMV
    if (ref_mv_count < 2) return 1;
  } else if (single_mode == GLOBALMV) {
    // when ref_mv_count == 0, GLOBALMV is same as NEARESTMV, skip this GLOBALMV
    if (ref_mv_count == 0) return 1;

    // when ref_mv_count == 1, NEARMV is same as GLOBALMV. Since NEARMV is
    // already skipped, do not skip GLOBALMV
