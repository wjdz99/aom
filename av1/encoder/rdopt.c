...
static int process_compound_inter_mode(
    ...
    int64_t ref_best_rd, int_mv *cur_mv, BLOCK_SIZE bsize, int *skip_build_pred,
  ...
  for (int ref_mv_idx = 0; ref_mv_idx < ref_set; ++ref_mv_idx) {
    ...
    if (is_comp_pred) {
      const int not_best_mode = process_compound_inter_mode(
          cpi, x, args, ref_best_rd, cur_mv, bsize, &skip_build_pred,
          &compmode_interinter_cost, rd_buffers, &orig_dst, &tmp_dst, &rate_mv,
          rd_stats, skip_rd);
    ...
    if (args->modelled_rd != NULL &&
        update_modelled_rd(args, xd, num_planes, &orig_dst,
                           ref_best_rd, is_comp_pred, this_mode,
                           rd, ref_mv_idx, refs)) continue;
