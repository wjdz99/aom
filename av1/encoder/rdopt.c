      ...
      for (int ref0 = 0; ref0 < n_refs - 1; ref0++) {
        for (int ref1 = ref0 + 1; ref1 < n_refs; ref1++) {
          ...
          for (int i = 0; i < n_refs + n_bits - 2; i++) {
            ...
            const int bit_type =
                av1_get_compound_ref_bit_type(n_bits, &cm->new_ref_frame_data, ref0, i);
            assert(i + 1 < n_refs);
            const int ctx = av1_get_ref_pred_context_nrs(xd, i, n_refs);
            cost += mode_costs->compound_ref_cost[ctx][bit_type][i - n_bits][bit];
            n_bits += bit;
            if (n_bits == 2) break;
          }
