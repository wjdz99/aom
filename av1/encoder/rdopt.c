...
static INLINE void update_start_end_stage_2(int *start_n_stage2,
                                            int *end_n_stage2,
                                            int *step_size_stage2, int winner,
                                            int end_n) {
...
static void rd_pick_palette_intra_sby(
  ...
  if (colors > 1 && colors <= 64) {
    ...
    if ((cpi->sf.prune_palette_search_level == 1) &&
      ...
      int end_n = AOMMIN(colors, PALETTE_MAX_SIZE);
      assert(PALETTE_MAX_SIZE == 8);
      assert(PALETTE_MIN_SIZE == 2);
#define STEP_SIZE_STAGE2 2
      ...
      int start_n_lookup_table[PALETTE_MAX_SIZE + 1] = { 0, 0, 0, 3, 3,
                                                         2, 3, 3, 2 };
      // step size for coarse palette search for dominant colors and k-means
      int step_size_lookup_table[PALETTE_MAX_SIZE + 1] = { 0, 0, 0, 3, 3,
                                                           3, 3, 3, 3 };
      // Choose the start index and step size for coarse search based on number
      // of colors
      int start_n = start_n_lookup_table[end_n];
      int step_size = step_size_lookup_table[end_n];
