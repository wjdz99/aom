...
static int64_t rd_pick_intrabc_mode_sb(const AV1_COMP *cpi, MACROBLOCK *x,
  ...
  for (enum IntrabcMotionDirection dir = IBC_MOTION_ABOVE;
    ...
    assert(fullms_params->mv_limits.col_min >= x->mv_limits.col_min);
    assert(fullms_params->mv_limits.col_max <= x->mv_limits.col_max);
    assert(fullms_params->mv_limits.row_min >= x->mv_limits.row_min);
    assert(fullms_params->mv_limits.row_max <= x->mv_limits.row_max);
