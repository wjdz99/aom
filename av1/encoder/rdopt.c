...
static INLINE int check_repeat_ref_mv(const MB_MODE_INFO_EXT *mbmi_ext,
  ...
  if (single_mode == NEARESTMV) {
    ...
    if (mbmi_ext->ref_mv_count[ref_frame_type] == 0) return 1;
    ...
    if (mbmi_ext->ref_mv_count[ref_frame_type] == 0) return 1;
    // when ref_mv_count == 1, GLOBALMV is same as NEARMV, NEARMV is already
    // skipped
    else if (mbmi_ext->ref_mv_count[ref_frame_type] == 1)
      return 0;
    ...
    // Check GLOBALMV is repeated in ref_mv_stack
...
static INLINE int build_cur_mv(int_mv *cur_mv, PREDICTION_MODE this_mode,
  ...
  const MB_MODE_INFO *mbmi = xd->mi[0];
  const int is_comp_pred = has_second_ref(mbmi);
  if (is_comp_pred) skip_repeat_mv = 0;
  int ret = 1;
