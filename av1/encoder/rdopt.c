...
static int inter_mode_search_order_independent_skip(
  ...
  if (sf->inter_sf.prune_nearmv_using_neighbors &&
    ...
    if (mode == NEAR_NEARMV || mode == NEARMV) {
      const MACROBLOCKD *const xd = &x->e_mbd;
      if (xd->left_available && xd->up_available) {
        ...
        num_ref_frame_pair_match =
            match_ref_frame_pair(xd->left_mbmi, ref_frame);
        num_ref_frame_pair_match +=
            match_ref_frame_pair(xd->above_mbmi, ref_frame);

        // Prune modes as follows:
        // qindex   0 to 85 : num_ref_frame_pair_match < 2
        // qindex  86 to 170: num_ref_frame_pair_match < 1
        // qindex 171 to 255: No pruning
        if (num_ref_frame_pair_match < num_ref_frame_pair_match_thresh)
          return 1;
      }
    }
  }
