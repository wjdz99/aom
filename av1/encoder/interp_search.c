...
static INLINE void find_best_non_dual_interp_filter(
  ...
  if (adaptive_interp_filter_search >= 2) {
    ...
    if (adaptive_interp_filter_search == 2) {
      ...
      // adaptive_interp_filter_search == 3 case
      const int shift = 3;
      int prune[SWITCHABLE_FILTERS] = { 0 };

      if ((switchable_interp_p0[1] < (switchable_interp_p0[0] >> shift)) ||
          (switchable_interp_p0[1] < (switchable_interp_p0[2] >> shift)))
        prune[1] = 1;
      if ((switchable_interp_p0[2] < (switchable_interp_p0[0] >> shift)) ||
          (switchable_interp_p0[2] < (switchable_interp_p0[1] >> shift)))
        prune[2] = 1;
      if (prune[1] == 1 && prune[2] == 1) {
        if (switchable_interp_p0[1] >= switchable_interp_p0[2])
          prune[1] = 0;
        else
          prune[2] = 0;
      }

      // const int shift =(x->qindex * 3 / QINDEX_RANGE);
      // printf(" (%d; %d,%d,%d) ", update_type, switchable_interp_p0[0],
      // switchable_interp_p0[1], switchable_interp_p0[2]);
      for (i = 0; i < SWITCHABLE_FILTERS; i++) {
        if (prune[i]) {
          DUAL_FILTER_TYPE filt_type = i + SWITCHABLE_FILTERS * i;
          reset_interp_filter_allowed_mask(&interp_filter_search_mask,
