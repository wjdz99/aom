...
static INLINE void find_best_non_dual_interp_filter(
  ...
  if (adaptive_interp_filter_search >= 2) {
    ...
    static const int thr[2][7] = { { 0, 8, 8, 8, 8, 0, 8 },
                                   { 0, 32, 8, 8, 8, 0, 16 } };
    const int multi = (adaptive_interp_filter_search > 2)
                          ? (x->qindex * 3 / QINDEX_RANGE) + 1
                          : 1;
    int thresh = thr[adaptive_interp_filter_search - 2][update_type];
    thresh *= multi;
