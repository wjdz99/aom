 ...
 *                                 computed prediction.
 * \param[in]    orig_dst          A prediction buffer to hold a computed
 *                                 prediction. This will eventually hold the
 *                                 final prediction, and the tmp_dst info will
 *                                 be copied here.
 * \param[in]    rd                The RD cost associated with the selected
 *                                 interpolation filter parameters.
 * \param[in]    switchable_rate   The rate associated with using a SWITCHABLE
 *                                 filter mode.
 * \param[in]    skip_build_pred   Indicates whether or not to build the inter
 ...
 * Additionally, the following structures are modified to hold all related
 * information to the current mode: rd, switchable_rate, skip_build_pred,
 * orig_dst, mbmi.
 */
