...
void av1_fht4x8_c(const int16_t *input, tran_low_t *output, int stride,
  ...
  for (i = 0; i < n2; ++i) {
    ...
    for (j = 0; j < n; ++j) {
      ...
      temp_in[j] = (tran_low_t)fdct_round_shift(input[i * stride + j] *
                                                (use_lgt_row ? 2 * Sqrt2 : 4) *
                                                (use_lgt_col ? 2 : 4));
#else
...
#endif
#else
      // Input scaling when LGT is not possible, AV1 only (1 above)
      temp_in[j] =
...
#if CONFIG_LGT
      // Input scaling when LGT might be active (1-4 above)
      temp_in[j] = (tran_low_t)fdct_round_shift(input[j * stride + i] *
                                                (use_lgt_col ? 2 * Sqrt2 : 4) *
                                                (use_lgt_row ? 2 : 4));
#else
      // Input scaling when LGT is not possible, Daala only (4 above)
...
#endif
#else
      // Input scaling when LGT is not possible, AV1 only (case 1 above)
      temp_in[j] =
          (tran_low_t)fdct_round_shift(input[i * stride + j] * 4 * Sqrt2);
#endif
    }
...
void av1_fht16x8_c(const int16_t *input, tran_low_t *output, int stride,
  ...
  for (i = 0; i < n2; ++i) {
    ...
    for (j = 0; j < n; ++j) {
      ...
      temp_in[j] = (tran_low_t)fdct_round_shift(input[i * stride + j] * 4 *
                                                (use_lgt_col ? 2 : 4));
#else
      // Input scaling when LGT is not possible, Daala only (4 above)
      temp_in[j] = input[j * stride + i] * 16;
