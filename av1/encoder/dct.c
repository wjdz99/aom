...
#if CONFIG_MRC_TX
static void get_masked_residual32(const tran_low_t **input,
                                  int *input_stride,
                                  const tran_low_t *pred, int pred_stride,
                                  tran_low_t *masked_input) {
  tran_low_t mrc_mask[32 * 32];
  get_mrc_mask(pred, pred_stride, mrc_mask, 32, 32, 32);
  int sum;
  int avg;
  for (int i = 0; i < 32; ++i) {
    for (int j = 0; j < 32; ++j) {
      sum += pred[i * pred_stride + j];
    }
  }
  avg = sum / (32 * 32);
  for (int i = 0; i < 32; ++i) {
    for (int j = 0; j < 32; ++j)
      masked_input[i * 32 + j] = (mrc_mask[i * pred_stride + j]) ?
                                 (*input)[i * (*input_stride) + j] : avg;
  }
  *input = masked_input;
  *input_stride = 32;
}
...
void av1_fht32x32_c(const int16_t *input, tran_low_t *output, int stride,
  ...
  if (tx_type == MRC_DCT) {
    ...
    get_masked_residual32(&input, &stride, txfm_param->dst,
                          txfm_param->dst_stride, masked_input);
  }
#endif  // CONFIG_MRC_TX
