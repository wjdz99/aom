...
void av1_single_motion_search(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  if (mbmi->motion_mode == SIMPLE_TRANSLATION) {
    if (x->valid_cost_b) {
      ...
      if (nw >= 1 && nh >= 1) {
        ...
        for (int k = 0; k < nh; k++) {
          for (int l = 0; l < nw; l++) {
            ...
            int unique = 1;
            for (int m = 0; m < cnt; m++) {
              if (RS3(fmv.row) == RS3(cand[m].row) &&
                  RS3(fmv.col) == RS3(cand[m].col)) {
                unique = 0;
                weight[m]++;
                break;
              }
            ...
            //            if (mv_row_max < mv.as_mv.row) mv_row_max =
            //            mv.as_mv.row; if (mv_row_min > mv.as_mv.row)
            //            mv_row_min = mv.as_mv.row;
            //
            //            if (mv_col_max < mv.as_mv.col) mv_col_max =
            //            mv.as_mv.col; if (mv_col_min > mv.as_mv.col)
            //            mv_col_min = mv.as_mv.col;
          }
          if (!valid) {
            break;
          }
        ...
        if (valid) {
          //          printf("   !!! cnt=%d; ", cnt);
          //          for (int m = 0; m < cnt; m++) {
          //            printf(" w:%d(%d, %d) ", weight[m], cand[m].row,
          //            cand[m].col);
          //          }
      ...
      //        const search_site_config *ss_cfg = &cpi->ss_cfg[SS_CFG_SRC];
      //        const int sr = GET_MV_RAWPEL(dissimilarity) +
      //        GET_MV_RAWPEL(mvd);
      //
  ...
  switch (mbmi->motion_mode) {
    case SIMPLE_TRANSLATION: {
      ...
      //      if (cnt < 1) printf("\n !!!!!!!!!!!!!!wrong !!!!\n");
      for (int m = 0; m < cnt; m++) {
        FULLPEL_MV smv = cand[m];
        bestsme = av1_full_pixel_search(
            cpi, x, bsize, &smv, step_param, cpi->sf.mv_sf.search_method, 0,
