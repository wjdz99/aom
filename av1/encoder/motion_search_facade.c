...
void av1_single_motion_search(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  if (cpi->sf.inter_sf.skip_repeated_newmv == 2 &&
    ...
    for (int prev_ref_idx = 0; prev_ref_idx < ref_mv_idx; ++prev_ref_idx) {
      ...
      if (mode_info[prev_ref_idx].full_mv_bestsme <
          mode_info[ref_mv_idx].full_mv_bestsme && mode_info[prev_ref_idx].drl_cost <
          mode_info[ref_mv_idx].drl_cost) {
  ...
  if (use_fractional_mv) {
    ...
    if (cpi->sf.inter_sf.skip_repeated_newmv >= 1 && args != NULL &&
      ...
      for (int prev_ref_idx = 0; prev_ref_idx < ref_mv_idx; ++prev_ref_idx) {
        ...
        if (0 && !mode_info[ref_mv_idx].skip &&
