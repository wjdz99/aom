      ...
      case SIMPLE_TRANSLATION:
        if (cpi->sf.mv_sf.use_accurate_subpel_search) {
          const int best_mv_var = cpi->find_fractional_mv_step(
              x, cm, mi_row, mi_col, &ms_params, &dis, &x->pred_sse[ref]);
        ...
        break;
      case OBMC_CAUSAL:
        av1_find_best_obmc_sub_pixel_tree_up(x, cm, mi_row, mi_col, &ms_params,
                                             &dis, &x->pred_sse[ref]);
        break;
      default: assert(0 && "Invalid motion mode!\n");
    }
...
void av1_joint_motion_search(const AV1_COMP *cpi, MACROBLOCK *x,
  ...
  for (ite = 0; ite < 4; ite++) {
    ...
    if (bestsme < INT_MAX && cpi->common.cur_frame_force_integer_mv == 0) {
      ...
      bestsme = cpi->find_fractional_mv_step(x, cm, mi_row, mi_col, &ms_params,
                                             &dis, &sse);
    }
