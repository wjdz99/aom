...
static AOM_INLINE const search_site_config * get_search_site_config(search_site_config *ss_cfg_buf,
  ...
  } else if (ref_stride == mv_search_params->search_site_cfg[SS_CFG_LOOKAHEAD]->stride) {
  ...
  if (ref_stride != ss_cfg_buf[search_method].stride) {
    const int level = search_method == NSTEP_8PT || search_method == CLAMPED_DIAMOND;
...
void av1_single_motion_search(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  const search_site_config *src_search_site_cfg = get_search_site_config(x->search_site_cfg_buf,
                                                                          mv_search_params, search_method,
                                                                          ref_stride);

  // Further reduce the search range.
  if (search_range < INT_MAX) {
    const search_site_config *search_site_cfg =
        &src_search_site_cfg[cpi->sf.mv_sf.search_method];
...
int av1_joint_motion_search(const AV1_COMP *cpi, MACROBLOCK *x,
  ...
  for (ite = 0; ite < 4; ite++) {
    ...
    const SEARCH_METHODS search_method = cpi->sf.mv_sf.search_method;
    const int ref_stride = xd->plane[0].pre[0].stride;
    const search_site_config *src_search_sites = get_search_site_config(x->search_site_cfg_buf,
                                                                            &cpi->mv_search_params, search_method,
                                                                            ref_stride);
