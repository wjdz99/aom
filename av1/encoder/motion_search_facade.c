      ...
      case SIMPLE_TRANSLATION:
        if (cpi->sf.mv_sf.use_accurate_subpel_search) {
          ...
          cpi->find_fractional_mv_step(xd, cm, &ms_params, subpel_start_mv,
                                       &best_mv->as_mv, &dis, &x->pred_sse[ref],
                                       NULL);
        }
        break;
      case OBMC_CAUSAL:
...
void av1_joint_motion_search(const AV1_COMP *cpi, MACROBLOCK *x,
  ...
  for (ite = 0; ite < 4; ite++) {
    ...
    if (bestsme < INT_MAX &&
      ...
      bestsme = cpi->find_fractional_mv_step(xd, cm, &ms_params, start_mv,
                                             &best_mv.as_mv, &dis, &sse, NULL);
    }
...
int_mv av1_simple_motion_search(AV1_COMP *const cpi, MACROBLOCK *x, int mi_row,
  ...
  if (use_subpel_search) {
    ...
    cpi->find_fractional_mv_step(xd, cm, &ms_params, subpel_start_mv,
                                 &best_mv.as_mv, &not_used, &x->pred_sse[ref],
                                 NULL);
  } else {
