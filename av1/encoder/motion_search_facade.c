      ...
      for (int prev_ref_idx = 0; prev_ref_idx < ref_mv_idx; ++prev_ref_idx) {
        ...
        if (best_mv->as_int == args->single_newmv[prev_ref_idx][ref].as_int) {
          ...
          if (prev_rate_cost <= this_rate_cost) {
            ...
            // then we terminate the search. Since av1_single_motion_search is
            // only called by handle_new_mv in SIMPLE_TRANSLATION mode, we set
            // the best_mv to INVALID mv to signal that we wish to terminate
