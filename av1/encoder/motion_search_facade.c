  ...
  if (cpi->sf.mv_sf.skip_fullpel_search_using_startmv &&
    ...
    for (int cand_idx = 0; cand_idx < cand_cnt; cand_idx++) {
      ...
      if (mbmi->ref_mv_idx != MAX_REF_MV_SEARCH - 1) {
        args->start_mv_stack[args->start_mv_cnt] = cand[cand_idx].fmv;
        args->start_mv_cnt++;
      ...
      for (int stack_idx = 0; stack_idx < stack_size; stack_idx++) {
        const int row =
            abs(args->start_mv_stack[stack_idx].row - cand[cand_idx].fmv.row);
        const int col =
            abs(args->start_mv_stack[stack_idx].col - cand[cand_idx].fmv.col);
