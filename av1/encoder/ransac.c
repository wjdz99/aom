...
static int get_region_points(double *corners1, double *corners2,
                             ...
                             int frm_width, int frm_height, int n_points,
  ...
  for (int i = 0; i < n_points; ++i) {
    if (coor_within_region((int)corners1[i * 2], (int)corners1[i * 2 + 1],
...
static int ransac(const int *matched_points, int npoints,
                  ...
                  ProjectPointsDoubleFunc projectpoints, int frm_width,
                  int frm_height) {
  ...
  for (int region = 0; region < GLOBAL_REGION_TYPES; ++region) {
    int npoints_region =
        get_region_points(corners1, corners2, region_corners1, region_corners2,
                          frm_width, frm_height, npoints, region);
    while (N > trial_count) {
      ...
      if (current_motion.num_inliers >= worst_kept_motion->num_inliers &&
        ...
        current_motion.variance =
            ...
            mean_distance * mean_distance *
                ((double)current_motion.num_inliers) /
                ((double)current_motion.num_inliers - 1.0);
        if (is_better_motion(&current_motion, worst_kept_motion)) {
          // This motion is better than the worst currently kept motion.
          // Remember the inlier points and variance.
          // The parameters for each kept motion
          ...
          assert(npoints > 0);
          fracinliers =
              (double)current_motion.num_inliers / (double)npoints_region;
          ...
          // Determine the new worst kept motion and its num_inliers
          // and variance.
          for (i = 0; i < num_desired_motions; ++i) {
    ...
    for (i = 0; i < num_desired_motions; ++i) {
      ...
      if (motions[i].num_inliers >= minpts) {
        copy_points_at_indices(points1, region_corners1,
                               motions[i].inlier_indices,
                               motions[i].num_inliers);
        copy_points_at_indices(points2, region_corners2,
                               motions[i].inlier_indices,
                               motions[i].num_inliers);

        find_transformation(
            motions[i].num_inliers, points1, points2,
...
int ransac_rotzoom(int *matched_points, int npoints, int *num_inliers_by_motion,
  ...
  return ransac(matched_points, npoints, num_inliers_by_motion,
                ...
                find_rotzoom, project_points_double_rotzoom, frm_width,
                frm_height);
}
...
int ransac_affine(int *matched_points, int npoints, int *num_inliers_by_motion,
  ...
  return ransac(matched_points, npoints, num_inliers_by_motion,
                params_by_motion, num_desired_motions, 3, is_degenerate_affine,
                find_affine, project_points_double_affine, frm_width,
                frm_height);
}
