  ...
  while (N > trial_count) {
    ...
    while (degenerate) {
      ...
      if (!get_rand_indices(npoints, minpts, indices, &seed)) {
        printf("get_rand_indices() failed, exiting.\n");
        ret_val = 1;
        goto finish_ransac;
      }
      ...
      if (num_degenerate_iter > MAX_DEGENERATE_ITER) {
        //        printf("num_degenerate_iter > MAX, exiting.\n");
        ret_val = 1;
        goto finish_ransac;
      }
    }
    ...
    if (find_transformation(minpts, points1, points2, params)) {
      //      printf("find_transformation() failed, continuing.\n");
      trial_count++;
      continue;
    }
    ...
    if (num_inliers >= num_inliers_by_motion[worst_kept_motion]) {
      ...
      if (is_better_motion(num_inliers, variance,
        ...
        // printf(
        //     "num_inliers: %d, npoints: %d, fracinliers: %f, pNooutliers: %f, "
        //     "PROBABILITY_REQUIRED: %f, temp: %d\n",
        //     num_inliers, npoints, fracinliers, pNoOutliers,
        //     PROBABILITY_REQUIRED, temp);
...
int ransac_translation(int *matched_points, int npoints,
  ...
  return ransac(matched_points, npoints, num_inliers_by_motion,
                params_by_motion, num_desired_motions,
                NUM_INITIAL_POINTS_AFFINE_AND_SIMPLER,
...
int ransac_rotzoom(int *matched_points, int npoints, int *num_inliers_by_motion,
  ...
  return ransac(matched_points, npoints, num_inliers_by_motion,
                params_by_motion, num_desired_motions,
                NUM_INITIAL_POINTS_AFFINE_AND_SIMPLER, is_degenerate_affine,
