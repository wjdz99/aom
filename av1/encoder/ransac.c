...
static int ransac(const int *matched_points, int npoints,
  ...
  motions =
      (RANSAC_MOTION *)aom_malloc(sizeof(RANSAC_MOTION) * num_desired_motions *
                                  GLOBAL_REGION_TYPES);
  for (i = 0; i < num_desired_motions * GLOBAL_REGION_TYPES; ++i) {
  ...
  for (int region = 0; region < GLOBAL_REGION_TYPES; ++region) {
    int npoints_region = get_region_points(corners1, corners2, region_corners1,
                                           region_corners2, npoints, region);
    while (N > trial_count) {
    ...
    qsort(motions + region * num_desired_motions,
          num_desired_motions, sizeof(RANSAC_MOTION), compare_motions);
