...
static int ransac(const int *matched_points, int npoints,
  ...
  int worst_kept_motion_index = 0;

  double *points1, *points2;
  double *corners1, *corners2;
  double *image1_coord;

  // Store the variance, and inlier indices for the num_desired_motions best
  // transformations found so far. The number of inliers for each motion is
  // required by the caller and stored in the method argument.
  ...
  inlier_indices_this_motion =
      (int *)aom_malloc(sizeof(*inlier_indices_this_motion) * npoints);
  memset(inlier_indices_this_motion, 0,
         sizeof(*inlier_indices_this_motion) * npoints);
  ...
  while (N > trial_count) {
    ...
    for (i = 0; i < npoints; ++i) {
      ...
      if (distance < INLIER_THRESHOLD) {
        inlier_indices_this_motion[num_inliers++] = i;
        sum_distance += distance;
        sum_distance_squared += distance * distance;
      }
    }

    if (num_inliers >= num_inliers_by_motion[worst_kept_motion_index]) {
      int temp;
      ...
      if (is_better_motion(num_inliers, variance,
                           num_inliers_by_motion[worst_kept_motion_index],
                           variance_by_motion[worst_kept_motion_index])) {
        // This is one of the top num_desired_motions transformations found so
        // far. Remember the inlier points and the variance. The parameters for
        // each motion will be recomputed at exit using only the inliers.
        num_inliers_by_motion[worst_kept_motion_index] = num_inliers;
        variance_by_motion[worst_kept_motion_index] = variance;
        memcpy(inlier_indices_by_motion + worst_kept_motion_index * npoints,
               inlier_indices_this_motion,
        ...
        for (int motion = 0; motion < num_desired_motions; ++motion) {
          if (is_better_motion(num_inliers_by_motion[worst_kept_motion_index],
                               variance_by_motion[worst_kept_motion_index],
                               num_inliers_by_motion[motion],
                               variance_by_motion[motion])) {
            worst_kept_motion_index = motion;
          }
        }
      }
    }
  ...
  for (int motion = 0; motion < num_desired_motions; ++motion) {
    ...
    find_transformation(num_inliers, points1, points2,
                        params_by_motion + (MAX_PARAMDIM - 1) * motion);
  }
