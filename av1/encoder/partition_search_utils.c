...
static INLINE void update_global_motion_used(PREDICTION_MODE mode,
                                             BLOCK_SIZE bsize,
...
static INLINE void reset_tx_size(MACROBLOCK *x, MB_MODE_INFO *mbmi,
...
static INLINE void update_inter_block_stats(const AV1_COMMON *const cm,
                                            TileDataEnc *tile_data,
                                            ThreadData *td, RD_COUNTS *rdc) {
  MACROBLOCK *x = &td->mb;
  MACROBLOCKD *const xd = &x->e_mbd;
  ...
  if (has_second_ref(mbmi)) {
    ...
    if (comp_ref_type == UNIDIR_COMP_REFERENCE) {
...
#if CONFIG_ENTROPY_STATS
        counts
            ->uni_comp_ref[av1_get_pred_context_uni_comp_ref_p1(xd)][1][bit1]++;
#endif  // CONFIG_ENTROPY_STATS
        if (bit1) {
          if (allow_update_cdf) {
      ...
      if (allow_update_cdf) update_cdf(av1_get_pred_cdf_comp_ref_p(xd), bit, 2);
#if CONFIG_ENTROPY_STATS
      ...
      if (ref1 != ALTREF_FRAME) {
        if (allow_update_cdf) {
          update_cdf(av1_get_pred_cdf_comp_bwdref_p1(xd), ref1 == ALTREF2_FRAME,
                     2);
        }
#if CONFIG_ENTROPY_STATS
    ...
    if (bit) {
      ...
      if (ref0 != ALTREF_FRAME) {
        if (allow_update_cdf) {
          update_cdf(av1_get_pred_cdf_single_ref_p6(xd), ref0 == ALTREF2_FRAME,
                     2);
        }
#if CONFIG_ENTROPY_STATS
  ...
  if (cm->seq_params.enable_interintra_compound &&
    ...
    if (mbmi->ref_frame[1] == INTRA_FRAME) {
...
#endif  // CONFIG_ENTROPY_STATS
      if (allow_update_cdf) update_cdf(fc->interintra_cdf[bsize_group], 1, 2);
#if CONFIG_ENTROPY_STATS
      ...
      if (allow_update_cdf) {
        ...
        if (av1_enable_derived_intra_mode(xd, bsize)) {
          update_cdf(
              get_derived_intra_mode_cdf(fc, xd->above_mbmi, xd->left_mbmi, 1),
              mbmi->use_derived_intra_mode[0], 2);
        }
        if (!mbmi->use_derived_intra_mode[0])
#endif  // CONFIG_DERIVED_INTRA_MODE
...
#endif  // CONFIG_ENTROPY_STATS
      if (allow_update_cdf) update_cdf(fc->interintra_cdf[bsize_group], 0, 2);
    }
  }
...
static INLINE void update_stats(const AV1_COMMON *const cm,
  ...
  if (!frame_is_intra_only(cm)) {
    ...
    if (!seg_ref_active) {
      ...
      if (inter_block) {
        update_inter_block_stats(cm, tile_data, td, rdc);
      }
    }
...
static INLINE void update_txfm_count(MACROBLOCK *x, MACROBLOCKD *xd,
...
static INLINE void tx_partition_count_update(
...
static INLINE void tx_partition_set_contexts(const AV1_COMMON *const cm,
                                             MACROBLOCKD *xd,
