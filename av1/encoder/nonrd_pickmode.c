...
static AOM_FORCE_INLINE bool skip_inter_mode_nonrd(
  ...
  if (x->sb_me && x->sb_me_tested == 0) {
    if (*this_mode == NEARESTMV && *ref_frame == LAST_FRAME &&
    ...
    if (*this_mode == NEARMV && *ref_frame == LAST_FRAME &&
...
static AOM_FORCE_INLINE bool handle_inter_mode_nonrd(
  ...
  if (this_mode == NEWMV && !force_mv_inter_layer && (x->sb_me_tested == 1 || !x->sb_me)) {
#if COLLECT_NONRD_PICK_MODE_STAT
  ...
  } else if (x->sb_me && x->sb_me_tested == 0 && this_mode == NEWMV && ref_frame == LAST_FRAME) {
  ...
  x->ms_stat_nonrd.nonskipped_search_times[bsize][this_mode] +=
   ...
   if (ref_frame == LAST_FRAME &&
       search_state->frame_mv[this_best_mode][ref_frame].as_mv.col == x->sb_me_mv_col &&
       search_state->frame_mv[this_best_mode][ref_frame].as_mv.row == x->sb_me_mv_row)
    x->sb_me_tested = 1;
