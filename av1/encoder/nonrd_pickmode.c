...
static AOM_INLINE bool prune_nearest_nearestmv_with_nearestmv(
    MV_REFERENCE_FRAME ref_frame, MV_REFERENCE_FRAME ref_frame2,
    const int_mv (*frame_mv)[REF_FRAMES],
  ...
  if (mode_checked[NEARESTMV][ref_frame] &&
      frame_mv[NEARESTMV][ref_frame].as_int ==
          frame_mv[NEAREST_NEARESTMV][ref_frame].as_int &&
      vars[INTER_OFFSET(NEARESTMV)][ref_frame] < UINT_MAX) {
    first_ref_valid = true;
    first_ref_bad =
        previous_mode_performed_poorly(NEARESTMV, ref_frame, vars, uv_dist);
  }
  if (mode_checked[NEARESTMV][ref_frame2] &&
      frame_mv[NEARESTMV][ref_frame2].as_int ==
          frame_mv[NEAREST_NEARESTMV][ref_frame2].as_int &&
      vars[INTER_OFFSET(NEARESTMV)][ref_frame2] < UINT_MAX) {
    second_ref_valid = true;
    second_ref_bad =
        previous_mode_performed_poorly(NEARESTMV, ref_frame2, vars, uv_dist);
  }
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes + tot_num_comp_modes; ++idx) {
    ...
    if (cpi->sf.rt_sf.prune_nearest_nearestmv_with_nearestmv &&
        this_mode == NEAREST_NEARESTMV &&
        prune_nearest_nearestmv_with_nearestmv(ref_frame, ref_frame2, frame_mv,
                                               mode_checked, vars, uv_dist)) {
      continue;
    }
    ...
    if (enable_filter_search && !force_mv_inter_layer && !comp_pred &&
      ...
      if (cpi->sf.rt_sf.prune_global_globalmv_with_zeromv &&
          this_mode == GLOBAL_GLOBALMV && use_model_yrd_large) {
        var_threshold =
            AOMMIN(var_threshold, vars[INTER_OFFSET(GLOBALMV)][ref_frame]);
        var_threshold =
            AOMMIN(var_threshold, vars[INTER_OFFSET(GLOBALMV)][ref_frame2]);
      }
      if (use_model_yrd_large) {
      ...
      if (this_mode == GLOBAL_GLOBALMV && var > var_threshold) {
