...
static INLINE void find_predictors(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  if (yv12 != NULL) {
    ...
    frame_mv[NEARMV][ref_frame] =
        av1_find_best_ref_mv_from_stack(mbmi_ext,
#if CONFIG_NEW_REF_SIGNALING && USE_NEW_REF_SIGNALING
                                        ref_frame_nrs,
#else
                                        ref_frame,
    ...
    if (!av1_is_scaled(sf) && bsize >= BLOCK_8X8 &&
        !(force_skip_low_temp_var && ref_frame_nrs > 1)) {
      av1_mv_pred(cpi, x, yv12_mb[ref_frame_nrs][0].buf, yv12->y_stride,
                  ref_frame_nrs, bsize);
    }
#else
    if (!av1_is_scaled(sf) && bsize >= BLOCK_8X8 &&
...
static AOM_INLINE void get_ref_frame_use_mask(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  use_alt_ref_frame =
      cpi->common.ref_frame_flags_nrs & (1 << altref_frame) ? use_alt_ref_frame : 0;
  use_golden_ref_frame =
      cpi->common.ref_frame_flags & (1 << golden_frame) ? use_golden_ref_frame : 0;
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (MV_REFERENCE_FRAME_NRS ref_frame_iter_nrs = 0;
       ref_frame_iter_nrs < cm->new_ref_frame_data.n_total_refs; ++ref_frame_iter_nrs) {
    MV_REFERENCE_FRAME_NRS ref_frame_iter =
...
#if CONFIG_NEW_REF_SIGNALING
  const MV_REFERENCE_FRAME_NRS last_frame = get_closest_pastcur_ref_index(cm);
  const MV_REFERENCE_FRAME_NRS golden_frame = cm->new_ref_frame_data.past_refs[0];
  ...
  for (int idx = 0; idx < num_inter_modes; ++idx) {
    ...
    ref_frame = convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data,
                                                      ref_frame_nrs);
#else
