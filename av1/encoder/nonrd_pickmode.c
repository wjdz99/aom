...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes + tot_num_comp_modes; ++idx) {
    ...
    // av1_setup_scale_factors_for_frame() below, as the prediction reference and
    // source are now the same resolution).
    // For compound prediction only do this if use_scaled_ref_frame[] is set for
    // both references.
    if (search_state.use_scaled_ref_frame[ref_frame] &&
        (is_single_pred || search_state.use_scaled_ref_frame[ref_frame2])) {
      struct scale_factors sf;
      av1_setup_scale_factors_for_frame(&sf, xd->plane[0].pre[0].width,
                                        xd->plane[0].pre[0].height, cm->width, cm->height);
