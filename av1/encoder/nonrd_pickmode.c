...
static INLINE void find_predictors(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  if (yv12 != NULL) {
    ...
    frame_mv[NEARMV][ref_frame] =
        av1_find_best_ref_mv_from_stack(mbmi_ext, ref_frame,
                                        cm->features.fr_mv_precision);
#else
    av1_find_best_ref_mvs_from_stack(mbmi_ext, ref_frame,
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (MV_REFERENCE_FRAME_NRS ref_frame_iter_nrs = 0;
    ...
    if (use_ref_frame_mask[ref_frame_iter_nrs]) {
      find_predictors(cpi, x, ref_frame_iter_nrs, frame_mv,
  ...
  for (int idx = 0; idx < num_inter_modes; ++idx) {
    ...
    if (skip_mode_by_low_temp(this_mode, ref_frame_nrs, bsize, x->content_state_sb,
                              frame_mv[this_mode][ref_frame_nrs],
                              force_skip_low_temp_var))
      continue;
#else
...
#if CONFIG_NEW_REF_SIGNALING
    if (ref_frame_nrs == last_frame && frame_mv[this_mode][ref_frame_nrs].as_int == 0) {
#else
