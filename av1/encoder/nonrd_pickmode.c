...
static AOM_INLINE int setup_compound_params_from_comp_idx(
    ...
    const int *use_ref_frame_mask, int comp_index, bool comp_use_zero_zero_only,
    MV_REFERENCE_FRAME *last_comp_ref_frame) {
  ...
  *ref_frame2 = rf[1];
  ...
  if (comp_use_zero_zero_only && *this_mode != GLOBAL_GLOBALMV) {
    return 0;
  }
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  bool comp_use_zero_zero_only = 0;
  ...
  for (int idx = 0; idx < num_inter_modes + tot_num_comp_modes; ++idx) {
    ...
    if (idx >= num_inter_modes) {
      ...
      if (!setup_compound_params_from_comp_idx(
              ...
              use_ref_frame_mask, comp_index, comp_use_zero_zero_only,
              &last_comp_ref_frame)) {
        continue;
      }
      comp_pred = 1;
