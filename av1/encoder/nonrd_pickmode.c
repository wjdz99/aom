...
static int combined_motion_search(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  //   mvp_full = av1_get_ref_mv_from_stack(0, mi->ref_frame,
  //                                        x->mv_best_ref_index[ref],
  //                                        x->mbmi_ext)
  //                  .as_mv;
  // else
  mvp_full = ref_mv;
...
static INLINE void init_mbmi(MB_MODE_INFO *mbmi, int mode_index,
  ...
  mbmi->num_proj_ref = 1;
  mbmi->interintra_mode = 0;  //(INTERINTRA_MODE)(II_DC_PRED - 1);
...
void av1_nonrd_pick_inter_mode_sb_fast(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (idx = 0; idx < num_inter_modes; ++idx) {
    ...
    */
    ...
    const int noskip_cost = x->skip_cost[skip_ctx][0];
    if (!this_early_term) {
      ...
      if (is_skippable) {
        ...
        // set_skip_flag(x, &this_rdc, bsize, sse_y);
      } else {
    ...
    */
    ...
    if (0) {  // cpi->oxcf.rc_mode == AOM_CBR && cpi->oxcf.speed >= 5) {
  ...
  //                           cpi->sf.adaptive_rd_thresh, bsize, mi->mode);

  store_coding_context(x, ctx, mi->mode, x->skip);
  // printf ("BLK %dx%d %d, MBMI skip %d, mode %d, MMode %d, MV %dx%d\n",
  // mi_row, mi_col, bsize, mi->skip, mi->mode, mi->motion_mode,
