...
void set_color_sensitivity(const AV1_COMP *cpi, MACROBLOCK *x, MACROBLOCKD *xd,
                           BLOCK_SIZE bsize, int y_sad, unsigned int source_variance) {
  x->color_sensitivity[0] = 0;
  x->color_sensitivity[1] = 0;
  if (cpi->oxcf.tool_cfg.enable_monochrome) return;
  const int factor = (bsize >= BLOCK_32X32) ? 2 : 3;
  int norm_sad = y_sad >> (b_width_log2_lookup[bsize] + b_height_log2_lookup[bsize]);
  // If the per-pixel spatial source variance is high and y_sad is low, then y-channel
  // is likely good for mode estimation, and so keep color_sen off.
  if (source_variance > 1000 && norm_sad < 48) return;
  for (int i = 1; i <= 2; ++i) {
    struct macroblock_plane *const p = &x->plane[i];
    struct macroblockd_plane *const pd = &xd->plane[i];
    const BLOCK_SIZE bs =
    ...
    const int uv_sad = cpi->ppi->fn_ptr[bs].sdf(p->src.buf, p->src.stride,
                                                pd->dst.buf, pd->dst.stride);
    x->color_sensitivity[i - 1] = uv_sad > (factor * (y_sad >> 3));
  }
}
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes; ++idx) {
    ...
    if (idx == 0) {
      ...
      // vector closest to 0; the uv-sad computed below is for zeromv.
      int y_sad = x->pred_mv0_sad[LAST_FRAME];
      if (x->pred_mv1_sad[LAST_FRAME] != INT_MAX &&
        (abs(frame_mv[NEARMV][LAST_FRAME].as_mv.col) + abs(frame_mv[NEARMV][LAST_FRAME].as_mv.row))
        < (abs(frame_mv[NEARESTMV][LAST_FRAME].as_mv.col) + abs(frame_mv[NEARESTMV][LAST_FRAME].as_mv.row)))
