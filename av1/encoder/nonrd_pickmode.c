...
static INLINE void find_predictors(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  if (yv12 != NULL) {
    ...
    frame_mv[NEAREST_NEARESTMV][ref_frame] = frame_mv[NEARESTMV][ref_frame];
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  int_interpfilters filter_last_nearest =
  ...
  for (int idx = 0; idx < num_inter_modes + num_comp_modes; ++idx) {
    ...
    if (idx == num_inter_modes) {
      ...
      comp_pred = 1;
      if (!use_ref_frame_mask[GOLDEN_FRAME]) continue;
    } else {
    ...
    mi->ref_frame[0] = ref_frame;
    mi->ref_frame[1] = NONE_FRAME;
    if (comp_pred) mi->ref_frame[1] = ref_frame2;
    ...
    mi->mv[1].as_int = 0;
    if (comp_pred) {
      mi->mv[1].as_int = frame_mv[this_mode][ref_frame2].as_int;
    }
...
#endif
    if (enable_filter_search && !comp_pred &&
      ...
      if (comp_pred) mi->interp_filters = filter_last_nearest;
      av1_enc_build_inter_predictor_y(xd, mi_row, mi_col);
      if (use_model_yrd_large) {
    ...
    if (this_mode == NEARESTMV && ref_frame == LAST_FRAME && !comp_pred)
      filter_last_nearest = mi->interp_filters;
    ...
    if (this_rdc.rdcost < best_rdc.rdcost) {
      ...
      if (comp_pred) {
        best_pickmode.best_mode = NEAREST_NEARESTMV;
      }
    } else {
  ...
  mi->skip_mode = 0;
  if (best_pickmode.best_mode == NEAREST_NEARESTMV) {
