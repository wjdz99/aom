...
static void estimate_single_ref_frame_costs(const AV1_COMMON *cm,
  ...
  if (seg_ref_active) {
    ...
    ref_costs_single[INTRA_FRAME_NRS] = 
     ...
     const int n_refs = cm->new_ref_frame_data.n_total_refs;
      ...
      for (int i = 0; i < n_refs - 1; i++) {
        for (int j = i; j < n_refs - 1; j++) {
          aom_cdf_prob ctx = 
            av1_get_single_ref_pred_context_nrs(xd, i, n_refs);
          const int bit = i == j;
          ref_costs_single[j] = 
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
...
#if CONFIG_NEW_REF_SIGNALING
    this_rdc.rate += ref_costs_single[ref_frame_nrs];//sarahparker_debug
#else
  ...
  estimate_intra_mode(cpi, x, bsize, use_modeled_non_rd_cost, best_early_term,
                      ref_costs_single[INTRA_FRAME_NRS], reuse_inter_pred,
