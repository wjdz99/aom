...
// mode
static const REF_MODE ref_mode_set_[NUM_INTER_MODES] = {
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  const REF_MODE *const ref_mode_set = ref_mode_set_;
  bool check_globalmv = true;
  PRED_BUFFER tmp[4];
  ...
  for (int idx = 0; idx < num_inter_modes + tot_num_comp_modes; ++idx) {
    ...
    for (PREDICTION_MODE inter_mv_mode = NEARESTMV; inter_mv_mode <= NEWMV;
      ...
      if (!comp_pred && ((mode_checked[inter_mv_mode][ref_frame] &&
                         ...
                         (this_mode == GLOBALMV && !check_globalmv))) {
        skip_this_mv = 1;
        break;
      }
    }
