...
static void estimate_intra_mode(
  ...
  // inter_layer_ref (previous spatial layer) is chosen as best reference;
  // only for temporal enhancement layer and on non-key frames.
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes; ++idx) {
    ...
    if (cpi->use_svc && ref_frame == inter_layer_ref) {
