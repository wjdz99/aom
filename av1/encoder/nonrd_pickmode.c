...
static void store_coding_context(MACROBLOCK *x, PICK_MODE_CONTEXT *ctx,
                                 PREDICTION_MODE mode, MV_REFERENCE_FRAME *refs) {
#else
static void store_coding_context(MACROBLOCK *x, PICK_MODE_CONTEXT *ctx) {
...
#if CONFIG_NEW_REF_SIGNALING
  int mode_rd_thresh =
      best_skip ? rd_threshes[mode] << 1 : rd_threshes[mode];
#else
  ...
  if (cpi->sf.inter_sf.adaptive_rd_thresh) {
    if (best_pickmode.best_ref_frame == INTRA_FRAME) {
      ...
      for (int i = 0; i < intra_modes; i++) {
        update_thresh_freq_fact(cpi, x, bsize, INTRA_FRAME,
                                intra_mode_list[i], best_pickmode.best_ref_frame, mi->mode);
      }
    } else {
      PREDICTION_MODE this_mode;
      for (this_mode = NEARESTMV; this_mode <= NEWMV; ++this_mode) {
        update_thresh_freq_fact(cpi, x, bsize, best_pickmode.best_ref_frame,
                                this_mode, best_pickmode.best_ref_frame, mi->mode);
      }
    }
  }
