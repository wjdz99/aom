...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  if (is_mode_intra(best_pickmode.best_mode) &&
      best_pickmode.best_mode_skip_txfm != 1 && cpi->rc.high_source_sad) {
    ...
    if (try_palette) {
      ...
      this_rdc.skip_txfm = av1_search_palette_mode_luma(
          cpi, x, bsize, intra_ref_frame_cost, ctx, &this_rdc, best_rdc.rdcost);
      if (this_rdc.rdcost < best_rdc.rdcost) {
  ...
  pd->dst = orig_dst;
  mi->palette_mode_info = best_pickmode.pmi;
  mi->mode = best_pickmode.best_mode;
  mi->ref_frame[0] = best_pickmode.best_ref_frame;
  mi->ref_frame[1] = best_pickmode.best_second_ref_frame;
