...
bool select_block_subpel_search_method(MACROBLOCK *x, int adaptive_subpel_search_level, const bool fullpel_performed_well) {
  ...
  if (adaptive_subpel_search_level == 2 && qband != 0 &&
      (is_fullpel_well_or_lowsad || x->source_variance < 220))
    return true;
...
static int combined_motion_search(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  *rate_mv = av1_mv_bit_cost(&mvp_full, &ref_mv, x->mv_costs->nmv_joint_cost,
  ...
  if (rv && search_subpel) {
    ...
    if (select_block_subpel_search_method(x, sf->rt_sf.adaptive_subpel_search_level, fullpel_performed_well))
