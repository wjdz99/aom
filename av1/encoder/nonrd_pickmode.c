...
static AOM_INLINE void get_ref_frame_use_mask(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  // For spatial layers: enable golden ref it is set by user and corresponds
  // to lower spatial layer. Enable it based on source content for now.
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes + tot_num_comp_modes; ++idx) {
    ...
    // For screen content: for enhancement spatial layers avoid
    // GOLDEN ref and if base layer is KEY frame.
    if (cpi->oxcf.tune_cfg.content == AOM_CONTENT_SCREEN &&
