...
static AOM_INLINE int skip_mode_by_threshold(
  ...
  int mode_rd_thresh = best_skip ? rd_threshes[mode_index] << extra_shift
  ...
  if (ref_frame != LAST_FRAME) {
    ...
    if (ref_frame == GOLDEN_FRAME && frames_since_golden > 4)
      mode_rd_thresh = mode_rd_thresh << 2;
  }
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  const int agressive_skip = cpi->sf.rt_sf.nonrd_agressive_skip &&
  ...
  for (int idx = 0; idx < num_inter_modes; ++idx) {
    ...
    if (best_early_term && (idx > 0 || agressive_skip)) {
