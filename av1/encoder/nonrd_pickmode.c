...
static const REF_MODE ref_mode_set_reduced[NUM_INTER_MODES_REDUCED] = {
  { LAST_FRAME, NEARESTMV },   { LAST_FRAME, NEARMV },
  { LAST_FRAME, NEWMV },       { LAST_FRAME, GLOBALMV },
  { GOLDEN_FRAME, NEARESTMV }, { GOLDEN_FRAME, NEARMV },
  { GOLDEN_FRAME, NEWMV },     { GOLDEN_FRAME, GLOBALMV },
};
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  unsigned int global_mv_var[REF_FRAMES];
  for (int idx = 0; idx < REF_FRAMES; idx++) {
    global_mv_var[idx] = UINT_MAX;
  }
#if CONFIG_AV1_TEMPORAL_DENOISING
  ...
  for (int idx = 0; idx < num_inter_modes + tot_num_comp_modes; ++idx) {
    ...
    if (comp_pred) {
      ...
      if (this_mode != GLOBALMV && frame_mv[this_mode][ref_frame].as_int ==
        ...
        int this_mode_cost =
            rate_mv +
            single_inter_mode_costs[INTER_OFFSET(this_mode)][ref_frame];
        if (this_mode == NEWMV || this_mode == NEARMV) {
          this_mode_cost +=
              get_drl_cost(NEWMV, 0, mbmi_ext, mode_costs->drl_mode_cost0,
                           av1_ref_frame_type(mi->ref_frame));
        }
        if (this_mode_cost > globalmv_mode_cost) {
          this_best_mode = GLOBALMV;
        }
        if (use_model_yrd_large && var < UINT_MAX) {
          global_mv_var[ref_frame] = var;
        }
      }
