...
static AOM_FORCE_INLINE bool handle_inter_mode_nonrd(
    ...
    int cb_pred_filter_search, int reuse_inter_pred, int *sb_me_is_tested) {
  AV1_COMMON *const cm = &cpi->common;
  MACROBLOCKD *const xd = &x->e_mbd;
  MB_MODE_INFO *const mi = xd->mi[0];
  ...
  if (this_mode == NEWMV && !force_mv_inter_layer &&
      (!x->sb_me_block || ref_frame > LAST_FRAME)) {
#if COLLECT_NONRD_PICK_MODE_STAT
  ...
  } else if (x->sb_me_block && this_mode == NEWMV && ref_frame == LAST_FRAME) {
  ...
  if (*best_early_term && (idx > 0 || rt_sf->nonrd_aggressive_skip)) {
    ...
    if (!x->sb_me_block || *sb_me_is_tested) return false;
  }
  return true;
}
