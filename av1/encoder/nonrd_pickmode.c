...
static INLINE int subpel_select(BLOCK_SIZE bsize, int_mv *mv, int subpel_orig,
                                int avg_frame_low_motion) {
  int mv_thresh = 4;
  mv_thresh = (bsize > BLOCK_32X32) ? 2 : (bsize > BLOCK_16X16) ? 4 : 6;
  if (avg_frame_low_motion > 0 && avg_frame_low_motion < 40) mv_thresh = 12;
  ...
  if (rv && search_subpel) {
    ...
    if (cpi->sf.rt_sf.force_half_pel_block &&
      ...
      ms_params.forced_stop =
          subpel_select(bsize, tmp_mv, cpi->sf.mv_sf.subpel_force_stop,
  ...
  if (ref_frame > LAST_FRAME && cpi->oxcf.rc_cfg.mode == AOM_CBR &&
    ...
    *rate_mv = av1_mv_bit_cost(&frame_mv[NEWMV][ref_frame].as_mv, &ref_mv,
    ...
    if (cpi->sf.rt_sf.force_half_pel_block &&
        cpi->sf.mv_sf.subpel_force_stop < HALF_PEL)
      ms_params.forced_stop =
          subpel_select(bsize, &best_mv, cpi->sf.mv_sf.subpel_force_stop,
                        cpi->rc.avg_frame_low_motion);
