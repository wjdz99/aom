...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes + tot_num_comp_modes; ++idx) {
    ...
    // block_ref_scale_factors[]. For compound prediction only do this if
    // use_scaled_ref_frame[] is set for both references.
  ...
  if (!is_inter_block(mi)) {
    ...
    // scaled reference frame, then don't use reuse_inter_pred.
