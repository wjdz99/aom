...
static void set_compound_mode(int idx, int num_inter_modes, int ref_frame,
                              int ref_frame2, int ref_mv_idx, MACROBLOCK *x,
                              int_mv frame_mv[MB_MODE_COUNT][REF_FRAMES],
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes + num_comp_modes; ++idx) {
    ...
    if (idx >= num_inter_modes) {
      ...
      set_compound_mode(idx, num_inter_modes, ref_frame, ref_frame2, ref_mv_idx,
                        x, frame_mv, &this_mode);
