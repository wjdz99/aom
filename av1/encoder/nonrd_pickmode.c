    ...
    *early_term = 0;
    ...
    if (ac_test && dc_test) {
      ...
      for (int i = 1; i <= 2; i++) {
        ...
        const TX_SIZE uv_tx_size = av1_get_tx_size(i, xd);
        ...
        const uint32_t uv_dc_thr =
           (puv->dequant_QTX[0] * puv->dequant_QTX[0]) >> (6 - sf);
        const uint32_t uv_ac_thr =
           (puv->dequant_QTX[1] * puv->dequant_QTX[1]) >> (6 - sf);
        int j = i - 1;
  ...
  for (i = 0; i < FILTER_SEARCH_SIZE; ++i) {
    ...
    if (use_model_yrd_large)
      model_skip_for_sb_y_large(cpi, bsize, mi_row, mi_col, x, xd, &pf_rate[i],
                                &pf_dist[i], &pf_var[i], &pf_sse[i],this_early_term,
