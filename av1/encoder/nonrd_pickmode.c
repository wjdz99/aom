...
void av1_nonrd_pick_intra_mode(AV1_COMP *cpi, MACROBLOCK *x, RD_STATS *rd_cost,
  ...
  #if CONFIG_DERIVED_INTRA_MODE
  const int is_dr_cost = x->mode_costs.kf_is_dr_mode_cost[above_ctx][left_ctx][1];
  const int is_none_dr_cost = x->mode_costs.kf_is_dr_mode_cost[above_ctx][left_ctx][0];
  ...
  for (int i = 0; i < INTRA_MODES; ++i) {
    ...
    if (is_dr) {
      ...
      cost[i] = is_dr_cost + x->mode_costs.kf_dr_mode_cost[above_ctx][left_ctx][index];
    } else {
      const int index = none_dr_mode_to_index[i];
      cost[i] =
          is_none_dr_cost + x->mode_costs.kf_none_dr_mode_cost[above_ctx][left_ctx][index];
    }
  }
  bmode_costs = cost;
#else
