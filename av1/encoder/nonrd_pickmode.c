...
static const INTER_FILTER filters_ref_set[9] = {
  ...
  { EIGHTTAP_REGULAR, EIGHTTAP_SMOOTH },
  { EIGHTTAP_SMOOTH, EIGHTTAP_REGULAR },
  { MULTITAP_SHARP,  MULTITAP_SHARP },
  { EIGHTTAP_REGULAR, MULTITAP_SHARP },
  { MULTITAP_SHARP, EIGHTTAP_REGULAR },
...
static void search_filter_ref(AV1_COMP *cpi, MACROBLOCK *x, RD_STATS *this_rdc,
  ...
  int dim = (cpi->sf.interp_sf.disable_dual_filter == 0) ? FILTER_SEARCH_SIZE : 1;
  ...
  InterpFilter filter_x, filter_y;
  int best_skip = 0;
  ...
  for (int i = 0; i < FILTER_SEARCH_SIZE * FILTER_SEARCH_SIZE; ++i) {
    int64_t cost;
    filter_x = filters_ref_set[i].filter_x;
    filter_y = filters_ref_set[i].filter_y;
    if (cpi->sf.interp_sf.disable_dual_filter && filter_x != filter_y)
      continue;
    mi->interp_filters.as_filters.x_filter = filter_x;
    mi->interp_filters.as_filters.y_filter = filter_y;
    av1_enc_build_inter_predictor_y(xd, mi_row, mi_col);
  ...
  assert(best_filter_index >= 0 &&
         best_filter_index <  FILTER_SEARCH_SIZE * FILTER_SEARCH_SIZE);
  if (reuse_inter_pred && *this_mode_pred != current_pred)
    free_pred_buffer(current_pred);

  filter_x = filters_ref_set[best_filter_index].filter_x;
  filter_y = filters_ref_set[best_filter_index].filter_y;
  mi->interp_filters.as_filters.x_filter = filter_x;
  mi->interp_filters.as_filters.y_filter = filter_y;
  ...
  *this_early_term = best_early_term;
  ...
  } else if (best_filter_index < dim * FILTER_SEARCH_SIZE - 1) {
    av1_enc_build_inter_predictor_y(xd, mi_row, mi_col);
  }
}
