...
static bool should_prune_intra_modes_using_nb_info(
    const MACROBLOCKD *xd, bool enable_intra_mode_pruning_using_nb_info,
    PREDICTION_MODE this_mode, PREDICTION_MODE above_mode,
    PREDICTION_MODE left_mode) {
  if (!enable_intra_mode_pruning_using_nb_info) return false;
  ...
  if (xd->up_available && this_mode != above_mode && xd->left_available &&
      this_mode != left_mode) {
    return true;
  } else {
    return false;
...
void av1_nonrd_pick_intra_mode(AV1_COMP *cpi, MACROBLOCK *x, RD_STATS *rd_cost,
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (should_prune_intra_modes_using_nb_info(
            xd, cpi->sf.rt_sf.enable_intra_mode_pruning_using_nb_info,
            this_mode, A, L)) {
