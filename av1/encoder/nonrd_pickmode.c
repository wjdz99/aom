...
static INLINE void set_early_term_based_on_uv_plane(
    ...
    MACROBLOCKD *xd, int mi_row, int mi_col, int *early_term, int num_blk,
    const unsigned int *sse_tx, const unsigned int *var_tx, int sum,
    unsigned int var, unsigned int sse) {
...
static void model_skip_for_sb_y_large(AV1_COMP *cpi, BLOCK_SIZE bsize,
  ...
  *early_term = 0;
  ...
  if (test_skip) {
    ...
    set_early_term_based_on_uv_plane(cpi, x, bsize, xd, mi_row,
                                     mi_col, early_term, num_blks, sse_tx,
                                     var_tx, sum, var, sse);
  }
