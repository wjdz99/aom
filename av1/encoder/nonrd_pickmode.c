...
static int combined_motion_search(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  *tmp_mv = x->best_mv;
  ...
  MvSubpelPrecision precision = get_mv_precision(mvp_full);
#else
  ...
  *rate_mv = av1_mv_bit_cost(
  ...
  if (rv && search_subpel) {
    ...
    *tmp_mv = x->best_mv;
#if CONFIG_FLEX_MVRES
    MvSubpelPrecision precision2 = get_mv_precision(tmp_mv->as_mv);
#else
    MvSubpelPrecision precision2 = mi->mv_precision;
#endif  // CONFIG_FLEX_MVRES
    *rate_mv = av1_mv_bit_cost(
