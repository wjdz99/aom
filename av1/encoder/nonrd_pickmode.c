...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  if (cpi->sf.rt_sf.nonrd_prune_ref_frame_search == 1)
    thresh_sad_pred = 3 * ((int64_t)x->pred_mv_sad[LAST_FRAME]);
  else if (cpi->sf.rt_sf.nonrd_prune_ref_frame_search == 2)
    thresh_sad_pred = 2 * ((int64_t)x->pred_mv_sad[LAST_FRAME]);
