...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes + tot_num_comp_modes; ++idx) {
    ...
    if (cpi->sf.rt_sf.prune_compoundmode_with_singlemode_var && comp_pred) {
      if (prune_compoundmode_with_singlemode_var(this_mode, ref_frame,
                                                 ref_frame2, frame_mv,
    ...
    // If the segment reference frame feature is enabled then do nothing if
    // the current ref frame is not allowed.
    ...
    if (idx == 0 && !skip_pred_mv) {
      ...
      // Use y-sad already computed in find_predictors: take the sad with
      // motion vector closest to 0; the uv-sad computed below in
      // set_color_sensitivity is for zeromv.
    ...
    if (enable_filter_search && !force_mv_inter_layer && !comp_pred &&
      ...
      // search_filter_ref() for all blocks, then check top and left values
      // and force smooth if both were selected to be smooth.
      if (cpi->sf.interp_sf.cb_pred_filter_search &&
          (mi->mv[0].as_mv.row & 0x07 || mi->mv[0].as_mv.col & 0x07)) {
      ...
      if (cpi->sf.rt_sf.prune_compoundmode_with_singlecompound_var &&
        ...
        const PREDICTION_MODE single_mode1 = compound_ref0_mode(this_mode);
        var_threshold =
