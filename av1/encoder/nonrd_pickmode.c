...
void set_color_sensitivity(const AV1_COMP *cpi, MACROBLOCK *x, MACROBLOCKD *xd,
                           BLOCK_SIZE bsize, unsigned int source_variance) {
  // If spatial source variance (for y-channel) is very high, likley
  // we don't need to incorporrate color.
  if (cpi->oxcf.tool_cfg.enable_monochrome || source_variance > 10000) return;
  // Use y-sad already computed in find_predictors: note this the
  // best sad over nearest and near mv; whereas for uv-sad below the
  // sad is for zeromv.
  int y_sad = x->pred_mv_sad[LAST_FRAME];
  for (int i = 1; i <= 2; ++i) {
    int factor = (bsize >= BLOCK_32X32) ? 4 : (bsize <= BLOCK_8X8) ? 1 : 2;
    struct macroblock_plane *const p = &x->plane[i];
    struct macroblockd_plane *const pd = &xd->plane[i];
    const BLOCK_SIZE bs =
        get_plane_block_size(bsize, pd->subsampling_x, pd->subsampling_y);
    int uv_sad = cpi->ppi->fn_ptr[bs].sdf(p->src.buf, p->src.stride,
                                          pd->dst.buf, pd->dst.stride);
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes; ++idx) {
    ...
    if (idx == 0) set_color_sensitivity(cpi, x, xd, bsize, x->source_variance);
