...
static void set_compound_mode(MACROBLOCK *x,
                              int_mv frame_mv[MB_MODE_COUNT][REF_FRAMES],
                              int idx, int num_inter_modes, int ref_frame,
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes + num_comp_modes; ++idx) {
    ...
    if (idx >= num_inter_modes) {
      ...
      set_compound_mode(x, frame_mv, idx, num_inter_modes, ref_frame,
                        ref_frame2, ref_mv_idx, &this_mode);
