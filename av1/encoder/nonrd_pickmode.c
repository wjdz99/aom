...
static void set_compound_mode(MACROBLOCK *x, int comp_index, int ref_frame,
                              ...
                              int_mv frame_mv[MB_MODE_COUNT][REF_FRAMES],
                              int_mv frame_mv2[MB_MODE_COUNT][REF_FRAMES],
  ...
  *ref_frame_comp = av1_ref_frame_type(mi->ref_frame);
  if (comp_index % 3 == 0) {
    frame_mv[*this_mode][*ref_frame_comp].as_int = 0;
    frame_mv2[*this_mode][*ref_frame_comp].as_int = 0;
  } else if (comp_index % 3 == 1) {
    *this_mode = NEAREST_NEARESTMV;
    frame_mv[*this_mode][*ref_frame_comp].as_int =
        xd->ref_mv_stack[*ref_frame_comp][0].this_mv.as_int;
    frame_mv2[*this_mode][*ref_frame_comp].as_int =
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  int ref_mv_idx = 0;
  MV_REFERENCE_FRAME ref_frame_comp = 0;
#if CONFIG_AV1_TEMPORAL_DENOISING
  ...
  for (int idx = 0; idx < num_inter_modes + tot_num_comp_modes; ++idx) {
    ...
    if (idx >= num_inter_modes) {
      ...
      set_compound_mode(x, comp_index, ref_frame, ref_frame2, ref_mv_idx,
                        frame_mv, frame_mv2, &this_mode, &ref_frame_comp);
    ...
    if (cpi->ppi->use_svc && svc->spatial_layer_id > 0 && !comp_pred &&
    ...
    if (this_mode == NEWMV && !force_mv_inter_layer && !comp_pred) {
    ...
    mi->mv[1].as_int = 0;
    if (comp_pred) {
      mi->mv[0].as_int = frame_mv[this_mode][ref_frame_comp].as_int;
      mi->mv[1].as_int = frame_mv2[this_mode][ref_frame_comp].as_int;
    }
  ...
  if (best_pickmode.best_second_ref_frame > INTRA_FRAME) {
    ...
    ref_frame_comp = av1_ref_frame_type(mi->ref_frame);
    mi->mv[0].as_int = frame_mv[best_pickmode.best_mode][ref_frame_comp].as_int;
    mi->mv[1].as_int =
        frame_mv2[best_pickmode.best_mode][ref_frame_comp].as_int;
  }
