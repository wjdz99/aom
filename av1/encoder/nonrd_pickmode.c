...
#define DECLARE_BLOCK_YRD_VARS()                                           \
  /*  When s_tx_8x8_dual_applicable is true, we compute the txfm for the   \
   * entire bsize and write macroblock_plane::coeff. So low_coeff is kept as a        \
   *  non-const so we can reassign it to macroblock_plane::coeff. */       \
