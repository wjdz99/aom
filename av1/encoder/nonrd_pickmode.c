...
static int combined_motion_search(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  *tmp_mv = x->best_mv;
  ...
  const int use_flex_mv = is_flex_mv_precision_active(cm, NEWMV);
  const MvSubpelPrecision precision =
      use_flex_mv ? get_mv_precision(mvp_full) : cm->mv_precision;
#else
  const MvSubpelPrecision precision = cm->mv_precision;
#endif  // CONFIG_FLEX_MVRES

  *rate_mv = av1_mv_bit_cost(&mvp_full, &ref_mv, cm->mv_precision,
...
#else
    const MvSubpelPrecision precision2 = cm->mv_precision;
#endif  // CONFIG_FLEX_MVRES
    *rate_mv = av1_mv_bit_cost(&tmp_mv->as_mv, &ref_mv, cm->mv_precision,
                               x->nmv_vec_cost, x->nmvcost,
#if CONFIG_FLEX_MVRES
                               0, precision2, x->flex_mv_precision_costs,
#endif  // CONFIG_FLEX_MVRES
                               MV_COST_WEIGHT);
  }
...
static int search_new_mv(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  if (ref_frame > LAST_FRAME && gf_temporal_ref &&
    ...
    MvSubpelPrecision precision =
        use_flex_mv ? get_mv_precision(frame_mv[NEWMV][ref_frame].as_mv)
                    : cm->mv_precision;
#else
