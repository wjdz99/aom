...
static void estimate_intra_mode(
  ...
  if (x->source_variance < spatial_var_thresh) {
    ...
    if (x->content_state_sb.source_sad >= kHighSad) force_intra_check = 1;
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (i > 2 ||
        !(force_intra_check == 1 && best_pickmode->best_mode != DC_PRED)) {
      if (!((1 << this_mode) &
...
void set_color_sensitivity(AV1_COMP *cpi, MACROBLOCK *x, MACROBLOCKD *xd,
  ...
  for (int i = 1; i <= 2; ++i) {
    if (x->color_sensitivity[i - 1] == 2 || source_variance < 100) {
      struct macroblock_plane *const p = &x->plane[i];
      struct macroblockd_plane *const pd = &xd->plane[i];
      const BLOCK_SIZE bs =
          get_plane_block_size(bsize, pd->subsampling_x, pd->subsampling_y);
      ...
      if (source_variance < 100 && norm_sad > 40)
