...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (MV_REFERENCE_FRAME ref_frame_iter = LAST_FRAME;
    ...
    if (use_ref_frame_mask[ref_frame_iter] || num_comp_modes > 0) {
  ...
  if (num_comp_modes > 0) {
    MB_MODE_INFO *const mbmi = xd->mi[0];
    MB_MODE_INFO_EXT *const mbmi_ext = &x->mbmi_ext;
    av1_find_mv_refs(cm, xd, mbmi, REF_FRAMES + FWD_REFS * BWD_REFS + LAST_GOLDEN_FRAMES,
  ...
  for (int idx = 0; idx < num_inter_modes + num_comp_modes; ++idx) {
    ...
    if (idx >= num_inter_modes) {
      ...
      if (idx == num_inter_modes) {
        ...
        frame_mv[this_mode][ref_frame].as_int =
            xd->ref_mv_stack[ref_frame_comp][1].this_mv.as_int;
        frame_mv[this_mode][ref_frame2].as_int =
            xd->ref_mv_stack[ref_frame_comp][1].comp_mv.as_int;
      }
  ...
  mi->mv[1].as_int = 0;
  if (best_pickmode.best_mode == GLOBAL_GLOBALMV ||
      best_pickmode.best_mode == NEAREST_NEARESTMV ||
      best_pickmode.best_mode == NEAR_NEARMV) {
    mi->ref_frame[1] = best_pickmode.best_second_ref_frame;
    mi->mv[1].as_int =
        frame_mv[best_pickmode.best_mode][best_pickmode.best_second_ref_frame].as_int;
  }
