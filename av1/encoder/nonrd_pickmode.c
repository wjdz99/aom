  ...
  if (x->sb_me) {
    if (*ref_frame > LAST_FRAME) {
      ...
      if (*this_mode == NEARESTMV &&
          (search_state->frame_mv[NEARESTMV][LAST_FRAME].as_mv.col != x->sb_me_mv_col ||
           search_state->frame_mv[NEARESTMV][LAST_FRAME].as_mv.row != x->sb_me_mv_row))
        return true;
      if (*this_mode == NEARMV &&
          (search_state->frame_mv[NEARMV][LAST_FRAME].as_mv.col != x->sb_me_mv_col ||
           search_state->frame_mv[NEARMV][LAST_FRAME].as_mv.row != x->sb_me_mv_row))
        return true;
    }
    return false;
  }
...
static AOM_FORCE_INLINE void handle_screen_content_mode_nonrd(
  ...
  if (cm->seq_params->bit_depth == 8 &&
      ...
      !cpi->oxcf.txfm_cfg.use_inter_dct_only && !x->force_zeromv_skip_for_blk && !x->sb_me &&
