...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  int inter_motion_sc = cpi->rc.high_source_sad && bsize <= BLOCK_16X16 &&
      (abs(mi->mv[0].as_mv.col) > 20 || abs(mi->mv[0].as_mv.row) > 20);
  ...
  try_palette = try_palette &&
                (is_mode_intra(best_pickmode->best_mode) || inter_motion_sc) &&
