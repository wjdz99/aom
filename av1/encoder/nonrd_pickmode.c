...
static void block_variance(const uint8_t *src, int src_stride,
  ...
  *sum = 0;
  ...
  // the aom_get8x8var_quad() processes four 8x8 blocks (in a 8x32) per call.
  // Hence the width and height of the block need to be atleast 8 and 32 samples
  // respectively.
  assert(w >= 32);
  assert(h >= 8);
  for (int i = 0; i < h; i += block_size) {
    for (int j = 0; j < w; j += 32) {
      aom_get8x8var_quad(src + src_stride * i + j, src_stride,
