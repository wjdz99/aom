...
// TODO (fkyslov) Refine logig of selecting REF FRAME SET.
// For now only LAST_FRAME is used
  ...
  force_skip_low_temp_var = get_force_skip_low_temp_var(&x->variance_low[0], mi_row, mi_col,
                                  bsize);
  ...
  for (idx = 0; idx < num_inter_modes; ++idx) {
    ...
    */
    ...
    if (cpi->oxcf.rc_mode == AOM_CBR && cpi->oxcf.speed >= 5) {
