...
void av1_nonrd_pick_intra_mode(AV1_COMP *cpi, MACROBLOCK *x, RD_STATS *rd_cost,
  ...
  const int64_t thresh_dist = 20000;
  const int64_t best_dist_norm =
      best_rdc.dist >> (b_width_log2_lookup[bsize] + b_height_log2_lookup[bsize]);

  // Try palette if it's enabled.
  bool try_palette = best_dist_norm > thresh_dist &&
  ...
  if (try_palette) {
    ...
    if (this_rdc.rdcost < best_rdc.rdcost && this_rdc.rate < (3 * (best_rdc.rate >> 1))) {
      best_mode = DC_PRED;
      mi->mv[0].as_int = INVALID_MV;
