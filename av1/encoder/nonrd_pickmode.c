...
static void block_variance(const uint8_t *src, int src_stride,
  ...
  *sum = 0;
  ...
  assert(w >= 32);
  assert(h >= 8);
  for (int col = 0; col < h; col += block_size) {
    for (int row = 0; row < w; row += 32) {
      aom_get_var_sse_sum_8x8_quad(src + src_stride * col + row, src_stride,
                                   ref + ref_stride * col + row, ref_stride,
...
static void block_variance_16x16_dual(const uint8_t *src, int src_stride,
  ...
  *sum = 0;
  ...
  for (int col = 0; col < h; col += block_size) {
    for (int row = 0; row < w; row += 32) {
      aom_get_var_sse_sum_16x16_dual(src + src_stride * col + row, src_stride,
                                     ref + ref_stride * col + row, ref_stride,
...
static void calculate_variance(int bw, int bh, TX_SIZE tx_size,
  ...
  for (col = 0; col < nh; col += 2) {
    for (row = 0; row < nw; row += 2) {
      sse_o[k] = sse_i[col * nw + row] + sse_i[col * nw + row + 1] +
                 sse_i[(col + 1) * nw + row] + sse_i[(col + 1) * nw + row + 1];
      sum_o[k] = sum_i[col * nw + row] + sum_i[col * nw + row + 1] +
                 sum_i[(col + 1) * nw + row] + sum_i[(col + 1) * nw + row + 1];
...
static void estimate_intra_mode(
  ...
  for (int midx = 0; midx < 4; ++midx) {
    ...
    if (is_prune_intra_mode(cpi, midx, force_intra_check, bsize,
