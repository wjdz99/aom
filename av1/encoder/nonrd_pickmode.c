...
static INLINE int get_force_skip_low_temp_var(uint8_t *variance_low, int mi_row,
  ...
  switch (bsize) {
    ...
    case BLOCK_128X64:
      assert(mi_col % 32 == 0);
      force_skip_low_temp_var = variance_low[1 + (mi_row % 32 != 0)];
      break;
    case BLOCK_64X128:
      assert(mi_row % 32 == 0);
      force_skip_low_temp_var = variance_low[3 + (mi_col % 32 != 0)];
      break;
    case BLOCK_64X64:
    ...
    case BLOCK_32X64:
      ...
      break;
    case BLOCK_32X32:
      force_skip_low_temp_var = variance_low[33 + (idx64 << 2) + idx32];
      break;
    case BLOCK_32X16:
    case BLOCK_16X32:
    case BLOCK_16X16:
      force_skip_low_temp_var =
          variance_low[49 + (idx64 << 4) + (idx32 << 2) + idx16];
      break;
    default: break;
  }
