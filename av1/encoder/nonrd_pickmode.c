...
static INLINE int subpel_select(AV1_COMP *cpi, MACROBLOCK *x, BLOCK_SIZE bsize,
                                int_mv *mv) {
  assert(cpi->sf.rt_sf.adjust_mv_pel_precision);
  if (cpi->sf.rt_sf.adjust_mv_pel_precision >= 2) {
    int mv_thresh = 4;
    ...
    if (abs(mv->as_fullmv.row) >= mv_thresh ||
      ...
      return HALF_PEL;
  } else if (cpi->sf.rt_sf.adjust_mv_pel_precision >= 1) {
  ...
  *rate_mv = av1_mv_bit_cost(&mvp_full, &ref_mv, x->mv_costs->nmv_joint_cost,
  ...
  if (rv && search_subpel) {
    ...
    av1_make_default_subpel_ms_params(&ms_params, cpi, x, bsize, &ref_mv,
                                      cost_list);
    if (cpi->sf.rt_sf.adjust_mv_pel_precision &&
