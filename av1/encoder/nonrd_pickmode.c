...
void set_color_sensitivity(const AV1_COMP *cpi, MACROBLOCK *x, MACROBLOCKD *xd,
  ...
  // If the per-pixel spatial source variance is high and the normalized y_sad is low,
  // then y-channel is likely good for mode estimation, and so keep color_sen off.
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes; ++idx) {
    ...
    if (idx == 0) {
      // Set color sensitivity, on the first tested mode only.
