...
static INLINE void subpel_select(AV1_COMP *cpi, BLOCK_SIZE bsize, int_mv *mv,
                                 int subpel_orig) {
  if (!cpi->sf.rt_sf.force_half_pel_block || subpel_orig >= HALF_PEL) {
    cpi->sf.mv_sf.subpel_force_stop = subpel_orig;
  } else {
    int mv_thresh = 4;
...
static int combined_motion_search(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  *rate_mv = av1_mv_bit_cost(&mvp_full, &ref_mv, x->mv_costs->nmv_joint_cost,
  ...
  if (rv && search_subpel) {
    ...
    const int subpel_orig = cpi->sf.mv_sf.subpel_force_stop;
    subpel_select(cpi, bsize, tmp_mv, subpel_orig);
...
static int search_new_mv(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  if (ref_frame > LAST_FRAME && cpi->oxcf.rc_cfg.mode == AOM_CBR &&
    ...
    *rate_mv = av1_mv_bit_cost(&frame_mv[NEWMV][ref_frame].as_mv, &ref_mv,
    ...
    SUBPEL_MOTION_SEARCH_PARAMS ms_params;
    int subpel_orig = cpi->sf.mv_sf.subpel_force_stop;
    subpel_select(cpi, bsize, &best_mv, subpel_orig);
    av1_make_default_subpel_ms_params(&ms_params, cpi, x, bsize, &ref_mv,
                                      cost_list);
