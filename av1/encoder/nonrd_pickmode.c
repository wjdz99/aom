...
static INLINE int subpel_select(AV1_COMP *cpi, MACROBLOCK *x, BLOCK_SIZE bsize,
  ...
  if (rt_sf->reduce_mv_pel_precision_highmotion >= 3) {
    int mv_thresh = 4;
    ...
    if (abs(mv->as_fullmv.row) >= mv_thresh ||
      ...
      return HALF_PEL;
  } else if (rt_sf->reduce_mv_pel_precision_highmotion >= 1) {
    int mv_thresh;
    const int th_vals[2][3] = { { 4, 8, 10 }, { 4, 6, 8 } };
    const int th_idx = rt_sf->reduce_mv_pel_precision_highmotion - 1;
  ...
  // large areas
  if (rt_sf->reduce_mv_pel_precision_lowcomplex >= 2) {
  ...
  } else if (rt_sf->reduce_mv_pel_precision_lowcomplex >= 1) {
...
static AOM_FORCE_INLINE bool handle_inter_mode_nonrd(
  ...
  if (this_mode == NEWMV && cpi->oxcf.tune_cfg.content == AOM_CONTENT_SCREEN &&
    ...
    if (search_state->this_mv->as_int == 0 &&
        x->content_state_sb.source_sad_nonrd != kZeroSad &&
        ((x->color_sensitivity[0] == 0 && x->color_sensitivity[1] == 0) ||
         cpi->rc.high_source_sad) &&
        x->source_variance == 0)
  ...
  mi->mode = this_mode;
  mi->mv[0].as_int = search_state->this_mv->as_int;
  mi->mv[1].as_int = 0;
  if (comp_pred)
  ...
  if (is_mv_subpel && enable_filt_search_this_mode) {
    ...
    if (this_mode == NEWMV) {
      search_state->this_mv[0] = mi->mv[0];
    }
#endif
  } else {
    mi->interp_filters =
  ...
  if (ref_frame == LAST_FRAME && search_state->this_mv->as_int == 0) {
  ...
  if (!comp_pred && search_state->this_mv->as_int == 0 && var < UINT_MAX) {
    search_state->vars[INTER_OFFSET(GLOBALMV)][ref_frame] = var;
  }
...
void av1_nonrd_pick_inter_mode_sb(AV1_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (int idx = 0; idx < num_inter_modes + tot_num_comp_modes; ++idx) {
    ...
    if (skip_inter_mode_nonrd(
      ...
      continue;
    search_state.this_mv = &search_state.frame_mv[this_mode][ref_frame];
