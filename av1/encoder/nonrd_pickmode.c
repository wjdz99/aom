...
void set_color_sensitivity(AV1_COMP *cpi, MACROBLOCK *x, MACROBLOCKD *xd,
  ...
  // color_sensitivity off. For low noise content.
  if (cpi->noise_estimate.enabled)
    noise_level = av1_noise_estimate_extract_level(&cpi->noise_estimate);
  if (noise_level == kLow && source_variance > 1000 && norm_sad < 50) return;
  for (int i = 1; i <= 2; ++i) {
    struct macroblock_plane *const p = &x->plane[i];
    struct macroblockd_plane *const pd = &xd->plane[i];
    const BLOCK_SIZE bs =
    ...
    x->color_sensitivity[i - 1] = uv_sad > (factor * (y_sad >> 3)) &&
                                  norm_uv_sad > thresh_norm_uv_sad;
  }
}
