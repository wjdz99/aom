...
static INLINE void init_mbmi(MB_MODE_INFO *mbmi, PREDICTION_MODE pred_mode,
  ...
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, mbmi->ref_frame_nrs[0]) == mbmi->ref_frame[0]);
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, mbmi->ref_frame_nrs[1]) == mbmi->ref_frame[1]);
#endif  // CONFIG_NEW_REF_SIGNALING
...
static void estimate_intra_mode(
  ...
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, mi->ref_frame_nrs[0]) == mi->ref_frame[0]);
  assert(convert_ranked_ref_to_named_ref_index(&cm->new_ref_frame_data, mi->ref_frame_nrs[1]) == mi->ref_frame[1]);
#endif  // CONFIG_NEW_REF_SIGNALING
