...
void temporal_gradient(const YV12_BUFFER_CONFIG *frame,
  ...
  inter_pred_params.conv_params = get_conv_params(0, plane, bit_depth);
  MV zeroMV = { .row = (int16_t) round(xdec * 8), .col = (int16_t) round(ydec * 8) };
...
void gradients_over_window(const YV12_BUFFER_CONFIG *frame,
  ...
  const int xs = AOMMAX(1, x_start - 1);
  const int ys = AOMMAX(1, y_start - 1);
  const int xe = AOMMIN(x_end + 2, frame_width - 2);
  const int ye = AOMMIN(y_end + 2, frame_height - 2);
    ...
    for (int y = 0; y < frame_height; y++) {
      for (int x = 0; x < frame_width; x++) {
        if (fabs(localmvs[y * frame_width + x].row) > 0 ||
          ...
          LOCALMV localmv = { .row = ((double) mv.row) / 8, .col = ((double) mv.row) / 8 };
