...
static void lucas_kanade(const YV12_BUFFER_CONFIG *from_frame,
  ...
  double *i_x = NULL, *i_y = NULL, *i_t = NULL, *weights = NULL;
  i_x = (double *)aom_malloc(n * n * sizeof(*i_x));
  i_y = (double *)aom_malloc(n * n * sizeof(*i_y));
  i_t = (double *)aom_malloc(n * n * sizeof(*i_t));
  if (!i_x || !i_y || !i_t) goto free_lk_buf;
  const int expand_multiplier = (int)pow(2, level);
  double sigma = 0.2 * n;
  weights = (double *)aom_malloc(n * n * sizeof(*weights));
  if (!weights) goto free_lk_buf;
