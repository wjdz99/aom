...
void gradients_over_window(const YV12_BUFFER_CONFIG *frame,
  ...
  double *fullpel_dx = malloc((ye - ys) * (xe - xs) * sizeof(deriv_x));
  double *fullpel_dy = malloc((ye - ys) * (xe - xs) * sizeof(deriv_y));
...
void lucas_kanade(const YV12_BUFFER_CONFIG *frame_to_filter,
                  ...
                  LK_PARAMS lk_params, const int num_ref_corners,
                  int *ref_corners, const int highres_frame_width,
                  const int highres_frame_height, const int bit_depth,
                  LOCALMV *mvs) {
...
void pyramid_optical_flow(const YV12_BUFFER_CONFIG *from_frame,
                          ...
                          const int bit_depth, OPFL_PARAMS opfl_params,
  ...
  int num_ref_corners =
      detect_corners(from_frame, to_frame, maxcorners, ref_corners, bit_depth);
  ...
  for (int i = levels - 1; i >= stop_level; i--) {
    if (method == LucasKanade) {
      lucas_kanade(&buffers1[i], &buffers2[i], i, opfl_params.lk_params,
                   ...
                   buffers1[0].y_crop_height, bit_depth, mvs);
    }
  }
  for (int i = 1; i < levels; i++) {
    free(images1[i]);
