...
static void tf_motion_search(AV1_COMP *cpi, MACROBLOCK *mb,
                             ...
                             int *subblock_mses, int *subblock_rot, int is_edge_blk, int is_second_arf,FRAME_UPDATE_TYPE update_type) {
  // Frame information
...
//        &distortion, &sse, NULL);
    ...
    if ((/*update_type == 3 ||*/ is_second_arf) && !is_edge_blk) {
...
void av1_tf_do_filtering_row(AV1_COMP *cpi, ThreadData *td, int mb_row, int is_second_arf, FRAME_UPDATE_TYPE update_type) {
    ...
    for (int frame = 0; frame < num_frames; frame++) {
      ...
      if (frame ==
        ...
        tf_motion_search(cpi, mb, frame_to_filter, frames[frame], block_size,
                         mb_row, mb_col, &ref_mv, subblock_mvs, subblock_mses, subblock_rot, is_edge_blk, is_second_arf, update_type);
      }
 ...
 * \return Nothing will be returned, but the contents of td->diff will be
 modified.
 */
static void tf_do_filtering(AV1_COMP *cpi, int is_second_arf, FRAME_UPDATE_TYPE update_type) {
  // Basic information.
  ...
  for (int mb_row = 0; mb_row < tf_ctx->mb_rows; mb_row++)
    av1_tf_do_filtering_row(cpi, td, mb_row, is_second_arf, update_type);
...
//  printf("\n tf: %d;  %d;   %d;   %d;  \n", cpi->common.current_frame.frame_number, cpi->common.show_frame, update_type, is_second_arf);
  ...
  if (mt_info->num_workers > 1)
    ...
    tf_do_filtering(cpi, is_second_arf, update_type);
