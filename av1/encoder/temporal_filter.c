    ...
    for (mb_col = 0; mb_col < mb_cols; mb_col++) {
      ...
      for (frame = 0; frame < frame_count; frame++) {
        ...
        if (frame == alt_ref_index) {
          ...
          // printf("\n err: %d(%d; %d;); err16: %d: ratio: %d; ", err,
          // mbd->mi[0]->mv[0].as_mv.row, mbd->mi[0]->mv[0].as_mv.col, err16,
          // (int)(((float)err/(float)err16 - 1.0)*100.0));
          // for (k = 0; k < 4; k++)
          //   printf(" (%d; %d;)%d; ", blk_mvs[k].row, blk_mvs[k].col,
          //          blk_bestsme[k]);
          // printf("  fw(%d; %d; %d; %d;)  ", fw[0], fw[1], fw[2], fw[3]);

          if ((err * 100 < err16 * 115) && blk_fw[0] == blk_fw[1] &&
              blk_fw[0] == blk_fw[2] && blk_fw[0] == blk_fw[3])
            use_32x32 = 1;
          else
            use_32x32 = 0;
        }
