...
// Applies temporal filter to reference frame with other frames.
// Inputs:
//   ref_frame: Pointer to the frame for filtering.
//   mbd: Pointer to the block for filtering.
//   block_size: Size of the block.
...
void av1_apply_temporal_filter_others(
    const YV12_BUFFER_CONFIG *ref_frame, const MACROBLOCKD *mbd,
    const BLOCK_SIZE block_size, const int mb_row, const int mb_col,
    const int num_planes, const int use_new_strategy, const int strength,
    const int use_subblock, const int *subblock_filter_weights,
    const double noise_level, const uint8_t *pred, uint32_t *accum,
    uint16_t *count) {
  if (use_new_strategy) {  // Commonly used for high-resolution video.
    const int is_high_bitdepth = ref_frame->flags & YV12_FLAG_HIGHBITDEPTH;
    if (is_high_bitdepth) {
      av1_apply_temporal_filter_planewise_c(ref_frame, mbd, TF_BLOCK, mb_row,
                                            mb_col, num_planes, noise_level,
                                            pred, accum, count);
    } else {
      av1_apply_temporal_filter_planewise(ref_frame, mbd, TF_BLOCK, mb_row,
    ...
    if (num_planes == 1) {
      av1_apply_temporal_filter_yonly(
          ref_frame, mbd, TF_BLOCK, mb_row, mb_col, adj_strength, use_subblock,
