...
static INLINE int mod_index(const uint64_t sum_dist, const int index,
                            const int strength, const int filter_weight,
                            const int is_high_bitdepth) {
  assert(index >= 0 && index <= 13);
  const uint64_t multiplier =
      is_high_bitdepth ? highbd_index_mult[index] : index_mult[index];
  assert(multiplier != 0);

  const uint32_t max_value = is_high_bitdepth ? UINT32_MAX : UINT16_MAX;
  const int shift = is_high_bitdepth ? 32 : 16;
  int mod = (int)((AOMMIN(sum_dist, max_value) * multiplier) >> shift);

  const int rounding = (1 << strength) >> 1;
  mod = (mod + rounding) >> strength;
  return (mod >= 16) ? 0 : (16 - mod) * filter_weight;
}
  ...
  for (int plane = 0; plane < MAX_MB_PLANE; ++plane) {
    ...
    for (int i = 0; i < plane_h; ++i) {
      for (int j = 0; j < plane_w; ++j) {
        const uint16_t ref_value = ref[ref_idx];
        const uint16_t pred_value = pred[plane_offset + pred_idx];
        const uint32_t diff = (ref_value > pred_value)
                                  ? (ref_value - pred_value)
                                  : (pred_value - ref_value);
        square_diff[plane_offset + pred_idx] = diff * diff;

        ++ref_idx;
        ++pred_idx;
      }
      ref_idx += (frame_stride - plane_w);
    }
    plane_offset += mb_pels;
  }

  // Get window size for pixel-wise filtering.
  assert(YUV_FILTER_WINDOW_LENGTH % 2 == 0);
    ...
    for (int i = 0; i < h; ++i) {
      for (int j = 0; j < w; ++j) {
        const int filter_weight =
            get_filter_weight(i, j, h, w, blk_fw, !(use_subblock));
        ...
        for (int wi = -half_window; wi <= half_window; ++wi) {
          for (int wj = -half_window; wj <= half_window; ++wj) {
            ...
            if (y >= 0 && y < h && x >= 0 && x < w) {
              modifier += square_diff[plane_offset + y * w + x];
              ++modifier_index;
            }
          }
        }
        ...
        if (plane == 0) {  // Filter Y-plane using both U-plane and V-plane.
          for (int p = 1; p < MAX_MB_PLANE; ++p) {
            const int subsampling_y_shift =
                mbd->plane[p].subsampling_y - subsampling_y;
            const int subsampling_x_shift =
                mbd->plane[p].subsampling_x - subsampling_x;
            const int yy = i >> subsampling_y_shift;
            const int xx = j >> subsampling_x_shift;
            const int ww = w >> subsampling_x_shift;
            modifier += square_diff[mb_pels * p + yy * ww + xx];
            ++modifier_index;
          }
        } else {  // Filter U-plane and V-plane using Y-plane.
          const int subsampling_y_shift =
              subsampling_y - mbd->plane[0].subsampling_y;
          const int subsampling_x_shift =
              subsampling_x - mbd->plane[0].subsampling_x;
          for (int ii = 0; ii < (1 << subsampling_y_shift); ++ii) {
            for (int jj = 0; jj < (1 << subsampling_x_shift); ++jj) {
              const int yy = (i << subsampling_y_shift) + ii;
              const int xx = (j << subsampling_x_shift) + jj;
              const int ww = w << subsampling_x_shift;
        ...
        const int pred_value = pred[plane_offset + pred_idx];
        accum[plane_offset + pred_idx] += modifier * pred_value;
        count[plane_offset + pred_idx] += modifier;
...
void apply_temporal_filter_block(
  ...
  if (use_new_temporal_mode) {
    ...
    if (num_planes <= 1) {
      ...
      // Process 3 planes together.
      av1_apply_temporal_filter_yuv_c(frame, mbd, TF_BLOCK, mb_row, mb_col,
