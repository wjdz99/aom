...
// for temporal filtering. More specifically, given a reference frame and
// another frame for filtering, this function searches the second frame to find
// out the most alike block as that from the reference frame. This found block
// will be further used for weighted averaging.
...
//   ref_frame: Pointer to the reference frame used in motion search.
//   pre_frame: Pointer to the prediciton frame used in motion search.
...
//   subblock_mvs: Pointer to the result motion vector of each sub-block.
//   subblock_errors: Pointer to the search error of each sub-block.
// Returns:
//   Search error of the entire block.
static int tf_motion_search(AV1_COMP *cpi, const YV12_BUFFER_CONFIG *ref_frame,
                            const YV12_BUFFER_CONFIG *pre_frame,
  ...
  const int y_stride = ref_frame->y_stride;
  assert(y_stride == pre_frame->y_stride);
  ...
  const int step_param = av1_init_search_range(
      AOMMIN(ref_frame->y_crop_width, ref_frame->y_crop_height));
  ...
  // Setup.
  mb->plane[0].src.buf = ref_frame->y_buffer + y_offset;
  mb->plane[0].src.stride = y_stride;
  mbd->plane[0].pre[0].buf = pre_frame->y_buffer + y_offset;
  ...
  if (force_integer_mv == 1) {  // Only do full search on the entire block.
    ...
    block_error = cpi->fn_ptr[TF_BLOCK].vf(
        pre_frame->y_buffer + y_offset + mv_offset, y_stride,
        ref_frame->y_buffer + y_offset, y_stride, &sse);
    ...
    *ref_mv = mb->best_mv.as_mv;
    ...
    for (int i = 0; i < mb_height; i += subblock_height) {
      for (int j = 0; j < mb_width; j += subblock_width) {
        ...
        mb->plane[0].src.buf = ref_frame->y_buffer + y_offset + offset;
        mbd->plane[0].pre[0].buf = pre_frame->y_buffer + y_offset + offset;
        av1_set_mv_search_range(&mb->mv_limits, &baseline_mv);
...
// Builds predictors for blocks in temporal filtering. This is the second step
// for temporal filtering, which is to construct predictions from all frames
// excluding the reference frame. These predictors are built based on the
// previous motion search results, and will be futher used for filtering.
// Inputs:
...
//   modified.
static void tf_build_predictors(const YV12_BUFFER_CONFIG *ref_frame,
                                const MACROBLOCKD *mbd,
                                const BLOCK_SIZE block_size, const int mb_row,
                                const int mb_col,
                                const struct scale_factors *scale,
                                const int num_planes, const int use_subblock,
    ...
    for (mb_col = 0; mb_col < mb_cols; mb_col++) {
      ...
      for (frame = 0; frame < frame_count; frame++) {
        ...
        if (blk_fw[0] || blk_fw[1] || blk_fw[2] || blk_fw[3]) {
          ...
          tf_build_predictors(frames[frame], mbd, TF_BLOCK, mb_row, mb_col,
