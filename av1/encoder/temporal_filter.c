...
static double estimate_noise(uint8_t *src, int width, int height, int stride,
                             int edge_thresh) {
  const double mult = 1.25331413732;
  int64_t sum = 0;
  int64_t num = 0;
  for (int i = 1; i < height - 1; ++i) {
    for (int j = 1; j < width - 1; ++j) {
      // Sobel gradients
      const int Gx =
          (src[(i - 1) * stride + j - 1] - src[(i - 1) * stride + j + 1]) +
          (src[(i + 1) * stride + j - 1] - src[(i + 1) * stride + j + 1]) +
          2 * (src[i * stride + j - 1] - src[i * stride + j + 1]);
      const int Gy =
          (src[(i - 1) * stride + j - 1] - src[(i + 1) * stride + j - 1]) +
          (src[(i - 1) * stride + j + 1] - src[(i + 1) * stride + j + 1]) +
          2 * (src[(i - 1) * stride + j] - src[(i + 1) * stride + j]);
      ...
      if (Ga < edge_thresh) {
        ...
        const int v =
            4 * src[i * stride + j] -
            2 * (src[i * stride + j - 1] + src[i * stride + j + 1] +
                 src[(i - 1) * stride + j] + src[(i + 1) * stride + j]) +
            (src[(i - 1) * stride + j - 1] + src[(i - 1) * stride + j + 1] +
             src[(i + 1) * stride + j - 1] + src[(i + 1) * stride + j + 1]);
        sum += abs(v);
        ++num;
  ...
  const double sigma = (double)sum / (6 * num) * mult;
  return sigma;
}

// Return noise estimate, or -1.0 if there was a failure
static double highbd_estimate_noise(uint8_t *src8, int width, int height, int stride,
                                    int bd, int edge_thresh) {
...
static void adjust_arnr_filter(AV1_COMP *cpi, int distance, int group_boost,
  ...
  struct lookahead_entry *buf =
      av1_lookahead_peek(cpi->lookahead, distance);
  double noiselevel;
  if (mbd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    noiselevel = estimate_noise(
        buf->img.y_buffer, buf->img.y_crop_width, buf->img.y_crop_height,
        buf->img.y_stride, EDGE_THRESHOLD);
  }
