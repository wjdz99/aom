...
void av1_temporal_filter_plane_c(uint8_t *frame1, unsigned int stride,
                                 ...
                                 uint16_t *square_diffs_y, int ss_x_shift,
                                 int ss_y_shift, int block_width,
  ...
  for (int i = 0, k = 0; i < block_height; i++) {
    for (int j = 0; j < block_width; j++, k++) {
      ...
      if (square_diffs_y != NULL) {
        for (int idy_y = 0; idy_y < (1 << ss_y_shift); ++idy_y) {
          for (int idx_y = 0; idx_y < (1 << ss_x_shift); ++idx_y) {
            const int yy = (i << ss_y_shift) + idy_y;  // Y-coord on Y-plane.
            const int xx = (j << ss_x_shift) + idx_y;  // X-coord on Y-plane.
...
void apply_temporal_filter_block(YV12_BUFFER_CONFIG *frame, MACROBLOCKD *mbd,
  ...
  if (use_new_temporal_mode) {
    ...
    av1_temporal_filter_plane_c(frame->y_buffer + mb_y_src_offset,
                                frame->y_stride, predictor, BW, NULL, 0, 0, BW,
                                BH, strength, noise_levels[0], decay_control,
                                blk_fw, use_32x32, accumulator, count);
    if (num_planes > 1) {
      av1_temporal_filter_plane_c(
          frame->u_buffer + mb_uv_src_offset, frame->uv_stride,
          predictor + BLK_PELS, mb_uv_width, square_diffs_y,
          mbd->plane[1].subsampling_x - mbd->plane[0].subsampling_x,
          mbd->plane[1].subsampling_y - mbd->plane[0].subsampling_y,
          mb_uv_width, mb_uv_height, strength, noise_levels[1], decay_control,
          blk_fw, use_32x32, accumulator + BLK_PELS, count + BLK_PELS);
      av1_temporal_filter_plane_c(
          frame->v_buffer + mb_uv_src_offset, frame->uv_stride,
          predictor + (BLK_PELS << 1), mb_uv_width, square_diffs_y,
          mbd->plane[1].subsampling_x - mbd->plane[0].subsampling_x,
          mbd->plane[1].subsampling_y - mbd->plane[0].subsampling_y,
