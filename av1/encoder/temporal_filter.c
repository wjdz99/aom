...
static void tf_build_predictors_mb_c(
  ...
  // Handle Y-plane and UV-plane (if needed) in sequence.
  int plane_offset = 0;
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    const struct buf_2d ref_buf = { NULL, ref_frame->buffers[plane],
                                    ref_frame->widths[plane == 0 ? 0 : 1],
                                    ref_frame->heights[plane == 0 ? 0 : 1],
                                    ref_frame->strides[plane == 0 ? 0 : 1] };
    ...
    for (int i = 0; i < plane_h; i += h) {
      for (int j = 0; j < plane_w; j += w) {
        ...
        const int subblock_offset = i * plane_w + j;
        av1_build_inter_predictor(&pred[plane_offset + subblock_offset],
