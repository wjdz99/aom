...
static int tf_get_filter_weight(const int block_error,
  ...
  // INT_MAX can ONLY be accessed by to-filter frame.
...
// Helper function to determine where a frame is encoded with high bit-depth.
...
//   noise_level: Estimated noise level. (Used in plane-wise filtering.)
//   pred: Pointer to the well-built predictors.
//   accum: Pointer to the pixel-wise accumulator for filtering.
//   count: Pointer to the pixel-wise counter fot filtering.
// Returns:
...
//   noise_level: Pre-estimated noise level.
    ...
    for (int mb_col = 0; mb_col < mb_cols; mb_col++) {
      ...
      for (int frame = 0; frame < num_frames; frame++) {
        ...
        if (frame == filter_frame_idx) {  // Frame to be filtered.
          ...
          if (block_error > (3000 << (bit_depth - 8))) {
            ref_mv = kZeroMv;
          }
        }
