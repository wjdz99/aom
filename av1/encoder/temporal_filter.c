...
// TODO: Not sure what index 4 and index 9 are for.
...
static INLINE int adjust_filter_weight_yuv(const int filter_weight,
  ...
  assert(YUV_FILTER_WINDOW_SIZE == 3);
  ...
  int adjusted_weight = (modifier >= 16) ? 0 : (16 - modifier) * filter_weight;
  ...
  for (int plane = 0; plane < MAX_MB_PLANE; ++plane) {
    ...
    for (int i = 0; i < plane_h; ++i) {
      for (int j = 0; j < plane_w; ++j) {
        const uint16_t ref_value =
            is_high_bitdepth ? ref16[ref_idx] : ref[ref_idx];
        const uint16_t pred_value = is_high_bitdepth
...
// Applies temporal filter to YUV planes (High-BitDepth video).
    ...
    if (use_new_temporal_mode) {
      ...
      if (num_planes <= 1) {
        ...
        av1_highbd_apply_temporal_filter_yuv(
            frame, mbd, TF_BLOCK, mb_row, mb_col, strength, !(use_32x32),
