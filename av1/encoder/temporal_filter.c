...
static void tf_setup_filtering_buffer(AV1_COMP *cpi,
  ...
  int arf_src_offset = cpi->gf_group.arf_src_offset[cpi->cur_gf_index];
  const FRAME_TYPE frame_type = cpi->gf_group.frame_type[cpi->cur_gf_index];
...
int av1_temporal_filter(AV1_COMP *cpi, const int filter_frame_lookahead_idx,
  ...
  const uint8_t group_idx = cpi->cur_gf_index;
  TemporalFilterCtx *tf_ctx = &cpi->tf_ctx;
