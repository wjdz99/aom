...
static int tf_motion_search(AV1_COMP *cpi,
  ...
  av1_full_pixel_search(
      cpi, mb, block_size, start_mv, step_param, full_search_method, 1,
      sadperbit16, cond_cost_list(cpi, cost_list), &baseline_mv, 0, &ss_cfg,
      &mb->best_mv.as_fullmv, &mb->second_best_mv.as_fullmv);
    ...
    *ref_mv = mb->best_mv.as_mv;
    ...
    for (int i = 0; i < mb_height; i += subblock_height) {
      for (int j = 0; j < mb_width; j += subblock_width) {
        ...
        mb->mv_cost_type = mv_cost_type;
        av1_full_pixel_search(
            cpi, mb, subblock_size, start_mv, step_param, full_search_method, 1,
            sadperbit16, cond_cost_list(cpi, cost_list), &baseline_mv, 0,
            &ss_cfg, &mb->best_mv.as_fullmv, &mb->second_best_mv.as_fullmv);
