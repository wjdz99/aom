...
// Only supports using 3x3 window for filtering, hence, there are totally 9
// non-zero numbers within the 14-element multiplier lookup table. 5 zeros
...
//   modified.
static void tf_build_predictors_mb_c(const YV12_BUFFER_CONFIG *ref_frame,
  ...
  const int is_hbd = is_cur_buf_hbd(mbd);             // Is high bit-depth?
  ...
  for (int p = 0; p < num_planes; ++p) {
    const int subsampling_y = mbd->plane[p].subsampling_y;
    const int subsampling_x = mbd->plane[p].subsampling_x;
    ...
    ref_buf.height = p ? ref_frame->uv_height : ref_frame->y_height;
    ref_buf.width = p ? ref_frame->uv_width : ref_frame->y_width;
    ref_buf.stride = p ? ref_frame->uv_stride : ref_frame->y_stride;
    ...
    for (int i = 0; i < num_blocks; ++i) {
      for (int j = 0; j < num_blocks; ++j) {
        ...
        av1_init_inter_params(&inter_pred_params, w, h, y, x, subsampling_x,
                              subsampling_y, bit_depth, is_hbd, is_intrabc,
                              scale, &ref_buf, interp_filters);
        inter_pred_params.conv_params = get_conv_params(0, p, bit_depth);
        av1_build_inter_predictor(&pred[mb_size * p + offset], pred_stride,
