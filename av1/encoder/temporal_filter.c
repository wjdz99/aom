...
static void tf_do_filtering_row(AV1_COMP *cpi, ThreadData *td, int mb_row) {
  TemporalFilterCtx *tf_params = &cpi->tf_ctx;
  YV12_BUFFER_CONFIG **frames = tf_params->frames;
  const int num_frames = tf_params->num_frames;
  const int filter_frame_idx = tf_params->filter_frame_idx;
  const int check_show_existing = tf_params->check_show_existing;
  const struct scale_factors *scale = &tf_params->sf;
  const double *noise_levels = tf_params->noise_levels;
  const int num_pels = tf_params->num_pels;
  const int q_factor = tf_params->q_factor;
  ...
  for (int mb_col = 0; mb_col < tf_params->mb_cols; mb_col++) {
...
static void tf_do_filtering(AV1_COMP *cpi) {
  ...
  for (int mb_row = 0; mb_row < tf_params->mb_rows; mb_row++)
 ...
 *         saved in `noise_levels`. All the updated params will be saved in
 * cpi->tf_params.
 */
static void tf_setup_filtering_buffer(AV1_COMP *cpi,
  ...
  const int num_planes = av1_num_planes(&cpi->common);
  double *noise_levels = tf_params->noise_levels;
  for (int plane = 0; plane < num_planes; ++plane) {
  ...
  tf_params->num_frames = num_frames;
  tf_params->filter_frame_idx = num_before;
  assert(frames[tf_params->filter_frame_idx] == to_filter_frame);
...
//   lookahead buffer cpi->lookahead. is_second_arf: Flag indiacting whether
//   second ARF filtering is required.
// Returns:
...
static void init_tf_ctx(AV1_COMP *cpi, int filter_frame_lookahead_idx,
  ...
  struct scale_factors *sf = &cpi->tf_ctx.sf;
...
int av1_temporal_filter(AV1_COMP *cpi, const int filter_frame_lookahead_idx,
  ...
  tf_dealloc_data(tf_data, tf_ctx->is_highbitdepth);
