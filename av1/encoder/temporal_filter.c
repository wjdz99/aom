...
// Applies temporal filter to reference frame itself. More concretely, the
// filter weights for all pixels are the same.
// Inputs:
//   mbd: Pointer to the block for filtering.
//   block_size: Size of the block.
//   num_planes: Number of planes in the frame.
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    for (int i = 0; i < h; ++i) {
      for (int j = 0; j < w; ++j) {
        ...
        accum[plane_offset + pred_idx] += filter_weight * pred_value;
        count[plane_offset + pred_idx] += filter_weight;
        ++pred_idx;
      }
    }
    plane_offset += mb_pels;
...
static INLINE int adjust_filter_weight_yonly(const int filter_weight,
  ...
  int modifier = sum_square_diff * 3;
...
void av1_apply_temporal_filter_yonly(const YV12_BUFFER_CONFIG *ref_frame,
  ...
  const uint8_t *ref = ref_frame->buffers[0] + frame_offset;
  const uint16_t *ref16 = CONVERT_TO_SHORTPTR(ref);
  ...
  for (int i = 0; i < h; ++i) {
    for (int j = 0; j < w; ++j) {
      ...
      const uint16_t pred_value =
          is_high_bitdepth ? pred16[pred_idx] : pred[pred_idx];
      const uint32_t diff = (ref_value > pred_value) ? (ref_value - pred_value)
                                                     : (pred_value - ref_value);
      square_diff[pred_idx] = diff * diff;

      ++ref_idx;
      ++pred_idx;
    }
    ref_idx += (frame_stride - w);
  }
  ...
  for (int i = 0; i < h; ++i) {
    for (int j = 0; j < w; ++j) {
      ...
      const int pred_value =
          is_high_bitdepth ? pred16[pred_idx] : pred[pred_idx];
      accum[pred_idx] += adjusted_weight * pred_value;
      count[pred_idx] += adjusted_weight;
...
void apply_temporal_filter_block(
  ...
  if (is_hbd) {
    if (use_new_temporal_mode) {
      ...
      if (num_planes <= 1) {
        av1_apply_temporal_filter_yonly(frame, mbd, TF_BLOCK, mb_row, mb_col,
    ...
    for (mb_col = 0; mb_col < mb_cols; mb_col++) {
      ...
      for (frame = 0; frame < frame_count; frame++) {
        ...
        if (blk_fw[0] || blk_fw[1] || blk_fw[2] || blk_fw[3]) {
          ...
          if (frame == alt_ref_index) {
            const int filter_weight =
                use_new_temporal_mode ? SCALE : blk_fw[0] * 16;
