...
static void temporal_filter_predictors_mb_c(const file_options_t *cfg,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    av1_highbd_build_inter_predictor(cfg,
                                     y_mb_ptr, stride, &pred[0], 16, &mv, scale,
  ...
  av1_build_inter_predictor(cfg,
                            y_mb_ptr, stride, &pred[0], 16, &mv, scale, 16, 16,
  ...
  av1_build_inter_predictor(cfg,
                            u_mb_ptr, uv_stride, &pred[256], uv_block_width,
  ...
  av1_build_inter_predictor(cfg,
                            v_mb_ptr, uv_stride, &pred[512], uv_block_width,
                            &mv, scale, uv_block_width, uv_block_height,
                            &conv_params, interp_filter,
#if CONFIG_GLOBAL_MOTION || CONFIG_WARPED_MOTION
                            &warp_types, x, y, 2, 0,
