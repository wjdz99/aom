...
static int temporal_filter_find_matching_mb_c(AV1_COMP *cpi,
  ...
  av1_full_pixel_search(cpi, x, BLOCK, &best_ref_mv1_full, step_param, NSTEP, 1,
                        sadpb, cond_cost_list(cpi, cost_list), &best_ref_mv1, 0,
                        0, x_pos, y_pos, 0);
  ...
  if (cpi->common.cur_frame_force_integer_mv == 1) {
    ...
    bestsme = cpi->fn_ptr[BLOCK].vf(y + offset, y_stride, src_address,
                                    src_stride, &sse);
  } else {
    bestsme = cpi->find_fractional_mv_step(
        x, &cpi->common, 0, 0, &best_ref_mv1,
        cpi->common.allow_high_precision_mv, x->errorperbit,
        &cpi->fn_ptr[BLOCK], 0, mv_sf->subpel_iters_per_step,
