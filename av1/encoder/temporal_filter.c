...
static void tf_determine_block_partition(const MV block_mv, const int block_rot, const int block_mse,
                                         MV *subblock_mvs, int *subblock_rot, int *subblock_mses);
...
//        &distortion, &sse, NULL);
    ...
    //printf("\n bbbbmv: %d; %d;", best_mv.as_mv.row, best_mv.as_mv.col);
      error_rot = INT_MAX;
      error_rot = av1_find_best_rotation(mbd, &ms_params, error, &best_mv.as_mv, &block_rot);
    if (block_rot) {
    ...
    for (int i = 0; i < mb_height; i += subblock_height) {
      for (int j = 0; j < mb_width; j += subblock_width) {
        ...
        //printf("\n bbbbmv: %d; %d;", best_mv.as_mv.row, best_mv.as_mv.col);
          error_rot = INT_MAX;
          rot = 0;
          error_rot = av1_find_best_rotation(mbd, &ms_params, error, &best_mv.as_mv, &rot);
          if (rot) {
//            printf("\n    %d:   %d;       %d; %d; ", subblock_idx, error,  error_rot, rot);
  ...
  //printf("\n found rot: %d; %d; %d; %d;   \n", subblock_rot[0], subblock_rot[1],subblock_rot[2],subblock_rot[3]);
