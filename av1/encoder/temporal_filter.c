  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    for (int i = 0; i < h; ++i) {
      for (int j = 0; j < w; ++j) {
        ...
        const int idx = plane_offset + pred_idx;  // Index with plane shift.
        const int pred_value = is_high_bitdepth ? pred16[idx] : pred[idx];
        // printf("%ld, %d, %lf\n", sum_square_diff, num_ref_pixels, r);
        const double scaled_diff = AOMMAX(
            -(double)(sum_square_diff / num_ref_pixels) / (2 * r * r), -15.0);
        const int adjusted_weight = (int)(exp(scaled_diff) * SCALE);
        // printf("%d, ", adjusted_weight);
        accum[idx] += adjusted_weight * pred_value;
        count[idx] += adjusted_weight;
...
void apply_temporal_filter_block(YV12_BUFFER_CONFIG *frame, MACROBLOCKD *mbd,
                                 ...
                                 int strength, double sigma, int *blk_fw,
  ...
  // High bitdepth
  if (is_hbd) {
    if (!use_new_temporal_mode) {  // High resolution video.
      av1_apply_temporal_filter_planewise_c(frame, mbd, TF_BLOCK, mb_row,
                                            mb_col, num_planes, sigma,
                                            predictor, accumulator, count);
    } else {  // Low resolution video.
  ...
  // Low bitdepth
  if (!use_new_temporal_mode) {  // High resolution video.
    av1_apply_temporal_filter_planewise(frame, mbd, TF_BLOCK, mb_row, mb_col,
                                        num_planes, sigma, predictor,
                                        accumulator, count);
  } else {  // Low resolution video.
    if (num_planes <= 1) {
      av1_apply_temporal_filter_yonly(frame, mbd, TF_BLOCK, mb_row, mb_col,
          ...
          if (frame == alt_ref_index) {
            ...
            if (is_hbd) {
              ...
              apply_temporal_filter_block(
                  f, mbd, num_planes, predictor, strength, sigma, blk_fw,
                  use_32x32, accumulator, count, use_new_temporal_mode, mb_row,
                  mb_col);
#else
              const int adj_strength = strength + 2 * (mbd->bd - 8);
              if (num_planes <= 1) {
