  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    for (int i = 0; i < h; ++i) {
      for (int j = 0; j < w; ++j) {
        ...
        const double n = 0.5 + log(2 * noise_levels[plane] + 5.0);
        // Smaller q -> smaller filtering weight. If q is large enough, this
        // decay factor has no effect. The q-threshold can be tuned.
        const double q = CLIP(pow((double)q_factor / 20, 2), 1e-5, 1);
        // Smaller strength -> smaller filtering weight. If strength is large
        // enough, this decay factor has no effect. The threshold can be tuned.
        const double s = CLIP(pow((double)filter_strength / 3, 2), 1e-5, 1);

        // Compute filter weight.
        // The combined error is "normalized". This factor can be tuned.
        const double scaled_error = (combined_error / 20) / (n * q * s);
