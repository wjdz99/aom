...
static int estimate_strength(AV1_COMP *cpi, int distance, int group_boost,
  ...
  struct lookahead_entry *buf =
      av1_lookahead_peek(cpi->lookahead, distance, ENC_STAGE);
  int strength;
  double noiselevel;
...
static void adjust_arnr_filter(AV1_COMP *cpi, int distance, int group_boost,
  ...
  const int frames_after_arf =
      av1_lookahead_depth(cpi->lookahead, ENC_STAGE) - distance - 1;
...
int av1_temporal_filter(AV1_COMP *cpi, int distance,
  ...
  for (frame = 0; frame < frames_to_blur; ++frame) {
    ...
    struct lookahead_entry *buf =
        av1_lookahead_peek(cpi->lookahead, which_buffer, ENC_STAGE);
    if (buf == NULL) {
