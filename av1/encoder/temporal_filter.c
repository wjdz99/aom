...
static void tf_setup_filtering_buffer(AV1_COMP *cpi,
  ...
  const FIRSTPASS_STATS *stats =
      cpi->twopass.stats_in - (cpi->rc.frames_since_key == 0);
  ...
  for (int i = 1; i <= max_after; i++) {
    ...
    accu_coeff1 *= stats[filter_frame_lookahead_idx + i].cor_coeff;
  }
  if (max_after >= 1) {
  ...
  for (int i = 1; i <= max_before; i++) {
    ...
    accu_coeff0 *= stats[filter_frame_lookahead_idx - i + 1].cor_coeff;
  }
  if (max_before >= 1) {
