...
void av1_ml_prune_4_partition(const AV1_COMP *const cpi, MACROBLOCK *const x,
  ...
  unsigned int vert_4_source_var[4] = { 0 };
  {
    ...
    horz_4_src.stride = x->plane[0].src.stride;
    vert_4_src.stride = x->plane[0].src.stride;

    for (int i = 0; i < 4; ++i) {
      horz_4_src.buf = src + i * block_size_high[horz_4_bs] * src_stride;
      vert_4_src.buf = src + i * block_size_wide[vert_4_bs];
      unsigned int horz_var, vert_var;

      if (is_cur_buf_hbd(xd)) {
        horz_var = av1_high_get_sby_perpixel_variance(cpi, &horz_4_src,
                                                      horz_4_bs, xd->bd);
        vert_var = av1_high_get_sby_perpixel_variance(cpi, &vert_4_src,
                                                      vert_4_bs, xd->bd);
        horz_4_source_var[i] =
        ...
        horz_var = av1_get_sby_perpixel_variance(cpi, &horz_4_src, horz_4_bs);
        vert_var = av1_get_sby_perpixel_variance(cpi, &vert_4_src, vert_4_bs);
