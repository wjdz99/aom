...
void av1_get_max_min_partition_features(AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  const float var_mv_row = sum_mv_row_sq / (float)blks - avg_mv_row * avg_mv_row;

  const float avg_mv_col = sum_mv_col / (float)blks;
  const float var_mv_col = sum_mv_col_sq / (float)blks - avg_mv_col * avg_mv_col;

  const float avg_log_sse = sum_log_sse / (float)blks;
  const float var_log_sse = sum_log_sse_sq / (float)blks - avg_log_sse * avg_log_sse;
