...
static bool ext_ml_model_decision_after_none(
    AV1_COMP *const cpi, const float *const features_after_none,
...
static bool ext_ml_model_decision_after_split_part2(
    AV1_COMP *const cpi, const float *const features_prune,
    int *prune_rect_part_horz, int *prune_rect_part_vert);

static bool ext_ml_model_decision_after_rect(
    AV1_COMP *cpi, const float *const features_after_rect,
...
static void write_features_to_file(const AV1_COMP *const cpi,
                                   const float *features,
                                   const int feature_size, const int id,
                                   const int bsize, const int mi_row,
                                   const int mi_col) {
  if (!WRITE_FEATURE_TO_FILE && !cpi->ext_part_controller.test_mode) return;
  ...
  if (!cpi->ext_part_controller.test_mode) {
...
void av1_simple_motion_search_based_split(
  ...
  write_features_to_file(cpi, features, FEATURE_SIZE_SMS_SPLIT, 0, bsize,
                         mi_row, mi_col);
...
void av1_simple_motion_search_prune_rect(
  ...
  if (cpi->sf.part_sf.simple_motion_search_prune_rect &&
    ...
    // Write features to file
    write_features_to_file(cpi, features, FEATURE_SIZE_SMS_PRUNE_PART, 1, bsize,
                           mi_row, mi_col);
...
void av1_simple_motion_search_early_term_none(
  ...
  // Write features to file
  write_features_to_file(cpi, features, FEATURE_SIZE_SMS_TERM_NONE, 3, bsize,
                         mi_row, mi_col);
...
void av1_ml_early_term_after_split(AV1_COMP *const cpi, MACROBLOCK *const x,
  ...
  // Write features to file
  write_features_to_file(cpi, features, FEATURES, 4, bsize, mi_row, mi_col);
...
void av1_ml_prune_rect_partition(AV1_COMP *const cpi, const MACROBLOCK *const x,
  ...
  // Write features to file
  write_features_to_file(cpi, features, /*feature_size=*/9, 5, bsize, mi_row,
                         mi_col);

  if (ext_ml_model_decision_after_split_part2(cpi, features, dst_prune_horz,
                                              dst_prune_vert)) {
    return;
  }
...
void av1_ml_prune_ab_partition(
  ...
  if (!frame_is_intra_only(&cpi->common)) {
    write_features_to_file(cpi, features, /*feature_size=*/10, 6, bsize, mi_row,
                           mi_col);
  }

  if (ext_ml_model_decision_after_rect(
          cpi, features, horza_partition_allowed, horzb_partition_allowed,
...
void av1_ml_prune_4_partition(
  ...
  // Write features to file
  if (!frame_is_intra_only(&cpi->common)) {
    write_features_to_file(cpi, features, FEATURES, 7, bsize, mi_row, mi_col);
  }
...
void av1_ml_predict_breakout(AV1_COMP *const cpi, BLOCK_SIZE bsize,
  ...
  // Write features to file
  write_features_to_file(cpi, features, FEATURES, 2, blk_params.bsize,
                         blk_params.mi_row, blk_params.mi_col);

  if (ext_ml_model_decision_after_none(cpi, features, do_square_split,
...
// do_square_split
// do_rectangular_split
bool ext_ml_model_decision_after_none(AV1_COMP *const cpi,
                                      const float *const features_after_none,
                                      int *do_square_split,
                                      int *do_rectangular_split) {
  AV1_COMMON *const cm = &cpi->common;
  ExtPartController *const ext_part_controller = &cpi->ext_part_controller;
...
bool ext_ml_model_decision_after_split_part2(AV1_COMP *const cpi,
                                             const float *const features_prune,
                                             int *prune_rect_part_horz,
...
static bool ext_ml_model_decision_after_rect(
  ...
  const AV1_COMMON *const cm = &cpi->common;
  ExtPartController *const ext_part_controller = &cpi->ext_part_controller;
  if (frame_is_intra_only(cm) || !ext_part_controller->ready) return false;
