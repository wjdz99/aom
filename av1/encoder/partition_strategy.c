...
static INLINE int get_arr_idx(int mi_bsize, int mi_in_sb) {
  int idx = -1;
  if (mi_bsize == 1) {
    idx = mi_in_sb;
  } else {
...
static INLINE SimpleMotionData *get_sms_data_entry(
  ...
  const int idx_row_in_sb = get_arr_idx(mi_high, mi_row_in_sb);
  const int idx_col_in_sb = get_arr_idx(mi_wide, mi_col_in_sb);
...
static void get_sms_data(AV1_COMP *const cpi, const TileInfo *const tile,
  ...
  if (cpi->ref_frame_flags & av1_ref_frame_flag_list[ref_frame]) {
    ...
    sms_data->sse = curr_sse;
    sms_data->var = curr_var;
    sms_data->dist = curr_sse;
    sms_data->rate = 0;
