...
void av1_intra_mode_cnn_partition(const AV1_COMMON *const cm, MACROBLOCK *x,
  ...
  if (bsize == BLOCK_64X64 && !part_info->cnn_output_valid) {
    ...
    const int dc_q =
        av1_dc_quant_QTX(x->qindex, cm->quant_params.y_dc_delta_q, bit_depth) >> (bit_depth - 8);
...
static AOM_INLINE void simple_motion_search_prune_part_features(
  ...
  // Q_INDEX
  const int dc_q = av1_dc_quant_QTX(x->qindex, cpi->common.quant_params.y_dc_delta_q, xd->bd) >> (xd->bd - 8);
