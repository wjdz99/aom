...
static INLINE void add_start_mv_to_partition(
  ...
  const int quarter_step_h = mi_size_high[bsize] / 4;
  const int quarter_step_w = mi_size_wide[bsize] / 4;
  ...
  for (int idx = 0; idx < subblock_count[partition]; idx++) {
    ...
    const int sub_row =
        mi_row + step_multiplier[partition][idx][0] * quarter_step_h;
    const int sub_col =
        mi_col + step_multiplier[partition][idx][1] * quarter_step_w;
