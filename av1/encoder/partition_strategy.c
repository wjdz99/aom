...
// PC_TREE.
static void get_min_bsize(const PC_TREE *pc_tree, int *min_bw,
                          int *min_bh) {
  if (!pc_tree) return;
...
void av1_ml_early_term_after_split(const AV1_COMP *const cpi,
  ...
  const AV1_COMMON *const cm = &cpi->common;
  const int frame_breadth = AOMMIN(cm->width, cm->height);
  // Currently not enabled for high resolution.
  if (frame_breadth >= 720) return;

  const int is_480p_or_larger = frame_breadth >= 480;
  const NN_CONFIG *nn_config = NULL;
  float thresh = -1e6;
  switch (bsize) {
    case BLOCK_128X128:
