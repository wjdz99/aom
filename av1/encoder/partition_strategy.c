...
void av1_prune_partitions_before_search(AV1_COMP *const cpi,
  ...
  if (cpi->third_pass_ctx) {
    ...
    PARTITION_TYPE tp_part =
        av1_third_passget_sb_part_type(cpi->third_pass_ctx, this_mi);
    ...
    if (!is_edge && block_size_wide[bsize] >= 16) {
      ...
      // rectangular partition in the different partition.
      if (tp_part != PARTITION_NONE) {
        if (tp_part == PARTITION_HORZ || tp_part == PARTITION_HORZ_4 ||
            tp_part == PARTITION_HORZ_A || tp_part == PARTITION_HORZ_B) {
          part_state->partition_rect_allowed[VERT] = 0;
        } else if (tp_part == PARTITION_VERT || tp_part == PARTITION_VERT_4 ||
                   tp_part == PARTITION_VERT_A || tp_part == PARTITION_VERT_B) {
          part_state->partition_rect_allowed[HORZ] = 0;
        }
      }

      int minSize =
          AOMMIN(block_size_wide[tp_bsize], block_size_high[tp_bsize]);
      int maxSize =
          AOMMAX(block_size_wide[tp_bsize], block_size_high[tp_bsize]);
