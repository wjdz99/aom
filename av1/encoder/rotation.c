...
//void av1_build_rotational_predictors(MACROBLOCKD *xd, BLOCK_SIZE bsize) {
//  int plane;
//  for (plane = 0; plane <= MAX_MB_PLANE - 1; ++plane) {
//    const struct macroblockd_plane *pd = &xd->plane[plane];
//    const int bw = 4 * mi_size_wide[bsize] >> pd->subsampling_x;
//    const int bh = 4 * mi_size_high[bsize] >> pd->subsampling_y;
//
//    build_rotational_predictor(xd, plane, bw, bh);
//  }
//}
...
unsigned int av1_find_best_rotation(MACROBLOCKD *xd,
  ...
  uint8_t *pre = ms_params->var_params.ms_buffers.ref->buf;
  int pre_stride = ms_params->var_params.ms_buffers.ref->stride;
...
//
//      printf("\n  1111111:  error: %d; error_rot %d; ", pre_besterr, var);
