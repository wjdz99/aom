...
#if CONFIG_EOB_FIRST
void av1_update_eob_context(int eob, int seg_eob, TX_SIZE txsize, PLANE_TYPE plane,
...
#if LV_MAP_PROB
    update_cdf(ec_ctx->coeff_base_cdf[txsize][plane][i][eob_pos_ctx], eob_pt==i, 2);
#endif   
    if (eob_pt == i) {
      break;
    }
...
void get_dist_cost_stats(LevelDownStats *stats, int scan_idx, 
  ...
  const int low_qc_cost = get_coeff_cost(stats->low_qc, scan_idx, txb_info, txb_costs);
  ...
  int coeff_ctx = get_nz_map_ctx(txb_info->qcoeff, scan_idx, scan, txb_info->bwl,
...
static void update_coeff(int coeff_idx, tran_low_t qc, TxbInfo *txb_info) {
  ...
  txb_info->dqcoeff[coeff_idx] =
      qcoeff_to_dqcoeff(qc, dqv, txb_info->shift);
}
...
for (int i = 1; i < max_eob_pt; i++) {
  ...
  aom_write_bin(w, eob_pt == i, ec_ctx->eob_flag_cdf[txs_ctx][plane_type][eob_pos_ctx], 2);
  //aom_write_symbol(w, eob_pt == i, ec_ctx->eob_flag_cdf[AOMMIN(txs_ctx,3)][plane_type][eob_pos_ctx], 2);
  if (eob_pt == i) {
    break;
  }
}
...
#if CONFIG_EOB_FIRST
  int eob_cost;
  eob_cost = get_eob_cost(eob, seg_eob, coeff_costs);
...
static int try_neighbor_level_down_nz(int coeff_idx, int nb_coeff_idx,
  ...
  if (nb_scan_idx < scan_idx) {
    ...
    const int new_ctx = get_nz_map_ctx_from_count(
        count - 1, coeff_idx, txb_info->bwl, 
  #if CONFIG_EOB_FIRST
        txb_info->height,
  #endif
        txb_info->tx_type);
    ...
    if (nb_scan_idx < eob) {
      ...
      if (scan_idx < nb_scan_idx) {
        ...
        txb_cache->nz_ctx_arr[nb_coeff_idx] = get_nz_map_ctx_from_count(
            count, nb_coeff_idx, txb_info->bwl, 
#if  CONFIG_EOB_FIRST
            txb_info->height,      
#endif
            txb_info->tx_type);
...
static int optimize_txb(TxbInfo * txb_info,
  ...
  for (int si = init_eob - 1; si >= 0; --si) {
    ...
    if (qc == 0) {
      ...
      prev_eob_rd_cost = RDCOST(txb_info->rdmult, accu_rate + stats.nz_rate, accu_dist);
  ...
  int non_zero_blk_rate = txb_costs->txb_skip_cost[txb_info->txb_ctx->txb_skip_ctx][0];
  prev_eob_rd_cost = RDCOST(txb_info->rdmult, accu_rate + non_zero_blk_rate, accu_dist);
  
  int zero_blk_rate = txb_costs->txb_skip_cost[txb_info->txb_ctx->txb_skip_ctx][1];
