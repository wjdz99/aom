...
static void write_coeffs_txb_wrap(const AV1_COMMON *cm, MACROBLOCK *x,
  ...
  tran_low_t *tcoeff = BLOCK_OFFSET(
      (x->mbmi_ext->cb_coef_buff->tcoeff[plane] + x->mbmi_ext->cb_offset),
      block);
  uint16_t eob = (x->mbmi_ext->cb_coef_buff->eobs[plane] + txb_offset)[block];
  TXB_CTX txb_ctx = {
    (x->mbmi_ext->cb_coef_buff->txb_skip_ctx[plane] + txb_offset)[block],
    (x->mbmi_ext->cb_coef_buff->dc_sign_ctx[plane] + txb_offset)[block]
  };
...
void av1_update_and_record_txb_context(int plane, int block, int blk_row,
  ...
  (x->mbmi_ext->cb_coef_buff->eobs[plane] + txb_offset)[block] = eob;
