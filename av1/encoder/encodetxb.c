...
int get_vq_cost(MACROBLOCK *x, int plane, int blk_row, int blk_col,
  ...
  int qgain = mbmi->qgain[plane][blk_idx];
  int codeword = mbmi->codeword[plane][blk_idx];

  int gain_sym1, gain_sym2, cw_sym1, cw_sym2;
  av1_vq_qgain_to_symbols(qgain, &gain_sym1, &gain_sym2);
  av1_vq_cw_to_symbols(codeword, &cw_sym1, &cw_sym2);
  assert(gain_sym1 >= 0 && gain_sym1 < NUM_GAIN_SYMBOL1);
  assert(cw_sym1 >= 0 && cw_sym1 < NUM_CW_SYMBOL1);

  int gain_cost =
    x->vq_gain_1_costs[gain_sym1] + x->vq_gain_2_costs[gain_sym1][gain_sym2];
  int cw_cost =
    x->vq_codeword_1_costs[cw_sym1] + x->vq_codeword_2_costs[cw_sym1][cw_sym2];
  return gain_cost + cw_cost;
}
...
void av1_write_vq_txb(const MACROBLOCKD *xd, aom_writer *w, int blk_row,
  ...
  int16_t qgain = mbmi->qgain[plane][blk_idx];
  int gain_sign = qgain > 0;
  int16_t gain_mag = gain_sign ? qgain : -qgain;
  aom_write_bit(w, gain_sign);

  int gain_sym1, gain_sym2;
  av1_vq_qgain_to_symbols(gain_mag, gain_sym1, gain_sym2);
  aom_write_symbol(w, gain_sym1, ec_ctx->vq_gain_sym1_cdf, NUM_GAIN_SYMBOL1);
  aom_write_symbol(w, gain_sym2, ec_ctx->vq_gain_sym2_cdf[gain_sym1],

  int cw_sym1, cw_sym2;
  av1_vq_cw_to_symbols(mbmi->codeword[plane][blk_idx], cw_sym1, cw_sym2);
  aom_write_symbol(w, cw_sym1, ec_ctx->vq_codeword_1_cdf, NUM_CW_SYMBOL1);
  aom_write_symbol(w, cw_sym2, ec_ctx->vq_codeword_2_cdf[cw_sym1],
                   NUM_CW_SYMBOL2);
#if VQ_BS_DEBUG
    ...
    if (is_inter) {
      ...
      if (ext_tx_set > 0) {
        ...
        if (tx_set_type == EXT_TX_SET_DTT4_IDTX_1DDCT_MDTX3) {
          ...
          int tx_type_cost =
              is_mdtx ? x->mdtx_type_intra_costs[square_tx_size][intra_dir]
      ...
      if (is_inter) {
        ...
        if (allow_update_cdf) {
          ...
          if (tx_set_type == EXT_TX_SET_DTT4_IDTX_1DDCT_MDTX3) {
            ...
            if (is_mdtx) {
              update_cdf(
                  fc->mdtx_type_intra_cdf[txsize_sqr_map[tx_size]][intra_dir],
...
#if CONFIG_VQ4X4
  const TxSetType tx_set_type =
      av1_get_ext_tx_set_type(tx_size, is_inter_block(mbmi),
                              cm->reduced_tx_set_used);
  if (tx_set_type == EXT_TX_SET_VQ) {
    const int blk_idx = av1_get_txk_type_index(plane_bsize, blk_row, blk_col);
    int gain_sym1, gain_sym2, cw_sym1, cw_sym2;
    av1_vq_qgain_to_symbols(mbmi->qgain[plane][blk_idx], gain_sym1, gain_sym2);
    av1_vq_cw_to_symbols(mbmi->codeword[plane][blk_idx], cw_sym1, cw_sym2);
    update_cdf(ec_ctx->vq_gain_sym1_cdf, gain_sym1, NUM_GAIN_SYMBOL1);
    update_cdf(ec_ctx->vq_gain_sym2_cdf[gain_sym1], gain_sym2, NUM_GAIN_SYMBOL2);
    update_cdf(ec_ctx->vq_codeword_1_cdf, cw_sym1, NUM_CW_SYMBOL1);
    update_cdf(ec_ctx->vq_codeword_2_cdf[cw_sym1], cw_sym2, NUM_CW_SYMBOL2);
