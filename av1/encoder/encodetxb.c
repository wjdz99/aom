...
static INLINE int get_coeff_cost_new(int ci, tran_low_t abs_qc, int coeff_ctx,
...
static INLINE void update_coeff_general(
    ...
    TX_TYPE tx_type, int bwl, int height, int rdmult, int shift,
...
static INLINE void update_coeff_simple(
    int *accu_rate, int64_t *accu_dist, int si, int eob, TX_SIZE tx_size,
    TX_TYPE tx_type, int bwl, int rdmult, int shift, const int16_t *dequant,
  ...
  if (qc == 0) {
    *accu_rate += txb_costs->base_cost[coeff_ctx][0];
    ...
    const int64_t dist = get_coeff_dist(tqc, dqc, shift);
    const int64_t dist0 = get_coeff_dist(tqc, 0, shift);
    const int rate = get_coeff_cost_new(ci, abs_qc, coeff_ctx, txb_costs, bwl,
                                        tx_type, levels);
    const int64_t rd = RDCOST(rdmult, rate, dist);
    ...
    const tran_low_t abs_qc_low = abs_qc - 1;
    const int64_t dist_low = get_coeff_dist(tqc, dqc_low, shift);
    const int rate_low = get_coeff_cost_new(ci, abs_qc_low, coeff_ctx,
                                            txb_costs, bwl, tx_type, levels);
    const int64_t rd_low = RDCOST(rdmult, rate_low, dist_low);
    if (rd_low < rd) {
      qcoeff[ci] = qc_low;
...
static INLINE void update_coeff_eob(
    ...
    int dc_sign_ctx, int rdmult, int shift, const int16_t *dequant,
...
static INLINE void update_skip(int *accu_rate, int64_t *accu_dist, int *eob,
                               int rdmult, int skip_cost, int non_skip_cost,
