...
static const int shift_mul = (1 << AV1_PROB_COST_SHIFT);
static const int loge_par = (14427 * shift_mul + 5000) / 10000;
int av1_cost_coeffs_txb_estimate(const MACROBLOCK *x, const int plane,
  ...
  // constant terms
  const int const_term = shift_mul;
  ...
  // const_term does not contain DC, and loge does not contain eob, so both
  // (eob-1)
