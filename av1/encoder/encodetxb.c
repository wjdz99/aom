...
void av1_write_coeffs_txb(const AV1_COMMON *const cm, MACROBLOCKD *xd,
  ...
  const TX_TYPE tx_type = get_tx_type(plane_type, xd, blk_row, blk_col, block, tx_size);
  ...
  if (eob == 0) return;
#if CONFIG_TXK_SEL
  av1_write_tx_type(cm, xd, blk_row, blk_col, block, plane, get_min_tx_size(tx_size), w);
#endif
...
void av1_write_coeffs_mb(const AV1_COMMON *const cm, MACROBLOCK *x,
  ...
  for (row = 0; row < max_blocks_high; row += bkh) {
    for (col = 0; col < max_blocks_wide; col += bkw) {
      ...
      av1_write_coeffs_txb(cm, xd, w, row, col, block, plane, tx_size, tcoeff, eob,
                           &txb_ctx);
      block += step;
    }
  }
...
#if CONFIG_TXK_SEL
  av1_update_tx_type_count(cm, xd, blk_row, blk_col, block, plane, mbmi->sb_type,
...
int64_t av1_search_txk_type(const AV1_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  for (tx_type = txk_start; tx_type <= txk_end; ++tx_type) {
    ...
    TX_TYPE ref_tx_type =
        get_tx_type(get_plane_type(plane), xd, blk_row, blk_col, block, tx_size);
    ...
    this_rd_stats.rate = av1_cost_coeffs(
        cpi, x, plane, blk_row, blk_col, block, tx_size, scan_order, a, l, use_fast_coef_costing);
