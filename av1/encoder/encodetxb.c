...
static void gen_base_count_mag_arr(int (*base_count_arr)[MAX_TX_SQUARE],
  ...
  for (int c = 0; c < eob; ++c) {
    int coeff_idx = scan[c];  // raster order
    if (!has_base(qcoeff[coeff_idx], 0)) continue;
    int row = coeff_idx / stride;
    int col = coeff_idx % stride;
...
static void gen_nz_ctx_arr(int (*nz_ctx_arr)[2], int(*nz_count_arr),
  ...
  for (int c = 0; c < eob; ++c) {
    int coeff_idx = scan[c];  // raster order
    int count = nz_count_arr[coeff_idx];
...
// TODO(angiebird): find a simpler computing steps
static INLINE tran_low_t get_lower_coeff(tran_low_t qc) {
  if (qc == 0) {
    return 0;
  }
  const int sign = qc > 0 ? 1 : -1;
  const tran_low_t abs_qc = abs(qc);
  return sign * (abs_qc - 1);
}
...
int64_t av1_search_txk_type(const AV1_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  for (tx_type = txk_start; tx_type <= txk_end; ++tx_type) {
    ...
    av1_optimize_b(cm, x, plane, block, tx_size, a, l);
