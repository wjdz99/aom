    ...
    if (is_inter) {
      ...
      if (ext_tx_set > 0) {
        ...
        if (mbmi->filter_intra_mode_info.use_filter_intra)
          ...
          intra_dir = mbmi->mode;
#if CONFIG_IST
        int tx_type_cost = x->mode_costs.intra_tx_type_costs[ext_tx_set][square_tx_size][intra_dir][tx_type & 0xf];
        if ((tx_type & 0xf) == DCT_DCT || (tx_type & 0xf) == ADST_ADST)
        {
          tx_type_cost += x->mode_costs.stx_flag_cost[square_tx_size][tx_type >> 4];
        }
        return tx_type_cost;
#else
