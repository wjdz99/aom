...
static int get_coeff_cost(tran_low_t qc, int scan_idx, TxbInfo *txb_info,
                          TxbProbs *txb_probs, TxbCache *txb_cache) {
  (void)txb_cache;
...
int get_txb_cost(TxbInfo *txb_info, TxbProbs *txb_probs, TxbCache *txb_cache) {
  int cost = 0;
  ...
  for (int c = 0; c < txb_info->eob; ++c) {
    ...
    int coeff_cost = get_coeff_cost(qc, c, txb_info, txb_probs, txb_cache);
    cost += coeff_cost;
  }
  return cost;
}
