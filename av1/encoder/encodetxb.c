...
static int get_tx_type_cost(const AV1_COMMON *cm, const MACROBLOCK *x,
  ...
  if (get_ext_tx_types(tx_size, is_inter, cm->reduced_tx_set_used) > 1 &&
    ...
    if (is_inter) {
#if CONFIG_MODEDEP_TX && USE_NSTX_INTER
      ...
      if (ext_tx_set > 0) {
        ...
        if (mbmi->filter_intra_mode_info.use_filter_intra)
          ...
          intra_dir = mbmi->mode;
#if CONFIG_MODEDEP_TX && USE_NSTX_INTRA
        const TxSetType tx_set_type =
            av1_get_ext_tx_set_type(tx_size, is_inter, cm->reduced_tx_set_used);
        if (tx_set_type == EXT_TX_SET_DTT4_IDTX_1DDCT_NSTX3) {
          ...
          int tx_type_cost =
              is_nstx ? x->nstx_type_intra_costs[square_tx_size][intra_dir]
    ...
    if (eset > 0) {
      ...
      if (is_inter) {
        ...
        if (tx_set_type == EXT_TX_SET_DTT4_IDTX_1DDCT_NSTX3) {
          ...
          if (is_nstx)
            ++counts->nstx_type_intra[txsize_sqr_map[tx_size]][intra_dir]
                                     [tx_type - NSTX_INTRA_1];
          else
        ...
        if (allow_update_cdf) {
          ...
          if (tx_set_type == EXT_TX_SET_DTT4_IDTX_1DDCT_NSTX3) {
            ...
            if (is_nstx) {
              update_cdf(
                  fc->nstx_type_intra_cdf[txsize_sqr_map[tx_size]][intra_dir],
