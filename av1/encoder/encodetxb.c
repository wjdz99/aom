...
#else
  const int coeff_ctx =
      get_nz_map_ctx(levels, scan_idx, coeff_idx, txb_info->bwl,
...
void av1_write_coeffs_txb(const AV1_COMMON *const cm, MACROBLOCKD *xd,
...
#if CONFIG_LV_MAP_MULTI
    coeff_ctx =
        get_nz_map_ctx(levels, c, pos, bwl, height, c == eob - 1, tx_type);
...
#endif
#else
    coeff_ctx = get_nz_map_ctx(levels, c, pos, bwl, height, tx_type);
    const tran_low_t v = tcoeff[pos];
    const int is_nz = (v != 0);
  ...
  for (int i = 1; i < eob; ++i) {
    c = eob - 1 - i;
    const int pos = scan[c];
    const int coeff_ctx = get_nz_map_ctx(levels, c, pos, bwl, height, tx_type);
    const tran_low_t v = tcoeff[pos];
    const int is_nz = (v != 0);
...
#if !USE_CAUSAL_BASE_CTX
      const int coeff_ctx =
          get_nz_map_ctx(levels, c, pos, bwl, height, tx_type);
#endif
...
#if CONFIG_LV_MAP_MULTI
  const int coeff_ctx =
      get_nz_map_ctx(txb_info->levels, scan_idx, pos, txb_info->bwl,
