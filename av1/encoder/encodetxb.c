...
void av1_write_coeffs_txb(const AV1_COMMON *const cm, MACROBLOCKD *xd,
  ...
  for (c = update_eob; c >= 0; --c) {
    ...
    int val = AOMMIN(COEFF_BASE_RANGE, level - 1 - NUM_BASE_LEVELS);
    aom_write_symbol(w, val, cm->fc->coeff_lps_cdf[txs_ctx][plane_type][ctx],
                     COEFF_BASE_RANGE + 1);
    if (val < COEFF_BASE_RANGE) continue;
  ...
  for (c = 0; c < eob; ++c) {
    ...
    if (is_nz) {
      ...
      if (level > NUM_BASE_LEVELS) {
        ...
        idx = AOMMIN(COEFF_BASE_RANGE, level - 1 - NUM_BASE_LEVELS);
        cost += coeff_costs->lps_cost[ctx][idx];
...
void av1_update_and_record_txb_context(int plane, int block, int blk_row,
...
#if LV_MAP_PROB
  update_cdf(cm->fc->txb_skip_cdf[txsize_ctx][txb_ctx.txb_skip_ctx],
             eob == 0, 2);
#endif
...
#if LV_MAP_PROB
    update_cdf(cm->fc->nz_map_cdf[txsize_ctx][plane_type][coeff_ctx],
               is_nz, 2);
#endif
...
#if LV_MAP_PROB
      update_cdf(cm->fc->eob_flag_cdf[txsize_ctx][plane_type][eob_ctx],
                 c == (eob - 1), 2);
#endif
    }
  }
...
#if LV_MAP_PROB
        update_cdf(cm->fc->coeff_base_cdf[txsize_ctx][plane_type][i][ctx],
                   1, 2);
#endif
        if (c == 0) {
          int dc_sign_ctx = txb_ctx.dc_sign_ctx;
...
#if LV_MAP_PROB
          update_cdf(cm->fc->dc_sign_cdf[plane_type][dc_sign_ctx],
                     v < 0, 2);
#endif
  ...
  for (c = update_eob; c >= 0; --c) {
    ...
    update_cdf(cm->fc->coeff_lps_cdf[txsize_ctx][plane_type][ctx],
               val, COEFF_BASE_RANGE + 1);
