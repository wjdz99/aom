...
#if CONFIG_NEW_QUANT
#if CONFIG_AOM_QM
                                           int dq_profile, int is_ac_coeff,
#else
...
#if CONFIG_AOM_QM
  int dqcoeff =
      av1_dequant_coeff_nuq(abs(qc), dqv, dq_profile, is_ac_coeff, shift);
#else
...
#if CONFIG_NEW_QUANT
#if CONFIG_AOM_QM
  const int dq_idx = txb_info->dq_profile;
#else
    ...
    if (qc == 0) {
      ...
      if (has_nz_tail < 2) {
        ...
        if (cur_eob_rd_cost <= prev_eob_rd_cost) {
          ...
          accu_dist = 0;
          get_dist_cost_stats(&stats, si, 1, txb_costs, txb_info, has_nz_tail);
