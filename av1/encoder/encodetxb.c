...
static INLINE int get_coeff_cost_eob(int ci, tran_low_t abs_qc,
...
static AOM_FORCE_INLINE void update_coeff_eob(
  ...
  if (qc == 0) {
    *accu_rate += txb_costs->base_cost[coeff_ctx][0];
    ...
    int rate_coeff_eob =
        new_eob_cost + get_coeff_cost_eob(ci, abs_qc, sign,
    ...
    if (abs_qc_low > 0) {
      const int rate_coeff_eob_low =
          new_eob_cost +
          get_coeff_cost_eob(ci, abs_qc_low, sign, coeff_ctx_new_eob,
