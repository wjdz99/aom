...
static void update_tx_type_count(const AV1_COMP *cpi, const AV1_COMMON *cm,
  ...
  if (is_inter) {
    ...
    if (cpi->oxcf.txfm_cfg.use_intra_dct_only) {
      ...
      // We need to fix txfm cfg for these cases. Meanwhile relieving the assert.
      assert(tx_type == default_type || cpi->sf.rt_sf.use_nonrd_pick_mode || cpi->oxcf.mode == REALTIME);
    }
  }
