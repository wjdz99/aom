  ...
  if (get_ext_tx_types(tx_size, is_inter, cm->reduced_tx_set_used) > 1 &&
    ...
    if (is_inter) {
      ...
      if (tx_set_type == EXT_TX_SET_ALL16_GFT) {
        int use_gft_cost = x->use_gft_costs[square_tx_size][tx_type >= GFT1_GFT1];
        int type_cost = tx_type >= GFT1_GFT1 ?
            x->gft_type_costs[square_tx_size][tx_type - GFT1_GFT1] :
            x->inter_tx_type_costs[ext_tx_set][square_tx_size][tx_type];
        return use_gft_cost + type_cost;
      } else {
