...
// hbt_hash_table[HBT_TABLE_SIZE * HBT_ARRAY_LENGTH];
...
int hbt_hash_miss(uint16_t hbt_table_index, int array_index,
  ...
  // Overwrite old entry
  hbt_hash_table[hbt_table_index * HBT_ARRAY_LENGTH + array_index].rate_cost =
      *rate_cost;
...
int hbt_hash_hit(uint16_t hbt_table_index, int array_index, TxbInfo *txb_info,
                 const struct macroblock_plane *p, int block, int *rate_cost) {
  const int16_t *scan = txb_info->scan_order->scan;
  int new_eob = 0;
  int update = 0;
  ...
  *rate_cost = hbt_hash_table[hbt_table_index * HBT_ARRAY_LENGTH + array_index]
                   .rate_cost;
...
int hbt_search_match(uint16_t hbt_table_index, uint32_t hbt_qc_hash,
  ...
  int array_index = hbt_qc_hash % HBT_ARRAY_LENGTH;
...
int hbt_create_hashes(TxbInfo *txb_info, const LV_MAP_COEFF_COST *txb_costs,
  ...
  //// Hash creation
  assert(HBT_EOB + 1 <=
         65);  // must hold a byte per coeff, plus one for dequant
  uint8_t txb_hash_data[65];
  const int16_t *scan = txb_info->scan_order->scan;
  uint8_t chunk = 0;
  ...
  hash_data_index++;  // Needed when packing_index != 0, to include final byte.
