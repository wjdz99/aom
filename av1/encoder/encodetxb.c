...
void av1_alloc_txb_buf(AV1_COMP *cpi) {
  ...
  if ((size_t)size > SIZE_MAX / (size_t)total_max_sb_square) {
    aom_internal_error(cm->error, AOM_CODEC_ERROR,
                       "%zu * %zu would overflow size_t", (size_t)size,
                       (size_t)total_max_sb_square);
  }
  ...
  if (sizeof(*coeff_buf_pool->tcoeff) > SIZE_MAX / num_tcoeffs) {
    aom_internal_error(cm->error, AOM_CODEC_ERROR,
                       "%zu * %zu would overflow size_t",
                       sizeof(*coeff_buf_pool->tcoeff), num_tcoeffs);
  }
  CHECK_MEM_ERROR(
  ...
  if (sizeof(*coeff_buf_pool->eobs) > SIZE_MAX / num_tcoeffs) {
    aom_internal_error(cm->error, AOM_CODEC_ERROR,
                       "%zu * %zu would overflow size_t",
                       sizeof(*coeff_buf_pool->eobs), num_tcoeffs);
  }
  CHECK_MEM_ERROR(
  ...
  if (sizeof(*coeff_buf_pool->entropy_ctx) > SIZE_MAX / num_tcoeffs) {
    aom_internal_error(cm->error, AOM_CODEC_ERROR,
                       "%zu * %zu would overflow size_t",
                       sizeof(*coeff_buf_pool->entropy_ctx), num_tcoeffs);
  }
