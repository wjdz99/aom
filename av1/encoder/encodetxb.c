  ...
  if (get_ext_tx_types(tx_size, is_inter, cm->reduced_tx_set_used) > 1 &&
    ...
    if (is_inter) {
      ...
      if (tx_set_type == EXT_TX_SET_ALL16_DDTX) {
        int use_ddtx_cost =
            x->use_ddtx_costs[square_tx_size][tx_type >= DDTX1_DDTX1];
        int tx_type_cost =
            tx_type >= DDTX1_DDTX1
                ? x->ddtx_type_costs[square_tx_size][tx_type - DDTX1_DDTX1]
...
#if CONFIG_DATA_DRIVEN_TX
          if (tx_set_type == EXT_TX_SET_ALL16_DDTX) {
            update_cdf(fc->use_ddtx_cdf[txsize_sqr_map[tx_size]],
                       tx_type >= DDTX1_DDTX1, 2);
            if (tx_type >= DDTX1_DDTX1) {
              update_cdf(fc->ddtx_type_cdf[txsize_sqr_map[tx_size]],
                         tx_type - DDTX1_DDTX1, DDTX_TYPES);
            } else {
...
#if CONFIG_DATA_DRIVEN_TX
        if (tx_set_type == EXT_TX_SET_ALL16_DDTX) {
				  ++counts->use_ddtx[txsize_sqr_map[tx_size]][tx_type >= DDTX1_DDTX1];
          if (tx_type >= DDTX1_DDTX1)
            ++counts->ddtx_type[txsize_sqr_map[tx_size]][tx_type - DDTX1_DDTX1];
          else
