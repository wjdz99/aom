...
void get_dist_cost_stats(LevelDownStats *stats, int scan_idx,
  ...
  int coeff_ctx = get_nz_map_ctx(levels, scan_idx, scan, txb_info->bwl,
                                 txb_info->height, txb_info->tx_type, 1);
...
static void update_coeff(const int coeff_idx, const tran_low_t qc,
                         const TxbInfo *const txb_info) {
  txb_info->qcoeff[coeff_idx] = qc;
  txb_info->levels[coeff_idx] =
      (uint8_t)clamp(abs(txb_info->qcoeff[coeff_idx]), 0, UINT8_MAX);
  txb_info->signs[coeff_idx] = (txb_info->qcoeff[coeff_idx] < 0);
  const int dqv = txb_info->dequant[coeff_idx != 0];
...
static INLINE void write_nz_map(aom_writer *const w,
                                const uint8_t *const levels, const uint16_t eob,
...
#if CONFIG_CTX1D
static INLINE void write_nz_map_vert(
    aom_writer *const w, const tran_low_t *const tcoeff,
    const uint8_t *const levels, const uint16_t eob, const int plane,
    const int16_t *const scan, const int16_t *const iscan,
    const TX_SIZE tx_size, const TX_TYPE tx_type, FRAME_CONTEXT *const fc) {
  (void)eob;
  const TX_SIZE txs_ctx = get_txsize_context(tx_size);
  const PLANE_TYPE plane_type = get_plane_type(plane);
  const TX_CLASS tx_class = get_tx_class(tx_type);
...
static INLINE void write_nz_map_horiz(
    aom_writer *const w, const tran_low_t *tcoeff, const uint8_t *const levels,
    const uint16_t eob, const int plane, const int16_t *const scan,
    const int16_t *const iscan, const TX_SIZE tx_size, const TX_TYPE tx_type,
    FRAME_CONTEXT *const fc) {
  (void)scan;
  (void)eob;
  const TX_SIZE txs_ctx = get_txsize_context(tx_size);
...
static INLINE void av1_txb_get_levels_and_signs(const tran_low_t *const coeff,
                                                const int size,
...
void av1_write_coeffs_txb(const AV1_COMMON *const cm, MACROBLOCKD *xd,
  ...
  av1_txb_get_levels_and_signs(tcoeff, seg_eob, levels, signs);
...
//                 ec_ctx->nz_map_cdf[txs_ctx][plane_type][coeff_ctx], 2);
  ...
  if (tx_class == TX_CLASS_2D) {
    write_nz_map(w, levels, eob, plane, scan, tx_size, tx_type, ec_ctx);
  } else {
    const int width = tx_size_wide[tx_size];
    const int eob_offset = width + height;
    ...
    if (eob_mode == 0) {
      ...
      if (tx_class == TX_CLASS_VERT)
        write_nz_map_vert(w, tcoeff, levels, eob, plane, scan, iscan, tx_size,
                          tx_type, ec_ctx);
      else
        write_nz_map_horiz(w, tcoeff, levels, eob, plane, scan, iscan, tx_size,
                           tx_type, ec_ctx);
    }
  }
#else
...
static int get_nz_eob_map_cost(const LV_MAP_COEFF_COST *coeff_costs,
                               const uint8_t *const levels, uint16_t eob,
                               int plane, const int16_t *scan, TX_SIZE tx_size,
                               TX_TYPE tx_type) {
  (void)plane;
...
#if CONFIG_CTX1D
static INLINE int get_nz_eob_map_cost_vert(
    const LV_MAP_COEFF_COST *const coeff_costs, const tran_low_t *const qcoeff,
    const uint8_t *const levels, const uint16_t eob, const int plane,
    const int16_t *const scan, const int16_t *const iscan,
    const TX_SIZE tx_size, const TX_TYPE tx_type) {
  (void)tx_size;
...
int av1_cost_coeffs_txb(const AV1_COMMON *const cm, MACROBLOCK *x, int plane,
  ...
  av1_txb_get_levels_and_signs(qcoeff, tx_size_2d[tx_size], levels, signs);
  ...
  TX_CLASS tx_class = get_tx_class(tx_type);
  if (tx_class == TX_CLASS_2D) {
    cost += get_nz_eob_map_cost(coeff_costs, levels, eob, plane, scan, tx_size,
                                tx_type);
  } else {
    const int width = tx_size_wide[tx_size];
    const int eob_offset = width + height;
    ...
    if (eob_mode == 0) {
      cost += get_nz_eob_map_cost(coeff_costs, levels, eob, plane, scan,
                                  tx_size, tx_type);
    } else {
      const int16_t *iscan = scan_order->iscan;
      assert(tx_class == TX_CLASS_VERT || tx_class == TX_CLASS_HORIZ);
      if (tx_class == TX_CLASS_VERT)
        cost += get_nz_eob_map_cost_vert(coeff_costs, qcoeff, levels, eob,
                                         plane, scan, iscan, tx_size, tx_type);
      else
        cost += get_nz_eob_map_cost_horiz(coeff_costs, qcoeff, levels, eob,
                                          plane, scan, iscan, tx_size, tx_type);
    }
  }
#else   // CONFIG_CTX1D
...
static void gen_txb_cache(TxbCache *const txb_cache,
                          const TxbInfo *const txb_info) {
  // gen_nz_count_arr
  const int16_t *scan = txb_info->scan_order->scan;
  ...
  for (int c = 0; c < txb_info->eob; ++c) {
    ...
    txb_cache->nz_count_arr[coeff_idx] =
        get_nz_count(levels, bwl, height, row, col, 1, prev_row, prev_col);
#else
    txb_cache->nz_count_arr[coeff_idx] =
        get_nz_count(levels, bwl, height, row, col
#if CONFIG_EOB_FIRST
                     ,
...
static int try_neighbor_level_down_nz(int coeff_idx, int nb_coeff_idx,
  ...
  if (nb_scan_idx < scan_idx) {
    ...
    txb_info->levels[nb_coeff_idx] =
        (uint8_t)clamp(abs(txb_info->qcoeff[nb_coeff_idx]), 0, UINT8_MAX);
    txb_info->signs[nb_coeff_idx] = (txb_info->qcoeff[nb_coeff_idx] < 0);
    const int new_ctx =
...
void test_try_change_eob(TxbInfo *txb_info, const LV_MAP_COEFF_COST *txb_costs,
  ...
  if (eob > 0) {
    ...
    if (abs(last_coeff) == 1) {
      ...
      txb_info->levels[last_ci] =
          (uint8_t)clamp(abs(txb_info->qcoeff[last_ci]), 0, UINT8_MAX);
      txb_info->signs[last_ci] = (txb_info->qcoeff[last_ci] < 0);
      set_eob(txb_info, new_eob);
...
static INLINE void av1_update_nz_eob_counts(
    FRAME_CONTEXT *const fc, FRAME_COUNTS *const counts, const uint16_t eob,
    const uint8_t *const levels, const int plane, const TX_SIZE tx_size,
    const TX_TYPE tx_type, const int16_t *const scan) {
  const PLANE_TYPE plane_type = get_plane_type(plane);
  const int bwl = b_width_log2_lookup[txsize_to_bsize[tx_size]] + 2;
  const int height = tx_size_high[tx_size];
...
static INLINE void av1_update_nz_eob_counts_vert(
    FRAME_CONTEXT *const fc, FRAME_COUNTS *const counts, const uint16_t eob,
    const tran_low_t *const tcoeff, const uint8_t *const levels,
    const int plane, const TX_SIZE tx_size, const TX_TYPE tx_type,
    const int16_t *const scan, const int16_t *const iscan) {
  (void)eob;
  const TX_SIZE txs_ctx = get_txsize_context(tx_size);
  const PLANE_TYPE plane_type = get_plane_type(plane);
...
void av1_update_and_record_txb_context(int plane, int block, int blk_row,
  ...
  av1_txb_get_levels_and_signs(tcoeff, tx_size_2d[tx_size], levels, signs);
  ...
  TX_CLASS tx_class = get_tx_class(tx_type);
  if (tx_class == TX_CLASS_2D) {
    av1_update_nz_eob_counts(ec_ctx, td->counts, eob, levels, plane, tx_size,
                             tx_type, scan);
  } else {
    const int width = tx_size_wide[tx_size];
    const int eob_offset = width + height;
    ...
    if (eob_mode == 0) {
      ...
      assert(tx_class == TX_CLASS_VERT || tx_class == TX_CLASS_HORIZ);
      if (tx_class == TX_CLASS_VERT)
        av1_update_nz_eob_counts_vert(ec_ctx, td->counts, eob, tcoeff, levels,
                                      plane, tx_size, tx_type, scan, iscan);
      else
        av1_update_nz_eob_counts_horiz(ec_ctx, td->counts, eob, tcoeff, levels,
                                       plane, tx_size, tx_type, scan, iscan);
    }
  }
#else   // CONFIG_CTX1D
