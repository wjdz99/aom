...
static AOM_INLINE void allocate_winner_mode_stats(const AV1_COMP *cpi,
  ...
  // winner_mode_stats buffer is not required in the following cases:
  // 1) When use_nonrd_pick_mode sf is set to 1 and hybrid_intra_pickmode sf is
  //    set to 0.
...
static AOM_INLINE void av1_alloc_mb_data(const AV1_COMP *cpi,
  ...
  allocate_winner_mode_stats(cpi, mb, sf);
}
