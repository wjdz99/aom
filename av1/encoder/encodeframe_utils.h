...
static AOM_INLINE void av1_dealloc_mb_data(struct AV1Common *cm,
  ...
  aom_free(mb->txfm_search_info.mb_rd_records);
  mb->txfm_search_info.mb_rd_records = NULL;
...
static AOM_INLINE void av1_alloc_mb_data(struct AV1Common *cm,
  ...
  if (!use_nonrd_pick_mode) {
    // Memory for mb_rd_records is allocated only when use_mb_rd_hash sf is
    // enabled.
    if (use_mb_rd_hash)
      mb->txfm_search_info.mb_rd_records =
