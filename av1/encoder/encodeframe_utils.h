...
static AOM_INLINE void av1_alloc_mb_data(struct AV1Common *cm,
                                         struct SPEED_FEATURES *sf,
                                         struct macroblock *mb) {
  const int use_mb_rd_hash = sf->rd_sf.use_mb_rd_hash;
  const int use_nonrd_pick_mode = sf->rt_sf.use_nonrd_pick_mode;

  if (!use_nonrd_pick_mode) {
    // Memory for mb_rd_record is allocated only when
    // use_mb_rd_hash sf is enabled.
    if (use_mb_rd_hash)
