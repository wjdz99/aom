...
// encoding of top-right block is completed. This function checks if top right
// dependency wait at mi level can be enabled.
static AOM_INLINE int enable_top_right_sync_wait_in_mis(const AV1_COMP *cpi) {
  ...
  return cpi->mt_info.row_mt_enabled && (!frame_is_intra_only(&cpi->common)) &&
         cpi->sf.rt_sf.use_nonrd_pick_mode && delay_wait_for_top_right_sb(cpi);
}
