...
void av1_calc_mb_wiener_var_row(AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  const size_t buf_size =
      is_high_bitdepth ? buf_width * buf_height * sizeof(*pred_buffer) * 2
                       : buf_width * buf_height * sizeof(*pred_buffer);
  CHECK_MEM_ERROR(cm, pred_buffer, aom_memalign(32, buf_size));
  uint8_t *dst_buffer;
  if (is_high_bitdepth) {
    uint8_t *tmp_pred = CONVERT_TO_BYTEPTR(pred_buffer);
    uint16_t *pred_buffer_16 = CONVERT_TO_SHORTPTR(tmp_pred);
    pred_buffer_16 = pred_buffer_16 + buf_width * offset + offset;
    dst_buffer = CONVERT_TO_BYTEPTR(pred_buffer_16);
  } else {
    dst_buffer = pred_buffer + buf_width * offset + offset;
  }
