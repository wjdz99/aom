    ...
    for (int sb_col = 0; sb_col < num_col_sbs; ++sb_col) {
      ...
      do {
        ...
        for (int mb_row = 0; mb_row < sb_height / mb_step; ++mb_row) {
          for (int mb_col = 0; mb_col < sb_width / mb_step; ++mb_col) {
            ...
            const double entropy =
                estimate_txfm_block_entropy(base_qindex, abs_coeff_mean,
                                            qcoeff, pix_num);
            sb_entropy += entropy;
          }
        }
        ...
        if (sb_row == 1 && sb_col == 12) {
          printf("iter %d, base_qindex %d, avg_sb_entropy %.0f, sb_entropy %.0f\n",
                 iter, base_qindex, avg_sb_entropy, sb_entropy);
        }
        */
      } while (iter < max_iter && (sb_entropy > upper_sb_entropy || sb_entropy < lower_sb_entropy));
      /*
      if (iter == 1) orig_entropy = sb_entropy;
      FILE *pfile = fopen("entropy.stat", "a");
      fprintf(pfile, "(%d, %d), avg_sb_entropy %.0f, orig_entropy %.0f, entropy %.0f, ratio %.2f, base_qindex %d\n",
              sb_row, sb_col, avg_sb_entropy, orig_entropy, sb_entropy, sb_entropy / orig_entropy, base_qindex);
      fclose(pfile);
      */
...
void av1_set_mb_wiener_variance(AV1_COMP *cpi) {
  ...
  for (mb_row = 0; mb_row < cpi->frame_info.mb_rows; ++mb_row) {
    for (mb_col = 0; mb_col < cpi->frame_info.mb_cols; ++mb_col) {
      ...
      const double entropy =
          estimate_txfm_block_entropy(cm->quant_params.base_qindex,
                                      abs_coeff_mean, qcoeff, pix_num);
      ...
      */
      ...
      //       cpi->mb_weber_stats[mb_row * cpi->frame_info.mb_cols + mb_col].mb_wiener_variance);
      /*
      FILE *pfile = fopen("entropy_variance.stat", "a");
  ...
  for (mb_row = 0; mb_row < cpi->frame_info.mb_rows; ++mb_row) {
    for (mb_col = 0; mb_col < cpi->frame_info.mb_cols; ++mb_col) {
      fprintf(pfile, "%.0f,",
              cpi->mb_weber_stats[mb_row * cpi->frame_info.mb_cols + mb_col].entropy);
    }
    fprintf(pfile, "\n");
  }
  fclose(pfile);
