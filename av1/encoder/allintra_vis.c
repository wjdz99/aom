...
static int64_t pick_norm_factor_and_block_size(AV1_COMP *const cpi,
  ...
  BLOCK_SIZE last_block_size = sb_size;
  *best_block_size = sb_size;
  // Pick from block size 64x64, 32x32 and 16x16.
  do {
    last_block_size = *best_block_size;
    if (mi_size_wide[last_block_size] < 8) break;
    *best_block_size = pick_block_size(cpi, last_block_size);
  } while (*best_block_size != last_block_size);

  int64_t norm_factor = 1;
  const BLOCK_SIZE norm_block_size = *best_block_size;
