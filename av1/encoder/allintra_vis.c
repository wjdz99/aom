...
void av1_set_mb_wiener_variance(AV1_COMP *cpi) {
  ...
  for (mb_row = 0; mb_row < cpi->frame_info.mb_rows; ++mb_row) {
    for (mb_col = 0; mb_col < cpi->frame_info.mb_cols; ++mb_col) {
      ...
      if (mi_row == 0 && mi_col == 48)
        fprintf(stderr,
                "sqrt dist = %ld, src var sqrt = %ld, rec var sqrt = %ld, "
                "dist_mean = %d, alpha = %lf\n\n",
                (int)sqrt(weber_stats->distortion),
                (int)sqrt(weber_stats->src_variance),
                (int)sqrt(weber_stats->rec_variance), dist_mean,
                weber_stats->alpha);
