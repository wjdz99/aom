...
int av1_get_sbq_variance_boost(const AV1_COMP *cpi, const MACROBLOCK *x) {
  ...
  qstep_ratio = fclamp(qstep_ratio, 1, VAR_BOOST_MAX_BOOST);
  ...
  int boost = -(int)((base_qindex + 544.0) * (target_qindex - base_qindex) /
                     (255.0 + 1024.0));
  boost = AOMMIN(VAR_BOOST_MAX_DELTAQ_RANGE, boost);

  // Variance Boost was designed to always operate in the lossy domain.
