...
void av1_init_quantizer(QuantDequantInfo *const quant_dequant_info,
                        const CommonQuantParams *quant_params,
                        aom_bit_depth_t bit_depth) {
  QUANTS *const quants = &quant_dequant_info->quants;
  Dequants *const dequants = &quant_dequant_info->dequants;
...
void av1_init_plane_quantizers(const AV1_COMP *cpi, MACROBLOCK *x,
  ...
  const QUANTS *const quants = &cpi->quant_dequant_info.quants;
  const Dequants *const dequants = &cpi->quant_dequant_info.dequants;
