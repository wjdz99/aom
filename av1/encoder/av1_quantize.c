    ...
    {  // rc == 0
      ...
      if (abs_coeff >= thresh0) {
        ...
        if (tmp32) {
          ...
          tran_low_t test = (tmp32 * dequant_ptr[0]) >> log_scale;
          dqcoeff_ptr[0] = (test ^ coeff_sign) - coeff_sign;
          eob = 0;
        }
      }
    }
    ...
    for (i = 1; i < n_coeffs; i++) {
      ...
      if (abs_coeff >= thresh1) {
        ...
        if (tmp32) {
          ...
          tran_low_t test = (tmp32 * dequant_ptr[1]) >> log_scale;
          dqcoeff_ptr[i] = (test ^ coeff_sign) - coeff_sign;
