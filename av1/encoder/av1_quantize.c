...
static INLINE int quantize_coeff_fp_nuq(
  ...
  int q = 0;
  tran_low_t zbin = 0;
  const int dequant_wt =
      (dequant * iwt + (1 << (AOM_QM_BITS - 1))) >>
      AOM_QM_BITS;

#if CONFIG_AOM_QM
  (void)cuml_bins_ptr;
  cuml_bins_type_nuq cuml_bins_aom_qm;
  get_cuml_bins_nuq(dequant, is_ac_coeff, cuml_bins_aom_qm, dq);
  zbin = cuml_bins_aom_qm[0];
#else
  zbin = cuml_bins_ptr[0];
#endif  // CONFIG_AOM_QM

  if (abs_coeff * wt >= (zbin * (1 << AOM_QM_BITS))) {
    int tmp = clamp(abs_coeff, INT16_MIN, INT16_MAX);
#if CONFIG_AOM_QM
    get_cuml_bins_nuq(dequant_wt, is_ac_coeff, cuml_bins_aom_qm, dq);
    zbin = cuml_bins_aom_qm[0];
#endif  // CONFIG_AOM_QM
    tmp -= zbin;
    q = NUQ_KNOTS + (((int64_t)tmp * wt * quant) >> (16 + AOM_QM_BITS));
#if CONFIG_AOM_QM
    (void)dequant_val;
    *dqcoeff_ptr = av1_dequant_abscoeff_nuq(q, dequant_wt, dq, is_ac_coeff, 0);
#else
    (void)dq;
    (void)is_ac_coeff;
    *dqcoeff_ptr = av1_dequant_abscoeff_nuq(q, dequant, dequant_val, 0);
