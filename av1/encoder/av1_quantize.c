...
void av1_quantize_fp_helper_c(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  if (!skip_block) {
    ...
    for (i = 0; i < n_coeffs; i++) {
      ...
      const int shift = log_scale;
      int tmp32 = 0;
#if CONFIG_AOM_QM
      if (abs_coeff * wt >=
          (dequant_ptr[rc != 0] << (AOM_QM_BITS - (1 + shift)))) {
#else
      if (abs_coeff >= (dequant_ptr[rc != 0] >> (1 + shift))) {
#endif
        abs_coeff += ROUND_POWER_OF_TWO(round_ptr[rc != 0], shift);
        abs_coeff = clamp(abs_coeff, INT16_MIN, INT16_MAX);
#if CONFIG_AOM_QM
        tmp32 = (int)((abs_coeff * wt * quant_ptr[rc != 0]) >>
                      ((16 - shift) + AOM_QM_BITS));
        qcoeff_ptr[rc] = (tmp32 ^ coeff_sign) - coeff_sign;
        dqcoeff_ptr[rc] = qcoeff_ptr[rc] * dequant / (1 << shift);
#else
        tmp32 = (int)((abs_coeff * quant_ptr[rc != 0]) >> (16 - shift));
        qcoeff_ptr[rc] = (tmp32 ^ coeff_sign) - coeff_sign;
        dqcoeff_ptr[rc] = qcoeff_ptr[rc] * dequant_ptr[rc != 0] / (1 << shift);
#endif
      }
...
void av1_quantize_fp_c(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  av1_quantize_fp_helper_c(coeff_ptr, n_coeffs, skip_block, zbin_ptr, round_ptr,
  ...
  switch (qparam->log_scale) {
    case 0:
      if (n_coeffs < 16) {
        ...
        // quantization.
        av1_quantize_fp_helper_c(coeff_ptr, n_coeffs, skip_block, p->zbin,
