...
void av1_set_quantizer(AV1_COMMON *const cm, int min_qmlevel, int max_qmlevel,
  ...
  if (enable_chroma_deltaq) {
    if (is_allintra && tuning == AOM_TUNE_SSIMULACRA2) {
      int chroma_delta_q_dc = 0;
      int chroma_delta_q_ac = 0;
      ...
      if (cm->seq_params->subsampling_x == 1 &&
        ...
        chroma_delta_q_dc = -clamp((quant_params->base_qindex / 2) - 14, 0, 16);
        chroma_delta_q_ac = chroma_delta_q_dc;
      } else if (cm->seq_params->subsampling_x == 0 &&
                 cm->seq_params->subsampling_y == 0) {
        // 4:4:4 subsampling: Constant chroma decrease with gradual ramp-down
        // for very high-quality qindexes.
        // Raising chroma delta_q AC by 24 was found to improve SSIMULACRA 2
        ...
        // chroma channels.
        // Raising chroma delta_q DC appears to be harmful, both for SSIMULACRA
        // 2 scores and subjective quality (harshens blocking artifacts).
        // The decrease ramp-down is there so very high-quality qindexes can
        // score better in an absolute sense.
        chroma_delta_q_dc = 0;
        chroma_delta_q_ac = clamp((quant_params->base_qindex / 2), 0, 24);
      }

      // TODO: bug https://crbug.com/aomedia/375221136 - find chroma_delta_q
      // values for 4:2:2 subsampling mode.
      quant_params->u_dc_delta_q = chroma_delta_q_dc;
      quant_params->u_ac_delta_q = chroma_delta_q_ac;
      quant_params->v_dc_delta_q = chroma_delta_q_dc;
      quant_params->v_ac_delta_q = chroma_delta_q_ac;
    } else {
