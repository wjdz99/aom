...
void av1_build_quantizer(aom_bit_depth_t bit_depth, int y_dc_delta_q,
  ...
  for (q = 0; q < QINDEX_RANGE; q++) {
    ...
    for (i = 0; i < 2; ++i) {
      ...
      // uv
      quant = i == 0 ? av1_dc_quant(q, u_dc_delta_q, bit_depth)
                     : av1_ac_quant(q, u_ac_delta_q, bit_depth);
      invert_quant(&quants->u_quant[q][i], &quants->u_quant_shift[q][i], quant);
      quants->u_quant_fp[q][i] = (1 << 16) / quant;
      quants->u_round_fp[q][i] = (qrounding_factor_fp * quant) >> 7;
      quants->u_zbin[q][i] = ROUND_POWER_OF_TWO(qzbin_factor * quant, 7);
      quants->u_round[q][i] = (qrounding_factor * quant) >> 7;
      deq->u_dequant[q][i] = quant;

      quant = i == 0 ? av1_dc_quant(q, v_dc_delta_q, bit_depth)
                     : av1_ac_quant(q, v_ac_delta_q, bit_depth);
      invert_quant(&quants->v_quant[q][i], &quants->v_quant_shift[q][i], quant);
      quants->v_quant_fp[q][i] = (1 << 16) / quant;
      quants->v_round_fp[q][i] = (qrounding_factor_fp * quant) >> 7;
      quants->v_zbin[q][i] = ROUND_POWER_OF_TWO(qzbin_factor * quant, 7);
      quants->v_round[q][i] = (qrounding_factor * quant) >> 7;
      deq->v_dequant[q][i] = quant;
    }
    ...
    for (i = 2; i < 8; i++) {  // 8: SIMD width
      ...
      deq->u_dequant[q][i] = deq->u_dequant[q][1];
      deq->v_dequant[q][i] = deq->v_dequant[q][1];
    }
  }
}
...
void av1_init_plane_quantizers(const AV1_COMP *cpi, MACROBLOCK *x,
  ...
  // U
  {
    x->plane[1].quant = quants->u_quant[qindex];
    x->plane[1].quant_fp = quants->u_quant_fp[qindex];
    x->plane[1].round_fp = quants->u_round_fp[qindex];
    x->plane[1].quant_shift = quants->u_quant_shift[qindex];
    x->plane[1].zbin = quants->u_zbin[qindex];
    x->plane[1].round = quants->u_round[qindex];
#if CONFIG_AOM_QM
...
#endif
    xd->plane[1].dequant = cpi->dequants.u_dequant[qindex];
#if CONFIG_NEW_QUANT
    for (dq = 0; dq < QUANT_PROFILES; dq++) {
      x->plane[1].cuml_bins_nuq[dq] = quants->uv_cuml_bins_nuq[dq][qindex];
      xd->plane[1].dequant_val_nuq[dq] =
  ...
  // V
  {
    x->plane[2].quant = quants->v_quant[qindex];
    x->plane[2].quant_fp = quants->v_quant_fp[qindex];
    x->plane[2].round_fp = quants->v_round_fp[qindex];
    x->plane[2].quant_shift = quants->v_quant_shift[qindex];
    x->plane[2].zbin = quants->v_zbin[qindex];
    x->plane[2].round = quants->v_round[qindex];
#if CONFIG_AOM_QM
    ...
    memcpy(&xd->plane[2].seg_iqmatrix[segment_id], cm->giqmatrix[qmlevel][1],
           sizeof(cm->giqmatrix[qmlevel][1]));
#endif
    xd->plane[2].dequant = cpi->dequants.v_dequant[qindex];
#if CONFIG_NEW_QUANT
    for (dq = 0; dq < QUANT_PROFILES; dq++) {
      x->plane[2].cuml_bins_nuq[dq] = quants->uv_cuml_bins_nuq[dq][qindex];
      xd->plane[2].dequant_val_nuq[dq] =
          cpi->dequants.uv_dequant_val_nuq[dq][qindex];
    }
#endif  // CONFIG_NEW_QUANT
  }
