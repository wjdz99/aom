...
static INLINE int quantize_coeff_bigtx_nuq(
    ...
    const tran_low_t *cuml_bins_ptr, const tran_low_t *dequant_val,
    tran_low_t *qcoeff_ptr, tran_low_t *dqcoeff_ptr, const qm_val_t wt,
    const qm_val_t iwt, int logsizeby16) {
...
void quantize_dc_nuq(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  if (!skip_block) {
    ...
    if (quantize_coeff_nuq(coeff_ptr[rc], quant, quant_shift, zbin_ptr[rc],
                           dequant, dq, rc, cuml_bins_ptr, dequant_val,
                           qcoeff_ptr, dqcoeff_ptr, wt, iwt))
      eob = 0;
  }
  *eob_ptr = eob + 1;
}
...
void quantize_dc_fp_nuq(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  if (!skip_block) {
    ...
    const qm_val_t wt = qm_ptr ? qm_ptr[rc] : (1 << AOM_QM_BITS);
    const qm_val_t iwt = iqm_ptr ? iqm_ptr[rc] : (1 << AOM_QM_BITS);
    if (quantize_coeff_fp_nuq(coeff_ptr[rc], quant, dequant, dq, rc,
  ...
  if (!skip_block) {
    ...
    for (i = 0; i < n_coeffs; i++) {
      ...
      if (quantize_coeff_nuq(coeff_ptr[rc], quant_ptr[rc != 0],
                             ...
                             dequant_ptr[rc != 0], dq, rc != 0,
                             cuml_bins_ptr[rc != 0], dequant_val[rc != 0],
                             &qcoeff_ptr[rc], &dqcoeff_ptr[rc], wt, iwt))
        eob = i;
    }
  }
  *eob_ptr = eob + 1;
  ...
  if (!skip_block) {
    ...
    for (i = 0; i < n_coeffs; i++) {
      ...
      if (quantize_coeff_bigtx_nuq(
              ...
              zbin_ptr[rc != 0], dequant_ptr[rc != 0], dq, rc != 0,
...
void quantize_32x32_fp_nuq_c(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  if (!skip_block) {
    ...
    for (i = 0; i < n_coeffs; i++) {
      ...
      const qm_val_t iwt = iqm_ptr ? iqm_ptr[rc] : (1 << AOM_QM_BITS);
      if (quantize_coeff_bigtx_fp_nuq(
              coeff_ptr[rc], quant_ptr[rc != 0], dequant_ptr[rc != 0], dq,
              rc != 0, cuml_bins_ptr[rc != 0], dequant_val[rc != 0],
              &qcoeff_ptr[rc], &dqcoeff_ptr[rc], wt, iwt,
              av1_get_tx_scale(TX_32X32)))
        eob = i;
    }
...
void av1_quantize_dc_nuq_facade(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  switch (qparam->log_scale) {
    case 0:
      ...
      break;
    case 1:
      quantize_dc_32x32_fp_nuq(coeff_ptr, n_coeffs, skip_block,
                               p->quant_fp_QTX[0], p->dequant_QTX[0], dq,
                               p->cuml_bins_nuq[dq][0],
                               p->dequant_val_nuq_QTX[dq][0], qcoeff_ptr,
                               dqcoeff_ptr, eob_ptr, qm_ptr, iqm_ptr);
      break;
#if CONFIG_TX64X64
    case 2:
      quantize_dc_64x64_fp_nuq(coeff_ptr, n_coeffs, skip_block,
                               p->quant_fp_QTX[0], p->dequant_QTX[0], dq,
                               p->cuml_bins_nuq[dq][0],
                               p->dequant_val_nuq_QTX[dq][0], qcoeff_ptr,
                               dqcoeff_ptr, eob_ptr, qm_ptr, iqm_ptr);
...
void highbd_quantize_dc_nuq(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  if (!skip_block) {
    ...
    if (highbd_quantize_coeff_nuq(
            coeff_ptr[rc], quant, quant_shift, zbin_ptr[rc], dequant, dq, rc,
            cuml_bins_ptr, dequant_val, qcoeff_ptr, dqcoeff_ptr, wt, iwt))
      eob = 0;
  }
  *eob_ptr = eob + 1;
}
...
void av1_highbd_quantize_dc_nuq_facade(
  ...
  switch (qparam->log_scale) {
    case 0:
      highbd_quantize_dc_fp_nuq(coeff_ptr, n_coeffs, skip_block,
                                p->quant_fp_QTX[0], p->dequant_QTX[0], dq,
                                p->cuml_bins_nuq[dq][0],
                                p->dequant_val_nuq_QTX[dq][0], qcoeff_ptr,
                                dqcoeff_ptr, eob_ptr, qm_ptr, iqm_ptr);
      break;
    case 1:
      highbd_quantize_dc_32x32_fp_nuq(coeff_ptr, n_coeffs, skip_block,
                                      p->quant_fp_QTX[0], p->dequant_QTX[0], dq,
                                      p->cuml_bins_nuq[dq][0],
                                      p->dequant_val_nuq_QTX[dq][0], qcoeff_ptr,
                                      dqcoeff_ptr, eob_ptr, qm_ptr, iqm_ptr);
...
#if CONFIG_TX64X64
    case 2:
      highbd_quantize_dc_64x64_fp_nuq(coeff_ptr, n_coeffs, skip_block,
                                      p->quant_fp_QTX[0], p->dequant_QTX[0], dq,
                                      p->cuml_bins_nuq[dq][0],
                                      p->dequant_val_nuq_QTX[dq][0], qcoeff_ptr,
                                      dqcoeff_ptr, eob_ptr, qm_ptr, iqm_ptr);
