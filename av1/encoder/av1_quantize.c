      ...
      case 0:
        if (n_coeffs < 16) {
          ...
          quantize_fp_helper_c(
              ...
              p->dequant_QTX, eob_ptr, sc->scan, sc->iscan, NULL, NULL,
              qparam->log_scale);
        } else {
...
void av1_quantize_b_nuq_facade(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  switch (qparam->log_scale) {
    case 0:
      ...
      break;
    case 1:
      quantize_32x32_nuq(coeff_ptr, n_coeffs, skip_block, p->quant_QTX,
                         p->quant_shift_QTX, p->dequant_QTX,
                         (const cuml_bins_type_nuq *)p->cuml_bins_nuq[dq],
...
#if CONFIG_TX64X64
    case 2:
      quantize_64x64_nuq(coeff_ptr, n_coeffs, skip_block, p->quant_QTX,
                         p->quant_shift_QTX, p->dequant_QTX,
                         (const cuml_bins_type_nuq *)p->cuml_bins_nuq[dq],
                         (const dequant_val_type_nuq *)
                         p->dequant_val_nuq_QTX[dq],
  ...
  switch (qparam->log_scale) {
    ...
    case 1:
      quantize_dc_32x32_fp_nuq(coeff_ptr, n_coeffs, skip_block,
                               p->quant_fp_QTX[0], p->dequant_QTX[0],
...
void av1_highbd_quantize_fp_facade(const tran_low_t *coeff_ptr,
  ...
  if (qm_ptr != NULL && iqm_ptr != NULL) {
    highbd_quantize_fp_helper_c(
        coeff_ptr, n_coeffs, skip_block, p->zbin_QTX, p->round_fp_QTX,
        p->quant_fp_QTX, p->quant_shift_QTX, qcoeff_ptr, dqcoeff_ptr,
        p->dequant_QTX, eob_ptr, sc->scan, sc->iscan, qm_ptr, iqm_ptr,
        qparam->log_scale);
  } else {
#endif  // CONFIG_AOM_QM
    ...
    if (n_coeffs < 16) {
      // TODO(jingning): Need SIMD implementation for smaller block size
      // quantization.
      av1_highbd_quantize_fp_c(coeff_ptr, n_coeffs, skip_block, p->zbin_QTX,
                               p->round_fp_QTX, p->quant_fp_QTX,
                               p->quant_shift_QTX, qcoeff_ptr, dqcoeff_ptr,
                               p->dequant_QTX, eob_ptr, sc->scan, sc->iscan,
                               qparam->log_scale);
...
void av1_highbd_quantize_b_facade(const tran_low_t *coeff_ptr,
  ...
  if (qm_ptr != NULL && iqm_ptr != NULL) {
    highbd_quantize_b_helper_c(coeff_ptr, n_coeffs, skip_block, p->zbin_QTX,
                               p->round_QTX, p->quant_QTX, p->quant_shift_QTX,
                               qcoeff_ptr, dqcoeff_ptr, p->dequant_QTX,
                               eob_ptr, sc->scan, sc->iscan, qm_ptr, iqm_ptr,
                               qparam->log_scale);
  } else {
#endif  // CONFIG_AOM_QM
      ...
      case 0:
        if (LIKELY(n_coeffs >= 8)) {
          ...
          aom_highbd_quantize_b_c(coeff_ptr, n_coeffs, skip_block, p->zbin_QTX,
                                  ...
                                  p->quant_shift_QTX, qcoeff_ptr, dqcoeff_ptr,
                                  p->dequant_QTX, eob_ptr, sc->scan,
                                  sc->iscan);
        }
        break;
      case 1:
        aom_highbd_quantize_b_32x32(coeff_ptr, n_coeffs, skip_block,
                                    p->zbin_QTX, p->round_QTX, p->quant_QTX,
                                    p->quant_shift_QTX, qcoeff_ptr, dqcoeff_ptr,
                                    p->dequant_QTX, eob_ptr, sc->scan,
                                    sc->iscan);
        break;
#if CONFIG_TX64X64
      case 2:
        aom_highbd_quantize_b_64x64(coeff_ptr, n_coeffs, skip_block,
                                    p->zbin_QTX, p->round_QTX, p->quant_QTX,
                                    p->quant_shift_QTX, qcoeff_ptr, dqcoeff_ptr,
                                    p->dequant_QTX, eob_ptr, sc->scan,
                                    sc->iscan);
...
void av1_highbd_quantize_b_nuq_facade(
  ...
  switch (qparam->log_scale) {
    case 0:
      highbd_quantize_nuq(coeff_ptr, n_coeffs, skip_block, p->quant_QTX,
                          p->quant_shift_QTX, p->dequant_QTX,
                          (const cuml_bins_type_nuq *)p->cuml_bins_nuq[dq],
                          (const dequant_val_type_nuq *)
                          p->dequant_val_nuq_QTX[dq],
