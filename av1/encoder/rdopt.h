...
static INLINE int has_best_pred_mv_sad(const MV_REFERENCE_FRAME *ref_frame,
  ...
  int has_best_past_pred_mv_sad = (ref_frame[0] == best_pred_mv_sad[0]) ||
                                  (ref_frame[1] == best_pred_mv_sad[0]);
  int has_best_future_pred_mv_sad = (ref_frame[0] == best_pred_mv_sad[1]) ||
                                    (ref_frame[1] == best_pred_mv_sad[1]);
...
static INLINE int prune_ref_by_selective_ref_frame(
  ...
  if (sf->inter_sf.prune_comp_ref_frames && comp_pred) {
    ...
    if (sf->inter_sf.prune_comp_ref_frames == 2 && closest_ref_frames == 0) {
      return 1;
    } else if (sf->inter_sf.prune_comp_ref_frames == 1 &&
               closest_ref_frames == 0) {
      // Prune reference frames with non minimum pred_mv_sad in past and future
      // directions.
      if (has_best_pred_mv_sad(ref_frame, x->best_pred_mv_sad) == 0) return 1;
    }
  }
