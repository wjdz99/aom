...
static INLINE int prune_ref_by_selective_ref_frame(
  ...
  if (sf->inter_sf.selective_ref_frame) {
    ...
    if (sf->inter_sf.selective_ref_frame >= 4 && comp_pred) {
      if ((cpi->future_ref_frames_info.total_ref_frame_count >= 3) &&
          ((ref_frame[0] ==
      ...
      if (cpi->future_ref_frames_info.total_ref_frame_count == 2) {
        int prune_flag =
            cpi->future_ref_frames_info.frame_distance_pair[0].distance >
            (MAX_GF_INTERVAL >> 1);
        if (prune_flag &&
            ((ref_frame[0] ==
              cpi->future_ref_frames_info.frame_distance_pair[0].frame) ||
