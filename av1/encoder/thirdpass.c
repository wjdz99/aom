...
void av1_get_third_pass_ratio(THIRD_PASS_DEC_CTX *ctx, int fidx, int fheight,
  ...
  const int fh_tp = ctx->frame_info[fidx].height;
  const int fw_tp = ctx->frame_info[fidx].width;
  assert(fh_tp <= fheight && fw_tp <= fwidth);

  *ratio_h = (double)fheight / fh_tp;
  *ratio_w = (double)fwidth / fw_tp;
}
...
THIRD_PASS_MI_INFO *av1_get_third_pass_mi(THIRD_PASS_DEC_CTX *ctx, int fidx,
  ...
  THIRD_PASS_MI_INFO *this_mi =
      ctx->frame_info[fidx].mi_info + mi_row_tp * mi_stride_tp + mi_col_tp;
  return this_mi;
}

void av1_third_pass_get_adjusted_mi(THIRD_PASS_MI_INFO *tp_mi, double ratio_h,
                                    double ratio_w, int *mi_row, int *mi_col) {
  *mi_row = (int)ceil(tp_mi->mi_row_start * ratio_h);
  *mi_col = (int)ceil(tp_mi->mi_col_start * ratio_w);
}
...
BLOCK_SIZE av1_get_third_pass_adjusted_blk_size(THIRD_PASS_MI_INFO *this_mi,
  ...
  const BLOCK_SIZE bsize_tp = this_mi->bsize;
  assert(bsize_tp != BLOCK_INVALID);

  const int w_tp = block_size_wide[bsize_tp];
  const int h_tp = block_size_high[bsize_tp];
