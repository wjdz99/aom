...
void av1_write_second_pass_gop_info(AV1_COMP *cpi) {
  ...
  if (oxcf->pass == AOM_RC_SECOND_PASS && oxcf->second_pass_log) {
    // Write the GOP length to a file.
    if (!cpi->second_pass_file_stream) {
      cpi->second_pass_file_stream = fopen(cpi->oxcf.second_pass_log, "wb");
      if (!cpi->second_pass_file_stream) {
...
void av1_read_second_pass_gop_info(AV1_COMP *cpi,
  ...
  if (oxcf->pass == AOM_RC_THIRD_PASS) {
    ...
    if (!cpi->second_pass_file_stream) {
      cpi->second_pass_file_stream = fopen(cpi->oxcf.second_pass_log, "rb");
      if (!cpi->second_pass_file_stream) {
        aom_internal_error(cpi->common.error, AOM_CODEC_ERROR,
                           "Could not open second pass file!");
      }
