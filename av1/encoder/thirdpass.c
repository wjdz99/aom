...
static int read_frame(THIRD_PASS_DEC_CTX *ctx) {
  ...
  if (!ctx->have_frame) {
    ...
    ctx->frame_size = ctx->bytes_in_buffer;
    ctx->have_frame = 1;
  }
  ...
  *last_idx = 0;
  ...
  while (num_show_frames < max_num) {
    ...
    if (cur_idx >= ctx->num_gop_info_left) {
      if (read_frame(ctx) < 0) return -1;
      if (get_frame_gop_info(ctx) < 0) return -1;
    }
...
int av1_set_gop_third_pass(THIRD_PASS_DEC_CTX *ctx, GF_GROUP *gf_group,
  ...
  int last_idx;
  if (read_gop_frames(ctx, 32, &last_idx) < 0) return -1;
