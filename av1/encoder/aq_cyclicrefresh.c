...
void av1_cyclic_refresh_update_parameters(AV1_COMP *const cpi) {
  ...
  const int frames_since_scene_change = (cpi->svc.number_temporal_layers > 1)
      ? cpi->rc.frames_since_key
      : AOMMIN(cpi->rc.frames_since_key, cr->counter_encode_maxq_scene_change);
  ...
  // refresh cycles after a key frame (svc) or scene change. For non
  // svc scene changes gradually reduce the boost and supress it further
  // if either of the previous two frames overshot.
  if (cr->percent_refresh > 0) {
