...
static void cyclic_refresh_update_map(AV1_COMP *const cpi) {
  ...
  // Don't contrain the map near key frame or near scene/slide change.
...
void av1_cyclic_refresh_update_parameters(AV1_COMP *const cpi) {
  ...
  if (frame_is_intra_only(cm) || is_lossless_requested(&cpi->oxcf.rc_cfg) ||
      ...
      (cpi->oxcf.tune_cfg.content == AOM_CONTENT_SCREEN &&
       rc->frames_since_key > 100 &&
       cr->counter_encode_maxq_scene_change > 100 &&
       cpi->rc.avg_frame_low_motion == 0)) {
