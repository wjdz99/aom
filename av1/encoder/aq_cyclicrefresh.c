...
void av1_cyclic_refresh_set_golden_update(AV1_COMP *const cpi) {
  ...
  if (cr->percent_refresh > 0)
    p_rc->baseline_gf_interval = AOMMIN(32 * (100 / cr->percent_refresh), 640);
  else
    p_rc->baseline_gf_interval = 320;
  if (rc->avg_frame_low_motion < 40) p_rc->baseline_gf_interval = 128;
}
