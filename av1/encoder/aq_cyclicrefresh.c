...
void av1_cyclic_refresh_update_parameters(AV1_COMP *const cpi) {
  ...
  //const int frames_since_scene_change = cpi->rc.frames_since_key;
  ...
  // refresh cycles after a key frame or scene change but gradually
  // reduce the boost and supress it further if either of the previous
  // two frames overshot.
  if (cr->percent_refresh > 0) {
