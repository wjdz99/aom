    ...
    for (x = 0; x < xmis; x++) {
...
#if CONFIG_EXT_SEGMENT
      av1_store_segment_id_into_map(mbmi->segment_id[QUALITY_SEG_IDX], &cpi->segmentation_map[map_offset], QUALITY_SEG_IDX);      
      ...
      if ((!is_inter_block(mbmi) || !skip) &&
        ...
        cr->last_coded_q_map[map_offset] = clamp(
          cm->base_qindex + cr->qindex_delta[mbmi->segment_id[QUALITY_SEG_IDX]], 0, MAXQ);
      }
      else if (is_inter_block(mbmi) && skip &&
        mbmi->segment_id[QUALITY_SEG_IDX] <= CR_SEGMENT_ID_BOOST2) {
        cr->last_coded_q_map[map_offset] =
          AOMMIN(clamp(cm->base_qindex + cr->qindex_delta[mbmi->segment_id[QUALITY_SEG_IDX]],
            0, MAXQ),
            cr->last_coded_q_map[map_offset]);
      }
#else
...
static void cyclic_refresh_update_map(AV1_COMP *const cpi) {
  ...
  int xmis, ymis, x, y;
#if CONFIG_EXT_SEGMENT
  memset(seg_map, CR_SEGMENT_ID_BASE << MAX_LOG2_ACTIVE_SEGMENTS, cm->mi_rows * cm->mi_cols);
#else
  ...
  do {
...
#if CONFIG_EXT_SEGMENT
            ? av1_get_qindex(&cm->seg[QUALITY_SEG_IDX], CR_SEGMENT_ID_BOOST2, cm->base_qindex)
#else
    ...
    if (sum_map >= xmis * ymis / 2) {
      for (y = 0; y < ymis; y++)
        for (x = 0; x < xmis; x++) {
#if CONFIG_EXT_SEGMENT
          av1_store_segment_id_into_map(CR_SEGMENT_ID_BOOST1, &seg_map[bl_index + y * cm->mi_cols + x], QUALITY_SEG_IDX);
#else
...
#if CONFIG_EXT_SEGMENT
    av1_set_segdata(seg, CR_SEGMENT_ID_BOOST1, QUALITY_SEG_LVL_ALT_Q, qindex_delta, QUALITY_SEG_IDX);
#else
...
#if CONFIG_EXT_SEGMENT
    av1_set_segdata(seg, CR_SEGMENT_ID_BOOST2, QUALITY_SEG_LVL_ALT_Q, qindex_delta, QUALITY_SEG_IDX);
#else
