...
#if CONFIG_EXTQUANT_HBD
  assert(bit_depth == AOM_BITS_8 ? (MAXQ_UNEXT <= (QINDEX_RANGE_UNEXT - 1))
                                 : (MAXQ <= (QINDEX_RANGE - 1)));
  memset(cr->last_coded_q_map, bit_depth == AOM_BITS_8 ? MAXQ_UNEXT : MAXQ,
...
#if CONFIG_EXTQUANT_HBD
    const int qindex2 =
        clamp(cm->base_qindex + cm->y_dc_delta_q + qindex_delta, 0,
              cm->seq_params.bit_depth == AOM_BITS_8 ? MAXQ_UNEXT : MAXQ);
#else
    const int qindex2 =
