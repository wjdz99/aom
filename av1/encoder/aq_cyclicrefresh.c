...
void av1_cyclic_refresh_update_segment(const AV1_COMP *cpi,
  ...
  if (mbmi->segment_id == CR_SEGMENT_ID_BOOST1)
    cr_counts->actual_num_seg1_blocks += num_bls;
  else if (mbmi->segment_id == CR_SEGMENT_ID_BOOST2)
    cr_counts->actual_num_seg2_blocks += num_bls;
  if (is_inter_block(mbmi) && abs(mv.row) < 16 && abs(mv.col) < 16)
    cr_counts->cnt_zeromv += num_bls;
}

void av1_cyclic_refresh_postencode(AV1_COMP *const cpi) {
  AV1_COMMON *const cm = &cpi->common;
  CR_COUNTS *const cr_counts = &cpi->td.cr_counts;
  const CommonModeInfoParams *const mi_params = &cm->mi_params;
  CYCLIC_REFRESH *const cr = cpi->cyclic_refresh;
  cr->cnt_zeromv = cr_counts->cnt_zeromv;
  cr->actual_num_seg1_blocks = cr_counts->actual_num_seg1_blocks;
  cr->actual_num_seg2_blocks = cr_counts->actual_num_seg2_blocks;
  cr->cnt_zeromv =
...
void av1_cyclic_refresh_setup(AV1_COMP *const cpi) {
  ...
  if (!cr->apply_cyclic_refresh) {
    ...
    CR_COUNTS *const cr_counts = &cpi->td.cr_counts;
    cr_counts->cnt_zeromv = 0;
    cr_counts->actual_num_seg1_blocks = 0;
    cr_counts->actual_num_seg2_blocks = 0;
