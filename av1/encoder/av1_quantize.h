...
/*!\brief Quantize transform coefficients
 *
 * \param[in]    quant_ptr[2]   16-bit fixed point representation of inverse
 * quantizer step size \param[in]    dequant_ptr[2] quantizer step size dc:
 * dequant_ptr[0] ac: dequant_ptr[1] \param[in]    round_ptr[2]   rounding dc:
 * round_ptr[0] ac: round_ptr[1] \param[in]    log_scale      the relative log
 * scale of the transform coefficients \param[in]    scan           scan[i]
 * indicates the position of ith to-be-coded coefficient \param[in] coeff_count
 * number of coefficients \param[out]   qcoeff_ptr     quantized coefficients
