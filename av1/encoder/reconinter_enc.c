...
void av1_build_inter_predictor(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  if (is_scaled) {
    ...
    int orig_pos_y = (inter_pred_params->pix_row + y) << SUBPEL_BITS;
    orig_pos_y += src_mv->row * (1 << (1 - ssy));
    int orig_pos_x = (inter_pred_params->pix_col + x) << SUBPEL_BITS;
...
static INLINE void build_prediction_by_above_pred(
  ...
  for (int j = 0; j < num_planes; ++j) {
    ...
    av1_init_inter_params(&inter_pred_params, bw, bh, mi_x, mi_y,
                          pd->subsampling_x, pd->subsampling_y, xd->bd,
                          is_cur_buf_hbd(xd), 0, xd->block_ref_scale_factors[0],
                          backup_mbmi.interp_filters);
