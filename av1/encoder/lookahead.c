...
struct lookahead_ctx *av1_lookahead_init(
    ...
    const int border_in_pixels, int is_scale, int lookahead_lag) {
  ...
  // Clamp the extra lookahead lag
  lookahead_lag = clamp(lookahead_lag, 0, MAX_LAP_BUFFERS);
  ...
  if (ctx) {
    ...
    if (lookahead_lag) {
      ctx->read_ctxs[LOOKAHEAD_STAGE].pop_sz = lag_in_frames;
      ctx->read_ctxs[LOOKAHEAD_STAGE].valid = 1;
    }
...
int av1_lookahead_push(struct lookahead_ctx *ctx, YV12_BUFFER_CONFIG *src,
  ...
  if (ctx->read_ctxs[LOOKAHEAD_STAGE].valid) {
    ctx->read_ctxs[LOOKAHEAD_STAGE].sz++;
  }
