...
struct lookahead_ctx *av1_lookahead_init(
  ...
  if (ctx) {
    ...
    unsigned int i;
    ctx->max_sz = depth;
    ctx->read_ctxs[ENC_STAGE].pop_sz = ctx->max_sz - MAX_PRE_FRAMES;
...
int av1_lookahead_push(struct lookahead_ctx *ctx, YV12_BUFFER_CONFIG *src,
  ...
  if (ctx->read_ctxs[ENC_STAGE].sz + 1 + MAX_PRE_FRAMES > ctx->max_sz) return 1;
  ctx->read_ctxs[ENC_STAGE].sz++;
...
struct lookahead_entry *av1_lookahead_pop(struct lookahead_ctx *ctx, int drain,
  ...
  if (ctx && read_ctx->sz && (drain || read_ctx->sz == read_ctx->pop_sz)) {
