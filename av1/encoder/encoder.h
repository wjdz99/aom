...
// Encoder row-multithreading related data
typedef struct {
...
#endif
  // Buffer to store the superblock column whose encoding is complete.
  // cur_col[i] stores the encoded superblock column of the ith superblock row.
  int *cur_col;
  // Number of superblock columns of the previous row to be complete for
  // the encoding of the current superblock to start. A value of 1 indicates
  // that inorder to start processing a superblock, the encoding of
  // top-right superblock should be complete.
  int sync_range;
  // Number of superblock rows.
  int rows;
  // The superblock row (in units of MI blocks) to be processed next.
  int next_mi_row;
  // Number of threads processing a tile.
...
typedef struct {
  // Allocated number of row synchronization related data.
  int allocated_tile_rows;
  int allocated_tile_cols;
  int allocated_sb_rows;

  // Mapping of threads to tiles.
  int thread_id_to_tile_id[MAX_NUM_THREADS];

#if CONFIG_MULTITHREAD
  // Mutex lock used to dispatch jobs.
  pthread_mutex_t *mutex_;
#endif

  // Row synchronization related function pointers.
  void (*sync_read_ptr)(AV1EncRowMTSync *const, int, int);
  void (*sync_write_ptr)(AV1EncRowMTSync *const, int, int, int);
...
typedef struct {
  ...
  // Loop-Filter multi-threading data.
  AV1LfSync lf_row_sync;

  // Loop-Restoration multi-threading data.
...
typedef struct AV1_COMP {
  ...
  // Multi-threading related information.
