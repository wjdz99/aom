...
typedef struct AV1_COMP {
  ...
  // Look-ahead processing context.
  struct lookahead_ctx *lookahead;

  // When set, this flag indicates that the current frame is a forward keyframe.
  int no_show_kf;

  // Stores the trellis optimization type at each segment level.
  // optimize_seg_arr[i] stores the trellis opt type for segment i.
  ...
  // Frame buffer to store the alt ref frame.
  ...
  // Indicates that the frame indexed by existing_fb_idx_to_show is to be output
  // when show_existing_frame equal to 1. When show_existing_frame is 0,
  // existing_fb_idx_to_show is set to INVALID_IDX.
