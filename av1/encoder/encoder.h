...
#if CONFIG_FRAME_SUPERRES
static INLINE int av1_superres_unscaled(const struct AV1_COMP *cpi) {
  return (cpi->common.superres_scale_numerator == SUPERRES_SCALE_DENOMINATOR);
}
#endif  // CONFIG_FRAME_SUPERRES
...
#if CONFIG_FRAME_SUPERRES
  return (cpi->common.superres_upscaled_width == oxcf->width &&
          cpi->common.superres_upscaled_height == oxcf->height);
#else
  return (cpi->common.width == oxcf->width &&
          cpi->common.height == oxcf->height);
#endif  // CONFIG_FRAME_SUPERRES
}

static INLINE int av1_frame_unscaled(const struct AV1_COMP *cpi) {
#if CONFIG_FRAME_SUPERRES
  return av1_superres_unscaled(cpi) && av1_resize_unscaled(cpi);
#else
  return av1_resize_unscaled(cpi);
#endif  // CONFIG_FRAME_SUPERRES
}
