...
static INLINE int is_frame_tpl_eligible(AV1_COMP *const cpi) {
  ...
  const int pyr_lev_from_top = cpi->gf_group.pyramid_height -
                               cpi->gf_group.pyramid_level[cpi->gf_group.index];
  if (pyr_lev_from_top > max_pyr_level_fromtop_deltaq ||
      cpi->gf_group.pyramid_height <= max_pyr_level_fromtop_deltaq + 1)
    return 0;
  else
    return 1;
}
