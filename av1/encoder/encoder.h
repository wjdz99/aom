...
typedef struct {
  // This buffer stores the winner reference mode information at
  // mi_alloc_bsize (4x4 or 8x8) level which is used during bitstream
  // preparation. Winner reference mode info of a block is stored at
  // frame_base[mi_row * stride + mi_col].
  MB_MODE_INFO_EXT_FRAME *frame_base;
  // Memory size to be allocated to frame_base.
  int alloc_size;
  // Stride of frame_base array.
  int stride;
} ModeInfoExtFrame;
...
typedef struct AV1_COMP {
  ...
  // Winner reference mode information at frame level.
  ModeInfoExtFrame mbmi_ext_info;
...
static INLINE void set_mode_info_offsets(
    ...
    const ModeInfoExtFrame *const mbmi_ext_info, MACROBLOCK *const x,
