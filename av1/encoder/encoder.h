...
static INLINE unsigned int get_token_alloc(int mb_rows, int mb_cols) {
  ...
  const int max_sb = ROUND_POWER_OF_TWO(mb_rows, MAX_SB_SIZE_LOG2 - 4) *
                     ROUND_POWER_OF_TWO(mb_cols, MAX_SB_SIZE_LOG2 - 4);
  ...
  // most 1 + (w + h - 2)(w - 1) tokens (99 for an 8x8 block) with
  // CONFIG_PALETTE_THROUGHPUT or 1 + h * w tokens (65 for an 8x8 block)
  // without (see cost_and_tokenize_map). At most, there can be palettes on two
  // planes.
  const int sb_palette_toks =
      2 * (1 + (CONFIG_PALETTE_THROUGHPUT ? 98 : 64)) * (MAX_SB_SQUARE / 64);

  return max_sb * (sb_coeff_toks + sb_palette_toks);
}
