...
static INLINE unsigned int get_token_alloc(int mb_rows, int mb_cols,
  ...
  const int sb_size_sqaure = sb_size * sb_size;
  ...
  const int sb_coeff_toks = 3 * (sb_size_sqaure + (sb_size_sqaure / 16) + 1);
  ...
  const int sb_palette_toks = 2 * (1 + 64) * (sb_size_sqaure / 64);
