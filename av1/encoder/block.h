...
typedef struct {
  ...
  // mv_costs_alloc[0][i] is the cost of motion vector with horizontal component
  // (mv_row) equal to i - MV_MAX.
  // mv_costs_alloc[1][i] is the cost of motion vector with vertical component
  // (mv_col) equal to i - MV_MAX.
  int dv_costs_alloc[2][MV_VALS];

  //! Points to the middle of \ref mv_costs_alloc
  int *dv_costs[2];
} IntraBCMvCosts;
