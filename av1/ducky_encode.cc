...
static INLINE void CheckCondition(const char *description, int condition,
                                  const char *func, int line) {
  if (condition == 0) {
    printf("\ncheck_condition failed: %05d %s %s\n", line, func, description);
  } else {
    // printf("\ncheck_condition ok: %05d %s %s\n", line, func, description);
  }
}

#define CHECK_CONDITION(condition) \
  CheckCondition(#condition, condition, __func__, __LINE__);
...
std::vector<FIRSTPASS_STATS> DuckyEncode::ComputeFirstPassStats() {
  ...
  for (i = 0; i < frame_count; ++i) {
    ...
    if (aom_img_read(&enc_resource.img, in_file)) {
      ...
      // compresses data
      const int res = av1_get_compressed_data(ppi->cpi, &cpi_data);
...
void DuckyEncode::EncodeFrame(std::vector<uint8_t> *bitstream_buf) {
  (void)bitstream_buf;
}
