  ...
  for (size_t idx = 0; idx < gop_struct.gop_frame_list.size(); ++idx) {
    ...
    for (int mi_row = 0; mi_row < mi_rows; ++mi_row) {
      for (int mi_col = 0; mi_col < mi_cols; ++mi_col) {
        ...
        if (tpl_stats_ptr->ref_frame_index[0] >= 0)
          block_stats.ref_frame_index[0] =
              tpl_frame->ref_map_index[tpl_stats_ptr->ref_frame_index[0]];
        if (tpl_stats_ptr->ref_frame_index[1] >= 0)
          block_stats.ref_frame_index[1] =
              tpl_frame->ref_map_index[tpl_stats_ptr->ref_frame_index[1]];

        if (tpl_stats_ptr->ref_frame_index[0] >= 0) {
          block_stats.mv[0] = {
            tpl_stats_ptr->mv[tpl_stats_ptr->ref_frame_index[0]].as_mv.row,
            tpl_stats_ptr->mv[tpl_stats_ptr->ref_frame_index[0]].as_mv.col, 3
          };
        }
        if (tpl_stats_ptr->ref_frame_index[1] >= 0) {
          block_stats.mv[1] = {
            tpl_stats_ptr->mv[tpl_stats_ptr->ref_frame_index[1]].as_mv.row,
            tpl_stats_ptr->mv[tpl_stats_ptr->ref_frame_index[1]].as_mv.col, 3
          };
        }
