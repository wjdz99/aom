...
struct MotionVector {
  int row;          // subpel row
  int col;          // subpel col
  int subpel_bits;  // number of fractionl bits used by row/col
  /* Here is how we obtain fullpel_value from subpel_value
   * subpel_scale = (1 << subpel_bits);
   * sign = subpel_value >= 0 ? 1 : -1;
   * fullpel_value = (abs(subpel_value) + subpel_scale / 2)) >>
   * subpel_bits;
   * fullpel_value *= sign;
   */
};
...
struct GopEncodeInfo {
  ...
  RefFrameTable ref_frame_table_snapshot_init;  // RefFrameTable snapshot
                                                // before coding this GOP
  RefFrameTable ref_frame_table_snapshot_end;   // RefFrameTable snapshot after
                                                // coding this GOP
};
