...
class RefFrameManager {
 public:
  RefFrameManager(int max_ref_frames) : max_ref_frames_(max_ref_frames) {
  ...
  friend std::ostream &operator<<(std::ostream &os,
    ...
    os << "=" << std::endl;
    os << "forward: ";
    for (auto &ref_idx : rfm.forward_stack_) {
      os << rfm.ref_frame_table_[ref_idx].order_idx << " ";
    }
    os << std::endl;
    os << "backward: ";
    for (auto ref_idx : rfm.backward_queue_) {
      os << rfm.ref_frame_table_[ref_idx].order_idx << " ";
    }
    os << std::endl;
    os << "last: ";
    for (auto ref_idx : rfm.last_queue_) {
      os << rfm.ref_frame_table_[ref_idx].order_idx << " ";
    }
    os << std::endl;
    return os;
