...
StatusOr<GopStructList> AV1RateControlQMode::DetermineGopInfo(
  ...
  const FirstpassInfo fp_info_copy = AnalyzeFpStats(firstpass_info);
  ...
  for (int i = 0; i < frame_count; ++i) {
    ...
    if (gop_frame.update_type == GopFrameType::kOverlay ||
      ...
      for (int i = 1; i < stats_size; ++i) {
        if (CheckFlash(fp_info_copy.stats_list, i)) continue;
        coeff_kf *= fp_info_copy.stats_list[i].cor_coeff;
        const double this_cor =
            coeff_kf * sqrt(std::max((fp_info_copy.stats_list[i].intra_error -
      ...
      for (int f = this_gop_len - 1; f > 0; --f) {
        ...
        for (int k = this_gop_len; k > f; --k) {
          if (CheckFlash(fp_info_copy.stats_list, k)) continue;
        ...
        const double this_cor =
            coeff_this *
            sqrt(std::max((fp_info_copy.stats_list[f].intra_error -
