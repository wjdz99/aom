        ...
        for (int j = 0; j < 2; ++j) {
          for (int k = 0; k < 2; ++k) {
            ...
            if (ref_unit_row >= 0 && ref_unit_row < frame_unit_rows &&
              ...
              // We apply propagation_fraction with power of 0.3 to improve propagation rate.
              // The power coefficient is tuned from empirical practice due to the fact that
              // the inter/intra cost is RD cost not prediction error which is used
              // in the original design.
              // This acttion will increase the propagation_fraction when the difference
              // between intra_cost and inter_cost are reasonable big but not enough for
              // propagation.
              // TODO(angiebird): Check this part again when we use prediction error for
              // inter/intra cost.
              const double propagation_fraction =
                  std::pow(GetPropagationFraction(unit_dep_stats), propagation_fraction_power);
              const double propagation_ratio =
                  1.0 / ref_frame_count * overlap_ratio;
              TplUnitDepStats &ref_unit_stats =
                  ref_frame_dep_stats.unit_stats[ref_unit_row][ref_unit_col];
