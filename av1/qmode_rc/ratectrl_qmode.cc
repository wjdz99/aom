...
std::vector<uint8_t> SetupDeltaQ(const TplFrameDepStats &frame_dep_stats,
  ...
  for (int sb_row = 0; sb_row < sb_rows; ++sb_row) {
    for (int sb_col = 0; sb_col < sb_cols; ++sb_col) {
      ...
      superblock_q_indices.push_back(offset + base_qindex);
    }
  }
  ...
  for (int i = 0; i < frame_count; i++) {
    ...
    if (gop_frame.update_type == GopFrameType::kOverlay ||
      ...
      if (rc_param_.max_distinct_q_indices_per_frame > 1) {
        param.superblock_q_indices =
            SetupDeltaQ(frame_dep_stats, rc_param_.frame_width,
                        rc_param_.frame_height, param.q_index);
      }
    } else {
      // Intermediate ARFs
      assert(gop_frame.layer_depth >= 1);
