...
// Returns the number of frames that use this_frame as reference in the current
// and next subGop.
int NumAsRef(const GopStruct &gop_struct,
             const std::vector<LookaheadStats> &lookahead_stats,
             const std::vector<RefFrameTable> &ref_frame_table_list,
  ...
  const int next_gop_frame_count =
      lookahead_stats.empty()
          ? 0
          : static_cast<int>(
                lookahead_stats[0].gop_struct[0].gop_frame_list.size());
  ...
  for (int i = 0; i < frame_count; ++i) {
    ...
    for (auto ref_frame : gop_frame.ref_frame_list) {
      if (ref_frame_table_list[i][ref_frame.index].global_coding_idx ==
          this_frame.global_coding_idx) {
        ++num;
        break;
      }
    }
  ...
  for (int i = frame_count; i < frame_count + next_gop_frame_count; ++i) {
    const GopFrame &gop_frame =
        lookahead_stats[0].gop_struct[0].gop_frame_list[i - frame_count];
    if (gop_frame.update_type == GopFrameType::kOverlay ||
        gop_frame.update_type == GopFrameType::kIntermediateOverlay) {
      continue;
    }
...
int GetIntArfQ(const GopStruct &gop_struct,
  ...
  const int frame_count = static_cast<int>(gop_struct.gop_frame_list.size());
  int arf_as_ref =
      NumAsRef(gop_struct, lookahead_stats, ref_frame_table_list, arf_frame);
  // Check whether this is the first internal arf
  ...
  if (is_first_int_arf) {
    // The first internal arf
    int int_arf_as_ref = NumAsRef(gop_struct, lookahead_stats,
                                  ref_frame_table_list, int_arf_frame);
    int arf_adjusted =
    ...
    if (arf_adjusted <= int_arf_as_ref) {
      ...
      return active_best_quality +
             (active_worst_quality - active_best_quality) *
                 AOMMAX(arf_adjusted - int_arf_as_ref, 0) / arf_adjusted;
    }
  } else {
    // Other internal arfs
