...
StatusOr<TplFrameDepStats> CreateTplFrameDepStatsWithoutPropagation(
  ...
  for (const TplBlockStats &block_stats :
    ...
    for (int r = 0; r < block_unit_rows; r++) {
      for (int c = 0; c < block_unit_cols; c++) {
        frame_dep_stats.alt_unit_stats[block_unit_row + r][block_unit_col + c] =
            unit_stats;
      }
    }
  }

  frame_dep_stats.rdcost = TplFrameDepStatsAccumulateInterCost(frame_dep_stats, 0);
  frame_dep_stats.alt_rdcost = TplFrameDepStatsAccumulateInterCost(frame_dep_stats, 1);
...
double TplFrameDepStatsAccumulateInterCost(
  ...
  if (unit_idx == 0) {
    ...
    for (const auto &row : frame_dep_stats.alt_unit_stats) {
      sum = std::accumulate(row.begin(), row.end(), sum, getInterCost);
    }
  }
  return std::max(sum, 1.0);
...
StatusOr<GopEncodeInfo> AV1RateControlQMode::GetGopEncodeInfoWithTpl(
  ...
  double base_rdcost = 0; // baseline total rdcost
  double hqr_rdcost = 0;  // high quality referece total rdcost
  double arf_rdcost_high = 0;
  ...
  double tp_frame_importance =
      (base_rdcost - hqr_rdcost) / (arf_rdcost_high - arf_rdcost_low);
