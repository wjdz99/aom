...
bool CheckFrameUseReference(
    int frame_ind, const GopFrame &frame, const GopFrame &candidate_referece,
    const std::vector<RefFrameTable> &ref_frame_table_list) {
  ...
  for (const auto &ref_frame : frame.ref_frame_list) {
    if (ref_frame_table_list[frame_ind][ref_frame.index].global_coding_idx ==
        candidate_referece.global_coding_idx) {
      return true;
    }
  }
  return false;
...
int CountUsedAsReference(const GopStruct &gop_struct,
  ...
  for (int i = frame_count; i < frame_count + next_gop_frame_count; ++i) {
    if (CheckFrameUseReference(
            i, lookahead_stats[0].gop_struct[0].gop_frame_list[i - frame_count],
