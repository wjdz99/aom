...
// Check whether a frame (with index frame_ind) uses cand_ref as a reference
// frame.
bool CheckFrameUseReference(
    int frame_ind, const GopFrame &frame, const GopFrame &cand_ref,
    const std::vector<RefFrameTable> &ref_frame_table_list) {
  if (frame.update_type == GopFrameType::kOverlay ||
      frame.update_type == GopFrameType::kIntermediateOverlay) {
    return false;
  }
  for (auto ref_frame : frame.ref_frame_list) {
    if (ref_frame_table_list[frame_ind][ref_frame.index].global_coding_idx ==
        cand_ref.global_coding_idx) {
      return true;
    }
  }
  return false;
...
int CountUsedAsReference(const GopStruct &gop_struct,
  ...
  for (int i = 0; i < frame_count; ++i) {
    ...
    if (CheckFrameUseReference(i, gop_frame, this_frame,
