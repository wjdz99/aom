...
struct RateControlParam {
  ...
  // Both must be positive if per-superblock quantization is to be used.
...
struct GopFrame {
  ...
  // lambda_scale. DetermineGopInfo may set this however it wants (e.g., based
  // on the frame type), but it is also allowed for a client of this API to
  // override that choice by modifying it before calling GetGopEncodeInfo.
...
struct FrameEncodeParameters {
  ...
  // Scaling factors for per-superblock lambda, to be multiplied by the value
  // returned by GetRDMult for the corresponding superblock's q_index.
