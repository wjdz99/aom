...
int GetNextKeyFrameOrderIndex(const std::vector<int> &key_frame_list,
  ...
  for (int key_frame_idx : key_frame_list) {
    if (global_order_idx < key_frame_idx) {
      return key_frame_idx;
    }
  }
  // If we can not find the next key frame, return show_frame_count of the
  // video.
  return show_frame_count;
}
...
void DuckyEncode::SetRelativeKeyFrameLocation(std::vector<int> key_frame_list,
                                              int global_order_idx,
