...
function(add_asm_library lib_name asm_sources)
  ...
  foreach(asm_config ${asm_configs})
    set(asm_lib_obj_dir
        "${AOM_CONFIG_DIR}/asm_objects/${lib_name}_${asm_config}")
    ...
    message("========================================")
    message("asm_config=${asm_config}")
    message("lib_name=${lib_name}")
    message("asm_lib_name=${asm_lib_name}")
    message("asm_lib_obj_dir=${asm_lib_obj_dir}")
    ...
    foreach(asm_source ${${asm_sources}})
      ...
      add_custom_command(OUTPUT "${asm_object}"
                         ...
                         VERBATIM)
      if(BUILD_SHARED_LIBS AND "${asm_object}" MATCHES "static")
        target_sources(aom_static PRIVATE "${asm_object}")
        message("aom_static depends on ${asm_object}")
      else()
        target_sources(aom PRIVATE "${asm_object}")
        message("aom depends on ${asm_object}")
      endif()
    endforeach()
