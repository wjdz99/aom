...
if(NOT AOM_BUILD_CMAKE_AOM_CONFIGURE_CMAKE_)
  ...
  # cache variable with its own help string).
  ...
  if(CMAKE_TOOLCHAIN_FILE) # Add toolchain file to config string.
    set(toolchain_string
        "-DCMAKE_TOOLCHAIN_FILE=\\\"${CMAKE_TOOLCHAIN_FILE}\\\"")
    set(AOM_CMAKE_CONFIG "${toolchain_string} ${AOM_CMAKE_CONFIG}")
  else() # Add detected CPU to the config string.
    set(AOM_CMAKE_CONFIG
        "-DAOM_TARGET_CPU=${AOM_TARGET_CPU} ${AOM_CMAKE_CONFIG}")
  endif()
  ...
  if(
    "${AOM_TARGET_CPU}" STREQUAL "x86" OR "${AOM_TARGET_CPU}" STREQUAL "x86_64")
    ...
    if(NOT AS_EXECUTABLE)
      message(
        FATAL_ERROR
          "Unable to find assembler. To build without "
  ...
  aom_check_source_compiles("pthread_check" "#include <pthread.h>"
                            HAVE_PTHREAD_H)
  ...
  # Test compiler flags.
  if(MSVC)
    add_compiler_flag_if_supported("/W3") # Disable MSVC warnings that suggest
    ...
    add_compiler_flag_if_supported("-Wvla") # TODO(jzern): this could be added
                                            # as a cxx flags for test/*.cc only,
    ...
    endif() # Flag(s) added here negate CMake defaults.
  ...
  if(EMSCRIPTEN) # Avoid CMake generation time errors resulting from collisions
                 # with the form  of target_link_libraries() used by
                 # Emscripten.cmake.
  ...
  math(EXPR AOM_RTCD_CUSTOM_COMMAND_COUNT
       "${AOM_RTCD_CUSTOM_COMMAND_COUNT} - 1")
