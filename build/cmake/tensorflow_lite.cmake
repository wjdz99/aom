...
function(checkout_submodules_)
  # As a quick sanity check, see if at least 1 expected file or directory
  # is present in each submodule. If so, assume they are all checked out
  # (if they are not, then the base directory will be empty).
  if((EXISTS "${AOM_ROOT}/third_party/tensorflow/tensorflow") AND
     (EXISTS "${AOM_ROOT}/third_party/tensorflow_aom/abseil-cpp/absl") AND
     (EXISTS "${AOM_ROOT}/third_party/tensorflow_aom/eigen/Eigen") AND
     (EXISTS "${AOM_ROOT}/third_party/tensorflow_aom/farmhash/Makefile.am") AND
     (EXISTS "${AOM_ROOT}/third_party/tensorflow_aom/flatbuffers/BUILD") AND
     (EXISTS "${AOM_ROOT}/third_party/tensorflow_aom/gemmlowp/BUILD"))
     return()
  endif()

  if(NOT GIT_FOUND)
    message(FATAL_ERROR "Tensorflow-Lite/dependencies not present; "
    "git could not be found; "
    "please check out submodules with 'git submodule update --init'")
  endif()
  # Note that "git submodule update --init" must be run from inside the
  # git repository; the --git-dir flag does not work.
  ...
  if(NOT ${submodule_result} EQUAL 0)
     message(FATAL_ERROR "Unable to run 'git submodule update --init': "
             "Return code: " ${submodule_result}
             ", STDOUT: " ${submodule_out}
             ", STDERR: " ${submodule_err})
  endif()
endfunction()
...
function(add_tensorflow_lite_dependency_)
  ...
  include_directories("${AOM_ROOT}/third_party/tensorflow_aom/flatbuffers/include/")
