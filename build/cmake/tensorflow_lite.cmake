...
# TF-Lite specifies a minimum CMake version of 3.16, but Jenkins uses 3.7.2. TF-
# Lite works with 3.7.2. Until Jenkins is upgraded, patch the the minimum
# requirement. This patch requires a full copy; otherwise, it will taint the
# client. This copy/patch routine can be removed when Jenkins is upgraded to
# 3.16.
function(copy_modify_tf_lite_src_)
  # Already copied / modified - return early.
  if(EXISTS "${CMAKE_BINARY_DIR}/tensorflow")
    return()
  endif()
  # Apply the patch in the current client -- needed because Tensorflow is
  # checked out as a git submodule.
  ...
  # Revert the patch in the current client. Since the working directory is
  # inside the submodule, it should not propagate outside.
  execute_process(COMMAND "${GIT_EXECUTABLE}" reset HEAD --hard
                  ...
                  RESULT_VARIABLE submodule_result)
  if(NOT ${submodule_result} EQUAL 0)
    message(FATAL_ERROR "Unable to revert CMake patch to TF-Lite: "
                        "Return code: " ${submodule_result} ", STDOUT: "
                        ${submodule_out} ", STDERR: " ${submodule_err})
  endif()
endfunction()

# Add the TF-lite link-related library to the target. This function handles the
# fact that different operating systems have different naming conventions.
