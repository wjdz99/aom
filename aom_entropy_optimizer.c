...
static void stats_parser(FILE *statsfile, FILE *probsfile, int dim_of_cts,
  ...
  int count = 1;
  for (int dim = 0; dim < dim_of_cts; ++dim) count = count * cts_each_dim[dim];
  total_parsed_count += count;
}

static void skip_stats(FILE *statsfile, int num_of_counts) {
  fseek(statsfile, sizeof(aom_count_type) * num_of_counts, SEEK_CUR);
  total_parsed_count += num_of_counts;
}
...
int main(int argc, const char **argv_) {
  ...
  if (total_parsed_count != (sizeof(FRAME_COUNTS) / sizeof(aom_count_type)))
    printf(
        "Warning: # of parsed data does not match # of data in "
        "FRAME_COUNTS, parsed %d, target %d\n",
        total_parsed_count,
        (unsigned int)(sizeof(FRAME_COUNTS) / sizeof(aom_count_type)));
