...
typedef void *(*alloc_img_data_cb_fn_t)(void *priv, size_t size);

/*!\brief Open a descriptor, allocating storage for the underlying image by
 * using the provided callback function.
 *
 * Returns a descriptor for storing an image of the given format. The
 * storage for the underlying image is allocated by using the provided callback
 * function. Unlike aom_img_alloc(), the returned descriptor does not own the
 * storage for the underlying image. The caller is responsible for freeing the
 * storage for the underlying image.
 *
