...
typedef struct aom_svc_ref_frame_config {
  ...
  // or 1 (use as a reference). The index 0 - 6 refers to the references:
  // last(0), last2(1), last3(2), golden(3), bwdref(4), altref2(5), altref(6).
  // ref_idx[7] maps a reference to one of the 8 buffers slots, values are
  // 0 - 7.
  // refresh[8] is a flag to indicate if a buffer is updated/refreshed with
  // the encoded frame. Values are 0 (no refresh) or 1 (refresh).
