...
enum aome_enc_control_id {
   ...
   * in all intra mode. The input should be the name of a text file, which
   * contains (rows x cols) float values separated by space.
   * Each float value represent the number of bits for each 16x16 block.
   * rows = ROUND_POWER_OF_TWO(frame_height + 3, 2) / 4
   * cols = ROUND_POWER_OF_TWO(frame_width + 3, 2) / 4
   *
   * \attention This feature requires requires --enable-rate-guide-deltaq=1.
   */
