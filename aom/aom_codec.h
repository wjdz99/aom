 ...
 * provided, which will type-check the parameter against the control code before
 * calling aom_codec_control - note that this macro requires the control code
 * to be directly encoded in it, e.g.,
 * AOM_CODEC_CONTROL_TYPECHECKED(&ctx, AOME_SET_CPUUSED, 8).
 *
 * The codec control codes can be found in aom.h, aomcx.h, and aomdx.h
 ...
 * are used to work-around type-checking). If the context is non-null and
 * an error occurs, ctx->err will be set to the same value as the return value.
 *
 * \param[in]     ctx              Pointer to this instance's context
 ...
 * to aom_codec_control(). However, it requires the explicit control code
 * be passed in (it cannot be passed in via a variable) -- otherwise a compiler
 * error will occur. After the type conversion, it calls aom_codec_control.
 */
#define AOM_CODEC_CONTROL_TYPECHECKED(ctx, id, data) \
  aom_codec_control_typechecked_##id(ctx, id, data) /**<\hideinitializer*/

/*!\brief Creates typechecked functions for AOM_CODEC_CONTROL_TYPECHECKED
 *
 * It defines a static function with the correctly typed arguments as a wrapper
 * to the type-unsafe aom_codec_control function.
 */
