...
/*!\brief Key & Value API
 *
 * aom_codec_set_option takes a context, a key (option name), a value and a
 * string buffer for the error message. If the context is non-null and an error
 * occurs, ctx->err will be set to the same value as the return value. The
 * details of the error will be stored in ctx->priv->err_detail.
 *
 * \param[in]     ctx        Pointer to this instance's context
