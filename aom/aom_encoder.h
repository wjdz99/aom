...
typedef struct aom_codec_enc_cfg {
   ...
   * for still picture encoding. If zero (the default), a reduced header
   * (reduced_still_picture_header is set to 1 in the sequence header OBU) is
   * used for still picture. This flag has no effect when a regular video with
