...
typedef struct aom_codec_cx_pkt {
  ...
  union {
    ...
    struct aom_psnr_pkt {
      ...
      unsigned int samples_hbd[4]; /**< Number of samples, total/y/u/v when
                                      input bit-depth < stream bit-depth */
      uint64_t sse_hbd[4]; /**< sum squared error, total/y/u/v when input
                              bit-depth < stream bit-depth */
      double psnr_hbd[4];  /**< PSNR, total/y/u/v when input bit-depth < stream
                              bit-depth */
