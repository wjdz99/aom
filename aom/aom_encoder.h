...
typedef struct aom_codec_enc_cfg {
   ...
   * sframes. It will only have an effect if sframe_dist != 0.
   *
   * If altref is enabled:
   *   - if sframe_mode == 1, only if the considered frame is an altref frame
   *     do we make it an sframe.
   *   - if sframe_mode == 2, the next altref frame will be made into an
   *     sframe.
   *
   * Otherwise: the considered frame will be made into an sframe.
   */
