...
size_t aom_metadata_array_free(aom_metadata_array_t *arr) {
  ...
  if (arr->metadata_array) {
    for (size_t i = 0; i < arr->sz; i++) {
      if (aom_metadata_free(arr->metadata_array[i]) == 0) {
        deleted_metadatas++;
      }
    }
...
int aom_add_metadata_to_img(aom_image_t *img, uint8_t type, uint8_t *data,
  ...
  if (!img) return -1;
  if (!img->metadata) {
    img->metadata = aom_metadata_array_alloc(0);
...
size_t aom_remove_metadata_from_img(aom_image_t *img) {
  if (img && img->metadata) {
    size_t sz = aom_metadata_array_free(img->metadata);
    img->metadata = NULL;
    return sz;
  }
  return 0;
