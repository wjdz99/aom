...
static bool ctrl_fn_map_eq(const aom_codec_ctrl_fn_map_t *e1,
                           const aom_codec_ctrl_fn_map_t *e2) {
  return e1->ctrl_id == e2->ctrl_id && e1->fn == e2->fn;
}
...
aom_codec_err_t aom_codec_control_(aom_codec_ctx_t *ctx, int ctrl_id, ...) {
  ...
  // Control ID must be non-zero (zero is a special value used as a catch-all).
  if (!ctrl_id) {
  ...
  for (aom_codec_ctrl_fn_map_t *entry = ctx->iface->ctrl_maps;
       !ctrl_fn_map_eq(entry, &CTRL_MAP_END); ++entry) {
    // 0 for control-id indicates a 'catch-all' that applies to all
    // control codes.
