...
int obu_read_temporal_unit(FILE *infile, uint8_t **buffer, size_t *bytes_read,
  ...
  *bytes_read = 0;
  while (1) {
    ...
    const size_t alloc_size = *buffer_size + (size_t)obu_size;
#if defined(AOM_MAX_ALLOCABLE_MEMORY)
    *buffer = ((int64_t)alloc_size > AOM_MAX_ALLOCABLE_MEMORY)
                  ? NULL
                  : (uint8_t *)realloc(*buffer, alloc_size);
#else
    *buffer = (uint8_t *)realloc(*buffer, alloc_size);
#endif
    if (!*buffer) {
