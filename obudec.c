...
int file_is_obu(struct ObuDecInputContext *obu_ctx) {
  ...
  size_t length_of_tu_size = 0;
  size_t length_of_fu_size = 0;
  uint64_t tu_size;
  uint64_t fu_size;

  if (is_annexb) {
    uint8_t annexb_header[OBU_MAX_LENGTH_FIELD_SIZE * 2];

    // read the size of first temporal unit
    if (obudec_read_leb128(f, &annexb_header[0], &length_of_tu_size,
                           &tu_size) != 0) {
    ...
    if (obudec_read_leb128(f, &annexb_header[length_of_tu_size],
                           &length_of_fu_size, &fu_size) != 0) {
  ...
  if (!is_annexb) {
    ...
    size_t coded_size;
    if (aom_uleb_encode(tu_size, sizeof(tu_size), obu_ctx->buffer,
                        &coded_size) != 0) {
      fprintf(stderr, "obudec: Failed to write back temporal unit size\n");
      return -1;
    }
    obu_ctx->bytes_buffered = (size_t)length_of_tu_size;

    if (aom_uleb_encode(fu_size, sizeof(fu_size),
                        obu_ctx->buffer + length_of_tu_size,
                        &coded_size) != 0) {
      fprintf(stderr, "obudec: Failed to write back temporal unit size\n");
      return -1;
    }
    obu_ctx->bytes_buffered += length_of_fu_size;

    memcpy(obu_ctx->buffer + obu_ctx->bytes_buffered, &detect_buf[0],
           (size_t)obu_length);
    obu_ctx->bytes_buffered += (size_t)obu_length;
  }
  ...
  *bytes_read = 0;
  ...
  if (obu_ctx->is_annexb) {
    ...
    if (obu_ctx->bytes_buffered == 0) {
      if (obudec_read_leb128(f, &obutd[0], &length_of_temporal_unit_size,
                             &size) != 0) {
        fprintf(stderr, "obudec: Failure reading temporal unit header\n");
        return -1;
      }
  ...
  if (!obu_ctx->is_annexb) {
    ...
    if (!feof(f)) {
      ...
      if (!obu_ctx->bytes_buffered) {
        ...
        size_t coded_size;
        if (aom_uleb_encode(data_size, sizeof(data_size), new_buffer,
                            &coded_size) != 0) {
          fprintf(stderr, "obudec: Failed to write back temporal unit size\n");
          return -1;
        }
