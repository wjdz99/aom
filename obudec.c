...
// To detect whether a file is in OBU format, the maximum amount of data
// we need to read is:
// * Temporal unit length
// * Frame length
// * OBU header
// * OBU payload length
...
static int obudec_read_one_obu(FILE *f, size_t buffer_capacity, int is_annexb,
  ...
  int status = obudec_read_obu_header_and_size(f, buffer_capacity, is_annexb,
                                               obu_data, &bytes_read,
                                               &obu_payload_length, obu_header);
...
int file_is_obu(struct ObuDecInputContext *obu_ctx) {
  ...
  if (payload_length > 0) {
    ...
    int status =
        obudec_read_obu_payload(f, payload_length, &obu_ctx->buffer[obu_length],
