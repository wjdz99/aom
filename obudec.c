...
#define OBU_MIN_HEADER_SIZE (OBU_HEADER_SIZE_BYTES + 1)
...
#define OBU_MIN_HEADER_SIZE (OBU_HEADER_SIZE_BYTES + PRE_OBU_SIZE_BYTES)
#define OBU_MAX_HEADER_SIZE \
...
// returns FILE ptr to position of the first length field byte.
int read_obu_size(FILE *infile, uint64_t *obu_size) {
  ...
  const long seek_pos = (long)bytes_read;
  if (seek_pos != 0 && fseek(infile, -seek_pos, SEEK_CUR) != 0) {
   fprintf(stderr, "SEEK FAIL\n");
   return 1;
  }

#if CONFIG_OBU_SIZING
  if (aom_uleb_decode(read_buffer, bytes_read, obu_size) != 0) return 1;
#else
  *obu_size = mem_get_le32(read_buffer);
#endif

  fprintf(stderr, "|R_O_S: bytes_read=%zu obu_size=%llu\n", bytes_read,
          *obu_size);
  return 0;
}
...
int obu_read_temporal_unit(FILE *infile, uint8_t **buffer, size_t *bytes_read,
  ...
  *bytes_read = 0;
  while (1) {
    ret = read_obu_size(infile, &obu_size);
    if (ret != 0) {
    ...
    fprintf(stderr, "|O_R_T_U: buffer_size=%zu\n", *buffer_size);
    fprintf(stderr, "|O_R_T_U: length_field_size=%zu\n", length_field_size);
    fprintf(stderr, "|O_R_T_U: total obu_size=%llu\n", obu_size);
    ...
    *bytes_read += obu_size;
    fprintf(stderr, "|O_R_T_U: bytes_read=%zu buffer_size=%zu\n", *bytes_read,
            *buffer_size);
    fprintf(stderr, "|O_R_T_U: 1\n");

    if (((data[length_field_size] >> 3) & 0xF) == OBU_TEMPORAL_DELIMITER) {
      ...
      *bytes_read -= obu_size;
      const long l_obu_size = (long)obu_size;
      fseek(infile, -l_obu_size, SEEK_CUR);
      break;
    }

    fprintf(stderr, "|O_R_T_U: 2\n");
#if CONFIG_SCALABILITY
...
#if CONFIG_OBU_SIZING
  if (aom_uleb_decode(obutd, OBU_MAX_HEADER_SIZE, &size) != 0) {
