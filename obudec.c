...
static int obudec_read_obu_header(FILE *f, size_t buffer_capacity,
                                  ...
                                  size_t *bytes_read, int is_annexb) {
...
static int obudec_read_one_obu(FILE *f, size_t buffer_capacity,
  ...
  size_t bytes_read = 0;
  if (obudec_read_obu_header(f, buffer_capacity, obu_data + leb128_length,
                             obu_header, &bytes_read, is_annexb) != 0) {
    return -1;
...
int file_is_obu(struct ObuDecInputContext *obu_ctx) {
  ...
  int is_annexb = obu_ctx->is_annexb;
  FILE *f = avx_ctx->file;
...
int obudec_read_temporal_unit(struct ObuDecInputContext *obu_ctx,
  ...
  *bytes_read = 0;
  ...
  while (1) {
    ...
    if (obudec_read_one_obu(f, capacity, data, &obu_size, &obu_header,
                            is_annexb) != 0) {
