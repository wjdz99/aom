...
static int read_obu_header(FILE *f, size_t *consumed,
                           ObuHeader *header) {
  if (!f || !consumed || !header) return -1;

  uint8_t obu_header_data[OBU_HEADER_SIZE + OBU_EXTENSION_SIZE] = { 0 };
  const size_t bytes_read = fread(&obu_header_data[0], 1, OBU_HEADER_SIZE, f);

  aom_codec_err_t parse_result =
      aom_read_obu_header(&obu_header_data[0], bytes_read, consumed, header);
  if (parse_result != AOM_CODEC_OK) return parse_result;

  if (header->has_extension) {
    obu_header_data[OBU_HEADER_SIZE] = fgetc(f);
    parse_result =
        aom_read_obu_header(&obu_header_data[0], bytes_read, consumed, header);
  }
