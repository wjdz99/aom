...
each transform listed is the largest transforms that fits in both TX_8X4 and
its corresponding partitioned block size.

The output has #if / #endif pairs to deal with configuration options. The
simplest case is because some block sizes and transform sizes only exist for
certain config options. More complicated, the correct answer for an entry might
depend on the set of config options. For example, see BLOCK_4X4: if
CONFIG_CHROMA_2X2 is defined, the blocks will divide down to 2X2, 2X4 or 4X2,
giving TX_2X2 in the output. If it is not defined, the transform sizes will
never get smaller than 4X4.
...
guaranteed by code in greatest_txforms_within, which can easily be changed if
we decide to allow such divisions.
...
def print_if_line(stream, yes_flags,
                  no_flags=None, end=False, extra_comment=None):
    '''Write the #if or #endif line for the given flags to stream'''
    assert yes_flags or no_flags
    if end:
        assert not extra_comment
        stream.write('#endif  // {}\n'
                     .format(if_condition(yes_flags, no_flags)))
        return
...
def greatest_txforms_within(bsize, txform):
    ...
    smaller than 4X4 unless the input transform was <= 4X4.
    ...
    if min(best) < 4 and max(txform) > 4:
        # Never divide something like 8X4 down to 2X2.
        best = (4, 4)
...
def txsize_options(block_size, tx_size, base_flags):
    ...
    quad = [greatest_txforms_within(bs, tx_size) for bs in bsizes]
    flags = set()
    for lst in quad:
...
def print_block_size_lines(stream, block_size, bs_flags):
    ...
    for i, rows in enumerate(arrays):
        ...
        for tx_size, tx_row in zip(__TX_SIZES, rows):
            ...
            if extra_tx_flags:
                print_if_line(stream, extra_tx_flags, end=True)

        if opt_flags:
            print_if_line(stream, opt_flags, end=True)
