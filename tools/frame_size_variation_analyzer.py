...
#    Example: python ../aom/tools/frame_size_variation_analyzer.py frame_sizes.csv 1000 30
...
def frame_size_analysis(filename, target_br, fps):
  ...
  mismatch = np.absolute(np.subtract(fsize, np.full(fsize.size, tbr)))
  ...
  win_mismatch = np.absolute(np.subtract(avg_fsize, np.full(avg_fsize.size, tbr)))
