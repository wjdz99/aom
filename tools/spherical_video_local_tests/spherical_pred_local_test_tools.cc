...
// TODO: implement y4m read function
...
void write_bmp(char *filename, int width, int height, uint8_t *y,
  ...
  uint8_t *lume = (uint8_t *)malloc(width * height * sizeof(uint8_t));
...
static void motion_search_test(const TestConfig *test_config) {
  ...
  picture_t ref_pic;
  cur_pic.img.plane[0] =
      (uint8_t *)malloc(handle->width * handle->height * sizeof(uint8_t));
  cur_pic.img.plane[1] =
      (uint8_t *)malloc(handle->width * handle->height / 4 * sizeof(uint8_t));
  cur_pic.img.plane[2] =
      (uint8_t *)malloc(handle->width * handle->height / 4 * sizeof(uint8_t));
  ref_pic.img.plane[0] =
      (uint8_t *)malloc(handle->width * handle->height * sizeof(uint8_t));
  ...
  int sad_bruteforce_erp = sad_of_blocks_from_idx(
      ...
      block_stride);
  sprintf(output, "%.*s_%d.bmp", strlen(filename) - 4, filename,
  ...
  write_bmp(output, handle->width, handle->height, cur_pic.img.plane[0],
            ...
            block_stride);
  sprintf(output, "%.*s_%d_to_%d_bruteforce_erp.bmp", strlen(filename) - 4,
          filename, test_config->cur_frame_no, test_config->ref_frame_no);
  ...
  get_pred_block_idx(test_config->block_x, test_config->block_y,
                     ...
                     handle->width, handle->height, block_stride, cur_block,
                     pred_block);
  sprintf(output, "%.*s_%d_to_%d_diamond_erp.bmp", strlen(filename) - 4,
          filename, test_config->cur_frame_no, test_config->ref_frame_no);
  write_bmp(output, handle->width, handle->height, ref_pic.img.plane[0],
            pred_block, test_config->block_width, test_config->block_height,
            block_stride);
...
static void write_block_map(FILE *fp, uint8_t *r, uint8_t *g, uint8_t *b,
  ...
  for (int j = height - 1; j >= 0; j--) {
    for (int i = 0; i < width; i++) {
      fwrite(&r[i + j * width], sizeof(char), 1, fp);
      fwrite(&g[i + j * width], sizeof(char), 1, fp);
      fwrite(&b[i + j * width], sizeof(char), 1, fp);
    }
  }
}
...
static void write_plane_data(const char *data_origin,
  ...
  char filepath[80];
  sprintf(filepath, "%s%04d_%d_%d_%s_%d.txt",
  ...
  char block_info[80];
  sprintf(block_info, "#%d block at (%d, %d), size %d x %d, SAD: %d\n\n",
          block_cnt, block_x, block_y, test_config->block_width,
  ...
  for (int idx_y = 0; idx_y < test_config->block_height; idx_y++) {
    for (int idx_x = 0; idx_x < test_config->block_width; idx_x++) {
      ...
      sprintf(coord_buf, "(%4d, %4d) ", coord_x[pos_coord], coord_y[pos_coord]);
      fputs(coord_buf, fp);
    }
    fputs("\n", fp);
  }
  ...
  for (int idx_y = 0; idx_y < test_config->block_height; idx_y++) {
    ...
    for (int idx_x = 0; idx_x < test_config->block_width; idx_x++) {
      ...
      sprintf(pixel_buf, "%3d, ", frame[pos_frame]);
      fputs(pixel_buf, fp);
    }
    fputs("]\n", fp);
  }
...
static void write_erp_interp_data(const char *data_origin,
  ...
  for (int idx_y = 0; idx_y < test_config->block_height; idx_y++) {
    for (int idx_x = 0; idx_x < test_config->block_width; idx_x++) {
      pos_coord = idx_x + idx_y * coord_stride;
      sprintf(coord_buf, "(%4.4f, %4.4f) ", coord_x[pos_coord],
              coord_y[pos_coord]);
      fputs(coord_buf, fp);
    }
    fputs("\n", fp);
  ...
  for (int idx_y = 0; idx_y < test_config->block_height; idx_y++) {
    for (int idx_x = 0; idx_x < test_config->block_width; idx_x++) {
      pos_coord = idx_x + idx_y * coord_stride;
      sprintf(int_coord_buf, "(%4d, %4d) ", int_coord_x[pos_coord],
              int_coord_y[pos_coord]);
  ...
  for (int idx_y = 0; idx_y < test_config->block_height; idx_y++) {
    for (int idx_x = 0; idx_x < test_config->block_width; idx_x++) {
      pos_coord = idx_x + idx_y * coord_stride;
      sprintf(kernel_buf, "(%1d, %1d) ", kernel_x[pos_coord],
              kernel_y[pos_coord]);
      fputs(kernel_buf, fp);
    }
    fputs("\n", fp);
...
static void init_final_search_result(FinalSearchResult *result, int row_cnt,
  ...
  // result->sad_erp_bruteforce_interp =
  // (int *)malloc(row_cnt * col_cnt * sizeof(int));
  result->sad_erp_diamond_p2p = (int *)malloc(row_cnt * col_cnt * sizeof(int));
