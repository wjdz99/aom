...
static void motion_search_test(const TestConfig *test_config) {
  ...
  int sad_bruteforce_erp = sad_of_blocks_from_idx(
      ...
      block_stride);
  snprintf(output, 80, "%.*s_%d.bmp", strlen(filename) - 4, filename,
  ...
  write_bmp(output, handle->width, handle->height, cur_pic.img.plane[0],
            ...
            block_stride);
  snprintf(output, 80, "%.*s_%d_to_%d_bruteforce_erp.bmp", strlen(filename) - 4,
  ...
  get_pred_block_idx(test_config->block_x, test_config->block_y,
                     ...
                     handle->width, handle->height, block_stride, cur_block,
                     pred_block);
  snprintf(output, 80, "%.*s_%d_to_%d_diamond_erp.bmp", strlen(filename) - 4,
           filename, test_config->cur_frame_no, test_config->ref_frame_no);
  write_bmp(output, handle->width, handle->height, ref_pic.img.plane[0],
            pred_block, test_config->block_width, test_config->block_height,
            block_stride);
...
static void write_plane_data(const char *data_origin,
  ...
  char filepath[80];
  snprintf(filepath, 80, "%s%04d_%d_%d_%s_%d.txt",
  ...
  char block_info[80];
  snprintf(block_info, 80, "#%d block at (%d, %d), size %d x %d, SAD: %d\n\n",
  ...
  for (int idx_y = 0; idx_y < test_config->block_height; idx_y++) {
    for (int idx_x = 0; idx_x < test_config->block_width; idx_x++) {
      ...
      snprintf(coord_buf, 14, "(%4d, %4d) ", coord_x[pos_coord],
               coord_y[pos_coord]);
      fputs(coord_buf, fp);
    }
    fputs("\n", fp);
  ...
  for (int idx_y = 0; idx_y < test_config->block_height; idx_y++) {
    ...
    for (int idx_x = 0; idx_x < test_config->block_width; idx_x++) {
      ...
      snprintf(pixel_buf, 6, "%3d, ", frame[pos_frame]);
      fputs(pixel_buf, fp);
    }
    fputs("]\n", fp);
  }
...
static void write_erp_interp_data(const char *data_origin,
  ...
  for (int idx_y = 0; idx_y < test_config->block_height; idx_y++) {
    for (int idx_x = 0; idx_x < test_config->block_width; idx_x++) {
      pos_coord = idx_x + idx_y * coord_stride;
      snprintf(coord_buf, 24, "(%4.4f, %4.4f) ", coord_x[pos_coord],
               coord_y[pos_coord]);
      fputs(coord_buf, fp);
    }
    fputs("\n", fp);
  ...
  for (int idx_y = 0; idx_y < test_config->block_height; idx_y++) {
    for (int idx_x = 0; idx_x < test_config->block_width; idx_x++) {
      pos_coord = idx_x + idx_y * coord_stride;
      snprintf(int_coord_buf, 14, "(%4d, %4d) ", int_coord_x[pos_coord],
  ...
  for (int idx_y = 0; idx_y < test_config->block_height; idx_y++) {
    for (int idx_x = 0; idx_x < test_config->block_width; idx_x++) {
      pos_coord = idx_x + idx_y * coord_stride;
      snprintf(kernel_buf, 8, "(%1d, %1d) ", kernel_x[pos_coord],
               kernel_y[pos_coord]);
      fputs(kernel_buf, fp);
    }
    fputs("\n", fp);
  ...
  for (int idx_y = 0; idx_y < test_config->block_height; idx_y++) {
    ...
    for (int idx_x = 0; idx_x < test_config->block_width; idx_x++) {
      ...
      snprintf(pixel_buf, 6, "%3d, ", result);
      fputs(pixel_buf, fp);
    }
    fputs("]\n", fp);
  }
...
static void write_final_search_result(const TestConfig *test_config,
  ...
  char filepath[80];
  snprintf(filepath, 80, "%sresults.csv",
