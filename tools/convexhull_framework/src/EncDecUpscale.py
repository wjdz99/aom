...
def GetBitstreamFile(method, codec, preset, yuvfile, qp, outpath):
    ...
    filename = GetShortContentName(yuvfile, False) + Prefix_EncodeCfg + "_QP_" + str(qp) + bs_suffix
...
def Encode(method, codec, preset, input, qp, num, framerate, width, height, path):
    ...
    VideoEncode(method, codec, input, qp, num, framerate, width, height, bsfile, preset)
    return bsfile
...
def Run_EncDec_Upscale(method, codec, preset, input, QP, num, framerate, inw, inh, outw, outh, path_bs, path_decoded,
                       path_upscaled, algo):
    logger.info("%s %s start encode file %s with QP = %d" % (method, codec, os.path.basename(input), QP))
    bsFile = Encode(method, codec, preset, input, QP, num, framerate, inw, inh, path_bs)
    ...
    upscaledYUV = UpScaling(decodedYUV, num, inw, inh, outw, outh, path_upscaled, algo)
    logger.info("finish Run Encode, Decode and Upscale")
    return upscaledYUV


def GetBsReconFileName(encmethod, codecname, preset, content, width, height, dnwidth, dnheight, dnScAlgo,
                       upScAlgo, qp, path_bs):
    dsyuv_name = GetDownScaledOutFile(content, width, height, dnwidth, dnheight, path_bs, dnScAlgo)
    ...
    reconfilename = GetUpScaledOutFile(decoded, dnwidth, dnheight, width, height, path_bs, upScAlgo)
