...
HDRToolsMetricsFullList = ['PSNR_Y', 'PSNR_U', 'PSNR_V', 'MS-SSIM_Y', 'MS-SSIM_U', 'MS-SSIM_V', 'SSIM_Y', 'SSIM_U', 'SSIM_V']
...
#return MS-SSIM or PSNR of 3 components
def ParseHDRToolLogFile(logfile):
    ...
    for metrics, idx in zip(HDRToolsMetricsFullList, range(len(HDRToolsMetricsFullList))):
...
####################################################################################
##################### Exposed Functions ############################################
def HDRTool_CalQualityMetrics(origfile, recfile, num, w, h, logfilepath, cfgfilepath):
