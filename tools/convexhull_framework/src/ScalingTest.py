...
def GeneratePerClipExcelFile(dnScalAlgos, upScalAlgos, clip, path_dnscl, path_log):
    ...
    for dn_algo, up_algo, col, i in zip(dnScalAlgos, upScalAlgos, ScalQty_WtCols,
        ...
        for ratio, row, idx in zip(DnScaleRatio, rows, range(len(DnScaleRatio))):
            ...
            dw = continfos[idx][2]
            dh = continfos[idx][3]
            dnScalOut = GetDownScaledOutFile(clip, dw, dh, path_dnscl, dn_algo)
...
def GenerateSummarySheet(wb, dnScalAlgos, upScalAlgos, ratio, path_dnscl,
                         path_log):
    ...
    for dn_algo, up_algo, col, i in zip(dnScalAlgos, upScalAlgos,
        ...
        for (clss, clip_list), row_clss in zip(ClipDict.items(), Rows_Class):
            for clip, row_cont in zip(clip_list, range(len(clip_list))):
                ...
                upScalOut = GetUpScaledOutFile(ds_clip, w, h, up_algo, path_dnscl)
                qtys = GatherQualityMetrics(upScalOut, path_log)
...
def SaveScalingResultsToExcel(dnScalAlgos, upScalAlgos, clip_list, path_dnscl,
                              path_log):
    ...
    for clip in clip_list:
        GeneratePerClipExcelFile(dnScalAlgos, upScalAlgos, clip, path_dnscl,
                                 path_log)
