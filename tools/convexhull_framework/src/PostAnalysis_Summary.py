...
def GetSummaryFileName(encMethod, codecName, preset, path):
    name = 'ConvexHullSummary_ScaleAlgosNum_%d_%s_%s_%s.xlsm' % (len(DnScaleRatio), encMethod, codecName, preset)
    return os.path.join(path, name)

def GetConvexHullRDFileName(encMethod, codecName, preset, path):
    name = 'ConvexHullRD_ScaleAlgosNum_%d_%s_%s_%s.xlsx' % (len(DnScaleRatio), encMethod, codecName, preset)
    return os.path.join(path, name)
        ...
        for rdcol, col_res, residx in zip(rdcols, cols_res, range(len(DnScaleRatio))):
            for i in range(len(QPs)):
                ...
                for col_upscl, sht in zip(cols_upscl, rdshts):
                    ...
                    avg_sht.write_formula(row_class + i, col_res + col_upscl, formula)
                    # write quality average formula
                    for j in range(len(QualityList)):
                        formula = GenerateFormula_SumRows(shtname, sum_rows, rdcol + 1 + j)
                        avg_sht.write_formula(row_class + i, col_res + col_upscl + 1 + j, formula)
    ...
    for col_res, residx in zip(cols_res, range(len(DnScaleRatio))):
        for i in range(len(QPs)):
            ...
            for col_upscl in cols_upscl:
                ...
                avg_sht.write_formula(last_class_row + i, col_res + col_upscl, formula)
                # quality average
                for j in range(len(QualityList)):
                    formula = GenerateFormula_SumRows_Weighted(sum_rows, col_res + col_upscl + 1 + j, weight_rows, 1, totalnum_content)
                    avg_sht.write_formula(last_class_row + i, col_res + col_upscl + 1 + j, formula)
                # for first resolution, no down and up scaling. only need one set of bitrate/quality data
                if residx == 0:
                    break
...
def WriteBDRateAverageSheet(wb, rdshts, contentsdict, rd_rows_class, rd_cols_bdmtrs, cellformat):
    ...
    for residx, col_res_bd in zip(range(1, len(DnScaleRatio)), cols_res_bd):
        bdavg_sht.write(0, col_res_bd, 'BD-Rate %.2f vs. %.2f' % (DnScaleRatio[residx], DnScaleRatio[0]))
        ...
        for rdcol, col_res in zip(rd_cols_bdmtrs, cols_res_bd):
            ...
            for col_upscl, sht in zip(cols_upscl_bd, rdshts):
                ...
                for j in range(len(QualityList)):
                    ...
                    bdavg_sht.write_formula(row_class, col_res + col_upscl + j, formula, cellformat)
    ...
    for col_res in cols_res_bd:
        for col_upscl in cols_upscl_bd:
            for j in range(len(QualityList)):
                ...
                bdavg_sht.write_formula(last_row, col_res + col_upscl + j, formula, cellformat)
...
def GenerateSummaryConvexHullExcelFile(encMethod, codecName, preset, summary_outpath, DnScalingAlgos, UpScalingAlgos):
    if not os.path.exists(summary_outpath):
        os.makedirs(summary_outpath)
    smfile = GetConvexHullRDFileName(encMethod, codecName, preset, summary_outpath)
