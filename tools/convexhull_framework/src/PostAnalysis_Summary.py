...
import xlsxwriter
import xlrd
from Config import QPs, DnScaleRatio, QualityList, VbaBinFile, CvxH_WtRows, CvxH_WtLastCol, LoggerName
from Utils import GetShortContentName, CalcRowsClassAndContentDict, SweepScalingAlgosInOneResultFile
import logging
...
# give all paths including convex hull result file (only one file for each content)
# to generate summary file for all contents in Input path
# assume all content's result has same test settings

####################################################################################################
### Helper Functions ###############################################################################
def GetSummaryFileName(encMethod, codecName, preset, path):
    name = 'ConvexHullSummary_ScaleAlgosNum_%d_%s_%s_%s.xlsm' % (len(dnScalAlgos), encMethod, codecName, preset)
    return os.path.join(path, name)

def CopyResultDataToSummaryFile_Onesheet(sht, wt_cols, contentsdict, rows_class, infile_path):
    rdrows = CvxH_WtRows
    ...
    # copy the results data from each content's result file to corresponding location in summary excel file
            ...
            for resfile in resultfiles:
                if key in resfile:
                    ...
                    for i, rdrow in zip(range(len(QPs)), rdrows):
                        data = rdsht.row_values(rdrow, 0, rd_endcol + 1)  # row_values(rowx, start_colx=0, end_colx=None)
                        sht.write_row(row_class + row_cont + i, 2, data)
                    break
            assert rdwb is not None
            if rdwb is None:
                logger.warning("not find convex hull result file for content:%s" % content)
...
def CalBDRate_OneSheet(sht,  cols, contentsdict, rows_class, cols_bdmtrs, cellformat):
    ...
    for cols_bd, residx in zip(cols_bdmtrs, range(1, len(DnScaleRatio))):
        sht.write(0, cols_bd, 'BD-Rate %.2f vs. %.2f' % (DnScaleRatio[residx], DnScaleRatio[0]))
        sht.write_row(1, cols_bd, QualityList)
        for (cls, contents), row_class in zip(contentsdict.items(), rows_class):
            rows_content = [i * len(QPs) for i in range(len(contents))]
            for row_cont in rows_content:
                for y in range(len(QualityList)):
                    refbr_b = xlrd.cellnameabs(row_class + row_cont + row_refst, cols[0])
                    refbr_e = xlrd.cellnameabs(row_class + row_cont + row_refst + bdstep, cols[0])
                    refq_b = xlrd.cellnameabs(row_class + row_cont + row_refst, cols[0] + 1 + y)
                    refq_e = xlrd.cellnameabs(row_class + row_cont + row_refst + bdstep, cols[0] + 1 + y)

                    testbr_b = xlrd.cellnameabs(row_class + row_cont, cols[residx])
                    testbr_e = xlrd.cellnameabs(row_class + row_cont + bdstep, cols[residx])
                    testq_b = xlrd.cellnameabs(row_class + row_cont, cols[residx] + 1 + y)
                    testq_e = xlrd.cellnameabs(row_class + row_cont + bdstep, cols[residx] + 1 + y)
                    ...
                    refbr_b, refbr_e, refq_b, refq_e, testbr_b, testbr_e, testq_b, testq_e)
                    sht.write_formula(row_class + row_cont, cols_bd + y, formula, cellformat)
...
def WriteBitrateQtyAverageSheet(wb, rdshts, contentsdict, rd_rows_class, rdcols):
    lsht = wb.add_worksheet('Average')
    lsht.write(2, 0, 'Content Class')
    lsht.write(2, 1, 'Content Number')
    lsht.write(2, 2, 'QP')
    ...
    for residx, col_res in zip(range(len(DnScaleRatio)), cols_res):
        lsht.write(0, col_res, 'ScalingRatio = %.2f' % (DnScaleRatio[residx]))
        if residx == 0:
            lsht.write(1, col_res + 1, 'None')
            lsht.write(2, col_res, 'Bitrate(kbps)')
            lsht.write_row(2, col_res + 1, QualityList)
        else:
            for dnsc, upsc, col_upscl in zip(dnScalAlgos, upScalAlgos, cols_upscl):
                lsht.write(1, col_res + col_upscl + 1, '%s--%s' % (dnsc, upsc))
                lsht.write(2, col_res + col_upscl, 'Bitrate(kbps)')
                lsht.write_row(2, col_res + col_upscl + 1, QualityList)

    startrow = 3
    step = len(QPs)
    rows_class_avg = [startrow + step * i for i in range(len(contentsdict))]
    totalnum_content = 0
    for (cls, contents), row_class, rdclassrow in zip(contentsdict.items(), rows_class_avg, rd_rows_class):
        lsht.write(row_class, 0, cls)
        totalnum_content = totalnum_content + len(contents)
        lsht.write(row_class, 1, len(contents))
        lsht.write_column(row_class, 2, QPs)
        rows_content = [i * len(QPs) for i in range(len(contents))]
        ...
        for rdcol, col_res, residx in zip(rdcols, cols_res, range(len(DnScaleRatio))):
            for i in range(len(QPs)):
                ...
                for col_upscl, sht in zip(cols_upscl, rdshts):
                    ...
                    lsht.write_formula(row_class + i, col_res + col_upscl, formula)
                    # write quality average formula
                    for j in range(len(QualityList)):
                        formula = GenerateFormula_SumRows(shtname, sum_rows, rdcol + 1 + j)
                        lsht.write_formula(row_class + i, col_res + col_upscl + 1 + j, formula)

                    # for first resolution, no down and up scaling. only need one set of bitrate/quality data
                    if residx == 0:
                        break

    # write total average
    last_class_row = rows_class_avg[-1] + len(QPs) + 1  # 1 for 1 row of interval
    lsht.write(last_class_row, 0, 'Total')
    lsht.write(last_class_row, 1, totalnum_content)
    lsht.write_column(last_class_row, 2, QPs)
    ...
    for col_res, residx in zip(cols_res, range(len(DnScaleRatio))):
        for i in range(len(QPs)):
            ...
            for col_upscl in cols_upscl:
                # bitrate average
                formula = GenerateFormula_SumRows_Weighted(sum_rows, col_res + col_upscl, weight_rows, 1, totalnum_content)
                lsht.write_formula(last_class_row + i, col_res + col_upscl, formula)
                # quality average
                for j in range(len(QualityList)):
                    formula = GenerateFormula_SumRows_Weighted(sum_rows, col_res + col_upscl + 1 + j, weight_rows, 1, totalnum_content)
                    lsht.write_formula(last_class_row + i, col_res + col_upscl + 1 + j, formula)
                # for first resolution, no down and up scaling. only need one set of bitrate/quality data
                if residx == 0:
                    break

def WriteBDRateAverageSheet(wb, rdshts, contentsdict, rd_rows_class, rd_cols_bdmtrs, cellformat):
    # write bdrate average sheet
    llsht = wb.add_worksheet('Average_BDRate')
    llsht.write(2, 0, 'Content Class')
    llsht.write(2, 1, 'Content Number')
    ...
    for residx, col_res_bd in zip(range(1, len(DnScaleRatio)), cols_res_bd):
        llsht.write(0, col_res_bd, 'BD-Rate %.2f vs. %.2f' % (DnScaleRatio[residx], DnScaleRatio[0]))
        for dnsc, upsc, col_upscl_bd in zip(dnScalAlgos, upScalAlgos, cols_upscl_bd):
            llsht.write(1, col_res_bd + col_upscl_bd, '%s--%s' % (dnsc, upsc))
            llsht.write_row(2, col_res_bd + col_upscl_bd, QualityList)

    totalnum_content = 0
    for (cls, contents), row_class, rdclassrow in zip(contentsdict.items(), rows_class_rdavg, rd_rows_class):
        llsht.write(row_class, 0, cls)
        contents = contentsdict[cls]
        totalnum_content = totalnum_content + len(contents)
        llsht.write(row_class, 1, len(contents))
        rows_content = [i * len(QPs) for i in range(len(contents))]
        ...
        for rdcol, col_res in zip(rd_cols_bdmtrs, cols_res_bd):
            ...
            for col_upscl, sht in zip(cols_upscl_bd, rdshts):
                ...
                for j in range(len(QualityList)):
                    ...
                    llsht.write_formula(row_class, col_res + col_upscl + j, formula, cellformat)

    # write total average
    last_row = rows_class_rdavg[-1] + 1
    llsht.write(last_row, 0, 'Total')
    llsht.write(last_row, 1, totalnum_content)
    sum_rows = [row_class for row_class in rows_class_rdavg]
    for col_res in cols_res_bd:
        for col_upscl in cols_upscl_bd:
            for j in range(len(QualityList)):
                formula = GenerateFormula_SumRows_Weighted(sum_rows, col_res + col_upscl + j, sum_rows, 1, totalnum_content)
                llsht.write_formula(last_row, col_res + col_upscl + j, formula, cellformat)
...
# 3. calculate BD rate across different scaling ratios for all scaling algorithms in convex hull test
# 4. calcualte average BD rate for each content class
# Arguments description:
# content_paths is where test contents located, which used for generating convex hull results.
# infile_path   is where convex hull results excel files located.
# classes       is the content classes and  here it requires contents are located in corresponding subfolder named
#               with its belonged class
# summary_outpath  is the folder where output summary file will be
# note: all results files under infile_path should have exactly same test items before running this summary script
def GenerateSummaryExcelFile(encMethod, codecName, preset, summary_outpath, infile_path, content_path, clips):
    global dnScalAlgos, upScalAlgos
    # find all scaling algos tested in results file, expect they are the same for every content
    ...
    # if to change them, modify CopyResultsDataToSummaryFile_Onesheet() and  CalcRowsCategAndContentDict() accordingly
    colstart = 3
    colInterval = 2
    rowstart = 2
    ...
    contentsdict, rows_class = CalcRowsClassAndContentDict(rowstart, content_path, clips, len(QPs))
    ...
    cols_bdmtrs = [start_col_bd + i * step for i in range(len(DnScaleRatio) - 1)]  # -1 because first resolution is used as reference

    for sht in shts:
        CopyResultDataToSummaryFile_Onesheet(sht, sum_wtcols, contentsdict, rows_class, infile_path)
        # calculate bd rate in each scaling sheet
        CalBDRate_OneSheet(sht, sum_wtcols, contentsdict, rows_class, cols_bdmtrs, cellformat)

    # calculate average bitrate and quality metrics for each category and write to "average" sheet
    ...
    WriteBDRateAverageSheet(wb, shts, contentsdict, rows_class, cols_bdmtrs, cellformat)
