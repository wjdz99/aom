...
def Interpolate(qp, logbr, qty):
    ...
    incoming qp should be in the decreasing order
    ...
    for i in range(0, len(qp)):
        ...
        #qp in the decreasing order
        if (i<len(qp)-1) and (qp[i] != qp[i + 1] - 1):
            for q in range(qp[i] - 1, qp[i + 1], -1):
...
def BD_RATE(qp1, br1, qtyMtrc1, qp2, br2, qtyMtrc2, EnablePreInterpolation):
    brqtypairs1 = [(qp1[i], br1[i], qtyMtrc1[i]) for i in range(min(len(qp1), len(qtyMtrc1), len(br1)))]
    brqtypairs2 = [(qp2[i], br2[i], qtyMtrc2[i]) for i in range(min(len(qp2), len(qtyMtrc2), len(br2)))]
    ...
    brqtypairs1.sort(key=lambda tup: tup[2])
    brqtypairs2.sort(key=lambda tup: tup[2])

    qp1 = [x[0] for x in brqtypairs1]
    logbr1 = [math.log(x[1]) for x in brqtypairs1]
    qmetrics1 = [100.0 if x[2] == float('inf') else x[2] for x in brqtypairs1]
    qp2 = [x[0] for x in brqtypairs2]
    logbr2 = [math.log(x[1]) for x in brqtypairs2]
    qmetrics2 = [100.0 if x[2] == float('inf') else x[2] for x in brqtypairs2]

    # perform an bi-linear interpolation based on QP
    if EnablePreInterpolation == True:
