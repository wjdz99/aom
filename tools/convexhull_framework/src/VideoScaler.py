...
import Utils
import logging
from Config import BinPath, LogCmdOnly, LoggerName
...
def ValidAlgo_ffmpeg(algo):
    if (algo == 'bicubic' or algo == 'lanczos' or algo == 'sinc' or algo == 'bilinear' or algo == 'spline'
       or algo == 'gauss' or algo == 'bicublin' or algo == 'neighbor'):
        return True
    else:
        return False
...
def RescaleWithFfmpeg(infile, inw, inh, outw, outh, algo, outfile, num, app_path):
    ...
    args = " -y -s:v %dx%d -i %s -vf scale=%dx%d -c:v rawvideo -pix_fmt yuv420p -sws_flags %s+accurate_rnd+full_chroma_int -sws_dither none" \
           % (inw, inh, infile, outw, outh, algo)
    if (algo == 'lanczos'):
        args += " -param0 5 "
    args += " -frames %d %s" % (num, outfile)
    args = ffmpeg_Path + args
    ExecuteCmd(args, LogCmdOnly)
...
def GetDownScaledOutFile(input, inw, inh, dnw, dnh, path, algo):
    ...
    filename = contentBaseName + ('_DnScaled_%s_%dx%d.yuv' % (actual_algo, dnw, dnh))
...
def GetUpScaledOutFile(infile, inw, inh, outw, outh, path, algo):
    ...
    filename = GetShortContentName(infile, False) + ('_UpScaled_%s_%dx%d.yuv' % (actual_algo, outw, outh))
