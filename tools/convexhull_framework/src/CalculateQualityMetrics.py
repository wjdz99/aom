...
import Utils
from CalcQtyWithVmafTool import VMAF_CalQualityMetrics, VMAF_GatherQualityMetrics, VMAFMetricsFullList
from CalcQtyWithHdrTools import HDRTool_CalQualityMetrics, HDRToolsMetricsFullList, HDRTool_GatherQualityMetrics
from CalcQtyWithFfmpeg import FFMPEG_GatherQualityMetrics, FFMPEGMetricsFullList, FFMPEG_CalQualityMetrics

subloggername = "CalcQtyMetrics"
loggername = LoggerName + '.' + '%s' % subloggername
logger = logging.getLogger(loggername)

MetricsFullDict = {'VMAF': VMAFMetricsFullList, 'HDRTools': HDRToolsMetricsFullList, 'FFMPEG': FFMPEGMetricsFullList}

def CalculateQualityMetric(content, framenum, reconYUV, width, height, logfilePath, cfgfilePath):
    ...
    for method in methods_torun:
        if method == 'VMAF':
            VMAF_CalQualityMetrics(content, reconYUV, framenum, width, height, logfilePath)
        elif method == 'HDRTools':
            HDRTool_CalQualityMetrics(content, reconYUV, framenum, width, height, logfilePath, cfgfilePath)
        elif method == 'FFMPEG':
            FFMPEG_CalQualityMetrics(content, reconYUV, framenum, width, height, logfilePath)
        else:
