...
def GetContentDict(contentpath, clips):
    ...
    if Have_Class_Subfolder:
        ...
        # this is for case no subfolder/class. As * is forbidden for folder name, no folder will be same as this
        cls = "*All"
...
def CreateChart_Scatter(wb, title, xaxis_name, yaxis_name):
    ...
    chart.set_y_axis({'name': yaxis_name, 'name_font': {'color': 'white'}, 'num_font': {'color': 'white'}})
    chart.set_style(12)
...
def AddSeriesToChart_Scatter(shtname, rows, coly, colx, chart, seriname, linecolor):
    ...
    chart.add_series({
        ...
        'marker': {'type': 'circle', 'size': 5, 'border': {'color': linecolor, 'size': 0.75}, 'fill': {'color': linecolor}},
    })

def AddSeriesToChart_Scatter_Rows(shtname, cols, rowy, rowx, chart, seriname, linecolor):
    ...
    chart.add_series({
        ...
        'marker': {'type': 'square', 'size': 5, 'border': {'color': 'white', 'size': 0.75}}
    })

def AddSeriesToChart_Line(shtname, rows, coly, colx, chart, seriname, shape, ssize, linecolor):
    yvalues = [shtname, rows[0], coly, rows[-1], coly]
    xvalues = [shtname, rows[0], colx, rows[-1], colx]
    chart.add_series({
        'name': seriname,
        ...
        'values': yvalues,
        'line': {'none': True},
        'marker': {'type': shape, 'size': ssize, 'border': {'color': linecolor, 'size': 2}, 'fill': {'color': linecolor}},
    })
...
def SetupLogging(level, logcmdonly, name, path):
    ...
    if logcmdonly or level != 0:
        global CmdLogger
        logfilename = os.path.join(path, 'ConvexHullTestCmd_%s.log' % time.strftime("%Y%m%d-%H%M%S"))
        CmdLogger = open(logfilename, 'w')

    if level != 0:
        logfilename = os.path.join(path, 'ConvexHullTest_%s.log' % time.strftime("%Y%m%d-%H%M%S"))
