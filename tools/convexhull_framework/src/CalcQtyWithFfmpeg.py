...
def FFMPEG_CalQualityMetrics(origfile, recfile, num, w, h, logfilePath):
    ...
    #here we have to pass the psnr_log file name to FFMPEG without the target log path first, then copy the result
    #psnr log to the target log path after the ffmpeg call. it doesn't work if the full path is directly passed to FFMPEG
    psnr_log = GetFfmpegLogFile(recfile, BinPath)
    psnr_log = os.path.basename(psnr_log)
    args = " -s %dx%d -pix_fmt yuv420p -i %s -s %dx%d -pix_fmt yuv420p -i %s -frames:v %d -lavfi psnr=%s -f null -" % \
