...
from Utils import GetShortContentName, GetVideoInfo, CreateChart_Scatter, AddSeriesToChart_Scatter, InsertChartsToSheet, \
    CreateNewSubfolder, GetContents, SetupLogging, UpdateChart, AddSeriesToChart_Scatter_Rows, Cleanfolder
from PostAnalysis_Summary import GenerateSummaryExcelFile, GenerateSummaryConvexHullExcelFile
from ScalingTest import Run_Scaling_Test, SaveScalingResultsToExcel
import Utils
from Config import LogLevels, FrameNum, DnScaleRatio, QPs, CvxH_WtCols, CvxH_WtRows, QualityList, LineColors, \
    DnScalingAlgos, UpScalingAlgos, ContentPath, SummaryOutPath, WorkPath, Path_RDResults, Clips, \
...
def GetRDResultExcelFile(content):
    ...
    filename = "RDResults_%s_%s_%s_%s.xlsx" % (contentBaseName, EncodeMethod, CodecName, EncodePreset)
    file = os.path.join(Path_RDResults, filename)
    return file

def setupWorkFolderStructure():
    global Path_Bitstreams, Path_DecodedYuv, Path_UpScaleYuv, Path_DnScaleYuv, Path_QualityLog, Path_TestLog, \
        Path_CfgFiles, Path_DecUpScaleYuv
...
'''
The convex_hull function is adapted based on the original python implementation from
https://en.wikibooks.org/wiki/Algorithm_Implementation/Geometry/Convex_hull/Monotone_chain
It is changed to return the lower and upper portions of the convex hull separately
to get the convex hull based on traditional rd curve, only the upper portion is needed.
'''
...
def AddConvexHullCurveToCharts(sht, startrow, startcol, charts, rdPoints, dnScaledRes, sum_sht, sum_start_row):
    ...
    for qty, idx, row in zip(QualityList, range(len(QualityList)), rows):
        ...
        AddSeriesToChart_Scatter_Rows(shtname, cols, row, row + 1, charts[idx], 'ConvexHull', ConvexHullColor)

    endrow = rows[-1] + numitems

    #add convex hull data into summary sheet. the length of each convex hull could be different
...
def CleanUp_workfolders():
    folders = [Path_DnScaleYuv, Path_Bitstreams, Path_DecodedYuv, Path_QualityLog, Path_TestLog, Path_CfgFiles]
    if not KeepUpscaledOutput:
...
def Run_ConvexHull_Test(content, dnScalAlgo, upScalAlgo):
    Utils.Logger.info("%s %s start running xcode tests with %s" % (EncodeMethod, CodecName, os.path.basename(content)))
    cls, width, height, fr, bitdepth, fmt, totalnum = GetVideoInfo(content, Clips)
    if totalnum < FrameNum:
        Utils.Logger.error("content %s includes total %d frames < specified % frames!" % (content, totalnum, FrameNum))
        return
    ...
    for i in range(len(DnScaledRes)):
        ...
        dnscalyuv = GetDownScaledOutFile(content, width, height, DnScaledW, DnScaledH, Path_DnScaleYuv, dnScalAlgo)
        if not os.path.isfile(dnscalyuv):
            dnscalyuv = DownScaling(content, FrameNum, width, height, DnScaledW, DnScaledH, Path_DnScaleYuv, dnScalAlgo)

        for QP in QPs:
            Utils.Logger.info("start transcode and upscale for %dx%d and QP %d" % (DnScaledW, DnScaledH, QP))
            #transcode and upscaling
            reconyuv = Run_EncDec_Upscale(EncodeMethod, CodecName, EncodePreset, dnscalyuv, QP, FrameNum, fr,
                                          DnScaledW, DnScaledH, width, height, Path_Bitstreams, Path_DecodedYuv,
                                          Path_DecUpScaleYuv, upScalAlgo)
            #calcualte quality distortion
            Utils.Logger.info("start quality metric calculation for %dx%d and QP %d" % (DnScaledW, DnScaledH, QP))
            CalculateQualityMetric(content, FrameNum, reconyuv, width, height, Path_QualityLog,
                                   Path_CfgFiles)
...
def SaveConvexHullResultsToExcel(content, dnScAlgos, upScAlgos, sum_wb, sum_start_row):
    ...
    for sht, indx in zip(shts, list(range(len(dnScAlgos)))):
        ...
        for col, i in zip(CvxH_WtCols, range(len(DnScaledRes))):
            ...
            for qp in QPs:
                bs, reconyuv = GetBsReconFileName(EncodeMethod, CodecName, EncodePreset, content, width, height,
                                                  DnScaledW, DnScaledH, dnScAlgos[indx], upScAlgos[indx], qp,
                                                  Path_Bitstreams)
            ...
            for x in range(len(QualityList)):
                ...
                AddSeriesToChart_Scatter(shtname, CvxH_WtRows, col + 1 + x, col, charts[x], seriname, LineColors[i])
        ...
        endrow, sum_end_row = AddConvexHullCurveToCharts(sht, startrow, startcol, charts, RDPoints, DnScaledRes, sum_sht, sum_startrow)
...
def ParseArguments(raw_args):
    parser = argparse.ArgumentParser(prog='ConvexHullTest.py', usage='%(prog)s [options]', description='')
    parser.add_argument('-f', '--function', dest='Function', type=str, required=True, metavar='',
                        choices=["clean", "scaling", "sumscaling", "encode", "convexhull", "summary"],
                        help="function to run: clean, scaling, sumscaling, encode, convexhull, summary")
    parser.add_argument('-k', "--KeepUpscaleOutput", dest='KeepUpscaledOutput', type=bool, default=False, metavar='',
                        help="in function clean, if keep upscaled yuv files. It is false by default")
    parser.add_argument('-s', "--SaveMemory", dest='SaveMemory', type=bool, default=False, metavar='',
                        help="save memory mode will delete most files in intermediate steps and keeps only necessary " +
                             "ones for RD calculation. It is false by default")
    parser.add_argument('-l', "--LoggingLevel", dest='LogLevel', type=int, default=3,
                        choices=range(len(LogLevels)), metavar='',
                        help="logging level: 0:No Logging, 1: Critical, 2: Error, 3: Warning, 4: Info, 5: Debug")
    parser.add_argument('-c', "--CodecName", dest='CodecName', type=str, choices=CodecNames, metavar='',
                        help="CodecName: av1 or hevc")
    parser.add_argument('-m', "--EncodeMethod", dest='EncodeMethod', type=str, choices=EncodeMethods, metavar='',
                        help="EncodeMethod: ffmpeg, aom, svt")
    parser.add_argument('-p', "--EncodePreset", dest='EncodePreset', type=str, metavar='',
                        help="EncodePreset: medium, slow, fast, etc for ffmpeg, 0,1,2... for aom and svt")
    if len(raw_args) == 1:
        parser.print_help()
        sys.exit(1)
    args = parser.parse_args(raw_args[1:])

    global Function, KeepUpscaledOutput, SaveMemory, LogLevel, CodecName, EncodeMethod, EncodePreset
    ...
    if Function == 'clean':
        ...
        for content in Contents:
            for dnScaleAlgo, upScaleAlgo in zip(DnScalingAlgos, UpScalingAlgos):
                Run_Scaling_Test(content, dnScaleAlgo, upScaleAlgo, Path_DnScaleYuv, Path_UpScaleYuv, Path_QualityLog,
        ...
        sum_wb, sum_start_row = GenerateSummaryConvexHullExcelFile(EncodeMethod, CodecName, EncodePreset, SummaryOutPath,
                                                                   DnScalingAlgos, UpScalingAlgos)
        for content in Contents:
            SaveConvexHullResultsToExcel(content, DnScalingAlgos, UpScalingAlgos, sum_wb, sum_start_row)
        sum_wb.close()
    elif Function == 'summary':
        smfile = GenerateSummaryExcelFile(EncodeMethod, CodecName, EncodePreset, SummaryOutPath, Path_RDResults,
                                          ContentPath, Clips)
