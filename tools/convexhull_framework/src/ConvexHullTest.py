...
def AddConvexHullCurveToCharts(sht, startrow, startcol, charts, rdPoints, dnScaledRes):
    ...
    for qty, idx, row in zip(QualityList, range(len(QualityList)), rows):
        ...
        brts = [h[0] for h in hull]
        qtys = [h[1] for h in hull]
        ...
        cols = [startcol + 1 + i for i in range(len(hull))]
...
def SaveConvexHullResultsToExcel(content, dnScAlgos, upScAlgos):
    ...
    for sht, indx in zip(shts, list(range(len(dnScAlgos)))):
        ...
        endrow = AddConvexHullCurveToCharts(sht, startrow, startcol, charts, RDPoints, DnScaledRes)
...
def ParseArguments(raw_args):
    ...
    parser.add_argument('-k', "--KeepUpscaleOutput", dest='KeepUpscaledOutput', type=bool, default=False, metavar='',
                        help="in function clean, if keep upscaled yuv files")
    parser.add_argument('-s', "--SaveMemory", dest='SaveMemory', type=bool, default=False, metavar='',
                        help="save memory mode will delete most files in intermediate steps and keeps only necessary ones for RD calculation")
    ...
    parser.add_argument('-p', "--EncodePreset", dest='EncodePreset', type=str, metavar='',
                        help="EncodePreset: medium or slow for ffmpeg, 0,1,2... for aom and svt")
    if len(raw_args) == 1:
        parser.print_help()
        sys.exit(1)
