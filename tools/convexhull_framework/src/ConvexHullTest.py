...
from Config import LogLevels, FrameNum, DnScaleRatio, QPs, CvxH_WtCols,\
    ...
    CvxHDataNum, Int_ConvexHullColor
...
def SaveConvexHullResultsToExcel(content, dnScAlgos, upScAlgos, EnablePreInterpolation=False):
    ...
    for sht, indx in zip(shts, list(range(len(dnScAlgos)))):
        ...
        endrow = AddConvexHullCurveToCharts(sht, charts, RDPoints, DnScaledRes,
                                            TargetQtyMetrics, EnablePreInterpolation, Int_RDPoints)
...
if __name__ == "__main__":
    ...
    if Function == 'clean':
        ...
        for content in Contents:
            SaveConvexHullResultsToExcel(content, DnScalingAlgos, UpScalingAlgos, True)
        ...
        CvxHsmfile = GenerateSummaryConvexHullExcelFile(EncodeMethod, CodecName, EncodePreset,
                                                       SummaryOutPath, RDResultFilesGenerated, True)
