...
def EncodeWithAOM_AV1(clip, test_cfg, QP, framenum, outfile, preset,
                      LogCmdOnly=False):
    args = " --verbose --codec=av1 -v --psnr --ivf  --frame-parallel=0" \
           " --cpu-used=%s --limit=%d --auto-alt-ref=1 --passes=1" \
           " --end-usage=q --i%s --threads=1 --lag-in-frames=19 --end-usage=q" \
           " --kf-min-dist=65 --kf-max-dist=65 --use-fixed-qp-offsets=1" \
           " --deltaq-mode=0 --enable-tpl-model=0 --enable-keyframe-filtering=0" \
           " --fps=%d/%d --input-bit-depth=%d --bit-depth=%d --qp=%d -w %d -h %d" \
    ...
    if test_cfg == "RA" or test_cfg == "AS":
        ...
        #TODO: update command line for LDB
        args += ""
    else:
