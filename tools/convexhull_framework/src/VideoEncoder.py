...
def EncodeWithAOM_AV2(clip, test_cfg, QP, framenum, outfile, preset, enc_perf,
    ...
    if test_cfg == "AI" or test_cfg == "STILL":
        ...
        args += " --min-gf-interval=%d --max-gf-interval=%d --gf-min-pyr-height=4" \
                " --gf-max-pyr-height=4 --kf-min-dist=%d --kf-max-dist=%d" \
                " --lag-in-frames=19 --auto-alt-ref=1 " % \
                (SUB_GOP_SIZE, SUB_GOP_SIZE, GOP_SIZE, GOP_SIZE)
    elif test_cfg == "LD":
        args += " --kf-min-dist=9999 --kf-max-dist=9999 --lag-in-frames=0" \
                " --min-gf-interval=%d --max-gf-interval=%d --gf-min-pyr-height=4 " \
                " --gf-max-pyr-height=4 --subgop-config-str=ld " % (SUB_GOP_SIZE, SUB_GOP_SIZE)
    else:
...
def EncodeWithSVT_AV1(clip, test_cfg, QP, framenum, outfile, preset, enc_perf,
    ...
    if test_cfg == "AI" or test_cfg == "STILL":
        ...
        args += " --keyint %d --hierarchical-levels 4 --pred-struct 2 " % GOP_SIZE
    elif test_cfg == "LD":
