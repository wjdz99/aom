...
def ParseArguments(raw_args):
    parser = argparse.ArgumentParser(prog='ConvexHullBDRate.py', usage='%(prog)s [options]', description='')
    parser.add_argument('-i1', '--input1', dest='Input1', type=str, required=True, metavar='',
                        help="convex hull summary excel file for base mode")
    parser.add_argument('-i2', '--input2', dest='Input2', type=str, required=True, metavar='',
    ...
    parser.add_argument('-o', '--output', dest='Output', type=str, required=True, metavar='',
                        help="output excel file with BDRATE for base and target modes")
    if len(raw_args) == 1:
        parser.print_help()
        sys.exit(1)
...
#this is a temporary fix to handle the case in which the bit rate or quality metric is zero.
#current bdrate macro can't handle the zero value case. need to add a small value 0.001 on all
#bit rate or quality value
def UpdateForZeroValues(rd_points):
    FoundZeroValue = [False, False]
    points = []
    for point in rd_points:
        if (point[0] == 0):
            FoundZeroValue[0] = True
        if (point[1] == 0):
            FoundZeroValue[1] = True

    for idx in range(len(rd_points)):
        br = float(rd_points[idx][0])
        q = float(rd_points[idx][1])
        if (FoundZeroValue[0] == True):
            br += 0.001
        if (FoundZeroValue[1] == True):
            q += 0.001
        points.append((br,q))
    return points
...
def WriteRDData(sht, rd_data, start_row, start_col, format):
    ...
    for qty in QualityList:
        row = start_row
        rd_points = UpdateForZeroValues(rd_data.RDPoints[qty])
        for (line, point) in zip(range(len(rd_points)), rd_points):
...
def WriteRDRecord(sht, base_data, target_data, start_row, bdrate_fmt, float_fmt):
    ...
    #write base data
    base_start_col = 3
    base_max_rows = WriteRDData(sht, base_data, start_row, base_start_col, float_fmt)

    #write target data
    target_start_col = base_start_col + 2 * len(QualityList) + 1
    target_max_rows = WriteRDData(sht, target_data, start_row, target_start_col, float_fmt)
    ...
    for (qty, col) in zip(QualityList, range(len(QualityList))):
        ...
        refbr_e = xlrd.cellnameabs(start_row + total_rows - 1, base_start_col + col * 2)
        refq_b = xlrd.cellnameabs(start_row, base_start_col + col * 2 + 1)
        refq_e = xlrd.cellnameabs(start_row + total_rows - 1, base_start_col + col * 2 + 1)

        testbr_b = xlrd.cellnameabs(start_row, target_start_col + col * 2)
        testbr_e = xlrd.cellnameabs(start_row + total_rows - 1, target_start_col + col * 2)
        testq_b = xlrd.cellnameabs(start_row, target_start_col + col * 2 + 1)
        testq_e = xlrd.cellnameabs(start_row + total_rows - 1, target_start_col + col * 2 + 1)
...
if __name__ == "__main__":
    #sys.argv = ["","-i1","ConvexHullRD_ScaleAlgosNum_5_ffmpeg_hevc_medium.xlsx","-i2","ConvexHullRD_ScaleAlgosNum_5_ffmpeg_hevc_veryslow.xlsx","-o","ConvexHullBDRate.xlsm"]
    ...
    for sht_name in base_shts:
        if sht_name in target_shts:
            ...
            for base_data in base_rd_data[sht_name]:
                ...
                if (target_data != ''):
                    total_rows = WriteRDRecord(sht, base_data, target_data, start_row, bdrate_fmt, float_fmt)
