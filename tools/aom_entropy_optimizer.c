...
static INLINE unsigned int optimize_tree_probs(unsigned int idx,
                                               const aom_tree_index *tree,
                                               const unsigned int *counts,
                                               aom_prob *probs) {
  const int l = tree[idx];
  const unsigned int left_count =
      (l <= 0) ? counts[-l] : optimize_tree_probs(l, tree, counts, probs);
  const int r = tree[idx + 1];
  const unsigned int right_count =
      (r <= 0) ? counts[-r] : optimize_tree_probs(r, tree, counts, probs);
...
static int parse_stats(aom_count_type **ct_ptr, FILE *probsfile, int tabs,
  ...
  if (dim_of_cts == 1) {
    ...
    (*ct_ptr) += total_modes;
    if (tree != NULL) {
      optimize_tree_probs(0, tree, counts1d, probs);
    } else {
...
// This function parses the counts for a syntax, either binary or multi-symbol,
...
//                     dimension, i.e.,
//                     {p1, p2, ...}.
//   prefix: decalaration header for the entropy table
static void optimize_entropy_table(aom_count_type *counts, FILE *probsfile,
                                   int dim_of_cts, int *cts_each_dim,
                                   const aom_tree_index *tree,
