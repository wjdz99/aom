...
// parses a binary file consisting of total counts written in accordance with
// the format of FRAME_COUNTS in entropymode.h, and computes the optimized
// probability table, which will be written to a new c file optimized_probs.c
// according to format of entropymode.c file.
//
// Command line: ./aom_entropy_optimizer [directory of the count file]
//
// The input file can either be generated by encoding a single clip if
// entropy_stats experiment is enabled, or collected at a larger scale at which
// a python script which will be provided soon can be used to aggregate multiple
// stats output.
...
static FILE *testfile;  // TODO(yuec): make it a command line option
...
static int parse_stats(aom_count_type **ct_ptr, FILE *probsfile, int tabs,
...
static void entropy_optimizer(aom_count_type *counts, FILE *probsfile,
                              int dim_of_cts, int *cts_each_dim,
                              const aom_tree_index *tree, int flatten_last_dim,
                              char *prefix) {
  ...
  fprintf(probsfile, "%s = {\n", prefix);
  parse_stats(&ct_ptr, probsfile, 1, dim_of_cts, cts_each_dim, tree,
              flatten_last_dim);
  fprintf(probsfile, "};\n\n");
  fprintf(testfile, "\n");
}
...
int main(int argc, const char **argv_) {
  ...
  FILE *statsfile = fopen(argv_[1], "rb");
  if (!statsfile) {
  ...
  cts_each_dim[0] = INTRA_MODES;
  cts_each_dim[1] = INTRA_MODES;
  entropy_optimizer(&(fc.uv_mode[0][0]), probsfile, 2, cts_each_dim,
  ...
  entropy_optimizer(&(fc.partition[0][0]), probsfile, 2, cts_each_dim,
  ...
  entropy_optimizer(&(fc.switchable_interp[0][0]), probsfile, 2, cts_each_dim,
  ...
  entropy_optimizer(&(fc.inter_compound_mode[0][0]), probsfile, 2, cts_each_dim,
  ...
  entropy_optimizer(&(fc.compound_interinter[0][0]), probsfile, 2, cts_each_dim,
  ...
  cts_each_dim[0] = BLOCK_SIZES;
  cts_each_dim[1] = 2;
  entropy_optimizer(&(fc.obmc[0][0]), probsfile, 2, cts_each_dim, NULL, 1,
  ...
  cts_each_dim[1] = 2;
  entropy_optimizer(&(fc.intra_inter[0][0]), probsfile, 2, cts_each_dim, NULL,
                    1,
  ...
  cts_each_dim[1] = 2;
  entropy_optimizer(&(fc.comp_inter[0][0]), probsfile, 2, cts_each_dim, NULL, 1,
  ...
  cts_each_dim[2] = 2;
  entropy_optimizer(&(fc.single_ref[0][0][0]), probsfile, 3, cts_each_dim, NULL,
                    1,
  ...
  cts_each_dim[2] = 2;
  entropy_optimizer(&(fc.comp_bwdref[0][0][0]), probsfile, 3, cts_each_dim,
                    NULL, 1,
                    "static const aom_prob "
  ...
  cts_each_dim[2] = 2;
  entropy_optimizer(&(fc.comp_ref[0][0][0]), probsfile, 3, cts_each_dim, NULL,
                    1,
  ...
  cts_each_dim[1] = 2;
  entropy_optimizer(&(fc.comp_inter_mode[0][0]), probsfile, 2, cts_each_dim,
                    NULL, 1,
                    "static const aom_prob "
...
// TODO(yuec): av1_tx_size_tree has variable sizes, so needs special handling
  ...
  cts_each_dim[0] = 2;
  entropy_optimizer(&(fc.quarter_tx_size[0]), probsfile, 1, cts_each_dim, NULL,
                    1, "static const aom_prob default_quarter_tx_size_prob");
#endif
#if CONFIG_VAR_TX
  cts_each_dim[0] = TXFM_PARTITION_CONTEXTS;
  cts_each_dim[1] = 2;
  entropy_optimizer(&(fc.txfm_partition[0][0]), probsfile, 2, cts_each_dim,
                    NULL, 1,
                    "static const aom_prob "
  ...
  cts_each_dim[1] = 2;
  entropy_optimizer(&(fc.skip[0][0]), probsfile, 2, cts_each_dim, NULL, 1,
                    "static const aom_prob default_skip_probs[SKIP_CONTEXTS]");

#if CONFIG_INTRABC
  cts_each_dim[0] = 2;
  entropy_optimizer(&(fc.intrabc[0]), probsfile, 1, cts_each_dim, NULL, 1,
...
// TODO(yuec): different trees are used depending on selected ext tx set
  ...
  entropy_optimizer(&(fc.inter_ext_tx[0][0]), probsfile, 2, cts_each_dim,
                    av1_ext_tx_tree, 0,
  ...
  cts_each_dim[1] = 2;
  entropy_optimizer(&(fc.filter_intra[0][0]), probsfile, 2, cts_each_dim, NULL,
                    1, "static const aom_prob default_filter_intra_probs[2]");
#endif
