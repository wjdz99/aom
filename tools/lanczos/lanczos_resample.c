...
static double window(double y, WINDOW_TYPE win) {
  switch (win) {
    case WIN_LANCZOS: {
      return sinc(y);
    }
    case WIN_LANCZOS_MOD: {
      return sinc(y * 0.95);
    }
    case WIN_GAUSSIAN: {
      const double sigma = 0.5;
      const double sigma2 = sigma * sigma;
      return exp(-0.5 * y * y / sigma2);
    }
    case WIN_HAMMING: {
      const double a0 = 25.0 / 46.0;
      return (a0 + (1.0 - a0) * cos(M_PI * y));
    }
    case WIN_BLACKMAN: {
      const double a0 = 0.42659;
      const double a1 = 0.49656;
      const double a2 = 0.076849;
      return a0 - a1 * cos(M_PI * (y + 1)) + a2 * cos(2 * M_PI * (y + 1));
    }
...
static double kernel(double x, int a, WINDOW_TYPE win_type) {
...
static void get_lanczos_downsampler(double x, int p, int q, int a, int bits,
                                    WINDOW_TYPE win_type, int16_t *ifilter) {
...
int get_resample_filter(int p, int q, int a, double x0, EXT_TYPE ext_type,
                        WINDOW_TYPE win_type, int subsampled, int bits,
