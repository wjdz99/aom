...
double get_inverse_x0(int p, int q, double x0) {
  if (x0 <= -1.0) x0 = get_centered_x0(p, q);
  return -x0 * p / q;
}
...
void get_resample_filter(int p, int q, int a, double x0, int bits,
  ...
  assert(p < MAX_RATIONAL_FACTOR);
  assert(q < MAX_RATIONAL_FACTOR);
  const int g = gcd(p, q);
  rf->p = p / g;
  rf->q = q / g;
  if (x0 <= -1.0) x0 = get_centered_x0(rf->p, rf->q);
  for (int i = 0; i < rf->p; ++i) {
