...
static void join_words(char *dest, char **words, int nwords) {
  for (int i = 0; i < nwords; ++i) {
    strcat(dest, words[i]);
    if (i < nwords - 1) {
      const int l = (int)strlen(dest);
      dest[l] = ' ';
      dest[l + 1] = 0;
    }
  }
...
static void get_resampled_hdr(char *dest, char **words, int nwords, int width,
                              int height) {
  sprintf(dest, "YUV4MPEG2 W%d H%d ", width, height);
  join_words(dest, words + 3, nwords - 3);
}
...
int main(int argc, char *argv[]) {
  ...
  if (horz_p <= horz_q)
    rywidth = get_resampled_output_length(ywidth, horz_p, horz_q, 1);
  if (vert_p <= vert_q)
    ryheight = get_resampled_output_length(yheight, vert_p, vert_q, 1);

  printf("OutputSize: %dx%d\n", rywidth, ryheight);

  char rhdr[256];
  get_resampled_hdr(rhdr, hdrwords, nhdrwords, rywidth, ryheight);
  ...
  for (int n = 0; n < num_frames; ++n) {
    ...
    s = src;
    r = res;
    resample_2d(s, ywidth, yheight, ywidth, &horz_rf, &vert_rf, bits - 2,
                bits + 2, &clip, r, rywidth, ryheight, rywidth);
    s += ysize;
    r += rysize;
    resample_2d(s, uvwidth, uvheight, uvwidth, &horz_rf, &vert_rf, bits - 2,
                bits + 2, &clip, r, ruvwidth, ruvheight, ruvwidth);
    s += uvsize;
    r += ruvsize;
    resample_2d(s, uvwidth, uvheight, uvwidth, &horz_rf, &vert_rf, bits - 2,
                bits + 2, &clip, r, ruvwidth, ruvheight, ruvwidth);
