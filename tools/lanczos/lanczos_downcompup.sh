...
set - e
          ...
          tmpdir = "/tmp" AOMENC = "${tmpdir}/aomenc_$$" AOMDEC =
    "${tmpdir}/aomdec_$$" RESAMPLE =
        "${tmpdir}/lanczos_resample_y4m_$$" cp./ aomenc $AOMENC cp./
        aomdec $AOMDEC cp./
        lanczos_resample_y4m $RESAMPLE

            trap 'echo "Exiting..."; rm -f ${AOMENC} ${AOMDEC} ${RESAMPLE}' EXIT

                extra = ""

    input_y4m = $1 nframes = $2 hdconfig = $3 vdconfig = $4 codecparams =
        $5 downcompup_y4m = $6 intfiles = $7

#Get codec params cq_level and cpu_used
            OIFS = "$IFS";
IFS = ':' codecparams_arr = ($codecparams);
IFS = "$OIFS" cq_level = ${ codecparams_arr[0] } cpu_used = $ {
  codecparams_arr[1]
}
if
  [[-z ${ cpu_used }]];
then cpu_used = "0" fi echo "cq_level: ${cq_level}" echo "cpu_used: ${cpu_used}"
#Get width and height
    hdr = $(head - 1 $input_y4m) twidth =
        $(awk - F ' ' '{ print $2 }' < < < "${hdr}") theight =
            $(awk - F ' ' '{ print $3 }' < < < "${hdr}") width =
${ twidth: 1 } height = $ {
theight:
  1
}
...
OIFS = "$IFS";
IFS = ':' intfiles_arr = ($intfiles);
IFS = "$OIFS" down_y4m = ${ intfiles_arr[0] } downcomp_bit =
    ${ intfiles_arr[1] } downcomp_y4m = $ {
  intfiles_arr[2]
}
if
  [[-z ${ down_y4m }]];
then down_y4m = ${ tmpdir } / down_$$.y4m fi if[[-z ${ downcomp_bit }]];
then downcomp_bit = ${ tmpdir } / downcomp_$$.bit fi if[[-z ${ downcomp_y4m }]]; then
  downcomp_y4m=${tmpdir}/downcomp_$$.y4m
fi
#Obtain the horizontal and vertical upsampling configs
hdconfig_arr=(${
  hdconfig  //:/ })
      huconfig = "${hdconfig_arr[1]}:${hdconfig_arr[0]}:${hdconfig_arr[2]}" if[
          [-n ${ hdconfig_arr[3] }]];
  then huconfig =
      "${huconfig}:i${hdconfig_arr[3]}" fi if[[-n ${ hdconfig_arr[4] }]]; then
  huconfig="${huconfig}:${hdconfig_arr[4]}"
fi
vdconfig_arr=(${
    vdconfig  //:/ })
        vuconfig =
            "${vdconfig_arr[1]}:${vdconfig_arr[0]}:${vdconfig_arr[2]}" if[
                [-n ${ vdconfig_arr[3] }]];
    then vuconfig =
        "${vuconfig}:i${vdconfig_arr[3]}" fi if[[-n ${ vdconfig_arr[4] }]];
    then vuconfig =
        "${vuconfig}:${vdconfig_arr[4]}" fi
...
#Compress
                $AOMENC -
        o $downcomp_bit $down_y4m-- codec = av1-- good-- threads = 0 --passes =
            1 --lag - in - frames = 19 --kf - max - dist =
                                        65 --kf - min - dist =
                                            0 --test - decode =
                                                warn - v-- psnr-- end - usage =
                                                    q-- cq - level = $ {
      cq_level
    }
    --cpu - used = $ { cpu_used }
    --limit = ${ nframes } ${ extra } $AOMDEC-- progress - S-- codec =
                  av1 - o $downcomp_y4m $downcomp_bit

#Upsample
                            $RESAMPLE $downcomp_y4m $nframes $huconfig $vuconfig
                                $downcompup_y4m ${ width } x$ {
      height
    }
...
#tiny_ssim_highbd $input_y4m $downcompup_y4m
    ...
    if
      [[-z ${ intfiles_arr[0] }]];
    then rm $down_y4m fi if[[-z ${ intfiles_arr[1] }]];
    then rm $downcomp_bit fi if[[-z ${ intfiles_arr[2] }]];
  then rm $downcomp_y4m fi
