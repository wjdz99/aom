  ...
  if (params->r[0] > 0 && params->r[1] > 0) {
    for (i = 0; i < height; ++i) {
      for (j = 0; j < width; ++j) {
        ...
        /*printf("%d=%d*(%d-%d)+%d*(%d-%d)\n", v, xq[0], flt0[j], u, xq[1],
               flt1[j], u);*/
        const int32_t e =
            ROUND_POWER_OF_TWO(v, SGRPROJ_RST_BITS + SGRPROJ_PRJ_BITS) - src[j];
        err += ((int64_t)e * e);
        /*printf("v:%d=%d*(%d-%d)+%d*(%d-%d), e:%d err:%d \n", v, xq[0],
           flt0[j], u, xq[1], flt1[j], u,e,err);*/
      }
      dat += dat_stride;
      src += src_stride;
      flt0 += flt0_stride;
...
static void search_sgrproj(const RestorationTileLimits *limits,
  ...
  int cur_bits_sgr = count_sgrproj_bits(&rusi->sgrproj, &rsc->sgrproj);
  const int64_t bits_sgr =
      x->sgrproj_restore_cost[1] + (cur_bits_sgr << AV1_PROB_COST_SHIFT);

  //printf("rest_unit_idx:%d,cur_bits_sgr:%d,xd0:%d,xd1%d\n", rest_unit_idx,
  //       cur_bits_sgr, rusi->sgrproj.xqd[0], rusi->sgrproj.xqd[1]);
