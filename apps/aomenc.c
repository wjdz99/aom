...
static void parse_global_config(struct AvxEncoderConfig *global, char ***argv) {
  ...
  if (global->pass) {
    ...
    if (global->pass > global->passes) {
      aom_warn("Assuming --pass=%d implies --passes=%d\n", global->pass,
               global->pass);
  ...
  if (global->usage == AOM_USAGE_REALTIME && global->passes > 1) {
    aom_warn("Enforcing one-pass encoding in realtime mode\n");
    if (global->pass > 1)
  ...
  if (global->usage == AOM_USAGE_ALL_INTRA && global->passes > 1) {
    aom_warn("Enforcing one-pass encoding in all intra mode\n");
    global->passes = 1;
  }
}
...
static void set_config_arg_ctrls(struct stream_config *config, int key,
  ...
  if (key == AOME_SET_ENABLEAUTOALTREF && config->arg_ctrls[j][1] > 1) {
    aom_warn("auto-alt-ref > 1 is deprecated... setting auto-alt-ref=1\n");
...
static int parse_stream_params(struct AvxEncoderConfig *global,
  ...
  for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step) {
    ...
    if (arg_match(&arg, &g_av1_codec_arg_defs.outputfile, argi)) {
      ...
      if (!config->use_16bit_internal) {
        aom_warn("%s option ignored with CONFIG_AV1_HIGHBITDEPTH=0.\n",
                 arg.name);
      }
      ...
      if (global->passes < 2)
        aom_warn("option %s ignored in one-pass mode.\n", arg.name);
      ...
      if (arg_parse_uint(&arg) == 1) {
        aom_warn("non-zero %s option ignored in realtime mode.\n", arg.name);
      }
  ...
  if (global->usage == AOM_USAGE_REALTIME && config->cfg.g_lag_in_frames != 0) {
    aom_warn("non-zero lag-in-frames option ignored in realtime mode.\n");
    config->cfg.g_lag_in_frames = 0;
  }

  if (global->usage == AOM_USAGE_ALL_INTRA) {
    if (config->cfg.g_lag_in_frames != 0) {
      aom_warn("non-zero lag-in-frames option ignored in all intra mode.\n");
      config->cfg.g_lag_in_frames = 0;
    }
  ...
  for (pass = global.pass ? global.pass - 1 : 0; pass < global.passes; pass++) {
    ...
    FOREACH_STREAM(stream, streams) {
      if (stream->config.write_webm) {
        stream->config.write_webm = 0;
        stream->config.write_ivf = 0;
        aom_warn("aomenc compiled w/o WebM support. Writing OBU stream.");
      }
    }
#endif
