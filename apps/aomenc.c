...
static void write_recon_file(struct stream_state *stream,
                             enum TestDecodeFatality fatal, FILE *file) {
  aom_image_t enc_img;
...
int main(int argc, const char **argv_) {
  ...
  FOREACH_STREAM(stream, streams) {
    if (stream->config.recon_fn != NULL) {
      reconfile = fopen(stream->config.recon_fn, "wb");
      if (!reconfile) fatal("Failed to open recon file");
    }
  }
  ...
  for (pass = global.pass ? global.pass - 1 : 0; pass < global.passes; pass++) {
    ...
    while (frame_avail || got_data) {
      ...
      if (got_data && reconfile != NULL) {
        FOREACH_STREAM(stream, streams) {
          write_recon_file(stream, global.test_decode, reconfile);
        }
      }
  ...
  if (reconfile != NULL) fclose(reconfile);
