...
static AOM_TOOLS_FORMAT_PRINTF(3, 4) void warn_or_exit_on_error(
  ...
  va_start(ap, s);
  warn_or_exit_on_errorv(ctx, fatal, s, false, ap);
  va_end(ap);
}
...
static void initialize_encoder(struct stream_state *stream,
  ...
  if (global->test_decode != TEST_DECODE_OFF) {
    ...
    if (strcmp(get_short_name_by_aom_encoder(global->codec), "av1") == 0) {
      ...
      ctx_exit_on_error(&stream->decoder, false,
      ...
      ctx_exit_on_error(&stream->decoder, false, "Failed to set is_annexb");
...
static void encode_frame(struct stream_state *stream,
  ...
  if (img) {
    if ((img->fmt & AOM_IMG_FMT_HIGHBITDEPTH) &&
      ...
      ctx_exit_on_error(&stream->encoder, false,
...
static void update_quantizer_histogram(struct stream_state *stream) {
  if (stream->config.cfg.g_pass != AOM_RC_FIRST_PASS) {
    ...
    AOM_CODEC_CONTROL_TYPECHECKED(&stream->encoder, AOME_GET_LAST_QUANTIZER_64,
                                  &q);
    ctx_exit_on_error(&stream->encoder, false, "Failed to read quantizer");
    stream->counts[q]++;
  }
}
