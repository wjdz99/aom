...
static const arg_def_t in_subsampling_x =
    ARG_DEF(NULL, "input-subsampling-x", 1, "chroma subsampling x val");
static const arg_def_t in_subsampling_y =
    ARG_DEF(NULL, "input-subsampling-y", 1, "chroma subsampling y val");
...
static int parse_stream_params(struct AvxEncoderConfig *global,
  ...
  for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step) {
    ...
    } else if (arg_match(&arg, &in_subsampling_x, argi)) {
      stream->chroma_subsampling_x = arg_parse_uint(&arg);
    } else if (arg_match(&arg, &in_subsampling_y, argi)) {
