...
// should be skipped.
static int skip_input_output_arg(const char *arg, const char *input_fname) {
...
static char *extract_encoder_settings(const char **argv, int argc,
  ...
  while (i < argc) {
    ...
    i += num_skip;
    if (num_skip == 0) {
      total_size += strlen(argv[i]) + 1;  // 1 is for the space character.
      ++i;
    }
  }
  if (total_size == 0) {  // No encoder settings.
    return NULL;
  }
  // The "++" is for simplicity -- we always allocate a trailing space, and
  // we also need space for the final null character.
  ++total_size;
  char *result = malloc(total_size);
  if (result == NULL) {
    fprintf(stderr,
            "Warning: out of memory, cannot extract encoder settings\n");
    return NULL;
  }
  char *cur = result;
  i = 1;
  ...
  // Replace last, extra space with the null character.
  --cur;
  *cur = '\0';
  return result;
}
