...
#elif __clang__
#define ENV_COMPILER  "clang %d.%d.%d", __clang_major__,
__clang_minor__, __clang_patchlevel__
#elif __GNUC__
#define ENV_COMPILER  "GCC %d.%d.%d", __GNUC__, __GNUC_MINOR__,
__GNUC_PATCHLEVEL__
#else
...
int main(int argc, const char **argv_) {
  ...
  for (pass = global.pass ? global.pass - 1 : 0; pass < global.passes; pass++) {
    ...
    if (!global.quiet && (pass + 1) == global.passes) {
        FOREACH_STREAM(stream, streams) {
            const double bpf = seen_frames ?
              ((double)stream->nbytes * 8.0 / (double)seen_frames) : 0.0;
            const double kbps = (bpf * (double)global.framerate.num /
                                (double)global.framerate.den) / 1000.0;
            ...
            fprintf(stdout, "\n         Bitrate(kbps)  |  PSNR(Y)  |  PSNR(U)"
                    "|  PSNR(V)  |  PSNR(A)  |  PSNR(O)  |  Encoding time (FPS)\n");
            fprintf(stdout,
                    "---------------------------------------------------"
                    "---------------------------------------------------------\n");
