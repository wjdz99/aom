...
static int file_is_raw(struct AvxInputContext *input) {
  ...
  if (buffer_and_input(input, 32, buf, 1) == 32) {
    int i;
...
#if CONFIG_WEBM_IO
  if (using_file && file_is_webm(input.webm_ctx, input.aom_input_ctx)) {
...
#if CONFIG_WEBM_IO
    if (!using_file) {
      fprintf(stderr, "aomdec does not support WebM container with piping.\n");
    }
#else
