...
static int raw_read_frame(FILE *infile, uint8_t **buffer, size_t *bytes_read,
  ...
  if (fread(raw_hdr, RAW_FRAME_HDR_SZ, 1, infile) != 1) {
    if (!feof(infile)) aom_warn("Failed to read RAW frame size\n");
  } else {
    ...
    if (frame_size > kCorruptFrameThreshold) {
      aom_warn("Read invalid frame size (%u)\n", (unsigned int)frame_size);
      frame_size = 0;
    }

    if (frame_size < kFrameTooSmallThreshold) {
      aom_warn("Warning: Read invalid frame size (%u) - not a raw file?\n",
               (unsigned int)frame_size);
    }
    ...
    if (frame_size > *buffer_size) {
      ...
      if (new_buf) {
        ...
        *buffer_size = 2 * frame_size;
        ...
        aom_warn("Failed to allocate compressed data buffer\n");
        frame_size = 0;
      }
    }
  }
  ...
  if (!feof(infile)) {
    if (fread(*buffer, 1, frame_size, infile) != frame_size) {
      aom_warn("Failed to read full frame\n");
      return 1;
    }
...
static int main_loop(int argc, const char **argv_) {
  ...
  if (interface && fourcc_interface && interface != fourcc_interface)
    aom_warn("Header indicates codec: %s\n",
  ...
  while (frame_avail || got_data) {
    ...
    if (!stop_after || frame_in < stop_after) {
      if (!read_frame(&input, &buf, &bytes_in_buffer, &buffer_size)) {
        ...
        if (aom_codec_decode(&decoder, buf, bytes_in_buffer, NULL)) {
          ...
          aom_warn("Failed to decode frame %d: %s", frame_in,
                   aom_codec_error(&decoder));

          if (detail) aom_warn("Additional information: %s", detail);
        ...
        if (framestats_file) {
          ...
          if (AOM_CODEC_CONTROL_TYPECHECKED(&decoder, AOMD_GET_LAST_QUANTIZER,
                                            &qp)) {
            aom_warn("Failed AOMD_GET_LAST_QUANTIZER: %s",
                     aom_codec_error(&decoder));
            if (!keep_going) goto fail;
          }
    ...
    if (flush_decoder) {
      ...
      if (aom_codec_decode(&decoder, NULL, 0, NULL)) {
        aom_warn("Failed to flush decoder: %s", aom_codec_error(&decoder));
      }
    }
    ...
    while ((img = aom_codec_get_frame(&decoder, &iter))) {
      ...
      if (AOM_CODEC_CONTROL_TYPECHECKED(&decoder, AOMD_GET_FRAME_CORRUPTED,
                                        &corrupted)) {
        aom_warn("Failed AOM_GET_FRAME_CORRUPTED: %s",
                 aom_codec_error(&decoder));
        if (!keep_going) goto fail;
      }
