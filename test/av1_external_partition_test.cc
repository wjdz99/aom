...
aom_ext_part_status_t ext_part_send_features(
  ...
  if (part_features->id == FEATURE_BEFORE_PART_NONE) {
    for (int i = 0; i < 17; ++i) {
      /*
      EXPECT_GT(part_features->before_part_none.f[i], 0.0)
          << "i = " << i << ", feature value: " <<
      part_features->before_part_none.f[i];
      */
    }
  } else if (part_features->id == FEATURE_BEFORE_PART_NONE_PART2) {
  } else if (part_features->id == FEATURE_AFTER_PART_NONE) {
    /*
    printf("FEATURE_AFTER_PART_NONE\n");
    printf("rate %.4f, dist %.4f, q %.1f, var %.4f --\n",
           part_features->after_part_none.rate,
           part_features->after_part_none.dist,
           part_features->after_part_none.q,
           part_features->after_part_none.source_variance);
    */
  }
  return AOM_EXT_PART_OK;
}
