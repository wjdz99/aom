...
vector<CompMaskPredParam> GetCompMaskPredParams(comp_mask_pred_func testImpl) {
  vector<CompMaskPredParam> params;
  int cnt = sizeof(valid_bsize) / sizeof(valid_bsize[0]);
  for (int i = 0; i < cnt; ++i) {
    params.push_back(CompMaskPredParam(testImpl, valid_bsize[i]));
  }
  return params;
}
...
TEST_P(AV1CompMaskVarianceTest, CheckOutput) {
  // inv = 0, 1
  RunCheckOutput((comp_mask_pred_func)GET_PARAM(0), GET_PARAM(1), 0);
  RunCheckOutput((comp_mask_pred_func)GET_PARAM(0), GET_PARAM(1), 1);
}
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::ValuesIn(GetCompMaskPredParams(&aom_comp_mask_pred_ssse3)));
#endif
...
TEST_P(AV1CompMaskUpVarianceTest, DISABLED_Speed) {
  RunSpeedTest((comp_mask_pred_func)GET_PARAM(0), GET_PARAM(1), 1);
}
