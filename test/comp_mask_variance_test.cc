    ...
    for (int sub = 0; sub < 8 * 8; ++sub) {
      ...
      for (int wedge_index = 0; wedge_index < wedge_types; ++wedge_index) {
        ...
        aom_highbd_comp_mask_upsampled_pred(
            NULL, NULL, 0, 0, NULL, CONVERT_TO_BYTEPTR(comp_pred1_), CONVERT_TO_BYTEPTR(pred_), w,
            h, subx, suby, CONVERT_TO_BYTEPTR(ref_), MAX_SB_SIZE, mask, w, inv,
            bd_, subpel_search);

        aom_highbd_comp_mask_pred = test_impl;  // test
        aom_highbd_comp_mask_upsampled_pred(
            NULL, NULL, 0, 0, NULL, CONVERT_TO_BYTEPTR(comp_pred2_), CONVERT_TO_BYTEPTR(pred_), w,
            h, subx, suby, CONVERT_TO_BYTEPTR(ref_), MAX_SB_SIZE, mask, w, inv,
            bd_, subpel_search);
        ASSERT_EQ(CheckResult(w, h), true)
            << " wedge " << wedge_index << " inv " << inv << "sub (" << subx
...
void AV1HighbdCompMaskUpVarianceTest::RunSpeedTest(
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (int j = 0; j < num_loops; ++j) {
      aom_highbd_comp_mask_upsampled_pred(
          NULL, NULL, 0, 0, NULL, CONVERT_TO_BYTEPTR(comp_pred1_), CONVERT_TO_BYTEPTR(pred_), w, h,
