...
void AV1CompMaskVarianceTest::RunCheckDiffMask(comp_mask_pred_func test_impl,
  ...
  const int w = block_size_wide[bsize];
  const int h = block_size_high[bsize];
  uint8_t mask[4 * 128 * 128];
  for (int index = 0; index < (int) sizeof(kValidBlockSize); ++index) {
    ...
    ASSERT_EQ(CheckResult(w, h), true) << " Diffwtd " << index << " inv " << inv;
  }
}
...
void AV1CompMaskUpVarianceTest::RunCheckDiffMask(comp_mask_pred_func test_impl,
  ...
  for (subpel_search = USE_4_TAPS; subpel_search <= USE_8_TAPS;
    ...
    for (int sub = 0; sub < 8 * 8; ++sub) {
      int subx = sub & 0x7;
      int suby = (sub >> 3);
      for (int index = 0; index < (int) sizeof(kValidBlockSize);
           ++index) {
        av1_diffwtd_mask(mask, inv, 38, pred_, w, ref_, MAX_SB_SIZE, h, w);
        ...
        aom_comp_mask_upsampled_pred(NULL, NULL, 0, 0, NULL, comp_pred2_, pred_,
                                     ...
                                     w, inv, subpel_search);
        ASSERT_EQ(CheckResult(w, h), true)
            << " Diffwtd " << index << " inv " << inv << "sub (" << subx << ","
            << suby << ")";
      }
    }
  }
...
void AV1HighbdCompMaskVarianceTest::RunCheckDiffMask(
  ...
  uint8_t mask[4 * 128 * 128];
  for (int index = 0; index < (int) sizeof(kValidBlockSize); ++index) {
    av1_diffwtd_mask_highbd(mask, inv, 38, pred_, w, ref_, MAX_SB_SIZE, h, w, bd);
