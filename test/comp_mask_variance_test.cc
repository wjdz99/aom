...
void AV1CompMaskVarianceTest::RunCheckDiffMask(comp_mask_pred_func test_impl,
  ...
  const int h = block_size_high[bsize];
  uint8_t *mask;
  mask = (uint8_t *)malloc(16 * 128 * 128);
...
void AV1CompMaskUpVarianceTest::RunCheckDiffMask(comp_mask_pred_func test_impl,
  ...
  for (subpel_search = USE_4_TAPS; subpel_search <= USE_8_TAPS;
    ...
    for (int sub = 0; sub < 8 * 8; ++sub) {
      ...
      // ref
      aom_comp_mask_upsampled_pred_c(NULL, NULL, 0, 0, NULL, comp_pred1_, pred_,
                                     w, h, subx, suby, ref_, MAX_SB_SIZE, mask,
                                     w, inv, subpel_search);
...
void AV1HighbdCompMaskUpVarianceTest::RunCheckDiffMask(
    highbd_comp_mask_pred_func test_impl, BLOCK_SIZE bsize, int inv,
    const unsigned int bd) {
    (void)test_impl;
    int bd_ = GET_PARAM(2);
