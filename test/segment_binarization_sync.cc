...
Segment GenerateSegment(int seed) {
  ...
  const int pred = rnd_.PseudoUniform(MAX_SEGMENTS);
  const int segment_id = rnd_.PseudoUniform(last_segid + 1);

  return { segment_id, pred, last_segid };
}
...
TEST(SegmentBinarizationSync, SearchForBinarizationMismatch) {
  ...
  for (int i = 0; i < count_tests; ++i) {
    ...
    const int seg_diff = neg_interleave(seg.id, seg.pred, max_segid);
    const int decoded_segid = neg_deinterleave(seg_diff, seg.pred, max_segid);
