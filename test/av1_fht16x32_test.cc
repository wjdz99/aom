...
typedef tuple<FhtFunc, IhtFunc, int, AomBitDepthT, int> Ht16x32Param;

void fht16x32_ref(const int16_t *in, TranLowT *out, int stride, int tx_type) {
  av1_fht16x32_c(in, out, stride, tx_type);
}

void iht16x32_ref(const TranLowT *in, uint8_t *out, int stride, int tx_type) {
