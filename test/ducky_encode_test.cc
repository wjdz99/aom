...
class DuckyEncodeRCTest : public ::testing::TestWithParam<VideoInfo> {
 public:
  DuckyEncodeRCTest() {
    video_info_ = GetParam();
    video_info_.file_path =
        libaom_test::GetDataPath() + "/" + video_info_.file_path;
  }

  ~DuckyEncodeRCTest() {}

 protected:
  void RunDuckyEncodeWithRC(RateControlParam rc_param) {
    rc_param_ = rc_param;
    rc_param_.frame_width = video_info_.frame_width;
    rc_param_.frame_height = video_info_.frame_height;
    rc_param_.bit_depth = video_info_.bit_depth;
    DuckyEncode ducky_encode(video_info_, BLOCK_64X64, kMaxRefFrames, 3, 128);
    AV1RateControlQMode qmode_rc;
    const Status status = qmode_rc.SetRcParam(rc_param_);
    ...
    firstpass_info.num_mbs_16x16 =
        ((video_info_.frame_width + kBlockSize - 1) / kBlockSize) *
        ((video_info_.frame_height + kBlockSize - 1) / kBlockSize);
...
TEST_P(DuckyEncodeRCTest, EncodeVideoWithRC) {
  ...
  RunDuckyEncodeWithRC(rc_param);
}

INSTANTIATE_TEST_SUITE_P(DuckyEncodeRC, DuckyEncodeRCTest,
                         ::testing::ValuesIn(kVideoInfoList));
