...
TEST(DuckyEncodeRCTest, EncodeVideoWithRC) {
  ...
  EXPECT_TRUE(gop_info.status().ok());
  ...
  for (const auto &gop_struct : gop_list) {
    ...
    EXPECT_TRUE(gop_encode_info.status().ok());
    gop_encode_info_list.push_back(std::move(*gop_encode_info));
  }
  ...
  for (const auto &frame_result : encoded_frames_list) {
    EXPECT_GT(frame_result.psnr, 30);
  }
}
