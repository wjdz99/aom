...
// Helper macros to ensure AOM_TEST_DATA_PATH is a quoted string.
// These are undefined right below GetDataPath
// NOTE: AOM_TEST_DATA_PATH MUST NOT be a quoted string before
...
static std::string GetDataPath() {
  const char *const data_path = getenv("AOM_TEST_DATA_PATH");
  if (data_path == NULL) {
#ifdef AOM_TEST_DATA_PATH
    // In some environments, we cannot set environment variables
    // Instead, we set the data path by using a preprocessor symbol
    // which can be set from make files
    return STRINGIFY(AOM_TEST_DATA_PATH);
#else
    return ".";
#endif
  }
