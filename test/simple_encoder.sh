...
simple_encoder() {
  local encoder =
      "${LIBAOM_BIN_PATH}/simple_encoder${AOM_TEST_EXE_SUFFIX}" local codec =
          "$1" local output_file =
              "${AOM_TEST_OUTPUT_DIR}/simple_encoder_${codec}.ivf"

      if[!-x "${encoder}"];
  then elog "${encoder} does not exist or is not executable." return 1 fi
...
simple_encoder_av1() {
  if
    ["$(av1_encode_available)" = "yes"];
  then simple_encoder av1 || return 1 fi
}
