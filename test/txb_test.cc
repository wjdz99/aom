...
typedef std::tr1::tuple<GetLevelCountsFunc> TxbTestParam;

class TxbTest : public ::testing::TestWithParam<TxbTestParam> {
 public:
  ...
  uint8_t *levels_;
  DECLARE_ALIGNED(16, uint8_t, level_counts_ref_[MAX_TX_SQUARE]);
  DECLARE_ALIGNED(16, uint8_t, level_counts_[MAX_TX_SQUARE]);
};
...
#if HAVE_SSE2
const TxbTestParam kTxbTestParamArraySse2[] = { make_tuple(
    &av1_get_br_level_counts_sse2) };

INSTANTIATE_TEST_CASE_P(SSE2, TxbTest,
                        ::testing::ValuesIn(kTxbTestParamArraySse2));
#endif
}  // namespace
