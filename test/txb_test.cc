...
class TxbTest : public ::testing::TestWithParam<GetLevelCountsParam> {
 public:
  ...
  void FillData(const int width, const int height) {
    intptr_t i, j;
    for (i = 0; i < height; ++i) {
      for (j = 0; j < width; ++j) {
        levels_[i * width + j] = rnd_.Rand8() % (NUM_BASE_LEVELS + 2);
      }
    }
...
TEST_P(TxbTest, DISABLED_Speed) { SpeedTestRun(); }
