...
TEST(NoiseStrengthSolver, SimplifiesCurve) {
  ...
  for (int i = 0; i < 256; ++i) {
    const double x = (i - 128.) / 64.;
    aom_noise_strength_solver_add_measurement(&solver, i, x * x);
  }
  EXPECT_EQ(1, aom_noise_strength_solver_solve(&solver));

  // First try to fit an unconstrained lut (will be simplified to < num_bins
  // poinits)
  aom_noise_strength_lut_t lut;
  EXPECT_EQ(1, aom_noise_strength_solver_fit_piecewise(&solver, -1, &lut));
  ASSERT_LE(100, lut.num_points);
  aom_noise_strength_lut_free(&lut);

  // Now add some constraints (this will prune points)
  EXPECT_EQ(1, aom_noise_strength_solver_fit_piecewise(&solver, 11, &lut));
  ASSERT_EQ(11, lut.num_points);

  // Check that the parabola is well represented
  EXPECT_NEAR(0.0, lut.points[0][0], 1e-5);
  ...
  EXPECT_NEAR(255.0, lut.points[10][0], 1e-5);
  EXPECT_NEAR(4.0, lut.points[10][1], 0.1);
...
TEST(NoiseModelGetGrainParameters, GetGrainParametersDefault) {
...
TEST(NoiseModelGetGrainParameters, TestNoiseStrengthShiftBounds) {
  ...
  for (const auto &test_case : test_cases) {
    auto &eqns = model.combined_state[0].strength_solver.eqns;
    // Set the fitted scale parameterst to be a constant value.
...
TEST(NoiseModelGetGrainParameters, GetGrainParametersKong) {
  ...
  const int kExpectedScalingPointsY[][2] = {
    { 0, 0 },     { 13, 44 },   { 27, 62 },   { 40, 76 },   { 54, 88 },
    { 67, 98 },   { 81, 108 },  { 94, 117 },  { 107, 125 }, { 121, 132 },
    { 134, 139 }, { 148, 146 }, { 161, 153 }, { 255, 192 },
  };
  ...
  EXPECT_EQ(14, film_grain.num_y_points);
