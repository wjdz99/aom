...
extern "C" double aom_randn(double);
...
TEST(NoiseModel, InitSuccessWithValidSquareShape) {
  aom_noise_model_params_t params = {.shape = AOM_NOISE_SHAPE_SQUARE, .lag = 2};
...
TEST(NoiseModel, InitFailsWithTooSmallLag) {
  aom_noise_model_t model;
  aom_noise_model_params_t params = {.shape = AOM_NOISE_SHAPE_SQUARE, .lag = 0};
  EXPECT_FALSE(aom_noise_model_init(&model, params));
  aom_noise_model_free(&model);
}
...
TEST(FlatBlockEstimator, FindFlatBlocks) {
  ...
  for (int y = 0; y < kBlockSize; ++y) {
    for (int x = 0; x < kBlockSize; ++x) {
      ...
      data[y * stride + x + 1 * kBlockSize] = 128 + aom_randn(5);
  ...
  for (int y = 0; y < kHeight; ++y) {
    for (int x = 0; x < kWidth; ++x) {
      for (int c = 0; c < 3; ++c) {
        ...
        const int std = (y < kHeight / 2) ? kLowStd : kHighStd;
        data_ptr_[c][y * kWidth + x] =
            std::min(255, int(2 + avg + aom_randn(std)));
...
TEST_F(NoiseModelUpdateTest, UpdateSuccessForCorrelatedNoise) {
  ...
  EXPECT_LT(0.99, normalized_cross_correlation(model_.latest_state[0].eqns.x,
                                               kCoeffs, kNumCoeffs));
