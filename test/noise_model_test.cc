...
// All of these args are bundled into one struct so that we can used
...
TYPED_TEST_P(NoiseModelUpdateTest, UpdateSuccessForCorrelatedNoise) {
  ...
  for (int c = 0; c < 3; ++c) {
    ...
    for (int y = 0; y < (kHeight >> y_shift); ++y) {
      for (int x = 0; x < (kWidth >> x_shift); ++x) {
        ...
        this->data_ptr_[c][y * kWidth + x] =
            uint8_t(value + this->noise_ptr_[c][y * kWidth + x] * kStd) << shift;
        //const int by = y / (this->kBlockSize >> y_shift);
        // const int bx = x / (this->kBlockSize >> x_shift);
        if (1  /*flat_blocks_[by * num_blocks_w + bx]  */) {
          this->denoised_ptr_[c][y * this->strides_[c] + x] = value << shift;
        } else {
          this->denoised_ptr_[c][y * this->strides_[c] + x] =
              this->data_ptr_[c][y * this->strides_[c] + x];
        }
      }
    }
  }
  ...
  for (int c = 0; c < 3; ++c) {
    ...
    std::vector<double> ref_psd(kBlockSize * kBlockSize);
    aom_noise_psd_get(this->noise_ptr_[c], kWidth, kHeight, &ref_psd[0], kBlockSize);
  ...
  for (int c = 0; c < 3; ++c) {
    for (int i = 0; i < model.latest_state[c].strength_solver.eqns.n; ++i) {
      EXPECT_NEAR(kStd, model.latest_state[c].strength_solver.eqns.x[i] / normalize,
                  kStdEps);
    }
  }
}
