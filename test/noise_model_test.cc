...
TYPED_TEST_P(FlatBlockEstimatorTest, FindFlatBlocksBestEffort) {
  /*
  const int kBlockSize = 32;
  aom_flat_block_finder_t flat_block_finder;
  ASSERT_EQ(1, aom_flat_block_finder_init(&flat_block_finder, kBlockSize,
  ...
  for (int y = 0; y < kBlockSize; ++y) {
    for (int x = 0; x < kBlockSize; ++x) {
      for (int block = 0; block < 8; ++block) {
        this->data_[y * stride + x + block * kBlockSize] =
            ((uint8_t)(128 + aom_randn(2 + block))) << shift;
      }
    }
  }
  ...
  // First block is the closest to being flat, so flat[0] = 1
...
REGISTER_TYPED_TEST_CASE_P(FlatBlockEstimatorTest, ExtractBlock, FindFlatBlocks,
                           FindFlatBlocksBestEffort);
