...
TEST(NoiseStrengthSolver, SimplifiesCurve) {
  ...
  for (int max_points : { 9, 5 }) {
    EXPECT_EQ(
        1, aom_noise_strength_solver_fit_piecewise(&solver, max_points, &lut));
    ASSERT_EQ(max_points, lut.num_points);
    ...
    EXPECT_NEAR(255.0, lut.points[max_points - 1][0], 1e-5);
    EXPECT_NEAR(4.0, lut.points[max_points - 1][1], 0.1);
    aom_noise_strength_lut_free(&lut);
  }
  aom_noise_strength_solver_free(&solver);
}
