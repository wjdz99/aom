...
class WienerDenoiseTest : public ::testing::TestWithParam<WienerTestArg> {
 ...
 protected:
  void SetUp() {
    ...
    float psd_value = aom_noise_psd_get_default_value(block_size,
                                                      GetParam().noise_level);
    for (int i = 0; i < block_size * block_size; ++i) {
      noise_psd_[0][i] = psd_value;
    }
    psd_value = aom_noise_psd_get_default_value(block_size / 2,
                                                GetParam().noise_level);
