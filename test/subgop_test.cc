...
class SubGopSwitchingTest
 ...
 protected:
  ...
  void set_subgop_config(::libaom_test::Encoder *encoder) {
    ...
    // encoder
    const char *subgop_str = last_subgop_str ? NULL : test_params_.subgop_str;
    ...
    if (subgop_str) {
      // Set min and max gf interval
      encoder->Control(AV1E_SET_MIN_GF_INTERVAL, 0);
      encoder->Control(AV1E_SET_MAX_GF_INTERVAL, max_gf_interval);
    }
    last_subgop_str = subgop_str;
  }
  ...
  virtual bool HandleEncodeResult(::libaom_test::VideoSource *video,
    ...
    if (!frame_num_in_subgop_) {
      FRAME_TYPE frame_type_test;

      // Get current frame type
      encoder->Control(AV1E_GET_FRAME_TYPE, &frame_type_test);
      // Get subgop config
      encoder->Control(AV1E_GET_SUB_GOP_CONFIG, &subgop_info_);
      ...
      if (frame_type_test == KEY_FRAME) subgop_size_++;
    }
    return 1;
  }
