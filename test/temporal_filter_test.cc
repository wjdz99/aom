 ...
 public:
    ...
    for (int plane = 0; plane < num_planes; plane++) {
      ...
      for (int ii = 0; ii < plane_h; ii++) {
        for (int jj = 0; jj < plane_w; jj++) {
          src1_[plane * 32 * 32 + ii * plane_stride + jj] = rnd_.Rand8();
          src2_[plane * 32 * 32 + ii * plane_stride2 + jj] = rnd_.Rand8();
        }
      }
    }
  }
  ...
  void GenExtremeData(int width, int height, int stride, uint8_t *data,
    ...
    for (int plane = 0; plane < num_planes; plane++) {
      ...
      for (int ii = 0; ii < plane_h; ii++) {
        for (int jj = 0; jj < plane_w; jj++) {
          data[plane * 32 * 32 + ii * plane_stride + jj] = val;
          data2[plane * 32 * 32 + ii * plane_stride2 + jj] = (255 - val);
        }
      }
    }
  }
  ...
  void GenRandomData(int width, int height, int stride, int stride2, int bd,
    ...
    for (int plane = AOM_PLANE_Y; plane < num_planes; plane++) {
      ...
      if (bd == 10) {
        for (int ii = 0; ii < plane_h; ii++) {
          for (int jj = 0; jj < plane_w; jj++) {
            src1_[plane * 32 * 32 + ii * plane_stride + jj] =
                rnd_.Rand16() & 0x3FF;
            src2_[plane * 32 * 32 + ii * plane_stride2 + jj] =
                rnd_.Rand16() & 0x3FF;
          }
        }
      } else {
    ...
    for (int plane = AOM_PLANE_Y; plane < num_planes; plane++) {
      ...
      if (bd == 10) {
        for (int ii = 0; ii < plane_h; ii++) {
          for (int jj = 0; jj < plane_w; jj++) {
            data[plane * 32 * 32 + ii * plane_stride + jj] = val;
            data2[plane * 32 * 32 + ii * plane_stride2 + jj] = (1023 - val);
          }
        }
      } else {
        for (int ii = 0; ii < plane_h; ii++) {
          for (int jj = 0; jj < plane_w; jj++) {
            data[plane * 32 * 32 + ii * plane_stride + jj] = val;
            data2[plane * 32 * 32 + ii * plane_stride2 + jj] = (4095 - val);
          }
        }
      }
    }
