...
bool VerifyAv1c(const uint8_t *const obu_buffer, size_t obu_buffer_length,
  ...
  bool parse_ok = get_av1c_from_obu(obu_buffer, obu_buffer_length, is_annexb,
                                    &av1_config) == 0;
  if (parse_ok) {
...
TEST(Av1c, ObuInvalidInputs) {
  ...
  ASSERT_EQ(-1, get_av1c_from_obu(&kLobfFullSequenceHeaderObu[0], 0, 0, NULL));
  ASSERT_EQ(-1, get_av1c_from_obu(&kLobfFullSequenceHeaderObu[0],
                                  sizeof(kLobfFullSequenceHeaderObu), 0, NULL));
  ASSERT_EQ(
      -1, get_av1c_from_obu(NULL, sizeof(kLobfFullSequenceHeaderObu), 0, NULL));
  ASSERT_EQ(
      -1, get_av1c_from_obu(&kLobfFullSequenceHeaderObu[0], 0, 0, &av1_config));
}
...
TEST(Av1c, ReadWriteConfig) {
  ...
  for (size_t i = 0; i < kAv1cNoConfigObusSize; ++i) {
    ASSERT_EQ(kAv1cAllZero[i], av1c_buffer[i])
        << "Mismatch in output av1C at offset=" << i;
  }
