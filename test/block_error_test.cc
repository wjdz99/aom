...
typedef std::tr1::tuple<BlockErrorFunc, BlockErrorFunc, TX_SIZE,
                        aom_bit_depth_t> BlockErrorParam;
 ...
 public:
  ...
  void BlockErrorRun(int testNum) {
    ...
    for (i = 0; i < testNum; ++i) {
      ...
      error = ASM_REGISTER_STATE_CHECK(blk_err_(
          coeff_, dqcoeff, block_size, &sse));
  ...
  void FillRandomData() {
    ...
    for (i = 0; i < block_size; ++i) {
      coeff_[i] = GetRandomNumWithRange(INT16_MIN + margin,
                                        INT16_MAX - margin);
...
TEST_P(BlockErrorTest, BitExact) {
  BlockErrorRun(kTestNum);
}
