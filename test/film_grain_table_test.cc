...
TEST(FilmGrainTableTest, SplitSingleSegment) {
  ...
  EXPECT_NE(nullptr, table.head->next);
...
TEST_F(FilmGrainTableIOTest, ReadTruncatedFile) {
  ...
  const char *kGrainFile = "/tmp/tuncated_file.dat";
  FILE *file = fopen(kGrainFile, "wb");
  fwrite("deadbeef", 8, 1, file);
  fclose(file);
  ASSERT_EQ(AOM_CODEC_ERROR,
            aom_film_grain_table_read(&table, kGrainFile, &error_));
}
...
TEST_F(FilmGrainTableIOTest, RoundTripReadWrite) {
  ...
  const char *kGrainFile = "/tmp/round_trip_write.dat";
  ASSERT_EQ(AOM_CODEC_OK,
            aom_film_grain_table_write(&table, kGrainFile, &error_));
  aom_film_grain_table_free(&table);
...
TEST_F(FilmGrainTableIOTest, RoundTripSplit) {
  const char *kGrainFile = "/tmp/round_trip_split.dat";
