...
void PixelProjErrorTest::runPixelProjErrorTest(int32_t run_times) {
  ...
  for (int iter = 0; iter < iters && !HasFatalFailure(); ++iter) {
    ...
    for (int i = 0; i < run_times; ++i) {
      err_ref = lowbd_pixel_proj_error_c(src, h_end, v_end, src_stride, dgd,
...
void PixelProjErrorTest::runPixelProjErrorTest_ExtremeValues() {
  ...
  for (int iter = 0; iter < iters && !HasFatalFailure(); ++iter) {
    ...
    err_ref = lowbd_pixel_proj_error_c(
...
INSTANTIATE_TEST_CASE_P(SSE4_1, PixelProjErrorTest,
                        ::testing::Values(lowbd_pixel_proj_error_sse4_1));
#endif  // HAVE_SSE4_1

#if HAVE_AVX2

INSTANTIATE_TEST_CASE_P(AVX2, PixelProjErrorTest,
                        ::testing::Values(lowbd_pixel_proj_error_avx2));
#endif  // HAVE_AVX2
