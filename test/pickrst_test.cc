...
void GetProjSubspaceTest::RunGetProjSubspaceTest(int32_t run_times) {
  ...
  constexpr int kR0[3] = { 1, 1, 0 };
  constexpr int kR1[3] = { 1, 0, 1 };
  for (int iter = 0; iter < iters && !HasFatalFailure(); ++iter) {
    ...
    params.r[0] = run_times == 1 ? (rng_.Rand8() % MAX_RADIUS) : r0[iter];
    params.r[1] = run_times == 1 ? (rng_.Rand8() % MAX_RADIUS) : r1[iter];
    uint8_t *dgd = dgd_;
    uint8_t *src = src_;
