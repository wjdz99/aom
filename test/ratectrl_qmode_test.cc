...
TEST(RateControlQModeTest, TestKeyframeDetection) {
  ...
  // keyframe_location contains the number of frames between key frames.
  ...
  size_t ref_number_of_keys = 0;
  size_t keyframe_index = 1;
  while (std::getline(keyframe_file, newline)) {
    ref_number_of_keys++;
  }
  // keyframe_location file contains the number of frames between key frames, so
  // its size is 1 smaller than the keyframe_list which contans the actual frame
  // number of key frames.
  EXPECT_EQ(ref_number_of_keys + 1, keyframe_list.size());
  if (ref_number_of_keys == keyframe_list.size()) {
    keyframe_file.clear();
    keyframe_file.seekg(0);
    while (std::getline(keyframe_file, newline)) {
      std::istringstream iss(newline);
      int next_keyframe_offset;
      iss >> next_keyframe_offset;
      keyframe_number += next_keyframe_offset;
