      ...
      for (int u = 0; u * h < max_h; ++u) {
        for (int v = 0; v * w < max_w; ++v) {
          ...
          TplBlockStats block_stats =
              create_toy_tpl_block_stats(h, w, r, c, intra_cost, 0);
...
double tpl_frame_stats_accumulate_intra_cost(const TplFrameStats &frame_stats) {
  double sum = 0;
...
TEST(RateControlQModeTest, CreateTplFrameDepStats) {
  ...
  const double expected_intra_cost_sum =
      tpl_frame_stats_accumulate_intra_cost(frame_stats);
...
TEST(RateControlQModeTest, TplBlockStatsToDepStats) {
  ...
  TplBlockStats block_stats =
      create_toy_tpl_block_stats(8, 4, 0, 0, intra_cost, inter_cost);
...
TEST(RateControlQModeTest, TplFrameDepStatsPropagateSingleZeroMotion) {
  ...
  tpl_frame_dep_stats_propagate(/*coding_idx=*/1, ref_frame_table,
                                &gop_dep_stats);
...
TEST(RateControlQModeTest, TplFrameDepStatsPropagateCompoundZeroMotion) {
  ...
  const RefFrameTable ref_frame_table = create_toy_ref_frame_table(frame_count);
  tpl_frame_dep_stats_propagate(/*coding_idx=*/2, ref_frame_table,
                                &gop_dep_stats);

  // cur frame with coding_idx 1
  const double expected_ref_sum =
      tpl_frame_stats_accumulate_intra_cost(frame_stats);
...
TEST(RateControlQModeTest, ComputeTplGopDepStats) {
  ...
  for (int i = 2; i >= 0; i--) {
    ...
    expected_sum += tpl_frame_stats_accumulate_intra_cost(
        tpl_gop_stats.frame_stats_list[i]);
    const double sum =
