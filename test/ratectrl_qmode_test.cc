...
TEST(RateControlQModeTest, TestKeyframeDetection) {
  ...
  // kf_group_duration contains the number of frames between key frames.
  // It's written out in find_next_key_frame right after define_kf_interval.
  std::ifstream keyframe_file("kf_group_duration");
  std::vector<int> key_frame_locations(1, 0);  // First frame is a key frame/
  std::partial_sum(std::istream_iterator<int>(keyframe_file),
                   std::istream_iterator<int>(),
                   std::back_inserter(key_frame_locations));
  EXPECT_THAT(get_key_frame_list(firstpass_info),
              testing::ElementsAreArray(key_frame_locations));
}
