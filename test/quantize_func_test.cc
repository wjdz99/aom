 ...
 protected:
  ...
  void QuantizeRun(bool is_loop, int q = 0, int test_num = 1) {
    ...
    for (int i = 0; i < test_num; ++i) {
      ...
      QuantizeFuncRun(coeff_ptr, n_coeffs, zbin, round, quant, quant_shift,
...
class FullPrecisionQuantizeTest : public QuantizeTestBase<tran_low_t> {
  virtual void QuantizeFuncRun(
      ...
      const int16_t *round_ptr, const int16_t *quant_ptr,
      const int16_t *quant_shift_ptr, tran_low_t *qcoeff_ptr,
      tran_low_t *qcoeff_ref_ptr, tran_low_t *dqcoeff_ptr,
      tran_low_t *dqcoeff_ref_ptr, const int16_t *dequant_ptr,
      uint16_t *eob_ref_ptr, uint16_t *eob_ptr, const int16_t *scan,
      const int16_t *iscan) {
