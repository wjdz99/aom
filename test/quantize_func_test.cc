...
typedef void (*LPQuantizeFunc)(LP_QUAN_PARAM_LIST);
...
template <LPQuantizeFunc fn>
void LowPrecisionQuantFuncWrapper(QUAN_PARAM_LIST) {
  (void)zbin_ptr;
  (void)quant_shift_ptr;
  (void)iscan;
  fn((const int16_t *)coeff_ptr, n_coeffs, round_ptr, quant_ptr,
     (const int16_t *)qcoeff_ptr, (const int16_t *)dqcoeff_ptr, dequant_ptr,
     eob_ptr, scan);
}
...
class LowPrecisionQuantizeTest
  ...
  void RunQuantizeFunc(const int16_t *coeff_ptr, intptr_t n_coeffs,
    ...
    quant_ref_(coeff_ptr, n_coeffs, round_ptr, quant_ptr, qcoeff_ref_ptr,
               dqcoeff_ref_ptr, dequant_ptr, eob_ref_ptr, scan);

    ASM_REGISTER_STATE_CHECK(quant_(coeff_ptr, n_coeffs, round_ptr, quant_ptr,
...
TEST_P(LowPrecisionQuantizeTest, DISABLED_ZeroInput) {
  FillCoeffZero();
  QuantizeRun(false);
}

TEST_P(LowPrecisionQuantizeTest, DISABLED_LargeNegativeInput) {
  FillDcLargeNegative();
  QuantizeRun(false, 0, 1);
}

TEST_P(LowPrecisionQuantizeTest, DISABLED_DcOnlyInput) {
  FillDcOnly();
  QuantizeRun(false, 0, 1);
}

TEST_P(LowPrecisionQuantizeTest, DISABLED_RandomInput) {
  QuantizeRun(true, 0, kTestNum);
}

TEST_P(LowPrecisionQuantizeTest, DISABLED_MultipleQ) {
  for (int q = 0; q < QINDEX_RANGE; ++q) {
    QuantizeRun(true, q, kTestNum);
  }
}
...
// Force the coeff to be half the value of the dequant.  This exposes a
// mismatch found in av1_quantize_fp_sse2().
TEST_P(LowPrecisionQuantizeTest, DISABLED_CoeffHalfDequant) {
  FillCoeff(16);
  QuantizeRun(false, 25, 1);
}
