...
class QuantizeTest : public ::libaom_test::EncoderTest,
 ...
 protected:
  ...
  void InitEncode() {
    ...
    aom_codec_err_t res = aom_codec_enc_init(
        av1_encoder, encoder_->CodecInterface(), &cfg_, init_flags_);
    ASSERT_EQ(AOM_CODEC_OK, res);

    aom_codec_alg_priv_t *const priv =
        reinterpret_cast<aom_codec_alg_priv_t *>(av1_encoder->priv);
    cpi_ = getCPI(priv);
    // Initially set quantization index = 0
    UpdateQuantizer(0);
  }

  void ExitEncode() {
    // allocated memory for Encoder by CreateEncoder is released here
    delete encoder_;
  }
