 ...
 protected:
  ...
  void QuantizeRun(bool isLoop, int q = 0, int testNum = 1) {
    ...
    while (i < testNum) {
      ...
      ASM_REGISTER_STATE_CHECK(quant_(coeff_ptr, n_coeffs, skip_block, zbin,
                                      round_fp, quant_fp, quant_shift,
                                      qcoeff, dqcoeff, dequant, &eob[1],
                                      sc->scan, sc->iscan));
  ...
  void FillCoeffGeneric(bool isConstant, int c = 0) {
    ...
    if (isConstant) {
      ...
      FillCoeffZero();
      int num = abs(rnd_.Rand16() % n_coeffs);
      for (i = 0; i < num; ++i) {
  ...
  void FillDcOnly() {
    FillCoeffZero();
    coeff_[0] = clamp(rnd_.Rand16(), INT16_MIN + 1, INT16_MAX);
  }
...
const QuantizeParam kQParamArraySSE2[] = {
  make_tuple(av1_quantize_fp_c, av1_quantize_fp_sse2, TX_16X16, AOM_BITS_8)
};
