 ...
 protected:
  ...
  void QuantizeRun(bool is_loop, int q = 0, int test_num = 1) {
    ...
    for (int i = 0; i < test_num; ++i) {
      ...
      ASSERT_NO_FATAL_FAILURE(CompareResults(
          qcoeff_ref, qcoeff, bufferSize, "Qcoeff", q, i));
      ASSERT_NO_FATAL_FAILURE(CompareResults(
          dqcoeff_ref, dqcoeff, bufferSize, "Dqcoeff", q, i));
  ...
  int coeff_num() { return tx_size_2d[tx_size_]; }

  void FillCoeffGeneric(tran_low_t c) {
    const int n_coeffs = coeff_num();
  ...
  void FillCoeffZero() { FillCoeffGeneric(0); }
  ...
  void FillCoeffRandom() { FillCoeffGeneric(); }
...
const QuantizeParam kQ32x32ParamArraySSSE3[] = {
