 ...
 protected:
  ...
  virtual bool HandleDecodeResult(const aom_codec_err_t res_dec,
    ...
    if (AOM_CODEC_OK == res_dec) {
      ...
      AOM_CODEC_CONTROL_TYPECHECKED(ctx_dec, AOMD_GET_STILL_PICTURE_PRESENT,
                                    &still_pic_info_);
      if (is_still_picture_present_ == true &&
          still_pic_info_.still_picture != 1) {
        is_still_picture_present_ = false;
      }
      if (is_still_picture_hdr_respected_ == true &&
          still_pic_info_.full_still_picture_hdr != enable_full_header_) {
