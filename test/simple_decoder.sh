...
simple_decoder_verify_environment() {
  if
    [!"$(av1_encode_available)" = "yes"] && [!-e "${AV1_IVF_FILE}"];
  then return 1 fi
}
...
simple_decoder() {
  local decoder = "$(aom_tool_path simple_decoder)" local input_file =
      "$1" local codec = "$2" local output_file =
          "${AOM_TEST_OUTPUT_DIR}/simple_decoder_${codec}.raw"

      if[!-x "${decoder}"];
  then elog "${decoder} does not exist or is not executable." return 1 fi
...
simple_decoder_av1() {
  if
    ["$(av1_decode_available)" = "yes"];
  then if[!-e "${AV1_IVF_FILE}"];
  then local file =
      "${AOM_TEST_OUTPUT_DIR}/test_encode.ivf" encode_yuv_raw_input_av1
      "${file}" --ivf simple_decoder "${file}" av1 ||
      return 1 else simple_decoder "${AV1_IVF_FILE}" av1 || return 1 fi fi
}
