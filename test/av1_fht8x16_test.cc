...
typedef tuple<FhtFunc, IhtFunc, int, AomBitDepthT, int> Ht8x16Param;

void fht8x16_ref(const int16_t *in, TranLowT *out, int stride, int tx_type) {
  av1_fht8x16_c(in, out, stride, tx_type);
}

void iht8x16_ref(const TranLowT *in, uint8_t *out, int stride, int tx_type) {
