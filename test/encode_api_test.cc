...
TEST(EncodeAPI, Buganizer310766628) {
  ...
  // This configure will trigger the assertion failure.
...
TEST(EncodeAPI, Buganizer310457427) {
  ...
  // This configure will trigger the float-cast-overflow bug which was caused by
  // division by zero.
  ...
  // Encode a frame. This will trigger the overflow bug.
  image = CreateGrayImage(AOM_IMG_FMT_I420, cfg.g_w, cfg.g_h);
  ASSERT_NE(image, nullptr);
  ASSERT_EQ(aom_codec_encode(&enc, image, frame_index, 1, flags), AOM_CODEC_OK);
  frame_index++;
