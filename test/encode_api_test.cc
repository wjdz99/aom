...
TEST(EncodeAPI, B310766628) {
  ...
  // This configure will trigger the assertion bug.
...
TEST(EncodeAPI, B310457427) {
  ...
  // This configure is different from the original fuzz bug where CBR mode was
  // used. We have to change it to VBR mode to trigger the bug by the second
  // configure below.
  struct Config init_config = { 12, 896, 1076, AOM_VBR };
  const unsigned int usage = AOM_USAGE_REALTIME;
  ASSERT_EQ(aom_codec_enc_config_default(iface, &cfg, usage), AOM_CODEC_OK);
  ...
  // This configure will trigger the overflow bug.
