...
void YUVTemporalFilterTest::ApplyTestFilter<uint8_t>(
  ...
  const int alloc_sz = MAX_MB_PLANE * mb_pels;
  ref_frame->buffer_alloc_sz = alloc_sz;
  ref_frame->buffer_alloc = (uint8_t *)malloc(sizeof(uint8_t) * alloc_sz);
  ...
  DECLARE_ALIGNED(16, uint8_t, pred[MAX_MB_PLANE * mb_pels]);
  DECLARE_ALIGNED(16, uint32_t, accum[MAX_MB_PLANE * mb_pels]);
  DECLARE_ALIGNED(16, uint16_t, count[MAX_MB_PLANE * mb_pels]);
  memcpy(ref_frame->buffer_alloc + 0 * mb_pels, y_src,
         mb_pels * sizeof(uint8_t));
  memcpy(ref_frame->buffer_alloc + 1 * mb_pels, u_src,
         mb_pels * sizeof(uint8_t));
  memcpy(ref_frame->buffer_alloc + 2 * mb_pels, v_src,
         mb_pels * sizeof(uint8_t));
  memcpy(pred + 0 * mb_pels, y_pre, mb_pels);
  memcpy(pred + 1 * mb_pels, u_pre, mb_pels);
  memcpy(pred + 2 * mb_pels, v_pre, mb_pels);
...
void YUVTemporalFilterTest::ApplyTestFilter<uint16_t>(
  ...
  const int alloc_sz = MAX_MB_PLANE * mb_pels;
  ref_frame->buffer_alloc_sz = alloc_sz;
  uint16_t *buffer_alloc16 = (uint16_t *)malloc(sizeof(uint16_t) * alloc_sz);
  ref_frame->buffer_alloc = CONVERT_TO_BYTEPTR(buffer_alloc16);
  ref_frame->buffers[0] = ref_frame->buffer_alloc + 0 * mb_pels;
  ref_frame->buffers[1] = ref_frame->buffer_alloc + 1 * mb_pels;
  ref_frame->buffers[2] = ref_frame->buffer_alloc + 2 * mb_pels;
  ...
  mbd->plane[2].subsampling_y = ss_y;
  mbd->plane[2].subsampling_x = ss_x;
  DECLARE_ALIGNED(16, uint16_t, pred16[MAX_MB_PLANE * mb_pels]);
  DECLARE_ALIGNED(16, uint32_t, accum[MAX_MB_PLANE * mb_pels]);
  DECLARE_ALIGNED(16, uint16_t, count[MAX_MB_PLANE * mb_pels]);
  memset(pred16, 0, sizeof(uint16_t) * MAX_MB_PLANE * mb_pels);
  memset(accum, 0, sizeof(uint32_t) * MAX_MB_PLANE * mb_pels);
  memset(count, 0, sizeof(uint16_t) * MAX_MB_PLANE * mb_pels);
  uint8_t *pred = CONVERT_TO_BYTEPTR(pred16);
  for (int i = 0; i < mb_pels; ++i) {
    buffer_alloc16[0 * mb_pels + i] = y_src[i];
    buffer_alloc16[1 * mb_pels + i] = u_src[i];
    buffer_alloc16[2 * mb_pels + i] = v_src[i];
    pred16[i + 0 * mb_pels] = y_pre[i];
    pred16[i + 1 * mb_pels] = u_pre[i];
    pred16[i + 2 * mb_pels] = v_pre[i];
    accum[i + 0 * mb_pels] = y_accum[i];
    accum[i + 1 * mb_pels] = u_accum[i];
    accum[i + 2 * mb_pels] = v_accum[i];
    count[i + 0 * mb_pels] = y_count[i];
    count[i + 1 * mb_pels] = u_count[i];
    count[i + 2 * mb_pels] = v_count[i];
  }
  ...
  for (int i = 0; i < mb_pels; ++i) {
    y_accum[i] = accum[i + mb_pels * 0];
    u_accum[i] = accum[i + mb_pels * 1];
    v_accum[i] = accum[i + mb_pels * 2];
    y_count[i] = count[i + mb_pels * 0];
    u_count[i] = count[i + mb_pels * 1];
    v_count[i] = count[i + mb_pels * 2];
  }
}
