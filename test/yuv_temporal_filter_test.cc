...
void YUVTemporalFilterTest::ApplyTestFilter<uint8_t>(
  ...
  YV12_BUFFER_CONFIG ref_frame;
  aom_alloc_frame_buffer(&ref_frame, block_width, block_height, 0, 0, 0, 0, 32);
  ref_frame.buffers[0] = ref_frame.y_buffer;
  ref_frame.buffers[1] = ref_frame.u_buffer;
  ref_frame.buffers[2] = ref_frame.v_buffer;
  ...
  DECLARE_ALIGNED(16, uint8_t, pred[3 * mb_pels]) = { 0 };
  DECLARE_ALIGNED(16, uint32_t, accum[3 * mb_pels]) = { 0 };
  DECLARE_ALIGNED(16, uint16_t, count[3 * mb_pels]) = { 0 };
  for (int i = 0; i < mb_pels; ++i) {
    ref_frame.y_buffer[i] = y_src[i];
    ref_frame.u_buffer[i] = u_src[i];
    ref_frame.v_buffer[i] = v_src[i];
    pred[i] = y_pre[i];
    pred[i + mb_pels] = u_pre[i];
    pred[i + mb_pels * 2] = v_pre[i];
  }
  ...
  for (int i = 0; i < mb_pels; ++i) {
    y_accum[i] = accum[i];
    u_accum[i] = accum[i + mb_pels];
    v_accum[i] = accum[i + mb_pels * 2];
    y_count[i] = count[i];
    u_count[i] = count[i + mb_pels];
    v_count[i] = count[i + mb_pels * 2];
  }
  aom_free_frame_buffer(&ref_frame);
