...
void YUVTemporalFilterTest::ApplyTestFilter<uint16_t>(
  ...
  DECLARE_ALIGNED(16, uint32_t, accum[MAX_MB_PLANE * mb_pels]);
  DECLARE_ALIGNED(16, uint16_t, count[MAX_MB_PLANE * mb_pels]);
  memcpy(buffer_alloc16 + 0 * mb_pels, y_src, mb_pels * sizeof(uint16_t));
  memcpy(buffer_alloc16 + 1 * mb_pels, u_src, mb_pels * sizeof(uint16_t));
  memcpy(buffer_alloc16 + 2 * mb_pels, v_src, mb_pels * sizeof(uint16_t));
