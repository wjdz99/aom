...
template <int shift> v128 imm_v128_shl_n_64(v128 a) {
  return v128_shl_n_64(a, shift);
}
template <int shift> v128 imm_v128_shr_n_u64(v128 a) {
  return v128_shr_n_u64(a, shift);
}
template <int shift> v128 imm_v128_shr_n_s64(v128 a) {
  return v128_shr_n_s64(a, shift);
}
template <int shift> v128 imm_v128_align(v128 a, v128 b) {
...
template <int shift> c_v128 c_imm_v128_shl_n_64(c_v128 a) {
  return c_v128_shl_n_64(a, shift);
}
template <int shift> c_v128 c_imm_v128_shr_n_u64(c_v128 a) {
  return c_v128_shr_n_u64(a, shift);
}
template <int shift> c_v128 c_imm_v128_shr_n_s64(c_v128 a) {
  return c_v128_shr_n_s64(a, shift);
}
template <int shift> c_v128 c_imm_v128_align(c_v128 a, c_v128 b) {
...
template <int shift> v256 imm_v256_shl_n_64(v256 a) {
  return v256_shl_n_64(a, shift);
}
template <int shift> v256 imm_v256_shr_n_u64(v256 a) {
  return v256_shr_n_u64(a, shift);
}
template <int shift> v256 imm_v256_shr_n_s64(v256 a) {
  return v256_shr_n_s64(a, shift);
}
template <int shift> v256 imm_v256_align(v256 a, v256 b) {
...
template <int shift> c_v256 c_imm_v256_shl_n_64(c_v256 a) {
  return c_v256_shl_n_64(a, shift);
}
template <int shift> c_v256 c_imm_v256_shr_n_u64(c_v256 a) {
  return c_v256_shr_n_u64(a, shift);
}
template <int shift> c_v256 c_imm_v256_shr_n_s64(c_v256 a) {
  return c_v256_shr_n_s64(a, shift);
}
template <int shift> c_v256 c_imm_v256_align(c_v256 a, c_v256 b) {
...
void TestSimd3Args(uint32_t iterations, uint32_t mask, uint32_t maskwidth,
  ...
  for (unsigned int count = 0;
       count < iterations && !error && !testing::Test::HasFailure(); count++) {
    for (unsigned int c = 0; c < sizeof(CArg1); c++)
      s1[c] = rnd.Rand8();

    for (unsigned int c = 0; c < sizeof(CArg2); c++)
      s2[c] = rnd.Rand8();

    for (unsigned int c = 0; c < sizeof(CArg3); c++)
      s3[c] = rnd.Rand8();

    if (maskwidth)
      SetMask(s3, sizeof(CArg3), mask, maskwidth);
