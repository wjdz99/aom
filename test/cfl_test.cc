...
TEST_P(CFLAverageSpeedTest, AverageSpeedTest4) {
  test_cfl_speed(subtract, &av1_subtract_average_c, pred_buf_q3_data, 4,
                 UINT16_MAX);
}

TEST_P(CFLAverageSpeedTest, AverageSpeedTest8) {
  test_cfl_speed(subtract, &av1_subtract_average_c, pred_buf_q3_data, 8,
                 UINT16_MAX);
}

TEST_P(CFLAverageSpeedTest, AverageSpeedTest16) {
  test_cfl_speed(subtract, &av1_subtract_average_c, pred_buf_q3_data, 16,
                 UINT16_MAX);
}

TEST_P(CFLAverageSpeedTest, AverageSpeedTest32) {
  test_cfl_speed(subtract, &av1_subtract_average_c, pred_buf_q3_data, 32,
                 UINT16_MAX);
}
...
INSTANTIATE_TEST_CASE_P(C, CFLAverageTest,
                        ::testing::Values(make_tuple(&av1_subtract_average_c)));
#if HAVE_SSE2
INSTANTIATE_TEST_CASE_P(SSE2, CFLAverageTest, ::testing::Values(make_tuple(
                                                  &av1_subtract_average_sse2)));

INSTANTIATE_TEST_CASE_P(
    SSE2, CFLAverageSpeedTest,
    ::testing::Values(make_tuple(&av1_subtract_average_sse2)));
#endif
