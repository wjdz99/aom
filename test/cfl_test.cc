...
#define ALL_CFL_TX_SIZES(function)                                      \
  make_tuple(TX_4X4, &function), make_tuple(TX_4X8, &function),         \
      make_tuple(TX_8X4, &function), make_tuple(TX_8X4, &function),     \
      make_tuple(TX_8X8, &function), make_tuple(TX_8X16, &function),    \
      make_tuple(TX_16X8, &function), make_tuple(TX_16X16, &function),  \
      make_tuple(TX_16X32, &function), make_tuple(TX_32X16, &function), \
      make_tuple(TX_32X32, &function)
...
static void assertFaster(int ref_elapsed_time, int elapsed_time) {
  EXPECT_GT(ref_elapsed_time, elapsed_time)
      << "Error: CFLSubtractSpeedTest, SIMD slower "
         "than C."
      << std::endl
...
#if HAVE_SSSE3
const sub_avg_param sub_avg_sizes_ssse3[] = { ALL_CFL_TX_SIZES(
    get_subtract_average_fn_ssse3) };
...
INSTANTIATE_TEST_CASE_P(SSSE3, CFLSubAvgTest,
                        ::testing::ValuesIn(sub_avg_sizes_ssse3));
