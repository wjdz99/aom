  ...
  void init(int width, int height) {
    ...
    for (int j = 0; j < height; j++) {
      for (int i = 0; i < width; i++) {
        ...
        sub_luma_pels_ref[j * CFL_BUF_LINE + i] = sub_luma_pels[j * CFL_BUF_LINE + i];
      }
    }
  }
};
...
const predict_param predict_sizes_sse2[] = { ALL_SIZES_CFL(
    av1_cfl_build_prediction_lbd_sse2) };
...
INSTANTIATE_TEST_CASE_P(SSE2, CFLPredictTest,
                        ::testing::ValuesIn(predict_sizes_sse2));
#endif
