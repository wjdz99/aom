...
template <typename F>
class CFLTest : public ::testing::TestWithParam<::testing::tuple<TX_SIZE, F>> {
 public:
  virtual ~CFLTest() {}
  virtual void SetUp() {
...
TEST_P(CFLSubsampleTest, SubsampleTest) {
  ...
  for (int it = 0; it < NUM_ITERATIONS; it++) {
    ...
    fun_under_test(1, 1)(data, CFL_BUF_LINE, sub_luma_pels, width, height);
    get_subsample_lbd_fn_c(1, 1)(data_ref, CFL_BUF_LINE, sub_luma_pels_ref,
                                 width, height);
...
TEST_P(CFLSubsampleTest, DISABLED_SubsampleSpeedTest) {
  int16_t sub_luma_pels[CFL_BUF_SQUARE];
  int16_t sub_luma_pels_ref[CFL_BUF_SQUARE];
  cfl_subsample_lbd_fn subsample = fun_under_test(1, 1);
  cfl_subsample_lbd_fn subsample_ref = get_subsample_lbd_fn_c(1, 1);
  aom_usec_timer ref_timer;
  aom_usec_timer timer;
  ...
  aom_usec_timer_start(&ref_timer);
  for (int k = 0; k < NUM_ITERATIONS_SPEED; k++) {
    subsample_ref(data_ref, CFL_BUF_LINE, sub_luma_pels, width, height);
  }
  aom_usec_timer_mark(&ref_timer);
  int ref_elapsed_time = (int)aom_usec_timer_elapsed(&ref_timer);
  ...
  aom_usec_timer_start(&timer);
  for (int k = 0; k < NUM_ITERATIONS_SPEED; k++) {
    subsample(data, CFL_BUF_LINE, sub_luma_pels_ref, width, height);
  }
  aom_usec_timer_mark(&timer);
  int elapsed_time = (int)aom_usec_timer_elapsed(&timer);
