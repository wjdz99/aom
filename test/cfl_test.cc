  ...
  void init(int width, int height) {
    ...
    for (int j = 0; j < height; j++) {
      for (int i = 0; i < width; i++) {
        ...
        sub_luma_pels[j * CFL_BUF_LINE + i] = rnd.Rand8() * 2 - 255;
...
TEST_P(CFLPredictTest, PredictTest) {
  ...
  for (int it = 0; it < NUM_ITERATIONS; it++) {
    init(width, height);
    predict(sub_luma_pels, chroma_pels, CFL_BUF_LINE, width, height, alpha_q3);
    av1_cfl_build_prediction_lbd_c(sub_luma_pels_ref, chroma_pels_ref,
                                   CFL_BUF_LINE, width, height, alpha_q3);
    for (int j = 0; j < height; j++) {
      for (int i = 0; i < width; i++) {
...
const subsample_param subsample_sizes_sse2[] = { CHROMA_420_CFL_SIZES(
    get_subsample_lbd_fn_sse2) };
...
const predict_param predict_sizes_avx2[] = { ALL_CFL_SIZES(
    av1_cfl_build_prediction_lbd_avx2) };
