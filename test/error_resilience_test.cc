 ...
 protected:
  ...
  virtual void FramePktHook(const aom_codec_cx_pkt_t *pkt) {
    ...
    if ((encode_flags & (AOM_EFLAG_NO_UPD_LAST | AOM_EFLAG_NO_UPD_GF |
        ...
        (AOM_EFLAG_NO_UPD_LAST | AOM_EFLAG_NO_UPD_GF | AOM_EFLAG_NO_UPD_ARF)) {
      ASSERT_EQ(
          static_cast<int>(pkt->data.frame.flags & AOM_FRAME_IS_DROPPABLE),
          AOM_FRAME_IS_DROPPABLE);
    }
    if (encode_flags & AOM_EFLAG_SET_S_FRAME) {
      ASSERT_EQ(static_cast<int>(pkt->data.frame.flags & AOM_FRAME_IS_SWITCH),
                AOM_FRAME_IS_SWITCH);
    }
    if (encode_flags & AOM_EFLAG_ERROR_RESILIENT) {
      ASSERT_EQ(static_cast<int>(pkt->data.frame.flags &
