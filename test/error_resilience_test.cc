...
class ErrorResilienceTestLarge
 ...
 protected:
  ...
  virtual void BeginPassHook(unsigned int /*pass*/) {
    ...
    mismatch_psnr_ = 0.0;
    mismatch_nframes_ = 0;
    current_frame_error_resilient_ = false;
    current_frame_switch_ = false;
    current_frame_droppable_ = false;
  }
    ...
    if (error_resilient_nframes_ > 0 &&
      ...
      for (unsigned int i = 0; i < error_resilient_nframes_; ++i) {
        if (error_resilient_frames_[i] == video->frame()) {
          ...
          current_frame_error_resilient_ = true;
          break;
        }
      }
    }
  ...
  virtual void FramePktHook(const aom_codec_cx_pkt_t *pkt) {
    ...
    // TODO(debargha, sarahparker): Fix the commented out tests here. The
    // failures seem to be due to incorrect flags passing for lagged encoding.
 ...
 private:
  ...
  bool current_frame_droppable_;
  bool current_frame_error_resilient_;
  bool current_frame_switch_;
};
