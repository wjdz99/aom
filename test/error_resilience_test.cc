...
class SFramePresenceTestLarge
 ...
 protected:
  ...
  virtual bool HandleDecodeResult(const aom_codec_err_t res_dec,
    ...
    if (is_sframe_present_ != 1 && AOM_CODEC_OK == res_dec) {
      ...
      if (sframe_info.is_s_frame) {
        ...
        if (enable_altref_ && is_sframe_position_violated == 0 &&
            sframe_info.is_s_frame_at_arf == 0)
          is_sframe_position_violated = 1;
      }
    }
  ...
  int is_sframe_position_violated;
  int enable_altref_;
...
TEST_P(SFramePresenceTestLarge, SFramePresenceTest) {
  ...
  if (enable_altref_) ASSERT_EQ(is_sframe_position_violated, 0);
}

/*  TODO(anyone): When altref is enabled S_FRAME test is failing.
 *  Add configuration for altref once the bug is fixed.
 */
