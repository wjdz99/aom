...
list(APPEND AOM_TEST_DATA_FILE_NAMES
    ...
    if (ENABLE_DECODE_PERF_TESTS AND CONFIG_AV1_ENCODER) list(
        APPEND AOM_TEST_DATA_FILE_NAMES "niklas_1280_720_30.yuv") endif()

        if (CONFIG_AV1_DECODER) list(
            APPEND AOM_TEST_DATA_FILE_NAMES
            ...
            "invalid-oss-fuzz-9720.ivf.res") endif()

            if (ENABLE_ENCODE_PERF_TESTS AND CONFIG_AV1_ENCODER) list(
                APPEND AOM_TEST_DATA_FILE_NAMES
                ...
                "thaloundeskmtg_640_480_30.yuv") endif()

#Parses test / test - data.sha1 and writes captured file names and checksums to
#$out_files and $out_checksums as lists.
                function(
                    make_test_data_lists test_data_file out_files
                        out_checksums) if (NOT test_data_file OR NOT EXISTS
                                           "${test_data_file}") message(FATAL_ERROR
                                                                        "Test "
                                                                        "info "
                                                                        "file "
                                                                        "missin"
...
#Iterate over the list of lines and split it into $checksums and $filenames.
                        foreach (line ${
                            files_and_checksums }) string(FIND
                                                          "${line}"
                                                          " *" delim_pos)

                            math(EXPR filename_pos "${delim_pos} + 2") string(
                                SUBSTRING "${line}" 0 ${
                                    delim_pos } checksum) string(SUBSTRING
                                                                 "${line}" ${
                                                                     filename_pos } -
                                                                 1 filename)

                                list(FIND AOM_TEST_DATA_FILE_NAMES ${
                                    filename } list_index) if (NOT ${
                                                                   list_index } EQUAL -
                                                               1)

#Include the name and checksum in output only when the file is needed.
                                    set(checksums ${ checksums } ${
                                        checksum }) set(filenames ${
                                        filenames } ${
                                        filename }) endif() endforeach()

                                        list(LENGTH filenames num_files) list(LENGTH checksums num_checksums) if (
                                            NOT checksums OR NOT filenames OR
                                                NOT num_files EQUAL
                                                    num_checksums) message(FATAL_ERROR
                                                                           "Par"
                                                                           "sin"
                                                                           "g "
                                                                           "of "
                                            ...
                                            set(${ out_checksums } ${
                                                checksums } PARENT_SCOPE) set(${
                                                out_files } ${
                                                filenames } PARENT_SCOPE) endfunction()
...
#$out_path_list.
                                                function(
                                                    expand_test_file_paths
                                                        test_files test_dir
                                                            out_path_list) foreach (filename ${
                                                    ${ test_files } }) set(path_list $ {
                                                  path_list
                                                } "${test_dir}/${filename}")
                                                    endforeach() set(${
                                                        out_path_list } ${
                                                        path_list } PARENT_SCOPE) endfunction()

                                                        function(
                                                            check_file local_path expected_checksum
                                                                out_needs_update) if (EXISTS
                                                                                      "${local_path}")
                                                            file(
                                                                SHA1
                                                                "${local_"
                                                                "path"
                                                                "}" file_checksum) else() set($ {
                                                              out_needs_update
                                                            } 1 PARENT_SCOPE) return ()
                                                                endif()
                                                                    ...
                                                                    if ("${"
                                                                        ...
                                                                        "checks"
                                                                        "um}") unset(${
                                                                        out_needs_update } PARENT_SCOPE) else() set($ {
                                                                      out_needs_update
                                                                    } 1 PARENT_SCOPE) return () endif()
                                                                        message(
                                                                            "${"
                                                                            "lo"
                                                                            "ca"
                                                                            ...
                                                                            function(
                                                                                ...
                                                                                message(
                                                                                    ...
                                                                                    file(
                                                                                        DOWNLOAD
                                                                                        "${file_url}"
                                                                                        "${local_path}" SHOW_PROGRESS EXPECTED_HASH
                                                                                            SHA1 =
                                                                                                ${ file_checksum })
                                                                                        message(
                                                                                            "Download of ${file_url} complete.")
                                                                                            endfunction()
