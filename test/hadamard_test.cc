...
template <typename OutputType>
void ReferenceHadamard16x16_fp(const int16_t *a, int a_stride, OutputType *b) {
  /* The source is a 16x16 block. The destination is rearranged to 8x32.
   * Input is 9 bit. */
  ReferenceHadamard8x8(a + 0 + 0 * a_stride, a_stride, b + 0);
  ReferenceHadamard8x8(a + 8 + 0 * a_stride, a_stride, b + 64);
...
template <typename OutputType>
void ReferenceHadamard32x32_fp(const int16_t *a, int a_stride, OutputType *b) {
  ReferenceHadamard16x16_fp(a + 0 + 0 * a_stride, a_stride, b + 0);
  ReferenceHadamard16x16_fp(a + 16 + 0 * a_stride, a_stride, b + 256);
  ReferenceHadamard16x16_fp(a + 0 + 16 * a_stride, a_stride, b + 512);
  ReferenceHadamard16x16_fp(a + 16 + 16 * a_stride, a_stride, b + 768);
...
template <typename OutputType>
void ReferenceHadamard_fp(const int16_t *a, int a_stride, OutputType *b, int bw,
                          int bh) {
  if (bw == 32 && bh == 32) {
    ReferenceHadamard32x32_fp(a, a_stride, b);
  } else if (bw == 16 && bh == 16) {
    ReferenceHadamard16x16_fp(a, a_stride, b);
  } else if (bw == 8 && bh == 8) {
    ReferenceHadamard8x8(a, a_stride, b);
  } else if (bw == 4 && bh == 4) {
    ReferenceHadamard4x4(a, a_stride, b);
...
class HadamardTestBase
 ...
 public:
  ...
  void CompareReferenceRandom_fp() {
    const int kMaxBlockSize = 32 * 32;
    const int block_size_ = bw_ * bh_;
    ...
    ReferenceHadamard_fp(a, bw_, b_ref, bw_, bh_);
    API_REGISTER_STATE_CHECK(h_func_(a, bw_, b));
    EXPECT_EQ(memcmp(b, b_ref, sizeof(b)), 0);
  }
  ...
  void VaryStride_fp() {
    ...
    for (int i = 8; i < 64; i += 8) {
      ...
      ReferenceHadamard_fp(a, i, b_ref, bw_, bh_);
      API_REGISTER_STATE_CHECK(h_func_(a, i, b));
      EXPECT_EQ(0, memcmp(b, b_ref, sizeof(b)));
    }
  }
...
TEST_P(HadamardLowbdTest, CompareReferenceRandom) {
  CompareReferenceRandom_fp();
}

TEST_P(HadamardLowbdTest, VaryStride) { VaryStride_fp(); }
