...
template <typename OutputType>
void ReferenceHadamard(const int16_t *a, int a_stride, OutputType *b,
                       int bwh) {
  if (bwh == 32)
...
template <typename HadamardFuncType>
std::ostream &operator<<(std::ostream &os, const FuncWithSize<HadamardFuncType> &hfs) {
  return os << "block size: " << hfs.block_size;
}

template <typename OutputType, typename HadamardFuncType>
class HadamardTestBase : public ::testing::TestWithParam<FuncWithSize<HadamardFuncType>> {
 public:
...
TEST_P(HadamardLowbdLPTest, CompareReferenceRandom) { CompareReferenceRandom(); }
