...
typedef tuple<FhtFunc, IhtFunc, int, AomBitDepthT, int> Ht16x8Param;

void fht16x8_ref(const int16_t *in, TranLowT *out, int stride, int tx_type) {
  av1_fht16x8_c(in, out, stride, tx_type);
}

void iht16x8_ref(const TranLowT *in, uint8_t *out, int stride, int tx_type) {
