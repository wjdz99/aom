...
typedef tuple<FhtFunc, IhtFunc, int, AomBitDepthT, int> Ht4x8Param;

void fht4x8_ref(const int16_t *in, TranLowT *out, int stride, int tx_type) {
  av1_fht4x8_c(in, out, stride, tx_type);
}

void iht4x8_ref(const TranLowT *in, uint8_t *out, int stride, int tx_type) {
