...
class AverageTestBase : public ::testing::Test {
 ...
 protected:
  ...
  // Sum Pixels
  unsigned int ReferenceAverage8x8(const uint8_t *source) {
  ...
  unsigned int ReferenceAverage4x4(const uint8_t *source) {
    unsigned int average = 0;
    for (int h = 0; h < 4; ++h)
...
typedef unsigned int (*AverageFunction)(const uint8_t *s, int p);
...
class AverageTest : public AverageTestBase,
 ...
 protected:
  void CheckAverages() {
    ...
    if (block_size == 8) {
      expected =
          ReferenceAverage8x8(source_data_ + GET_PARAM(2));
    } else if (block_size == 4) {
      expected =
          ReferenceAverage4x4(source_data_ + GET_PARAM(2));
    }
