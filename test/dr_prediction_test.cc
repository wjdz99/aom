 ...
 protected:
  ...
  void RunTest(bool speedtest, bool needsaturation, int p_angle) {
    ...
    for (int tx = 0; tx < TX_SIZES_ALL; ++tx) {
      ...
      // Create input buffers based on the transform dimensions to allow
      // checking for over-reads. We need to allow reading the previous
      // 16 bytes from the input pointers.
      const int lu_offset = 16;
      const int left_buf_size =
          ((MAX_TX_SIZE + MAX_TX_SIZE) << upsample_left_) + lu_offset;
      const int above_buf_size =
          ((MAX_TX_SIZE + MAX_TX_SIZE) << upsample_above_) + lu_offset;

      std::vector<Pixel> left_data(left_buf_size);
      std::vector<Pixel> above_data(above_buf_size);

      const Pixel *above = &above_data[lu_offset];
      const Pixel *left = &left_data[lu_offset];

      Pixel sat = (1 << bd_) - 1;
      for (unsigned i = 0; i < left_data.size(); ++i) {
        left_data[i] = needsaturation ? sat : rng_.Rand8();
      }
      for (unsigned i = 0; i < above_data.size(); ++i) {
        above_data[i] = needsaturation ? sat : rng_.Rand8();
      }
