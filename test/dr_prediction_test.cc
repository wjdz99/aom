 ...
 protected:
  ...
  void RunTest(bool speedtest, bool needsaturation, int p_angle) {
    ...
    for (int tx = 0; tx < TX_SIZES_ALL; ++tx) {
      ...
      int lu_padding = 16;
      DECLARE_ALIGNED(16, Pixel, left_data[NUM_INTRA_NEIGHBOUR_PIXELS]);
      DECLARE_ALIGNED(16, Pixel, above_data[NUM_INTRA_NEIGHBOUR_PIXELS]);

      const Pixel *above = &above_data[lu_padding];
      const Pixel *left = &left_data[lu_padding];

      Pixel sat = (1 << bd_) - 1;
      for (unsigned i = 0; i < NUM_INTRA_NEIGHBOUR_PIXELS; ++i) {
