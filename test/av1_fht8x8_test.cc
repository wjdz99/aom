...
using std::tr1::tuple;
typedef tuple<FhtFunc, IhtFunc, int, AomBitDepthT, int> Ht8x8Param;

void fht8x8_ref(const int16_t *in, TranLowT *out, int stride, int tx_type) {
  av1_fht8x8_c(in, out, stride, tx_type);
}

void iht8x8_ref(const TranLowT *in, uint8_t *out, int stride, int tx_type) {
  av1_iht8x8_64_add_c(in, out, stride, tx_type);
}

#if CONFIG_HIGHBITDEPTH
typedef void (*IHbdHtFunc)(const TranLowT *in, uint8_t *out, int stride,
                           int tx_type, int bd);
