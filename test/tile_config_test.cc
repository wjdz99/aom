 ...
 protected:
  ...
  virtual bool HandleDecodeResult(const aom_codec_err_t res_dec,
    ...
    if (AOM_CODEC_OK == res_dec) {
      ...
      int i, j = 0;
      for (i = 0; i < tile_info.tile_columns - 1; i++) {
        if (tile_config_param_.tile_widths[j] != tile_info.tile_widths[i]) {
          tile_config_violated_ = true;
        }
        j = (j + 1) % (int)tile_config_param_.tile_width_count;
      }
      if (tile_config_param_.tile_widths[j] < tile_info.tile_widths[i]) {
        tile_config_violated_ = true;
      }
      ...
      j = 0;
      for (i = 0; i < tile_info.tile_rows - 1; i++) {
        if (tile_config_param_.tile_heights[j] != tile_info.tile_heights[i]) {
          tile_config_violated_ = true;
        }
        j = (j + 1) % (int)tile_config_param_.tile_height_count;
      }
      if (tile_config_param_.tile_heights[j] < tile_info.tile_heights[i]) {
        tile_config_violated_ = true;
      }
