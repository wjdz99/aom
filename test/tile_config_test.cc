 ...
 protected:
  ...
  virtual bool HandleDecodeResult(const aom_codec_err_t res_dec,
    ...
    if (AOM_CODEC_OK == res_dec) {
      ...
      int config_tile_columns = 1 << (int)tile_config_param_.tile_cols;
      int config_tile_rows = 1 << (int)tile_config_param_.tile_rows;
...
TEST_P(UniformTileConfigTestLarge, UniformTileConfigTest) {
  ...
  if (video.y4m_.pic_w / (int)tile_config_param_.sb_size >=
          (1 << tile_config_param_.tile_cols) ||
      video.y4m_.pic_h / (int)tile_config_param_.sb_size >=
          (1 << tile_config_param_.tile_rows)) {
