...
static const TileGroupConfigParams TestParams[] = { { 5, 4, 4 }, { 3, 3, 3 },
                                                    { 5, 3, 3 }, { 7, 5, 5 },
                                                    { 7, 3, 3 }, { 7, 4, 4 } };
 ...
 protected:
  ...
  virtual bool HandleDecodeResult(const aom_codec_err_t res_dec,
    ...
    if (AOM_CODEC_OK == res_dec) {
      ...
      aom_codec_ctx_t *ctx_dec = decoder->GetDecoder();
      AOM_CODEC_CONTROL_TYPECHECKED(ctx_dec, AOMD_GET_TILE_INFO, &tile_info);
      AOM_CODEC_CONTROL_TYPECHECKED(ctx_dec, AOMD_GET_SHOW_EXISTING_FRAME,
                                    &show_existing_frame_);
      if (tile_info.num_tile_groups != tile_group_config_params_.num_tg &&
          !show_existing_frame_)
        num_tg_mismatch_ = true;
      EXPECT_EQ(num_tg_mismatch_, false);
    }
    return AOM_CODEC_OK == res_dec;
  }

  int show_existing_frame_;
  bool num_tg_mismatch_;
  aom_rc_mode end_usage_check_;
  ::libaom_test::TestMode encoding_mode_;
