...
// Copied from av1/common/resize.c
...
class TestImage {
 public:
  TestImage(int w_src, int h, int superres_denom, int bd)
      : w_src_(w_src), h_(h), superres_denom_(superres_denom), bd_(bd) {
    assert(bd < 16);
 ...
 private:
  int w_src_, w_dst_, h_, superres_denom_, bd_;
  ...
  void CorrectnessTest() {
    ...
    for (int i = 0; i < kTestIters; ++i) {
      for (int superres_denom = 9; superres_denom <= 16; superres_denom++) {
        ...
        image_ = new TestImage<Pixel>(width_src, height, superres_denom, bd_);
...
class LowBDConvolveHorizRSTest
 ...
 public:
  ...
  void RunOne(bool ref) {
    ...
    const int32_t x0_qn =
        get_upscale_convolve_x0(width_src, width_dst, x_step_qn);
