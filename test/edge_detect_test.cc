...
class GaussianBlurTest : public ::testing::TestWithParam<ImageParam> {};
...
TEST_P(GaussianBlurTest, UniformBrightness) {
  // For varying levels of brightness, the algorithm should
  // produce the same output.
  const int width = GetParam().width;
  const int height = GetParam().height;
...
const ImageParam image_params[] = {
  // (width, height)
  { 8, 4 },  { 8, 8 },  { 8, 12 },  { 8, 24 },  { 8, 28 },  { 8, 32 },
  { 16, 4 }, { 16, 8 }, { 16, 12 }, { 16, 24 }, { 16, 28 }, { 16, 32 },
  { 32, 4 }, { 32, 8 }, { 32, 12 }, { 32, 24 }, { 32, 28 }, { 32, 32 }
};

INSTANTIATE_TEST_CASE_P(GaussianImageTests, GaussianBlurTest,
                        testing::ValuesIn(image_params));

class EdgeDetectTest : public ::testing::TestWithParam<ImageParam> {};

// No edges on a uniformly bright image.
TEST_P(EdgeDetectTest, UniformBrightness) {
  const int width = GetParam().width;
  const int height = GetParam().height;
  for (int brightness = 0; brightness < 255; ++brightness) {
    uint8_t *orig = (uint8_t *)malloc(width * height);
...
TEST_P(EdgeDetectTest, BlackWhite) {
  const int width = GetParam().width;
  const int height = GetParam().height;
  uint8_t *orig = (uint8_t *)malloc(width * height);
...
INSTANTIATE_TEST_CASE_P(EdgeDetectImages, EdgeDetectTest,
                        testing::ValuesIn(image_params));
