...
TEST(CodingPathSync, SearchForHbdLbdMismatch) {
  ...
  for (int i = 0; i < count_tests; ++i) {
    ...
    auto lbd_yuv = dec_lbd.decode(frame);
    auto hbd_yuv = dec_hbd.decode(frame);

    // TODO(Issue 39): re-enable the comparison when both coding paths match,
    // so they don't diverge anymore.
    // Until then, still to the decoding, to prevent crashes from creeping in,
    // (as in now-fixed Issue 677) making this test harder to re-enable.
    if (0) {
