...
// The following code needs to run on any target.  Even though the
// tests for unsupported architectures get filtered out, some
// functions of the class are run and gcc occationally generates
// unsupported instructions unless we restrict it.  The rest of the
// code depends heavily on the v64, v128 and v256 types, which require
// full architecture awareness of the compiler, so we can't compile
// the entire file for a safe target and still rely on gtest.
#if HAVE_SSE2 && defined(__GNUC__) && !defined(__clang__)
