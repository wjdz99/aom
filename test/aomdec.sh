...
aomdec_av1_ivf() {
  if [ "$(aomdec_can_decode_av1)" = "yes" ]; then
    ...
    if [ ! -e "${file}" ]; then
      $(aomenc_encode_test_fast_params) --output="${file} --ivf"
    fi
...
aomdec_av1_obu_annexb() {
  if [ "$(aomdec_can_decode_av1)" = "yes" ]; then
    ...
    if [ ! -e "${file}" ]; then
      $(aomenc_encode_test_fast_params) --output="${file}" --obu --annexb=1
    fi
    for threads in 2 3 4 5 6 7 8; do
      aomdec "${file}" --summary --noblit --threads=$threads --annexb
    done
  fi
}
...
aomdec_av1_obu_section5() {
  if [ "$(aomdec_can_decode_av1)" = "yes" ]; then
    ...
    if [ ! -e "${file}" ]; then
      $(aomenc_encode_test_fast_params) --output="${file}" --obu
    fi
    for threads in 2 3 4 5 6 7 8; do
...
aomdec_av1_webm() {
  if [ "$(aomdec_can_decode_av1)" = "yes" ] && \
    ...
    if [ ! -e "${file}" ]; then
      $(aomenc_encode_test_fast_params) --output="${file}"
    fi
