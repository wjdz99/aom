...
namespace AV1Kmeans {
typedef void (*av1_calc_indices_dim1_func)(const int *data, const int *centroids,
...
const BLOCK_SIZE kValidBlockSize[] = {
  ...
  BLOCK_64X32, BLOCK_64X64, BLOCK_16X64,  BLOCK_64X16
};

typedef std::tuple<av1_calc_indices_dim1_func, BLOCK_SIZE> av1_calc_indices_dim1Param;
...
class AV1KmeansTest
 ...
 protected:
  void RunCheckOutput(av1_calc_indices_dim1_func test_impl, BLOCK_SIZE bsize, int centroids);
  void RunSpeedTest(av1_calc_indices_dim1_func test_impl, BLOCK_SIZE bsize, int centroids);
...
void AV1KmeansTest::SetUp() {
  ...
  for (int i = 0; i < 5096; ++i) {
    data_[i] = (int) rnd_.Rand31();
  }
  for(int i = 0; i < 8; i++){
    centroids_[i] = (int) rnd_.Rand31();
  }
}

void AV1KmeansTest::TearDown() {
...
void AV1KmeansTest::RunSpeedTest(av1_calc_indices_dim1_func test_impl,
  ...
  const int h = block_size_high[bsize];
  const int n = w * h;
  const int num_loops = 100000000000 / n;
  ...
  printf("av1_calc_indices_dim1 indices= %d centroids=%d: %7.2f/%7.2fns", n, k, elapsed_time[0],
         elapsed_time[1]);
