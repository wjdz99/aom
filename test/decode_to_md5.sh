...
decode_to_md5() {
  local decoder = "$(aom_tool_path decode_to_md5)" local input_file =
      "$1" local codec = "$2" local expected_md5 = "$3" local output_file =
          "${AOM_TEST_OUTPUT_DIR}/decode_to_md5_${codec}"
  ...
  then elog "${decoder} does not exist or is not executable." return 1 fi
      ...
      local md5_last_frame = "$(tail -n1 " $ {
    output_file
  }
  " | awk '{print $1}')" local actual_md5 = "$(echo " $ { md5_last_frame }
  " | awk '{print $1}')" if["${actual_md5}" = "${expected_md5}"];
  then return 0 else elog "MD5 mismatch:" elog "Expected: ${expected_md5}" elog
                          "Actual: ${actual_md5}" return 1 fi
}

DISABLED_decode_to_md5_av1() {
#expected MD5 sum for the last frame.
  local expected_md5 = "567dd6d4b7a7170edddbf58bbcc3aff1" local file =
      "${AV1_IVF_FILE}"
...
#TODO(urvang) : Check in the encoded file(like libvpx does) to avoid
  ...
  then file = "${AOM_TEST_OUTPUT_DIR}/test_encode.ivf" encode_yuv_raw_input_av1
              "${file}" --ivf fi decode_to_md5
      "${file}"
      "av1"
      "${expected_md5}" fi
}
