...
int check_upsampled_pred_sse2(int width, int height, int subpel_x_q3,
  ...
  uint8_t out_c[MAX_SB_SIZE * MAX_SB_SIZE];
  uint8_t out_asm[MAX_SB_SIZE * MAX_SB_SIZE];
  uint8_t *ref8 = &ref[(MAX_SB_SIZE + 64) * 32 + 32];
...
TEST(UpsampledPredTest, SSE2) {
  uint8_t ref[(MAX_SB_SIZE + 64) * (MAX_SB_SIZE + 64)];
  ACMRandom rnd_;
  for (int subpel_x_q3 = 0; subpel_x_q3 < 8; subpel_x_q3++) {
    for (int subpel_y_q3 = 0; subpel_y_q3 < 8; subpel_y_q3++) {
      for (int width_log2 = 2; width_log2 < MAX_SB_SIZE_LOG2; width_log2++) {
        for (int height_log2 = 2; height_log2 < MAX_SB_SIZE_LOG2;
             height_log2++) {
          int errors = 0;
          int width = 1 << width_log2;
          int height = 1 << height_log2;
          for (int y = 0; y < (MAX_SB_SIZE + 64); y++) {
            for (int x = 0; x < (MAX_SB_SIZE + 64); x++) {
...
int check_hbd_upsampled_pred_sse2(int width, int height, int subpel_x_q3,
  ...
  uint16_t out_c[MAX_SB_SIZE * MAX_SB_SIZE];
  uint16_t out_asm[MAX_SB_SIZE * MAX_SB_SIZE];
  uint8_t *ref8 = CONVERT_TO_BYTEPTR(&ref[(MAX_SB_SIZE + 64) * 32 + 32]);
  ...
  aom_highbd_upsampled_pred_sse2(out_asm, width, height, subpel_x_q3,
                                 subpel_y_q3, ref8, MAX_SB_SIZE, 10);
  int errors = 0;
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      errors += out_c[y * width + x] != out_asm[y * width + x];
...
TEST(HBDUpsampledPredTest, SSE2) {
  uint16_t ref[(MAX_SB_SIZE + 64) * (MAX_SB_SIZE + 64)];
  ACMRandom rnd_;
  for (int subpel_x_q3 = 0; subpel_x_q3 < 8; subpel_x_q3++) {
    for (int subpel_y_q3 = 0; subpel_y_q3 < 8; subpel_y_q3++) {
      for (int width_log2 = 2; width_log2 < MAX_SB_SIZE_LOG2; width_log2++) {
