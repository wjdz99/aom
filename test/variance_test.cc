...
void SubpelVarianceTest<SubpelVarianceFunctionType>::SpeedTest() {
  ...
  printf("sub_pixel_variance_%dx%d_%d: %d us %d us %f \n", width(), height(),
         params_.bit_depth, elapsed_time_c, elapsed_time,
         (float)elapsed_time_c / elapsed_time);
}
...
TEST_P(AvxSubpelVarianceTest, Speed) { SpeedTest(); }  // added now
