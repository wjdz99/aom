...
void MseHBDWxHTestClass<MseHBDWxHFunctionType>::SpeedTest() {
  ...
  for (int k = 0; k < block_size(); ++k) {
    dst_[k] = rnd_.Rand8() & mask();
    src_[k] = rnd_.Rand8() & mask();
  }
...
typedef MseHBDWxHTestClass<MseHBDWxH16bitFunc> AvxMseHBDWxHTest;
...
TEST_P(AvxMseHBDWxHTest, RefMse) { RefMatchTestMse(); }
TEST_P(AvxMseHBDWxHTest, DISABLED_SpeedMse) { SpeedTest(); }
