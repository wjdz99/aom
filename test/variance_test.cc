...
class MseWxHTestClass
 ...
 public:
  virtual void SetUp() {
    ...
    src_ = reinterpret_cast<uint16_t *>(aom_memalign(16, block_size() * sizeof(uint16_t)));
    dst_ = reinterpret_cast<uint8_t *>(aom_memalign(16, block_size() * sizeof(uint8_t)));
...
void MseWxHTestClass<MseWxHFunctionType>::SpeedTest() {
  ...
  int sstride = s_stride();
    uint8_t *const dst8 = dst_;
    for (int k = 0; k < block_size(); ++k) {
      dst8[k] = rnd_.Rand8();
      src_[k] = rnd_.Rand8();
    }
    src_[w*h - 5] = rnd_.Rand16();
      aom_usec_timer_start(&ref_timer); 
      for (int i = 0; i < run_time; i++) {
          params_.func(dst8, dstride, src_, sstride, w, h);
       }
      ...
      printf(
          "%dx%d\tc_time=%lf \t simd_time=%lf \t gain=%lf\n",
          width(), height() ,elapsed_time_c, elapsed_time_simd,
...
void MseWxHTestClass<MseWxHFunctionType>::RefMatchTestMse() {
  ...
  int sstride = s_stride();
    ...
    ASM_REGISTER_STATE_CHECK(
        mse_ref = params_.func(dst8, dstride, src_, sstride, w,  h));
    ASM_REGISTER_STATE_CHECK(
        mse_mod = aom_mse_wxh_16bit_avx2(dst8, dstride, src_, sstride, w, h));
...
TEST_P(AvxMseWxHTest, SpeedMse) { SpeedTest(); }
...
INSTANTIATE_TEST_SUITE_P(C, AvxMseTest,::testing::Values(MseParams(4, 4, &aom_mse16x16_c),
...
#if HAVE_AVX2
/*aom_mse_wxh_16bit_c is passed which will act as a reference
and avx function aom_mse_wxh_16bit_avx2 will be directly used in function
*/
typedef TestParams<MseWxH16bitFunc> MseWxHParams;
INSTANTIATE_TEST_SUITE_P(
    AVX2, AvxMseWxHTest,
    ::testing::Values(MseWxHParams(3, 3, &aom_mse_wxh_16bit_c, 0),
                      MseWxHParams(3, 2, &aom_mse_wxh_16bit_c, 0),
                      MseWxHParams(2, 3, &aom_mse_wxh_16bit_c, 0),
                      MseWxHParams(2, 2, &aom_mse_wxh_16bit_c, 0)));
