...
namespace ARCH_POSTFIX(simd_test) {
...
#define MAP(name) \
  ...
  // Map reference funcions to machine tuned functions.  Since the
  // functions depend on machine tuned types, the non-machine tuned
  // instantiations of the test can't refer to these functions
  // directly, so we refer to them by name and do the mapping here.
  ...
  void test_simd1(uint32_t iterations, uint32_t mask, uint32_t maskwidth,
    ...
    void *simd;
    int error = 0;
    DECLARE_ALIGNED(sizeof(c_arg), uint16_t,
                    s[sizeof(c_arg) / sizeof(uint16_t)]);
    ...
    if (simd == NULL || ref_simd == NULL) {
      std::cerr << "Internal errer: Unknown intrinsic function " << name
                << std::endl;
      abort();
    }
    ...
    for (unsigned int count = 0; count < iterations && !error; count++) {
      ...
      if (is_same<c_ret, c_v64>::value && is_same<c_arg1, c_v64>::value &&
        ...
        error = compare_simd2<v128, v128, uint32_t, c_ret, c_arg1, c_arg2>(
            ...
            (void *)ref_simd, ref_d, s1, s2);
        // V64_V64U32
      } else if (is_same<c_ret, c_v64>::value &&
                 is_same<c_arg1, c_v64>::value &&
                 is_same<c_arg2, uint32_t>::value) {
  ...
  template void test_simd2<c_v128, c_v128, c_v128>(uint32_t, uint32_t, uint32_t,
                                                   const char *);
  template void test_simd2<c_v128, c_v128, uint32_t>(uint32_t, uint32_t,
                                                     uint32_t, const char *);
