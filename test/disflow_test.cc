...
class ComputeFlowTest
 ...
 public:
  ComputeFlowTest() : target_func_(GetParam()) {
...
void ComputeFlowTest::RunCheckOutput(int run_times) {
  ...
  double u_rand = ((double)rnd_.Rand8() / 255) * 10 - 5;
  double u_ref = u_rand;
  double u_test = u_rand;

  double v_rand = ((double)rnd_.Rand8() / 255) * 10 - 5;
  double v_ref = v_rand;
  ...
  int x = rnd_(344 - 8 + 1) + 8;
  int y = rnd_(280 - 8 + 1) + 8;
  ...
  target_func_(src, ref, x, y, width, height, stride, &u_test, &v_test);
