...
typedef void (*HbdHtFunc)(const int16_t *input, int32_t *output, int stride,
                          TX_TYPE tx_type, int base_qinex, int bd);
typedef void (*IHbdHtFunc)(const int32_t *coeff, uint16_t *output, int stride,
                           TX_TYPE tx_type, int base_qindex, int bd);
#else
typedef void (*HbdHtFunc)(const int16_t *input, int32_t *output, int stride,
...
#if CONFIG_DATA_DRIVEN_TX
    txfm_ref_(input_, coeffs_, stride, tx_type_, 25, bit_depth_);
    inv_txfm_ref_(coeffs_, output_ref_, stride, tx_type_, 25, bit_depth_);
    ASM_REGISTER_STATE_CHECK(
        inv_txfm_(coeffs_, output_, stride, tx_type_, 25, bit_depth_));
#else
...
#if CONFIG_DATA_DRIVEN_TX
    fwd_func_(input, inv_input, stride, tx_type_, 25, bit_depth_);
#else
