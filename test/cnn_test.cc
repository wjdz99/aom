 ...
 protected:
  static void RunCNNTest(int image_width, int image_height, float *input,
    ...
    for (int channel = 0; channel < out_channels; ++channel) {
      output[channel] =
          (float *)aom_malloc(sizeof(*(output[channel])) * out_size);
    }

    av1_cnn_predict((const float **)&input, image_width, image_height,
                    in_stride, &cnn_config, (float **)output, out_stride);
    ...
    for (int channel = 0; channel < out_channels; ++channel) {
      for (int i = 0; i < out_size; ++i) {
        ...
        EXPECT_NEAR(expected[index], output[channel][i], PIXELWISE_FLOAT_TOL)
            << index << ": " << expected[index] << "/" << output[channel][index]
            << std::endl;
        mse += SQR(expected[index] - output[channel][index]);
      }
    }
