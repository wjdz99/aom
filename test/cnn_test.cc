...
typedef libaom_test::FuncParam<CNNConvolveFunc> CNNConvolveTestFunc;

class CNNConvolveTest : public ::testing::TestWithParam<CNNConvolveTestFunc> {
 protected:
  ...
  void RunCNNConvolveSetup(int run_times) {
    ...
    for (int layer = 0; layer < cnn_config->num_layers; ++layer) {
      ...
      av1_find_layer_output_size(in_height, in_width,
                                 &cnn_config->layer_config[layer], &out_width,
                                 &out_height);
...
INSTANTIATE_TEST_SUITE_P(AVX2, CNNConvolveTest,
                         ::testing::Values(CNNConvolveTestFunc(
