...
class CNNTest : public ::testing::Test {
 protected:
  static void RunCNNTest(int image_width, int image_height, float *input,
    ...
    float *output = (float *)aom_malloc(sizeof(*input) * image_size);
...
TEST_F(CNNTest, TestRELUMultiLayerMultiKernel) {
  ...
  for (int layer = 0; layer < cnn_config.num_layers; ++layer) {
    ...
    weight_offset += layer_config->filter_width * layer_config->filter_height *
                     layer_config->in_channels * layer_config->out_channels;
...
TEST_F(CNNTest, TestSoftsignMultiLayerMultiKernel) {
  ...
  for (int layer = 0; layer < cnn_config.num_layers; ++layer) {
    ...
    ASSERT_TRUE(layer_config->weights != 0);
    ASSERT_TRUE(layer_config->bias != 0);
  }
