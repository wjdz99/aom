...
class CNNTest : public ::testing::Test {
 protected:
  static const CNNTestConfig SetupCNNTest(
    ...
    const size_t image_size = sizeof(image_width) * image_width * image_height;
  ...
  static const CNNTestConfig SetupTestSoftsignMultiLayerMultiKernel() {
    int image_width = 8;
    int image_height = 8;
    int layers = 3;
    int is_residue = 0;
    float inputs[64] = { 0.517f, 0.505f, 0.769f, 0.537f, 0.55f,  0.264f, 0.991f,
...
TEST_F(CNNTest, TestNonActivationSingleLayerSingleKernel) {
  ...
  for (int i = 0; i < image_size; ++i) {
    EXPECT_LE(fabsf(cnn_test_config.expected_tensor[i] - output_tensor[i]), 1);
    int truncated_output = (int)output_tensor[i];
    mean_squared_error +=
        SQR(cnn_test_config.expected_tensor[i] - truncated_output);
  }
...
TEST_F(CNNTest, TestRELUMultiLayerMultiKernel) {
  ...
  // Because the expected output are ints, we don't care about residuals gained
...
TEST_F(CNNTest, TestSoftsignMultiLayerMultiKernel) {
  ...
  // Because the expected output are floats in [0,1], we care about
  // the actual floating point values.
  double mean_squared_error = 0;
  for (int i = 0; i < image_size; ++i) {
    EXPECT_LE(fabsf(cnn_test_config.expected_tensor[i] - output_tensor[i]), 1);
    mean_squared_error +=
        SQR(cnn_test_config.expected_tensor[i] - output_tensor[i]);
  }
