...
if [ -z "${AOM_TEST_TOOLS_COMMON_SH}" ];
then AOM_TEST_TOOLS_COMMON_SH = included
...
test_end() {
  if
    ["$1" != "${AOM_TOOL_TEST}"];
  then echo "FAIL completed test mismatch!." echo "  completed test: ${1}" echo
            "  active test: ${AOM_TOOL_TEST}." return 1 fi AOM_TOOL_TEST =
                '<unset>'
}
...
#to the cfg pointer.
  cmake_config=$(awk -F ' = ' '/cfg/ { print $NF; exit }' "${aom_config_c}" \
    | sed -e s/\"// -e s/\"\;//)
  echo cmake generated via command: cmake path/to/aom ${
    cmake_config}
}
...
cleanup() {
  if
    [-n "${AOM_TOOL_TEST}"] && ["${AOM_TOOL_TEST}" != '<unset>'];
  then echo "FAIL: $AOM_TOOL_TEST" fi if["${AOM_TEST_PRESERVE_OUTPUT}" = "yes"];
  then return fi if[-n "${AOM_TEST_OUTPUT_DIR}"] &&
      [-d "${AOM_TEST_OUTPUT_DIR}"];
  then rm - rf "${AOM_TEST_OUTPUT_DIR}" fi
}
...
source_version() {
  ...
  then(cd "$(dirname " $ { 0 } ")" git describe) else cmake_version fi
}

#Echoes warnings to stdout when source version and CMake build generated
#version are out of sync.
check_version_strings() {
  cmake_version = $(cmake_version) source_version = $(source_version)

      if["${cmake_version}" != "${source_version}"];
  then echo
      "Warning: version has changed since last cmake run." vlog
      "  cmake version: ${cmake_version} version now: ${source_version}" fi
}

#$1 is the name of an environment variable containing a directory name to
#test.
test_env_var_dir() {
  local dir = $(eval echo "\${$1}") if[!-d "${dir}"];
  then elog "'${dir}': No such directory" elog
            "The $1 environment variable must be set to a valid "
            "directory." return 1 fi
}
...
verify_aom_test_environment() {
  test_env_var_dir "LIBAOM_BIN_PATH" && test_env_var_dir "LIBAOM_CONFIG_PATH" &&
...
aom_config_option_enabled() {
  aom_config_option = "${1}" aom_config_file =
      "${LIBAOM_CONFIG_PATH}/config/aom_config.h" config_line =
          $(grep
            "${aom_config_option}"
            "${aom_config_file}") if echo "${config_line}" |
          egrep - q '1$';
  then echo yes fi
}
...
is_windows_target() {
  if test_configuration_target
    | grep - q - e win32 - e win64 > / dev / null 2 > &1;
  then echo yes fi
}
...
aom_tool_path() {
  ...
  do
    if
      [-x "${tool_path}"] && [-f "${tool_path}"];
  then echo "${tool_path}" return 0 fi done
...
aom_tool_available() {
  local tool_name = "$1" local tool =
      "${LIBAOM_BIN_PATH}/${tool_name}${AOM_TEST_EXE_SUFFIX}"[-x "${tool}"] &&
      echo yes
}
...
aomenc_encode_test_fast_params() {
  ...
  --lag - in - frames = 0 --test - decode = fatal "
}
...
#Tell grep to invert matches using the - v argument.
      exclude = '-v' else unset exclude fi
  ...
  do
    if echo
      "${s}" | egrep - q $ { exclude }
  "${filter}" > / dev / null 2 > &1;
  then filtered_strings =
      "${filtered_strings} ${s}" fi done else filtered_strings =
          "${strings}" fi echo "${filtered_strings}"
}
...
run_tests() {
  local env_tests = "verify_aom_test_environment $1" local tests_to_filter =
      "$2" local test_name = "${AOM_TEST_NAME}"

      if[-z "${test_name}"];
  then test_name = "$(basename " $ { 0 %.* }
  ")" fi

      if["${AOM_TEST_RUN_DISABLED_TESTS}" != "yes"];
  then
#Filter out DISABLED tests.
      tests_to_filter =
          $(filter_strings "${tests_to_filter}" ^ DISABLED exclude) fi

      if[-n "${AOM_TEST_FILTER}"];
  then
#Remove tests not matching the user's filter.
      tests_to_filter =
          $(filter_strings "${tests_to_filter}" ${ AOM_TEST_FILTER }) fi

#User requested test listing : Dump test names and return.
      if["${AOM_TEST_LIST_TESTS}" = "yes"]; then
    for test_name in $tests_to_filter;
  do
      echo ${test_name}
    done
    return
...
#Run tests.
  ...
  do
    test_begin
        "${test}" vlog
        "  RUN  ${test}"
        "${test}" vlog "  PASS ${test}" test_end "${test}" done

            local tested_config =
                "$(test_configuration_target) @ $(source_version)" echo
                "${test_name}: Done, all tests pass for ${tested_config}."
}
...
aom_test_check_environment() {
  if
    [-z "${LIBAOM_BIN_PATH}"] || [-z "${LIBAOM_CONFIG_PATH}"] ||
        [-z "${LIBAOM_TEST_DATA_PATH}"];
  then return 1 fi
}
...
yuv_raw_input() {
  echo "" $ { YUV_RAW_INPUT }
  "
      --width = "${YUV_RAW_INPUT_WIDTH}" --height =
...
encode_yuv_raw_input_av1() {
  ...
  then local output = "$1" local encoder =
      "$(aom_tool_path aomenc)" shift eval "${encoder}" $(yuv_raw_input)
...
AOM_TEST_PRESERVE_OUTPUT=${AOM_TEST_PRESERVE_OUTPUT:
  ...
  if
    ["$(is_windows_target)" = "yes"];
  then AOM_TEST_EXE_SUFFIX = ".exe" fi
#Variables shared by tests.
      AV1_ENCODE_CPU_USED =
  ${ AV1_ENCODE_CPU_USED: -1 } AV1_ENCODE_TEST_FRAME_LIMIT =
  ${ AV1_ENCODE_TEST_FRAME_LIMIT: -5 } AV1_IVF_FILE =
      "${AV1_IVF_FILE:-${AOM_TEST_OUTPUT_DIR}/av1.ivf}" AV1_OBU_ANNEXB_FILE =
          "${AV1_OBU_ANNEXB_FILE:-${AOM_TEST_OUTPUT_DIR}/"
          "av1.annexb.obu}" AV1_OBU_SEC5_FILE =
              "${AV1_OBU_SEC5_FILE:-${AOM_TEST_OUTPUT_DIR}/"
              "av1.section5.obu}" AV1_WEBM_FILE =
                  "${AV1_WEBM_FILE:-${AOM_TEST_OUTPUT_DIR}/av1.webm}"

      YUV_RAW_INPUT =
          "${LIBAOM_TEST_DATA_PATH}/"
          "hantro_collage_w352h288.yuv" YUV_RAW_INPUT_WIDTH =
              352 YUV_RAW_INPUT_HEIGHT = 288

      Y4M_NOSQ_PAR_INPUT =
          "${LIBAOM_TEST_DATA_PATH}/"
          "park_joy_90p_8_420_a10-1.y4m" Y4M_720P_INPUT =
...
#Setup a trap function to clean up after tests complete.
  ...
  ") test configuration:
      LIBAOM_BIN_PATH = ${ LIBAOM_BIN_PATH } LIBAOM_CONFIG_PATH =
          ${
            LIBAOM_CONFIG_PATH
          } LIBAOM_TEST_DATA_PATH = ${ LIBAOM_TEST_DATA_PATH } AOM_TEST_EXE_SUFFIX =
              ${
                AOM_TEST_EXE_SUFFIX
              } AOM_TEST_FILTER = ${ AOM_TEST_FILTER } AOM_TEST_LIST_TESTS =
                  ${
                    AOM_TEST_LIST_TESTS
                  } AOM_TEST_OUTPUT_DIR = ${ AOM_TEST_OUTPUT_DIR } AOM_TEST_PREFIX =
                      ${ AOM_TEST_PREFIX } AOM_TEST_PRESERVE_OUTPUT =
                          ${
