...
class RcInterfaceTest
    : public ::libaom_test::EncoderTest,
      public ::libaom_test::CodecTestWith2Params<int, aom_rc_mode> {
 public:
  RcInterfaceTest()
 ...
 protected:
  ...
  virtual void PreEncodeFrameHook(libaom_test::VideoSource *video,
    ...
    if (video->frame() == 0) {
      ...
      //   encoder->Control(AV1E_SET_RTC_EXTERNAL_RATECTRL, 1);
    }
    frame_params_.frame_type =
        video->frame() % key_interval_ == 0 ? KEY_FRAME : INTER_FRAME;
    if (rc_cfg_.rc_mode == AOM_CBR && frame_params_.frame_type == INTER_FRAME) {
 ...
 private:
  void SetConfig(aom_rc_mode rc_mode) {
    rc_cfg_.width = 1280;
    rc_cfg_.height = 720;
  ...
  int aq_mode_;
  int key_interval_;
  aom::AV1FrameParamsQpRTC frame_params_;
  bool encoder_exit_;
};
...
// AV1_INSTANTIATE_TEST_SUITE(RcInterfaceSvcTest, ::testing::Values(0, 3));
}  // namespace
