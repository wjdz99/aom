  ...
  void PreEncodeFrameHook(libaom_test::VideoSource *video,
    ...
    if (dynamic_temporal_layers_) {
      ...
      if (superframe_cnt_ == 100 && layer_id_.spatial_layer_id == 0) {
        ...
        SetConfigSvc(3, 3);
        encoder->Control(AV1E_SET_SVC_PARAMS, &svc_params_);
        // In the fixed SVC mode (which is what is used in this test) a Key frame
        // is required here on SL0 since 120p will try to predict from LAST which
        // was the 480p, so decoder will throw an error (must be smaller than 4x4). 
