...
#define DO_LOBD_TEST \
  CONFIG_CONVOLVE_ROUND && !CONFIG_COMPOUND_ROUND && HAVE_SSE4_1
...
template <int N, typename src_pixel_t, int M, typename dst_pixel_t>
class TestImage {
 public:
  TestImage(int w, int h, int bd);
  ...
  dst_pixel_t *GetDstData(bool ref, bool borders) {
    dst_pixel_t *block = &dst_data_[ref ? 0 : GetBlockSize(true)];
 ...
 private:
  ...
  std::vector<dst_pixel_t> dst_data_;
};

template <int N, typename src_pixel_t, int M, typename dst_pixel_t>
TestImage<N, src_pixel_t, M, dst_pixel_t>::TestImage(int w, int h, int bd)
    : w_(w), h_(h), bd_(bd) {
  assert(bd < 16);
  assert(bd <= N && N == 8 * sizeof(src_pixel_t));
  assert(bd <= M && M == 8 * sizeof(dst_pixel_t));
...
template <int N, typename src_pixel_t, int M, typename dst_pixel_t>
void TestImage<N, src_pixel_t, M, dst_pixel_t>::Initialize(ACMRandom *rnd) {
  PrepBuffers(rnd, w_, h_, src_stride_, bd_, false, &src_data_[0]);
  PrepBuffers(rnd, w_, h_, dst_stride_, bd_, true, &dst_data_[0]);
}

template <int N, typename src_pixel_t, int M, typename dst_pixel_t>
void TestImage<N, src_pixel_t, M, dst_pixel_t>::Check(
    const char *test_name) const {
  // If memcmp returns 0, there's nothing to do.
  const int nelts = GetBlockSize(true);
  const dst_pixel_t *ref_dst = &dst_data_[0];
  const dst_pixel_t *tst_dst = &dst_data_[nelts];

  if (0 == memcmp(ref_dst, tst_dst, M / 8 * nelts)) return;
  ...
  for (int r = 0; r < h_ + 2 * vpad; ++r) {
    for (int c = 0; c < w_ + 2 * hpad; ++c) {
      dst_pixel_t ref_value = ref_dst[r * stride + c];
      dst_pixel_t tst_value = tst_dst[r * stride + c];
...
class ConvolveScaleTest {
 public:
  typedef void (*convolve_fun_t)(const src_pixel_t *src, int src_stride,
                                 dst_pixel_t *dst, int dst_stride, int w, int h,
 ...
 protected:
  void Init(int w, int h, int ntaps_x, int ntaps_y, int bd, bool avg) {
    width_ = w;
    height_ = h;
    ntaps_x_ = ntaps_x;
    ntaps_y_ = ntaps_y;
    ...
    convolve_params_ = get_conv_params(0, avg != false, 0);

    delete image_;
    image_ = new TestImage<N, src_pixel_t, M, dst_pixel_t>(w, h, bd);
  }
  ...
  TestImage<N, src_pixel_t, M, dst_pixel_t> *image_;
...
#if DO_LOBD_TEST
typedef ConvolveScaleTest<8, uint8_t, 32, CONV_BUF_TYPE> LowBDBaseTest;
typedef tuple<LowBDBaseTest::convolve_fun_t, BlockDimension, uint16_t, uint16_t,
              bool>
    LowBDParams;

// Test parameter list:
//  <fun, dims, ntaps_x, ntaps_y, avg>
class LowBDConvolveScaleTest : public ::testing::TestWithParam<LowBDParams>,
                               public LowBDBaseTest {
 public:
  virtual ~LowBDConvolveScaleTest() {}
  virtual void SetUp() {
    convolve_ = GET_PARAM(0);

    BlockDimension block = GET_PARAM(1);
    int w = std::tr1::get<0>(block);
    int h = std::tr1::get<1>(block);
    int ntaps_x = GET_PARAM(2);
    int ntaps_y = GET_PARAM(3);
    bool avg = GET_PARAM(4);

    Init(w, h, ntaps_x, ntaps_y, 8, avg);
  }
 ...
 protected:
  void Run() {
    ...
    for (int i = 0; i < TEST_ITERS; ++i) {
      ...
      convolve_(tst_src, src_stride, tst_dst, dst_stride, width_, height_,
 ...
 private:
  LowBDBaseTest::convolve_fun_t convolve_;
};
#endif  // DO_LOBD_TEST
