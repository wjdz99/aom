...
enum ntaps_t { EIGHT_TAP, TEN_TAP, TWELVE_TAP };
int ntaps_to_int(ntaps_t ntaps) { return 8 + (int)ntaps * 2; }

// A 16-bit filter with a configurable number of taps.
struct TestFilter {
  void set(ntaps_t ntaps, bool backwards);
...
void TestFilter::set(ntaps_t ntaps, bool backwards) {
  int n = ntaps_to_int(ntaps);
  assert(8 <= n && n <= 12);
...
typedef tuple<BlockDimension, ntaps_t, ntaps_t, bool> BaseParams;
...
typedef tuple<lowbd_convolve_fun_t, BlockDimension, ntaps_t, ntaps_t, bool>
    LowBDParams;
...
class LowBDConvolveScaleTest
 ...
 public:
  ...
  void SetUp() {
    ...
    ntaps_t ntaps_x = GET_PARAM(2);
    ntaps_t ntaps_y = GET_PARAM(3);
    int bd = 8;
...
const ntaps_t kNTaps[] = { EIGHT_TAP, TEN_TAP, TWELVE_TAP };
...
typedef tuple<highbd_convolve_fun_t, BlockDimension, ntaps_t, ntaps_t, bool,
              int>
    HighBDParams;
