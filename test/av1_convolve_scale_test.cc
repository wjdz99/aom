...
#define DO_LOBD_TEST CONFIG_CONVOLVE_ROUND && !CONFIG_COMPOUND_ROUND && HAVE_SSE4_1
...
template<typename pixel_t>
void PrepBuffers (ACMRandom *rnd, int w, int h,
                  int stride, int bd, bool trash_edges, pixel_t *data)
{
  assert(rnd);
  pixel_t mask = (1 << bd) - 1;
...
typedef tuple<BlockDimension, uint16_t, uint16_t, bool> LowBDParams;

class LowBDConvolveScaleTest
    : public ::testing::TestWithParam<LowBDParams>,
      public ConvolveScaleTest<8, uint8_t, 32, CONV_BUF_TYPE> {
 public:
  virtual ~LowBDConvolveScaleTest() {}

  virtual void SetUp() {
    BlockDimension block = GET_PARAM(0);
    int w = std::tr1::get<0>(block);
    int h = std::tr1::get<1>(block);
    int ntaps_x = GET_PARAM(1);
    int ntaps_y = GET_PARAM(2);
 ...
 protected:
  void Run() {
    ...
    for (int i = 0; i < TEST_ITERS; ++i) {
      ...
      av1_convolve_2d_scale_sse4_1(tst_src, src_stride, tst_dst, dst_stride, width_,
