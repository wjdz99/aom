...
typedef std::tr1::tuple<hbddual_loop_op_t, hbddual_loop_op_t, int> hbddual_loop_param_t;
...
template<typename Pixel_t, int PIXEL_WIDTH_t = 16>
...
void call_dualfilter(uint16_t *s,DUAL_LOOP_PARAM, int bd, hbddual_loop_op_t op) {
...
typedef LoopTestParam<hbddual_loop_op_t, hbddual_loop_param_t> Loop8Test9Param_hbd;
  ...
  for (int i = 0; i < count_test_block; ++i) {\
    ...
    DECLARE_ALIGNED(16, const uint8_t,\
                    thresh[16]) = { tmp, tmp, tmp, tmp, tmp, tmp, tmp, tmp,\
                                    tmp, tmp, tmp, tmp, tmp, tmp, tmp, tmp };\
    InitInput<a>(s, ref_s, &rnd, *limit, mask_, p, i);\
    call_filter(ref_s + 8 + p * 8, p, blimit, limit, thresh, bit_depth_,ref_loopfilter_op_);\
    ASM_REGISTER_STATE_CHECK(\
        call_filter(s + 8 + p * 8, p, blimit, limit, thresh, bit_depth_,loopfilter_op_));\
    for (int j = 0; j < kNumCoeffs; ++j) {\
...
TEST_P(Loop8Test6Param_hbd, OperationCheck) {
  OPCHECK(uint16_t,16)
}
TEST_P(Loop8Test6Param_lbd, OperationCheck) {
  OPCHECK(uint8_t,8)
}
...
TEST_P(Loop8Test6Param_hbd, ValueCheck) {
  VALCHECK(uint16_t,16)
}
TEST_P(Loop8Test6Param_lbd, ValueCheck) {
  VALCHECK(uint8_t,8)
}
...
TEST_P(Loop8Test6Param_hbd, DISABLED_Speed) {
  SPEEDCHECK(uint16_t,16)
}
TEST_P(Loop8Test6Param_lbd, DISABLED_Speed) {
  SPEEDCHECK(uint8_t,8)
}
...
#define OPCHECKd(a,b) \
 ACMRandom rnd(ACMRandom::DeterministicSeed());\
  ...
  for (int i = 0; i < count_test_block; ++i) {\
    ...
    ASM_REGISTER_STATE_CHECK(call_dualfilter(s + 8 + p * 8, p, blimit0, limit0,\
...
TEST_P(Loop8Test9Param_hbd, OperationCheck) {
  OPCHECKd(uint16_t,16)
}
TEST_P(Loop8Test9Param_lbd, OperationCheck) {
  OPCHECKd(uint8_t,8)
}
...
TEST_P(Loop8Test9Param_hbd, ValueCheck) {
  VALCHECKd(uint16_t,16)
}
TEST_P(Loop8Test9Param_lbd, ValueCheck) {
  VALCHECKd(uint8_t,8)
}
...
#define SPEEDCHECKd(a,b)\
  ...
  for (int i = 0; i < count_test_block; ++i) {\
    call_dualfilter(s + 8 + p * 8, p, blimit0, limit0, thresh0, blimit1, limit1,\
                   thresh1,bit_depth_,loopfilter_op_);\
  }\

TEST_P(Loop8Test9Param_hbd, DISABLED_Speed) {
  SPEEDCHECKd(uint16_t,16)
}
TEST_P(Loop8Test9Param_lbd, DISABLED_Speed) {
  SPEEDCHECKd(uint8_t,8)
}
...
#if CONFIG_DEBLOCK_13TAP
  make_tuple(&aom_lpf_horizontal_6_sse2, &aom_lpf_horizontal_6_c,
             8),
