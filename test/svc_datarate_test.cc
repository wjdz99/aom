 ...
 protected:
  ...
  virtual void PreEncodeFrameHook(::libaom_test::VideoSource *video,
    ...
    if (number_spatial_layers_ == 2)
      spatial_layer_id = (svc_frame_cnt_ % 2 == 0) ? 0 : 1;
  ...
  virtual void EndPassHook(void) {
    duration_ = ((last_pts_ + 1) * timebase_ ) / number_spatial_layers_;
    for (int i = 0; i < number_temporal_layers_ * number_spatial_layers_;
         i++) {
  ...
  virtual int set_layer_pattern(int frame_cnt, aom_svc_layer_id_t *layer_id,
    ...
    } else if (number_temporal_layers_ == 0 && number_spatial_layers_ == 2) {
      layer_id->temporal_layer_id = 0;
  ...
  virtual void BasicRateTargetingSVC1TL2SLTest() {
    ...
    framerate_factor_[0] = 1;
    ASSERT_NO_FATAL_FAILURE(RunLoop(&video));
    for (int i = 0; i < number_temporal_layers_ * number_spatial_layers_;
         i++) {
      ASSERT_GE(effective_datarate_tl[i], target_layer_bitrate_[i] * 0.80)
          << " The datarate for the file is lower than target by too much!";
      // FIX THIS THRESHOLD: 4.0 IS TOO LARGE...SOME ISSUE WITH RC.
      ASSERT_LE(effective_datarate_tl[i], target_layer_bitrate_[i] * 4.0)
          << " The datarate for the file is greater than target by too much!";
    }
  }
  ...
  int framerate_factor_[AOM_MAX_TS_LAYERS];
};
