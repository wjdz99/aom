 ...
 protected:
  ...
  virtual void PreEncodeFrameHook(::libaom_test::VideoSource *video,
    ...
    if (intra_only_ == 1 && frame_to_start_decoding_ > 0) {
      // Set an Intra-only frame on SL0 at frame_to_start_decoding_.
      // In order to allow decoding to start on SL0 in mid-sequence we need to
      // make sure to refresh the slots used on SL0 stream, which is 0 and 3
      // for these patterns. The other slots are used for SL > 0 layers.
...
// with Intra-only frame inserted in the stream. And start decoding
// on SL0 at sync frame in mid-sequence.
