...
class DatarateTestSVC
 ...
 protected:
    ...
    if (number_temporal_layers_ == 2 && number_spatial_layers_ == 1) {
      ...
      if (is_key_frame) {
        if (layer_id->spatial_layer_id == 0) {
          ...
          ref_frame_config->ref_idx[LAST_FRAME] = 2;
          ref_frame_config->ref_idx[GOLDEN_FRAME] = 3;
          ref_frame_config->refresh[2] = 1;
          ref_frame_config->refresh[3] = 1;
        } else if (layer_id->spatial_layer_id == 2) {
          ...
          // This will get set to Intra-only frame internally.
          ref_frame_config->ref_idx[LAST_FRAME] = 4;
          ref_frame_config->ref_idx[GOLDEN_FRAME] = 5;
          ref_frame_config->refresh[4] = 1;
          ref_frame_config->refresh[5] = 1;
        }
      } else if (superframe_cnt_ % 4 == 0) {
        ...
        if (layer_id->spatial_layer_id == 0) {  // SL0
          ...
          // Update slot 0 (LAST).
          ref_frame_config->reference[LAST_FRAME] = 1;
          for (i = 0; i < INTER_REFS_PER_FRAME; i++)
        ...
        if (layer_id->spatial_layer_id == 0) {  // SL0
          ...
          ref_frame_config->ref_idx[LAST_FRAME] = 3;
        } else if (layer_id->spatial_layer_id == 2) {  // SL2
          ...
          ref_frame_config->ref_idx[LAST_FRAME] = 5;
        }
      }
    }
  ...
  virtual void BasicRateTargetingSVC3TL3SLSimulcast() {
    ...
    ResetModel();
    simulcast_mode_ = 1;
    frame_to_start_decoding_ = 150;  // Key frame interval.
    ...
    // Only top spatial layer (SL2) is decoded, starting frame SL0 keyframe at
    // frame 150, so there (300 - 150)/2 = 75 non-reference frames, so mismatch
    // is 75.
    EXPECT_EQ((int)GetMismatchFrames(), 75);
  }
