...
const EncodeParameters kAV1EncodeParameterSet[] = {
  { 1, /*AOM_CR_STUDIO_RANGE, AOM_CS_BT_601,*/ { 0, 0 } },
  { 0, /*AOM_CR_FULL_RANGE, AOM_CS_BT_709,*/ { 0, 0 } },
  { 1, /*AOM_CR_FULL_RANGE, AOM_CS_BT_2020,*/ { 0, 0 } },
  { 0, /*AOM_CR_STUDIO_RANGE, AOM_CS_UNKNOWN,*/ { 640, 480 } },
};
...
class AVxEncoderParmsGetToDecoder
 ...
 protected:
  ...
  virtual void DecompressedFrameHook(const aom_image_t &img,
    ...
    (void)pts;
    if (encode_parms.render_size[0] > 0 && encode_parms.render_size[1] > 0) {
      EXPECT_EQ(0, encode_parms.render_size[0] - (int)img.r_w);
      EXPECT_EQ(0, encode_parms.render_size[1] - (int)img.r_h);
    }
  }
