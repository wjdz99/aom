...
class BlendA64MaskTest8B : public BlendA64MaskTest<F8B, uint8_t, uint8_t> {
 protected:
  void Execute(const uint8_t *p_src0, const uint8_t *p_src1, int run_times) {
    ...
    if (run_times > 10) {
      printf("%3dx%-3d subx %d suby %d :%7.2f/%7.2fns", h_, w_, subx_, suby_,
             time1, time2);
      printf("(%3.2f)\n", time1 / time2);
    }
  }
...
TEST_P(BlendA64MaskTest8B, Speed) {
  for (int bsize = 0; bsize < BLOCK_SIZES_ALL; ++bsize) {
    ...
    RunOneTest(bsize, 1, 1, 10000000);
    RunOneTest(bsize, 1, 0, 10000000);
    RunOneTest(bsize, 0, 1, 10000000);
    RunOneTest(bsize, 0, 0, 10000000);
  }
}
#if HAVE_SSE4_1
