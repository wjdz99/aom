...
TEST(scan_test, sort_prob) {
  int size = 7;
  uint32_t prob[7] = {4, 2, 1, 6, 3, 8, 2};
  uint32_t ref_prob[7] = {8, 6, 4, 3, 2, 2, 1};
  sort_prob(prob, 0, size);
  for (int i = 0; i < size; ++i) {
    EXPECT_EQ(ref_prob[i], prob[i]);
  }
}
...
TEST(scan_test, update_scan_order) {
  ...
  int16_t scan[16];
  int16_t iscan[16];
  int16_t ref_iscan[16] = {0, 1, 2,  6,  3, 4,  5,  10,
