...
class PostProcFiltersTest
    : public ::libaom_test::CodecTestWith2Params<unsigned int, unsigned int>,
      public ::libaom_test::EncoderTest {
 protected:
  PostProcFiltersTest()
      : EncoderTest(GET_PARAM(0)), cpu_used_(GET_PARAM(1)), bd_(GET_PARAM(2)) {
  ...
  virtual void PreEncodeFrameHook(::libaom_test::VideoSource *video,
    ...
    if (frame_number_ == 0) {
      ...
      encoder->Control(AOME_SET_CQ_LEVEL, cqlevel_);
    }
  ...
  virtual void PostEncodeFrameHook(::libaom_test::Encoder *encoder) {
    ...
    if (!set_skip_postproc_filtering_) {
      ...
      // Null will be returned if we query the recon frame when
      // AV1E_SET_SKIP_POSTPROC_FILTERING is set to 1.
      if (frame_number_ < 10) ASSERT_EQ(img_enc, nullptr);
      // Recon frame cannot be null when AV1E_SET_SKIP_POSTPROC_FILTERING is set
      // to zero.
      else if (frame_number_ < 20)
        ASSERT_NE(img_enc, nullptr);
      else
  ...
  void DoTest() {
    ...
    apply_postproc_filters_md5_enc = md5_enc_;
    md5_enc_.clear();
    ...
    toggle_apply_postproc_filters_md5_enc = md5_enc_;
    md5_enc_.clear();
 ...
 private:
  static constexpr int frames_ = 30;
  static constexpr unsigned int cqlevel_ = 18;
  unsigned int cpu_used_;
  unsigned int bd_;
};
