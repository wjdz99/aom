...
typedef void (*ComputeMeanStddevFunc)(const unsigned char *frame, int stride,
...
void AV1CornerMatchTest::RunCheckOutput() {
  ...
  for (int i = 0; i < num_iters; ++i) {
    ...
    aom_compute_mean_stddev_c(input1.get(), w, x1, y1, &c_mean1,
                              &c_one_over_stddev1);
    aom_compute_mean_stddev_c(input2.get(), w, x2, y2, &c_mean2,
                              &c_one_over_stddev2);

    double simd_mean1, simd_one_over_stddev1, simd_mean2, simd_one_over_stddev2;
    target_compute_mean_stddev_func(input1.get(), w, x1, y1, &simd_mean1,
                                    &simd_one_over_stddev1);
    target_compute_mean_stddev_func(input2.get(), w, x2, y2, &simd_mean2,
