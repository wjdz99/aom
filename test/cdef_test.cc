...
namespace {
    ...
    (HAVE_SSE2 || HAVE_SSSE3 || HAVE_SSE4_1 || HAVE_AVX2 || HAVE_NEON)
void cdef_filter_block_highbd_empty(uint16_t *dst16, int dstride,
...
#if !CONFIG_AV1_HIGHBITDEPTH
  if (depth != 8) {
    /* skip test when highbd is disabled */
    return;
  }
#endif
  const int size = 8;
...
#else
                       ::testing::Values(&cdef_filter_block_highbd_empty),
                       ::testing::Values(&cdef_filter_block_highbd_empty),
#endif
