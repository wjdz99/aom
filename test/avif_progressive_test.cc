...
TEST(AVIFProgressiveTest, DimensionChangeLargeImageSlowSpeedDifferentImage) {
  ::libaom_test::Y4mVideoSource src("dog_1080p.y4m", 0, 2);  // LargeImage
  src.OpenSource();
  src.ReadSourceToStart();
  ...
  EXPECT_EQ(AOM_CODEC_OK,
            aom_codec_control(&enc, AOME_SET_NUMBER_SPATIAL_LAYERS, 2));
  EXPECT_EQ(AOM_CODEC_OK, aom_codec_control(&enc, AOME_SET_CPUUSED, 2));  // SlowSpeed
  EXPECT_EQ(AOM_CODEC_OK,
            aom_codec_control(&enc, AV1E_SET_COLOR_RANGE, AOM_CR_FULL_RANGE));
  EXPECT_EQ(AOM_CODEC_OK,
            aom_codec_control(&enc, AOME_SET_TUNING, AOM_TUNE_SSIM));
