...
TEST(AVIFProgressiveTest, DimensionDecreasing) {
  ...
  EXPECT_EQ(AOM_CODEC_OK,
            aom_codec_control(&enc, AV1E_SET_COLOR_RANGE, AOM_CR_FULL_RANGE));
  EXPECT_EQ(AOM_CODEC_OK,
            aom_codec_set_option(&enc, "cq-level", "30"));
  EXPECT_EQ(AOM_CODEC_OK,
            aom_codec_set_option(&enc, "tune", "ssim"));
  ...
  // Second frame
  EXPECT_EQ(&img, aom_img_wrap(&img, AOM_IMG_FMT_I420, 64, 64, 1,
                               buffer.data()));
  cfg.g_w = 64;
  cfg.g_h = 64;
  EXPECT_EQ(AOM_CODEC_OK, aom_codec_enc_config_set(&enc, &cfg));
