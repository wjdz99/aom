...
// progressive AVIF image in libavif's ProgressiveTest.DimensionChange.
...
TEST(AVIFProgressiveTest, DISABLED_DimensionChange) {
  ...
  EXPECT_EQ(AOM_CODEC_OK, aom_codec_control(&enc, AV1E_SET_COLOR_PRIMARIES,
                                            AOM_CICP_CP_UNSPECIFIED));
  EXPECT_EQ(AOM_CODEC_OK,
            aom_codec_control(&enc, AV1E_SET_TRANSFER_CHARACTERISTICS,
                              AOM_CICP_TC_UNSPECIFIED));
  EXPECT_EQ(AOM_CODEC_OK, aom_codec_control(&enc, AV1E_SET_MATRIX_COEFFICIENTS,
                                            AOM_CICP_MC_UNSPECIFIED));
  EXPECT_EQ(AOM_CODEC_OK,
            aom_codec_control(&enc, AV1E_SET_CHROMA_SAMPLE_POSITION,
                              AOM_CSP_UNKNOWN));
