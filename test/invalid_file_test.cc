...
class InvalidFileTest : public ::libaom_test::DecoderTest,
 ...
 protected:
  InvalidFileTest()
      : DecoderTest(GET_PARAM(0)), res_file_(NULL), frame_number_(0) {}
  ...
  virtual bool HandleDecodeResult(const aom_codec_err_t res_dec,
    ...
    if (input.threads > 1) {
      ...
      EXPECT_TRUE(res_dec == expected_res_dec ||
          ...
          << "Results don't match: frame number = " << frame_number_ << ". ("
          << decoder->DecodeError() << "). Expected: " << expected_res_dec
          << " or " << AOM_CODEC_CORRUPT_FRAME;
    } else {
