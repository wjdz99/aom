...
TEST(SearchWienerTest, DISABLED_12bitSignedIntegerOverflowInLinsolveWiener) {
  ...
  // allocated with width and heigt aligned to a multiple of 2. Align the width
  // to a multiple of 2 so that the stride set by aom_img_wrap() is correct. It
  // is not necessary to align the height to a multiple of 2 because
  // aom_codec_encode() will only read cfg.g_h rows.
