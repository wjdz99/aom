...
static void compute_stats_win_opt_c(int wiener_win, const uint8_t *dgd,
  ...
  int32_t M_row_i32[WIENER_WIN][WIENER_WIN] = { { 0 } };
  int32_t H_row_i32[WIENER_WIN2][WIENER_WIN * 8] = { { 0 } };
  int32_t sumX = 0;
  ...
  for (i = v_start; i < v_end; i = i + downsample_factor) {
    ...
    sumX_row_i32 = 0;
    memset(M_row_i32, 0, sizeof(int32_t) * WIENER_WIN2);
    memset(H_row_i32, 0, sizeof(int32_t) * WIENER_WIN2 * (WIENER_WIN * 8));
