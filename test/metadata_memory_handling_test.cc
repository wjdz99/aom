...
TEST(MetadataMemoryHandlingTest, AddMetadataToImage) {
  ...
  status = aom_add_metadata_to_img(NULL, OBU_METADATA_TYPE_ITUT_T35, data, 10);
  EXPECT_EQ(status, -1);
}
...
TEST(MetadataMemoryHandlingTest, RemoveMetadataFromImage) {
  ...
  status = aom_remove_metadata_from_img(NULL);
  EXPECT_EQ(status, -1);
}
...
TEST(MetadataMemoryHandlingTest, CopyMetadataToFrameBUffer) {
  ...
  metadata_array_2 = aom_metadata_array_alloc(0);
  ...
  // Expecting to delete one aom_metadata_t
  status = aom_metadata_array_free(metadata_array);
  EXPECT_EQ(status, 1);

  // Expecting to delete 0 aom_metadata_t
  status = aom_metadata_array_free(metadata_array_2);
  EXPECT_EQ(status, 0);
}