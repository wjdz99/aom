...
std::string print(uint8_t *a, int size) {
  ...
  for (int i = 0; i < size; i++) {
    char buf[3];
    sprintf(buf, "%02x", a[!CONFIG_BIG_ENDIAN ? size - 1 - i : i]);
    text += buf;
  }
...
void test_simd1(uint32_t iterations, uint32_t mask, uint32_t maskwidth,
  ...
  for (unsigned int count = 0; count < iterations && !error; count++) {
    ...
    // U64_V64
    else if (is_same<c_ret, uint64_t>::value && is_same<c_arg, c_v64>::value)
      error = compare_simd1<ret, arg, c_ret, c_arg>(
    ...
    // S64_V64
      ...
      error = compare_simd1<ret, arg, c_ret, c_arg>(
          ...
          (void *)ref_simd, ref_d, s);
    // U64_V128
    else if (is_same<c_ret, uint64_t>::value && is_same<c_arg, c_v128>::value)
      error = compare_simd1<ret, arg, c_ret, c_arg>(
...
void test_simd2(uint32_t iterations, uint32_t mask, uint32_t maskwidth,
  ...
  for (unsigned int count = 0; count < iterations && !error; count++) {
    ...
    // V128_V128V128
    else if (is_same<c_ret, c_v128>::value && is_same<c_arg1, c_v128>::value &&
    ...
    // U32_V64V64
    else if (is_same<c_ret, uint32_t>::value && is_same<c_arg1, c_v64>::value &&
             is_same<c_arg2, c_v64>::value)
      error = compare_simd2<ret, arg1, arg2, c_ret, c_arg1, c_arg2>(
    ...
    // U32_V128V128
             ...
             is_same<c_arg1, c_v128>::value && is_same<c_arg2, c_v128>::value)
      error = compare_simd2<ret, arg1, arg2, c_ret, c_arg1, c_arg2>(
