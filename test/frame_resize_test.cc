 ...
 public:
  ...
  void RunTest() {
    ...
    for (int level = 1; level < n_levels_; level++) {
      int width2 = (width_ >> level);
      int height2 = (height_ >> level);
...
class AV1ResizeXTest : public ::testing::TestWithParam<Resize_x_TestParams> {
 public:
  ...
  void RunTest() {
    ...
    for (int level = 1; level < n_levels_; ++level) {
      int width2 = (width_ >> level);
      resize_horz_dir_c(src_, width_, ref_dest_, height_, width2 << 1, width2);
  ...
  void SpeedTest() {
    ...
    for (int level = 1; level < n_levels_; ++level) {
      ...
      aom_usec_timer_start(&ref_timer);
      for (int j = 0; j < kIters; ++j) {
        resize_horz_dir_c(src_, width_, ref_dest_, height_, width2 << 1,
                          width2);
      }
      aom_usec_timer_mark(&ref_timer);
      const int64_t ref_time = aom_usec_timer_elapsed(&ref_timer);
