...
typedef void (*build_comp_diff_wtd_mask_func)(
    uint8_t *mask, DIFFWTD_MASK_TYPE mask_type, const uint8_t *src0,
    int src0_stride, const uint8_t *src1, int src1_stride, int h, int w);

typedef std::tuple<BLOCK_SIZE, build_comp_diff_wtd_mask_func>
...
::testing::internal::ParamGenerator<BuildCompDiffwtdMaskDParam> BuildParams(
    build_comp_diff_wtd_mask_func filter) {
...
class BuildCompDiffwtdMaskTest
 ...
 protected:
  ...
  void RunTest(build_comp_diff_wtd_mask_func test_impl, const int is_speed,
...
typedef void (*build_comp_diff_wtd_mask_highbd_func)(
    uint8_t *mask, DIFFWTD_MASK_TYPE mask_type, const uint8_t *src0,
    int src0_stride, const uint8_t *src1, int src1_stride, int h, int w,
    int bd);
...
class BuildCompDiffwtdMaskHighbdTest
 ...
 protected:
  ...
  void RunTest(build_comp_diff_wtd_mask_highbd_func test_impl,
               const int is_speed, const DIFFWTD_MASK_TYPE type) {
    const int sb_type = GET_PARAM(0);
    ...
    ACMRandom rnd(ACMRandom::DeterministicSeed());
    for (int i = 0; i < width * height; i++) {
      src0[i] = rnd.Rand16() & mask;
      src1[i] = rnd.Rand16() & mask;
    }
    const int run_times = is_speed ? (10000000 / (width + height)) : 1;
    aom_usec_timer timer;
    aom_usec_timer_start(&timer);
    for (int i = 0; i < run_times; ++i) {
      ...
      av1_build_compound_diffwtd_mask_highbd_c(mask_ref, type, src0_8, width,
                                               src1_8, width, height, width, bd);
    }
    const double t1 = get_time_mark(&timer);
    aom_usec_timer_start(&timer);
    for (int i = 0; i < run_times; ++i) {
      uint8_t * src0_8 = CONVERT_TO_BYTEPTR(src0);
      uint8_t * src1_8 = CONVERT_TO_BYTEPTR(src1);
      test_impl(mask_test, type, src0_8, width, src1_8, width, height, width, bd);
    }
    const double t2 = get_time_mark(&timer);
    if (is_speed) {
      printf("mask %d %3dx%-3d:%7.2f/%7.2fns", type, width, height, t1, t2);
...
typedef void (*build_comp_diff_wtd_mask_d16_func)(
    uint8_t *mask, DIFFWTD_MASK_TYPE mask_type, const CONV_BUF_TYPE *src0,
    int src0_stride, const CONV_BUF_TYPE *src1, int src1_stride, int h, int w,
    ConvolveParams *conv_params, int bd);

typedef std::tuple<int, build_comp_diff_wtd_mask_d16_func, BLOCK_SIZE>
    BuildCompDiffwtdMaskD16Param;

#if HAVE_SSE4_1 || HAVE_AVX2 || HAVE_NEON
::testing::internal::ParamGenerator<BuildCompDiffwtdMaskD16Param> BuildParams(
    build_comp_diff_wtd_mask_d16_func filter) {
...
class BuildCompDiffwtdMaskD16Test
 ...
 protected:
  ...
  void RunCheckOutput(build_comp_diff_wtd_mask_d16_func test_impl) {
  ...
  void RunSpeedTest(build_comp_diff_wtd_mask_d16_func test_impl,
