...
TEST_P(AV1ResolutionChange, UnformattedInput) {
  struct FrameSize {
    unsigned int width;
    unsigned int height;
  };
  ...
  cfg.g_lag_in_frames = 0;
  cfg.rc_end_usage = rc_mode_;
  // For unformatted input, if max frame sizes are not set, the first encoded
  // frame size will be locked as the max frame size, and the encoder will
  // identify it as unsupported bitstream.
  int max_width = cfg.g_w;  // default frame width
  int max_height = cfg.g_h; // default frame height
  for (const auto &frame_size : kFrameSizes) {
    max_width = frame_size.width > max_width ? frame_size.width : max_width;
    max_height = frame_size.height > max_height ? frame_size.height : max_height;
  }
