...
TEST_P(AV1ResolutionChange, InvalidRefSize) {
  ...
  // TODO(jzern): Setting g_w and g_h shouldn't be necessary due to the call to
  // aom_codec_enc_config_set() at the start of the loop. Without this,
  // however, there will be some heap overflows due to the default being a
  ...
  EXPECT_EQ(frame_count,
            kNumFramesPerResolution *
                static_cast<int>(sizeof(kFrameSizes) / sizeof(kFrameSizes[0])));
}

INSTANTIATE_TEST_SUITE_P(
    Realtime, AV1ResolutionChange,
    ::testing::Combine(::testing::Values(AOM_USAGE_REALTIME),
                       ::testing::Values(AOM_CBR, AOM_VBR),
                       ::testing::Range(6, 11)));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Combine(::testing::Values(AOM_USAGE_GOOD_QUALITY),
                       ::testing::Values(AOM_CBR, AOM_VBR, AOM_Q),
                       ::testing::Range(2, 6)));
INSTANTIATE_TEST_SUITE_P(
