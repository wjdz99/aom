...
#decode_with_drops.
decode_with_drops() {
  local decoder = "$(aom_tool_path decode_with_drops)" local input_file =
      "$1" local codec = "$2" local output_file =
          "${AOM_TEST_OUTPUT_DIR}/decode_with_drops_${codec}" local drop_mode =
              "$3"
...
DISABLED_decode_with_drops_av1() {
  ...
  then local file = "${AV1_IVF_FILE}" if[!-e "${AV1_IVF_FILE}"];
