...
TEST(AomLeb128, EncodeTest) {
  ...
  ASSERT_EQ(0, aom_uleb_encode(test_value, kWriteBufferSize, &write_buffer[0],
                               &bytes_written));
  ASSERT_EQ(3u, bytes_written);
...
TEST(AomLeb128, EncDecTest) {
  ...
  uint8_t write_buffer[kWriteBufferSize] = { 0 };
  size_t bytes_written = 0;
  ASSERT_EQ(0, aom_uleb_encode(value, kWriteBufferSize, &write_buffer[0],
                               &bytes_written));
  ASSERT_EQ(3u, bytes_written);
...
TEST(AomLeb128, FixedSizeEncDecTest) {
  ...
  size_t bytes_written = 0;
  ASSERT_EQ(
      0, aom_uleb_encode_fixed_size(value, kWriteBufferSize, kWriteBufferSize,
                                    &write_buffer[0], &bytes_written));
  ASSERT_EQ(4u, bytes_written);
  uint32_t decoded_value = 0;
  aom_uleb_decode(&write_buffer[0], bytes_written, &decoded_value);
  ASSERT_EQ(value, decoded_value);
}
