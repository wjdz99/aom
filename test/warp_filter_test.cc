...
static int32_t random_param(const ACMRandom &rnd, int bits) {
...
static void generate_model(const ACMRandom &rnd, int32_t *mat, int32_t *alpha,
                           int32_t *beta, int32_t *gamma, int32_t *delta) {
  while (1) {
    mat[0] = random_param(rnd, WARPEDMODEL_PREC_BITS + 6);
    mat[1] = random_param(rnd, WARPEDMODEL_PREC_BITS + 6);
    mat[2] = (random_param(rnd, WARPEDMODEL_PREC_BITS - 3)) +
             (1 << WARPEDMODEL_PREC_BITS);
    mat[3] = random_param(rnd, WARPEDMODEL_PREC_BITS - 3);
    // 50/50 chance of generating ROTZOOM vs. AFFINE models
    if (rnd.Rand8() & 1) {
      // AFFINE
      mat[4] = random_param(rnd, WARPEDMODEL_PREC_BITS - 3);
      mat[5] = (random_param(rnd, WARPEDMODEL_PREC_BITS - 3)) +
               (1 << WARPEDMODEL_PREC_BITS);
    } else {
      mat[4] = -mat[3];
      mat[5] = mat[2];
...
class AV1WarpFilterTest : public ::testing::TestWithParam<WarpTestParam> {
 ...
 protected:
  void RunCheckOutput() {
    ...
    for (i = 0; i < h; ++i)
      for (j = 0; j < w; ++j) input[i * w + j] = rnd.Rand8();

    /* Try different sizes of prediction block */
    for (i = 0; i < num_iters; ++i) {
      generate_model(rnd, mat, &alpha, &beta, &gamma, &delta);
