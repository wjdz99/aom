...
class AV1ConvolveCopyTest : public AV1ConvolveTest<convolve_copy_func> {
 public:
  void RunTest() {
    ...
    std::unique_ptr<uint8_t[]> reference2(new uint8_t[width * height]);
    aom_convolve_copy_c(input, width, reference2.get(), width, width, height);
    std::unique_ptr<uint8_t[]> test2(new uint8_t[width * height]);
...
class AV1ConvolveCopyHighbdTest
 ...
 public:
  void RunTest() {
    ...
    // Test again with dst_stride=width.
    std::unique_ptr<uint16_t[]> reference2(new uint16_t[width * height]);
    aom_highbd_convolve_copy_c(input, width, reference2.get(), width, width,
                               height);
    std::unique_ptr<uint16_t[]> test2(new uint16_t[width * height]);
    GetParam().TestFunction()(input, width, test2.get(), width, width, height);
    AssertOutputBufferEq(reference2.get(), test2.get(), width, height, width);
  }
};
