...
TEST_F(AV1ConvolveParametersTest, GetLowbdTestParams) {
  ...
  for (const auto &p : v) {
    ...
    ASSERT_EQ(av1_convolve_x_sr_c, p.TestFunction());
  }
}
...
TEST_F(AV1ConvolveParametersTest, GetHighbdTestParams) {
  ...
  for (const auto &p : v) {
    ...
    ASSERT_EQ(av1_highbd_convolve_x_sr_c, p.TestFunction());
    if (p.BitDepth() == 10) {
      ++num_10;
    } else {
      ++num_12;
...
INSTANTIATE_TEST_SUITE_P(
    SSE2, AV1ConvolveCopyHighbdTest,
    BuildHighbdParams(av1_highbd_convolve_2d_copy_sr_sse2));
#endif

#if HAVE_AVX2
INSTANTIATE_TEST_SUITE_P(
    AVX2, AV1ConvolveCopyHighbdTest,
...
TEST_F(AV1ConvolveParametersTest, GetLowbdLumaTestParams) {
  ...
  for (const auto &e : v) {
    ...
    ASSERT_EQ(av1_dist_wtd_convolve_x_c, e.TestFunction());
  }
}
...
TEST_F(AV1ConvolveParametersTest, GetHighbdLumaTestParams) {
  ...
  for (const auto &e : v) {
    ...
    ASSERT_EQ(av1_highbd_dist_wtd_convolve_x_c, e.TestFunction());
    if (e.BitDepth() == 10) {
      ++num_10;
    } else {
      ++num_12;
