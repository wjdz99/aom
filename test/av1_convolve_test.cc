...
class AV1ConvolveTest : public ::testing::TestWithParam<TestParam<T>> {
 public:
  ...
  const uint8_t *FirstRandomInput8(const TestParam<T> &param) {
    ...
    Randomize(input8_1_, param.Block().Width(), param.Block().Height());
  ...
  const uint8_t *SecondRandomInput8(const TestParam<T> &param) {
    EXPECT_EQ(8, param.BitDepth());
    EXPECT_GE(MAX_SB_SIZE, param.Block().Width());
    Randomize(input8_2_, param.Block().Width(), param.Block().Height());
  ...
  const uint16_t *FirstRandomInput16(const TestParam<T> &param) {
    ...
    EXPECT_GE(MAX_SB_SIZE, param.Block().Width());
    Randomize(input16_1_, param.Block().Width(), param.Block().Height(),
              param.BitDepth());
  ...
  const uint16_t *SecondRandomInput16(const TestParam<T> &param) {
    EXPECT_TRUE(param.BitDepth() == 10 || param.BitDepth() == 12);
    EXPECT_GE(MAX_SB_SIZE, param.Block().Width());
    Randomize(input16_2_, param.Block().Width(), param.Block().Height(),
              param.BitDepth());
 ...
 private:
  void Randomize(uint8_t *p, int width, int height) {
    for (int j = 0; j < height + kInputPadding; ++j) {
      for (int i = 0; i < width + kInputPadding; ++i) {
        p[i] = rnd_.Rand8();
      }
      p += width + kInputPadding;
    }
  }

#if CONFIG_AV1_HIGHBITDEPTH
  void Randomize(uint16_t *p, int width, int height, int bit_depth) {
    for (int j = 0; j < height + kInputPadding; ++j) {
      for (int i = 0; i < width + kInputPadding; ++i) {
...
class AV1CompoundConvolve2DCopyTest
 ...
 private:
  void Convolve(compound_conv_2d_copy_func test_func, const uint8_t *src1,
    ...
    ConvolveParams conv_params =
        GetConvolveParams(0, conv_buf, kOutputStride, 8, compound);
    ASSERT_EQ(0U, ((unsigned long)conv_buf) % 32);
