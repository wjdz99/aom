...
class ParamIterator<Highbd<T>> {
 public:
  ParamIterator(const BlockSize &block) : iter_(block), values_() {}
...
class AV1Convolve2DCopyTest : public AV1ConvolveTest<BlockSize> {
 public:
 ...
 protected:
  void TestConvolve(const BlockSize &block) {
    ...
    const uint8_t *input = RandomInput8(width, height);
    uint8_t *reference = Output8(width, height);
    av1_convolve_2d_copy_sr(input, width, reference, width, width, height);
    uint8_t *test = Output8(width, height);
    test_func_(input, width, test, width, width, height);
    AssertEq(reference, test, width, height, width);
  }

  convolve_2d_copy_func test_func_;
};

TEST_P(AV1Convolve2DCopyTest, C) { RunTest(av1_convolve_2d_copy_sr_c); }

#if HAVE_SSE2
TEST_P(AV1Convolve2DCopyTest, SSE2) { RunTest(av1_convolve_2d_copy_sr_sse2); }
#endif

#if HAVE_AVX2
TEST_P(AV1Convolve2DCopyTest, AVX2) { RunTest(av1_convolve_2d_copy_sr_avx2); }
#endif

#if HAVE_NEON
TEST_P(AV1Convolve2DCopyTest, NEON) { RunTest(av1_convolve_2d_copy_sr_neon); }
#endif

INSTANTIATE_TEST_CASE_P(AV1Convolve, AV1Convolve2DCopyTest,
                        ::testing::ValuesIn(GetBlockSizes()));
...
class AV1HighbdConvolve2DCopyTest : public AV1ConvolveTest<Highbd<BlockSize>> {
 public:
 ...
 protected:
  ...
  highbd_convolve_2d_copy_func test_func_;
};

TEST_P(AV1HighbdConvolve2DCopyTest, C) {
  RunTest(av1_highbd_convolve_2d_copy_sr_c);
}

#if HAVE_SSE2
TEST_P(AV1HighbdConvolve2DCopyTest, SSE2) {
  RunTest(av1_highbd_convolve_2d_copy_sr_sse2);
}
#endif

#if HAVE_AVX2
TEST_P(AV1HighbdConvolve2DCopyTest, AVX2) {
  RunTest(av1_highbd_convolve_2d_copy_sr_avx2);
}
#endif

INSTANTIATE_TEST_CASE_P(AV1Convolve, AV1HighbdConvolve2DCopyTest,
                        ::testing::ValuesIn(GetBlockSizes()));
...
#if HAVE_NEON
TEST_P(AV1HighbdCompoundConvolve2DTest, NEON) {
