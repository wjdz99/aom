...
class TestParam {
 public:
  TestParam(const BlockSize &block, int bd, T test_func) : block_(block), bd_(bd), test_func_(test_func) {}
...
template <typename T>
std::vector<TestParam<T>> GetTestParams(std::initializer_list<int> bit_depths, T test_func) {
...
template <typename T>
std::vector<TestParam<T>> GetLowbdTestParams(T test_func) { return GetTestParams({ 8 }, test_func); }

template <typename T>
::testing::internal::ParamGenerator<TestParam<T>> BuildLowbdParams(T test_func) {
...
template <typename T>
::testing::internal::ParamGenerator<TestParam<T>> BuildHighbdParams(T test_func) {
...
#if CONFIG_AV1_HIGHBITDEPTH
  uint16_t input16_1_[(MAX_SB_SIZE + kInputPadding) * (MAX_SB_SIZE + kInputPadding)];
  uint16_t input16_2_[(MAX_SB_SIZE + kInputPadding) * (MAX_SB_SIZE + kInputPadding)];
 ...
 public:
  void RunTest() {
    for (int sub_x = 0; sub_x < 16; ++sub_x) {
      for (int filter = EIGHTTAP_REGULAR; filter < INTERP_FILTERS_ALL; ++filter) {
...
class AV1HighbdConvolveCopyTest : public AV1ConvolveTest<highbd_convolve_copy_func> {
 public:
  void RunTest() {
...
#if HAVE_SSE2
INSTANTIATE_TEST_SUITE_P(SSE2_COPY, AV1HighbdConvolveCopyTest,
                         BuildHighbdParams(av1_highbd_convolve_2d_copy_sr_sse2));
#endif

#if HAVE_AVX2
INSTANTIATE_TEST_SUITE_P(AVX2_COPY, AV1HighbdConvolveCopyTest,
...
class AV1Convolve2DTest : public AV1ConvolveTest<convolve_2d_func> {
 ...
 private:
  void TestConvolve(const InterpFilter h_f, const InterpFilter v_f, const int sub_x, const int sub_y) {
    const int width = GetParam().Block().Width();
    const int height = GetParam().Block().Height();
    const InterpFilterParams *filter_params_x =
...
class AV1HighbdConvolve2DTest : public AV1ConvolveTest<highbd_convolve_2d_func> {
 public:
  void RunTest() {
    for (int sub_x = 0; sub_x < 16; ++sub_x) {
      for (int sub_y = 0; sub_y < 16; ++sub_y) {
...
template <typename T>
::testing::internal::ParamGenerator<TestParam<T>> BuildLowbdLumaParams(T test_func) {
...
template <typename T>
::testing::internal::ParamGenerator<TestParam<T>> BuildHighbdLumaParams(T test_func) {
...
class CompoundParam {
 public:
  CompoundParam(bool use_dist_wtd_comp_avg, int fwd_offset,
                int bck_offset)
      : use_dist_wtd_comp_avg_(use_dist_wtd_comp_avg),
...
std::vector<CompoundParam> GetCompoundParams() {
  ...
  for (int k = 0; k < 2; ++k) {
    for (int l = 0; l < 4; ++l) {
      result.push_back(CompoundParam(true,
...
class AV1CompoundConvolveXTest : public AV1ConvolveTest<convolve_x_func> {
 ...
 private:
  void TestConvolve(const int sub_pix, const InterpFilter filter, const CompoundParam& compound) {
    const int width = GetParam().Block().Width();
    const int height = GetParam().Block().Height();
    const int bit_depth = GetParam().BitDepth();
    ...
    Convolve(GetParam().TestFunction(), input1, input2, test, test_conv_buf, compound,
             sub_pix, filter);
 ...
 private:
  void Convolve(convolve_x_func test_func, const uint8_t *src1,
    ...
    const int width = GetParam().Block().Width();
    const int height = GetParam().Block().Height();
    const InterpFilterParams *filter_params = FilterParams(filter, GetParam().Block());
...
INSTANTIATE_TEST_SUITE_P(C_X, AV1HighbdCompoundConvolveXTest,
                         BuildHighbdLumaParams(av1_highbd_dist_wtd_convolve_x_c));

#if HAVE_SSE4_1
INSTANTIATE_TEST_SUITE_P(SSE4_1_X, AV1HighbdCompoundConvolveXTest,
                         BuildHighbdLumaParams(av1_highbd_dist_wtd_convolve_x_sse4_1));
#endif

#if HAVE_AVX2
INSTANTIATE_TEST_SUITE_P(AVX2_X, AV1HighbdCompoundConvolveXTest,
...
INSTANTIATE_TEST_SUITE_P(C_Y, AV1HighbdCompoundConvolveYTest,
                         BuildHighbdLumaParams(av1_highbd_dist_wtd_convolve_y_c));

#if HAVE_SSE4_1
INSTANTIATE_TEST_SUITE_P(SSE4_1_Y, AV1HighbdCompoundConvolveYTest,
                         BuildHighbdLumaParams(av1_highbd_dist_wtd_convolve_y_sse4_1));
#endif

#if HAVE_AVX2
INSTANTIATE_TEST_SUITE_P(AVX2_Y, AV1HighbdCompoundConvolveYTest,
...
#if HAVE_SSE2
INSTANTIATE_TEST_SUITE_P(SSE2_COPY, AV1CompoundConvolve2DCopyTest,
                         BuildLowbdLumaParams(av1_dist_wtd_convolve_2d_copy_sse2));
#endif

#if HAVE_AVX2
INSTANTIATE_TEST_SUITE_P(AVX2_COPY, AV1CompoundConvolve2DCopyTest,
                         BuildLowbdLumaParams(av1_dist_wtd_convolve_2d_copy_avx2));
#endif

#if HAVE_NEON
INSTANTIATE_TEST_SUITE_P(NEON_COPY, AV1CompoundConvolve2DCopyTest,
...
INSTANTIATE_TEST_SUITE_P(C_COPY, AV1HighbdCompoundConvolve2DCopyTest,
                         BuildHighbdLumaParams(av1_highbd_dist_wtd_convolve_2d_copy_c));

#if HAVE_SSE4_1
INSTANTIATE_TEST_SUITE_P(SSE4_1_COPY, AV1HighbdCompoundConvolve2DCopyTest,
                         BuildHighbdLumaParams(av1_highbd_dist_wtd_convolve_2d_copy_sse4_1));
#endif

#if HAVE_AVX2
INSTANTIATE_TEST_SUITE_P(AVX2_COPY, AV1HighbdCompoundConvolve2DCopyTest,
...
class AV1CompoundConvolve2DTest : public AV1ConvolveTest<convolve_2d_func> {
      ...
      for (int v_f = EIGHTTAP_REGULAR; v_f < INTERP_FILTERS_ALL; ++v_f) {
        for (int sub_x = 0; sub_x < 16; ++sub_x) {
          for (int sub_y = 0; sub_y < 16; ++sub_y) {
            for (const auto& compound : compound_params) {
              TestConvolve(static_cast<InterpFilter>(h_f),
                           static_cast<InterpFilter>(v_f), sub_x, sub_y, compound);
            }
          }
        }
      }
 ...
 private:
  void TestConvolve(const InterpFilter h_f, const InterpFilter v_f,
                    const int sub_x, const int sub_y, const CompoundParam& compound) {
    const BlockSize &block = GetParam().Block();
    const int width = block.Width();
    const int height = block.Height();
    const int bit_depth = GetParam().BitDepth();
...
INSTANTIATE_TEST_SUITE_P(C_2D, AV1HighbdCompoundConvolve2DTest,
                         BuildHighbdLumaParams(av1_highbd_dist_wtd_convolve_2d_c));

#if HAVE_SSE4_1
INSTANTIATE_TEST_SUITE_P(SSE4_1_2D, AV1HighbdCompoundConvolve2DTest,
                         BuildHighbdLumaParams(av1_highbd_dist_wtd_convolve_2d_sse4_1));
#endif

#if HAVE_AVX2
INSTANTIATE_TEST_SUITE_P(AVX2_2D, AV1HighbdCompoundConvolve2DTest,
