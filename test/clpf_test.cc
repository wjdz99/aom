...
TEST_P(ClpfBlockHbdTest, TestSIMDNoMismatch) {
  ...
  DECLARE_ALIGNED(16, uint16_t, d[size * size]);
  DECLARE_ALIGNED(16, uint16_t, ref_d[size * size]);
  ...
  // Test every combination of:
  // * Input with 1-12 bits of noise
  // * Noise level around every value from 0 to 4095
  // * Blocks anywhere in the frame (along all egdes and also fully inside)
  // * All strengths
  for (level = 0; level < 4096 && !error; level++) {
    for (bits = 1; bits < 13 && !error; bits++) {
      for (int i = 0; i < size * size; i++)
        s[i] = clamp((rnd.Rand16() & ((1 << bits) - 1)) + level, 0, 4095);

      for (ypos = 0; ypos < size && !error; ypos += h * !error) {
        for (xpos = 0; xpos < size && !error; xpos += w * !error) {
          for (strength = 0; strength < 7 && !error; strength += !error) {
            ref_clpf(s, ref_d, size, size, xpos, ypos, w, h, size, size,
                     1 << strength);
            ASM_REGISTER_STATE_CHECK(clpf(s, d, size, size, xpos, ypos, w, h,
                                          size, size, 1 << strength));
  ...
  pos--;
  EXPECT_EQ(0, error)
      << "Error: ClpfBlockHbdTest, SIMD and C mismatch." << std::endl
      << "First error at " << pos % size << "," << pos / size << " ("
      << (int16_t)ref_d[pos] << " != " << (int16_t)d[pos] << ") " << std::endl
      << "strength: " << (1 << strength) << std::endl
      << "xpos: " << xpos << std::endl
...
TEST_P(ClpfHbdSpeedTest, TestSpeed) {
  ...
  for (int i = 0; i < size * size; i++) s[i] = rnd.Rand16() & ((1 << 12) - 1);
