...
double ref_delta_rate_cost(int64_t delta_rate, int64_t recrf_dist,
                           int64_t srcrf_dist, int pixel_num) {
  double bits_per_pixel = (double)delta_rate / pixel_num;
  double p = pow(2, bits_per_pixel);
  double beta = (double)srcrf_dist / recrf_dist;
  double flow_rate_per_pixel = sqrt(p * p / (beta * p * p + (1 - beta)));
...
TEST(TplModelTest, DeltaRateCost2) {
  ...
  for (size_t t = 0; t < srcrf_dist_arr.size(); ++t) {
    ...
    double ref_rate_cost =
        ref_delta_rate_cost(delta_rate, recrf_dist, srcrf_dist, pixel_num);
