...
TEST(TPLModelTest, EstimateFrameRateTest) {
  ...
  unsigned char q_index_list[256];
  TplTxfmStats stats_list[256];
  ...
  for (int i = 0; i < size; i++) {
    ...
    for (int j = 0; j < 256; j++) { /* this is always 256 */
      stats_list[i].abs_coeff_sum[j] = 1;
    }
  }

  double result = av1_estimate_gop_bitrate(q_index_list, size, stats_list);
  EXPECT_GT(result, 0);
}
