...
TEST(TPLModelTest, EstimateFrameRateTest) {
  ...
  double result =
      av1_estimate_gop_bitrate(q_index_list, valid_list, frame_count, stats_list, NULL);
...
int find_gop_q_iterative(double bit_budget, double arf_qstep_ratio,
  ...
  double curr_estimate =
      av1_estimate_gop_bitrate(gf_group.q_val, stats_valid_list, gf_group.size, stats_list, NULL);
  ...
  for (int q = 254; q >= 0; q--) {
    av1_q_mode_compute_gop_q_indices(gf_frame_index, q, arf_qstep_ratio,
                                     bit_depth, &gf_group, gf_group.q_val);
    curr_estimate = av1_estimate_gop_bitrate(gf_group.q_val, stats_valid_list, gf_group.size,
                                             stats_list, NULL);
...
TEST(TplModelTest, QModeEstimateBaseQTest) {
  ...
  for (double bit_budget : bit_budgets) {
    ...
    const int result = av1_q_mode_estimate_base_q(
        &gf_group, stats_list, stats_valid_list, bit_budget, gf_frame_index, arf_qstep_ratio,
        bit_depth, scale_factor, q_index_list, NULL);
    const int test_result =
        find_gop_q_iterative(bit_budget, arf_qstep_ratio, gf_group, stats_valid_list,
            stats_list, gf_frame_index, bit_depth);
