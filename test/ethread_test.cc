...
TEST_P(AVxFirstPassEncoderThreadTest, FirstPassStatsTest) {
  ...
  // 3. row_mt_=1 and threads=4
  // 4. row_mt_=1 and threads=2
  ...
  // Test row_mt_: 0 vs 1 at single thread case(threads = 1, tiles_ = 0)
  cfg_.g_threads = 1;
  ...
  // Check if threads > 1 with row_mt_=1 generate bit-exact.
  row_mt_ = 1;
