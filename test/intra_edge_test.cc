...
typedef void (*FE8B)(uint8_t *p, int sz, int strength);
typedef libaom_test::FuncParam<FE8B> FilterEdgeTestFuncs;

class FilterEdgeTest8B : public FilterEdgeTest<FE8B, uint8_t> {
 protected:
  void Execute(uint8_t *edge_tst) {
    params_.ref_func(edge_ref_, sz_, strength_);
    ASM_REGISTER_STATE_CHECK(params_.tst_func(edge_tst, sz_, strength_));
  }
};
...
typedef void (*FEHB)(uint16_t *p, int sz, int strength);
