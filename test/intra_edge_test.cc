...
TEST_P(UpsampleTest8B, RandomValues) {
  for (int iter = 0; iter < kIterations && !HasFatalFailure(); ++iter) {
    ...
    for (i = 0; i < kOffset + sz_; ++i) {
      edge_ref_data_[i] = rng_.Rand8();
      edge_tst_data_[i] = edge_ref_data_[i];
    }

    // Extend final sample
    int p = edge_ref_data_[i - 1];
    while (i < kBufSize) {
      edge_ref_data_[i] = p;
      edge_tst_data_[i] = p;
      i++;
...
#if HAVE_SSE4_1
INSTANTIATE_TEST_CASE_P(SSE4_1, UpsampleTest8B,
                        ::testing::Values(TestFuncs(
                            av1_upsample_intra_edge_c, av1_upsample_intra_edge_sse4_1)));
#endif  // HAVE_SSE4_1
...
#if HAVE_SSE4_1
INSTANTIATE_TEST_CASE_P(SSE4_1, UpsampleTestHB,
                        ::testing::Values(TestFuncsHBD(
