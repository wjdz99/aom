...
// The C version of od_dir_find8() is only 30% slower or so than the C version
// which sometimes will make a speed test fail on a loaded system, so the test
// is disabled for now.
#define TEST_FINDDIR_SPEED 0
...
#if HAVE_SSE4_1
INSTANTIATE_TEST_CASE_P(
    SSSE4_1, DeringSpeedTest,
    ::testing::Values(make_tuple(&od_filter_dering_direction_4x4_sse4_1,
                                 &od_filter_dering_direction_4x4_c, 4),
                      make_tuple(&od_filter_dering_direction_8x8_sse4_1,
                                 &od_filter_dering_direction_8x8_c, 8)));
#if TEST_FINDDIR_SPEED
INSTANTIATE_TEST_CASE_P(SSSE4_1, DeringFindDirSpeedTest,
                        ::testing::Values(make_tuple(&od_dir_find8_sse4_1,
                                                     &od_dir_find8_c)));
#endif
#endif
