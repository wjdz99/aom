...
class AV1HighBDJNTCOMPAVGUPSAMPLEDTest
 ...
 protected:
  ...
  void RunSpeedTest(highbdjntcompavgupsampled_func test_impl) {
    ...
    for (int i = 0; i < num_loops; ++i)
      aom_highbd_jnt_comp_avg_upsampled_pred_c(
          NULL, NULL, 0, 0, NULL, CONVERT_TO_BYTEPTR(output), CONVERT_TO_BYTEPTR(pred8), in_w, in_h,
    ...
    for (int i = 0; i < num_loops; ++i)
      test_impl(NULL, NULL, 0, 0, NULL, CONVERT_TO_BYTEPTR(output2), CONVERT_TO_BYTEPTR(pred8),
