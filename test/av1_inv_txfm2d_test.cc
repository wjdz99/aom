...
void RunAV1InvTxfm2dTest(Fwd_Txfm2d_Func *fwdList, Inv_Txfm2d_Func *refList,
                         Lbd_Inv_Txfm2d_Func *testList, int runTimes) {
  const int bd = 8;
  for (int tx_size = TX_4X4; tx_size < TX_SIZES_ALL; ++tx_size) {
    for (int tx_type = 0; tx_type < TX_TYPES; ++tx_type) {
      Fwd_Txfm2d_Func fwd_func = fwdList[tx_size];
      ...
      if (ref_func != NULL && target_func != NULL) {
        DECLARE_ALIGNED(16, int16_t, input[64 * 64]) = { 0 };
        DECLARE_ALIGNED(32, int32_t, inv_input[64 * 64]) = { 0 };
        DECLARE_ALIGNED(16, uint8_t, output[64 * 64]) = { 0 };
        DECLARE_ALIGNED(16, uint16_t, ref_output[64 * 64]) = { 0 };
        int stride = 64;
        ACMRandom rnd(ACMRandom::DeterministicSeed());
...
Fwd_Txfm2d_Func fwd_func_c_list[TX_SIZES_ALL] = {
  NULL,                  // TX_4X4
  av1_fwd_txfm2d_8x8_c,  // TX_8X8
  NULL,                  // TX_16X16
  NULL,                  // TX_32X32
#if CONFIG_TX64X64
  NULL,  // TX_64X64
...
TEST(av1_inv_txfm2d_add_sse2, match) {
  RunAV1InvTxfm2dTest(fwd_func_c_list, inv_func_c_list, ldb_inv_func_sse2_list,
                      1);
}
