  ...
  void RunRoundtripCheck() {
    ...
    for (int ci = 0; ci < count; ci++) {
      ...
      if (TxfmUsesApproximation()) {
        ...
        inv_txfm_func(ref_coeffs_int, expected, tx_w, tx_type_, 25, bd);
#else
...
#if CONFIG_DATA_DRIVEN_TX
      fwd_txfm_func(input, coeffs, tx_w, tx_type_, 25, bd);
#else
...
#if CONFIG_DATA_DRIVEN_TX
      inv_txfm_func(coeffs, actual, tx_w, tx_type_, 25, bd);
#else
...
void AV1LbdInvTxfm2d::RunAV1InvTxfm2dTest(TX_TYPE tx_type, TX_SIZE tx_size,
  ...
  for (int cnt = 0; cnt < randTimes; ++cnt) {
...
#if CONFIG_DATA_DRIVEN_TX
    fwd_func_(input, inv_input, stride, tx_type, 25, bd);
#else
...
#if CONFIG_DATA_DRIVEN_TX
      ref_func_(inv_input, ref_output, stride, tx_type, 25, bd);
#else
    ...
    for (int i = 0; i < run_times; ++i) {
#if CONFIG_DATA_DRIVEN_TX
      target_func_(inv_input, output, stride, tx_type, tx_size, 25, eob);
#else
