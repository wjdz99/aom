...
typedef ::testing::tuple<tx_type_t, tx_size_t, int, double> AV1InvTxfm2dParam;
  ...
  void RunRoundtripCheck() {
    ...
    for (int ci = 0; ci < count; ci++) {
      ...
      if (TxfmUsesApproximation()) {
        ...
        ASSERT_EQ(tx_type_, (tx_type_t)DCT_DCT);
...
vector<AV1InvTxfm2dParam> GetInvTxfm2dParamList() {
  ...
  for (int s = 0; s < TX_SIZES; ++s) {
    ...
    for (int t = 0; t < TX_TYPES; ++t) {
      const tx_type_t tx_type = static_cast<tx_type_t>(t);
      const tx_size_t tx_size = static_cast<tx_size_t>(s);
  ...
  for (int bd_idx = 0; bd_idx < BD_NUM; ++bd_idx) {
    ...
    for (int tx_size = 0; tx_size < TX_SIZES_ALL; ++tx_size) {
      for (int tx_type = 0; tx_type < TX_TYPES; ++tx_type) {
        if (libaom_test::IsTxSizeTypeValid(static_cast<tx_size_t>(tx_size),
                                           static_cast<tx_type_t>(tx_type)) ==
            false) {
          continue;
        }
        TXFM_2D_FLIP_CFG cfg;
        av1_get_inv_txfm_cfg(static_cast<tx_type_t>(tx_type),
                             static_cast<tx_size_t>(tx_size), &cfg);
        int8_t stage_range_col[MAX_TXFM_STAGE_NUM];
        int8_t stage_range_row[MAX_TXFM_STAGE_NUM];
        av1_gen_inv_stage_range(stage_range_col, stage_range_row, &cfg,
                                static_cast<tx_size_t>(tx_size), bd);
...
class AV1LbdInvTxfm2d : public ::testing::TestWithParam<AV1LbdInvTxfm2dParam> {
 public:
  ...
  void RunAV1InvTxfm2dTest(tx_type_t tx_type, tx_size_t tx_size, int run_times,
                           int gt_int16 = 0);
...
void AV1LbdInvTxfm2d::RunAV1InvTxfm2dTest(tx_type_t tx_type, tx_size_t tx_size,
...
TEST_P(AV1LbdInvTxfm2d, match) {
  for (int j = 0; j < (int)(TX_SIZES_ALL); ++j) {
    for (int i = 0; i < (int)TX_TYPES; ++i) {
      if (libaom_test::IsTxSizeTypeValid(static_cast<tx_size_t>(j),
                                         static_cast<tx_type_t>(i))) {
        RunAV1InvTxfm2dTest(static_cast<tx_type_t>(i),
                            static_cast<tx_size_t>(j), 1);
      }
    }
  }
}
  ...
  for (int j = 1; j < (int)(TX_SIZES_ALL); ++j) {
    for (int i = 0; i < (int)TX_TYPES; ++i) {
      if (libaom_test::IsTxSizeTypeValid(static_cast<tx_size_t>(j),
                                         static_cast<tx_type_t>(i))) {
        RunAV1InvTxfm2dTest(static_cast<tx_type_t>(i),
                            static_cast<tx_size_t>(j), 10000000);
      }
    }
  }
}
