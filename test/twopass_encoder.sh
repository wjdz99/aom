...
# 100.
twopass_encoder() {
  local encoder = "$(aom_tool_path twopass_encoder)" local codec =
      "$1" local output_file =
          "${AOM_TEST_OUTPUT_DIR}/twopass_encoder_${codec}.ivf" local limit = 7
...
twopass_encoder_av1() {
  ...
  then twopass_encoder av1 || return 1 fi
}
