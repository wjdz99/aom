...
typedef std::tr1::tuple<get_crc_value_func, int> HashParam;
...
void AV1CrcHashTest::RunSpeedTest(get_crc_value_func test_impl) {
  ...
  printf("hash %3dx%-3d:%7.2f/%7.2fns", bsize_, bsize_, time[0], time[1]);
  printf("(%3.2f)\n", time[0] / time[1]);
}
