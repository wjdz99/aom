...
// Params: test mode, speed, aq mode, frame drop threshold.
class DatarateTestFrameDropLarge
 ...
 public:
  DatarateTestFrameDropLarge() : DatarateTest(GET_PARAM(0)) {
    set_cpu_used_ = GET_PARAM(2);
    aq_mode_ = GET_PARAM(3);
    frame_drop_thresh_ = GET_PARAM(4);
  }
  ...
  virtual void ChangingDropFrameThreshTest() {
    ...
    if (last_drop > 0) {
      ASSERT_LE(first_drop_, last_drop)
          << " The first dropped frame for drop_thresh " << frame_drop_thresh_
          << " > first dropped frame for drop_thresh "
          << frame_drop_thresh_ - kDropFrameThreshTestStep;
    }
    ASSERT_GE(num_drops_, last_num_drops * 0.7)
        << " The number of dropped frames for drop_thresh "
        << frame_drop_thresh_ << " < number of dropped frames for drop_thresh "
        << frame_drop_thresh_ - kDropFrameThreshTestStep;
