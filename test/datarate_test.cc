...
class DatarateTestLarge
 ...
 protected:
  ...
  virtual void BasicRateTargetingCBRTestErrorResilienceOn() {
  ...
  virtual void BasicRateTargetingCBRPeriodicKeyFrameOvershootTest() {
    if (GET_PARAM(4) > 0) return;
    cfg_.rc_buf_initial_sz = 500;
    cfg_.rc_buf_optimal_sz = 500;
    cfg_.rc_buf_sz = 1000;
...
TEST_P(DatarateTestLarge, PeriodicKeyFrameCBROvershoot) {
  BasicRateTargetingCBRPeriodicKeyFrameOvershootTest();
}
...
// Check basic rate targeting for periodic key frame, aligned with scene change.
TEST_P(DatarateTestRealtime, PeriodicKeyFrameCBROvershoot) {
  BasicRateTargetingCBRPeriodicKeyFrameOvershootTest();
}

// Check basic rate targeting for periodic key frame.
TEST_P(DatarateTestRealtime, ErrorResilienceOnOvershoot) {
  BasicRateTargetingCBRTestErrorResilienceOn();
}
