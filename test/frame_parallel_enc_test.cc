...
class AVxFrameParallelEncodeTest
 ...
 protected:
  ...
  void DoTest(::libaom_test::VideoSource *input_video) {
    /* The is the actual parallel encode of frames using multiple cpis.
     * The parallel frames are independently encoded.
     * Threads are distributed among the parallel frames and non-parallel frames
     * use all the threads.
     * Example: for 8 threads, in case of 4 frames in a parallel encode set,
     * each frame gets 2 threads. In case of 3 frames in a parallel encode set,
     * threads are distributed as 2, 3 ,3.
     */
    cfg_.g_threads = 8;
    ...
    /* The is the simulation of parallel encode of frames using 1 cpi.
     * This mode is enabled in library by setting threads to 2.
