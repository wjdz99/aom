...
int put_accounting(char *buffer) {
  ...
  for (i = 0; i < num_strs; i++) {
    buf += snprintf(buf, MAX_BUFFER, "\"%s\"", accounting->syms.dictionary.strs[i]);
    if (i < num_strs - 1) *(buf++) = ',';
  }
  buf += put_str(buf, "],\n");
...
void inspect(void *pbi, void *data) {
  ...
  (void)data;
  // We allocate enough space and hope we don't write out of bounds. Totally unsafe
  // but this speeds things up, especially when compiled to Javascript.
