...
We are using Google C Coding Style defined by the
...
Some Git installation have clang-format integration. Here are some examples:
...
[Gerrit](http://gerrit.googlecode.com/) code review tool. This tool implements
a workflow on top of the Git version control system to ensure that all changes
get peer reviewed and tested prior to their distribution.

### Login cookie

Browse to [AoMedia Git index](https://aomedia.googlesource.com/) and login with
your account (Gmail credentials, for example). Next, follow the Generate
Password link at the top of the page. You’ll be given instructions for creating
a cookie to use with our Git repos.
...
If your commit does not include a ‘Change-Id:’ line in its commit message
(see [Commit message hook](#commit-message-hook), above), you’ll have to add
one. Do so by running git commit --amend to edit the commit message, and then
create a ‘Change-Id: I …’ by using the ID number of the change you’re trying to
update, which can be found using the web UI.

### Submitting your change

Once your change has been Approved and Verified, you can “submit” it through the
Gerrit UI. This will usually automatically merge your change into the branch you
specified, creating a new merge commit in Git if necessary. Sometimes this can’t
be done automatically. If you run into this problem, you must rebase or merge
your changes manually. We strongly prefer that you rebase your changes instead
of merging them. To do so, run:
