...
int arg_cfg(int *argc, char ***argv, const char *file) {
  ...
  while (fgets(line, sizeof(line) - 1, f)) {
    ...
    char *new_line = (char *)malloc(sizeof(*left) * 128);

    if (argv_local != argv_org) free(argv_local);

    if (!strcmp(right, "ON"))
      snprintf(new_line, sizeof(sizeof(*new_line) * 128), "--%s", left);
    else
      snprintf(new_line, sizeof(sizeof(*new_line) * 128), "--%s=%s", left, right);
