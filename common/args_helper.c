  ...
  *err_msg = NULL;
  ...
  if (arg.name) {
    ...
    if (!arg.val && def->has_val) {
      *err_msg = aom_malloc(sizeof(**err_msg) * ERR_MSG_MAX_LEN);
...
unsigned int arg_parse_uint_helper(const struct arg *arg, char **err_msg) {
  ...
  *err_msg = NULL;
  ...
  if (arg->val[0] != '\0' && endptr[0] == '\0') {
    ...
    *err_msg = aom_malloc(sizeof(**err_msg) * ERR_MSG_MAX_LEN);
    snprintf(*err_msg, ERR_MSG_MAX_LEN,
             "Option %s: Value %lu out of range for unsigned int\n", arg->name,
             rawval);
    return 0;
  }

  *err_msg = aom_malloc(sizeof(**err_msg) * ERR_MSG_MAX_LEN);
  snprintf(*err_msg, ERR_MSG_MAX_LEN, "Option %s: Invalid character '%c'\n",
           arg->name, *endptr);
  return 0;
}
...
struct aom_rational arg_parse_rational_helper(const struct arg *arg,
  ...
  *err_msg = NULL;
  ...
  if (arg->val[0] != '\0' && endptr[0] == '/') {
    ...
    *err_msg = aom_malloc(sizeof(**err_msg) * ERR_MSG_MAX_LEN);
    snprintf(*err_msg, ERR_MSG_MAX_LEN, "Option %s: Expected / at '%c'\n",
             arg->name, *endptr);
    return rat;
  }
...
int arg_parse_enum_helper(const struct arg *arg, char **err_msg) {
  ...
  *err_msg = aom_malloc(sizeof(**err_msg) * ERR_MSG_MAX_LEN);
  snprintf(*err_msg, ERR_MSG_MAX_LEN, "Option %s: Invalid value '%s'\n",
           arg->name, arg->val);
  return 0;
}
...
int arg_parse_list_helper(const struct arg *arg, int *list, int n,
  ...
  *err_msg = NULL;
  ...
  while (ptr[0] != '\0') {
    int64_t rawval = (int64_t)strtol(ptr, &endptr, 10);
    if (rawval < INT_MIN || rawval > INT_MAX) {
      *err_msg = aom_malloc(sizeof(**err_msg) * ERR_MSG_MAX_LEN);
      snprintf(*err_msg, ERR_MSG_MAX_LEN,
               "Option %s: Value %ld out of range for signed int\n", arg->name,
      ...
      *err_msg = aom_malloc(sizeof(**err_msg) * ERR_MSG_MAX_LEN);
      snprintf(*err_msg, ERR_MSG_MAX_LEN,
               "Option %s: List has more than %d entries\n", arg->name, n);
      return 0;
    } else if (*endptr == ',') {
      endptr++;
    } else if (*endptr != '\0') {
