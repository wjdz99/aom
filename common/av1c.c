...
static int parse_sequence_header(const uint8_t *const buffer, size_t length,
                                 av1c *config) {
  int result = 0;
  // One local reader instance to pass amongst helpers: All functions defined in
  // this file share this reader via the reader pointer declared and assigned in
  // this function.
...
int get_av1c_from_obu(const uint8_t *const buffer, size_t length, int is_annexb,
  ...
  const size_t consumed = obu_header_length;
