...
// character.
static int parse_single_tag(const char *buf, y4m_input *y4m_ctx) {
  // Empty tag.
  if (buf[0] == '\0') {
    return 1;
  }
  /*Process the tag.*/
  switch (buf[0]) {
    case 'W': return sscanf(buf + 1, "%d", &y4m_ctx->pic_w) == 1;
    case 'H': return sscanf(buf + 1, "%d", &y4m_ctx->pic_h) == 1;
    case 'F':
      return sscanf(buf + 1, "%d:%d", &y4m_ctx->fps_n, &y4m_ctx->fps_d) == 2;
    case 'I': y4m_ctx->interlace = buf[1]; return buf[1] != '\0';
    case 'A':
      return sscanf(buf + 1, "%d:%d", &y4m_ctx->par_n, &y4m_ctx->par_d) == 2;
    case 'C':
      if (strlen(buf) > 16) return 0;
      // Copies NULL terminator.
      memcpy(y4m_ctx->chroma_type, buf + 1, strlen(buf));
      assert(y4m_ctx->chroma_type[strlen(buf) - 1] == '\0');
      return 1;
    default: return 1;  // Ignore unknown tags.
  }
}
...
// Returns 1 if tags were parsed successfully and a newline was encountered,
// 0 otherwise.
static int parse_tags(y4m_input *y4m_ctx, FILE *file) {
  ...
  do {
    ...
    if (!parse_single_tag(y4m_ctx, tag)) {
      return 0;
    }
  } while (end != '\n');
