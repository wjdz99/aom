...
static bool parse_string(char *dst, size_t len, const char *buf) {
  if (len < strlen(buf) + 1) {
    fprintf(stderr, "Buffer not large enough (%zu bytes) for value: %s\n", len,
            buf);
    return false;
  }
  strcpy(dst, buf);
...
static bool parse_single_tag(const char *buf, y4m_input *_y4m) {
  ...
  switch (buf[0]) {
    ...
    case 'C':
      return parse_string(_y4m->chroma_type, /*max_buf_size=*/16, buf + 1);
...
static bool copy_tag(char *buf, size_t buf_len, char *end_tag, FILE *_fin) {
  ...
  if (i == buf_len) {
    fprintf(stderr, "Error: Y4M header tags must be less than %zu characters\n",
            i);
    return false;
  }
  *end_tag = buf[i];
