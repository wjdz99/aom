...
typedef void (*WRITER)(void *, const uint8_t *, size_t, size_t);

static void write_file(void *fp, const uint8_t *buffer, size_t size,
                       size_t nmemb) {
  fwrite(buffer, size, nmemb, (FILE *)fp);
}

static void write_md5(void *md5, const uint8_t *buffer, size_t size,
                      size_t nmemb) {
