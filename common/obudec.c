...
// value is 0 and the 'bytes_read' value is set to 0. If 'buffered' is 1, it is
...
static int obudec_read_obu_header(struct AvxInputContext *input_ctx,
  ...
  *bytes_read = buffer_and_input(input_ctx, 1, obu_data, buffered);
  ...
  if (has_extension) {
    if (buffer_and_input(input_ctx, 1, &obu_data[1], buffered) != 1) {
...
// actual bytes read added to 'bytes_read'. If 'buffered' is 1, it is buffered
// in the detect buffer first.
static int obudec_read_obu_payload(struct AvxInputContext *input_ctx,
  ...
  if (buffer_and_input(input_ctx, payload_length, obu_data, buffered) !=
      payload_length) {
...
static int obudec_read_obu_header_and_size(
    ...
    ObuHeader *obu_header, int buffered) {
...
int file_is_obu(struct ObuDecInputContext *obu_ctx) {
  ...
  if (is_annexb) {
    ...
    if (obudec_read_leb128(avx_ctx, &detect_buf[length_of_unit_size],
                           &annexb_header_length, &unit_size, 1) != 0) {
  ...
  if (payload_length > 0) {
    ...
    const int status = obudec_read_obu_payload(avx_ctx, payload_length,
                                               &obu_ctx->buffer[bytes_read],
                                               &payload_bytes, 0);
    if (status < 0) {
      rewind_detect(avx_ctx);
      obudec_free(obu_ctx);
  ...
  if (obu_ctx->is_annexb) {
    ...
    if (obu_ctx->bytes_buffered == 0) {
      if (obudec_read_leb128(obu_ctx->avx_ctx, &tuheader[0],
                             &length_of_temporal_unit_size, &size, 0) != 0) {
        fprintf(stderr, "obudec: Failure reading temporal unit header\n");
        return -1;
      }
    ...
    while (1) {
      ...
      if (obudec_read_one_obu(
              obu_ctx->avx_ctx, &obu_ctx->buffer, obu_ctx->bytes_buffered,
              &obu_ctx->buffer_capacity, &obu_size, &obu_header, 0, 0) != 0) {
