...
#define FOREACH_STREAM(iterator, list) \
  for (struct stream_state *iterator = list; iterator; iterator = iterator->next)
...
int main(int argc, const char **argv_) {
  ...
  for (pass = global.pass ? global.pass - 1 : 0; pass < global.passes; pass++) {
    ...
    if (!input.bit_depth) {
      ...
      FOREACH_STREAM(stream, streams) { stream->config.cfg.g_input_bit_depth = input.bit_depth; }
    }
    ...
    FOREACH_STREAM(stream, streams) { set_stream_dimensions(stream, input.width, input.height); }
    ...
    if (global.verbose && pass == 0) {
      FOREACH_STREAM(stream, streams) { show_stream_config(stream, &global, &input); }
    }
    ...
    while (frame_avail || got_data) {
      ...
      if (frames_in > global.skip_frames) {
        ...
        got_data = 0;
        FOREACH_STREAM(stream, streams) { get_cx_data(stream, &global, &got_data); }
    ...
    FOREACH_STREAM(stream, streams) { close_output_file(stream, global.codec->fourcc); }

    FOREACH_STREAM(stream, streams) { stats_close(&stream->stats, global.passes - 1); }

#if CONFIG_FP_MB_STATS
    FOREACH_STREAM(stream, streams) { stats_close(&stream->fpmb_stats, global.passes - 1); }
#endif
